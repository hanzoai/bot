...
profile_path="${1:-${RUNNER_TEMP:-/tmp}/bot-live.profile}"
...
activate_claude_oauth_access_token() {
  local credentials="${BOT_CLAUDE_CREDENTIALS_JSON:-}"
...
write_secret_file "$HOME/.codex/auth.json" BOT_CODEX_AUTH_JSON
write_secret_file "$HOME/.codex/config.toml" BOT_CODEX_CONFIG_TOML
write_secret_file "$HOME/.claude.json" BOT_CLAUDE_JSON
write_secret_file "$HOME/.claude/.credentials.json" BOT_CLAUDE_CREDENTIALS_JSON
write_secret_file "$HOME/.claude/settings.json" BOT_CLAUDE_SETTINGS_JSON
write_secret_file "$HOME/.claude/settings.local.json" BOT_CLAUDE_SETTINGS_LOCAL_JSON
write_secret_file "$HOME/.gemini/settings.json" BOT_GEMINI_SETTINGS_JSON

if [[ -n "${GITHUB_ENV:-}" ]]; then
  {
    echo "BOT_PROFILE_FILE=$profile_path"
  } >>"$GITHUB_ENV"
fi
