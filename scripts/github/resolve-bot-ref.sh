...
REMOTE_URL="${BOT_REF_REMOTE:-https://github.com/hanzoai/bot.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-bot-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Bot branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve Bot ref: ${REF}" >&2
exit 1
