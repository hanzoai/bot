...
const (
	...
	envDocsI18nCommandWaitDelay = "BOT_DOCS_I18N_COMMAND_WAIT_DELAY"
	envDocsI18nCodexExecutable  = "BOT_DOCS_I18N_CODEX_EXECUTABLE"
)
...
func runCodexExecPrompt(ctx context.Context, req codexPromptRequest) (string, error) {
	outputFile, err := os.CreateTemp("", "bot-docs-i18n-codex-*.txt")
	if err != nil {
		return "", err
	}
...
func isolatedCodexHomeBase() (string, error) {
	...
	base := filepath.Join(cacheDir, "bot-docs-i18n")
