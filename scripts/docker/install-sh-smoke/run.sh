...
SMOKE_MODE="${BOT_INSTALL_SMOKE_MODE:-install}"
...
FRESH_VERSION="${BOT_INSTALL_FRESH_VERSION:-}"
FRESH_TAG_URL="${BOT_INSTALL_FRESH_TAG_URL:-}"
UPDATE_BASELINE_VERSION="${BOT_INSTALL_UPDATE_BASELINE:-latest}"
UPDATE_BASELINE_TAG_URL="${BOT_INSTALL_UPDATE_BASELINE_TAG_URL:-}"
UPDATE_EXPECT_VERSION="${BOT_INSTALL_UPDATE_EXPECT_VERSION:-}"
UPDATE_TAG_URL="${BOT_INSTALL_UPDATE_TAG_URL:-}"
SELF_UPDATE_WARNING_FIXED_VERSION="${BOT_INSTALL_SELF_UPDATE_WARNING_FIXED_VERSION:-2026.5.25}"
FRESHNESS_VERSION="${BOT_INSTALL_FRESHNESS_VERSION:-latest}"
...
verify_candidate_ai_runtime() {
  ...
  BOT_ALLOW_ROOT=1 bot infer image providers --json >/dev/null
}
...
is_self_swapped_package_process_exit() {
  local stderr="$1"
  [[ "$stderr" == *"[bot] Failed to start CLI:"* ]] &&
    [[ "$stderr" == *"ERR_MODULE_NOT_FOUND"* ]] &&
    [[ "$stderr" == *"/node_modules/bot/dist/"* ]]
}
...
allow_legacy_update_warning() {
  [[ "${BOT_INSTALL_ALLOW_LEGACY_UPDATE_WARNING:-0}" == "1" ]] && return 0
...
run_install_smoke() {
  if [[ -n "$FRESH_VERSION" && -n "$FRESH_TAG_URL" ]]; then
    ...
    BOT_NO_ONBOARD=1 BOT_NO_PROMPT=1 \
    ...
    if [[ -n "${BOT_INSTALL_LATEST_OUT:-}" ]]; then
      ...
      if [[ -n "$latest_npm_version" ]]; then
        printf "%s" "$latest_npm_version" > "${BOT_INSTALL_LATEST_OUT:-}"
      else
        printf "%s" "$FRESH_VERSION" > "${BOT_INSTALL_LATEST_OUT:-}"
      fi
    fi
...
run_update_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: BOT_INSTALL_UPDATE_EXPECT_VERSION is required for update mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: BOT_INSTALL_UPDATE_TAG_URL is required for update mode" >&2
    return 1
  fi
  ...
  echo "==> Run bot update from host-served tgz"
  local update_status
  ...
  if allow_legacy_update_warning; then
    update_env+=(BOT_UPDATE_IN_PROGRESS=1)
  fi
  update_stderr_file="$(mktemp)"
  set +e
  UPDATE_JSON="$(
    run_with_heartbeat "bot update" \
      "${update_env[@]}" \
      bot update --tag "$UPDATE_TAG_URL" --yes --json 2>"$update_stderr_file"
  )"
  update_status=$?
  set -e
  ...
  if [[ "$update_stderr" == *"config was written by version"* ]] && allow_legacy_update_warning; then
    ...
    echo "ERROR: bot update emitted a self-update version-skew warning" >&2
    return 1
  fi
  if [[ "$update_status" -ne 0 ]]; then
    if is_self_swapped_package_process_exit "$update_stderr"; then
      ...
      echo "ERROR: bot update failed with exit code $update_status" >&2
...
const doctorStep = steps.find((step) => step?.name === "bot doctor");
...
if (!doctorStep) {
  throw new Error("missing bot doctor step in update JSON");
}
...
if (!doctorSucceeded && !doctorWasAdvisory) {
  throw new Error(`bot doctor step failed: ${JSON.stringify(doctorStep)}`);
}
NODE
...
run_npm_global_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: BOT_INSTALL_UPDATE_EXPECT_VERSION is required for npm-global mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: BOT_INSTALL_UPDATE_TAG_URL is required for npm-global mode" >&2
    return 1
  fi
...
case "$SMOKE_MODE" in
  ...
  freshness)
    ...
    ;;
  *)
    echo "ERROR: unsupported BOT_INSTALL_SMOKE_MODE=$SMOKE_MODE" >&2
    exit 1
    ;;
esac
