...
workflow_sha=""
image_refs=()
shared_package_sha256="${BOT_SHARED_IMAGE_PACKAGE_SHA256:-}"
shared_archive_sha256="${BOT_SHARED_IMAGE_ARCHIVE_SHA256:-}"
shared_run_id="${BOT_SHARED_IMAGE_RUN_ID:-}"
shared_run_attempt="${BOT_SHARED_IMAGE_RUN_ATTEMPT:-}"
...
if (value.schema !== "bot.shared-docker-image-artifact/v1") fail("schema");
...
case "$command_name" in
  pack | load)
    ...
    if [[ "$command_name" == "pack" ]]; then
      ...
      require_positive_decimal "BOT_SHARED_IMAGE_RUN_ID" "$shared_run_id"
      require_positive_decimal "BOT_SHARED_IMAGE_RUN_ATTEMPT" "$shared_run_attempt"
