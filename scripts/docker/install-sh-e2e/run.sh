...
EXPECTED_VERSION="$(npm view "bot@${INSTALL_TAG}" version)"
...
run_profile() {
  ...
  SESSION_JSONL="/root/.bot-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "/root/.bot-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
