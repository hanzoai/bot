// Zai Fallback Repro script supports Bot repository automation.
...
export async function runZaiFallbackRepro(deps: RunZaiFallbackReproDeps = {}): Promise<number> {
  ...
  const baseDir = await mkdtemp(path.join(os.tmpdir(), "bot-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "bot.json");
  try {
    ...
    const sessionId = env.BOT_ZAI_FALLBACK_SESSION_ID ?? createUuid();
    ...
    const run1 = await run(
      "run1",
      ["bot", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
      envValidAnthropic,
    );
    if (run1.code !== 0) {
      return run1.code ?? 1;
    ...
    const run2 = await run(
      "run2",
      ["bot", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
      envInvalidAnthropic,
    );
