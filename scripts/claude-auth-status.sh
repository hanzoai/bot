...
# Checks both Claude Code and Bot auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
BOT_AUTH="$HOME/.bot/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    bot models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    jq -n \
        ...
        '{
            ...
            bot: {status: $bs, expires_at_ms: ($be | tonumber)},
...
echo ""
echo "Bot Auth (~/.bot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'bot doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active bot >/dev/null 2>&1; then
