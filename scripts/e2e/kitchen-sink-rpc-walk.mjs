...
const PLUGIN_SPEC =
  process.env.BOT_KITCHEN_SINK_NPM_SPEC || "npm:@hanzo/bot-kitchen-sink@latest";
const PLUGIN_ID = process.env.BOT_KITCHEN_SINK_PLUGIN_ID || "bot-kitchen-sink-fixture";
...
Runs the external Kitchen Sink plugin RPC walk against a built Bot entry.

Environment:
  BOT_ENTRY                         Built Bot entrypoint. Defaults to dist/index.mjs or dist/index.js.
  BOT_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@hanzo/bot-kitchen-sink@latest.
  BOT_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: bot-kitchen-sink-fixture.
  BOT_KITCHEN_SINK_PERSONALITY      Plugin fixture personality. Default: conformance.
  BOT_KITCHEN_SINK_RPC_PORT         Gateway loopback port. Default: OS-selected free port.
  BOT_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  BOT_KITCHEN_SINK_RPC_COMMAND_MS   Bot command timeout.
  BOT_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  BOT_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  BOT_KITCHEN_SINK_RPC_FETCH_MS     HTTP readiness probe timeout.
  BOT_KITCHEN_SINK_RPC_FETCH_BODY_BYTES  HTTP readiness probe response ceiling.
  BOT_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  BOT_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  BOT_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS  Per-command stdout/stderr capture ceiling.
  BOT_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function resolveKitchenSinkRpcConfig(env = process.env) {
  const commandTimeoutMs = readPositiveTimerMs(
    env.BOT_KITCHEN_SINK_RPC_COMMAND_MS,
    DEFAULT_COMMAND_TIMEOUT_MS,
    "BOT_KITCHEN_SINK_RPC_COMMAND_MS",
  );
  return {
    commandMaxRssMiB: readPositiveInt(
      env.BOT_KITCHEN_SINK_COMMAND_MAX_RSS_MIB,
      DEFAULT_MAX_COMMAND_RSS_MIB,
      "BOT_KITCHEN_SINK_COMMAND_MAX_RSS_MIB",
    ),
    commandTimeoutMs,
    fetchBodyMaxBytes: readPositiveInt(
      env.BOT_KITCHEN_SINK_RPC_FETCH_BODY_BYTES,
      DEFAULT_FETCH_BODY_MAX_BYTES,
      "BOT_KITCHEN_SINK_RPC_FETCH_BODY_BYTES",
    ),
    fetchTimeoutMs: readPositiveTimerMs(
      env.BOT_KITCHEN_SINK_RPC_FETCH_MS,
      DEFAULT_FETCH_TIMEOUT_MS,
      "BOT_KITCHEN_SINK_RPC_FETCH_MS",
    ),
    installTimeoutMs: readPositiveTimerMs(
      env.BOT_KITCHEN_SINK_RPC_INSTALL_MS,
      Math.max(commandTimeoutMs, DEFAULT_INSTALL_TIMEOUT_MS),
      "BOT_KITCHEN_SINK_RPC_INSTALL_MS",
    ),
    maxRssMiB: readPositiveInt(
      env.BOT_KITCHEN_SINK_MAX_RSS_MIB,
      DEFAULT_MAX_RSS_MIB,
      "BOT_KITCHEN_SINK_MAX_RSS_MIB",
    ),
    outputCaptureChars: readPositiveInt(
      env.BOT_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS,
      DEFAULT_OUTPUT_CAPTURE_CHARS,
      "BOT_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS",
    ),
    readyTimeoutMs: readPositiveTimerMs(
      env.BOT_KITCHEN_SINK_RPC_READY_MS,
      DEFAULT_READY_TIMEOUT_MS,
      "BOT_KITCHEN_SINK_RPC_READY_MS",
    ),
    rpcTimeoutMs: readPositiveTimerMs(
      env.BOT_KITCHEN_SINK_RPC_CALL_MS,
      DEFAULT_RPC_TIMEOUT_MS,
      "BOT_KITCHEN_SINK_RPC_CALL_MS",
    ),
  };
}
...
export async function resolveKitchenSinkRpcPort(env = process.env, options = {}) {
  const rawPort = (env.BOT_KITCHEN_SINK_RPC_PORT || "").trim();
  if (rawPort) {
    const port = readPositiveInt(rawPort, 0, "BOT_KITCHEN_SINK_RPC_PORT");
    if (port > 65535) {
      throw new Error(
        `BOT_KITCHEN_SINK_RPC_PORT must be a TCP port from 1 to 65535. Got: ${JSON.stringify(rawPort)}`,
      );
    }
    return port;
  }
...
function resolveBotRunner() {
  if (process.env.BOT_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.BOT_ENTRY],
      label: process.env.BOT_ENTRY,
    };
  }
  ...
  return { pnpm: true, baseArgs: ["bot"], label: "pnpm bot" };
}

export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "bot-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      BOT_NO_ONBOARD: "1",
      BOT_SKIP_PROVIDERS: "0",
      BOT_KITCHEN_SINK_PERSONALITY:
        process.env.BOT_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
function resolveCommandParentSignalKillGraceMs(env) {
  const raw = env.VITEST && env.BOT_TEST_KITCHEN_SINK_PARENT_SIGNAL_KILL_GRACE_MS;
  if (!raw) {
...
async function runBot(runner, args, env, options = {}) {
  const config = resolveKitchenSinkRpcConfig(env);
  const command = await resolveBotCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveBotCommand(runner, args, env, options = {}) {
  if (runner.pnpm) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.BOT_CONFIG_PATH),
        configPath: options.env.BOT_CONFIG_PATH,
...
export function usesBuiltBotEntry(runner, cwd = process.cwd(), env = process.env) {
  if (runner?.pnpm || !runner?.baseArgs?.[0]) {
    return false;
  }
  const entry = runner.baseArgs[0];
  if (env.BOT_ENTRY && entry === env.BOT_ENTRY) {
    return true;
  }
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.BOT_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function samplePosixProcessTree(pid, run, commandLineNeedles) {
  ...
  try {
    ...
    const gatewayTitleMatches = descendants.filter((row) =>
      row.command.toLowerCase().includes("bot-gateway"),
    );
...
export async function main() {
  const config = resolveKitchenSinkRpcConfig();
  let runner = resolveBotRunner();
  const port = await resolveKitchenSinkRpcPort();
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.BOT_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  let child;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runBot(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,
    ...
    assertKitchenSinkUiDescriptors(uiDescriptors, {
      expectDescriptor: env.BOT_KITCHEN_SINK_PERSONALITY !== "conformance",
    });
