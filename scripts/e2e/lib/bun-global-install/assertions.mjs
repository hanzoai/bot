...
const runWithTimeout = async (timeout, command, commandArgs) => {
  const killGrace = parsePositiveNumber(
    process.env.BOT_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS ??
      String(DEFAULT_TIMEOUT_KILL_GRACE_MS),
    "BOT_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS",
  );
...
if (mode === "assert-image-providers") {
  const raw = process.env.BOT_IMAGE_PROVIDERS_JSON ?? "";
  let parsed;
  try {
...
if (mode === "assert-release-versions") {
  ...
  const rootAiVersion = rootManifest.dependencies?.["@hanzo/bot-ai"];
  if (
    ...
    throw new Error(
      `candidate version mismatch: bot=${String(rootVersion)}, dependency=${String(rootAiVersion)}, @hanzo/bot-ai=${String(aiVersion)}`,
    );
  }
