...
const pollMs = clampTimerTimeoutMs(
  readPositiveIntEnv("BOT_PLUGIN_LIFECYCLE_METRIC_POLL_MS", 100),
);
const timeoutMs = clampTimerTimeoutMs(
  readPositiveIntEnv("BOT_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS", 300000),
);
const timeoutKillGraceMs = clampTimerTimeoutMs(
  readPositiveIntEnv("BOT_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS", 2000),
);
const maxRssKbThreshold = readPositiveIntEnv(
  "BOT_PLUGIN_LIFECYCLE_MAX_RSS_KB",
  4 * 1024 * 1024,
);
const maxWallMs = readPositiveIntEnv("BOT_PLUGIN_LIFECYCLE_MAX_WALL_MS", timeoutMs);
const maxCpuCoreRatio = readPositiveNumberEnv("BOT_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO", 16);
...
const pageSize = readPositiveIntEnvOrGetconf("BOT_PROC_PAGE_SIZE", "PAGESIZE");
const clockTicks = readPositiveIntEnvOrGetconf("BOT_PROC_CLK_TCK", "CLK_TCK");
