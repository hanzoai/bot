...
const packageName = "@hanzo/bot-kitchen-sink";
...
async function buildNpmPackArtifact(fixture) {
  const packRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "bot-clawhub-fixture-"));
  try {
    ...
    packageDetail(artifact) {
      ...
      const packageDetail = {
        package: {
          ...
          verification: {
            tier: "source-linked",
            sourceRepo: "https://github.com/bot/kitchen-sink",
            hasProvenance: false,
            scanStatus: "passed",
          },
...
async function main() {
  ...
  zip.file("package/bot.plugin.json", manifestJson, { date: new Date(0) });
  ...
  const artifactResolverDetail = {
    package: versionDetail.package ?? {
      name: packageName,
      displayName: packageDetail.package?.displayName ?? "Bot Kitchen Sink",
