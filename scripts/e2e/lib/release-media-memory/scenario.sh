...
export OPENAI_API_KEY="sk-bot-release-media-memory"
export BOT_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="BOT_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/bot-release-media-memory.XXXXXX)"
...
entry="$(bot_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'HOME=%s\n' "$HOME"
  printf 'BOT_HOME=%s\n' "$BOT_HOME"
  printf 'BOT_STATE_DIR=%s\n' "$BOT_STATE_DIR"
  printf 'BOT_CONFIG_PATH=%s\n' "$BOT_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(bot_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$BOT_CONFIG_PATH" "$CONFIG_JSON"
bot plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$BOT_STATE_DIR/workspace/memory"
...
cat >"$BOT_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

bot memory index --force >"$INDEX_LOG" 2>&1
bot memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
bot memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
