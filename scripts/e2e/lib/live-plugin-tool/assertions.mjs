...
const agentTurnTimeoutSeconds = readPositiveIntEnv(
  "BOT_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS",
  300,
);
...
const AGENT_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "BOT_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
...
const SESSION_SCAN_MAX_ENTRIES = readPositiveIntEnv(
  "BOT_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES",
  50_000,
);
...
function stateDir() {
  return process.env.BOT_STATE_DIR || path.join(process.env.HOME, ".bot");
}

function configPath() {
  return process.env.BOT_CONFIG_PATH || path.join(stateDir(), "bot.json");
}

function agentOutputPath() {
  return process.env.BOT_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH || "/tmp/bot-agent.json";
}

function agentErrorPath() {
  return process.env.BOT_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH || "/tmp/bot-agent.err";
}
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/bot-plugin-inspect.json")
    ? readJson("/tmp/bot-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "bot.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const list = readJson("/tmp/bot-plugins-list.json");
  ...
  const inspect = readJson("/tmp/bot-plugin-inspect.json");
...
function assertAgentTurn() {
  ...
  const sqliteScan = scanSqliteSessionTranscript(
    path.join(agentStateDir, "agent", "bot-agent.sqlite"),
