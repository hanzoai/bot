...
const onboard = readJson("/tmp/bot-onboard.json");
const inspect = readJson("/tmp/bot-codex-inspect.json");
...
if (!codexRecord.spec?.includes("@hanzo/bot-codex")) {
  throw new Error(`expected @hanzo/bot-codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @hanzo/bot-codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@hanzo/bot-codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@hanzo/bot-codex");
...
assertOpenAiEnvAuthProfileStore(authRaw, {
  ...
  rawKeyNeedle: "sk-bot-codex-on-demand-e2e",
});
