...
function shouldScanLogFile(entry) {
  if (!(/\.(?:log|jsonl)$/u.test(entry) || /bot-kitchen-sink-/u.test(path.basename(entry)))) {
    return false;
  }
...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".bot")];
  const deny = [
  ...
  if (scannedFiles === 0) {
    throw new Error(
      "kitchen-sink log scan found no files under the isolated scratch root or Bot home",
    );
  }
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(botPeerPath)) {
    throw new Error(`missing kitchen-sink bot peer symlink: ${botPeerPath}`);
  }
  if (!fs.lstatSync(botPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink bot peer is not a symlink: ${botPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(botPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink bot peer ${linkedHostRoot} to target ${hostRoot}`);
  }
