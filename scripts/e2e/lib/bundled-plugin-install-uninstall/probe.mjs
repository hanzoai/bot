...
function resolveStateDir() {
  if (process.env.BOT_STATE_DIR) {
    return process.env.BOT_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".bot");
}
...
function resolveBotEntry() {
  ...
  throw new Error("Missing BOT_ENTRY and dist/index.(m)js");
}
...
async function loadManifestEntries() {
  const explicit = (process.env.BOT_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `BOT_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("BOT_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("BOT_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `BOT_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.BOT_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
