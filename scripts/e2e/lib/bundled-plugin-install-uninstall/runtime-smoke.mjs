...
const RUNTIME_PORT_BASE_ENV = "BOT_BUNDLED_PLUGIN_RUNTIME_PORT_BASE";
...
const WATCHDOG_MS = readPositiveIntEnv("BOT_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS", 1000);
const READY_TIMEOUT_MS = readPositiveIntEnv("BOT_BUNDLED_PLUGIN_RUNTIME_READY_MS", 900000);
const RPC_TIMEOUT_MS = readPositiveIntEnv("BOT_BUNDLED_PLUGIN_RUNTIME_RPC_MS", 60000);
const RPC_READY_TIMEOUT_MS = readPositiveIntEnv(
  "BOT_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS",
  210000,
);
const COMMAND_TIMEOUT_MS = readPositiveIntEnv("BOT_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS", 120000);
const HTTP_PROBE_TIMEOUT_MS = readPositiveIntEnv("BOT_BUNDLED_PLUGIN_RUNTIME_HTTP_MS", 5000);
...
function manifestPath(pluginDir, pluginRoot) {
  const candidates = [
    ...(isNonEmptyString(pluginRoot) ? [path.join(pluginRoot, "bot.plugin.json")] : []),
    path.join(process.cwd(), "dist", "extensions", pluginDir, "bot.plugin.json"),
    path.join(process.cwd(), "dist-runtime", "extensions", pluginDir, "bot.plugin.json"),
  ];
...
function configPathFromEnv(env = process.env) {
  return (
    env.BOT_CONFIG_PATH || path.join(env.HOME || os.homedir(), ".bot", "bot.json")
  );
}
  ...
  const child = childProcess.spawn(
    ...
    ],
    {
      env: {
        ...
        ...params.env,
        BOT_NO_ONBOARD: "1",
        BOT_SKIP_CHANNELS: params.skipChannels ? "1" : "0",
...
export async function rpcCall(method, params, options) {
  const rpcStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-plugin-runtime-rpc-"));
  const args = [
    options.entrypoint,
    "gateway",
    "call",
...
async function smokePlugin(pluginId, pluginDir, requiresConfig, pluginIndex, pluginRoot) {
  ...
  const entrypoint = process.env.BOT_ENTRY;
  if (!entrypoint) {
    throw new Error("missing BOT_ENTRY");
  }
  ...
  const logPath = `/tmp/bot-plugin-runtime-${pluginIndex}-${pluginId}.log`;
...
async function smokeTtsGlobalDisable(pluginId, pluginDir, provider, pluginIndex, pluginRoot) {
  ...
  const logPath = `/tmp/bot-plugin-runtime-${pluginIndex}-${pluginId}-tts-disabled.log`;
...
async function smokeOpenAiTts(pluginIndex) {
  ...
  const logPath = `/tmp/bot-plugin-runtime-${pluginIndex}-openai-tts-live.log`;
  const child = startGateway({ entrypoint, port, logPath, env, skipChannels: true });
  try {
    await waitForReady({ child, port, logPath });
    await assertBaseGatewayProbes({ entrypoint, port, env });
...
export function createIsolatedStateEnv(label) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `bot-${label}-`));
