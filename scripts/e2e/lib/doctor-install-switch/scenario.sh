...
export PATH="/tmp/bot-bin:$PATH"
mkdir -p /tmp/bot-bin
cp scripts/e2e/lib/doctor-install-switch/shims/systemctl /tmp/bot-bin/systemctl
cp scripts/e2e/lib/doctor-install-switch/shims/loginctl /tmp/bot-bin/loginctl
chmod +x /tmp/bot-bin/systemctl /tmp/bot-bin/loginctl

package_tgz="${BOT_CURRENT_PACKAGE_TGZ:?missing BOT_CURRENT_PACKAGE_TGZ}"
git_root="/tmp/bot-git"
mkdir -p "$git_root"
...
(
  cd "$git_root"
  bot_e2e_maybe_timeout "${BOT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit >/tmp/bot-git-install.log 2>&1
  git init -q
  git config user.email "docker-e2e@bot.local"
  git config user.name "Bot Docker E2E"
  git add -A --
  git commit -qm "test fixture"
)
npm_log="/tmp/bot-doctor-switch-npm-install.log"
if ! bot_e2e_maybe_timeout "${BOT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz" >"$npm_log" 2>&1; then
  bot_e2e_print_log "$npm_log"
  exit 1
fi

npm_bin="/tmp/npm-prefix/bin/bot"
npm_root="/tmp/npm-prefix/lib/node_modules/bot"
...
git_cli="$git_root/bot.mjs"

package_version="$(node -p "require(\"$npm_root/package.json\").version")"
update_doctor_env="BOT_UPDATE_IN_PROGRESS=1"
update_doctor_env+=" BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1"
update_doctor_env+=" BOT_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART=1"
update_doctor_env+=" BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR=1"
update_doctor_env+=" BOT_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION=0"
...
run_flow() {
  ...
  local install_log="/tmp/bot-doctor-switch-${name}-install.log"
  local doctor_log="/tmp/bot-doctor-switch-${name}-doctor.log"
  local command_timeout="${BOT_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"

  echo "== Flow: $name =="
  bot_test_state_create "switch-${name}" empty
  export USER="testuser"

  if ! bot_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd" >"$install_log" 2>&1; then
  ...
  unit_path="$HOME/.config/systemd/user/bot-gateway.service"
  if [ ! -f "$unit_path" ]; then
  ...
  if ! bot_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd" >"$doctor_log" 2>&1; then
...
run_cross_state_approval_flow() {
  ...
  local automated_log="/tmp/bot-doctor-switch-${name}-automated.log"
  local direct_log="/tmp/bot-doctor-switch-${name}-direct.log"
  local command_timeout="${BOT_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local default_state_dir="$HOME/.bot"
  ...
  local state_database="$custom_state_dir/state/bot.sqlite"
  ...
  if ! bot_e2e_maybe_timeout "$command_timeout" env \
    BOT_STATE_DIR="$custom_state_dir" \
    BOT_CONFIG_PATH="$custom_state_dir/bot.json" \
    BOT_UPDATE_IN_PROGRESS=1 \
    "$npm_bin" doctor --repair --yes --non-interactive >"$automated_log" 2>&1; then
    bot_e2e_print_log "$automated_log"
    exit 1
  fi
...
run_proxy_env_flow() {
  ...
  if ! bot_e2e_maybe_timeout "$command_timeout" env \
    ...
    BOT_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART=1 \
    BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR=1 \
    BOT_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION=0 \
...
run_wrapper_flow() {
  ...
  local install_log="/tmp/bot-doctor-switch-${name}-install.log"
  local reinstall_log="/tmp/bot-doctor-switch-${name}-reinstall.log"
  local env_repair_log="/tmp/bot-doctor-switch-${name}-env-repair.log"
  local doctor_log="/tmp/bot-doctor-switch-${name}-doctor.log"
  local clear_log="/tmp/bot-doctor-switch-${name}-clear.log"
  local command_timeout="${BOT_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local wrapper="$HOME/.local/bin/bot-wrapper"
  node scripts/e2e/lib/doctor-install-switch/write-wrapper.mjs \
    "$wrapper" \
    "$npm_bin" \
    "$HOME/bot-wrapper-argv.log"

  local unit_path="$HOME/.config/systemd/user/bot-gateway.service"

  if ! bot_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force >"$install_log" 2>&1; then
    bot_e2e_print_log "$install_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_exec_arg "$unit_path" 2 "gateway"
  assert_env_value "$unit_path" "BOT_WRAPPER" "$wrapper"

  if ! bot_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --force >"$reinstall_log" 2>&1; then
  ...
  sed -i "/^Environment=BOT_WRAPPER=/d" "$unit_path"
  if ! bot_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
  ...
  sed -i "s#^Environment=BOT_WRAPPER=.*#Environment=BOT_WRAPPER=/tmp/stale-bot-wrapper#" "$unit_path"
  if ! bot_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    bot_e2e_print_log "$env_repair_log"
    exit 1
  fi
  ...
  if ! bot_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes >"$doctor_log" 2>&1; then
    bot_e2e_print_log "$doctor_log"
    exit 1
  fi
  if ! grep -Fq "Gateway service invokes BOT_WRAPPER:" "$doctor_log"; then
  ...
  if ! bot_e2e_maybe_timeout "$command_timeout" env BOT_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
    bot_e2e_print_log "$clear_log"
    exit 1
  fi
  assert_no_env_key "$unit_path" "BOT_WRAPPER"
