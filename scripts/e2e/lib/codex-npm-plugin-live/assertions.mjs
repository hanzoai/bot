...
const allowBetaCompatDiagnostics =
  process.env.BOT_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS === "1";
const sessionStoreContract =
  process.env.BOT_CODEX_NPM_PLUGIN_SESSION_STORE_CONTRACT || "sqlite";
const bindingStoreContract =
  process.env.BOT_CODEX_NPM_PLUGIN_BINDING_STORE_CONTRACT || "plugin-kv";
...
const MAX_ERROR_TAIL_BYTES = readPositiveIntEnv(
  "BOT_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES",
  64 * 1024,
);
const MAX_TRANSCRIPT_FILES = readPositiveIntEnv(
  "BOT_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES",
  64,
);
...
function readCodexBinding(sessionId, sessionKey, entry) {
  ...
  if (bindingStoreContract !== "plugin-kv") {
    throw new Error(
      `BOT_CODEX_NPM_PLUGIN_BINDING_STORE_CONTRACT must be plugin-kv or legacy-sidecar; got ${bindingStoreContract}`,
    );
  }

  const dbPath = path.join(stateDir(), "state", "bot.sqlite");
  if (!fs.existsSync(dbPath)) {
    throw new Error(`missing Bot state database: ${dbPath}`);
  }
...
function readLegacySessionEntry(sessionId) {
  ...
  if (typeof entry.sessionFile !== "string" || !fs.existsSync(entry.sessionFile)) {
    throw new Error(`missing Bot session file: ${entry.sessionFile}`);
  }
  const transcriptLines = readTextFileBounded(
    entry.sessionFile,
    "Bot legacy session transcript",
  ...
  if (transcriptLines.length > MAX_TRANSCRIPT_WALK_ENTRIES) {
    throw new Error(
      `Bot transcript exceeded ${MAX_TRANSCRIPT_WALK_ENTRIES} events for ${sessionId}`,
    );
  }
  const transcriptEvents = transcriptLines.map((line, index) => {
    try {
      ...
      throw new Error(`invalid Bot legacy transcript event ${index + 1} for ${sessionId}`);
    }
  });
  return {
    entry,
...
function readSessionEntry(sessionId) {
  ...
  if (sessionStoreContract !== "sqlite") {
    throw new Error(
      `BOT_CODEX_NPM_PLUGIN_SESSION_STORE_CONTRACT must be sqlite or legacy-json; got ${sessionStoreContract}`,
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        throw new Error(`invalid Bot transcript summary for ${sessionId}`);
      }
      ...
      if (transcriptSummary.transcript_bytes > MAX_TRANSCRIPT_SCAN_BYTES) {
        throw new Error(
          `Bot transcript exceeded ${MAX_TRANSCRIPT_SCAN_BYTES} bytes for ${sessionId}`,
        );
      }
      ...
      const transcriptEvents = transcriptRows.map((transcriptRow) => {
        if (typeof transcriptRow.event_json !== "string") {
          throw new Error(`invalid Bot transcript event for ${sessionId}`);
        }
...
function assertPlugin() {
  const spec = process.argv[3] || "npm:@hanzo/bot-codex";
  const list = readJson("/tmp/bot-codex-plugins-list.json");
  const inspect = readJson("/tmp/bot-codex-plugin-inspect.json");
...
function codexNpmProjectRoot() {
  return npmProjectRootForInstalledPackage(codexInstallPath(), "@hanzo/bot-codex");
}
...
function assertNpmDeps() {
  ...
  if (!fs.existsSync(pluginPackageJson)) {
    throw new Error(`missing npm-installed @hanzo/bot-codex package.json: ${pluginPackageJson}`);
  }
  ...
  if (pluginPackage.name !== "@hanzo/bot-codex") {
  ...
  if (!openAiCodexPackageJson) {
    throw new Error("missing @openai/codex dependency under .bot/npm");
  }
...
function assertPreflight() {
  ...
  const output = readTextFileBounded("/tmp/bot-codex-preflight.log", "Codex preflight log");
...
function extractBotToolTimeline(transcriptEvents) {
  const timeline = [];
...
function assertFollowthroughTranscript({ transcriptEvents, progressMarker, completeMarker }) {
  const timeline = extractBotToolTimeline(transcriptEvents);
...
function assertAgentTurnEvidence({ marker, sessionId, modelRef, stdoutPath, stderrPath }) {
  const stdout = readTextFileBounded(stdoutPath, "Bot agent JSON");
  const stderr = readTextFileTail(stderrPath, "Bot agent stderr");
  const response = JSON.parse(stdout);
  const text = extractAgentReplyTexts(JSON.stringify(response)).join("\n");
  if (!text.includes(marker)) {
    throw new Error(
      `Bot agent reply did not contain ${marker}:\nstdout=${stdout}\nstderr=${stderr}`,
    );
  }
  ...
  if (!Number.isSafeInteger(transcriptEventCount) || transcriptEventCount < 1) {
    throw new Error(`missing Bot transcript events for ${sessionId}`);
  }
...
function assertAgentTurn() {
  assertAgentTurnEvidence({
    ...
    stdoutPath: "/tmp/bot-codex-agent.json",
    stderrPath: "/tmp/bot-codex-agent.err",
  });
}
...
function assertFollowthrough() {
  ...
  const stdoutPath = "/tmp/bot-codex-followthrough.json";
  const stderrPath = "/tmp/bot-codex-followthrough.err";
  const response = JSON.parse(readTextFileBounded(stdoutPath, "Bot follow-through JSON"));
...
function assertUninstalled() {
  ...
  const list = readJson("/tmp/bot-codex-plugins-list-after-uninstall.json");
  const plugin = (list.plugins || []).find((entry) => entry.id === "codex");
  if (plugin?.status === "loaded" || plugin?.enabled === true) {
...
function assertAgentError() {
  ...
  if (!Number.isInteger(status) || status === 0) {
    throw new Error(
      `expected Bot agent to fail after Codex uninstall, got status ${process.argv[3]}`,
    );
  }
  const stdout = fs.existsSync("/tmp/bot-codex-agent-after-uninstall.json")
    ? readTextFileTail(
        "/tmp/bot-codex-agent-after-uninstall.json",
        "post-uninstall agent stdout",
      )
    : "";
  const stderr = fs.existsSync("/tmp/bot-codex-agent-after-uninstall.err")
    ? readTextFileTail(
