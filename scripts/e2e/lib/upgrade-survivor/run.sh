...
ARTIFACT_ROOT="$(dirname "${BOT_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/bot-upgrade-survivor-artifacts/summary.json}")"
export BOT_UPGRADE_SURVIVOR_RUNTIME_ROOT="${BOT_UPGRADE_SURVIVOR_RUNTIME_ROOT:-/tmp/bot-upgrade-survivor-runtime}"
RUNTIME_ROOT="$BOT_UPGRADE_SURVIVOR_RUNTIME_ROOT"
STATE_HOME_ROOT="${BOT_UPGRADE_SURVIVOR_STATE_HOME_ROOT:-$RUNTIME_ROOT/state-home}"
mkdir -p "$ARTIFACT_ROOT"
mkdir -p "$RUNTIME_ROOT"
export TMPDIR="${BOT_UPGRADE_SURVIVOR_TMPDIR:-$RUNTIME_ROOT/tmp}"
export BOT_TEST_STATE_TMPDIR="${BOT_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR:-$RUNTIME_ROOT/state-tmp}"
mkdir -p "$TMPDIR" "$BOT_TEST_STATE_TMPDIR"
...
SUMMARY_JSON="${BOT_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${BOT_UPGRADE_SURVIVOR_BASELINE:?missing BOT_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${BOT_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${BOT_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${BOT_CURRENT_PACKAGE_TGZ:-}}"
...
export BOT_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
...
validate_baseline_package_spec() {
  ...
  echo "BOT_UPGRADE_SURVIVOR_BASELINE must be bot@latest, bot@beta, bot@alpha, an exact Bot release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "BOT_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    bot@*)
      baseline_spec="$raw"
      baseline_version="${raw#bot@}"
      ;;
    *@*)
      echo "BOT_UPGRADE_SURVIVOR_BASELINE must be bot@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="bot@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "BOT_UPGRADE_SURVIVOR_BASELINE must be bot@latest, bot@beta, bot@alpha, bot@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "BOT_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.BOT_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.BOT_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.BOT_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      bot_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/bot\n' "$npm_config_prefix"
}

legacy_runtime_deps_symlink_plugin() {
  local plugin="${BOT_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "BOT_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@bot-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${BOT_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$BOT_STATE_DIR/extensions/opik-bot"
...
JSON
  cat >"$shadow_root/bot.plugin.json" <<'JSON'
{
  "id": "opik-bot",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.bot-runtime-deps.json" \
      "$plugin_dir/.bot-runtime-deps-stamp.json" \
      "$plugin_dir/.bot-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.bot-install-stage-upgrade-survivor" \
      "$plugin_dir/.bot-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$BOT_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$BOT_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/bot-upgrade-survivor-dep" \
      "$plugin_dir/.bot-runtime-deps-copy-upgrade-survivor/node_modules/bot-upgrade-survivor-dep" \
      "$plugin_dir/.bot-install-stage-upgrade-survivor" \
      "$plugin_dir/.bot-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/bot-upgrade-survivor-dep" \
      "$BOT_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/bot-upgrade-survivor-dep" \
      "$BOT_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/bot-upgrade-survivor-dep"
    printf '{"name":"bot-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/bot-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.bot-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.bot-runtime-deps-stamp.json"
    printf '{"name":"bot-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.bot-runtime-deps-copy-upgrade-survivor/node_modules/bot-upgrade-survivor-dep/package.json"
    printf '{"name":"bot-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/bot-upgrade-survivor-dep/package.json"
    printf '{"name":"bot-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$BOT_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/bot-upgrade-survivor-dep/package.json"
    printf '{"name":"bot-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$BOT_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/bot-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"bot-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
reset_run_state() {
  rm_rf_retry "$npm_config_prefix" "$TMPDIR" "$BOT_TEST_STATE_TMPDIR" "$STATE_HOME_ROOT"
  rm -f "$SYSTEMCTL_SHIM_PID_FILE" "$SYSTEMCTL_SHIM_DAEMON_LOG"
  mkdir -p "$npm_config_prefix" "$npm_config_cache" "$TMPDIR" "$BOT_TEST_STATE_TMPDIR"
}

install_baseline() {
  normalize_baseline
  echo "Installing baseline package: $baseline_spec"
  if ! bot_e2e_maybe_timeout "${BOT_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix "$npm_config_prefix" "$baseline_spec" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
    echo "baseline npm install failed" >&2
    bot_e2e_print_log "$BASELINE_INSTALL_LOG" >&2
    return 1
  fi
  if ! command -v bot >/dev/null; then
    echo "baseline install did not expose bot on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" bot --version 2>&1)"; then
    echo "baseline bot --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline bot --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  bot_e2e_eval_test_state_from_b64 "${BOT_TEST_STATE_FUNCTION_B64:?missing BOT_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.bot /root/workspace
    bot_test_state_create /root minimal
  else
    bot_test_state_create "$STATE_HOME_ROOT" minimal
  fi
  export BOT_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" bot config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
    echo "generated baseline config failed baseline validation" >&2
    bot_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG" >&2
    return 1
  fi
}
...
install_update_restart_service_unit() {
  if ! bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u BOT_GATEWAY_TOKEN -u BOT_GATEWAY_PASSWORD bot gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
    echo "baseline gateway service install failed" >&2
    bot_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR" >&2
    bot_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON" >&2
    return 1
  fi
}
...
assert_baseline_state() {
  BOT_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing BOT_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(BOT_ALLOW_ROOT=1)
  fi
  if ! bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" bot "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
    echo "bot update failed" >&2
    bot_e2e_print_log "$UPDATE_ERR" >&2
    bot_e2e_print_log "$UPDATE_JSON" >&2
    return 1
  fi
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
assert_root_managed_vps_cli_usable() {
  ...
  bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" bot config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" bot plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" bot doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
    echo "bot doctor failed" >&2
    bot_e2e_print_log "$DOCTOR_LOG" >&2
    return 1
  fi
}

validate_post_doctor_config() {
  if ! bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" bot config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${BOT_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$BOT_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
...
start_gateway() {
  ...
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u BOT_GATEWAY_TOKEN -u BOT_GATEWAY_PASSWORD bot gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_status() {
  ...
  if ! bot_e2e_maybe_timeout "$COMMAND_TIMEOUT" bot gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
    echo "gateway status failed" >&2
    bot_e2e_print_log "$STATUS_ERR" >&2
    bot_e2e_print_log "$GATEWAY_LOG" >&2
    return 1
  fi
  ...
  if [ "$status_seconds" -gt "$budget" ]; then
    ...
    bot_e2e_print_log "$STATUS_JSON" >&2
    return 1
  fi
