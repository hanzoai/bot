BOT_PLUGINS_FIXTURE_PID_FILES=()
BOT_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=0
BOT_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=""

bot_plugins_read_positive_int_env() {
...
bot_plugins_read_nonnegative_decimal_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [[ -z "${!name+x}" ]]; then
...
bot_plugins_cleanup_fixture_servers() {
  local pid_file
  local pid
  for pid_file in "${BOT_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    [[ -f "$pid_file" ]] || continue
    pid="$(cat "$pid_file" 2>/dev/null || true)"
    if [[ "$pid" =~ ^[0-9]+$ ]]; then
      bot_plugins_stop_fixture_process "$pid"
    fi
    rm -f "$pid_file"
  done
}

bot_plugins_signal_fixture_process() {
  local pid="$1"
  local signal="$2"
...
bot_plugins_fixture_process_alive() {
  local pid="$1"
  kill -0 "$pid" >/dev/null 2>&1 || kill -0 -- "-$pid" >/dev/null 2>&1
}

bot_plugins_stop_fixture_process() {
  local pid="$1"
  local _
  local attempts interval
  attempts="$(bot_plugins_read_positive_int_env BOT_PLUGINS_FIXTURE_STOP_ATTEMPTS 40)" || return $?
  interval="$(bot_plugins_read_nonnegative_decimal_env BOT_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS 0.25)" || return $?
  if declare -F bot_e2e_stop_process >/dev/null 2>&1; then
    bot_e2e_stop_process "$pid"
    return
  fi
  bot_plugins_signal_fixture_process "$pid" TERM
  for _ in $(seq 1 "$attempts"); do
    ! bot_plugins_fixture_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return; }
    sleep "$interval"
  done
  bot_plugins_signal_fixture_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}

bot_plugins_print_fixture_log() {
  local log_file="$1"
  ...
  local max_bytes
  max_bytes="$(bot_plugins_read_positive_int_env BOT_DOCKER_E2E_LOG_PRINT_BYTES 65536)" || return $?
...
bot_plugins_validate_fixture_log_print_bytes() {
  bot_plugins_read_positive_int_env BOT_DOCKER_E2E_LOG_PRINT_BYTES 65536 >/dev/null
}

bot_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  BOT_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  bot_plugins_install_fixture_cleanup_trap
}

bot_plugins_install_fixture_cleanup_trap() {
  if [[ "${BOT_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED:-0}" = "1" ]]; then
    return
  fi

  local existing_trap
  existing_trap="$(trap -p EXIT || true)"
  if [[ -n "$existing_trap" && "$existing_trap" != *bot_plugins_fixture_exit_trap* ]]; then
    local existing_action="${existing_trap#trap -- }"
    existing_action="${existing_action% EXIT}"
    eval "BOT_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=$existing_action"
  fi

  BOT_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=1
  trap bot_plugins_fixture_exit_trap EXIT
}

bot_plugins_fixture_exit_trap() {
  local status="$?"
  bot_plugins_cleanup_fixture_servers
  if [[ -n "${BOT_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION:-}" ]]; then
    eval "$BOT_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION"
  fi
  exit "$status"
}
...
packageJson.bot.extensions = ["./index.js", " "];
...
start_npm_fixture_registry() {
  ...
  for _ in $(seq 1 100); do
    ...
    if ! kill -0 "$server_pid" 2>/dev/null; then
      bot_plugins_print_fixture_log "$server_log"
      return 1
    fi
    sleep 0.1
  done
