// Filesystem script supports Bot repository automation.
...
const BOT_VERSION_PATTERN = /Bot\s+([0-9][^\s]*)/gi;
...
export async function extractLastBotVersionFromLog(
  logPath: string,
  pattern = BOT_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.BOT_PARALLELS_ARTIFACT_ROOT || path.join(repoRoot, ".artifacts", "parallels");
