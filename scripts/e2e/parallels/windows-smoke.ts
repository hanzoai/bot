...
// Windows Smoke script supports Bot repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://bot.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-bot-native-e2e-",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Bot\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: newest "pre-bot-native-e2e-*" snapshot
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://bot.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run bot update --channel dev.
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  private gatewayRecoveryAfterMs =
    readPositiveIntEnv("BOT_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S", 180) * 1000;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("bot-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("bot-parallels-windows-tgz.");
    try {
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsBotResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanBot: boolean): void {
    const cleanScript = cleanBot
      ? "npm.cmd uninstall -g bot --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
Invoke-Bot --version
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-Bot --version");
...
Invoke-Bot onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "bot onboard failed with exit code $LASTEXITCODE" }
...
${windowsScopedEnvFunction}
$script:BotUpdateExit = 0
Invoke-WithScopedEnv @{ BOT_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; BOT_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-Bot update --channel dev --yes --json
  $script:BotUpdateExit = $LASTEXITCODE
}
if ($script:BotUpdateExit -ne 0) { throw "bot update failed with exit code $script:BotUpdateExit" }
Invoke-Bot --version
Invoke-Bot update status --json`,
...
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-Bot gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-Bot gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-Bot gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-Bot gateway status ${suffix}`);
  }
...
if (-not $agentOk) { throw 'bot agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastBotVersion(this.runDir, phaseName, /Bot\s+([0-9][^\s]*)/gi);
  }
