// Npm Update Scripts script supports Bot repository automation.
...
const windowsStalePostSwapImportRegex = String.raw`node_modules\\bot\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`;
const macosGuestPath =
  "/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/usr/local/bin:/usr/local/sbin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin";
const macosBotCommand = '"$BOT_BIN"';
...
function posixPrintLogTailFunction(): string {
  ...
  log_file="$1"
  max_bytes="\${BOT_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES:-262144}"
  case "$max_bytes" in
...
for attempt in 1 2; do
  ...
  rm -f "$HOME/.bot/agents/main/sessions/$session_id.jsonl"
  output_file="$(mktemp)"
  set +e
  BOT_ALLOW_ROOT="\${BOT_ALLOW_ROOT:-}" ${input.auth.apiKeyEnv}=${shellQuote(input.auth.apiKeyValue)} ${command} agent --local --agent main --session-id "$session_id" --message 'Reply with exact ASCII text OK only.' --thinking off --timeout ${resolveParallelsModelTimeoutSeconds(platform)} --json >"$output_file" 2>&1
  rc=$?
  set -e
...
if [ "$agent_ok" != true ]; then
  echo "bot agent finished without OK response" >&2
  exit 1
fi`;
}
...
function windowsUpdateWithBundledPluginsDisabled(input: NpmUpdateScriptInput): string {
  const registryEntry = input.npmRegistry
    ...
    : "";
  return `$script:BotUpdateExit = 0
$updateOutput = Invoke-WithScopedEnv @{ BOT_DISABLE_BUNDLED_PLUGINS = '1'; BOT_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'${registryEntry} } {
  Invoke-Bot update --tag ${psSingleQuote(input.updateTarget)} --yes --json --no-restart 2>&1
  $script:BotUpdateExit = $LASTEXITCODE
}
$updateExit = $script:BotUpdateExit
$updateOutput`;
}

function windowsGatewayReadyScript(): string {
  return `function Wait-BotGateway {
  $deadline = (Get-Date).AddSeconds(180)
  $attempt = 0
  while ((Get-Date) -lt $deadline) {
    Invoke-Bot gateway status --deep --require-rpc --timeout 15000
    if ($LASTEXITCODE -eq 0) { return }
    $attempt += 1
    if ($attempt -eq 4) {
      Invoke-Bot gateway start *>&1 | Out-Host
    }
    Start-Sleep -Seconds 5
  }
  throw "gateway did not become ready after update"
}
Invoke-Bot gateway restart *>&1 | Out-Host
...
${windowsCodexPlatformPackageRepairFunction()}
$sessionPath = Join-Path $env:USERPROFILE '.bot\\agents\\main\\sessions\\parallels-npm-update-windows.jsonl'
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $sessionsDir = Join-Path $env:USERPROFILE '.bot\\agents\\main\\sessions'
  $sessionPath = Join-Path $sessionsDir "$sessionId.jsonl"
  Remove-Item $sessionPath -Force -ErrorAction SilentlyContinue
  $output = Invoke-Bot agent --local --agent main --session-id $sessionId --model ${psSingleQuote(input.auth.modelId)} --message 'Reply with exact ASCII text OK only.' --thinking off --timeout ${resolveParallelsModelTimeoutSeconds("windows")} --json 2>&1
...
if (-not $agentOk) { throw 'bot agent finished without OK response' }`;
}
...
BOT_BIN="$(resolve_required_command bot)"
scrub_future_plugin_entries() {
  python3 - <<'PY'
import json
from pathlib import Path
path = Path.home() / ".bot" / "bot.json"
if not path.exists():
    raise SystemExit(0)
try:
...
stop_bot_gateway_processes() {
  BOT_DISABLE_BUNDLED_PLUGINS=1 "$BOT_BIN" gateway stop || true
  pkill -f 'bot.*gateway' >/dev/null 2>&1 || true
...
start_bot_gateway() {
  stop_bot_gateway_processes
  rm -f /tmp/bot-parallels-macos-gateway.log
  trap '' HUP
  /usr/bin/env BOT_HOME="$HOME" BOT_STATE_DIR="$HOME/.bot" BOT_CONFIG_PATH="$HOME/.hanzoai/bot.json" ${input.auth.apiKeyEnv}=${shellQuote(
    input.auth.apiKeyValue,
  )} "$BOT_BIN" gateway run --bind loopback --port 18789 --force >/tmp/bot-parallels-macos-gateway.log 2>&1 </dev/null &
  sleep 1
}
wait_for_gateway() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    if "$BOT_BIN" gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    attempt=$((attempt + 1))
  ...
  print_log_tail /tmp/bot-parallels-macos-gateway.log >&2
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_bot_gateway_processes
${posixNpmRegistryEnv(input.npmRegistry)}BOT_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 BOT_DISABLE_BUNDLED_PLUGINS=1 "$BOT_BIN" update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck(macosBotCommand, input.expectedNeedle)}
start_bot_gateway
wait_for_gateway
"$BOT_BIN" models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands(macosBotCommand, input.auth.modelId, "macos")}
"$BOT_BIN" config set agents.defaults.skipBootstrap true --strict-json
"$BOT_BIN" config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript(macosBotCommand, input, "macos", "parallels-npm-update-macos")}`;
}
...
function Remove-FuturePluginEntries {
  $configPath = Join-Path $env:USERPROFILE '.bot\\bot.json'
...
'@
  $nodeScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) ('bot-future-plugin-scrub-' + [guid]::NewGuid().ToString('N') + '.cjs')
  try {
...
function Stop-BotGatewayProcesses {
  Invoke-Bot gateway stop *>&1 | Out-Host
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'bot.*gateway' } |
...
if ($updateExit -ne 0) {
  ...
  if (-not $stalePostSwapImport) { throw "bot update failed with exit code $updateExit" }
  Write-Host "bot update returned a stale post-swap module import; continuing to post-update health checks"
}
...
export BOT_ALLOW_ROOT=1
${posixPrintLogTailFunction()}
scrub_future_plugin_entries() {
  node - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || "/root", ".bot", "bot.json");
...
stop_bot_gateway_processes() {
  BOT_DISABLE_BUNDLED_PLUGINS=1 BOT_ALLOW_ROOT=1 bot gateway stop || true
  pkill -f 'bot.*gateway' >/dev/null 2>&1 || true
}
start_bot_gateway() {
  pkill -f "bot gateway run" >/dev/null 2>&1 || true
  rm -f /tmp/bot-parallels-linux-gateway.log
  setsid sh -lc ${shellQuote(
    `exec env BOT_HOME=/root BOT_STATE_DIR=/root/.bot BOT_CONFIG_PATH=/root/.hanzoai/bot.json BOT_DISABLE_BONJOUR=1 BOT_ALLOW_ROOT=1 ${input.auth.apiKeyEnv}=${shellQuote(
      input.auth.apiKeyValue,
    )} bot gateway run --bind loopback --port 18789 --force >/tmp/bot-parallels-linux-gateway.log 2>&1`,
...
wait_for_gateway() {
  ...
  attempt=0
  while [ "$SECONDS" -lt "$deadline" ]; do
    if bot gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    attempt=$((attempt + 1))
    if [ "$attempt" -eq 4 ]; then
  ...
  print_log_tail /tmp/bot-parallels-linux-gateway.log >&2
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_bot_gateway_processes
${posixNpmRegistryEnv(input.npmRegistry)}BOT_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 BOT_DISABLE_BUNDLED_PLUGINS=1 bot update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("bot", input.expectedNeedle)}
start_bot_gateway
wait_for_gateway
bot models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("bot", input.auth.modelId, "linux")}
bot config set agents.defaults.skipBootstrap true --strict-json
bot config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("bot", input, "linux", "parallels-npm-update-linux")}`;
}
...
while ($true) {
  $version = Invoke-Bot --version
  $version
  if ($LASTEXITCODE -eq 0) { break }
  if ((Get-Date) -ge $versionDeadline) { throw "bot --version failed with exit code $LASTEXITCODE" }
  Start-Sleep -Seconds 2
}`;
  }
...
while ($true) {
  ...
  if ((Get-Date) -ge $versionDeadline) {
    if ($LASTEXITCODE -ne 0) { throw "bot --version failed with exit code $LASTEXITCODE" }
    throw ${mismatch}
  }
  Start-Sleep -Seconds 2
}`;
