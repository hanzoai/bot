...
// Linux Smoke script supports Bot repository automation.
...
function parseBotPackageVersion(value: string): string | null {
  return value.match(/\b(\d{4}\.\d{1,2}\.\d{1,2}(?:-[A-Za-z0-9.]+)?)\b/u)?.[1] ?? null;
}

function compareBotPackageVersions(left: string, right: string): number {
  const parse = (value: string): [number, number, number] => {
    const match = parseBotPackageVersion(value)?.match(/^(\d{4})\.(\d+)\.(\d+)/u);
    if (!match) {
      return [0, 0, 0];
    }
...
class LinuxSmoke extends SmokeRunController<LinuxOptions> {
  ...
  private disableBonjour = parseBoolEnv(process.env.BOT_PARALLELS_LINUX_DISABLE_BONJOUR);
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("bot-parallels-linux.");
    this.phases = new PhaseRunner(this.runDir);
    this.tgzDir = await makeTempDir("bot-parallels-linux-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () =>
      this.installMainTgz("bot-main-fresh.tgz"),
    );
  ...
  protected async runUpgradeLane(): Promise<void> {
    ...
    await this.phase("upgrade.install-main", 420, () =>
      this.installMainTgz("bot-main-upgrade.tgz"),
    );
...
npm uninstall -g bot >/dev/null 2>&1 || true
rm -rf /root/.bot /root/.npm/_cacache
rm -f /tmp/bot-parallels-linux-gateway.log`);
  }

  private installLatestRelease(): void {
    this.downloadGuestFile(this.options.installUrl, "/tmp/bot-install.sh");
...
if command -v curl >/dev/null 2>&1; then
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec(["bot", "--version"]);
  ...
  private injectBadPluginFixture(): void {
    this.guestBash(String.raw`set -euo pipefail
plugin_dir=/root/.bot/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{"name":"@hanzo/bot-test-bad-plugin","version":"1.0.0","bot":{"extensions":["./index.cjs"],"setupEntry":"./setup-entry.cjs"}}
JSON
cat >"$plugin_dir/bot.plugin.json" <<'JSON'
...
config_path = Path("/root/.hanzoai/bot.json")
...
PY`);
  ...
  private shouldExpectBadPluginDiagnostic(lane: "fresh" | "upgrade"): boolean {
    const version = parseBotPackageVersion(this.versionForLane(lane));
    if (!version) {
      return true;
    }
    return compareBotPackageVersions(version, BAD_PLUGIN_DIAGNOSTIC_MIN_VERSION) >= 0;
  }
  ...
  private startGatewayBackground(): void {
    const bonjourEnv = this.disableBonjour ? " BOT_DISABLE_BONJOUR=1" : "";
    this.guestBash(
      String.raw`pkill -f "bot gateway run" >/dev/null 2>&1 || true
rm -f /tmp/bot-parallels-linux-gateway.log
setsid sh -lc ` +
        shellQuote(
          `exec env BOT_HOME=/root BOT_STATE_DIR=/root/.bot BOT_CONFIG_PATH=/root/.hanzoai/bot.json BOT_ALLOW_ROOT=1${bonjourEnv} ${this.auth.apiKeyEnv}=${shellQuote(
  ...
  private showGatewayStatusCompat(check = true): boolean {
    const help = this.guestExec(["bot", "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["bot", "gateway", "status", "--deep", "--require-rpc"]
      : ["bot", "gateway", "status", "--deep"];
    const result = run(
      "prlctl",
      ["exec", this.options.vmName, "/usr/bin/env", "HOME=/root", "BOT_ALLOW_ROOT=1", ...args],
      {
        check: false,
        quiet: true,
        timeoutMs: this.remainingPhaseTimeoutMs(),
...
if isinstance(paths, list):
    load["paths"] = [path for path in paths if path != "/root/.bot/test-bad-plugin"]
allow = plugins.get("allow")
if isinstance(allow, list):
    plugins["allow"] = [plugin_id for plugin_id in allow if plugin_id != "test-bad-plugin"]
config_path.write_text(json.dumps(config, indent=2) + "\n")
PY
rm -rf /root/.bot/test-bad-plugin`);
  }
  ...
  private verifyLocalTurn(): void {
    this.guestExec(["bot", "models", "set", this.auth.modelId]);
...
JSON
bot config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    ...
    this.guestExec(["bot", "config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env BOT_ALLOW_ROOT=1 ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} bot agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private async extractLastVersion(phaseId: string): Promise<string> {
    return await extractLastBotVersion(
      this.runDir,
      phaseId,
      /(Bot [^\r\n]+ \([0-9a-f]{7,}\))/g,
    );
