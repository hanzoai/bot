// Smoke Common helper supports Bot script workflows.
import { readFile, rm } from "node:fs/promises";
import path from "node:path";
import { extractLastBotVersionFromLog } from "./filesystem.ts";
...
export async function extractLastBotVersion(
  runDir: string,
  phaseName: string,
  pattern: RegExp,
): Promise<string> {
  return await extractLastBotVersionFromLog(path.join(runDir, `${phaseName}.log`), pattern);
}
