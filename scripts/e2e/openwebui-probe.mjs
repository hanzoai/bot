...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.BOT_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "bot/default") ?? modelIds.find((id) => id === "bot");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing bot model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `bot model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
