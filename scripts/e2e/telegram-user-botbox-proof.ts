...
// Telegram User Crabbox Proof script supports Bot repository automation.
...
type SessionFile = {
  command: "telegram-user-botbox-session";
  createdAt: string;
  botbox: {
    class: string;
...
const DEFAULT_OUTPUT_ROOT = ".artifacts/qa-e2e/telegram-user-botbox";
...
const REMOTE_ROOT = "/tmp/bot-telegram-user-botbox";
...
export function readTelegramUserProofLogTailBytes(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveIntEnv("BOT_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES", 256 * 1024, env);
}
...
function usageText() {
  return [
    "Usage:",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts [probe] [--text /status] [--expect Bot]",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts start [--tdlib-url <url>]",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts send --session <session.json> --text <text>",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts run --session <session.json> -- <remote command>",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts view --session <session.json> --message-id <id>",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts screenshot --session <session.json>",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts status --session <session.json>",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts finish --session <session.json>",
    "  node --import tsx scripts/e2e/telegram-user-botbox-proof.ts publish --session <session.json> --pr <number>",
    "",
    "Useful options:",
    ...
    "  --repo <owner/name>           GitHub repo for publish. Default: hanzoai/bot.",
    "  --session <path>              Session file from start. Default: <output-dir>/session.json.",
    "  --summary <text>              Artifact publish summary.",
    "  --sut-container               Isolate the local Bot SUT in Docker.",
...
export function parseArgs(argvInput: string[]): Options {
  ...
  const opts: Options = {
    botboxClass: "standard",
    command,
    botboxBin: trimToValue(process.env.BOT_TELEGRAM_USER_CRABBOX_BIN) ?? "botbox",
    desktopChatTitle:
      trimToValue(process.env.BOT_TELEGRAM_USER_DESKTOP_CHAT_TITLE) ?? "Bot Testing",
    dryRun: false,
    expect: ["Bot"],
    gatewayPort: 19_879,
    idleTimeout: "60m",
    ...
    ttl: "120m",
    userDriverScript:
      trimToValue(process.env.BOT_TELEGRAM_USER_DRIVER_SCRIPT) ?? DEFAULT_USER_DRIVER,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--class") {
      opts.botboxClass = readValue();
    } else if (arg === "--botbox-bin") {
      opts.botboxBin = readValue();
...
function repoRoot() {
  ...
  if (
    ...
    throw new Error("Run from the Bot repo root.");
  }
  return cwd;
}
...
function childProcessBaseEnv() {
  const keys = [
    ...
    "LC_ALL",
    "NODE_OPTIONS",
    "BOT_BUILD_PRIVATE_QA",
    "BOT_ENABLE_PRIVATE_QA_CLI",
    "PATH",
    "PNPM_HOME",
    "SHELL",
    "TEMP",
...
function gatewayEnv(params: {
  ...
  return {
    ...
    ...(params.gatewayPassword ? { BOT_GATEWAY_PASSWORD: params.gatewayPassword } : {}),
...
export function createBotGatewaySpawnSpec(params: {
  ...
  if (params.pnpmExecPath) {
    return {
      args: ["bot", "gateway", "--port", String(params.gatewayPort)],
  ...
  const spec = createPnpmRunnerSpawnSpec({
    ...
    pnpmArgs: ["bot", "gateway", "--port", String(params.gatewayPort)],
  });
  return {
    args: spec.args,
...
export function writeSutConfig(params: {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bot-tg-botbox-sut-"));
...
type StartLocalSutDeps = {
  createGatewaySpawnSpec?: typeof createBotGatewaySpawnSpec;
...
export async function startLocalSut(
  ...
  const createGatewaySpawnSpec = deps.createGatewaySpawnSpec ?? createBotGatewaySpawnSpec;
...
export function createContainerizedSutSpawnSpec(params: {
  ...
  fs.writeFileSync(
    inputPath,
    `${JSON.stringify({
      gatewayPassword: params.gatewayEnv.BOT_GATEWAY_PASSWORD,
  ...
  return {
    args: [
      "-n",
      "/usr/local/sbin/bot-mantis-sut-container",
      "run",
...
export function runSutContainerAction(
  ...
  const result = run(
    "sudo",
    ["-n", "/usr/local/sbin/bot-mantis-sut-container", action, containerName, runtimeRoot],
    {
      encoding: "utf8",
      env: childProcessBaseEnv(),
      stdio: "pipe",
...
async function startLocalSutDaemon(params: {
  ...
  try {
    if (params.sutContainer) {
      ...
      containerName = `bot-telegram-sut-${randomUUID()}`;
    ...
    const gatewaySpec = createBotGatewaySpawnSpec({
      env: gatewayEnvVars,
      gatewayPort: params.gatewayPort,
      pnpmExecPath: params.pnpmBin,
      repoRoot: params.repoRoot,
...
export function createCrabboxWarmupArgs(
  opts: Pick<
    Options,
    "botboxClass" | "idleTimeout" | "mcpAppFixture" | "provider" | "target" | "ttl"
  >,
) {
  return [
    "warmup",
...
tdlib_url=${tdlibUrl}
setup_step_timeout_kill_after="\${BOT_TELEGRAM_USER_SETUP_KILL_AFTER_SECONDS:-30}s"
apt_timeout="\${BOT_TELEGRAM_USER_APT_TIMEOUT_SECONDS:-900}s"
download_timeout="\${BOT_TELEGRAM_USER_DOWNLOAD_TIMEOUT_SECONDS:-600}"
download_connect_timeout="\${BOT_TELEGRAM_USER_DOWNLOAD_CONNECT_TIMEOUT_SECONDS:-15}"
download_retries="\${BOT_TELEGRAM_USER_DOWNLOAD_RETRIES:-3}"
download_retry_delay="\${BOT_TELEGRAM_USER_DOWNLOAD_RETRY_DELAY_SECONDS:-5}"
tdlib_clone_timeout="\${BOT_TELEGRAM_USER_TDLIB_CLONE_TIMEOUT_SECONDS:-600}s"
tdlib_build_timeout="\${BOT_TELEGRAM_USER_TDLIB_BUILD_TIMEOUT_SECONDS:-1800}s"
run_setup_step() {
  local label="$1"
...
function writeReport(params: {
  ...
  const reportPath = path.join(params.outputDir, "telegram-user-botbox-proof.md");
  fs.writeFileSync(
    reportPath,
    [
...
const FULL_ARTIFACT_JSON_NAMES = new Set([
  "probe.json",
  "status.json",
  "telegram-user-botbox-proof-summary.json",
  "telegram-user-botbox-session-summary.json",
]);
const FULL_ARTIFACT_FILE_EXTENSIONS = new Set([".gif", ".log", ".md", ".mp4", ".png"]);
const FULL_ARTIFACT_PROOF_REPORT = "telegram-user-botbox-proof.md";
...
function readSession(root: string, opts: Options, outputDir: string) {
  ...
  if (session.command !== "telegram-user-botbox-session") {
...
async function startSession(root: string, opts: Options, outputDir: string) {
  ...
  const hasConvexEnv =
    trimToValue(process.env.BOT_QA_CONVEX_SITE_URL) &&
    trimToValue(process.env.BOT_QA_CONVEX_SECRET_CI);
  if (!hasConvexEnv && !fs.existsSync(convexEnvFile)) {
    throw new Error(`Missing Convex env file: ${opts.envFile ?? DEFAULT_CONVEX_ENV_FILE}`);
  }
  await runCommand({ command: opts.botboxBin, args: ["--version"], cwd: root });
  if (opts.dryRun) {
    return {
      command: "telegram-user-botbox-session",
      botboxClass: opts.botboxClass,
      outputDir,
  ...
  try {
    ...
    return {
      ...
      webvnc: `${opts.botboxBin} webvnc --provider ${opts.provider} --target ${opts.target} --id ${leaseId} --open`,
      commands: {
        send: `bot-telegram-user-botbox-proof send --session ${path.relative(root, pathname)} --text '/status'`,
        view: `bot-telegram-user-botbox-proof view --session ${path.relative(root, pathname)} --message-id <message-id>`,
        run: `bot-telegram-user-botbox-proof run --session ${path.relative(root, pathname)} -- bash -lc 'source ${REMOTE_ROOT}/env.sh && python3 ${REMOTE_ROOT}/user-driver.py transcript --limit 20 --json'`,
        finish: `bot-telegram-user-botbox-proof finish --session ${path.relative(root, pathname)} --preview-crop telegram-window`,
      },
    };
  } catch (error) {
    const cleanupErrors: unknown[] = [];
...
async function sendSessionProbe(root: string, opts: Options, outputDir: string) {
  ...
  await scpToRemote(root, session.botbox.inspect, probeScript, `${REMOTE_ROOT}/remote-probe.sh`);
  await sshRun(root, session.botbox.inspect, `bash ${REMOTE_ROOT}/remote-probe.sh`);
  const localProbe = path.join(session.outputDir, `probe-${stamp}.json`);
  await scpFromRemote(root, session.botbox.inspect, remoteProbe, localProbe);
  return {
...
async function runSessionCommand(root: string, opts: Options, outputDir: string) {
  ...
  await sshRun(root, session.botbox.inspect, command, { outputFile: logPath });
...
async function screenshotSession(root: string, opts: Options, outputDir: string) {
  ...
  const screenshotPath = path.join(
    session.outputDir,
    `telegram-user-botbox-${new Date().toISOString().replace(/[:.]/gu, "-")}.png`,
  );
  await runCommand({
    command: opts.botboxBin,
    args: [
      "screenshot",
      "--provider",
      session.botbox.provider,
      "--target",
      session.botbox.target,
      "--id",
      session.botbox.id,
      "--output",
      screenshotPath,
...
async function statusSession(root: string, opts: Options, outputDir: string) {
  ...
  const inspect = await inspectCrabbox(opts, root, session.botbox.id);
  return {
    botbox: {
      id: session.botbox.id,
      slug: inspect.slug,
      state: inspect.state,
    },
    session: path.relative(root, pathname),
    status: "pass",
    webvnc: `${opts.botboxBin} webvnc --provider ${session.botbox.provider} --target ${session.botbox.target} --id ${session.botbox.id} --open`,
  };
}
...
async function viewSession(root: string, opts: Options, outputDir: string) {
  ...
  await sshRun(root, session.botbox.inspect, renderProofViewCommand(link), {
    outputFile: logPath,
  });
  return {
...
async function finishSession(root: string, opts: Options, outputDir: string) {
  ...
  const videoPath = path.join(session.outputDir, "telegram-user-botbox-session.mp4");
  const motionVideoPath = path.join(session.outputDir, "telegram-user-botbox-session-motion.mp4");
  const motionGifPath = path.join(session.outputDir, "telegram-user-botbox-session-motion.gif");
  const croppedMotionVideoPath = path.join(
    session.outputDir,
    "telegram-user-botbox-session-motion-telegram-window.mp4",
  );
  const croppedMotionGifPath = path.join(
    session.outputDir,
    "telegram-user-botbox-session-motion-telegram-window.gif",
  );
  const screenshotPath = path.join(session.outputDir, "telegram-user-botbox-session.png");
  ...
  const terminateDesktopSession = async () => {
    ...
    await terminateRemoteDesktopSession(root, session.botbox.inspect).catch((error: unknown) => {
      summary.desktopSessionTerminateError = error instanceof Error ? error.message : String(error);
    });
  };
  try {
    await stopRemoteRecording(root, session.botbox.inspect, session);
    await scpFromRemote(root, session.botbox.inspect, session.recorder.remoteVideo, videoPath);
    await scpFromRemote(
      root,
      session.botbox.inspect,
    ...
    await scpFromRemote(root, session.botbox.inspect, session.recorder.log, ffmpegLogPath).catch(
      () => {},
    );
    await runCommand({
      command: opts.botboxBin,
    ...
    if (session.botbox.createdLease && !opts.keepBox) {
      await stopCrabbox(root, opts, session.botbox.id).catch((error: unknown) => {
        summary.botboxStopError = error instanceof Error ? error.message : String(error);
      });
    }
    if (opts.keepBox) {
      summary.keepBox = true;
      summary.webvnc = `${opts.botboxBin} webvnc --provider ${session.botbox.provider} --target ${session.botbox.target} --id ${session.botbox.id} --open`;
    }
    fs.rmSync(session.localRoot, { force: true, recursive: true });
    const summaryPath = path.join(session.outputDir, "telegram-user-botbox-session-summary.json");
...
async function publishSessionArtifacts(root: string, opts: Options, outputDir: string) {
  ...
  if (!opts.publishFullArtifacts) {
    ...
    fs.copyFileSync(
      publishGifPath,
      path.join(publishDir, "telegram-user-botbox-session-motion.gif"),
    );
  }
  await runCommand({
    command: opts.botboxBin,
...
async function main() {
  ...
  const localRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bot-telegram-botbox-"));
  const summary: JsonObject = {
    artifacts: {},
    botbox: { provider: opts.provider, target: opts.target },
    outputDir,
  ...
  try {
    ...
    if (opts.dryRun) {
      summary.status = "pass";
      summary.plan = {
        command: "telegram-user-botbox-proof",
        botboxClass: opts.botboxClass,
        outputDir,
        provider: opts.provider,
        target: opts.target,
    ...
    const videoPath = path.join(outputDir, "telegram-user-botbox-proof.mp4");
    ...
    const motionVideoPath = path.join(outputDir, "telegram-user-botbox-proof-motion.mp4");
    const motionGifPath = path.join(outputDir, "telegram-user-botbox-proof-motion.gif");
    summary.mediaPreview = await createMotionPreview({
      motionGifPath,
      motionVideoPath,
      opts,
    ...
    const screenshotPath = path.join(outputDir, "telegram-user-botbox-proof.png");
    await runCommand({
      command: opts.botboxBin,
      args: [
        "screenshot",
    ...
    if (opts.keepBox && leaseId) {
      summary.keepBox = true;
      summary.webvnc = `${opts.botboxBin} webvnc --provider ${opts.provider} --target ${opts.target} --id ${leaseId} --open`;
    }
    summary.finishedAt = new Date().toISOString();
    const summaryPath = path.join(outputDir, "telegram-user-botbox-proof-summary.json");
    fs.writeFileSync(summaryPath, `${JSON.stringify(summary, null, 2)}\n`);
