...
# Bot under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `bot update`. Verifies that:
#
...
#
# Requires: Docker, a built bot-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="$(docker_e2e_resolve_image "bot-multi-node-update-e2e" BOT_MULTI_NODE_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${BOT_MULTI_NODE_UPDATE_E2E_SKIP_BUILD:-0}"
DOCKER_RUN_TIMEOUT="${BOT_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${BOT_CURRENT_PACKAGE_TGZ:-}")"
...
docker_e2e_run_with_harness \
  ...
  -e BOT_NO_ONBOARD=1 \
  -e BOT_NO_PROMPT=1 \
  -e BOT_SKIP_PROVIDERS=1 \
  -e BOT_SKIP_CHANNELS=1 \
  -e BOT_DISABLE_BONJOUR=1 \
...
echo ""
echo "── Step 2: Install Bot under node-A ──"

# Use node-A to install bot with npm prefix A.
...
echo "Installing Bot package under node-A prefix: $NPM_PREFIX_A"
bot_e2e_install_package "$ARTIFACTS/install-a.log" "Bot package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking bot location..."

BOT_A="$(command -v bot)"
echo "bot binary: $BOT_A"
echo "bot version: $(bot --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/bot"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/bot-gateway.service"
...
if ! bot gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
# but bot is NOT installed there.
...
echo "which bot: $(command -v bot)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run bot update (this is the bug) ──"
...
echo "Running bot update --yes --json..."
UPDATE_EXIT=0
bot update --yes --json \
  --tag /tmp/bot-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: bot update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but Bot package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the Bot package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/bot/package.json" ]; then
  echo "WARNING: Bot was ALSO installed under node-B prefix (split install)"
else
  echo "OK: Bot is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell Bot root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
