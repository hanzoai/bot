// Bot rescue-message Docker harness.
...
import type { BotConfig } from "../../dist/config/types.bot.js";
...
function makeParams(commandBody: string, cfg: BotConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: BotConfig, isGroup = false): Promise<string> {
...
async function main() {
  const tempState = await createE2eStateDir("bot-bot-");
  tempState.registerExitCleanup();
  const stateDir = tempState.stateDir;
  const configPath = process.env.BOT_CONFIG_PATH ?? path.join(stateDir, "bot.json");
  ...
  const denied = await invoke("/bot status", {
  ...
  const refusedTui = await invoke("/bot talk to agent", cfg);
  assert(
  ...
  const plan = await invokeWithDeps(
    "/bot set default model openai/gpt-5.2",
    cfg,
    deterministicInference,
  );
  assert(
    plan.includes("Reply /bot yes to apply"),
    "persistent change did not require approval",
  );
  const applied = await invokeWithDeps("/bot yes", cfg, deterministicInference);
  assert(applied.includes("Default model: openai/gpt-5.2"), "approved change did not apply");

  const configValid = await invoke("/bot validate config", cfg);
  assert(configValid.includes("Config valid:"), "config validation did not report valid config");

  const configSetPlan = await invoke("/bot config set gateway.port 19001", cfg);
  assert(
    configSetPlan.includes("Reply /bot yes to apply"),
    "generic config set did not require approval",
  );
  const configSetApplied = await invoke("/bot yes", cfg);
  assert(configSetApplied.includes("[bot] done: config.set"), "generic config set failed");

  const refPlan = await invoke(
    "/bot config set-ref gateway.auth.token env BOT_GATEWAY_TOKEN",
    cfg,
  );
  assert(
    refPlan.includes("Reply /bot yes to apply"),
    "SecretRef set did not require approval",
  );
  const refApplied = await invoke("/bot yes", cfg);
  assert(refApplied.includes("[bot] done: config.setRef"), "SecretRef set failed");

  const agentPlan = await invoke("/bot create agent work workspace /tmp/bot-work", cfg);
  assert(
    agentPlan.includes("Reply /bot yes to apply"),
    "agent creation did not require approval",
  );
  const agentApplied = await invoke("/bot yes", cfg);
  assert(agentApplied.includes("[bot] done: agents.create"), "agent creation did not apply");

  const setupPlan = await invokeWithDeps(
    "/bot setup workspace /tmp/bot-setup model openai/gpt-5.2",
    cfg,
    deterministicInference,
  );
  assert(setupPlan.includes("Reply /bot yes to apply"), "setup did not require approval");
  const setupApplied = await invokeWithDeps("/bot yes", cfg, deterministicInference);
  assert(setupApplied.includes("[bot] done: bot.setup"), "setup did not apply");

  const gatewayRestarts: string[] = [];
  const gatewayCommand = makeParams("/bot restart gateway", cfg).command;
  const gatewayPlan = await runSystemAgentRescueMessage({
    cfg,
    command: gatewayCommand,
    commandBody: "/bot restart gateway",
    agentId: "default",
    isGroup: false,
    deps: {
  ...
  assert(
    revokedApproval === "No pending Bot rescue change is waiting for approval.",
  ...
  assert(
    gatewayApplied?.includes("[bot] done: gateway.restart"),
  ...
  const doctorCommand = makeParams("/bot doctor fix", cfg).command;
  const doctorReply = await runSystemAgentRescueMessage({
    cfg,
    command: doctorCommand,
    commandBody: "/bot doctor fix",
    agentId: "default",
    isGroup: false,
    deps: {
  ...
  assert(
    doctorReply?.includes("bot doctor --fix"),
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as BotConfig;
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "BOT_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/bot-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.entries?.work?.workspace === "/tmp/bot-work",
  ...
  assert(
    audits.some((audit) => audit.operation === "bot.setup"),
    "setup audit missing",
  );
  ...
  console.log("Bot rescue Docker E2E passed");
}
