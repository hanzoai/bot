...
const REMOTE_ROOT = "/tmp/bot-telegram-user-crabbox";
...
function usageText() {
  return [
    "Usage:",
    "  node --import tsx scripts/e2e/telegram-user-crabbox-proof.ts [probe] [--text /status] [--expect Bot]",
...
export function parseArgs(argvInput: string[]): Options {
  ...
  const opts: Options = {
    ...
    command,
    crabboxBin: trimToValue(process.env.BOT_TELEGRAM_USER_CRABBOX_BIN) ?? "crabbox",
    desktopChatTitle:
...
export function writeSutConfig(params: {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bot-tg-crabbox-sut-"));
...
async function startSession(root: string, opts: Options, outputDir: string) {
  ...
  try {
    ...
    return {
      ...
      commands: {
        send: `bot-telegram-user-crabbox-proof send --session ${path.relative(root, pathname)} --text '/status'`,
        view: `bot-telegram-user-crabbox-proof view --session ${path.relative(root, pathname)} --message-id <message-id>`,
        run: `bot-telegram-user-crabbox-proof run --session ${path.relative(root, pathname)} -- bash -lc 'source ${REMOTE_ROOT}/env.sh && python3 ${REMOTE_ROOT}/user-driver.py transcript --limit 20 --json'`,
        finish: `bot-telegram-user-crabbox-proof finish --session ${path.relative(root, pathname)} --preview-crop telegram-window`,
      },
    };
  } catch (error) {
    const cleanupErrors: unknown[] = [];
...
async function main() {
  ...
  const localRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bot-telegram-crabbox-"));
  const summary: JsonObject = {
    artifacts: {},
