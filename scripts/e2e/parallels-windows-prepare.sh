...
VM_NAME="Windows 11"
BOT_PARALLELS_WINDOWS_API=1
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TODAY="$(date +%F)"
CLEAN_SNAPSHOT="windows-11-clean-os-${TODAY}"
BASELINE_SNAPSHOT="pre-bot-native-e2e-${TODAY}"
GUEST_PROFILE=""
GUEST_PROFILE_PS=""
GUEST_ARCH=""
...
SNAPSHOT=""
SECURE_STAGE_DIR="C:/ProgramData/BotPrerequisiteInstallers"
COMMAND="help"
if [[ "${BOT_PARALLELS_WINDOWS_LIBRARY_ONLY:-0}" != "1" ]]; then
  COMMAND="${1:-help}"
  if [[ $# -gt 0 ]]; then
    shift
  fi
...
Commands:
  ...
  prepare     Create a clean snapshot, provision the reusable Bot baseline, and snapshot it.
  verify      Verify base prerequisites and prove the guest contains no Bot product state.
...
It installs only reusable Bot prerequisites: WSL platform/package, Git, and Node/npm.
It refuses to create the baseline when the guest contains an Bot CLI, app package, process,
...
create_clean_snapshot_if_raw() {
  ...
  create_snapshot "$CLEAN_SNAPSHOT" "Clean Windows baseline before Bot development prerequisites."
}
...
if (Get-Command bot.cmd -ErrorAction SilentlyContinue) { $dirty.Add('bot.cmd on PATH') }
if (Test-Path (Join-Path $env:USERPROFILE '.bot')) { $dirty.Add('Bot CLI state directory exists') }
if (Test-Path (Join-Path $env:APPDATA 'BotTray')) { $dirty.Add('BotTray AppData exists') }
if (Test-Path (Join-Path $env:APPDATA 'BotTray-Dev')) { $dirty.Add('BotTray-Dev AppData exists') }
if (Test-Path (Join-Path $env:LOCALAPPDATA 'BotTray')) { $dirty.Add('Bot Companion install/state directory exists') }
if (Test-Path (Join-Path $env:LOCALAPPDATA 'BotTray-Dev')) { $dirty.Add('Bot Companion dev install/state directory exists') }
if (Get-AppxPackage -Name '*Bot*' -ErrorAction SilentlyContinue) { $dirty.Add('Bot app package installed') }
if (Get-Process -Name '*Bot*' -ErrorAction SilentlyContinue) { $dirty.Add('Bot process running') }
$uninstallRoots = @(
  'HKCU:/Software/Microsoft/Windows/CurrentVersion/Uninstall/*',
  'HKLM:/Software/Microsoft/Windows/CurrentVersion/Uninstall/*',
  'HKLM:/Software/WOW6432Node/Microsoft/Windows/CurrentVersion/Uninstall/*'
)
if (Get-ItemProperty $uninstallRoots -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -like 'Bot Companion*' }) {
  $dirty.Add('Bot Companion uninstall registration exists')
}
...
ensure_wsl_package() {
  ...
  run_windows_installer prlctl exec "$VM_NAME" msiexec.exe /i 'C:\ProgramData\BotPrerequisiteInstallers\WSL.msi' /qn /norestart '/L*v' 'C:\Windows\Temp\bot-wsl-install.log'
  finish_installer_reboot
  guest_user_cmd 'wsl.exe --version' >/dev/null || {
    guest_system_ps "Get-Content 'C:/Windows/Temp/bot-wsl-install.log' -Tail 80" >&2 || true
    die "WSL package install did not produce a working wsl.exe"
  }
  guest_user_cmd 'wsl.exe --set-default-version 2' >/dev/null
  guest_system_ps "Remove-Item -LiteralPath '${wsl_msi}','C:/Windows/Temp/bot-wsl-install.log' -Force -ErrorAction SilentlyContinue"
}
...
winget_download() {
  ...
  local download_dir="${GUEST_PROFILE//\//\\}\\Downloads\\BotPrereqs"
  guest_user_ps "if (Test-Path -LiteralPath '${GUEST_PROFILE_PS}/Downloads/BotPrereqs') { Remove-Item -LiteralPath '${GUEST_PROFILE_PS}/Downloads/BotPrereqs' -Recurse -Force }"
...
downloaded_installer() {
  local pattern="$1"
  guest_user_ps "Get-ChildItem -LiteralPath '${GUEST_PROFILE_PS}/Downloads/BotPrereqs' -File | Where-Object { \$_.Name -like '${pattern}' } | Sort-Object LastWriteTime -Descending | Select-Object -First 1 -ExpandProperty FullName" | tr -d '\r' | tail -n 1
}
...
prepare() {
  ...
  guest_user_cmd 'wsl.exe --set-default-version 2' >/dev/null
  verify_baseline
  create_snapshot "$BASELINE_SNAPSHOT" "E2E-ready Bot Windows baseline with WSL 2, Git, Node/npm, and no Bot product state."
