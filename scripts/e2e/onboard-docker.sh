...
IMAGE_NAME="$(docker_e2e_resolve_image "bot-onboard-e2e" BOT_ONBOARD_E2E_IMAGE)"
BOT_TEST_STATE_FUNCTION_B64="$(docker_e2e_test_state_function_b64)"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env BOT_ONBOARD_MAX_MEMORY_MIB 2048)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env BOT_ONBOARD_MAX_CPU_PERCENT 1200)"
DOCKER_RUN_TIMEOUT="${BOT_ONBOARD_DOCKER_RUN_TIMEOUT:-1200s}"
...
CONTAINER_NAME="bot-onboard-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/bot-onboard.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/bot-onboard-stats.XXXXXX")"
