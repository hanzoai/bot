...
push_relay_base_url="${BOT_PUSH_SANDBOX_RELAY_BASE_URL:-https://ios-push-relay-sandbox.bot.ai}"
simulator_proof_secret_env="${BOT_SIMULATOR_PUSH_PROOF_SECRET_ENV:-BOT_SIMULATOR_PUSH_PROOF_SECRET}"
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  ...
  xcodebuild_overrides+=(
    "BOT_PUSH_MODE=simulatorSandbox"
    "BOT_PUSH_RELAY_BASE_URL=${push_relay_base_url}"
    "BOT_APNS_ENTITLEMENT_ENVIRONMENT=development"
  )
fi

unset "${simulator_proof_secret_env}"
if [[ "${simulator_proof_secret_env}" != "BOT_SIMULATOR_PUSH_PROOF_SECRET" ]]; then
  unset BOT_SIMULATOR_PUSH_PROOF_SECRET
fi
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  # shellcheck disable=SC2086
  SIMCTL_CHILD_BOT_SIMULATOR_PUSH_PROOF_SECRET="${simulator_proof_secret}" \
