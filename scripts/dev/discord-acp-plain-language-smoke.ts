#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports Bot repository automation.
...
const DEFAULT_BOT_CLI_TIMEOUT_MS = 60_000;
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "bot") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or bot.`,
  );
}
...
function usage(): string {
  return (
    ...
    "  --state-dir <p>              Override Bot state dir for plugin-state polling\n" +
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const channelId =
    resolveArg("--channel", argv) || process.env.BOT_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver", argv) || process.env.BOT_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token", argv) || process.env.BOT_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.BOT_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
  ...
  const botTokenPrefix =
    ...
    "Bot";
  const targetAgent =
    resolveArg("--agent", argv) || process.env.BOT_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
  ...
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms", argv) || process.env.BOT_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.BOT_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir", argv) || resolveStateDir());
  const botBin =
    resolveArg("--bot-bin", argv) ||
    process.env.BOT_DISCORD_SMOKE_BOT_BIN ||
    "bot";
...
async function botCliJson<T>(params: {
  botBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.botBin, params.args, {
    ...
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_BOT_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
