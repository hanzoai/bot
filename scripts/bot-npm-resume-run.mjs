...
const WORKFLOW_PATH = ".github/workflows/bot-npm-release.yml";
...
function requiredString(value, label) {
  if (typeof value !== "string" || value.length === 0) {
    fail(`Bot npm resume run is missing ${label}.`);
  }
  return value;
}
...
function requiredSha(value, label) {
  ...
  if (!SHA_PATTERN.test(sha)) {
    fail(`Bot npm resume run has invalid ${label}.`);
  }
  return sha;
}
...
export function validateBotNpmResumeRun({
  ...
  if (!branchMatch) {
    fail(`Bot npm resume run has an untrusted workflow ref: ${url}`);
  }
  ...
  if (
    ...
    fail(`Bot npm resume run has an untrusted workflow identity: ${url}`);
  }

  const tagObjectSha = requiredSha(tagRef?.object?.sha, "tooling tag object SHA");
  if (tagRef?.object?.type !== "tag") {
    fail(`Bot npm resume run tooling ref is not a signed annotated tag: ${url}`);
  }
  ...
  if (
    ...
    fail(
      `Bot npm resume run is not bound to a real, main-reachable protected tooling tag: ${url}`,
    );
  }
  ...
  if (
    ...
    fail(`Bot npm resume run lacks successful parent release approval validation: ${url}`);
  }
...
export function runBotNpmResumeGh(args, params = {}) {
...
export function resolveBotNpmResumeRun({ repo, runId, runGh = runBotNpmResumeGh }) {
  if (!/^[1-9][0-9]*$/u.test(runId)) {
    fail("Bot npm resume run id must be a positive integer.");
  }
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(repo)) {
    fail("Bot npm resume repository must be owner/name.");
  }
  ...
  return validateBotNpmResumeRun({
...
function main(argv = process.argv.slice(2)) {
  const result = resolveBotNpmResumeRun(parseArgs(argv));
