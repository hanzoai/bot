...
LIVE_MAX_MODELS="${BOT_LIVE_MAX_MODELS:-}"
if [[ -n "$LIVE_MAX_MODELS" && ! "$LIVE_MAX_MODELS" =~ ^\+?[0-9]+$ ]]; then
  echo "invalid BOT_LIVE_MAX_MODELS: $LIVE_MAX_MODELS" >&2
  exit 2
fi
LIVE_MODEL_TIMEOUT_MS="${BOT_LIVE_MODEL_TIMEOUT_MS:-}"
if [[ -n "$LIVE_MODEL_TIMEOUT_MS" ]]; then
  LIVE_MODEL_TIMEOUT_MS="$(bot_live_read_positive_int_env BOT_LIVE_MODEL_TIMEOUT_MS "$LIVE_MODEL_TIMEOUT_MS")"
fi
TEMP_DIRS=()
DOCKER_HOME_MOUNT=()
cleanup_temp_dirs() {
...
if bot_live_truthy "${BOT_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  BOT_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${BOT_LIVE_PROVIDERS:-}" || -n "${BOT_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      bot_live_collect_auth_dirs_from_csv "${BOT_LIVE_PROVIDERS:-}"
      bot_live_collect_auth_dirs_from_csv "${BOT_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      bot_live_collect_auth_files_from_csv "${BOT_LIVE_PROVIDERS:-}"
      bot_live_collect_auth_files_from_csv "${BOT_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${BOT_DOCKER_PROFILE_ENV_ONLY:-0}"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> External auth dirs: ${AUTH_DIRS_CSV:-none}"
echo "==> External auth files: ${AUTH_FILES_CSV:-none}"
DOCKER_RUN_ARGS=()
bot_live_init_docker_run_args DOCKER_RUN_ARGS "${BOT_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e BOT_LIVE_TEST=1 \
  -e BOT_LIVE_MODELS="${BOT_LIVE_MODELS:-modern}" \
  -e BOT_LIVE_PROVIDERS="${BOT_LIVE_PROVIDERS:-}" \
  -e BOT_LIVE_MAX_MODELS="$LIVE_MAX_MODELS" \
  -e BOT_LIVE_MODEL_TIMEOUT_MS="$LIVE_MODEL_TIMEOUT_MS" \
  -e BOT_LIVE_REQUIRE_PROFILE_KEYS="${BOT_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e BOT_LIVE_GATEWAY_MODELS="${BOT_LIVE_GATEWAY_MODELS:-}" \
  -e BOT_LIVE_GATEWAY_PROVIDERS="${BOT_LIVE_GATEWAY_PROVIDERS:-}" \
  -e BOT_LIVE_GATEWAY_MAX_MODELS="${BOT_LIVE_GATEWAY_MAX_MODELS:-}" \
