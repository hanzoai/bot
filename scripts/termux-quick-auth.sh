...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/bot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Bot Re-Auth" -c "After getting key, run: ssh $SERVER '~/bot/scripts/mobile-reauth.sh'" --id bot-auth
        ;;
    *)
