...
# Bot CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
BOT_EFFECTIVE_HOME="$(resolve_bot_effective_home)"
PREFIX="${BOT_PREFIX:-${HOME}/.bot}"
BOT_VERSION="${BOT_VERSION:-latest}"
DEFAULT_NODE_VERSION="24.15.0"
ARMV7_DEFAULT_NODE_VERSION="22.22.3"
NODE_VERSION="${BOT_NODE_VERSION:-${DEFAULT_NODE_VERSION}}"
NODE_VERSION_REQUESTED=0
if [[ -n "${BOT_NODE_VERSION:-}" ]]; then
...
INSTALL_METHOD="${BOT_INSTALL_METHOD:-npm}"
GIT_DIR="${BOT_GIT_DIR:-${BOT_EFFECTIVE_HOME}/bot}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.bot; use \$BOT_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/bot, or \$BOT_HOME/bot)
  --version <ver>                     Bot version (default: latest)
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${BOT_GIT_DIR:-${BOT_EFFECTIVE_HOME}/bot}}"
...
resolve_git_bot_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "bot" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_bot() {
  local requested="${BOT_VERSION:-latest}"
  if is_bot_source_package_install_spec "$requested"; then
    fail "npm installs do not support Bot GitHub source targets like '${requested}'. Use --install-method git --version main, latest, beta, an exact version, or a built .tgz package."
  fi
  ...
  log "Installing Bot (${requested})..."
  if [[ "$SET_NPM_PREFIX" -eq 1 ]]; then
    fix_npm_prefix_if_needed
  fi

  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "bot@latest"; then
      log "npm install bot@latest failed; retrying bot@next"
      emit_json "{\"event\":\"step\",\"name\":\"bot\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "bot@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "bot@${requested}"
  fi
...
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/bot/dist/entry.js" "\$@"
EOF
...
install_bot_from_git() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"bot\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
  if [[ -d "$repo_dir/.git" ]]; then
    log "Installing Bot from git checkout: ${repo_dir}"
  else
    log "Installing Bot from GitHub (${repo_url})..."
  fi
...
EOF
  chmod +x "${PREFIX}/bin/bot"
  emit_json "{\"event\":\"step\",\"name\":\"bot\",\"status\":\"ok\",\"method\":\"git\"}"
}
...
main() {
  ...
  if [[ -n "$installed_version" ]]; then
    ...
    log "Bot installed (${installed_version})."
  else
...
if [[ "${BOT_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
