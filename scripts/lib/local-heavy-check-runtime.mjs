...
export function isLocalCheckEnabled(env) {
  const raw = env.BOT_LOCAL_CHECK?.trim().toLowerCase();
...
export function applyLocalTsgoPolicy(args, env, hostResources) {
  ...
  if (defaultProjectRun) {
    ...
    insertBeforeSeparator(
      nextArgs,
      "--tsBuildInfoFile",
      nextEnv.BOT_TSGO_BUILD_INFO_FILE ?? DEFAULT_LOCAL_TSGO_BUILD_INFO_FILE,
    );
  }
  ...
  if (nextEnv.BOT_TSGO_PPROF_DIR && !hasFlag(nextArgs, "--pprofDir")) {
    insertBeforeSeparator(nextArgs, "--pprofDir", nextEnv.BOT_TSGO_PPROF_DIR);
  }
...
export function shouldAcquireLocalHeavyCheckLockForOxlint(
  ...
  if (env.BOT_OXLINT_FORCE_LOCK === "1") {
    return true;
  }
...
export function acquireLocalHeavyCheckLockSync(params) {
  ...
  const timeoutMs = readPositiveInt(
    env.BOT_HEAVY_CHECK_LOCK_TIMEOUT_MS,
    DEFAULT_LOCK_TIMEOUT_MS,
    "BOT_HEAVY_CHECK_LOCK_TIMEOUT_MS",
  );
  const pollMs = readPositiveInt(
    env.BOT_HEAVY_CHECK_LOCK_POLL_MS,
    DEFAULT_LOCK_POLL_MS,
    "BOT_HEAVY_CHECK_LOCK_POLL_MS",
  );
  const progressMs = readPositiveInt(
    env.BOT_HEAVY_CHECK_LOCK_PROGRESS_MS,
    DEFAULT_LOCK_PROGRESS_MS,
    "BOT_HEAVY_CHECK_LOCK_PROGRESS_MS",
  );
  const staleLockMs = readPositiveInt(
    env.BOT_HEAVY_CHECK_STALE_LOCK_MS,
    DEFAULT_STALE_LOCK_MS,
    "BOT_HEAVY_CHECK_STALE_LOCK_MS",
  );
...
function resolveHeavyCheckLocksDir(cwd, env) {
  const lockScope = env.BOT_HEAVY_CHECK_LOCK_SCOPE?.trim().toLowerCase();
  if (lockScope === "worktree") {
    return path.join(resolveGitWorktreeRoot(cwd), ".artifacts", "bot-local-checks");
  }

  return path.join(resolveGitCommonDir(cwd), "bot-local-checks");
}
...
function readLocalCheckMode(env, defaultMode) {
  const raw = env.BOT_LOCAL_CHECK_MODE?.trim().toLowerCase();
