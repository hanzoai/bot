// Verifies installed packages can bootstrap the default Bot workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "bot-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "bot.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".bot", "workspace");
