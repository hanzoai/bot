...
const BUNDLED_PLUGIN_BUILD_IDS_ENV = "BOT_BUNDLED_PLUGIN_BUILD_IDS";
/** @internal Shared repository-script contract. */
export const DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV = "BOT_INTERNAL_DOCKER_BUILD_PLUGIN_IDS";
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  if (params.packageJson?.bot?.release?.publishToNpm === true) {
    return false;
  }
...
export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.bot?.extensions)
    ? packageJson.bot.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.bot?.setupEntry === "string" &&
    packageJson.bot.setupEntry.trim().length > 0
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const candidate of candidates) {
    ...
    const hasManifest =
      relativeFiles?.includes("bot.plugin.json") ?? fs.existsSync(manifestPath);
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "bot.plugin.json"));
    }
    if (hasPackageJson) {
