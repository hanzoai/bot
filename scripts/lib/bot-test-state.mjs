#!/usr/bin/env node
// Creates isolated Bot test HOME/state directories and shell snippets.
...
function usage() {
  return `Usage:
  node scripts/lib/bot-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/bot-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/bot-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "bot-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "bot-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderAuthProfileSecretKeyExport() {
  return [
    'BOT_AUTH_PROFILE_SECRET_KEY_FILE="$BOT_TEST_STATE_HOME/.bot-test-auth-profile-secret-key"',
    'if [ -s "$BOT_AUTH_PROFILE_SECRET_KEY_FILE" ]; then',
    '  BOT_AUTH_PROFILE_SECRET_KEY="$(cat "$BOT_AUTH_PROFILE_SECRET_KEY_FILE")"',
    "else",
    '  BOT_AUTH_PROFILE_SECRET_KEY="$(od -An -N 32 -tx1 /dev/urandom | tr -d " \\n")"',
    '  ( umask 077; printf "%s\\n" "$BOT_AUTH_PROFILE_SECRET_KEY" > "$BOT_AUTH_PROFILE_SECRET_KEY_FILE" )',
    "fi",
    'if [ -z "$BOT_AUTH_PROFILE_SECRET_KEY" ]; then',
    '  echo "failed to generate BOT_AUTH_PROFILE_SECRET_KEY" >&2',
    "  return 1 2>/dev/null || exit 1",
    "fi",
    "export BOT_AUTH_PROFILE_SECRET_KEY",
  ];
}
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'BOT_TEST_STATE_JSON'`,
    json,
    "BOT_TEST_STATE_JSON",
  ].join("\n");
}
...
function buildCreatePlan(options = {}) {
  ...
  const env = {
    ...
    BOT_AUTH_PROFILE_SECRET_KEY: generateAuthProfileSecretKey(),
...
/** Create an isolated Bot test state directory and optional scenario config. */
async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `bot-${label}-`));
...
/** Render shell commands that create and export an isolated Bot test state. */
function renderShellSnippet(options = {}) {
  const label = normalizeLabel(options.label);
  const scenario = requireScenario(options.scenario);
  const config = scenarioConfig(scenario, options);
  const env = scenarioEnv(scenario);
  const homeTemplate = `bot-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'BOT_TEST_STATE_TMP_ROOT="${BOT_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    'BOT_TEST_STATE_TMP_ROOT="${BOT_TEST_STATE_TMP_ROOT%/}"',
    '[ -n "$BOT_TEST_STATE_TMP_ROOT" ] || BOT_TEST_STATE_TMP_ROOT="/tmp"',
    "export BOT_TEST_STATE_TMP_ROOT",
    'mkdir -p "$BOT_TEST_STATE_TMP_ROOT"',
    `BOT_TEST_STATE_HOME="$(mktemp -d "$BOT_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$BOT_TEST_STATE_HOME"',
    'export USERPROFILE="$BOT_TEST_STATE_HOME"',
    'export BOT_HOME="$BOT_TEST_STATE_HOME"',
    'export BOT_STATE_DIR="$BOT_TEST_STATE_HOME/.bot"',
    'export BOT_CONFIG_PATH="$BOT_STATE_DIR/bot.json"',
    ...renderAuthProfileSecretKeyExport(),
    'export BOT_TEST_WORKSPACE_DIR="$BOT_TEST_STATE_HOME/workspace"',
    'mkdir -p "$BOT_STATE_DIR" "$BOT_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$BOT_CONFIG_PATH"', config);
  if (configWrite) {
...
/** Render a reusable shell function for creating isolated Bot test state. */
function renderShellFunction() {
  return `bot_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown Bot test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    /*)
      BOT_TEST_STATE_HOME="$raw_label"
      mkdir -p "$BOT_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${BOT_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      tmp_root="\${tmp_root%/}"
      [ -n "$tmp_root" ] || tmp_root="/tmp"
      mkdir -p "$tmp_root"
      BOT_TEST_STATE_HOME="$(mktemp -d "$tmp_root/bot-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$BOT_TEST_STATE_HOME"
  export USERPROFILE="$BOT_TEST_STATE_HOME"
  ...
  export BOT_TEST_WORKSPACE_DIR="$BOT_TEST_STATE_HOME/workspace"
  unset BOT_AGENT_DIR
  unset BOT_SERVICE_REPAIR_POLICY
  mkdir -p "$BOT_STATE_DIR" "$BOT_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$BOT_CONFIG_PATH" <<'BOT_TEST_STATE_JSON'
{}
BOT_TEST_STATE_JSON
      ;;
    update-stable)
...
BOT_TEST_STATE_JSON
      ;;
    external-service)
      export BOT_SERVICE_REPAIR_POLICY="external"
      cat > "$BOT_CONFIG_PATH" <<'BOT_TEST_STATE_JSON'
{}
BOT_TEST_STATE_JSON
      ;;
