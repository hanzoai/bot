// Bot release ClawHub plan script supports release workflow routing.
...
type BotReleaseClawHubPlanArgs = {
...
type BotReleaseClawHubRuntimeStateArgs = {
  repository: string;
...
type BotReleaseClawHubRuntimeState = {
  verifierArgs: string[];
  proofLines: {
    normal: string;
    bootstrap: string;
...
export function buildBotReleaseClawHubRuntimeState(
  args: BotReleaseClawHubRuntimeStateArgs,
): BotReleaseClawHubRuntimeState {
...
export function parseBotReleaseClawHubPlanArgs(
  argv: string[],
): BotReleaseClawHubPlanArgs {
  const values = [...argv];
  if (values[0] === "--") {
    values.shift();
  }
...
export async function buildBotReleaseClawHubPlan(
  args: BotReleaseClawHubPlanArgs,
  options: {
    rootDir?: string;
    fetchImpl?: typeof fetch;
    registryBaseUrl?: string;
  } = {},
): Promise<BotReleaseClawHubPlan> {
