...
export function resolvePackageCandidatePackCommand(sourceDir: string, packDir: string) {
  const packageHelper = join(sourceDir, "scripts", "package-bot-for-docker.mjs");
...
export async function runBundledPluginPostinstall(params: LaneCommandParams) {
  ...
  delete installEnv.BOT_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "bot/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.BOT_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.BOT_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.BOT_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing BOT_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "BOT_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.BOT_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.BOT_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
export async function runInstalledBrowserOverrideImportSmoke(
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      BOT_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      BOT_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
export function installedEntryPath(prefixDir: string) {
  return join(installedPackageRoot(prefixDir), "bot.mjs");
}
