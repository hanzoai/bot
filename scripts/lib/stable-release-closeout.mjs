...
function isCloseoutEvidenceAsset(assetName, tag) {
  ...
  return (
    assetName === `bot-${releaseVersion}-stable-main-closeout.json` ||
    assetName === `bot-${releaseVersion}-stable-main-closeout.json.sha256`
  );
}
...
export function verifyStableMainCloseout(params) {
  ...
  if (mainVersion && !isStableMainVersionAtLeast(mainVersion, version)) {
    errors.push(
      `main package.json version is ${mainVersion}, expected shipped version ${version} or a later stable Bot CalVer.`,
    );
  }
  ...
  const expectedMacAssets = [
    `Bot-${macAssetVersion}.zip`,
    `Bot-${macAssetVersion}.dmg`,
    `Bot-${macAssetVersion}.dSYM.zip`,
  ];
    ...
    const requiredPlatformFamilies = [
      {
        label: "Android",
        prefix: "Bot-Android",
        expected: ["Bot-Android-SHA256SUMS.txt", "Bot-Android.apk"],
      },
      {
        label: "Windows",
        prefix: "BotCompanion-",
        expected: [
          "BotCompanion-SHA256SUMS.txt",
