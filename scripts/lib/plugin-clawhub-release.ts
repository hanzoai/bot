// Plugin Clawhub Release script supports Bot repository automation.
...
const BOT_PLUGIN_CLAWHUB_REPOSITORY = "hanzoai/bot";
const BOT_PLUGIN_CLAWHUB_WORKFLOW_FILENAME = "plugin-clawhub-release.yml";
...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/bot-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.bot?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.bot?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
...
async function hasClawHubTrustedPublisher(
  ...
  try {
    ...
    return isBotPluginTrustedPublisher(trustedPublisherDetail.trustedPublisher);
  } finally {
    request.clearTimeout();
  }
}

function isBotPluginTrustedPublisher(value: unknown): boolean {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  const trustedPublisher = value as ClawHubTrustedPublisherConfig;
  return (
    trustedPublisher.repository === BOT_PLUGIN_CLAWHUB_REPOSITORY &&
    trustedPublisher.workflowFilename === BOT_PLUGIN_CLAWHUB_WORKFLOW_FILENAME &&
