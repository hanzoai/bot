...
export function buildDeprecatedPluginSdkModuleSpecifiers(
  ...
  const unscoped = [
    ...
    ...deprecatedSubpaths.map((subpath) => `bot/plugin-sdk/${subpath}`),
  ];
  // tsconfig aliases the scoped @hanzo/bot-plugin-sdk package to the same
...
export const BANNED_INTERNAL_PLUGIN_SDK_FACADE_MODULES = [
  // Reply facades: canonical seams are bot/plugin-sdk/channel-inbound and
  // bot/plugin-sdk/channel-outbound (defineChannelMessageAdapter family).
  {
    modulePath: "src/plugin-sdk/channel-message",
    canonical: "bot/plugin-sdk/channel-outbound",
  },
  {
  ...
  },
  {
    ...
    canonical: "bot/plugin-sdk/channel-inbound",
  },
  {
