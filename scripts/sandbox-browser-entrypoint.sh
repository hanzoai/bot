...
CDP_PORT="${BOT_BROWSER_CDP_PORT:-9222}"
CDP_SOURCE_RANGE="${BOT_BROWSER_CDP_SOURCE_RANGE:-}"
CDP_AUTH_TOKEN="${BOT_BROWSER_CDP_AUTH_TOKEN:-}"
VNC_PORT="${BOT_BROWSER_VNC_PORT:-5900}"
NOVNC_PORT="${BOT_BROWSER_NOVNC_PORT:-6080}"
ENABLE_NOVNC="${BOT_BROWSER_ENABLE_NOVNC:-1}"
HEADLESS="${BOT_BROWSER_HEADLESS:-0}"
ALLOW_NO_SANDBOX="${BOT_BROWSER_NO_SANDBOX:-0}"
NOVNC_PASSWORD="${BOT_BROWSER_NOVNC_PASSWORD:-}"
...
AUTO_START_TIMEOUT_MS="${BOT_BROWSER_AUTO_START_TIMEOUT_MS:-12000}"
...
if [[ -z "${CDP_AUTH_TOKEN}" ]]; then
  ...
  BOT_BROWSER_CHROME_CDP_PORT="${CHROME_CDP_PORT}" python3 - <<'PY' &
import base64
import hmac
import ipaddress
import os
...
LISTEN_PORT = int(os.environ["BOT_BROWSER_CDP_PORT"])
UPSTREAM_PORT = int(os.environ["BOT_BROWSER_CHROME_CDP_PORT"])
AUTH_TOKEN = os.environ["BOT_BROWSER_CDP_AUTH_TOKEN"]
SOURCE_RANGE = os.environ.get("BOT_BROWSER_CDP_SOURCE_RANGE", "").strip()
...
EXPECTED_BASIC = "Basic " + base64.b64encode(f"bot:{AUTH_TOKEN}".encode()).decode()
    ...
    def handle(self):
        ...
        if not has_auth(header_bytes):
            self.request.sendall(
                ...
                b'WWW-Authenticate: Basic realm="Bot CDP"\r\n'
                b"Connection: close\r\n"
                b"Content-Length: 0\r\n\r\n"
            )
            return
