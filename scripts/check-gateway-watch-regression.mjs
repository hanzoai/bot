...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[bot] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
export function buildTimedWatchCommand(
  ...
  const isolatedStateDir = path.join(isolatedHomeDir, ".bot");
  const isolatedConfigPath = path.join(isolatedStateDir, "bot.json");
  ...
  const shellSource = [
    ...
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$BOT_CONFIG_PATH"`,
  ...
  const env = {
    ...
    BOT_CONFIG_PATH: isolatedConfigPath,
    BOT_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-gateway-watch-"));
