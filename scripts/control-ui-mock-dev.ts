...
async function createChatPickerScenario(
  ...
  return {
    ...
    methodResponses: {
      ...
      "bot.chat.history": custodianHistory,
      "bot.changes.list": custodianChanges,
...
function installControlUiCustodianMock(replyDelayMs: number): void {
  ...
  type MockWindow = Window & {
    botControlUiE2eGateway?: MockGatewayControls;
  };

  const gateway = (window as MockWindow).botControlUiE2eGateway;
  ...
  MockWebSocket.prototype.send = function (data): void {
    ...
    if (frame?.type !== "req" || frame.method !== "bot.chat") {
    ...
    gateway.deferNext("bot.chat");
    sendOriginal(this, data);
    window.setTimeout(
      () => gateway.resolveDeferred("bot.chat", response),
