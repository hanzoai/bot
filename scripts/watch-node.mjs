...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.BOT_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[bot] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[bot] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[bot] This usually means a file in node_modules is empty or truncated.");
  console.error("[bot] Recommended recovery:");
  console.error("[bot]   rm -rf node_modules");
  console.error("[bot]   pnpm store prune");
  console.error("[bot]   pnpm install");
  console.error("");
  console.error("[bot] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `bot doctor --fix --non-interactive` once.",
        deps,
      );
