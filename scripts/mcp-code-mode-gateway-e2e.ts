// Mcp Code Mode Gateway E2E script supports Bot repository automation.
...
async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-mcp-code-mode-"));
  const keep = process.env.BOT_MCP_CODE_MODE_GATEWAY_E2E_KEEP === "1";
  const previousEnv = {
  ...
  try {
    ...
    await provider?.stop();
    resetConfigRuntimeState();
    restoreEnvValue("BOT_STATE_DIR", previousEnv.stateDir);
    restoreEnvValue("BOT_CONFIG_PATH", previousEnv.configPath);
    restoreEnvValue("BOT_TEST_FAST", previousEnv.testFast);
    if (!keep) {
