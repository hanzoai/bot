#!/usr/bin/env node
// Resolves and delegates to the repo-local or PATH botbox binary.
...
} from "./botbox-wrapper-providers.mjs";
import {
...
const REMOTE_CHANGED_GATE_BUNDLE_FILE = ".bot-botbox-changed-gate.bundle";
...
const ignoreRepoBinary = process.env.BOT_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
const repoLocal = ignoreRepoBinary ? null : resolveCrabboxBinary(process.env, process.platform);
const pathLocal = resolvePathBinary("botbox", process.env, process.platform);
const binary =
  repoLocal ??
  pathLocal ??
...
function resolveCrabboxBinary(env, platform) {
  const base = resolve(repoRoot, "../botbox/bin/botbox");
...
function resolveGitCommonCrabboxBinary(env, platform) {
  ...
  const base = resolve(absoluteGitCommonDir, "../..", "botbox/bin/botbox");
  for (const candidate of commandCandidates(base, platform)) {
    if (isExecutableFile(candidate, platform)) {
      return candidate;
    }
...
function configProvider() {
  try {
    const config = readFileSync(resolve(repoRoot, ".botbox.yaml"), "utf8");
...
function botboxOptionArgs(commandArgs) {
...
function commandProvider(commandArgsInput) {
  ...
  commandArgs = botboxOptionArgs(commandArgs);
...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.BOT_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function enforceBrokeredAws(commandArgs, providerName) {
  ...
  console.error(
    [
      "[botbox] provider=aws requires a configured Crabbox broker for Bot proof.",
      "[botbox] run `botbox login --url https://botbox.bot.ai --provider aws`, then retry.",
      "[botbox] for intentional direct AWS provider debugging, set BOT_CRABBOX_ALLOW_DIRECT_AWS=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function botboxConfigDir() {
  if (process.platform === "darwin") {
    return resolve(homedir(), "Library", "Application Support", "botbox");
  }
  if (process.platform === "win32") {
    return resolve(process.env.APPDATA || resolve(homedir(), "AppData", "Roaming"), "botbox");
  }
  return resolve(process.env.XDG_CONFIG_HOME || resolve(homedir(), ".config"), "botbox");
}
...
function blacksmithTestboxPrivateKeyPath(id) {
  return resolve(botboxConfigDir(), "testboxes", id, "id_ed25519");
}
...
function blacksmithTestboxClaimsDir() {
  ...
  const stateDir = configuredStateRoot
    ? resolve(configuredStateRoot, "botbox")
    : resolve(botboxConfigDir(), "state");
...
function enforceCrabboxOwnedBlacksmithLease(commandArgs) {
  ...
  if (!pathExists(keyPath)) {
    console.error(
      [
        `[botbox] provider=blacksmith-testbox --id ${id} has no Crabbox SSH key at ${userDisplayPath(keyPath)}.`,
        "[botbox] create reusable Testboxes through Crabbox before reusing them: node scripts/botbox-wrapper.mjs warmup --provider blacksmith-testbox --idle-timeout 90m",
        "[botbox] direct `blacksmith testbox warmup` leases can be used with `blacksmith testbox run`, but Crabbox cannot sync or run them by id.",
      ].join("\n"),
    );
    process.exit(2);
  }
  ...
  if (claimRepoRoot && claimRepoRoot !== repoRoot && !hasOption(commandArgs, "--reclaim")) {
    console.error(
      `[botbox] lease ${id} is claimed by repo ${claimRepoRoot}; use --reclaim to claim it for ${repoRoot}`,
    );
    process.exit(2);
  }
}
...
function restoreTemporaryBlacksmithTestboxClaim(commandArgs, capturedLeaseId) {
  ...
  if (exactLeaseId) {
    ...
    try {
      ...
      console.error(
        `[botbox] warning: failed to inspect temporary Testbox claims: ${error instanceof Error ? error.message : String(error)}`,
      );
      return;
    }
  } else {
  ...
  for (const claimPath of claimPaths) {
    ...
    try {
      ...
      console.error(
        `[botbox] warning: failed to restore temporary Testbox claim: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
...
function preserveTemporaryCrabboxRuns() {
  ...
  const sourceRuns = resolve(childCwd, ".botbox", "runs");
  if (!pathExists(sourceRuns)) {
    return;
  }

  const targetRuns = resolve(repoRoot, ".botbox", "runs");
  ...
  if (preserved > 0) {
    console.error(
      `[botbox] preserved ${preserved} temporary run artifact ${preserved === 1 ? "directory" : "directories"} under ${relative(repoRoot, targetRuns)}`,
    );
  }
}
...
function remoteGitBootstrapForChangedGate(changedGateBase, changedGateAlias) {
  ...
  return [
    `bot_changed_gate_base=${quotedBase};`,
    `bot_changed_gate_alias=${quotedAlias};`,
    'if ! command -v git >/dev/null 2>&1; then echo "git is required for Bot remote changed-gate sync" >&2; exit 2; fi;',
    `bot_changed_gate_bundle=${quotedBundleFile};`,
    'if [ ! -f "$bot_changed_gate_bundle" ]; then echo "missing changed-gate bundle: $bot_changed_gate_bundle" >&2; exit 2; fi;',
    'bot_changed_gate_bundle_tmp="$(mktemp /tmp/bot-changed-gate.XXXXXX)" || exit 2;',
    "trap 'rm -f \"$bot_changed_gate_bundle_tmp\"' EXIT HUP INT TERM;",
    'cp "$bot_changed_gate_bundle" "$bot_changed_gate_bundle_tmp" || exit 2;',
    'rm -rf -- "$bot_changed_gate_bundle" || exit 2;',
    "rm -rf .git || exit 2;",
    "git init -q || exit 2;",
    "git remote add origin https://github.com/hanzoai/bot.git 2>/dev/null || git remote set-url origin https://github.com/hanzoai/bot.git || exit 2;",
    'git fetch -q --depth=2 origin "$bot_changed_gate_base:refs/remotes/origin/main" || exit 2;',
    'if [ -n "$bot_changed_gate_alias" ]; then git update-ref "$bot_changed_gate_alias" refs/remotes/origin/main || exit 2; fi;',
    'if [ ! -f "$bot_changed_gate_bundle_tmp" ]; then echo "changed-gate bundle disappeared before import" >&2; exit 2; fi;',
    "bot_changed_gate_target=refs/remotes/origin/main;",
    'if [ -s "$bot_changed_gate_bundle_tmp" ]; then git fetch -q "$bot_changed_gate_bundle_tmp" HEAD:refs/heads/bot-changed-gate-tree || exit 2; bot_changed_gate_tree="$(git rev-parse refs/heads/bot-changed-gate-tree^{tree})" || exit 2; bot_changed_gate_head="$(git -c user.name=Bot -c user.email=ci@bot.local commit-tree "$bot_changed_gate_tree" -p refs/remotes/origin/main -m remote-changed-gate-tree)" || exit 2; git update-ref refs/heads/bot-changed-gate-head "$bot_changed_gate_head" || exit 2; bot_changed_gate_target=refs/heads/bot-changed-gate-head; fi;',
    'rm -f "$bot_changed_gate_bundle_tmp" || exit 2;',
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$botModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($botModulesDir) {",
    'if (-not (Test-Path $botModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $botModulesDir" }',
    '$botWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$botSelfModules = Join-Path $botModulesDir "node_modules"',
    'if (-not (Test-Path $botSelfModules)) { cmd /c mklink /J "$botSelfModules" "$botModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $botWorkspaceModules)) { cmd /c mklink /J "$botWorkspaceModules" "$botModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remotePosixJsEnvBootstrap() {
  return [
    ...
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) bot_env_ignore=1; bot_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${bot_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) bot_env_ignore=1; bot_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) bot_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then bot_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) bot_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$bot_env_ignore" = "1" ]; then bot_env_args+=("PATH=${BOT_CRABBOX_BOOTSTRAP_PATH:-$PATH}:${1#PATH=}"); else bot_env_args+=("$1"); fi; bot_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) bot_env_args+=("$1"); shift ;;',
    '--) bot_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$bot_env_ignore" = "1" ] && [ "$bot_env_path_seen" = "0" ]; then bot_env_args+=("PATH=${BOT_CRABBOX_BOOTSTRAP_PATH:-$PATH}"); fi;',
    'command env "${bot_env_args[@]}" "$@";',
    "};",
  ];
}

function remoteAwsMacosJsBootstrap({ packageManager = false, bun = false } = {}) {
  const nodeVersion = process.env.BOT_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "bot_botbox_bootstrap_macos_js() {",
    'tool_root="${BOT_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.bot-botbox-toolchain}";',
    ...
    'macos_locale="${BOT_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.bot-botbox-node-ready";',
  ...
  if (bun) {
    bootstrap.push(
      ...
      'bun_ready_marker="$bun_root/.bot-botbox-bun-ready";',
  ...
  bootstrap.push('export BOT_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "bot_botbox_bootstrap_macos_js");
  return bootstrap.join(" ");
}

function remoteWsl2JsBootstrap({ packageManager = false } = {}) {
  const nodeVersion = process.env.BOT_CRABBOX_WSL2_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "bot_botbox_bootstrap_wsl2_js() {",
    'tool_root="${BOT_CRABBOX_WSL2_TOOLCHAIN_DIR:-$HOME/.bot-botbox-toolchain}";',
    `node_version=${shellQuote(nodeVersion)};`,
    'arch="$(uname -m)";',
  ...
  bootstrap.push('export BOT_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "bot_botbox_bootstrap_wsl2_js");
  return bootstrap.join(" ");
}
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    bun: needsBun,
    shellCommand: `bot_botbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function prepareRemoteWsl2JsBootstrapScript(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "bot-botbox-wsl2-script-"));
...
function remoteAwsMacosSwiftBootstrap() {
  return [
    "bot_botbox_require_macos_swift_62() {",
    'bot_xcode="";',
    'for bot_candidate in /Applications/Xcode_26.1.app /Applications/Xcode_26*.app /Applications/Xcode-26*.app; do if [ -d "$bot_candidate" ]; then bot_xcode="$bot_candidate"; fi; done;',
    'if [ -n "$bot_xcode" ]; then bot_developer="$bot_xcode/Contents/Developer"; if [ ! -d "$bot_developer" ]; then bot_developer="$bot_xcode"; fi; sudo xcode-select -s "$bot_developer" || return 1; fi;',
    'bot_swift_version="$(swift --version 2>&1)" || { status=$?; printf "%s\\n" "$bot_swift_version" >&2; return "$status"; };',
    'printf "%s\\n" "$bot_swift_version" >&2;',
    'bot_swift_major_minor="$(printf "%s\\n" "$bot_swift_version" | sed -nE "s/.*Apple Swift version ([0-9]+)\\.([0-9]+).*/\\1 \\2/p" | head -n 1)";',
    'if [ -z "$bot_swift_major_minor" ]; then echo "[botbox] Bot macOS app proof requires Swift tools 6.2+; unable to parse swift --version." >&2; return 2; fi;',
    "set -- $bot_swift_major_minor;",
    'if [ "$1" -lt 6 ] || { [ "$1" -eq 6 ] && [ "$2" -lt 2 ]; }; then',
    'echo "[botbox] Bot macOS app proof requires Swift tools 6.2+ (Xcode 26.x)." >&2;',
    'echo "[botbox] current Swift is $1.$2; select/install Xcode 26.x or use a Blacksmith macOS runner with Xcode_26.1.app." >&2;',
    "return 2;",
    "fi;",
    'bot_xcodebuild_version="$(xcodebuild -version 2>&1)" || { printf "%s\\n" "$bot_xcodebuild_version" >&2; echo "[botbox] Bot macOS app proof requires Xcode 26.x; active developer directory does not provide usable xcodebuild." >&2; return 2; };',
    'printf "%s\\n" "$bot_xcodebuild_version" >&2;',
    'bot_xcode_major="$(printf "%s\\n" "$bot_xcodebuild_version" | sed -nE "s/^Xcode ([0-9]+)(\\..*)?$/\\1/p" | head -n 1)";',
    'if [ "$bot_xcode_major" != "26" ]; then echo "[botbox] Bot macOS app proof requires Xcode 26.x; current xcodebuild is ${bot_xcode_major:-unknown}." >&2; return 2; fi;',
    "};",
    "bot_botbox_require_macos_swift_62",
  ].join(" ");
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "bot-botbox-macos-script-"));
  const scriptPath = resolve(scriptRoot, "script.sh");
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/bot-botbox-script.XXXXXX")" || exit $?',
    'cleanup_bot_botbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_bot_botbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `BOT_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "bot", "botbox-sync");
  }
  return resolve(tmpdir(), "bot-botbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.BOT_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "BOT_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set BOT_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower BOT_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "bot-botbox-sync-"));
  let active = false;
  ...
  function create() {
    ...
    if (options.changedGateBase) {
      ...
      let bundleTempDir;
      try {
        bundleTempDir = mkdtempSync(resolve(syncRoot, "bot-botbox-bundle-"));
  ...
  return {
    ...
    restoreIfMissing() {
      ...
      console.error(`[botbox] temporary full checkout disappeared; recreating ${dir}`);
      if (active) {
        const remove = gitOutput(["worktree", "remove", "--force", dir]);
        if (remove.status !== 0) {
          console.error(`[botbox] warning: git worktree remove failed for ${dir}: ${remove.text}`);
        }
        active = false;
      }
      rmSync(dir, { recursive: true, force: true });
...
function startFullCheckoutKeepalive(checkout, options = {}) {
  ...
  const refresh = () => {
    try {
      if (!checkout.exists()) {
        if (options.onMissing) {
          if (!missingReported) {
            ...
            console.error(
              `[botbox] temporary full checkout disappeared while Crabbox was running; terminating because the child cwd cannot be repaired: ${checkout.dir}`,
            );
            options.onMissing();
          }
          return;
      ...
      console.error(
        `[botbox] warning: failed to refresh temporary full checkout ${checkout.dir}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  };
...
function assertFullCheckoutAvailableBeforeExit(dir) {
  ...
  console.error(
    `[botbox] temporary full checkout vanished before Crabbox finished syncing: ${dir}`,
  );
  return false;
}
...
const displayBinary = binary === "botbox" ? "botbox" : relative(repoRoot, binary);
...
console.error(
  `[botbox] bin=${displayBinary} version=${version.text || "unknown"} provider=${provider || "unknown"} providers=${providers.join(",") || "unknown"}`,
);

if (version.status !== 0 || help.status !== 0) {
  console.error("[botbox] selected binary failed basic --version/--help sanity checks");
  process.exit(2);
}
...
if (provider && !isProviderAdvertised(provider, providers)) {
  if (providers.length === 0) {
    console.error(
      "[botbox] could not parse provider list from --help; refusing to run with --provider without validation",
    );
    process.exit(2);
  }
  console.error(
    `[botbox] selected binary does not advertise provider ${provider}; update Crabbox or choose a supported provider`,
  );
  process.exit(2);
}
...
if (canonicalProvider === "blacksmith-testbox") {
  if (isWindowsRemoteTarget(normalizedArgs)) {
    console.error(
      [
        "[botbox] provider=blacksmith-testbox supports Linux Testbox proof only; it cannot run Windows or WSL2 targets.",
        "[botbox] use provider=azure or provider=aws for brokered Crabbox Windows/WSL2 proof, provider=parallels for local Windows, or dispatch .github/workflows/windows-testbox-probe.yml for Blacksmith Windows runner probes.",
      ].join("\n"),
    );
    process.exit(2);
  }
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        `[botbox] provider=blacksmith-testbox requires Crabbox >= ${formatVersionTuple(minimumBlacksmithCrabboxVersion)} for current Testbox sync, queue, and cleanup behavior.`,
        `[botbox] selected binary reported version=${version.text || "unknown"}.`,
        "[botbox] if using ../botbox, rebuild it: version=$(git -C ../botbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../botbox -trimpath -ldflags \"-s -w -X github.com/bot/botbox/internal/cli.version=${version}\" -o bin/botbox ./cmd/botbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (canonicalProvider === "blacksmith-testbox") {
  ...
  const fallback = commandProviderValue
    ? "rerun without --provider to use .botbox.yaml"
    : envProviderLocal
      ? "unset CRABBOX_PROVIDER to use .botbox.yaml"
      : "pass another --provider to override it";
  console.error(
    `[botbox] provider=blacksmith-testbox ${source}; if Testbox is queued or down, ${fallback}`,
  );
  console.error(
    "[botbox] delegated Testbox proof uses the wrapper exitCode and timing JSON; the linked Actions run can show cancelled during external lease cleanup",
  );
...
try {
  ...
  console.error(`[botbox] ${error instanceof Error ? error.message : String(error)}`);
  process.exit(2);
}
...
try {
  if (shouldUseFullCheckoutForCleanRemoteSync(normalizedArgs, provider)) {
    ...
    console.error(
      `[botbox] sparse clean checkout detected; syncing from temporary full checkout ${checkout.dir}`,
    );
    if (checkout.changedGateBase) {
      console.error(
        `[botbox] remote changed gate detected; overlaying local HEAD as worktree changes from ${checkout.changedGateBase}`,
      );
    }
  }
} catch (error) {
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    console.error(
      `[botbox] provider=aws macOS raw boxes may lack Node/Corepack/pnpm/Bun for ${runtimeEntrypoint || "--script-stdin"}; bootstrapping pinned user-local JavaScript tooling before the command`,
    );
  } else {
    const id = optionValue(normalizedArgs, "--id");
    const hydrate = id
      ? `pnpm botbox:hydrate -- --id ${id}`
      : "pnpm botbox:warmup, then pnpm botbox:hydrate -- --id <id>";
    console.error(
      `[botbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm/Bun for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for Bot CI-like proof; not switching providers automatically`,
    );
  }
}
if (normalizedArgs[0] === "run" && isBrokeredWsl2RemoteTarget(normalizedArgs, provider)) {
  ...
  if (wsl2Requirements.runtimeEntrypoint || wsl2Requirements.packageManager) {
    console.error(
      `[botbox] provider=${provider} WSL2 raw boxes may lack Node/Corepack/pnpm for ${wsl2Requirements.runtimeEntrypoint || "package-manager"}; using no-hydrate pinned user-local JavaScript tooling before the command`,
    );
  }
}
...
if (
  ...
  console.error(
    "[botbox] provider=docker enabling host Docker socket pass-through for Bot Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/bot-botbox-docker-work";
  console.error(
...
child.on("exit", (code, signal) => {
  ...
  if (!signal && code === 0) {
    try {
      ...
      console.error(
        `[botbox] failed to record Testbox lease freshness: ${error instanceof Error ? error.message : String(error)}`,
      );
      exitCode = 2;
    }
  }
  ...
  if (
    ...
    console.error(
      `[botbox] run --id ${reusedRunLeaseId} failed fast; reusable leases expire after their idle timeout and rejected flags also exit immediately. Check the first error line above, verify the lease with \`node scripts/botbox-wrapper.mjs list\`, or warm a fresh one with \`node scripts/botbox-wrapper.mjs warmup\`.`,
    );
  }
...
child.on("error", (error) => {
  ...
  cleanupOnce();
  console.error(`[botbox] failed to execute ${displayBinary}: ${error.message}`);
  process.exit(2);
});
...
function resolveChildKillGraceMs(env) {
  if (!env.VITEST || !env.BOT_TEST_CRABBOX_CHILD_KILL_GRACE_MS) {
    return 5_000;
  }
  const value = Number.parseInt(env.BOT_TEST_CRABBOX_CHILD_KILL_GRACE_MS, 10);
  return Number.isFinite(value) && value >= 0 ? value : 5_000;
}

function resolveMetadataProbeTimeoutMs(env) {
  if (!env.VITEST || !env.BOT_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS) {
    return CRABBOX_METADATA_PROBE_TIMEOUT_MS;
  }
  const value = Number.parseInt(env.BOT_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS, 10);
