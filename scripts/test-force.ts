...
// Test Force script supports Bot repository automation.
...
function usage(): string {
  return [
    ...
    "",
    "Clears the configured Bot gateway port, then runs the local test suite.",
    "",
    "Options:",
...
function runTests() {
  const isolatedLock =
    process.env.BOT_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `bot-gateway.lock.test.${Date.now()}`);
