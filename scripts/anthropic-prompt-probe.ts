// Anthropic Prompt Probe script supports Bot repository automation.
import { spawn } from "node:child_process";
// Live prompt probe for Anthropic setup-token and Claude CLI prompt-path debugging.
// Usage:
// BOT_PROMPT_TRANSPORT=direct|gateway
// BOT_PROMPT_MODE=extra
// BOT_PROMPT_TEXT='...'
// BOT_PROMPT_CAPTURE=1
...
const TRANSPORT = process.env.BOT_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE = "extra";
const PROMPT_TEXT = process.env.BOT_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.BOT_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.BOT_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = parseBooleanEnv({
  fallback: false,
  name: "BOT_PROMPT_CAPTURE",
  raw: process.env.BOT_PROMPT_CAPTURE,
});
const INCLUDE_RAW = parseBooleanEnv({
  fallback: false,
  name: "BOT_PROMPT_INCLUDE_RAW",
  raw: process.env.BOT_PROMPT_INCLUDE_RAW,
});
const KEEP_TMP = parseBooleanEnv({
  fallback: false,
  name: "BOT_PROMPT_KEEP_TMP",
  raw: process.env.BOT_PROMPT_KEEP_TMP,
});
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.BOT_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 45_000,
  label: "BOT_PROMPT_TIMEOUT_MS",
  min: 1,
  raw: process.env.BOT_PROMPT_TIMEOUT_MS,
});
const GATEWAY_TIMEOUT_MS = parseStrictIntegerOption({
  fallback: 120_000,
  label: "BOT_PROMPT_GATEWAY_TIMEOUT_MS",
  min: 1,
  raw: process.env.BOT_PROMPT_GATEWAY_TIMEOUT_MS,
});
...
const CAPTURE_PROXY_MAX_BODY_BYTES = parseStrictIntegerOption({
  ...
  label: "BOT_PROMPT_CAPTURE_MAX_BODY_BYTES",
  min: 1,
  raw: process.env.BOT_PROMPT_CAPTURE_MAX_BODY_BYTES,
});
...
async function resolveSetupTokenSource(): Promise<TokenSource> {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set BOT_LIVE_SETUP_TOKEN_VALUE or BOT_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["bot.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-gateway-prompt-probe-"));
...
async function main() {
  if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
    throw new Error("missing BOT_PROMPT_TEXT or BOT_PROMPT_LIST_JSON");
  }
