...
const DEFAULT_OUTPUT_NAME = "bot-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "BOT_TRUSTED_PACKAGE_TOKEN";
...
export const BOT_PACKAGE_SPEC_RE =
  /^bot@(alpha|beta|extended-stable|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-bot-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedBotTarballFilename(value) {
  ...
  if (
    ...
    throw new Error(
      `npm pack reported unsafe Bot tarball filename: ${JSON.stringify(filename)}`,
    );
  }
  return filename;
}

export function validateBotPackageSpec(spec) {
  if (!BOT_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be bot@alpha, bot@beta, bot@extended-stable, bot@latest, or an exact Bot release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateBotPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an Bot branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `bot-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (packedFilename) {
      filename = resolvePackedBotTarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        filename = resolvePackedBotTarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => {
        try {
          return resolvePackedBotTarballFilename(entry) === entry;
        } catch {
          return false;
        }
      })
  ...
  if (!filename) {
    throw new Error(`npm pack produced no Bot tarball in ${outputDir}`);
  }
...
export const cleanPackedBotTarballsForTest = cleanPackedBotTarballs;
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking Bot package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-bot-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
  ...
  if (pkg.name !== "bot") {
    throw new Error(`package candidate must be named "bot"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
