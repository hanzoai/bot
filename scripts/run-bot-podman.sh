...
#   bot --container bot dashboard --no-open
#   bot --container bot channels login
#
...
run_podman_detached() {
  bot_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      BOT_GATEWAY_TOKEN|BOT_PODMAN_CONTAINER|BOT_PODMAN_IMAGE|BOT_IMAGE|BOT_PODMAN_PULL|BOT_PODMAN_RUN_TIMEOUT|BOT_PODMAN_GATEWAY_HOST_PORT|BOT_GATEWAY_PORT|BOT_PODMAN_BRIDGE_HOST_PORT|BOT_BRIDGE_PORT|BOT_GATEWAY_BIND|BOT_PODMAN_USERNS|BOT_BIND_MOUNT_OPTIONS|BOT_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-bot-podman.sh as your normal user so Podman stays rootless."
fi
...
PODMAN_RUN_TIMEOUT="${BOT_PODMAN_RUN_TIMEOUT:-600s}"
...
PUBLISH_HOST="${BOT_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$BOT_IMAGE"
...
resolve_config_gateway_bind() {
  ...
  BOT_CONTAINER="" BOT_CONFIG_DIR="$config_dir" \
    bot config get gateway.bind 2>/dev/null || true
}
...
# BOT_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${BOT_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v bot >/dev/null 2>&1; then
    echo "Warning: bot not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    BOT_CONTAINER="" BOT_CONFIG_DIR="$config_dir" \
      bot config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    BOT_CONTAINER="" BOT_CONFIG_DIR="$config_dir" \
      bot config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  BOT_CONTAINER="" BOT_CONFIG_DIR="$config_dir" \
    bot config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$BOT_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -v "$CONFIG_DIR:/home/node/.bot:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.bot/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$BOT_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME bot dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME bot devices approve --latest  # if pairing required"
