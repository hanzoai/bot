#!/usr/bin/env node
// Development runner that rebuilds Bot, runs runtime postbuild steps, and
...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "bot.plugin.json"));
    }
...
const listRequiredBotExtensionAliasOutputs = (deps) => {
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  ...
  const aliasDir = path.join(distRoot, "extensions", "node_modules", "bot");
  return [
...
const listRequiredStaticExtensionAssetOutputs = (deps) => {
  if (deps.env.BOT_RUNTIME_POSTBUILD_STATIC_ASSETS === "0") {
    return [];
  }
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  const runtimeRoot = resolveRuntimePostBuildRuntimeRoot(deps);
...
export const listRequiredRuntimePostBuildOutputs = (deps) => {
  ...
  return [
    ...
    ...listRequiredBotExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps) => {
  ...
  if (
    deps.env.BOT_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.BOT_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by BOT_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by BOT_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "BOT_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "BOT_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "BOT_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "BOT_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "BOT_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "BOT_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "BOT_RUN_NODE_BUILD_LOCK_STALE_MS";
const RUN_NODE_SKIP_DTS_BUILD_ENV = "BOT_RUN_NODE_SKIP_DTS_BUILD";
...
const shouldUseRunNodeProgress = (deps) =>
  ...
  deps.env.BOT_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[bot] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `bot-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.BOT_TRACE_SYNC_IO === "1") {
...
const runBot = async (deps) => {
  ...
  const useProcessGroup = shouldUseRunNodeChildProcessGroup(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "bot.mjs", ...deps.args], {
    cwd: deps.cwd,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[bot] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const canUseStampedGatewayClientDist = (deps) => {
  ...
  if (
    ...
    deps.env.BOT_FORCE_RUNTIME_POSTBUILD === "1"
  ) {
    return false;
  }
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.BOT_BUILD_PRIVATE_QA = "1";
    deps.env.BOT_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.BOT_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
