#!/usr/bin/env bash
# Reset Bot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.
...
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Bot"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Bot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Bot"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.bot.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/bot-restart-${LOCK_KEY}"
...
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.bot/disable-launchagent"
ATTACH_ONLY=1
BACKGROUND_ONLY=0
TARGET_ONLY=0
TARGET_APP_BUNDLE="${ROOT_DIR}/dist/Bot.app"
TARGET_EXECUTABLE="${TARGET_APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}"
INSTALLED_EXECUTABLE="/Applications/Bot.app/${APP_EXECUTABLE_RELATIVE_PATH}"
STAGED_APP_DIR="${ROOT_DIR}/dist/.bot-replacement-${LOCK_KEY}-$$"
STAGED_APP_BUNDLE="${STAGED_APP_DIR}/Bot.app"
...
canonicalize_app_bundle() {
  ...
  if [[ ! -d "${APP_BUNDLE}" ]]; then
    fail "BOT_APP_BUNDLE does not exist: ${APP_BUNDLE}"
  fi
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  --target-only    Restart only this checkout's dist app; fail if another Bot app is active"
      log ""
      log "Env:"
      log "  BOT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node bot.mjs daemon install --force --runtime node"
...
if [[ "$TARGET_ONLY" -eq 1 && -n "$APP_BUNDLE" ]]; then
  fail "--target-only does not accept BOT_APP_BUNDLE"
fi
canonicalize_app_bundle
...
bot_process_pids() {
  ...
  done < <(known_bot_executables) | sort -u
}
...
foreign_bot_process_pids() {
  ps axww -o pid=,command= 2>/dev/null \
    | while read -r pid command_line; do
        ...
        if [[ "${executable}" == *"/Bot.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
          || "${executable}" == *"/apps/macos/.build/debug/Bot" \
          || "${executable}" == *"/apps/macos/.build-local/debug/Bot" \
          || "${executable}" == *"/apps/macos/.build/release/Bot" ]]; then
          printf '%s\n' "${pid}"
        fi
      done
}
...
print_managed_bot_supervisor_label() {
  local domain="$1"
  local label="$2"
  local job=""
...
managed_bot_supervisor_labels() {
  ...
  while IFS='|' read -r domain label; do
    ...
    print_managed_bot_supervisor_label "${domain}" "${label}" &
...
kill_managed_bot() {
  ...
  remaining_pids="$(managed_bot_process_pids)"
  while IFS= read -r pid; do
    [[ -n "${pid}" ]] || continue
    kill -KILL "${pid}" 2>/dev/null || true
  done <<< "${remaining_pids}"
  sleep 0.3
  [[ -z "$(managed_bot_process_pids)" ]]
}

stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.bot.mac 2>/dev/null || true
}
...
if [[ "$TARGET_ONLY" -eq 1 ]]; then
  if ! managed_supervisors="$(managed_bot_supervisor_labels | sort -u | /usr/bin/paste -sd, -)"; then
    fail "Unable to inspect loaded launchd jobs before target-only restart"
  fi
  if [[ -n "${managed_supervisors}" ]]; then
    fail "Managed Bot app is supervised by launchd job(s): ${managed_supervisors}; stop those jobs before a target-only restart"
  fi
  if [[ -n "$(foreign_bot_process_pids)" ]]; then
    fail "Another Bot app or test process is active; target-only restart deferred"
  fi
  log "==> Keeping managed Bot running while the replacement builds"
else
  stop_launch_agent
  log "==> Killing existing Bot instances"
  if ! kill_all_bot; then
    fail "Bot instances did not exit after cleanup attempts"
  fi
fi
...
choose_app_bundle() {
  ...
  if [[ -d "${ROOT_DIR}/dist/Bot.app" ]]; then
    APP_BUNDLE="$(cd "${ROOT_DIR}/dist/Bot.app" && pwd -P)"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Bot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  if [[ -d "/Applications/Bot.app" ]]; then
    APP_BUNDLE="$(cd "/Applications/Bot.app" && pwd -P)"
    return 0
  fi

  fail "App bundle not found. Set BOT_APP_BUNDLE to your installed Bot.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -c "cd '${ROOT_DIR}' && node bot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -c "cd '${ROOT_DIR}' && node bot.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".bot", "bot.json"), "utf8");
...
if [[ "$TARGET_ONLY" -eq 1 ]]; then
  if [[ -n "$(foreign_bot_process_pids)" ]]; then
    fail "Another Bot app or test process appeared during build; target-only restart deferred"
  fi
  log "==> Switching managed installed and exact target Bot instances"
  if ! kill_managed_bot; then
    fail "Managed Bot instances did not exit after cleanup attempts"
  fi
fi
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -c "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.bot.gateway.plist' | head -n 40 || true"
fi
