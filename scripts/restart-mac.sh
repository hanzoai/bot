...
WAIT_FOR_LOCK=0
LOG_PATH="${BOT_RESTART_LOG:-/tmp/bot-restart.log}"
NO_SIGN=0
SIGN=0
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node bot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node bot.mjs daemon restart"
