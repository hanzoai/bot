...
function usage() {
  return "Usage: node scripts/check-bot-package-tarball.mjs [--require-bundled-workspace-deps] <bot.tgz>";
}
...
function parseArgs(argv) {
  ...
  for (const rawArg of args) {
    ...
    if (arg.startsWith("-")) {
      throw new Error(`Unknown Bot package tarball check option: ${arg}`);
    }
    if (tarball) {
      throw new Error(`Unexpected Bot package tarball check argument: ${arg}`);
    }
    tarball = arg;
  }
  if (!tarball) {
...
if (!fs.existsSync(tarball)) {
  fail(`Bot package tarball does not exist: ${tarball}`);
}
...
const REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES = ["@hanzo/bot-ai"];
...
const REQUIRED_BUNDLED_WORKSPACE_RUNTIME_ENTRIES = new Map([
  [
    "@hanzo/bot-ai",
    [
      { specifier: "@hanzo/bot-ai", entry: "dist/index.mjs" },
      { specifier: "@hanzo/bot-ai/providers", entry: "dist/providers.mjs" },
      {
        specifier: "@hanzo/bot-ai/transports",
        entry: "dist/transports.mjs",
        whenExported: "./transports",
      },
      {
        specifier: "@hanzo/bot-ai/internal/runtime",
...
function collectRequiredBundledWorkspaceDependencyErrors(
  ...
  for (const name of REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES) {
    ...
    if (!bundledDependencies.has(name)) {
      errors.push(
        `package.json dependencies.${name} must be listed in bundleDependencies because it is private to the Bot workspace`,
      );
    }
...
const phaseTimingsEnabled = process.env.BOT_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
...
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-bot-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-package-tarball-"));
try {
...
const PACKAGE_INSTALL_GUARD_RELATIVE_PATH = "dist/bot-install-guard";
...
if (shouldValidateShrinkwrap) {
  try {
    ...
    if (shrinkwrap.name !== "bot") {
      errors.push("npm-shrinkwrap.json root name must be bot");
    }
    ...
    if (!rootPackage || rootPackage.name !== "bot") {
      errors.push("npm-shrinkwrap.json packages root must name bot");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`Bot package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`Bot package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("Bot package tarball integrity passed.");
