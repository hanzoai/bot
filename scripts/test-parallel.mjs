...
const useVmForks =
  ...
  (process.env.BOT_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks);
...
// for lower wall-clock time. CI can opt in via BOT_TEST_PARALLEL_GATEWAY=1.
const keepGatewaySerial =
  isWindowsCi ||
  process.env.BOT_TEST_SERIAL_GATEWAY === "1" ||
  testProfile === "serial" ||
  (isCI && process.env.BOT_TEST_PARALLEL_GATEWAY !== "1");
...
function resolveReportDir() {
  const raw = process.env.BOT_VITEST_REPORT_DIR?.trim();
  if (!raw) {
    return null;
  }
  try {
