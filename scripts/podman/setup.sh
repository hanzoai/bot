#!/usr/bin/env bash
# One-time host setup for rootless Bot in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.bot by default, and uses the repo-local
# launch script at ./scripts/run-bot-podman.sh.
#
...
BOT_IMAGE="${BOT_PODMAN_IMAGE:-${BOT_IMAGE:-bot:local}}"
BOT_CONTAINER_NAME="${BOT_PODMAN_CONTAINER:-bot}"
...
PODMAN_PULL_TIMEOUT="${BOT_PODMAN_SETUP_PULL_TIMEOUT:-600s}"
...
run_podman_build() {
  bot_host_timeout_cmd "$PODMAN_BUILD_TIMEOUT" podman build "$@"
}
...
if [[ "$BOT_IMAGE" == "bot:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
...
if [[ -z "$BOT_HOME" ]]; then
  echo "Unable to resolve HOME for user $BOT_USER." >&2
  exit 1
fi
if [[ -z "$BOT_CONFIG_DIR" ]]; then
  BOT_CONFIG_DIR="$BOT_HOME/.bot"
fi
if [[ -z "$BOT_WORKSPACE_DIR" ]]; then
  BOT_WORKSPACE_DIR="$BOT_CONFIG_DIR/workspace"
fi
validate_absolute_path "home directory" "$BOT_HOME"
validate_mount_source_path "config directory" "$BOT_CONFIG_DIR"
validate_mount_source_path "workspace directory" "$BOT_WORKSPACE_DIR"
validate_container_name "$BOT_CONTAINER_NAME"
validate_image_name "$BOT_IMAGE"
validate_port "gateway host port" "$HOST_GATEWAY_PORT"
validate_port "seed gateway port" "$SEED_GATEWAY_PORT"

install -d -m 700 "$BOT_CONFIG_DIR" "$BOT_WORKSPACE_DIR"
ensure_private_existing_dir_owned_by_user "config directory" "$BOT_CONFIG_DIR"
ensure_private_existing_dir_owned_by_user "workspace directory" "$BOT_WORKSPACE_DIR"

BOT_IMAGE_APT_PACKAGES="${BOT_IMAGE_APT_PACKAGES-${BOT_DOCKER_APT_PACKAGES:-}}"
BOT_IMAGE_PIP_PACKAGES="${BOT_IMAGE_PIP_PACKAGES:-}"
BUILD_ARGS=()
BUILD_GIT_COMMIT="$(bot_resolve_git_commit "$REPO_PATH")"
BUILD_TIMESTAMP="$(bot_resolve_build_timestamp)"
BUILD_ARGS+=(--build-arg "BOT_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}")
if [[ "$BUILD_GIT_COMMIT" =~ ^[0-9a-fA-F]{40}$ ]]; then
  BUILD_ARGS+=(--build-arg "GIT_COMMIT=${BUILD_GIT_COMMIT}")
fi
if [[ -n "$BOT_IMAGE_APT_PACKAGES" ]]; then
  BUILD_ARGS+=(--build-arg "BOT_IMAGE_APT_PACKAGES=${BOT_IMAGE_APT_PACKAGES}")
fi
if [[ -n "$BOT_IMAGE_PIP_PACKAGES" ]]; then
  BUILD_ARGS+=(--build-arg "BOT_IMAGE_PIP_PACKAGES=${BOT_IMAGE_PIP_PACKAGES}")
fi
if [[ -n "${BOT_EXTENSIONS:-}" ]]; then
  BUILD_ARGS+=(--build-arg "BOT_EXTENSIONS=${BOT_EXTENSIONS}")
fi
if [[ -n "${BOT_INSTALL_BROWSER:-}" ]]; then
  BUILD_ARGS+=(--build-arg "BOT_INSTALL_BROWSER=${BOT_INSTALL_BROWSER}")
fi

if [[ "$BOT_IMAGE" == "bot:local" ]]; then
  echo "Building image $BOT_IMAGE ..."
  run_podman_build -t "$BOT_IMAGE" -f "$REPO_PATH/Dockerfile" "${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"}" "$REPO_PATH"
else
  if podman image exists "$BOT_IMAGE" >/dev/null 2>&1; then
    echo "Using existing image $BOT_IMAGE"
  else
    echo "Pulling image $BOT_IMAGE ..."
    run_podman_pull "$BOT_IMAGE"
  fi
fi
...
EOF
  ...
  echo "Generated BOT_GATEWAY_TOKEN and wrote it to $ENV_FILE"
fi
upsert_env_var "$ENV_FILE" "BOT_PODMAN_CONTAINER" "$BOT_CONTAINER_NAME"
upsert_env_var "$ENV_FILE" "BOT_PODMAN_IMAGE" "$BOT_IMAGE"

CONFIG_JSON="$BOT_CONFIG_DIR/bot.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  ...
  QUADLET_DST="$QUADLET_DIR/bot.container"
  echo "Installing Quadlet to $QUADLET_DST ..."
  mkdir -p "$QUADLET_DIR"
  ensure_safe_existing_dir "quadlet directory" "$QUADLET_DIR"
  BOT_HOME_ESCAPED="$(escape_sed_replacement_pipe_delim "$BOT_HOME")"
  BOT_CONFIG_ESCAPED="$(escape_sed_replacement_pipe_delim "$BOT_CONFIG_DIR")"
  BOT_WORKSPACE_ESCAPED="$(escape_sed_replacement_pipe_delim "$BOT_WORKSPACE_DIR")"
  BOT_IMAGE_ESCAPED="$(escape_sed_replacement_pipe_delim "$BOT_IMAGE")"
  BOT_CONTAINER_ESCAPED="$(escape_sed_replacement_pipe_delim "$BOT_CONTAINER_NAME")"
  sed \
    -e "s|{{BOT_HOME}}|$BOT_HOME_ESCAPED|g" \
    -e "s|{{BOT_CONFIG_DIR}}|$BOT_CONFIG_ESCAPED|g" \
    -e "s|{{BOT_WORKSPACE_DIR}}|$BOT_WORKSPACE_ESCAPED|g" \
    -e "s|{{IMAGE_NAME}}|$BOT_IMAGE_ESCAPED|g" \
    -e "s|{{CONTAINER_NAME}}|$BOT_CONTAINER_ESCAPED|g" \
    "$QUADLET_TEMPLATE" | write_file_atomically "$QUADLET_DST" 644

  if command -v systemctl >/dev/null 2>&1; then
    echo "Reloading and starting user service..."
    if systemctl --user daemon-reload && systemctl --user start bot.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start bot.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-bot-podman.sh launch"
echo "  ./scripts/run-bot-podman.sh launch setup"
echo "  bot --container $BOT_CONTAINER_NAME dashboard --no-open"
