...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port bot-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    echo "  cd ~/bot && ./scripts/docker/setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d bot-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart bot-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f bot-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.bot"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show bot.json
  if [[ -f "${config_dir}/bot.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/bot.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/bot.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No bot.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec bot-gateway \
...
clawdock-update() {
  ...
  _clawdock_compose build bot-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d bot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build bot-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "BOT_GATEWAY_TOKEN=$token" bot-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" bot-gateway \
    bash -c './bot.mjs config set gateway.remote.token "$TOKEN" && ./bot.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec bot-gateway \
    bash -c "./bot.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart bot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps bot-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec bot-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
...
clawdock-help() {
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to bot project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.bot)${_CLR_RESET}"
