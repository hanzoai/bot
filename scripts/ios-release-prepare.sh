...
- regenerates apps/ios/Bot.xcodeproj via xcodegen
EOF
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical Bot Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi

if [[ -n "${BOT_PUSH_RELAY_BASE_URL:-}" || -n "${IOS_PUSH_RELAY_BASE_URL:-}" ]]; then
...
RELEASE_GIT_COMMIT="$(BOT_REQUIRE_BUILD_METADATA=1 bot_resolve_git_commit "${ROOT_DIR}")"
...
BOT_APP_BUNDLE_ID = ai.botfoundation.app
BOT_SHARE_BUNDLE_ID = ai.botfoundation.app.share
BOT_ACTIVITY_WIDGET_BUNDLE_ID = ai.botfoundation.app.activitywidget
BOT_WATCH_APP_BUNDLE_ID = ai.botfoundation.app.watchkitapp
BOT_CODE_SIGN_ENTITLEMENTS = Sources/BotAppAttest.entitlements
BOT_APNS_ENTITLEMENT_ENVIRONMENT = production
BOT_APP_ATTEST_ENVIRONMENT = production
BOT_PUSH_MODE = appStore
BOT_PUSH_RELAY_BASE_URL =
EOF
