...
// Bot Npm Postpublish Verify script supports Bot repository automation.
...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./bot-npm-release-check.ts";
...
const PACKAGED_BUNDLED_PLUGIN_ARTIFACTS = new Set(
  listBundledPluginPackArtifacts({
    env: { ...process.env, BOT_BUNDLED_PLUGIN_BUILD_IDS: undefined },
  }),
);
...
type BotNpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function botNpmPostpublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/bot-npm-postpublish-verify.ts <version>";
}

export function parseBotNpmPostpublishVerifyArgs(
  argv: readonly string[],
): BotNpmPostpublishVerifyArgs {
  ...
  if (!version) {
    throw new Error(botNpmPostpublishVerifyUsage());
  }
  if (version.startsWith("-")) {
    throw new Error(`Unknown bot npm postpublish verifier option: ${version}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected bot npm postpublish verifier argument: ${extraArg}`);
  }
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`bot@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
const NPM_PROVENANCE_REPOSITORY = "https://github.com/hanzoai/bot";
const NPM_PROVENANCE_WORKFLOW_PATH = ".github/workflows/bot-npm-release.yml";
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  if (
    ...
    throw new Error(
      `npm provenance attestation does not bind ${version} to the trusted Bot GitHub release workflow.`,
    );
  }
...
function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@hanzo/bot-llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "bot.cmd")
    : pathPosix.join(prefixDir, "bin", "bot");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  await verifyNpmProvenanceAttestation({
    ...
    integrity,
    attestations,
    expectedWorkflowRef: process.env.BOT_NPM_EXPECTED_WORKFLOW_REF,
    expectedWorkflowSha: process.env.BOT_NPM_EXPECTED_WORKFLOW_SHA,
  });
  console.log(
    `bot-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `bot-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "bot");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed bot binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`bot-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  const args = parseBotNpmPostpublishVerifyArgs(argv);
  if (args.help) {
    console.log(botNpmPostpublishVerifyUsage());
    return;
  }
  ...
  console.log(
    `bot-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`bot-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
