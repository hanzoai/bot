...
# Output:
# - dist/Bot.app
# - dist/Bot-<version>.zip
# - dist/Bot-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.bot.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/bot-sparkle-build.XXXXXX")" || {
...
ZIP="$ROOT_DIR/dist/Bot-$VERSION.zip"
DMG="$ROOT_DIR/dist/Bot-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/Bot-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/Bot-$VERSION.dSYM.zip"
