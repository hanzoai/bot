...
export function resolveCompileConcurrency(
  ...
  const raw = env.BOT_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "BOT_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.BOT_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
