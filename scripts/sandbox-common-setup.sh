...
BASE_IMAGE="${BASE_IMAGE:-bot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-bot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=bot.sandbox=1)
NOTE
