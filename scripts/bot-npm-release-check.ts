...
// Bot Npm Release Check script supports Bot repository automation.
...
const skipPackValidationEnv = "BOT_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return readPositiveEnvInt(
    "BOT_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "bot") {
    errors.push(`package.json name must be "bot"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.bot !== "bot.mjs") {
    errors.push(
      `package.json bin.bot must be "bot.mjs"; found "${pkg.bin?.bot ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`bot-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `bot-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
