...
const MANUAL_SECTION_START = "<!-- bot-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- bot-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving Bot messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to Bot.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an Bot plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.bot?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    const release = packageJson.bot?.release;
    if (release?.publishToClawHub === true || release?.publishToNpm === true) {
      return `included in Bot; ${resolveInstallRoute(packageJson, "external")}`;
    }
    return "included in Bot";
  }
  const install = packageJson.bot?.install;
  const release = packageJson.bot?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.bot?.install?.clawhubSpec === "string" ||
    typeof packageJson.bot?.install?.npmSpec === "string";
...
function renderReferenceIndex(records) {
  ...
  return `---
summary: "Generated index of Bot plugin reference pages"
read_when:
  - You need a reference page for a specific Bot plugin
...
\`bot.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "bot.plugin.json");
