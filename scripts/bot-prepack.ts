...
// Bot Prepack script supports Bot repository automation.
...
const ALLOW_UNRELEASED_CHANGELOG_ENV = "BOT_PREPACK_ALLOW_UNRELEASED_CHANGELOG";
const PREPARED_RELEASE_ENV = "BOT_PREPACK_PREPARED";
const OCM_INTERNAL_NPM_BIN_ENV = "OCM_INTERNAL_NPM_BIN";
const OCM_WORKSPACE_DIRS_ENV = "BOT_OCM_WORKSPACE_DEPENDENCY_DIRS";
...
const SELF_CONTAINED_SOURCE_PACK_COMMAND =
  "node scripts/package-bot-for-docker.mjs --allow-unreleased-changelog";
...
export function collectSourcePackWorkspaceDependencyErrors(
  ...
  const aiDependency = packageJson.dependencies?.["@hanzo/bot-ai"];
  if (typeof aiDependency !== "string" || !aiDependency.trim().startsWith("workspace:")) {
    return [];
  }
  if (ocmExternalizesWorkspacePackage("@hanzo/bot-ai", env)) {
    return [];
  }
  return [
    `plain root packing cannot safely resolve @hanzo/bot-ai from ${aiDependency}: pnpm rewrites the workspace dependency to an exact version without bundling the package`,
    `use \`${SELF_CONTAINED_SOURCE_PACK_COMMAND}\` for a self-contained source package; official npm release automation prepares and publishes @hanzo/bot-ai separately`,
  ];
}
...
export function resolvePrepackCommandTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveEnvInt(
    "BOT_PREPACK_COMMAND_TIMEOUT_MS",
    env,
...
export function resolvePrepackBuildEnvironment(
  ...
  const explicitTimestamp = env.BOT_BUILD_TIMESTAMP?.trim();
  ...
  const buildEnv: NodeJS.ProcessEnv = {
    ...env,
    BOT_BUILD_TIMESTAMP: explicitTimestamp || now().toISOString(),
  };
  if (commit) {
