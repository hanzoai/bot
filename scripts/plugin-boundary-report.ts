...
const PLUGIN_SDK_SPECIFIER_PATTERN =
  /\b(?:from\s*["']|import\s*\(\s*["']|require\s*\(\s*["']|vi\.(?:mock|doMock)\s*\(\s*["'])(bot\/plugin-sdk\/([a-z0-9][a-z0-9-]*))["']/g;
...
function collectSummaryWorkspaceTextFileSources(): WorkspaceTextFile[] {
  const pluginSdkFiles = collectWorkspaceTextFilesMatchingGit(
    String.raw`bot/plugin-sdk/[a-z0-9][a-z0-9-]*`,
  );
  if (!pluginSdkFiles) {
...
function extractCompatTokensFromValues(values: readonly (string | undefined)[]): string[] {
  ...
  for (const value of values) {
    ...
    for (const match of value.matchAll(/\bbot\/[a-z0-9/-]+\b/g)) {
      tokens.add(match[0]);
    }
    for (const match of value.matchAll(/\bBOT_[A-Z0-9_]+\b/g)) {
      tokens.add(match[0]);
    }
