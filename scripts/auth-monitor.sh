...
#
# Suggested cron: */30 * * * * /path/to/bot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Bot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., bot-alerts)
...
STATE_FILE="$HOME/.bot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Bot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use bot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Bot to $NOTIFY_PHONE..."
            if bot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null; then
                notification_sent=1
            fi
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Bot is down. Run on the Bot host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1
