...
SERVICE_TEMPLATE="$SCRIPT_DIR/systemd/bot-auth-monitor.service"
SYSTEMD_USER_DIR="$HOME/.config/systemd/user"
SERVICE_TARGET="$SYSTEMD_USER_DIR/bot-auth-monitor.service"
TIMER_TARGET="$SYSTEMD_USER_DIR/bot-auth-monitor.timer"
...
SERVICE_TEMP="$(mktemp "$SYSTEMD_USER_DIR/bot-auth-monitor.service.XXXXXX")"
SERVICE_RENDERED=""
cleanup_service_temp() {
    rm -f "$SERVICE_TEMP" "$SERVICE_RENDERED"
}
trap cleanup_service_temp EXIT
SERVICE_RENDERED="$(mktemp "$SYSTEMD_USER_DIR/bot-auth-monitor.service.rendered.XXXXXX")"
...
RENDERED_NTFY_LINE="$(render_environment_line "NOTIFY_NTFY" "bot-alerts" "$NTFY_TOPIC")"
...
trap - EXIT
cp "$SCRIPT_DIR/systemd/bot-auth-monitor.timer" "$TIMER_TARGET"
systemctl --user daemon-reload
systemctl --user enable --now bot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status bot-auth-monitor.timer"
echo ""
echo "Quick commands:"
