#!/usr/bin/env node
// Builds the Bot package artifact used by Docker E2E.
...
const AI_RUNTIME_PACKAGE = "@hanzo/bot-ai";
const AI_RUNTIME_BACKUP_DIR = ".bot-ai-package-backup";
const ACTIVE_CHILD_KILLERS = new Set();
const PACKAGE_BUILD_PLUGIN_SELECTION_ENV_NAMES = [
  "BOT_EXTENSIONS",
  "BOT_DOCKER_BUILD_EXTENSIONS",
...
function resolvePackedBotFileName(value) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("bot-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  if (
    !/^bot-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
    filename !== path.basename(filename) ||
    filename !== path.win32.basename(filename)
  ) {
    throw new Error(`npm pack reported unsafe Bot tarball filename: ${filename}`);
  }
  return filename;
}
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building Bot package artifacts",
    command: "node",
    ...
    // BOT_RUN_NODE_SKIP_DTS_BUILD=0 this packaging path requires.
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "BOT_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestBotTarball(outputDir, packOutput) {
  let fromOutput = "";
  try {
    ...
    if (Array.isArray(parsed)) {
      for (const entry of parsed) {
        ...
        const filename = resolvePackedBotFileName(entry.filename);
        if (filename) {
          fromOutput = filename;
        }
      }
  ...
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedBotFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedBotFileName(entry) === entry;
      } catch {
        return false;
      }
    })
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed Bot tarball in ${outputDir}`);
  }
...
async function cleanPackedBotTarballs(outputDir) {
  let entries;
  try {
...
function isPackedAiRuntimeTarball(filename) {
  return /^bot-ai-[A-Za-z0-9._-]+\.tgz$/u.test(filename);
}
...
export async function prepareBundledAiRuntimePackage(
  ...
  const aiRuntimePath = path.join(sourceDir, "node_modules", "@bot", "ai");
  ...
  const extractAiRuntime =
    ...
    ((tarballPath, destination) =>
      ...
      run("tar", ["-xzf", tarballPath, "-C", destination, "--strip-components=1"], destination, {
        timeoutMs: resolveTimeoutMs(
          "BOT_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  if (!hasAiRuntimeWorkspace) {
    throw new Error("@hanzo/bot-ai dependency requires the packages/ai workspace");
  }
  if (typeof aiRuntimeDependency !== "string") {
    throw new Error("root package.json must declare @hanzo/bot-ai as a dependency");
  }
  ...
  try {
    ...
    if (packedAiTarballs.length !== 1) {
      throw new Error(
        `expected one packed @hanzo/bot-ai tarball in ${outputDir}, found ${packedAiTarballs.length}`,
      );
    }
    ...
    if (typeof stagedPackageJson.version !== "string" || !stagedPackageJson.version) {
      throw new Error("packed @hanzo/bot-ai package must declare a version");
    }
    for (const [name, version] of Object.entries(stagedPackageJson.dependencies ?? {})) {
      if (packageJson.dependencies[name] !== version) {
        throw new Error(
          `root package.json must declare ${name}@${version} to bundle @hanzo/bot-ai without duplicate dependencies`,
        );
      }
    }
...
export async function packBotPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing Bot package");
  await prepareChangelog(sourceDir);
  let packOutput;
  let cleanupBundledAiRuntime = async () => {};
  try {
    await cleanPackedBotTarballs(outputDir);
  ...
  let tarball = await newestBotTarball(outputDir, options.pnpmPack ? "" : packOutput);
  if (options.outputName) {
...
export async function writePackageInventoryForDocker(sourceDir, runImpl = run) {
  ...
  await runImpl(
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "BOT_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
...
async function main() {
  ...
  console.error("==> Writing Bot package inventory");
  ...
  console.error("==> Checking Bot package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [
      path.join(ROOT_DIR, "scripts/check-bot-package-tarball.mjs"),
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "BOT_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
        DEFAULT_PACKAGE_TARBALL_CHECK_TIMEOUT_MS,
      ),
    },
  );
  console.error(
    `==> Bot package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
