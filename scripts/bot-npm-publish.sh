...
usage() {
  echo "usage: bash scripts/bot-npm-publish.sh --publish [package.tgz]"
}
...
publish_plan="$(
  PACKAGE_VERSION="${package_version}" REQUESTED_PUBLISH_TAG="${BOT_NPM_PUBLISH_TAG:-}" \
    BYPASS_EXTENDED_STABLE_GUARD="${BYPASS_EXTENDED_STABLE_GUARD:-}" \
    node scripts/bot-npm-extended-stable-release.mjs publish-plan
)"
