---
name: release-bot-ci
description: "Run, watch, debug, and summarize Bot full release CI, release checks, live provider gates, install/update proofs, and release-secret preflights."
---

# Bot Release CI

Use this with `$release-bot-maintainer` and `$bot-testing` when a release candidate needs full validation, install/update proof, live provider checks, or CI recovery.
...
- Reused Testboxes are provenance-gated after their first successful run.
  ...
  `BOT_TESTBOX_ALLOW_STALE=1` for release evidence.
...
```bash
node .agents/skills/release-bot-ci/scripts/verify-provider-secrets.mjs --required openai,anthropic,fireworks
...
```bash
gh workflow run bot-performance.yml \
  --repo hanzoai/bot \
  --ref main \
...
Publish with `bot-release-publish.yml` using `release_profile=from-validation`
...
2. List failed jobs only:
   ```bash
   gh run view <child-run-id> --repo hanzoai/bot --json jobs \
