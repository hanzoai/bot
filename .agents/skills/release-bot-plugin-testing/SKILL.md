---
name: release-bot-plugin-testing
description: Plan and run pre-release Bot plugin validation across bundled plugins, package artifacts, lifecycle commands, doctor/fix, config round-trip, gateway startup, SDK compatibility, Docker E2E, Package Acceptance, and Testbox proof.
---

# Bot Pre-Release Plugin Testing
...
release?" It complements `bot-testing`; use that skill too when choosing
...
the main Bot checkout. Do not run `pnpm install` there. For broad or
...
```bash
BOT_TESTBOX=1 pnpm check:changed
pnpm run test:extensions:package-boundary:canary
pnpm run test:extensions:package-boundary:compile
pnpm test:docker:plugins
BOT_PLUGINS_E2E_CLAWHUB=0 pnpm test:docker:plugins
...
```bash
BOT_BUNDLED_PLUGIN_SWEEP_TOTAL=8 \
BOT_BUNDLED_PLUGIN_SWEEP_INDEX=<0-7> \
...
Use `source=npm -f package_spec=bot@beta` for published beta proof. Keep
...
ghx workflow run plugin-npm-release.yml \
  ...
  -f plugins=@hanzo/bot-meta-provider \
  -f ref="${release_sha}" \
...
Start packaged Bot in Docker with clean state:
...
- `bot status --json` includes plugin diagnostics
- `bot plugins inspect --all --json` is parseable
...
`bot/plugin-sdk/*` subpaths. Assert `plugins inspect` sees its tool,
