...
const DEFAULT_CHECKOUT = "/Users/steipete/bot";
const DEFAULT_EXPECTED_ORIGIN = "hanzoai/bot";
...
export function classifyActions(
  ...
  const generatedMacProtocolChanged = changedPaths.some((changedPath) =>
    /^apps\/shared\/BotKit\/Sources\/BotProtocol\//u.test(changedPath),
  );
  const runMacos =
  ...
  const macUiVerification =
    ...
    changedPaths.some((changedPath) =>
      /^(?:apps\/macos\/Sources\/|apps\/shared\/BotKit\/Sources\/|apps\/swabble\/Sources\/)/u.test(
        changedPath,
      ),
    );
...
function defaultLockPath(checkout) {
  ...
  return path.join(tmpdir(), `bot-live-updater-${key}.lock`);
}

function defaultStatePath(checkout) {
  return path.join(realpathSync(checkout), ".git", "bot-live-updater-state.json");
}
...
function writeMaintenanceState(statePath, state) {
  ...
  const temporary = path.join(directory, `.bot-live-updater-${process.pid}-${randomUUID()}`);
...
function resolveManagedGatewayCommand(
  ...
  const defaultEnvDir = path.join(stateDir ?? path.join(home, ".bot"), "service-env");
...
function readManagedGatewayLaunchAgent(checkout) {
  ...
  const plistPath = path.join(home, "Library/LaunchAgents/ai.bot.gateway.plist");
  let plistStat;
  try {
  ...
  const stateDir =
    typeof environmentVariables?.BOT_STATE_DIR === "string"
      ? environmentVariables.BOT_STATE_DIR
      : path.join(home, ".bot");
  ...
  const configPath =
    typeof plist?.EnvironmentVariables?.BOT_CONFIG_PATH === "string"
      ? plist.EnvironmentVariables.BOT_CONFIG_PATH
      : path.join(gatewayCommand.stateDir, "bot.json");
  return {
    configPath,
...
function inspectManagedGatewayDeployment(checkout) {
  ...
  const home = process.env.HOME;
  if (!home || !existsSync(path.join(home, "Library/LaunchAgents/ai.bot.gateway.plist"))) {
    return null;
  }
...
function replaceLaunchAgentEntrypoint(deployment, entrypoint) {
  const temporaryPath = `${deployment.plistPath}.bot-live-updater-${randomUUID()}`;
...
export function runBuiltGatewayCli(checkout, args, deployment, options = {}) {
  ...
  delete baseEnv.BOT_GATEWAY_URL;
  delete baseEnv.BOT_GATEWAY_TOKEN;
  delete baseEnv.BOT_GATEWAY_PASSWORD;
  delete baseEnv.BOT_CONFIG_PATH;
  delete baseEnv.BOT_GATEWAY_PORT;
  ...
  if (wrapperPath && envFilePath) {
    ...
    try {
      const configuredPath = execFileSync(
        executable,
        [...wrapperPrefix, "/usr/bin/printenv", "BOT_CONFIG_PATH"],
  ...
  const overlayPath = path.join(
    ...
    `.bot-live-updater-config-${randomUUID()}.json`,
  );
  writeFileSync(
    overlayPath,
    `${JSON.stringify({
  ...
  const envOverrides = [`BOT_CONFIG_PATH=${overlayPath}`, `BOT_GATEWAY_PORT=${port}`];
  const env = Object.assign(baseEnv, {
    BOT_CONFIG_PATH: overlayPath,
...
export function prepareGatewaySuspension(
  ...
  const requestId = `bot-live-updater-${randomUUID()}`;
  let result;
  try {
    result = JSON.parse(
...
function runBuildWithPreservedMacApp(runCommand, checkout, sleep = defaultSleep) {
  const appBundle = path.join(checkout, "dist/Bot.app");
  ...
  const preservedBundle = path.join(
    checkout,
    ".git",
    `.bot-live-mac-${process.pid}-${randomUUID()}.app`,
  );
...
function restartGateway(
  ...
  assertExactBuild(checkout, expectedSha);
  if (!deployment) {
    runCommand("pnpm", ["bot", "gateway", "restart"], checkout);
    return startedAtMs;
  }
  if (bootstrap) {
...
function verifyGateway(runCommand, checkout, expectedSha, deployment = null) {
  ...
  runCommand(
    "pnpm",
    ["bot", "gateway", "status", "--deep", "--require-rpc", "--json"],
    checkout,
  );
  runCommand("pnpm", ["bot", "health", "--verbose", "--json"], checkout);
}
...
function isCurrentGatewayLogSource(source, sourceRoot, managedSourceRoots) {
  ...
  while (candidate !== path.dirname(candidate)) {
    ...
    if (existsSync(packagePath) && existsSync(gitPath)) {
      try {
        if (JSON.parse(readFileSync(packagePath, "utf8")).name === "bot") {
...
function readFallbackGatewayLogs(sinceMs) {
  ...
  const directories = new Set(["/tmp/bot", path.join(tmpdir(), "bot")]);
  const contents = [];
  for (const directory of directories) {
    for (const date of dates) {
      const logPath = path.join(directory, `bot-${date}.log`);
...
function defaultVerifyMacTarget(checkout) {
  ...
  const executable = path.join(checkout, "dist/Bot.app/Contents/MacOS/Bot");
