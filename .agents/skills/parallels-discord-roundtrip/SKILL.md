...
- Use the `bot` wrapper for guest `message send/read`; `node bot.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/bot-parallels-smoke.*`
...
- host inbound nonce appears in `bot message read` output
