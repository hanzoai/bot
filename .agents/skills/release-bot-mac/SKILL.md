---
name: release-bot-mac
description: "Run or recover Bot macOS release signing, notarization, appcast, and asset promotion."
---

# Bot Mac Release

Use with `$release-bot-maintainer`, `$release-bot-ci`, `$one-password`, and `$release-private` if it exists when stable macOS assets, release-ops mac preflight, notarization, appcast promotion, or mac release recovery is involved.
...
Target release-ops repo environment: `bot/releases`, env `mac-release`.
...
- `hanzoai/bot` is the public product repo. Its GitHub Releases page is
  where macOS assets are ultimately attached.
- `hanzoai/bot` `macos-release.yml` is public handoff validation only.
  It never signs, notarizes, or uploads macOS assets, regardless of
  `preflight_only`.
- `bot/releases` is the restricted release-ops repo. Its macOS workflows
  sign, notarize, validate, and promote assets onto the
  `hanzoai/bot` GitHub release.
...
- Bot uses `scripts/notarize-mac-artifact.sh`.
...
```bash
gh workflow run macos-release.yml --repo hanzoai/bot \
...
```bash
gh workflow run bot-macos-publish.yml --repo bot/releases --ref main \
  -f tag=vYYYY.M.PATCH \
...
```bash
gh workflow run bot-macos-validate.yml --repo bot/releases --ref main \
  -f tag=vYYYY.M.PATCH \
  -f source_ref=release/YYYY.M.PATCH
```
...
- Release-ops `bot/releases` publish/validate workflows run from their own
  trusted `main` workflow ref. Real publish has a guard that rejects any other
  workflow ref. That displayed `main` ref is expected; the public Bot
  source is selected by `tag` and optional `source_ref`.

## Verify

- `gh release view vYYYY.M.PATCH --repo hanzoai/bot` shows zip, dmg, dSYM zip, not draft, not prerelease.
- Public `main` `appcast.xml` points at `Bot-YYYY.M.PATCH.zip`.
