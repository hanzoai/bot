---
name: bot-refactor-docs
description: Refactor an existing Bot docs page with source-audited preservation, restructuring, and verification.
---
...
Use this skill when the user gives a target Bot docs page and asks to
rewrite, refactor, reorganize, split, shorten, or improve it.

This skill builds on `bot-docs`: use that skill for style, page types,
...
Read `../bot-docs/SKILL.md` first. Apply its page-type, style,
...
Before editing, decide the intended page type from `bot-docs`.
