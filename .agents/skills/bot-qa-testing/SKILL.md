---
name: bot-qa-testing
description: Run, watch, debug, extend, or explain Bot qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
5. If the user wants to watch the live UI, find the current `bot-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm bot qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
pnpm bot qa matrix --profile release
```
...
- That item is the first place to look for:
  ...
  - `BOT_QA_PROVIDER_MODE`
  - `BOT_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `Bot`
  - item: `BOT_QA_CONVEX_SITE_URL`
  - item: `BOT_QA_CONVEX_SECRET_MAINTAINER`
  - item: `BOT_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `BOT QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `BOT_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `BOT_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `Bot/BOT_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
BOT_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
BOT_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
BOT_QA_PROVIDER_MODE="mock-openai" \
BOT_NPM_TELEGRAM_PACKAGE_SPEC="bot@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `BOT_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `BOT_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `BOT_QA_PROVIDER_MODE`, map it explicitly to `BOT_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `BOT_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo hanzoai/bot --ref main \
  -f package_spec=bot@YYYY.M.D-beta.N \
  -f package_label=bot@YYYY.M.D-beta.N \
...
```bash
gh api repos/hanzoai/bot/actions/runs/<run-id>/artifacts
```
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal Bot WhatsApp accounts in the shared pool.
...
- Validate with `pnpm bot qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Bot identity combines with the character.
...
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `BOT_HOME` sandboxed.
