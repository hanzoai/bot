...
metadata:
  bot:
    homepage: https://github.com/bot/discrawl
    requires:
      bins:
        - discrawl
    install:
      - kind: go
        module: github.com/bot/discrawl/cmd/discrawl@latest
        bins:
          - discrawl
---
...
- Repo: `bot/discrawl`; use `~/GIT/_Perso/discrawl` only after verifying
  its remote targets `bot/discrawl`, otherwise use a fresh checkout
...
`bot/discrawl` checkout before concluding the feature is missing.
