...
- Read `docs/reference/RELEASING.md`,
  `scripts/bot-npm-extended-stable-release.mjs`, and the relevant release
...
- Cover the core `bot` package and every npm-publishable official plugin
...
- Read `SECURITY.md` and use `$security-triage` for security candidates. Route
  unpublished advisory work through `$bot-ghsa-maintainer`; never expose
  private details before the security owner authorizes disclosure.
- Use `$bot-testing` for proof selection, `$autoreview` before handoff,
  and `$bot-pr-maintainer` for GitHub operations.
...
dist_tags=$(npm view bot dist-tags --json --userconfig "$npm_userconfig")
published_version=$(printf '%s' "$dist_tags" | jq -r '."extended-stable" // empty')
if [[ -n "$published_version" ]]; then
  npm view "bot@${published_version}" version \
...
- mode, published `bot@extended-stable` version or approved bootstrap
