---
name: release-bot-maintainer
description: Prepare or verify Bot stable, beta, and extended-stable releases, including backport discovery, changelogs, release notes, publish commands, and artifacts.
---

# Bot Release Maintainer
...
- Core `bot` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- As soon as the Code SHA exists, dispatch `Bot Performance`
...
- In maintainer Testbox mode, use `BOT_TESTBOX=1 scripts/pr prepare-run
<PR>` only after the exact PR head has passed `CI` and every scheduled
  ...
  `BOT_TESTBOX=1 scripts/pr prepare-sync-head <PR>`, wait for those hosted
...
- If an exact PR-head CI run has no active jobs because Blacksmith capacity is
  ...
  `gh api 'repos/hanzoai/bot/contents/.github/workflows/ci.yml?ref=<pr-head-branch>'
--jq .content | base64 --decode | rg -q 'pull_request_number:'`. If absent,
  ...
  `gh workflow run ci.yml --repo hanzoai/bot --ref <pr-head-branch> -f
...
include_android=true -f release_gate=true`.
  ...
  cancelled, or been skipped. Then rerun `BOT_TESTBOX=1 scripts/pr
prepare-run <PR>`.
- Generate the changelog once after the final Code SHA is fully green. Do not
  ...
  `$bot-changelog-update` for the rewrite. Do not continue release prep if
...
distribution: the `bot` npm package, official npm plugins, and matching
Docker Gateway images. Treat
`docs/reference/RELEASING.md`,
`scripts/bot-npm-extended-stable-release.mjs`, and the release workflows
...
5. Run `pnpm release:generated:check`, `pnpm deps:npm-lock:check`, and
   `BOT_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main`
...
6. Keep repository variables `RELEASE_ROLLBACK_DRILL_ID` and
   ...
   `bot-stable-main-closeout.yml` starts from the `main` push carrying the
...
- Every stable Bot release ships the npm package, macOS app, and signed
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/hanzoai/bot/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `bot` repo.
...
- Stable Windows Hub release closeout requires the signed
  `BotCompanion-Setup-x64.exe`, `BotCompanion-Setup-arm64.exe`, and
  `BotCompanion-SHA256SUMS.txt` assets on the canonical
  `hanzoai/bot` GitHub Release. Pass the exact signed
  `hanzoai/bot-windows-node` release tag as `windows_node_tag` to
  `Bot Release Publish`, together with the candidate-approved
  ...
  dispatches the public `Windows Node Release` workflow while the Bot
  release is still a draft, carries those pinned source asset digests
  unchanged, verifies the expected Bot Foundation Authenticode signer on
  ...
  `BotCompanion-*` target asset names, then replaces the expected contract
  assets with the pinned source bytes.
- Website Windows Hub download links should target exact canonical
  `hanzoai/bot/releases/download/vYYYY.M.PATCH/...` assets for the current
  ...
  is visible from both the GitHub release page and bot.ai.
...
- Generate `$bot-changelog-update`'s full contribution manifest before
...
- Before publishing or closing a release, run
  `$bot-changelog-update`'s `verify-release-notes.mjs` with every stable
...
- To update an existing GitHub Release body, resolve the numeric release id and
  ...
  `gh api repos/hanzoai/bot/releases/tags/vYYYY.M.PATCH --jq .id`, then
  `gh api -X PATCH repos/hanzoai/bot/releases/<id> -F body=@/tmp/notes.md`.
  Do not trust `gh release edit --notes-file` or `--input` JSON if verification
  disagrees; verify with `gh api repos/hanzoai/bot/releases/<id>` because
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `bot YYYY.M.PATCH-beta.N`
...
Use the Bot account's existing release-post style:

- Format: `Bot YYYY.M.PATCH 🦞` or `🦞 Bot YYYY.M.PATCH is live`, blank line,
  then 3-4 emoji-led bullets, blank line, one short punchline, then the release
  link.
- For beta: say `Bot YYYY.M.PATCH-beta.N 🦞` or `Bot YYYY.M.PATCH beta N is
...
- Before tagging, diff publishable plugin package manifests against the last
  ...
  (`bot.release.publishToNpm: true` or `publishToClawHub: true`) whose
...
- Bootstrap a new ClawHub package only from the trusted workflow source:
  `gh workflow run plugin-clawhub-new.yml --ref main -f plugins=@hanzo/bot-name -f ref=<full-release-sha> -f pretag_validation=true -f dry_run=true`.
...
```bash
node --import tsx scripts/bot-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `BOT_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `BOT_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the Bot npm release path before creating the tag.
...
- Full pre-npm beta test roster:
  ...
  - all Parallels install/update tests:
    ...
    rerun lanes from `bot-parallels-smoke`
...
- Post-published beta verification roster:
  - `node --import tsx scripts/bot-npm-postpublish-verify.ts <beta-version>`
  ...
  - published npm Telegram proof: dispatch Actions > `NPM Telegram Beta E2E`
    from `main` with `package_spec=bot@<beta-version>` and
    ...
    `BOT_NPM_TELEGRAM_PACKAGE_SPEC` and Convex CI env only as a fallback
    or debugging path.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `BOT_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  ...
  - `180m` for local full QA live OpenAI + Anthropic rosters when explicitly
    ...
    `QA-Lab - All Lanes`
  - Parallels caps from the `bot-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `bot update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `hanzoai/bot` and the release-ops mac preflight in
  `bot/releases` for every release.
...
- Bot publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` uses the restricted release-ops
  `bot/releases/.github/workflows/bot-npm-dist-tags.yml` workflow
...
- Prefer fixing the release-ops workflow token path over any local 1Password
  ...
  repo's `NPM_TOKEN` secret, scoped to the `bot` package with read/write
...
- If the release-ops dist-tag workflow cannot promote because `NPM_TOKEN` is
  ...
  - Promote with a fresh OTP:
    `npm dist-tag add bot@YYYY.M.PATCH latest --otp "$OTP"`.
  - Verify with a cache-bypassed registry read, for example:
    `npm view bot dist-tags --json --prefer-online --cache /tmp/bot-npm-cache-verify-$$`
    and `npm view bot@latest version dist.tarball --json --prefer-online`.
...
- npm registry metadata is eventually consistent immediately after trusted
  ...
  steps instead of reading the registry again. If the Bot npm child
...
- `.github/workflows/macos-release.yml` in `hanzoai/bot` is now a
  ...
  artifacts.
- `bot/releases/.github/workflows/bot-macos-validate.yml` is the
...
- Real mac preflight and real mac publish both use
  `bot/releases/.github/workflows/bot-macos-publish.yml`.
...
- Focused plugin-only repairs use `plugin_publish_scope=selected` with a nonempty
  ...
  is disabled.
- Dispatch regular beta and stable `Bot Release Publish` runs from trusted
...
- The `npm-release` environment must be approved by `@hanzo/bot-bot-release-managers` before publish continues.
- Mac publish uses
  `bot/releases/.github/workflows/bot-macos-publish.yml` for
...
- Real release-ops mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `hanzoai/bot`
  automatically when `BOT_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the Bot package
...
7. Immediately dispatch Actions > `Bot Performance` from the pinned
...
20. For stable releases, start `.github/workflows/macos-release.yml` in
    `hanzoai/bot` and wait for the public validation-only run to pass.
21. For stable releases, start
    `bot/releases/.github/workflows/bot-macos-validate.yml` with the
    same tag and wait for the release-ops mac validation lane to pass.
22. For stable releases, start
    `bot/releases/.github/workflows/bot-macos-publish.yml` with
...
24. Start `.github/workflows/bot-release-publish.yml` from the exact pinned
    ...
    `hanzoai/bot-windows-node` tag as `windows_node_tag` and its
    candidate-approved installer digest map as `windows_node_installer_digests`.
25. Wait for `npm-release` approval from `@hanzo/bot-bot-release-managers`.
26. Wait for the real publish workflow to run postpublish verification,
    ...
    waited plugin publish or Windows Hub promotion fails after Bot npm
    succeeds, the workflow keeps the release draft with Bot npm evidence
...
32. For stable releases, start
    `bot/releases/.github/workflows/bot-macos-publish.yml` for the
...
- Use `bot-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
