---
name: telegram-botbox-e2e-proof
description: Use when reviewing, reproducing, or proving Bot Telegram behavior with a real Telegram user on Crabbox, including PR review workflows that need an agent-controlled Telegram Desktop recording, TDLib user-driver commands, Convex-leased credentials, WebVNC observation, and motion-trimmed artifacts.
---
...
Run from the Bot repo and branch under test:

```bash
proof_cmd="${BOT_TELEGRAM_USER_PROOF_CMD:-bot-telegram-user-botbox-proof}"
"$proof_cmd" start \
  --tdlib-url http://artifacts.bot.ai/tdlib-v1.8.0-linux-x64.tgz \
  --output-dir .artifacts/qa-e2e/telegram-user-botbox/pr-review
```
...
- starts a mock Bot Telegram SUT from the current checkout
- selects the configured Telegram chat in the visible Linux desktop
- starts a 24fps desktop recording
- writes `.artifacts/qa-e2e/telegram-user-botbox/pr-review/session.json`
...
"$proof_cmd" start \
  --tdlib-url http://artifacts.bot.ai/tdlib-v1.8.0-linux-x64.tgz \
  --mock-response-file .artifacts/qa-e2e/telegram-user-botbox/reply.txt \
  --output-dir .artifacts/qa-e2e/telegram-user-botbox/pr-review
```
...
proof_cmd="${BOT_TELEGRAM_USER_PROOF_CMD:-bot-telegram-user-botbox-proof}"
"$proof_cmd" view \
  --session .artifacts/qa-e2e/telegram-user-botbox/pr-review/session.json \
...
"$proof_cmd" run \
  --session .artifacts/qa-e2e/telegram-user-botbox/pr-review/session.json \
  -- bash -lc 'source /tmp/bot-telegram-user-botbox/env.sh && python3 /tmp/bot-telegram-user-botbox/user-driver.py transcript --limit 20 --json'
```

Useful remote user-driver commands:

```bash
source /tmp/bot-telegram-user-botbox/env.sh
python3 /tmp/bot-telegram-user-botbox/user-driver.py status --json
python3 /tmp/bot-telegram-user-botbox/user-driver.py chats --json
python3 /tmp/bot-telegram-user-botbox/user-driver.py transcript --limit 20 --json
python3 /tmp/bot-telegram-user-botbox/user-driver.py send --text '/status@{sut}'
python3 /tmp/bot-telegram-user-botbox/user-driver.py probe --text '@{sut} Reply exactly: USER-E2E-{run}' --expect USER-E2E-
```
...
publish uses that; otherwise it uses `telegram-user-botbox-session-motion.gif`.
...
```bash
mkdir -p .artifacts/qa-e2e/telegram-user-botbox/pr-123/comparison
cp <main-output>/telegram-user-botbox-session-motion-telegram-window.gif \
  .artifacts/qa-e2e/telegram-user-botbox/pr-123/comparison/main-before.gif
cp <pr-output>/telegram-user-botbox-session-motion-telegram-window.gif \
  .artifacts/qa-e2e/telegram-user-botbox/pr-123/comparison/pr-after.gif
botbox artifacts publish \
  --repo hanzoai/bot \
  --pr 123 \
  --dir .artifacts/qa-e2e/telegram-user-botbox/pr-123/comparison \
