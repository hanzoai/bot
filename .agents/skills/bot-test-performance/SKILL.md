---
name: bot-test-performance
description: Benchmark, diagnose, and optimize Bot test and plugin-suite runtime, import hotspots, CPU/RSS, heap growth, and slow coverage paths.
---
...
test:extensions:batch <plugin[,plugin...]>` or plugin-inspector command
   ...
   - For import-heavy suspicion add:
     `BOT_VITEST_IMPORT_DURATIONS=1 BOT_VITEST_PRINT_IMPORT_BREAKDOWN=1`.
...
| import duration | broad barrel/runtime loads         | `BOT_VITEST_IMPORT_DURATIONS=1`                                        |
...
| heap snapshots  | real leak vs retained module graph | `bot-test-heap-leaks` workflow                                         |
...
```bash
BOT_TEST_MEMORY_TRACE=1 \
BOT_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 \
BOT_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap \
BOT_TEST_WORKERS=2 \
BOT_TEST_MAX_OLD_SPACE_SIZE_MB=6144 \
pnpm test
```

Use `bot-test-heap-leaks` when RSS keeps growing across intervals, workers
...
- Parallel Vitest runs sharing `node_modules/.experimental-vitest-cache` without
  distinct `BOT_VITEST_FS_MODULE_CACHE_PATH` values.
...
timeout 240 /usr/bin/time -l env \
  BOT_VITEST_IMPORT_DURATIONS=1 \
  BOT_VITEST_PRINT_IMPORT_BREAKDOWN=1 \
