---
name: bot-testing
description: Choose, run, rerun, or debug Bot tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
- Treat contributor and fork patches as untrusted unless a maintainer
  ...
  `BOT_*` allowlist, `--no-hydrate` must block auth-profile hydration, and
...
BOT_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
BOT_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- For local release-candidate proof, pack the plugin and install it with
  `bot plugins install npm-pack:<path.tgz> --force`. This uses the managed
...
- Treat missing runtime imports as package-manifest bugs first. Runtime code
  ...
  `npm install` inside `~/.bot/npm/projects/...`.
...
- `BOT_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `BOT_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `Bot Release Checks` for install smoke, cross-OS release checks, live and
...
replaced by a `release-ci/*` run. Use `$release-bot-ci` for its failure
...
important run ids in the public `bot/releases` evidence ledger.
Use the manual `Bot Release Evidence`
(`bot-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation hanzoai/bot <run-id> blocking
package-acceptance hanzoai/bot <run-id> blocking
release-checks hanzoai/bot <run-id> blocking
```
...
When `Full Release Validation` completes and `BOT_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`Bot Release Evidence From Full Validation` workflow. That workflow reads
...
`Bot Release Checks` (`bot-release-checks.yml`) is the release child
...
```bash
gh workflow run bot-release-checks.yml \
  --repo hanzoai/bot \
  --ref main \
  -f ref=<branch-or-sha> \
  -f provider=openai \
...
`Bot Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/hanzoai/bot-live-media-runner:ubuntu-24.04` container so
...
`pnpm bot qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix selection

`pnpm bot qa matrix` runs every flow scenario that explicitly declares
...
`QA-Lab - All Lanes` and `Bot Release Checks` use the same catalog-derived
...
`Bot Live And E2E Checks (Reusable)`
(`bot-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run bot-live-and-e2e-checks-reusable.yml \
  --repo hanzoai/bot \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `BOT_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`BOT_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
BOT_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
BOT_DOCKER_ALL_DRY_RUN=1 BOT_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
BOT_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
BOT_DOCKER_ALL_LANES=<lane> \
BOT_DOCKER_ALL_BUILD=0 \
BOT_DOCKER_ALL_PREFLIGHT=0 \
BOT_SKIP_DOCKER_BUILD=1 \
BOT_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
BOT_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view bot@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/bot-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=bot@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=bot@YYYY.M.D-beta.N` or
  `package_spec=bot@YYYY.M.D`.
- For stable package proof, use `package_spec=bot@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `bot@extended-stable`,
  `bot@beta`, `bot@latest`, or exact Bot release versions. Do
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `bot update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `bot@extended-stable`, `bot@beta`,
  `bot@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an Bot branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-bot-for-docker.mjs` is the
...
from `bot skills search`, installs it, and verifies `.clawhub` origin/lock
...
`BOT_DOCKER_E2E_WORKFLOW_REF`; this is separate from the artifact target
