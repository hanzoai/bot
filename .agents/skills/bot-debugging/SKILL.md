---
name: bot-debugging
description: Debug Bot model, provider, tool-surface, code-mode, streaming, and live/Crabbox behavior by choosing the right logs, probes, and proof path before changing code.
---

# Bot Debugging

Use this skill when Bot behavior differs between local tests, live models,
providers, code mode, Tool Search, Crabbox, or CI, and the next move should be a
debug signal rather than a guess.

## Read First

- `docs/logging.md` for log files, `bot logs`, and targeted debug flags.
...
BOT_DEBUG_MODEL_PAYLOAD=full-redacted BOT_DEBUG_SSE=peek bot gateway
```

Useful flags:

- `BOT_DEBUG_MODEL_TRANSPORT=1`: request start, fetch response, SDK
  headers, first SSE event, stream done, and transport errors at `info`.
- `BOT_DEBUG_MODEL_PAYLOAD=summary`: bounded payload summary.
- `BOT_DEBUG_MODEL_PAYLOAD=tools`: all model-facing tool names.
- `BOT_DEBUG_MODEL_PAYLOAD=full-redacted`: capped, redacted JSON payload.
  Use only while debugging; prompts/message text may still appear.
- `BOT_DEBUG_SSE=events`: first-event and stream-completion timing.
- `BOT_DEBUG_SSE=peek`: first five redacted SSE events.
- `BOT_DEBUG_CODE_MODE=1`: code-mode tool-surface diagnostics.
