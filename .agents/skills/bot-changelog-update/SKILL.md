---
name: bot-changelog-update
description: Regenerate Bot release changelog sections from git history before beta, stable, or extended-stable releases.
---

# Bot Changelog Update
...
Use it with `release-bot-maintainer`; this skill owns changelog content,
...
3. Generate the complete contribution record and editorial manifest before
   ...
   ```bash
   node .agents/skills/bot-changelog-update/scripts/verify-release-notes.mjs \
     --base <base-tag> \
     --target <target-ref> \
     --main-ref origin/main \
     --version <YYYY.M.PATCH> \
     --manifest /tmp/bot-release-<YYYY.M.PATCH>.json \
     --write-ledger
   ```
...
6. Preserve attribution:
   ...
   - never thank bots, `@claude`, `@bot`, `@clawsweeper`, or `@steipete`
...
- unqualified `#NNN` references resolve against `hanzoai/bot`;
  cross-repository references such as `bot/imsg#141` remain literal
