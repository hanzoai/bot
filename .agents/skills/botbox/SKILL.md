---
name: botbox
description: "Crabbox/Testbox remote proof for Bot: trusted-source routing, untrusted isolation, Linux/macOS/Windows/WSL2, live E2E, desktop, diagnostics, cleanup."
---

# Crabbox

Remote Bot proof. Heavy tests. Builds. Typecheck/lint fan-out. Docker.
...
Need direct AWS semantics? Pass `--provider aws`. Need normal trusted Bot
...
```sh
command -v botbox
../botbox/bin/botbox --version
node scripts/botbox-wrapper.mjs run --help | sed -n '1,100p'
...
```sh
if [ -x ../botbox/bin/botbox ]; then
  export CRABBOX=../botbox/bin/botbox
else
  export CRABBOX="$(command -v botbox)"
fi
"$CRABBOX" --version
```

Read `.botbox.yaml`; never guess provider default.

No binary? Clean sibling checkout only:

```sh
if [ -n "$(git -C ../botbox status --short)" ]; then
  git -C ../botbox status --short
  exit 1
fi
git -C ../botbox pull --ff-only
mkdir -p ../botbox/bin
(cd ../botbox && go build -o bin/botbox ./cmd/botbox)
../botbox/bin/botbox --version
```
...
node scripts/botbox-wrapper.mjs run \
  ...
  BOT_TEST_PROJECTS_PARALLEL=6 \
  BOT_VITEST_MAX_WORKERS=1 \
  BOT_TESTBOX=1 BOT_TESTBOX_REMOTE_RUN=1 \
  pnpm check:changed
```
...
```sh
node scripts/botbox-wrapper.mjs warmup \
...
```sh
TMPDIR=/tmp BOT_TESTBOX=1 "$AUTOREVIEW" \
...
"$CRABBOX" doctor
"$CRABBOX" whoami
"$CRABBOX" login --url https://botbox.bot.ai --provider aws
```
...
node scripts/botbox-wrapper.mjs run \
  ...
  --no-hydrate --fresh-pr hanzoai/bot#123 \
  --timing-json --shell -- \
...
Isolate mutable state: `BOT_STATE_DIR=$(mktemp -d)`. Test-only local plugin
artifacts may use `BOT_ALLOW_PLUGIN_INSTALL_OVERRIDES=1`; never call them
...
Bot setup belongs hydration workflow/repo scripts.
