...
goal is to keep Bot fast while distinguishing runner registration, runner
...
- GitHub runner registrations for `bot` currently report a 10,000 per
  5-minute bucket in `actions_runner_registration`. Verify the live bucket
  before each tuning pass because GitHub can change it. The `bot`
...
- **Bot test runtime:** jobs start quickly but one lane dominates wall time.
...
7. Split truly slow tests with `$bot-test-performance`; do not hide a slow
...
./node_modules/.bin/oxfmt --check .github/workflows/ci.yml .github/workflows/codeql-critical-quality.yml docs/ci.md test/scripts/ci-workflow-guards.test.ts .agents/skills/bot-ci-limits/SKILL.md .agents/skills/bot-ci-limits/agents/openai.yaml
git diff --check
```
...
ghx pr checks <pr> -R hanzoai/bot --watch --interval 15
```
...
BOT_TESTBOX=1 scripts/pr prepare-run <pr>
```
