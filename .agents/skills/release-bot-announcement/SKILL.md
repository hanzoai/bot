---
name: release-bot-announcement
description: "Draft or post Bot beta, stable, or extended-stable Discord release announcements from changelog, tag, registry, and validation evidence. Use when announcing a release, release candidate, or asking what users should test after an Bot release."
---

# Bot Release Announcement

Use with `release-bot-maintainer` after a release is live.
...
- exact version, e.g. `Bot 2026.5.25-beta.1`
...
- update command promoted near the top:
  ```sh
  bot update --channel beta --yes
  bot --version
  ```
- fresh install path:
  `Install from https://bot.ai`
- GitHub release link
...
```sh
bot update --channel stable --yes
bot --version
```

Fresh installs still point to `https://bot.ai`.
...
sending it. Never use Bot channel sends, bots, webhooks, relays, or tokens.
