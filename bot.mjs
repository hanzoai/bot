...
const ensureSupportedRuntimeVersion = () => {
  if (process.versions.bun) {
    ...
    // rejecting Bun outright so capable Bun builds can run Bot.
    ...
    process.stderr.write(
      "bot: this Bun runtime is unsupported because it does not provide node:sqlite.\n" +
  ...
  process.stderr.write(
    `bot: Node.js ${SUPPORTED_NODE_RANGE} is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[bot] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.BOT_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    BOT_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["bot: missing dist/entry.(m)js (build output)."];
  ...
  lines.push("For releases, use `npm install -g bot@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.BOT_CONTAINER)) {
    return true;
  }
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.BOT_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.BOT_CONFIG_PATH?.trim();
  if (explicit) {
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.BOT_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.BOT_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `Bot ${version} (${commit})\n` : `Bot ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.BOT_BUNDLED_VERSION) ?? "0.0.0";
}
