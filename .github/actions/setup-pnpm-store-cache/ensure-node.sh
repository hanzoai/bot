  ...
  case "$requested" in
    *x)
      ...
      if [[ "${requested%%.*}" == "22" ]]; then
        bot_node_version_at_least "$actual" "22.22.3"
      elif [[ "${requested%%.*}" == "24" ]]; then
        bot_node_version_at_least "$actual" "24.15.0"
      elif [[ "${requested%%.*}" == "25" ]]; then
        bot_node_version_at_least "$actual" "25.9.0"
      fi
      ;;
    *.*.*)
      [[ "$actual" == "$requested" ]]
...
bot_find_toolcache_node() {
  ...
  for node_root in ${roots[@]+"${roots[@]}"}; do
    while IFS= read -r candidate; do
      ...
      if bot_node_version_matches "$candidate_version" "$requested_node"; then
        printf '%s\n' "$candidate"
        return 0
      fi
...
bot_resolve_node_download_version() {
  local requested_node="$1"
  ...
  curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 \
    ...
    BOT_NODE_PREFIX="$prefix" python3 -c 'import json, os, sys
prefix = os.environ["BOT_NODE_PREFIX"]
...
bot_download_node() {
  ...
  version="$(bot_resolve_node_download_version "$requested_node")"
  platform="$(bot_node_download_platform)" || return 1
  temp_root="${RUNNER_TEMP:-/tmp}"
  if command -v cygpath >/dev/null 2>&1; then
    temp_root="$(cygpath -u "$temp_root" 2>/dev/null || printf '%s\n' "$temp_root")"
  fi
  install_root="${temp_root}/bot-node-${version}-${platform}"
  if [[ "$platform" == win-* ]]; then
    ...
    if command -v pwsh >/dev/null 2>&1; then
      ...
      bot_prepend_node_bin "$node_bin_dir" "$ps_bin_dir"
      ...
      bot_prepend_node_bin "$node_bin_dir"
    fi
  else
    ...
    rm -f "$archive_path"
    bot_prepend_node_bin "$install_root/bin"
  fi
}
...
bot_ensure_node() {
  ...
  active_node_version="$(bot_active_node_version)"
  if bot_node_version_matches "$active_node_version" "$requested_node"; then
    echo "Using active Node ${active_node_version} at $(command -v node)"
    return 0
  fi

  node_bin="$(bot_find_toolcache_node "$requested_node" || true)"
  if [[ -n "$node_bin" ]]; then
    echo "Using Node $("$node_bin" -p 'process.versions.node') from $node_bin"
    bot_prepend_node_bin "$(dirname "$node_bin")"
  else
    bot_download_node "$requested_node" || true
  fi
