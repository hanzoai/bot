# Bot Docs MDX Repair Agent

You are repairing generated Bot documentation after a fast MDX validation failure.
...
1. Read `.bot-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".bot-sync/mdx/${LOCALE}.json"`.
