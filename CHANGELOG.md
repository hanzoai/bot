...
- CLI/Pairing: make `bot qr --remote` prefer `gateway.remote.url` over tailscale/public URL resolution and register the `bot bot qr` legacy alias path. (#18091)
...
- Gateway/Subagents: preserve queued announce items and summary state on delivery errors, retry failed announce drains, and avoid dropping unsent announcements on timeout/failure. (#16729) Thanks @Botdette-Workspace.
...
- Scripts/Security: validate GitHub logins and avoid shell invocation in `scripts/update-contributors.ts` to prevent command injection via malicious commit records. Thanks @scanleale.
...
- Botdock: avoid Zsh readonly variable collisions in helper scripts. (#15501) Thanks @nkelner.
...
- Heartbeat: filter noise-only system events so scheduled reminder notifications do not fire when cron runs carry only heartbeat markers. (#13317) Thanks @pvtbotn.
...
- Docker: add BotDock shell helpers for Docker workflows. (#12817) Thanks @Olshansk.
...
- Onboarding/Providers: add LiteLLM provider onboarding and preserve custom LiteLLM proxy base URLs while enforcing API-key auth mode. (#12823) Thanks @ryan-botbe.
...
- macOS: honor Nix-managed defaults suite (`ai.bot.mac`) for nixMode to prevent onboarding from reappearing after bundle-id churn. (#12205) Thanks @joshp123.
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `hanzo-bot doctor --fix` to repair, then update plugins (`bot plugins update`) if you use any.
...
- **Docs:** new FAQ/BotHub/config examples/showcase entries and clarified auth, sandbox, and systemd docs.
...
- Commands: unify /status (inline) and command auth across providers; group bypass for authorized control commands; remove Discord /botd slash handler.
