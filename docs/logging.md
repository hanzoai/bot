...
read_when:
  - You need a beginner-friendly overview of Bot logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

Bot has two main log surfaces:
...
`/tmp/hanzoai/bot-<profile>-YYYY-MM-DD.log`
...
`bot-dev-YYYY-MM-DD.log`. Case, underscores, and literal dashes use a
...
The date uses the gateway host's local timezone. When `/tmp/bot` is unsafe
or unavailable (and always on Windows), Bot uses a user-scoped
`bot-<uid>` directory under the OS temp dir instead. Dated log files are
pruned after 24 hours.

Each file rotates when the next write would exceed `logging.maxFileBytes`
(default: 100 MB). Bot keeps up to five numbered archives beside the
active file, such as `bot-YYYY-MM-DD.1.log` or
`bot-dev-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead
of suppressing diagnostics.

You can override the path in `~/.hanzoai/bot.json`:

```json
{
  "logging": {
    "file": "/path/to/bot.log"
  }
}
```
...
or times out before `logs.tail` answers, `bot logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `bot logs --follow` is stricter: on Linux it uses the active
...
```bash
bot channels logs --channel whatsapp
```
...
Bot preserves the original structured log arguments alongside these fields
...
`bot gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
BOT_DEBUG_MODEL_TRANSPORT=1 bot gateway
BOT_DEBUG_MODEL_PAYLOAD=tools BOT_DEBUG_SSE=events bot gateway
```

Available flags:

- `BOT_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `BOT_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `BOT_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `BOT_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `BOT_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `BOT_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `BOT_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal Bot logging, so `bot logs --follow`
...
`BOT_DEBUG_MODEL_TRANSPORT`, so basic model transport hygiene is visible
without debug flags.

### Trace correlation

File logs are JSONL. When a log call carries a valid diagnostic trace context,
Bot writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
config value; `bot doctor --fix` maps a stored `consoleStyle: "compact"`
to `"pretty"`.

### Redaction

Bot can redact sensitive tokens before they hit console output, file logs,
...
Bot also redacts safety-boundary payloads shown to UI clients, support
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `BOT_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `bot doctor` first.
