...
- repo: `bot/docs`
- local checkout: `~/Projects/bot-docs`

## Source of truth

- English docs are authored in `hanzoai/bot`.
...
1. Edit English docs in `hanzoai/bot`.
2. Push to `main`.
3. `hanzoai/bot/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `bot/docs`.
...
5. `bot/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `bot/docs`.
...
- Do not treat locale visibility in generated `docs/docs.json` as proof that translation artifacts exist. Verify each generated locale folder and its translation memory in `bot/docs`.
...
- Sync metadata is written to `.bot-sync/source.json` in the publish repo.
- Source repo secret: `BOT_DOCS_SYNC_TOKEN`
- Publish repo secret: `BOT_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `bot/docs` first.
