---
summary: "First-run setup flow for Bot (macOS app)"
read_when:
...
<Tip>
Need the app first? [Download Bot for macOS](/platforms/macos#download),
...
<Step title="CLI">
  Local setup installs the global `bot` CLI via npm, pnpm, or bun,
...
<Step title="Connect your AI">
  ...
  page entirely and opens the normal agent UI. Bot and provider setup
...
the provider supplies its starter model and configuration; Bot verifies
...
start without working inference. After that live check passes, Bot becomes
available to help configure the remaining workspace, Gateway, channels, and
other optional features. When Bot offers a short list of choices, the app
shows native option cards; choosing one sends the selection, and **Skip for
now** always leaves the choice optional. Bot is also available later under
Settings → Bot.
</Step>
...
<Frame caption="Choose what permissions do you want to give Bot">
...
<Step title="Finish">
  After inference passes, Bot owns the remaining optional setup and can
  ...
  agent setup conversation before Bot. See
