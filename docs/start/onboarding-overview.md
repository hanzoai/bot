---
summary: "Overview of Bot onboarding options and flows"
read_when:
...
Bot has terminal and macOS app onboarding. Both establish inference first:
they detect existing AI access, require a live completion, and only then start
Bot to configure the remaining setup. A reachable, configured Gateway
...
After that completion passes, Bot can configure the workspace, Gateway,
...
and credential only after a passing completion, then starts Bot to
configure the workspace, Gateway, channels, agents, plugins, and other optional
features. **Skip for now** exits without starting Bot. There is no
in-flow classic handoff; exit and run `bot onboard --classic` when you want
the classic wizard instead.

After inference passes, Bot can hand channel setup to a masked terminal
wizard. It does not open guided or classic provider setup; exit Bot and
run `bot onboard` to change the model provider or its authentication.

Use `bot onboard --classic` for detailed model/auth, channel, skill,
remote Gateway, or import setup. Adding `--install-daemon` also selects the
classic flow and installs the background service in one step. Use `bot
bot` for conversational non-inference setup and repair. `bot
onboard --modern` is a compatibility alias that uses the same live-inference
gate.

Full reference: [Onboarding (CLI)](/start/wizard)
CLI command docs: [`bot onboard`](/cli/onboard)

## macOS app onboarding

Open the Bot app. If its configured local or remote Gateway is reachable
and the default agent already has a configured model, the app skips onboarding
and Bot and opens the normal agent UI immediately.
...
credentials use masked input. Once inference passes, Bot starts and
...
If your provider is not listed, run `bot onboard --classic`, choose
