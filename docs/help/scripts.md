...
- Prefer CLI surfaces when they exist (example: `bot models status --check`).
...
- `scripts/claude-auth-status.sh [full|json|simple]` - checks Claude Code + Bot auth status.
- `scripts/auth-monitor.sh` - polls status and sends a notification (via Bot send, and/or ntfy.sh) when the token nears expiry. Env: `WARN_HOURS` (default `2`), `NOTIFY_PHONE`, `NOTIFY_NTFY`. Run on a schedule via the bundled `scripts/systemd/bot-auth-monitor.{service,timer}` (every 30 minutes).
...
- `BOT_GH_READ_APP_ID`
- `BOT_GH_READ_PRIVATE_KEY_FILE`

Optional env:

- `BOT_GH_READ_INSTALLATION_ID` when you want to skip repo-based installation lookup
- `BOT_GH_READ_PERMISSIONS` as a comma-separated override for the read permission subset to request
...
- `scripts/gh-read run list -R hanzoai/bot`
- `scripts/gh-read api repos/hanzoai/bot/pulls/123`
