---
summary: "How Bot validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing Bot update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `bot doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` covers plugin install smoke, local folder installs,
  ...
  and Claude-bundle enable/inspect. Set `BOT_PLUGINS_E2E_CLAWHUB=0` to
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `bot plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `bot config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `bot update --yes --json`, and requires the candidate update command to
...
```bash
BOT_UPGRADE_SURVIVOR_BASELINE_SPEC=bot@2026.4.23 \
BOT_UPGRADE_SURVIVOR_SCENARIO=versioned-runtime-deps \
pnpm test:docker:published-upgrade-survivor

BOT_UPGRADE_SURVIVOR_BASELINE_SPEC=bot@latest \
BOT_UPGRADE_SURVIVOR_SCENARIO=bootstrap-persona \
pnpm test:docker:published-upgrade-survivor
```

Available scenarios: `base`, `acpx-bot-tools-bridge`, `feishu-channel`,
`bootstrap-persona`, `channel-post-core-restore`, `plugin-deps-cleanup`,
`configured-plugin-installs`, `stale-source-plugin-shadow`, `tilde-log-path`,
and `versioned-runtime-deps`. In aggregate runs, `BOT_UPGRADE_SURVIVOR_SCENARIOS=reported-issues`
...
- `source=npm`: validate `bot@extended-stable`, `bot@beta`,
  `bot@latest`, or an exact published version.
...
- `source=trusted-url`: validate an HTTPS tarball with required
  ...
  `BOT_TRUSTED_PACKAGE_TOKEN` secret.
...
`package_acceptance_package_spec=bot@YYYY.M.PATCH` so the same upgrade matrix
...
`last-stable-4` resolves to the four latest stable npm-published Bot
...
`package_spec=bot@extended-stable`. Package Acceptance resolves that
