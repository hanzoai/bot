...
Bot has three Vitest suites (unit/integration, e2e, live) plus Docker
...
- Linux VM-backed QA lane: `pnpm bot qa suite --runner multipass --scenario channel-chat-baseline`
...
it("uses a temp workspace", () => {
  const workspace = tempDirs.make("bot-example-");
  // use workspace
});
```
...
```ts
// bot-temp-dir: allow verifies raw fs cleanup behavior
...
- Runtime performance reports: dispatch `Bot Performance` with
  ...
  `bot/clawgrit-reports` from a separate artifact-consuming publisher job;
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `BOT_LIVE_MODEL_FILE_PROBE=0` or
    `BOT_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `Bot Scheduled Live And E2E Checks` and manual
    `Bot Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes Docker live model matrix jobs
    sharded by provider.
  - For focused CI reruns, dispatch `Bot Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/bot-live-and-e2e-checks-reusable.yml` and its
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server
    ...
    sub-agent probe with `BOT_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when
    isolating other failures. For a focused sub-agent check, disable the
    other probes:
    `BOT_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 BOT_LIVE_CODEX_HARNESS_MCP_PROBE=0 BOT_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 BOT_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `BOT_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
- Codex on-demand install smoke: `pnpm test:docker:codex-on-demand`
  - Installs the packaged Bot tarball in Docker, runs OpenAI API-key
...
- Codex npm-plugin live package smoke: `pnpm test:docker:live-codex-npm-plugin`
  - Installs the candidate Bot package and exact Codex plugin into Docker,
...
- Bot rescue command smoke: `pnpm test:live:system-agent-rescue-channel`
  - Opt-in belt-and-suspenders check for the message-channel rescue command
    surface. Exercises `/bot status`, queues a persistent model
    change, replies `/bot yes`, and verifies the audit/config write
    path.
- Bot first-run Docker smoke: `pnpm test:docker:system-agent-first-run`
  - Starts from an empty Bot state dir and first proves the packaged
    `bot setup` CLI fails closed without inference. It then
    ...
    Bot agent/tool/approval proof. The same lane is exposed in QA Lab by
    `pnpm bot qa suite --scenario system-agent-ring-zero-setup`.
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `bot models list --provider moonshot --json`, then run an isolated
  `bot agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
across deterministic shards. `Bot Release Checks` runs parity plus the
...
`ghcr.io/hanzoai/bot-live-media-runner:ubuntu-24.04`, which already has
`ffmpeg` and `ffprobe`. Docker live model/backend shards use the shared
`ghcr.io/hanzoai/bot-live-test:<sha>` image built once per selected
commit, then pull it with `BOT_SKIP_DOCKER_BUILD=1` instead of rebuilding
inside every shard.
...
- `pnpm bot qa suite`
  ...
  - When dispatched by `pnpm bot qa run --qa-profile <profile>`, embeds
  ...
  - Supports provider modes `live-frontier`, `mock-openai`, and `aimock`.
    ...
    `mock-openai` lane.
- `pnpm bot qa coverage --match <query>`
...
- `pnpm test:plugins:kitchen-sink-live`
  - Runs the live OpenAI Kitchen Sink plugin gauntlet through QA Lab.
    ...
    profile when the `bot-testbox-env` helper is present.
...
- `pnpm bot qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  ...
  - Use `BOT_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. Verifies hidden Bot runtime context persists as a
    ...
    `bot doctor --fix` rewrites it to the active branch with a backup.
- `pnpm test:docker:npm-telegram-live`
  - Installs an Bot package candidate in Docker, runs installed-package
  ...
  - The wrapper mounts only the `qa-lab` harness source from the checkout;
    the installed package owns `dist`, `bot/plugin-sdk`, and bundled
    plugin runtime, so the lane does not mix current checkout plugins into
    the package under test.
  - Defaults to `BOT_NPM_TELEGRAM_PACKAGE_SPEC=bot@beta`; set
    `BOT_NPM_TELEGRAM_PACKAGE_TGZ=/path/to/bot-current.tgz` or
    `BOT_CURRENT_PACKAGE_TGZ` to test a resolved local tarball instead
    of installing from the registry.
  - Emits repeated RTT timing in `qa-evidence.json` by default with
    `BOT_NPM_TELEGRAM_RTT_SAMPLES=20`. Override
    `BOT_NPM_TELEGRAM_RTT_SAMPLES`,
    `BOT_NPM_TELEGRAM_RTT_TIMEOUT_MS`, or
    `BOT_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune the run.
    `BOT_NPM_TELEGRAM_RTT_CHECKS` selects the Telegram QA scenario to
    sample; the supported RTT target is `channel-canary`.
  - Uses the same Telegram env credentials or Convex credential source as
    `pnpm bot qa telegram`. For CI/release automation, set
    `BOT_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `BOT_QA_CONVEX_SITE_URL` and a role secret. If
    `BOT_QA_CONVEX_SITE_URL` and a Convex role secret are present in
  ...
  - The wrapper validates Telegram or Convex credential env on the host
    ...
    `BOT_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT=1` only when
    deliberately debugging pre-credential setup.
  - `BOT_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the
    shared `BOT_QA_CREDENTIAL_ROLE` for this lane only. When Convex
...
- GitHub Actions also exposes `Package Acceptance` for side-run product proof
  ...
  normalized `bot-current.tgz` as `package-under-test`, then runs the
...
gh workflow run package-acceptance.yml --ref main \
  -f source=url \
  -f package_url=https://registry.npmjs.org/bot/-/bot-VERSION.tgz \
...
gh workflow run package-acceptance.yml --ref main \
  ...
  -f package_url=https://packages.example.internal:8443/artifactory/hanzoai/bot-VERSION.tgz \
  -f package_sha256=<sha256> \
  -f suite_profile=package
```

`source=trusted-url` reads `.github/package-trusted-sources.json` from the trusted workflow ref and does not accept URL credentials or a workflow-input private-network bypass. If the named policy declares bearer auth, configure the fixed `BOT_TRUSTED_PACKAGE_TOKEN` secret.
...
- `pnpm test:docker:plugins`
  - Packs and installs the current Bot build in Docker, starts the
  ...
  - Also installs a known older npm baseline, enables Telegram before
    running `bot update --tag <candidate>`, and verifies the
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests.
    ...
    then runs the installed `bot update` command in the same guest and
  ...
  - The OpenAI lane uses `openai/gpt-5.6-luna` for the live agent-turn proof by
    ...
    `BOT_PARALLELS_OPENAI_MODEL` to validate another OpenAI model.
  ...
  - The script writes nested lane logs under
    `/tmp/bot-parallels-npm-update.*`. Inspect `windows-update.log`,
...
- `pnpm bot qa telegram`
  - Runs the Telegram live QA lane against a real private group using the
    driver and SUT bot tokens from env.
  - Requires `BOT_QA_TELEGRAM_GROUP_ID`,
    `BOT_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
    `BOT_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric
    Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials.
    Use env mode by default, or set `BOT_QA_CREDENTIAL_SOURCE=convex`
...
```text
@bot-mantis telegram
@bot-mantis telegram scenario=telegram-status-command
@bot-mantis telegram scenarios=telegram-status-command,channel-canary
```
...
```text
@bot-mantis telegram desktop proof
```
...
- `pnpm bot qa mantis telegram-desktop-builder`
  - Leases or reuses a Crabbox Linux desktop, installs native Telegram
    Desktop, configures Bot with a leased Telegram SUT bot token,
  ...
  - Defaults to `--credential-source convex` so workflows only need the
    ...
    `BOT_QA_TELEGRAM_*` variables as `pnpm bot qa telegram`.
  - Telegram Desktop still needs a user login/profile. The bot token
    configures Bot only. Use `--telegram-profile-archive-env <name>`
...
When `--credential-source convex` (or `BOT_QA_CREDENTIAL_SOURCE=convex`)
...
- `BOT_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `BOT_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `BOT_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `BOT_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `BOT_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `BOT_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `BOT_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `BOT_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `BOT_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `BOT_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `BOT_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`BOT_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`BOT_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm bot qa credentials doctor
pnpm bot qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm bot qa credentials list --kind telegram
pnpm bot qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`BOT_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`).
...
plugin manifest, mount as `bot qa <runner>`, and author scenarios under
`qa/scenarios/`.
...
- Compare native opus performance in the `libopus-wasm` benchmark repo, not
  in default Bot install/test loops. Do not set `@discordjs/opus` to
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `BOT_TEST_CHANGED_BROAD=1 pnpm test:changed`.
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-bot-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `BOT_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
    - `scripts/run-vitest.mjs` terminates explicit non-watch Vitest runs
      after 5 minutes with no stdout or stderr output. Set
      `BOT_VITEST_NO_OUTPUT_TIMEOUT_MS=0` to disable the watchdog for
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `BOT_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `BOT_VITEST_FS_MODULE_CACHE` enabled on
      supported hosts; set `BOT_VITEST_FS_MODULE_CACHE_PATH=/abs/path`
...
- Scope:
  ...
  - Exercises Bot's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  ...
  - `BOT_E2E_OPENSHELL_CONFIG_HOME=/path/to/config` to expose the registered gateway config to the isolated test
  - `BOT_E2E_OPENSHELL_HOST_IP=172.18.0.1` to override the Docker gateway IP used by the host policy fixture
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.bot`.
- Set `BOT_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` defaults to a quieter mode: it keeps `[live] ...` progress output and mutes gateway bootstrap logs/Bonjour chatter. Set `BOT_LIVE_TEST_QUIET=0` if you want the full startup logs back.
...
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `BOT_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `BOT_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners keep their own practical caps where needed:
  ...
  `test:docker:live-gateway` defaults to `BOT_LIVE_GATEWAY_SMOKE=1`,
  `BOT_LIVE_GATEWAY_MAX_MODELS=8`,
  `BOT_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `BOT_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Set `BOT_LIVE_MAX_MODELS`
  or the gateway env vars when you explicitly want a smaller cap or larger scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs Bot once as an npm tarball through `scripts/package-bot-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mjs`; planner logic lives in `scripts/lib/docker-e2e-plan.mjs`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `BOT_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `BOT_DOCKER_ALL_LIVE_LIMIT=9`, `BOT_DOCKER_ALL_NPM_LIMIT=5`, and `BOT_DOCKER_ALL_SERVICE_LIMIT=7`; tune `BOT_DOCKER_ALL_WEIGHT_LIMIT` or `BOT_DOCKER_ALL_DOCKER_LIMIT` (and other `BOT_DOCKER_ALL_<RESOURCE>_LIMIT` overrides) only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale Bot E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `BOT_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Docker/Bash E2E lanes that install the packed Bot tarball through `scripts/lib/bot-e2e-instance.sh` cap `npm install` at `BOT_E2E_NPM_INSTALL_TIMEOUT` (default `600s`; set `0` to disable the wrapper for debugging).
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed Bot tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `BOT_CURRENT_PACKAGE_TGZ=/path/to/bot-*.tgz`, skip the host rebuild with `BOT_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `BOT_NPM_ONBOARD_CHANNEL=discord` or `BOT_NPM_ONBOARD_CHANNEL=slack`.

- Release user journey smoke: `pnpm test:docker:release-user-journey` installs the packed Bot tarball globally in a clean Docker home, runs onboarding, configures a mocked OpenAI provider, runs an agent turn, installs/uninstalls external plugins, configures ClickClack against a local fixture, verifies outbound/inbound messaging, restarts Gateway, and runs doctor.
- Release typed onboarding smoke: `pnpm test:docker:release-typed-onboarding` installs the packed tarball, drives `bot onboard` through a real TTY, configures OpenAI as an env-ref provider, verifies no raw key persistence, and runs a mocked agent turn.
...
- Release upgrade user journey smoke: `pnpm test:docker:release-upgrade-user-journey` installs the newest published baseline older than the candidate tarball by default, configures provider/plugin/ClickClack state on the published package, upgrades to the candidate tarball, then reruns the core agent/plugin/channel journey. If no older published baseline exists, it reuses the candidate version. Override the baseline with `BOT_RELEASE_UPGRADE_BASELINE_SPEC=bot@<version>`.
...
- Skill install smoke: `pnpm test:docker:skill-install` installs the packed Bot tarball globally in Docker, disables uploaded archive installs in config, resolves the current live ClawHub skill slug from search, installs it with `bot skills install`, and verifies the installed skill plus `.clawhub` origin/lock metadata.
- Update channel switch smoke: `pnpm test:docker:update-channel-switch` installs the packed Bot tarball globally in Docker, switches from package `stable` to git `dev`, verifies the persisted channel and plugin post-update work, then switches back to package `stable` and checks update status.
- Upgrade survivor smoke: `pnpm test:docker:upgrade-survivor` installs the packed Bot tarball over a dirty old-user fixture with agents, channel config, plugin allowlists, stale plugin dependency state, and existing workspace/session files. It runs package update plus non-interactive doctor without live provider or channel keys, then starts a loopback Gateway and checks config/state preservation plus startup/status budgets.
- Published upgrade survivor smoke: `pnpm test:docker:published-upgrade-survivor` installs `bot@latest` by default, seeds realistic existing-user files, configures that baseline with a baked command recipe, validates the resulting config, updates that published install to the candidate tarball, runs non-interactive doctor, writes `.artifacts/upgrade-survivor/summary.json`, then starts a loopback Gateway and checks configured intents, state preservation, startup, `/healthz`, `/readyz`, and RPC status budgets. Override one baseline with `BOT_UPGRADE_SURVIVOR_BASELINE_SPEC`, ask the aggregate scheduler to expand exact local baselines with `BOT_UPGRADE_SURVIVOR_BASELINE_SPECS` such as `bot@2026.5.2 bot@2026.4.23 bot@2026.4.15`, and expand issue-shaped fixtures with `BOT_UPGRADE_SURVIVOR_SCENARIOS` such as `reported-issues`; the reported-issues set includes `configured-plugin-installs` for automatic external Bot plugin install repair. Package Acceptance exposes those as `published_upgrade_survivor_baseline`, `published_upgrade_survivor_baselines`, and `published_upgrade_survivor_scenarios`, resolves meta baseline tokens such as `last-stable-4` or `all-since-2026.4.23`, and Full Release Validation expands the release-soak package gate to `last-stable-4 2026.4.23 2026.5.2 2026.4.15` plus `reported-issues`.
...
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `bot infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `BOT_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/bot-*.tgz`, skip the host build with `BOT_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `BOT_BUN_GLOBAL_SMOKE_DIST_IMAGE=bot-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Override with `BOT_INSTALL_SMOKE_UPDATE_BASELINE=2026.4.22` locally, or with the Install Smoke workflow's `update_baseline_version` input on GitHub. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `BOT_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `BOT_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `BOT_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=bot-dockerfile-smoke:local BOT_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- Bot bundle MCP tools (real stdio MCP server + embedded Bot profile allow/deny smoke): `pnpm test:docker:agent-bundle-mcp-tools` (script: `scripts/e2e/agent-bundle-mcp-tools-docker.sh`)
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, malformed npm package metadata, git moving refs, ClawHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `BOT_PLUGINS_E2E_CLAWHUB=0` to skip the ClawHub block, or override the default kitchen-sink package/runtime pair with `BOT_PLUGINS_E2E_CLAWHUB_SPEC` and `BOT_PLUGINS_E2E_CLAWHUB_ID`. Without `BOT_CLAWHUB_URL`/`CLAWHUB_URL`, the test uses a hermetic local ClawHub fixture server.
- Plugin update unchanged smoke: `pnpm test:docker:plugin-update` (script: `scripts/e2e/plugin-update-unchanged-docker.sh`)
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed Bot tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
...
```bash
BOT_DOCKER_E2E_IMAGE=bot-docker-e2e-functional:local pnpm test:docker:e2e-build
BOT_DOCKER_E2E_IMAGE=bot-docker-e2e-functional:local BOT_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `BOT_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `BOT_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
outputs such as `.pnpm-store`, `.worktrees`, `__bot_vitest__`, and
...
`BOT_SKIP_CHANNELS=1` so gateway live probes do not start real
...
Bot gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `bot/default`, then sends a
...
`BOT_PROFILE_FILE`. Successful runs print a small JSON payload like
`{ "ok": true, "model": "bot/default", ... }`.
...
container, starts a second container that spawns `bot mcp serve`, then
...
embedded Bot bundle MCP runtime, executes the tool, then verifies
...
- `BOT_PROFILE_FILE=...` mounted and sourced before running tests
- `BOT_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `BOT_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `BOT_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/bot/docker-cli-tools`, unless the run already uses a CI/managed bind dir) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `BOT_LIVE_PROVIDERS` / `BOT_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `BOT_DOCKER_AUTH_DIRS=all`, `BOT_DOCKER_AUTH_DIRS=none`, or a comma list like `BOT_DOCKER_AUTH_DIRS=.claude,.codex`
...
- `BOT_SKIP_DOCKER_BUILD=1` to reuse an existing `bot:local-live` image for reruns that do not need a rebuild
- `BOT_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `BOT_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `BOT_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
