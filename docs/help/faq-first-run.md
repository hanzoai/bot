...
read_when:
  ...
  - Cannot access docs.bot.ai, cannot open dashboard, install stuck
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    **local config or environment issues** a remote helper cannot inspect, so this beats
    ...
    | Command | Shows |
    | --- | --- |
    | `bot status` | Gateway/agent health + basic config snapshot |
    | `bot status --all` | Full read-only diagnosis, pasteable |
    | `bot models status` | Provider auth + model availability |
    | `bot doctor` | Validates and repairs common config/state issues |
    | `bot logs --follow` | Live log tail |
    | `bot gateway status --deep` | Deep gateway/config/plugin health check |
    | `bot health --verbose` | Detailed health report |

    Found a real bug or fix? File an issue or send a PR:
    [Issues](https://github.com/hanzoai/bot/issues) /
    [Pull requests](https://github.com/hanzoai/bot/pulls).
  ...
  <Accordion title="Heartbeat keeps skipping. What do the skip reasons mean?">
    ...
    Older heartbeat `tasks:` blocks migrate to independently scheduled cron jobs with `bot doctor --fix`.

    Docs: [Heartbeat](/gateway/heartbeat), [Automation](/automation).

  </Accordion>

  <Accordion title="Recommended way to install and set up Bot">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash
    ...
    No global install yet? Run `pnpm bot onboard` instead. If Control UI assets are
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `BOT_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `BOT_GATEWAY_PASSWORD`).
    - No shared secret configured yet? Run `bot doctor --generate-gateway-token` (or `bot doctor --fix --generate-gateway-token`).

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `bot gateway --tailscale serve`, open `https://<magicdns>/`. With `gateway.auth.allowTailscale: true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes a trusted gateway host); HTTP APIs still need shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `bot gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - When a supported native channel can infer approvers safely, Bot auto-enables DM-first native approvals if `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="What runtime do I need?">
    ...
    Bun can install dependencies and run package scripts, but it cannot run the Bot CLI or Gateway because it lacks `node:sqlite`.
  </Accordion>
  ...
  <Accordion title="It is stuck on wake up my friend / onboarding will not hatch. What now?">
    ...
    TUI without sending anything — add a provider with `bot configure --section model`.
  ...
  <Accordion title="Can I migrate my setup to a new machine without redoing onboarding?">
    ...
    **memory + bootstrap files**, but not session history or auth. Those live under
    `~/.bot/` (for example `~/.bot/agents/<agentId>/agent/bot-agent.sqlite`).
  ...
  <Accordion title="Cannot access docs.bot.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.bot.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.bot.ai`, then retry. Help us
  ...
  <Accordion title="Difference between stable and beta">
    ...
    **same version** after promotion.
    ...
    See what changed: [CHANGELOG.md](https://github.com/hanzoai/bot/blob/main/CHANGELOG.md).
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published to npm it uses dist-tag `dev`.
    ...
    Windows installer (PowerShell): `iwr -useb https://bot.ai/install.ps1 | iex`
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --verbose
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --beta --verbose
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --install-method git --verbose
    ```
  ...
  <Accordion title="Windows install says git not found or bot not recognized">
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    Still reproducing this on latest Bot? Track/report it: [Issue #30640](https://github.com/hanzoai/bot/issues/30640).
  ...
  <Accordion title="How do I install Bot on Linux?">
  ...
  <Accordion title="How do I install Bot on a VPS?">
  ...
  <Accordion title="Can I ask Bot to update itself?">
    ...
    bot update
    bot update status
    bot update --channel stable|extended-stable|beta|dev
  ...
  <Accordion title="What does onboarding actually do?">
    `bot onboard` is the recommended setup path. In **local mode** it walks through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. Run Bot with **API keys** (Anthropic/OpenAI/others) or **local-only models**
    ...
    Bot also supports hosted subscription-style options including **Qwen Cloud
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    Yes. Bot supports Claude CLI reuse for Pro/Max/Team/Enterprise plans. Anthropic
    currently treats the `claude -p` path Bot uses as subscription-plan usage subject
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic setup-token auth is also still a supported token path, but Bot prefers
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Set a **fallback model** so Bot keeps replying while a provider is rate-limited.
  ...
  <Accordion title="Is AWS Bedrock supported?">
    Yes. Bot has a bundled **Amazon Bedrock (Converse)** provider. With AWS env
    markers present (`AWS_ACCESS_KEY_ID`, `AWS_PROFILE`, `AWS_BEARER_TOKEN_BEDROCK`),
    Bot auto-enables the implicit Bedrock provider for model discovery; otherwise
  ...
  <Accordion title="How does Codex auth work?">
    Bot supports **OpenAI Codex** via OAuth (ChatGPT sign-in). A fresh
    ...
    `openai/gpt-5.5` explicitly; Bot does not silently downgrade. Legacy
    Codex-prefixed model refs are legacy config repaired by `bot doctor
  ...
  <Accordion title="Why does Bot still mention legacy OpenAI Codex prefix?">
    ...
    - Legacy `openai-codex/*` model refs = legacy route repaired by `bot doctor --fix`.
    - `openai/gpt-5.5` plus an ordered `openai` API-key profile = API-key auth for an OpenAI agent model.
    - Legacy `openai-codex` auth profile ids = legacy ids migrated by `bot doctor --fix`.

    Want direct OpenAI Platform billing? Set `OPENAI_API_KEY`. Want ChatGPT/Codex
    subscription auth? Run `bot models auth login --provider openai`. Keep
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    `bot models status` shows the currently visible provider usage/quota windows, but
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    ...
    tools/workflows like Bot. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    ...
    2. Enable the plugin: `bot plugins enable google`
    3. Login: `bot models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Bot needs large context + strong safety; small cards truncate context
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Bot runs on macOS or Linux (Windows via WSL2). A Mac mini is a popular
  ...
  <Accordion title="If I buy a Mac mini to run Bot, can I connect it to my MacBook Pro?">
    ...
    node host and pairs to the Gateway. Check with `bot nodes status` / `bot nodes list`.
  ...
  <Accordion title="Can I use Bun?">
    You can use Bun to install dependencies or run package scripts. The Bot CLI and
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    not the bot username. Setup asks for numeric user IDs only; `bot doctor --fix`
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Bot instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    Running Bot via systemd: make sure the service PATH includes
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes, with `bot update --channel ...` on an existing install. This does **not
    delete your data** - only the Bot code install changes. State (`~/.bot`) and
    workspace (`~/.bot/workspace`) stay untouched.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --install-method git
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --install-method npm
    ```
  ...
  <Accordion title="How important is it to run Bot on a dedicated machine?">
  ...
  <Accordion title="Can I run Bot in a VM and what are the requirements?">
