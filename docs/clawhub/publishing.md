...
An owner is a ClawHub publisher handle, such as `@alice` or `@bot`.
...
`@hanzo/bot-dronzer` can only be published as `@bot`. To publish as
...
```text
Package scope "@bot" must match selected owner "@vintageayu".
Publish as "@bot" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-bot --to opik
```
...
`@hanzo/bot-dronzer` claims the `@bot` namespace, so only publishers with
access to `@bot` can publish or transfer into it.
