...
Bot bundles it as the `together` provider.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    bot onboard --auth-choice together-api-key
    ```
  </Step>
...
<AccordionGroup>
  <Accordion title="Environment note">
    ...
    gateway processes. Use `~/.bot/.env` or `env.shellEnv` config for
  ...
  <Accordion title="Troubleshooting">
    - Verify your key works: `bot models list --provider together`
