...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in Bot
---

ClawRouter gives Bot one policy-scoped key for multiple upstream model
...
| Default URL   | `https://clawrouter.bot.ai`         |
...
<Steps>
  ...
  <Step title="Configure Bot">
    ...
    bot onboard --auth-choice clawrouter-api-key
    bot plugins enable clawrouter
    ```
    ...
    ClawRouter origin; the default is `https://clawrouter.bot.ai`.

  </Step>
  <Step title="List granted models">
    ```bash
    bot models list --all --provider clawrouter
    ```
  ...
  <Step title="Select a model">
    ```bash
    bot models set clawrouter/<provider>/<model>
    ```

    You can also select a returned model for one run with
    `bot agent --model clawrouter/<provider>/<model> --message "..."`.
...
```bash
bot config patch --file ./clawrouter.patch.json5 --dry-run --json
bot config patch --file ./clawrouter.patch.json5
```
...
bot models list --all --provider clawrouter --json

# Minimal real inference probe through the configured ClawRouter provider.
bot models status --probe --probe-provider clawrouter --probe-max-tokens 8 --json
...
`X-Request-ID`, so an Bot model-call event can be joined to ClawRouter's
...
ClawRouter models. A catalog model is advertised as an Bot model when:
...
Adding a model to a supported ClawRouter provider needs no Bot release:
...
ClawRouter owns upstream credentials; its catalog tells Bot which
...
ClawRouter's `/v1/usage` response feeds the normal Bot provider-usage
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter credential; Bot does not fall back to upstream provider keys.                                          |
