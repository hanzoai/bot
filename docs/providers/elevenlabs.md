---
summary: "Use ElevenLabs speech, Scribe STT, and realtime transcription with Bot"
read_when:
  - You want ElevenLabs text-to-speech in Bot
...
Bot uses ElevenLabs for text-to-speech, batch speech-to-text with Scribe
...
| Capability               | Bot surface                                                     | Default                  |
| ------------------------ | -------------------------------------------------------------------- | ------------------------ |
...
Set `modelId` to `eleven_v3` to use ElevenLabs v3 TTS. Bot keeps
...
returned audio stream instead of waiting for Bot to download the whole
audio file first. `latencyTier` maps to ElevenLabs' `optimize_streaming_latency`
query parameter for models that accept it; Bot omits that parameter for
...
Bot sends multipart audio to ElevenLabs `/v1/speech-to-text` with
