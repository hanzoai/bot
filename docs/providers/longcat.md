...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    bot onboard --auth-choice longcat-api-key
    ```
  </Step>
  <Step title="Verify the model">
    ```bash
    bot models list --provider longcat
    ```
  </Step>
</Steps>
...
LongCat exposes binary thinking control. Bot maps enabled thinking levels
to `thinking: { type: "enabled" }` and `/think off` to
`thinking: { type: "disabled" }`. LongCat does not currently document
`reasoning_effort`, so Bot does not send it.

LongCat returns reasoning in `reasoning_content`. Bot preserves that field
...
model through an OpenAI-compatible runtime and use Bot's existing
...
<AccordionGroup>
  <Accordion title="The key works in a shell but not in the Gateway">
    ...
    variable. Put `LONGCAT_API_KEY` in `~/.bot/.env`, configure it through
  ...
  <Accordion title="The model does not appear">
    Run `bot plugins list` and confirm the `longcat` plugin is
    enabled, then run `bot models list --provider longcat`.
  </Accordion>
</AccordionGroup>
