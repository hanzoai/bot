---
summary: "Use Amazon Bedrock (Converse API) models with Bot"
read_when:
  - You want to use Amazon Bedrock models with Bot
  - You need AWS credential/region setup for model calls
title: "Amazon Bedrock"
---

Bot can use **Amazon Bedrock** models via its **Bedrock Converse**
...
<Tabs>
  <Tab title="Access keys / env vars">
    **Best for:** developer machines, CI, or hosts where you manage AWS credentials directly.
    ...
    <Tip>
    With env-marker auth (`AWS_ACCESS_KEY_ID`, `AWS_PROFILE`, or `AWS_BEARER_TOKEN_BEDROCK`), Bot auto-enables the implicit Bedrock provider for model discovery without extra config.
    </Tip>
    ...
    **Best for:** EC2 instances with an IAM role attached, using the instance metadata service for authentication.
    ...
    <Steps>
      <Step title="Enable discovery explicitly">
        When using IMDS, Bot cannot detect AWS auth from env markers alone, so you must opt in:

        ```bash
        bot config set plugins.entries.amazon-bedrock.config.discovery.enabled true
        bot config set plugins.entries.amazon-bedrock.config.discovery.region us-east-1
        ```
      </Step>
      <Step title="Optionally add an env marker for auto mode">
        If you also want the env-marker auto-detection path to work (for example, for `bot status` surfaces):
...
Bot can automatically discover Bedrock models that support **streaming**
...
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is `true`,
  Bot will try discovery even when no AWS env marker is present.
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is unset,
  Bot only auto-adds the
...
<Note>
For explicit `models.providers["amazon-bedrock"]` entries, Bot can still resolve Bedrock env-marker auth early from AWS env markers such as `AWS_BEARER_TOKEN_BEDROCK` without forcing full runtime auth loading. The actual model-call auth path still uses the AWS SDK default chain.
</Note>
...
<AccordionGroup>
  <Accordion title="Discovery config options">
    ...
    | ------ | ------- | ----------- |
    | `enabled` | auto | In auto mode, Bot only enables the implicit Bedrock provider when it sees a supported AWS env marker. Set `true` to force discovery. |
  ...
  <Accordion title="Context window and max-token limits">
    ...
    status. Bot ships a lookup table of known context windows and output
...
the instance profile, and enables Bot discovery on the EC2 host.
...
<AccordionGroup>
  <Accordion title="Inference profiles">
    Bot discovers **regional and global inference profiles** alongside
    ...
    first in `bot models list` since they generally offer better capacity
    ...
    foundation models in `bot models list`.
  ...
  <Accordion title="Service tier">
    ...
    Fable 5, Opus 5, and Sonnet 5 only support the `default` tier; Bot warns and
  ...
  <Accordion title="Claude Opus 5, 4.8, and 4.7 temperature">
    ...
    and Opus 4.7. Bot omits `temperature` automatically for any matching Bedrock
  ...
  <Accordion title="Claude Opus 5">
    ...
    Bot applies the 1,000,000-token context window, 128,000-token output
    ...
    `/think xhigh|max` keeps adaptive thinking enabled. Bot omits custom
  ...
  <Accordion title="Claude Fable 5">
    ...
    Bot applies Fable's 1M context window, 128K output limit, always-on
  ...
  <Accordion title="Claude Mythos 5">
    ...
    required limited-access approval. Bot recognizes the foundation model
  ...
  <Accordion title="Claude Sonnet 5">
    ...
    Bot recognizes the Bedrock foundation model
    ...
    Bedrock keeps adaptive thinking enabled for Sonnet 5. Bot defaults to
  ...
  <Accordion title="Notes and caveats">
    ...
    - Bot surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
