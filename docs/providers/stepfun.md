---
summary: "Use StepFun models with Bot"
read_when:
  - You want StepFun models in Bot
  - You need StepFun setup guidance
title: "StepFun"
---

StepFun ships as an external official plugin (`@hanzo/bot-stepfun-provider`) with two provider ids:
...
<Tabs>
  <Tab title="Standard">
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        bot onboard --auth-choice stepfun-standard-api-key-intl
        ```

        China endpoint:

        ```bash
        bot onboard --auth-choice stepfun-standard-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        bot models list --provider stepfun
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Step Plan">
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        bot onboard --auth-choice stepfun-plan-api-key-intl
        ```

        China endpoint:

        ```bash
        bot onboard --auth-choice stepfun-plan-api-key-cn
        ```
      </Step>
      <Step title="Non-interactive alternative">
        ```bash
      ...
      <Step title="Verify models are available">
        ```bash
        bot models list --provider stepfun-plan
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Notes">
    - `step-3.7-flash` accepts text and image input through Bot. StepFun's API also supports video, which is not yet a model input modality in Bot.
    ...
    - Use `bot models list` and `bot models set <provider/model>` to inspect or switch models.
