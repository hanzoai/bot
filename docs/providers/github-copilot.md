---
summary: "Sign in to GitHub Copilot from Bot using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `bot models auth login-github-copilot` flow
...
## Three ways to use Copilot in Bot

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain and store a GitHub token. When
    Bot runs, it validates Copilot access and resolves the account-specific
    ...
    <Steps>
      ...
      <Step title="Set a default model">
        ```bash
        bot models set github-copilot/claude-opus-5
        ```
  ...
  <Tab title="Copilot SDK harness plugin (copilot)">
    Install the external `@hanzo/bot-copilot` plugin when you want GitHub's
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    ...
    The `copilot-proxy` plugin ships with Bot and is enabled by default.
    Configure the base URL and model ids with:

    ```bash
    bot models auth login --provider copilot-proxy --set-default
    ```
...
<Steps>
  <Step title="Pick the Enterprise auth choice">
    ...
    **GitHub Copilot (Enterprise / data residency)**. You will be prompted for
    ...
    Bot derives those endpoints from the tenant root automatically.

    ```bash
    bot models auth login --provider github-copilot --method device-enterprise
    ```
...
tenant, or one tenant to another), Bot will not reuse the existing token —
...
`bot models auth login-github-copilot` shortcut, account validation,
...
| ---------------------------------------------------------------------- | --------------- | ---------------------------------------------------- |
| `bot models auth login-github-copilot`                            | `--yes`         | Overwrite an existing auth profile without prompting |
| `bot models auth login --provider github-copilot --method device` | `--set-default` | Also apply the provider's recommended default model  |
...
bot models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

The device-login flow requires an interactive TTY. For headless setup, import
an existing GitHub OAuth access token with `bot onboard --non-interactive`:
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    Bot refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    churn. Newly published Copilot models become visible without an Bot
  ...
  <Accordion title="Transport selection">
    ...
    models keep the OpenAI Responses transport. Bot selects the correct
    transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    Bot sends Copilot-compatible request headers with a Copilot CLI request
  ...
  <Accordion title="Environment variable resolution order">
    Bot resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Bot uses the highest-priority one.
    The device-login flow (`bot models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    `github-copilot:github`). At runtime, Bot validates Copilot access,
...
have logged in, Bot can use it for embeddings without a separate API key.

### Config

Set `memory.search.provider` explicitly to use GitHub Copilot embeddings. If a
GitHub token is available, Bot discovers available embedding models from
...
1. Bot resolves your GitHub token (from env vars or auth profile).
...
available, Bot skips Copilot and tries the next provider.
