---
summary: "Use NovitaAI's OpenAI-compatible API with Bot"
read_when:
  - You want to run Bot with NovitaAI models
...
It ships as a bundled Bot provider (no separate plugin install), so
...
```bash
bot onboard --auth-choice novita-api-key
```
...
```bash
bot models list --provider novita
```
...
- `401`/`403`: verify the key in Novita's key management page and re-run
  `bot onboard --auth-choice novita-api-key` if the stored profile is
  stale.
