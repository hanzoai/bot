...
<AccordionGroup>
  ...
  <Accordion title="Realtime voice">
    ...
    `echo`, `marin`, `sage`, `shimmer`, and `verse`. Bot defaults to
    `marin` and maps unknown or unsupported configured voices back to it.

    Prerequisites, in order:

    1. A ChatGPT OAuth auth profile: `bot models auth login --provider openai`.
       An existing Codex CLI (`~/.codex`) sign-in is **not** read; the profile
       must exist in Bot. A Platform API key with `/v1/live` access works
    ...
    delegated work through the configured Bot agent; the browser never
    receives the OAuth token.

    The canonical Bot path creates the call on `api.openai.com/v1/live`
    and joins its sideband there. The legacy `chatgpt.com` backend route returns
    `403` and is not used.

    Maintainers can exercise Bot's complete OAuth path with the opt-in
    live test. It skips when no ChatGPT OAuth credential is available and
    never prints token material:

    ```bash
    BOT_LIVE_TEST=1 BOT_LIVE_GPT_LIVE=1 node scripts/run-vitest.mjs run --config test/vitest/vitest.live.config.ts extensions/openai/realtime-quicksilver.live.test.ts
    ```
