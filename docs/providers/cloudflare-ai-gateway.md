...
read_when:
  - You want to use Cloudflare AI Gateway with Bot
  - You need the account ID, gateway ID, or API key env var
---

[Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/) sits in front of provider APIs and adds analytics, caching, and controls. For Anthropic, Bot uses the Anthropic Messages API through your Gateway endpoint.

| Property      | Value                                                                                    |
| ------------- | ---------------------------------------------------------------------------------------- |
| Provider      | `cloudflare-ai-gateway`                                                                  |
| Plugin        | official external package (`@hanzo/bot-cloudflare-ai-gateway-provider`)                   |
...
When thinking is enabled for Anthropic Messages models, Bot strips trailing
...
```bash
bot plugins install @hanzo/bot-cloudflare-ai-gateway-provider
bot gateway restart
```
...
<Steps>
  <Step title="Set the provider API key and Gateway details">
    ...
    ```bash
    bot onboard --auth-choice cloudflare-ai-gateway-api-key
    ```
  ...
  <Step title="Set a default model">
    Add the model to your Bot config:
  ...
  <Step title="Verify the model is available">
    ```bash
    bot models list --provider cloudflare-ai-gateway
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    <Warning>
    A key exported only in an interactive shell will not help a launchd/systemd daemon unless that environment is imported there as well. Set the key in `~/.bot/.env` or via `env.shellEnv` to ensure the gateway process can read it.
    </Warning>
