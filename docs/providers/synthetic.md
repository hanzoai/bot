---
summary: "Use Synthetic's Anthropic-compatible API in Bot"
read_when:
...
Bot bundles it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Warning>
Bot's Anthropic client appends `/v1` to the base URL automatically, so use
...
`bot models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Bot still appends `/v1` automatically.
