---
summary: "Use Qianfan's unified API to access many models in Bot"
read_when:
...
Qianfan is Baidu's MaaS platform: a unified, OpenAI-compatible API that routes requests to many models behind a single endpoint and API key. Bot ships it as the official external plugin `@hanzo/bot-qianfan-provider`.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    bot onboard --auth-choice qianfan-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    bot models list --provider qianfan
    ```
  </Step>
</Steps>
