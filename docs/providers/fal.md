---
summary: "fal image, video, and music generation setup in Bot"
title: "Fal"
read_when:
  - You want to use fal image generation in Bot
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate, video_generate, or music_generate
---

Bot ships a bundled `fal` provider for hosted image, video, and music
generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    bot onboard --auth-choice fal-api-key
    ```
...
Krea 2 models use fal's native Krea payload schema. Bot sends
...
`aspectRatio`; Bot maps `size` to the closest supported Krea aspect ratio
...
control in Bot, so `background: "transparent"` is reported as an ignored
override for fal models.
Krea 2 endpoints do not expose an `output_format` request field through fal, so
Bot rejects `outputFormat` overrides for Krea requests.
