...
providers. Route Bot through LiteLLM for centralized cost tracking, logging, virtual keys with
spend limits, and backend failover without changing Bot config.
...
<Tabs>
  <Tab title="Onboarding (recommended)">
    ```bash
    bot onboard --auth-choice litellm-api-key
    ```

    For non-interactive setup against a remote proxy, pass the proxy URL explicitly:

    ```bash
    bot onboard --non-interactive --accept-risk --auth-choice litellm-api-key \
  ...
  <Tab title="Manual setup">
    <Steps>
      ...
      <Step title="Point Bot to LiteLLM">
        ```bash
...
<AccordionGroup>
  ...
  <Accordion title="Proxy behavior notes">
    ...
    - Bot connects through LiteLLM's proxy-style OpenAI-compatible `/v1` endpoint.
    - Native-OpenAI-only request shaping does not apply through a configured LiteLLM base URL:
      ...
      payload shaping.
    - Hidden Bot attribution headers (`originator`, `version`, `User-Agent`) are only sent to
