---
summary: "Use Anthropic Claude via API keys or Claude CLI in Bot"
read_when:
...
Anthropic builds the **Claude** model family. Bot supports two auth routes:
...
Bot detects the available Anthropic credential and selects the matching usage surface:
...
Admin API cost history comes from Anthropic's [Usage and Cost API](https://platform.claude.com/docs/en/manage-claude/usage-cost-api). It is actual provider billing, separate from Bot's session-derived estimated cost.

<Warning>
Bot's Claude CLI backend runs the installed Claude Code CLI in
...
<Tabs>
    ...
    **Best for:** standard API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Verify the model is available">
        ```bash
        bot models list --provider anthropic
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Claude CLI">
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        Bot detects and reuses the existing Claude CLI credentials.
      </Step>
      <Step title="Verify the model is available">
        ```bash
        bot models list --provider anthropic
    ...
    <Warning>
    Claude CLI reuse expects the Bot process to run on the same host as the
    ...
    a provider with Bot-managed OAuth such as
    ...
    **Anthropic setup-token** under **Connect with an API key or token**, or use:
    ...
    ```bash
    bot models auth login --provider anthropic --method setup-token
    ```
    ...
    Bot uses Claude Code's non-interactive `claude -p` path for Claude CLI
    ...
    Bot release. Check `claude auth status`, `/status`, and
    ...
    Claude CLI. Bot also supports subscription-style options from
...
No additional Bot config is required for discovery. The Anthropic plugin
...
30 seconds while visible, so Claude sessions created outside Bot appear
...
`sessions.catalog.continue`. Bot re-resolves the local catalog record,
...
and its local `claude` executable resolves. Bot re-resolves the catalog
...
advertises `agent.cli.claude.run.v1`. Bot never modifies Claude Desktop
...
With an Anthropic API key configured, Bot refreshes the Claude catalog from
Anthropic's models endpoint, so newly published snapshots of supported model
families appear without an Bot release. Models the shipped catalog already
...
capabilities match the request shaping Bot would apply to it. A brand-new
model generation therefore stays hidden until Bot adds support for it,
...
`sonnet` tracks the current Sonnet. Upgrading Bot can therefore move a
...
higher native effort levels. Bot omits manual thinking budgets, custom
...
`/think off` and `/think minimal` map to `low` effort instead. Bot also
...
adaptive-thinking contract. Bot defaults to `high`, maps `/think off` and
...
Claude Opus 4.8 keeps thinking off by default in Bot. When you explicitly
enable adaptive thinking with `/think high|xhigh|max`, Bot sends
...
Claude model. Bot opts into Anthropic's recommended per-category routing
...
1. For every direct API-key request to `anthropic/claude-opus-5` or
   `anthropic/claude-fable-5`, Bot sends the
...
2. Only a safety-classifier decline triggers the fallback. Rate limits,
   ...
   Bot's normal [model failover](/concepts/model-failover).
...
need to be in your configured Bot fallback chain.
...
- Anthropic bills the fallback attempt at the serving model's rates. Bot
...
- A mid-stream decline additionally bills the already-streamed primary-model partial
  ...
  usage but not folded into Bot's per-turn estimate.
...
Bot supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    For Claude Opus 5 and Opus 4.8, Bot's shared `/fast` toggle uses
    ...
    <Note>
    - Native fast mode is a research preview for Claude Opus 5 and Opus 4.8. It can deliver up to 2.5x higher output-token throughput and is billed at `$10/$50` per million input/output tokens. Bot applies the same 2x multiplier to cache pricing in its cost estimate.
    ...
    - Claude Sonnet 5 supports neither native fast mode nor Priority Tier, so Bot omits both fields.
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. Bot
    ...
    When an image or PDF is attached to a conversation, Bot automatically
  ...
  <Accordion title="1M context window">
    ...
    thinking: Opus 4.8,
    Opus 4.7, Opus 4.6, and Sonnet 4.6. Bot sizes these models
    ...
    these models and Bot no longer sends the retired
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but Bot strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    Anthropic auth is **per agent**; new agents do not inherit the main agent's keys. Re-run onboarding for that agent (or configure an API key on the gateway host), then verify with `bot models status`.
  </Accordion>

  <Accordion title='No credentials found for profile "anthropic:default"'>
    Run `bot models status` to see which auth profile is active. Re-run onboarding, or configure an API key for that profile path.
  </Accordion>

  <Accordion title="No available auth profile (all in cooldown)">
    Check `bot models status --json` for `auth.unusableProfiles`. Anthropic rate-limit cooldowns can be model-scoped, so a sibling Anthropic model may still be usable. Add another Anthropic profile or wait for cooldown.
  </Accordion>
</AccordionGroup>
