...
**claude-max-api-proxy** is a community npm package (not an Bot plugin) that
...
plan articles, and [Anthropic provider](/providers/anthropic) for Bot's
...
<Steps>
  ...
  <Step title="Configure Bot">
    Point Bot at the proxy as a custom OpenAI-compatible endpoint:
...
<Note>
The model ids below are the proxy's own catalog, not Bot's Anthropic
...
<AccordionGroup>
  <Accordion title="Proxy-style OpenAI-compatible notes">
    This uses Bot's generic custom `/v1` OpenAI-compatible route, the same
    ...
    - No Responses `store`, prompt-cache hints, or OpenAI reasoning-compat
      payload shaping.
    - Bot's OpenAI/Codex attribution headers (`originator`, `version`,
...
<CardGroup cols={2}>
  <Card title="Anthropic provider" href="/providers/anthropic" icon="bolt">
    Native Bot integration with Claude CLI or API keys.
  </Card>
