...
| Package       | `@hanzo/bot-vercel-ai-gateway-provider` |
...
<Tip>
Bot auto-discovers the Gateway `/v1/models` catalog, so both the
`/models vercel-ai-gateway` chat command and
`bot models list --provider vercel-ai-gateway` include current model
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    bot plugins install @hanzo/bot-vercel-ai-gateway-provider
    ```
  </Step>
  ...
  <Step title="Verify the model is available">
    ```bash
    bot models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Bot normalizes Claude shorthand model refs at runtime:
...
<Tip>
Use either form in your configuration; Bot resolves the canonical
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Bot Gateway runs as a daemon (launchd/systemd), make sure
    ...
    the key in `~/.bot/.env` or via `env.shellEnv` to ensure the gateway
    process can read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow the upstream model prefix when Bot recognizes
