---
summary: "Expose Bot channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Bot-backed channels
  - Running `bot mcp serve`
  - Managing Bot-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`bot mcp` has two jobs:

- run Bot as an MCP server with `bot mcp serve`
- manage Bot-managed outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

`serve` is Bot acting as an MCP server. The other subcommands are Bot acting as an MCP client-side registry for servers its own runtimes may consume later.

<Note>
  `list`, `show`, `set`, and `unset` only read and write Bot-managed `mcp.servers` entries in Bot config. They do not include mcporter servers from `config/mcporter.json`; use `mcporter list` for that registry.
</Note>

Use [`bot acp`](/cli/acp) when Bot should host a coding harness session itself and route that runtime through ACP.

## Choose the right MCP path

| Goal                                                                | Use                                                                  | Why                                                                                                             |
| ------------------------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Let an external MCP client read/send Bot channel conversations | `bot mcp serve`                                                 | Bot is the MCP server and exposes Gateway-backed conversations over stdio.                                 |
| Save third-party MCP servers for Bot-managed agent runs        | `bot mcp add`, `set`, `configure`, `tools`, `login`             | Bot is the MCP client-side registry and later projects those servers into eligible runtimes.               |
| Check a saved server without running an agent turn                  | `bot mcp status`, `doctor`, `probe`                             | `status` and `doctor` inspect config; `probe` opens a live MCP connection and lists capabilities.               |
...
| Run ACP-hosted harness sessions                                     | [`bot acp`](/cli/acp) and [ACP Agents](/tools/acp-agents-setup) | ACP bridge mode does not accept per-session MCP server injection; configure gateway/plugin bridges instead.     |

<Tip>
If you are not sure which path you need, start with `bot mcp status --verbose`. It shows what Bot has saved without starting any MCP servers.
</Tip>

## Bot as an MCP server

This is the `bot mcp serve` path.

### When to use serve

Use `bot mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to Bot-backed channel conversations
- you already have a local or remote Bot Gateway with routed sessions
- you want one MCP server that works across Bot's channel backends instead of running separate per-channel bridges

Use [`bot acp`](/cli/acp) instead when Bot should host the coding runtime itself and keep the agent session inside Bot.

### How it works

`bot mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote Bot Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `bot mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the Bot Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `bot agent` and `bot infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by Bot (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when Bot already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    bot mcp serve --url wss://gateway-host:18789 --token-file ~/.bot/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    bot mcp serve --url wss://gateway-host:18789 --password-file ~/.bot/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    bot mcp serve --verbose
    bot mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the Bot equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying Bot channel configuration
...
Bot ships a deterministic Docker smoke for this bridge:

```bash
pnpm test:docker:mcp-channels
```

That smoke runs a single container: it seeds conversation state, starts the Gateway, then spawns `bot mcp serve` as a stdio child process and drives it as an MCP client. It verifies conversation discovery, transcript reads, attachment metadata reads, live event queue behavior, and Claude-style channel and permission notifications over the real stdio MCP bridge. Outbound send routing (`messages_send` reusing the stored conversation route) is covered separately by unit tests in `src/mcp/channel-server.test.ts`.
...
## Bot as an MCP client registry

This is the `bot mcp list`, `show`, `status`, `doctor`, `probe`, `add`, `set`,
`configure`, `tools`, `login`, `logout`, `reload`, and `unset` path.

These commands do not expose Bot over MCP. They manage Bot-managed MCP server definitions under `mcp.servers` in Bot config. They do not read mcporter servers from `config/mcporter.json`.

Those saved definitions are for runtimes that Bot launches or configures later, such as embedded Bot and other runtime adapters. Bot stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write Bot config
    ...
    - embedded Bot exposes configured MCP tools in normal `coding` and `messaging` tool profiles; `minimal` still hides them, and `tools.deny: ["bundle-mcp"]` disables them explicitly
    - per-server `toolFilter.include` and `toolFilter.exclude` filter discovered MCP tools before they become Bot tools
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Bot consumes Bot `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
Bot projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific Bot
...
Bot strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.
...
- `bot mcp list`
- `bot mcp show [name]`
- `bot mcp status [--verbose]`
- `bot mcp doctor [name] [--probe]`
- `bot mcp probe [name]`
- `bot mcp add <name> [flags]`
- `bot mcp set <name> <json>`
- `bot mcp configure <name> [flags]`
- `bot mcp tools <name> [--include csv] [--exclude csv] [--clear]`
- `bot mcp login <name> [--code code]`
- `bot mcp logout <name>`
- `bot mcp reload`
- `bot mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `bot mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
bot mcp list
bot mcp show context7 --json
bot mcp status --verbose
bot mcp doctor --probe
bot mcp probe context7 --json
bot mcp add memory --command npx --arg -y --arg @modelcontextprotocol/server-memory
bot mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
bot mcp tools context7 --include 'resolve-library-id,get-library-docs'
bot mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
bot mcp configure docs --timeout 20 --connect-timeout 5 --include 'search,read_*'
bot mcp configure docs --auth oauth --oauth-scope 'docs.read'
bot mcp login docs
bot mcp logout docs
bot mcp unset context7
```

### Common server recipes

These examples save server definitions only. Run `bot mcp doctor --probe` afterward to prove that the server starts and exposes tools.
...
<Tabs>
  ...
  <Tab title="Desktop/CUA">
    ...
    bot mcp tools cua-driver --include 'list_apps,get_window_state,click,type_text'
    bot mcp doctor cua-driver --probe
    ```
...
<AccordionGroup>
  <Accordion title="status --json">
    ```json
    {
      "path": "/home/user/.hanzoai/bot.json",
      "servers": [
        {
          "name": "docs",
          "configured": true,
      ...
      "servers": [
        {
          ...
          "issues": [
            {
              "level": "warning",
              "message": "OAuth credentials are not authorized; run bot mcp login docs"
            }
          ]
        }
      ]
...
| `auth: "oauth"`             | Use MCP OAuth credentials saved by `bot mcp login`          |
...
Sensitive values in `url` (userinfo) and `headers` are redacted in logs and status output. `bot mcp doctor` warns when sensitive-looking `headers` or `env` entries contain literal values, so operators can move those values out of committed config.

### OAuth workflow

OAuth is for HTTP MCP servers that advertise the MCP OAuth flow. Static `Authorization` headers are ignored for a server while `auth: "oauth"` is enabled. Credentials saved by `bot mcp login` work with embedded MCP, CLI runners, and the local Codex app-server.

Native MCP OAuth sessions live in the owner-only shared SQLite database at `<state-dir>/state/bot.sqlite` (`mcp_oauth_stores`). The row can contain access and refresh tokens, dynamic client registration secrets, discovery metadata, and the temporary PKCE verifier. Refresh, login, and logout use the same SQLite lease, so parallel Bot processes cannot consume one refresh token or resurrect a logged-out session.

Upgrades from the retired `<state-dir>/mcp-oauth/*.json` store are handled only by `bot doctor --fix`. Runtime code never reads, writes, or falls back to those files.

Until credentials are available, Bot omits only that MCP server from the agent runtime instead of failing the agent turn. The operator, or an agent with shell access, can then run `bot mcp login <name>` and use the server on a later turn.

If a server rejects a token with `insufficient_scope`, Bot preserves the requested scope and asks for `bot mcp login <name>` instead of repeating a refresh that cannot grant new scope. That login starts a new authorization request while keeping the previous token until replacement credentials are saved.

When a remote MCP service is already backed by a separate Bot refresh-capable auth profile, you can optionally set `oauth.authProfileId`. Bot refreshes either credential source before runtime projection and passes only the current access token to the downstream MCP client.
...
<Steps>
  <Step title="Save the server">
    ...
    ```bash
    bot mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"scope":"docs.read"}}'
    ```

    For an auth-profile-backed bearer, save the profile binding:

    ```bash
    bot mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"authProfileId":"docs:mcp"}}'
    ```
  ...
  <Step title="Start login">
    ...
    Bot prints the authorization URL and stores temporary OAuth verifier state in shared SQLite.

  </Step>
  <Step title="Finish with the code">
    After approving in the browser, pass the returned code back to Bot.

    ```bash
    bot mcp login docs --code abc123
    ```

  </Step>
  <Step title="Check authorization">
    Use status or doctor to confirm that tokens are present and do not require additional authorization. If status reports `authorization-required` or doctor asks for additional authorization, run `bot mcp login <name>` again.
...
If the provider rotates tokens or the authorization state gets stuck, run `bot mcp logout <name>`, then repeat `login`. `logout` can clear credentials for a saved HTTP server even after `auth: "oauth"` has been removed from config, as long as the server name and URL still identify the credential store entry.
...
| --------------------------- | -------------------------------------------------------------------------------------- |
| `url`                       | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`                 | Set to `"streamable-http"` to select this transport; when omitted, Bot uses `sse` |
| `headers`                   | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs`       | Per-server connection timeout in ms (optional)                                         |
| `requestTimeoutMs`          | Per-server MCP request timeout in milliseconds                                         |
| `auth: "oauth"`             | Use MCP OAuth credentials saved by `bot mcp login`                                |
...
Bot config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `bot mcp set` and repaired by `bot doctor --fix` in existing config, but `transport` is what embedded Bot consumes directly.
...
Use the page for operator edits and quick inventory. Use `bot mcp doctor --probe` or `bot mcp probe` when you need live server proof.
...
7. Run `bot mcp doctor --probe` when you need live proof that the edited server starts and lists tools.
...
- active runtimes may need `bot mcp reload`, Gateway config publish, or process restart depending on which process owns the MCP clients

## MCP Apps

Bot can render tools that implement the stable [MCP Apps extension](https://modelcontextprotocol.io/extensions/apps). Apps are opt-in because their HTML comes from the configured MCP server and can request app-visible tools or resources from that same server.

Enable the host bridge:

```bash
bot config set mcp.apps.enabled true --strict-json
```

Restart the Gateway after changing this setting. When enabled, Bot starts a sandbox-only HTTP(S) listener on the Gateway port plus one (for the default Gateway, `18790`). The Control UI loads Apps from that separate origin; the listener never serves Control UI, authenticated Gateway routes, or user data.
...
- Bot advertises the `io.modelcontextprotocol/ui` extension only when Apps are enabled.
...
- App-only tools (`_meta.ui.visibility: ["app"]`) stay out of model tool lists. Apps can call only app-visible tools on their owning server that also pass the effective Bot tool policy for the run that created the view.
...
- `bot security audit` warns while the bridge is enabled. Disable it with `bot config set mcp.apps.enabled false --strict-json` when it is not needed.
