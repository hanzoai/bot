---
summary: "CLI reference for `bot setup` (system-agent chat with onboarding fallback)"
read_when:
  - You want to chat with Bot for setup or repair
...
`bot setup` is the system-agent entry point. On a configured system, bare
`bot setup` opens an interactive Bot chat. On a fresh system, it
...
1. Any onboarding option (`--wizard`, `--baseline`, workspace, reset,
   ...
   options) runs onboarding exactly as `bot onboard` does.
2. `-m`/`--message` or `--yes` runs the system agent.
3. With no routing option, a configured interactive system opens Bot. A
...
In guided mode, `--workspace <dir>` is the workspace proposed to Bot;
...
candidates; when several local models are available, Bot prefers the
...
`setup` accepts the same onboarding flags as `bot onboard`, including
...
terminal hatch as `bot onboard --tui`. See [Onboard](/cli/onboard) and
[CLI automation](/start/wizard-cli-automation) for the full flag reference and
non-interactive examples. `bot onboard --modern` remains a compatibility
entry for the same inference-gated Bot assistant.

<Note>
`bot setup` is for mutable config installs. In Nix mode (`BOT_NIX_MODE=1`) Bot refuses setup writes because the config file is managed by Nix. Use the first-party [nix-bot Quick Start](https://github.com/bot/nix-bot#quick-start) or the equivalent source config for another Nix package.
</Note>
...
| Flag                       | Description                                                                                          |
| -------------------------- | ---------------------------------------------------------------------------------------------------- |
| `-m, --message <text>`     | Run one Bot request.                                                                            |
...
| `--json`                   | Configured system: Bot overview. Onboarding route: onboarding summary.                          |
...
`bot setup --baseline` preserves the older baseline-only behavior: it
...
it and asks you to run `bot doctor` before retrying.
...
bot setup
bot setup -m "status"
bot setup -m "restart gateway" --yes
bot setup --json
bot setup --wizard
bot setup --baseline
bot setup --workspace ~/.bot/workspace
bot setup --import-from hermes --import-source ~/.hermes
bot setup --non-interactive --accept-risk --mode remote --remote-url wss://gateway-host:18789 --remote-token <token>
```
