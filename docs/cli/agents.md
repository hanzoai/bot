---
summary: "CLI reference for `bot agents` (list/add/delete/bindings/bind/unbind/set identity)"
read_when:
...
Manage isolated agents (workspaces + auth + routing). Running `bot agents` with no subcommand is equivalent to `bot agents list`.
...
bot agents add work --workspace ~/.bot/workspace-work
bot agents add work --workspace ~/.bot/workspace-work --bind telegram:*
bot agents add ops --workspace ~/.bot/workspace-ops --bind telegram:ops --non-interactive
bot agents bindings
bot agents bind --agent work --bind telegram:ops
bot agents unbind --agent work --bind telegram:ops
...
If you also want different visible skills per agent, configure `agents.defaults.skills` and `agents.entries.*.skills` in `bot.json`. See [Skills config](/tools/skills-config) and [Configuration reference](/gateway/config-agents#agentsdefaultsskills).

List bindings:

```bash
bot agents bindings
bot agents bindings --agent work
bot agents bindings --json
```

Add bindings:

```bash
bot agents bind --agent work --bind telegram:ops --bind discord:guild-a
```

You can also add bindings when creating an agent:

```bash
bot agents add work --workspace ~/.bot/workspace-work --bind telegram:* --bind discord:*
```

If you omit `accountId` (`--bind <channel>`), Bot resolves it from plugin setup hooks, forced account binding, or the channel's configured account count.

If you omit `--agent` for `bind` or `unbind`, Bot targets the current default agent.
...
bot agents bind --agent work --bind telegram:*
...
bot agents bind --agent work --bind telegram:alerts
```
...
```bash
bot agents unbind --agent work --bind telegram:ops
bot agents unbind --agent work --all
```
...
```bash
bot agents set-identity --agent main --name "Bot" --emoji "🦞" --avatar avatars/bot.png
```
