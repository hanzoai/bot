---
summary: "CLI reference for `bot plugins` (init, build, validate, list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
bot plugins list [--enabled] [--verbose] [--json]
bot plugins search <query> [--limit <n>] [--json]
bot plugins install <path-or-spec> [--link] [--force] [--pin] [--marketplace <source>]
bot plugins inspect <id> [--runtime] [--json]
bot plugins inspect --all [--runtime] [--json]
bot plugins info <id>                    # alias for inspect
bot plugins enable <id>
bot plugins disable <id>
bot plugins uninstall <id> [--dry-run] [--keep-files] [--force]
bot plugins update <id-or-npm-spec> | --all [--dry-run]
bot plugins registry [--refresh] [--json]
bot plugins doctor
bot plugins init <id> [--name <name>] [--type tool|provider] [--directory <path>]
bot plugins build [--entry <path>] [--check]
bot plugins validate [--entry <path>]
bot plugins marketplace entries [--offline] [--feed-profile <name>] [--json]
bot plugins marketplace list <source> [--json]
bot plugins marketplace refresh [--feed-profile <name>] [--expected-sha256 <sha256>] [--json]
```

For slow install, inspect, uninstall, or registry-refresh investigation, run the
command with `BOT_PLUGIN_LIFECYCLE_TRACE=1`. The trace writes phase timings
to stderr and keeps JSON output parseable. See [Debugging](/help/debugging#plugin-lifecycle-trace).

<Note>
In Nix mode (`BOT_NIX_MODE=1`), `bot.json` is immutable. `install`, `update`, `uninstall`, `enable`, and `disable` all refuse to run. Edit the Nix source for this install instead (`programs.bot.config` or `instances.<name>.config` for nix-bot), then rebuild. See the agent-first [Quick Start](https://github.com/bot/nix-bot#quick-start).
</Note>

<Note>
Bundled plugins ship with Bot. Some are enabled by default (for example bundled model providers, bundled speech providers, and the bundled browser plugin); others require `plugins enable`.

Native Bot plugins ship `bot.plugin.json` with an inline JSON Schema (`configSchema`, even if empty). Compatible bundles use their own bundle manifests instead.

`plugins list` shows `Format: bot` or `Format: bundle`. Verbose list/info output also shows the bundle subtype (`codex`, `claude`, or `cursor`) plus detected bundle capabilities.
</Note>
...
argument is the plugin id; `--name` sets the display name. Bot uses the
...
`plugin:validate` that build then call `bot plugins build`/`validate`.

`plugins build` imports the built entry, reads its static tool metadata, writes
`bot.plugin.json`, and keeps `package.json`'s `bot.extensions` aligned.
...
```bash
bot plugins init acme-models --name "Acme Models" --type provider
cd acme-models
npm install
npm run build
npm test
...
`bot plugins build`/`validate`; those commands are for the tool
...
```bash
bot plugins search "calendar"                      # search ClawHub plugins
bot plugins install @hanzo/bot-<package>            # trusted official catalog
bot plugins install <package>                       # arbitrary npm package
bot plugins install clawhub:<package>                # ClawHub only
bot plugins install npm:<package>                    # npm only
bot plugins install npm-pack:<path.tgz>               # local npm-pack tarball
bot plugins install git:github.com/<owner>/<repo>     # git repo
bot plugins install git:github.com/<owner>/<repo>@<ref>
bot plugins install <path>                            # local path or archive
bot plugins install -l <path>                         # link instead of copy
bot plugins install <plugin>@<marketplace>             # marketplace shorthand
bot plugins install <plugin> --marketplace <name>      # marketplace (explicit)
bot plugins install <package> --force                  # confirm source / overwrite existing
bot plugins install <package> --pin                    # pin resolved npm version
bot plugins install clawhub:<package> --acknowledge-clawhub-risk
bot plugins install <package> --dangerously-force-unsafe-install
```
...
<Warning>
Bare package names install from npm by default during the launch cutover, unless they match a bundled or official plugin id, in which case Bot uses that local/official copy instead of hitting the npm registry. Use `npm:<package>` when you deliberately want an external npm package instead. Use `clawhub:<package>` for ClawHub. Treat plugin installs like running code; prefer pinned versions.
</Warning>

<Warning>
ClawHub packages and Bot's bundled/official catalog are trusted install
...
`bundle-plugin` packages (not skills; use `bot skills search` for those).
...
summary, and an install hint such as `bot plugins install clawhub:<package>`.

<Note>
ClawHub is the primary distribution and discovery surface for most plugins. Npm
remains a supported fallback and direct-install path. Bot-owned
`@hanzo/bot-*` plugin packages are published on npm again; see the current list
on [npmjs.com/org/bot](https://www.npmjs.com/org/bot) or the
...
<AccordionGroup>
  <Accordion title="Config includes and invalid-config repair">
    If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `bot.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

    If config is invalid before install, `plugins install` normally fails closed and tells you to run `bot doctor --fix` first. During Gateway startup and hot reload, invalid plugin config fails closed like any other invalid config; `bot doctor --fix` can quarantine the invalid plugin entry. The only pre-existing-config exception is a narrow bundled-plugin recovery path for plugins that explicitly opt into `bot.install.allowInvalidConfigRecovery`.

    When the existing host config is valid but the newly installed plugin's own config is absent, Bot records the install disabled instead of writing an invalid enabled entry. Configure `plugins.entries.<id>.config`, then run `bot plugins enable <id>`. If an existing plugin config entry is present but invalid, install fails without rewriting it.

  </Accordion>
  <Accordion title="--force confirmation and reinstall vs update">
    `--force` confirms a non-ClawHub source without prompting. It does not bypass `security.installPolicy` or remaining install safety checks. When the plugin or hook pack is already installed, it also reuses the existing target and overwrites it in place. Use it after reviewing an arbitrary npm, local, archive, git, or marketplace source, or when intentionally reinstalling the same id. For routine upgrades of an already tracked npm plugin, prefer `bot plugins update <id-or-npm-spec>`.

    If you run `plugins install` for a plugin id that is already installed, Bot stops and points you at `plugins update <id-or-npm-spec>` for a normal upgrade, or at `plugins install <package> --force` when you genuinely want to overwrite the current install from a different source. Arbitrary sources still show the interactive provenance warning; noninteractive installs must pass `--force` after review. Trusted ClawHub and Bot-catalog sources do not need it. With `--link`, `--force` confirms the source but does not change the linked-path install mode.
  ...
  <Accordion title="--dangerously-force-unsafe-install">
    `--dangerously-force-unsafe-install` is deprecated and is now a no-op. Bot no longer runs built-in install-time dangerous-code blocking for plugin installs.
  ...
  <Accordion title="Hook packs and npm specs">
    ...
    Raw `@hanzo/bot-*` specs that match bundled plugins resolve to the image-owned bundled copy before npm fallback. For example, `bot plugins install @hanzo/bot-discord@2026.5.20 --pin` uses the bundled Discord plugin from the current Bot build instead of creating a managed npm override. To force the external npm package, use `bot plugins install npm:@hanzo/bot-discord@2026.5.20 --pin`.

    Bare specs and `@latest` stay on the stable track. Bot date-stamped correction versions such as `2026.5.3-1` count as stable for this check. If npm resolves either form to a prerelease, Bot stops and asks you to opt in explicitly with a prerelease tag (`@beta`/`@rc`) or an exact prerelease version (`@1.2.3-beta.4`).

    For npm installs without an exact version (`npm:<package>` or `npm:<package>@latest`), Bot checks the resolved package metadata before install. If the latest stable package requires a newer Bot plugin API or minimum host version, Bot inspects older stable versions and installs the newest compatible release instead. Exact versions and explicit dist-tags stay strict: an incompatible selection fails and asks you to upgrade Bot or choose a compatible version.

    If a bare install spec matches an official plugin id (for example `diffs`), Bot installs the catalog entry directly. To install an npm package with the same name, use an explicit scoped spec (for example `@scope/diffs`).
  ...
  <Accordion title="Git repositories">
    ...
    Git installs clone into a temporary directory, check out the requested ref when present, then use the normal plugin directory installer, so manifest validation, operator install policy, package-manager install work, and install records behave like npm installs. Recorded git installs include the source URL/ref plus the resolved commit so `bot plugins update` can re-resolve the source later.

    After installing from git, use `bot plugins inspect <id> --runtime --json` to verify runtime registrations such as gateway methods and CLI commands. If the plugin registered a CLI root with `api.registerCli`, run that command directly through the Bot root CLI, for example `bot demo-plugin ping`.

  </Accordion>
  <Accordion title="Archives">
    Supported archives: `.zip`, `.tgz`, `.tar.gz`, `.tar`. Native Bot plugin archives must contain a valid `bot.plugin.json` at the extracted plugin root; archives that only contain `package.json` are rejected before Bot writes install records.
...
```bash
bot plugins install clawhub:bot-codex-app-server
bot plugins install clawhub:bot-codex-app-server@1.2.3
```

Bare npm-safe plugin specs install from npm by default during the launch cutover unless they match an official plugin id:

```bash
bot plugins install bot-codex-app-server
```

Use `npm:` to make npm-only resolution explicit:

```bash
bot plugins install npm:bot-codex-app-server
bot plugins install npm:@hanzo/bot-discord@2026.5.20
bot plugins install npm:@scope/plugin-name@1.0.1
```

Bot checks the advertised plugin API / minimum gateway compatibility before install. When the selected ClawHub version publishes a ClawPack artifact, Bot downloads the versioned npm-pack `.tgz`, verifies the ClawHub digest header and the artifact digest, then installs it through the normal archive path. Older ClawHub versions without ClawPack metadata still install through the legacy package archive verification path. Recorded installs keep their ClawHub source metadata, artifact kind, npm integrity, npm shasum, tarball name, and ClawPack digest facts for later updates.
Unversioned ClawHub installs keep an unversioned recorded spec so `bot plugins update` can follow newer ClawHub releases; explicit version or tag selectors such as `clawhub:pkg@1.2.3` and `clawhub:pkg@beta` remain pinned to that selector.
...
```bash
bot plugins marketplace list <marketplace-name>
bot plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` to pass the marketplace source explicitly:

```bash
bot plugins install <plugin-name> --marketplace <marketplace-name>
bot plugins install <plugin-name> --marketplace <owner/repo>
bot plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
bot plugins install <plugin-name> --marketplace ./my-marketplace
```
...
<Tabs>
  ...
  <Tab title="Remote marketplace rules">
    For remote marketplaces loaded from GitHub or git, plugin entries must stay inside the cloned marketplace repo. Bot accepts relative path sources from that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path plugin sources from remote manifests.
  </Tab>
</Tabs>

For local paths and archives, Bot auto-detects:

- native Bot plugins (`bot.plugin.json`)
...
`~/.bot/extensions` or `<workspace>/.bot/extensions`; those
...
run `bot plugins enable <plugin-id>` or set
...
```bash
bot plugins list
bot plugins list --enabled
bot plugins list --verbose
bot plugins list --json
```
...
run `bot plugins list --enabled --verbose` or
`bot plugins inspect <id>` with a listed plugin id to confirm the plugin
ids and copy trusted ids into `plugins.allow` in `bot.json`. When the
...
so Bot records install provenance.
...
`/app/extensions/synology-chat`. Bot discovers that mounted source overlay
...
- `bot plugins inspect <id> --runtime --json` shows registered hooks and diagnostics from a module-loaded inspection pass. Runtime inspection never installs dependencies; use `bot doctor --fix` to clean legacy dependency state or recover missing downloadable plugins that are referenced by config.
- `bot gateway status --deep --require-rpc` confirms the reachable Gateway URL/profile, service/process hints, config path, and RPC health.
...
Plugin install metadata is machine-managed state, not user config. Installs and updates write it to the shared SQLite state database under the active Bot state directory. The `installed_plugin_index` row stores durable `installRecords` metadata, including records for broken or missing plugin manifests, plus a manifest-derived cold registry cache used by `bot plugins update`, uninstall, diagnostics, and the cold plugin registry.

`plugins.installs` is a retired authored-config surface. Runtime and update commands read only the SQLite installed-plugin index. Run `bot doctor --fix` to import legacy config records into the index and remove the retired key before normal runtime use.
...
bot plugins update <id-or-npm-spec> --dry-run
bot plugins update @hanzo/bot-voice-call
bot plugins update @acme/demo
bot plugins update bot-codex-app-server --acknowledge-clawhub-risk
bot plugins update bot-codex-app-server --dangerously-force-unsafe-install
```
...
<AccordionGroup>
  <Accordion title="Resolving plugin id vs npm spec">
    When you pass a plugin id, Bot reuses the recorded install spec for that plugin. That means previously stored dist-tags such as `@beta` and exact pinned versions continue to be used on later `update <id>` runs.

    During `update <id> --dry-run`, exact pinned npm installs stay pinned. If Bot can also resolve the package's registry default line and that default line is newer than the installed pinned version, the dry run reports the pin and prints the explicit `@latest` package update command to follow the registry default line.

    That targeted-update rule differs from the bulk `bot plugins update --all` maintenance path. Bulk updates still respect ordinary tracked install specs, but trusted official Bot plugin records can sync to the current official catalog target instead of staying on a stale exact official package. Use targeted `update <id>` when you intentionally want to keep an exact or tagged official spec untouched.

    For npm installs, you can also pass an explicit npm package spec with a dist-tag or exact version. Bot resolves that package name back to the tracked plugin record, updates that installed plugin, and records the new npm spec for future id-based updates.
  ...
  <Accordion title="--acknowledge-clawhub-risk on update">
    Community ClawHub-backed plugin updates run the same exact-release trust check as installs before downloading the replacement package. Use `--acknowledge-clawhub-risk` for reviewed automation that should continue when the selected ClawHub release has a risky trust warning. Official ClawHub packages and bundled Bot plugin sources bypass this release-trust prompt.
  </Accordion>
</AccordionGroup>
...
bot plugins inspect <id> --runtime
bot plugins inspect <id> --json
bot plugins inspect --all
```

Inspect shows identity, load status, source, manifest capabilities, policy flags, diagnostics, install metadata, bundle capabilities, and any detected MCP or LSP server support without importing plugin runtime by default. JSON output includes the plugin manifest contracts, such as `contracts.agentToolResultMiddleware` and `contracts.trustedToolPolicies`, so operators can audit trusted-surface declarations before enabling or restarting a plugin. Add `--runtime` to load the plugin module and include registered hooks, tools, commands, services, gateway methods, and HTTP routes. Runtime inspection reports missing plugin dependencies directly; installs and repairs stay in `bot plugins install`, `bot plugins update`, and `bot doctor --fix`.

Plugin-owned CLI commands are usually installed as root `bot` command groups, but plugins may also register nested commands under a core parent such as `bot nodes`. After `inspect --runtime` shows a command under `cliCommands`, run it at the listed path; for example a plugin that registers `demo-git` can be verified with `bot demo-git ping`.
...
For module-shape failures such as missing `register`/`activate` exports, rerun with `BOT_PLUGIN_LOAD_DEBUG=1` to include a compact export-shape summary in the diagnostic output.

## Registry

```bash
bot plugins registry
bot plugins registry --refresh
bot plugins registry --json
```

The local plugin registry is Bot's persisted cold read model for installed plugin identity, enablement, source metadata, and contribution ownership. Normal startup, provider owner lookup, channel setup classification, and plugin inventory can read it without importing plugin runtime modules.

Use `plugins registry` to inspect whether the persisted registry is present, current, or stale. Use `--refresh` to rebuild it from the persisted plugin index, config policy, and manifest/package metadata. This is a repair path, not a runtime activation path.

`bot doctor --fix` also repairs registry-adjacent managed npm drift. If an orphaned or recovered `@hanzo/bot-*` package under a managed plugin npm project or the legacy flat managed npm root shadows a bundled plugin, doctor removes that stale package and rebuilds the registry so startup validates against the bundled manifest. When an authoritative install record selects one managed generation but older flat or generation directories remain, doctor retires those stale trees for pruning after the gateway restarts. Doctor also relinks the host `bot` package into managed npm plugins that declare `peerDependencies.bot`, so package-local runtime imports such as `bot/plugin-sdk/*` resolve after updates or npm repairs.

## Marketplace

```bash
bot plugins marketplace entries
bot plugins marketplace entries --offline
bot plugins marketplace entries --json
bot plugins marketplace entries --feed-profile <name>
bot plugins marketplace entries --feed-url <url>
bot plugins marketplace list <source>
bot plugins marketplace list <source> --json
bot plugins marketplace refresh
bot plugins marketplace refresh --feed-profile <name>
bot plugins marketplace refresh --feed-url <url>
bot plugins marketplace refresh --expected-sha256 <sha256> --json
```

`plugins marketplace entries` lists entries from the configured Bot marketplace feed. By default it attempts the hosted feed and falls back to the latest accepted snapshot or bundled data. Use `--feed-profile <name>` to read a specific configured profile, `--feed-url <url>` to read an explicit hosted feed URL, and `--offline` to read the latest accepted snapshot without fetching the feed.

`plugins marketplace refresh` refreshes the configured hosted feed snapshot and reports whether Bot accepted hosted data, a hosted snapshot, or bundled fallback data. Use `--expected-sha256` when a caller needs the command to fail unless a fresh hosted payload matches a pinned checksum.
...
Marketplace refresh loads a hosted Bot marketplace feed and persists the
...
refreshes fail if Bot cannot persist the validated snapshot.

The built-in `clawhub-public` profile expects payload identity
`clawhub-official`. Bot will bundle ClawHub's production public key after
...
Bot verifies the DSSE envelope and, when a profile declares `feedId`,
