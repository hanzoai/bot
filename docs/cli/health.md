---
summary: "CLI reference for `bot health` (gateway health snapshot via RPC)"
read_when:
...
- When outbound deliveries or inbound channel events are dead-lettered, text output reports their counts and oldest failure age. Inbound counts are grouped by channel account; inspect or recover individual events with [`bot channels dead-letters`](/cli/channels#inbound-dead-letters).

## Related

- [CLI reference](/cli)
- [`bot status`](/cli/status) — local diagnosis and channel probes without a full health snapshot
