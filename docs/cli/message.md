---
summary: "CLI reference for `bot message` (send + channel actions)"
read_when:
...
`bot message` resolves channel SecretRefs before running the action,
...
```bash
bot message send --channel slack --target channel:C123 \
...
```bash
bot message send --channel telegram --target 123456789 --message "Open app:" \
  --presentation '{"blocks":[{"type":"buttons","buttons":[{"label":"Launch","webApp":{"url":"https://example.com/app"}}]}]}'
```

```bash
bot message send --channel telegram --target @mychat \
...
```bash
bot message broadcast --targets <target...> [--channel all] [--message <text>] [--media <url>] [--dry-run]
```
