---
summary: "CLI reference for `bot daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `bot daemon ...` in scripts
...
Legacy alias for Gateway service management. `bot daemon ...` maps to the same service-control commands as `bot gateway ...`. Prefer [`bot gateway`](/cli/gateway) for current docs and examples.
