---
summary: "CLI reference for `bot backup` (archives and SQLite snapshots)"
read_when:
  - You want a first-class backup archive for local Bot state
  - You need a compact, verified snapshot of one Bot SQLite database
...
Create a local backup archive for Bot state, config, auth profiles, channel/provider credentials, sessions, and optionally workspaces.

```bash
bot backup create
bot backup create --output ~/Backups
bot backup create --dry-run --json
bot backup create --verify
bot backup create --no-include-workspace
bot backup create --only-config
bot backup verify ./2026-03-09T08-00-00.000+08-00-bot-backup.tar.gz
bot backup sqlite create --global --repository ~/Backups/bot-sqlite
bot backup sqlite create --agent main --repository ~/Backups/bot-sqlite
bot backup sqlite list --repository ~/Backups/bot-sqlite
bot backup sqlite verify ~/Backups/bot-sqlite/<snapshot-id>
bot backup sqlite verify ~/Backups/bot-sqlite/<snapshot-id> --scratch ~/Private/bot-scratch
bot backup sqlite restore ~/Backups/bot-sqlite/<snapshot-id> --target ./restored/bot.sqlite
```

## Notes

- The archive embeds a `manifest.json` with the resolved source paths and archive layout.
- Default output is a timestamped `.tar.gz` archive in the current working directory. Timestamped filenames use your machine's local timezone and include the UTC offset. If the current working directory is inside a backed-up source tree, Bot falls back to your home directory for the default archive location.
...
- `bot backup verify <archive>` checks that the archive contains exactly one root manifest, rejects traversal-style archive paths and SQLite sidecars, confirms every manifest-declared payload exists, validates every SQLite snapshot's file shape, and runs full integrity and role checks on canonical Bot databases. Dedicated plugin schemas remain opaque because they may require owner-defined SQLite capabilities. `bot backup create --verify` runs that validation immediately after writing the archive.
- `bot backup create --only-config` backs up just the active JSON config file.

## SQLite snapshots

Use `bot backup sqlite` when you need a portable artifact for one Bot-owned SQLite database instead of a broad state archive.

Snapshot creation accepts exactly one named source:

| Command                                                         | Database               |
| --------------------------------------------------------------- | ---------------------- |
| `bot backup sqlite create --global --repository <dir>`     | Shared Bot state  |
| `bot backup sqlite create --agent <id> --repository <dir>` | One per-agent database |
...
SQLite snapshots can contain auth profiles, session state, plugin state, and other sensitive records. Protect repositories with the same permissions, encryption, retention policy, and destination restrictions as the live Bot state directory.

### Verify and restore

```bash
bot backup sqlite verify <snapshot-directory>
bot backup sqlite restore <snapshot-directory> --target <new-database-path>
```

Verification checks the strict manifest shape, artifact size and SHA-256, SQLite integrity, foreign keys, schema version, database role and owner, and Bot-owned index definitions.
...
Restore repeats verification and writes only to a fresh target. It refuses an existing target, `-wal`, `-shm`, or `-journal` sidecar and never performs an in-place replacement of a live Bot database. The target parent has the same path-security requirements as verification scratch. Activating a restored database remains an explicit offline operator step.
...
`bot backup create` plans sources from your local Bot install:

- The state directory (usually `~/.bot`)
- The active config file path
- The resolved `credentials/` directory when it exists outside the state directory
- Workspace directories discovered from the current config, unless you pass `--no-include-workspace`

Auth profiles and other per-agent runtime state live in SQLite under the state directory (`agents/<agentId>/agent/bot-agent.sqlite`), so they are covered by the state backup entry automatically.

`--only-config` skips state, credentials-directory, and workspace discovery and archives only the active config file path.

Bot canonicalizes paths before building the archive: if config, the credentials directory, or a workspace already live inside the state directory, they are not duplicated as separate top-level backup sources. Missing paths are skipped.

During archive creation, Bot excludes known live-mutation paths before `tar` reads them. This avoids races between a file's recorded size and concurrent writes. The filter applies these state-relative rules under each backed-up state directory:
...
`bot backup` bypasses the normal config preflight so it can still help during recovery. Workspace discovery depends on a valid config, so `bot backup create` fails fast when the config file exists but is invalid and workspace backup is still enabled.
...
Bot does not enforce a built-in maximum backup size or per-file size limit. An archive write that produces no data for five minutes fails and removes its partial temporary file instead of hanging indefinitely. Practical limits otherwise come from:

- Available space for the temporary archive write plus the final archive
- Time to walk large workspace trees and compress them into a `.tar.gz`
- Time to rescan the archive with `--verify` or `bot backup verify`
- Destination filesystem behavior: Bot requires no-overwrite hard-link publication so a final archive path never exposes an in-progress copy; unsupported filesystems fail with an actionable error
