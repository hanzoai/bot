---
summary: "CLI reference for `bot update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `bot update` output or options
...
Update Bot and switch between stable/extended-stable/beta/dev channels.
...
bot update --yes
bot update --acknowledge-clawhub-risk
bot update --json
bot --update
```

`bot --update` rewrites to `bot update` (useful for shells and
launcher scripts).
...
| `--tag <dist-tag\|version\|spec>`                | Override the package target for this update only. It cannot be combined with an effective `extended-stable` channel, whose verified exact target is mandatory. For other package installs, `main` maps to `github:hanzoai/bot#main`; GitHub/git source specs are packed into a temporary tarball before the staged global npm install. |
...
| `--acknowledge-clawhub-risk`                     | Allow post-update plugin sync to continue past community ClawHub trust warnings without an interactive prompt. Without it, risky community releases are skipped and left unchanged when Bot cannot prompt. Official ClawHub packages and bundled plugin sources bypass this prompt.                                                     |

There is no `--verbose` flag. Use `--dry-run` to preview planned actions,
`--json` for machine-readable results, and `bot update status --json`
...
<Note>
In Nix mode (`BOT_NIX_MODE=1`), mutating `bot update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-bot, use the agent-first [Quick Start](https://github.com/bot/nix-bot#quick-start). `bot update status` and `bot update --dry-run` remain read-only.
</Note>
...
`bot update` installed the new core package but post-core plugin sync,
managed npm plugin metadata, registry refresh, or doctor repair did not
converge.

```bash
bot update repair
bot update repair --channel beta
bot update repair --acknowledge-clawhub-risk
bot update repair --json
```
...
| `--acknowledge-clawhub-risk`                     | Same behavior as on `bot update`.                                                                                                                                                                                                                              |
| `--no-restart`                                   | Accepted for parity; repair never restarts the Gateway.                                                                                                                                                                                                             |

`update repair` runs `bot doctor --fix`, reloads the repaired config and
...
- `dev` -> ensures a git checkout (default `~/bot`, or
  `$BOT_HOME/bot` when `BOT_HOME` is set; override with
  `BOT_GIT_DIR`), updates it, and installs the global CLI from that
  checkout.
...
detached helper and exits, and that helper runs `bot update --yes --json`
...
healthy. If the plist is installed but launchd is not supervising it, Bot
...
`Gateway: restart failed: ...` with a manual `bot gateway restart` hint.
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `bot update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"`: Bot could not find a supervising
  service boundary and durable service identity for a safe handoff (for
  example, systemd handoff requires the `BOT_SYSTEMD_UNIT` unit identity,
...
`bot status` and `bot status --all` show an `Update restart` row
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `BOT_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
  ...
  <Step title="Run doctor">
    `bot doctor` runs as the final safe-update check.
  </Step>
...
beta release, Bot falls back to the recorded default/latest spec and
reports a warning. For npm plugins, Bot also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `bot update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (for example an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `bot update repair` and `bot plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `bot update repair`. When a failed update leaves a managed plugin unusable, Bot disables its runtime entry and resets active slots without changing the operator-authored `plugins.allow` or `plugins.deny` policy.

After the per-plugin sync step, `bot update` runs a mandatory **post-core convergence** pass before the gateway restarts: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly declared `main` exists). Failures from this pass, and an invalid config snapshot, return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `bot update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `bot update repair` and `bot plugins inspect <id> --runtime --json`. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here (mirroring the `skipDisabledPlugins` policy used by the missing-payload check), so a stale disabled plugin record cannot block an otherwise valid update.
...
version. For default/`latest` intent, Bot does not query plugin
...
For package-manager installs, `bot update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: Bot installs the new package into a temporary npm prefix,
...
the updater approves only the candidate Bot lifecycle; transitive
dependency scripts remain blocked. Bot then swaps the clean package tree
...
plugin records aligned with the installed Bot build, while leaving full
plugin-command completion rebuilds to explicit
`bot completion --write-state` runs.
