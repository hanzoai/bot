...
node connection and adds native Mac capabilities. Use `bot node run` on a
...
`bot node run` can publish plugin or MCP-backed tools after it connects.
...
`nodeHost.mcp.servers` in `bot.json` on the node machine, then restart the
...
- `--context-path <path>`: Gateway WebSocket context path (e.g. `/bot-gw`). Appended to the WebSocket URL.
...
`bot node run` and `bot node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):
...
trusted private-DNS names, set `BOT_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `bot.json` config
key.
`bot node install` persists it into the supervised node service when it is
...
runs `bot node identity --json` on the node host over SSH and approves on
...
```bash
bot node identity --json
```

It prints the device ID and public key from the `primary` row in
`state/bot.sqlite` and never creates the database or a new identity.
...
Run `bot devices list` again before approval.
...
Bot state directory (`~/.bot` by default, or `$BOT_STATE_DIR`
when set):

| State                                                    | Purpose                                                                                                                          |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `state/bot.sqlite` (`node_host_config`)             | Client instance ID, display name, and Gateway connection metadata. The client sends this ID as `instanceId`.                     |
| `state/bot.sqlite` (`device_identities`, `primary`) | Signed Ed25519 keypair and derived device ID. For signed connections, this device ID is the routed node ID and pairing identity. |
| `state/bot.sqlite` (`device_auth_tokens`)           | Paired device tokens, keyed by cryptographic device ID and role.                                                                 |
...
`node.json` with `bot doctor --fix` likewise does not reset pairing. To
revoke and re-pair a node:

1. On the Gateway, run `bot nodes remove --node <id|name|ip>`.
2. On the node, restart the installed service with `bot node restart`, or
   stop and rerun the foreground `bot node run` command. This starts the
   device-pairing flow. If `bot devices list` does not show a request
...
3. On the Gateway, run `bot devices list`, then
...
5. On the Gateway, run `bot nodes pending`, then
...
Older Bot releases stored node-host state in `node.json`, the signed
identity in `identity/device.json`, and paired auth in
`identity/device-auth.json`. Stop the node host and run
`bot doctor --fix` once; Doctor claims each retired source, validates it,
...
or an interrupted Doctor claim remains. Keep `state/bot.sqlite` private;
...
- `$BOT_STATE_DIR/state/bot.sqlite#exec_approvals_config`, or
  `~/.bot/state/bot.sqlite#exec_approvals_config` when the variable is unset
- [Exec approvals](/tools/exec-approvals)
- `bot approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Bot prepares a canonical `systemRunPlan`
