---
summary: "CLI reference for `bot workboard` cards, dispatch, and worker runs"
read_when:
...
`bot workboard` is the terminal surface for the bundled [Workboard plugin](/plugins/workboard). It lets an operator list cards, create a card, inspect one card, and ask the running Gateway to dispatch ready work into subagent worker runs.

Enable the plugin before using the command:

```bash
bot plugins enable workboard
bot gateway restart
```
...
```bash
bot workboard list [--board <id>] [--status <status>] [--include-archived] [--json]
bot workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
bot workboard show <id> [--json]
bot workboard move <id> --status <status> [--json]
bot workboard dispatch [--board <id>] [--max-starts <count>] [--admin] [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
bot workboard list
bot workboard list --board default --status ready
bot workboard list --json
```
...
```bash
bot workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
bot workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
bot workboard show 7f4a2c10
bot workboard show 7f4a2c10 --json
```
...
```bash
bot workboard move 7f4a2c10 --status review
bot workboard move 7f4a2c10 --status done --json
```
...
```bash
bot workboard dispatch
bot workboard dispatch --json
bot workboard dispatch --max-starts 10
bot workboard dispatch --admin
bot workboard dispatch --url http://127.0.0.1:18789 --token "$BOT_GATEWAY_TOKEN"
```
...
Local `list`, `create`, `show`, and `move` commands operate on the local Bot state directory used by the current profile. Use `--dev` or `--profile <name>` on the top-level `bot` command when you need a different state root.
...
```bash
bot plugins inspect workboard --runtime --json
```
...
Then retry `bot workboard dispatch`. Data-only fallback is useful for local state cleanup, but worker runs need a live Gateway.
...
```bash
bot workboard list --status ready
```
