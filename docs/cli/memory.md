---
summary: "CLI reference for `bot memory` (status/index/search/promote/promote-explain/rem-harness/rem-backfill/session-backfill)"
read_when:
...
```bash
bot memory status [--agent <id>] [--deep] [--index] [--fix] [--json] [--verbose]
```
...
```bash
bot memory index [--agent <id>] [--force] [--verbose]
```
...
```bash
bot memory search [query] [--query <text>] [--agent <id>] [--max-results <n>] [--min-score <n>] [--json]
```
...
```bash
bot memory promote [--agent <id>] [--limit <n>] [--min-score <n>] \
...
```bash
bot memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
bot memory rem-harness [--agent <id>] [--path <file-or-dir>] [--grounded] [--include-promoted] [--json]
```
...
```bash
bot memory rem-backfill --path <file-or-dir> [--agent <id>] [--stage-short-term] [--json]
bot memory rem-backfill --rollback [--rollback-short-term] [--json]
```
...
```bash
bot memory session-backfill --agent <id> [--from YYYY-MM-DD] [--to YYYY-MM-DD] \
  [--limit-days <n>] [--archive-files <path...>] [--rem | --apply] [--json]
bot memory session-backfill --agent <id> --rollback [--json]
```
