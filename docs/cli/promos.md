---
summary: "CLI reference for `bot promos` (list and claim promotional model offers)"
read_when:
...
```bash
bot promos list
bot promos claim <slug>
bot promos claim <slug> --api-key <key> --set-default
```
...
2. Validates the promotion's provider, auth choice, and declared plugin packages
   against your installed Bot version. Unknown ids or package mismatches are
...
3. Reuses your existing provider credentials when you have them. Otherwise it
   ...
   prompts, matching the `bot onboard` non-interactive flags; to keep the
...
`bot models set <model>`.

## Passive discovery in `models list`

`bot models list` also surfaces promotions without you asking ClawHub
directly:
...
- The first time a new offer is seen, a one-time notice points at
  `bot promos list`. Offers you have already listed or claimed are never
  announced again.
