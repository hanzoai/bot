...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Bot Gateway.

`bot acp` speaks ACP over stdio for IDEs and forwards prompts to the Gateway over WebSocket, keeping ACP sessions mapped to Gateway session keys. It is a Gateway-backed ACP bridge, not a full ACP-native editor runtime: it focuses on session routing, prompt delivery, and streaming updates.

If you want an external MCP client to talk directly to Bot channel conversations instead of hosting an ACP harness session, use [`bot mcp serve`](/cli/mcp) instead.

## What this is not

`bot acp` means Bot acts as an ACP server: an IDE or ACP client connects to Bot, and Bot forwards that work into a Gateway session.

This is different from [ACP Agents](/tools/acp-agents), where Bot runs an external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Bot: use `bot acp`
- Bot should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Bot parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                     |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Bot Gateway or agent instead.                                                                                                                          |
...
bot acp --url wss://gateway-host:18789 --token-file ~/.bot/gateway.token
...
bot acp client --server-args --url wss://gateway-host:18789 --token-file ~/.bot/gateway.token

# Override the server command (default: bot)
bot acp client --server "node" --server-args bot.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Bot through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive `bot acp` over stdio with an ACP JSON-RPC client. Cover `initialize`, `session/new`, `session/list` with an absolute `cwd`, `session/resume`, `session/close`, duplicate close, and missing resume.
...
Avoid using `bot gateway call sessions.list` as the only ACP proof. That CLI path may request a fresh-token operator scope upgrade; ACP bridge correctness is proven by ACP stdio frames plus the Gateway `sessions.list` log.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want it to drive an Bot Gateway session.
...
If you want ACPX-backed sessions to see Bot plugin tools or selected built-in tools such as `cron`, enable the gateway-side ACPX MCP bridges instead of trying to pass per-session `mcpServers`. See [ACP Agents](/tools/acp-agents-setup#plugin-tools-mcp-bridge) and [Bot tools MCP bridge](/tools/acp-agents-setup#bot-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your Bot bot over ACP, use `acpx` with its built-in `bot` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx bot` at `bot acp`.
3. Target the Bot session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Bot ACP session
acpx bot exec "Summarize the active Bot session state."

# Persistent named session for follow-up turns
acpx bot sessions ensure --name codex-bridge
acpx bot -s codex-bridge --cwd /path/to/repo \
  "Ask my Bot work agent for recent context relevant to this repo."
```

If you want `acpx bot` to target a specific Gateway and session key every time, override the `bot` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "bot": {
      "command": "env BOT_HIDE_BANNER=1 BOT_SUPPRESS_NOTES=1 bot acp --url ws://127.0.0.1:18789 --token-file ~/.bot/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Bot checkout, use the direct CLI entrypoint instead of the dev runner so the ACP stream stays clean:

```bash
env BOT_HIDE_BANNER=1 BOT_SUPPRESS_NOTES=1 node bot.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client pull contextual information from an Bot agent without scraping a terminal.
...
- `--token` and `--password` can be visible in local process listings on some systems. Prefer `--token-file`/`--password-file` or environment variables (`BOT_GATEWAY_TOKEN`, `BOT_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: `gateway.auth.*` then env (`BOT_GATEWAY_*`), falling back to `gateway.remote.*` only when `gateway.auth.*` is unset (a configured-but-unresolved local SecretRef fails closed instead of silently falling back)
...
- `bot acp client` sets `BOT_SHELL=acp-client` on the spawned bridge process, which can be used for context-specific shell/profile rules.
