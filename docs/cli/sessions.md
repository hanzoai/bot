---
summary: "CLI reference for `bot sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `bot channels status --probe`,
`bot status --deep`, or `bot health --verbose` when you need live
channel connectivity.

```bash
bot sessions
bot sessions --agent work
bot sessions --all-agents
bot sessions --active 120
bot sessions --limit 25
bot sessions --store ./tmp/sessions.json
bot sessions --json
```
...
`bot sessions` and the Gateway `sessions.list` RPC are bounded by default
...
`bot sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.bot/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.bot/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
```bash
bot sessions tail
bot sessions tail --follow
bot sessions tail --session-key "agent:main:telegram:direct:123" --tail 25
bot sessions --agent work tail --follow
bot sessions --all-agents tail --follow
```

`bot sessions tail` renders recent runtime trajectory events as compact
...
```bash
bot sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
bot sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.bot/trajectory-exports/` under the selected workspace.
...
bot sessions cleanup --agent work --dry-run
bot sessions cleanup --all-agents --dry-run
bot sessions cleanup --enforce
bot sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
bot sessions cleanup --dry-run --fix-dm-scope
bot sessions cleanup --json
```

`bot sessions cleanup` uses `session.maintenance` settings from config
([Configuration reference](/gateway/config-agents#session)):

- Scope note: `bot sessions cleanup` maintains session stores,
...
`bot sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.bot/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.bot/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
...
Reclaim context budget for a wedged or oversized session. `bot sessions
compact <key>` is the first-class wrapper around the `sessions.compact`
Gateway RPC and requires a running Gateway.

```bash
bot sessions compact "agent:main:main"
bot sessions compact "agent:main:main" --max-lines 200
bot sessions compact "agent:work:main" --agent work --json
```
...
<Note>
`bot agent --message '/compact ...'` is **not** a compaction path. Slash
...
`bot gateway call sessions.compact --params '<json>'` accepts:
...
```json
{
  ...
  "key": "agent:main:main",
  "compacted": true,
  "archived": "/home/user/.bot/agents/main/sessions/transcripts/<id>.jsonl.bak",
  "kept": 200
}
```
