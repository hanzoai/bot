...
### `bot devices approve [requestId] [--latest]`
...
<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Bot supersedes the previous pending entry with a new `requestId`. Run `bot devices list` right before approval to get the current id.
</Note>

Approval behavior:

- If the device is already paired and requests broader scopes or role, Bot keeps the existing approval and creates a new pending upgrade request. Compare `Requested` vs `Approved` in `bot devices list`, or preview with `--latest`, before approving.
...
### `bot devices remove <deviceId>`

Remove one paired device entry.

```bash
bot devices remove <deviceId>
bot devices remove <deviceId> --json
```
...
### `bot devices rename --device <id> --name <label>`
...
```bash
bot devices rename --device <deviceId> --name "Kitchen Mac"
bot devices rename --device <deviceId> --name "Kitchen Mac" --json
```
...
### `bot devices clear --yes [--pending]`

Clear paired devices in bulk. Gated by `--yes`.

```bash
bot devices clear --yes
bot devices clear --yes --pending
bot devices clear --yes --pending --json
```
...
3. Rotate the operator token for the affected device:
   ...
   ```bash
   bot devices rotate --device <deviceId> --role operator
   ```
...
## Paperclip / `bot_gateway` first-run approval

Paperclip agents connecting through the `bot_gateway` adapter go through the same first-run device pairing approval as any other new client. If Paperclip reports `bot_gateway_pairing_required`, approve the pending device and retry.

```bash
bot devices approve --latest
```

The preview prints the exact `bot devices approve <requestId>` command; verify the details, then rerun that command with the request ID to approve it. For a remote gateway or explicit credentials, pass the same options while previewing and approving:

```bash
bot devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `bot devices list` first to confirm a pending request exists.
