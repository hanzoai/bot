---
summary: "CLI reference for provisioning and managing isolated per-tenant Bot cells"
read_when:
...
`bot fleet` manages complete Bot instances called **cells**. Each cell has its own Gateway, state, credentials, channel accounts, container, and loopback-only host port. Use one cell for each tenant trust boundary; do not use one shared Gateway as a hostile multi-tenant boundary.
...
Fleet supports Docker and Podman. The default image is `ghcr.io/hanzoai/bot:latest`.
...
The ID becomes part of the container name: `bot-cell-<tenant>`.
...
| ------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `--image <ref>`           | `ghcr.io/hanzoai/bot:latest`    | Container image for the cell.                                                                  |
...
Create and upgrade accept digest-pinned image references such as `--image ghcr.io/hanzoai/bot@sha256:<digest>`. Fleet passes the image reference through verbatim to Docker or Podman, which lets an operator keep a cell on immutable image bytes instead of a moving tag.
...
```bash
bot fleet logs acme
bot fleet logs acme --follow
bot fleet logs acme --tail 200
bot fleet logs acme --since 10m
```
...
```bash
bot fleet start acme
bot fleet stop acme
bot fleet restart acme
```
...
```bash
bot fleet upgrade acme --image ghcr.io/hanzoai/bot:<version>
```
...
The Gateway token is intentionally not stored in the fleet registry. Before removing the old container, Fleet reads its environment and carries `BOT_GATEWAY_TOKEN` into the replacement. Do not manually remove the old container before an upgrade if the token exists nowhere else you control.
...
```bash
bot fleet stop acme
bot fleet backup acme --out ./acme.tgz
```

Restore that archive into the registered cell:

```bash
bot fleet restore acme --from ./acme.tgz
```
...
Cell state and auth-profile encryption keys use separate per-tenant host paths under the active Bot state directory:
...
| `BOT_GATEWAY_TOKEN` | Generated or supplied cell token     |
