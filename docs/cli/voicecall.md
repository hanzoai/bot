---
summary: "CLI reference for `bot voicecall` (voice-call plugin command surface)"
read_when:
...
bot voicecall smoke    [-t <phone>] [--message <text>] [--mode <m>] [--yes] [--json]
bot voicecall call     -m <text> [-t <phone>] [--mode <m>]
bot voicecall start    --to <phone> [--message <text>] [--mode <m>]
bot voicecall continue --call-id <id> --message <text>
bot voicecall speak    --call-id <id> --message <text>
bot voicecall dtmf     --call-id <id> --digits <digits>
bot voicecall end      --call-id <id>
bot voicecall status   [--call-id <id>] [--json]
bot voicecall tail     [--file <path>] [--since <n>] [--poll <ms>]
bot voicecall latency  [--file <path>] [--last <n>]
bot voicecall expose   [--mode <m>] [--path <p>] [--port <port>] [--serve-path <p>]
```
...
| `--message <text>` | `Bot voice call smoke test.` | Message to speak during the smoke call. |
...
```bash
bot voicecall smoke
bot voicecall smoke --to "+15555550123"        # dry run
bot voicecall smoke --to "+15555550123" --yes  # live notify call
```
...
```bash
bot voicecall call --to "+15555550123" --message "Hello"
bot voicecall call -m "Heads up" --mode notify
```
...
```bash
bot voicecall status
bot voicecall status --json
