---
summary: "CLI reference for `bot skills` (search/install/update/verify/list/info/check/workshop)"
read_when:
...
bot skills search --limit 20 --json
bot skills install @owner/<slug>
bot skills install @owner/<slug> --version <version>
bot skills install skills-sh:<owner>/<repo>/<slug>
bot skills install git:owner/repo
bot skills install git:owner/repo@main
bot skills install ./path/to/skill --as custom-name
bot skills install @owner/<slug> --force
bot skills install @owner/<slug> --force-install
bot skills install @owner/<slug> --acknowledge-clawhub-risk
bot skills install @owner/<slug> --agent <id>
bot skills install @owner/<slug> --global
bot skills update @owner/<slug>
bot skills update @owner/<slug> --force-install
bot skills update @owner/<slug> --acknowledge-clawhub-risk
bot skills update @owner/<slug> --global
bot skills update --all
bot skills update --all --agent <id>
bot skills update --all --global
bot skills verify @owner/<slug>
bot skills verify @owner/<slug> --version <version>
bot skills verify @owner/<slug> --tag <tag>
bot skills verify @owner/<slug> --card
bot skills verify @owner/<slug> --global
bot skills list
bot skills list --eligible
bot skills list --json
bot skills list --verbose
bot skills list --agent <id>
bot skills info <name>
bot skills info <name> --json
bot skills info <name> --agent <id>
bot skills check
bot skills check --agent <id>
bot skills check --json
bot skills workshop propose-create --name "qa-check" --description "QA checklist" --proposal ./PROPOSAL.md
bot skills workshop propose-update qa-check --proposal ./PROPOSAL.md
bot skills workshop list
bot skills workshop inspect <proposal-id>
bot skills workshop revise <proposal-id> --proposal ./PROPOSAL.md
bot skills workshop apply <proposal-id>
bot skills workshop reject <proposal-id> --reason "Not reusable"
bot skills workshop quarantine <proposal-id> --reason "Needs security review"
```
...
Bot does not download from skills.sh. These entries are shown as
...
or zip/archive paths, and `bot skills update` updates ClawHub-tracked
installs only.
...
| `verify` provenance              | When ClawHub returns server-resolved source provenance, verify JSON also includes a commit-pinned `bot.verifiedSourceUrl`. Unavailable or self-declared source URLs stay only in the raw provenance envelope and are not promoted.                                           |
...
| Skill Card fingerprint           | Installed ClawHub bundles can include a generated `skill-card.md`. Bot treats verification as a ClawHub server decision and does not reject an installed skill just because that generated card changes the bundle fingerprint.                                              |
...
skill publishers and bundled Bot skill sources bypass this release-trust
prompt.

## Skill Workshop

`bot skills workshop` manages pending skill proposals in the selected
...
```bash
bot skills workshop propose-create \
  --name "qa-check" \
...
bot skills workshop revise <proposal-id> --proposal ./PROPOSAL.md
bot skills workshop apply <proposal-id>
bot skills workshop reject <proposal-id> --reason "Duplicate"
bot skills workshop quarantine <proposal-id> --reason "Needs security review"
```
