---
summary: "CLI reference for `bot secrets` (reload, audit, configure, apply)"
read_when:
...
```bash
bot secrets reload
bot secrets reload --json
bot secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
```bash
bot secrets configure
bot secrets configure --plan-out /tmp/bot-secrets-plan.json
bot secrets configure --apply --yes
bot secrets configure --providers-only
bot secrets configure --skip-provider-setup
bot secrets configure --agent ops
bot secrets configure --json
```
...
- Targets secret-bearing fields in `bot.json` plus `auth-profiles.json` for the selected agent scope; canonical supported surface: [SecretRef Credential Surface](/reference/secretref-credential-surface).
...
bot secrets apply --from /tmp/bot-secrets-plan.json --json
```
