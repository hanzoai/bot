---
summary: "CLI reference for `bot path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
Bot state is spread across human-edited markdown, commented JSONC
...
`bot path` gives those callers a stable address instead of a one-off
...
Skip `bot path` for ordinary whole-file edits, rich config migrations, or
...
```bash
bot path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
bot path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
bot path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
bot path resolve 'oc://AGENTS.md/runtime-safety/bot-gateway'
```
...
```bash
bot path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
bot path resolve 'oc://gateway.jsonc/version'

# Wildcard search
bot path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
bot path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
bot path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
bot path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
bot path emit ./AGENTS.md
```
...
bot path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
bot path set 'oc://bot.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
bot path set 'oc://bot.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
bot path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
bot path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
bot path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
bot path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
bot path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
bot path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
bot path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
bot path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
bot path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
bot path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
bot path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ bot path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ bot path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ bot path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ bot path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ bot path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ bot path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ bot path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
```bash
$ bot path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ bot path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
bot path resolve 'oc://AGENTS.md/tools/gh/risk' --human
bot path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
bot path find 'oc://AGENTS.md/tools/**/risk'
bot path find 'oc://session.jsonl/[event=action]/userId'
bot path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
bot path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
bot path set 'oc://gateway.jsonc/version' '2.0'
bot path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ bot path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
bot path emit ./AGENTS.md
bot path emit ./gateway.jsonc --json
```
...
`bot path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__BOT_REDACTED__` (verbatim or as a substring) is refused at write
  time.
