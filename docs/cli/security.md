---
summary: "CLI reference for `bot security` (audit and fix common security footguns)"
read_when:
...
bot security audit --deep --password <password>
bot security audit --deep --token <token>
bot security audit --auth password --password <password>
...
Run `bot doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new token.
...
- Warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `bot.browserConfigEpoch`) and recommends `bot sandbox recreate --browser --all`.
...
```bash
bot security audit --json | jq '.summary'
bot security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

With `--fix --json`, output includes both fix actions and the final report:

```bash
bot security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files (`credentials/*.json`, `auth-profiles.json`, `bot-agent.sqlite`, and legacy session artifacts)
- also tightens config include files referenced from `bot.json`
