...
This is the full reference for `bot onboard`.
...
<Steps>
  <Step title="Reset (optional)">
    ...
    - If the config file is invalid, onboarding stops and tells you to run
      `bot doctor` first, then re-run setup.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic Claude CLI**: preferred local path when a Claude CLI sign-in already exists; Bot still supports Anthropic setup-token auth as an alternative.
    ...
    - Adding or reauthenticating OpenAI preserves an existing explicit primary model, including `openai/gpt-5.5`. If the account does not expose GPT-5.6, select `openai/gpt-5.5` explicitly; Bot does not silently downgrade the model.
    ...
    - Auth profiles live in `~/.bot/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.bot/credentials/oauth.json` is legacy import-only.
  ...
  <Step title="Web search">
    ...
    - Configure later: `bot configure --section web`.
  ...
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first; if task creation is denied, Bot falls back to a per-user Startup-folder login item and starts the Gateway immediately.
  ...
  <Step title="Health check">
    ...
    - Tip: `bot status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  ...
  <Step title="Skills (recommended)">
    ...
    - Skips skills whose Homebrew, uv, or Go installer prerequisite is unavailable, groups them with manual setup guidance, and points you at `bot doctor` once the prerequisite is installed.
...
`main` is a reserved agent id and cannot be used for `bot agents add`.
...
- Linux x86-64: downloads the official native GraalVM build from the `signal-cli` GitHub releases and stores it under `~/.bot/tools/signal-cli/<version>/`.
