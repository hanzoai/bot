...
Bot agents use a workspace directory. Default: `~/.bot/workspace` (configurable via `agents.defaults.workspace`, supports `~`).
...
Before proposing or building a custom system, feature, workflow, tool, integration, or automation, check for open-source projects, maintained libraries, existing Bot plugins, or free platforms that already solve it well enough. Prefer those when adequate. Build custom only when existing options are unsuitable, too expensive, unmaintained, unsafe, non-compliant, or the user explicitly asks for custom. Avoid paid-service recommendations unless the user explicitly approves spend. Keep this lightweight, a preflight gate, not a research assignment.
...
- The macOS app manages permissions (screen recording, notifications, microphone) and exposes the `bot` CLI via its bundled binary.
...
- Prefer the `bot` CLI for scripting; the desktop app handles permissions.
...
- For browser-driven verification, use the `bot browser` CLI (bundled `browser` plugin) with the Bot-managed Chrome/Brave/Edge/Chromium profile.
