---
summary: "Bot SQLite database locations, schema versions, integrity checks, and downgrade recovery"
read_when:
  - Diagnosing a newer database schema error
  - Checking database compatibility before an update or downgrade
  - Recovering a database for an older Bot release
title: "Database schemas"
---

Bot stores control-plane state in a global SQLite database and agent data in one SQLite database per agent. Schema migrations run forward when a database opens. Older Bot builds refuse databases written by a newer schema.

## Database layout

| Scope                | Default path                                               | Contents                                                                                              |
| -------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Global control plane | `~/.bot/state/bot.sqlite`                        | Shared configuration state, registries, approvals, plugin state, and shared runtime state             |
| Per-agent data plane | `~/.bot/agents/<agentId>/agent/bot-agent.sqlite` | Sessions, transcripts, memory indexes, auth state, conversation state, and agent-scoped runtime state |
...
- The primary `schema_meta` row records `role`, `agent_id`, `schema_version`, and `app_version`. `app_version` is the Bot build that last wrote the schema metadata.

Bot applies forward-only migrations when it opens an older supported database. It refuses a database whose `user_version` is newer than the running build and reports a `newer schema version` error. The Gateway checks all registered databases before startup. `bot update` also refuses a package or source target whose declared schema support is older than an on-disk database. Target packages published before schema metadata was added cannot be preflighted.

Installing Bot manually through npm bypasses the updater guard. Database open checks still refuse an incompatible build.

## Agent schema history

| Version | Change                                                                                                                                                                                                                                                         | First release                                   |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| 1       | Initial per-agent store ([#88349](https://github.com/hanzoai/bot/pull/88349))                                                                                                                                                                            | `v2026.5.30-beta.1`, stable through `v2026.7.1` |
| 2       | Memory index identity ([#104449](https://github.com/hanzoai/bot/pull/104449))                                                                                                                                                                            | `v2026.7.2-beta.1`                              |
| 4       | Sessions and transcripts moved into SQLite ([#98236](https://github.com/hanzoai/bot/pull/98236))                                                                                                                                                         | `v2026.7.2-beta.1`                              |
| 5-6     | Terminal freshness and state lifecycle ([#104859](https://github.com/hanzoai/bot/pull/104859))                                                                                                                                                           | `v2026.7.2-beta.1`                              |
| 7       | Per-entry lifecycle status projection ([#106151](https://github.com/hanzoai/bot/pull/106151))                                                                                                                                                            | `v2026.7.2-beta.1`                              |
| 8       | Per-transcript session provenance ([#106766](https://github.com/hanzoai/bot/pull/106766))                                                                                                                                                                | `v2026.7.2-beta.2`                              |
| 9       | `STRICT` tables ([#108663](https://github.com/hanzoai/bot/pull/108663))                                                                                                                                                                                  | `v2026.7.2-beta.2`                              |
| 10      | Materialized active transcript paths ([#108851](https://github.com/hanzoai/bot/pull/108851))                                                                                                                                                             | Unreleased                                      |
| 11      | Leases, durable delivery, conversation addresses, and heartbeat outcomes ([#109636](https://github.com/hanzoai/bot/pull/109636), [#95838](https://github.com/hanzoai/bot/pull/95838), [#109999](https://github.com/hanzoai/bot/pull/109999)) | Unreleased                                      |
...
| 2       | Metadata-only message audit events ([#103903](https://github.com/hanzoai/bot/pull/103903))         | `v2026.7.2-beta.1`  |
| 3       | `STRICT` tables and schema-drift hardening ([#108663](https://github.com/hanzoai/bot/pull/108663)) | `v2026.7.2-beta.2`  |
...
2. Run the older build against a separate state directory (`BOT_STATE_DIR`). It starts fresh; your migrated data stays untouched for when you return to the newer build.
3. Follow the manual downgrade procedure below. It is unsupported and risks data loss without a verified backup.

Since 2026.7.2, `bot update` refuses to install a release that cannot open your current databases, so the updater will not put you in this situation. Installing an older version manually through npm bypasses that guard; the databases still refuse the old binary, but only after it is installed.
