...
read_when:
  - You want an Bot agent to join a Zoom meeting
  - You are configuring Chrome, BlackHole, or SoX for Zoom meeting talk-back
title: "Zoom meetings plugin"
---

The `zoom-meetings` plugin joins Zoom meeting links as a guest through the Zoom Web App in the Bot Chrome profile. It accepts meeting links under `zoom.us/j/...` and account subdomains such as `example.zoom.us/j/...`. It does not create meetings, dial in, use the Zoom Meeting SDK, or capture audio/video recordings.
...
Run `bot plugins disable zoom-meetings` if you do not want the plugin active.

```bash
bot zoommeetings setup
bot zoommeetings join 'https://zoom.us/j/1234567890'
```
...
| Mode         | Behavior                                                                    |
| ------------ | --------------------------------------------------------------------------- |
| `agent`      | Realtime transcription consults the configured Bot agent; TTS replies. |
...
Zoom live captions are enabled after admission in every mode so Bot can
persist meeting notes. The `transcript` action still returns the bounded live
buffer only for `transcribe` sessions. On leave, Bot stores the durable
transcript and derived summary in the shared state database; list or export
them with [`bot transcripts`](/cli/transcripts).
...
Zoom host and account policy can disable browser join, require authentication or email verification, show a CAPTCHA, or require host admission. Complete that step in the Bot Chrome profile, then retry status or speech. The plugin does not bypass Zoom policy.
