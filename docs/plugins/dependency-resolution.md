---
summary: "How Bot installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Bot installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Bot handles plugin dependencies at install/update time only. Runtime
loading never runs a package manager, repairs a dependency tree, or mutates
the Bot package directory.
...
- SDK/core imports are peer or supplied Bot imports.
- Local development plugins bring their own already-installed dependencies.
- npm and git plugins install into Bot-owned package roots.

Bot owns only the plugin lifecycle:
...
Bot uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.bot/npm/projects/<encoded-package>`.
- git packages clone under `~/.bot/git`.
...
```bash
cd ~/.bot/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`bot plugins install npm-pack:<path.tgz>` uses the same per-plugin npm
project root for a local npm-pack tarball: Bot reads the tarball's npm
...
`~/.bot/npm/projects/<encoded-package>` can unblock a temporary
...
`node_modules` beside the plugin package. Bot scans the managed project
...
Bot-owned npm plugin packages never ship npm lockfiles. The repository
...
lockfiles according to their own packaging policy; Bot's installer leaves
...
Bot-owned npm plugin packages can also publish with explicit
...
`bot.release.bundleRuntimeDependencies: false`; they still ship a
...
`bot` package also resolves dependencies at install time and does not
bundle its full dependency tree. See
[dependency locking](/gateway/security/dependency-locking).

Plugins that import `bot/plugin-sdk/*` declare `bot` as a peer
dependency. Bot does not let npm install a separate registry copy of the
...
resolution/materialization, and Bot reasserts plugin-local
`node_modules/bot` links for installed packages that declare the host
...
Local plugins are developer-controlled directories. Bot never runs
...
bot plugins install <source>
bot doctor --fix
```

`doctor --fix` cleans legacy Bot-generated dependency state and can
...
Lightweight and core-critical bundled plugins ship as part of Bot. They
...
In source checkouts, Bot treats the repository as a pnpm monorepo.
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g bot`        | Built runtime tree inside the package | Bot package and explicit plugin install/update/doctor flows     |
...
| `bot plugins install ...`   | Managed npm project/git/ClawHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older Bot versions generated bundled-plugin dependency roots at startup
...
`plugin-runtime-deps` targets, `.bot-runtime-deps*` manifests, generated
...
Older npm installs also used a shared `~/.bot/npm/node_modules` root.
