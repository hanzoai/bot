---
summary: "Create your first Bot plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new Bot plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend Bot without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the Bot repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
bot plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because Bot discovers
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Bot to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through Bot model fallback.
  </Card>
...
```json bot.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to Bot",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    appear in `contracts.tools` so Bot can discover ownership without
  ...
  <Step title="Register the tool">
    ...
    `defineChannelPluginEntry` from `bot/plugin-sdk/core` instead.
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    bot plugins inspect my-plugin --runtime --json
    ```

    If the plugin registers a CLI command, run that command too and confirm
    output, for example `bot demo-plugin ping`.

    For a bundled plugin in this repository, Bot discovers source-checkout
  ...
  <Step title="Test the package install">
    ...
    `bot.extensions` at built JavaScript like `./dist/index.js`, and make
    ...
    bot plugins install npm-pack:/tmp/<plugin-package>.tgz --force
    bot plugins inspect my-plugin --runtime --json
    ```

    `npm-pack:` uses Bot's managed per-plugin npm project, so it catches
  ...
  <Step title="Install">
    ...
    ```bash
    bot plugins install clawhub:your-org/your-plugin
    ```
...
plugin is enabled. Optional tools need explicit user opt-in before Bot
...
operator, installed plugin code, or a modified Bot runtime. Sensitive
...
aligned with `api.registerTool(..., { optional: true })` so Bot can avoid
...
`bot/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `bot` metadata</Check>
<Check>**bot.plugin.json** manifest is present and valid</Check>
...
1. Watch [hanzoai/bot](https://github.com/hanzoai/bot/releases) releases (`Watch` > `Releases`). Beta tags look like `v2026.3.N-beta.1`. You can also follow [@bot](https://x.com/bot) on X for release announcements.
