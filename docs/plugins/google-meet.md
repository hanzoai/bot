...
read_when:
  - You want an Bot agent to join a Google Meet call
  - You want an Bot agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

The `google-meet` plugin joins explicit Meet URLs on behalf of an Bot agent. It is deliberately narrow:
...
- `mode: "agent"` (default) transcribes participant speech with a realtime provider, routes it to the configured Bot agent, and speaks the answer with regular Bot TTS. `mode: "bidi"` lets a realtime voice model answer directly. `mode: "transcribe"` joins observe-only with no talk-back.
...
Run `bot plugins disable google-meet` if you do not want the plugin active.

Check setup, then join:

```bash
bot googlemeet setup
bot googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
bot googlemeet setup --transport chrome-node --mode transcribe
```
...
```bash
bot googlemeet setup --transport twilio
```
...
```bash
bot googlemeet create --transport chrome-node --mode agent
bot googlemeet create --no-join
```
...
- **`browser`**: used without OAuth credentials. Bot opens `https://meet.google.com/new` on the pinned Chrome node and waits for Google to redirect to a real meeting-code URL; the Bot Chrome profile on that node must already be signed in to Google. Join and create both reuse an existing Meet tab (or an in-progress `.../new` / Google account prompt tab) before opening a new one; tab matching ignores harmless query strings like `authuser`.
...
```bash
bot googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
This only applies to API-created rooms, so OAuth must be configured. If you authenticated before this option existed, rerun `bot googlemeet auth login --json` after adding the `meetings.space.settings` scope to your OAuth consent screen.
...
```bash
bot googlemeet test-listen <meet-url> --transport chrome-node
```
...
Local Chrome joins through the signed-in Bot browser profile and needs `BlackHole 2ch` for the mic/speaker path. A single BlackHole device is enough for a first smoke test but can echo; use separate virtual devices or a Loopback-style graph for clean duplex audio.
...
| Runs where           | What                                                                                            |
| -------------------- | ----------------------------------------------------------------------------------------------- |
| Gateway host         | Bot Gateway, agent workspace, model/API keys, realtime provider, Google Meet plugin config |
| Parallels macOS VM   | Bot CLI/node host, Chrome, SoX, BlackHole 2ch, a Chrome profile signed in to Google        |
...
```bash
bot plugins install npm:@hanzo/bot-google-meet
bot node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
BOT_ALLOW_INSECURE_PRIVATE_WS=1 \
  bot node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```

Use the same flag when installing as a LaunchAgent (it is process environment, stored in the LaunchAgent environment when present on the install command, not an `bot.json` setting):

```bash
BOT_ALLOW_INSECURE_PRIVATE_WS=1 \
  bot node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
bot node restart
```
...
```bash
bot googlemeet test-speech https://meet.google.com/abc-defg-hij
```
...
If `chromeNode.node` is omitted, Bot auto-selects only when exactly one connected node advertises both `googlemeet.chrome` and browser control; pin `chromeNode.node` (node id, display name, or remote IP) when several capable nodes are connected.
...
| `No connected Google Meet-capable node`                  | Install `npm:@hanzo/bot-google-meet` in the VM, run `bot plugins enable browser`, start `bot node run`, and approve pairing. If Google Meet was explicitly disabled, enable it too. Confirm `gateway.nodes.commands.allow` includes `googlemeet.chrome` and `browser.proxy`. |
...
| Chrome opens but cannot join                             | Sign in to the browser profile in the VM, or keep `chrome.guestName` set. Guest auto-join uses Bot browser automation through the node browser proxy; point the node's `browser.defaultProfile` (or a named existing-session profile) at the profile you want.                   |
| Duplicate Meet tabs                                      | Leave `chrome.reuseExistingTab: true`. Bot activates an existing tab for the same URL, and creation reuses an in-progress `.../new` or Google account prompt tab, before opening another.                                                                                        |
| No audio                                                 | Route Meet mic/speaker through the virtual audio path used by Bot; use separate virtual devices or Loopback-style routing for clean duplex audio.                                                                                                                                |

## Install notes

The Chrome talk-back default uses two external tools that Bot does not bundle or redistribute; install them as host dependencies through Homebrew:
...
SoX is licensed `LGPL-2.0-only AND GPL-2.0-only`; BlackHole is GPL-3.0. If you build an installer or appliance that bundles BlackHole with Bot, review BlackHole's upstream licensing or get a separate license from Existential Audio.
...
Opens the Meet URL through Bot browser control and joins as the signed-in Bot browser profile. On macOS, the plugin checks for `BlackHole 2ch` before launch and, if configured, runs an audio bridge health/startup command before opening Chrome. For local Chrome, pick the profile with `browser.defaultProfile`; `chrome.browserProfile` is passed to `chrome-node` hosts instead.

```bash
bot googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
bot googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Chrome mic/speaker audio routes through the local Bot audio bridge. If `BlackHole 2ch` is not installed, the join fails with a setup error instead of joining without an audio path.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Bot agent. Be brief.",
...
Provide Twilio credentials through environment to keep secrets out of `bot.json`:
...
```bash
bot config validate
bot plugins list | grep -E 'google-meet|voice-call'
bot googlemeet setup
```
...
`meetings.space.created` is required by `spaces.create`. `meetings.space.readonly` resolves Meet URLs/codes to spaces. `meetings.space.settings` lets Bot pass `SpaceConfig` settings such as `accessType` during API room creation. `meetings.conference.media.readonly` is for Meet Media API preflight and media work; Google may require Developer Preview enrollment for actual Media API use. `calendar.events.readonly` is only needed for `--today`/`--event` calendar lookup. `drive.meet.readonly` is only needed for `--include-doc-bodies` export. If you only need browser-based Chrome joins, skip OAuth entirely.
...
```bash
bot googlemeet auth login --json
```
...
```bash
BOT_GOOGLE_MEET_CLIENT_ID="your-client-id" \
BOT_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
bot googlemeet auth login --json --manual
```
...
```bash
bot googlemeet doctor --oauth --create-space --json
```

Prove read access to an existing space:

```bash
bot googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
bot googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```

A `403` from these checks usually means the Meet REST API is disabled, the refresh token is missing the required scope, or the Google account cannot access that space. A refresh-token error means rerun `bot googlemeet auth login --json` and store the new `oauth` block.

No OAuth is needed for the browser fallback; Google auth there comes from the signed-in Chrome profile on the selected node, not Bot config.

These environment variables are accepted as fallbacks:

- `BOT_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `BOT_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `BOT_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `BOT_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `BOT_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `BOT_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `BOT_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`

### Resolve, preflight, and read artifacts

```bash
bot googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
bot googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

After Meet has created conference records:

```bash
bot googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
bot googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
bot googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
bot googlemeet latest --today
bot googlemeet calendar-events --today --json
bot googlemeet artifacts --event "Weekly sync"
bot googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
bot googlemeet latest --meeting https://meet.google.com/abc-defg-hij
bot googlemeet artifacts --conference-record conferenceRecords/abc123 --json
bot googlemeet attendance --conference-record conferenceRecords/abc123 --json
```

Close the room for an API-created space:

```bash
bot googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```

Calls `spaces.endActiveConference` and requires OAuth with the `meetings.space.created` scope for a space the authorized account can manage. Accepts a Meet URL, meeting code, or `spaces/{id}` and resolves it to the API space resource first. This is separate from `googlemeet leave`: `leave` stops Bot's local/session participation; `end-active-conference` asks Google Meet to end the active conference for the space.

Write a readable report:

```bash
bot googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
bot googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
bot googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
BOT_LIVE_TEST=1 \
BOT_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
pnpm test:live -- extensions/google-meet/google-meet.live.test.ts
```

```bash
bot googlemeet setup --transport chrome-node --mode transcribe
bot googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

| Variable                                                                                                                  | Purpose                                                                |
| ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `BOT_LIVE_TEST=1`                                                                                                    | Enables guarded live tests                                             |
| `BOT_GOOGLE_MEET_LIVE_MEETING`                                                                                       | Retained Meet URL, code, or `spaces/{id}`                              |
| `BOT_GOOGLE_MEET_CLIENT_ID` / `GOOGLE_MEET_CLIENT_ID`                                                                | OAuth client id                                                        |
| `BOT_GOOGLE_MEET_REFRESH_TOKEN` / `GOOGLE_MEET_REFRESH_TOKEN`                                                        | Refresh token                                                          |
| `BOT_GOOGLE_MEET_CLIENT_SECRET`, `BOT_GOOGLE_MEET_ACCESS_TOKEN`, `BOT_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` | Optional; same fallback names without the `BOT_` prefix also work |
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Bot browser profile, then retry meeting creation.",
  "manualAction": {
...
Creating joins by default, but Chrome/Chrome-node still needs a signed-in Google profile to join through the browser; if signed out, Bot returns `manualAction` or a browser fallback error and asks the operator to finish Google login before retrying.
...
The common Chrome agent path only needs the plugin enabled, BlackHole, SoX, a realtime provider key, and a configured Bot TTS provider:
...
| `chrome.guestName`                | `"Bot Agent"`                       | Shown on the signed-out Meet guest screen                                                                                                                                                                         |
...
| `realtime.instructions`           | brief spoken-reply instructions          | Tells the model to speak briefly and use `bot_agent_consult` for deeper answers                                                                                                                              |
...
| `realtime.agentId`                | `"main"`                                 | Bot agent id used for `bot_agent_consult`                                                                                                                                                               |
...
An `bot doctor --fix` migration exists for the legacy `realtime.provider: "google"` shape: it moves that intent to `realtime.voiceProvider: "google"` plus `realtime.transcriptionProvider: "openai"` when those fields are not already set.
...
| `speechReady` / `speechBlockedReason` / `speechBlockedMessage` | Whether managed Chrome speech is allowed now; `speechReady: false` means Bot did not send the intro/test phrase   |
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Bot agent | Normal Bot TTS runtime            | You want "my agent is in the meeting" behavior        |
...
```bash
bot googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `bot logs --follow` shows DTMF TwiML served before realtime TwiML, then a realtime bridge with the initial greeting queued.
...
```bash
bot plugins list | grep google-meet
bot googlemeet setup
```
...
- **API creation**: `oauth.clientId` and `oauth.refreshToken` (or matching `BOT_GOOGLE_MEET_*` env vars) are present, and the refresh token was minted after create support was added; older tokens may lack `meetings.space.created`, so rerun `bot googlemeet auth login --json`.
- **Browser fallback**: `defaultTransport: "chrome-node"` and `chromeNode.node` point at a connected node with `browser.proxy` and `googlemeet.chrome`; the Bot Chrome profile on that node is signed in and can open `https://meet.google.com/new`.
...
- **Audio-choice interstitial**: if Meet shows "Do you want people to hear you in the meeting?", leave the tab open. Bot should click **Use microphone** or (create-only) **Continue without microphone** and keep waiting for the generated URL; if it cannot, the error should mention `meet-audio-choice-required`, not `google-login-required`.

### Agent joins but does not talk

```bash
bot googlemeet setup
bot googlemeet doctor
```

Use `mode: "agent"` for the STT -> Bot agent -> TTS path, `mode: "bidi"` for the direct realtime voice fallback. `mode: "transcribe"` intentionally starts no talk-back bridge. For observe-only debugging, run `bot googlemeet status --json <session-id>` after participants speak and check `captioning`, `transcriptLines`, `lastCaptionText`. If `inCall` is true but `transcriptLines` stays `0`, Meet captions may be disabled, no one has spoken since the observer was installed, the Meet UI changed, or live captions are unavailable for the meeting language/account.

`googlemeet test-speech` always checks the realtime path and reports whether bridge output bytes were observed for that invocation. If `speechOutputVerified` is false and `speechOutputTimedOut` is true, the realtime provider may have accepted the utterance but Bot did not see new output bytes reach the Chrome audio bridge.
...
```bash
bot googlemeet recover-tab
bot googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
bot voicecall smoke --to "+15555550123"
```

Only add `--yes` to intentionally place a live outbound call:

```bash
bot voicecall smoke --to "+15555550123" --yes
```
...
- `bot googlemeet doctor <session-id>`: confirm the delegated Twilio call ID, whether DTMF was queued, and whether the intro greeting was requested.
- `bot voicecall status --call-id <id>`: confirm the call is still active.
- `bot voicecall tail`: confirm Twilio webhooks are arriving at the Gateway.
- `bot logs --follow`: look for the Twilio Meet sequence: Google Meet delegates the join, Voice Call stores and serves pre-connect DTMF TwiML, Voice Call serves realtime TwiML for the Twilio call, then Google Meet requests intro speech with `voicecall.speak`.
- Re-run `bot googlemeet setup --transport twilio`; a green setup check is required but does not prove the meeting PIN sequence is correct.
...
- If the participant joins but you do not hear the greeting, check `bot logs --follow` for the post-DTMF `voicecall.speak` request and either media-stream TTS playback or the Twilio `<Say>` fallback. If the transcript still shows "enter the meeting PIN", the phone leg has not joined the Meet room yet, so participants will not hear speech.
