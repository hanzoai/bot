...
- A Platform (API-key) route refuses a native home signed in with a ChatGPT
  ...
  and let Bot inject the key it already holds.

A stored OpenAI profile is fine alongside `homeScope: "user"`; Bot keeps it
for agent-scoped connections and simply does not hand it to the native home. Use
`bot models auth list --provider openai` to inspect stored profiles and
`bot models auth logout <profileId> --yes` to remove one you no longer want.
