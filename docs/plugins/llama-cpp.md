---
summary: "Run local GGUF text inference and memory embeddings in Bot with llama.cpp"
read_when:
  ...
  - You need the Bot plugin that owns the node-llama-cpp runtime
...
The main `bot` npm package does not include `node-llama-cpp`. Keeping the
native dependency in this plugin prevents normal Bot npm updates from
deleting a manually installed runtime inside the Bot package directory.

## Local text inference

Choose **Local model (llama.cpp)** during interactive onboarding. Bot asks
...
RAM for model weights, plus context and Bot runtime overhead. The default
...
to Bot for execution rather than running inside node-llama-cpp.
...
Run `bot memory status --deep` after the provider has loaded to inspect
...
The same last-known facts can appear in `bot doctor` when the running
...
If `node-llama-cpp` is missing or fails to load, Bot reports the failure
with:

1. Install the plugin: `bot plugins install @hanzo/bot-llama-cpp-provider`.
