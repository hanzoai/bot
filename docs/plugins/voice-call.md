...
read_when:
  - You want to place an outbound voice call from Bot
...
Voice calls for Bot via a plugin: outbound notifications, multi-turn
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        bot plugins install @hanzo/bot-voice-call
        ```
      </Tab>
...
| `store`                         | unset        | Overrides the default `$BOT_STATE_DIR/voice-calls` path (normally `~/.bot/voice-calls`). |
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    release (`2026.6.0`), so run `bot doctor --fix` to rewrite committed
...
- Voice Call exposes the shared `bot_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Bot tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `bot_agent_consult`.
...
configured Bot agent without paying a full agent-consult round trip on
...
`bot_agent_consult` still run the full Bot agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call bot_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `bot doctor --fix`; committed config should use `tts.providers.<provider>`.
...
Provider signatures are checked against the public URL Bot reconstructs
...
Use `bot logs --follow` for the live phase trace. A healthy Twilio Meet
...
`bot voicecall tail` still shows persisted call records; useful for
...
- `bot logs --follow` shows realtime TwiML served, the realtime bridge started, and the initial greeting queued.
