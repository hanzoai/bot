---
summary: "Bundled `oc-path` plugin: ships the `bot path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`bot path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Bot repo under
...
- **Local automation**: shell scripts resolve or update one workspace value
  with `bot path … --json` instead of carrying separate markdown, JSONC,
...
bot path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
bot path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `bot` CLI** on the host where you
...
Plugin metadata lives in `extensions/oc-path/bot.plugin.json`:
...
lazily the first time you run `bot path …`, so installs that never use
...
state. Bare `bot path` invocations work immediately on the same host;
the CLI loads the plugin on demand.

Disable with:

```bash
bot plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `bot path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__BOT_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`bot path` CLI reference](/cli/path)
