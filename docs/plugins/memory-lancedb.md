...
```bash
bot plugins install @hanzo/bot-memory-lancedb
```

The plugin is published to npm; it is not bundled into the Bot runtime
...
`bot doctor` reports when Remember across conversations is unavailable
...
Bot has a built-in dimension for `text-embedding-3-small` (1536) and
...
`memory_recall` tool, the `memory_forget` query path, and `bot ltm
...
```bash
bot ltm list [--agent <id>] [--limit <n>] [--order-by-created-at]
bot ltm search <query> [--agent <id>] [--limit <n>]
bot ltm stats [--agent <id>]
```

`ltm query` runs a non-vector query directly against the LanceDB table:

```bash
bot ltm query --agent research --cols id,text,createdAt --limit 20
bot ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
LanceDB data defaults to `~/.bot/memory/lancedb`. Override with `dbPath`:
...
On upgrade, `bot doctor --fix` assigns those legacy rows once to the
...
plugin package (not the Bot core dist). Gateway startup does not repair
...
```bash
bot ltm stats
bot ltm search "recent preference"
```
