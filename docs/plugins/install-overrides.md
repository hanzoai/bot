...
[`bot plugins install`](/cli/plugins).
...
```bash
export BOT_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export BOT_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/bot-codex-2026.5.8.tgz",
  "bot-web-search": "npm:@hanzo/bot-web-search@2026.5.8"
}'
```
...
When a setup-time flow installs a plugin whose id appears in the map, Bot
...
- Overrides do not inherit official trusted-source status. Even when the
  catalog entry normally represents an Bot-owned package, an override is
...
BOT_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/bot-codex-2026.5.8.tgz"}' \
pnpm bot onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$BOT_STATE_DIR/npm/projects" -path '*/node_modules/@hanzo/bot-codex/package.json' -print
grep -R '"@hanzo/bot-codex"' "$BOT_STATE_DIR/npm/projects"/*/package-lock.json
```
