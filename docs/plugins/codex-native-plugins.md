---
summary: "Configure native Codex plugins for Codex-mode Bot agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Bot agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are configuring an existing workspace-directory Codex plugin
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Bot agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Bot turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. Bot does not translate
Codex plugins into synthetic `codex_plugin_*` Bot dynamic tools.
...
`codexPlugins` has no effect on Bot-provider runs, ACP conversation
...
```bash
bot migrate codex --dry-run --verify-plugin-apps
```
...
Bot does not call `plugin/install` or start authentication for a
`workspace-directory` plugin. Install, enable, and authenticate it in Codex
before adding or enabling the Bot policy. Bot keeps apps hidden when
...
Bot reports `marketplace_missing` for each enabled workspace plugin and
...
`enable`/`disable` write only to `~/.hanzoai/bot.json`; they never edit
...
`auth_required`, reauthorize the app in Codex before enabling it in Bot.
For a `workspace-directory` entry, enabling it here changes only Bot
...
| Enabled    | Codex reports the plugin enabled, and Bot config allows it for Codex harness turns.                                           |
...
- During planning, Bot reads source Codex `plugin/read` details and
...
For `workspace-directory` plugins, setup happens outside Bot. Bot
...
disabled, inaccessible, or unauthenticated plugin exposes no apps; Bot
...
Bot reads Codex app inventory through app-server `app/list`, caches it
...
drops it, and Bot rebuilds it from the next `app/list` read.
...
A plugin app is exposed only when Bot can map it back to the configured
...
Bot injects a restrictive `config.apps` patch for the Codex thread:
...
get `open_world_enabled: true`. Bot does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: Bot returns a deterministic decline.
- `true`: Bot auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: Bot exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into Bot plugin
  approvals before returning the Codex approval response.
- `"ask"`: Bot uses the same Codex write/destructive gating as
  ...
  admitted app using `"ask"`, Bot selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  Bot; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                    |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in Bot.                                                      |
...
| `plugin_detail_unavailable`                       | Bot could not read plugin ownership details.                                                                                    | Inspect the target app-server's `plugin/list` and `plugin/read` responses.                                             |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | Bot schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known.  |
...
account. Bot can enable an accessible app for the thread even when the
...
`/new` or `/reset`. Bot does not repair or authenticate workspace plugins.
...
`remotePluginId`; Bot keeps owned apps hidden when that selector or the
...
not already active; install, enable, and authenticate it outside Bot.
...
Codex thread bindings keep the app config they started with until Bot
