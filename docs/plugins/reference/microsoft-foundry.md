---
summary: "Adds Microsoft Foundry model provider support to Bot."
read_when:
...
Adds Microsoft Foundry model provider support to Bot.

## Distribution

- Package: `@hanzo/bot-microsoft-foundry`
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. Bot refreshes Microsoft Foundry runtime tokens through
...
Bot uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so Bot
...
deployment through onboarding or include model metadata so Bot can verify
