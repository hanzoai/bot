...
read_when:
  ...
  - You want a reference for all registration methods on BotPluginApi
...
<Note>
  This page is for plugin authors using `bot/plugin-sdk/*` inside
  Bot. For external apps, scripts, dashboards, CI jobs, and IDE extensions
...
prefer `bot/plugin-sdk/channel-core`; keep `bot/plugin-sdk/core` for
...
`bot.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. Bot's
...
<Warning>
  ...
  `bot/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`bot/plugin-sdk/discord` and `bot/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `BotPluginApi` object with these
methods:
...
`bot/plugin-sdk/session-discussion`. Its `info({ sessionKey })` method
...
| `api.registerNodeHostCommand(command)` | Command handled by `bot node run`; optional `agentTool` metadata can expose it as an agent-visible tool while the node is connected |
...
agentPromptGuidance: [
  "Global command hint.",
  { text: "Only show this in the main Bot prompt.", surfaces: ["bot_main"] },
];
```

Structured `surfaces` may include `bot_main`, `codex_app_server`,
`cli_backend`, `acp_backend`, or `subagent`. `pi_main` remains a deprecated alias
for `bot_main`. Omit `surfaces` for intentional all-surface guidance. Do
...
| `api.registerNodeCliFeature(registrar, opts?)`  | Node feature CLI under `bot nodes`                                |
...
| `api.registerMigrationProvider(provider)`       | Importer for `bot migrate`                                        |
...
| `api.registerSecurityAuditCollector(collector)` | Findings collector for `bot security audit`                       |
...
- Unauthenticated requesters on a shared-thread harness still see the advertised
  ...
  requester. Bot never falls back to another requester's credentials.
...
state, then return only lines for that run. Bot freezes those lines and
...
Telegram's normal inbound agent path after the handler succeeds. Bot keeps
...
plugin trust boundary; Bot does not treat installed plugins as mutually
...
runtime, for example `["bot", "codex"]`. Installed plugins without that
contract, or without explicit enablement, cannot register this middleware; keep
normal Bot plugin hooks for work that does not need pre-model tool-result
timing. The old
...
Gateway on a local discovery transport such as mDNS/Bonjour. Bot calls the
...
`bot nodes canvas` explicit plugin-owned node features.
...
without relying exclusively on a literal `--json` flag. Bot evaluates this
...
`bot/plugin-sdk/cli-argv` when the resolver needs command-path tokens; it
...
- Users select the backend through model refs or model-scoped `agentRuntime.id`;
  `bot.json` does not rewrite the adapter.
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping Bot thinking levels to a native effort
...
- Backends that can disable all native tools for a specific run may declare
  ...
  `ctx.toolAvailability.bot` names. Declare
  ...
  staged policy, and return `toolAvailabilityEnforced: true`. Bot disables
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `bot/plugin-sdk/memory-host-core` facade until a focused public consumer
...
Keep Bot as the source of truth for due checks and execution.
...
| `api.config`             | `BotConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `bot/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Bot is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through Bot's plugin
...
- **`@hanzo/bot-openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@hanzo/bot-openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `bot/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `bot/plugin-sdk/speech`, `.../provider-model-shared`, or another
