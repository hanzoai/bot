---
summary: "Manage Bot plugins from the Control UI or CLI"
read_when:
...
rules, and edge cases, see [`bot plugins`](/cli/plugins).
...
configured Control UI base path. For example, a base path of `/bot` uses
`/bot/settings/plugins`. The page has two tabs:
...
- **Discover** is the store: featured plugins included with Bot, official
...
or **Disable**. Workboard, for example, is included with Bot and disabled
...
OAuth-backed MCP connectors still need a one-time `bot mcp login <name>`
...
`plugins list` is a cold inventory check: what Bot can discover from
...
an install hint (`bot plugins install clawhub:<package>`) per result.

## Enable and disable plugins

```bash
bot plugins enable <plugin-id>
bot plugins disable <plugin-id>
```
...
bot plugins install clawhub:<package>@1.2.3
bot plugins install clawhub:<package>@beta

# Install from npm.
bot plugins install npm:<package>
bot plugins install npm:@scope/bot-plugin@1.2.3
bot plugins install npm:@hanzo/bot-codex

# Install from a local npm-pack artifact.
bot plugins install npm-pack:<path.tgz>

# Install from git or a local development checkout.
bot plugins install git:github.com/acme/bot-plugin@v1.0.0
...
name matches a bundled or official plugin id, in which case Bot uses
that local/official copy instead. Use `clawhub:`, `npm:`, `git:`, or
`npm-pack:` for deterministic source selection. Bot's bundled and official
...
ClawHub, or hook-pack install, use `bot plugins update` instead. With
...
Bot records the install but leaves the plugin disabled. Configure
`plugins.entries.<id>.config`, then run `bot plugins enable <id>`. If an
...
```bash
bot plugins update <plugin-id>
bot plugins update <npm-package-or-spec>
bot plugins update --all
bot plugins update <plugin-id> --dry-run
```
...
`bot plugins update --all` is the bulk maintenance path. It still
respects ordinary tracked install specs, but trusted official Bot
...
```bash
bot plugins update @scope/bot-plugin@beta
bot plugins update @scope/bot-plugin
```
...
See [`bot plugins`](/cli/plugins#update) for the exact fallback and
pinning rules.

## Uninstall plugins

```bash
bot plugins uninstall <plugin-id> --dry-run
bot plugins uninstall <plugin-id>
bot plugins uninstall <plugin-id> --keep-files
```
...
In Nix mode (`BOT_NIX_MODE=1`), plugin install, update, uninstall,
...
| ----------- | --------------------------------------------------------------------------- | -------------------------------------------------------------- |
| ClawHub     | You want Bot-native discovery, scan summaries, versions, and hints     | `bot plugins install clawhub:<package>`                   |
| git         | You want a branch, tag, or commit from a repository                         | `bot plugins install git:github.com/<owner>/<repo>@<ref>` |
...
| npm pack    | You are proving a local package artifact through npm install semantics      | `bot plugins install npm-pack:<path.tgz>`                 |
| npmjs.com   | You already ship JavaScript packages or need npm dist-tags/private registry | `bot plugins install npm:@acme/bot-plugin`           |
...
ClawHub is the primary public discovery surface for Bot plugins. Publish
...
Native npm plugins must ship a plugin manifest (`bot.plugin.json`) plus
...
bot plugins install npm:@acme/bot-plugin
bot plugins install npm:@acme/bot-plugin@beta
bot plugins install npm:@acme/bot-plugin@1.0.0
```
...
- [Building plugins](/plugins/building-plugins) shows the full plugin
  package shape (including `bot.plugin.json`) and first publish
  workflow.
