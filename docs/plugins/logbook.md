...
Bot-owned state stays on the Gateway under `<state-dir>/logbook/`, but
...
- A connected node that exposes `screen.snapshot` or `logbook.snapshot`. The
  ...
  (`bot node host run`) gets the plugin-provided `logbook.snapshot`
...
- The bundled Codex plugin enabled and authenticated. Codex currently provides
  ...
  `bot models auth login --provider openai`; see
...
```bash
bot plugins enable codex
bot plugins enable logbook
```
...
```bash
bot gateway restart
bot plugins inspect logbook --runtime --json
...
1. `bot plugins list --enabled` includes `logbook`.
