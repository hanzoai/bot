...
Bot Gateway. It is not a replacement for GitHub Issues, Linear, Jira, or
...
1. Open **Plugins** in the Control UI, or use `/settings/plugins` relative to
   the configured Control UI base path. For example, a base path of `/bot`
   uses `/bot/settings/plugins`.
2. Find **Workboard** and choose **Enable**. Because Workboard is included with
   Bot, it does not need an **Install** action.
...
that Gateway's Bot state (see [Storage](#storage)).
...
Bot subagent sessions still own execution. One dispatch pass:
...
Gateway (`BOT_GATEWAY_URL` or `gateway.mode: remote`) apply, the CLI runs
...
`defaultAssignee`, and `orchestratorProfile`. Bot records this intent and
...
bot workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
bot workboard show <card-id> [--json]
bot workboard move <card-id> --status <status> [--json]
bot workboard dispatch [--board <id>] [--json]
```
...
under the Bot state directory: boards, cards, labels, lifecycle events,
...
`bot doctor --fix` to migrate the shipped legacy plugin-state namespaces
