...
read_when:
  ...
  - You are changing plugin behavior across Bot and Codex harness turns
---
...
continuation, and native compaction. Bot owns channel routing, session
files, visible message delivery, Bot dynamic tools, approvals, media
...
Bot compatibility route keeps the normal Bot system prompt even when
it uses Codex-flavored OpenAI auth or transport.

Bot starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and Bot agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. Lightweight
Bot runs (for example cron) still suppress project-doc loading.

Bot developer instructions cover Bot runtime concerns: source-channel
delivery, Bot dynamic tools, ACP delegation, adapter context, and the
...
Most Bot dynamic tools use the searchable `bot` namespace. Tools
marked `catalogMode: "direct-only"` use `bot_direct`, which Codex keeps
directly model-visible as `DirectModelOnly` instead of exposing it to nested
Code Mode execution.

## Thread bindings and model changes

When an Bot session is attached to an existing Codex thread, the next
...
Supervised bindings are the exception. The Bot model picker stays locked,
...
model-difference warning; the outer Bot model and fallback chain never
...
not resume a thread, subscribe Bot to its live events, or answer its
approvals.
...
On the same supervision connection, Bot starts the canonical
...
resumed. The canonical thread has the full Bot harness tool surface;
...
Bot allows archive for a local `idle` or `notLoaded` row only after explicit
...
prove that another process is not using the thread. Bot blocks a known
...
contract: the agent replies normally and Bot posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable Bot tool
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Bot plugin hooks                 | Bot                 | Product/plugin compatibility across Bot and Codex harnesses.   |
| Codex app-server extension middleware | Bot bundled plugins | Per-turn adapter behavior around Bot dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Bot does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, Bot injects
...
so Codex's app-server reviewer and Bot's approval bridge handle real
...
controls; they are not exposed as Bot plugin hooks in the v1 contract.

For Bot dynamic tools, Bot executes the tool after Codex asks for
...
canonical tool record; Bot can mirror selected events but cannot rewrite
...
matching app-server approval. If an Bot `before_tool_call` hook returns
`requireApproval` while the native payload sets `bot_approval_mode:
...
request for the same tool use, Bot opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through Bot approvals when
...
notifications and Bot adapter state, not native Codex hook commands.
...
debugging. They do not invoke Bot plugin hooks.
...
| Bot channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                    |
| Bot dynamic tools                        | Supported                                                                        | Codex asks Bot to execute these tools, so Bot stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | Bot projects Bot-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. Bot disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Bot-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through Bot after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | Bot records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Bot does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Bot owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Bot-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Bot can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Bot does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Bot can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, Bot only returns explicit allow or deny
...
request, Bot remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through Bot's plugin approval
...
resolve the same gateway record before Bot returns the app-server answer.
...
default `messages.queue.mode: "steer"`, Bot batches steer-mode chat
...
that case, Bot waits for the active run to finish before starting the
...
harness, Bot also calls Codex app-server `feedback/upload` for relevant
...
Bot session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, Bot does not print those Codex ids
...
belongs to Codex app-server. Bot does not run preflight compaction for
Codex turns, replace Codex compaction with context-engine compaction, or fall
back to Bot or public OpenAI summarization when native compaction cannot
be started. Bot keeps a transcript mirror for channel history, search,
...
Bot keeps the request and shared-client lease open until Codex emits the
...
timeout, Bot requests a native turn interrupt. The lease and per-thread
...
period, Bot retires the connection before releasing the fence. Remote
...
compaction is Codex's job; Bot only starts native compaction for manually
requested triggers.

When a context engine requests Codex thread-bootstrap projection, Bot
...
Codex reasoning or plan records when the app-server emits them. Bot
...
not rewrite Codex-native tool result records. It only applies when Bot
writes an Bot-owned session transcript tool result.

## Media and delivery

Bot continues to own media delivery and media provider selection. Image,
...
through the normal Bot delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, Bot forwards that exact file through the normal reply-media
