...
At startup, Bot does roughly this:
...
The manifest is the control-plane source of truth. Bot uses it to:
...
derived set is empty, Bot keeps the scope empty instead of widening to
...
Bot does not cache plugin discovery results or direct manifest registry
...
Bot still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Bot calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Bot no longer calls, such as
...
| _(built-in model lookup)_         | Bot tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared Bot refreshers                                                                                          |
...
that still runs on Bot's normal inference loop.

`resolveUsageAuth` decides whether Bot should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Bot's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Bot adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  schemas while Bot owns the provider/runtime boundary.
...
- Bot only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Bot plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope. The default surface (`gatewayRuntimeScopeSurface: "write-default"`) is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) and any non-trusted-proxy auth method get a single `operator.write` scope, even if the caller sends `x-bot-scopes`
  - `trusted-proxy` callers without an explicit `x-bot-scopes` header also keep the legacy `operator.write`-only surface
  - `trusted-proxy` callers that do send `x-bot-scopes` get the declared scopes instead
  - a route can opt into `gatewayRuntimeScopeSurface: "trusted-operator"` to always honor `x-bot-scopes` for identity-bearing auth modes (falling back to the full CLI default scope set when the header is absent)
...
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, opt into `trusted-operator` scope surface, require an identity-bearing auth mode, and document the explicit `x-bot-scopes` header contract.
...
Use narrow SDK subpaths instead of the monolithic `bot/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                            | Purpose                                      |
| ---------------------------------- | -------------------------------------------- |
| `bot/plugin-sdk/plugin-entry` | Plugin registration primitives               |
| `bot/plugin-sdk/channel-core` | Channel entry/build helpers                  |
| `bot/plugin-sdk/core`         | Generic shared helpers and umbrella contract |
...
<Info>
`bot/plugin-sdk/channel-lifecycle`, small channel helper facades,
`bot/plugin-sdk/config-runtime`, and `bot/plugin-sdk/infra-runtime`
...
External plugins should only import `bot/plugin-sdk/*` subpaths. Never
...
`bot/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Bot writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Bot's
...
- if both `catalog` and `discovery` are registered, Bot uses `catalog`
  and emits a warning
...
- Read-only command paths such as `bot status`, `bot status --all`,
  `bot channels status`, `bot channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `bot.extensions`:
...
Security guardrail: every `bot.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `bot plugins install` installs plugin dependencies with a
...
Optional: `bot.setupEntry` can point at a lightweight setup-only module.
When Bot needs setup surfaces for a disabled channel plugin, or
...
Optional: `bot.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Bot load the
...
Channel plugins can advertise setup/discovery metadata via `bot.channel` and
install hints via `bot.install`. This keeps the core catalog data-free.
...
Useful `bot.channel` fields beyond the minimal example:
...
contain `{ "entries": [ { "name": "@scope/pkg", "bot": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `bot.install` block. The
...
2. **Add typed plugin registration/runtime surfaces.** Extend
   `BotPluginApi` and/or `api.runtime` with the smallest useful typed
   capability surface.
...
This is how Bot stays opinionated without becoming hardcoded to one
