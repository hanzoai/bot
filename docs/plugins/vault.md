...
read_when:
  - You want Bot to read API keys from HashiCorp Vault
...
The bundled Vault plugin lets Bot resolve `exec` SecretRefs from
HashiCorp Vault at Gateway startup and reload time. Bot stores Vault
references in config, keeps resolved values in the in-memory secrets snapshot,
and does not write the resolved API keys back to `bot.json`.

Use this when you already run Vault or want model provider keys to live outside
Bot config files. For the SecretRef runtime model, see
...
Enable the bundled plugin before running the `bot vault` commands:

```bash
bot plugins enable vault
```

## Store a provider key in Vault

Bot defaults to KV v2 mounted at `secret`, matching Vault dev-server
examples. For production Vault, set `BOT_VAULT_KV_MOUNT` to your actual KV
mount path before creating SecretRef ids. With the Bot defaults, this
SecretRef id:
...
Use a scoped client token for Bot, not a root token. For the default KV v2
...
that starts Bot. The default auth method reads a Vault client token from
`VAULT_TOKEN`:
...
```bash
export VAULT_ADDR=https://vault.example.com
export BOT_VAULT_AUTH_METHOD=token_file
...
These variables must be present when Bot starts. The Vault plugin forwards
...
```bash
export VAULT_ADDR=https://vault.example.com
export BOT_VAULT_AUTH_METHOD=jwt
export BOT_VAULT_AUTH_MOUNT=jwt
export BOT_VAULT_AUTH_ROLE=bot
export BOT_VAULT_JWT_FILE=/var/run/secrets/tokens/vault
```
...
```bash
export VAULT_ADDR=https://vault.example.com
export BOT_VAULT_AUTH_METHOD=kubernetes
export BOT_VAULT_AUTH_ROLE=bot
```

Set `BOT_VAULT_AUTH_MOUNT` only when Vault mounted Kubernetes auth somewhere
other than `auth/kubernetes`. Set `BOT_VAULT_JWT_FILE` only when the service
...
export BOT_VAULT_KV_MOUNT=secret
export BOT_VAULT_KV_VERSION=2
```
...
```bash
bot vault status --provider-alias corp-vault
```

`bot vault status` never prints `VAULT_TOKEN`; it reports only whether the
...
```bash
bot secrets apply --from ./vault-secrets-plan.json --dry-run --allow-exec
bot secrets apply --from ./vault-secrets-plan.json --allow-exec
bot secrets audit --check --allow-exec
bot secrets reload
```

Use `--allow-exec` because the Vault plugin resolves through an Bot-managed
exec SecretRef provider.

If the Gateway is not running yet, start it normally after applying the plan
instead of running `bot secrets reload`.
...
```bash
bot vault setup --openai-id providers/openai/apiKey
bot vault setup --anthropic-id providers/anthropic/apiKey
bot vault setup --openrouter-id providers/openrouter/apiKey
```
...
Bare target paths apply to `bot.json`. Use
`auth-profiles:<agentId>:<path>` for existing `auth-profiles.json` targets.
The target path must be a registered Bot SecretRef target. The setup
command does not create arbitrary named secrets in Bot; Vault remains the
secret store, and Bot stores SecretRefs only on supported config fields.
...
- one auth source:
  - `VAULT_TOKEN`
  - `BOT_VAULT_AUTH_METHOD=token_file` plus `VAULT_TOKEN_FILE`
  - `BOT_VAULT_AUTH_METHOD=jwt` plus `BOT_VAULT_AUTH_MOUNT`,
    `BOT_VAULT_AUTH_ROLE`, and `BOT_VAULT_JWT_FILE`
  - `BOT_VAULT_AUTH_METHOD=kubernetes` plus `BOT_VAULT_AUTH_ROLE`; optionally
    override `BOT_VAULT_AUTH_MOUNT` or `BOT_VAULT_JWT_FILE`
- optional `VAULT_NAMESPACE`, `BOT_VAULT_KV_MOUNT`, and
  `BOT_VAULT_KV_VERSION`

When using Kubernetes, prefer `BOT_VAULT_AUTH_METHOD=kubernetes`
when Vault has Kubernetes auth configured for the cluster. Use
`BOT_VAULT_AUTH_METHOD=jwt` only when Vault is configured to treat the cluster
...
deployment config. Bot does not require a fixed mount, role, or path: each
Gateway environment can set its own `BOT_VAULT_KV_MOUNT`,
`BOT_VAULT_AUTH_ROLE`, and SecretRef ids. If one shared Gateway must resolve
