...
An **agent harness** is the low level executor for one prepared Bot agent
...
runtime and the normal Bot provider transport is the wrong abstraction:
...
- a model runtime that needs its own resume id in addition to the Bot
  session transcript
...
Core still forwards a compatible, explicitly selected or ordered Bot auth
...
Verified Bot continuations also pass `params.expectedRuntimeArtifact`.
...
The prepared attempt also includes `params.runtimePlan`, an Bot-owned
policy bundle for runtime decisions that must stay shared across Bot and
native harnesses:
...
Harnesses may use the plan for decisions that need to match Bot behavior,
...
all of them before dispatch. Implicit selection uses Bot if no plugin can
own the full set; an explicit or persisted plugin selection fails closed.

## Register a harness

**Import:** `bot/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "bot/plugin-sdk/agent-harness";
...
Delegates receive only work admission and embedded execution. Bot requires
...
Bot chooses a harness after provider/model resolution:
...
4. If no registered harness matches, Bot uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, Bot does not
...
Bot. The harness then claims that provider in `supports(...)`.
...
- app-server request: Bot sends the bare model id to Codex and lets the
...
behavior stay on Bot. Plaintext official HTTP endpoints are rejected. Older `codex/gpt-*`
...
blocks older or unversioned servers, so Bot only runs against the protocol
...
seam is for async tool-result transforms that must run before Bot or
...
`bot/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so Bot's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; Bot does not infer it from assistant prose. The helper
...
`bot/plugin-sdk/agent-harness-runtime` after they finalize an attempt. It
dispatches the portable `agent_end` hook and Bot's research capture
...
user-input helpers from `bot/plugin-sdk/agent-harness-runtime` to format
the prompt, deliver it through Bot's blocking reply path, and normalize
...
`bot/plugin-sdk/agent-harness-tool-runtime`. It owns
...
The bundled `codex` harness is the native Codex mode for embedded Bot
...
refs should be repaired with `bot doctor --fix`, and legacy `codex/*`
model refs remain compatibility aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Bot still owns the chat channel,
...
By default, Bot uses `auto` provider/model runtime policy: registered
...
token. Keep that binding explicitly associated with the Bot session, and
keep mirroring user-visible assistant/tool output into the Bot
transcript.

The Bot transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in Bot harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so Bot
can clear it when the owning Bot session is reset.

## Tool and media results

Core constructs the Bot tool list and passes it into the prepared
...
outputs on the same delivery path as Bot-backed runs.
...
Bot-tool media. On `message_tool_only` routes, this narrow provenance lets
...
outcome accumulator. A harness that executes Bot dynamic tools or native
...
- Report `outcome: "success"` or `outcome: "failure"`. Include the structured
  ...
  display text.
- Use `nativeMutation` only for native tools that do not use an Bot tool
  definition. Supply protocol-owned mutation and replay facts there; do not
  copy Bot's mutation classifier into the harness.
...
without terminal reports, Bot cannot preserve mutating-tool failure truth
across later tool calls, including quiet heartbeat completion.

### Settled tool finalization

Bot may need one final visible answer after a harness has completed every
...
Bot invokes the callback once as a terminal sub-operation, outside the
...
`source: "bot-transcript"`. It must capture the active branch after the
...
capability boundary. Bot does not provide a generic fallback because it
...
compatibility. When the selected harness omits it, Bot preserves the
