...
read_when:
  - You want to connect Bot to a Raft workspace
...
Raft connects an Bot agent to a Raft External Agent through the local
...
```bash
bot plugins install @hanzo/bot-raft
bot gateway restart
```
...
- The Raft CLI installed on the same host as the Bot Gateway, on the
  service's `PATH`.
...
7. Starts one serialized Bot agent turn per accepted wake.

The bridge owns Raft delivery retries and reconnects. The Bot turn
...
```bash
raft --profile bot message check
raft --profile bot message send
```

<Note>
Raft is not a push-message transport. Bot does not automatically send the model's final text back through the bridge, so the agent must use the Raft CLI after processing a wake.
</Note>

## Verify

Check that Bot can find the CLI and has a configured profile:

```bash
bot channels status --probe
bot plugins inspect raft --runtime --json
```
