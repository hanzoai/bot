...
- Pin a version: `bot plugins install @hanzo/bot-zalouser@<version>`
- From a source checkout: `bot plugins install ./path/to/local/zalouser-plugin`
...
Bot stores each raw `zca-js` message callback before processing it. Pending messages resume from the account queue after a Gateway restart, and processing stays serialized per direct chat or group.

The `zca-js` socket listener does not expose a delivery acknowledgement or automatically replay old messages after reconnect. The durable queue therefore protects the local crash window after a callback reaches Bot; it cannot recover a message the socket never delivered. Replay tombstones are mostly a safeguard against a repeated callback with the same Zalo message id.
...
<Note>
`channels.zalouser.groups.<id>.allow` is a legacy field name; current config uses `enabled`. `bot doctor --fix` migrates `allow` to `enabled` automatically.
</Note>
...
- When a group message is skipped because a mention is required, Bot stores it as pending group history and includes it on the next processed group message.
...
Profile names select the saved Zalo login credentials in Bot state. Resolution order:
