...
read_when:
  - You want Bot to receive DMs via Nostr
  - You're setting up decentralized messaging
title: "Nostr"
---

Nostr is a downloadable channel plugin (`@hanzo/bot-nostr`) that lets Bot receive and answer NIP-04 encrypted direct messages over Nostr relays. One account per gateway; DMs only.

## Install

```bash
bot plugins install @hanzo/bot-nostr
```
...
```bash
bot plugins install --link <path-to-local-nostr-plugin>
```

Restart the gateway after installing or enabling plugins. Onboarding (`bot onboard`) and `bot channels add` surface Nostr from the shared channel catalog once the plugin is installed.

### Non-interactive setup

```bash
bot channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
bot channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
1. Note the bot pubkey from gateway logs or `bot channels status` (hex; convert to npub in your client if needed).
