...
3. Set tokens for Hanzo Bot and start the gateway.
...
Hanzo Bot can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Hanzo Bot falls back to the user token.
...
By default, Hanzo Bot replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Hanzo Bot infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `hanzo-bot pairing approve slack <code>`.
...
- On startup, Hanzo Bot resolves channel/user names in allowlists to IDs (when tokens allow)
