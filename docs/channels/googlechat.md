...
Google Chat runs as the official `@hanzo/bot-googlechat` plugin: DMs and spaces through Google Chat API webhooks (HTTP endpoint only, no Pub/Sub).
...
```bash
bot plugins install ./path/to/local/googlechat-plugin
```
...
7. Configure Bot with the service account and the webhook audience (must match the Chat app config):
   - Env: `GOOGLE_CHAT_SERVICE_ACCOUNT_FILE=/path/to/service-account.json` (default account only), or
   - Config: see [Config highlights](#config-highlights). `bot channels add --channel googlechat` also accepts `--audience-type`, `--audience`, `--webhook-path`, and `--webhook-url`.
...
Google Chat webhooks require a public HTTPS endpoint. For security, expose **only the `/googlechat` path** to the internet and keep the Bot dashboard and other endpoints private.
...
Requests to `your-domain.com/` are ignored or 404, while `your-domain.com/googlechat` routes to Bot.
...
2. Bot authenticates every request before dispatch:
...
7. When an exec or plugin approval starts from Google Chat and a stable `users/<id>` approver is configured, Bot posts a native approval card (`cardsV2`) in the originating space or thread. Card buttons carry opaque callback tokens; the manual `/approve <id> <decision>` prompt appears only when native delivery is unavailable.

### Inbound durability

After request authentication, Bot removes the add-on authorization object from storage and durably queues Google Chat `MESSAGE` events before returning `200`. A persistence failure returns `503`, allowing Google Chat to retry instead of acknowledging an event that could be lost.
...
- `bot channels status --probe` surfaces auth errors and missing audience config (`audience` and `audienceType` are both required).
...
- `bot logs --follow` while sending a test message shows whether requests reach the gateway.
