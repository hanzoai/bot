...
Matrix is an open, decentralized messaging protocol. Hanzo Bot connects as a Matrix **user**
...
```bash
hanzo-bot plugins install @bot/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `hanzo-bot plugins install @bot/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Hanzo Bot calls the same
...
- On first connection, Hanzo Bot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Hanzo Bot logs a warning.
...
- On startup, Hanzo Bot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
