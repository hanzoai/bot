...
Nextcloud Talk is a downloadable channel plugin (`@hanzo/bot-nextcloud-talk`) that connects Bot to a self-hosted Nextcloud instance through a Talk webhook bot. Direct messages, rooms, reactions, and markdown messages are supported; media goes out as URLs.
...
```bash
bot plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Bot" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Bot:
   ...
   ```bash
   bot channels add --channel nextcloud-talk \
...
- With `apiUser`/`apiPassword` and `webhookPublicUrl` set, `bot channels status` probes the bot and warns when the `response` feature is missing.
