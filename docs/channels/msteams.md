...
Microsoft Teams ships as a bundled plugin in current Bot releases; no separate install is required in the normal packaged build.

On an older build or a custom install that excludes bundled Teams, install the npm package directly:

```bash
bot plugins install @hanzo/bot-msteams
```
...
```bash
bot plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-bot-bot --allow-anonymous
devtunnel port create my-bot-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-bot-bot
...
**4. Configure Bot** using the credentials from the output:
...
- Do not rely on UPN/display-name matching for allowlists; they can change. Bot disables direct name matching by default; opt in with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, Bot resolves team/channel and user allowlist names to IDs (when Graph permissions allow) and logs the mapping. Unresolved names are kept as typed but ignored for routing unless `channels.msteams.dangerouslyAllowNameMatching: true` is set.
...
For production, Bot supports **federated authentication** as an alternative to client secrets, via `channels.msteams.authType: "federated"`. Two methods:
...
3. At runtime, Bot uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint.
...
**Default:** when `authType` is unset, Bot uses client-secret authentication (`appPassword`). Existing configs keep working unchanged.
...
These auth-related config keys can be set via environment variables instead of `bot.json` (other config keys, such as `groupPolicy` or `historyLimit`, are config-only):
...
Bot exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve verified roster details for a configured conversation.
...
Teams delivers messages via HTTP webhook. Bot applies fixed HTTP server
...
| Teams environment | Bot config                                             | Proactive `serviceUrl`                             |
| ----------------- | ----------------------------------------------------------- | -------------------------------------------------- |
...
Per-user sharing is more secure since only chat participants can access the file. Bot requires a successful member lookup for group chats; timeouts, transport failures, empty results, and Graph API denials fail the send instead of widening access to the organization.
...
- CLI: `bot message poll --channel msteams --target conversation:<id> --poll-question "..." --poll-option "..." --poll-option "..."`.
- Votes are recorded by the gateway in Bot plugin-state SQLite under `state/bot.sqlite`.
- Existing `msteams-polls.json` files are imported by `bot doctor --fix`, not by the running plugin.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries, and there is no poll-results CLI yet.

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. Bot renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer, so Bot downgrades them to readable text before delivery.
...
- Proactive messages are only possible **after** a user has interacted, because Bot stores conversation references at that point.
...
- **Ignore** the `groupId` query parameter for Bot routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
