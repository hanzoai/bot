...
<Steps>
  <Step title="Create the bot token in BotFather">
    Both flows end with a token you paste into Bot — pick one:
...
```
    ...
    Telegram does **not** use `bot channels login telegram`; set the token in config/env, then start the gateway.
...
```
  ...
  <Step title="Add the bot to a group">
    ...
    Get the group chat ID from `bot logs --follow`, a forwarded-ID bot, or Bot API `getUpdates`. After the group is allowed, `/whoami@<bot_username>` confirms the user and group IDs.
...
Run `/dashboard` in a DM with the bot to open the Bot dashboard inside Telegram.
...
In groups and forum topics, an explicit mention of the configured bot handle (for example `@my_bot`) addresses the selected Bot agent, even when the agent persona name differs from the Telegram username. Group silence policy still applies to unrelated traffic, but the bot handle itself is never "someone else."
...
<Tabs>
  <Tab title="DM policy">
    ...
    Setup asks for numeric user IDs only. If your config has `@username` allowlist entries from an older setup, run `bot doctor --fix` to resolve them to numeric IDs (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `bot doctor --fix` can recover entries into `channels.telegram.allowFrom` for allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
    ...
    Safer (no third-party bot): DM your bot, run `bot logs --follow`, read `from.id`.
...
```
    ...
    Group history context is always on and bounded by `historyLimit`. Set `channels.telegram.historyLimit: 0` to disable the group history window. `bot doctor --fix` removes the retired `includeGroupHistoryContext` key.

    Getting the group chat ID: forward a group message to `@userinfobot` / `@getidsbot`, read `chat.id` from `bot logs --follow`, inspect Bot API `getUpdates`, or (once the group is allowed) run `/whoami@<bot_username>`.
...
- DM messages can carry `message_thread_id`; Bot preserves it for replies. DM topic sessions split only when Telegram `getMe` reports `has_topics_enabled: true` for the bot; otherwise DMs stay on the flat session.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Bot streams partial replies in real time in direct chats, groups, and topics: send a preview message, then `editMessageText` repeatedly, finalizing in place.
    ...
    - legacy `channels.telegram.streamMode`, boolean `streaming` values, and retired native draft preview keys are detected; run `bot doctor --fix` to migrate them
    ...
    <Note>
      Selected quote replies are the exception. When `replyToMode` is `first`, `all`, or `batched` and the inbound message has selected quote text, Bot sends the final answer through Telegram's native quote-reply path instead of editing the answer preview, so `streaming.preview.toolProgress` cannot show status lines that turn. Current-message replies without selected quote text still stream. Set `replyToMode: "off"` when tool-progress visibility matters more than native quote replies, or `streaming.preview.toolProgress: false` to accept that trade-off.
    </Note>

    For text-only replies: short previews get the final edit in place; long finals that split into multiple messages reuse the preview as the first chunk, then send only the remainder; progress-mode finals clear the status draft and use normal final delivery; if the final edit fails before completion is confirmed, Bot falls back to normal final delivery and cleans up the stale preview. For complex replies (media payloads), Bot always falls back to normal final delivery and cleans up the preview.

    Preview streaming and block streaming are mutually exclusive — when block streaming is explicitly enabled, Bot skips the preview stream to avoid double-streaming.
...
```
    ...
    When enabled: the agent is told rich messages are available for this bot/account (with the supported Markdown + HTML-island authoring contract); Markdown text renders through Bot's Markdown IR as typed Bot API 10.2 rich blocks (headings, tables, details, checklists, rich media, formulas, maps, collages); media captions still use Telegram HTML captions (rich messages do not replace captions, and captions cap at 1024 characters).
...
```
    ...
    - `deleteWebhook`, `deleteMyCommands`, or `setMyCommands` failing with `404: Not Found` while direct Bot API curl commands work usually means `channels.telegram.apiRoot` was set to the full `/bot<TOKEN>` endpoint. `apiRoot` must be the Bot API root only; `bot doctor --fix` removes an accidental trailing `/bot<TOKEN>`.
    - `getMe returned 401` means Telegram rejected the configured bot token. Update `botToken`, `tokenFile`, or `TELEGRAM_BOT_TOKEN` (default account) with the current BotFather token; Bot stops before polling so this is not reported as a webhook cleanup failure.
...
```
  ...
  <Accordion title="Reply threading tags">
    ...
    When reply threading is enabled and the original text/caption is available, Bot adds a native quote excerpt automatically. Telegram caps native quote text at 1024 UTF-16 code units; longer messages are quoted from the start and fall back to a plain reply if Telegram rejects the quote.
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**: `/acp spawn <agent> --thread here|auto` binds the current topic to a new ACP session; follow-ups route there directly, and Bot pins the spawn confirmation in-topic. Controlled by `session.threadBindings.spawnSessions` (default: `true`).
    ...
    The retired `dm.threadReplies` and `direct.*.threadReplies` overrides are gone; BotFather threaded mode is the single source of truth. Run `bot doctor --fix` to remove stale config keys.
...
```
    ...
    Sticker context fields: `Sticker.emoji`, `Sticker.setName`, `Sticker.fileId`, `Sticker.fileUniqueId`, `Sticker.cachedDescription`. Descriptions are cached in Bot SQLite plugin state to reduce repeated vision calls.
...
```
  ...
  <Accordion title="Reaction notifications">
    Telegram reactions arrive as `message_reaction` updates, separate from message payloads. When enabled, Bot enqueues system events like `Telegram reaction added: 👍 by Alice (@alice) on msg 42`.
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while Bot processes an inbound message. `messages.ackReactionScope` decides *when* it is sent.
...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    In long-polling mode, Bot persists its restart watermark only after an update dispatches successfully; a failed handler leaves that update retryable in the same process instead of marking it completed.
    ...
    Webhook mode validates request guards, the Telegram secret token, and the JSON body, then commits the update to its durable ingress queue before returning an empty `200`. Successful durable adoption includes `x-bot-delivery-accepted: durable`; health, routing, authentication, validation, and storage-error responses omit this header. Reverse proxies and host controllers can require the header to distinguish Bot adoption from a generic empty `200` without inferring acceptance from response timing.

    After the durable write, Bot claims and processes updates through the core channel-ingress drain (per-chat/per-topic lanes, complete at turn adoption, pre-adoption stall timeout). Slow agent turns do not hold Telegram's delivery ACK.
...
```bash
bot message send --channel telegram --target 123456789 --message "hi"
bot message send --channel telegram --target @name --message "hi"
bot message send --channel telegram --target -1001234567890:topic:42 --message "hi topic"
```

    Polls use `bot message poll` and support forum topics:
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat; only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, Bot preserves the topic for the approval prompt and follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `bot channels status` warns when config expects unmentioned group messages.
    - `bot channels status --probe` checks explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - Review `bot logs --follow` for skip reasons.
  ...
  <Accordion title="Polling or network instability">
    ...
    - During polling startup, Bot reuses the successful startup `getMe` probe for grammY so the runner does not need a second `getMe` before the first `getUpdates`.
    - If `deleteWebhook` fails with a transient network error during polling startup, Bot continues into long polling instead of making another pre-poll control-plane call. A still-active webhook then surfaces as a `getUpdates` conflict; Bot rebuilds the transport and retries webhook cleanup.
    - `Polling stall detected` in logs means Bot restarts polling and rebuilds the transport after 120 seconds without completed long-poll liveness by default.
    - `bot channels status --probe` and `bot doctor` warn when a running polling account has not completed `getUpdates` after startup grace, a running webhook account has not completed `setWebhook` after startup grace, or the last successful polling transport activity is stale.
    ...
    - If `BOT_PROXY_URL` is set for a service environment and no standard proxy env is present, Telegram uses that URL for Bot API transport too.
...
```
    ...
    - Node 22+ defaults to `autoSelectFamily=true` (except WSL2). Telegram DNS result order honors `BOT_TELEGRAM_DNS_RESULT_ORDER`, then `channels.telegram.network.dnsResultOrder`, then the process default (for example `NODE_OPTIONS=--dns-result-order=ipv4first`), falling back to `ipv4first` on Node 22+ if none applies.
...
```
    ...
    - Temporary environment overrides: `BOT_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`, `BOT_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`, `BOT_TELEGRAM_DNS_RESULT_ORDER=ipv4first`.
