...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `bot doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `bot logs --follow`
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Bot can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    `streamMode` is separate from block streaming. When block streaming is explicitly enabled for Telegram, Bot skips the preview stream to avoid double-streaming.
...
```
    ...
    - `~/.bot/telegram/sticker-cache.json`
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `bot channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `bot logs --follow` for skip reasons
