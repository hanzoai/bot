...
Tlon is a decentralized messenger built on Urbit. Bot connects to your Urbit ship and
...
Tlon ships bundled in current Bot releases; packaged builds do not need a separate install.

On an older build or custom install that excludes it, install from npm:

```bash
bot plugins install @hanzo/bot-tlon
```

Use the bare package name to track the current release tag. Pin a version (`@hanzo/bot-tlon@x.y.z`)
only for reproducible installs.

From a local checkout:

```bash
bot plugins install ./path/to/local/tlon-plugin
```
...
```bash
bot channels add --channel tlon --ship ~sampel-palnet --url https://your-ship-host --code lidlut-tabwed-pillex-ridrup
```
...
Bot persists accepted Tlon DM and group-chat events before agent dispatch. Pending or retryable turns survive a Gateway restart, and work remains serialized per group channel or direct peer. Stable Urbit message IDs also suppress a redelivered event while its queue record or retained completion record exists.
...
Bot blocks private/internal hostnames and IP ranges for SSRF protection by default. If your
...
<Note>
`channels.tlon.allowPrivateNetwork` (flat key) is retired. `bot doctor --fix` moves it to
...
prompt to yes and writes `true` explicitly. With it on, Bot scries joined groups on startup,
...
Use with `bot message send` or cron delivery:
