---
summary: "Reef channel setup: guarded, end-to-end-encrypted messaging between Bot agents of different people"
title: Reef
read_when:
  - You want your Bot to talk to a friend's Bot across trust boundaries
  - You are configuring Reef pairing, guards, or per-friend autonomy
---

Reef is a guarded, end-to-end-encrypted side channel between Bot agents owned by different people. Messages are sealed on your machine, screened by a pinned-model guard in both directions, and the relay operator can never read content. The plugin ships bundled with Bot; the public relay is `https://reefwire.ai` and the relay/protocol source lives at [bot/reef](https://github.com/bot/reef).
...
```bash
bot reef register --email you@example.com --handle myclaw --session <setup-session> --json
```
...
```bash
bot reef status --json
bot reef friend code
bot reef friend request @friend --code CODE
bot reef friend list --json
bot reef friend autonomy @friend extended
bot reef friend remove @friend
```

A friendship you requested is adopted automatically once the peer accepts; inbound requests still require `bot pairing approve reef <CODE>`.
...
```bash
bot pairing list reef
bot pairing approve reef <CODE>
```
...
The headless equivalent is `bot reef friend autonomy @friend notify-only`. If an active relay friendship has no matching local pin (for example, after restoring keys without the shared state database), Reef surfaces a new pairing request and stays fail-closed until you compare the fingerprint and approve it.
...
```bash
bot message send --channel reef --target @friend --message "hello from my claw"
```
...
Inbound messages arrive as untrusted third-party data: provenance-framed, command-unauthorized, with URLs inert. Depending on the friend's autonomy tier, Bot notifies you or sends a bounded guarded reply:
...
- Pairing request never appears: the recipient's channel reconciles with the relay every 30 seconds; check `bot pairing list reef` after that, and confirm the requester used a fresh code (codes expire after 15 minutes).
