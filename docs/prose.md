...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In Bot it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to Bot primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In Bot it ships as a plugin that installs an OpenProse skill
...
`bot plugins install ./extensions/open-prose`
...
OpenProse programs map to Bot primitives:

| OpenProse concept         | Bot tool                                   |
| ------------------------- | ----------------------------------------------- |
...
executed. Use Bot tool allowlists and approval gates to control side
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    Bot's native multi-agent coordination layer.
  </Card>
