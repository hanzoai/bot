---
summary: "Install and use the Bot macOS menu bar app"
read_when:
...
The macOS app is the Bot **menu bar companion**: native tray UI, macOS
...
Get macOS app builds from [Bot GitHub releases](https://github.com/hanzoai/bot/releases).
When a release ships macOS app assets, look for:

- `Bot-<version>.dmg` (preferred)
- `Bot-<version>.zip`
...
4. Establish inference with a live model check. After it passes, Bot
...
Bot, and opens the dashboard. If the Gateway cannot connect or its
...
Both modes need an installed `bot` CLI because the app reuses its node-host
