...
Bot can request a read-only summary of the current calendar day from a
...
- An iPhone or iPad running the Bot iOS app where HealthKit reports health data as
  available.
...
- Readable Health data for any metrics you expect to see. An Apple Watch can
  contribute data to the Apple Health store, but the Bot watchOS app is
...
`bot.json`. Preserve any commands already present:
...
3. Read the disclosure, then choose which Health categories Bot may read
   in Apple's permission sheet.

The switch records your explicit Bot sharing choice. It does not claim
...
```bash
bot nodes describe --node "<iOS device name>"
```
...
- Bot requests read access only. It cannot add or modify Health data.
- Bot reads HealthKit only when `health.summary` is invoked. There is no
...
- HealthKit deliberately does not reveal whether read access was denied. A
  ...
  data type. Bot cannot distinguish those cases.
...
Run `bot nodes pending` and approve any capability update, then inspect
`bot nodes describe --node "<iOS device name>"` again.
...
Bot's access in Apple's Health settings, but do not treat an empty result
