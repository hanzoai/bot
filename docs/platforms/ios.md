...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `bot.internal.`), **or**
...
```bash
bot gateway --port 18789 --tailscale serve
```
...
Gateway has not been configured yet, run `bot onboard` first so setup-code
...
4. The official app connects automatically. If **Pending approval** shows a
   ...
   **Full** or **Limited** access. Plaintext LAN `ws://` setup is automatically
   ...
   Tailscale Serve, scan a new full-access code from Control UI or `bot qr`,
...
If the app retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Run `bot devices list` again before approval.
...
Apple's Watch app, install Bot from **Watch app -> My Watch -> Available
Apps**, then open Bot once on both devices.
...
Bot is active, even when the paired iPhone is unavailable.
...
- Bot is active on the watch. Apple does not allow ordinary watchOS apps to
...
3. Open Bot on the watch before the short-lived setup code expires.
4. Verify the separate Apple Watch row with `bot nodes status`.
...
- `BOT_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway (`gateway.push.apns.relay.baseUrl` is the config-first path).
- The App Store release build's push mode hardcodes the hosted relay host and never reads a relay-URL override — the `BOT_PUSH_RELAY_BASE_URL` build-time env var only affects local/sandbox iOS build modes.

## Authentication and trust flow

The relay exists to enforce two constraints direct APNs-on-gateway cannot provide for official iOS builds:

- Only genuine Bot iOS builds distributed through Apple can use the hosted relay.
...
Why this design was created: to keep production APNs credentials out of user gateways, avoid storing raw official-build APNs tokens on the gateway, allow hosted relay usage only for official Bot iOS builds, and prevent one gateway from sending wake pushes to iOS devices owned by a different gateway.
...
Recommended gateway-host storage, consistent with other provider credentials under `~/.bot/credentials/`:

```bash
mkdir -p ~/.bot/credentials/apns
chmod 700 ~/.bot/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.bot/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.bot/credentials/apns/AuthKey_KEYID.p8
export BOT_APNS_PRIVATE_KEY_PATH="$HOME/.bot/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_bot-gw._tcp` on `local.` and, when configured, the same wide-area DNS-SD discovery domain. Same-LAN gateways appear automatically from `local.`; cross-network discovery can use the configured wide-area domain without changing the beacon type.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `bot.internal.`) and Tailscale split DNS. See [Bonjour](/gateway/bonjour) for the CoreDNS example.
...
```bash
bot nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__bot__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__bot__/canvas/` and `/__bot__/a2ui/`, from the Gateway HTTP server (same port as `gateway.port`, default `18789`).
...
The iOS app is a mobile node surface, not a Codex Computer Use backend. Codex Computer Use and `cua-driver mcp` control a local macOS desktop through MCP tools; the iOS app exposes iPhone capabilities through Bot node commands such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Bot by invoking node commands, but those calls go through the gateway node protocol and follow iOS foreground/background limits. Use [Codex Computer Use](/plugins/codex-computer-use) for local desktop control and this page for iOS node capabilities.
...
- Pairing prompt never appears: run `bot devices list` and approve manually.
- Watch shows no iPhone state: confirm the iPhone reports `watchPaired: true`
  ...
  from **My Watch -> Available Apps**. After either change, open Bot on the
