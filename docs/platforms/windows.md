...
Bot ships a native **Windows Hub** companion app plus Windows CLI support.
...
Windows Hub publishes independently from the Bot CLI and Gateway. Download
the latest stable Hub installer from the
[Windows Hub releases page](https://github.com/hanzoai/bot-windows-node/releases/latest)
or directly via `releases/latest/download`:

- [BotCompanion-Setup-x64.exe](https://github.com/hanzoai/bot-windows-node/releases/latest/download/BotCompanion-Setup-x64.exe)
- [BotCompanion-Setup-arm64.exe](https://github.com/hanzoai/bot-windows-node/releases/latest/download/BotCompanion-Setup-arm64.exe)

If a link above 404s, visit the [Windows Hub releases page](https://github.com/hanzoai/bot-windows-node/releases)
and open the newest stable Windows Hub release. Regular Bot stable releases
also mirror a pinned, release-validated Windows Hub build; that mirror can lag a
newer standalone Hub release.

After install, launch **Bot Companion** from the Start menu or system
...
app-owned `BotGateway` WSL distro, installs the Gateway inside it, and
...
Windows Hub can register as an Bot node so the agent can use declared
...
For terminal-first use, install Bot from PowerShell:
...
the readable `gateway.cmd` script in the Bot state dir but launches it
through a generated `gateway.vbs` WScript wrapper, so the background Gateway
does not open a visible console window. If task creation is denied, Bot
...
```powershell
bot onboard --non-interactive --skip-health
bot gateway run
```
...
Then install Bot inside WSL with the Linux quickstart:
...
```bash
systemctl --user is-enabled bot-gateway.service
...
Check Task Manager for `Bot.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If not, launch **Bot Companion** from
the Start menu.
...
```powershell
notepad "$env:LOCALAPPDATA\BotTray\Logs\Setup\easy-setup-latest.txt"
```
