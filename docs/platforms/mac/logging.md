...
- Location: `~/Library/Logs/Bot/diagnostics.jsonl`.
...
## Enable for Bot (`ai.bot`)

Write the plist to a temp file first, then install it atomically as root:

```bash
cat <<'EOF' >/tmp/ai.bot.plist
...
EOF
sudo install -m 644 -o root -g wheel /tmp/ai.bot.plist /Library/Preferences/Logging/Subsystems/ai.bot.plist
```
...
- Remove the override: `sudo rm /Library/Preferences/Logging/Subsystems/ai.bot.plist`.
