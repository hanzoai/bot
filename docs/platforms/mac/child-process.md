...
enables the launchd service via the external `bot` CLI (no embedded
...
- The app installs a per-user LaunchAgent labeled `ai.bot.gateway` (or
  `ai.bot.<profile>` when using `--profile`/`BOT_PROFILE`).
...
Replace the label with `ai.bot.<profile>` when running a named profile.
...
`~/.bot/disable-launchagent`, so the app only attaches to an already
