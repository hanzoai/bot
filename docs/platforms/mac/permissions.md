...
- Same path: run a release app from `/Applications/Bot.app`; keep development builds at one fixed path such as `dist/Bot.app`.
- Same bundle identifier: Bot's bundle ID is `ai.bot.mac`; changing it creates a new permission identity.
...
Prefer granting Accessibility to Bot.app, Peekaboo.app, or another signed helper with its own bundle identifier instead of a generic `node` binary.
...
macOS keeps Accessibility, Event Posting, input listening, and Screen Recording in separate TCC buckets. One successful grant does not prove the others are usable. Bot's Computer Control status checks Accessibility, Event Posting, and Screen Recording separately; this is why screenshots can succeed while clicks and typing fail.

An Accessibility row can also remain visibly enabled while its code requirement is pinned to an older build. When Bot reports **Accessibility grant may be stale**, select Bot under **System Settings -> Privacy & Security -> Accessibility**, remove it with **-**, then re-add `/Applications/Bot.app`. Quit and reopen Bot afterward because Accessibility trust can remain cached in the running process.
...
Example resets (using Bot's bundle ID, `ai.bot.mac`):

```bash
sudo tccutil reset Accessibility ai.bot.mac
sudo tccutil reset ScreenCapture ai.bot.mac
...
Workaround: move files into the Bot workspace (`~/.bot/workspace`) if you want to avoid per-folder grants.
