...
This enables Tailscale SSH, so you can connect via `ssh bot` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@bot` (or use the Tailscale IP).
...
systemctl --user restart bot-gateway
```
...
# Check daemon status
systemctl --user status bot-gateway
...
journalctl --user -u bot-gateway -n 50
```
