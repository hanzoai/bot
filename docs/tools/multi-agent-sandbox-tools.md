...
<Note>
Legacy `agents.defaults.*`/`agents.entries.*.*` config keys (such as `sandbox.perSession`, `agentRuntime`, `embeddedPi`) are migrated by `bot doctor`; prefer `agents.defaults` + `agents.entries` going forward.
</Note>
...
<Warning>
`agents.defaults.sandbox.mode: "non-main"` checks the session key against the main session key (always `"main"`; `session.mainKey` is not user-configurable, and Bot warns and ignores any other value), not the agent id. Group/channel sessions always get their own keys, so they are treated as non-main and will be sandboxed. If you want an agent to never sandbox, set `agents.entries.*.sandbox.mode: "off"`.
</Warning>
...
<Steps>
  ...
  <Step title="Monitor logs">
    ```bash
    bot logs --follow | grep -E "routing|sandbox|tools"
    ```
  </Step>
</Steps>
