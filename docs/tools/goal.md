...
read_when:
  - You want Bot to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current Bot session.
...
stale or unknown token snapshot when the goal starts, Bot waits for the
...
Bot usage and model controls.

## Model tools

Bot exposes three goal tools to agent harnesses:
...
Bot keeps the line compact by truncating long objectives. Paused,
...
`/goal` works in command-capable Bot sessions, including the TUI and
...
fresh token snapshot yet. Usage refreshes as Bot records session usage
