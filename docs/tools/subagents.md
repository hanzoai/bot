...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, Bot best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - Bot hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, Bot first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, Bot falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - Bot hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.entries.*` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal Bot config agents from `agents_list`.
...
- **Run timeout:** pass `runTimeoutSeconds` to set a timeout for a specific native, ACP, or visible sub-agent run. When omitted, Bot uses `agents.defaults.subagents.runTimeoutSeconds` if configured; otherwise it falls back to `0` (no timeout). An explicit `0` disables the timeout for that run.
- **Process lifetime:** a detached Bot sub-agent has its own run lifecycle. A background task created inside an external CLI backend is different: it shares the parent CLI subprocess and stops if that parent reaches `agents.defaults.timeoutSeconds`.
...
When active children exist, Bot injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    Bot creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish Bot, Codex
...
and `agents_list` omits it. Run `bot doctor --fix` to clean stale
...
- If a nested requester subagent session is gone, Bot falls back to that session's requester when available.
...
target agent first. After that, Bot applies the sub-agent restriction
layer.
...
Bot does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, Bot persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`bot tasks maintenance --apply` to reconcile the task record, or
`bot doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
