...
By default, the CLI installs skills into `./skills` under your current working directory. If a Hanzo Bot workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). Hanzo Bot loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.bot/skills` or bundled skills, workspace skills take precedence.
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/bot/skills` and `~/.bot/skills`). This is designed to find older skill installs without extra flags.
