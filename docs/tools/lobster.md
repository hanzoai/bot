...
see [Task Flow](/automation/taskflow) (`bot tasks flow`); for the task
...
Bot runs Lobster workflows **in-process** using the bundled
...
[Lobster repo](https://github.com/bot/lobster) and put `lobster` on
`PATH`.
...
### Important limitation: embedded Lobster vs `bot.invoke`

The bundled Lobster plugin runs workflows **in-process** inside the gateway.
In that embedded mode, `bot.invoke` does **not** automatically inherit a
gateway URL/auth context for nested Bot CLI tool calls.
...
environment where `bot.invoke` is already configured with the correct
...
- non-`bot.invoke` steps inside the Lobster pipeline until a supported
...
a bare envelope: Bot creates or resumes a durable flow record, applies the
...
- **No secrets** - Lobster doesn't manage OAuth; it calls Bot tools that
  do.
