...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Bot
...
Today this applies to Bot embedded runs and Bot dynamic tools in the Codex
app-server harness. Tokenjuice hooks Bot's tool-result middleware and
...
```bash
bot plugins install clawhub:@hanzo/bot-tokenjuice
```
...
```bash
bot config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
bot plugins disable tokenjuice
```
