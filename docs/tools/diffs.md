...
```bash
bot plugins install clawhub:@hanzo/bot-diffs-language-pack
```
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/bot",
        },
      },
    },
  },
...
- Viewer HTML and metadata live in the shared `state/bot.sqlite` database under the Diffs plugin blob namespace. HTML is gzip-compressed; SQLite stores only a SHA-256 hash of the random URL token, not the token itself.
- Rendered PNG/PDF files remain temporary materializations under `$TMPDIR/bot-diffs` because channel delivery requires a file path. SQLite owns their expiry metadata; no JSON sidecars are written.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in Bot config.
  </Step>
