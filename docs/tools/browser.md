...
Bot can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls. It runs through a small local control service inside the Gateway (loopback only) and is isolated from your personal browser.

- Think of it as a **separate, agent-only browser**. The `bot` profile never touches your personal browser profile.
...
If `bot browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `bot browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `bot browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
- `user`: built-in Chrome DevTools MCP attach profile for your **real
  ...
  prompt the first time Bot attaches, so someone must be at the computer.
- `chrome`: built-in [Chrome extension](/tools/chrome-extension) profile for
  ...
  desk because it drives tabs through the Bot browser extension instead of
  the remote-debugging port, so there is no "Allow remote debugging?" prompt.

For agent browser tool calls:

- Default: use the isolated `bot` browser.
...
Session tab cleanup applies only to tabs created by the Bot browser tool
with `action: "open"`. Bot does not adopt tabs that were already open,
...
safely after restart. This durable path can cover Bot-managed profiles,
regular remote CDP profiles, and existing-session profiles with an explicit
`cdpUrl`, provided Bot can resolve both the native target and a stable
browser identity. Before closing a durable record, Bot verifies that the
...
but Bot does not automatically attach them to channel replies. To share a
...
understanding model is configured, Bot keeps the normal image result so the
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). `BOT_GATEWAY_PORT` takes priority over `gateway.port`; either shifts the derived ports in the same family.
- Local `bot` profiles auto-assign `cdpPort`/`cdpUrl` from a range starting 9 ports above the control port (default `18800`-`18899`); set those only for
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, Bot pauses new launch
...
- Before each fresh Bot-managed Chrome launch, Bot best-effort disables network prediction, suppressing Chromium's observed speculative preconnect for those denied loads. This is defense in depth, not a policy boundary: a browser reused across a control-service restart and other browser backends may not share the hardening. Playwright routing is still not a network firewall and does not intercept redirect hops, a popup's first request, Service Worker traffic, page code that runs after the bounded guard window, or every background/subresource path. Complete egress isolation requires owner-side isolation or a policy-enforcing proxy.
- In strict SSRF mode, remote CDP endpoint discovery and `/json/version` probes (`cdpUrl`) are checked too.
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the Bot-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
- Bot-managed local CDP readiness probes and DevTools WebSocket connections bypass the managed network proxy for the exact launched loopback endpoint, so `bot browser start` still works when an operator proxy blocks loopback egress.
...
- `POST /start?headless=true` and `bot browser start --headless` request a
  ...
  remote CDP profiles reject the override because Bot does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  ...
  `bot browser --json status`; trailing `bot browser status --json`
...
- `BOT_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `BOT_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- For a running local managed profile, status and doctor query Chrome's
  ...
  `bot browser --json status`. A passive status call does not launch Chrome.
...
- Default profile is `bot` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
- `driver: "extension"` drives your signed-in Chrome through the [Bot Chrome extension](/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
Per-profile `executablePath` only affects local managed profiles that Bot
...
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local Bot-managed browser profile.
- `headless` only affects local managed profiles that Bot launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `bot browser stop` stops the browser process that
  Bot launched
- attach-only and remote CDP profiles: `bot browser stop` closes the active
  ...
  though no browser process was launched by Bot
...
- If you set `nodeHost.browserProxy.allowProfiles`, Bot treats it as a least-privilege boundary limiting which profile names the proxy will target.
...
CDP connection URLs over HTTPS and WebSocket. Bot can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Bot
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and Bot runs on the host, treat
...
Bot process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-Bot WebSocket base, such
...
an address Bot cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, Bot treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by Bot.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Bot accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  Bot calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. Bot connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). Bot tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise Bot
  ...
  accepts it, Bot falls back to that root as well. This lets a bare `ws://`
...
`bot browser doctor` uses the same discovery-first, WebSocket-fallback
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-bot-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Bot
...
- Configure `gateway.auth.token`, `gateway.auth.password`, `BOT_GATEWAY_TOKEN`, or
  `BOT_GATEWAY_PASSWORD` explicitly if you want a stable secret you control
...
Bot can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Bot attaches.
...
```bash
bot browser --browser-profile user start
bot browser --browser-profile user status
bot browser --browser-profile user tabs
bot browser --browser-profile user snapshot --format ai
```
...
- `bot doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `bot` profile because it can
  act inside your signed-in browser session.
- Bot does not launch the browser for this driver; it only attaches.
- Bot uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, Bot skips
...
Compared to the managed `bot` profile, existing-session drivers are more constrained:
...
- **Dialog visibility** - Managed browser action responses include `blockedByDialog` and `browserState.dialogs.pending` when an action opens a modal dialog; snapshots also include pending dialog state. Respond with `browser dialog --accept/--dismiss --dialog-id <id>` while a dialog is pending. Dialogs handled outside Bot appear under `browserState.dialogs.recent`.
...
control API** plus a matching `bot browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means Bot cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "bot" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by bot` when a
...
- For the local loopback `bot` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Bot's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (bot, chrome, or remote CDP).
