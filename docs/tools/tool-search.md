---
summary: "Tool Search: compact large Bot tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want Bot agents to use a large tool catalog without adding every tool schema to the prompt
  - You want Bot tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for Bot runs
---

Tool Search is an experimental Bot agent runtime feature. It gives agents one
...
This page documents Bot Tool Search. It is not the Codex-native tool
...
For the generic Bot runtime that exposes a QuickJS-WASI `exec`/`wait`
surface instead of Tool Search controls, see [Code Mode](/tools/code-mode).

When enabled for Bot runs, the model automatically receives a bounded
...
JavaScript body in an isolated Node subprocess with an `bot.tools` bridge:

```js
const hits = await bot.tools.search("create a GitHub issue");
const tool = await bot.tools.describe(hits[0].id);
return await bot.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include catalog-eligible Bot tools, plugin tools, MCP
...
Bot. Direct-only tools remain model-visible and are not added to the
catalog.

Codex harness runs do not receive these experimental Bot Tool Search
controls. Bot passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the Bot embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible Bot and plugin tools.
...
7. Expose the Bot code bridge, the structured fallback tools, or the
   compact directory surface alongside those stable, directly callable tools.

At execution time every real tool call returns to Bot. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`bot.tools.call(...)` crosses the bridge back into the Gateway, where the
...
All modes use the same policy-filtered catalog and normal Bot execution
...
for the current run while Bot tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small Bot tool
...
catalog includes catalog-eligible Bot, MCP, and client tools after normal
...
and built from the already policy-filtered catalog. Bot reuses the
...
authorized catalog changes, Bot builds a new directory for the new
snapshot.

## API

`bot.tools.search(query, options?)`
...
Bot core or plugin tool may also include a compact `output` hint, such as
...
```js
const hits = await bot.tools.search("calendar event", { limit: 5 });
```

`bot.tools.describe(id)`
...
```js
const calendarCreate = await bot.tools.describe("mcp:calendar:create_event");
```

`bot.tools.call(id, args)`

Calls a selected tool through Bot and returns the raw `{ tool, result }`
envelope. JSON-returning tools normally place their value in
`result.details`. Bot validates a trusted core or plugin tool's declared
...
If a trusted tool also declares `outputSchema`, Bot compiles that schema
...
```js
await bot.tools.call(calendarCreate.id, {
  summary: "Planning",
...
hidden directory tool name directly, Bot resolves it from the authorized
catalog before normal execution.
Directory-mode client tool names must not collide with Bot, plugin, or MCP
...
network grants, and no child-process or worker grants. Bot enforces a
...
Normal Bot behavior still applies to final calls:
...
Enable Tool Search for Bot runs with the default code bridge:

```bash
bot config set tools.toolSearch true
```
...
Use the structured fallback tools instead for Bot runs:
...
Use the compact directory surface instead for Bot runs:
...
- `sources`: catalog entry counts split into `bot`, `mcp`, and `client`
...
operation. Bot does not record serialized tool or prompt byte counts. The
...
- whether the result came from Bot, MCP, or a client tool

## E2E validation

The QA Lab gateway scenario proves both paths with the Bot runtime:

```bash
pnpm bot qa suite --provider-mode mock-openai --scenario tool-search-gateway-e2e
```
