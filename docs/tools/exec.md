...
Run shell commands in the workspace. `exec` is a mutating shell surface: commands can create, edit, or delete files wherever the selected host or sandbox filesystem permits. Disabling Bot filesystem tools such as `write`, `edit`, or `apply_patch` does not make `exec` read-only.
...
- On non-Windows gateway hosts, bash and zsh exec commands use a startup snapshot. Bot captures sourceable aliases/functions and a small safe environment set from shell startup files into `$BOT_STATE_DIR/cache/shell-snapshots/`, then sources that snapshot before each exec command. Secret-looking variables are excluded; sandbox and node exec do not use this snapshot. Set `BOT_EXEC_SHELL_SNAPSHOT=0` in the Gateway process environment to disable this snapshot path.
- Host execution (`gateway`/`node`) rejects `env.PATH` and loader overrides (`LD_*`/`DYLD_*`) to prevent binary hijacking or injected code.
- Bot sets `BOT_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
- For channel-origin runs, Bot also exposes a narrow sender/chat identity JSON payload in `BOT_CHANNEL_CONTEXT` when the channel provided those ids.
- `exec` cannot run `bot channels login` or `/approve` shell commands: `bot channels login` is an interactive channel-auth flow, and `/approve` needs to go through the approval command handler, not a shell. Run channel login in a terminal on the gateway host, or use a channel-specific login agent tool when one exists (for example `whatsapp_login`).
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`. Bot prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation); `tools.exec.pathPrepend` applies here too.
