...
Bot supports **Ollama Web Search** as a bundled `web_search` provider,
...
<Note>
  Bot never auto-selects Ollama Web Search over a higher-priority
...
config examples. If nothing is set, Bot defaults to
...
- If the resolved host is `https://ollama.com`, Bot calls
  `https://ollama.com/api/web_search` directly with the API key as bearer
  auth.
- Otherwise Bot calls the local proxy endpoint
  ...
  the local host.
- Bot warns during setup if Ollama is unreachable or not signed in, but
