...
When a site requires login, sign in manually in the host browser's `bot`
...
Bot controls a dedicated Chrome profile named `bot` (orange-tinted
UI), separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent uses its isolated `bot` browser.
...
Two ways to access the `bot` profile:
...
subcommand (default is `bot`):

```bash
bot browser --browser-profile <name> open https://x.com
```
...
```bash
bot browser --browser-profile bot open https://x.com
```
