...
frontmatter and a markdown body. Bot loads bundled skills plus any local
...
Bot loads from these sources, **highest precedence first**. When the same
...
Skill roots support grouped layouts. Bot discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an Bot
  skill root. Use `bot migrate plan codex` to inventory those skills, then
  `bot migrate codex` to copy them into your Bot workspace.
</Note>

## Node-hosted skills

A connected headless node can publish skills installed in its active Bot
skills directory (`~/.bot/skills` by default; profile environment overrides
...
When `BOT_STATE_DIR` points somewhere other than the default
`~/.bot`, session skill indexes exclude home-scoped personal or
...
`metadata.bot.requires` in its frontmatter, same as any other skill.
...
```bash
bot skills workshop list
bot skills workshop inspect <proposal-id>
bot skills workshop apply <proposal-id>
```
...
`bot skills` commands for install and update, or the `clawhub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `bot skills install @owner/<slug>`                |
| Install an external skills.sh ref  | `bot skills install skills-sh:owner/repo/slug`    |
| Install from a Git repository      | `bot skills install git:owner/repo@ref`           |
| Install a local skill directory    | `bot skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `bot skills install @owner/<slug> --global`       |
| Update all workspace skills        | `bot skills update --all`                         |
| Update a shared managed skill      | `bot skills update @owner/<slug> --global`        |
| Update all shared managed skills   | `bot skills update --all --global`                |
| Verify a skill's trust envelope    | `bot skills verify @owner/<slug>`                 |
| Print the generated Skill Card     | `bot skills verify @owner/<slug> --card`          |
| Publish / sync via ClawHub CLI     | `clawhub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `bot skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.bot/skills` directory, visible to all local agents unless agent
    ...
    `bot skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `bot skills verify @owner/<slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `bot.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.bot/skills` and personal `~/.agents/skills` may contain
...
<Note>
  Bot follows the [AgentSkills](https://agentskills.io) spec. Frontmatter
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Bot keeps the skill's instructions out of the agent's normal
...
Bot filters skills at load time using `metadata.bot` (JSON5 object
embedded in the frontmatter, see the parsing note above). A skill with no
`metadata.bot` block is always eligible unless explicitly disabled.
...
<ParamField path="requires.config" type="string[]">
  Each `bot.json` path must be truthy.
</ParamField>
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Bot lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `bot.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** Bot does not auto-install Homebrew or translate brew
    ...
    - **Go:** Bot requires Go 1.21 or newer for automatic skill installs.
      If `go` is missing and Homebrew is available, Bot installs Go via
      ...
      dependency always targets a dedicated Bot-managed bin directory
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.bot/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.bot.primaryEnv`.
...
<Note>
  ...
  `metadata.bot.skillKey`, use that key under `skills.entries` instead.
...
<Steps>
  <Step title="Reads skill metadata">
    Bot resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, Bot also materializes the same
...
Bot snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, Bot refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, Bot watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, Bot can treat macOS-only skills as eligible when
    ...
    answering bin probes, Bot clears its cached bin matches.
...
When skills are eligible, Bot injects a compact XML block into the system
...
(`skills.limits.maxSkillsPromptChars`), Bot first preserves as many skill
...
note pointing at `bot skills check` whenever compact formatting or list
