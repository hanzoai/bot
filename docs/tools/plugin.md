---
summary: "Install, configure, and manage Bot plugins"
read_when:
...
Plugins extend Bot with channels, model providers, agent harnesses, tools,
...
- an Bot checkout or installation with the `bot` CLI available
...
<Steps>
  <Step title="Find the plugin">
    ...
    they match an official plugin id. Raw `@hanzo/bot-*` specs that match a
  ...
  <Step title="Install the plugin">
    ...
    # From npm.
    bot plugins install npm:<package>

    # From git.
    bot plugins install git:github.com/<owner>/<repo>@<ref>

    # From a local development checkout.
    bot plugins install ./my-plugin
    bot plugins install --link ./my-plugin
    ```

    Treat plugin installs like running code. Prefer pinned versions for
    reproducible production installs. ClawHub packages and Bot's
  ...
  <Step title="Configure and enable it">
    ...
    ```bash
    bot plugins enable <plugin-id>
    ```

    If `plugins.allow` is set, the installed plugin id must be in that list
    before the plugin can load. `bot plugins install` adds the installed
  ...
  <Step title="Verify runtime registration">
    ```bash
    bot plugins inspect <plugin-id> --runtime --json
    ```
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want Bot-native discovery, scans, version metadata, and install hints | `bot plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `bot plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `bot plugins install git:github.com/<owner>/<repo>@<ref>` |
| local path  | You are developing or testing a plugin on the same machine                     | `bot plugins install --link ./my-plugin`                  |
| marketplace | You are installing a Claude-compatible marketplace plugin                      | `bot plugins install <plugin> --marketplace <source>`     |
...
bare spec installs through npm during the launch cutover. Raw `@hanzo/bot-*`
specs that match bundled plugins also resolve to the bundled copy before npm
fallback. Use `npm:@hanzo/bot-<plugin>@<version>` to deliberately install the
external npm package instead of the bundled copy. Use `clawhub:`, `npm:`,
`git:`, or `npm-pack:` for deterministic source selection. See
[`bot plugins`](/cli/plugins#install) for the full command contract.

For npm installs, unpinned specs and `@latest` choose the newest stable
package that advertises compatibility with this Bot build. If npm's
current latest release declares a newer `bot.compat.pluginApi` or
`bot.install.minHostVersion` than this build supports, Bot scans
...
later, and only in Bot processes where plugin hooks are loaded, so use
...
compatibility but is a no-op: it does not bypass install policy or Bot's
...
snippet. Run [`bot plugins list --enabled --verbose`](/cli/plugins#list)
or [`bot plugins inspect <id>`](/cli/plugins#inspect) on the listed
plugin id before copying trusted plugins into `bot.json`. The same
...
`plugins.allow` or reinstall from a trusted source so Bot records install
provenance.

Run `bot doctor` or `bot doctor --fix` when config validation
reports stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin
paths.

## Understand plugin formats

Bot recognizes two plugin formats:

| Format                 | How it loads                                                                 | Use when                                                               |
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native Bot plugin | `bot.plugin.json` plus a runtime module loaded in process               | You are installing or building Bot-specific runtime capabilities  |
| Compatible bundle      | Codex, Claude, or Cursor plugin layout mapped into Bot plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `bot plugins list`, `bot plugins inspect`,
`bot plugins enable`, and `bot plugins disable`. See
...
Plugin-managed internal hooks show up in `bot hooks list` with
`plugin:<id>`. You cannot enable or disable them through `bot hooks`;
enable or disable the plugin instead.

## Verify the active Gateway

`bot plugins list` and plain `bot plugins inspect` read cold config,
...
sure any manual restart targets the actual `bot gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `bot plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `bot plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `bot doctor --fix` if it points to stale plugin state                                             | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `bot plugins registry --refresh`                    |
| `BOT_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `bot plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |

When an enabled managed plugin fails payload verification during Gateway
startup, Bot quarantines that exact installed plugin root for the boot and
continues serving other plugins. `bot status --all`, `bot health`,
and `bot doctor` report it as `configured-unavailable`. Fix or reinstall
...
`bot doctor --fix` to remove stale plugin and channel entries. Unknown
...
plugin id. If both plugins are explicitly enabled, Bot keeps that request
...
Bot needs at runtime. Update or reinstall after the publisher ships
...
and validation follows with `plugin present but blocked`, Bot found
plugin files owned by a different Unix user than the process loading them.
Keep the plugin config in place; fix the filesystem ownership or run Bot
as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Bot config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Bot as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/bot-config/npm
```

After fixing ownership, rerun `bot doctor --fix` or
`bot plugins registry --refresh` so the persisted plugin registry
...
```bash
bot config set logging.level trace
bot logs --follow
```
...
Bot caches successful plugin tool factory results for repeated
...
- [`bot plugins`](/cli/plugins) - full CLI reference
