...
`plugins.entries.firecrawl.config.webFetch` via `bot doctor --fix`.
...
- If `provider` is omitted, Bot auto-detects the first ready web-fetch
...
In this mode, Bot still applies hostname-based SSRF checks before sending
