...
Snap's AppArmor confinement interferes with how Bot spawns and monitors
the browser process.

Other common Linux launch failures:

- `The profile appears to be in use by another Chromium process`: stale
  `Singleton*` lock files in the managed profile directory. Bot removes
...
- `Missing X server or $DISPLAY`: a visible browser was explicitly requested
  ...
  If you set `BOT_BROWSER_HEADLESS=0`, `browser.headless: false`, or
  `browser.profiles.<name>.headless: false`, remove that headed override, set
  `BOT_BROWSER_HEADLESS=1`, start `Xvfb`, run
  `bot browser start --headless` for a one-shot managed launch, or run
  Bot in a real desktop session.
...
If you must keep snap Chromium, configure Bot to attach to a
...
```bash
systemctl --user enable --now bot-browser.service
```
...
| `BOT_BROWSER_HEADLESS` | Per-process override for local managed browser headless mode         | unset                                                              |
...
1. Use the managed browser instead:
   `bot browser --browser-profile bot start` (or set
   `browser.defaultProfile: "bot"`).
...
- Local `bot`-driver profiles auto-assign `cdpPort`/`cdpUrl`; only set
