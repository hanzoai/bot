...
<Steps>
  ...
  <Step title="Store the key">
    ...
    gateway install, add it to `~/.bot/.env`), or configure via:
...
Choosing **Kimi** during `bot onboard` or `bot configure --section web`
also prompts for:
...
Bot only returns a Kimi `web_search` result after Moonshot's response
...
grounding (for example "I cannot browse the internet"), Bot returns a
