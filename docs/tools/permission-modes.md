...
read_when:
  ...
  - Comparing Bot exec approvals with ACPX harness permissions
...
```bash
bot config set tools.exec.mode auto
bot approvals get
bot gateway restart
```
...
Set ACPX permissions separately from Bot exec approvals:
...
| Use Codex/Bot auto-review before humans  | `tools.exec.mode: "auto"`                                   |
...
Host exec uses the stricter result of Bot config and the host-local approvals file. ACPX harness permissions do not loosen host exec approvals, and host exec approvals do not loosen ACPX harness prompts.
