...
summary: "Build, test, and publish custom SKILL.md workspace skills for your Bot agents."
read_when:
...
Bot loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.bot/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    Bot watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ```bash
    bot agent --message "give me a greeting"
    ```
...
metadata: { "bot": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `bot.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `bot.json`:
...
bot skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.bot` gating fields
...
<Tip>
  ...
  - **Test locally** — use `bot agent --message "..."` before sharing.
