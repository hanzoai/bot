...
`~/.hanzoai/bot.json`. Agent-specific visibility lives under
...
<ParamField path="skills.install.nodeManager" type='"npm" | "pnpm" | "yarn" | "bun"' default='"npm"'>
  ...
  installs - the Bot CLI and Gateway runtime require Node because the
  canonical state store uses `node:sqlite`. `bot setup --node-manager` and
  `bot onboard --node-manager` accept `npm`, `pnpm`, or `bun`; set
...
policy runs after Bot has staged source material and before the install
...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. Bot runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the Bot process into the
...
`botVersion`, `targetType`, `targetName`, `sourcePath`, `sourcePathKind`,
...
Bot does not execute install policy during normal Gateway startup.
Installs and updates fail closed when policy is enabled but unavailable.
`bot doctor` performs static validation; `bot doctor --deep`
...
```json
{
  ...
  "targetType": "skill",
  "targetName": "weather",
  "sourcePath": "/var/folders/.../bot-skill-clawhub/root",
...
`metadata.bot.skillKey`, use that key instead. Quote hyphenated names
...
<Warning>
  Agent skill allowlists are a visibility and loading filter for Bot
  ...
  registries such as `~/.bot/skills/config/mcporter.json`. For
...
<ParamField path="skills.workshop.autonomous.enabled" type="boolean" default="false">
  When `true`, Bot can create pending proposals from durable corrections
...
Managed `~/.bot/skills` and personal `~/.agents/skills` directories
