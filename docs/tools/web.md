...
normalized results, cached by query for 15 minutes (configurable). Bot
...
<Steps>
  ...
  <Step title="Configure">
    ...
    [Bot](/cli/bot): say `configure web search` in `bot setup`
    or in the Control UI's **Settings → Ask Bot** chat. The hosted flow
  ...
  <Step title="Use it">
    ```javascript
    await web_search({ query: "Bot plugin SDK" });
    ```
...
no `provider` is set, Bot checks providers in this order and uses the
...
`bot configure --section web`. Bot does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Bot resolves only the
...
`web_search` tool automatically when Bot web search is enabled and no
...
search and Bot's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. Bot uses the
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  Bot's managed `web_search` instead
...
use the managed `web_search` fallback through Bot's dynamic tool namespace.
Use an explicit managed provider when you need Bot's provider-specific
...
Codex app-server first with `bot models auth login --provider openai`.
The parent agent can use any model or runtime; only the bounded search worker
runs through Codex.

## Network safety

Managed HTTP `web_search` provider calls use Bot's guarded fetch path,
scoped to the current provider's own hostname. For that hostname only,
Bot allows Surge, Clash, and sing-box fake-IP DNS answers in
...
Grok can also reuse an xAI OAuth auth profile from `bot models auth login
...
Bot keeps startup resilient and reports a warning so you can reinstall the
plugin or run `bot doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let Bot auto-detect the first ready web-fetch
...
When you choose **Kimi** during `bot onboard` or
`bot configure --section web`, Bot can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `bot doctor --fix`.
When you choose Grok during `bot onboard` or `bot configure --section web`,
Bot also offers optional `x_search` setup with the same credential right
...
provider, Bot does not show the `x_search` prompt.

### Storing API keys

<Tabs>
  <Tab title="Config file">
    Run `bot configure --section web` or set the key directly:
...
optional structured filters. Bot constructs the built-in xAI `x_search`
