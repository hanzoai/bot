...
`~/.hanzo/bot/bot.json` under `tools.web.search.apiKey`.
...
environment. For a gateway install, put it in `~/.hanzo/bot/.env`.

If no base URL is set, Hanzo Bot chooses a default based on the API key source:
