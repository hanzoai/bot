---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, Bot ACP, OpenCode) through the ACP backend"
read_when:
...
Bot run external coding harnesses (Claude Code, Cursor, Copilot, Droid,
Bot ACP, OpenCode, Gemini CLI, and other supported ACPX harnesses)
...
existing Bot channel conversations, use
[`bot mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ Bot | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                 |
| Expose an Bot Gateway session _as_ an ACP server for an editor or client                   | [`bot acp`](/cli/acp)            | Bridge mode: an IDE/client speaks ACP to Bot over stdio/WebSocket                                                                                                      |
| Reuse a local AI CLI as a text-only fallback model                                              | [CLI Backends](/gateway/cli-backends) | Not ACP: no Bot tools, no ACP controls, no harness runtime                                                                                                             |
...
bot config set plugins.entries.acpx.enabled true
```
...
Bot only teaches agents about ACP spawning when ACP is **truly usable**:
...
<AccordionGroup>
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. Bot owns routing,
...
Bot plugin tools and built-in Bot tools are **not** exposed to ACP
...
| `bot`   | Bot Gateway bridge through `bot acp` | Lets an ACP-aware harness talk back to an Bot Gateway session.                 |
...
Custom acpx agent aliases can be configured in acpx itself, but Bot
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the Bot session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from Bot's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up Bot-owned wrapper and adapter process trees after `close`, and reaps stale Bot-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    Bot dynamic tools still execute through Bot, while Codex-native
    tools such as shell/apply-patch execute inside Codex. For Codex-native
    tool events, Bot injects a per-turn native hook relay so plugin hooks
    can block `before_tool_call`, observe `after_tool_call`, and route Codex
    `PermissionRequest` events through Bot approvals. Codex `Stop` hooks
    are relayed to Bot `before_agent_finalize`, where plugins can request
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    Bot picks `runtime: "acp"`, resolves the harness `agentId`, binds to
    ...
    `gemini`, or `opencode`. Do not pass a normal Bot config agent id
    ...
    runtime. When an Bot agent is configured with
    `runtime.type="acp"`, Bot uses `runtime.acp.agent` as the underlying
    harness id.
...
is enabled. Use **sub-agents** when you want Bot-native delegated runs.
...
1. Bot ACP session control plane.
2. Official `@hanzo/bot-acpx` runtime plugin.
...
- **ACP session** - the durable Codex/Claude/Gemini runtime state Bot routes to.
...
spawned ACP session - no child thread, same chat surface. Bot keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; Bot returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnSessions` gates child thread creation for `--thread auto|here` - not `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, Bot inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations - `/acp ...` commands are handled by Bot even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.
  ...
  <Accordion title="Thread-bound sessions">
    ...
    does not support thread bindings, Bot returns a clear
...
- Bot ensures the configured ACP session exists after channel-specific admission and before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, Bot inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    ACP sessions. If `agentId` is omitted, Bot uses `acp.defaultAgent`
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, Bot may default to persistent behaviour per
...
<ParamField path="streamTo" type='"parent"'>
  ...
  session as system events. Bot records the full relay history in the
...
<ParamField path="model" type="string">
  ...
  `models` and support `session/set_model`; otherwise Bot/acpx fails
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    - Runtime-generated completion events are materialized per target. Bot agents get Bot's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_BOT_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in Bot where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. Bot converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see Bot-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    sessions, Bot uses an agent-to-agent (A2A) follow-up path after
    ...
    Bot skips the A2A follow-up only when the requester is the parent of
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an Bot channel session key; Bot still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Bot session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, **not** inside the Bot
sandbox.
...
- Bot's sandbox policy does **not** wrap ACP harness execution.
- Bot still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced Bot-native work.
...
If no target resolves, Bot returns a clear error
...
| ---------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, Bot normalizes `openai/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai/gpt-5.4/high` to `reasoning_effort`.                                         |
| `/acp set thinking <level>`  | canonical option `thinking`          | Bot sends the backend-advertised equivalent when present, preferring `thinking`, then `effort`, `reasoning_effort`, or `thought_level`. For Codex ACP, the adapter maps values to `reasoning_effort`. |
| `/acp permissions <profile>` | canonical option `permissionProfile` | Bot sends the backend-advertised equivalent when present, such as `approval_policy`, `permission_profile`, `permissions`, or `permission_mode`.                                                       |
| `/acp timeout <seconds>`     | canonical option `timeoutSeconds`    | Bot sends the backend-advertised equivalent when present, such as `timeout` or `timeout_seconds`.                                                                                                     |
...
the plugin-tools and Bot-tools MCP bridges, and ACP permission modes,
...
| Vendor auth error from the harness                                                        | Bot is healthy, but the target CLI/provider is not logged in.                                                     | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| ACP session stalls indefinitely after completing work                                     | Harness process finished but ACP session did not report completion.                                                    | Update Bot; current acpx cleanup reaps Bot-owned stale wrapper and adapter processes on close and Gateway startup.                                             |
| Harness sees `<<<BEGIN_BOT_INTERNAL_CONTEXT>>>`                                      | Internal event envelope leaked across the ACP boundary.                                                                | Update Bot and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                          |
...
the next native tool call, restart the Codex app-server or Bot Gateway
