---
summary: "Export redacted trajectory bundles for debugging an Bot agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Bot session
...
Trajectory capture is Bot's per-session flight recorder. It records a
...
Bot writes the bundle under the workspace:

```text
.bot/trajectory-exports/bot-trajectory-<session>-<timestamp>/
```
...
The name resolves inside `.bot/trajectory-exports/`. Absolute paths and
`~` paths are rejected.
...
bundle; do not use allow-all. In group chats, Bot sends the approval
...
`.bot/trajectory-exports`), `--store <path>` (session store override),
...
Trajectory capture is on by default for Bot agent runs.
...
| `metadata.json`       | Bot version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
```bash
export BOT_TRAJECTORY=0
```

This disables runtime trajectory capture before starting Bot.
...
Bot flushes runtime trajectory rows during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`BOT_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Bot:

```bash
export BOT_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Bot logs an `bot-trajectory-flush` timeout and
...
`BOT_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are for support and debugging, not public posting. Bot
...
- confirm Bot was started without `BOT_TRAJECTORY=0`
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.bot/trajectory-exports/`
