...
read_when:
  - Looking for an overview of Bot's media capabilities
...
Bot generates images, videos, and music, understands inbound media
...
Bot until the shared realtime-voice contract can represent it.
</Note>
...
For async tools, Bot submits the request to the provider, returns a task
...
Bot wakes the agent with the generated media paths so it can tell the
...
Bot sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    other native model types; Bot has no provider contract for those
  ...
  <Accordion title="xAI">
    ...
    not registered in Bot until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
