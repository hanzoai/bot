---
summary: "Use Bot Code Mode to discover, call, and combine large tool catalogs in compact JavaScript or TypeScript workflows"
title: "Code Mode"
sidebarTitle: "Code Mode"
read_when:
  - You want to enable Bot code mode for an agent run
...
Code mode is an experimental Bot agent-runtime feature. It defaults to the
...
This page documents Bot code mode, not Codex Code Mode. The two features
...
- Bot code mode runs in the generic Bot agent runtime, gated by
...
In Bot code mode, `command` is a JavaScript or TypeScript alias for
...
- Every catalog-eligible enabled tool (Bot core, plugin, MCP, client) is hidden as a
  standalone model tool and exposed inside the guest program through `ALL_TOOLS`
  and `tools`.
- The `exec` description carries a bounded quick index of exact Bot/plugin
...
- Provider neutral: works for Bot, plugin, MCP, and client tools without
...
`BOT_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
| Stability           | experimental Bot surface (Codex Code Mode is a separate, stable Codex harness surface) |
| Target surface      | generic Bot agent runs                                                                 |
...
- **Code mode**: the Bot runtime mode that hides catalog-compatible model
...
If code mode is enabled but QuickJS-WASI cannot load, Bot fails closed
...
For OpenAI models, the flag matters only when the run resolves to the Bot
embedded agent runtime. Default OpenAI routing uses the Codex-style harness
surface, where Bot code mode does not apply; the catalog flag never
...
1. Resolve the agent, model, provider, sandbox, channel, sender, and run
   policy.
2. Build the effective Bot tool list, adding eligible plugin, MCP, and
   client tools.
...
when `tools.codeMode.enabled: true` is configured. Code mode and Bot Tool
...
- `command` is accepted as an exec-compatible alias for hook policies and
  trusted rewrites (the normal Bot shell exec tool also uses a `command`
...
- If `language` is `"typescript"`, Bot transpiles before evaluation.
...
final value is JSON-compatible after Bot's output adapter runs.
...
`wait` exists because nested Bot tools can be slow, interactive, approval
...
2. On suspension, Bot snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot and
   re-registers host callbacks by stable names.
4. Bot delivers nested tool results into the restored VM and drains
...
bounded, deterministic subset of exact Bot/plugin ids, compact input
...
present only for a complete compact hint derived from a trusted Bot core
or plugin `outputSchema`. MCP and client output-schema claims are not promoted
into this trusted catalog hint.

Plugin tools use `source: "bot"` with `sourceName` set to the owning
...
const hits = await tools.web_search({ query: "Bot code mode" });
```
...
Bot tools can declare `outputSchema` for the structured value placed in
...
- Bot compiles the schema before running the tool, then validates final
...
directory. For each code-mode `exec` call, Bot builds the run-scoped tool
...
ordinary Bot tools, not through the code-mode bridge.

## Tool catalog

The hidden catalog includes tools after effective policy filtering, in this
order: Bot core tools, bundled plugin tools, external plugin tools, MCP
...
where `<source>` is `bot`, `mcp`, or `client` (plugin tools use
`bot` with the plugin id as `<owner>`; core tools use `bot:core:*`).
Examples:

```text
bot:core:message
bot:browser:browser_request
...
Code mode supersedes the Bot Tool Search model surface for runs where it
is active.

When `tools.codeMode.enabled` is true and code mode activates:

- Bot does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same Bot executor path that Tool
  Search uses.

See [Tool Search](/tools/tool-search) for the Bot compact catalog bridge
that code mode supersedes for active runs.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal Bot
shell `exec` tool is enabled, it is hidden from the model and cataloged like
any other tool.

Inside the guest runtime:

- `tools.call("bot:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, Bot omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters Bot,
...
- For read-only work, `exec` can set `restartSafe: true`. Bot then rejects
...
- Bot caps the number of concurrently suspended runs per process (64) and
...
Bot loads `quickjs-wasi` as a direct dependency in the owning package; it
...
The runtime executes in a Node.js worker thread, outside Bot's main
...
breakdown (`bot`/`mcp`/`client` counts), cumulative search/describe/call
counts for the run's catalog, and the model-visible tool names (`exec`,
`wait`, and retained direct-only tools).

The run metadata (`meta.agentMeta` in `bot agent --json`, mirrored on the
...
tool inputs beyond existing Bot trajectory policy.
...
For payload-shape debugging, use `BOT_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `BOT_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, `tools.callValue`, and `tools.call` work for Bot tools
...
5. Send an agent turn with Bot, plugin, MCP, and client test tools.
...
8. In `exec`, call Bot/plugin/client tools through `tools.search`,
