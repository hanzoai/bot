...
Bot supports Grok as a `web_search` provider, using xAI web-grounded
...
lets Bot reuse it as a fallback for the bundled xAI model provider.
...
Choosing **Grok** during `bot onboard` or `bot configure --section
web` lets Bot reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

Bot then offers a follow-up step to enable `x_search` with the same xAI
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    bot onboard --auth-choice xai-oauth
...
**Credential alternatives:** `bot models auth login --provider xai
...
through an operator proxy or xAI-compatible Responses endpoint. Bot
