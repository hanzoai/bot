...
read_when:
  ...
  - Enabling the plugin-tools or Bot-tools MCP bridge
...
| `bot`   | Bot ACP bridge (native `bot acp`)                                                            |
...
normalized by Bot before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Bot/acpx cannot force a model selection.
...
Packaged installs use the official `@hanzo/bot-acpx` runtime plugin for ACP.
...
```bash
bot plugins install ./path/to/local/acpx-plugin
```
...
signal. Set `BOT_ACPX_RUNTIME_STARTUP_PROBE=0` or
`BOT_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before Bot passes it through the current acpx command-string registry.
...
manually, and there is no separate postinstall step for Bot itself. If an
adapter download or spawn fails, `/acp doctor` reports the failure.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Bot plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Bot plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
bot config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `bot-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Bot
  plugins.
...
### Bot tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Bot tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
bot config set plugins.entries.acpx.config.botToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `bot-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Bot tools. The initial server exposes `cron`.
...
```bash
bot config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use Bot agent/run timeouts, including `/acp timeout`.
...
```bash
bot config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from Bot exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between Bot `tools.exec.mode`, Codex Guardian
...
```bash
bot config set plugins.entries.acpx.config.permissionMode approve-all
bot config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
Bot defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `PermissionPromptUnavailableError: Permission prompt unavailable in non-interactive mode`.
