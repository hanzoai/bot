...
`bot.invoke` already has the correct gateway URL/auth context.

For the bundled **embedded** Lobster runner inside Bot, this nested CLI
...
- Lobster steps that do not rely on nested `bot.invoke` calls.
