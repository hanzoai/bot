      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; Bot appends /v1/search
          },
        },
      },
    },
...
Cloudflare AI Gateway). Bot normalizes bare hosts by prepending
...
- Both providers return a `session_id`; Bot surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- Bot always forwards a resolved result count to Parallel as
  ...
  Bot's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.
