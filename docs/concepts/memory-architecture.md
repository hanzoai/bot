---
summary: "End-to-end architecture of Bot memory: tiers, provenance, dreaming, recall lanes, the user model, and standing intents"
title: "Memory architecture"
sidebarTitle: "Memory architecture"
read_when:
  - You want the complete picture of how Bot memory works end to end
...
Bot memory is a set of plain files and one SQLite index, organized into
...
2. **Writing is the hard part.** Retrieval over notes files is competitive
   ...
   arXiv:2410.10813). Bot therefore moves curation off the busy reply
   path and into a dedicated background pass.
3. **The write path is the security boundary.** Content-level scanning of
   memory cannot catch poisoned facts reliably, so Bot enforces
...
event benchmarks). Bot therefore compiles intentions out of the model:
...
measure poorly. Bot defends structurally:
...
more conservatively they write; Bot keeps those properties even with
