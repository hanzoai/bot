...
`bot/plugin-sdk/channel-outbound` / `channel-inbound` subpaths. For the
...
Public surface: `bot/plugin-sdk/channel-outbound` (send/receipt/durable/live/reply-pipeline
helpers) and `bot/plugin-sdk/channel-inbound` (inbound context, `runChannelInboundEvent`,
...
Bot only advances the persisted restart watermark past updates that
...
- **No `MessageOrigin` / `shouldDropBotEcho`.** The original plan called
  for a `source: "bot"` origin tag on gateway-failure messages plus a
...
- **iMessage** (`extensions/imessage/src/monitor/echo-cache.ts`,
  ...
  cache, or Bot can re-ingest its own replies as inbound user messages.
...
- Whether Telegram should eventually replace the grammY (`1.43.0`) polling
  ...
  redelivery, not only Bot's persisted restart watermark
