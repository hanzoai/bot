---
summary: "Delegate architecture: running Bot as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Run Bot as a **named delegate**: an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human - it sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate

A delegate is an Bot agent that:
...
Bot's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Bot's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.bot/workspace-delegate",
  tools: {
...
- Cron run history: Bot's shared SQLite state database.
- Session transcripts: `~/.bot/agents/delegate/sessions`.
- Identity provider audit logs (Exchange, Google Workspace).

All delegate actions flow through Bot's session store. For compliance, retain and review these logs.
...
```bash
bot agents add delegate --workspace ~/.bot/workspace-delegate
```

This creates:

- Workspace: `~/.bot/workspace-delegate`
- Agent state: `~/.bot/agents/delegate/agent`
- Sessions: `~/.bot/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.bot/workspace" },
      {
        id: "delegate",
        workspace: "~/.bot/workspace-delegate",
        tools: {
...
~/.bot/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.bot/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.bot/workspace-org",
        agentDir: "~/.bot/agents/org-assistant/agent",
