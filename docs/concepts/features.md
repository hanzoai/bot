...
- iMessage, Telegram, and WebChat ship with the core install; every other channel is an
  official plugin installed with `bot plugins install @hanzo/bot-<id>` (or on demand
  during `bot onboard` / `bot channels add`)
...
- External plugin channels maintained outside the Bot repo: WeChat, Yuanbao, and Zalo ClawBot
...
<CardGroup cols={2}>
  ...
  <Card title="Plugins" href="/tools/plugin" icon="plug">
    Official and external plugins that extend Bot.
  </Card>
</CardGroup>
