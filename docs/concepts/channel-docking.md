---
summary: "Move one Bot session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Bot session. It keeps the same
...
Alice can message Bot on Telegram and Discord:
...
Bot keeps the current session context and changes the reply route:
...
4. Keep the same Bot session, but receive future replies in Discord.
...
Bot generates one `/dock-<channel>` command for every loaded channel plugin
...
Send the dock command from a direct chat with Bot, not from a group chat.
