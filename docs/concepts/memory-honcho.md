...
[Honcho](https://honcho.dev) adds AI-native memory to Bot through an
...
```bash
bot plugins install @honcho-ai/bot-honcho
bot honcho setup
bot gateway --force
```
...
Settings live under `plugins.entries["bot-honcho"].config`:
    ...
    entries: {
      "bot-honcho": {
        config: {
          ...
          workspaceId: "bot", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `bot honcho setup` detects and
...
During conversation, Honcho tools query the service during Bot's
...
```bash
bot honcho setup                        # Configure API key and migrate files
bot honcho status                       # Check connection status
bot honcho ask <question>               # Query Honcho about the user
bot honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/bot-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Bot integration guide](https://docs.honcho.dev/v3/guides/integrations/bot)
