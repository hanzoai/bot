...
Bot has two independent streaming layers, and there is **no true
...
rejected everywhere. `bot doctor --fix` migrates legacy configs into the
nested shape.
...
streaming sends media early, Bot remembers that delivery for the turn. If
...
distinct text around media that was already streamed, Bot still sends the
...
When block streaming is enabled, Bot can **merge consecutive block
...
top-level boolean/string spellings are rewritten by `bot doctor --fix`).
...
| -------- | ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Telegram | `streamMode`, scalar/boolean `streaming`                    | Rewritten to `streaming.mode` by `bot doctor --fix`; not read at runtime                                                                        |
| Discord  | `streamMode`, boolean `streaming`                           | Rewritten to `streaming.mode` by `bot doctor --fix`; not read at runtime                                                                        |
| Slack    | `streamMode`; boolean `streaming`; legacy `nativeStreaming` | Rewritten to `streaming.mode` (and `streaming.nativeTransport` for the boolean/legacy forms) by `bot doctor --fix`; not read at runtime         |
| Matrix   | scalar/boolean `streaming`                                  | Rewritten to `streaming.mode` (including Matrix's `"quiet"` mode) by `bot doctor --fix`; not read at runtime                                    |
| Feishu   | boolean `streaming`                                         | Rewritten to `streaming.mode` by `bot doctor --fix`; not read at runtime                                                                        |
| QQ Bot   | boolean `streaming`; `streaming.c2cStreamApi`               | Rewritten to `streaming.mode` (and `streaming.nativeTransport` for the boolean/`c2cStreamApi` forms) by `bot doctor --fix`; not read at runtime |
...
- If the final edit fails before the completed text is confirmed, Bot uses
...
- Telegram selected quote replies are an exception: when `replyToMode` is not
  `"off"` and selected quote text is present, Bot skips the answer preview
...
- To keep preview streaming but hide tool-progress lines, set
  ...
  that use Bot's compact progress renderer, including Discord, Matrix,
