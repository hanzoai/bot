...
The inferred commitments experiment is retired. Bot no longer extracts new
conversation follow-ups or delivers them through heartbeat, and the former
`commitments` config block is removed by `bot doctor --fix`.
...
```bash
bot commitments --all
bot commitments dismiss cm_abc123
```

See [`bot commitments`](/cli/commitments) for the maintenance command
reference.
