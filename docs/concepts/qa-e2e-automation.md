...
```bash
pnpm bot qa matrix --provider-mode mock-openai
```
...
BOT_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
  pnpm bot qa matrix --provider-mode live-frontier
```

Plain `pnpm bot qa matrix` runs every flow scenario that explicitly
...
E2EE CLI scenarios also drive `bot matrix encryption setup` and
