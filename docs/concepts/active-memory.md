...
enabled, Bot indexes that agent's session transcripts and runs an Active
...
the reply without recalled transcript context. Bot's built-in memory
...
not automatically receive private transcript authorization. `bot doctor`
reports an unsupported provider or missing `memory_search` tool.

## Advanced Active Memory quick start

Paste into `bot.json` for an advanced safe default: plugin on, scoped to
...
case-insensitive. If `allowedChatIds` is non-empty and Bot cannot
...
With those on, Bot appends diagnostic lines after the normal reply (as a
...
external context-engine plugin (`bot plugins install
...
store. By default, Bot removes the temporary sub-agent session rows after
...
2. For Remember across conversations, confirm the agent's effective
   ...
   `bot doctor` to verify the current memory provider supports protected
...
6. Verify memory search itself works with `bot status --deep`.
...
provider must support Bot's protected same-agent/private-session recall
path.

<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memory.search.provider` is unset, Bot uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `bot status --deep` to inspect the memory-search backend and
      index health.
