---
summary: "How Bot separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between Bot, Codex, ACP, or another native agent runtime
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `anthropic`, `github-copilot`, `openai`      | How Bot authenticates, discovers models, and names model refs. |
| Model         | `claude-opus-4-6`, `gpt-5.6-sol`             | The model selected for the agent turn.                              |
| Agent runtime | `claude-cli`, `codex`, `copilot`, `bot` | The low-level loop or backend that executes the prepared turn.      |
| Channel       | Discord, Slack, Telegram, WhatsApp           | Where messages enter and leave Bot.                            |
...
runtime keys are legacy and ignored. `bot doctor --fix` removes old
...
- **Embedded harnesses** run inside Bot's prepared agent loop: the
  built-in `bot` runtime, plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | Bot name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
makes native app-server features available; `bot doctor --fix` owns
...
That means Bot selects an OpenAI model ref, then asks the Codex
...
Bot session store becomes Codex.
...
3. **Bot explicitly chosen for an OpenAI model** -> keep the model ref as `openai/<model>` and set provider/model runtime policy to `agentRuntime.id: "bot"`. A selected `openai` OAuth profile is routed internally through Bot's Codex-auth transport.
4. **Legacy Codex model refs in config** -> repair with `bot doctor --fix` to `openai/<model>`; doctor keeps the Codex auth route by adding provider/model-scoped `agentRuntime.id: "codex"` where the old model ref implied it. Legacy **`codex-cli/*`** model refs repair to the same `openai/<model>` Codex app-server route; Bot no longer keeps a bundled Codex CLI backend.
...
| Surface                     | Bot embedded                              | Codex app-server                                                            |
| --------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Bot, through the Bot embedded runner | Codex app-server                                                            |
| Canonical thread state      | Bot transcript                            | Codex thread, plus Bot transcript mirror                               |
| Bot dynamic tools      | Native Bot tool loop                      | Bridged through the Codex adapter                                           |
| Native shell and file tools | Bot path                                  | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Bot context assembly               | Bot projects assembled context into the Codex turn                     |
| Compaction                  | Bot or selected context engine            | Codex-native compaction, with Bot notifications and mirror maintenance |
| Channel delivery            | Bot                                       | Bot                                                                    |

Design rule: if Bot owns the surface, it can provide normal plugin hook
behavior. If the native runtime owns the surface, Bot needs runtime
events or native hooks. If the native runtime owns canonical thread state,
Bot mirrors and projects context rather than rewriting unsupported
internals.

## Runtime selection

Bot resolves an embedded runtime after provider and model resolution, in
this order:
...
4. If nothing claims the turn in `auto` mode, Bot falls back to
   `bot` as the compatibility runtime. Use an explicit runtime id when
   the run must be strict.

Whole-session and whole-agent runtime pins are ignored: `BOT_AGENT_RUNTIME`,
session `agentHarnessId`/`agentRuntimeOverride` state, `agents.defaults.agentRuntime`,
and `agents.entries.*.agentRuntime`. Run `bot doctor --fix` to remove stale
...
never silently routed back to Bot. Only `auto` may route an unmatched
turn to Bot.
...
harness. Explicit Bot runtime config remains an opt-in compatibility
route for `openai/*` agent turns; when paired with a selected `openai` OAuth
profile, Bot routes that path internally through the Codex-auth
...
If `bot doctor` warns that the `codex` plugin is enabled while legacy
Codex model refs remain in config, treat that as legacy route state and run
`bot doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@hanzo/bot-copilot` plugin registers an opt-in `copilot` runtime
...
prefix in `extensions/copilot/doctor-contract-api.ts`, which `bot doctor`
...
When a runtime is not Bot, its docs should state which Bot surfaces
it supports:

| Question                               | Why it matters                                                                                    |
| -------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Who owns the model loop?               | Determines where retries, tool continuation, and final answer decisions happen.                   |
| Who owns canonical thread history?     | Determines whether Bot can edit history or only mirror it.                                   |
| Do Bot dynamic tools work?        | Messaging, sessions, cron, and Bot-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Bot-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume Bot equivalence where the native runtime owns more state.            |
