...
Bot ships one **embedded agent runtime**: a built-in agent loop, tool
...
Recommended: use `bot setup` to create `~/.hanzoai/bot.json` if missing and initialize the workspace files.
...
Inside the workspace, Bot expects these user-editable files:
...
On the first turn of a new session, Bot injects the contents of these files into the system prompt's Project Context. `MEMORY.md` is only injected when it exists at the workspace root.

Blank files are skipped. Large files are trimmed and truncated with a marker so prompts stay lean (read the file for full content). A missing file (other than `MEMORY.md`) injects a single "missing file" marker line instead; `bot setup` creates a safe default template for it.

`BOOTSTRAP.md` is only created for a **brand new workspace** (no other bootstrap files present). While it is pending, Bot keeps it in Project Context and adds system-prompt bootstrap guidance for the initial ritual instead of copying it into the user message. If you delete it after completing the ritual, it is not recreated on later restarts.

After a workspace has been observed, Bot stores its setup state and
attestation in the shared SQLite database at
`~/.bot/state/bot.sqlite`. If a recently attested workspace
...
not read those files. Run `bot doctor --fix` to validate them, import their
...
Bot loads skills from these locations (highest precedence first):
...
The embedded agent runtime is Bot-owned: model discovery, tool wiring,
...
`~/.bot/agents/<agentId>/sessions/` as legacy migration inputs, deleted or
...
Bot. Bot does not read session folders from other tools.
...
- If you omit the provider, Bot tries an alias first, then a unique
  ...
  configured default model, Bot falls back to the first configured
