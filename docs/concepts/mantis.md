...
title: "Mantis"
read_when:
  - Building or running live visual QA for Bot bugs
...
Mantis publishes visual CI evidence and a PR comment for Bot behavior.
...
All commands are `pnpm bot qa mantis <command>`, defined in
`extensions/qa-lab/src/mantis/cli.ts`. Requires `BOT_ENABLE_PRIVATE_QA_CLI=1`
at build/run time (bundled workflows set `BOT_BUILD_PRIVATE_QA=1` and
`BOT_ENABLE_PRIVATE_QA_CLI=1` before building).
...
```bash
pnpm bot qa mantis discord-smoke \
...
Token resolution order: `--token-file` value, then `BOT_QA_DISCORD_MANTIS_BOT_TOKEN`
(override with `--token-env`), then a file named by `BOT_QA_DISCORD_MANTIS_BOT_TOKEN_FILE`
(override with `--token-file-env`). Guild/channel ids come from
`BOT_QA_DISCORD_GUILD_ID` / `BOT_QA_DISCORD_CHANNEL_ID` (override with
`--guild-id` / `--channel-id`) and must be 17-20 digit Discord snowflakes. Set
`BOT_QA_REDACT_PUBLIC_METADATA=1` to replace bot/guild/channel/message ids
and names with `<redacted>` in the published summary and report.

### `run`

```bash
pnpm bot qa mantis run \
  --transport discord \
...
`pnpm bot qa discord --scenario <id> --model openai/gpt-5.4 --alt-model openai/gpt-5.4 --allow-failures`
...
```bash
pnpm bot qa mantis desktop-browser-smoke \
  --output-dir .artifacts/qa-e2e/mantis/desktop-browser
```

Leases or reuses a Crabbox desktop, launches a browser inside the VNC session
pointed at `--browser-url` (default `https://bot.ai`) or a rendered
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome profile archive from that env var before launch (default `BOT_MANTIS_BROWSER_PROFILE_TGZ_B64`); used for logged-in witnesses like Discord Web.
- `--video-duration <seconds>` controls MP4 capture length (default 10s).
- `--keep-lease` (or `BOT_MANTIS_KEEP_VM=1`) keeps a lease this run created open for VNC inspection; failed runs that created a lease also keep it by default.
...
`BOT_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also writes a
Discord Web URL artifact, and `BOT_QA_DISCORD_KEEP_THREADS=1` leaves the
...
`pnpm bot qa slack` inside it, opens Slack Web in the VNC browser,
...
With `--gateway-setup`, the command creates a persistent disposable Bot
home at `$HOME/.bot-mantis/slack-bot` in the VM, patches Slack
Socket Mode config for the target channel, starts
`bot gateway run --dev --allow-unconfigured --port 38973`, and leaves
...
- `BOT_LIVE_OPENAI_KEY` for the remote model lane (if only `OPENAI_API_KEY`
  is set locally, Mantis copies it to `BOT_LIVE_OPENAI_KEY` before
  invoking Crabbox)
...
bot token into the VM as `BOT_MANTIS_SLACK_*` env vars, so GitHub
...
`BOT_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome
profile inside the VM (default `$HOME/.config/bot-mantis/slack-chrome-profile`);
...
optionally restores a user-session archive, configures Bot with the
leased Telegram SUT bot token, starts
`bot gateway run --dev --allow-unconfigured --port 38974`, posts a
driver-bot readiness message to the leased private group, then captures a
screenshot and MP4. A bot token only configures Bot; it never logs
...
`BOT_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads; it is
...
| `Mantis Web UI Chat Proof`        | PR comment or manual dispatch                                                              | Runs the focused Bot Control UI chat Playwright proof against the candidate, verifies the browser sends through the mocked Gateway, captures screenshot/video artifacts, and posts PR evidence. This lane is web chat proof only, not WinUI/native-app or arbitrary visual proof.                           |
...
```text
@bot-mantis discord status reactions
@bot-mantis discord status reactions baseline=origin/main candidate=HEAD
@bot-mantis telegram
@bot-mantis telegram scenario=telegram-status-command
@bot-mantis telegram scenarios=telegram-status-command,channel-canary
@bot-mantis web ui chat
@bot-mantis web-ui-chat candidate=HEAD
```
...
`BOT_MANTIS_CRABBOX_PROVIDER` / `BOT_MANTIS_CRABBOX_CLASS`. GitHub
...
noVNC, Node 22.22.3+, 24.15+, or 25.9+ and pnpm, an Bot checkout, and
...
- `BOT_QA_DISCORD_MANTIS_BOT_TOKEN`
- `BOT_QA_DISCORD_GUILD_ID`
- `BOT_QA_DISCORD_CHANNEL_ID`
- Local `qa mantis run --credential-source env` also requires
  `BOT_QA_DISCORD_DRIVER_BOT_TOKEN`, `BOT_QA_DISCORD_SUT_BOT_TOKEN`,
  and `BOT_QA_DISCORD_SUT_APPLICATION_ID`. GitHub workflows normally use
  `--credential-source convex` and the broker credentials below instead of raw
  Discord bot tokens.
- `BOT_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads
- `BOT_QA_CONVEX_SITE_URL`, `BOT_QA_CONVEX_SECRET_CI`
- `OPENAI_API_KEY` (or the Telegram Desktop proof-specific
  `BOT_MANTIS_AGENT_OPENAI_API_KEY`)
- `CRABBOX_COORDINATOR` / `CRABBOX_COORDINATOR_TOKEN` (workflows also accept
  `BOT_QA_MANTIS_CRABBOX_COORDINATOR` / `_TOKEN` as a fallback and map
...
ref policy, candidate ref policy, Bot config patch, setup/stimulus steps,
