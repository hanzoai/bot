---
summary: "How Bot resolves provider/model refs, config keys, and the `/model` chat command"
read_when:
...
<CardGroup cols={2}>
  ...
  <Card title="Models CLI reference" href="/cli/models">
    Full `bot models` command and flag reference.
  </Card>
...
endpoints, and authored request behavior stay on Bot. Plaintext official
...
<Steps>
  ...
  <Step title="Auth failover">
    Auth-profile rotation happens inside a provider before Bot moves to the next fallback model.
  </Step>
</Steps>
...
| Auto fallback                                                           | Temporary recovery state, stored as `modelOverrideSource: "auto"`. Bot periodically reprobes the original primary, clears the auto selection on recovery, and announces fallback/recovery transitions once per state change.                              |
...
`openai/gpt-5.5` explicitly; Bot does not silently downgrade it.
...
```bash
bot config set agents.defaults.modelPolicy.allow '["openai/gpt-5.4","anthropic/*"]' --strict-json
```

`bot models set`, provider setup, and `bot models aliases add` can add entries under `agents.defaults.models`, but they never change `modelPolicy.allow`. This keeps model metadata and aliases independent from override policy.
</Accordion>
...
bot models scan
bot models aliases list|add|remove
bot models fallbacks list|add|remove|clear
bot models image-fallbacks list|add|remove|clear
bot models auth list|add|login|paste-api-key|paste-token|setup-token|order
```

`bot models` with no subcommand is a shortcut for `models status`, which also surfaces OAuth expiry for auth-store profiles (warns within 24h by default). Full flags, JSON shapes, and auth-profile subcommands: [Models CLI reference](/cli/models).

<AccordionGroup>
  <Accordion title="Scanning (OpenRouter free models)">
    `bot models scan` inspects OpenRouter's public free-model catalog and can probe candidates for tool and image support live. The catalog itself is public, so metadata-only scans (`--no-probe`) need no key; live probing and `--set-default`/`--set-image` require an OpenRouter API key (auth profile or `OPENROUTER_API_KEY`) and fail closed to metadata-only output without one.
...
Bot can refresh the model metadata shipped by installed provider plugins
without waiting for a new Bot release. The Gateway makes one background
...
[`bot/catalog`](https://github.com/bot/catalog) GitHub repository.
Its scheduled workflow refreshes from Bot's shipped plugin manifests and
pricing sources; every catalog content change is preserved as a public commit.

Run `bot models refresh` for an immediate metadata and pricing check, or
...
Marker persistence is source-authoritative: Bot writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values, whenever it regenerates `models.json` — including command-driven paths like `bot agent`.

## Related

- [Agent runtimes](/concepts/agent-runtimes) — Bot, Codex, and other agent loop runtimes
