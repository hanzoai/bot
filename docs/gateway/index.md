...
- Replaces the legacy `gateway` command. CLI entry point: `hanzo-bot gateway`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `BOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
...
- macOS: `bot.molt.<profile>` (legacy `com.bot.*` may still exist)
...
- `health` — full health snapshot (same shape as `hanzo-bot health --json`).
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `hanzo-bot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.bot.*` is cleaned up).
  - `hanzo-bot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Hanzo Bot profile services.
  ...
  - Cleanup: `hanzo-bot gateway uninstall` (current service) and `hanzo-bot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `hanzo-bot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Hanzo Bot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.bot.*` labels still unload cleanly).
- To stop it cleanly, use `hanzo-bot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `hanzo-bot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `hanzo-bot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Hanzo Bot installs a **systemd user service** by default on Linux/WSL2. We
...
`hanzo-bot gateway install` writes the user unit. `hanzo-bot doctor` audits the
...
- `hanzo-bot gateway health|status` — request health/status over the Gateway WS.
- `hanzo-bot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `hanzo-bot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `hanzo-bot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `hanzo-bot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `hanzo-bot gateway` and the legacy TCP control port.
