---
summary: "Use OpenShell as a managed sandbox backend for Bot agents"
title: OpenShell
read_when:
...
locally, Bot delegates sandbox lifecycle to the `openshell` CLI, which
...
- OpenShell plugin installed (`bot plugins install @hanzo/bot-openshell-sandbox`)
...
- Bot Gateway running on the host
...
Restart the Gateway. On the next agent turn Bot creates an OpenShell
...
- Before `exec`, Bot syncs the local workspace into the sandbox.
- After `exec`, Bot syncs the remote workspace back to local.
- File tools go through the sandbox bridge, but local stays source of truth
  between turns.

Best for development workflows: local edits outside Bot show up on the
...
- On first sandbox creation, Bot seeds the remote workspace from local
  once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. Bot does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside Bot after the initial seed is invisible to the remote sandbox. Run `bot sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"bot"`  | Sandbox source for first-time create                                                   |
...
it cannot select different OpenShell workspaces per Bot agent or session.
Changing it does not migrate existing sandboxes. Delete Bot's OpenShell
...
Bot keeps a registered sandbox's shipped legacy runtime name after an
...
upgrade. Bot preserves the registered runtime identity, refuses to create
a replacement implicitly, and reports the scoped `bot sandbox recreate`
...
Bot does not apply Docker image, root-filesystem, network, user, or package
settings to this backend.

Custom images used with the Bot filesystem bridge must provide:
...
1. Bot runs `sandbox get` for the sandbox name (with the selected
...
2. Bot runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
...
- [Sandbox CLI](/cli/sandbox) - `bot sandbox` commands
