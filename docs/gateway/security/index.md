...
<Warning>
  **Personal assistant trust model.** This guidance assumes one trusted
  ...
  Bot is not a hostile multi-tenant security boundary for multiple
...
```bash
bot security audit
bot security audit --deep    # attempts a live Gateway probe
bot security audit --fix     # apply safe remediations
bot security audit --json
```
...
Most failures here are not exotic exploits - they are "someone messaged the bot and the bot did what they asked." Bot's stance, in order:
...
By default, Bot routes all DMs into the main session for cross-device continuity. If multiple people can DM the bot (open DMs or a multi-person allowlist), isolate DM sessions:
...
- `gateway` reads config with `config.schema.lookup` / `config.get`. It cannot write config, update Bot, or restart the Gateway.
- `cron` creates scheduled jobs that keep running after the original chat/task ends.

The `gateway` tool stays owner-only because config reads can expose secrets and host topology. Agents request persistent config or lifecycle changes through the `bot` delegation tool; Bot maps them to typed operations and requires human approval before applying them. See [Bot setup agent](/cli/bot#operations-and-approval).
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Bot cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
...
Bot can refresh the skills list mid-session: the skills watcher updates the snapshot on the next agent turn when `SKILL.md` changes, and connecting a macOS node can make macOS-only skills eligible (based on bin probing). Treat skill folders as trusted code and restrict who can modify them.
...
- Installing/updating plugins runs executable code:
  ...
  - ClawHub packages and Bot's bundled/official catalog are trusted sources. A new arbitrary npm, `npm-pack:`, git, local path/archive, or marketplace source warns before install; noninteractive installs require `--force` after you review and trust that source. `--force` confirms provenance and permits overwrite; it does not bypass `security.installPolicy` or remaining install safety checks. Updates reuse the already selected source.
  - Bot does not run built-in local dangerous-code blocking during install/update. Use `security.installPolicy` for operator-owned local allow/block decisions and `bot security audit --deep` for diagnostic scanning.
  - npm and git plugin installs run package-manager dependency convergence only during the explicit install/update flow. Local paths and archives are treated as self-contained packages; Bot copies/references them without running `npm install`.
...
- `"none"` (default): tools see a sandbox workspace under `~/.bot/sandboxes`; agent workspace is off-limits.
...
- Keep filesystem roots narrow - avoid broad roots like your home directory for agent/sandbox workspaces, which can expose sensitive local files (for example state/config under `~/.bot`) to filesystem tools.
...
```json5
{
  agents: {
    list: [
      { id: "personal", workspace: "~/.bot/workspace-personal", sandbox: { mode: "off" } },
    ],
  },
}
```
...
- Prefer a dedicated profile for the agent (the default `bot` profile); avoid your personal daily-driver profile.
...
The Gateway multiplexes WebSocket + HTTP on one port (default `18789`; config/flags/env: `gateway.port`, `--port`, `BOT_GATEWAY_PORT`). That HTTP surface includes the Control UI (SPA assets, default base path `/`) and the canvas host (`/__bot__/canvas` and `/__bot__/a2ui` - arbitrary HTML/JS; treat as untrusted content when loaded in a normal browser; do not expose it to untrusted networks/users or share an origin with privileged web surfaces).
...
When the bundled `bonjour` plugin is enabled, the Gateway broadcasts presence via mDNS (`_bot-gw._tcp`, port 5353) for local device discovery. Full mode includes TXT records that expose operational details: `cliPath` (filesystem path revealing username and install location), `sshPort` (advertises SSH availability), `displayName`/`lanHost` (hostname info). Broadcasting infrastructure details makes LAN reconnaissance easier.
...
- Or set `BOT_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
`bot doctor --generate-gateway-token` can generate one for you.
...
- `"password"`: prefer setting via `BOT_GATEWAY_PASSWORD`.
...
- Bot's gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS there.
- If the gateway itself terminates HTTPS, `gateway.http.securityHeaders.strictTransportSecurity` emits the HSTS header from Bot responses.
...
- Published package dependency lock: source checkouts use `pnpm-lock.yaml`; the published `bot` npm package and Bot-owned npm plugin packages include `npm-shrinkwrap.json` so installs use the reviewed transitive dependency graph from the release instead of resolving a fresh graph at install time. This is a supply-chain hardening and release reproducibility boundary, not a sandbox - see [npm shrinkwrap](/gateway/security/shrinkwrap).
- Secure file operations: Bot uses `@openclaw/fs-safe` for root-bounded file access, atomic writes, archive extraction, temp workspaces, and secret-file helpers. Optional native acceleration defaults **off**; set `BOT_FS_SAFE_NATIVE_MODE=auto` to use an installed platform binding or `require` to fail closed when native support is unavailable. Details: [Secure file operations](/gateway/security/secure-file-operations).
...
| `state/bot.sqlite`                        | Shared runtime state, including native MCP OAuth access/refresh tokens, dynamic client registration secrets, and discovery state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `agents/<agentId>/agent/bot-agent.sqlite` | Per-agent runtime state, including model auth profiles.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `$CODEX_HOME/**` or `~/.codex/**`              | Native Codex runtime state. The ordinary harness accesses it only with explicit `plugins.entries.codex.config.appServer.homeScope: "user"`. The separate supervision connection accesses it when its resolved home scope is `"user"`, which is the default for stdio or Unix when unset. Contains the native Codex account, config, plugins, and thread store. Supervision lists source metadata and keeps a continued Chat's canonical native branch and later turns on that connection; branching copies bounded persisted user and assistant history into an authenticated, model-locked Bot Chat. Enable only for an owner-controlled Gateway. See [Codex harness](/plugins/codex-harness#share-threads-with-codex-desktop-and-cli) and [Codex supervision](/plugins/codex-supervision). |
| `secrets.json` (optional)                      | File-backed secret payload used by `file` SecretRef providers (`secrets.providers`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `agents/<agentId>/agent/auth.json`             | Legacy compatibility file; static `api_key` entries are scrubbed when discovered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `agents/<agentId>/agent/bot-agent.sqlite` | Per-agent runtime state, including session rows and transcripts that can contain private messages and tool output.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- Pairing allowlists: `~/.bot/credentials/<channel>-allowFrom.json` (default account) / `<channel>-<accountId>-allowFrom.json` (non-default accounts)
- Model auth profiles: `~/.bot/agents/<agentId>/agent/bot-agent.sqlite` (`auth_profile_store`)
- MCP OAuth sessions: `~/.bot/state/bot.sqlite` (`mcp_oauth_stores`)
...
`bot doctor` can warn and offer to tighten these.

### Workspace `.env` files

Bot loads workspace-local `.env` files for agents and tools, but never lets them silently override gateway runtime controls:

- Provider credential environment variables are blocked from untrusted workspace `.env` files - for example `GEMINI_API_KEY`, `GOOGLE_API_KEY`, `XAI_API_KEY`, `MISTRAL_API_KEY`, `GROQ_API_KEY`, `DEEPSEEK_API_KEY`, `PERPLEXITY_API_KEY`, `BRAVE_API_KEY`, `TAVILY_API_KEY`, `EXA_API_KEY`, `FIRECRAWL_API_KEY`, and provider auth keys declared by installed trusted plugins. Put provider credentials in the Gateway process environment, `~/.bot/.env` (`$BOT_STATE_DIR/.env`), the config `env` block, or an optional login-shell import instead.
- Any key starting with `BOT_` is blocked from untrusted workspace `.env` files, reserving the whole runtime namespace so a future `BOT_*` control is fail-closed by default rather than silently inheritable from checked-in or attacker-supplied `.env` content.
...
Bot stores session transcripts on disk under `~/.bot/agents/<agentId>/sessions/*.jsonl` for session continuity and optional memory indexing - any process/user with filesystem access can read them. Treat disk access as the trust boundary and lock down `~/.bot` permissions; run agents under separate OS users or hosts for stronger isolation.
...
1. Check Gateway logs with `bot logs` (or `bot --profile <profile> logs` for a named profile). The default path is `/tmp/hanzoai/bot-YYYY-MM-DD.log`; named profiles use `/tmp/hanzoai/bot-<profile>-YYYY-MM-DD.log`, unless `logging.file` overrides it.
...
4. Re-run `bot security audit --deep` and confirm critical findings are resolved.
...
Found a vulnerability in Bot? Report responsibly:

1. Email: [security@bot.ai](mailto:security@bot.ai)
