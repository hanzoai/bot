---
summary: "Pre-flight and rollback checklist before exposing an Bot Gateway beyond loopback"
...
- Gateway host, OS user, and state directory (default `~/.bot`).
...
- Backup location for `~/.hanzoai/bot.json` and credentials.
...
```bash
bot gateway probe --url ws://127.0.0.1:18789 --token "$BOT_GATEWAY_TOKEN"
```
...
Run `bot security audit --deep` after proxy changes. Trusted-proxy
...
1. Re-run `bot security audit --deep`.
...
5. Re-run `bot security audit --deep`.
