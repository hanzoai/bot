...
Bot. Operator and node clients (CLI, web UI, macOS app, iOS/Android nodes,
headless nodes) connect over WebSocket and declare a **role** and **scope** at
handshake time.

## npm packages

These packages ship with Bot release trains. During the initial rollout,
npm may return `E404` until the first package-bearing release is published.

- [`@hanzo/bot-gateway-protocol`](https://www.npmjs.com/package/@hanzo/bot-gateway-protocol)
  ...
  [`protocol.schema.json`](https://unpkg.com/@hanzo/bot-gateway-protocol@beta/protocol.schema.json)
  machine-readable contract.
- [`@hanzo/bot-gateway-client`](https://www.npmjs.com/package/@hanzo/bot-gateway-client)
  publishes the reference Node client and a browser-safe entry at
  `@hanzo/bot-gateway-client/browser`.

For application lifecycle guidance, see
[Building a Gateway client](https://docs.bot.ai/gateway/clients). For apps
that supervise the Gateway as a child process, see
[Embedding Bot](https://docs.bot.ai/gateway/embedding).
...
`@hanzo/bot-gateway-protocol/gateway-error-details`; the browser-safe gateway client
re-exports them from `@hanzo/bot-gateway-client/browser`.

The schemas are exported as `GatewayErrorDetailsSchema`,
`MissingScopeErrorDetailsSchema` from `@hanzo/bot-gateway-protocol/schema`.
...
<AccordionGroup>
  ...
  <Accordion title="Talk and TTS">
    ...
    - `talk.client.toolCall` lets client-owned realtime transports forward provider tool calls to gateway policy. The first supported tool is `bot_agent_consult`; clients get a run id and wait for normal chat lifecycle events before submitting the provider-specific tool result. Voice-bound high-impact actions return `VOICE_CONFIRMATION_REQUIRED:<id>` until a later finalized user utterance explicitly confirms that exact final execution action and the next consult supplies the `confirmationId`; policy or hook rewrites require confirmation again.
...
Use [`bot audit`](/cli/audit) for text queries and bounded JSON exports.
...
- `skills.install` (`operator.admin`) has three modes:
  ...
  - Gateway installer mode: `{ name, installId, timeoutMs? }` runs a declared
    `metadata.bot.install` action on the gateway host. Older clients may
...
- Final `agent` results may include `result.deliveryStatus` when delivery was
  ...
  [`bot agent --json --deliver`](/cli/agent#json-delivery-status).
...
(Bot wraps it via the thin `src/gateway/client.ts` facade). These
...
| Preauth / connect-challenge timeout       | `15_000` ms                                           | `packages/gateway-client/src/timeouts.ts` (`BOT_HANDSHAKE_TIMEOUT_MS` env can raise the paired server/client budget) |
...
- Bot also has a narrow backend/container-local self-connect path for
...
- Omitting device identity has scope consequences. When a device-less
  operator connection is allowed through an explicit trust path, Bot
