...
| `group:bot`   | All built-in tools above except `read`/`write`/`edit`/`apply_patch`/`exec`/`process`/`canvas` (excludes plugin tools)                                                                                                                                  |
...
- `bundle-mcp` for Bot-managed MCP servers from `mcp.servers`
...
Without that sandbox-layer entry, the MCP server can still load successfully while its tools are filtered before the provider request. Use `bot doctor` to catch this shape for Bot-managed servers in `mcp.servers`. MCP servers loaded from bundled plugin manifests or Claude `.mcp.json` use the same sandbox gate, but this diagnostic does not enumerate those sources yet; use the same allowlist entries if their tools disappear in sandboxed turns.

### `tools.codeMode`

`tools.codeMode` enables the generic Bot code-mode surface. When enabled
for a run with tools, normal Bot tools move behind the in-sandbox `tools.*`
...
Keys use explicit prefixes: `channel:<channelId>:<senderId>`, `id:<senderId>`, `e164:<phone>`, `username:<handle>`, `name:<displayName>`, or `"*"`. Channel ids are canonical Bot ids; aliases such as `teams` normalize to `msteams`. Legacy unprefixed keys are accepted as `id:` only. Matching order is channel+id, id, e164, username, name, then wildcard.
...
<AccordionGroup>
  <Accordion title="Attachment notes">
    ...
    - Subagent attachments are materialized into the child workspace at `.bot/attachments/<uuid>/` with a `.manifest.json`.
...
Older configs used `tools.experimental.planTool`. Run `bot doctor --fix` to move the value to `tools.updatePlan`.
...
Provider plugins publish their own model catalog rows. Add custom providers via `models.providers` in config or `~/.bot/agents/<agentId>/agent/models.json`.

Configuring a custom/local provider `baseUrl` is also the narrow network trust decision for model HTTP requests: Bot allows that exact `scheme://host:port` origin through the guarded fetch path, without adding a separate config option or trusting other private origins.
...
<AccordionGroup>
  <Accordion title="Auth and merge precedence">
    ...
    - Override agent config root with `BOT_AGENT_DIR`.
...
<AccordionGroup>
  <Accordion title="Top-level catalog">
    ...
    - `models.providers`: custom provider map keyed by provider id.
      - Safe edits: use `bot config set models.providers.<id> '<json>' --strict-json --merge` or `bot config set models.providers.<id>.models '<json-array>' --strict-json --merge` for additive updates. `config set` refuses destructive replacements unless you pass `--replace`.
  ...
  <Accordion title="Model catalog entries">
    ...
    - `models.providers.*.models.*.contextTokens`: optional runtime context cap. This overrides provider-level `contextTokens`; use it when you want a smaller effective context budget than the model's native `contextWindow`; `bot models list` shows both values when they differ.

    #### Custom provider capability declarations

    Provider catalogs own `compat` for bundled and catalog-known model routes. Do not copy those flags into config: Bot uses the catalog row when the configured `api` and `baseUrl` still identify that route. `bot doctor --fix` removes matching legacy overrides and reports divergent values for review.
    ...
    | `reasoningEffortMap` | Maps Bot thinking labels to endpoint-specific labels. |
...
<AccordionGroup>
  <Accordion title="Cerebras (GLM 4.7 / GPT OSS)">
    The official external `cerebras` provider plugin can configure this via `bot onboard --auth-choice cerebras-api-key`. Use explicit provider config only when overriding defaults.
  ...
  <Accordion title="MiniMax M3 (direct)">
    ...
    Set `MINIMAX_API_KEY`. Shortcuts: `bot onboard --auth-choice minimax-global-api` or `bot onboard --auth-choice minimax-cn-api`. The model catalog defaults to M3 and also includes the M2.7 variants. On the Anthropic-compatible streaming path, Bot disables MiniMax M2.x thinking by default unless you explicitly set `thinking` yourself; MiniMax-M3 (and M3.x) stays on the provider's omitted/adaptive thinking path by default. `/fast on` or `params.fastMode: true` rewrites `MiniMax-M2.7` to `MiniMax-M2.7-highspeed`.
  ...
  <Accordion title="Moonshot AI (Kimi)">
    ...
    Native Moonshot endpoints advertise streaming usage compatibility on the shared `openai-completions` transport, and Bot keys that off endpoint capabilities rather than the built-in provider id alone.
  ...
  <Accordion title="OpenCode">
    ...
    Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `bot onboard --auth-choice opencode-zen` or `bot onboard --auth-choice opencode-go`.
  ...
  <Accordion title="Z.AI (GLM-4.7)">
    ...
    Set `ZAI_API_KEY`. Model refs use the canonical `zai/*` provider ID. Shortcut: `bot onboard --auth-choice zai-api-key`.
