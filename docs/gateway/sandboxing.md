...
Hanzo Bot can run **tools inside Docker containers** to reduce blast radius.
...
- Bot blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
