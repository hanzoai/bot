---
summary: "Use the 1Password plugin, bundled skill, or official MCP with Bot"
read_when:
  - You want API keys out of bot.json and inside 1Password
...
Bot pairs with **1Password** in four independent ways:

- **Config secrets:** any [SecretRef](/gateway/secrets) field in `bot.json` can resolve through the `op` CLI at runtime, so API keys never live in the config file.
...
- **Browser sign-in:** the `claude-cli` backend can use Claude Code's Chrome integration with [1Password for Claude](https://support.1password.com/1password-claude/), letting the agent sign in to websites without the password ever reaching the model or Bot.
...
```bash
bot plugins enable onepassword
mkdir -p ~/.bot/credentials/onepassword
chmod 700 ~/.bot/credentials/onepassword
printf '%s' "$OP_SERVICE_ACCOUNT_TOKEN" > \
  ~/.bot/credentials/onepassword/service-account-token
chmod 600 ~/.bot/credentials/onepassword/service-account-token
unset OP_SERVICE_ACCOUNT_TOKEN
```

When `BOT_STATE_DIR` is set, use that directory instead of
`~/.bot`. Then generate and apply a SecretRef plan:

```bash
bot onepassword secretref setup \
  --openai-id op://Automation/OpenAI/credential \
  --anthropic-id op://Automation/Anthropic/credential \
  --plan-out ./bot-1password-secrets-plan.json

bot onepassword secretref status
bot secrets apply --from ./bot-1password-secrets-plan.json --dry-run --allow-exec
bot secrets apply --from ./bot-1password-secrets-plan.json --allow-exec
...
registered Bot credential targets, bounds parallel `op read` calls, and
...
Bot bundles a `1password` skill that teaches agents to select an available
...
into a one-off command. When a credential is already wired to an Bot
SecretRef target, the owning Bot workflow should resolve it; the agent does
...
and the Bot plugin does not call it. If an MCP-managed Environment launches
Bot with variables already mounted, use Bot's `env` SecretRefs for
...
[1Password for Claude](https://support.1password.com/1password-claude/) lets Claude request a login while the 1Password browser extension fills the credential directly into the page over an encrypted channel. The secret never enters the model context, the transcript, or Bot. When Bot runs the [`claude-cli` backend](/gateway/cli-backends#claude-cli-specifics) with Claude Code's Chrome integration enabled, agent tasks can use that flow for websites that need a real signed-in session.
...
- A person at the gateway host: every credential use shows a 1Password prompt confirmed there (for example with Touch ID). Under a restrictive exec policy the browser tool calls themselves are also relayed to your channel as Bot approvals first.

Before wiring this into Bot, verify the pieces in an interactive session on the gateway host: run `claude --chrome`, confirm the extension connects, and check that the `claude-in-chrome` tools include the credential tools. If they do not appear there, they will not appear through Bot either.
...
- Never place secret values in `bot.json`, logs, or chat. Scope the service
  account to only the vaults and items Bot needs.
...
- Authentication fails: check the plugin token file, its contents, and the
  service account's vault permissions with `bot onepassword status`.
