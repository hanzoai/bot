...
- `bot status --deep` - asks the running gateway for a live probe (`health` with `probe:true`), including per-account channel probes when supported.
- `bot status --usage` - show model provider usage/quota snapshots.
- `bot health` - asks the running gateway for its health snapshot (WS-only; no direct channel sockets from the CLI).
- `bot health --verbose` (alias `--debug`) - forces a live health probe and prints gateway connection details.
- `bot health --json` - machine-readable health snapshot output.
- Send `/status` as a standalone chat command in any channel to get a status reply without invoking the agent.
- Logs: run `bot logs --follow` (or `bot --profile <profile> logs --follow`) and filter for `web-heartbeat`, `web-reconnect`, `web-auto-reply`, `web-inbound`.

For Discord and other chat providers, session rows are not socket liveness.
`bot sessions`, Gateway `sessions.list`, and the agent `sessions_list` tool
...
- Session store: `ls -l ~/.bot/agents/<agentId>/agent/bot-agent.sqlite`. Count and recent recipients are surfaced via `status`.
- Relink flow: `bot channels logout && bot channels login --verbose` when status codes 409-515 or `loggedOut` appear in logs. The QR login flow auto-restarts once for status 515 after pairing.
...
- The same heartbeat drives the bounded stability recorder: `bot gateway stability` (or the `diagnostics.stability` Gateway RPC). Fatal Gateway exits, shutdown timeouts, and restart startup failures persist the latest snapshot under `~/.bot/logs/stability/`. Inspect the newest bundle with `bot gateway stability --bundle latest`.
- For bug reports, run `bot gateway diagnostics export` and attach the generated zip: a Markdown summary, the newest stability bundle, sanitized log metadata, sanitized Gateway status/health snapshots, and config shape. Chat text, webhook bodies, tool outputs, credentials, cookies, account/message identifiers, and secret values are omitted or redacted. See [Diagnostics Export](/gateway/diagnostics).
...
- When a channel cannot open its durable ingress queue, its start fails and the gateway records the account as unable to receive. `bot channels status` reports `Channel cannot admit inbound events; its durable ingress queue is unavailable. Outbound may still work.`
...
When no `x-bot-session-key` header or `user` field is provided, `/v1/chat/completions` generates a new random session for each request. Monitoring services that ping every 15 minutes create ~96 sessions/day, each consuming 4-22KB. Over time this causes session store bloat and can lead to context window overflow.
...
`bot health` asks the running gateway for its health snapshot (no direct channel
