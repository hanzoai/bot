---
summary: "Gateway config reference for core Bot keys, defaults, and links to dedicated subsystem references"
...
Field-level reference for `~/.hanzoai/bot.json`: keys, defaults, and links to deeper subsystem pages. For task-oriented setup guidance, see [Configuration](/gateway/configuration). Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages, not here.
...
- `bot config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full Bot agent run behind Control UI Talk realtime consults
  ...
  - `talk.realtime.consultRouting`: Gateway relay fallback for finalized realtime Talk transcripts that skip `bot_agent_consult`
...
```json5
{
  models: {
    ...
    catalogRefresh: {
      enabled: true,
      // url: "https://catalog.example.com/bot/catalog.json",
    },
  },
}
```
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. Bot probes the configured health endpoint, starts
...
retired `models.pricing` toggle is removed automatically by `bot doctor
--fix`; use `models.catalogRefresh.enabled: false` when Bot must avoid all
hosted catalog traffic.

## MCP

Bot-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded Bot and other runtime adapters. The `bot mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `bot mcp set` and
  `bot doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.enabled`: set `false` to keep a saved server definition
  while excluding it from embedded Bot MCP discovery and tool projection.
...
- `mcp.servers.<name>.auth`: set `"oauth"` for HTTP MCP servers that require
  OAuth. Run `bot mcp login <name>` to store tokens under Bot state.
...
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is Bot metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed Bot agent ids.
  ...
  `default_tools_approval_mode` for that server. Bot strips the `codex`
...
See [MCP](/cli/mcp#bot-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.bot.install`
...
- Loaded from package or bundle directories under `~/.bot/extensions` and `<workspace>/.bot/extensions`, plus files or directories listed in `plugins.load.paths`.
...
- Discovery accepts native Bot plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Bot plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central Bot option registry.
...
It does not enable Codex plugins for Bot provider runs, ACP
...
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  ...
  to decline them, `"auto"` to route Codex-required approvals through Bot
...
be installed and enabled, and their owned apps must be accessible; Bot
...
- Enabled Claude bundle plugins can also contribute embedded Bot defaults from `settings.json`; Bot applies those as sanitized agent settings, not as raw Bot config patches.
...
- Host-local opens with a stable native CDP target and browser identity are
  ...
  post-restart activity. Bot verifies the profile and browser instance
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Bot to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise Bot treats the loopback port as a
...
- `existing-session` profiles can set `cdpUrl` when Chrome is already running
  ...
  mode Bot passes the endpoint to Chrome MCP instead of using auto-connect;
...
- Local managed `bot` profiles auto-assign `cdpPort` and `cdpUrl`; set
...
- `tailscale.serviceName`: optional Tailscale Service name for Serve mode, such
  as `svc:bot`. When set, Bot passes it to `tailscale serve
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, Bot
...
- `terminal.shell`: optional shell executable. When unset, Bot uses `$SHELL` on Unix and `%ComSpec%` on Windows.
...
- `BOT_HANDSHAKE_TIMEOUT_MS`: optional environment override for the built-in pre-auth Gateway WebSocket handshake timeout.
...
- `gateway.nodes.pairing.sshVerify`: SSH-verified auto-approval for first-time node device pairing (default: enabled). The gateway SSHes back to the pairing host (BatchMode, strict host keys) and approves only on an exact `bot node identity` device-key match. Same eligibility floor as `autoApproveCidrs`; probes are limited to private/CGNAT source addresses unless `cidrs` overrides them. Set `false` to disable, or `{ user, identity, timeoutMs, cidrs }` to tune. See [Node pairing](/gateway/pairing#ssh-verified-device-auto-approval-default).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/bot/tls/server.crt",
      keyPath: "/etc/bot/tls/server.key",
      caPath: "/etc/bot/tls/ca-bundle.crt",
    },
  },
}
```
...
Cloud workers are opt-in. If `cloudWorkers` is absent, or `profiles` is empty, Bot accepts no new worker creation. Durable records created earlier still reconcile and remain visible; the existing gateway/node projection is unchanged.
...
This foundation installs and verifies the gateway build and provides tunnel start/stop lifecycle, but it does not launch the general Bot CLI. The self-contained worker entry and loop land in the next cloud-worker milestone.
...
<Warning>
  ...
  Its operator must supply the expected `hostKey`; Bot will not learn or accept a key from the first connection.
  Destroying its lease only releases Bot's logical record; it does not stop or clean the host.
</Warning>
...
- `hooks.token` should be distinct from active Gateway shared-secret auth (`gateway.auth.token` / `BOT_GATEWAY_TOKEN` or `gateway.auth.password` / `BOT_GATEWAY_PASSWORD`); startup logs a non-fatal security warning when it detects reuse.
- `bot security audit` flags hook/Gateway auth reuse as a critical finding, including Gateway password auth supplied only at audit time (`--auth password --password <password>`). Run `bot doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new hook token.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.bot/hooks/transforms`; workspace skill directories are rejected. If `bot doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `bot`. Override with `BOT_MDNS_HOSTNAME`.
- `BOT_DISABLE_BONJOUR=1` disables mDNS advertising outright, overriding `discovery.mdns.mode`.
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `bot doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
Run [`bot doctor --fix`](/cli/doctor) to move it to `logging.audit`.
...
- Default log file: `/tmp/hanzoai/bot-YYYY-MM-DD.log`; named profiles use `/tmp/hanzoai/bot-<profile>-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `consoleStyle`: `"pretty"` or `"json"`. The earlier `"compact"` value is retired; [`bot doctor --fix`](/cli/doctor) maps it to `"pretty"`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). Bot keeps up to five numbered archives beside the active file.
...
- Redaction is always on and is no longer configurable. [`bot doctor --fix`](/cli/doctor) removes the retired switch from older config files; the runtime always applies `tools`-mode redaction to logs and transcripts. UI, tool, and diagnostic safety surfaces redact secrets independently of this policy.
...
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: environment toggle for latest experimental GenAI inference span shape, including `{gen_ai.operation.name} {gen_ai.request.model}` span names, `CLIENT` span kind, and `gen_ai.provider.name` instead of legacy `gen_ai.system`. By default spans keep `bot.model.call` and `gen_ai.system` for compatibility; GenAI metrics use bounded semantic attributes.
- `BOT_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. Bot then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
when preserving announce delivery. `bot doctor --fix` strips a leftover
...
into it by [`bot doctor --fix`](/cli/doctor).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `bot.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary. Set `BOT_INCLUDE_ROOTS` (absolute paths) to allow additional roots outside the config directory.
...
- Bot-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `bot.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for Bot-owned writes; those writes fail closed instead of flattening the config.
