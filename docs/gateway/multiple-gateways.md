---
summary: "Run multiple Bot Gateways on one host (isolation, ports, and profiles)"
read_when:
...
bot --profile rescue gateway install --port 19789
```
...
During `bot --profile rescue onboard`:
...
- Profile/config file
- State directory
- Workspace (default: `~/.bot/workspace-rescue`)
...
# extra gateway
bot --profile ops setup
bot --profile ops gateway --port 19789
```
...
```bash
bot gateway install
bot --profile ops gateway install --port 19789
```
...
`BOT_ALLOW_MULTI_GATEWAY=1` skips the per-config singleton.
...
bot --profile rescue gateway status --deep
bot --profile rescue gateway probe
bot status
