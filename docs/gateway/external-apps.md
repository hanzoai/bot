...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to Bot
...
External apps talk to Bot through the Gateway protocol: WebSocket
...
<Note>
  ...
  [Building a Gateway client](https://docs.bot.ai/gateway/clients). If your
  app supervises the Gateway as a child process, also read
  [Embedding Bot](https://docs.bot.ai/gateway/embedding). During the
  initial package rollout, npm may return `E404` until the first package-bearing
  Bot release is published.
</Note>

<Note>
  This page is for code outside the Bot process. Plugin code that runs
  inside Bot should use documented `bot/plugin-sdk/*` subpaths instead.
</Note>
...
| ---------------------------------------------------------------- | ------------- | --------------------------------------------------------------------------------------------- |
| [Gateway client guide](https://docs.bot.ai/gateway/clients) | Release train | npm packages, auth, reconnect, history, events, approvals, and version policy.                |
| [Embedding guide](https://docs.bot.ai/gateway/embedding)    | Release train | Child-process environment, readiness, lifecycle, recovery, RPC ownership, and packaging.      |
...
| [`bot agent`](/cli/agent)                                   | Ready         | One-shot script integration when shelling out to the CLI is enough.                           |
| [`bot message`](/cli/message)                               | Ready         | Sending messages or channel actions from scripts.                                             |
...
4. Pin the Bot version you test against.
5. Recheck the RPC reference when upgrading Bot.
...
fail-closed, and the host must not freeze or snapshot it. Bot retries the
...
Preparation is refuse-only: Bot closes new root/session/command admission,
...
<Tip>
  For host wake scheduling, keep the Bot-facing part in an in-process
...
Use Gateway RPC when code lives outside Bot:
...
Use the Plugin SDK when code runs inside Bot:
...
External apps should not import `bot/plugin-sdk/*`; those subpaths are for
plugins loaded by Bot.
