...
Hanzo Bot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `hanzo-bot doctor`, `hanzo-bot logs`, `hanzo-bot health`, `hanzo-bot status`, `hanzo-bot service`, `hanzo-bot help`).
- Run `hanzo-bot doctor` to see the exact issues.
- Run `hanzo-bot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
Hanzo Bot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Hanzo Bot runs your login shell and imports only the missing expected keys (never overrides).
...
On first use, Hanzo Bot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Hanzo Bot derives defaults (only when you haven’t set them explicitly):
...
- The legacy single-account Baileys auth dir is migrated by `hanzo-bot doctor` into `whatsapp/default`.
...
Hanzo Bot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Hanzo Bot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `hanzo-bot plugins install @bot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Hanzo Bot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Hanzo Bot
...
When a file exceeds this limit, Hanzo Bot logs a warning and injects a truncated
...
message envelopes). If unset, Hanzo Bot uses the host timezone at runtime.
...
`"[bot]"`, Hanzo Bot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Hanzo Bot generates audio
...
Hanzo Bot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Hanzo Bot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `hanzo-bot configure --section web`, or use `BRAVE_API_KEY` env var)
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Hanzo Bot uses
...
- Shortcut: `hanzo-bot onboard --auth-choice opencode-zen`.
...
Shortcut: `hanzo-bot onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `hanzo-bot onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `hanzo-bot onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `hanzo-bot onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `hanzo-bot onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `hanzo-bot onboard --auth-choice minimax-api`.
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Hanzo Bot stays in idle-only mode for backward compatibility.
...
Hanzo Bot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for hanzo-bot and expose a small loopback control service.
...
- When a connection comes from one of these IPs, Hanzo Bot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `hanzo-bot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  ...
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Hanzo Bot
...
Gmail helper config (used by `hanzo-bot webhooks gmail setup` / `run`):
...
Note: when `tailscale.mode` is on, Hanzo Bot defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__bot__/ws`
...
Controls LAN mDNS discovery broadcasts (`_bot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `bot` (advertises `bot.local`). Override with `BOT_MDNS_HOSTNAME`.
