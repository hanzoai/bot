...
Bot reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.hanzoai/bot.json`. If the file is missing, Bot uses safe defaults.

The active config path must be a regular file. Bot-owned writes replace it atomically (rename onto the path), so a symlinked `bot.json` gets its target replaced rather than written through - avoid symlinked config layouts. If you keep config outside the default state directory, point `BOT_CONFIG_PATH` directly at the real file.
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    bot onboard       # full onboarding flow
...
`bot config schema` prints the canonical JSON Schema used by Control UI
...
- Run `bot doctor --fix` (`--repair` is the same flag; `--yes` skips prompts) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically - only `bot doctor --fix`
does. If `bot.json` fails validation (including plugin-local validation), Gateway
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push for public App Store builds uses the hosted Bot relay: `https://ios-push-relay.bot.ai`.
    ...
    ```bash
    bot config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `BOT_GATEWAY_TOKEN` or `gateway.auth.password` / `BOT_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-bot-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **Bot-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      Bot updates that included file and leaves `bot.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for Bot-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `bot.json`. To share a tree across machines or users, set
      `BOT_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
invalid external edits without rewriting `bot.json`. Bot-owned config
...
config`, inspect the config, run `bot config validate`, then run `bot
...
When you edit a source file that is referenced through `$include`, Bot plans
...
Env var equivalent: `BOT_LOAD_SHELL_ENV=1`. Default `timeoutMs`: `15000`.
</Accordion>
