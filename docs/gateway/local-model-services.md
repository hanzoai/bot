---
summary: "Start local model servers on demand before Bot model and embedding requests"
read_when:
  - You want Bot to start a local model server only when its model or embedding provider is selected
...
`models.providers.<id>.localService` starts a provider-owned local model server on demand. When a model or embedding request selects that provider, Bot probes the health endpoint, starts the process if it is down, waits for readiness, then sends the request. Use it to avoid keeping expensive local servers running all day.

## How it works

1. A model or embedding request resolves to a configured provider.
2. If that provider has `localService`, Bot probes `healthUrl`.
3. On a successful probe, Bot uses the already-running server.
4. On a failed probe, Bot spawns `command` with `args`.
5. Bot polls the health endpoint until `readyTimeoutMs` expires.
6. The request goes through the normal model or embedding transport.
7. If Bot started the process and `idleStopMs` is set, it stops the process after the last in-flight request has been idle that long.

Bot does not install launchd, systemd, Docker, or any daemon for this. The server is a plain child process of whichever Bot process first needed it.
...
If another Bot process already has a healthy server at the same `healthUrl`, this process reuses it without adopting it (each process only manages the child it personally started). Startup and exit logs include bounded, redacted child-output tails plus timing and exit details; configured environment values are never emitted.
...
| `env`            | no       | Environment variables merged over the Bot process environment.                                                                  |
...
| `idleStopMs`     | no       | Idle shutdown delay for an Bot-started process. `0` or omitted keeps it alive until Bot exits.                             |
...
Replace `command` with the result of `which inferrs` on the machine running Bot. Full inferrs setup: [Inferrs](/providers/inferrs).
...
<CardGroup cols={2}>
  ...
  <Card title="Inferrs" href="/providers/inferrs" icon="cpu">
    Run Bot through the inferrs OpenAI-compatible local server.
  </Card>
</CardGroup>
