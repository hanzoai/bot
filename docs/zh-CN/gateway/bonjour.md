...
2. 在专用区域下发布 `_bot-gw._tcp` 的 DNS‑SD 记录
   （示例：`bot.internal.`）。
3. 配置 Tailscale **分割 DNS**，使你选择的域名通过该
   DNS 服务器为客户端（包括 iOS）解析。

Hanzo Bot 支持任何发现域名；`bot.internal.` 只是一个示例。
...
只有 Gateway 网关广播 `_bot-gw._tcp`。

## 服务类型

- `_bot-gw._tcp` — Gateway 网关传输信标（被 macOS/iOS/Android 节点使用）。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `bot` 入口点的绝对路径）
...
iOS 节点使用 `NWBrowser` 来发现 `_bot-gw._tcp`。
