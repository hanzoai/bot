...
所有日志配置都在 `~/.bot/bot.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/bot/bot-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- `bot.tokens`（计数器，属性：`bot.token`、`bot.channel`、`bot.provider`、`bot.model`）
- `bot.cost.usd`（计数器，属性：`bot.channel`、`bot.provider`、`bot.model`）
- `bot.run.duration_ms`（直方图，属性：`bot.channel`、`bot.provider`、`bot.model`）
- `bot.context.tokens`（直方图，属性：`bot.context`、`bot.channel`、`bot.provider`、`bot.model`）

消息流：

- `bot.webhook.received`（计数器，属性：`bot.channel`、`bot.webhook`）
- `bot.webhook.error`（计数器，属性：`bot.channel`、`bot.webhook`）
- `bot.webhook.duration_ms`（直方图，属性：`bot.channel`、`bot.webhook`）
- `bot.message.queued`（计数器，属性：`bot.channel`、`bot.source`）
- `bot.message.processed`（计数器，属性：`bot.channel`、`bot.outcome`）
- `bot.message.duration_ms`（直方图，属性：`bot.channel`、`bot.outcome`）

队列 + 会话：

- `bot.queue.lane.enqueue`（计数器，属性：`bot.lane`）
- `bot.queue.lane.dequeue`（计数器，属性：`bot.lane`）
- `bot.queue.depth`（直方图，属性：`bot.lane` 或 `bot.channel=heartbeat`）
- `bot.queue.wait_ms`（直方图，属性：`bot.lane`）
- `bot.session.state`（计数器，属性：`bot.state`、`bot.reason`）
- `bot.session.stuck`（计数器，属性：`bot.state`）
- `bot.session.stuck_age_ms`（直方图，属性：`bot.state`）
- `bot.run.attempt`（计数器，属性：`bot.attempt`）

### 导出的 span（名称 + 关键属性）

- `bot.model.usage`
  - `bot.channel`、`bot.provider`、`bot.model`
  - `bot.sessionKey`、`bot.sessionId`
  - `bot.tokens.*`（input/output/cache_read/cache_write/total）
- `bot.webhook.processed`
  - `bot.channel`、`bot.webhook`、`bot.chatId`
- `bot.webhook.error`
  - `bot.channel`、`bot.webhook`、`bot.chatId`、`bot.error`
- `bot.message.processed`
  - `bot.channel`、`bot.outcome`、`bot.chatId`、`bot.messageId`、`bot.sessionKey`、`bot.sessionId`、`bot.reason`
- `bot.session.stuck`
  - `bot.state`、`bot.ageMs`、`bot.queueDepth`、`bot.sessionKey`、`bot.sessionId`
