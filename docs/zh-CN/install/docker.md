...
- 需要再次获取带令牌的 URL？运行 `docker compose run --rm bot-cli dashboard --no-open`。
...
如果你想将额外的主机目录挂载到容器中，在运行 `docker-setup.sh` 之前设置 `BOT_EXTRA_MOUNTS`。这接受逗号分隔的 Docker 绑定挂载列表，并通过生成 `docker-compose.extra.yml` 将它们应用到 `bot-gateway` 和 `bot-cli`。
...
镜像以 `node`（uid 1000）运行。如果你在 `/home/node/.bot` 上看到权限错误，确保你的主机绑定挂载由 uid 1000 拥有。
...
- Gateway 网关容器是会话的真实来源（`~/.bot/agents/<agentId>/sessions/`）。
...
- 镜像：`bot-sandbox:bookworm-slim`
- 每个智能体一个容器
- 智能体工作区访问：`workspaceAccess: "none"`（默认）使用 `~/.bot/sandboxes`
...
这使用 `Dockerfile.sandbox` 构建 `bot-sandbox:bookworm-slim`。
...
这构建 `bot-sandbox-common:bookworm-slim`。要使用它：

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "bot-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
这使用 `Dockerfile.sandbox-browser` 构建 `bot-sandbox-browser:bookworm-slim`。容器运行启用 CDP 的 Chromium 和可选的 noVNC 观察器（通过 Xvfb 有头）。
...
- 镜像缺失：使用 [`scripts/sandbox-setup.sh`](https://github.com/bot/bot/blob/main/scripts/sandbox-setup.sh) 构建或设置 `agents.defaults.sandbox.docker.image`。
- 容器未运行：它会按需为每个会话自动创建。
