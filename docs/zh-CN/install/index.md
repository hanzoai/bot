...
read_when:
  - 安装 Hanzo Bot
  - 你想从 GitHub 安装
summary: 安装 Hanzo Bot（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 bot、node-llama-cpp、sharp 等
pnpm add -g bot@latest           # 重新运行以执行 postinstall 脚本
```
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `bot`）。
