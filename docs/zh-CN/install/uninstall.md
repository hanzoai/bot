...
如果 Gateway 网关服务持续运行但 `bot` 缺失，请使用此方法。

### macOS（launchd）

默认标签是 `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.bot.*` 可能仍然存在）：
...
如果你使用了配置文件，请将标签和 plist 名称替换为 `bot.molt.<profile>`。如果存在任何旧版 `com.bot.*` plist，请将其移除。

### Linux（systemd 用户单元）

默认单元名称是 `bot-gateway.service`（或 `bot-gateway-<profile>.service`）：
...
如果你使用了 `https://hanzo.bot/install.sh` 或 `install.ps1`，CLI 是通过 `npm install -g bot@latest` 安装的。
使用 `npm rm -g bot` 移除（或 `pnpm remove -g` / `bun remove -g`，如果你是用那种方式安装的）。
