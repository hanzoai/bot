---
read_when:
  - 你想了解 `hanzo.bot/install.sh` 的工作机制
  - 你想自动化安装（CI / 无头环境）
  - 你想从 GitHub 检出安装
...
Hanzo Bot 提供两个安装器脚本（托管在 `hanzo.bot`）：

- `https://hanzo.bot/install.sh` — "推荐"安装器（默认全局 npm 安装；也可从 GitHub 检出安装）
- `https://hanzo.bot/install-cli.sh` — 无需 root 权限的 CLI 安装器（安装到带有独立 Node 的前缀目录）
- `https://hanzo.bot/install.ps1` — Windows PowerShell 安装器（默认 npm；可选 git 安装）
...
如果安装器完成但在新终端中找不到 `bot`，通常是 Node/npm PATH 问题。参见：[安装](/install#nodejs--npm-path-sanity)。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g bot@latest`
...
如果你在**已有的 Hanzo Bot 源码检出目录中**运行安装器（通过 `package.json` + `pnpm-workspace.yaml` 检测），它会提示：
