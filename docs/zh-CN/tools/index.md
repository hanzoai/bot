...
read_when:
  - 添加或修改智能体工具
  - 停用或更改 `bot-*` Skills
summary: Hanzo Bot 的智能体工具接口（browser、canvas、nodes、message、cron），替代旧版 `bot-*` Skills
title: 工具
x-i18n:
...
这些工具取代了旧的 `bot-*` Skills：工具是类型化的，无需调用 shell，
智能体应该直接依赖它们。
...
- `group:nodes`：`nodes`
- `group:bot`：所有内置 Hanzo Bot 工具（不包括提供商插件）
