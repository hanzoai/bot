...
read_when:
  - 添加或修改插件/扩展
  - 记录插件安装或加载规则
summary: Hanzo Bot 插件/扩展：发现、配置和安全
title: 插件
x-i18n:
...
插件只是一个**小型代码模块**，用额外功能（命令、工具和 Gateway 网关 RPC）扩展 Hanzo Bot。

大多数时候，当你想要一个尚未内置到核心 Hanzo Bot 的功能（或你想将可选功能排除在主安装之外）时，你会使用插件。
...
- 从 2026.1.15 起 Microsoft Teams 仅作为插件提供；如果使用 Teams，请安装 `@bot/msteams`。
...
Hanzo Bot 插件是通过 jiti 在运行时加载的 **TypeScript 模块**。**配置验证不会执行插件代码**；它使用插件清单和 JSON Schema。参见 [插件清单](/plugins/manifest)。
...
每个插件必须在其根目录中包含 `bot.plugin.json` 文件。如果路径指向文件，则插件根目录是文件的目录，必须包含清单。

如果多个插件解析到相同的 id，上述顺序中的第一个匹配项获胜，较低优先级的副本被忽略。

### 包集合

插件目录可以包含带有 `bot.extensions` 的 `package.json`：
...
渠道插件可以通过 `bot.channel` 广播新手引导元数据，通过 `bot.install` 广播安装提示。这使核心目录保持无数据。
...
如果插件导出 `id`，Hanzo Bot 会使用它，但当它与配置的 id 不匹配时会发出警告。
...
- 插件配置使用嵌入在 `bot.plugin.json`（`configSchema`）中的 JSON Schema 进行验证。
...
Hanzo Bot 在运行时根据发现的插件增强 `uiHints`：
...
插件可以注册**模型提供商认证**流程，以便用户可以在 Hanzo Bot 内运行 OAuth 或 API 密钥设置（无需外部脚本）。
...
- 主包：`bot`（本仓库）
- 插件：`@bot/*` 下的独立 npm 包（例如：`@bot/voice-call`）
...
- 单独发布的插件应运行自己的 CI（lint/构建/测试）并验证 `bot.extensions` 指向构建的入口点（`dist/index.js`）。
