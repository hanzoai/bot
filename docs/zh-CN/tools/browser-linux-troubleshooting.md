...
然后更新你的 Hanzo Bot 配置（`~/.bot/bot.json`）：
...
启用：`systemctl --user enable --now bot-browser.service`
...
1. **使用托管浏览器：** `hanzo-bot browser start --browser-profile bot`
   （或设置 `browser.defaultProfile: "bot"`）。
2. **使用扩展中继：** 安装扩展，打开一个标签页，然后点击 Hanzo Bot 扩展图标来附加它。

注意事项：

- `chrome` 配置文件在可能时使用你的**系统默认 Chromium 浏览器**。
- 本地 `bot` 配置文件自动分配 `cdpPort`/`cdpUrl`；仅为远程 CDP 设置这些。
