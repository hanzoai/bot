...
- [ ]（可选）安装程序 E2E（Docker，运行 `curl -fsSL https://hanzo.bot/install.sh | bash`，新手引导，然后运行真实工具调用）：
...
- [ ] 按照 [macOS 发布](/platforms/mac/release) 获取确切命令和所需环境变量。
  ...
  - 如果进行公证，使用从 App Store Connect API 环境变量创建的 `bot-notary` 钥匙串配置文件（参见 [macOS 发布](/platforms/mac/release)）。
...
- [ ] 验证注册表：`npm view hanzo-bot version`、`npm view hanzo-bot dist-tags` 和 `npx -y bot@X.Y.Z --version`（或 `--help`）。

### 故障排除（来自 2.0.0-beta2 发布的笔记）

- **npm pack/publish 挂起或产生巨大 tarball**：`dist/Hanzo Bot.app` 中的 macOS 应用包（和发布 zip）被扫入包中。通过 `package.json` 的 `files` 白名单发布内容来修复（包含 dist 子目录、docs、skills；排除应用包）。用 `npm pack --dry-run` 确认 `dist/Hanzo Bot.app` 未列出。
...
- [ ] 附加产物：`npm pack` tarball（可选）、`Hanzo Bot-X.Y.Z.zip` 和 `Hanzo Bot-X.Y.Z.dSYM.zip`（如果生成）。
- [ ] 提交更新后的 `appcast.xml` 并推送（Sparkle 从 main 获取源）。
- [ ] 从干净的临时目录（无 `package.json`），运行 `npx -y bot@X.Y.Z send --help` 确认安装/CLI 入口点正常工作。
- [ ] 宣布/分享发布说明。

## 插件发布范围（npm）

我们只发布 `@bot/*` 范围下的**现有 npm 插件**。不在 npm 上的内置插件保持**仅磁盘树**（仍在 `extensions/**` 中发布）。
