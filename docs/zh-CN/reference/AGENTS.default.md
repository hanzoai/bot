...
Hanzo Bot 为智能体使用专用的工作区目录。默认：`~/.bot/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
如果你将此工作区视为 Hanzo Bot 的"记忆"，请将其设为 git 仓库（最好是私有的），这样 `AGENTS.md` 和你的记忆文件就会被备份。
...
- macOS 应用管理权限（屏幕录制、通知、麦克风）并通过其内置二进制文件暴露 `bot` CLI。
