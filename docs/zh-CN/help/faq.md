...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [安装和设置 Hanzo Bot 的推荐方式是什么？](#whats-the-recommended-way-to-install-and-set-up-bot)
  ...
  - [无法访问 docs.bot.ai（SSL 错误），怎么办？](#i-cant-access-docsbotai-ssl-error-what-now)
...
- [如何安装 beta 版本，beta 和 dev 有什么区别？](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [Windows 安装提示找不到 git 或无法识别 bot](#windows-install-says-git-not-found-or-bot-not-recognized)
  - [文档没有解答我的问题——如何获得更好的答案？](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [如何在 Linux 上安装 Hanzo Bot？](#how-do-i-install-bot-on-linux)
  - [如何在 VPS 上安装 Hanzo Bot？](#how-do-i-install-bot-on-a-vps)
  - [云/VPS 安装指南在哪里？](#where-are-the-cloudvps-install-guides)
  - [可以让 Hanzo Bot 自行更新吗？](#can-i-ask-bot-to-update-itself)
  ...
  - [如果我买了 Mac mini 运行 Hanzo Bot，能连接到我的 MacBook Pro 吗？](#if-i-buy-a-mac-mini-to-run-bot-can-i-connect-it-to-my-macbook-pro)
  - [可以使用 Bun 吗？](#can-i-use-bun)
  - [Telegram：`allowFrom` 填什么？](#telegram-what-goes-in-allowfrom)
  - [多人能否使用同一个 WhatsApp 号码配合不同的 Hanzo Bot 实例？](#can-multiple-people-use-one-whatsapp-number-with-different-bot-instances)
  ...
  - [在专用机器上运行 Hanzo Bot 有多重要？](#how-important-is-it-to-run-bot-on-a-dedicated-machine)
  - [VPS 的最低要求和推荐操作系统是什么？](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [可以在虚拟机中运行 Hanzo Bot 吗？有什么要求？](#can-i-run-bot-in-a-vm-and-what-are-the-requirements)
- [什么是 Hanzo Bot？](#what-is-bot)
  - [用一段话描述 Hanzo Bot？](#what-is-bot-in-one-paragraph)
  - [价值主张是什么？](#whats-the-value-proposition)
  - [刚设置好，应该先做什么？](#i-just-set-it-up-what-should-i-do-first)
  - [Hanzo Bot 日常最常用的五个场景是什么？](#what-are-the-top-five-everyday-use-cases-for-bot)
  - [Hanzo Bot 能否帮助 SaaS 进行获客、外联、广告和博客？](#can-bot-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills 与自动化](#skills-and-automation)
  ...
  - [Hanzo Bot 能否按计划或在后台持续运行任务？](#can-bot-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [磁盘上的文件位置](#where-things-live-on-disk)
  - [Hanzo Bot 使用的所有数据都保存在本地吗？](#is-all-data-used-with-bot-saved-locally)
  - [Hanzo Bot 将数据存储在哪里？](#where-does-bot-store-its-data)
  ...
  - [如何完全卸载 Hanzo Bot？](#how-do-i-completely-uninstall-bot)
...
- [配置基础](#config-basics)
  ...
  - [Hanzo Bot 浏览器可以无头运行吗？](#can-the-bot-browser-run-headless)
...
- [远程 Gateway 网关与节点](#remote-gateways-nodes)
  ...
  - [两个 Hanzo Bot 实例（本地 + VPS）可以互相通信吗？](#can-two-bot-instances-talk-to-each-other-local-vps)
...
- [环境变量和 .env 加载](#env-vars-and-env-loading)
  - [Hanzo Bot 如何加载环境变量？](#how-does-bot-load-environment-variables)
...
- [会话与多聊天](#sessions-multiple-chats)
  ...
  - [能否创建一个 Hanzo Bot 实例团队——一个 CEO 和多个智能体？](#is-there-a-way-to-make-a-team-of-bot-instances-one-ceo-and-many-agents)
  - [为什么上下文在任务中途被截断了？如何防止？](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [如何完全重置 Hanzo Bot 但保留安装？](#how-do-i-completely-reset-bot-but-keep-it-installed)
  ...
  - [为什么 Hanzo Bot 不在群组中回复？](#why-doesnt-bot-reply-in-a-group)
...
- [模型：默认值、选择、别名、切换](#models-defaults-selection-aliases-switching)
  ...
  - [Hanzo Bot、Flawd 和 Krill 使用什么模型？](#what-do-bot-flawd-and-krill-use-for-models)
...
- [Gateway 网关：端口、“已在运行”和远程模式](#gateway-ports-already-running-and-remote-mode)
  ...
  - [为什么 `hanzo-bot gateway status` 显示 `Runtime: running` 但 `RPC probe: failed`？](#why-does-bot-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [为什么 `hanzo-bot gateway status` 显示 `Config (cli)` 和 `Config (service)` 不同？](#why-does-bot-gateway-status-show-config-cli-and-config-service-different)
  - ["another gateway instance is already listening"是什么意思？](#what-does-another-gateway-instance-is-already-listening-mean)
  - [如何以远程模式运行 Hanzo Bot（客户端连接到其他位置的 Gateway 网关）？](#how-do-i-run-bot-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [日志与调试](#logging-and-debugging)
  ...
  - [我在 Windows 上关闭了终端——如何重启 Hanzo Bot？](#i-closed-my-terminal-on-windows-how-do-i-restart-bot)
  ...
  - [通俗解释：`hanzo-bot gateway restart` 与 `hanzo-bot gateway`](#eli5-bot-gateway-restart-vs-bot-gateway)
...
- [安全与访问控制](#security-and-access-control)
  - [将 Hanzo Bot 暴露给入站私信安全吗？](#is-it-safe-to-expose-bot-to-inbound-dms)
...
https://github.com/bot/bot/issues
https://github.com/bot/bot/pulls
...
相关：[迁移](/install/migrating)、[磁盘上的文件位置](/help/faq#where-does-bot-store-its-data)、
...
查看 GitHub 变更日志：
https://github.com/bot/bot/blob/main/CHANGELOG.md

最新条目在顶部。如果顶部部分标记为 **Unreleased**，则下一个带日期的部分是最新发布版本。条目按**亮点**、**变更**和**修复**分组（需要时还有文档/其他部分）。

### 无法访问 docs.bot.ai（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.bot.ai`。禁用该功能或将 `docs.bot.ai` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docsbotai-shows-an-ssl-error-comcastxfinity)。
请帮助我们在此处报告以解除封锁：https://spa.xfinity.com/check_url_status。

如果仍然无法访问该网站，文档在 GitHub 上有镜像：
https://github.com/bot/bot/tree/main/docs
...
Hanzo Bot 通过 OAuth（ChatGPT 登录）支持 **OpenAI Code (Codex)**。向导可以运行 OAuth 流程，并在适当时将默认模型设置为 `openai-codex/gpt-5.2`。参阅[模型提供商](/concepts/model-providers)和[向导](/start/wizard)。

### 是否支持 OpenAI 订阅认证（Codex OAuth）

是的。Hanzo Bot 完全支持 **OpenAI Code (Codex) 订阅 OAuth**。新手引导向导可以为你运行 OAuth 流程。
...
可以。通过 `~/.bot/bot.json` 中的 `skills.load.extraDirs` 添加额外目录（最低优先级）。默认优先级保持不变：`<workspace>/skills` → `~/.bot/skills` → 内置 → `skills.load.extraDirs`。`hanzo-skills` 默认安装到 `./skills`，Hanzo Bot 将其视为 `<workspace>/skills`。
...
使用 **Hanzo Skills**（CLI）或将 Skills 放入你的工作区。macOS Skills UI 在 Linux 上不可用。
浏览 Skills：https://skills.hanzo.bot。

安装 Hanzo Skills CLI（选择一个包管理器）：
...
不能直接运行。macOS Skills 受 `metadata.bot.os` 和所需二进制文件限制，Skills 只有在 **Gateway 网关主机**上符合条件时才会出现在系统提示中。在 Linux 上，`darwin` 专用 Skills（如 `apple-notes`、`apple-reminders`、`things-mac`）不会加载，除非你覆盖限制。

你有三种支持的模式：

**方案 A - 在 Mac 上运行 Gateway 网关（最简单）。**
在 macOS 二进制文件所在的地方运行 Gateway 网关，然后从 Linux 通过[远程模式](#how-do-i-run-bot-in-remote-mode-client-connects-to-a-gateway-elsewhere)或 Tailscale 连接。Skills 正常加载，因为 Gateway 网关主机是 macOS。
...
Hanzo Skills 安装到当前目录下的 `./skills`（或回退到你配置的 Hanzo Bot 工作区）；Hanzo Bot 在下一个会话中将其视为 `<workspace>/skills`。对于跨智能体共享的 Skills，将它们放在 `~/.bot/skills/<name>/SKILL.md`。某些 Skills 期望通过 Homebrew 安装二进制文件；在 Linux 上意味着 Linuxbrew（参阅上面的 Homebrew Linux 常见问题条目）。参阅[Skills](/tools/skills)和 [Hanzo Skills](/tools/hanzo-skills)。
...
Hanzo Bot 从 `$BOT_CONFIG_PATH`（默认：`~/.bot/bot.json`）读取可选的 **JSON5** 配置：
...
- 控制 UI 将令牌存储在浏览器 localStorage 键 `bot.control.settings.v1` 中。
...
配置文件还会为服务名称添加后缀（`bot.molt.<profile>`；旧版 `com.bot.*`、`bot-gateway-<profile>.service`、`Hanzo Bot Gateway 网关 (<profile>)`）。
...
智能体的出站附件必须包含 `MEDIA:<path-or-url>` 行（独占一行）。参阅 [Hanzo Bot 助手设置](/start/bot)和 [Agent send](/tools/agent-send)。
...
仍然卡住？在 [Discord](https://discord.com/invite/hanzo) 中提问或发起 [GitHub 讨论](https://github.com/bot/bot/discussions)。
