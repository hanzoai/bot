...
read_when:
  - Moving Bot runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.bot/state/bot.sqlite`
...
- Configuration stays file-backed: `bot.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside Bot's database.
...
- Raw stream logging uses `BOT_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of Bot
  runtime or tests.
...
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/bot-agent.sqlite`.
- Config remains file-backed. `bot.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/bot.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/bot-state-schema.sql`,
      `src/state/bot-agent-schema.sql`,
      `src/state/*generated*`.
...
- `bot doctor --fix` owns legacy file-to-database migration. Runtime
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the Bot `sessionId` as the canonical
...
former stores now write to `state/bot.sqlite` or the owning
`bot-agent.sqlite` database.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `bot-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/bot-agent-db.ts` opens
  `agents/<agentId>/agent/bot-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one Bot session to one or more external
  ...
  Bot session can represent multiple external DM peers without demoting
...
- TUI last-session restore pointers now live in typed shared
  ...
  excludes heartbeat sessions. `bot doctor --fix` strictly validates the
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `bot.json` remains the config store;
...
- Workspace bootstrap completion, attestation recency, and generated bootstrap
  ...
  `bot-workspace-state.json`, `.bot/workspace-state.json`, state-dir
  `workspace-attestations/*.attested`, or sibling `<workspace>.attested`
  sidecars. `bot doctor --fix` validates and claims legacy sources,
...
- GitHub Copilot compaction no longer writes `bot-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and Bot keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`BotKit`) uses the same
  `state/bot.sqlite#table/device_identities` shape and row keys for device
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `bot update --channel` now fails on invalid legacy config and asks the
...
- APNs runtime reads and writes only `apns_registrations`. Explicit
  `bot doctor --fix` strictly imports the retired
...
- Node-host config now uses a typed singleton row in the shared SQLite database.
  ...
  remains; explicit `bot doctor --fix` strictly imports and removes it
...
- Plugin state now uses the shared `state/bot.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/bot.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `bot sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `BOT_CACHE_TRACE_FILE`,
  `BOT_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `BOT_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.bot-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.bot-wiki/state.json` or the unused
  `.bot-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `bot.json`. The config
...
- System-agent rescue pending approvals now use core SQLite plugin state instead
  of `crestodian/rescue-pending/*.json` or `bot/rescue-pending/*.json`.
...
- Codex app-server conversation bindings now key SQLite plugin state by
  Bot session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `bot-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the Bot temp root, not durable Bot state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$BOT_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$BOT_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.bot/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `bot-cli-images` cache
...
- `bot secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/bot.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- MCP OAuth sessions now use versioned `mcp_oauth_stores` rows in shared
  `state/bot.sqlite`. SDK-owned token, client-registration, and discovery
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.bot-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.bot-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store compressed SQLite plugin-blob rows
  instead of writing `.bot-wiki/cache/agent-digest.json` and
  `.bot-wiki/cache/claims.jsonl`. The cache is rebuildable, so doctor
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  Bot temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `bot backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `bot backup restore` validates the archive before extraction, reuses the
...
- `bot reset --scope config+creds+sessions` removes per-agent
  `bot-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureBotModelCatalog`; there is no `models.json` compatibility API in
...
- QMD's own `index.sqlite`, YAML collection config, and model downloads remain
  external-tool artifacts under `~/.bot/agents/<agentId>/qmd`; they are not
  mirrored into `plugin_blob_entries`. Bot-owned QMD coordination is
...
- The optional `memory-lancedb` plugin no longer creates
  `~/.bot/memory/lancedb` as an implicit Bot-managed store. It is an
...
`bot doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `bot migrate` must not import legacy Bot state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `bot doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/bot.sqlite`:
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `bot backup create` does this archive verification by default;
...
5. Verify every SQLite snapshot's file shape, then open canonical Bot
   ...
   `bot backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `bot sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `bot backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  Bot test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `bot-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- `bot/rescue-pending/*.json`
- `plugins/phone-control/armed.json`
- Memory Wiki `.bot-wiki/log.jsonl`
- Memory Wiki `.bot-wiki/state.json`
- Memory Wiki `.bot-wiki/locks/`
- Memory Wiki `.bot-wiki/source-sync.json`
- Memory Wiki `.bot-wiki/import-runs/*.json`
- Memory Wiki `.bot-wiki/cache/agent-digest.json`
- Memory Wiki `.bot-wiki/cache/claims.jsonl`
- ClawHub `.clawhub/lock.json`
- ClawHub `.clawhub/origin.json`
- Browser profile decoration `.bot-profile-decorated`
