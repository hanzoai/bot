---
summary: "Run Bot on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Bot
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Bot Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Bot on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Bot bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/bot-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `BOT_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
- Service env for a stable startup path: `BOT_NO_RESPAWN=1` and
...
The standard `bot onboard --install-daemon` path installs a systemd user
unit; edit it with:
...
`sudo systemctl edit bot-gateway.service`.
