...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.bot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Recommendation: **deep integration in Hanzo Bot**, but keep a separable core library.

### Why integrate into Hanzo Bot?

- Hanzo Bot already knows:
