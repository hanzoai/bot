---
summary: "Developer workflow for Bot agent runtime: build, test, and live validation"
title: "Bot agent runtime workflow"
read_when:
  - Working on Bot agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

Developer workflow for the agent runtime (`src/agents/`) in the Bot repo.
...
wrapper (sets `BOT_LIVE_TEST=1` and needs provider credentials):
...
- Run the Gateway in dev mode (skips channel connections via `BOT_SKIP_CHANNELS=1`): `pnpm gateway:dev`
- Trigger one agent turn through the Gateway: `pnpm bot agent --message "Hello" --thinking low`
...
State lives in the Bot state directory: `~/.bot` by default, or
`$BOT_STATE_DIR` when set. Paths relative to that directory:

| Path                                           | Holds                                                              |
| ---------------------------------------------- | ------------------------------------------------------------------ |
| `bot.json`                                | Config                                                             |
| `state/bot.sqlite`                        | Shared runtime state database                                      |
| `agents/<agentId>/agent/bot-agent.sqlite` | Per-agent model auth profiles (API keys + OAuth) and runtime state |
...
- Sessions only: do not delete `agents/<agentId>/agent/bot-agent.sqlite`; session rows live there alongside other per-agent state. Use `/new` or `/reset` to start a fresh session for one chat, and `bot sessions cleanup` for session maintenance.
- Keep auth: leave `agents/<agentId>/agent/bot-agent.sqlite` and `credentials/` in place.

Legacy `auth-profiles.json` files are no longer read at runtime;
`bot doctor --fix` imports them into the SQLite store.
...
- [Bot agent runtime architecture](/agent-runtime-architecture)
