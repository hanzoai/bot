---
summary: "Install Bot - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Bot, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://bot.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Bot and Node kept under a local prefix such as
`~/.bot`, without depending on a system-wide Node install:

```bash
curl -fsSL https://bot.ai/install-cli.sh | bash
```
...
`bot update --channel dev` and `bot update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    for the Bot package install. If you install manually with npm, your own
  ...
  <Tab title="bun">
    ...
    <Note>
    Bun can install the global package, but the resulting `bot` executable requires a supported Node runtime because Bot state uses `node:sqlite`.
    </Note>
...
Or skip the link and use `pnpm bot ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
bot --version      # confirm the CLI is available
bot doctor         # check for config issues
bot gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `bot onboard --install-daemon` or `bot gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Bot on a cloud server or VPS. See [Linux server](/vps) for the full
