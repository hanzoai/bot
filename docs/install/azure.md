---
summary: "Run Bot Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Bot running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Bot Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

Set up an Azure Linux VM with the Azure CLI, apply Network Security Group (NSG) hardening, configure Azure Bastion for SSH access, and install Bot.
...
- Install Bot with the installer script
- Verify the gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    VM_SUBNET_NAME="snet-bot-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-bot-vm"
    VM_NAME="vm-bot"
    ADMIN_USERNAME="bot"
    BASTION_NAME="bas-bot"
    BASTION_PIP_NAME="pip-bot-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Bot (in the VM shell)">
    ```bash
    curl -fsSL https://bot.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node and dependencies if not already present, installs Bot, and launches onboarding. See [Install](/install) for details.
...
- More detail on Azure deployment with the GitHub Copilot model provider: [Bot on Azure with GitHub Copilot](https://github.com/johnsonshi/bot-azure-github-copilot)
