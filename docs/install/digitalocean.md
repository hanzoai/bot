---
summary: "Host Bot on a DigitalOcean Droplet"
read_when:
  - Setting up Bot on DigitalOcean
  - Looking for a simple paid VPS for Bot
title: "DigitalOcean"
---

Run a persistent Bot Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own Bot state and services.
    adduser bot
    usermod -aG sudo bot
    ...
    Use the root shell only for system bootstrap. Run Bot commands as the non-root `bot` user so state lives under `/home/bot/.bot/` and the Gateway installs as that user's systemd `--user` service.
...
- `~/.bot/` -- `bot.json`, channel/provider credentials, per-agent `auth-profiles.json`, and session data.
...
DigitalOcean snapshots back up the whole Droplet; `bot backup create` is portable across hosts.
