...
Hanzo Bot supports a **Nix mode** that makes configuration deterministic and disables auto-install flows.
...
Hanzo Bot reads JSON5 config from `BOT_CONFIG_PATH` and stores mutable data in `BOT_STATE_DIR`.
...
[`scripts/package-mac-app.sh`](https://github.com/bot/bot/blob/main/scripts/package-mac-app.sh) copies this template into the app bundle and patches dynamic fields
