...
read_when:
  - You want to understand `bot.ai/install.sh`
  - You want to automate installs (CI / headless)
  - You want to install from a GitHub checkout
title: "Installer internals"
---

Bot ships three installer scripts, served from `bot.ai`.

| Script                             | Platform             | What it does                                                                                   |
| ---------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL  | Installs Node if needed, installs Bot via npm (default) or git, can run onboarding.       |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | Installs Node + Bot into a local prefix (`~/.bot`) via npm or git. No root required. |
| [`install.ps1`](#installps1)       | Windows (PowerShell) | Installs Node if needed, installs Bot via npm (default) or git, can run onboarding.       |
...
<Tabs>
  <Tab title="install.sh">
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --help
    ```

  </Tab>
  <Tab title="install-cli.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install-cli.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install-cli.sh | bash -s -- --help
    ```

  </Tab>
  <Tab title="install.ps1">
    ```powershell
    iwr -useb https://bot.ai/install.ps1 | iex
    ```

    ```powershell
    & ([scriptblock]::Create((iwr -useb https://bot.ai/install.ps1))) -Tag beta -NoOnboard -DryRun
    ```
...
<Note>
If install succeeds but `bot` is not found in a new terminal, see [Node.js troubleshooting](/install/node#troubleshooting).
</Note>
...
<Steps>
  ...
  <Step title="Post-install tasks">
    - Resolves the just-installed `bot` binary for follow-up commands
...
<Tabs>
  ...
  <Tab title="Dry run">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --dry-run
    ```
  </Tab>
...
| `BOT_VERSION=latest\|next\|<semver>\|<spec>` | npm version, dist-tag, or package spec                             |
| `BOT_BETA=0\|1`                              | Use beta if available                                              |
| `BOT_HOME=<path>`                            | Base directory for Bot state and default git/onboarding paths |
| `BOT_GIT_DIR=<path>`                         | Checkout directory                                                 |
| `BOT_GIT_UPDATE=0\|1`                        | Toggle git updates                                                 |
| `BOT_NO_PROMPT=1`                            | Disable prompts                                                    |
| `BOT_VERIFY_INSTALL=1`                       | Run the post-install smoke verify                                  |
| `BOT_NO_ONBOARD=1`                           | Skip onboarding                                                    |
| `BOT_DRY_RUN=1`                              | Dry run mode                                                       |
| `BOT_VERBOSE=1`                              | Debug mode                                                         |
| `BOT_NPM_LOGLEVEL=error\|warn\|notice`       | npm log level (default: `error`, hides npm deprecation noise)      |
...
(default `~/.bot`) and no system Node dependency. Supports npm installs
...
<Steps>
  ...
  <Step title="Install Bot under prefix">
    - `npm` method (default): installs under the prefix with npm, then writes wrapper to `<prefix>/bin/bot`
    - `git` method: clones/updates a checkout (default `~/bot`) and still writes the wrapper to `<prefix>/bin/bot`

  </Step>
  <Step title="Refresh loaded gateway service">
    If a gateway service is already loaded from that same prefix, the script runs
    `bot gateway install --force`, which activates the replacement service,
...
<Tabs>
  ...
  <Tab title="Custom prefix + version">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install-cli.sh | bash -s -- --prefix /opt/bot --version latest
    ```
  </Tab>
  <Tab title="Git install">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install-cli.sh | bash -s -- --install-method git --git-dir ~/bot
    ```
  </Tab>
  <Tab title="Automation JSON output">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install-cli.sh | bash -s -- --json --prefix /opt/bot
    ```
  </Tab>
  <Tab title="Run onboarding">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install-cli.sh | bash -s -- --onboard
    ```
  </Tab>
</Tabs>
...
| `--npm`                                 | Shortcut for npm method                                                         |
| `--git \| --github`                     | Shortcut for git method                                                         |
| `--git-dir \| --dir <path>`             | Git checkout directory (default: `~/bot`)                                  |
...
| `BOT_HOME=<path>`                      | Base directory for Bot state and default git/onboarding paths |
| `BOT_GIT_DIR=<path>`                   | Git checkout directory for git installs                            |
| `BOT_GIT_UPDATE=0\|1`                  | Toggle git updates for existing checkouts                          |
| `BOT_NO_ONBOARD=1`                     | Skip onboarding                                                    |
| `BOT_NPM_LOGLEVEL=error\|warn\|notice` | npm log level (default: `error`)                                   |

  </Accordion>
</AccordionGroup>

<Note>
`bot@main` and other GitHub source specs are not valid `--version` targets for npm installs. Use `--install-method git --version main` instead.
</Note>
...
<Steps>
  ...
  <Step title="Ensure a supported Node.js runtime">
    If missing, attempts install via winget, then Chocolatey, then Scoop. If no package manager is available, the script downloads the official Node.js 26 Windows zip into `%LOCALAPPDATA%\Bot\deps\portable-node` and adds it to the current process and user PATH. Node 22.22.3+, Node 24.15+, and Node 25.9+ are supported; Node 23 is unsupported.
  </Step>
  <Step title="Install Bot">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\bot.cmd`. If Git is missing, the script bootstraps user-local MinGit under `%LOCALAPPDATA%\Bot\deps\portable-git` and adds it to the current process and user PATH.

  </Step>
  <Step title="Post-install tasks">
    - Adds needed bin directory to user PATH when possible
    - Refreshes a loaded gateway service best-effort (`bot gateway install --force`, then restart)
...
<Tabs>
  ...
  <Tab title="Git install">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://bot.ai/install.ps1))) -InstallMethod git
    ```
  </Tab>
  <Tab title="GitHub main checkout">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://bot.ai/install.ps1))) -InstallMethod git -Tag main
    ```
  </Tab>
  <Tab title="Custom git directory">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://bot.ai/install.ps1))) -InstallMethod git -GitDir "C:\bot"
    ```
  </Tab>
  <Tab title="Dry run">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://bot.ai/install.ps1))) -DryRun
    ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="install.sh (non-interactive npm)">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://bot.ai/install.sh | bash -s -- --no-prompt --no-onboard
    ```
  </Tab>
