...
<Tabs>
  ...
  <Tab title="Automation JSON output">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://hanzo.bot/install-cli.sh | bash -s -- --json --prefix /opt/bot
    ```
  </Tab>
...
<Tabs>
  ...
  <Tab title="Custom git directory">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://hanzo.bot/install.ps1))) -InstallMethod git -GitDir "C:\bot"
    ```
  </Tab>
  <Tab title="Dry run">
    ```powershell
