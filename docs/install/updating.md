...
Detects your install type (npm, pnpm, Bun, or git), fetches the latest version, runs `bot doctor`, and restarts the gateway.
...
bot update --dry-run   # preview without applying
```

`bot update` has no `--verbose` flag (the installer does). For diagnostics use
`--dry-run` to preview planned actions, `--json` for structured results, or
`bot update status --json` to inspect channel and availability state.
...
foreground-only. Bot reads the public npm `extended-stable` selector,
...
successful core update; a direct `npm install -g bot@extended-stable`
...
Catalog installs created by current Bot versions retain that default
intent. Older records that contain only an exact version remain pinned because
Bot cannot safely distinguish an old automatic pin from a user pin; run
`bot plugins update @hanzo/bot-name` once on the extended-stable channel
to opt that plugin back into exact-core tracking.

`--channel dev` gives a persistent moving GitHub `main` checkout. For a one-off
package update, `--tag main` maps to the `github:hanzoai/bot#main` package
...
credentials, and workspace in `~/.bot`; it only changes which Bot
...
```bash
bot update --channel dev --dry-run
bot update --channel stable --dry-run
```
...
converting it. If the gateway is already installed, `bot update` refreshes
the service metadata and restarts it unless you pass `--no-restart`.

For package installs with a managed Gateway service, `bot update` targets
the package root used by that service. If the shell `bot` command comes
...
```bash
ssh you@server 'cd /path/to/bot && scripts/update-gateway.sh'
```

Override the restart for custom service units, or skip it entirely:

```bash
BOT_UPDATE_RESTART_CMD='systemctl --user restart bot-gateway.service' scripts/update-gateway.sh
BOT_UPDATE_RESTART_CMD='' scripts/update-gateway.sh
```

For a plain single-user source install, prefer `bot update --channel dev`
...
If `bot update` fails after the npm package install phase, re-run the
...
```bash
curl -fsSL https://bot.ai/install.sh | bash -s -- --install-method npm
```

Pin the recovery to a specific version or dist-tag with `--version`:

```bash
curl -fsSL https://bot.ai/install.sh | bash -s -- --install-method npm --version <version-or-dist-tag>
```
...
Prefer `bot update` for supervised installs: it can coordinate the package
...
For a root-owned Linux system-global install, if `bot update` fails with
...
```bash
bot gateway stop
sudo /usr/bin/npm i -g bot@latest
...
bot gateway status --deep --json
bot doctor --lint --json
```

When `bot update` manages a global npm install, it installs the target
into a temporary npm prefix first. The candidate package validates the host
Node version during `preinstall`; only then does Bot verify the packaged
...
swap. On npm 12 and newer, the updater approves only the candidate Bot
...
command fails, Bot retries once with `--omit=optional`, which helps hosts
where native optional dependencies cannot compile.

Bot-managed npm update and plugin-update commands also clear npm's
...
explicit Bot update means "install the selected release now."

```bash
pnpm add -g bot@latest
```

If pnpm 11 installed Bot 2026.7.1, run that manual command once. That
...
or when the invoking package is orphaned or not the only active Bot
install there.

If Bot shares a pnpm 11 global install group with another package, the
...
<AccordionGroup>
  <Accordion title="Read-only package tree">
    Bot treats packaged global installs as read-only at runtime, even when the global package directory is writable by the current user. Plugin package installs live in Bot-owned npm/git roots under the user config directory, and Gateway startup does not mutate the Bot package tree.

    Some Linux npm setups install global packages under root-owned directories such as `/usr/lib/node_modules/bot`. Bot supports that layout because plugin install/update commands write outside that global package directory.

  </Accordion>
  <Accordion title="Hardened systemd units">
    Give Bot write access to its config/state roots so explicit plugin installs, plugin updates, and doctor cleanup can persist their changes:

    ```ini
    ReadWritePaths=/var/lib/bot /home/bot/.bot /tmp
    ```

  </Accordion>
  <Accordion title="Disk-space preflight">
    Before package updates and explicit plugin installs, Bot tries a best-effort disk-space check for the target volume. Low space produces a warning with the checked path, but does not block the update because filesystem quotas, snapshots, and network volumes can change after the check. The actual package-manager install and post-install verification remain authoritative.
  </Accordion>
</AccordionGroup>

## Auto-updater

Off by default. Enable it in `~/.hanzoai/bot.json`:
...
| `dev`             | No automatic apply. Use `bot update` manually.                                                                           |
...
For downgrade or incident recovery, set `BOT_NO_AUTO_UPDATE=1` in the gateway environment to block automatic applies even when `update.auto.enabled` is configured. Startup update hints can still run unless `update.checkOnStart` is also disabled.
...
exits, and lets the normal `bot update --yes --json` CLI path stop the
...
app runs `bot update --tag <app-version> --json`, restarts its Gateway,
...
1. Reinstall older Bot code while keeping the current state.
...
`bot update` preserves an automatic pre-update config copy, but it does not
create a full state recovery point. Before a significant update, create one
explicitly:

```bash
mkdir -p ~/Backups/bot
bot backup create --output ~/Backups/bot --verify
```

The archive manifest records the Bot version and the source paths included
...
```bash
npm view bot versions --json
bot update --tag <known-good-version> --dry-run
bot update --tag <known-good-version>
```

`bot update --tag` is preferred over a direct package-manager install. It
...
filesystem swap or command-shim replacement fails, Bot restores the old
...
```bash
bot gateway stop
npm i -g bot@<known-good-version>
bot gateway install --force
bot gateway restart
```
...
newer release by setting `BOT_NO_AUTO_UPDATE=1` in the Gateway environment.
...
Before starting an older file-backed Bot release, use the current CLI to
...
Broad `bot backup create` archives support creation and verification, but
...
restore. `bot backup sqlite restore` likewise writes a verified database
...
- Run `bot doctor` again and read the output carefully.
- For `bot update --channel dev` on source checkouts, the updater auto-bootstraps `pnpm` when needed. If you see a pnpm/corepack bootstrap error, install `pnpm` manually (or re-enable `corepack`) and rerun the update.
