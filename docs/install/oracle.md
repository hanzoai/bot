---
summary: "Host Bot on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Bot on Oracle Cloud
  - Looking for free VPS hosting for Bot
  - Want 24/7 Bot on a small server
title: "Oracle Cloud"
---

Run a persistent Bot Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=bot
    ```

    From now on, connect via Tailscale: `ssh ubuntu@bot`.
...
- `chmod 700 ~/.bot` to restrict credential file permissions.
- `bot security audit` for an Bot-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Bot features work fine; a small number of native binaries need ARM builds:
...
- `~/.bot/` -- `bot.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.bot/workspace/` -- the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@bot
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=bot --reset` to re-authenticate.

**Gateway will not start** -- Run `bot doctor --non-interactive` and check logs with `journalctl --user -u bot-gateway.service -n 50`.
