---
summary: "Host Bot on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying Bot to Upstash Box
  - You want a managed Linux environment for Bot with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent Bot Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current Bot Box walkthrough at
[Bot Setup](https://upstash.com/docs/box/guides/bot-setup).

## Connect with an SSH tunnel

Forward the Bot dashboard port to your local machine. Use your Box API key
...
```bash
bot config set gateway.bind lan
nohup bot gateway > gateway.log 2>&1 &
```
