...
- `location.get` is a node command, invoked via `node.invoke` or `bot nodes location get`.
- Off by default.
...
The bundled Linux Node plugin adds `location.get` to the CLI `bot node` service, including headless hosts without the Linux desktop app. Location defaults to off. Enable it under the plugin entry, then restart the node service:
...
- Always: "Allow requested location checks while Bot is in the background."
