...
a `gpt-live-*` model. The OAuth prerequisite is an Bot auth profile
created with `bot models auth login --provider openai` — an existing
...
`chatgpt.com` backend route also returns `403`; Bot uses the native
...
the configured Bot agent; the browser receives neither the OAuth token nor
a Platform API key.
...
| `realtime.instructions`                  | -                                          | Appends provider-facing system instructions to Bot's built-in realtime prompt.                                                                                                                                                                                                     |
