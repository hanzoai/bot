...
read_when:
  - You want Bot to identify the active Mac
...
the most recent physical mouse or keyboard input. Bot uses that signal to
...
- The Bot macOS app is paired and connected in node mode.
- **Settings -> Permissions -> Active computer detection** is enabled. It is off by default.
- **Accessibility** permission is granted to the signed Bot app.
...
3. Move the mouse or press a key on that Mac, then run:
   ...
   ```bash
   bot nodes status
   bot nodes describe --node <node-id-or-name>
   ```
...
used for UI automation. Bot sends idle duration, not input content. It does not send key values,
...
Bot waits 750 milliseconds so the connecting Mac can submit its first
...
- If no active Mac is available or primary delivery fails, Bot waits five
...
| Symptom                                   | Check                                                                                                                                                                |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No row is marked `active`                 | Confirm active computer detection is enabled, a native macOS node is connected, and `bot nodes describe --node <id>` shows `permissions.accessibility: true`.   |
| The wrong Mac remains active              | Use that Mac physically, wait for the coalescing window, then rerun `bot nodes status`. Synthetic computer-control actions do not count.                        |
