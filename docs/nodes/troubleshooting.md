...
```bash
bot nodes describe --node <idOrNameOrIp>
bot nodes canvas snapshot --node <idOrNameOrIp>
bot logs --follow
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`bot approvals get --node ...`), not in the gateway pairing record.
...
bot nodes status
bot approvals get --node <idOrNameOrIp>
bot approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
| `ACCESSIBILITY_REQUIRED`               | Grant Accessibility to the current Bot app bundle in macOS System Settings.                                                                                                        |
