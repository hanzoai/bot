...
summary: "Architecture and product boundary for supervising native Codex sessions from Bot."
read_when:
...
Codex supervision lets an Bot operator discover native Codex sessions and,
when safe, create a local branch through the normal Bot Chat surface.
Codex App Server remains the thread and model-loop owner. Bot supplies the
...
- native thread bindings to Bot sessions
...
Codex Desktop -> private stdio App Server -> user Codex home
                                             ^
Bot Codex plugin -> supervision App Server connection
...
Ordinary Bot Codex sessions -> managed agent-home stdio by default
  -> ordinary full harness threads -> Bot Chat and channel delivery
```
...
process-local; a thread unknown to Bot's supervision process is `notLoaded`
...
unbounded `thread/read`. Bot also rejects any serialized item page above
...
eventually discovered without importing them into Bot storage.

Catalog discovery is passive. Listing or reading metadata must not call
`thread/resume`, subscribe the Bot client to live thread requests, or
answer an approval.
...
```text
bot codex sessions [--search <text>] [--host <id>] [--limit <count>] [--cursor <cursor>] [--json] [gateway-options]
bot codex continue <thread-id> [--json] [gateway-options]
bot codex archive <thread-id> --confirm-no-other-runner [--json] [gateway-options]
```
...
2. Otherwise projects bounded user and assistant history through the source's
   ...
   Bot Chat and records a pending harness branch.
...
2. On that same connection, starts the canonical full Codex harness thread with
   `threadSource: "appServer"`, Bot's cwd, policy, config, environment, the
   full Bot harness tool surface, and exactly the model and provider
...
omit Bot model and provider overrides, so Codex restores the persisted pair.
If a separate native Codex control changes the canonical thread, Bot accepts
that native persisted selection. The outer Bot model and fallback chain
...
Deleting or resetting the Bot entry would otherwise discard the native
...
canonical harness thread on the first turn prevents Bot from becoming a
...
Atlas, or ChatGPT source remains eligible for both native and Bot catalogs.
...
known active Bot binding owner for the exact target or any non-archived
spawned descendant also rejects archive. Bot paginates Codex's experimental
...
`bot doctor --fix` migrates shipped `plugins.entries.codex-supervisor`
...
- audited handoff between Codex Desktop, CLI, and Bot
...
- Later resumes omit Bot model/provider overrides, preserve Codex's
  canonical persisted selection, accept separate native changes to that thread,
  and never substitute the outer Bot model or fallback chain.
