...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.bot`, `$BOT_STATE_DIR`, or active Bot state dirs.
...
- Worker must send completion/failure via `bot message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/bot`; use an isolated checkout.
...
6. Launch the worker in the isolated worktree, never the primary checkout. For Bot, the primary checkout under `~/Projects/bot` remains forbidden.
...
bot message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use bot system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t bot-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
