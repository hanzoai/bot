---
name: node-connect
description: "Diagnose Bot Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Bot is advertising that route, then fix pairing/auth.
...
- whether `bot devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `bot qr --json`. It uses the same setup-code payload Android scans.

```bash
bot config get gateway.mode
bot config get gateway.bind
bot config get gateway.tailscale.mode
bot config get gateway.remote.url
bot config get gateway.auth.mode
bot config get gateway.auth.allowTailscale
bot config get plugins.entries.device-pair.config.publicUrl
bot qr --json
bot devices list
bot nodes status
```

If this Bot instance is pointed at a remote gateway, also run:
...
If `bot qr --json` says `Gateway is only bound to loopback`:
...
- `bot devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run bot qr again, rescan, then approve the pending device pairing.`
