...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${BOT_CONFIG_PATH:-${BOT_STATE_DIR:-$HOME/.bot}/bot.json}"
...
```text
${BOT_STATE_DIR:-$HOME/.bot}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with bot message send if route provided.
```
