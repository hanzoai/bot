...
1. Download the runtime for your OS (extracts into `$BOT_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.bot/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$BOT_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.bot/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Bot config file (`$BOT_CONFIG_PATH`, default `~/.hanzoai/bot.json`):
