...
describe("irc setup", () => {
  ...
  it("validates required input and applies normalized account config", () => {
    ...
    expect(
      validateInput({
        input: { host: "irc.libera.chat", nick: "bot" },
      } as never),
    ).toBeNull();

    expect(
      validateInput({
        input: { host: "irc.libera.chat", nick: "bot", port: "+07000" },
      } as never),
    ).toBeNull();

    expect(
      validateInput({
        input: { host: "irc.libera.chat", nick: "bot", port: "7000x" },
      } as never),
    ).toBe("IRC port must be between 1 and 65535.");

    expect(
      validateInput({
        input: { host: "irc.libera.chat", nick: "bot", port: "70000" },
      } as never),
    ).toBe("IRC port must be between 1 and 65535.");
