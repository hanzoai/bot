...
describe("xai tts", () => {
  ...
  describe("listXaiTtsVoices", () => {
    it("maps the authenticated catalog and sends the expected request", async () => {
      vi.stubEnv("BOT_VERSION", "2026.7.9");
      ...
      expect(headers.get("user-agent")).toBe("bot/2026.7.9");
      vi.unstubAllEnvs();
    });
  ...
  describe("xaiTTS diagnostics", () => {
    ...
    it("sends an bot User-Agent on xAI TTS requests", async () => {
      ...
      const headers = new Headers(init?.headers ?? {});
      expect(headers.get("user-agent")).toBe("bot/2026.3.22");
