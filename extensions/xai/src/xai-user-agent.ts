// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatBotUserAgent`.

import { BOT_VERSION as PACKAGE_VERSION } from "bot/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "bot";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.BOT_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.BOT_SERVICE_VERSION) ??
...
// an empty record so the bot identity is not forwarded to the proxy.
