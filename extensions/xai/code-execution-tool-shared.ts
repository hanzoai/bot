...
export function buildMissingCodeExecutionApiKeyPayload() {
  return {
    ...
    message:
      "code_execution needs xAI credentials. Run `bot onboard --auth-choice xai-oauth` to sign in with Grok, run `bot onboard --auth-choice xai-api-key`, set `XAI_API_KEY` in the Gateway environment, or configure `plugins.entries.xai.config.webSearch.apiKey`.",
