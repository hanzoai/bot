...
const testStateEnv: NodeJS.ProcessEnv = {
  ...process.env,
  BOT_STATE_DIR: fs.mkdtempSync(
    path.join(resolvePreferredBotTmpDir(), "bot-zalo-media-"),
  ),
};
...
describe("zalo outbound hosted media", () => {
  ...
  it("loads outbound media under Bot control and returns a hosted URL", async () => {
