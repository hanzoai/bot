...
import { normalizeBotRouterRootUrl } from "./provider-catalog.js";

const CLAWROUTER_USAGE_RESPONSE_MAX_BYTES = 1024 * 1024;

type BotRouterUsageFetchGuard = typeof fetchWithSsrFGuard;

type BotRouterBudget = {
  configured?: unknown;
  ledger?: unknown;
  windowKey?: unknown;
  limitMicros?: unknown;
...
type BotRouterUsagePayload = {
  budget?: BotRouterBudget;
  usage?: {
    summary?: {
...
function buildSummary(payload: BotRouterUsagePayload): string | undefined {
...
async function readBotRouterUsagePayload(
  response: Response,
  timeoutMs: number,
): Promise<BotRouterUsagePayload> {
  const buffer = await readResponseWithLimit(response, CLAWROUTER_USAGE_RESPONSE_MAX_BYTES, {
    chunkTimeoutMs: timeoutMs,
    onOverflow: ({ maxBytes }) => new Error(`BotRouter usage response exceeds ${maxBytes} bytes`),
    onIdleTimeout: ({ chunkTimeoutMs }) =>
      new Error(`BotRouter usage response stalled: no data received for ${chunkTimeoutMs}ms`),
  });
  return JSON.parse(
    new TextDecoder("utf-8", { fatal: true }).decode(buffer),
  ) as BotRouterUsagePayload;
}

export async function fetchBotRouterUsage(params: {
  token: string;
  baseUrl?: string;
  timeoutMs: number;
  /** Test-only seam; production keeps the shared SSRF guard owning transport. */
  fetchGuard?: BotRouterUsageFetchGuard;
  ...
  const rootUrl = normalizeBotRouterRootUrl(params.baseUrl);
  ...
  const { response, release } = await fetchGuard(
    withTrustedEnvProxyGuardedFetchMode({
      ...
      auditContext: "botrouter.usage",
    }),
  );
  try {
    if (!response.ok) {
      await response.body?.cancel().catch(() => undefined);
      throw new Error(`BotRouter usage request failed (HTTP ${response.status})`);
    }
    const payload = await readBotRouterUsagePayload(response, params.timeoutMs);
