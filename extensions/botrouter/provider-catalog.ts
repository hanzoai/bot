// BotRouter provider catalog maps credential-scoped routes to Bot transports.
...
const CLAWROUTER_DEFAULT_BASE_URL = "https://botrouter.bot.ai";

const PROVIDER_ID = "botrouter";
const CATALOG_CACHE_TTL_MS = 60_000;
const ROUTE_METADATA_KEY = "botrouterRoute";
...
function readCatalogRows(body: unknown): readonly unknown[] {
  ...
  if (!Array.isArray(providers)) {
    throw new Error("BotRouter catalog response must contain providers[]");
  }
  return providers;
}
...
export function normalizeBotRouterRootUrl(baseUrl: string | undefined): string {
  const normalized = trimTrailingSlashes(baseUrl?.trim() || CLAWROUTER_DEFAULT_BASE_URL);
  return normalized.endsWith("/v1") ? normalized.slice(0, -3) : normalized;
}

export function normalizeBotRouterApiBaseUrl(baseUrl: string | undefined): string {
  return `${normalizeBotRouterRootUrl(baseUrl)}/v1`;
}
...
export async function buildBotRouterProviderConfig(params: {
  apiKey: string;
  ...
  const rows = await getCachedLiveProviderModelRows({
    ...
    auditContext: "botrouter-model-discovery",
  });
...
export function normalizeBotRouterResolvedModel(
...
export function prepareBotRouterRequestModel(model: ProviderRuntimeModel): ProviderRuntimeModel {
  const route = readRouteMetadata(model.params);
  if (!route) {
    return model;
  }
