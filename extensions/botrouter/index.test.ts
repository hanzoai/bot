...
vi.mock("bot/plugin-sdk/provider-auth-runtime", () => providerAuthRuntimeMocks);
...
describe("BotRouter plugin", () => {
    ...
    void wrapped?.(
      {
        ...
        params: {
          botrouterRoute: {
            api: "anthropic-messages",
            baseUrl: "https://botrouter.example/v1/native/anthropic",
  ...
  it("attaches bounded attribution without overriding configured metadata", () => {
    ...
    const wrapped = wrapBotRouterProviderStream({
      provider: "botrouter",
    ...
    void wrapped?.(
      {
        ...
        id: "openai/gpt-5.5",
        headers: {
          "x-botrouter-client": "managed-bot",
    ...
    expect(calls[0]?.headers?.["X-BotRouter-Session-Id"]).toHaveLength(256);
    expect(calls[0]?.headers?.["X-BotRouter-Session-Id"]).toMatch(/~[a-f0-9]{16}$/u);
  ...
  it("sanitizes Unicode attribution to distinct printable ASCII ByteStrings", () => {
    ...
    for (const headers of captured) {
      expect(headers["X-BotRouter-Agent-Id"]).toMatch(/^agent-___-id~[a-f0-9]{16}$/u);
      expect(headers["X-BotRouter-Session-Id"]).toMatch(/^session-___-id~[a-f0-9]{16}$/u);
      expect(() => new Headers(headers)).not.toThrow();
    }
    expect(captured[0]?.["X-BotRouter-Agent-Id"]).not.toBe(captured[1]?.["X-BotRouter-Agent-Id"]);
    expect(captured[0]?.["X-BotRouter-Session-Id"]).not.toBe(
      captured[1]?.["X-BotRouter-Session-Id"],
    );
  });
  ...
  it("keeps encoded and lone-surrogate attribution ids distinct", () => {
    ...
    const captureAgentId = (agentId: string) => {
      const wrapped = wrapBotRouterProviderStream({
        ...
        agentId,
        streamFn: ((model) => {
          captured.push(model.headers?.["X-BotRouter-Agent-Id"] ?? "");
          return {} as ReturnType<StreamFn>;
        }) satisfies StreamFn,
      } as never);
      void wrapped?.(
    ...
    for (const value of captured) {
      ...
      expect(() => new Headers({ "X-BotRouter-Agent-Id": value })).not.toThrow();
    }
  });
  ...
  it("resolves managed secret refs before scoped discovery", async () => {
    ...
    if (!result || !("provider" in result)) {
      throw new Error("expected BotRouter catalog provider result");
    }
  ...
  it("resolves configured catalog models through a stored auth profile", async () => {
    ...
    const context = {
      ...
      authProfileId: "botrouter-profile",
    ...
    expect(provider?.resolveDynamicModel?.(context as never)).toMatchObject({
      ...
      provider: "botrouter",
      api: "openai-responses",
      baseUrl: "https://botrouter.bot.ai/v1",
      params: {
        botrouterRoute: {
          api: "openai-responses",
          baseUrl: "https://botrouter.bot.ai/v1",
