...
import { prepareBotRouterRequestModel } from "./provider-catalog.js";

const ENV_API_KEY_MARKER = "CLAWROUTER_API_KEY";
const ATTRIBUTION_VALUE_MAX_LENGTH = 256;
const REQUEST_ID_MAX_LENGTH = 128;
const CLIENT_HEADER = "X-BotRouter-Client";
const AGENT_HEADER = "X-BotRouter-Agent-Id";
const SESSION_HEADER = "X-BotRouter-Session-Id";
...
function withBotRouterHeaders(
  ...
  setHeaderDefault(next, CLIENT_HEADER, "bot");
...
function createBotRouterStreamWrapper(ctx: ProviderWrapStreamFnContext): StreamFn | undefined {
  ...
  return (model, context, options) => {
    ...
    const preparedModel = prepareBotRouterRequestModel(model);
    const hasExplicitRequestId =
      findHeader(options?.headers ?? {}, REQUEST_ID_HEADER) !== undefined;
    return underlying(
      {
        ...preparedModel,
        headers: withBotRouterHeaders(preparedModel.headers, {
          agentId: ctx.agentId,
...
export function wrapBotRouterProviderStream(
  ctx: ProviderWrapStreamFnContext,
): StreamFn | undefined {
  return createBotRouterStreamWrapper(ctx);
}
