...
const CATALOG = {
  version: "botrouter.client-catalog.v1",
  providers: [
    {
      id: "openai",
      displayName: "OpenAI",
...
function buildFetchGuard(catalog: unknown = CATALOG): {
  ...
  const fetchGuardMock: MockedFunction<LiveModelCatalogFetchGuard> = vi.fn(async () => ({
    ...
    finalUrl: "https://botrouter.example/v1/catalog",
    release: async () => undefined,
  }));
  return { fetchGuard: fetchGuardMock, fetchGuardMock };
}

describe("BotRouter provider catalog", () => {
  beforeEach(() => {
    clearLiveCatalogCacheForTests();
  });

  it("maps every supported catalog protocol to its Bot transport", async () => {
    const { fetchGuard, fetchGuardMock } = buildFetchGuard();
    const provider = await buildBotRouterProviderConfig({
      apiKey: "botrouter-test-key",
    ...
    expect(provider.models.find((model) => model.id === "google/gemini-3.5-flash")).toMatchObject({
      ...
      baseUrl: "https://botrouter.example/v1/native/google-gemini/v1beta",
    });
  ...
  it("rewrites only native protocol model ids at the request boundary", async () => {
    ...
    expect(prepareBotRouterRequestModel(normalized as ProviderRuntimeModel)).toMatchObject({
      id: "claude-sonnet-4-6",
      params: undefined,
    });

    const openai = provider.models.find((model) => model.id === "openai/gpt-5.5");
    const normalizedOpenAi = normalizeBotRouterResolvedModel({
      ...openai,
      baseUrl: provider.baseUrl,
      provider: "botrouter",
    } as ProviderRuntimeModel);
    expect(prepareBotRouterRequestModel(normalizedOpenAi as ProviderRuntimeModel).id).toBe(
      "openai/gpt-5.5",
    );
  });
  ...
  it("caches catalog rows per credential scope", async () => {
    ...
    await buildBotRouterProviderConfig(params);
    await buildBotRouterProviderConfig(params);

    expect(fetchGuardMock).toHaveBeenCalledOnce();
    const headers = fetchGuardMock.mock.calls[0]?.[0].init?.headers;
    expect(headers).toBeInstanceOf(Headers);
    expect((headers as Headers).get("authorization")).toBe("Bearer botrouter-test-key");
  });

  it("does not advertise Gemini without a streaming route", async () => {
    const catalog = structuredClone(CATALOG);
    const geminiProvider = expectDefined(catalog.providers[3], "Gemini BotRouter provider");
    geminiProvider.routes = geminiProvider.routes.filter(
      (route) => !route.path.includes(":streamGenerateContent"),
    );
    expectDefined(geminiProvider.models[0], "Gemini BotRouter model").capabilities = [
      "llm.generate",
    ];
    const provider = await buildBotRouterProviderConfig({
      apiKey: "botrouter-test-key",
