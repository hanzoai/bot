...
type BotRouterUsageFetchGuard = NonNullable<
  Parameters<typeof fetchBotRouterUsage>[0]["fetchGuard"]
>;

function mockFetchGuard(response: Response): MockedFunction<BotRouterUsageFetchGuard> {
...
describe("BotRouter usage", () => {
  it("maps the managed monthly budget and usage totals", async () => {
    ...
    const snapshot = await fetchBotRouterUsage({
      token: "proxy-key",
    ...
    expect(fetchGuard).toHaveBeenCalledWith(
      expect.objectContaining({
        url: "https://botrouter.example/v1/usage",
  ...
  it("cancels non-OK usage response body before throwing", async () => {
    ...
    ).rejects.toThrow("BotRouter usage request failed (HTTP 403)");
  ...
  it("observes peer closure when a real loopback server returns non-OK", async () => {
    ...
    ).rejects.toThrow("BotRouter usage request failed (HTTP 503)");
  ...
  it("bounds successful usage response bodies", async () => {
    ...
    ).rejects.toThrow("BotRouter usage response exceeds");
  });
