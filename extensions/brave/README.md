...
Official Brave Search provider plugin for Bot.
...
```bash
bot plugins install @hanzo/bot-brave-plugin
```
...
```bash
bot config set plugins.entries.brave.enabled true
bot config set tools.web.search.provider brave
```
