...
import { resolveMemoryBackendConfig } from "bot/plugin-sdk/memory-core-host-runtime-files";
import { normalizePluginsConfig } from "bot/plugin-sdk/plugin-config-runtime";
import {
  definePluginEntry,
  type AnyAgentTool,
  type BotPluginApi,
...
function getToolConfig(options: MemoryToolOptions): BotConfig | undefined {
...
function createLazyStandingIntentTool(ctx: BotPluginToolContext): AnyAgentTool | null {
...
function configuredMemoryAgentIds(config: BotConfig): string[] {
...
function registerMemoryManagerWarmup(
  ...
  api.on("gateway_start", (_event, ctx) => {
    const config = (api.runtime.config?.current?.() ?? ctx.config ?? api.config) as BotConfig;
  ...
  register(api) {
    ...
    api.on("before_prompt_build", async (event, ctx) => {
      ...
      try {
        ...
        const config = (api.runtime.config?.current?.() ?? api.config) as BotConfig;
        const { sessionAgentId: agentId } = resolveSessionAgentIds({
          sessionKey: ctx.sessionKey,
          config,
          agentId: ctx.agentId,
