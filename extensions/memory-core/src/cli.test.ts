...
vi.mock("./cli.host.runtime.js", async () => {
  const [
    ...
    import("bot/plugin-sdk/memory-core-host-runtime-cli"),
    import("bot/plugin-sdk/memory-core-host-runtime-core"),
    import("bot/plugin-sdk/memory-core-host-runtime-files"),
  ]);
  return {
    defaultRuntime,
    formatErrorMessage,
...
let defaultRuntime: typeof import("bot/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("bot/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("bot/plugin-sdk/memory-core-host-runtime-cli").setVerbose;
let fixtureRoot = "";
...
beforeAll(async () => {
  ...
  } = await import("bot/plugin-sdk/memory-core-host-runtime-cli");
  defaultRuntime = loadedDefaultRuntime;
  isVerbose = loadedIsVerbose;
  setVerbose = loadedSetVerbose;
...
describe("memory cli", () => {
  ...
  it("prints index identity mismatch reasons", async () => {
    ...
    expectLogged(log, "Fix: Run: bot memory status --index --agent main");
    expect(close).toHaveBeenCalled();
  });
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      const log = spyRuntimeLogs(defaultRuntime);
      await runMemoryCli(["status"]);
      expectLogged(log, "Fix: bot memory status --fix --agent main");
      ...
      await runMemoryCli(["status", "--fix"]);
      expectNotLogged(log, "Fix: bot memory status --fix --agent main");
    });
  });
  ...
  it("writes grounded rem backfill entries into DREAMS.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(dreams).toContain("bot:dreaming:backfill-entry");
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## Bot / runtime / workflow preferences and corrections",
    ...
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "*January 1, 2025*",
          "",
          "<!-- bot:dreaming:backfill-entry day=2025-01-01 source=memory/2025-01-01.md -->",
          "",
          "What Happened",
          "1. Remove this entry.",
          "",
          "<!-- bot:dreaming:diary:end -->",
          "",
        ].join("\n"),
        "utf-8",
      );
  ...
  it("applies top promote candidates into MEMORY.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(memoryText).toContain("bot-memory-promotion:");
