...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "bot/plugin-sdk/memory-core-host-engine-qmd";
import type {
...
vi.mock("bot/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): BotConfig {
  return {
    memory: {
      search: {
        provider: "openai",
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("bot.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bot-qmd-workspace-"));
