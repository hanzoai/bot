...
describe("standing intents", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "bot-standing-intents-")),
    );
    vi.stubEnv("BOT_STATE_DIR", stateDir);
    await fs.mkdir(path.dirname(resolveBotAgentSqlitePath({ agentId: "main" })), {
      recursive: true,
    });
  });
  ...
  it("injects owner-created intents and skips rows without a known creator", async () => {
    ...
    const db = openBotAgentDatabase({ agentId: "main" }).db;
