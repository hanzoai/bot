import { readPositiveIntegerParam, readStringParam } from "bot/plugin-sdk/channel-actions";
...
import { resolveSessionAgentIds } from "bot/plugin-sdk/memory-core-host-runtime-core";
...
function resolveExecutionContext(api: BotPluginApi, agentId: string) {
  const config = api.runtime.config.current() as BotConfig;
...
export function registerSessionBackfillGatewayMethods(api: BotPluginApi): void {
