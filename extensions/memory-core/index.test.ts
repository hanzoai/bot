...
import type { BotPluginApi, BotPluginCommandDefinition } from "bot/plugin-sdk/core";
import type { MemoryPluginRuntime } from "bot/plugin-sdk/memory-core-host-runtime-core";
...
describe("memory-core plugin runtime registration", () => {
  ...
  it("registers the standing-intent tool and deterministic prompt hook", () => {
    ...
    plugin.register(
      createTestPluginApi({
        ...
        registerTool(_factory, options?: Parameters<BotPluginApi["registerTool"]>[1]) {
  ...
  it("hides intent create, list, and cancel from non-owner turns", () => {
    let intentFactory:
      | ((ctx: { config?: BotConfig; senderIsOwner?: boolean }) => unknown)
      | undefined;
    plugin.register(
      createTestPluginApi({
        config: {},
  ...
  it("warms each configured memory manager at gateway start and logs failures at debug", async () => {
    const gatewayStartHandlers: Array<(event: unknown, ctx: { config: BotConfig }) => void> =
      [];
      ...
      on(hookName, handler) {
        if (hookName === "gateway_start") {
          gatewayStartHandlers.push(
            handler as unknown as (event: unknown, ctx: { config: BotConfig }) => void,
          );
        }
      },
    });
