...
async function withIsolatedHome<T>(run: () => Promise<T>): Promise<T> {
  ...
  return await withTempDir("bot-feishu-media-", async (tempHome) => {
    try {
...
describe("sendMediaFeishu msg_type routing", () => {
  ...
  afterAll(() => {
    ...
    vi.doUnmock("bot/plugin-sdk/media-runtime");
    vi.resetModules();
  });
...
describe("saveMessageResourceFeishu", () => {
  ...
  it("saves message resource streams directly to the media store", async () => {
    await withIsolatedHome(async () => {
      ...
      expect(result.saved.path).toContain(`${path.sep}.bot${path.sep}media${path.sep}inbound`);
