...
} from "bot/plugin-sdk/channel-inbound-debounce";
import { hasControlCommand, isControlCommandMessage } from "bot/plugin-sdk/command-detection";
...
async function resolveNonBotReaction(params?: { cfg?: BotConfig; uuid?: () => string }) {
  return await resolveReactionSyntheticEvent({
    cfg: params?.cfg ?? cfg,
    accountId: "default",
...
function buildDebounceConfig(): BotConfig {
  return {
    messages: {
      inbound: {
        debounceMs: 0,
...
describe("Feishu inbound debounce regressions", () => {
  ...
  it("passes prefetched botName through to handleFeishuMessage", async () => {
    ...
    vi.spyOn(dedup, "hasProcessedFeishuMessage").mockResolvedValue(false);
    const onMessage = await setupDebounceMonitor({ botName: "Bot Bot" });
    ...
    const firstParams = mockCallAt(handleFeishuMessageMock, 0, "Feishu message dispatch")[0] as
      | { botName?: string }
      | undefined;
    expect(firstParams?.botName).toBe("Bot Bot");
  });
