...
function areAnyFeishuReactionActionsEnabled(cfg: BotConfig): boolean {
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("bot/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("bot/plugin-sdk/config-mutation");
