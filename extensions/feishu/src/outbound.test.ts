...
describe("feishuOutbound.sendPayload native cards", () => {
  ...
  async function createTmpImage(ext = ".png"): Promise<{ dir: string; file: string }> {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-feishu-payload-"));
    const file = path.join(dir, `sample${ext}`);
    await fs.writeFile(file, "image-data");
    return { dir, file };
  }
...
describe("feishuOutbound comment-thread routing", () => {
  ...
  it.each([
    ["local path", path.join(os.tmpdir(), "bot-feishu-comment-local-voice.mp3")],
    ["loopback URL", "http://127.0.0.1:3000/tmp/bot-voice.mp3"],
...
describe("feishuOutbound.sendMedia replyToId forwarding", () => {
  ...
  it.each([
    ["local path", path.join(os.tmpdir(), "bot-feishu-local-voice.mp3")],
    ["file URL", "file:///tmp/bot-feishu-local-voice.mp3"],
    ["relative path", "./outbound/bot-feishu-local-voice.mp3"],
    ["loopback URL", "http://127.0.0.1:3000/tmp/bot-voice.mp3"],
    ["localhost URL", "https://localhost/tmp/bot-voice.mp3"],
    ["private-DNS URL", "https://files.example.test/bot-voice.mp3"],
    ["credentialed URL", "https://user@example.com/bot-voice.mp3"],
