...
import type { AnyAgentTool, BotConfig } from "bot/plugin-sdk/plugin-entry";
import { readRegularFile } from "bot/plugin-sdk/security-runtime";
...
async function writeBase64ToTempFile(params: { base64: string; ext: string }): Promise<string> {
  const dir = resolvePreferredBotTmpDir();
  await fs.mkdir(dir, { recursive: true, mode: 0o700 });
  const ext = `.${normalizeCanvasSnapshotFileExtension(params.ext)}`;
  const filePath = path.join(dir, `bot-canvas-snapshot-${randomUUID()}${ext}`);
