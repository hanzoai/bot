...
import { asOptionalRecord as readRecord } from "bot/plugin-sdk/string-coerce-runtime";
...
export function migrateLegacyCanvasHostConfig(config: BotConfig): {
  config: BotConfig;
  changes: string[];
} | null {
  ...
  const next = { ...config, plugins } as BotConfig & { canvasHost?: unknown };
