---
name: canvas
description: "Present HTML on connected Bot node canvases, navigate/eval/snapshot, and debug canvas host URLs."
...
Active config: `$BOT_CONFIG_PATH` or `~/.hanzoai/bot.json`.
...
4. Present the hosted URL: `/__bot__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__bot__/canvas/index.html
http://<gateway-host>:<gateway.port>/__bot__/canvas/games/snake.html
```

Path mapping:

- `/__bot__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__bot__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
