...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleBotDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Bot devices detected for ${params.auth.userId}: ${deviceHealth.staleBotDevices.map((device) => device.deviceId).join(", ")}. Run 'bot matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
