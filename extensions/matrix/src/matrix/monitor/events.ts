...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'bot matrix verify status --verbose --account ${accountId}' and 'bot matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Bot runtime is using the same account. " +
    `Check 'bot matrix verify status --verbose --account ${accountId}' and 'bot matrix devices list --account ${accountId}'.`
  );
}
