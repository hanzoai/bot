...
function createDeps(
  ...
  return {
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleBotDevices: [] as MatrixManagedDeviceInfo[],
      currentBotDevices: [] as MatrixManagedDeviceInfo[],
    })),
...
describe("runMatrixStartupMaintenance", () => {
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      currentDeviceId: null,
      staleBotDevices: [
        { deviceId: "DEV123", displayName: "Bot Device", current: false },
      ],
      currentBotDevices: [],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Bot devices detected for @bot:example.org: DEV123. Run 'bot matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'bot matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
