...
import { BOT_CODEX_CONFIG_ARG } from "./codex-adapter.js";
...
const DOCUMENTED_BOT_BRIDGE_COMMAND =
  "env BOT_HIDE_BANNER=1 BOT_SUPPRESS_NOTES=1 bot acp --url ws://127.0.0.1:18789 --token-file ~/.bot/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @agentclientprotocol/codex-acp@1.1.2";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/bot/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-close ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "bot" ? "bot acp" : agentName),
        list: () => ["codex", "bot"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("adds the Bot session key to both managed tools MCP bridges", () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    const { runtime } = makeRuntime(baseStore, {
      ...
      botToolsMcpBridgeEnabled: true,
      mcpServers: [
        {
          name: "bot-plugin-tools",
          command: "node",
        ...
        },
        {
          name: "bot-tools",
          command: "node",
          args: ["dist/mcp/bot-tools-serve.js"],
          env: [],
        },
      ],
    });
    ...
    expect(readScopedMcpEnv("agent:worker:main", "bot-plugin-tools")).toContainEqual({
      name: "BOT_TOOLS_MCP_AGENT_SESSION_KEY",
      value: "agent:worker:main",
    });
    expect(readScopedMcpEnv("agent:research:main", "bot-tools")).toContainEqual({
      name: "BOT_TOOLS_MCP_AGENT_SESSION_KEY",
      value: "agent:research:main",
    });
  });

  it("keeps managed Bot tools MCP delegates reachable for fresh sessions", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("uses the no-MCP delegate for startup probes when the Bot tools bridge is enabled", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("normalizes Bot Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("strips the Bot Anthropic provider prefix for Claude ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("keeps Claude ACP model ids intact after stripping the Bot provider prefix", () => {
  ...
  it.each([
    ...
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bot-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      botGatewayInstanceId: "gateway-test",
      botProcessLeaseStore: leaseStore.store,
      botWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        botLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("disables delegate prompt timeout for Bot-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(
      ...
      `npx @agentclientprotocol/codex-acp@1.1.2 ${BOT_CODEX_CONFIG_ARG} '{"model":"gpt-5.4","model_reasoning_effort":"medium"}'`,
    );
    expect(testing.isCodexAcpCommand("bot acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/bot/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/bot/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("bot acp")).toBe(false);
  ...
  it("releases managed Bot tools MCP delegates after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("cleans up Bot-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/bot/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
        ...
        botProcessCleanup: {
          listProcesses: vi.fn(async () => [
            ...
            },
            {
              ...
              ppid: 900,
              command:
                "node /tmp/bot/plugin-runtime-deps/node_modules/@agentclientprotocol/codex-acp/dist/index.js",
            },
          ]),
  ...
  it("persists ACPX process lease identity for later wrapper reconnects", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/bot/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain(BOT_ACPX_LEASE_ID_ARG);
    expect(launchCommands[0]).toContain(BOT_GATEWAY_INSTANCE_ID_ARG);
    expect(savedRecords[0]?.agentCommand).toBe(launchCommands[0]);
    expect(savedRecords[0]?.botGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.botLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("keeps reusable persistent ACP launch commands stable across ensures", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-existing ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
  ...
  it("recreates a missing sidecar with the persisted lease identity", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-missing ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  it("does not reuse commands leased by another gateway instance", async () => {
    const foreignCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-foreign ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-foreign`;
    let savedRecord: Record<string, unknown> = {
      name: "agent:codex:acp:binding:test",
      acpxRecordId: "record-1",
      acpSessionId: "session-1",
    ...
    expect(resolvedCommands[0]).toContain(`${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`);
    expect(savedRecord.pid).toBe(888);
    expect(leaseStore.leases.size).toBe(1);
  });

  it("rejects reconnect operations for commands leased by another gateway", async () => {
    const foreignCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-foreign-operation ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-foreign`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: foreignCommand,
  ...
  it("adopts legacy persistent commands before their next reconnect", async () => {
    ...
    expect(savedRecord.agentCommand).toContain(BOT_ACPX_LEASE_ID_ARG);
    expect(savedRecord.agentCommand).toContain(BOT_GATEWAY_INSTANCE_ID_ARG);
    ...
    expect(lease?.leaseId).toBe(savedRecord.botLeaseId);
  ...
  it("restores a pending process lease before runTurn reconnects", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-turn-reconnect ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("restores a missing sidecar from the persisted lease PID", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-live-reconnect ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("retires runTurn pending leases when handle resolution fails", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-turn-resolution ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    let loads = 0;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => {
        loads += 1;
  ...
  it("restores a pending process lease before startTurn reconnects", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-start-reconnect ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("tracks control-operation reconnects without leaving pending leases", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-control-reconnect ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("preserves a promoted PID when the session record save fails", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-partial-save ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  it("keeps a shared pending lease until the last concurrent operation finishes", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-concurrent-operations ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("retires an old lease after the session record switches identity", async () => {
    const oldCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-old-operation ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const newCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-new-session ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    let savedRecord: Record<string, unknown> = {
      name: "agent:codex:acp:binding:test",
  ...
  it("keeps launch ownership while a concurrent reconnect operation is active", async () => {
    ...
    const leaseId = String(savedRecord?.botLeaseId);
  ...
  it("serializes last-owner retirement with the next lease acquisition", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-retirement-race ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("rechecks a reusable sidecar after the prior owner retires it", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-reusable-race ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const sessionKey = "agent:codex:acp:binding:test";
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: sessionKey,
  ...
  it("retires close pending leases when cleanup fails", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-close-failure ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("preserves PID-bearing close leases when cleanup fails", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-close-live ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("keeps close leases retryable when process listing is unavailable", async () => {
    const leasedCommand = `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-close-process-list ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        name: "agent:codex:acp:binding:test",
        agentCommand: leasedCommand,
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    expect(loadedRecord?.botGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.botLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.botGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.botLeaseId).toBe("lease-current");
  });
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-old ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} lease-current ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 920,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${BOT_ACPX_LEASE_ID_ARG} other-lease ${BOT_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ]),
          killProcess: vi.fn((pid, signal) => {
            killed.push({ pid, signal });
  ...
  it("routes bot ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes non-bot sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for bot sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:bot:acp:test",
      backend: "acpx",
      runtimeSessionName: "bot-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the bot agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "bot" ? "codex" : agentName),
        list: () => ["codex", "bot"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the bot bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "bot acp" : agentName),
        list: () => ["codex", "bot"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped bot bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "bot" ? DOCUMENTED_BOT_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "bot"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node bot entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "bot" ? "env BOT_HIDE_BANNER=1 node bot.mjs acp" : agentName,
        list: () => ["codex", "bot"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:bot:acp:test",
        agentCommand: DOCUMENTED_BOT_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:bot:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:bot:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to bot bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
