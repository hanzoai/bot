...
import { formatPluginConfigIssue } from "bot/plugin-sdk/extension-shared";
...
function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "bot.plugin.json")) &&
...
function resolveAcpxPluginRootFromBotLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Bot layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromBotLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveBotRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const botRoot = resolveBotRoot(pluginRoot);
  const distEntry = path.join(botRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(botRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveBotToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const botRoot = resolveBotRoot(pluginRoot);
  const distEntry = path.join(botRoot, "dist", "mcp", "bot-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(botRoot, "src", "mcp", "bot-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.botToolsMcpBridge && resolved[ACPX_BOT_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_BOT_TOOLS_MCP_SERVER_NAME} is reserved when botToolsMcpBridge=true`,
    );
  }
  ...
  if (params.botToolsMcpBridge) {
    resolved[ACPX_BOT_TOOLS_MCP_SERVER_NAME] = resolveBotToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
/** Convert Bot MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const botToolsMcpBridge = normalized.botToolsMcpBridge === true;
