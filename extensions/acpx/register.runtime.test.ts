...
vi.mock("bot/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.BOT_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.BOT_SKIP_ACPX_RUNTIME;
  } else {
    process.env.BOT_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/bot-acpx-register-test",
    stateDir: "/tmp/bot-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
