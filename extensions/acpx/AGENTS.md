...
The ACPX extension is a thin Bot wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `bot/acpx`, not in this extension.
...
Use this flow when Bot needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `bot/acpx` repo first.
2. In Bot, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Bot and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Bot ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Bot-specific glue, move it to `bot/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
