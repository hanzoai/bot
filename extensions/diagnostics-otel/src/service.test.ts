...
vi.mock("bot/plugin-sdk/fetch-runtime", () => ({
...
const ORIGINAL_BOT_OTEL_PRELOADED = process.env.BOT_OTEL_PRELOADED;
...
afterAll(() => {
  ...
  vi.doUnmock("bot/plugin-sdk/fetch-runtime");
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.BOT_OTEL_PRELOADED;
  ...
  afterEach(async () => {
    ...
    resetDiagnosticEventsForTest();
    if (ORIGINAL_BOT_OTEL_PRELOADED === undefined) {
      delete process.env.BOT_OTEL_PRELOADED;
    } else {
      process.env.BOT_OTEL_PRELOADED = ORIGINAL_BOT_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["bot.provider"]).toBe("openai");
    for (const key of [
      "bot.callId",
      "bot.call_id",
      "bot.chatId",
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("bot.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "bot.channel": "telegram",
      "bot.webhook": "telegram-post",
    });
    expect(
    ...
    expect(telemetryState.counters.get("bot.message.queued")?.add).toHaveBeenCalledWith(1, {
      "bot.channel": "telegram",
      "bot.source": "telegram",
    });
    expect(telemetryState.histograms.get("bot.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("bot.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "bot.channel": "telegram",
      "bot.source": "telegram",
    });
    expect(telemetryState.histograms.get("bot.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "bot.lane": "main",
    });
    expect(telemetryState.counters.get("bot.message.processed")?.add).toHaveBeenCalledWith(1, {
      "bot.channel": "telegram",
      "bot.outcome": "completed",
    });
    expect(telemetryState.counters.get("bot.message.received")?.add).toHaveBeenCalledWith(1, {
      "bot.channel": "telegram",
      "bot.source": "webhook",
    });
    expect(telemetryState.counters.get("bot.message.received")?.add).toHaveBeenCalledWith(1, {
    ...
    expect(
      telemetryState.counters.get("bot.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("bot.message.dispatch.completed")?.add,
    ...
    expect(
      telemetryState.histograms.get("bot.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "bot.channel": "telegram",
    ...
    expect(telemetryState.histograms.get("bot.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "bot.lane": "main",
      },
    );
    expect(telemetryState.counters.get("bot.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("bot.session.stuck")?.add).toHaveBeenCalledWith(1, {
    ...
    expect(telemetryState.counters.get("bot.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "bot.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("bot.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "bot.agent": "agent.default",
        "bot.channel": "telegram",
        "bot.trigger": "user",
      },
    );
    ...
    const webhookSpanOptions = startedSpanOptions("bot.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("bot.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("bot.message.processed");
    expect(messageSpanOptions?.attributes?.["bot.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["bot.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["bot.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("bot.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("bot.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.BOT_OTEL_PRELOADED = "1";
    const { service, ctx } = await startOtelService({ traces: true, metrics: true, logs: true });
    ...
    const runDurationRecordCall = lastHistogramRecord("bot.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["bot.provider"]).toBe("openai");
    expect(runDurationAttributes?.["bot.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("bot.run");
    expect(runSpanOptions?.attributes?.["bot.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("bot.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "bot.exporter": "diagnostics-otel",
      "bot.signal": "logs",
      "bot.status": "started",
      "bot.reason": "configured",
    });
  ...
  test("exports trusted security events as bounded OTLP logs", async () => {
    ...
    expect(emitCall.body).toBe("bot.security.event");
    expect(emitCall.severityText).toBe("WARN");
    expect(emitCall.severityNumber).toBe(13);
    expect(emitCall.attributes).toMatchObject({
      "bot.security.event_id": "security-event-1",
      "bot.security.category": "tool",
      "bot.security.action": "tool.execution.blocked",
      "bot.security.outcome": "denied",
      "bot.security.severity": "medium",
      "bot.security.reason": "tools.deny",
      "bot.security.actor.kind": "agent",
      "bot.security.actor.id_hash": "agent-hash-1",
      "bot.security.actor.role": "operator",
      "bot.security.actor.scopes": "operator.read,operator.approvals",
      "bot.security.target.kind": "plugin",
      "bot.security.target.name": "@acme/security-event-plugin",
      "bot.security.target.owner": "plugin-installer",
      "bot.security.policy.id": "tools.exec",
      "bot.security.policy.decision": "deny",
      "bot.security.policy.reason": "allowlist.miss",
      "bot.security.control.id": "exec-approval",
      "bot.security.control.family": "approval",
      "bot.security.attribute.params_kind": "object",
      "bot.security.attribute.secretish": "unknown",
    });
    ...
    expect(Object.hasOwn(emitCall.attributes ?? {}, "bot.security.attribute.__proto__")).toBe(
      false,
    );
  ...
  test("exports trusted security events as stdout JSONL logs", async () => {
    ...
    try {
      ...
      expect(record.body).toBe("bot.security.event");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(record.attributes).toMatchObject({
        "bot.security.event_id": "security-event-stdout",
        "bot.security.category": "tool",
        "bot.security.action": "tool.execution.blocked",
        "bot.security.outcome": "denied",
        "bot.security.severity": "medium",
      ...
      expect(Object.hasOwn(record.attributes ?? {}, "bot.security.attribute.__proto__")).toBe(
        false,
      );
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("bot.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "bot.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("bot.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "bot.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("bot.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "bot.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("bot.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["bot.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["bot.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["bot.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "bot.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("bot.payload.large")?.add).toHaveBeenCalledWith(1, {
      "bot.payload.action": "rejected",
      "bot.payload.surface": "gateway.frame",
      "bot.channel": "web",
      "bot.plugin": "none",
      "bot.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("bot.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "bot.signal": "logs",
      "bot.status": "failure",
      "bot.reason": "emit_failed",
      "bot.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    telemetryState.counters.get("bot.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("bot.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["bot.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["bot.blocked_by"]).toBe("policy-plugin");
  ...
  test("run.completed error span carries the redacted message off the metric attrs", async () => {
    ...
    expect(startedSpanOptions("bot.run")?.attributes?.["bot.error"]).toBe(
      "upstream model stream stalled then aborted",
    );
    expect(spanByName("bot.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "upstream model stream stalled then aborted",
    });
    // The raw message must never widen metric cardinality.
    const runDuration = lastHistogramRecord("bot.run.duration_ms");
    expect(runDuration?.[1]?.["bot.outcome"]).toBe("error");
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "bot.error")).toBe(false);
  });
  ...
  test("run.completed bounds sensitive error text before export", async () => {
    ...
    const status = mockCallArg(spanByName("bot.run").setStatus, 0) as {
      message?: string;
    };
  ...
  test("harness.run.completed error span carries the redacted message", async () => {
    ...
    expect(startedSpanOptions("bot.harness.run")?.attributes?.["bot.error"]).toBe(
      "model run failed during resolve phase",
    );
    expect(spanByName("bot.harness.run").setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "model run failed during resolve phase",
    });
    const harnessDuration = lastHistogramRecord("bot.harness.duration_ms");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "bot.error")).toBe(false);
  });
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("bot.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["bot.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const attrs = startedSpanOptions("bot.model.call")?.attributes;
    expect(attrs?.["bot.content.input_messages"]).toBe("user prompt");
  });
  ...
  test("preserves OTLP TLS env options when passing env proxy agents", async () => {
    const certDir = mkdtempSync(path.join(tmpdir(), "bot-otel-tls-"));
    try {
  ...
  test("exports diagnostic logs as stdout JSONL without constructing the OTLP log exporter", async () => {
    await startOtelService({
      ...
      captureContent: true,
      configure: (ctx) => {
        ctx.config.diagnostics!.otel!.serviceName = "rovoclaw-bot";
      },
    });
    const stdout = captureStdoutWrites();
    ...
    try {
      ...
      expect(record.signal).toBe("bot.diagnostic.log");
      expect(record["service.name"]).toBe("rovoclaw-bot");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      ...
      expect(record.attributes).toMatchObject({
        "bot.log.level": "WARN",
        "bot.subsystem": "diagnostic",
      });
      const tokenAttr = record.attributes?.["bot.token"];
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "bot.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "bot.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "bot.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/bot/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/bot/src/private.ts:42",
      },
    ...
    expect(emitCall.attributes["bot.good"]).toBe(`${"y".repeat(4095)}...(truncated)`);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(Object.hasOwn(emitCall.attributes, `bot.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "bot.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "bot.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "bot.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "bot.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "bot.code.location")).toBe(false);
  });
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("bot.tokens");
  ...
  test("advertises explicit duration buckets on the bot run/harness/context histograms", async () => {
    const priorSdkBoundaries = [
      0, 5, 10, 25, 50, 75, 100, 250, 500, 750, 1000, 2500, 5000, 7500, 10000,
    ];
    await startOtelService({ metrics: true });

    const runDurationOptions = histogramCreateOptions("bot.run.duration_ms");
    ...
    const harnessDurationOptions = histogramCreateOptions("bot.harness.duration_ms");
    ...
    const contextOptions = histogramCreateOptions("bot.context.tokens");
  ...
  test.each([
    ...
    expect(telemetryState.counters.get("bot.tokens")?.add).toHaveBeenCalledWith(2, {
      "bot.channel": "unknown",
      "bot.agent": "unknown",
      "bot.provider": "openai",
      "bot.model": "gpt-5.4",
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("bot.tokens")?.add.mock.calls),
  ...
  test.each([
    ...
    expect(telemetryState.counters.get("bot.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "bot.lane": expected,
      },
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("bot.queue.lane.enqueue")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("bot.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "bot.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "bot.sessionId")).toBe(false);
  ...
  test("separates request and turn GenAI client duration by operation", async () => {
    ...
    const botModelCallDuration = telemetryState.histograms.get(
      "bot.model_call.duration_ms",
    );
    expect(botModelCallDuration?.record).toHaveBeenCalledTimes(4);
    expect(
      botModelCallDuration?.record.mock.calls.map(
        (call) => call[1]?.["bot.model_call.observation_unit"],
      ),
    ).toEqual(["request", "request", "turn", "turn"]);
    const spanObservationUnits = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "bot.model.call")
      .map(
        (call) =>
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "bot.agent": "main",
      "bot.skill.activation": "read",
      "bot.skill.name": "tiny-llm-brainstorm",
      "bot.skill.source": "workspace",
      "bot.toolName": "read",
    };
    expect(telemetryState.counters.get("bot.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "bot.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("bot.run");
    expect(spanNames).toContain("bot.model.call");
    expect(spanNames).toContain("bot.harness.run");
    expect(spanNames).toContain("bot.tool.execution");

    const runOptions = startedSpanOptions("bot.run");
    expect(runOptions?.attributes?.["bot.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["bot.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["bot.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["bot.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "bot.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "bot.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "bot.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("bot.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "bot.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "bot.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "bot.sessionKey")).toBe(false);
    expect(modelOptions?.startTime).toBeTypeOf("number");
    expect(modelOptions?.kind).toBe(2);
    expect(modelCall?.[2]).toBeUndefined();

    const harnessCall = startedSpanCall("bot.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["bot.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["bot.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["bot.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["bot.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["bot.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["bot.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["bot.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["bot.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["bot.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["bot.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["bot.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "bot.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "bot.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "bot.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "bot.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("bot.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["bot.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["bot.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["bot.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["bot.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["bot.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "bot.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "bot.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "bot.sessionKey")).toBe(false);
    ...
    const modelCallDuration = lastHistogramRecord("bot.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["bot.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["bot.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("bot.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["bot.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["bot.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("bot.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["bot.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["bot.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("bot.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["bot.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["bot.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("bot.model.call");
    expect(modelSpanAttributes["bot.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["bot.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["bot.model_call.time_to_first_byte_ms"]).toBe(45);
    expect(modelSpanAttributes["bot.model_call.prompt.input_messages_count"]).toBe(2);
    expect(modelSpanAttributes["bot.model_call.prompt.input_messages_chars"]).toBe(3456);
    expect(modelSpanAttributes["bot.model_call.prompt.system_prompt_chars"]).toBe(789);
    expect(modelSpanAttributes["bot.model_call.prompt.tool_definitions_count"]).toBe(4);
    expect(modelSpanAttributes["bot.model_call.prompt.tool_definitions_chars"]).toBe(2345);
    expect(modelSpanAttributes["bot.model_call.prompt.total_chars"]).toBe(6590);
    expect(modelSpanAttributes["bot.model_call.usage.input_tokens"]).toBe(100);
    expect(modelSpanAttributes["bot.model_call.usage.output_tokens"]).toBe(20);
    expect(modelSpanAttributes["bot.model_call.usage.cache_read_input_tokens"]).toBe(30);
    expect(modelSpanAttributes["bot.model_call.usage.cache_creation_input_tokens"]).toBe(5);
    expect(modelSpanAttributes["bot.model_call.usage.reasoning_output_tokens"]).toBe(8);
    expect(modelSpanAttributes["bot.model_call.usage.prompt_tokens"]).toBe(135);
    expect(modelSpanAttributes["bot.model_call.usage.total_tokens"]).toBe(155);
    ...
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "bot.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("bot.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["bot.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["bot.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["bot.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "bot.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "bot.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("bot.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["bot.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "bot.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "bot.runId")).toBe(false);

    const toolSpan = spanByName("bot.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("bot.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("bot.model.failover");
    expect(failoverOptions?.attributes?.["bot.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["bot.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["bot.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["bot.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["bot.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["bot.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["bot.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["bot.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "bot.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "bot.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("bot.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("bot.model.failover")).toStrictEqual([
      1,
      {
        "bot.failover.reason": "overloaded",
        "bot.failover.suspended": "true",
        "bot.lane": "main",
        "bot.model": "claude-opus-4-6",
        "bot.provider": "anthropic",
        "bot.failover.to_model": "gpt-5.4",
        "bot.failover.to_provider": "openai",
      },
    ]);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("bot.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "bot.toolName": "browser",
        "bot.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "bot.tool.owner": "browser-tools",
        "bot.tool.params.kind": "object",
        "bot.deniedReason": "tools.deny",
      },
    ]);
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("bot.model.failover");
    expect(failoverOptions?.attributes?.["bot.lane"]).toBe("session");
  ...
  test("uses latest GenAI request and agent span shapes only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("bot.model.call")).toBeUndefined();
    ...
    expect(agentTurnOptions?.attributes?.["bot.model_call.observation_unit"]).toBe("turn");
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("bot.model.call");
    expect(modelCallOptions?.attributes?.["bot.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "bot.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "bot.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("bot.provider.request", {
      "bot.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("bot.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["bot.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "bot.upstreamRequestIdHash")).toBe(
      false,
    );
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("bot.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "bot.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["bot.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["bot.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["bot.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["bot.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["bot.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["bot.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["bot.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["bot.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["bot.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["bot.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["bot.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["bot.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["bot.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("bot.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "bot.toolName": "process",
      "bot.loop.level": "critical",
      "bot.loop.action": "block",
      "bot.loop.detector": "known_poll_no_progress",
      "bot.loop.count": 20,
      "bot.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("bot.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["bot.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["bot.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["bot.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["bot.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["bot.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["bot.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "bot.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("bot.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("bot.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "bot.memory.level": "critical",
        "bot.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("bot.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "bot.memory.level": "critical",
      "bot.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("bot.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["bot.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["bot.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["bot.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["bot.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["bot.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["bot.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["bot.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["bot.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["bot.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["bot.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "bot.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("bot.diagnostic.async_queue.dropped");
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "bot.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "bot.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "bot.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["bot.run"]).toBeUndefined();
    expect(parentBySpanName["bot.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["bot.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("bot.message.processed");
    const harnessSpan = spanByName("bot.harness.run");
    const runSpan = spanByName("bot.run");
    const usageSpan = spanByName("bot.model.usage");
    const modelCallSpan = spanByName("bot.model.call");
    ...
    expect(parentBySpanName["bot.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["bot.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["bot.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["bot.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["bot.model.call"]?.spanId).toBe(runSpanContext.spanId);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("bot.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("bot.message.processed")[0]).toBeUndefined();
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("bot.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("bot.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("bot.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("bot.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("bot.message.processed")["bot.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("bot.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("bot.harness.run")?.[2]).toBeUndefined();
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("bot.run")).toBeTypeOf("number");
  });
  ...
  test.each([
    ...
    const runContexts = startedSpanParentContextsByName("bot.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("bot.run").spanContext();
    const modelParentContext = startedSpanParentContexts("bot.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("bot.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("bot.model.usage")?.[2]).toBeUndefined();
  });
  ...
  test.each([
    ...
    expect(parentBySpanName["bot.run"]).toBeUndefined();
    expect(parentBySpanName["bot.model.call"]).toBeUndefined();
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["bot.run"]).toBeUndefined();
    expect(parentBySpanName["bot.model.call"]).toBeUndefined();
    expect(parentBySpanName["bot.tool.execution"]).toBeUndefined();
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "bot.run"),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "bot.model.call",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "bot.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "bot.harness.run",
      ),
    ).toHaveLength(1);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("bot.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["bot.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["bot.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["bot.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["bot.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("bot.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["bot.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["bot.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["bot.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["bot.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["bot.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["bot.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["bot.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "bot.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "bot.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "bot.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("bot.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("bot.exec")).toBeTypeOf("number");
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("bot.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "bot.channel": "matrix",
      "bot.delivery.kind": "text",
    });
    ...
    expect(deliveryDurationRecords[0]?.[1]["bot.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["bot.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["bot.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["bot.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["bot.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["bot.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["bot.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "bot.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["bot.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["bot.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["bot.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["bot.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["bot.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["bot.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["bot.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["bot.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "bot.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "bot.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "bot.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "bot.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "bot.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "bot.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "bot.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("bot.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["bot.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["bot.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["bot.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("bot.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["bot.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["bot.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["bot.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["bot.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("bot.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["bot.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["bot.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["bot.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("bot.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["bot.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["bot.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["bot.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("bot.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["bot.status"]).toBe("released");
    expect(telemetryState.counters.get("bot.talk.event")?.add).toHaveBeenCalledWith(1, {
      "bot.talk.brain": "agent-consult",
      "bot.talk.event_type": "input.audio.delta",
      "bot.talk.mode": "realtime",
      "bot.talk.provider": "openai",
      "bot.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("bot.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "bot.talk.brain": "agent-consult",
        "bot.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("bot.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "bot.talk.brain": "agent-consult",
      "bot.talk.event_type": "latency.metrics",
      "bot.talk.mode": "realtime",
      "bot.talk.provider": "openai",
      "bot.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("bot.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("bot.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "bot.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "bot.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "bot.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("bot.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "bot.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "bot.content.tool_output")).toBe(
      false,
    );
  ...
  test("exports bounded redacted content when capture is enabled", async () => {
    ...
    const modelAttrs = startedSpanOptions("bot.model.call")?.attributes;
    const toolAttrs = startedSpanOptions("bot.tool.execution")?.attributes;

    expect(modelAttrs?.["bot.content.output_messages"]).toBe("model reply");
    expect(Object.hasOwn(modelAttrs ?? {}, "bot.content.system_prompt")).toBe(false);
    expect(String(modelAttrs?.["bot.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["bot.content.tool_input"]).toBe("tool input");
    ...
    expect(toolAttrs?.["gen_ai.tool.call.arguments"]).toBe(
      toolAttrs?.["bot.content.tool_input"],
    );
    expect(typeof toolAttrs?.["bot.content.tool_output"]).toBe("string");
    expect(String(toolAttrs?.["bot.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["bot.content.tool_output"])).not.toContain("a".repeat(11));
    expect(toolAttrs?.["gen_ai.tool.call.result"]).toBe(
      toolAttrs?.["bot.content.tool_output"],
    );
  });
  ...
  test("omits absent model content fields when capture is enabled", async () => {
    ...
    const attrs = startedSpanOptions("bot.model.call")?.attributes ?? {};
    expect(attrs["bot.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "bot.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "bot.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "bot.content.tool_definitions")).toBe(false);
  ...
  test("exports Claude CLI turn content through the existing Phoenix GenAI keys", async () => {
    ...
    emitTrustedModelCallCompletedWithContent(
      ...
      },
      {
        ...
        systemPrompt: "Bot appended instructions",
      },
    );
    await flushDiagnosticEvents();

    const attrs = startedSpanOptions("bot.model.call")?.attributes;
    expect(attrs?.["bot.api"]).toBe("claude-code");
    expect(attrs?.["bot.transport"]).toBe("stdio-live");
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.input.messages"))).toEqual([
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("bot.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["bot.reason"])).toContain("…");
