...
export function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "bot.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "bot.model_call.response_bytes", evt.responseStreamBytes);
...
export function assignModelCallPromptStatsAttrs(
  ...
  for (const [key, value] of [
    ["bot.model_call.prompt.input_messages_count", stats.inputMessagesCount],
    ["bot.model_call.prompt.input_messages_chars", stats.inputMessagesChars],
    ["bot.model_call.prompt.system_prompt_chars", stats.systemPromptChars],
    ["bot.model_call.prompt.tool_definitions_count", stats.toolDefinitionsCount],
    ["bot.model_call.prompt.tool_definitions_chars", stats.toolDefinitionsChars],
    ["bot.model_call.prompt.total_chars", stats.totalChars],
  ] as const) {
...
export function assignModelCallUsageAttrs(
  ...
  for (const [key, value] of [
    ["bot.model_call.usage.input_tokens", usage.input],
    ["bot.model_call.usage.output_tokens", usage.output],
    ["bot.model_call.usage.cache_read_input_tokens", usage.cacheRead],
    ["bot.model_call.usage.cache_creation_input_tokens", usage.cacheWrite],
    ["bot.model_call.usage.reasoning_output_tokens", usage.reasoningTokens],
    ["bot.model_call.usage.prompt_tokens", promptTokens],
    ["bot.model_call.usage.total_tokens", usage.total],
...
export function assignGenAiModelCallAttrs(
  ...
  attrs["bot.model_call.observation_unit"] = modelCallObservationUnit(evt);
}
...
export function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("bot.provider.request", {
    "bot.upstreamRequestIdHash": boundedHash,
  });
}
