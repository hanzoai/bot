...
vi.mock("bot/plugin-sdk/setup-tools", () => ({
...
function createApi(params?: {
  speech?: Partial<
    Awaited<ReturnType<BotPluginApi["runtime"]["tts"]["textToSpeechTelephony"]>>
  >;
  runCommand?: BotPluginApi["runtime"]["system"]["runCommandWithTimeout"];
}): BotPluginApi {
  return {
    config: {},
    registerTool: vi.fn(),
...
function createContext(
  overrides: Partial<BotPluginToolContext> = {},
): BotPluginToolContext {
  return {
...
describe("WhatsApp call tool", () => {
  ...
  beforeEach(async () => {
    ...
    rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-whatsapp-call-test-"));
  ...
  it("synthesizes a private WAV and calls only the current requester", async () => {
    ...
    }) as BotPluginApi["runtime"]["system"]["runCommandWithTimeout"];
