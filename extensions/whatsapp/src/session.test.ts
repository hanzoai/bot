...
import { resetLogger, setLoggerOverride } from "bot/plugin-sdk/runtime-env";
...
const TEST_UNDICI_RUNTIME_DEPS_KEY = "__BOT_TEST_UNDICI_RUNTIME_DEPS__";
...
const BOT_WHATSAPP_WEB_SOCKET_URL_ENV = "BOT_WHATSAPP_WEB_SOCKET_URL";
...
function createTempCaFile(contents: string): string {
  const dir = createTempAuthDir("bot-wa-proxy-ca");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("bot-wa-creds-test");
  ...
  it("creates standalone directory sockets without inbound message consumers", async () => {
    const authDir = createTempAuthDir("bot-wa-directory-socket");
  ...
  it("prints compact terminal QR output when requested", async () => {
    const authDir = createTempAuthDir("bot-wa-terminal-qr");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("bot-wa-creds-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("bot-wa-authdir-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("bot-wa-auth-parent-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("bot-wa-creds-symlink-save");
      const targetPath = path.join(authDir, "target-creds.json");
      const credsPath = path.join(authDir, "creds.json");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("bot-wa-creds-parent-symlink-save");
      const targetBaseDir = path.join(rootDir, "target-base");
      const linkedBaseDir = path.join(rootDir, "linked-base");
      const authDir = path.join(linkedBaseDir, "default");
      fsSync.mkdirSync(targetBaseDir);
  ...
  it("uses BOT_WHATSAPP_WEB_SOCKET_URL as the default Baileys WebSocket URL", async () => {
    vi.stubEnv(BOT_WHATSAPP_WEB_SOCKET_URL_ENV, " ws://127.0.0.1:49153/ws/chat ");
  ...
  it("ignores blank Baileys WebSocket URL environment overrides", async () => {
    vi.stubEnv(BOT_WHATSAPP_WEB_SOCKET_URL_ENV, " ");
  ...
  it("rejects invalid BOT_WHATSAPP_WEB_SOCKET_URL values", async () => {
    vi.stubEnv(BOT_WHATSAPP_WEB_SOCKET_URL_ENV, "http://127.0.0.1:14567/ws");

    await expect(createWaSocket(false, false)).rejects.toThrow(
      "BOT_WHATSAPP_WEB_SOCKET_URL must use ws:// or wss://.",
    );
  ...
  it("preserves explicit Baileys WebSocket URL options over invalid environment", async () => {
    vi.stubEnv(BOT_WHATSAPP_WEB_SOCKET_URL_ENV, "http://127.0.0.1:49153/ws/chat");
  ...
  it("adds managed proxy CA trust to WhatsApp env proxy agents", async () => {
    ...
    vi.stubEnv("BOT_PROXY_ACTIVE", "1");
    vi.stubEnv("BOT_PROXY_CA_FILE", caFile);
  ...
  it("logWebSelfId prints cached E.164 when creds exist", () => {
    const authDir = createTempAuthDir("bot-wa-log-self");
    fsSync.writeFileSync(
  ...
  it("logWebSelfId prints cached lid details when creds include a lid", () => {
    const authDir = createTempAuthDir("bot-wa-log-self-lid");
    fsSync.writeFileSync(
      path.join(authDir, "creds.json"),
      JSON.stringify({
        me: {
  ...
  it("does not clobber creds backup when creds.json is corrupted", async () => {
    const authDir = createTempAuthDir("bot-wa-corrupt-backup");
  ...
  it("revalidates setup ownership immediately before a delayed creds.update write", async () => {
    const authDir = createTempAuthDir("bot-wa-guarded-creds");
  ...
  it("revalidates setup ownership before Baileys persists signal keys", async () => {
    const authDir = createTempAuthDir("bot-wa-guarded-keys");
    const guardError = new Error("verified inference route changed");
    let routeOwner = "original";
    const beforeCredentialPersistence = vi.fn(async () => {
      if (routeOwner !== "original") {
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("bot-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("bot-wa-a");
    const authDirB = createTempAuthDir("bot-wa-b");
    const onError = vi.fn();
  ...
  it("rotates creds backup when creds.json is valid JSON", async () => {
    const authDir = createTempAuthDir("bot-wa-rotate-backup");
    const credsPath = path.join(authDir, "creds.json");
    const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("bot-wa-rotate-backup-symlink");
      const credsPath = path.join(authDir, "creds.json");
      const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    try {
      await writeCredsJsonAtomically("/tmp/bot-oauth/whatsapp/default", {
        me: { id: "123@s.whatsapp.net" },
      });

      const write = firstWriteFileCall(openMock.writeFileSpy);
      expect(write.path).toContain(
        path.join("/tmp", "bot-oauth", "whatsapp", "default", ".creds."),
      );
      expect(typeof write.data).toBe("string");
      expect(write.options.mode).toBe(0o600);
      expect(write.options.flag).toBe("wx");
      ...
      expect(chmodSpy).toHaveBeenCalledWith(
        path.join("/tmp", "bot-oauth", "whatsapp", "default", "creds.json"),
        0o600,
      );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("bot-wa-creds-atomic");
    const credsPath = path.join(authDir, "creds.json");
