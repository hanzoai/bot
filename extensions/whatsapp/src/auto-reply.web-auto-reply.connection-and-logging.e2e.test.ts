...
import { setLoggerOverride } from "bot/plugin-sdk/runtime-env";
...
describe("web auto-reply connection", () => {
  ...
  it("treats status 440 as non-retryable and stops without retrying", async () => {
    ...
    expectErrorContaining(runtime.error, "bot channels logout --channel whatsapp");
    ...
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      ...
      try {
        ...
        expect(firstArgs.Body).toMatch(
          new RegExp(
            `\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\+1: \\[bot\\] first`,
          ),
        );
        expect(firstArgs.Body).not.toContain("second");
        expect(secondArgs.Body).toMatch(
          new RegExp(
            `\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\+1: \\[bot\\] second`,
          ),
        );
