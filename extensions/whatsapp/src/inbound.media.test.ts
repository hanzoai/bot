...
vi.mock("bot/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/channel-pairing")>(
    "bot/plugin-sdk/channel-pairing",
  );
  return {
    ...actual,
    readChannelAllowFromStore(...args: unknown[]) {
...
vi.mock("bot/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/media-store")>(
    "bot/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("bot/plugin-sdk/plugin-state-test-runtime")>(
      "bot/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/bot-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `bot-inbound-media-${crypto.randomUUID()}`);
