...
vi.mock("bot/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/logging-core")>(
    "bot/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("Bot WhatsApp listener close");
