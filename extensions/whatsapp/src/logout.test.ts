...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "bot-test-web-logout-"));
    previousOAuthDir = process.env.BOT_OAUTH_DIR;
    process.env.BOT_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the Bot auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Bot auth root", async () => {
