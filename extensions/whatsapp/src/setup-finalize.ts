...
import type { ChannelSetupWizard } from "bot/plugin-sdk/setup";
...
type WhatsAppConfig = NonNullable<NonNullable<BotConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: BotConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: BotConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: BotConfig, accountId: string): string {
  if (
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("bot channels login"),
      }),
      "WhatsApp",
    );
  }
