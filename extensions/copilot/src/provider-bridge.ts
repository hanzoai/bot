...
import { isNonSecretApiKeyMarker } from "bot/plugin-sdk/provider-auth";
import { isBlockedHostnameOrIp } from "bot/plugin-sdk/ssrf-runtime";
...
const COPILOT_BYOK_TRANSPORT_POLICY_ERROR =
  "[copilot-attempt] BYOK does not support Bot provider request proxy, TLS, or private-network policy overrides";
const COPILOT_BYOK_ENDPOINT_POLICY_ERROR =
  "[copilot-attempt] BYOK endpoint is blocked by Bot SSRF policy";
...
/**
 * Maps Bot's prepared model facts into the Copilot SDK's session-level
 * provider contract. The SDK owns the wire request; Bot only supplies
