...
export function scanPolicyIngress(cfg: Record<string, unknown>): readonly PolicyIngressEvidence[] {
  ...
  const channelDefaultsSource = "oc://bot.config/channels/defaults";
  ...
  entries.push({
    id: "session-dm-scope",
    kind: "sessionDmScope",
    source: "oc://bot.config/session/dmScope",
    value: dmScope ?? "main",
  ...
  for (const [channel, value] of Object.entries(channels)) {
    ...
    const channelSource = `oc://bot.config/channels/${ocPathSegment(channel)}`;
