...
import type { ChannelInboundMediaInput } from "bot/plugin-sdk/channel-inbound";
...
import { openLocalFileSafely } from "bot/plugin-sdk/security-runtime";
    ...
    if (isHeicAttachment(params.attachmentPath, params.mimeType)) {
      try {
        ...
        const converted = await withTempWorkspace(
          { rootDir: resolvePreferredBotTmpDir(), prefix: "bot-imessage-heic-" },
          async (workspace) => {
