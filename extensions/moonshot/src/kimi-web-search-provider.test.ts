...
describe("kimi web search provider", () => {
  ...
  it("accepts final responses backed by Kimi web search tool replay", async () => {
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          choices: [
            {
              finish_reason: "stop",
              message: { content: "Bot is available on GitHub." },
            },
          ],
        }),
      );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "Bot is available on GitHub.");
  ...
  it("accepts final responses with search result citations", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        search_results: [{ title: "Bot", url: "https://github.com/hanzoai/bot" }],
        choices: [
          {
            finish_reason: "stop",
            message: { content: "Bot is on GitHub." },
          },
        ],
      }),
    );
    ...
    await withEnvAsync({ KIMI_API_KEY: "kimi-test-key" }, async () => {
      ...
      expect(result.provider).toBe("kimi");
      expectStringFieldContains(result, "content", "Bot is on GitHub.");
      expect(result.citations).toEqual(["https://github.com/hanzoai/bot"]);
      expect(result).not.toHaveProperty("error");
    });
  });
