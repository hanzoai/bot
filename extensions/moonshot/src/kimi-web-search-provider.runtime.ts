...
function resolveKimiBaseUrl(kimi?: KimiConfig, botConfig?: BotConfig): string {
  ...
  const moonshotBaseUrl = botConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: BotConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.bot.ai/tools/kimi-search",
