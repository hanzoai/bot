...
import { readPluginPackageVersion } from "bot/plugin-sdk/extension-shared";
import {
...
const USER_AGENT = `bot-parallel/${PLUGIN_VERSION} (${process.platform})`;
...
export async function executeParallelWebSearchProviderTool(
  ...
  // Generic `query` arg fallback: bot's operator-facing CLI
  // (`bot capability web.search ...`) always passes the shared
  ...
  // Always pass max_results so Parallel matches the bot web_search default
