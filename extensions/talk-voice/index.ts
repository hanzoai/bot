// Talk Voice plugin entrypoint registers its Bot integration.
...
import type { SpeechVoiceOption } from "bot/plugin-sdk/speech";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
} from "bot/plugin-sdk/string-coerce-runtime";
import { resolveActiveTalkProviderConfig } from "bot/plugin-sdk/talk-config-runtime";
  ...
  register(api: BotPluginApi) {
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        const cfg = api.runtime.config.current() as BotConfig;
