...
import { CUSTOM_LOCAL_AUTH_MARKER } from "bot/plugin-sdk/provider-auth";
...
vi.mock("bot/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("bot/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("bot/plugin-sdk/provider-auth");
  vi.doUnmock("bot/plugin-sdk/provider-setup");
  vi.resetModules();
});
