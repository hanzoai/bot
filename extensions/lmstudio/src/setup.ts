...
import type { ProviderAppGuidedSetupContext } from "bot/plugin-sdk/plugin-entry";
import {
...
import { withAgentModelAliases } from "bot/plugin-sdk/provider-onboard";
import {
...
} from "bot/plugin-sdk/provider-setup";
import { WizardCancelledError, type WizardPrompter } from "bot/plugin-sdk/setup";
...
function resolveLmstudioSetupDefaultBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return isTruthyEnvValue(env.BOT_DOCKER_SETUP)
    ? LMSTUDIO_DOCKER_HOST_BASE_URL
    : LMSTUDIO_DEFAULT_BASE_URL;
}
...
function stripLmstudioStoredAuthConfig(cfg: BotConfig): BotConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<BotConfig["agents"]>["defaults"]>["models"];
  discoveredModels: ModelDefinitionConfig[];
}) {
