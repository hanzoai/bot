...
- Polling and webhook both: enqueue then pump
  ...
  loops.
- Stall timeout: `BOT_TELEGRAM_SPOOLED_HANDLER_TIMEOUT_MS` →
...
- Do not reintroduce `sendMessageDraft` for answer streaming. Telegram drafts
  ...
  requires a separate `sendMessage`. Bot uses `sendMessage` plus
...
- Reply context comes from Bot-observed messages. Bot API updates expose
