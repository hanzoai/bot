...
import { getAgentScopedMediaLocalRoots } from "bot/plugin-sdk/media-runtime";
import { resolveChunkMode } from "bot/plugin-sdk/reply-dispatch-runtime";
...
type ResolveConfiguredBindingRouteFn =
  typeof import("bot/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("bot/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
...
type ResolveCommandArgMenuFn =
  typeof import("bot/plugin-sdk/command-auth-native").resolveCommandArgMenu;
...
type LoadModelCatalogFn = typeof import("bot/plugin-sdk/agent-runtime").loadModelCatalog;
type ResolveDefaultModelForAgentFn =
  typeof import("bot/plugin-sdk/agent-runtime").resolveDefaultModelForAgent;
...
vi.mock("bot/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/conversation-runtime")>(
...
vi.mock("bot/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/command-auth-native")>(
    "bot/plugin-sdk/command-auth-native",
  );
...
vi.mock("bot/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/plugin-runtime")>(
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  it("keeps one live config snapshot through native command execution", async () => {
    const startupCfg: BotConfig = { session: { store: "/tmp/startup-sessions.json" } };
    const runtimeCfg: BotConfig = { session: { store: "/tmp/runtime-sessions.json" } };
  ...
  it.each([
    ...
    { sessionRuntime: "bot", expectedRuntime: "bot" },
  ])(
  ...
  it("passes persisted topic session identity to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/bot-sessions/sessions.json");
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as BotConfig,
  ...
  it("passes session identity to plugin commands when the entry has no file", async () => {
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:main",
        sessionId: "sess-main",
        sessionFile: "sqlite:main:sess-main:/tmp/bot-sessions/sessions.json",
      },
      "plugin command params",
    );
  });
  ...
  it("passes SQLite transcript markers to plugin commands without path resolution", async () => {
    const storePath = "/tmp/bot-sessions/sessions.json";
