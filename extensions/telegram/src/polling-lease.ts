...
function createDuplicatePollingError(params: {
  ...
  return new Error(
    `Telegram polling already active for bot token ${params.tokenFingerprint} on account "${params.existing.accountId}" (${ageSeconds}s old); refusing duplicate poller for account "${params.accountId}". Stop the existing Bot gateway/poller or use a different bot token.`,
  );
}
