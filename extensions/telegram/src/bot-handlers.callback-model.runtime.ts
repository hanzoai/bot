...
import { buildCommandsMessagePaginated } from "bot/plugin-sdk/command-status";
import type { BotConfig } from "bot/plugin-sdk/config-contracts";
import {
  applyModelOverrideToSessionEntry,
  ModelSelectionLockedError,
} from "bot/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "bot/plugin-sdk/models-provider-runtime";
import { parseStrictPositiveInteger } from "bot/plugin-sdk/number-runtime";
import { logVerbose } from "bot/plugin-sdk/runtime-env";
import { patchSessionEntry } from "bot/plugin-sdk/session-store-runtime";
import {
  resolveAgentDir,
...
export async function handleTelegramModelCallback(params: {
  ...
  try {
    ...
    const scopeText = isDefaultSelection
      ...
      : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in bot.json is unchanged; /reset or a new session may return to that default.`;
