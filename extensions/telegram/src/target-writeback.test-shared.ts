...
vi.mock("bot/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/config-mutation")>(
    "bot/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("bot/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/cron-store-runtime")>(
    "bot/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/bot.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
