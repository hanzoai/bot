...
// fix for the leak observed in bot#68128 is the transport lifecycle that
...
function resolveBotProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.BOT_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveBotProxyUrlForTelegram() : undefined;
