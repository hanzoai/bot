...
let botState: BotTestState | undefined;
...
function cfgWithOwner(ownerId = OWNER_ID): BotConfig {
  return {
    channels: {
      telegram: {
        allowFrom: [ownerId],
...
describe("telegram ingress supersede policy", () => {
  ...
  it("gates command supersede on authorized sender", async () => {
    // /new is a recognized text alias in Bot command set.
