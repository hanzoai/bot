...
import { escapeRegExp, formatEnvelopeTimestamp } from "bot/plugin-sdk/channel-test-helpers";
import type { TelegramGroupConfig } from "bot/plugin-sdk/config-contracts";
import {
...
vi.mock("bot/plugin-sdk/conversation-runtime", { spy: true });

const harness = await import("./bot.create-telegram-bot.test-harness.js");
const pluginStateTestRuntime = await import("bot/plugin-sdk/plugin-state-test-runtime");
const configMutation = await import("bot/plugin-sdk/config-mutation");
const sessionStoreRuntime = await import("bot/plugin-sdk/session-store-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "bot-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  beforeEach(async () => {
    ...
    process.env.BOT_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
    clearPluginInteractiveHandlers();
  ...
  it.each(["stop", "/stop@bot_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "Bot Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
  ...
  it("routes plugin callback_query payloads to plugin handlers without fallback callback_data text", async () => {
    ...
    expect(
      registerPluginInteractiveHandler("bot-code-agent", {
        channel: "telegram",
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("bot pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Bot", username: "bot_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "Bot", username: "bot_bot" },
          text: "Binding: Review pull request 54118 (bot)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "Bot", username: "bot_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "Bot Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Bot", username: "bot_bot" },
  ...
  const groupPolicyCases: MessagePolicyCase[] = [
    ...
    makeMessagePolicyCase({
      ...
      message: { from: { id: 999999, username: "notallowed" }, text: "@bot_bot hello" },
      expectedReplyCount: 0,
    }),
    makeMessagePolicyCase({
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "bot_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  it("marks explicit Telegram bot-handle mentions in the inbound context", async () => {
    ...
    await dispatchMessage({
      message: {
        chat: { id: 7, type: "group", title: "Test Group" },
        text: "@bot_bot status",
        entities: [{ type: "mention", offset: 0, length: "@bot_bot".length }],
        date: 1736380800,
        message_id: 4,
        from: { id: 9, first_name: "Ada" },
      },
    ...
    expect(payload.BotUsername).toBe("bot_bot");
  });
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/bot-telegram-group-activation.json";
