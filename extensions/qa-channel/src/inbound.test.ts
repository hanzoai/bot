...
vi.mock("bot/plugin-sdk/outbound-media", async (importOriginal) => {
  const actual = await importOriginal<typeof import("bot/plugin-sdk/outbound-media")>();
  return {
    ...actual,
...
describe("handleQaInbound", () => {
  ...
  it("marks group messages that match configured mention patterns", async () => {
    const runtime = createPluginRuntimeMock();
    vi.mocked(runtime.channel.mentions.buildMentionRegexes).mockReturnValue([/\b@?bot\b/i]);
    setQaChannelRuntime(runtime);
