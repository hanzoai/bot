...
vi.mock("@hanzo/bot/plugin-sdk/google-gemini-cli-auth", () => ({
  isWSL2Sync: () => false,
...
describe("loginGeminiCliOAuth", () => {
  ...
  beforeEach(() => {
    ...
    process.env.BOT_GEMINI_OAUTH_CLIENT_SECRET = "GOCSPX-test-client-secret";
  ...
  it("falls back across loadCodeAssist endpoints with aligned headers and metadata", async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url === USERINFO_URL) {
        return responseJson({ email: "user@bot.ai" });
      }
      if (url === LOAD_PROD) {
