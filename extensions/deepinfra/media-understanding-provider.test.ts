...
vi.mock("bot/plugin-sdk/media-understanding", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/media-understanding")>(
    "bot/plugin-sdk/media-understanding",
  );
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("bot/plugin-sdk/media-understanding");
  vi.resetModules();
});
