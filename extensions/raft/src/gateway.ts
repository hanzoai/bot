...
import type { ChannelGatewayContext } from "bot/plugin-sdk/channel-contract";
import { keepHttpServerTaskAlive, waitUntilAbort } from "bot/plugin-sdk/channel-outbound";
...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. Bot passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error("Raft requires Bot channel runtime support. Update Bot and retry.");
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. Bot has no
      ...
      // Raft owns wake metadata and its schema evolution. Bot accepts only
