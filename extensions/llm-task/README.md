...
`bot.invoke --each`) without adding new Bot code per workflow.
...
This extension depends on Bot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Bot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.bot/extensions` as a standalone plugin directory.
