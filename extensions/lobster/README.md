...
```bash
bot plugins install @hanzo/bot-lobster
```
...
- This plugin integrates Lobster with Bot _without core changes_.
...
## Using `bot.invoke` (Lobster → Bot tools)

Some Lobster pipelines may include a `bot.invoke` step to call back into Bot tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).

For this to work, the Bot Gateway must expose the tool bridge endpoint and the target tool must be allowed by policy:

- Bot provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, Bot returns `404 Tool not available`.
...
- https://docs.bot.ai/tools/lobster

## Package

- Plugin id: `lobster`
- Tool: `lobster`
- Package: `@hanzo/bot-lobster`
