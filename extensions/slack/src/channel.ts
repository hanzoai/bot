...
//
// `bot/plugin-sdk/channel-policy` is intentionally NOT lazy here —
...
const EXTENSION_SHARED_MODULE_ID = "bot/plugin-sdk/extension-shared";
const TARGET_RESOLVER_RUNTIME_MODULE_ID = "bot/plugin-sdk/target-resolver-runtime";
  ...
  base: {
    ...
    conversationBindings: {
      isCurrentConversationBindingSupported: ({ accountId }) => {
        const cfg = getOptionalSlackRuntime()?.config.current() as BotConfig | undefined;
    ...
    actions: createSlackActions(SLACK_CHANNEL, {
      invoke: async (action, cfg, toolContext) =>
        ...
        )(action, cfg as BotConfig, toolContext as SlackActionContext | undefined),
    }),
