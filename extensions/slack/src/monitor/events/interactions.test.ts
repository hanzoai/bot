...
vi.mock("bot/plugin-sdk/heartbeat-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("bot/plugin-sdk/heartbeat-runtime")>();
  return {
    ...actual,
...
describe("registerSlackInteractionEvents", () => {
  ...
  it("registers a matcher that accepts plugin action ids beyond the Bot prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher.test("bot:verify")).toBe(true);
  ...
  it("treats Slack reply buttons as plain interaction events instead of plugin dispatch", async () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "bot:reply_button" }],
            },
          ],
        },
      },
    ...
    const eventText = mockCallArg(enqueueSystemEventMock, 0, "enqueueSystemEvent");
    expect(eventText).toContain('"actionId":"bot:reply_button"');
    expectRecordFields(
      requireRecord(
      ...
      {
        contextKey: "slack:interaction:C1:100.200:bot:reply_button",
        deliveryContext: {
          accountId: "default",
          channel: "slack",
          threadId: "100.100",
  ...
  it("keeps typed callback payloads opaque even when they resemble approval commands", async () => {
    ...
    await getHandler()({
      ...
      action: {
        type: "button",
        action_id: "bot:callback_button:1:1",
  ...
  it("resolves typed exec approvals from Slack-private action data", async () => {
    ...
    await handler({
          ...
          blocks: [
            {
              ...
              elements: [
                { type: "button", action_id: "bot:approval_button:1:1" },
                { type: "button", action_id: "bot:reply_button" },
              ],
            },
          ],
        },
      ...
      action: {
        type: "button",
        action_id: "bot:approval_button:1:1",
        block_id: "exec_actions",
        value:
          'bot:approval:v1:{"approvalId":"plugin:looks-plugin","approvalKind":"exec","decision":"allow-once"}',
        text: { type: "plain_text", text: "Allow once" },
      },
    });
  ...
  it("resolves typed question buttons without enqueueing an agent interaction", async () => {
    ...
    await getHandler()({
      ...
      action: {
        type: "button",
        action_id: "bot:question_button:1:2",
        block_id: "bot_reply_buttons_1",
  ...
  it("cleans stale typed buttons and shows the canonical first-answer winner", async () => {
    ...
    await getHandler()({
        ...
        message: {
          ...
          blocks: [
            ...
            { type: "section", text: { type: "mrkdwn", text: "Command preview" } },
            {
              ...
              elements: [
                { type: "button", action_id: "bot:approval_button:1:1" },
                { type: "button", action_id: "bot:approval_button:1:2" },
              ],
            },
          ],
        },
      ...
      action: {
        ...
        block_id: "exec_actions",
        value:
          'bot:approval:v1:{"approvalId":"req-123","approvalKind":"exec","decision":"allow-once"}',
        text: { type: "plain_text", text: "Allow once" },
      },
    });
  ...
  it("fails closed for malformed Slack approval envelopes", async () => {
    ...
    await getHandler()({
      ...
      action: {
        ...
        action_id: "bot:approval_button:1:1",
        block_id: "exec_actions",
        value: 'bot:approval:v1:{"approvalId":"req-123","decision":"allow-once"}',
        text: { type: "plain_text", text: "Allow once" },
      },
    });
  ...
  it("uses the typed plugin kind for unprefixed approval ids", async () => {
    ...
    await handler({
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "bot:approval_button:1:1" }],
            },
          ],
        },
      },
      action: {
        ...
        block_id: "plugin_actions",
        value:
          'bot:approval:v1:{"approvalId":"req-123","approvalKind":"plugin","decision":"allow-always"}',
  ...
  it.each([
    { name: "current", actionId: "bot:reply_link:1:1", value: undefined },
    {
      name: "legacy",
      actionId: "bot:reply_button:1:1",
      ...
      blocks: [
        {
          ...
          elements: [
            {
              type: "mrkdwn",
              text: ":white_check_mark: *bot:datetime* selected by <@U333>",
            },
          ],
        },
      ],
  ...
  it("dispatches plugin-owned modal submissions with full view state before compacting events", async () => {
    ...
    await viewHandler({
      ...
      body: {
        ...
        view: {
          id: "V777",
          callback_id: "bot:contract_confirm_hearing",
          private_metadata: JSON.stringify({
            channelId: "D777",
            channelType: "im",
            userId: "U777",
    ...
    expectRecordFields(requireRecord(dispatchCall, "dispatch call"), {
      ...
      dedupeId: "view_submission:bot:contract_confirm_hearing:V777:U777",
    });
    ...
    expectRecordFields(interaction, {
      ...
      namespace: "dean.contract",
      payload: "confirm_hearing",
      callbackId: "bot:contract_confirm_hearing",
      viewId: "V777",
  ...
  it("dispatches callback-id-only plugin modal submissions without agent routing metadata", async () => {
    ...
    await viewHandler({
      ...
      body: {
        ...
        view: {
          id: "V778",
          callback_id: "bot:dean.contract:confirm_hearing",
          state: {
            values: {
              contract: {
    ...
    expectRecordFields(requireRecord(dispatchCall, "dispatch call"), {
      channel: "slack",
      data: "dean.contract:confirm_hearing",
      dedupeId: "view_submission:bot:dean.contract:confirm_hearing:V778:U777",
    });
    ...
    expectRecordFields(requireRecord(registrationCtx.interaction, "registration interaction"), {
      ...
      namespace: "dean.contract",
      payload: "confirm_hearing",
      callbackId: "bot:dean.contract:confirm_hearing",
      viewId: "V778",
    });
    expect(enqueueSystemEventMock).not.toHaveBeenCalled();
  });

  it("dispatches metadata-routed plugin modal submissions with non-bot callback ids", async () => {
    enqueueSystemEventMock.mockClear();
    dispatchPluginInteractiveHandlerMock.mockResolvedValueOnce({
      matched: true,
      handled: true,
