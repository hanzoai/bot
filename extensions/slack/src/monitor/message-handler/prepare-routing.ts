...
type SlackRouteBinding = NonNullable<BotConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  BotConfig,
  { bindingsRef: BotConfig["bindings"]; normalizedCfg: BotConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: BotConfig): BotConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as BotConfig)
    : cfg;
