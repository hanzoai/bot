...
function createArgMenusHarness(
  cfg: BotConfig = { commands: { native: true, nativeSkills: false } },
) {
...
describe("Slack native command argument menus", () => {
  ...
  it("prefers the configured slash command over native commands", async () => {
    ...
    expect(
      [...configuredHarness.commands.keys()].some(
        (command) => command instanceof RegExp && command.test("/bot"),
      ),
    ).toBe(true);
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^bot_cmdarg/),
      ),
    ).toBe(true);
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("bot_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("bot_cmdarg_0_1");
...
function createPolicyHarness(overrides?: {
  ...
  const ctx = {
    ...
    slashCommand: {
      ...
      name: overrides?.slashCommandName ?? "bot",
...
describe("slack slash command session metadata", () => {
  ...
  it("refreshes slash routing config between invocations", async () => {
    ...
    const sourceCfg = (harness.ctx as { cfg: BotConfig }).cfg;
    const runtimeCfg = {
      ...sourceCfg,
      session: { dmScope: "per-channel-peer" },
    } as BotConfig;
    resolveAgentRouteMock.mockImplementation((params: { cfg: BotConfig }) => ({
      agentId: "main",
      accountId: "acct",
      sessionKey:
