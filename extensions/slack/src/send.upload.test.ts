...
const fetchWithSsrFGuard = vi.fn(
  async (
    params: Parameters<typeof import("bot/plugin-sdk/ssrf-runtime").fetchWithSsrFGuard>[0],
  ) => {
...
vi.mock("bot/plugin-sdk/extension-shared", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/extension-shared")>(
    "bot/plugin-sdk/extension-shared",
  );
  return {
    ...actual,
...
vi.mock("bot/plugin-sdk/fetch-runtime", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/fetch-runtime")>(
    "bot/plugin-sdk/fetch-runtime",
  );
  return {
    ...actual,
