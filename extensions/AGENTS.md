...
- Extension production code should import from `bot/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `bot.plugin.json` and the package
  `bot` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `bot/plugin-sdk/*`.
