...
import type { WorkerProfile } from "bot/plugin-sdk/plugin-entry";
import type { SpawnResult } from "bot/plugin-sdk/process-runtime";
import { describe, expect, it, vi } from "vitest";
import { resolveCrabboxBinary } from "./botbox-worker-profile.js";
import { createCrabboxWorkerProvider, resolveBotRoot } from "./botbox-worker-provider.js";
...
const BOT_ROOT = path.resolve(path.sep, "workspace", "bot");
const SIBLING_BINARY = path.resolve(BOT_ROOT, "../botbox/bin/botbox");
...
describe("Crabbox worker provider", () => {
  ...
  it("stops a newly provisioned lease when inspect cannot supply a host key", async () => {
    ...
    expect(calls[0]?.argv).toEqual([
      ...
      "public",
      "--id",
      expect.stringMatching(/^bot-[a-f0-9]{32}$/u),
      "--json",
    ]);
  ...
  it.each([
    ...
    async ({ provider, missing }) => {
      ...
      const botboxProvider = providerWithRunner(runCommand);

      await expect(
        botboxProvider.provision({ ...PROFILE, provider }, `provision:${provider}`),
      ).rejects.toMatchObject({
        code: "invalid_profile",
        message: HOST_KEY_ERROR,
      });
  ...
  it.each([
    ...
    { profile: { ...PROFILE, binary: "botbox" }, message: "absolute path" },
  ...
  it("routes lifecycle calls from the passed profile context", async () => {
    const binary = path.resolve(path.sep, "custom", "botbox");
    const calls: string[][] = [];
    const provider = createCrabboxWorkerProvider({
      runCommand: async (argv) => {
        calls.push(argv);
  ...
  it("resolves its lease-bound identity marker through current inspect output", async () => {
    ...
    ).resolves.toEqual({ kind: "path", path: "/tmp/botbox-worker-key" });
    expect(calls).toEqual([
      [
        SIBLING_BINARY,
        "inspect",
  ...
  it("rejects a Crabbox identity marker for another lease before invoking the CLI", async () => {
    ...
    await expect(
      provider.resolveSshIdentity({
        leaseId: LEASE_ID,
        profile: PROFILE,
        keyRef: { source: "file", provider: "botbox", id: "/leases/cbx_other/identity" },
      }),
  ...
  it("rejects non-Crabbox lifecycle lease ids before invoking the CLI", async () => {
    ...
    const lease = lifecycleLease("lease:not-botbox");
...
describe("Crabbox binary resolution", () => {
  it("prefers explicit, then sibling, then PATH, then the bare command", () => {
    ...
    const pathBinary = path.join(toolsDir, "botbox");
    const relativePathBinary = path.resolve("relative-tools", "botbox");
    const explicitBinary = path.resolve(path.sep, "custom", "botbox");
  ...
  it("derives the package root from source and bundled plugin roots", () => {
    expect(resolveBotRoot(path.join(BOT_ROOT, "extensions", "botbox"))).toBe(
      BOT_ROOT,
    );
    expect(resolveBotRoot(path.join(BOT_ROOT, "dist", "extensions", "botbox"))).toBe(
      BOT_ROOT,
    );
  });
});
