...
import { runCommandWithTimeout, type SpawnResult } from "bot/plugin-sdk/process-runtime";
import { truncateUtf16Safe } from "bot/plugin-sdk/text-utility-runtime";
import { parseInspectJson, type ParsedInspect } from "./botbox-worker-inspect.js";
import {
  identityRefId,
  nonEmptyString,
  operationSlug,
...
} from "./botbox-worker-profile.js";

export { resolveBotRoot } from "./botbox-worker-profile.js";

const CRABBOX_WORKER_PROVIDER_ID = "botbox";
const CRABBOX_KEY_REF_PROVIDER = "botbox";
...
function provisionProfileError(result: SpawnResult): WorkerProviderError | undefined {
  ...
  if (/\bprovider=\S+\s+is one-shot; use botbox run\b/u.test(output)) {
    return new WorkerProviderError("Crabbox profile provider is run-only");
  }
  if (/\bprovider=\S+\s+requires module source; use botbox run --script\b/u.test(output)) {
  ...
  try {
    result = await runCrabboxCommand({
      ...
      args: [
        ...
        params.inspect.id,
        "--keep=true",
        // Workspace transfer is owned by the worker tunnel; botbox run must not
...
export function createCrabboxWorkerProvider(
  ...
  const botRoot = dependencies.botRoot ?? process.cwd();
