    ...
    if (settingsPath && settings.hooks !== undefined) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation: "Review hook commands before recreating equivalent Bot automation.",
        }),
      );
    }
    ...
    if (settingsPath && settings.env !== undefined) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Move non-secret values manually and store credentials through Bot credential flows.",
        }),
      );
    }
  }
