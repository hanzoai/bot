...
import type { BotPluginNodeInvokePolicy } from "bot/plugin-sdk/plugin-entry";
import { FILE_TRANSFER_NODE_INVOKE_COMMANDS } from "./node-invoke-policy-commands.js";

type LoadFileTransferNodeInvokePolicy = () => Promise<BotPluginNodeInvokePolicy>;
...
export function createLazyFileTransferNodeInvokePolicy(
  ...
  let policyPromise: Promise<BotPluginNodeInvokePolicy> | undefined;

  return {
    commands: [...FILE_TRANSFER_NODE_INVOKE_COMMANDS],
    async handle(ctx) {
      let policy: BotPluginNodeInvokePolicy;
      try {
