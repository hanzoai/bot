...
function resolveMSTeamsReadGroupPolicy(cfg: BotConfig) {
  const teams = cfg.channels?.msteams;
...
function hasMutableChannelConfig(cfg: BotConfig): boolean {
...
function bothUnknownScopesAllowed(cfg: BotConfig): boolean {
  const teams = cfg.channels?.msteams;
...
export async function assertMSTeamsTeamEnumerationAllowed(params: {
  ...
  const directRoute = resolveMSTeamsRouteConfig({
    ...
    conversationId: "__bot_all_channels__",
    allowNameMatching: isDangerousNameMatchingEnabled(teams),
  });
