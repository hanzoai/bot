...
vi.mock("bot/plugin-sdk/secret-file", async (importOriginal) => {
  const actual = await importOriginal<typeof import("bot/plugin-sdk/secret-file")>();
  return {
    ...actual,
    readSecretFile,
  };
...
describe("createMSTeamsApp", () => {
  ...
  it("preserves both Teams SDK and Bot User-Agent fragments", async () => {
    const creds: MSTeamsCredentials = {
      type: "secret",
      appId: "test-app-id",
      appPassword: "test-secret",
    ...
    expect(headers?.["User-Agent"]).toMatch(/^teams\.ts\[apps\]\/\S+ Bot\/\S+$/);
  });
  ...
  it("passes China cloud to the SDK App without requiring a configured serviceUrl", async () => {
    ...
    // default. Bot proactive sends use stored reference serviceUrls instead.
