...
import { appendRegularFile } from "bot/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/bot-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/bot";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/bot-codex-home";
...
type QaMultipassPlan = {
  ...
  channelDriverSelection?: BotCrablineChannelDriverSelection;
...
function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `bot-qa-${createVmSuffix()}`;
...
function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "BOT_CONFIG_PATH" &&
          key !== "BOT_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `BOT_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `BOT_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm bot qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredBotTmpDir(), `${plan.vmName}-qa-suite-`),
  );
