...
import { listSlackReactions } from "@hanzo/bot-slack/api.js";
...
export function resolveCodexFileApprovalTargetPath(token: string) {
  return path.join(os.homedir(), `.bot-qa-codex-file-approval-${token.toLowerCase()}.txt`);
}
...
function findPendingCodexPluginApprovalRecord(params: {
  ...
  for (const entry of list) {
    ...
    if (
      request.pluginId === "bot-codex-app-server" &&
