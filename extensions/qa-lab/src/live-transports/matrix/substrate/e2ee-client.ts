...
import type {
  ...
  MessageEventContent,
} from "@hanzo/bot-matrix/test-api.js";
import type {
...
type MatrixQaE2eeRuntime = typeof import("@hanzo/bot-matrix/test-api.js");
...
function resolveMatrixQaPluginStateNamespaceKey(options: OpenKeyedStoreOptions): string {
  return `${options.env?.BOT_STATE_DIR ?? ""}\0${options.namespace}`;
}
...
export async function loadMatrixQaE2eeRuntime(): Promise<MatrixQaE2eeRuntime> {
  const { loadQaRunnerBundledPluginTestApi } =
    await import("bot/plugin-sdk/qa-runner-runtime");
