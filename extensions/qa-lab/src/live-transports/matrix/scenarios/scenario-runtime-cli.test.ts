...
import {
  ...
  resolveMatrixQaBotCliEntryPath,
...
const testing = { killMatrixQaCliChild, resolveMatrixQaBotCliEntryPath };
...
describe("Matrix QA CLI runtime", () => {
  ...
  it("prefers the ESM Bot CLI entrypoint when present", async () => {
    const root = await mkdtemp(path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-entry-"));
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(path.join(root, "dist", "index.mjs"), "");
      expect(testing.resolveMatrixQaBotCliEntryPath(root)).toBe(
  ...
  it("can preserve expected non-zero CLI output for negative scenarios", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-nonzero-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
      ...
      const result = await runMatrixQaBotCli({
        allowNonZero: true,
  ...
  it("can pass stdin to CLI commands", async () => {
    const root = await mkdtemp(path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-stdin-"));
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
        path.join(root, "dist", "index.mjs"),
  ...
  it("can close stdin after interactive CLI prompts", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-interactive-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
      ...
      const session = startMatrixQaBotCli({
  ...
  it("includes timed-out CLI output in diagnostics", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-timeout-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
  ...
  it("kills CLI commands that ignore graceful timeout termination", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-timeout-kill-"),
    );
  ...
  it("preserves timeout diagnostics when wait attaches after timeout", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-late-wait-timeout-"),
    );
    const pidPath = path.join(root, "cli.pid");
    let childPid: number | undefined;
    try {
  ...
  it("settles and kills descendants that keep timed-out CLI stdio open", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-timeout-tree-"),
    );
  ...
  it("kills ignored-stdio descendants after a timed-out CLI exits gracefully", async () => {
    ...
    const root = await mkdtemp(
      path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-timeout-ignored-stdio-"),
    );
    const childPidPath = path.join(root, "child.pid");
    const grandchildPidPath = path.join(root, "grandchild.pid");
    let childPid: number | undefined;
  ...
  it("kills ignored-stdio descendants after manual CLI session kill", async () => {
    ...
    const root = await mkdtemp(
      path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-session-kill-ignored-stdio-"),
    );
    const childPidPath = path.join(root, "child.pid");
    const grandchildPidPath = path.join(root, "grandchild.pid");
    let childPid: number | undefined;
