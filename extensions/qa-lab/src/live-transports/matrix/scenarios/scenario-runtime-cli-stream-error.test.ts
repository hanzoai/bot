...
import { startMatrixQaBotCli } from "./scenario-runtime-cli.js";
...
async function createCliRoot(): Promise<{
  ...
  const root = await mkdtemp(
    path.join(resolvePreferredBotTmpDir(), "matrix-qa-cli-stream-error-"),
  );
...
describe("Matrix QA CLI runtime stream errors", () => {
  ...
  it.each(["stdout", "stderr"] as const)(
    ...
    async (streamName) => {
      ...
      let session: ReturnType<typeof startMatrixQaBotCli> | undefined;
      let childClosed = false;
      try {
        session = startMatrixQaBotCli({
