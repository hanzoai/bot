...
export type MatrixQaCliRuntime = Awaited<ReturnType<typeof createMatrixQaBotCliRuntime>>;
...
export function requireMatrixQaGatewayConfigPath(context: MatrixQaScenarioContext) {
  const configPath = requireMatrixQaCliRuntimeEnv(context).BOT_CONFIG_PATH?.trim();
  if (!configPath) {
...
export async function createMatrixQaRecoveryCliRuntime(params: {
  ...
  return await createMatrixQaBotCliRuntime({
