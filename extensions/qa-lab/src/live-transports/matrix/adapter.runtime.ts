...
import { buildQaTarget } from "bot/plugin-sdk/qa-channel-protocol";
...
export async function createMatrixQaTransportAdapter(
  ...
  return {
    ...
    async sendInbound(input) {
      ...
      const hasPortableMention = input.text.includes("@bot");
      const body = input.text.replaceAll("@bot", provisioning.sut.userId);
    ...
    createGatewayConfig: () =>
      buildMatrixQaConfig({} as BotConfig, {
    ...
    createRuntimeEnvPatch: () => ({
      BOT_QA_MATRIX_DRIVER_USER_ID: provisioning.driver.userId,
      BOT_QA_MATRIX_OBSERVER_USER_ID: provisioning.observer.userId,
      BOT_QA_MATRIX_SUT_ACCOUNT_ID: accountId,
      BOT_QA_MATRIX_MAIN_ROOM_ID:
        provisioning.topology.rooms.find((room) => room.key === "main")?.roomId ??
        provisioning.roomId,
      BOT_QA_MATRIX_SECONDARY_ROOM_ID:
