...
describe("buildQaGatewayConfig", () => {
  it("keeps mock-openai as the default provider lane", () => {
    ...
    expect(cfg.messages?.groupChat?.mentionPatterns).toEqual(["\\b@?bot\\b"]);
  ...
  it("keeps forced Codex cells free of Bot request params", () => {
    const cfg = buildQaGatewayConfig({
      bind: "loopback",
      gatewayPort: 18789,
      gatewayToken: "token",
  ...
  it("pins control ui to a provided built root when available", () => {
    const cfg = buildQaGatewayConfig({
      ...
      gatewayToken: "token",
      workspaceDir: "/tmp/qa-workspace",
      controlUiRoot: "/tmp/bot/dist/control-ui",
      ...createQaChannelTransportParams(),
    });

    expect(cfg.gateway?.controlUi?.enabled).toBe(true);
    expect(cfg.gateway?.controlUi?.root).toBe("/tmp/bot/dist/control-ui");
  });
