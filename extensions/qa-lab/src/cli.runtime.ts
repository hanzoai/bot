...
import {
  ...
  resolveBotCrablineChannelDriverSelection,
...
const QA_CREDENTIAL_PAYLOAD_MAX_BYTES_ENV = "BOT_QA_CREDENTIAL_PAYLOAD_MAX_BYTES";
...
function parseQaRuntimePair(value: string | undefined): [RuntimeId, RuntimeId] | undefined {
  ...
  if (runtimes.length !== 2) {
    throw new Error('--runtime-pair must use exactly two runtimes, e.g. "bot,codex".');
  }
  const [left, right] = runtimes;
  if (!left || !right) {
    throw new Error('--runtime-pair only supports "bot" and "codex".');
  }
  if (left === right) {
...
export async function runQaProfileCommand(opts: QaProfileCommandOptions) {
  ...
  const executionSelection = resolveQaRunProfileExecutionSelection({
    ...
    defaultChannel:
      profileReport.channelDriver === "crabline" ? BOT_CRABLINE_DEFAULT_CHANNEL : undefined,
    supportsChannel:
...
async function withTemporaryQaProfileEnv<T>(profile: string, run: () => Promise<T>): Promise<T> {
  ...
  process.env.BOT_QA_PROFILE = profile;
  try {
    return await run();
  } finally {
...
export async function runQaSuiteCommand(opts: QaSuiteCommandOptions) {
  ...
  const runtimePairLaneSelection = resolveQaRuntimePairLaneScenarioIds({
    ...
    channelDriver,
    claudeCliAuthMode,
    defaultChannel: channelDriver === "crabline" ? BOT_CRABLINE_DEFAULT_CHANNEL : undefined,
...
export async function runQaJsonlReplayCommand(opts: {
  ...
  const repoRoot = path.resolve(opts.repoRoot ?? process.cwd());
  const runtimePair = parseQaRuntimePair(opts.runtimePair) ?? ["bot", "codex"];
  if (runtimePair[0] !== "bot" || runtimePair[1] !== "codex") {
    throw new Error('--runtime-pair for jsonl-replay must be "bot,codex".');
  }
...
export async function runQaLabUiCommand(opts: {
  ...
  const server = await startQaLabServer({
    ...
    controlUiProxyToken: process.env.BOT_QA_CONTROL_UI_PROXY_TOKEN,
