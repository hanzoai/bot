  ...
  try {
    commandResult = await params.runCommand({
      ...
      env: {
        ...
        BOT_DOCKER_ALL_LANES: laneNames.join(","),
        BOT_DOCKER_ALL_LANE_TIMEOUT_MS: String(params.commandTimeoutMs),
        BOT_DOCKER_ALL_LOG_DIR: dockerOutputDir,
        BOT_DOCKER_ALL_PROFILE: "all",
        BOT_DOCKER_ALL_TIMINGS_FILE: path.join(dockerOutputDir, "lane-timings.json"),
      },
