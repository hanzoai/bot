...
async function createBuiltInQaTransport(
  ...
  if (context.driver === "crabline") {
    const { resolveBotCrablineChannelDriverSelection } = await import("@openclaw/crabline");
    const selection = resolveBotCrablineChannelDriverSelection({ channel: context.channelId });
