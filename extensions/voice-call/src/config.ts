...
import { REALTIME_VOICE_AGENT_CONSULT_TOOL_POLICIES } from "bot/plugin-sdk/realtime-voice";
...
import { normalizeWebhookPath } from "bot/plugin-sdk/webhook-ingress";
import { z } from "zod";
...
const VoiceCallRealtimeConfigSchema = z
  .object({
    ...
    /** Tool policy for the shared Bot agent consult tool. */
    toolPolicy: VoiceCallRealtimeToolPolicySchema.default("safe-read-only"),
    /** Guidance for when the realtime model should call the Bot agent consult tool. */
