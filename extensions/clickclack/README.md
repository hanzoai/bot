# ClickClack Bot channel

Official Bot channel plugin for ClickClack.

## Install

```sh
bot plugins install @hanzo/bot-clickclack
```
...
`/api/bot-setup-codes/claim#CODE` endpoint. Bot validates the versioned
claim response and saves its canonical API base.

For manual token setup:

```sh
bot channels add clickclack \
...
Run `bot onboard` for guided setup. The workspace value can be a
...
authentication gateway such as Cloudflare Access while the local Bot
...
extension publishes Bot's native commands for composer autocomplete,
...
ClickClack can create one managed channel for each Bot session:
...
concrete Bot session id as well as the pinned server and channel. Resetting
...
two Bot gateways using the same ClickClack workspace do not adopt each
...
workspace-scoped credential that cannot access the old workspace, Bot
...
See `docs/channels/clickclack.md` in the Bot repository, or the published docs at `https://docs.bot.ai/channels/clickclack`.
