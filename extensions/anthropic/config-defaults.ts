...
function usesClaudeCliModelSelection(config: BotConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: BotConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: BotConfig): string[] {
  const refs = new Set<string>(
...
/** Apply Anthropic and Claude CLI defaults to an Bot config object. */
