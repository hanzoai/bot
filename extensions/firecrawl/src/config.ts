...
import { canResolveEnvSecretRefInReadOnlyPath } from "bot/plugin-sdk/extension-shared";
import { resolvePositiveTimeoutSeconds } from "bot/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "bot/plugin-sdk/secret-input";
...
function resolveFirecrawlSearchConfig(cfg?: BotConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: BotConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: BotConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: BotConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
...
export function resolveFirecrawlOnlyMainContent(cfg?: BotConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: BotConfig, override?: number): number {
