...
import { createSubsystemLogger } from "bot/plugin-sdk/logging-core";
import {
  resolveClosestSize,
  resolveGeneratedMediaMaxBytes,
} from "bot/plugin-sdk/media-generation-runtime";
...
function resolveConfiguredOpenAIImageBaseUrl(cfg: BotConfig | undefined, model: string) {
...
function shouldAllowPrivateImageEndpoint(req: {
  ...
  return process.env.BOT_QA_ALLOW_LOCAL_IMAGE_PROVIDER === "1";
}
...
function hasExplicitOpenAIImageApiKeyConfig(cfg: BotConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return providerConfig?.apiKey !== undefined || providerConfig?.auth === "api-key";
}

function hasExplicitDirectOpenAIImageConfig(cfg: BotConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  if (!providerConfig) {
    return false;
  }
...
function hasChatGPTImageRouteConfig(cfg: BotConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return (
...
function forceOpenAIImageApiKeyAuth(cfg: BotConfig | undefined): BotConfig | undefined {
