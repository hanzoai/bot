...
import { readCodexCliCredentialsCached } from "bot/plugin-sdk/provider-auth";
...
const LIVE_ENABLED =
  process.env.BOT_LIVE_TEST === "1" && process.env.BOT_LIVE_GPT_LIVE === "1";
const describeLive = LIVE_ENABLED ? describe : describe.skip;
const LIVE_TIMEOUT_MS = 60_000;

type BrowserWithGptLivePeer = typeof globalThis & {
  botGptLivePeer?: RTCPeerConnection;
};
...
async function createBrowserOffer(page: Page): Promise<string> {
  return await page.evaluate(async () => {
    ...
    (globalThis as BrowserWithGptLivePeer).botGptLivePeer = peer;
    return sdp;
  });
}
...
async function applyBrowserAnswer(page: Page, sdp: string): Promise<void> {
  await page.evaluate(async (answerSdp) => {
    const peer = (globalThis as BrowserWithGptLivePeer).botGptLivePeer;
    if (!peer) {
...
async function closeBrowserPeer(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    target.botGptLivePeer?.close();
    delete target.botGptLivePeer;
  });
}
