...
import { resolveExpiresAtMsFromEpochSeconds } from "bot/plugin-sdk/number-runtime";
import {
  createProviderHttpError,
  readProviderJsonResponse,
  resolveProviderRequestHeaders,
} from "bot/plugin-sdk/provider-http";
import { captureWsEvent } from "bot/plugin-sdk/proxy-capture";
  ...
  const payload = await (async () => {
    try {
      if (!response.ok) {
        ...
        // Bot auth source won. Keep the status metadata, but give callers
