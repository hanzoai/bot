...
describe("native Codex thread tool", () => {
  ...
  async function withFixture(run: () => void | Promise<void>): Promise<void> {
    await withTempDir("bot-codex-threads-", async (tempRoot) => {
      root = tempRoot;
  ...
  function createTool(params?: {
    ...
    const context: BotPluginToolContext = {
      config: {},
      agentId: "main",
  ...
  it("forks a native thread and attaches the fork to the Bot session", () =>
    withFixture(async () => {
      const request = vi.fn(async (_config, method: string) =>
        method === CODEX_CONTROL_METHODS.readThread
  ...
  it("rejects archive when another Bot session owns the thread", () =>
    withFixture(async () => {
      await writeCodexAppServerBinding("session-id", {
      ...
      ).rejects.toThrow("owned by another Bot session");
  ...
  it("rejects archive when a spawned descendant is owned by an Bot session", () =>
    withFixture(async () => {
      await writeCodexAppServerBinding("session-id", {
        threadId: "current-thread",
        cwd: "/tmp/project",
      ...
      ).rejects.toThrow("spawned descendant is owned by an Bot session");
