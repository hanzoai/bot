...
export function createCodexSessionCatalogControl(params: {
  ...
  const requestOptionsByConfig = new WeakMap<BotConfig, CodexCatalogRequestOptions>();
  ...
  return {
    ...
    async listPage(pageParams) {
      ...
      if (pageParams.forceRefresh !== true && cached) {
        ...
        // session created outside Bot). The TTL is a refresh trigger, never a serve gate.
        cache.delete(key);
