...
const LIVE =
  process.env.BOT_LIVE_TEST === "1" &&
  process.env.BOT_LIVE_CODEX_THREAD_COEXISTENCE === "1";
...
describeLive("native Codex thread coexistence", () => {
  it("shares thread storage across independent app-server processes", async () => {
    await withTempDir("bot-codex-coexistence-", async (root) => {
      try {
        ...
        const started = await withClient(clientOptions, async (first) => {
          ...
          await first.request(
            "thread/name/set",
            { threadId: response.thread.id, name: "Bot coexistence source" },
            { timeoutMs: 60_000 },
          );
          return response;
        });
        ...
        const forked = await withClient(clientOptions, async (second) => {
          ...
          await second.request(
            "thread/name/set",
            { threadId: response.thread.id, name: "Bot coexistence fork" },
            { timeoutMs: 60_000 },
          );
          await second.request(
            "thread/archive",
