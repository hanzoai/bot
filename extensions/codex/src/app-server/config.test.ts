...
describe("Codex app-server config", () => {
  it("only auto-approves app-server approvals for full yolo runtime policy", () => {
    ...
    expect(
      shouldAutoApproveCodexAppServerApprovals({
        ...
        sandbox: "danger-full-access",
        networkProxy: {
          profileName: "bot-network",
          configFingerprint: "network-proxy-v1",
          configPatch: {
            "features.network_proxy.enabled": true,
            default_permissions: "bot-network",
            permissions: {},
          },
        },
      }),
  ...
  it("parses typed plugin config before falling back to environment knobs", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        BOT_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        BOT_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
  ...
  it("uses read-only filesystem rules for read-only network proxy profiles", () => {
    ...
    expect(profileName).toMatch(/^bot-network-[a-f0-9]{16}$/u);
    ...
    expect(
      readCodexPluginConfig({
        appServer: {
          ...
          remoteWorkspace: {
            localRoot: "/Users/kevinlin/code/bot",
            remoteRoot: "/home/oai/bot-workspaces",
          },
        },
      }),
    ).toStrictEqual({});
    ...
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          ...
          url: "wss://codex-app-server.example.internal/ws",
          authToken: "capability-token",
          remoteWorkspaceRoot: " /home/oai/bot-workspaces ",
        },
      },
    });
  ...
  it.each([
    ...
    { label: "private build without a forced runtime", env: { BOT_BUILD_PRIVATE_QA: "1" } },
    {
      label: "forced runtime without a private build",
      env: { BOT_QA_FORCE_RUNTIME: "codex" },
    },
    {
      label: "forced private-QA Codex runtime without explicit sandbox configuration",
      env: { BOT_BUILD_PRIVATE_QA: "1", BOT_QA_FORCE_RUNTIME: "codex" },
    },
    {
      label: "forced private-QA Bot runtime",
      env: { BOT_BUILD_PRIVATE_QA: "1", BOT_QA_FORCE_RUNTIME: "bot" },
    },
  ...
  it("checks shared user config before enabling model-backed approval review", async () => {
    await withTempDir("bot-codex-user-home-", async (codexHome) => {
      await fs.writeFile(
  ...
  it("treats only explicit OpenAI model context as safe for Codex-backed auto-review", () => {
    ...
    for (const openAIProvider of [
      ...
      {
        baseUrl: "https://api.openai.com/v1",
        headers: { "x-bot-reviewer-proxy": "local" },
        models: [],
      },
      {
        baseUrl: "https://api.openai.com/v1",
...
`,
  ...
  it("keeps explicit yolo mode when requirements disallow full access", () => {
    ...
    expectRuntimePolicy(
      resolveRuntimeForTest({
        pluginConfig: {},
        env: { BOT_CODEX_APP_SERVER_MODE: "yolo" },
        requirementsToml,
      }),
      {
        approvalPolicy: "never",
  ...
  it("rejects the retired dynamic tool profile key", () => {
    expect(
      readCodexPluginConfig({
        codexDynamicToolsProfile: "bot-compat",
        codexDynamicToolsLoading: "direct",
      }),
    ).toEqual({});
  });
  ...
  it("treats configured and environment commands as explicit overrides", () => {
    expectFields(
      resolveRuntimeForTest({
        ...
        env: { BOT_CODEX_APP_SERVER_BIN: "/usr/local/bin/codex" },
      }).start,
      "configured start",
      {
  ...
  it("keeps desktop ownership for Computer Use persisted in an agent Codex home", async () => {
    await withTempDir("bot-codex-agent-home-", async (agentDir) => {
      const startOptions = resolveRuntimeForTest({ pluginConfig: {} }).start;
  ...
  it("uses desktop-first when persisted Codex state cannot be read", async () => {
    await withTempDir("bot-codex-unreadable-home-", async (agentDir) => {
      const startOptions = resolveRuntimeForTest({ pluginConfig: {} }).start;
  ...
  it("rejects Codex app-server command overrides that include inline arguments", () => {
    ...
    ).toThrow("BOT_CODEX_APP_SERVER_BIN must be only the Codex app-server executable path");
  });
  ...
  it("resolves Computer Use setup from plugin config and environment fallbacks", () => {
    expect(
      resolveCodexComputerUseConfig({
        ...
        env: {
          BOT_CODEX_COMPUTER_USE_PLUGIN_NAME: "env-fallback-plugin",
        },
      }),
    ).toEqual({
      enabled: true,
    ...
    expectFields(
      resolveCodexComputerUseConfig({
        pluginConfig: {},
        env: {
          BOT_CODEX_COMPUTER_USE: "1",
          BOT_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE: "github:example/plugins",
          BOT_CODEX_COMPUTER_USE_AUTO_INSTALL: "true",
          BOT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: "30000",
        },
      }),
      "computer use config",
      {
    ...
    for (const value of ["0x10", "1e3"]) {
      expectFields(
        resolveCodexComputerUseConfig({
          ...
          env: {
            BOT_CODEX_COMPUTER_USE: "1",
            BOT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: value,
          },
        }),
        "computer use config",
        {
  ...
  it("resolves Computer Use operational policy knobs", () => {
    expectFields(
      resolveCodexComputerUseConfig({
        ...
        env: {
          BOT_CODEX_COMPUTER_USE_HEALTH_CHECK_ENABLED: "false",
          BOT_CODEX_COMPUTER_USE_HEALTH_CHECK_INTERVAL_MINUTES: "240",
          BOT_CODEX_COMPUTER_USE_STRICT_READINESS: "false",
          BOT_CODEX_COMPUTER_USE_AUTO_REPAIR: "false",
        },
      }),
      "computer use config",
      {
    ...
    expectFields(
      resolveCodexComputerUseConfig({
        pluginConfig: { computerUse: { enabled: true } },
        env: {
          BOT_CODEX_COMPUTER_USE_HEALTH_CHECK_ENABLED: "1",
          BOT_CODEX_COMPUTER_USE_HEALTH_CHECK_INTERVAL_MINUTES: "90",
          BOT_CODEX_COMPUTER_USE_STRICT_READINESS: "true",
          BOT_CODEX_COMPUTER_USE_AUTO_REPAIR: "true",
          BOT_CODEX_COMPUTER_USE_PLUGIN_CACHE_MODE: "stale-copy",
        },
      }),
      "computer use config",
      {
  ...
  it("allows environment mode fallback to opt in to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      modelProvider: "openai",
      env: { BOT_CODEX_APP_SERVER_MODE: "guardian" },
    });
  ...
  it("maps normalized Bot auto exec mode to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
      modelProvider: "openai",
  ...
  it("forces guarded app-server policy fields for auto mode", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        BOT_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        BOT_CODEX_APP_SERVER_SANDBOX: "danger-full-access",
      },
      execMode: "auto",
      modelProvider: "openai",
    });
  ...
  it("preserves explicit read-only app-server sandbox for auto mode", () => {
    ...
    const envRuntime = resolveRuntimeForTest({
      ...
      modelProvider: "openai",
      env: {
        BOT_CODEX_APP_SERVER_MODE: "yolo",
        BOT_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        BOT_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
    });
  ...
  it.each(["deny", "allowlist"] as const)(
    "blocks Codex app-server local execution for normalized Bot %s exec mode",
    (execMode) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("maps normalized Bot ask exec mode away from Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "ask",
    });
  ...
  it("fails closed when normalized Bot ask mode cannot use user approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "ask",
  ...
  it.each([
    ...
    "fails closed when normalized Bot $execMode mode can only use $policies approvals",
    ({ execMode, policies }) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("keeps normalized Bot full exec mode on default Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "full",
    });
  ...
  it("uses user approvals when normalized Bot auto mode cannot use Codex auto-review", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
      requirementsToml:
  ...
  it("keeps normalized Bot auto mode when legacy app-server yolo was schema-defaulted", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          command: "codex",
  ...
  it("forces guarded policy fields for normalized Bot auto mode", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          approvalPolicy: "never",
  ...
  it.each(["always"] as const)(
    ...
    (ask) => {
      ...
      const execPolicy = resolveBotExecPolicyForCodexAppServer({ config });
  ...
  it("fails closed when legacy full exec with ask cannot use full Codex sandbox", () => {
    ...
    expect(() =>
      resolveRuntimeForTest({
        execPolicy: resolveBotExecPolicyForCodexAppServer({ config }),
        requirementsToml: 'allowed_sandbox_modes = ["read-only", "workspace-write"]\n',
      }),
    ).toThrow("legacy full exec security with ask requires Codex app-server danger-full-access");
  });

  it("clamps legacy full exec with ask when an Bot sandbox is active", () => {
    const config = {
      tools: {
        exec: {
          security: "full",
  ...
  it("ignores removed BOT_CODEX_APP_SERVER_GUARDIAN fallback", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { BOT_CODEX_APP_SERVER_GUARDIAN: "1" },
    });
  ...
  it("publishes stable defaults without schema-defaulting mode-derived policy fields", async () => {
    const manifest = JSON.parse(
      await fs.readFile(new URL("../../bot.plugin.json", import.meta.url), "utf8"),
    ) as {
      configSchema: {
        properties: {
          appServer: {
