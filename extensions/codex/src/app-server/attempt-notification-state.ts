...
import {
  ...
  isPendingBotDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingBotDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingBotDynamicToolCompletionNotification(
    notification,
    params.pendingBotDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after Bot hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served Bot
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingBotDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }
