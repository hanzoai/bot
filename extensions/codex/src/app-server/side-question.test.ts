...
vi.mock("bot/plugin-sdk/agent-harness", () => ({
  createBotCodingTools: (...args: unknown[]) => createBotCodingToolsMock(...args),
}));
...
describe("runCodexAppServerSideQuestion", () => {
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createBotCodingToolsMock);
  ...
  it("allocates one fallback run ID per side-question invocation", async () => {
    ...
    const runIds = createBotCodingToolsMock.mock.calls.map(
  ...
  it("uses the default supervision runtime, native auth, and exact bound model pair", async () => {
    ...
    expect(createBotCodingToolsMock).toHaveBeenCalledWith(
  ...
  it("disables hosted search when side-question sender policy removes managed web_search", async () => {
    createBotCodingToolsMock.mockImplementation((options: { senderId?: string }) =>
  ...
  it("applies native search restrictions to side forks and suppresses managed search", async () => {
    ...
    expect(toolResponse).toEqual({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown Bot tool: web_search" }],
    });
  ...
  it("preserves managed web_search while planning hosted search for Responses side questions", async () => {
    createBotCodingToolsMock.mockImplementation(
  ...
  it("rejects /btw before forking when the current Bot session is sandboxed", async () => {
    await expect(
      runCodexAppServerSideQuestion(
        sideParams({
      ...
      "Codex-native /btw side-question mode is unavailable because Bot sandboxing is active for this session.",
    );
  ...
  it("rejects /btw before forking when exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because Bot exec host=node is active for this session.",
    );
  ...
  it("bridges side-thread dynamic tool requests to Bot tools", async () => {
    const client = createFakeClient();
    let toolResponse: unknown;
    client.request.mockImplementation(async (method: string) => {
      if (method === "thread/fork") {
  ...
  it("omits computer control from side threads without a compaction owner", async () => {
    ...
    expect(toolResponse).toEqual({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown Bot tool: computer" }],
    });
  });
