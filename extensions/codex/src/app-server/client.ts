...
export class CodexAppServerClient {
  ...
  /** Registers a handler for app-server requests sent back to Bot. */
...
class CodexAppServerVersionError extends Error {
  ...
  constructor(detectedVersion: string | undefined) {
    const detected = detectedVersion
      ...
      : "Bot could not determine the running Codex version";
    super(
...
function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // Bot, Codex Desktop, or an env override, so only the slash-delimited
...
/** Returns true for app-server approval request methods Bot can answer. */
