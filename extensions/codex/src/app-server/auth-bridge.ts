...
import { readSecretFile } from "bot/plugin-sdk/secret-file";
...
async function assertNativeCodexAccountMatchesRoute(
  ...
  if (accountType === "chatgpt") {
    throw createCodexAppServerAuthError(
      'Codex Platform route requires an API-key account, but the native Codex home is signed in with a ChatGPT subscription. Sign that home in with `codex login --with-api-key`, or set appServer.homeScope="agent" so Bot can inject its own key.',
    );
  }
}
