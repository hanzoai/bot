...
const mocks = vi.hoisted(() => ({
  ...
  resolveDefaultAgentDir: vi.fn(() => "/tmp/bot-agent"),
}));
...
describe("shared Codex app-server client", () => {
  ...
  it("closes the shared app-server when the version gate fails", async () => {
    ...
    await sendInitializeResult(harness, "bot/0.117.9 (macOS; test)");
  ...
  it("skips auth-store resolution only while the same config-owned client stays warm", async () => {
    ...
    await sendInitializeResult(first, "bot/0.143.0 (Linux; test)");
    ...
    await sendInitializeResult(replacement, "bot/0.143.0 (Linux; test)");
  ...
  it("rejects an aborted startup acquire while another caller keeps initialization alive", async () => {
    ...
    await sendInitializeResult(harness, "bot/0.143.0 (Linux; test)");
  ...
  it("falls back to the next managed app-server when desktop initialize is unsupported", async () => {
    ...
    mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(async (startOptions) => ({
      ...
      managedFallbackCommandPaths: ["/cache/bot/codex"],
    }));

    const listPromise = listCodexAppServerModels({ timeoutMs: 1000 });
    await sendInitializeResult(desktop, "bot/0.124.9 (macOS; test)");
    await sendInitializeResult(pluginLocal, "bot/0.143.0 (macOS; test)");
  ...
  it("keeps capture clients separate from ordinary shared clients", async () => {
    await withTempDir("bot-codex-capture-client-", async (root) => {
      ...
      await sendInitializeResult(normal, "bot/0.143.0 (Linux; test)");
      ...
      await sendInitializeResult(captured, "bot/0.143.0 (Linux; test)");
  ...
  it("binds the managed fallback candidate that actually initialized", async () => {
    await withTempDir("bot-codex-capture-fallback-", async (root) => {
      ...
      await sendInitializeResult(desktop, "bot/0.124.9 (macOS; test)");
      await sendInitializeResult(fallback, "bot/0.143.0 (macOS; test)");
  ...
  it("detects persisted Computer Use enabled after managed client startup", async () => {
    await withTempDir("bot-codex-managed-selection-", async (root) => {
      const harness = createClientHarness();
      vi.spyOn(CodexAppServerClient, "start").mockReturnValue(harness.client);
      mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(
        async (startOptions) => ({
      ...
      await sendInitializeResult(harness, "bot/0.143.0 (macOS; test)");
      ...
      expect(readCodexAppServerClientProcessIdentity(client)).toEqual({
        ...
        commandSource: "resolved-managed",
        nativeCommand: "/cache/bot/codex.native",
        serverVersion: "0.143.0",
        userAgent: "bot/0.143.0 (macOS; test)",
      });
  ...
  it.each(["abort", "timeout"] as const)(
    ...
    async (mode) => {
      await withTempDir("bot-codex-guarded-request-cancel-", async (root) => {
        const harness = createClientHarness();
        vi.spyOn(CodexAppServerClient, "start").mockReturnValue(harness.client);
        mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(
          async (startOptions) => ({
  ...
  it("closes and clears a shared app-server when initialize times out", async () => {
    ...
    await sendInitializeResult(second, "bot/0.143.0 (macOS; test)");
  ...
  it("separates prepared profile clients by secret-free account identity", async () => {
    ...
    await sendInitializeResult(firstHarness, "bot/0.143.0 (macOS; test)");
    ...
    await sendInitializeResult(secondHarness, "bot/0.143.0 (macOS; test)");
  ...
  it("registers persisted profile refresh for isolated app-server startup", async () => {
    ...
    const clientPromise = createIsolatedCodexAppServerClient({
      ...
      agentDir: "/tmp/bot-persisted-agent",
    });
    await sendInitializeResult(harness, "bot/0.143.0 (macOS; test)");
  ...
  it("skips target auth resolution when native source auth is requested", async () => {
    ...
    expect(mocks.resolveCodexAppServerAuthProfileIdForAgent).not.toHaveBeenCalled();
    const bridgeCall = bridgeStartOptionsCall();
    expect(bridgeCall.agentDir).toBe("/tmp/bot-target-agent");
    expect(bridgeCall.authProfileId).toBeNull();
    expect(bridgeCall.config).toBe(config);
    const applyCall = applyAuthProfileCall();
    expect(applyCall.agentDir).toBe("/tmp/bot-target-agent");
  ...
  it("uses the selected agent dir for shared app-server auth bridging", async () => {
    ...
    await expect(listPromise).resolves.toEqual({ models: [] });
    const bridgeCall = bridgeStartOptionsCall();
    expect(bridgeCall?.agentDir).toBe("/tmp/bot-agent-nova");
    expect(bridgeCall?.authProfileId).toBe("openai:work");
    const applyCall = applyAuthProfileCall();
    expect(applyCall?.agentDir).toBe("/tmp/bot-agent-nova");
    expect(applyCall?.authProfileId).toBe("openai:work");
  });
  ...
  it("keeps an active shared client alive when another agent dir uses a different key", async () => {
    ...
    const firstList = listCodexAppServerModels({
      timeoutMs: 1000,
      agentDir: "/tmp/bot-agent-one",
    });
    await sendInitializeResult(first, "bot/0.143.0 (macOS; test)");
    await sendEmptyModelList(first);
    await expect(firstList).resolves.toEqual({ models: [] });

    const secondList = listCodexAppServerModels({
      timeoutMs: 1000,
      agentDir: "/tmp/bot-agent-two",
    });
    await sendInitializeResult(second, "bot/0.143.0 (macOS; test)");
    await sendEmptyModelList(second);
    await expect(secondList).resolves.toEqual({ models: [] });
  ...
  it("rechecks persisted native Computer Use before managed binary resolution", async () => {
    await withTempDir("bot-codex-shared-native-", async (agentDir) => {
      ...
      await sendInitializeResult(harness, "bot/0.144.1 (macOS; test)");
    ...
    server.on("connection", (socket, request) => {
      ...
      socket.on("message", (data) => {
        ...
        if (message.method === "initialize") {
          socket.send(
            JSON.stringify({ id: message.id, result: { userAgent: "bot/0.143.0" } }),
          );
          return;
        }
