...
function setBotCodingToolsFactoryForTests(
...
function resetBotCodingToolsFactoryForTests(): void {
  dynamicToolBuildState.botCodingToolsFactory = undefined;
}
...
describe("Codex app-server dynamic tool build", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-codex-tools-"));
  });

  afterEach(async () => {
    resetBotCodingToolsFactoryForTests();
    vi.restoreAllMocks();
    vi.unstubAllEnvs();
  ...
  it("maps sandbox exec-server cwd through the remote workspace mapping", () => {
    expect(
      resolveCodexAppServerExecutionCwd({
        effectiveCwd: "/Users/kevinlin/code/bot",
        environment: {
          id: "sandbox-1",
          cwd: "/Users/kevinlin/code/bot/sandbox",
        } as never,
        nativeToolSurfaceEnabled: true,
        localWorkspaceRoot: "/Users/kevinlin/code/bot",
        remoteWorkspaceRoot: "/home/oai/bot-workspaces",
      }),
    ).toBe("/home/oai/bot-workspaces/sandbox");
  });
  ...
  it("forwards client caps alongside channel authority context", async () => {
    ...
    setBotCodingToolsFactoryForTests((options) => {
  ...
  it("preserves the host-provided Bot tool through the Codex allowlist", async () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    ...
    setBotCodingToolsFactoryForTests(() => [
      { ...createRuntimeDynamicTool("bot"), catalogMode: "direct-only" },
    ]);

    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      isHostScopedToolActive: (toolName) => toolName === "bot",
      pluginConfig: { codexDynamicToolsExclude: ["bot"] },
    });

    expect(tools.map((tool) => tool.name)).toEqual(["bot"]);
  });

  it.each([
    { label: "host scope is inactive", hostActive: false, toolsAllow: ["bot"] },
    {
  ...
  it("reports hosted search denied when effective tool policy removes web_search", async () => {
    ...
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    setBotCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
  ...
  it("quarantines unreadable tool entries before Codex-specific filtering", async () => {
    ...
    setBotCodingToolsFactoryForTests(() => sourceTools);
  ...
  it("quarantines non-object plugin schemas before Codex-specific filtering", async () => {
    ...
    setBotCodingToolsFactoryForTests(() => [brokenTool, messageTool]);
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("exposes Bot sandbox shell tools under distinct names for non-Docker sandbox backends", async () => {
    setBotCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("read"),
      createRuntimeDynamicTool("write"),
  ...
  it("exposes Docker sandbox shell tools when Bot sandboxing disables native Code Mode", async () => {
    setBotCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("exec"),
      createRuntimeDynamicTool("process"),
      createRuntimeDynamicTool("message"),
  ...
  it("restores the policy-filtered Bot shell when a finite allowlist disables native Code Mode", async () => {
    const execTool = createRuntimeDynamicTool("exec");
    const processTool = createRuntimeDynamicTool("process");
    const messageTool = createRuntimeDynamicTool("message");
    setBotCodingToolsFactoryForTests(() => [execTool, processTool, messageTool]);
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("exposes Docker sandbox shell tools when native Code Mode cannot honor sandbox paths", async () => {
    ...
    const tools = await buildDynamicToolsForTest(params, workspaceDir, {
      sandbox: {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/bot-data:/data:rw"] },
      } as never,
      nativeToolSurfaceEnabled: false,
    });
  ...
  it("preserves before-tool wrapping through Codex runtime normalization", async () => {
    ...
    setBotCodingToolsFactoryForTests(() => [wrappedTool]);
  ...
  it("passes the delegation capability into shared Bot tool construction", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("enables gateway subagent binding for forced private QA Codex runs", async () => {
    vi.stubEnv("BOT_BUILD_PRIVATE_QA", "1");
    vi.stubEnv("BOT_QA_FORCE_RUNTIME", "codex");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
  ...
  it("disables Codex native tool surfaces whenever an Bot sandbox is active", () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        enabled: true,
        backendId: "docker",
        docker: { binds: ["/tmp/bot-data:/tmp/bot-data:rw"] },
      } as never),
    ).toBe(false);
    ...
    expect(
      shouldEnableCodexAppServerNativeToolSurface(params, {
        ...
        docker: {
          binds: [
            "/tmp/bot-data:/tmp/bot-data:rw",
            "/tmp/bot-data/secrets:/tmp/bot-data/secrets:ro",
          ],
        },
      } as never),
    ).toBe(false);
  ...
  it("exposes the final delivery control only on Codex message-tool-only schemas", async () => {
    ...
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    // Mirror production createBotCodingTools: attempt-fresh tool instances
