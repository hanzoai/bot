...
import { onAgentEvent, type AgentEventPayload } from "bot/plugin-sdk/agent-harness-runtime";
import {
...
describe("runCodexAppServerAttempt dynamic tools", () => {
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    try {
      ...
      expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown Bot tool: lookup$/u);
    ...
    expect(resultEvent?.data?.result?.content?.[0]?.text).toBe("Unknown Bot tool: lookup");
      ...
      emitDynamicToolTerminalDiagnostic({
        ...
        response: {
          ...
          contentItems: [
            {
              type: "inputText",
              text: "Bot dynamic tool call timed out after 1ms while running tool echo.",
            },
          ],
        },
      });
