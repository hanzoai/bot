 ...
 * or whether Bot must keep exec/process on a configured node host.
 */
import type { BotConfig } from "bot/plugin-sdk/config-contracts";
import { resolveSandboxRuntimeStatus } from "bot/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "bot/plugin-sdk/session-store-runtime";
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "Bot exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected Bot node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because Bot exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected Bot node.",
    "Use a normal Codex harness turn so Bot exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: BotConfig): AgentEntry[] {
