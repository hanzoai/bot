...
export function createCodexAttemptLifecycleController(
  ...
  const { state, activeTurnItemIds, pendingBotDynamicToolCompletionIds, turnWatches } =
    turnRuntime;
  const releaseTurnAfterTerminalDynamicTool = (value: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingBotDynamicToolCompletionIdsCount: pendingBotDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
  ...
  const emitFastModeAutoProgress = async (payload: {
    ...
    try {
      await params.onToolResult?.({
        text: summary,
        channelData: { botProgressKind: FAST_MODE_AUTO_PROGRESS_KIND },
      });
    } catch (error) {
