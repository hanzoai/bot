...
import { createTempHomeEnv, withEnvAsync } from "bot/plugin-sdk/test-env";
...
it("disables native user-scope skills only for non-default state directories", async () => {
  const tempHome = await createTempHomeEnv("bot-codex-native-skills-");
  try {
    ...
    await withEnvAsync(
      { HOME: home, BOT_STATE_DIR: path.join(home, ".bot") },
      async () => {
        await expect(
    ...
    const isolation = await withEnvAsync(
      {
        ...
        BOT_STATE_DIR: path.join(home, "scratch-state"),
      },
      async () =>
...
it.runIf(process.platform !== "win32")(
  ...
  async () => {
    const tempHome = await createTempHomeEnv("bot-codex-native-skills-fail-closed-");
    try {
      const home = await fs.realpath(tempHome.home);
      ...
      const isolation = await withEnvAsync(
        { HOME: home, BOT_STATE_DIR: path.join(home, "scratch-state") },
...
it("captures a personal skill created during the authoritative Codex reload", async () => {
  const tempHome = await createTempHomeEnv("bot-codex-native-skills-reload-race-");
  try {
    const home = await fs.realpath(tempHome.home);
...
it("preserves direct skills under a state-owned default Codex home", async () => {
  const tempHome = await createTempHomeEnv("bot-codex-native-state-home-");
  try {
    ...
    const isolation = await withEnvAsync(
      { HOME: stateHome, BOT_STATE_DIR: stateHome },
