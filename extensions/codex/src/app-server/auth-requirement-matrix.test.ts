...
const AGENT_DIR = "/tmp/bot-codex-auth-matrix";
...
/**
 * Expected handoff per cell. `prepared` names the auth material Bot hands to the
...
const HANDOFF_MATRIX: {
  ...
  // Agent scope owns an isolated CODEX_HOME, so Bot may inject prepared auth.
  {
    homeScope: "agent",
 ...
 * Bot verifies the account it found instead of logging in over it.
 */
