...
import { resolveRequiredHomeDir, resolveStateDir } from "bot/plugin-sdk/state-paths";
...
async function usesDefaultStateDir(): Promise<boolean> {
  if (!process.env.BOT_STATE_DIR?.trim()) {
    return true;
  }
  ...
  const [stateDir, defaultStateDir] = await Promise.all([
    ...
    canonicalizeExistingPath(path.join(home, ".bot")),
  ]);
...
/** Resolves the native user-scope skills that an isolated Bot thread must disable. */
