...
vi.mock("bot/plugin-sdk/gateway-runtime", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/gateway-runtime")>(
    "bot/plugin-sdk/gateway-runtime",
  );
  return {
    ...actual,
...
describe("registerCodexSessionCli", () => {
  ...
  describe("sessions", () => {
    ...
    it("renders connected sessions, metadata, pagination, and offline host errors", async () => {
      ...
      expect(output).toContain("/Users/test/Projects/bot");
  ...
  describe("continue", () => {
    it("continues a Gateway-local thread and sanitizes the human-readable session key", async () => {
      ...
      expect(output).toBe("Bot session: harness:codex:supervision:branch\n");
      expect(output).not.toContain("\u001b");
    });
