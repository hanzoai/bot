...
const MATERIALIZED_SKILLS_REMOTE_PARTS = [".bot", "sandbox-skills"] as const;
...
class OpenShellSandboxBackendImpl {
  ...
  private buildLegacyRuntimeUnavailableError(detail: string): Error {
    const recreateCommand = `bot sandbox recreate --session ${shellEscape(this.params.createParams.scopeKey)}`;
    return new Error(
      [
        `Registered legacy OpenShell sandbox "${this.params.execContext.sandboxName}" is not usable.`,
        detail,
        `Bot will not recreate this retired runtime name. Run \`${recreateCommand}\` to migrate this scope to the current naming format.`,
      ]
        .filter(Boolean)
        .join(" "),
    );
  ...
  private async syncWorkspaceFromRemote(): Promise<void> {
    await withTempWorkspace(
      { rootDir: resolveOpenShellTmpRoot(), prefix: "bot-openshell-sync-" },
      async ({ dir: tmpDir }) => {
        const result = await runOpenShellCli({
          context: this.params.execContext,
          args: [
  ...
  private async uploadPathToRemote(localPath: string, remotePath: string): Promise<void> {
    await withTempWorkspace(
      { rootDir: resolveOpenShellTmpRoot(), prefix: "bot-openshell-upload-" },
      async ({ dir: tmpDir }) => {
...
function buildLegacyOpenShellSandboxName(scopeKey: string): string {
  ...
  return `bot-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
...
async function moveMaterializedSkillsShadowAside(params: {
  ...
  const preserveRoot = await fs.mkdtemp(
    path.join(path.dirname(params.tmpDir), "bot-openshell-preserve-"),
  );
...
function resolveOpenShellTmpRoot(): string {
  return path.resolve(resolvePreferredBotTmpDir());
}
