...
import { FsSafeError, pathExists, root as fsRoot } from "bot/plugin-sdk/security-runtime";
import {
...
const WIKI_VAULT_SCAFFOLD = ["AGENTS.md", "WIKI.md", "index.md", ".bot-wiki/log.jsonl"];
...
This vault is maintained by the Bot memory-wiki plugin.
...
- Compiled query and prompt snapshots live in Bot plugin state, not vault files.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".bot-wiki/log.jsonl", "", createdFiles);
