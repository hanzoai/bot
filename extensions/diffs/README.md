...
```bash
bot plugins install @hanzo/bot-diffs
```
...
- Rendered PNG/PDF files remain temporary materializations in `$TMPDIR/bot-diffs` because delivery APIs require a file path. No JSON metadata sidecars are written or imported.
