 ...
 * silk-wasm availability checks — all without importing `bot/plugin-sdk`.
 ...
 * `BOT_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective Bot home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of Bot treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `bot` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `BOT_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveBotHome(): string {
  const raw = process.env.BOT_HOME?.trim();
...
/**
 * Return a legacy path under `~/.bot/qqbot` without creating it.
 *
...
function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".bot", "qqbot", ...subPaths);
}

/** Return a path under `~/.bot/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<bot-home>/.bot/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Bot's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `BOT_HOME` (when set) so files written by agents into
 * the Bot-managed media tree are reachable by this plugin even when
 * `HOME` and `BOT_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveBotHome(), ".bot", "media", "qqbot", ...subPaths);
}

/** Return a path under `<bot-home>/.bot/media/qqbot`, creating it on demand. */
...
/**
 * Return `<bot-home>/.bot/media`, Bot's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `bot/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `BOT_HOME`.
 */
function getBotMediaDir(): string {
  return path.join(resolveBotHome(), ".bot", "media");
}
...
function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const botHomeDir = resolveBotHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When BOT_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the Bot-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".bot", "workspace", "qqbot"),
      path.join(botHomeDir, ".bot", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Bot's shared `~/.bot/media`
  ...
  const allowedRoots = [getBotMediaDir(), getQQBotMediaPath()];
