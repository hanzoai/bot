...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.bot/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".bot", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".bot", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "bot.sqlite");
