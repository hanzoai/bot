...
vi.mock("bot/plugin-sdk/outbound-media", () => ({
...
import { resolveLocalPathFromRootsSync } from "bot/plugin-sdk/security-runtime";
import {
...
vi.mock("bot/plugin-sdk/security-runtime", { spy: true });
...
let botHome: string;
let originalBotHome: string | undefined;
...
function makeCtx() {
  return {
    ...
    mediaAccess: {
      localRoots: ["/tmp/bot-sandbox"],
      workspaceDir: "/tmp/workspace",
      readFile: async () => Buffer.from("report"),
    },
    mediaLocalRoots: ["/tmp/bot-sandbox"],
...
beforeEach(async () => {
  vi.clearAllMocks();
  originalBotHome = process.env.BOT_HOME;
...
describe("resolveOutboundMediaPath", () => {
  ...
  it("does not treat workspaceDir as an allowed host absolute root", () => {
    ...
    ).toEqual(["/tmp/bot-sandbox"]);
  });

  it("maps only authorized virtual workspace roots for host-read loading", () => {
    expect(
      resolveWorkspaceScopedLocalRoots(
        ["/workspace/attachments", "/tmp/bot-sandbox", "/workspace/../media"],
        "/tmp/agent-workspace",
      ),
    ).toEqual(["/tmp/agent-workspace/attachments", "/tmp/bot-sandbox"]);
  });
...
describe("trySendViaHostRead error handling", () => {
  it("returns OutboundResult.error when loadOutboundMediaFromUrl rejects", async () => {
    ...
    const result = await sendPhoto(makeCtx(), "/tmp/bot-sandbox/report.docx");
  ...
  it("falls back to normal local sends for trusted media paths outside host-read roots", async () => {
    const trustedMediaDir = path.join(botHome, ".bot", "media", "qqbot");
  ...
  it("returns OutboundResult.error when senderSendMedia rejects", async () => {
    ...
    const result = await sendPhoto(makeCtx(), "/tmp/bot-sandbox/chart.png");
  ...
  it("loads virtual-root workspace media through the real outbound loader", async () => {
    const actualOutboundMedia = await vi.importActual<
      typeof import("bot/plugin-sdk/outbound-media")
    ...
    mockedSenderSendMedia.mockResolvedValue({ id: "media-1", timestamp: 123 });
    const workspaceDir = path.join(botHome, "agent-workspace");
