...
vi.mock("bot/plugin-sdk/setup-tools", async (importOriginal) => {
  const actual = await importOriginal<typeof import("bot/plugin-sdk/setup-tools")>();
  return {
    ...actual,
...
async function withTempFile(run: (filePath: string) => Promise<void>) {
  const workDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-signal-download-"));
  try {
...
describe("installSignalCliFromRelease", () => {
  ...
  it("removes the download temp dir on the success path too", async () => {
    setProcessPlatform("linux", "x64");
    const staging = await fs.mkdtemp(path.join(os.tmpdir(), "bot-signal-staging-"));
    try {
...
describe("installSignalCli", () => {
  it("uses Homebrew on macOS instead of downloading the first GitHub release archive", async () => {
    ...
    const brewPrefix = await fs.mkdtemp(path.join(os.tmpdir(), "bot-signal-brew-"));
