...
import { readProviderJsonObjectResponse } from "bot/plugin-sdk/provider-http";
import { runPluginCommandWithTimeout } from "bot/plugin-sdk/run-command";
import type { RuntimeEnv } from "bot/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "bot/plugin-sdk/setup-tools";
...
import { withTempDownloadPath } from "bot/plugin-sdk/temp-path";
...
export async function installSignalCliFromRelease(
  ...
  return await withTempDownloadPath(
    { prefix: "bot-signal", fileName: asset.name },
    async (archivePath) => {
