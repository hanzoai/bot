...
import { createCrabboxWorkerProvider, resolveBotRoot } from "./crabbox-worker-provider.js";
...
const SIBLING_BINARY = path.resolve(BOT_ROOT, "../crabbox/bin/crabbox");
...
describe("Crabbox binary resolution", () => {
  ...
  it("derives the package root from source and bundled plugin roots", () => {
    expect(resolveBotRoot(path.join(BOT_ROOT, "extensions", "crabbox"))).toBe(
      BOT_ROOT,
    );
    expect(resolveBotRoot(path.join(BOT_ROOT, "dist", "extensions", "crabbox"))).toBe(
      BOT_ROOT,
    );
  });
});
