...
vi.mock("bot/plugin-sdk/sandbox", () => ({
...
const nonFullRegistrationModes = [
  ...
  "setup-runtime",
  "cli-metadata",
] as const satisfies readonly BotPluginApi["registrationMode"][];
...
function createApi(
  ...
  registrationMode: BotPluginApi["registrationMode"] = "full",
): {
  api: BotPluginApi;
  registerService: ReturnType<typeof vi.fn>;
  services: BotPluginService[];
} {
  const services: BotPluginService[] = [];
  const registerService = vi.fn((service: BotPluginService): void => {
  ...
  return {
    api: api as unknown as BotPluginApi,
    registerService,
    services,
  };
}
...
describe("registerMxcPlugin", () => {
  ...
  test("registers the sandbox backend on Windows when feature probes pass", () => {
    ...
    void services[0]?.stop?.({} as BotPluginServiceContext);
