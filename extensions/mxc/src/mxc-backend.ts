...
function createLauncherPayloadFile(
  ...
  const payloadDir = mkdtempSync(
    path.join(resolvePreferredBotTmpDir(), "bot-mxc-payload-"),
  );
...
function createSandboxTempDir(hostEnv: BaselineHostEnv): string {
  return mkdtempSync(path.join(resolveSandboxTempDir(hostEnv), "bot-mxc-sandbox-"));
}
...
/** Manager for `bot sandbox list` and `bot sandbox remove`. */
