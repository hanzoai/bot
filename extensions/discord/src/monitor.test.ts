...
import { typedCases } from "bot/plugin-sdk/test-fixtures";
...
describe("DiscordMessageListener", () => {
  ...
  it("logs handler failures", async () => {
    const logger = {
      ...
      typeof import("bot/plugin-sdk/logging-core").createSubsystemLogger
    >;
    const handler = vi.fn(async () => {
...
describe("discord allowlist helpers", () => {
  ...
  it("matches ids by default and names only when enabled", () => {
    ...
    expect(
      allowListMatches(allow, { name: "friends-of-bot" }, { allowNameMatching: false }),
    ).toBe(false);
    expect(allowListMatches(allow, { name: "steipete" }, { allowNameMatching: true })).toBe(true);
    expect(
      allowListMatches(allow, { name: "friends-of-bot" }, { allowNameMatching: true }),
    ).toBe(true);
...
const channelRuntimeModule = await import("bot/plugin-sdk/system-event-runtime");
vi.spyOn(channelRuntimeModule, "enqueueSystemEvent").mockImplementation(enqueueSystemEventSpy);

const routingModule = await import("bot/plugin-sdk/routing");
...
function makeReactionListenerParams(overrides?: {
  ...
  return {
    ...
    accountId: "acc-1",
    runtime: {} as import("bot/plugin-sdk/runtime-env").RuntimeEnv,
