...
import { saveRemoteMedia } from "bot/plugin-sdk/media-runtime";
import {
...
vi.mock("bot/plugin-sdk/media-runtime", { spy: true });
...
beforeEach(() => {
  ...
  saveRemoteMediaMock.mockImplementation(
    async (options: { fallbackContentType?: string; filePathHint?: string }) => ({
      id: "test-media",
      path: `/tmp/bot-discord-test/${options.filePathHint ?? "media"}`,
      size: 5,
...
function createThreadBinding(
  overrides?: Partial<import("bot/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("bot/plugin-sdk/conversation-runtime").SessionBindingRecord;
}
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("bot/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("preflights direct-message voice notes without mention gating", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hello bot from dm audio");
    ...
    expect(preflight.preflightAudioTranscript).toBe("hello bot from dm audio");
  });
  ...
  it("suppresses repeated bot messages before downloading attachments (#58789)", async () => {
    ...
    const message = createDiscordMessage({
      id: "m-loop-1",
      channelId,
      content: "chatter <@bot-bot>",
      mentionedUsers: [{ id: "bot-bot" }],
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("bot/plugin-sdk/config-contracts").BotConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("hydrates mention metadata from REST when bot mention syntax is present but mentions are missing", async () => {
    ...
    client.rest = {
      get: vi.fn(async () => ({
        ...
        mentions: [{ id: botId, username: "Bot", bot: true }],
        mention_roles: [],
        mention_everyone: false,
      })),
  ...
  it("still allows bot control commands with an explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "<@bot-bot> /new incident room",
      mentionedUsers: [{ id: "bot-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
  ...
  it("records local image media for skipped mention-gated guild history", async () => {
    ...
    expect(entries?.[0]?.media?.[0]?.path).toBe("C:\\bot\\media\\history.png");
    expect(saveRemoteMediaMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("records sticker image media for skipped mention-gated guild history", async () => {
    ...
    saveRemoteMediaMock.mockResolvedValueOnce({
      id: "test-sticker",
      path: "/tmp/bot-discord-test/sticker.png",
      size: 5,
      contentType: "image/png",
    });
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    ...
    const preflight = expectPreflightResult(result);
    expect(preflight.wasMentioned).toBe(true);
    expect(preflight.preflightAudioTranscript).toBe("hey bot");
  });
  ...
  it("drops guild message without mention when channel has configuredBinding and requireMention: true", async () => {
    const conversationRuntime = await import("bot/plugin-sdk/conversation-runtime");
