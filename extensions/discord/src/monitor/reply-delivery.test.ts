...
vi.mock("bot/plugin-sdk/channel-outbound", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/channel-outbound")>(
...
describe("deliverDiscordReply", () => {
      ...
      replies: [
        {
          text: [
            "[tool:exec]",
            "<parameter=command>",
            'cat /proc/mounts 2>/dev/null | grep -i "libra|rav|bot" | head -20',
            "</parameter>",
            "",
            "<function=exec>",
            "<parameter=command>",
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    expect(params.mediaAccess).toEqual({ localRoots: ["/tmp/bot-media"] });
  });
