...
import * as commandRegistryModule from "bot/plugin-sdk/command-auth-native";
import type {
  ChatCommandDefinition,
  CommandArgsParsing,
} from "bot/plugin-sdk/command-auth-native";
import type { ModelsProviderData } from "bot/plugin-sdk/command-auth-native";
...
import * as commandTextModule from "bot/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "bot-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  ...
  it("uses the hot-reloaded runtime config when old components reset to default", async () => {
    const context = createModelPickerContext();
    (context.cfg as { agents?: BotConfig["agents"] }).agents = {
      defaults: {
  ...
  it("keeps a pending model stable when hot reload reorders the catalog", async () => {
    const context = createModelPickerContext();
    const runtimeCfg = { ...context.cfg } as BotConfig;
    vi.spyOn(runtimeConfigSnapshotModule, "getRuntimeConfigSnapshot").mockReturnValue(runtimeCfg);
    vi.spyOn(runtimeConfigSnapshotModule, "getRuntimeConfigSourceSnapshot").mockReturnValue(
      runtimeCfg,
    );
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "bot", label: "Bot Default", description: "Use Bot." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();
