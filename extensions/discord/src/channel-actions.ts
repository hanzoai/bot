...
import { createUnionActionGate } from "bot/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "bot/plugin-sdk/channel-contract";
import type { DiscordActionConfig, BotConfig } from "bot/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: BotConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: BotConfig) {
