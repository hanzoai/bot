  ...
  switch (state.state) {
    ...
    case "needs-pairing":
      setGate({
        ...
        detail:
          "Open the Bot toolbar popup and paste the output of bot browser extension pair.",
      });
      break;
    case "approval":
      setGate({
        ...
        detail:
          "On the Gateway, run bot devices list, inspect this dedicated browser identity, then approve its current request.",
      });
      break;
    case "denied":
...
function connectPanelPort() {
  ...
  let nextPort;
  try {
    nextPort = chrome.runtime.connect({ name: "bot-copilot-panel" });
  } catch {
