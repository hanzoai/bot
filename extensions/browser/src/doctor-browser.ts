...
import { DEFAULT_BOT_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
function collectChromeMcpProfiles(cfg: BotConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: BotConfig): ManagedProfile[] {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "bot";
    if (driver !== "existing-session") {
...
function isLegacyClawdProfileConfigured(cfg: BotConfig, legacyProfileDir: string): boolean {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return false;
  }
...
export function detectLegacyClawdBrowserProfileResidue(
  ...
  if (
    resolved.defaultProfile !== DEFAULT_BOT_BROWSER_PROFILE_NAME ||
...
function formatLegacyClawdBrowserProfileResidueNote(
  ...
  return [
    ...
    `- The canonical Bot-managed browser profile is ${residue.canonicalUserDataDir}.`,
    `- If no browser is using the legacy profile, run ${formatCliCommand("bot doctor --fix")} to archive it safely instead of deleting it in place.`,
  ].join("\n");
}
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- Bot-managed browser profile(s) are configured: ${managedProfileLabel}.`,
        "- No Chromium-based browser executable was found on this host for Bot-managed launch.",
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- Bot-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time Bot connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. Bot does not bundle Chrome.`,
