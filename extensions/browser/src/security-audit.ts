 ...
 */
import type { BotPluginSecurityAuditContext } from "bot/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "bot/plugin-sdk/secret-input";
import { formatCliCommand } from "bot/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "bot/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: BotPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("bot security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.BOT_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.BOT_GATEWAY_PASSWORD) ||
