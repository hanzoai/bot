...
function formatBrowserDoctorGatewayError(error: unknown): string {
  ...
  return "Gateway auth SecretRef is unavailable in this command path; browser doctor cannot reach the admin-scoped browser.request endpoint. Set BOT_GATEWAY_TOKEN or BOT_GATEWAY_PASSWORD, then retry.";
}
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "bot"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `bot browser start`",
  });
  if (status.graphics) {
    checks.push({
      name: "graphics",
...
type BrowserProfileDriver = "bot" | "existing-session" | "extension";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "bot" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (bot|existing-session). Default: bot")
    .action(
      async (
        opts: {
          name: string;
