...
const mocks = vi.hoisted(() => ({
  ...
  stopBotChrome: vi.fn(async (_running: RunningChrome) => {}),
}));

vi.mock("./chrome.js", () => ({
  stopBotChrome: mocks.stopBotChrome,
}));
...
beforeEach(() => {
  ...
  mocks.stopBotChrome.mockReset().mockResolvedValue(undefined);
});
...
describe("browser runtime shutdown profile races", () => {
  it("invalidates every deferred start before draining late children and clearing state", async () => {
    ...
    expect(mocks.stopBotChrome.mock.calls.map(([running]) => running)).toEqual(children);
