...
function formatLocalPortOwnershipHint(profile: ResolvedBrowserProfile): string {
  const resetHint =
    `If Bot should own this local profile, run action=reset-profile profile=${profile.name} ` +
  ...
  return (
    ...
    `set browser.profiles.${profile.name}.attachOnly=true so Bot attaches without trying ` +
    "to manage the local process. For Browserless Docker, set EXTERNAL to the same WebSocket " +
    "endpoint Bot can reach via browser.profiles.<name>.cdpUrl."
  );
}
...
export function createProfileAvailability({
  ...
  const formatChromeMcpAttachFailure = (lastError: unknown): string => {
    ...
    if (message.includes("DevToolsActivePort") || message.includes("Could not connect to Chrome")) {
      return (
        ...
        'If you do not need your signed-in browser session, use the managed "bot" profile instead.' +
        detail
      );
    }
    return (
  ...
  const launchManagedChrome = async (
    ...
    try {
      return await launchBotChrome(current.resolved, profile, {
        ...launchOptions,
        signal,
      });
    } catch (err) {
  ...
  const ensureBrowserAvailableOnce = async (
    ...
    if (!httpReachable) {
      ...
      // Browser control service can restart while a loopback Bot browser is still
      ...
      if (attachOnly || remoteCdp) {
        if (capabilities.mode === "local-extension") {
          ...
          throw new BrowserProfileUnavailableError(
            `The Bot Chrome extension is not connected for profile "${profile.name}". ` +
    ...
    if (attachOnly || remoteCdp) {
      ...
      if (capabilities.mode === "local-extension") {
        const { EXTENSION_PAIRING_HINT } = await getExtensionRelayModule();
        throw new BrowserProfileUnavailableError(
          `The extension relay for profile "${profile.name}" is running but the Bot Chrome extension is not connected. ${EXTENSION_PAIRING_HINT}`,
        );
      }
