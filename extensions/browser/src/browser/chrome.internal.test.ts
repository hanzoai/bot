...
vi.mock("bot/plugin-sdk/ssrf-runtime-internal", () => ({
...
vi.mock("../infra/tmp-bot-dir.js", () => ({
  resolvePreferredBotTmpDir: () => "/tmp/bot-browser-test",
}));
 ...
 * not exercise: launchBotChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopBotChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildBotChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveBotUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveBotUserDataDir();
      expect(dir.endsWith(path.join("bot", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveBotUserDataDir("my-profile");
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchBotChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchBotChrome(resolved, profile);
  ...
  describe("launchBotChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "bot-launch-"));
    });
    ...
    const captureFailedLaunchStderr = async (params: {
      ...
      const result = await launchBotChrome(
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchBotChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("returns structured no-display details before spawning headed Chrome", async () => {
      ...
      const error = await launchBotChrome(makeResolved(), profile, {
        platform: "linux",
    ...
    it("rejects a runtime spawn error before polling CDP", async () => {
      ...
      await expect(
        launchBotChrome(makeResolved(), makeProfile(51112), {
    ...
    it("rejects a bootstrap spawn error without attempting the runtime launch", async () => {
      ...
      await expect(launchBotChrome(makeResolved(), makeProfile(51113))).rejects.toBe(
        spawnError,
      );
    ...
    it("keeps handling process errors after a successful spawn", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const running = await launchBotChrome(
            makeResolved(),
    ...
    it("aborts a deferred managed launch and proves its exact child exited", async () => {
      ...
      const launch = launchBotChrome(makeResolved(), makeProfile(51114), {
        signal: controller.signal,
      });
    ...
    it("aborts bootstrap immediately and never reaches the runtime launch", async () => {
      ...
      const launch = launchBotChrome(makeResolved(), makeProfile(51115), {
        signal: controller.signal,
      });
    ...
    it("returns the exact child when abort cleanup cannot prove process exit", async () => {
      ...
      const launch = launchBotChrome(makeResolved(), makeProfile(51116), {
        signal: controller.signal,
      });
      await probeEntered.promise;
    ...
    it.each([
      ...
      async ({ cdpUrl, configuredProbeHost }) => {
        ...
        await expect(launchBotChrome(makeResolved(), profile)).rejects.toThrow(portBusy);
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchBotChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("clears stale singleton locks even when the profile-in-use marker rolls out of the stderr tail", async () => {
      const configPath = path.join(tmpDir, "bot.json");
      await fsp.writeFile(
        configPath,
        JSON.stringify({
          logging: {
    ...
    it("preserves the exact surviving child when a singleton retry cleanup fails", async () => {
      ...
      try {
        const error = await launchBotChrome(
          makeResolved({ localLaunchTimeoutMs: 20 }),
          profile,
        ).catch((err: unknown) => err);
        ...
        for (const testCase of [
          ...
          await withMockChromeCdpServer({
            ...
            run: async (baseUrl) => {
              ...
              const userDataDir = resolveBotUserDataDir(`${profile.name}-${testCase.pid}`);
              ...
              try {
                await expect(
                  launchBotChrome(makeResolved(), profileWithUniqueName),
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchBotChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("launchBotChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "bot-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveBotUserDataDir(profileName));
        ...
        const staged = resolveBotUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_BOT_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const botState = await createBotTestState({
        layout: "state-only",
        prefix: "bot-redact-off-",
      });
      await botState.writeConfig({ logging: { redactSensitive: "off" } });
      const configDir = botState.root;
      ...
      let message = "";
      try {
        await launchBotChrome(resolved, profile);
      } catch (err) {
        message = err instanceof Error ? err.message : String(err);
      }
    ...
    it("logs a warning when decorateBotProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateBotProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateBotProfile }, "decorateBotProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".bot-profile-decorated") || s.endsWith("Preferences")) {
  ...
  describe("launchBotChrome managed-proxy CDP bypass", () => {
    ...
    it("surfaces loopbackMode=block as BrowserProfileUnavailableError without spawning Chrome", async () => {
      ...
      await expect(launchBotChrome(makeResolved(), profile)).rejects.toBeInstanceOf(
    ...
    it("does not register a bypass for a remote attachOnly CDP URL (loopback gate)", async () => {
      stubExecutableAndPrefsExist();
      // For this test we want launchBotChrome to reject before any
      ...
      await expect(launchBotChrome(makeResolved(), remoteProfile)).rejects.toThrow(
        /is remote; cannot launch local Chrome/,
      );
