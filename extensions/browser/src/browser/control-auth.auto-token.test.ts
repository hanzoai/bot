...
const mocks = vi.hoisted(() => ({
  ...
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: BotConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
      const draft = structuredClone(mocks.getRuntimeConfig());
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/bot.json" } });
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<BotConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: BotConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): BotConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: BotConfig) {
