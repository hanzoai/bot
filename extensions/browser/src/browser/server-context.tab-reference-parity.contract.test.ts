...
function setupProfile() {
  ...
  return { fetchMock, bot: ctx.forProfile("bot") };
}
...
async function setupProfileWithLabels() {
  ...
  await ctx.bot.labelTab("ABCDEF123456", "docs");
  await ctx.bot.labelTab("ABC999", "app");
  return ctx;
}
...
describe("tab reference parity contract", () => {
  it("listTabs attaches tabId, label, and suggestedTargetId for every fixture tab", async () => {
    const { bot } = await setupProfileWithLabels();
    const tabs = await bot.listTabs();
    expect(tabs).toEqual(
  ...
  it("rejects an ambiguous raw prefix consistently across label/ensure/focus/close", async () => {
    const labelCtx = await setupProfileWithLabels();
    await expect(labelCtx.bot.labelTab("ABC", "parity")).rejects.toBeInstanceOf(
      BrowserTargetAmbiguousError,
    );
    const ensureCtx = await setupProfileWithLabels();
    await expect(ensureCtx.bot.ensureTabAvailable("ABC")).rejects.toBeInstanceOf(
      BrowserTargetAmbiguousError,
    );
    const focusCtx = await setupProfileWithLabels();
    await expect(focusCtx.bot.focusTab("ABC")).rejects.toBeInstanceOf(
      BrowserTargetAmbiguousError,
    );
    const closeCtx = await setupProfileWithLabels();
    await expect(closeCtx.bot.closeTab("ABC")).rejects.toBeInstanceOf(
      BrowserTargetAmbiguousError,
    );
  });
});
