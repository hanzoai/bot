...
describe("system profile import state", () => {
  ...
  it("reuses only the recorded target and otherwise avoids collisions", () => {
    expect(resolveSuggestedImportTarget({ profileNames: ["bot"] })).toBe("imported");
    expect(resolveSuggestedImportTarget({ profileNames: ["bot", "imported"] })).toBe(
      "imported-2",
    );
    expect(
      resolveSuggestedImportTarget({
        profileNames: ["bot", "imported", "imported-2"],
        state: {
          version: 1,
          status: "imported",
          browser: "chrome",
