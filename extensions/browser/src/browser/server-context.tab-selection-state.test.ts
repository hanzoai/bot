...
describe("browser server-context tab selection state", () => {
  ...
  it("does not sticky-adopt a CDP-created tab when the discovered URL is policy-blocked", async () => {
    ...
    await expect(bot.openTab("about:blank", { label: "good" })).resolves.toEqual(
      expect.objectContaining({ targetId: "GOOD" }),
    );
    expect(state.profiles.get("bot")?.lastTargetId).toBe("GOOD");
    const aliasesBefore = structuredClone(state.profiles.get("bot")?.tabAliases);

    await expect(bot.openTab("https://example.com", { label: "blocked" })).rejects.toThrow(
      /private|blocked|ssrf/i,
    );
    const profileState = state.profiles.get("bot");
    ...
    await expect(bot.ensureTabAvailable()).resolves.toEqual(
      expect.objectContaining({ targetId: "GOOD" }),
    );
  });
  ...
  it("does not migrate a disappeared sticky alias onto a sole blocked discovery", async () => {
    ...
    await bot.openTab("about:blank", { label: "good" });
    const aliasesBefore = structuredClone(state.profiles.get("bot")?.tabAliases);
  ...
  it("returns an undiscovered CDP target without adopting or cleaning it", async () => {
    ...
    const opening = bot.openTab("https://example.com/start", { label: "undiscovered" });
  ...
  it("returns an unadopted target when CDP cannot prove the committed navigation", async () => {
    ...
    await expect(bot.openTab("https://example.com", { label: "unsettled" })).resolves.toEqual({
      targetId: "UNSETTLED",
      title: "",
  ...
  it("rejects invalid labels before direct CDP target creation", async () => {
    ...
    await expect(bot.openTab("about:blank", { label: "not allowed" })).rejects.toThrow(
      /tab label/i,
    );

    expect(createTargetViaCdp).not.toHaveBeenCalled();
    expect(fetchMock).not.toHaveBeenCalled();
    expect(state.profiles.get("bot")?.tabAliases).toBeUndefined();
  });
  ...
  it("can bootstrap a managed loopback tab under strict SSRF because CDP control stays local", async () => {
    ...
    const selected = await bot.ensureTabAvailable();
  ...
  it("opens a real tab when only browser-internal CDP targets are listed", async () => {
    ...
    expect(state.profiles.get("bot")?.lastTargetId).toBe("REAL");
    expect(createTargetViaCdp).toHaveBeenCalledWith({
      cdpUrl: "http://127.0.0.1:18800",
      url: "about:blank",
      ssrfPolicy: undefined,
  ...
  it("uses the loopback CDP control policy for /json/new fallback requests", async () => {
    ...
    const opened = await bot.openTab("https://example.com", { label: "raw" });
    ...
    expect(state.profiles.get("bot")?.lastTargetId).toBe("NEW");
  ...
  it("assigns stable tab ids and prefers labels as suggested target ids", async () => {
    ...
    expect(await bot.listTabs()).toEqual([
      expect.objectContaining({
        targetId: "DOCS_RAW",
        tabId: "t1",
    ...
    await expect(bot.labelTab("t1", "docs")).resolves.toEqual(
      expect.objectContaining({
        targetId: "DOCS_RAW",
        tabId: "t1",
        label: "docs",
  ...
  it("carries a stale alias to a single replacement target", async () => {
    ...
    expect((await bot.listTabs()).map((tab) => tab.tabId)).toEqual(["t1", "t2"]);
    expect(await bot.listTabs()).toEqual([
  ...
  it("carries stable aliases across confident raw target replacement", async () => {
    ...
    await expect(bot.labelTab("OLD_RAW", "checkout")).resolves.toEqual(
      expect.objectContaining({
        targetId: "OLD_RAW",
        tabId: "t1",
    ...
    await expect(bot.listTabs()).resolves.toEqual([
      expect.objectContaining({
        targetId: "NEW_RAW",
        tabId: "t1",
        label: "checkout",
    ...
    expect(state.profiles.get("bot")?.lastTargetId).toBe("NEW_RAW");
  });
  ...
  it("expires aliases when duplicate-URL targets are replaced ambiguously", async () => {
    ...
    expect((await bot.listTabs()).map((tab) => [tab.targetId, tab.tabId])).toEqual([
      ["OLD_LEFT", "t1"],
      ["OLD_RIGHT", "t2"],
    ]);
    await bot.labelTab("t1", "left");
    await bot.labelTab("t2", "right");
    state.profiles.get("bot")!.lastTargetId = "OLD_LEFT";
    ...
    expect(state.profiles.get("bot")?.lastTargetId).toBe("OLD_LEFT");
    await expect(bot.ensureTabAvailable("left")).rejects.toThrow(/tab not found/i);
    await expect(bot.ensureTabAvailable()).rejects.toThrow(/tab not found/i);

    await bot.labelTab("t3", "fresh-right");
    targets = [
      { id: "NEW_LEFT", title: "Left", url: "https://app.example/same" },
  ...
  it("resolves friendly tab references before backend focus and close calls", async () => {
    ...
    await bot.labelTab("DOCS_RAW", "docs");
    await expect(bot.ensureTabAvailable("t1")).resolves.toEqual(
      expect.objectContaining({ targetId: "DOCS_RAW" }),
    );
    await bot.focusTab("docs");
    await bot.closeTab("t1");
