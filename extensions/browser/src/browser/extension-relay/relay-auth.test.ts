...
beforeEach(() => {
  stateDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bot-relay-auth-")));
...
describe("extension relay host-local secret", () => {
  ...
  it("gives different hosts (state dirs) different secrets", async () => {
    ...
    const otherDir = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "bot-relay-auth-2-")),
    );
    try {
      const b = await ensureExtensionRelayToken({ ...process.env, BOT_STATE_DIR: otherDir });
      expect(b).not.toBe(a);
    } finally {
