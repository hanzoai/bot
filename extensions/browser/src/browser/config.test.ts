...
beforeEach(async () => {
  ...
  isolatedStateDir = botState.stateDir;
});
afterEach(async () => {
...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.driver).toBe("bot");
  ...
  it("embeds the host-local relay secret as Basic auth in the extension cdpUrl", () => {
    ...
    expect(chrome?.cdpUrl).toBe(
      `http://bot:${token}@127.0.0.1:${resolved.extensionRelayDefaultPort}`,
    );
  });
  ...
  describe("managed browser headless mode", () => {
    ...
    it("lets explicit profile headless=false beat the Linux no-display fallback", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        profiles: {
          bot: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      });
      const profile = resolveProfile(resolved, "bot")!;
    ...
    it("lets BOT_BROWSER_HEADLESS override profile/global config", () => {
      const resolved = resolveBrowserConfig({
        profiles: {
          bot: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
    ...
    it("returns an actionable error only when headed mode is explicitly selected", () => {
      ...
      const defaultProfile = resolveProfile(defaultResolved, "bot")!;
      expect(
      ...
      const profile = resolveProfile(profileResolved, "bot")!;
      expect(
        ...
        message: expect.stringContaining("browser.profiles.bot.headless=false"),
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "bot")!;
