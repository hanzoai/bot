...
describe("browser server-context existing-session profile", () => {
  ...
  it("reports endpoint cdpUrl for existing-session profiles", async () => {
    ...
    state.resolved.profiles["chrome-live"] = {
      ...chromeLiveProfile,
      cdpUrl: "http://bot:relay-token@127.0.0.1:9222",
    };
    const ctx = createBrowserRouteContext({ getState: () => state });
    ...
    expect(ensuredProfile?.cdpUrl).toBe("http://bot:relay-token@127.0.0.1:9222");
  });
  ...
  it("surfaces DevToolsActivePort attach failures instead of a generic tab timeout", async () => {
    ...
    const assertion = expect(pending).rejects.toThrow(
      /could not connect to Chrome.*managed "bot" profile.*DevToolsActivePort/s,
    );
