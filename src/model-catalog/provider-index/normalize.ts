...
import type {
  BotProviderIndex,
  BotProviderIndexPluginInstall,
  BotProviderIndexPlugin,
  BotProviderIndexProviderAuthChoice,
  BotProviderIndexProvider,
} from "./types.js";
...
const BOT_PROVIDER_INDEX_VERSION = 1;
...
function normalizeInstall(value: unknown): BotProviderIndexPluginInstall | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizePlugin(value: unknown): BotProviderIndexPlugin | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizeOnboardingScopes(
  value: unknown,
): BotProviderIndexProviderAuthChoice["onboardingScopes"] | undefined {
...
function normalizeAssistantVisibility(
  value: unknown,
): BotProviderIndexProviderAuthChoice["assistantVisibility"] | undefined {
...
function normalizeAuthChoice(params: {
  ...
  providerName: string;
  value: unknown;
}): BotProviderIndexProviderAuthChoice | undefined {
...
function normalizeAuthChoices(params: {
  ...
  providerName: string;
  value: unknown;
}): readonly BotProviderIndexProviderAuthChoice[] | undefined {
  if (!Array.isArray(params.value)) {
    return undefined;
  }
  const choices = params.value
    .map((value) => normalizeAuthChoice({ ...params, value }))
    .filter((choice): choice is BotProviderIndexProviderAuthChoice => Boolean(choice));
...
function normalizeProvider(
  ...
  value: unknown,
): BotProviderIndexProvider | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
export function normalizeBotProviderIndex(value: unknown): BotProviderIndex | undefined {
  if (!isRecord(value) || value.version !== BOT_PROVIDER_INDEX_VERSION) {
    return undefined;
  }
  if (!isRecord(value.providers)) {
    return undefined;
  }
  const providers: Record<string, BotProviderIndexProvider> = {};
  ...
  return {
    version: BOT_PROVIDER_INDEX_VERSION,
