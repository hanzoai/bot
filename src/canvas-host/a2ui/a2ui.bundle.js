...
var BotA2UIHost = class extends i$6 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      box-sizing: border-box;
      padding:
        var(--bot-a2ui-inset-top, 0px)
        var(--bot-a2ui-inset-right, 0px)
        var(--bot-a2ui-inset-bottom, 0px)
        var(--bot-a2ui-inset-left, 0px);
    }
  ...
  `;
	connectedCallback() {
		...
		globalThis.botA2UI = api;
		this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
		this.#statusListener = (evt) => this.#handleActionStatus(evt);
		for (const eventName of ["bot:a2ui-action-status"]) {
	...
	#handleA2UIAction(evt) {
		...
		if (handler?.postMessage) {
			try {
				if (handler === globalThis.botCanvasA2UIAction) {
...
if (!customElements.get("bot-a2ui-host")) {
	customElements.define("bot-a2ui-host", BotA2UIHost);
}
