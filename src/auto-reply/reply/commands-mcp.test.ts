...
const workspaceHarness = createCommandWorkspaceHarness("bot-command-mcp-");
...
describe("handleCommands /mcp", () => {
  ...
  it("writes MCP config and shows it back", async () => {
    await withTempHome("bot-command-mcp-home-", async () => {
  ...
  it("routes group /mcp show privately and redacts the delivered config", async () => {
    await withTempHome("bot-command-mcp-home-", async () => {
      ...
      expect(namedGroupText).not.toContain("/tmp/bot.json");
      ...
      expect(allGroupText).not.toContain("/tmp/bot.json");
  ...
  it.each([
    ...
    await withTempHome("bot-command-mcp-home-", async () => {
      ...
      expect(groupText).not.toContain("/tmp/bot.json");
