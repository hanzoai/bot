...
function makeWorkspace(): string {
  return tempDirs.make("bot-session-export-");
}
...
describe("writeSessionExportFile", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = makeWorkspace();
      const aliasParent = tempDirs.make("bot-session-export-alias-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = makeWorkspace();
      const outsideDir = tempDirs.make("bot-session-export-outside-");
