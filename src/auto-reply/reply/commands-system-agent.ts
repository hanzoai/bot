// Implements maintenance commands for Bot-backed session cleanup.
...
export const handleSystemAgentCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (!params.command.isAuthorizedSender) {
    logVerbose(
      `Ignoring /bot from unauthorized sender: ${params.command.senderId || "<unknown>"}`,
    );
  ...
  return {
    ...
    reply: {
      text:
        ...
        })) ?? "Bot did not find a rescue request.",
    },
  };
};
