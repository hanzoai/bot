...
vi.mock("../../agents/acp-spawn.js", () => ({
  resolveAcpSpawnRuntimePolicyError: (params: { cfg?: BotConfig }) =>
...
function createDiscordParams(commandBody: string, cfg: BotConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: BotConfig = baseCfg) {
...
async function runDiscordAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(createThreadParams(commandBody, cfg), true);
}

async function runTelegramAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
...
async function runTelegramDmAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runSlackDmAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
function createMatrixThreadParams(commandBody: string, cfg: BotConfig = baseCfg) {
...
async function runMatrixAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runMatrixThreadAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(createMatrixThreadParams(commandBody, cfg), true);
}

async function runFeishuDmAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runLineDmAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
async function runIMessageDmAcpCommand(commandBody: string, cfg: BotConfig = baseCfg) {
  return handleAcpCommand(
    createConversationParams(
      commandBody,
      {
...
describe("/acp command", () => {
  ...
  it("inherits the target agent workspace when /acp spawn omits --cwd", async () => {
    ...
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "bot-codex-"));
    try {
      const cfg = {
        ...baseCfg,
        agents: {
  ...
  it("sanitizes leaked task and runtime details in ACP status output", async () => {
    ...
    expect(result?.reply?.text).not.toContain("Bot runtime context (internal):");
