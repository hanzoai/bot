...
function setupResolvedAcpSessionNotice(params: { bound: boolean; messageThreadId?: string }) {
  ...
  return {
    cfg: { acp: { enabled: true, dispatch: { enabled: true } } } as BotConfig,
...
function createNativeApprovalTestConfig(channel: NativeApprovalTestChannel): BotConfig {
...
describe("dispatchReplyFromConfig", () => {
  ...
  it("retargets reply_dispatch to a bound generic ACP session before model fallback", async () => {
    ...
    acpMocks.readAcpSessionEntry.mockImplementation(
      (params: { sessionKey: string; cfg?: BotConfig }) =>
  ...
  it("normalizes accumulated block TTS-only media before final delivery", async () => {
    ...
    replyMediaPathMocks.createReplyMediaPathNormalizer.mockReturnValue(
      async (payload: ReplyPayload) => ({
        ...payload,
        mediaUrl: "/tmp/bot-media/normalized-tts.ogg",
        mediaUrls: ["/tmp/bot-media/normalized-tts.ogg"],
      }),
    );
    const dispatcher = createDispatcher();
    const ctx = buildTestCtx({
    ...
    const finalPayload = firstFinalReplyPayload(dispatcher);
    expect(finalPayload?.mediaUrl).toBe("/tmp/bot-media/normalized-tts.ogg");
    expect(finalPayload?.mediaUrls).toStrictEqual(["/tmp/bot-media/normalized-tts.ogg"]);
