...
const mocks = vi.hoisted(() => ({
  ...
  triggerBotRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerBotRestart: mocks.triggerBotRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerBotRestart.mockReset();
    mocks.triggerBotRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerBotRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerBotRestart).not.toHaveBeenCalled();
  ...
  it("clears the success sentinel when fallback restart fails", async () => {
    mocks.triggerBotRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
