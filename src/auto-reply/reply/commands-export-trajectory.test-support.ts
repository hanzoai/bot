...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-export-command-"));
  tempDirs.push(dir);
  return dir;
}
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "bot sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(reply.text).toContain("https://docs.bot.ai/tools/trajectory");
    ...
    expect(execCall.defaults.sessionStore).toBe("/tmp/bot-sessions.json");
    ...
    expect(command).not.toContain("bot sessions export-trajectory");
    ...
    expect(String(request.workspace)).toContain("bot-export-command-");
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("bot sessions export-trajectory");
