...
describe("subagents info", () => {
  const TEST_SESSION_STORE_PATH = path.join(
    os.tmpdir(),
    `bot-commands-subagents-info-${process.pid}.json`,
  );

  function buildCommandTestConfig(): BotConfig {
    return {
      ...baseCommandTestConfig,
      session: {
  ...
  function buildInfoContext(params: { cfg: BotConfig; runs: object[]; restTokens: string[] }) {
    return {
      params: {
        cfg: params.cfg,
  ...
  it("sanitizes leaked task details in /subagents info", () => {
    ...
    expect(text).not.toContain("Bot runtime context (internal):");
