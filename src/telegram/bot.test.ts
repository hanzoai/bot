...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/bot-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
...
describe("createTelegramBot", () => {
  ...
  it("requests pairing by default for unknown DM senders", async () => {
    ...
    expect(pairingText).toContain("hanzo-bot pairing approve telegram PAIRME12");
