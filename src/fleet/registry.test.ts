...
describe("fleet cell registry", () => {
  ...
  const tempRoot = createSuiteTempRootTracker({ prefix: "bot-fleet-registry-" });
  ...
  function params(tenantId: string, requestedPort?: number): ReserveFleetCellParams {
    ...
    return {
      ...
      runtime: "docker",
      containerName: `bot-cell-${tenantId}`,
  ...
  it("returns empty reads without creating state on a fresh install", () => {
    ...
    const databasePath = path.join(root, "state", "bot.sqlite");
  ...
  it("persists, orders, updates, and deletes cells", () => {
    ...
    updateFleetCellImage(env, "zulu", "ghcr.io/hanzoai/bot:v2");
    expect(getFleetCell(env, "zulu")?.image).toBe("ghcr.io/hanzoai/bot:v2");
  ...
  it("rejects duplicate tenant ids without replacing the row", () => {
    ...
    expect(() =>
      reserveFleetCell(env, {
        ...params("alpha", 19_301),
        image: "ghcr.io/hanzoai/bot:other",
      }),
