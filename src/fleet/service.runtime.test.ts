...
function fleetLabels(tenant = "acme", attemptId = TEST_ATTEMPT_ID): Record<string, string> {
  return {
    "bot.fleet.tenant": tenant,
    "bot.fleet.owner": cellOwnerId(path.join(root, "fleet", "cells", tenant)),
    "bot.fleet.attempt": attemptId,
    "bot.fleet.env-keys": "FEATURE",
  };
}
...
describe("fleet service", () => {
  ...
  const tempRoot = createSuiteTempRootTracker({ prefix: "bot-fleet-service-" });
  ...
  it("creates a bootable token-only cell config and returns the secret-bearing result", async () => {
    ...
    expect(containers.createNetwork).toHaveBeenCalledWith(
      ...
      "bot-cell-acme-net",
      {
        "bot.fleet.tenant": "acme",
        "bot.fleet.owner": cellOwnerId(path.join(root, "fleet", "cells", "acme")),
        "bot.fleet.attempt": expect.stringMatching(/^[a-f0-9]{32}$/u),
      },
      { internal: false },
    );
    expect(containers.start).toHaveBeenCalledWith("docker", "bot-cell-acme");
    expect(profile?.networkName).toBe("bot-cell-acme-net");
    ...
    const config = JSON.parse(await fs.readFile(path.join(dataDir, "bot.json"), "utf8")) as {
      gateway?: {
        mode?: string;
        bind?: string;
  ...
  it("keeps an unhealthy created cell and its runtime evidence", async () => {
    ...
    await expect(service.create({ tenant: "sick", gatewayToken: "token" })).rejects.toThrow(
      "Fleet cell sick was created but did not become healthy within 60s; inspect it with `bot fleet status sick` or `bot fleet logs sick`, or remove it with `bot fleet rm sick --force`.",
    );
  ...
  it.each(["start", "stop", "restart"] as const)("runs the %s lifecycle action", async (action) => {
    ...
    expect(containers[action]).toHaveBeenCalledWith("docker", "bot-cell-acme");
  });
  ...
  it("carries inspected environment and resources through upgrade", async () => {
    ...
    const diskLabels = { ...fleetLabels(), "bot.fleet.disk-limit": "10g" };
    containers.inspect
    ...
    const result = await service.upgrade("acme", "ghcr.io/hanzoai/bot:v2");

    expect(result).toEqual({
      tenant: "acme",
      action: "upgrade",
      image: "ghcr.io/hanzoai/bot:v2",
    });
    expect(containers.pull).toHaveBeenCalledWith("docker", "ghcr.io/hanzoai/bot:v2");
    expect(containers.stop).toHaveBeenCalledWith("docker", "bot-cell-acme");
    expect(containers.remove).toHaveBeenCalledWith("docker", "bot-cell-acme", false);
    expect(containers.inspectNetwork).toHaveBeenCalledWith("docker", "bot-cell-acme-net");
    const [profile, start] = containers.run.mock.calls[0] ?? [];
    ...
    expect(getFleetCell(env, "acme")?.image).toBe("ghcr.io/hanzoai/bot:v2");
  });

  it("passes digest-pinned images verbatim to create and upgrade", async () => {
    const containers = createContainerMock();
    const digest = `ghcr.io/hanzoai/bot@sha256:${"a".repeat(64)}`;
    const service = createFleetService({
      env,
      containers: containers.runtime,
      generateAttemptId: () => NEXT_ATTEMPT_ID,
  ...
  it("restores the immutable old image when replacement fails", async () => {
    ...
    expect(getFleetCell(env, "acme")?.image).toBe("ghcr.io/hanzoai/bot:latest");
  });
  ...
  it("restores the old cell when the replacement registry update fails", async () => {
    ...
    expect(containers.run).toHaveBeenCalledTimes(2);
    expect(containers.run.mock.calls[1]?.[0].image).toBe("sha256:old-image-id");
    expect(containers.remove).toHaveBeenCalledWith("docker", "bot-cell-acme", true);
    expect(containers.removeNetwork).not.toHaveBeenCalled();
    expect(getFleetCell(env, "acme")?.image).toBe("ghcr.io/hanzoai/bot:latest");
  });
  ...
  it("cleans up its exact-attempt network when network creation fails", async () => {
    ...
    expect(containers.run).not.toHaveBeenCalled();
    expect(containers.removeNetwork).toHaveBeenCalledWith("docker", "bot-cell-acme-net");
    expect(getFleetCell(env, "acme")).toBeUndefined();
  });
  ...
  it("never removes a same-tenant container owned by another profile", async () => {
    const containers = createContainerMock(
      runningInspection({
        labels: {
          ...fleetLabels(),
          "bot.fleet.owner": "11111111111111111111111111111111",
        },
      }),
    );
    containers.run.mockRejectedValue(new Error("container name is already in use"));
