...
export function createFleetService(options: FleetServiceOptions = {}) {
  ...
  return {
        ...
        operation: async (checkpoint) => {
          ...
          if (result.started) {
            try {
              ...
              throw new Error(
                `Fleet cell ${tenantId} was created but did not become healthy within 60s; inspect it with \`bot fleet status ${tenantId}\` or \`bot fleet logs ${tenantId}\`, or remove it with \`bot fleet rm ${tenantId} --force\`.`,
                { cause: error },
              );
            }
          }
    ...
    async logs(logOptions: FleetLogsOptions): Promise<void> {
      ...
      const gatewayCredential = inspection.environment.BOT_GATEWAY_TOKEN;
