...
const FLEET_CONTAINER_STATE_DIR = "/home/node/.bot";
const FLEET_CONTAINER_AUTH_SECRET_DIR = "/home/node/.config/bot";
const TEST_ENVIRONMENT_FILE = "/tmp/bot-fleet-env/cell.env";

function makeProfile(overrides: Partial<CellContainerProfile> = {}): CellContainerProfile {
  const tenantId = overrides.tenantId ?? "acme";
  const dataDir = overrides.dataDir ?? "/tmp/bot/fleet/cells/acme";
  return {
    tenantId,
    ...
    dataDir,
    authSecretDir: overrides.authSecretDir ?? "/tmp/bot/fleet/auth-profile-secrets/acme",
...
describe("fleet tenant ids", () => {
  ...
  it("derives stable container and data paths", () => {
    expect(cellContainerName("acme-2")).toBe("bot-cell-acme-2");
    expect(cellNetworkName("acme-2")).toBe("bot-cell-acme-2-net");
    expect(cellDataDir("/srv/bot", "acme-2")).toBe(
      path.join("/srv/bot", "fleet", "cells", "acme-2"),
    );
    expect(cellAuthSecretDir("/srv/bot", "acme-2")).toBe(
      path.join("/srv/bot", "fleet", "auth-profile-secrets", "acme-2"),
    );
  });
});
...
describe("fleet image references", () => {
  it("normalizes valid references and rejects option-like values", () => {
    expect(validateFleetImage(" ghcr.io/hanzoai/bot:v1 ")).toBe(
      "ghcr.io/hanzoai/bot:v1",
    );
...
describe("fleet disk limits", () => {
  ...
  it("adds storage-opt and the replay label only when configured", () => {
    ...
    expect(withDisk).toContain("bot.fleet.disk-limit=10g");
    expect(withoutDisk).not.toContain("--storage-opt");
    expect(withoutDisk.join(" ")).not.toContain("bot.fleet.disk-limit");
  });
});
...
describe("fleet cell environment", () => {
  ...
  it("pins the documented container paths and token", () => {
    expect(buildCellEnvironment("secret", { REGION: "west" })).toEqual({
      HOME: "/home/node",
      BOT_HOME: "/home/node",
      BOT_STATE_DIR: FLEET_CONTAINER_STATE_DIR,
      BOT_CONFIG_PATH: `${FLEET_CONTAINER_STATE_DIR}/bot.json`,
      BOT_WORKSPACE_DIR: `${FLEET_CONTAINER_STATE_DIR}/workspace`,
...
describe("fleet container arguments", () => {
  it("builds the complete hardened run profile", () => {
    ...
    expectOption(args, "--name", "bot-cell-acme");
    ...
    expectOption(args, "--network", "bot-cell-acme-net");
