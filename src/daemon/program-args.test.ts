...
describe("resolveGatewayProgramArguments", () => {
  it("prefers index.js over legacy entry.js when both exist in the same dist directory", async () => {
    const entryPath = path.resolve("/opt/bot/dist/entry.js");
    const indexPath = path.resolve("/opt/bot/dist/index.js");
  ...
  it("keeps entry.js when index.js is missing", async () => {
    const entryPath = path.resolve("/opt/bot/dist/entry.js");
    const indexPath = path.resolve("/opt/bot/dist/index.js");
    const indexMjsPath = path.resolve("/opt/bot/dist/index.mjs");
    process.argv = ["node", entryPath];
    fsMocks.realpath.mockResolvedValue(entryPath);
  ...
  it("uses realpath-resolved dist entry when running via npx shim", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/bot");
    const entryPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/bot/dist/entry.js");
  ...
  it("prefers symlinked path over realpath for stable service config", async () => {
    ...
    const realpathResolved = path.resolve(
      "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/bot@2026.1.21-2/node_modules/bot/dist/entry.js",
    );
    ...
    expect(result.programArguments[1]).toBe(
      path.resolve("/Users/test/Library/pnpm/global/5/node_modules/bot/dist/index.js"),
    );
  ...
  it("falls back to node_modules package dist when .bin path is not resolved", async () => {
    const argv1 = path.resolve("/tmp/.npm/_npx/63c3/node_modules/.bin/bot");
    const indexPath = path.resolve("/tmp/.npm/_npx/63c3/node_modules/bot/dist/index.js");
    process.argv = ["node", argv1];
  ...
  it("uses trusted Windows where.exe when resolving the Node runtime", async () => {
    const repoIndexPath = path.resolve("/repo/src/index.ts");
    const repoEntryPath = path.resolve("/repo/src/entry.ts");
    const launcherPath = String.raw`D:\Bot\bot.exe`;
  ...
  it("rejects a non-executable wrapper file", async () => {
    ...
    ).rejects.toThrow("BOT_WRAPPER must point to an executable file");
  });
});
