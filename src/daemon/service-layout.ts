/** Summarizes installed service command paths and Bot package layout. */
...
async function resolveBotPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveBotPackageRoot(entrypointReal) : undefined;
