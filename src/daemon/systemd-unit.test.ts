...
describe("buildSystemdUnit", () => {
  ...
  it("renders EnvironmentFile entries before inline Environment values", () => {
    const unit = buildSystemdUnit({
      description: "Bot Gateway",
      programArguments: ["/usr/bin/bot", "gateway", "run"],
      environmentFiles: ["/home/test/.bot/.env"],
      environment: {
        BOT_GATEWAY_PORT: "18789",
      },
    });
    expect(unit).toContain("EnvironmentFile=-/home/test/.bot/.env");
    expect(unit).toContain("Environment=BOT_GATEWAY_PORT=18789");
    expect(unit.indexOf("EnvironmentFile=-/home/test/.bot/.env")).toBeLessThan(
      unit.indexOf("Environment=BOT_GATEWAY_PORT=18789"),
    );
  });
});
