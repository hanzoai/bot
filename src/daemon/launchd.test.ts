...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.BOT_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.BOT_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-launchctl-test-"));
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.BOT_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env.BOT_TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env.BOT_TEST_LAUNCHCTL_LOG = logPath;
    process.env.BOT_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    ...
    if (originalLogPath === undefined) {
      delete process.env.BOT_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.BOT_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.BOT_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.BOT_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    await withLaunchctlStub({ listOutput: "123 0 ai.bot.gateway\n" }, async ({ env }) => {
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when BOT_PROFILE is default", () => {
    const env = { HOME: "/Users/test", BOT_PROFILE: "default" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.bot.gateway.plist",
    );
  });
  ...
  it("uses profile-specific label when BOT_PROFILE is set to a custom value", () => {
    ...
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.bot.jbphoenix.plist",
    );
  });
  ...
  it("ignores empty BOT_LAUNCHD_LABEL and falls back to profile", () => {
    ...
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.bot.myprofile.plist",
    );
  });
  ...
  it("trims whitespace from BOT_PROFILE", () => {
    const env = { HOME: "/Users/test", BOT_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.bot.myprofile.plist",
    );
  });
