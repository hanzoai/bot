...
function auditGatewayToken(
  ...
  issues.push({
    ...
    detail: "Run `bot gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service BOT_GATEWAY_TOKEN does not match gateway.auth.token in bot.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.BOT_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.BOT_GATEWAY_TOKEN);
}
...
async function auditGatewayRuntime(
  ...
  if (isBunRuntime(execPath)) {
    issues.push({
      ...
      message: "Gateway service uses Bun; Bot runtime state requires node:sqlite.",
      detail: execPath,
      level: "recommended",
    });
    return;
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  ...
  issues.push({
    ...
    message: `Gateway service was installed by Bot ${serviceVersion}; current CLI is ${VERSION}.`,
