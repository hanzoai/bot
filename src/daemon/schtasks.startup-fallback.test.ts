...
function resolveStartupEntryPath(env: Record<string, string>, extension = "cmd") {
  const taskName = env.BOT_WINDOWS_TASK_NAME ?? "Bot Gateway";
  return path.join(
...
async function writeNodeScript(env: Record<string, string>, port = "18789") {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "BOT_SERVICE_KIND=node"`,
      `set "BOT_GATEWAY_PORT=${port}"`,
      `"C:\\bin\\bot.cmd" node run --host 127.0.0.1 --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
...
function makeNodeServiceEnv(env: Record<string, string>): Record<string, string> {
  return {
    ...
    BOT_WINDOWS_TASK_NAME: "Bot Node",
  };
}
        ...
        stdout: JSON.stringify(
          processAlive
            ? [
                {
                  ProcessId: processId,
                  CommandLine: "C:\\bin\\bot.cmd node run --host 127.0.0.1 --port 18789",
                },
...
function expectStartupFallbackSpawn() {
  ...
  expect((options.env as Record<string, string> | undefined)?.BOT_GATEWAY_PORT).toBe("18789");
...
function installGatewayScheduledTask(
  ...
  return installScheduledTask({
    ...
    environment: { BOT_GATEWAY_PORT: port },
...
function installNodeScheduledTask(env: Record<string, string>, stdout = new PassThrough()) {
  return installScheduledTask({
    ...
    stdout,
    programArguments: ["node", "bot", "node", "run", "--host", "127.0.0.1", "--port", "18789"],
    environment: {
      BOT_SERVICE_KIND: "node",
...
describe("Windows startup fallback", () => {
  it("skips task ownership probes when no Startup fallback exists", async () => {
    await withWindowsEnv("bot-win-startup-", async ({ env }) => {
  ...
  it("migrates an exact persisted wrapper that owns the replacement port", async () => {
    await withWindowsEnv("bot-win-startup-", async ({ env }) => {
      ...
      await fs.writeFile(
        ...
        [
          "@echo off",
          'set "BOT_GATEWAY_PORT=18789"',
          '"C:\\bin\\bot-doppler.exe" gateway --port 18789',
          "",
        ].join("\r\n"),
        "utf8",
      );
            ...
            stdout: JSON.stringify(
              processAlive
                ? [
                    {
                      ProcessId: 4242,
                      CommandLine: '"C:\\bin\\bot-doppler.exe" gateway --port 18789',
                    },
                    { ProcessId: 9999, CommandLine: "powershell.exe" },
                  ]
                : [{ ProcessId: 9999, CommandLine: "powershell.exe" }],
        ...
        if (
          ...
          return makeSpawnSyncResult({
            stdout: JSON.stringify([
              {
                ProcessId: 3131,
                CommandLine: "C:\\manual\\bot.cmd gateway --port 18789",
              },
              {
                ProcessId: 4242,
                CommandLine:
            ...
            ? [
                {
                  pid: 4242,
                  command: "node.exe",
                  commandLine: 'node "C:\\bot\\dist\\index.js" gateway --port 18789',
                },
              ]
            : [],
        hints: [],
  ...
  it("accepts a clean hidden-launcher exit when its gateway listener is running", async () => {
    await withWindowsEnv("bot-win-startup-", async ({ env }) => {
      const hiddenEnv = { ...env, BOT_WINDOWS_TASK_HIDDEN_LAUNCHER: "1" };
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 5151,
                CommandLine: "node bot node run --host 127.0.0.1 --port 18789",
              },
            ]),
            stderr: "",
            status: 0,
  ...
  it("does not report a node task as running from a gateway listener", async () => {
    await withWindowsEnv("bot-win-startup-", async ({ env }) => {
      env.BOT_SERVICE_KIND = "node";
      env.BOT_WINDOWS_TASK_NAME = "Bot Node";
      findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4242]);
      schtasksResponses.push(
        { code: 0, stdout: "", stderr: "" },
        { code: 0, stdout: notYetRunTaskQueryOutput(), stderr: "" },
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 4242,
                CommandLine: "C:\\manual\\bot.cmd node run --host 127.0.0.1 --port 18789",
              },
              {
                ProcessId: 5151,
                CommandLine: "C:\\bin\\bot.cmd node run --host 127.0.0.1 --port 18789",
