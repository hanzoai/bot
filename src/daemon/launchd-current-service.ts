...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.BOT_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.BOT_SERVICE_MARKER) === "bot" &&
    Boolean(normalizeOptionalString(env.BOT_SERVICE_KIND))
  ) {
