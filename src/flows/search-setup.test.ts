        ...
        plugins: {
          ...
          installs: {
            ...
            [entry.pluginId]: {
              ...
              installPath: `/tmp/bot-plugins/${entry.pluginId}`,
            },
          },
        },
      },
...
describe("runSearchSetupFlow", () => {
  ...
  it("names no-provider and user-skip outcomes as kept-current", async () => {
    ...
    const original: BotConfig = { gateway: { mode: "local" } };
    const noProviderConfig: BotConfig = {
      ...original,
  ...
  it("localizes setup copy for web search provider selection", async () => {
    ...
    await withEnvAsync({ BOT_LOCALE: "zh-CN" }, async () => {
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            label: "暂时跳过",
            hint: "稍后可用 bot configure --section web 配置",
          }),
        ]),
      }),
    );
  ...
  it("installs an external catalog search provider before enabling it", async () => {
    ...
    expect(installRequest.entry?.install?.npmSpec).toBe("@hanzo/bot-brave-plugin");
    ...
    expect(next.plugins?.installs?.brave?.spec).toBe("@hanzo/bot-brave-plugin");
  });
