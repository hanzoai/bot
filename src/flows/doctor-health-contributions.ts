...
async function runGatewayHealthChecks(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if ((await hasActiveGatewayExecCredential(ctx)) && ctx.options.allowExec !== true) {
    note(
      "Gateway health probes skipped because gateway credentials use an exec SecretRef. Run `bot doctor --allow-exec` to verify Gateway health with exec SecretRefs.",
      "Gateway",
    );
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("bot.doctorHealthContributionsTestApi")
