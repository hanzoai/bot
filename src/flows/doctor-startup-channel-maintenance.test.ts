...
describe("doctor startup channel maintenance", () => {
  ...
  it("maps channel doctor preview warnings to structured findings", async () => {
    ...
    await expect(
      collectChannelPreviewWarningHealthFindings({
        cfg,
        doctorFixCommand: "bot doctor --fix --dry-run",
        env: { BOT_TEST: "1" },
        allowExec: true,
      }),
    ).resolves.toEqual([
      {
        ...
        fixHint:
          "Run `bot doctor --fix --dry-run` if the channel warning recommends repair, or update the affected channel config manually.",
      },
    ]);
  ...
  it("runs Matrix startup migration during repair flows", async () => {
    ...
    expect(call.env).toEqual({ BOT_TEST: "1" });
