...
export type CoreHealthCheckDeps = {
  readonly detectUnavailableSkills: (cfg: BotConfig) => Promise<readonly SkillStatusEntry[]>;
  readonly collectSecurityWarnings: (cfg: BotConfig) => Promise<readonly string[]>;
...
async function collectSecurityWarningsWithRuntime(cfg: BotConfig): Promise<readonly string[]> {
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "bot.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `bot configure` and set Gateway mode (local/remote), or `bot config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `bot config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `bot config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};
...
function resolveDoctorMode(cfg: BotConfig): "local" | "remote" {
...
export function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `bot doctor --allow-exec` to verify exec SecretRefs during doctor, or `bot secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
    ...
    return [
      {
        ...
        path: "gateway.auth",
        fixHint: "Run `bot doctor --fix --generate-gateway-token` to generate a token.",
      },
    ];
  },
};
    ...
    return [
      ...detected.preview.map(
        (line): HealthFinding => ({
          ...
          fixHint: "Run `bot doctor --fix` to migrate legacy state.",
        }),
      ),
      ...detected.warnings.map(
        (warning): HealthFinding => ({
          ...
          message: warning,
          path: detected.stateDir,
          fixHint: "Resolve the warning, then rerun `bot doctor --fix`.",
        }),
      ),
    ];
  },
      ...
      (issue): HealthFinding => ({
        ...
        fixHint: issue.repairBlocked
          ? [
              ...
              "or set the affected OpenAI models to an Bot runtime policy.",
            ].join(" ")
          : [
              "Run `bot doctor --fix`: it enables plugins.entries.codex,",
              "or set the affected OpenAI models to an Bot runtime policy.",
            ].join(" "),
      }),
    );
...
const telegramGeneralTopicConversationsCheck: HealthCheck = {
  ...
  async detect(ctx) {
    ...
    return repairs.map((repair) => ({
      ...
      fixHint: "Run `bot doctor --fix` to merge the stale topic-qualified identity.",
    }));
  },
  async repair(ctx) {
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `bot doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyClawdBrowserProfileResidue): HealthFinding {
  return {
    ...
    fixHint:
      "Run `bot doctor --fix` to archive the stale clawd profile safely instead of deleting it in place.",
  };
}
...
const browserClawdProfileResidueCheck: HealthCheck = {
  ...
  kind: "core",
  description:
    "Legacy clawd managed browser profile residue has been archived after the Bot rename.",
  source: "doctor",
  async detect(ctx, scope) {
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active bot.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
