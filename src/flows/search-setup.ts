...
type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<BotConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<BotConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.bot.ai/tools/web";
...
function defaultModelUsesCodexRuntime(config: BotConfig): boolean {
...
function rawKeyValue(config: BotConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: BotConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: BotConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: BotConfig, result: BotConfig): BotConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as BotConfig["plugins"],
  };
}
...
type SearchSetupResult =
  | { outcome: "completed"; config: BotConfig }
  | {
...
function completedSearchSetup(config: BotConfig): SearchSetupResult {
...
export async function runSearchSetupFlow(
  ...
  const finalizeSelection = (nextConfig: BotConfig) =>
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Bot will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.bot.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Bot will store a reference instead of the API key.",
