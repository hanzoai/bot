...
import { BUNDLED_PLUGIN_ROOT_DIR } from "bot/plugin-sdk/test-fixtures";
...
describe("Dockerfile", () => {
  ...
  it("uses full bookworm for build stages and slim bookworm for runtime", async () => {
    ...
    expect(dockerfile).toContain("FROM ${BOT_NODE_BOOKWORM_IMAGE} AS workspace-deps");
    expect(dockerfile).toContain("FROM ${BOT_NODE_BOOKWORM_IMAGE} AS build");
    expect(dockerfile).toContain("FROM ${BOT_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime");
    ...
    expect(dockerfile).not.toContain("BOT_VARIANT");
  });
  ...
  it("uses portable copies for workspace dependency inputs", async () => {
    ...
    const workspaceDepsEnd = dockerfile.indexOf("FROM ${BOT_BUN_IMAGE} AS bun-binary");
  ...
  it("copies install workspace manifests before pnpm install", async () => {
    ...
    expect(dockerfile).toContain("done < /out/bot-selected-plugin-dirs");
  ...
  it("keeps validated plugin selection outside the build-context copy destination", async () => {
    ...
    expect(dockerfile).not.toContain("/app/.bot-selected-plugin-dirs");
    expect(dockerfile).not.toContain("./.bot-selected-plugin-dirs");
    expect(dockerfile).toContain("grep -qx 'matrix' /tmp/bot-selected-plugin-dirs");
    expect(dockerfile).toContain(
      'selected_plugin_dirs="$(cat /tmp/bot-selected-plugin-dirs)"',
    );
    expect(dockerfile).toContain('BOT_EXTENSIONS="$(cat /tmp/bot-selected-plugin-dirs)"');
  });
  ...
  it("does not let pnpm resync the full source workspace during Docker build scripts", async () => {
    ...
    expect(collapsed).not.toContain(
      'BOT_DOCKER_BUILD_EXTENSIONS="$BOT_EXTENSIONS" BOT_RUN_NODE_SKIP_DTS_BUILD=',
    );
  ...
  it("shares public source provenance across backend and Control UI builds", async () => {
    ...
    const timestampArgIndex = dockerfile.indexOf('ARG BOT_BUILD_TIMESTAMP=""');
    ...
    expect(dockerfile).toContain("BOT_BUILD_TIMESTAMP=${BOT_BUILD_TIMESTAMP}");
    expect(dockerfile).toContain('BOT_BUILD_TIMESTAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"');
  ...
  it("documents provenance arguments for manual source builds", async () => {
    ...
    expect(docs).toContain('--build-arg "BOT_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}"');
  ...
  it("prunes runtime dependencies and omitted plugin packages after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG BOT_EXTENSIONS");
    expect(dockerfile).toContain(
    ...
    expect(dockerfile).toContain("ARG BOT_BUNDLED_PLUGIN_DIR");
    expect(dockerfile).toContain(
    ...
    expect(dockerfile).toContain("readlink -f /app/node_modules/@hanzo/bot-ai");
    expect(dockerfile).toContain('mv "$ai_runtime_tmp/ai" /app/node_modules/@hanzo/bot-ai');
  ...
  it("keeps the Codex plugin in official Docker release images", async () => {
    ...
    const releaseKeepList = "BOT_EXTENSIONS=diagnostics-otel,codex";

    expect(workflow.match(new RegExp(releaseKeepList, "g"))).toHaveLength(4);
    expect(workflow).not.toContain("BOT_EXTENSIONS=diagnostics-otel\n");
  });
  ...
  it("uses one source commit and timestamp for every official Docker artifact", async () => {
    ...
    expect(
      workflow.split(
        "BOT_BUILD_TIMESTAMP=${{ needs.resolve_build_provenance.outputs.built_at }}",
      ).length - 1,
    ).toBe(4);
  });
  ...
  it("publishes official Docker browser images with baked Chromium", async () => {
    ...
    expect(workflow).toContain("BOT_INSTALL_BROWSER=1");
    ...
    expect(workflow).toContain("grep -q '^ARG BOT_INSTALL_BROWSER' Dockerfile");
  ...
  it("publishes official Docker releases to GHCR and Docker Hub", async () => {
    ...
    expect(workflow).toContain("DOCKERHUB_IMAGE_NAME: hanzoai/bot");
  ...
  it("smokes runtime workspace templates before Docker release manifests publish", async () => {
    ...
    expect(workflow).not.toContain('test -f "${temp_root}/home/.bot/workspace/HEARTBEAT.md"');
  });
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    const dockerfile = collapseDockerContinuations(await readFile(dockerfilePath, "utf8"));
    expect(dockerfile).toContain(`ARG BOT_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bBOT_BUNDLED_PLUGINS_DIR\b/m);
  });
  ...
  it("pre-creates named-volume mount points before switching to the node user", async () => {
    ...
    // before the leaf .config/bot dir (issue #85968).
    ...
    expect(dockerfile).not.toContain("mkdir -p /home/node/.bot");
    expect(dockerfile).toContain("/home/node/.bot/workspace");
    expect(dockerfile).toContain("/home/node/.config/bot");
    expect(dockerfile).toContain(
