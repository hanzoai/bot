...
import { normalizeCsvOrLooseStringList } from "@hanzo/bot-normalization-core/string-normalization";
...
/** Parses the JSON5 Bot manifest block embedded inside a string frontmatter field. */
...
/** Extracts normalized runtime requirement lists from an Bot manifest block. */
...
/** Extracts normalized OS allowlist entries from an Bot manifest block. */
export function resolveBotManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

type ParsedBotManifestInstallBase = {
...
export function applyBotManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedBotManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
