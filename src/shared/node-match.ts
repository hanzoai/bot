...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current Bot nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentBotClient(clientId: string | undefined): boolean {
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentBotClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
