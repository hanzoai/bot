...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("bot security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("bot security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusModelSelectionLines(params: {
  ...
  for (const sess of mismatches.slice(0, limit)) {
    ...
    lines.push(
      ...
      reasonLine,
      clearLine,
      "  Docs: https://docs.bot.ai/concepts/models#selection-source-and-fallback-behavior",
    );
  }
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.bot.ai/faq",
    "Troubleshooting: https://docs.bot.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("bot status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("bot logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("bot node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("bot status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("bot gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`bot devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("bot devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("bot devices list")}`),
  ];
}
