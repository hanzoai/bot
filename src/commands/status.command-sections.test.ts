...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:bot security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:bot security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:bot status --all",
      "  Need to debug live? cmd:bot logs --follow",
      "  Fix reachability first: cmd:bot gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:bot devices approve req-123)",
      "muted(Fallback: cmd:bot devices approve --latest)",
      "muted(Inspect: cmd:bot devices list)",
    ]);
  });
