...
export function writeSessionSqliteMigrationFailureReports(
  ...
  const payload = {
    ...
    recoveryCommand: "bot doctor --session-sqlite recover --github-issue",
...
export function createSessionSqliteMigrationFailureIssue(
  ...
  const body = [
    "Bot doctor generated this sanitized report from a local session SQLite migration recovery.",
    "",
    reportBody,
  ].join("\n");
...
function createPrefilledGithubIssueUrl(title: string, body: string): string {
  ...
  return `https://github.com/hanzoai/bot/issues/new?${params.toString()}`;
}
