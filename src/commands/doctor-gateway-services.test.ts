...
const originalUpdateInProgress = process.env.BOT_UPDATE_IN_PROGRESS;
const originalParentSupportsConfigWrite =
  process.env.BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE;
const originalParentSupportsGatewayRestart =
  process.env.BOT_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART;
const originalParentAllowsGatewayServiceRepair =
  process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR;
const originalParentAllowsGatewayActivation =
  process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION;
...
const LEGACY_MAC_LABEL = "com.bot.gateway";
const LEGACY_MAC_PLIST = "/Users/test/Library/LaunchAgents/com.bot.gateway.plist";
...
async function runRepair(cfg: BotConfig, options: { allowExecSecretRefs?: boolean } = {}) {
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.readWindowsProcessArgsSync.mockReturnValue(["node", "bot.mjs", "update"]);
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: BotConfig, env) => {
      const configToken =
  ...
  it("reports the installed Gateway heap limit and derivation", async () => {
    const command = createGatewayCommand("/opt/bot/dist/index.js");
  ...
  it("does not duplicate gateway runtime warnings already emitted by the node install plan", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: [nvmNode, "/usr/local/bin/bot", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("passes planned managed env keys into service audit for legacy inline secret detection", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...
      workingDirectory: "/tmp",
      environment: {
        BOT_SERVICE_MANAGED_ENV_KEYS: "TAVILY_API_KEY",
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
  ...
  it("repairs gateway services whose pinned port differs from current config", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: ["/usr/bin/node", "/usr/local/bin/bot", "gateway", "--port", "18888"],
      workingDirectory: "/tmp",
      environment: {},
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/bot/dist/index.js",
      installEntrypoint:
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/bot@2026.3.12/node_modules/bot/dist/index.js",
      realpath: async (value: string) => {
        const normalized = value.replaceAll("\\", "/").replace(/^[A-Z]:/i, "");
        if (normalized.includes("/global/5/node_modules/bot/")) {
          return normalized.replace(
            "/global/5/node_modules/bot/",
            "/global/5/node_modules/.pnpm/bot@2026.3.12/node_modules/bot/",
          );
        }
        return normalized;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/bot/../bot/dist/index.js",
      installEntrypoint: "/opt/bot/dist/index.js",
  ...
  it("keeps wrapper-managed gateway services aligned during entrypoint drift checks", async () => {
    const wrapperPath = "/usr/local/bin/bot-doppler";
    mocks.readCommand.mockResolvedValue({
      programArguments: [wrapperPath, "gateway", "--port", "18789"],
      environment: {
        BOT_WRAPPER: wrapperPath,
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: true,
    ...
    mocks.buildGatewayInstallPlan.mockImplementation(async ({ env }) => ({
      programArguments: [env.BOT_WRAPPER, "gateway", "--port", "18789"],
      environment: {
        BOT_WRAPPER: env.BOT_WRAPPER,
      },
    }));
    ...
    expect(requireRecord(installPlanOptions.env, "install env").BOT_WRAPPER).toBe(wrapperPath);
    expect(
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      "Gateway service invokes BOT_WRAPPER: /usr/local/bin/bot-doppler",
      "Gateway",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint:
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/bot/dist/index.js",
      installEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/bot/dist/index.js",
    });
  ...
  it("skips entrypoint rewrites for an active systemd unit", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      ...createGatewayCommand("/opt/old-bot/dist/index.js"),
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...createGatewayCommand("/opt/new-bot/dist/index.js"),
      workingDirectory: "/tmp",
    });
  ...
  it("leaves all service metadata unchanged when an active unit has command drift plus other issues", async () => {
    mockProcessPlatform("linux");
    mocks.readCommand.mockResolvedValue({
      programArguments: ["/usr/bin/bot", "run"],
      environment: {},
      sourcePath: "/home/test/.config/systemd/user/bot-gateway.service",
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
  ...
  it("skips entrypoint rewrite in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/bot/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/bot/dist/index.js",
    ...
    expectNoteContaining("bot gateway install --force", "Gateway service config");
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  });
  ...
  it("defers systemd service config rewrites during non-interactive update repairs", async () => {
    mockProcessPlatform("linux");
    process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/bot/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/bot/dist/index.js",
      installWorkingDirectory: "/tmp",
  ...
  it("does not persist or stage embedded service tokens during systemd update repairs", async () => {
    ...
    process.env.BOT_UPDATE_IN_PROGRESS = "1";
    process.env.BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
  ...
  it.each([
    ["update command", ["node", "bot.mjs", "update"]],
    ["--update shorthand", ["node", "bot.mjs", "--update"]],
    ["doctor update prompt", ["node", "bot.mjs", "doctor"]],
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
        BOT_SERVICE_VERSION: "2026.5.25",
        BOT_WINDOWS_TASK_NAME: "Bot Gateway Work",
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
        {
          ...
          message: "Gateway service was installed by an older Bot version.",
          level: "recommended",
        },
      ],
    });
    ...
    expectNoteContaining(
      "Gateway service was installed by an older Bot version.",
      "Gateway service config",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  ...
  it("stages running Windows repairs when the update parent forbids activation", async () => {
    ...
    process.env.BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "0";
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
        BOT_SERVICE_VERSION: "2026.5.25",
  ...
  it("does not rewrite a service when the update parent rejects ownership", async () => {
    mockProcessPlatform("win32");
    process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "0";
    process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "0";
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
  ...
  it.each([
    {
      ...
      args: ["node", "bot.mjs", "update", "--no-restart"],
    },
    {
    ...
    },
    {
      ...
      args: ["node", "bot.mjs", "update", "wizard"],
    },
    {
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: { BOT_SERVICE_VERSION: "2026.5.25" },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      workingDirectory: "/tmp",
      environment: { BOT_SERVICE_VERSION: "2026.5.26" },
    });
    mocks.readRuntime.mockResolvedValue({ status: "running" });
  ...
  it("persists embedded service tokens before Windows update repairs rewrite the task", async () => {
    ...
    process.env.BOT_UPDATE_IN_PROGRESS = "1";
    process.env.BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    process.env.BOT_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART = "1";
    process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    process.env.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "1";
  ...
  it("reports service config drift but skips service rewrite when service repair policy is external", async () => {
    await withEnvAsync({ BOT_SERVICE_REPAIR_POLICY: "external" }, async () => {
      setupGatewayEntrypointRepairScenario({
        currentEntrypoint: "/Users/test/Library/npm/node_modules/bot/dist/entry.js",
        installEntrypoint: "/Users/test/Library/npm/node_modules/bot/dist/index.js",
        installWorkingDirectory: "/tmp",
  ...
  it("warns when the gateway service entrypoint resolves to a source checkout", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "bot-doctor-service-layout-"));
      try {
  ...
  it("does not duplicate Gateway service config panels for a source-checkout entrypoint with audit findings", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(
        path.join(os.tmpdir(), "bot-doctor-service-config-dedup-"),
      );
      try {
        await fs.mkdir(path.join(root, ".git"), { recursive: true });
        await fs.mkdir(path.join(root, "src"), { recursive: true });
        ...
        const installEntrypoint = "/usr/local/lib/node_modules/bot/dist/index.js";
        setupGatewayEntrypointRepairScenario({
        ...
        const forceMatches = consolidated.match(/bot gateway install --force/g) ?? [];
  ...
  it("keeps the gateway install force hint when a source-checkout warning is suppressed and repair is declined", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(
        path.join(os.tmpdir(), "bot-doctor-service-config-force-hint-"),
      );
      try {
        await fs.mkdir(path.join(root, ".git"), { recursive: true });
        await fs.mkdir(path.join(root, "src"), { recursive: true });
        ...
        expect(gatewayServiceConfigNotes[1]?.[0]).toContain("bot gateway install --force");
      } finally {
        await fs.rm(root, { recursive: true, force: true });
      }
    });
