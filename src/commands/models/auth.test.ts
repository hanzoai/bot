...
vi.mock("../../plugins/provider-auth-choice-helpers.js", async (importOriginal) => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: BotConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: BotConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let lastUpdatedConfig: BotConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: BotConfig) => BotConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/bot/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: BotConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/bot/agents/coder" : "/tmp/bot/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: BotConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/bot/workspaces/coder" : "/tmp/bot/workspace",
    );
  ...
  it("runs plugin-owned openai login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/bot/agents/main", {
      externalCli: {
        mode: "scoped",
    ...
    expect(upsertCall.agentDir).toBe("/tmp/bot/agents/main");
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Default model available: openai/gpt-5.5 (current default unchanged; run bot models set openai/gpt-5.5 to apply)",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Configure the `web_search` tool with `bot configure --section web`. Docs: https://docs.bot.ai/tools/web",
    );
  ...
  it("uses the requested agent store for provider auth login", async () => {
    ...
    expect(authRunCall.agentDir).toBe("/tmp/bot/agents/coder");
    expect(authRunCall.workspaceDir).toBe("/tmp/bot/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/bot/agents/coder");
  });
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(providerResolutionCall.workspaceDir).toBe("/tmp/bot/workspace");
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/bot/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/bot/workspace");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai. Verify plugins via `bot plugins list --json`.',
    );
  });
  ...
  it("writes pasted Anthropic setup-tokens and logs the preference note", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic setup-token auth is supported in Bot.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Bot prefers Claude CLI reuse when it is available on the host.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic staff told us this Bot path is allowed again.",
    );
  });
  ...
  it("rejects OpenAI API keys pasted as OpenAI Codex token material", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like an OpenAI API key. Use bot models auth paste-api-key --provider openai for API-key auth.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects token material pasted into the OpenAI Codex API-key command", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like token or OAuth material, not an OpenAI API key. Use bot models auth paste-token --provider openai for token auth material.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "bot agents list" to see configured agents.',
    );
  ...
  it("uses the requested agent store for setup-token provider auth", async () => {
    ...
    expect(tokenAuthCall.agentDir).toBe("/tmp/bot/agents/coder");
    expect(tokenAuthCall.workspaceDir).toBe("/tmp/bot/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/bot/agents/coder");
  });
