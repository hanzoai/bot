...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under BOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-oauth-"));
    process.env.BOT_STATE_DIR = tempStateDir;
    process.env.BOT_AGENT_DIR = path.join(tempStateDir, "agent");
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to BOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-minimax-"));
    process.env.BOT_STATE_DIR = tempStateDir;
    process.env.BOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.BOT_AGENT_DIR;
