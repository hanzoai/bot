...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.missingGuiSession && platform === "darwin") {
    ...
    hints.push(
      `Sign in to the macOS desktop as this user, then run: ${formatCliCommand("bot gateway restart", env)}`,
    );
    hints.push(
  ...
  if (runtime.missingSupervision && platform === "darwin") {
    hints.push(
      `LaunchAgent installed but not loaded. Run: ${formatCliCommand("bot gateway restart", env)}`,
    );
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
  ...
  if (runtime.status === "stopped") {
    if (platform === "linux" && isSystemdStartLimitHit(runtime)) {
      ...
      hints.push(
        ...
        `Recover with: ${formatCliCommand("bot gateway restart", env)}, then inspect logs if it keeps crashing.`,
      );
    } else {
  ...
  if (platform === "linux" && isSystemdCgroupHygieneRisk(runtime.systemd)) {
    const unit =
      runtime.systemd?.unit ?? `${resolveGatewaySystemdServiceName(env.BOT_PROFILE)}.service`;
    ...
    if (summary) {
      hints.push(
        ...
        `After reviewing service settings, run: ${formatCliCommand("bot gateway restart", env)}`,
      );
    }
  }
  return hints;
