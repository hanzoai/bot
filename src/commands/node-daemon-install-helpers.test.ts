...
describe("buildNodeInstallPlan", () => {
  ...
  it("marks node gateway credentials as file-backed service env", async () => {
    ...
    mocks.renderSystemNodeWarning.mockReturnValue(undefined);
    mocks.buildNodeServiceEnvironment.mockReturnValue({
      BOT_GATEWAY_TOKEN: "node-token",
      BOT_GATEWAY_PASSWORD: "node-password",
      BOT_SERVICE_VERSION: "2026.3.22",
    });
    ...
    expect(plan.environment.BOT_GATEWAY_TOKEN).toBe("node-token");
    expect(plan.environment.BOT_GATEWAY_PASSWORD).toBe("node-password");
    expect(plan.environmentValueSources).toEqual({
      BOT_GATEWAY_TOKEN: "file",
      BOT_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
