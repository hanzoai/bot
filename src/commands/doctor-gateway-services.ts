...
import { replaceConfigFile, type BotConfig } from "../config/config.js";
...
import { BOT_WRAPPER_ENV_KEY } from "../daemon/program-args.js";
...
function buildGatewayServiceRepairEnv(
  ...
  const wrapperPath = command?.environment?.[BOT_WRAPPER_ENV_KEY]?.trim();
  if (!wrapperPath || Object.hasOwn(process.env, BOT_WRAPPER_ENV_KEY)) {
    return process.env;
  }
  return {
    ...process.env,
    [BOT_WRAPPER_ENV_KEY]: wrapperPath,
  };
}
...
function resolveGatewayServiceWrapperPath(
  ...
  return normalizeOptionalString(command?.environment?.[BOT_WRAPPER_ENV_KEY]) ?? null;
}
...
function resolveSystemdUnitNameFromServicePath(sourcePath: string | undefined): string {
  ...
  return base.endsWith(".service") ? base : "bot-gateway.service";
}
...
export function extraGatewayServiceToHealthFinding(service: ExtraGatewayService): HealthFinding {
  return {
    ...
    fixHint:
      service.legacy === true
        ? "Run bot doctor --fix to remove legacy gateway services."
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (serviceWrapperPath) {
    note(`Gateway service invokes ${BOT_WRAPPER_ENV_KEY}: ${serviceWrapperPath}`, "Gateway");
  }
  ...
  const sourceCheckoutWarning = serviceLayout?.entrypointSourceCheckout
    ? [
        ...
        "Run `bot doctor --fix` from the intended package install, or reinstall the gateway service with `bot gateway install --force`.",
      ].join("\n")
    : null;
  ...
  if (serviceRewriteBlocked) {
    note(
      "Gateway service is running; leaving supervisor metadata unchanged. Stop the service first or use `bot gateway install --force` when you want to replace the active launcher.",
      "Gateway service config",
    );
    return cfg;
  }
  ...
  if (
    ...
    note(
      "Update-mode doctor detected gateway service drift but left the live systemd unit unchanged. Review the service file and run `bot gateway install --force` when you want Bot to replace operator-owned systemd directives.",
      "Gateway service config",
    );
    return cfg;
  }
  ...
  if (!repair) {
    if (!emittedSourceCheckoutWarning) {
      note(
        "Run `bot gateway install --force` when you want to replace the gateway service definition.",
        "Gateway service config",
      );
    }
    return cfg;
  ...
  const installedWindowsTaskName = command.environment?.BOT_WINDOWS_TASK_NAME?.trim();
  const serviceRepairEnv =
    updateRepairWillRewriteWindowsTask && installedWindowsTaskName
      ? {
          ...serviceInstallEnv,
          BOT_WINDOWS_TASK_NAME: installedWindowsTaskName,
        }
      : serviceInstallEnv;
  ...
  try {
    ...
    if (
      ...
      if (installedWindowsTaskName) {
        ...
        restartEnv.BOT_WINDOWS_TASK_NAME = installedWindowsTaskName;
      }
