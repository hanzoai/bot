...
const resolveAgentDir = vi.hoisted(() =>
  vi.fn<(_cfg: BotConfig, agentId: string) => string>(() => "/tmp/agent-default"),
);
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn<(_cfg: BotConfig, agentId: string) => string>(() => "/tmp/agent-default/workspace"),
);
...
describe("noteMemorySearchHealth", () => {
  ...
  it("warns when local provider is set but readiness was not confirmed", async () => {
    ...
    expect(message).toContain("bot plugins install @hanzo/bot-llama-cpp-provider");
  });
  ...
  it("does not warn when local provider readiness probe was intentionally skipped", async () => {
    ...
    await noteMemorySearchHealth(cfg, {
      gatewayMemoryProbe: {
        ...
        ready: false,
        error:
          "memory embedding readiness not checked; run `bot memory status --deep` to probe",
        skipped: true,
      },
    });
  ...
  it("warns when local provider skipped readiness but configured local model is missing", async () => {
    resolveMemorySearchConfig.mockReturnValue({
      provider: "local",
      local: { modelPath: "/definitely/missing/bot-memory-model.gguf" },
      remote: {},
    });
  ...
  it("does not warn when an enabled alternate memory plugin owns the memory slot", async () => {
    ...
    const cfgWithLancedb = {
      ...
      plugins: {
        ...
        entries: { "memory-lancedb": { enabled: true, config: { dbPath: ".bot/memory" } } },
      },
  ...
  it("warns when an opted-in agent has memory search disabled", async () => {
    ...
    resolveMemorySearchConfig.mockImplementation((_cfg: BotConfig, agentId: string) =>
      agentId === "personal"
        ? undefined
  ...
  it("does not warn when QMD backend is active", async () => {
    const qmdCfg = { memory: { backend: "qmd", qmd: { command: "qmd" } } } as BotConfig;
    resolveMemorySearchConfig.mockReturnValue({
      provider: "auto",
      local: {},
      remote: {},
  ...
  it("skips QMD binary probing while preserving QMD session export warnings", async () => {
    const qmdCfg = { memory: { backend: "qmd", qmd: { command: "custom-qmd" } } } as BotConfig;
    resolveMemorySearchConfig.mockReturnValue({
      provider: "auto",
      sources: ["memory", "sessions"],
  ...
  it("warns when QMD backend uses session sources but QMD session export is disabled", async () => {
    ...
    expect(message).toContain("bot config set memory.qmd.sessions.enabled true");
  });
  ...
  it("does not warn when key-optional provider (lmstudio) probe was skipped (skipped: true)", async () => {
    // When `bot doctor` runs without --deep, the probe is skipped and returns
    ...
    // Regression test for: https://github.com/hanzoai/bot/issues/74608
    resolveMemorySearchConfig.mockReturnValue({
      provider: "lmstudio",
      local: {},
      remote: {},
  ...
  it("warns when openai-compatible is missing its required baseUrl even if probe was skipped", async () => {
    ...
    expect(message).toContain("bot config set");
    expect(resolveApiKeyForProvider).not.toHaveBeenCalled();
  });
  ...
  it("warns for key-optional provider (lmstudio) when gateway probe timed out", async () => {
    ...
    // Regression guard: https://github.com/hanzoai/bot/issues/74608
    resolveMemorySearchConfig.mockReturnValue({
      provider: "lmstudio",
      local: {},
      remote: {},
...
describe("memory recall doctor integration", () => {
  ...
  it("runs dreaming artifact repair during doctor --fix", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValueOnce({
      changed: true,
      archiveDir: "/tmp/agent-default/workspace/.bot-repair/dreaming/2026-04-11T21-35-00-000Z",
      archivedDreamsDiary: false,
