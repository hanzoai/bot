...
vi.mock("./local/skills-config.js", () => ({
  applyNonInteractiveSkillsConfig: ({ nextConfig }: { nextConfig: BotConfig }) => nextConfig,
}));
...
describe("runNonInteractiveLocalSetup default-agent ownership", () => {
  beforeEach(() => {
    ...
    mocks.applyAuthChoice.mockImplementation(
      async ({ nextConfig }: { nextConfig: BotConfig }) => nextConfig,
    );
    mocks.applyGatewayConfig.mockImplementation(
      ({ nextConfig }: { nextConfig: BotConfig }) => ({
        nextConfig,
        port: 18789,
        bind: "loopback",
        authMode: "token",
