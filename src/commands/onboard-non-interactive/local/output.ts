...
function recoveryHintForGatewayHealthFailure(
  ...
  switch (classification) {
    case "auth-mismatch":
      return "Fix: run `bot doctor --fix`.";
    case "module-missing":
      return "Fix: run `bot doctor --fix`.";
    case "service-missing":
      return "Fix: run `bot gateway install --force`.";
    case "service-stopped":
      return "Fix: run `bot gateway restart`.";
    case "startup-blocked":
      return "Fix: run `bot gateway status --deep`.";
    case "not-listening":
      return "Fix: start `bot gateway run`, or run `bot gateway restart` for a managed gateway.";
    default:
      return undefined;
  }
}
