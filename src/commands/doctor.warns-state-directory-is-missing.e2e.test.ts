...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    await withEnvAsync({ BOT_STATE_DIR: missingDir }, async () => {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      });
  ...
  it("does not let BOT_GATEWAY_TOKEN hide an unresolved SecretRef-managed token", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: {
          mode: "local",
    ...
    const previousFallbackToken = process.env.BOT_GATEWAY_TOKEN;
    const previousRefToken = process.env.BOT_MISSING_GATEWAY_REF_TOKEN;
    process.env.BOT_GATEWAY_TOKEN = "fallback-token-1234567890";
    delete process.env.BOT_MISSING_GATEWAY_REF_TOKEN;
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
      ...
      if (previousFallbackToken === undefined) {
        ...
        process.env.BOT_GATEWAY_TOKEN = previousFallbackToken;
      }
      if (previousRefToken === undefined) {
        delete process.env.BOT_MISSING_GATEWAY_REF_TOKEN;
      } else {
        process.env.BOT_MISSING_GATEWAY_REF_TOKEN = previousRefToken;
      }
    }
  ...
  it("skips token-mode exec token probes even when env password is set", async () => {
    ...
    const previousPassword = process.env.BOT_GATEWAY_PASSWORD;
    process.env.BOT_GATEWAY_PASSWORD = "fallback-password";
    try {
      callGateway.mockClear();
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
      ...
      if (previousPassword === undefined) {
        ...
        process.env.BOT_GATEWAY_PASSWORD = previousPassword;
      }
    }
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves", async () => {
    ...
    const previousToken = process.env.BOT_GATEWAY_TOKEN;
    process.env.BOT_GATEWAY_TOKEN = "resolved-token-1234567890";
    try {
      await doctorCommand(createDoctorRuntime(), {
        nonInteractive: true,
        workspaceSuggestions: false,
