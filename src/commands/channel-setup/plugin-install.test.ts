...
import { bundledPluginRoot, bundledPluginRootAt } from "bot/plugin-sdk/test-fixtures";
...
const discoverBotPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  discoverBotPlugins: (args: unknown) => discoverBotPlugins(args),
}));
...
const bundledChatNpmSpec = "@hanzo/bot-bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/bot", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/bot-test/${id}`,
    source: `/tmp/bot-test/${id}/index.ts`,
    manifestPath: `/tmp/bot-test/${id}/bot.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadBotPlugins).toHaveBeenCalledTimes(1);
  expect(requireMockCallArg(vi.mocked(loadBotPlugins), 0).onlyPluginIds).toStrictEqual([]);
}
...
beforeEach(() => {
  ...
  discoverBotPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
function expectLoadBotPluginFields(expected: Record<string, unknown>, callIndex = 0) {
  expectRecordFields(
    requireMockCallArg(vi.mocked(loadBotPlugins), callIndex),
    "loadBotPlugins args",
    expected,
  );
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: BotConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/bot-ledger-channel";
    process.env.BOT_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: BotConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      ...
      npmResolution: {
        name: "@hanzo/bot-wecom",
        version: "2026.5.4-beta.1",
        resolvedSpec: "@hanzo/bot-wecom@2026.5.4-beta.1",
      },
    });
    ...
    expectRecordFields(requireMockCallArg(installPluginFromNpmSpec, 0), "npm install args", {
      spec: "@hanzo/bot-wecom@beta",
      expectedPluginId: "wecom-bot-plugin",
    });
    expect(result.cfg.plugins?.installs?.["wecom-bot-plugin"]?.spec).toBe("@hanzo/bot-wecom");
  });
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    expectRecordFields(
      ...
      {
        value: "local",
        hint: bundledPluginRootAt("/opt/bot", "bundled-chat"),
      },
    );
  });
  ...
  it("offers ClawHub as the first-class install source for channel catalog entries", async () => {
    ...
    await ensureChannelSetupPluginInstalled({
      ...
      entry: {
        ...
        install: {
          clawhubSpec: "clawhub:bot/clawhub-chat@2026.5.2",
    ...
    expectRecordFields(options[0], "clawhub option", {
      value: "clawhub",
      label: "Download from ClawHub (clawhub:bot/clawhub-chat@2026.5.2)",
    });
    expectRecordFields(options[1], "skip option", {
      value: "skip",
    });
  ...
  it("skips the install prompt when autoConfirmSingleSource is set and only npm is available", async () => {
    ...
    const npmOnlyEntry: ChannelPluginCatalogEntry = {
      ...
      install: {
        npmSpec: "@hanzo/bot-wecom@2026.4.23",
      },
    };
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
    ...
    expect(select).not.toHaveBeenCalled();
    expect(result.installed).toBe(true);
    expect(result.pluginId).toBe("wecom-bot-plugin");
  });
  ...
  it("scopes snapshots by activation-declared channel ownership when direct channel lists are empty", () => {
    ...
    loadPluginManifestRegistry.mockImplementation((args: unknown) => {
      if (
        isRecord(args) &&
        args.config === cfg &&
        args.workspaceDir === "/tmp/bot-workspace" &&
