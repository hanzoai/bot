...
const hasUsableCustomProviderApiKey = vi.hoisted(() =>
  vi.fn<(_cfg?: BotConfig, _provider?: string, _env?: NodeJS.ProcessEnv) => boolean>(
    () => false,
  ),
);
...
const createProviderAuthChecker = vi.hoisted(() =>
  vi.fn((params: { cfg?: BotConfig; workspaceDir?: string; env?: NodeJS.ProcessEnv }) => {
    const checker = vi.fn(
...
describe("promptDefaultModel", () => {
  it("adds runtime-route hints for canonical OpenAI models", async () => {
    ...
    await promptDefaultModel({
      config: { agents: { defaults: {} } } as BotConfig,
      prompter,
      allowKeep: false,
      includeManual: false,
    ...
    expect(canonical.hint).not.toContain("Bot runtime route");
  });
  ...
  it.each([
    ...
    "labels official OpenAI with %s as an Bot runtime route",
    async (_label, defaults) => {
      ...
      expect(option.hint).toContain("Bot runtime route");
  ...
  ] as const)("labels an OpenAI %s as an Bot runtime route", async (_label, api, baseUrl) => {
    loadModelCatalog.mockResolvedValue([
  ...
  it("uses the configured default agent dir for provider-scoped catalog auth", async () => {
    ...
    const env = {
      ...process.env,
      BOT_STATE_DIR: "/tmp/bot-picker-state",
    };
    const config = {
      agents: {
    ...
    expect(loadPreferredProviderPickerCatalog).toHaveBeenCalledWith({
      cfg: config,
      preferredProvider: "nvidia",
      agentDir: "/tmp/bot-picker-state/agents/worker/agent",
      env,
    });
  });
