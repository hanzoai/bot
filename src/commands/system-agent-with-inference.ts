// Bot command gate: prove inference before starting conversational setup.
...
/**
 * Start Bot only after the configured default model completes a real
...
export async function runSystemAgentWithInference(
  ...
  if (opts.yes && !opts.message?.trim()) {
    failOneShotExecution(
      opts,
      runtime,
      new Error("Bot --yes requires --message so approval is limited to one request."),
    );
    return;
  }
  const oneShot = isOneShotRequest(opts);
  if (!oneShot && !hasInteractiveTty(opts)) {
    runtime.error("Bot needs an interactive TTY. Use --message for one command.");
    runtime.exit(1);
    return;
  }
  ...
  if (oneShot) {
    const guidance = "Run `bot onboard` to connect and live-test AI first.";
    if (opts.json) {
      writeRuntimeJson(runtime, {
        ok: false,
        status: inference.status,
        error: `Bot requires working inference: ${inference.error}`,
        guidance,
      });
    } else {
      runtime.error(
        [`Bot requires working inference: ${inference.error}`, guidance].join("\n"),
      );
    }
    if (!requestExitAfterOneShotOutput(runtime, 1)) {
      runtime.exit(1);
  ...
  runtime.log("Bot requires working inference. Starting guided AI setup…");
