...
type PluginRegistryHealthIssue =
  ...
  | {
      kind: "managed-npm-bot-peer-link";
      packageName: string;
      packageDir: string;
      reason: string;
    }
...
function readPluginManifestId(packageDir: string): string | undefined {
  const manifest = readJsonObject(path.join(packageDir, "bot.plugin.json"));
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("bot doctor --fix")} to remove stale managed npm packages and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("bot doctor --fix")} to remove stale local install records and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return [];
...
/** Relinks managed npm plugin packages to the current Bot host packages. */
export async function maybeRepairManagedNpmBotPeerLinks(
  params: PluginRegistryDoctorRepairParams,
): Promise<boolean> {
  ...
  if (!params.prompter.shouldRepair) {
    ...
    if (issues.length > 0) {
      note(
        [
          "Managed npm Bot host peer links need repair:",
          ...issues.map((issue) => `- ${issue.packageName}: ${issue.reason}`),
          `Repair with ${formatCliCommand("bot doctor --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (repaired > 0) {
    note(
      `Repaired Bot host peer link(s) for ${repaired} managed npm plugin package(s).`,
      "Plugin registry",
    );
  }
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm Bot host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
...
async function listManagedNpmBotPeerLinkIssues(
  params: PluginRegistryDoctorRepairParams,
): Promise<{
  peerLinkIssues: BotPeerLinkAuditIssue[];
...
export async function detectPluginRegistryHealthIssues(
  ...
  const managedNpmAudit = await listManagedNpmBotPeerLinkIssues(params);
  ...
  switch (issue.kind) {
    case "registry-missing-or-stale":
      return {
        ...
        path: issue.path,
        fixHint: "Run `bot doctor --fix` to rebuild the plugin registry from enabled plugins.",
      };
    case "stale-managed-npm-bundled-plugin":
      return {
        ...
        fixHint:
          "Run `bot doctor --fix` to remove stale managed npm packages and rebuild the plugin registry.",
      };
    case "stale-local-bundled-plugin-install-record":
      return {
        ...
        target: issue.pluginId,
        fixHint:
          "Run `bot doctor --fix` to remove stale local install records and rebuild the plugin registry.",
      };
    case "managed-npm-bot-peer-link":
      return {
        checkId: PLUGIN_REGISTRY_CHECK_ID,
        severity: "warning",
        message: `Managed npm package ${issue.packageName} has a broken Bot peer link: ${issue.reason}.`,
        path: issue.packageDir,
        target: issue.packageName,
        fixHint: "Run `bot doctor --fix` to relink managed npm plugin packages.",
      };
    case "managed-npm-package-unreadable":
      return {
        ...
        path: issue.packageDir,
        fixHint: "Restore access to the package files, then run `bot doctor` again.",
      };
...
export function pluginRegistryIssueToRepairEffect(
  ...
  switch (issue.kind) {
    ...
    case "managed-npm-bot-peer-link":
      return {
        kind: "package",
        action: "would-relink-managed-npm-bot-peer",
        target: issue.packageDir,
        dryRunSafe: false,
      };
    case "managed-npm-package-unreadable":
...
export async function maybeRepairPluginRegistryState(
  ...
  const repairedManagedNpmBotPeerLinks = await maybeRepairManagedNpmBotPeerLinks(params);
    ...
    if (preflight.action === "migrate") {
      note(
        [
          ...
          `Repair with ${formatCliCommand("bot doctor --fix")} to rebuild ${shortenHomePath(preflight.filePath)} from enabled plugins.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (
    ...
    repairedManagedNpmBotPeerLinks
  ) {
