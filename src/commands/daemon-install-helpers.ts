...
function configContainsSecretRef(config: BotConfig | undefined): boolean {
  if (!config) {
    return false;
  }
...
// they share the BOT_ prefix that is otherwise stripped from preserved
...
const PRESERVED_BOT_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "BOT_CLI_CONTAINER_BYPASS",
  "BOT_CONTAINER_HINT",
]);
...
function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("BOT_") && !PRESERVED_BOT_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[BOT_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${BOT_WRAPPER_ENV_KEY} because it points to the Windows task script; using the Bot gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveBotWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [BOT_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, BOT_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.BOT_PROFILE)
        : undefined,
    platform,
