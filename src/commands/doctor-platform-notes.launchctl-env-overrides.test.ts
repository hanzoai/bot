...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  ...
  it("collects clear unsetenv instructions for token override", async () => {
    ...
    expect(warning).toContain("BOT_GATEWAY_TOKEN");
    expect(warning).toContain("launchctl unsetenv BOT_GATEWAY_TOKEN");
    expect(warning).not.toContain("BOT_GATEWAY_PASSWORD");
  });
  ...
  it("bounds launchctl getenv calls and ignores timeout failures", async () => {
    ...
    expect(processMocks.runExec).toHaveBeenNthCalledWith(
      1,
      "/bin/launchctl",
      ["getenv", "BOT_GATEWAY_TOKEN"],
      { logOutput: false, timeoutMs: 5_000 },
    );
    expect(processMocks.runExec).toHaveBeenNthCalledWith(
      2,
      "/bin/launchctl",
      ["getenv", "BOT_GATEWAY_PASSWORD"],
      { logOutput: false, timeoutMs: 5_000 },
    );
    expect(noteFn).not.toHaveBeenCalled();
  });
...
describe("noteMacStaleBotUpdateLaunchdJobs", () => {
  it("uses service env for gateway platform stale updater warnings", async () => {
    ...
    await collectMacGatewayPlatformWarnings({} as BotConfig, {
      platform: "darwin",
      service,
      findJobs,
    });
  ...
  it("prints stale updater job cleanup guidance on macOS", async () => {
    ...
    const [message, title] = requireNoteCall(noteFn);
    expect(title).toBe("Gateway (macOS)");
    expect(message).toContain("Stale Bot updater launchd job(s) detected");
    expect(message).toContain("ai.bot.update.2026.5.12");
    expect(message).toContain("ai.bot.manual-update.1717168800");
    expect(message).toContain("launchctl remove <label>");
    expect(message).toContain("bot gateway restart");
  });
...
describe("collectMacGatewayPlatformWarnings", () => {
  it("collects guidance when launch agent writes are disabled", async () => {
    ...
    try {
      const warnings = await collectMacGatewayPlatformWarnings({} as BotConfig, {
        platform: "darwin",
  ...
  it("does nothing when launch agent writes are not disabled", async () => {
    ...
    try {
      await expect(
        collectMacGatewayPlatformWarnings({} as BotConfig, {
          platform: "darwin",
          service: { readCommand: vi.fn(async () => null) },
          findJobs: vi.fn(async () => []),
        }),
