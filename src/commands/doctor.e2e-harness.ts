...
const resolveBotPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
function createLegacyStateMigrationDetectionResult(params?: {
  ...
  return {
    ...
    rescuePending: {
      sourcePaths: ["/tmp/state/crestodian/rescue-pending", "/tmp/state/bot/rescue-pending"],
      hasLegacy: false,
    },
    channelPairing: {
      sourceDir: "/tmp/oauth",
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadBotPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("bot/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("bot/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/bot-root.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/bot-root.js")>();
  return {
    ...actual,
...
beforeEach(() => {
  ...
  resolveBotPackageRoot.mockReset().mockResolvedValue(null);
