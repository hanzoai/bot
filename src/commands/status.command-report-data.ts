// Builds the data model for the standard `bot status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("bot security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("bot status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("bot tasks maintenance --apply")}`,
