...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "bot-tui") {
    return true;
  }
  return executable === "bot" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local Bot TUI processes that can contend with gateway responsiveness. */
...
function hasWhatsappEnabled(cfg: BotConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("bot doctor --fix")}.`,
      );
    }
  }
