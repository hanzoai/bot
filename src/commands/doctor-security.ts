...
function collectExecPolicyConflictWarnings(cfg: BotConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Bot default (full)";
  const defaultRequestedAskSource = "Bot default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("bot approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: BotConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: BotConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: BotConfig): string[] {
  ...
  return [
    "- WARNING: bot.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("bot secrets configure")} or ${formatCliCommand("bot secrets apply")}, then verify with ${formatCliCommand("bot secrets audit --check")}.`,
  ];
}
...
export async function noteSecurityWarnings(cfg: BotConfig) {
  ...
  if (warnings.length > 0) {
    warnings.push(`- Run: ${formatCliCommand("bot security audit --deep")}`);
    note(warnings.join("\n"), "Security");
  }
}
