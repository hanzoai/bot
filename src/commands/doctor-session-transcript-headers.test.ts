...
describe("doctor SQLite session transcript header repair", () => {
  ...
  beforeEach(async () => {
    ...
    state = await createBotTestState({
      layout: "state-only",
      prefix: "bot-doctor-transcript-headers-",
    });
    cfg = {
  ...
  it("detects read-only, repairs atomically, and keeps event identities stable", async () => {
    ...
    runBotAgentWriteTransaction((transactionDatabase) => {
    ...
    expect(note).toHaveBeenCalledWith(
      '- Found 1 canonical session transcript without a header.\n- Run "bot doctor --fix" to repair it before resuming the session.',
  ...
  it("does not admit legacy headerless rows into the current runtime shape", async () => {
    ...
    const database = openBotAgentDatabase({ agentId: AGENT_ID, env: state.env });
