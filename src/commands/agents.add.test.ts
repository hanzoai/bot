...
describe("agents add command", () => {
  const suiteTempDirs = createSuiteTempRootTracker({ prefix: "bot-agents-add-" });
  ...
  beforeEach(() => {
    ...
    createAgentMock.mockImplementation(
      async (params: { name: string; workspace: string; bindingSpecs?: string[] }) => {
        ...
        if (agentId === "bot" || agentId === "crestodian") {
  ...
  async function withAgentsAddStateRoot(
    ...
    await withEnvAsync({ BOT_STATE_DIR: root }, async () => await run(root));
  }
  ...
  it.each(RESERVED_SYSTEM_AGENT_IDS_FOR_TEST)(
    ...
    async (name) => {
      ...
      expect(runtime.error).toHaveBeenCalledWith(
        `"${name}" is reserved. Choose another name, or run ${formatCliCommand("bot agents list")} to inspect configured agents.`,
      );
      expect(runtime.exit).toHaveBeenCalledWith(1);
      expect(writeConfigFileMock).not.toHaveBeenCalled();
    },
  ...
  it("uses the explicit agent target and skips catalog validation", async () => {
    ...
    wizardMocks.createClackPrompter.mockReturnValue({
      intro: vi.fn(),
      text: vi.fn().mockResolvedValueOnce("Jon").mockResolvedValueOnce("/tmp/bot-jon"),
  ...
  it("copies only portable auth profiles when seeding a new agent store", async () => {
    await withAgentsAddStateRoot("bot-agents-add-auth-copy-", async (root) => {
  ...
  it("copies portable Codex OAuth profiles inline", async () => {
    await withAgentsAddStateRoot("bot-agents-add-oauth-copy-", async (root) => {
      const sourceAgentDir = path.join(root, "main", "agent");
      const destAgentDir = path.join(root, "work", "agent");
  ...
  it("skips unresolved OAuth profiles when seeding a new agent store", async () => {
    await withAgentsAddStateRoot("bot-agents-add-oauth-ref-skip-", async (root) => {
      const sourceAgentDir = path.join(root, "main", "agent");
      const destAgentDir = path.join(root, "work", "agent");
      const profileId = "openai:oauth";
      const ref = {
        source: "bot-credentials" as const,
