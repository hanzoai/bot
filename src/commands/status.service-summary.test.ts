...
describe("readServiceStatusSummary", () => {
  it("marks Bot-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["bot", "gateway", "run"] })),
    ...
    expect(summary.managedByBot).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByBot).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["bot", "gateway", "run", "--port", "18789"],
    ...
    expect(loadedArgs?.env?.BOT_GATEWAY_PORT).toBe("18789");
    const runtimeEnv = requireMockArg(readRuntime, "readRuntime") as NodeJS.ProcessEnv;
    expect(runtimeEnv?.BOT_GATEWAY_PORT).toBe("18789");
    expect(summary.installed).toBe(true);
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "bot-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "bot-gateway.service");
      ...
      expect(layout.sourcePathReal).toBe(path.join(realRoot, "bot-gateway.service"));
