// Implements guided and non-interactive `bot channels add` account setup.
...
async function resolveCatalogChannelEntry(raw: string, cfg: BotConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as BotConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run bot doctor --fix, then retry bot channels add.`
  ...
  if (!plugin || !setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("bot channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
