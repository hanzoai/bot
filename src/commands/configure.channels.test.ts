...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirmArg().message).toBe(
    `Delete ${label} configuration from ~/.hanzoai/bot.json?`,
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from bot.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from bot.json", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram configuration from ~/.hanzoai/bot.json?",
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirmArg().message).toBe("Delete done configuration from ~/.hanzoai/bot.json?");
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirmArg().message).toBe(
      "Delete Telegram\\nBot configuration from ~/.hanzoai/bot.json?",
    );
    expect(note).toHaveBeenCalledWith(
