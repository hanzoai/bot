...
vi.mock("../daemon/program-args.js", () => ({
  BOT_WRAPPER_ENV_KEY: "BOT_WRAPPER",
  resolveGatewayProgramArguments: mocks.resolveGatewayProgramArguments,
  resolveBotWrapperPath: mocks.resolveBotWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  mocks.resolveBotWrapperPath.mockImplementation(async (value: string | undefined) =>
...
async function buildPluginConfigExecSecretRefPlan(home: string) {
  mockNodeGatewayPlanFixture({ serviceEnvironment: { BOT_PORT: "3000" } });
...
describe("buildGatewayInstallPlan", () => {
  ...
  // Prevent tests from reading the developer's real ~/.bot/.env when
  ...
  it("adds the active bot command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const botBinPath = path.join(isolatedHome, ".npm-global", "bin", "bot");
    process.argv = ["node", botBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(botBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".bot"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes BOT_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/bot-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        BOT_PORT: "3000",
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.BOT_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.BOT_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".bot", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        BOT_PORT: "3000",
    ...
    expect(plan.environment.BOT_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring BOT_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.BOT_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("renders config env SecretRefs as file-backed managed values on Linux", async () => {
    ...
    expect(plan.environment.BOT_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
  ...
  it("keeps config env SecretRefs managed when auth profiles reuse the key", async () => {
    ...
    expect(plan.environment.BOT_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
  ...
  it("includes passEnv values for configured exec SecretRef providers", async () => {
    ...
    expect(plan.environment.BOT_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/bot-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/bot-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BOT_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
    ...
    expect(plan.environment.BOT_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.BOT_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".bot"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        BOT_PORT: "3000",
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.BOT_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("retains config SecretRef env values for macOS LaunchAgent env files", async () => {
    ...
    expect(plan.environment.BOT_SERVICE_MANAGED_ENV_KEYS).toBe("TELEGRAM_DEFAULT_BOTTOKEN");
  });
  ...
  it("does not retain config env values for macOS LaunchAgent env files", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BOT_SERVICE_MANAGED_ENV_KEYS: "BRAVE_API_KEY",
      },
      existingEnvironmentValueSources: {
        BRAVE_API_KEY: "file",
        BOT_SERVICE_MANAGED_ENV_KEYS: "inline",
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.BOT_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        BOT_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.BOT_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps differently-cased source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.BOT_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.BOT_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale BOT_ALLOW_ROOT", async () => {
    const result = await buildEnvironment({ BOT_ALLOW_ROOT: "1" });
    expect(result.BOT_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves BOT_CLI_CONTAINER_BYPASS and BOT_CONTAINER_HINT", async () => {
    const result = await buildEnvironment({
      BOT_CLI_CONTAINER_BYPASS: "1",
      BOT_CONTAINER_HINT: "ci",
    });
    expect(result.BOT_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.BOT_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary BOT_FOO", async () => {
    const result = await buildEnvironment({ BOT_FOO: "bar" });
    expect(result.BOT_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated BOT_* keys", async () => {
    const result = await buildEnvironment({
      BOT_CLI_CONTAINER_BYPASS: "1",
      BOT_CONTAINER_HINT: "ci",
      BOT_BAZ: "qux",
    ...
    expect(result.BOT_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.BOT_CONTAINER_HINT).toBe("ci");
    expect(result.BOT_BAZ).toBeUndefined();
  });
});
