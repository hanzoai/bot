...
describe("resolveGatewayInstallToken", () => {
  ...
  it("returns unavailable reason when token and password are both configured and mode is unset", async () => {
    ...
    expect(result.unavailableReason).toContain("bot config set gateway.auth.mode token");
    expect(result.unavailableReason).toContain("bot config set gateway.auth.mode password");
  ...
  it("passes the install env through to gateway auth resolution", async () => {
    const env = {
      BOT_GATEWAY_PASSWORD: "dotenv-password", // pragma: allowlist secret
