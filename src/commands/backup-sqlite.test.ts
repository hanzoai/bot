...
import { BOT_AGENT_SCHEMA_VERSION } from "../state/bot-agent-db.js";
...
import { BOT_STATE_SCHEMA_VERSION } from "../state/bot-state-db.js";
...
function createGlobalDatabase(databasePath: string): void {
  ...
  try {
    database.exec(`
      ...
      PRAGMA user_version = ${BOT_STATE_SCHEMA_VERSION};
...
function createAgentDatabase(databasePath: string, agentId: string): void {
  ...
  try {
    database.exec(`
      ...
      PRAGMA user_version = ${BOT_AGENT_SCHEMA_VERSION};
      CREATE TABLE durable_entries (
        id INTEGER PRIMARY KEY,
        value TEXT NOT NULL
      );
    ...
    database
      ...
      .run(BOT_AGENT_SCHEMA_VERSION, agentId);
...
describe("SQLite backup commands", () => {
  it("creates, lists, verifies, and fresh-restores the global database", async () => {
    const tempDir = tempDirs.make("bot-backup-sqlite-");
    const stateDir = path.join(tempDir, "state");
    const repositoryPath = path.join(tempDir, "snapshots");
    const scratchPath = path.join(tempDir, "scratch");
    const restorePath = path.join(tempDir, "restore", "bot.sqlite");
  ...
  it("creates a snapshot for a normalized per-agent database", async () => {
    ...
    const repositoryPath = path.join(tempDir, "snapshots");
    process.env.BOT_STATE_DIR = stateDir;
    const databasePath = resolveBotAgentSqlitePath({ agentId: "ops-team" });
    await fs.mkdir(path.dirname(databasePath), { recursive: true });
  ...
  it("requires exactly one named Bot database source", async () => {
    const runtime = createRuntimeCapture();
