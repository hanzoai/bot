...
/**
 * Catalog #18 — `bot sessions --json` reports `agentRuntime.id: "bot"` for
 * ACP sessions because the old metadata resolver only consulted agent-config
 * policies (env / agent / defaults / implicit fallback to "bot"). The session
 * key clearly carries the ACP runtime indicator (the `:acp:` segment), but
 * `sessions.ts:294` used to ignore it.
 *
 * Empirical observation from a deployed bot container against a copilot
 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "bot"`, `source: "implicit"`.
...
/**
 * Build a minimal `BotConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "bot"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): BotConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="bot", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "bot". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "bot" and reflects the actual runtime driving the session.
  ...
  it("preserves locked Codex ownership ahead of stale Bot session metadata", () => {
