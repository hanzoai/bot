...
beforeEach(() => {
  ...
  loadBotPlugins.mockReset().mockReturnValue({ plugins: [], diagnostics: [] });
...
const resolveBotPackageRoot = vi.fn().mockResolvedValue(null);
...
const loadBotPlugins = vi.fn().mockReturnValue({ plugins: [], diagnostics: [] });
