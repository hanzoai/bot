...
async function maybeRemoveLegacyUsageCostCacheFiles(params: {
  ...
  if (!params.shouldRepair) {
    note(
      `${files.length} rebuildable usage-cost cache ${files.length === 1 ? "file remains" : "files remain"}. Run \`bot doctor --fix\` to remove ${files.length === 1 ? "it" : "them"}.`,
      "Usage cost cache",
    );
    return;
  }
...
async function maybeRemoveLegacySkillUploadTree(params: {
  ...
  if (!params.shouldRepair) {
    note(
      "Legacy skill-upload staging remains. Run `bot doctor --fix` to discard it; active uploads now live in SQLite and must be retried.",
      "Skill uploads",
    );
    return;
  }
...
export async function maybeRepairLegacyRuntimeFiles(
  ...
  if (shouldRepair) {
    for (const entry of listBotRegisteredAgentDatabases({ env })) {
