...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentDir: (configForTest: unknown, agentId: string) =>
    `${process.env.BOT_STATE_DIR ?? "/tmp/bot-state"}/agents/${agentId}/agent`,
  resolveAgentWorkspaceDir: (configForTest: unknown, agentId: string) =>
    `/tmp/bot-workspaces/${agentId}`,
...
vi.mock("../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: () => "/tmp/bot-workspace",
}));
...
function resolveTestAuthStoreKey(agentDir?: string): string {
  return agentDir?.trim() || process.env.BOT_AGENT_DIR || "__main__";
}
    ...
    auth: [
      {
        ...
        run: async (ctx) => {
          ...
          return {
            ...
            ...(params.applyConfig ? { configPatch: params.applyConfig as BotConfig } : {}),
  ...
  const createZaiMethod = (choiceId: "zai-api-key" | "zai-coding-global"): ProviderAuthMethod => ({
    ...
    run: async (ctx) => {
      ...
      return {
        ...
        configPatch: providerConfigPatch("zai", { baseUrl }) as BotConfig,
...
describe("applyAuthChoice", () => {
  ...
  async function readAuthProfiles() {
    return readTestAuthProfileStore(resolveAgentDir({} as BotConfig, "main"));
  }
  ...
  beforeAll(async () => {
    authTestRoot = (await setupAuthTestEnv("bot-auth-")).stateDir;
  ...
  it("fails fast when a removed provider auth choice is passed to the interactive flow", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "openai-chatgpt-import" is no longer supported. Use "openai" instead, or run bot onboard to choose interactively.',
      );
    } finally {
      spy.mockRestore();
    }
  ...
  it("guides external provider auth-choice replacements before the plugin is installed", async () => {
    ...
    try {
      await expect(
        ...
        'Auth choice "modelstudio-api-key" is no longer supported. Use "qwen-api-key" instead, or run bot onboard to choose interactively.',
      );
    } finally {
