...
import { findStaleBotUpdateLaunchdJobs } from "../daemon/launchd.js";
...
/** Returns a warning for stale Bot updater launchd jobs left after interrupted updates. */
async function collectMacStaleBotUpdateLaunchdJobsWarning(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleBotUpdateLaunchdJobs;
  ...
  const jobs = await (deps?.findJobs ?? findStaleBotUpdateLaunchdJobs)(scanEnv).catch(
    () => [],
  );
  if (jobs.length === 0) {
    return null;
  ...
  return [
    "- Stale Bot updater launchd job(s) detected.",
    ...jobs.map((job) => {
      const exitStatus =
    ...
    `  ${formatCliCommand("bot gateway restart")}`,
  ].join("\n");
}

/** Emits stale updater launchd job notes using the gateway service environment when available. */
export async function noteMacStaleBotUpdateLaunchdJobs(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleBotUpdateLaunchdJobs;
  env?: NodeJS.ProcessEnv;
  ...
  const warning = await collectMacStaleBotUpdateLaunchdJobsWarning({
    env: serviceEnv,
...
async function collectMacLaunchctlGatewayEnvOverrideWarning(
  ...
  return [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "BOT_GATEWAY_PASSWORD"}\` is set; explicit environment URL or node-host targets can use a different password than gateway.auth.password.`
      : undefined,
...
export async function collectMacGatewayPlatformWarnings(
  ...
  const serviceEnv =
    platform === "darwin" ? await resolveGatewayServiceEnvForPlatformNotes(deps) : deps?.env;
  const staleUpdateWarning = await collectMacStaleBotUpdateLaunchdJobsWarning({
    env: serviceEnv,
    findJobs: deps?.findJobs,
    platform,
  });
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.BOT_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- BOT_NO_RESPAWN is not set to 1; set it when you want routine gateway restarts to stay in-process instead of handing off to a managed supervisor.",
    );
  }
