...
type StatusScanSharedMocks = {
  ...
  resolveGatewayPort: Mock<(cfg?: BotConfig) => number>;
...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/bot-${configPathLabel}-missing-${process.pid}.json`),
    resolveGatewayPort: vi.fn((cfg?: BotConfig) => cfg?.gateway?.port ?? 18789),
...
export function createStatusScanConfig<T extends object = BotConfig>(
  overrides: T = {} as T,
): BotConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): BotConfig {
