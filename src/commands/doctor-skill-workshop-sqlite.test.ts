...
beforeEach(async () => {
  testState = await createBotTestState({
    layout: "state-only",
    prefix: "bot-doctor-workshop-sqlite-",
  });
});
...
describe("doctor Skill Workshop SQLite migration", () => {
  it("imports verified sidecars, preserves review artifacts, and removes legacy JSON", async () => {
    const oldWorkspace = await tempDirs.make("bot-workshop-old-workspace-");
    const currentWorkspace = await tempDirs.make("bot-workshop-current-workspace-");
    ...
    expect(openBotStateDatabase().db.prepare("PRAGMA user_version").get()).toEqual({
    ...
    const secondWorkspace = await tempDirs.make("bot-workshop-second-agent-");
