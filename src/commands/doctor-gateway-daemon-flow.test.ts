...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.bot.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.bot.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        BOT_STATE_DIR: "/tmp/bot-service",
        BOT_CONFIG_PATH: "/tmp/bot-service/bot.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { BOT_STATE_DIR?: string; BOT_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.BOT_STATE_DIR).toBe("/tmp/bot-service");
    expect(handoffEnv?.BOT_CONFIG_PATH).toBe("/tmp/bot-service/bot.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-bot/bin/bot logs --follow",
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "bot-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "bot-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "bot-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("bot gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system Bot gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        ...
        detail: "unit: /etc/systemd/system/bot-gateway.service",
        scope: "system",
        marker: "bot",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level Bot gateway service detected while the user gateway service is not installed.",
        "- bot-gateway.service (unit: /etc/systemd/system/bot-gateway.service)",
        "Bot will not install a second user-level gateway service automatically.",
        "Run `bot gateway status --deep` or `bot doctor --deep` to inspect duplicate services.",
