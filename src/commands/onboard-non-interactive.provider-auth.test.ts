...
function captureEnv(): EnvSnapshot {
  return {
    ...
    disableConfigCache: process.env.BOT_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("BOT_STATE_DIR", prev.stateDir);
  restoreEnvVar("BOT_CONFIG_PATH", prev.configPath);
  restoreEnvVar("BOT_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("BOT_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("BOT_SKIP_CRON", prev.skipCron);
  restoreEnvVar("BOT_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("BOT_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("BOT_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("BOT_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("bot-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("bot-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("bot-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("bot-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("bot-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
