...
import { resolveBotPackageRootsSync } from "../infra/bot-root.js";
...
function resolveSandboxScript(
  ...
  // Scan every bot package root the shared resolver finds (symlinked launcher via realpath,
  ...
  const roots = resolveBotPackageRootsSync({
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("bot.doctorSandboxTestApi")] = {
...
function codexBwrapNeedsNetworkNamespaceProbe(cfg: BotConfig): boolean {
  const network = cfg.agents?.defaults?.sandbox?.docker?.network?.trim().toLowerCase();
  return network === undefined || network === "" || network === "none";
}

async function probeCodexBwrapNamespaces(cfg: BotConfig): Promise<CodexBwrapNamespaceProbe> {
...
async function noteCodexBwrapNamespaceWarning(cfg: BotConfig): Promise<void> {
  ...
  const lines = [
    ...
    "",
    "Fix the host namespace policy for the Bot service user, then restart the gateway.",
    "Prefer an AppArmor profile that grants the required namespaces to the Bot service process.",
...
function resolveSandboxBackend(cfg: BotConfig): string {
...
${formatLegacyRegistryInspectionLine(file)}`,
    ...
    fixHint: `Run ${formatCliCommand("bot doctor --fix")} to migrate valid entries to SQLite.`,
  };
}
  ...
  if (!prompter.shouldRepair) {
    note(
      [
        ...
        `Run ${formatCliCommand("bot doctor --fix")} to migrate them to SQLite.`,
      ].join("\n"),
      "Sandbox",
    );
    return;
