...
const mocks = vi.hoisted(() => ({
  ...
  maybeRepairManagedNpmBotPeerLinks: vi.fn(),
...
vi.mock("../doctor-plugin-registry.js", () => ({
  maybeRepairManagedNpmBotPeerLinks: mocks.maybeRepairManagedNpmBotPeerLinks,
...
vi.mock("./shared/channel-doctor.js", () => ({
  ...
  collectChannelDoctorRepairMutations: ({ cfg }: { cfg: BotConfig }) => {
...
vi.mock("./shared/empty-allowlist-scan.js", () => ({
  scanEmptyAllowlistPolicyWarnings: (cfg: BotConfig) =>
    cfg.channels?.signal
...
vi.mock("./shared/allowlist-policy-repair.js", () => ({
  maybeRepairAllowlistPolicyAllowFrom: async (cfg: BotConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/bundled-plugin-load-paths.js", () => ({
  maybeRepairBundledPluginLoadPaths: (cfg: BotConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/invalid-plugin-config.js", () => ({
  maybeRepairInvalidPluginConfig: (cfg: BotConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/legacy-tools-by-sender.js", () => ({
  maybeRepairLegacyToolsBySenderKeys: (cfg: BotConfig) => {
...
vi.mock("./shared/exec-safe-bins.js", () => ({
  maybeRepairExecSafeBinProfiles: (cfg: BotConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("doctor repair sequencing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.applyPluginAutoEnable.mockImplementation((params: { config: BotConfig }) => ({
      config: params.config,
      changes: [],
    }));
    mocks.materializePluginAutoEnableCandidates.mockImplementation(
      (params: { config: BotConfig }) => ({
        config: params.config,
        changes: [],
      }),
    );
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementation((cfg: BotConfig) => ({
      config: cfg,
      changes: [],
    }));
    mocks.maybeRepairManagedNpmBotPeerLinks.mockResolvedValue(false);
    ...
    mocks.maybeRepairOpenAICodexAuthConfig.mockImplementation((cfg: BotConfig) => ({
      changes: [],
      config: cfg,
      warnings: [],
    }));
    ...
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementation((cfg: BotConfig) => ({
      config: cfg,
      changes: [],
    }));
    ...
    mocks.maybeRepairStaleConfiguredAuthOrders.mockImplementation(
      ({ cfg }: { cfg: BotConfig }) => ({ config: cfg, changes: [] }),
    );
    ...
    mocks.maybeRepairStalePluginConfig.mockImplementation((cfg: BotConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("runs the doctor-only onboarding recommendation scope migration", async () => {
    const env = { BOT_STATE_DIR: "/tmp/bot-doctor-test" };
    const candidate = {} as BotConfig;
    mocks.migrateLegacyOnboardingRecommendationsScope.mockReturnValue({
  ...
  it("repairs managed npm plugin drift before missing plugin install repair", async () => {
    ...
    mocks.maybeRepairManagedNpmBotPeerLinks.mockImplementation(async () => {
      events.push("bot-peer-links");
      return true;
    });
    ...
    expect(events).toEqual(["bundled-shadow-cleanup", "bot-peer-links", "missing-installs"]);
    ...
    expect(mocks.maybeRepairManagedNpmBotPeerLinks).toHaveBeenCalledOnce();
    const peerLinkCall = mocks.maybeRepairManagedNpmBotPeerLinks.mock.calls[0]?.[0];
  ...
  it("auto-enables newly installed configured plugins after doctor repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "brave" from @hanzo/bot-brave-plugin.'],
      warnings: [],
    });
    mocks.applyPluginAutoEnable.mockImplementationOnce((params: { config: BotConfig }) => ({
      config: {
        ...params.config,
        plugins: {
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "brave" from @hanzo/bot-brave-plugin.',
  ...
  it("applies doctor contracts exposed by newly installed plugins", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "discord" from @hanzo/bot-discord.'],
      warnings: [],
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "discord" from @hanzo/bot-discord.',
  ...
  it("explicitly enables plugins repaired from env-only configuration", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "exa" from @hanzo/bot-exa-plugin.'],
      warnings: [],
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "exa" from @hanzo/bot-exa-plugin.',
  ...
  it("surfaces ClawHub notices from successful missing configured plugin repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      ...
      warnings: [],
      notices: [
        'ClawHub trust warning for "@hanzo/bot-brave-plugin@1.2.3": scan=pending; reasons=pending.',
      ],
    });
    mocks.maybeRepairStalePluginConfig.mockImplementationOnce((cfg: BotConfig) => ({
      config: {
        ...cfg,
        plugins: {
          ...cfg.plugins,
          ...
          plugins: {
            ...
            entries: {
              brave: {
                enabled: true,
                source: "clawhub",
                package: "@hanzo/bot-brave-plugin",
              },
            },
          },
        } as BotConfig,
  ...
  it("runs group allowFrom fallback migration after open-policy allowFrom repair", async () => {
    const events: string[] = [];
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementationOnce((cfg: BotConfig) => {
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementationOnce((cfg: BotConfig) => {
  ...
  it("does not remove deferred configured plugins during the package update doctor pass", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [
        'Skipped package-manager repair for configured plugin "brave" during package update; rerun "bot doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
    const result = await runDoctorRepairSequence({
  ...
  it("preserves configured plugins when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "brave" from @hanzo/bot-brave-plugin: package install failed',
      ],
  ...
  it("preserves configured channels when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured channel plugin "whatsapp" from @hanzo/bot-whatsapp: package install failed',
      ],
