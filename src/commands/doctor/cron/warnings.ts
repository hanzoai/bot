...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("bot cron list")} and ${formatCliCommand("bot cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
function collectCronDeliveryTargetAdvisory(params: {
  ...
  return [
    ...
    `Reactivate the channel plugin or update the job's \`delivery.channel\` after reviewing with ${formatCliCommand("bot cron list")} and ${formatCliCommand("bot cron show <job-id>")}.`,
  ].join("\n");
}
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.bot/bin/ensure-whatsapp.sh` is not maintained by current Bot and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("bot channels status --probe")}, ${formatCliCommand("bot doctor")}, and ${formatCliCommand("bot gateway status")} for current health checks.`,
