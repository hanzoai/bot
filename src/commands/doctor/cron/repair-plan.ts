...
export function formatUnresolvedCommandPromptAdvisory(names: string[]): string | null {
  ...
  return [
    ...
    '- Recreate the job as a command cron job (`bot cron add ... --command "<shell>"`) or grant explicit shell/process tool access before relying on it.',
  ].join("\n");
}
...
export function formatUnresolvedShellPromptAdvisory(names: string[]): string | null {
  ...
  return [
    ...
    '- For a deterministic run, recreate the job as a command cron job (`bot cron add ... --command "<shell>"`).',
  ].join("\n");
}
...
export function formatScheduledToolPolicyAdvisory(params: {
  ...
  lines.push(
    ...
    "- Reauthorize with `bot cron edit <id> --tools <tool,...>`, or use `--clear-tools` to adopt the current default cap.",
  );
  return lines.join("\n");
}
