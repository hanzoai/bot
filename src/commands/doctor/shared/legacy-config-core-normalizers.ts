...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as BotConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as BotConfig["channels"],
  };
}
...
>;
type ModelsConfigPatch = Partial<NonNullable<BotConfig["models"]>>;
  ...
  return changed
    ? {
        config: {
          ...
          models: {
            ...rawModels,
            providers: nextProviders as NonNullable<BotConfig["models"]>["providers"],
          },
        },
        changed: true,
      }
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  const nextCfg = changed
    ? {
        ...cfgWithProviders,
        agents: nextAgents as BotConfig["agents"],
      }
    : cfgWithProviders;
  return nextCfg;
}
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<BotConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as BotConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyOllamaNativeNumCtxParams(
  ...
  type ProviderConfigMap = NonNullable<NonNullable<BotConfig["models"]>["providers"]>;
  for (const [providerId, rawProvider] of Object.entries(rawProviders)) {
    if (!isRecord(rawProvider)) {
      continue;
    }
