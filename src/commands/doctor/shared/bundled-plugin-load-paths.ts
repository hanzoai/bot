...
function resolveBundledWorkspaceDir(cfg: BotConfig): string | undefined {
  const defaultAgentId = tryResolveDefaultAgentId(cfg);
  return defaultAgentId ? resolveAgentWorkspaceDir(cfg, defaultAgentId) : undefined;
}

function isBotNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "bot" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by Bot. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed Bot package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isBotNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; Bot loads the packaged bundled plugin from "${hit.toPath}".`,
  );
