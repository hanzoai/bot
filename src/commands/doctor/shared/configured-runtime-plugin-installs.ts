...
type ConfiguredRuntimePluginInstallCandidate = {
  ...
  /** Keep this official runtime package on the same release cohort as Bot. */
  versionBoundToBot?: boolean;
};
...
export const VERSION_BOUND_RUNTIME_PLUGIN_IDS: ReadonlySet<string> = new Set(
  CONFIGURED_RUNTIME_PLUGIN_INSTALL_CANDIDATES.filter(
    (candidate) => candidate.versionBoundToBot,
...
function acpxRuntimeIsConfigured(cfg: BotConfig): boolean {
