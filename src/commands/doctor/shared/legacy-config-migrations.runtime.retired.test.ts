...
describe("retired runtime config migrations", () => {
  it("strips the retired compaction gate while keeping an enabled byte threshold", () => {
    ...
    expect(retiredRule?.message).toBe(
      'agents.defaults.compaction.truncateAfterCompaction is retired; byte-triggered compaction now opts in via maxActiveTranscriptBytes alone. Run "bot doctor --fix".',
    );
  });
