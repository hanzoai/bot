...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "bot/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/bot";
  ...
  it("removes stale bundled paths from old versioned Bot package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "bot");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
      "bot@2026.3.28_@napi-rs+canvas@0.1.97",
      "node_modules",
      "bot",
    );
  ...
  it("removes stale legacy bundled paths from old versioned Bot package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "bot");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; Bot loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "bot doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "bot", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
