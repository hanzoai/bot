...
describe("legacy migrate sandbox scope aliases", () => {
  ...
  it("disables the default sandbox browser network without granting inherited egress", () => {
    ...
    expect(res.changes).toStrictEqual([
      ...
      'Disabled agents.defaults.sandbox.browser and moved its unsupported network "none" → "bot-sandbox-browser".',
    ]);
  ...
  it("disables explicit per-agent network none in keyed rosters", () => {
    ...
    expect(res.changes).toStrictEqual([
      'Disabled agents.entries.main.sandbox.browser and moved its unsupported network "none" → "bot-sandbox-browser".',
    ]);
  ...
  it("disables explicit per-agent network none in legacy list rosters", () => {
    ...
    expect(res.changes).toStrictEqual([
      'Disabled agents.list.0.sandbox.browser and moved its unsupported network "none" → "bot-sandbox-browser".',
    ]);
