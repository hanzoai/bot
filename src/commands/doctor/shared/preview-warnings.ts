...
import { isRecord as hasRecord } from "@hanzo/bot-normalization-core/record-coerce";
...
function listAgentRecords(cfg: BotConfig): Record<string, unknown>[] {
  return listAgentEntries(cfg).filter(hasRecord);
}

function hasChannels(cfg: BotConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: BotConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: BotConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: BotConfig): boolean {
...
function hasConfiguredSafeBins(cfg: BotConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: BotConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: BotConfig): {
...
function collectVisibleReplyToolPolicyWarnings(cfg: BotConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Bot falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Bot falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
function collectChannelBoundMessageToolPolicyWarnings(cfg: BotConfig): string[] {
...
function collectProfileConfiguredToolSectionWarnings(cfg: BotConfig): string[] {
  const warnings: string[] = [];
...
export async function resolveDoctorChannelPreviewConfig(params: {
  ...
  allowExec?: boolean;
}): Promise<{ cfg: BotConfig; diagnostics: string[] }> {
