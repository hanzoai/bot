...
function isRuntimeDependencyMarkerName(name: string): boolean {
  return (
    name === ".bot-runtime-deps.json" ||
    name === ".bot-runtime-deps-stamp.json" ||
    name.startsWith(".bot-runtime-deps-")
  );
}

function isInstallStageDebrisName(name: string): boolean {
  return /^\.bot-install-stage(?:-.+)?$/u.test(name);
}
...
function collectExplicitStageTargets(env: NodeJS.ProcessEnv): CleanupTarget[] {
  return splitPathList(env.BOT_PLUGIN_STAGE_DIR).map((entry) => ({
    kind: "explicit-stage",
...
async function hasBotRenameResidue(root: string): Promise<boolean> {
  const nodeModulesRoot = path.join(root, "node_modules");
  if (await isFile(path.join(nodeModulesRoot, ".bot-rename-tmp"))) {
    return true;
  }
  ...
  for (const entry of entries) {
    ...
    if (await isFile(path.join(entryPath, ".bot-rename-tmp"))) {
      return true;
    }
    ...
    for (const scopedEntry of scopedEntries) {
      ...
      if (await isFile(path.join(entryPath, scopedEntry.name, ".bot-rename-tmp"))) {
        return true;
      }
    }
  }
...
async function hasExplicitStageDebrisProof(root: string): Promise<boolean> {
  ...
  return await hasBotRenameResidue(root);
}
...
function filterLegacyStaleRootCandidates(
  ...
  for (const target of targets) {
    ...
    if (!cleanupRootPaths.some((rootPath) => isPathInsideRoot(targetPath, rootPath))) {
      warnings.push(
        `Skipped legacy plugin dependency state ${targetPath}: outside Bot cleanup roots`,
      );
      continue;
    }
    safeTargets.push({ ...target, path: targetPath });
...
async function resolveSafeRemovalTarget(
  ...
  if (!cleanupRoots.some((root) => isPathInsideRoot(realPath, root.realPath))) {
    return {
      warning: `Skipped legacy plugin dependency state ${targetPath}: resolved outside Bot cleanup roots`,
    };
  }
  return { target: targetPath };
}
...
/** Remove legacy plugin dependency state under trusted Bot cleanup roots. */
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("bot.pluginDependencyCleanupTestApi")
