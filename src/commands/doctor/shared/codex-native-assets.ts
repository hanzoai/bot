...
function isCodexRuntimeConfigured(cfg: BotConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: BotConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: BotConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      ...
      "- To review or promote them: install the Codex plugin (bot plugins install npm:@hanzo/bot-codex), then run bot migrate plan codex.",
    ].join("\n"),
  ];
}

if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("bot.codexNativeAssetsTestApi")] = {
