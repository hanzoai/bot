...
describe("buildProviderStatusIndex", () => {
  ...
  it("prefers inspectAccount for read-only status surfaces", async () => {
    ...
    const map = await buildProviderStatusIndex({} as BotConfig);
  ...
  it("records accounts that throw during read-only resolution as not configured", async () => {
    ...
    await expect(buildProviderStatusIndex({} as BotConfig)).resolves.toEqual(
      new Map([
        [
          "quietchat:default",
          {
  ...
  it("does not inspect linkage for an unconfigured account", async () => {
    ...
    const status = (await buildProviderStatusIndex({} as BotConfig)).get("quietchat:default");
  ...
  it("uses the shipped custom state resolver when canonical linkage is unknown", async () => {
    ...
    const status = (await buildProviderStatusIndex({} as BotConfig)).get("legacychat:default");
  ...
  it("rethrows unexpected read-only account resolution errors", async () => {
    ...
    await expect(buildProviderStatusIndex({} as BotConfig)).rejects.toThrow("plugin crash");
  });
  ...
  it("uses repair hints instead of unknown for bound missing external channels", () => {
    ...
    expect(lines).toEqual([
      "Feishu default: missing plugin - Install the official external plugin with: bot plugins install @hanzo/bot-feishu, or run: bot doctor --fix.",
    ]);
  });
