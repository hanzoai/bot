...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "Bot ignores provider credentials and protected runtime controls from workspace `.env`",
    );
    expect(markdown).toContain("~/.bot/.env");
    expect(markdown).toContain("$BOT_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("BOT_LOAD_SHELL_ENV=1");
