...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "bot-plugin-hooks-"));
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".bot", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): BotConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("bot-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Bot hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".bot", "extensions", "claude-bundle");
