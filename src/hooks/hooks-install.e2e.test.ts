...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevBundledDir = process.env.BOT_BUNDLED_HOOKS_DIR;
    process.env.BOT_STATE_DIR = path.join(baseDir, "state");
    process.env.BOT_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      ...
      process.env.BOT_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
