...
describe("hooks workspace", () => {
  ...
  it("warns, skips oversized metadata, and continues discovering other hooks", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "bot-hooks-oversized-mixed-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("loads hooks whose metadata sits exactly at the byte limit", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "bot-hooks-exact-limit-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("still loads a plain hook when its package.json is oversized", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "bot-hooks-oversized-compat-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("does not let workspace hooks override managed hooks with the same name", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "bot-hooks-collision-"));
    ...
    expect(entries[0]?.hook.source).toBe("bot-managed");
  });

  it("treats configured extraDirs as managed hook sources", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "bot-hooks-extra-"));
    const workspaceDir = path.join(root, "workspace");
    ...
    fs.writeFileSync(
      ...
      ["---", "name: shared-hook", 'metadata: {"bot":{"events":["command:new"]}}', "---"].join(
        "\n",
      ),
    );
