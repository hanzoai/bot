...
describe("system agent setup-flow operations", () => {
  ...
  it("prints one-shot setup pointers", async () => {
    ...
    expect(output).toContain("bot configure --section web");
    expect(output).toContain("bot configure --section gateway");
  });
  ...
  it("prints one-shot pointers for hosted skills, search, and Gateway setup", async () => {
    ...
    expect(lines.join("\n")).toContain("bot configure --section skills");
    expect(lines.join("\n")).toContain("bot configure --section web");
    expect(lines.join("\n")).toContain("bot configure --section gateway");
  });
