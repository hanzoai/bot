// Bot dialogue parses direct commands and optionally asks the assistant planner.
...
/**
 * Dialogue helpers for turning user text into Bot operations.
 *
...
/** Resolve user input to an Bot operation, optionally using the assistant planner. */
...
function logAssistantPlan(
  ...
  runtime.log(`[bot] planner: ${modelLabel}`);
  if (plan.reply) {
    runtime.log(plan.reply);
  }
  runtime.log(`[bot] interpreted: ${plan.command}`);
}
