...
export const SYSTEM_AGENT_CONFIG_WRITE_DENYLIST: Readonly<Record<string, string>> = {
  $include: "alternate-config inclusion; edit bot.json in a trusted shell",
  auth: "provider auth; exit Bot and run `bot onboard`",
  env: "environment/credential injection; edit bot.json in a trusted shell",
  models:
    "provider/catalog definitions feed routing; use `set_default_model` or `bot onboard`",
  secrets: "secret providers; edit bot.json in a trusted shell",
};
