// Bot TUI backend runs setup-helper dialogue inside the shared local TUI shell.
...
class SystemAgentTuiBackend implements TuiBackend {
  readonly connection = { url: "bot local" };
  ...
  stop(): void {
    // The enclosing TUI owns terminal shutdown; Bot has no transport to close.
  }
  ...
  async listAgents(): Promise<TuiAgentsList> {
    return {
      ...
      mainKey: "main",
      scope: "per-sender",
      agents: [{ id: SYSTEM_AGENT_ID, kind: "system", name: "Bot" }],
    };
  }
  ...
  async patchSession(opts: SessionsPatchParams): Promise<SessionsPatchResult> {
    if (opts.model !== undefined) {
      throw new Error(
        "Bot cannot change the model inside its active verified session. Exit and run `bot onboard`, then start Bot again.",
      );
    }
    return {
      ok: true,
  ...
  async createSession(_opts: TuiSessionCreateOptions) {
    ...
    return {
      ok: true as const,
      key: SYSTEM_AGENT_SESSION_KEY,
      entry: { sessionId: "bot", updatedAt: Date.now() },
    };
  }
  ...
  private emitFinal(runId: string, sessionKey: string, text: string): void {
    const assistant = message(
      "assistant",
      text || "Bot listened and found nothing to change.",
    );
  ...
  private async respond(runId: string, sessionKey: string, text: string): Promise<void> {
    ...
    try {
      ...
      if ((reply.action === "open-tui" || reply.action === "open-setup") && reply.handoff) {
        // The outer loop owns interactive handoffs after the Bot TUI exits.
...
async function runSetupHandoff(
  ...
  if (
    ...
    runtime.error(
      "Setup cannot replace the inference route powering Bot. Exit and run `bot onboard`, then start Bot again.",
    );
    return;
  }
  ...
  if (handoff.target === "gateway") {
    if (opts.runGatewaySetupHandoff) {
      ...
      runtime.log("Done — gateway settings saved. Run `bot gateway restart` to apply them.");
      return;
    }
    const [
      { resolveGatewayPort },
    ...
    if (!snapshot.exists || !snapshot.valid || !snapshot.hash) {
      throw new Error(
        "Gateway setup requires a valid saved config snapshot. Run `bot doctor --fix`, then retry.",
      );
    }
  ...
  if (handoff.target === "search") {
    ...
    if (!snapshot.exists || !snapshot.valid || !snapshot.hash) {
      throw new Error(
        "Web search setup requires a valid saved config snapshot. Run `bot doctor --fix`, then retry.",
      );
    }
    const baseConfig = snapshot.sourceConfig ?? snapshot.config;
...
export async function runSystemAgentTui(
  ...
  for (;;) {
    ...
    // replay it when Bot re-enters the chat shell.
    ...
    if (handoff.kind === "model-setup") {
      runtime.error(
        "Bot cannot replace its active inference route. Run `bot onboard` outside this session, then start Bot again.",
      );
      return;
    }
