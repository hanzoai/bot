// Classifies whether a user's chat message approves a pending Bot proposal.
...
export type SystemAgentApprovalClassifier = (params: {
  ...
  proposal?: string;
  /** Exact execution owner that completed the live Bot inference gate. */
...
export async function classifySystemAgentApprovalIntent(
  ...
  try {
    ...
    if (!route || route.runner !== "embedded" || route.agentHarnessRuntimeOverride !== "bot") {
      return "other";
    }
