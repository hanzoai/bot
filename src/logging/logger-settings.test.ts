...
beforeEach(() => {
  envSnapshot = captureEnv(["BOT_TEST_FILE_LOG", "BOT_LOG_LEVEL"]);
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    expect(settings.file).toBe("/tmp/bot-configured.log");
  ...
  it("uses defaults when no logging config is available", () => {
    ...
    expect(path.basename(settings.file)).toMatch(/^bot-vitest-\d+-\d{4}-\d{2}-\d{2}\.log$/);
    expect(settings.file).not.toBe(
      `/tmp/hanzoai/bot-${new Date().toISOString().slice(0, 10)}.log`,
    );
  });
});
