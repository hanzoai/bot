...
vi.mock("../terminal/links.js", () => ({
  formatDocsLink: () => "docs.bot.ai/cli/skills",
}));
...
describe("skills workshop cli", () => {
  ...
  beforeEach(async () => {
    testState = await createBotTestState({
      layout: "state-only",
      prefix: "bot-skills-cli-workshop-state-",
    });
    mocks.workspaceDir = await tempDirs.make("bot-skills-cli-workshop-");
  ...
  it("lists and inspects an agent proposal after its workspace changes", async () => {
    ...
    mocks.workspaceDir = await tempDirs.make("bot-skills-cli-workshop-second-");
    await runCommand(["skills", "workshop", "list"]);
    expect(mocks.runtimeStdout.at(-1)).toContain(`${proposalId}  pending  create`);
