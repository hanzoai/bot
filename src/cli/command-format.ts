...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+bot\b|^bot\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE = /^(?:(?:pnpm|npm|bunx|npx)\s+bot|bot)\b.*\supdate(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const rawContainer = env.BOT_CONTAINER_HINT?.trim();
