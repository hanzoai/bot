        ...
        await runNodesCommand("remove", async () => {
          ...
          if (!nodeId) {
            defaultRuntime.error(
              `--node is required. Run ${formatCliCommand("bot nodes pending")} to choose a node request.`,
            );
    ...
    nodes
      ...
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("rename", async () => {
          ...
          if (!nodeId || !name) {
            defaultRuntime.error(
              `--node and --name are required. Run ${formatCliCommand("bot nodes pending")} to choose a node, then rerun with --name <displayName>.`,
            );
            defaultRuntime.exit(1);
            return;
          }
