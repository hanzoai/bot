...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): BotConfig | null {
...
function coerceStatusConfig(value: unknown): BotConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleBotUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
   ...
   * can happen after `npm install -g bot@<v>` updates the gateway binary
   * without a corresponding `bot plugins update`.
   */
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleBotUpdateLaunchdJobs }) =>
            findStaleBotUpdateLaunchdJobs(serviceEnv),
          )
          .catch(() => [])
      : [];
