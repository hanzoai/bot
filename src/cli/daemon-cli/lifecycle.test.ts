...
vi.mock("../terminal-interactivity.js", () => ({
  ...
  NON_INTERACTIVE_GATEWAY_STOP_MESSAGE:
    "This stops the operator's running gateway service. Use an isolated dev gateway (bot gateway run --dev, or --profile <name> with a free port) for testing, or re-run with --force if you really mean it.",
}));
...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.BOT_CONTAINER_HINT;
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["bot", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("uses the installed service environment for managed restart health", async () => {
    process.env.BOT_STATE_DIR = "/tmp/bot-caller-state";
    process.env.BOT_SYSTEMD_UNIT = "bot-gateway-maintenance.service";
    service.readCommand.mockResolvedValue({
      programArguments: ["bot", "gateway", "--port", "18789"],
      environment: {
        BOT_STATE_DIR: "/tmp/bot-service-state",
    ...
    expect(waitParams.env?.BOT_STATE_DIR).toBe("/tmp/bot-service-state");
    expect(waitParams.env?.BOT_SYSTEMD_UNIT).toBe("bot-gateway-maintenance.service");
  });
  ...
  it("re-reads the installed service environment after restart repair", async () => {
    service.readCommand
      .mockResolvedValueOnce({
        programArguments: ["bot", "gateway", "--port", "18789"],
        environment: { BOT_STATE_DIR: "/tmp/bot-stale-state" },
      })
      .mockResolvedValue({
        programArguments: ["bot", "gateway", "--port", "19001"],
        environment: { BOT_STATE_DIR: "/tmp/bot-repaired-state" },
      });
    ...
    expect(waitForGatewayHealthyRestart).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          BOT_STATE_DIR: "/tmp/bot-repaired-state",
        }),
      }),
    );
  });
    ...
    runServiceStart.mockImplementation(
      async (params: {
        ...
        await params.repairLoadedService?.({
          json: true,
          stdout: process.stdout,
          state: { command: { environment: { BOT_SERVICE_VERSION: "2026.4.24" } } },
          issues: [{ code: "version-mismatch", message: "old service" }],
        });
      },
    );
  ...
  it("blocks non-interactive stop without force before managed service access", async () => {
    ...
    expect(writeJson).toHaveBeenCalledWith(
      expect.objectContaining({
        ok: false,
        error: expect.stringContaining("bot gateway run --dev"),
      }),
    );
  ...
  it("does not fall back to unmanaged restart when launchd repair reports headless GUI bootstrap failure", async () => {
    ...
    recoverInstalledLaunchAgent.mockRejectedValue(
      new Error("LaunchAgent bot gateway restart requires a logged-in macOS GUI session"),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
  ...
  it("delegates system-scope restart to systemctl without unmanaged signaling when root (bot#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
  ...
  it("surfaces systemd sudo guidance and never signals when restarting a system-scope unit as non-root (bot#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "bot.service",
    ...
    restartSystemdService.mockRejectedValue(
      new Error(
        "bot.service is a system-scope unit (/etc/systemd/system/bot.service); run `sudo systemctl restart bot.service` to restart it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    mockUnmanagedRestart();
  ...
  it("delegates system-scope stop to systemctl without unmanaged signaling when root (bot#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "bot-gateway.service",
  ...
  it("surfaces systemd sudo guidance and never signals when stopping a system-scope unit as non-root (bot#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "bot-gateway.service",
    ...
    stopSystemdService.mockRejectedValue(
      new Error(
        "bot-gateway.service is a system-scope unit (/etc/systemd/system/bot-gateway.service); run `sudo systemctl stop bot-gateway.service` to stop it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
