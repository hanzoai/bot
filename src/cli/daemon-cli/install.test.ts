...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: {
    ...
    if (params?.wrapperPath || params?.env?.BOT_WRAPPER) {
      environment.BOT_WRAPPER = params.wrapperPath ?? params.env?.BOT_WRAPPER;
    }
    return {
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/bot.json" },
  })),
}));
...
vi.mock("../../daemon/program-args.js", () => ({
  ...
  resolveBotWrapperPath: async (value: string | undefined) => value?.trim() || undefined,
}));
...
describe("runDaemonInstall", () => {
  ...
  it("blocks managed install when explicit no-auth would bind to LAN", async () => {
    ...
    expect(actionState.failed[0]?.message).toContain("bot config set gateway.auth.mode token");
    expect(buildGatewayInstallPlanMock).not.toHaveBeenCalled();
    expect(installDaemonServiceAndEmitMock).not.toHaveBeenCalled();
  });
  ...
  it("reinstalls when the loaded service still embeds BOT_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["bot", "gateway", "run"],
      environment: {
        BOT_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service BOT_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["bot", "gateway", "run"],
      environment: {
        BOT_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
    buildGatewayInstallPlanMock.mockResolvedValueOnce({
      programArguments: ["bot", "gateway", "run"],
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/bot-doppler", "gateway", "run"],
      environment: {
    ...
    expectFields(installPlanArg, { wrapperPath: "/usr/local/bin/bot-doppler" });
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/bot-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service BOT_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when BOT_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["bot", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["bot", "gateway", "run"],
      environment: {
        BOT_STATE_DIR: "/tmp/bot-doctor-manual",
        BOT_CONFIG_PATH: "/tmp/bot-doctor-manual/bot.json",
        BOT_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.BOT_STATE_DIR).toBeUndefined();
      expect(env.BOT_CONFIG_PATH).toBeUndefined();
