...
export async function repairLoadedGatewayServiceForStart(
  ...
  const wrapperPath = await resolveBotWrapperPath(installEnv[BOT_WRAPPER_ENV_KEY]);
  const installedPort =
    parseTcpPortFromArgs(params.state.command?.programArguments) ??
    parseTcpPort(params.state.command?.environment?.BOT_GATEWAY_PORT);
  ...
  return {
    ...
    message:
      params.action === "restart"
        ...
        : "Gateway service definition repaired and started. Reopen the Control UI with `bot dashboard` or copy a fresh auth URL with `bot dashboard --no-open`.",
