...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expectMockLineContains(runtime.error, formatCliCommand("bot gateway restart"));
  });
  ...
  it("prints established gateway client guidance gathered by deep status", () => {
    ...
    expectMockLineContains(runtime.log, "newer-bot");
  ...
  it("prints stale updater launchd job guidance", () => {
    ...
    expectMockLineContains(runtime.error, "Stale Bot updater launchd job(s) detected.");
    expectMockLineContains(runtime.error, "ai.bot.update.2026.5.12");
    expectMockLineContains(runtime.error, "ai.bot.manual-update.1717168800");
  ...
  it("prints macOS launchd stdout and suppressed stderr when gateway is not listening", () => {
    ...
    expectMockLineContains(runtime.error, "/Users/test/Library/Logs/bot/gateway.log");
  ...
  it("prints CLI and gateway versions with readable guidance when they differ", () => {
    ...
    expectMockLineContains(runtime.log, "CLI version: 2026.4.23 (/usr/local/bin/bot)");
    expectMockLineContains(runtime.log, "Gateway version: 2026.5.6");
    expectMockLineContains(runtime.error, "this Bot command is version 2026.4.23");
    expectMockLineContains(
      runtime.error,
      "if this mismatch is unexpected, update PATH so `bot` points to the version you want",
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/bot-cli/bot.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/bot-daemon/bot.json",
            exists: true,
            valid: true,
    ...
    printDaemonStatus(
      {
        ...
        extraServices: [{ label: "ai.bot.gateway.rescue", scope: "user", detail: "loaded" }],
      },
      { json: false },
    );

    expectMockLineContains(runtime.log, "Other gateway-like services detected");
    expectMockLineContains(runtime.log, "ai.bot.gateway.rescue");
  ...
  it("prints a terse plugin drift warning outside deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "bot gateway status --deep");
  ...
  it("prints detailed plugin drift entries in deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "bot plugins update whatsapp");
    expectMockLineContains(runtime.log, "bot gateway restart");
  });
  ...
  it("prints exact package update commands for pinned npm plugin drift in deep mode", () => {
    ...
    expectMockLineContains(
      runtime.log,
      "bot plugins update @hanzo/bot-brave-plugin@2026.6.10-beta.1",
    );
    expectMockLineContains(runtime.log, "bot gateway restart");
  });
