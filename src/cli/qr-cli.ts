...
function readDevicePairPublicUrlFromConfig(cfg: BotConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.BOT_GATEWAY_PASSWORD)) {
    return false;
  }
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: BotConfig): Promise<void> {
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the Bot mobile app (Onboarding -> Scan QR).",
          "",
        ];
