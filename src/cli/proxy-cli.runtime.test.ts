...
describe("proxy cli runtime", () => {
  const envKeys = [
    "BOT_STATE_DIR",
    "BOT_DEBUG_PROXY_CERT_DIR",
    "BOT_DEBUG_PROXY_SESSION_ID",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "bot-proxy-cli-runtime-"));
    process.env.BOT_STATE_DIR = tempDir;
    process.env.BOT_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.BOT_DEBUG_PROXY_ENABLED;
    delete process.env.BOT_DEBUG_PROXY_SESSION_ID;
  ...
  afterEach(async () => {
    ...
    const { closeBotStateDatabaseForTest } = await import("../state/bot-state-db.js");
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, BOT_PROXY_URL, or --proxy-url so it uses a reachable http:// or https:// proxy.\n",
    );
  });
  ...
  it("prints actionable output when proxy config is disabled and missing", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        ...
        source: "disabled",
        errors: [
          "proxy validation requires proxy.enabled=true with proxy.proxyUrl or BOT_PROXY_URL, or --proxy-url",
        ],
      },
      checks: [],
    });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or BOT_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
      ...
      `${JSON.stringify(
        {
          ...
          config: {
            ...
            source: "missing",
            errors: [
              "proxy validation requires proxy.proxyUrl, --proxy-url, or BOT_PROXY_URL",
            ],
          },
          checks: [],
        },
