...
describe("program routes", () => {
  ...
  it("passes parsed agents list flags through", async () => {
    await expect(expectRoute(["agents"]).run(["node", "bot", "agents"])).resolves.toBe(true);
  ...
  it("passes parsed channel read-only route flags through", async () => {
    ...
    await expect(listRoute.run(["node", "bot", "channels", "list", "--json"])).resolves.toBe(
      true,
    );
  ...
  it("routes plugins list JSON without importing the full plugins CLI", async () => {
    ...
    expect(route.loadPlugins).toBeUndefined();
    expect(route.canRun?.(["node", "bot", "plugins", "list"])).toBe(false);

    await expect(
      route.run(["node", "bot", "plugins", "list", "--json", "--enabled", "--verbose"]),
    ).resolves.toBe(true);
  ...
  it("returns false for plugins list JSON route with unsupported arguments", async () => {
    await expectRunFalse(
      ["plugins", "list"],
      ["node", "bot", "plugins", "list", "--json", "--wat"],
    );
  });
  ...
  it("routes machine-readable gateway health without plugin preload", async () => {
    ...
    expect(route.loadPlugins).toBeUndefined();
    await expect(
      route.run(["node", "bot", "gateway", "health", "--json", "--timeout", "5000"]),
    ).resolves.toBe(true);
  ...
  it("falls back for text gateway health output", async () => {
    await expectRunFalse(["gateway", "health"], ["node", "bot", "gateway", "health"]);
  });

  it("returns false for gateway status route when option values are missing", async () => {
    await expectRunFalse(["gateway", "status"], ["node", "bot", "gateway", "status", "--url"]);
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "bot", "gateway", "status", "--token"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "bot", "gateway", "status", "--password"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "bot", "gateway", "status", "--timeout"],
    );
  });
  ...
  it("returns false for gateway status route when probe-only flags are present", async () => {
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "bot", "gateway", "status", "--ssh", "user@host"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "bot", "gateway", "status", "--ssh-identity", "~/.ssh/id_test"],
    );
    await expectRunFalse(
      ["gateway", "status"],
      ["node", "bot", "gateway", "status", "--ssh-auto"],
    );
  });
  ...
  it("passes --no-probe through to daemon status", async () => {
    const route = expectRoute(["gateway", "status"]);
    await expect(route.run(["node", "bot", "gateway", "status", "--no-probe"])).resolves.toBe(
      true,
    );
  ...
  it.each([
    { path: ["health"], argv: ["node", "bot", "health", "--wat"] },
    { path: ["status"], argv: ["node", "bot", "status", "--wat"] },
    { path: ["sessions"], argv: ["node", "bot", "sessions", "--wat"] },
    {
      path: ["agents", "list"],
      argv: ["node", "bot", "agents", "list", "--wat"],
    },
    { path: ["agents"], argv: ["node", "bot", "agents", "--wat"] },
  ])(
  ...
  it("routes status --json through the lean JSON command", async () => {
    const route = expectRoute(["status"]);
    await expect(
      route.run(["node", "bot", "status", "--json", "--deep", "--usage", "--timeout", "5000"]),
    ).resolves.toBe(true);
  ...
  it("routes tasks list JSON through the lean task JSON command", async () => {
    ...
    expect(rootRoute.canRun?.(["node", "bot", "tasks"])).toBe(false);
    await expect(
      rootRoute.run([
        "node",
        "bot",
    ...
    await expect(
      listRoute.run(["node", "bot", "tasks", "list", "--json", "--runtime=cron"]),
    ).resolves.toBe(true);
  ...
  it("routes tasks audit JSON through the lean task JSON command", async () => {
    ...
    expect(route.loadPlugins).toBeUndefined();
    expect(route.canRun?.(["node", "bot", "tasks", "audit"])).toBe(false);
    await expect(
      route.run([
        "node",
        "bot",
  ...
  it("returns false for task JSON routes when option values are missing or unknown", async () => {
    await expectRunFalse(["tasks"], ["node", "bot", "tasks", "--json", "--runtime"]);
    await expectRunFalse(["tasks", "list"], ["node", "bot", "tasks", "list"]);
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "bot", "tasks", "audit", "--json", "--limit"],
    );
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "bot", "tasks", "audit", "--json", "--limit", "5abc"],
    );
    await expectRunFalse(
      ["tasks", "audit"],
      ["node", "bot", "tasks", "audit", "--json", "--unknown"],
    );
    expect(
      findRoutedCommand(["tasks", "cli"], ["node", "bot", "tasks", "--runtime", "cli"]),
    ).toBeNull();
  });
});
