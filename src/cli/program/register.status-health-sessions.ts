...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          ["bot sessions --limit 25", "Show the newest 25 sessions."],
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          [
            "bot sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .bot/trajectory-exports")
  ...
  sessionsCmd
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `bot sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["bot commitments", "List pending inferred follow-ups."],
          ["bot commitments --all", "List all inferred follow-ups."],
          ["bot commitments --agent work", "List one agent's inferred follow-ups."],
          ["bot commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
