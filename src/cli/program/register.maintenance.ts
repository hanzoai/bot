...
/** Register maintenance commands that inspect or mutate local Bot state. */
export function registerMaintenanceCommands(program: Command) {
  const doctor = program
    ...
    .option(
      "--github-issue",
      "With --session-sqlite recover: prepare and optionally create an hanzoai/bot issue",
      false,
    )
    .option(
      "--json",
    ...
    .action(async (opts, command) => {
      ...
      if (hasSessionSqliteOnlyDoctorOptions(opts)) {
        defaultRuntime.error(
          "doctor session SQLite options require --session-sqlite. Use `bot doctor --session-sqlite dry-run ...`.",
        );
        defaultRuntime.exit(2);
        return;
      }
      if (hasLintOnlyDoctorOptions(opts)) {
        defaultRuntime.error(
          "doctor lint options require --lint. Use `bot doctor --lint ...`.",
        );
        defaultRuntime.exit(2);
        return;
      }
