...
/** Register `bot agent` for one Gateway-backed agent turn. */
...
${formatHelpExamples([
  ...
  ["bot agent --agent ops --message-file ./task.md", "Read a multiline message file."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.bot.ai/cli/agent")}`,
  ...
  agent
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ['bot agent exec "Fix the failing test"', "Run in the current directory."],
          [
            "bot agent exec --message-file task.md --cwd ./repo",
            "Read a prompt file and set the workspace.",
          ],
          [
            'bot agent exec "Summarize this repo" --model openai/gpt-5.6-sol --fallback anthropic/claude-sonnet-4-6 --json',
            "Use an explicit fallback chain and JSON output.",
          ],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agent#agent-exec", "docs.bot.ai/cli/agent#agent-exec")}`,
    )
