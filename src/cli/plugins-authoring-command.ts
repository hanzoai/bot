...
export function buildToolPluginPackageManifest(params: {
  ...
  const bot =
    params.packageManifest.bot &&
    typeof params.packageManifest.bot === "object" &&
    !Array.isArray(params.packageManifest.bot)
      ? { ...(params.packageManifest.bot as JsonObject) }
      : {};
  const existingExtensions = Array.isArray(bot.extensions)
    ? bot.extensions.filter((entry): entry is string => typeof entry === "string")
    : [];
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("bot.plugin.json generated metadata is stale. Run bot plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `bot.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("bot.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`bot.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `bot.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include bot.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json bot.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run bot plugins build.");
...
function writeToolPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  ...
  const description = `Add ${params.name} tools to Bot.`;
...
import { getToolPluginMetadata } from "bot/plugin-sdk/tool-plugin";
...
function writeProviderPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  const packageName = `bot-plugin-${params.id}`;
  ...
  const description = `Add ${params.name} models to Bot.`;
  const packageManifest = {
    name: packageName,
    version: "0.1.0",
    description: `Bot provider plugin for ${params.name}.`,
    type: "module",
    scripts: {
      build: "tsc -p tsconfig.json",
      test: "vitest run --config ./vitest.config.ts",
  ...
  const indexSource = `import { definePluginEntry } from "bot/plugin-sdk/plugin-entry";
import { createProviderApiKeyAuthMethod } from "bot/plugin-sdk/provider-auth-api-key";
import { buildSingleProviderApiKeyCatalog } from "bot/plugin-sdk/provider-catalog-shared";
...
`;
  const testSource = `import { describe, expect, it } from "vitest";
import type { BotPluginApi, ProviderPlugin } from "bot/plugin-sdk/plugin-entry";
import entry from "./index.js";
...
describe(${idLiteral}, () => {
  it("registers the provider", () => {
    ...
    } as Partial<BotPluginApi>;

    entry.register(api as BotPluginApi);
...
Bot provider plugin for ${params.name}.
...
jobs:
  publish:
    permissions:
      ...
      contents: read
      id-token: write
    uses: bot/clawhub/.github/workflows/package-publish.yml@${CLAWHUB_PACKAGE_PUBLISH_WORKFLOW_REF}
    with:
