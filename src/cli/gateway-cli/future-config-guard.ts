...
type GatewayRunFutureConfigGuardParams = {
  ...
  config?: Pick<BotConfig, "env" | "meta"> | null;
};

function resolveGatewayRunFutureConfigBlock(params: GatewayRunFutureConfigGuardParams) {
  const processServiceMode = Boolean(process.env.BOT_SERVICE_MARKER?.trim());
  ...
  const candidateServiceMode =
    ...
    Boolean(
      candidateConfig
        ? createConfigRuntimeEnv(candidateConfig, process.env).BOT_SERVICE_MARKER?.trim()
        : undefined,
    );
