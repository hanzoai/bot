...
describe("skills cli commands", () => {
  ...
  it("shows skills.sh entries in normal ClawHub search results", async () => {
    searchSkillsFromClawHubMock.mockResolvedValue([
      {
        slug: "weather",
        installRef: "skills-sh:bot/skills/weather",
    ...
    expect(runtimeLogs).toEqual([
      "skills-sh:bot/skills/weather  Weather  Forecast helpers  Not scanned by ClawHub",
    ]);
  });
  ...
  it("rejects --version for skills-sh refs", async () => {
    await expect(
      runCommand(["skills", "install", "skills-sh:bot/skills/weather", "--version", "1.2.3"]),
  ...
  it("rejects the legacy skills-sh slash syntax before network access", async () => {
    await expect(
      runCommand(["skills", "install", "skills-sh/bot/skills/weather"]),
    ).rejects.toThrow("__exit__:1");

    expect(runtimeErrors).toContain(
      "Invalid skills.sh skill reference: skills-sh/bot/skills/weather",
    );
    expect(installSkillFromClawHubMock).not.toHaveBeenCalled();
    expect(installSkillFromSourceMock).not.toHaveBeenCalled();
  });
  ...
  it.each(["install", "verify"])(
    ...
    (commandName) => {
      ...
      expect(help).toContain(`bot skills ${commandName} @owner/weather`);
      expect(help).not.toContain(`bot skills ${commandName} weather`);
    },
  );
  ...
  it("installs a skill into the shared global skills directory", async () => {
    primeCalendarInstall("/tmp/bot-config");
  ...
  it.each([
    ...
    primeCalendarUpdate("/tmp/bot-config");
    ...
    expect(resolveDefaultAgentIdMock).not.toHaveBeenCalled();
    expect(resolveAgentWorkspaceDirMock).not.toHaveBeenCalled();
    expect(readTrackedClawHubSkillSlugsMock).toHaveBeenCalledWith("/tmp/bot-config");
    expect(updateSkillsFromClawHubMock).toHaveBeenCalledWith({
      workspaceDir: "/tmp/bot-config",
      slug,
  ...
  it("includes verified ClawHub source URLs in verify JSON output", async () => {
    ...
    const payload = JSON.parse(runtimeStdout.at(-1) ?? "{}") as {
      bot?: { verifiedSourceUrl?: string };
    };
    expect(payload.bot?.verifiedSourceUrl).toBe(verifiedSourceUrl);
    expect(defaultRuntime.exit).not.toHaveBeenCalled();
  });
  ...
  it("keeps non-JSON skills list output on stdout with human-readable formatting", async () => {
    ...
    expect(runtimeStdout.at(-1)).toContain("bot skills search");
  });
});
