...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: BotConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
const launchdUpdateCleanupMocks = vi.hoisted(() => ({
  disableCurrentBotUpdateLaunchdJob: vi.fn(async () => false),
}));
const databasePreflightMocks = vi.hoisted(() => ({
  preflightBotDatabaseSchemas: vi.fn(),
}));
...
vi.mock("../state/bot-database-preflight.js", () => ({
  BOT_DATABASE_SCHEMA_DOCS_URL: "https://docs.bot.ai/reference/database-schemas",
  preflightBotDatabaseSchemas: databasePreflightMocks.preflightBotDatabaseSchemas,
}));

vi.mock("../infra/bot-root.js", () => ({
  resolveBotPackageRoot: vi.fn(),
  resolveBotPackageRootSync: vi.fn(() => process.cwd()),
}));
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/bot-config",
...
vi.mock("../daemon/launchd.js", async (importOriginal) => ({
  ...
  disableCurrentBotUpdateLaunchdJob:
    launchdUpdateCleanupMocks.disableCurrentBotUpdateLaunchdJob,
}));
...
describe("update-cli", () => {
  ...
  const stripBotPackageAlias = (spec: string) => {
    const trimmed = spec.trim();
    return trimmed.toLowerCase().startsWith("bot@")
      ? trimmed.slice("bot@".length)
      : trimmed;
  };

  const isNpmGitPackageSpec = (spec: string) => {
    const target = stripBotPackageAlias(spec);
  ...
  const syncPluginCall = (index = 0) => {
    const calls = syncPluginsForUpdateChannel.mock.calls as unknown as Array<
      [Record<string, unknown> & { channel?: string; config?: BotConfig }]
    >;
  ...
  const npmPluginUpdateCall = (index = 0) => {
    const calls = updateNpmInstalledPlugins.mock.calls as unknown as Array<
      [Record<string, unknown> & { config?: BotConfig; timeoutMs?: number }]
    >;
    return calls[index]?.[0];
  };
  ...
  const setupConfigMutationWithRetryMock = () => {
    vi.mocked(mutateConfigFileWithRetry).mockImplementation(async (params) => {
      ...
      const nextConfig = structuredClone(snapshot.sourceConfig) as BotConfig;
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    for (const entrypoint of entrypoints) {
      ...
      fsSync.writeFileSync(
        packageJsonPath,
        JSON.stringify({ name: "bot", version: "2026.4.24" }),
        "utf8",
      );
  ...
  const FRESH_POST_UPDATE_ENTRYPOINT = "/tmp/bot-updated-entry.mjs";
  ...
  const pluginSyncResult = (config: BotConfig, changed = false) => ({
    changed,
    config,
    summary: {
      switchedToBundled: [],
  ...
  const npmPluginUpdateResult = (config: BotConfig) => ({
    changed: false,
    config,
    outcomes: [],
  });
    ...
    return withEnvAsync(
      {
        ...
        BOT_UPDATE_POST_CORE_CHANNEL: "stable",
        ...env,
      },
      async () => {
  ...
  const setupInstalledPackageAtNodeModules = async (nodeModules: string, version = "2026.4.21") => {
    const pkgRoot = path.join(nodeModules, "bot");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(path.dirname(entryPath), { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "bot", version }),
      "utf-8",
    );
  ...
  const mockRunningManagedGateway = (
    programArguments: string[] = ["bot", "gateway", "run"],
  ) => {
    serviceReadCommand.mockResolvedValue({
      programArguments,
      environment: {
  ...
  const setupNpmUpdatedRootRefresh = () => {
    const updatedRoot = createCaseDir("bot-updated-root");
  ...
  const runControlPlaneUpdate = async (params: {
    ...
    const stateDir = await createTrackedTempDir("bot-update-sentinel-state-");
    const metaDir = await createTrackedTempDir("bot-update-sentinel-meta-");
    ...
    return readRestartSentinel({ BOT_STATE_DIR: stateDir } as NodeJS.ProcessEnv);
  };

  const setupInteractiveClawHubRisk = async () => {
    mockPackageInstallStatus(createCaseDir("bot-update"));
    setTty(true);
    setStdoutTty(true);
  ...
  beforeEach(() => {
    ...
    delete process.env.BOT_SERVICE_KIND;
    ...
    databasePreflightMocks.preflightBotDatabaseSchemas.mockReturnValue({
      incompatible: [],
      indeterminate: [],
    });
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        if (destination) {
          await fs.writeFile(path.join(destination, "bot-9999.0.0.tgz"), "packed\n", "utf8");
        }
      }
      return commandResult();
    });
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["bot", "gateway", "run"] } : null,
    );
    ...
    checkShellCompletionStatus.mockResolvedValue({
      ...
      cacheExists: false,
      cachePath: "/tmp/bot-completion.zsh",
    ...
    launchdUpdateCleanupMocks.disableCurrentBotUpdateLaunchdJob.mockReset();
    launchdUpdateCleanupMocks.disableCurrentBotUpdateLaunchdJob.mockResolvedValue(false);
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("bot-completion-timeout");
    ...
    expect(call?.[1]).toEqual([path.join(root, "bot.mjs"), "completion", "--write-state"]);
    expect(call?.[2]?.env?.BOT_COMPLETION_SKIP_PLUGIN_COMMANDS).toBe("1");
  ...
  it("disarms legacy launchd updater jobs before refusing mutating updates in Nix mode", async () => {
    await withEnvAsync({ BOT_NIX_MODE: "1" }, async () => {
      await expect(updateCommand({ yes: true })).rejects.toThrow("BOT_NIX_MODE=1");
    });

    expect(launchdUpdateCleanupMocks.disableCurrentBotUpdateLaunchdJob).toHaveBeenCalledOnce();
  ...
  it("delegates mutating updates when an external supervisor owns gateway lifecycle", async () => {
    await withEnvAsync({ BOT_SUPERVISOR_MODE: "external" }, async () => {
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("bot-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logOutput).toContain("bot completion --write-state");
  ...
  it("respawns into the updated package root before running post-update tasks", async () => {
    ...
    expect(call?.[2]?.env?.BOT_UPDATE_IN_PROGRESS).toBe("1");
    expect(call?.[2]?.env?.BOT_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.BOT_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.BOT_COMPATIBILITY_HOST_VERSION).toBe("1.0.0");
  ...
  it("finishes package updates when the post-core process writes a result but keeps handles open", async () => {
    ...
    spawn.mockImplementationOnce((_command: unknown, _argv: unknown, options: unknown) => {
      const resultPath = (options as { env?: NodeJS.ProcessEnv }).env
        ?.BOT_UPDATE_POST_CORE_RESULT_PATH;
      if (!resultPath) {
  ...
  it("does not restart a stopped managed gateway after post-core plugin errors", async () => {
    ...
    const root = createCaseDir("bot-update");
  ...
  it("does not carry gateway service markers into the post-core update process", async () => {
    ...
    expect(spawnEnv?.BOT_SERVICE_MARKER).toBeUndefined();
    expect(spawnEnv?.BOT_SERVICE_KIND).toBeUndefined();
  ...
  it("passes pre-update plugin install records into the post-core update process", async () => {
    ...
    const pluginInstallRecords = {
      demo: {
        ...
        installPath: "/tmp/bot-demo-plugin",
      },
    } as const;
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      const recordsPath = env?.BOT_UPDATE_POST_CORE_INSTALL_RECORDS_PATH;
      const sourceConfigPath = env?.BOT_UPDATE_POST_CORE_SOURCE_CONFIG_PATH;
      if (!recordsPath) {
  ...
  it("clears stale npm resolution metadata before post-core downgrade resume", async () => {
    ...
    const pluginInstallRecords = {
      msteams: {
        source: "npm",
        spec: "@hanzo/bot-msteams",
        installPath: "/tmp/bot-msteams-plugin",
        version: "1.0.0",
        resolvedName: "@hanzo/bot-msteams",
        resolvedVersion: "1.0.0",
        resolvedSpec: "@hanzo/bot-msteams@1.0.0",
  ...
  it("respawns into the updated git root before requested channel persistence", async () => {
    ...
    expect(call?.[2]?.env?.BOT_UPDATE_POST_CORE).toBe("1");
    expect(call?.[2]?.env?.BOT_UPDATE_POST_CORE_CHANNEL).toBe("dev");
    expect(call?.[2]?.env?.BOT_UPDATE_POST_CORE_REQUESTED_CHANNEL).toBe("dev");
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("bot-downgraded-root");
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
  ...
  it("runs the fresh doctor for a core-changing downgrade without plugin changes", async () => {
    const downgradedRoot = createCaseDir("bot-downgraded-fresh-doctor-root");
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
  ...
  it("pins the compatibility host version to the downgraded target during current-process post-core plugin convergence (#87914)", async () => {
    const downgradedRoot = createCaseDir("bot-downgraded-compat-root");
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
    ...
    updateNpmInstalledPlugins.mockImplementation(async () => {
      hostVersionDuringPluginUpdate = process.env.BOT_COMPATIBILITY_HOST_VERSION;
    ...
    try {
      ...
      expect(process.env.BOT_COMPATIBILITY_HOST_VERSION).toBeUndefined();
    } finally {
      delete process.env.BOT_COMPATIBILITY_HOST_VERSION;
    }
  });
  ...
  it("runs updated plugin migrations for a plugin-only current-process update", async () => {
    ...
    vi.mocked(readConfigFileSnapshot).mockImplementation(async (options) => {
      if (!options) {
        strictValidationEnv = process.env.BOT_UPDATE_IN_PROGRESS;
      }
      return baseSnapshot;
    });
  ...
  it("runs the fresh plugin doctor with the selected Node runner", async () => {
    ...
    await completeChangedPostCorePluginUpdate({ nodeRunner: "/opt/bot-service/bin/node" });

    expect(vi.mocked(runExec).mock.calls[0]?.[0]).toBe("/opt/bot-service/bin/node");
  });
  ...
  it("post-core resume children exit after writing a plugin update result", async () => {
    const resultDir = createCaseDir("bot-post-core-result");
    const resultPath = path.join(resultDir, "plugins.json");
    await fs.mkdir(resultDir, { recursive: true });

    await runPostCoreCommand(
      { restart: false },
      { BOT_UPDATE_POST_CORE_RESULT_PATH: resultPath },
    );
  ...
  it("post-core resume mode uses the parent install records snapshot for missing payload warnings", async () => {
    const resultDir = createCaseDir("bot-post-core-records");
    ...
    await runPostCoreCommand(
      ...
      { BOT_UPDATE_POST_CORE_INSTALL_RECORDS_PATH: recordsPath },
    );
  ...
  it("post-core resume mode prefers post-doctor disk install records over the stale parent snapshot", async () => {
    const resultDir = createCaseDir("bot-post-core-disk-records");
    const recordsPath = path.join(resultDir, "plugin-install-records.json");
    await fs.mkdir(resultDir, { recursive: true });
    await fs.writeFile(
      recordsPath,
    ...
    const postDoctorRecords = {
      codex: {
        source: "npm",
        spec: "@hanzo/bot-codex@2026.5.17",
  ...
  it("post-core resume mode persists the requested update channel with the updated process", async () => {
    ...
    await runPostCoreCommand(
      { restart: false },
      {
        BOT_UPDATE_POST_CORE_CHANNEL: "dev",
        BOT_UPDATE_POST_CORE_REQUESTED_CHANNEL: "dev",
      },
    );
  ...
  it("prints plugin channel fallbacks near the post-core plugin summary", async () => {
    ...
    await runPostCoreCommand({ restart: false }, { BOT_UPDATE_POST_CORE_CHANNEL: "beta" });
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@hanzo/bot-demo@1.0.0",
        resolvedSpec: "@hanzo/bot-demo@1.0.0",
  ...
  it("keeps json update output successful when post-core plugin updates warn", async () => {
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @hanzo/bot-demo@1.0.0"
    ...
    expect(pluginWarning(jsonOutput)?.guidance).toEqual([
      ...
      "Run bot plugins inspect demo --runtime --json for details.",
    ]);
  ...
  it("does not print duplicate failed ClawHub sync trust warnings in human post-core output", async () => {
    ...
    syncPluginsForUpdateChannel.mockImplementationOnce(
      async (params: { config: BotConfig; logger?: { warn?: (message: string) => void } }) => {
        params.logger?.warn?.(trustWarning);
        return {
          changed: false,
          config: params.config,
  ...
  it("does not print duplicate ClawHub update trust warnings in human post-core output", async () => {
    ...
    expect(output).toContain("Run bot update repair to retry post-update plugin repair.");
    expect(output).toContain("Run bot plugins inspect demo --runtime --json for details.");
  });

  it("detects missing plugin payloads from persisted records before npm updates", async () => {
    const installPath = createCaseDir("bot-missing-plugin-payload");
  ...
  it("prints non-fatal plugin warnings in human update output", async () => {
    ...
    expect(logs).toContain("Run bot update repair to retry post-update plugin repair.");
    expect(logs).toContain("Run bot plugins inspect demo --runtime --json for details.");
  });
      ...
      outcomes: [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; Bot will continue without it. Failed to update demo: registry timeout',
        },
      ],
    });
    vi.mocked(defaultRuntime.writeJson).mockClear();
  ...
  it("marks blocked ClawHub update skips as post-update warnings", async () => {
    ...
    expect(pluginOutcome(jsonOutput)?.pluginId).toBe("demo");
    expect(pluginOutcome(jsonOutput)?.status).toBe("skipped");
    expect(pluginOutcome(jsonOutput)?.message).toContain("Run bot update repair");
  });
  ...
  it("preserves fresh-process plugin warning details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(() => {
        void (async () => {
          const resultPath = env?.BOT_UPDATE_POST_CORE_RESULT_PATH;
          if (resultPath) {
            await fs.writeFile(
              resultPath,
              JSON.stringify({
                ...
                warnings: [
                  {
                    ...
                    message:
                      'Plugin "demo" could not be processed after the core update: Failed to update demo: registry timeout Run bot update repair to retry post-update plugin repair. Run bot plugins inspect demo --runtime --json for details.',
                    guidance: [
                      "Run bot update repair to retry post-update plugin repair.",
                      "Run bot plugins inspect demo --runtime --json for details.",
                    ],
  ...
  it("refuses an incompatible package target before service stop or install", async () => {
    mockPackageInstallStatus(createCaseDir("bot-schema-refusal"));
    ...
    databasePreflightMocks.preflightBotDatabaseSchemas.mockReturnValue({
      incompatible: [
        {
          kind: "agent",
          path: "/tmp/bot/agents/main/agent/bot-agent.sqlite",
          agentId: "main",
          foundVersion: 11,
          supportedVersion: 9,
    ...
    expect(databasePreflightMocks.preflightBotDatabaseSchemas).toHaveBeenCalledWith({
      env: process.env,
  ...
  it("skips package schema preflight when target metadata is missing", async () => {
    mockPackageInstallStatus(createCaseDir("bot-schema-missing"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: "9999.0.0",
      nodeEngine: ">=22.19.0",
    ...
    expect(databasePreflightMocks.preflightBotDatabaseSchemas).not.toHaveBeenCalled();
    expect(packageInstallCommandCall()).toBeDefined();
  });

  it("refuses a package update when exact target metadata lookup fails", async () => {
    mockPackageInstallStatus(createCaseDir("bot-schema-metadata-failure"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: null,
      nodeEngine: null,
    ...
    expectNoSideEffects(databasePreflightMocks.preflightBotDatabaseSchemas, serviceStop);
    expect(packageInstallCommandCall()).toBeUndefined();
    expect(defaultRuntime.error).toHaveBeenCalledWith(
      expect.stringContaining("could not inspect exact package target bot@9999.0.0"),
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(1);
  });

  it("continues a package update when target schemas are compatible", async () => {
    mockPackageInstallStatus(createCaseDir("bot-schema-compatible"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: "9999.0.0",
      nodeEngine: ">=22.19.0",
    ...
    expect(databasePreflightMocks.preflightBotDatabaseSchemas).toHaveBeenCalledTimes(2);
    expect(packageInstallCommandCall()?.[0]).toContain("bot@9999.0.0");
  });

  it("reports an incompatible package target during dry-run", async () => {
    mockPackageInstallStatus(createCaseDir("bot-schema-dry-run"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: "9999.0.0",
      nodeEngine: ">=22.19.0",
    ...
    expect(logs).toContain("https://docs.bot.ai/reference/database-schemas");
    expect(serviceStop).not.toHaveBeenCalled();
    expect(packageInstallCommandCall()).toBeUndefined();
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });
  ...
  it("reports indeterminate package databases during dry-run", async () => {
    mockPackageInstallStatus(createCaseDir("bot-schema-indeterminate-dry-run"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
      version: "9999.0.0",
      nodeEngine: ">=22.19.0",
    ...
    databasePreflightMocks.preflightBotDatabaseSchemas.mockReturnValue({
      incompatible: [],
      indeterminate: [
        { kind: "state", path: "/tmp/bot/state/bot.sqlite", reason: "database busy" },
      ],
    });
    ...
    expect(logs).toContain(
      "could not inspect state database /tmp/bot/state/bot.sqlite: database busy; retry once the gateway releases it",
    );
  });
  ...
  it("refuses a package target that changes after the service stops", async () => {
    ...
    serviceReadCommand.mockResolvedValue({
      ...
      environment: { BOT_STATE_DIR: "/managed/state" },
    });
    serviceLoaded.mockResolvedValue(true);
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "9999.0.0",
    ...
    databasePreflightMocks.preflightBotDatabaseSchemas
    ...
    expect(databasePreflightMocks.preflightBotDatabaseSchemas.mock.calls[1]?.[0].env).toEqual(
      expect.objectContaining({ BOT_STATE_DIR: "/managed/state" }),
    );
    expect(packageInstallCommandCall()).toBeUndefined();
  ...
  it("refuses a git target that changes after the service stops", async () => {
    ...
    databasePreflightMocks.preflightBotDatabaseSchemas
      ...
      .mockReturnValueOnce({
        incompatible: [],
        indeterminate: [
          { kind: "agent", path: "/tmp/bot-agent.sqlite", reason: "database busy" },
        ],
      });
  ...
  it("fails a post-stop git refusal when no managed service was running", async () => {
    ...
    databasePreflightMocks.preflightBotDatabaseSchemas
      ...
      .mockReturnValueOnce({
        incompatible: [],
        indeterminate: [{ kind: "state", path: "/tmp/bot.sqlite", reason: "database busy" }],
      });
  ...
  it.each([
    {
      ...
      assert: () => {
        expect(getLogOutput()).toContain("Bot update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "bot", "update", "status", "--json"]);
  ...
  it.each([
    ...
    async ({ installKind, options, storedChannel, expectedChannel, expectedPersistedChannel }) => {
      ...
      if (storedChannel) {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: storedChannel } } as BotConfig,
        });
      }
      ...
      if (expectedChannel !== undefined) {
        ...
        expectPackageInstallSpec("bot@9999.0.0");
      }
  ...
  it("falls back to latest when beta tag is older than release", async () => {
    ...
    expectPackageInstallSpec("bot@1.2.3-1");
  });
  ...
  it("installs the verified exact package and persists an explicit extended-stable channel", async () => {
    ...
    expectPackageInstallSpec("bot@2026.6.33");
  ...
  it("uses the same exact resolver for a bare update with stored extended-stable", async () => {
    ...
    mockPackageInstallStatus(tempDir);
    readPackageVersion.mockResolvedValue("2026.6.33");
    const config = { update: { channel: "extended-stable" } } as BotConfig;
    vi.mocked(readConfigFileSnapshot).mockResolvedValue(configSnapshot(config));
      ...
      warnings: [
        {
          ...
          guidance: ["Run bot update repair."],
        },
      ],
      errored: true,
      smokeFailures: [],
  ...
  it("runs the package update when latest target lookup is unresolved", async () => {
    ...
    expectPackageInstallSpec("bot@latest");
    expectFreshPostUpdateDoctor({ yes: false });
  });
  ...
  it("allows package updates from inherited gateway service env when the managed gateway is not running", async () => {
    ...
    expect(defaultRuntime.error).not.toHaveBeenCalledWith(
      [
        ...
        "Run `bot update` from a shell outside the gateway service, or stop the gateway service first and then update.",
      ].join("\n"),
    );
    expectPackageInstallSpec("bot@9999.0.0");
  });
  ...
  it("refuses package updates from inside the active gateway process tree", async () => {
    ...
    expect(errors).toContain(
      "bot update detected it is running inside the gateway process tree.",
    );
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors).toContain(
      "Bare `npm i -g bot` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      options: { yes: true, tag: "github:hanzoai/bot#main" },
      packageSpec: undefined,
      expectedSpec: "github:hanzoai/bot#main",
    },
    {
      name: "aliased git package spec",
      options: { yes: true, tag: "Bot@github:hanzoai/bot#main" },
      packageSpec: undefined,
      expectedSpec: "Bot@github:hanzoai/bot#main",
    },
    {
      name: "full git URL package spec",
      options: { yes: true, tag: "https://github.com/hanzoai/bot.git#main" },
      packageSpec: undefined,
      expectedSpec: "https://github.com/hanzoai/bot.git#main",
    },
    {
      name: "hosted GitHub URL package spec without git suffix",
      options: { yes: true, tag: "https://github.com/hanzoai/bot#main" },
      packageSpec: undefined,
      expectedSpec: "https://github.com/hanzoai/bot#main",
    },
    {
      name: "aliased hosted GitHub URL package spec without git suffix",
      options: { yes: true, tag: "bot@https://github.com/hanzoai/bot#main" },
      packageSpec: undefined,
      expectedSpec: "https://github.com/hanzoai/bot#main",
    },
    {
      name: "GitHub shorthand package spec",
      options: { yes: true, tag: "hanzoai/bot#main" },
      packageSpec: undefined,
      expectedSpec: "hanzoai/bot#main",
    },
    {
      name: "SCP-style SSH package spec",
      options: { yes: true, tag: "git@github.com:hanzoai/bot.git#main" },
      packageSpec: undefined,
      expectedSpec: "git@github.com:hanzoai/bot.git#main",
    },
    {
      name: "BOT_UPDATE_PACKAGE_SPEC override",
      options: { yes: true, tag: "latest" },
      packageSpec: "http://10.211.55.2:8138/bot-next.tgz",
      expectedSpec: "http://10.211.55.2:8138/bot-next.tgz",
    },
  ] as const)(
    ...
    async ({ options, packageSpec, expectedSpec }) => {
      ...
      mockPackageInstallStatus(createCaseDir("bot-update"));
      if (packageSpec) {
        await withEnvAsync({ BOT_UPDATE_PACKAGE_SPEC: packageSpec }, async () => {
          await updateCommand(options);
        });
      } else {
        await updateCommand(options);
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "bot", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await createTrackedTempDir("bot-update-staged-fail-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "bot");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "bot", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("runs old package doctors without fix mode when service ownership is unknown", async () => {
    const tempDir = await createTrackedTempDir("bot-update-package-");
    const { nodeModules, pkgRoot, entryPath } = await setupInstalledPackageRoot(tempDir);
    serviceReadCommand.mockResolvedValue({
      programArguments: ["bot-wrapper", "gateway", "run"],
    });
    serviceLoaded.mockResolvedValue(true);
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.BOT_UPDATE_IN_PROGRESS,
    ).toBe("1");
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)
        ?.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION,
    ).toBe("0");
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)
        ?.BOT_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR,
    ).toBe("0");
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)
        ?.BOT_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART,
    ).toBe("1");
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.BOT_SERVICE_REPAIR_POLICY,
    ).toBeUndefined();
  ...
  it("continues package post-core work for explicit post-update doctor advisories", async () => {
    const tempDir = await createTrackedTempDir("bot-update-package-doctor-warning-");
    ...
    await withEnvAsync({ BOT_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION: "1" }, async () => {
    ...
    expect(postCoreCall?.[2]?.env?.BOT_UPDATE_POST_CORE).toBe("1");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    const jsonOutput = lastWriteJsonCall() as UpdateRunResult | undefined;
    const doctorStep = jsonOutput?.steps.find((step) => step.name === "bot doctor");
    expect(jsonOutput?.status).toBe("ok");
  ...
  it("fails package updates when the post-update doctor is killed after verification", async () => {
    const tempDir = await createTrackedTempDir("bot-update-package-doctor-timeout-");
    const { nodeModules, entryPath } = await setupInstalledPackageRoot(tempDir);
    vi.mocked(resolveNpmChannelTag).mockResolvedValue({
      tag: "latest",
      version: "2026.4.21",
  ...
  it("runs package post-update doctor from the verified package root after a staged swap", async () => {
    const tempDir = await createTrackedTempDir("bot-update-staged-doctor-");
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (argv[0] === "npm" && argv[1] === "i" && argv.includes("--prefix")) {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({ name: "bot", version: "2026.5.14" }),
          "utf-8",
        );
    ...
    expect(
      (doctorCall?.[1].env as NodeJS.ProcessEnv | undefined)?.BOT_COMPATIBILITY_HOST_VERSION,
    ).toBe("2026.5.14");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });
  ...
  it("stops a running managed gateway before package replacement", async () => {
    ...
    suspendScheduledTaskAutoStartForUpdate.mockResolvedValue(true);
    resumeScheduledTaskAutoStartAfterUpdate.mockResolvedValue(true);
    const tempDir = await createTrackedTempDir("bot-update-stop-service-");
    ...
    const serviceStopCall = serviceStop.mock.calls[0]?.[0] as
      | { env?: NodeJS.ProcessEnv }
      | undefined;
    expect(serviceStopCall?.env?.BOT_SERVICE_MARKER).toBe("bot");
    expect(serviceStopCall?.env?.BOT_SERVICE_KIND).toBe("gateway");
  ...
  it("restores Windows Scheduled Task autostart when service stop fails", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    mockPackageInstallStatus(createCaseDir("bot-update-stop-failure"));
    mockRunningManagedGateway();
    suspendScheduledTaskAutoStartForUpdate.mockResolvedValue(true);
  ...
  it("preserves both the update and Scheduled Task recovery failures", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    mockPackageInstallStatus(createCaseDir("bot-update-recovery-failure"));
    serviceReadCommand.mockResolvedValue({
      programArguments: ["bot", "gateway", "run"],
      environment: {
        BOT_SERVICE_MARKER: "bot",
  ...
  it.each(["SIGINT", "SIGBREAK"] as const)(
    ...
    async (signal) => {
      ...
      resumeScheduledTaskAutoStartAfterUpdate.mockResolvedValue(true);
      mockPackageInstallStatus(createCaseDir("bot-update-suspension-signal"));
      serviceReadCommand.mockResolvedValue({
        programArguments: ["bot", "gateway", "run"],
        environment: {
          BOT_SERVICE_MARKER: "bot",
  ...
  it.each(["running", "stopped"] as const)(
    ...
    async (runtimeStatus) => {
      const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
      mockPackageInstallStatus(createCaseDir("bot-update-stopped-task"));
      serviceReadCommand.mockResolvedValue({
        programArguments: ["bot", "gateway", "run"],
        environment: {
          BOT_SERVICE_MARKER: "bot",
  ...
  it("stops a running managed git gateway when wrapper commands hide the service root", async () => {
    const wrapperPath = path.join(
      createCaseDir("bot-update-wrapper-service"),
      "gateway-wrapper",
    );
  ...
  it("stops a managed gateway rooted at the git checkout when switching package installs to dev", async () => {
    const packageRoot = createCaseDir("bot-update-package-root");
    const gitRoot = await createTrackedTempDir("bot-update-git-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "bot", version: "2026.4.21" }),
      "utf-8",
    );
    ...
    await withEnvAsync({ BOT_GIT_DIR: gitRoot }, async () => {
  ...
  it("stops a managed gateway rooted at the package install when switching package installs to dev", async () => {
    const packageRoot = await createTrackedTempDir("bot-update-package-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "bot", version: "2026.4.20" }),
      "utf-8",
    );
  ...
  it("does not stop or restart a managed gateway owned by another git checkout", async () => {
    const otherRoot = await createTrackedTempDir("bot-update-other-service-root-");
  ...
  it("keeps managed service stop output off stdout during json package updates", async () => {
    const tempDir = await createTrackedTempDir("bot-update-json-stop-service-");
    const { nodeModules } = await setupInstalledPackageRoot(tempDir);
    const stdoutWrite = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
    mockRunningManagedGateway();
    serviceStop.mockImplementationOnce(async (params: { stdout?: NodeJS.WritableStream }) => {
      params.stdout?.write("Stopped systemd service: bot-gateway.service\n");
    });
    mockFileBackedPathExists();
    mockNpmGlobalRoot(nodeModules);
  ...
  it("disarms legacy launchd updater jobs before stopping the gateway", async () => {
    const tempDir = await createTrackedTempDir("bot-update-launchd-loop-");
    const { nodeModules } = await setupInstalledPackageRoot(tempDir);
    mockRunningManagedGateway();
    launchdUpdateCleanupMocks.disableCurrentBotUpdateLaunchdJob.mockResolvedValue(true);
    mockFileBackedPathExists();
    mockNpmGlobalRoot(nodeModules);
    ...
    const cleanupOrder =
      launchdUpdateCleanupMocks.disableCurrentBotUpdateLaunchdJob.mock.invocationCallOrder[0];
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await createTrackedTempDir("bot-update-current-");
    ...
    expectPackageInstallSpec("bot@2026.4.23");
    const doctorCall = doctorCommandCall();
    expect(doctorCall?.[0][0]).toContain("node");
    expect(doctorCall?.[0].slice(1)).toEqual([entryPath, "doctor", "--non-interactive"]);
    ...
    expect(postCoreSpawn?.[2].env?.BOT_UPDATE_POST_CORE).toBe("1");
    expect(postCoreSpawn?.[2].env?.BOT_UPDATE_POST_CORE_CHANNEL).toBe("stable");
    expect(updateNpmInstalledPlugins).not.toHaveBeenCalled();
    expect(getLogOutput()).not.toContain("already-current");
  });

  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await createTrackedTempDir("bot-update-optional-");
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "bot");
    mockPackageInstallStatus(pkgRoot);
    mockCurrentProcessFreshDoctor();
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "bot", version: "9999.0.0" }),
      "utf-8",
    );
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
      const absolutePath = path.join(pkgRoot, relativePath);
  ...
  it("uses the owning npm binary for package updates when PATH npm points elsewhere", async () => {
    ...
    const pkgRoot = path.join(brewRoot, "bot");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const tempDir = createCaseDir("bot-update");
    const localAppData = createCaseDir("bot-localappdata");
  ...
  it("warns when a package update targets a managed service root outside the shell root", async () => {
    const shellRoot = createCaseDir("bot-shell-root");
    const serviceRoot = await createTrackedTempDir("bot-service-root-");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "bot", version: "2026.5.18" }),
      "utf-8",
    );
    ...
    expect(logs).toContain(
      `Shell Bot root differs from the managed gateway service root: ${shellRoot}`,
    );
    expect(logs).toContain("make sure `bot` on PATH resolves to the managed service root");
  ...
  it("runs managed service package follow-up commands with the service Node", async () => {
    const shellRoot = createCaseDir("bot-shell-root");
    const servicePrefix = await createTrackedTempDir("bot-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const serviceRoot = path.join(nodeModules, "bot");
  ...
  it("uses the managed service Node when package roots match but node binaries differ", async () => {
    const root = createCaseDir("bot-same-root");
  ...
  it("refreshes the managed service to current Node when its baked Node cannot run the target", async () => {
    const servicePrefix = await createTrackedTempDir("bot-service-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "bot");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const serviceNpm = path.join(servicePrefix, "bin", "npm");
    const entrypoint = path.join(root, "dist", "index.js");
    await fs.mkdir(path.dirname(entrypoint), { recursive: true });
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "bot", version: "2026.7.1" }),
          "utf-8",
        );
        await fs.writeFile(stageEntryPoint, "export {};\n", "utf-8");
        await writePackageDistInventory(stageRoot);
  ...
  it("pins package install to the service root when nodes differ and no owning npm exists at the prefix", async () => {
    const servicePrefix = await createTrackedTempDir("bot-no-npm-prefix-");
    const nodeModules = path.join(servicePrefix, "lib", "node_modules");
    const root = path.join(nodeModules, "bot");
    const serviceNode = path.join(servicePrefix, "bin", "node");
    const entrypoint = path.join(root, "dist", "index.js");
  ...
  it("refreshes post-doctor config before post-update plugin sync", async () => {
    const tempDir = createCaseDir("bot-update");
    mockPackageInstallStatus(tempDir);
    const preUpdateConfig = { update: { channel: "stable" } } as BotConfig;
    ...
    syncPluginsForUpdateChannel.mockImplementation(async ({ config }) => ({
      ...
      config: {
        ...
        plugins: {
          ...config.plugins,
          load: { paths: ["/tmp/bot-updated-plugin"] },
        },
      },
      summary: {
        switchedToBundled: [],
    ...
    const syncConfig = syncPluginCall()?.config as
      | (BotConfig & { meta?: { lastTouchedVersion?: string } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          baseHash?: string;
          nextConfig?: BotConfig & { meta?: { lastTouchedVersion?: string } };
        }
      | undefined;
  ...
  it.each([
    {
      ...
      prepare: async (configPath: string, preUpdateConfig: BotConfig) => {
        await fs.writeFile(
    ...
    },
    {
      ...
      prepare: async (configPath: string, preUpdateConfig: BotConfig) => {
        ...
        return { BOT_UPDATE_POST_CORE_STARTED_AT_MS: String(updateStartedAtMs) };
      },
    },
    {
    ...
    const postDoctorConfig = { update: { channel: "stable" } } as BotConfig;
    await fs.mkdir(tempDir, { recursive: true });
  ...
  it("persists authored channel values when post-core restore input is resolved", async () => {
    ...
    await runPostCoreUpdate({ BOT_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath });

    const syncConfig = syncPluginCall()?.config as
      | (BotConfig & { channels?: { whatsapp?: { token?: string } } })
      | undefined;
    const lastWrite = lastReplaceConfigCall() as
      | {
          nextConfig?: BotConfig & {
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("bot-update");
    const gitRoot = path.join(tempDir, "..", "bot");
    ...
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as BotConfig,
      sourceConfig: { update: { channel: "stable" } } as BotConfig,
      runtimeConfig: { update: { channel: "stable" } } as BotConfig,
      config: { update: { channel: "stable" } } as BotConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "git",
    ...
    const syncCall = syncPluginCall() as
      | { channel?: string; config?: BotConfig; workspaceDir?: string }
      | undefined;
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs).toContain(
      "Commit, stash, or discard the local changes, then rerun `bot update`.",
    );
    expect(serviceStop).not.toHaveBeenCalled();
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  ...
  it("restores an unknown package service without rewriting its missing updated entrypoint", async () => {
    ...
    expect(runRestartScript).toHaveBeenCalledWith("/tmp/bot-restart-test.sh");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });
  ...
  it("restores a same-version service without rewriting when its root is ambiguous", async () => {
    const oldRoot = createCaseDir("bot-old-root");
    const updatedRoot = createCaseDir("bot-updated-root");
  ...
  it.each([
    ...
    },
    {
      ...
      expectedEnv: () => ({
        ...
        BOT_CONFIG_PATH: path.resolve("./config/bot.json"),
      }),
      assertExtra: () => {
        expect(runDaemonInstall).not.toHaveBeenCalled();
      },
  ...
  it("marks the whole update command as update-in-progress", async () => {
    await withEnvAsync({ BOT_UPDATE_IN_PROGRESS: undefined }, async () => {
      ...
      vi.mocked(runGatewayUpdate).mockImplementationOnce(async () => {
        observedUpdateEnv = process.env.BOT_UPDATE_IN_PROGRESS;
  ...
  it("updateFinalizeCommand runs doctor and plugin convergence with full update env", async () => {
    await withEnvAsync(
      {
        BOT_UPDATE_IN_PROGRESS: undefined,
        BOT_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR: undefined,
        BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: undefined,
        BOT_UPDATE_POST_CORE_CONVERGENCE: undefined,
      },
      async () => {
        ...
        expect(doctorEnv?.BOT_UPDATE_IN_PROGRESS).toBe("1");
        expect(doctorEnv?.BOT_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBe("1");
        expect(doctorEnv?.BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBe("1");
        expect(doctorEnv?.BOT_UPDATE_POST_CORE_CONVERGENCE).toBe("1");
        expect(process.env.BOT_UPDATE_IN_PROGRESS).toBeUndefined();
        expect(process.env.BOT_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR).toBeUndefined();
        expect(process.env.BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE).toBeUndefined();
        expect(process.env.BOT_UPDATE_POST_CORE_CONVERGENCE).toBeUndefined();
  ...
  it("updateFinalizeCommand repairs doctor by default and refreshes plugin state after doctor", async () => {
    vi.mocked(resolveGatewayInstallEntrypoint).mockResolvedValueOnce("/tmp/bot-entry.mjs");
    ...
    expect(freshDoctorCall?.[2]).toMatchObject({
      ...
      env: {
        ...
        BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE: "1",
  ...
  it("updateFinalizeCommand restores channels from the RPC pre-update config payload", async () => {
    const tempDir = createCaseDir("bot-rpc-finalize");
    const sourceConfigPath = path.join(tempDir, "source-config.json");
    const preUpdateConfig = {
      channels: {
        whatsapp: {
    ...
    await withEnvAsync(
      {
        BOT_UPDATE_POST_CORE_SOURCE_CONFIG_PATH: sourceConfigPath,
      },
      async () => {
  ...
  it("updateFinalizeCommand reapplies requested channel against post-doctor config", async () => {
    const preDoctorConfig = { update: { channel: "stable" } } as BotConfig;
    const postDoctorConfig = { update: { channel: "beta" } } as BotConfig;
    const preDoctorSnapshot: ConfigFileSnapshot = {
      ...baseSnapshot,
      sourceConfig: preDoctorConfig,
      resolved: preDoctorConfig,
  ...
  it("updateFinalizeCommand converges on the effective channel from env without persisting update.channel", async () => {
    const noChannelConfig = {} as BotConfig;
    ...
    const priorEffective = process.env.BOT_UPDATE_EFFECTIVE_CHANNEL;
    // Simulate a no-config git/source update whose effective channel is dev.
    process.env.BOT_UPDATE_EFFECTIVE_CHANNEL = "dev";
    try {
      await updateFinalizeCommand({ json: true, restart: false });
    } finally {
      if (priorEffective === undefined) {
        delete process.env.BOT_UPDATE_EFFECTIVE_CHANNEL;
      } else {
        process.env.BOT_UPDATE_EFFECTIVE_CHANNEL = priorEffective;
      }
    }
  ...
  it.each([
    ...
    },
    {
      ...
      requireTty: false,
      expectedError:
        "Update wizard requires a TTY. Use `bot update --channel <stable|extended-stable|beta|dev>` instead.",
    },
  ] as const)(
  ...
  it("uses ~/bot as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        {
          HOME: undefined,
          BOT_GIT_DIR: undefined,
          BOT_HOME: undefined,
          USERPROFILE: undefined,
        },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "bot"));
        },
      );
    } finally {
  ...
  it("uses BOT_HOME for the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    try {
      await withEnvAsync(
        { BOT_GIT_DIR: undefined, BOT_HOME: "/srv/bot-home" },
        async () => {
          expect(resolveGitInstallDir()).toBe(path.posix.join("/srv/bot-home", "bot"));
        },
      );
    } finally {
      homedirSpy.mockRestore();
  ...
  it("creates the parent directory before cloning the default dev checkout", async () => {
    const root = await createTrackedTempDir("bot-update-home-");
    const home = path.join(root, "custom-bot-home");
    const checkoutDir = path.join(home, "bot");

    await withEnvAsync({ BOT_GIT_DIR: undefined, BOT_HOME: home }, async () => {
