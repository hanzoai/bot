...
describe("update-cli", () => {
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-update-"));
    try {
      await fs.writeFile(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-update-wizard-"));
    const previousGitDir = process.env.BOT_GIT_DIR;
    try {
      setTty(true);
      ...
      process.env.BOT_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
