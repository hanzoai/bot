...
const DEFAULT_CLI_NAME = "bot";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(bot)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `bot`. */
...
/** Replace a leading `bot` command prefix with the active CLI name. */
