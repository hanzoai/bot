...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["bot channels list", "List configured channels."],
          ["bot channels list --all", "Show configured, bundled, and installable channels."],
          ["bot channels add", "Open guided channel setup."],
          ["bot channels status --probe", "Run channel status checks and probes."],
          [
            "bot channels add --channel telegram --token <token>",
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["bot channels add", "Open guided setup for available chat channels."],
          [
            "bot channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["bot channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
