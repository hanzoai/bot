...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($BOT_CONFIG_PATH, default ~/.hanzoai/bot.json):",
        ),
      );
      ...
      if (!cfg.discovery?.wideArea?.domain?.trim()) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: set discovery.wideArea.domain in the active Bot config ($BOT_CONFIG_PATH, default ~/.hanzoai/bot.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
