...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      ...
      ...(params.wrapperPath ? { BOT_WRAPPER: params.wrapperPath } : {}),
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-daemon-cli-"));
    ...
    setTestEnvValue("BOT_CONFIG_PATH", path.join(tmpDir, "bot.json"));
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).BOT_WRAPPER).toBe(
      "/usr/local/bin/bot-doppler",
    );
  });
