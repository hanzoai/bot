...
describe("markAuthProfileFailure — locked update failure", () => {
  it("drops bookkeeping without an unlocked full-store save", async () => {
    ...
    const previousTestConsole = process.env.BOT_TEST_CONSOLE;
    const previousLogLevel = process.env.BOT_LOG_LEVEL;
    ...
    try {
      ...
      if (previousTestConsole === undefined) {
        ...
        process.env.BOT_TEST_CONSOLE = previousTestConsole;
      }
      if (previousLogLevel === undefined) {
        delete process.env.BOT_LOG_LEVEL;
      } else {
        process.env.BOT_LOG_LEVEL = previousLogLevel;
      }
...
describe("markAuthProfileFailure — WHAM-aware Codex cooldowns", () => {
  ...
  it.each([
    ...
    expect(headers.originator).toBe("bot");
    expect(headers["User-Agent"]).toMatch(/^bot\//);
    const stats = store.usageStats?.["openai:default"];
