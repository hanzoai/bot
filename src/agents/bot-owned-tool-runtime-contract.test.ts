// Verifies Bot-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "bot/plugin-sdk/agent-core";
import {
...
describe("Bot-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetBotOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installBotOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installBotOwnedToolHooks();
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/bot-reply.png"]);
    expect(
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installBotOwnedToolHooks({ blockReason: "blocked by policy" });
