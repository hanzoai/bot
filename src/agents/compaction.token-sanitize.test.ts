...
import { serializeConversation, type AgentMessage } from "bot/plugin-sdk/agent-core";
...
vi.mock("bot/plugin-sdk/agent-sessions", async () => {
  const actual = await vi.importActual<typeof import("bot/plugin-sdk/agent-sessions")>(
    "bot/plugin-sdk/agent-sessions",
  );
  return {
    ...actual,
