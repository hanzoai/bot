...
/** Opening delimiter for protected Bot runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_BOT_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected Bot runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_BOT_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[BOT_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[BOT_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const BOT_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
/** Header for context attached to the immediately preceding user message. */
export const BOT_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "Bot runtime context for the immediately preceding user message.";
/** Header for runtime events passed as prompt context. */
export const BOT_RUNTIME_EVENT_HEADER = "Bot runtime event.";
/** Custom message type used for structured runtime-context messages. */
export const BOT_RUNTIME_CONTEXT_CUSTOM_TYPE = "bot.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["Bot runtime context (internal):", BOT_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === BOT_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === BOT_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === BOT_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(`${BOT_RUNTIME_EVENT_HEADER}\n${BOT_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isBotRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === BOT_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

/** Remove all structured runtime-context custom messages. */
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isBotRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isBotRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!isBotRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!isBotRuntimeContextCustomMessage(message)) {
      return true;
    }
...
export function relocateCurrentRuntimeContextCarrierToTail<T>(messages: T[]): T[] {
  ...
  if (lastIndex < 0 || !messages.some(isBotRuntimeContextCustomMessage)) {
    return messages;
  }
  ...
  while (
    ...
    isBotRuntimeContextCustomMessage(messages[firstNonCarrierFromEnd])
  ) {
    firstNonCarrierFromEnd -= 1;
  }
  const rest = messages.filter((message) => !isBotRuntimeContextCustomMessage(message));
  ...
  const carriers = messages.filter(isBotRuntimeContextCustomMessage);
