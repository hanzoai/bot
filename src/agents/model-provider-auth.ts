...
const configFingerprintCache = new WeakMap<BotConfig, string>();
...
function resolveProviderAuthConfigFingerprint(cfg: BotConfig | undefined): string | null {
  if (!cfg) {
    return null;
  }
...
export function createProviderAuthChecker(params: {
  ...
  const evaluateModelAuth = (
    ...
    const cacheKey = hasRouteFacts
      ? `${key}\0${hashRuntimeConfigValue(ref as unknown as BotConfig)}`
      : key;
...
function collectProviderAuthWarmRuntimeAuthLookups(cfg: BotConfig): {
