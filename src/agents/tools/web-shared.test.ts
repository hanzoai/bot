...
import { MAX_TIMER_TIMEOUT_SECONDS } from "@hanzo/bot-normalization-core/number-coercion";
...
describe("web cache keys", () => {
  it("keeps case-sensitive request components distinct in cache keys", () => {
    const upper = normalizeCacheKey(" fetch:https://example.com/get?marker=BotCase ");
    const lower = normalizeCacheKey("fetch:https://example.com/get?marker=botcase");

    expect(upper).toBe("fetch:https://example.com/get?marker=BotCase");
    expect(lower).toBe("fetch:https://example.com/get?marker=botcase");
