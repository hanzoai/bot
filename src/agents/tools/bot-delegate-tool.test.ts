...
import { createBotDelegateToolsForRun } from "./bot-delegate-tool.js";
...
describe("bot delegation tool", () => {
  it("relays context and surfaces pending approval", async () => {
    ...
    const tool = createBotDelegateToolsForRun({
      sessionAgentId: "main",
      runSessionKey: "agent:main:dm:one",
      agentChannel: "webchat",
    })[0];
    if (!tool) {
      throw new Error("expected Bot delegation tool");
    }

    const result = await tool.execute("call-1", { message: "Add channel." });

    expect(callGateway).toHaveBeenCalledWith("bot.chat", {
  ...
  it("uses the owner-only core gate", () => {
    expect(GATEWAY_OWNER_ONLY_CORE_TOOLS).toContain("bot");
  });
});
