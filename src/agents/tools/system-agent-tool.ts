/**
 * bot built-in tool: ring-zero setup/repair actions for the Bot
 * agent. Never exposed to normal agents — construction is bound to a host-owned
 * per-run scope, and every action funnels through Bot's typed operation
...
const SystemAgentToolSchema = Type.Object({
  ...
  target: Type.Optional(
    stringEnum(["guided", "classic", "channels", "search", "gateway"], {
      description:
        "Setup target for open_setup. channels/search/gateway open masked terminal flows; guided/classic require exiting Bot and running bot onboard.",
    }),
  ),
...
function requireParam(params: Record<string, unknown>, name: string): string {
  ...
  if (!value?.trim()) {
    throw new ToolInputError(`bot: "${name}" is required for this action`);
  }
  return value.trim();
}
...
function readSetupTarget(
  ...
  throw new ToolInputError(`bot: unknown setup target "${target}"`);
}
...
function operationForAction(params: Record<string, unknown>): SystemAgentOperation {
  ...
  switch (action) {
    ...
    case "plugin_install": {
      ...
      if (validationError) {
        throw new ToolInputError(`bot: ${validationError}`);
      }
    ...
    default:
      throw new ToolInputError(`bot: unknown action "${action}"`);
  }
}
...
export function createSystemAgentTool(options: SystemAgentToolOptions): AnyAgentTool {
  return {
    ...
    description: [
      ...
      "Provider/auth/credentials: exit; run `bot onboard`. Never request credentials.",
                ...
                : directive.kind === "gateway-config-setup"
                  ...
                  : directive.kind === "memory-import"
                    ...
                    : directive.kind === "model-setup"
                      ? `${SYSTEM_AGENT_DIRECTIVE_PREFIX} the active inference route cannot be changed inside Bot. Tell the user to exit Bot and run \`bot onboard\`; do not ask for provider credentials here.`
                        ...
                        : directive.target === "channels"
                          ...
                          : directive.target === "search"
                            ...
                            : directive.target === "gateway"
                              ...
                              : `${SYSTEM_AGENT_DIRECTIVE_PREFIX} ${directive.target} setup cannot run inside Bot because it may change the active inference route. Tell the user to exit Bot and run \`bot onboard\`.`,
          {},
        );
      }
