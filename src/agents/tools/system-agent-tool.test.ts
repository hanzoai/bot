// Bot ring-zero tool tests: approval gating, action mapping, verification.
...
describe("bot tool", () => {
  ...
  it("records interactive directives for the host without executing operations", async () => {
    ...
    expect(toolText(configureModel)).toContain(
      "active inference route cannot be changed inside Bot",
    );
    expect(toolText(configureModel)).toContain("bot onboard");
    ...
    expect(toolText(guidedSetup)).toContain("cannot run inside Bot");
    expect(toolText(guidedSetup)).toContain("bot onboard");
  ...
  it("mirrors directive transitions for out-of-process (CLI MCP) hosts", () => {
    ...
    expect(
      resolveSystemAgentDirectiveTransition({
        ...
        resultText:
          "directive: the active inference route cannot be changed inside Bot; run bot onboard.",
      }),
    ).toEqual({ kind: "model-setup", workspace: "/tmp/work" });
    expect(
      resolveSystemAgentDirectiveTransition({
        args: { action: "open_setup", target: "classic" },
        resultText: "directive: classic setup cannot run inside Bot; run bot onboard.",
      }),
