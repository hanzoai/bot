...
describe("createBotCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    ...
    const prevShellEnvTimeoutMs = process.env.BOT_SHELL_ENV_TIMEOUT_MS;
    ...
    const result = await (async () => {
      try {
        ...
        if (prevShellEnvTimeoutMs === undefined) {
          delete process.env.BOT_SHELL_ENV_TIMEOUT_MS;
        } else {
          process.env.BOT_SHELL_ENV_TIMEOUT_MS = prevShellEnvTimeoutMs;
        }
      }
    })();
