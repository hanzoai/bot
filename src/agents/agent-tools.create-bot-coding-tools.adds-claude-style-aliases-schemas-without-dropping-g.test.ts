...
import type { AgentTool, AgentToolResult } from "bot/plugin-sdk/agent-core";
...
describe("createBotCodingTools read behavior", () => {
  ...
  it("uses host decoding only for host-backed sandbox paths", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-sbx-encoding-"));
  ...
  it("maps inbound media refs to sandbox-staged media for reads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-read-media-sbx-"));
  ...
  it("returns empty content for explicit offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-read-offset-eof-"));
  ...
  it("returns empty content for adaptive offsets beyond EOF", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-read-offset-adaptive-"));
    await fs.writeFile(path.join(tmpDir, "notes.txt"), "one\ntwo\nthree\n", "utf8");
    try {
      const readTool = createSandboxedReadTool({
        root: tmpDir,
  ...
  it("returns already-read adaptive content when pagination reaches EOF", async () => {
    ...
    const readTool = createBotReadTool({
      name: "read",
      label: "read",
