...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".bot/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.bot/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with Bot result normalization. */
...
/** Wrap the base read tool with Bot paging, MIME, and image handling. */
...
export function wrapReadToolWithSkillContent(
  ...
  const virtualRead = createBotReadTool(virtualBase, options);
  return {
    ...tool,
    execute: async (toolCallId, args, signal, onUpdate) => {
      const record = getToolParamsRecord(args);
