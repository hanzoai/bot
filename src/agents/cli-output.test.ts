  ...
  it("preserves Claude max-turn terminal context for actionable run errors", () => {
    ...
    expect(
      formatCliOutputError(result!, {
        runId: "run-max-turns",
        sessionId: "bot-session-max-turns",
      }),
    ).toBe(
      "Claude CLI stopped after reaching the maximum number of turns (limit: 1). " +
        "Bot run: run-max-turns. Bot session: bot-session-max-turns. " +
...
describe("createCliJsonlStreamingParser", () => {
  ...
  it("streams Gemini message deltas and tool events", () => {
    ...
    expect(starts).toEqual([
      {
        toolCallId: "tool-1",
        name: "mcp_bot_create_goal",
        kind: "tool_use",
        args: { objective: "Update files" },
      },
    ]);
    expect(results).toEqual([
      { toolCallId: "tool-1", name: "mcp_bot_create_goal", isError: false, result: "created" },
    ]);
    expect(parser.getOutput()).toEqual({
      text: "Checking tools. Done.",
      sessionId: "gemini-session-stream",
      ...
      [
        ...
        JSON.stringify({
          ...
          event: {
            type: "content_block_delta",
            index: 0,
            delta: { type: "input_json_delta", partial_json: '{"query":"bot"}' },
          },
        }),
        JSON.stringify({
          type: "stream_event",
  ...
  it.each([
    {
      ...
      name: "web_search",
      input: { query: "bot" },
      result: [{ type: "web_search_result", title: "Bot", url: "https://example.com" }],
      isError: false,
    },
    {
