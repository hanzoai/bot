/**
 * Materializes selected Bot skills as a temporary Claude CLI plugin.
 */
...
const BOT_CLAUDE_PLUGIN_NAME = "bot-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredBotTmpDir(), "bot-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, BOT_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: BOT_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Bot skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
