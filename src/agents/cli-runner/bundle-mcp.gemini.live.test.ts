...
const LIVE = isLiveTestEnabled(["BOT_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "bot-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("bot_live_probe", "Bot Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Bot-configured streamable-http server", async () => {
    const geminiCommand = process.env.BOT_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("botLiveProbe");
