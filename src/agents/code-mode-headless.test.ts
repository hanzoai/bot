...
describe("headless Code Mode", () => {
  ...
  it("completes multi-round tool calls without publishing active runs", async () => {
    ...
    const result = expectCompleted(
      await runCodeModeScriptHeadless({
        ctx,
        code: `
          const first = await tools.callValue("bot:core:headless_first", {});
          const second = await tools.callValue("bot:core:headless_second", {
            value: first.value,
          });
          return second;
        `,
    ...
    const result = expectCompleted(
      await runCodeModeScriptHeadless({
        ...
        code: `return await Promise.race([
          tools.callValue("bot:core:headless_first_race", {}),
          tools.callValue("bot:core:headless_second_race", {}),
        ]);`,
        wallClockMs: 5_000,
      }),
    );
  ...
  it("drains a headless nested combinator after its outer race wins", async () => {
    ...
    const result = expectCompleted(
      await runCodeModeScriptHeadless({
        ...
        code: `return await Promise.race([
          Promise.all([tools.callValue("bot:core:headless_nested_race_never", {})]),
          tools.callValue("bot:core:headless_nested_race_fast", {}),
        ]);`,
        wallClockMs: 5_000,
      }),
    );
  ...
  it.each([
    {
      label: "directly",
      auditCode: 'void tools.callValue("bot:core:headless_early_audit", {});',
    },
    {
      label: "in a detached already-settled Promise.race",
      auditCode:
        'void Promise.race([tools.callValue("bot:core:headless_early_audit", {}), Promise.resolve()]);',
    },
    {
      label: "in a detached Promise.all",
      auditCode: 'void Promise.all([tools.callValue("bot:core:headless_early_audit", {})]);',
    },
    {
      label: "in a detached Promise.allSettled",
      auditCode:
        'void Promise.allSettled([tools.callValue("bot:core:headless_early_audit", {})]);',
    },
    {
      label: "in a detached Promise.any",
      auditCode: 'void Promise.any([tools.callValue("bot:core:headless_early_audit", {})]);',
    },
    {
      label: "in a detached Promise.race",
      auditCode: 'void Promise.race([tools.callValue("bot:core:headless_early_audit", {})]);',
    },
  ])(
    ...
    async ({ auditCode }) => {
      ...
      const result = expectCompleted(
        await runCodeModeScriptHeadless({
          ...
          code: `${auditCode}
          return await tools.callValue("bot:core:headless_awaited_fast", {});`,
          wallClockMs: 5_000,
        }),
      );
    ...
    const result = expectCompleted(
      await runCodeModeScriptHeadless({
        ...
        code: `return Promise.race([
          tools.callValue("bot:core:headless_race_winner", {}),
          tools.callValue("bot:core:headless_race_loser", {}),
        ]).then((value) => {
          void tools.callValue("bot:core:headless_race_audit", {});
          return value;
        });`,
        wallClockMs: 5_000,
      }),
  ...
  it("drains every detached headless tool before completing the guest", async () => {
    ...
    const result = expectCompleted(
      await runCodeModeScriptHeadless({
        ctx: createHeadlessHarness([first, second]),
        code: `void tools.callValue("bot:core:headless_detached_first", {});
          void tools.callValue("bot:core:headless_detached_second", {});
          return "done";`,
        wallClockMs: 5_000,
      }),
    );
      ...
      const result = expectCompleted(
        await runCodeModeScriptHeadless({
          ...
          code: `return await Promise.${combinator}([
            tools.callValue("bot:core:headless_fast", {}),
            tools.callValue("bot:core:headless_slow", {}),
          ]);`,
          wallClockMs: 5_000,
        }),
      );
  ...
  it("preserves headless fail-fast Promise.all while draining the slower nested tool", async () => {
    ...
    const result = expectCompleted(
      await runCodeModeScriptHeadless({
        ...
        code: `try {
          await Promise.all([
            tools.callValue("bot:core:headless_failed", {}),
            tools.callValue("bot:core:headless_slow", {}),
          ]);
  ...
  it("fails before settling tool calls beyond the total budget", async () => {
    ...
    const result = expectFailed(
      await runCodeModeScriptHeadless({
        ...
        code: `
          await tools.call("bot:core:budgeted", {});
          await tools.call("bot:core:budgeted", {});
          return true;
        `,
        maxToolCalls: 1,
  ...
  it("bounds output and returned values across separate worker legs", async () => {
    ...
    const result = expectFailed(
      await runCodeModeScriptHeadless({
        ...
        code: `
          text("x".repeat(700));
          await tools.call("bot:core:output_boundary", {});
          return "y".repeat(700);
        `,
  ...
  it("enforces one wall-clock deadline across worker and tool legs", async () => {
    ...
    const resultPromise = runCodeModeScriptHeadless({
      ...
      code: `
        await tools.call("bot:core:slow_leg", {});
        return true;
      `,
      wallClockMs: 15_000,
    });
