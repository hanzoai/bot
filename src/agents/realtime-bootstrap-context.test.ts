...
async function makeWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-realtime-bootstrap-"));
  tempDirs.push(dir);
  return dir;
}

function makeConfig(workspaceDir: string): BotConfig {
...
describe("resolveRealtimeBootstrapContextInstructions", () => {
  it("formats the default profile bootstrap files without exposing local paths", async () => {
    ...
    expect(instructions).toContain("Bot realtime voice profile context");
    ...
    expect(instructions).not.toContain("bot_agent_consult");
