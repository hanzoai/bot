...
async function runAuthContractAttempt(params: {
  ...
  const cfg = params.cfg ?? ({} as BotConfig);
...
describe("Auth profile runtime contract - embedded Bot and CLI adapter", () => {
  let tmpDir: string;
  let storePath: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-auth-contract-"));
  ...
  it("forwards a legacy OpenAI Codex auth profile through the embedded Bot path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
  ...
  it("forwards an OpenAI auth profile through the explicit embedded OpenAI Bot path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      ...
      authProfileProvider: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
      authProfileOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProfileId,
      cfg: providerRuntimeConfig(AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider, "bot"),
    });
  ...
  it("routes explicit OpenAI Bot runs with legacy Codex OAuth through OpenAI transport", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      providerOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
