// Verifies Bot plugin tools are resolved with browser/runtime context.
...
const TEST_AGENT_DIR = path.join(os.tmpdir(), "bot-plugin-tool-auth-test");
...
function firstResolvePluginToolsParams(): Record<string, unknown> {
  // Captures the plugin runtime contract passed from Bot tool resolution.
...
describe("createBotTools browser plugin integration", () => {
  afterEach(() => {
  ...
  it("keeps the browser tool returned by plugin resolution", () => {
    ...
    const tools = resolveBotPluginToolsForOptions({
      options: { config },
  ...
  it("keeps provider availability and credential resolution aligned for env-only auth", async () => {
    const envName = "BOT_PLUGIN_TOOL_AUTH_TEST_KEY";
  ...
  it("preserves ungated plugin resolution when no authoritative auth store is supplied", () => {
    hoisted.resolvePluginTools.mockReturnValue([]);
    const config = { plugins: { allow: ["browser"] } } as BotConfig;
  ...
  it("does not pass a stale active snapshot as plugin runtime config for a resolved run config", () => {
    ...
    let capturedRuntimeConfig: BotConfig | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      capturedRuntimeConfig = (params as { context?: { runtimeConfig?: BotConfig } }).context
        ?.runtimeConfig;
      return [];
    });
  ...
  it("does not let a source-less pinned config snapshot override explicit plugin tool config", () => {
    ...
    let capturedRuntimeConfig: BotConfig | undefined;
    let getRuntimeConfig: (() => BotConfig | undefined) | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      const context = (
        params as {
          context?: {
  ...
  it("exposes a live runtime config getter to plugin tool factories", () => {
    ...
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      getRuntimeConfig = (
        params as { context?: { getRuntimeConfig?: () => BotConfig | undefined } }
