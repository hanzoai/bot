...
let createBotCodingTools: typeof import("./agent-tools.js").createBotCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      BOT_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "bot-test-no-bundled-extensions"),
    },
    async () => {
      ({ createBotCodingTools } = await import("./agent-tools.js"));
    },
  );
});
...
vi.mock("bot/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
