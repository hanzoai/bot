...
const CDP_AUTH_TOKEN_ENV_KEY = "BOT_BROWSER_CDP_AUTH_TOKEN";
const SANDBOX_BROWSER_IMAGE_CONTRACT_LABEL = "org.bot.sandbox-browser.contract";
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!hasContainer) {
    ...
    args.push("-e", `BOT_BROWSER_ENABLE_NOVNC=${params.cfg.browser.noVncEnabled ? "1" : "0"}`);
    args.push("-e", `BOT_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `${CDP_AUTH_TOKEN_ENV_KEY}=${cdpAuthToken}`);
    args.push(
      "-e",
      `BOT_BROWSER_AUTO_START_TIMEOUT_MS=${params.cfg.browser.autoStartTimeoutMs}`,
    );
    if (cdpSourceRange) {
