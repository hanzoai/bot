...
describe("getBlockedBindReason", () => {
  ...
  it("still blocks OS-home credential paths when BOT_HOME points elsewhere", () => {
    withEnv({ HOME: "/home/tester", BOT_HOME: "/srv/bot-home" }, () => {
  ...
  it("blocks canonical OS-home aliases for credential paths", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "bot-home-"));
...
describe("validateBindMounts", () => {
  it("allows legitimate project directory mounts", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "bot-sbx-safe-"));
    expect(
      validateBindMounts([
  ...
  it("allows drive-absolute Windows bind sources", () => {
    expect(validateBindMounts(["D:/data/bot/src:/src:ro"])).toBeUndefined();
    expect(validateBindMounts(["D:\\data\\bot\\output:/output:rw"])).toBeUndefined();
  });

  it("compares Windows allowed roots case-insensitively", () => {
    expect(
      validateBindMounts(["d:/DATA/Bot/src:/src:ro"], {
        allowedSourceRoots: ["D:/data/bot"],
      }),
    ).toBeUndefined();
  ...
  it("blocks symlink escapes into blocked directories", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "bot-sbx-"));
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "bot-not-created");
    expect(() =>
  ...
  it("blocks bind sources outside allowed roots when allowlist is configured", () => {
    const allowedRoot = mkdtempSync(join(tmpdir(), "bot-sbx-allowed-root-"));
    const externalRoot = mkdtempSync(join(tmpdir(), "bot-sbx-external-"));
    expect(() =>
  ...
  it("allows bind sources in allowed roots when allowlist is configured", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "bot-sbx-allowed-"));
    expect(
  ...
  it("blocks reserved container target paths by default", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "bot-sbx-reserved-default-"));
    expect(() =>
  ...
  it("allows reserved container target paths with explicit dangerous override", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "bot-sbx-reserved-"));
    expect(
...
describe("validateApparmorProfile", () => {
  it("allows named profile/undefined", () => {
    expect(validateApparmorProfile("bot-sandbox")).toBeUndefined();
...
describe("validateSandboxSecurity", () => {
  it("passes with safe config", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "bot-sbx-safe-config-"));
    expect(
      validateSandboxSecurity({
