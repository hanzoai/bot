...
const {
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "bot-sandbox-registry-"));
  const previousStateDir = process.env.BOT_STATE_DIR;
  Reflect.set(process.env, "BOT_STATE_DIR", baseDir);

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_BOT_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_BOT_STATE_DIR === undefined) {
    deleteTestEnvValue("BOT_STATE_DIR");
  } else {
    setTestEnvValue("BOT_STATE_DIR", PREVIOUS_BOT_STATE_DIR);
  }
});
...
describe("registry race safety", () => {
  it("does not migrate legacy registry files from runtime reads", async () => {
    ...
    await expectPathMissing(path.join(TEST_STATE_DIR, "state", "bot.sqlite"));
  });
