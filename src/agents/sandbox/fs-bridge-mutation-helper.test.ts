...
function runWritePlan(args: string[], input?: string) {
  ...
  return spawnSync("/bin/sh", ["-c", plan.script, "bot-sandbox-fs", ...(plan.args ?? [])], {
    input,
    encoding: "utf8",
    stdio: ["pipe", "pipe", "pipe"],
  });
...
describe("sandbox pinned mutation helper", () => {
  it("writes through a pinned directory fd", async () => {
    await withTempDir({ prefix: "bot-mutation-helper-" }, async (root) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "bot-mutation-copy-" }, async (root) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "bot-mutation-copy-failure-" }, async (root) => {
        const sourceRoot = path.join(root, "source");
        const destinationRoot = path.join(root, "destination");
        await fs.mkdir(sourceRoot, { recursive: true });
