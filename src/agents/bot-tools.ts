...
import { resolveBotPluginToolsForOptions } from "./bot-plugin-tools.js";
import { filterToolsByClientCaps } from "./bot-tools.client-caps.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./bot-tools.media-factory-plan.js";
import type { ModelAwareToolContext } from "./bot-tools.model-context.js";
import { applyNodesToolWorkspaceGuard } from "./bot-tools.nodes-workspace-guard.js";
import {
  collectPresentBotTools,
  shouldIncludeAskUserToolForBotTools,
  shouldIncludeUpdatePlanToolForBotTools,
} from "./bot-tools.registration.js";
import { createBotSwarmToolGroups } from "./bot-tools.swarm.js";
...
import { createBotDelegateToolsForRun } from "./tools/bot-delegate-tool.js";
...
export { filterToolsByClientCaps } from "./bot-tools.client-caps.js";
export function createBotTools(
  ...
  const swarmToolGroups = createBotSwarmToolGroups({
    config: resolvedConfig,
  ...
  options?.recordToolPrepStage?.("bot-tools:session-workspace");
  ...
  const imageTool =
    ...
    resolveImageToolFactoryAvailable({
      ...
      : null;
  options?.recordToolPrepStage?.("bot-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("bot-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("bot-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("bot-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("bot-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("bot-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("bot-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("bot-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("bot-tools:nodes-tool");
  ...
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForBotTools({
    config: resolvedConfig,
    pluginToolDenylist: options?.pluginToolDenylist,
  });
  const includeAskUserTool = shouldIncludeAskUserToolForBotTools({
    config: resolvedConfig,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentBotTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...(includeTranscriptsTool
      ...
      : []),
    ...collectPresentBotTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool(),
          ...createBotDelegateToolsForRun({ ...options, sessionAgentId }),
        ]),
    ...
    ...collectPresentBotTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("bot-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveBotPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("bot-tools:plugin-tools");
  }

  allTools = filterToolsByClientCaps(allTools, options?.clientCaps);
  options?.recordToolPrepStage?.("bot-tools:client-capabilities");
  ...
  options?.recordToolPrepStage?.("bot-tools:tool-hooks");
  return allTools
    .map((tool) =>
