...
function nativeHookRelayStateDbArgForTests(): string {
  return `--state-db ${resolveBotStateSqlitePath()}`;
}
...
function getNativeHookRelaySharedStateForTests(): NativeHookRelaySharedStateForTests {
  ...
  )[Symbol.for("bot.nativeHookRelay.state")];
  if (!state) {
...
describe("native hook relay registry", () => {
  it("registers a short-lived relay and builds hidden CLI commands", () => {
    const relay = registerNativeHookRelay({
      ...
      ttlMs: 10_000,
      command: {
        executable: "/opt/Open Claw/bot.mjs",
    ...
    expect(relay.commandForEvent("pre_tool_use")).toBe(
      `${NATIVE_HOOK_RELAY_EXEC_PREFIX}/usr/local/bin/node '/opt/Open Claw/bot.mjs' hooks relay --provider codex --relay-id ` +
  ...
  it("restores a missing direct bridge record during renewal", async () => {
    ...
    expect(
      deleteNativeHookRelayBridgeRecordIfOwned({
        ...before,
        stateDbPath: resolveBotStateSqlitePath(),
      }),
    ).toBe(true);
  ...
  it("uses the Codex no-op output when no Bot hook decides", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      runId: "run-1",
  ...
  it("maps Codex PreToolUse to Bot before_tool_call and blocks before execution", async () => {
  ...
  it("normalizes Codex exec_command cmd input before running Bot policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("normalizes Codex exec_command argv cmd input before running Bot policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("passes config to trusted policies for native pre-tool session extension reads", async () => {
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "bot-native-relay-policy-"));
  ...
  it("uses the Codex cwd when deriving apply_patch paths for PreToolUse", async () => {
    ...
    const cwd = path.join("/tmp", "bot-native-hook-cwd");
  ...
  it("maps Codex PostToolUse to Bot after_tool_call observation", async () => {
  ...
  it("maps Codex MCP PreToolUse to Bot before_tool_call and can block", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          entities: [{ name: "Bot", entityType: "project", observations: ["test"] }],
        },
      },
    });
  ...
  it("lets security-style plugins block native MCP calls by scanning tool params", async () => {
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          command: "rm -rf /tmp/bot-important-state",
        },
      },
    });
  ...
  it("maps Codex MCP PostToolUse to Bot after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_response: {
          content: [{ type: "text", text: '{ "name": "bot" }' }],
  ...
  it("routes Codex MCP PermissionRequest payloads through Bot approval policy", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      agentId: "agent-1",
      sessionId: "session-1",
  ...
  it("defers PermissionRequest when Bot approval does not decide", async () => {
  ...
  it("does not reuse pending PermissionRequest approvals when a tool call id is reused with different input", async () => {
    ...
    const second = invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_name: "Bash",
        tool_use_id: "reused-call-id",
        tool_input: { command: "rm -rf /tmp/bot-important-state" },
      },
    });
...
describe("native hook relay command builder", () => {
  it("uses the Codex hook relay command shape", () => {
    expect(
      ...
      `${NATIVE_HOOK_RELAY_EXEC_PREFIX}bot hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event permission_request --timeout 5000`,
    );
  });
  ...
  it("execs niced relays so the Codex timeout owns the relay process", () => {
    ...
    expect(command).toBe(
      process.platform === "win32"
        ? "bot hooks relay --provider codex --relay-id relay-1 --event post_tool_use --timeout 5000"
        : "exec nice -n 10 bot hooks relay --provider codex --relay-id relay-1 --event post_tool_use --timeout 5000",
    );
  });
  ...
  it("includes explicit unavailable noop mode only for PreToolUse", () => {
    expect(
      ...
      `${NATIVE_HOOK_RELAY_EXEC_PREFIX}bot hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event pre_tool_use --pre-tool-use-unavailable noop --timeout 5000`,
    );
  });
});
