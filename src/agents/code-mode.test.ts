...
describe("Code Mode", () => {
  ...
  it("keeps a thousand-tool catalog index deterministic and within its character budget", () => {
    ...
    expect(index.length).toBeLessThanOrEqual(8_000);
    expect(index).toContain('"bot:catalog-owner:tool_0000"');
    expect(index).toContain("additional Bot/plugin tools omitted");
    expect(index).not.toContain('"bot:catalog-owner:tool_1023"');
  });
  ...
  it("returns structured values from named tools while preserving the raw call envelope", async () => {
    ...
    const details = await runUntilCompleted({
      ...
      waitTool: expectDefined(codeModeTools[1], "codeModeTools[1] test invariant"),
      code: `
        const id = "bot:fake-code-mode:fake_create_ticket";
    ...
    expect(details.value).toEqual({
      ...
      envelope: {
        tool: expect.objectContaining({
          id: "bot:fake-code-mode:fake_create_ticket",
          name: "fake_create_ticket",
        }),
