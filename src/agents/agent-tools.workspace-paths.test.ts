...
describe("workspace path resolution", () => {
  it("uses cwd for coding filesystem tools while workspaceDir remains the agent workspace", async () => {
    await withTempDir("bot-agent-ws-", async (workspaceDir) => {
      await withTempDir("bot-task-cwd-", async (cwd) => {
        const tools = createBotCodingTools({ workspaceDir, cwd });
      ...
      await withTempDir("bot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "bot" }],
          });
  ...
  it.runIf(process.platform === "win32")(
    ...
    async () => {
      await withTempDir("bot-windows-case-", async (workspaceDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("bot-ws-symlink-write-", async (workspaceDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("bot-ws-symlink-edit-", async (workspaceDir) => {
        const realDir = path.join(workspaceDir, "oc_system", "memory");
        const aliasDir = path.join(workspaceDir, "memory");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("bot-ws-symlink-escape-", async (rootDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir("bot-ws-symlink-leaf-", async (workspaceDir) => {
  ...
  it("allows workspaceOnly reads for resolved skill roots without allowing other filesystem access", async () => {
    await withTempDir("bot-skill-read-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
  ...
  it("rejects symlink escapes inside resolved skill roots", async () => {
    ...
    await withTempDir("bot-skill-read-symlink-", async (rootDir) => {
      const workspaceDir = path.join(rootDir, "workspace");
      const skillDir = path.join(rootDir, "global-skills", "demo");
      await fs.mkdir(workspaceDir, { recursive: true });
      await fs.mkdir(skillDir, { recursive: true });
