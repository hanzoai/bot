...
async function writeLegacyCompletedWorkspaceState(workspaceDir: string): Promise<void> {
  await fs.mkdir(path.join(workspaceDir, ".bot"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".bot", "workspace-state.json"),
    `${JSON.stringify({
      version: 1,
      bootstrapSeededAt: "2026-05-16T00:00:00.000Z",
      setupCompletedAt: "2026-05-16T00:00:01.000Z",
...
describe("resolveBootstrapFilesForRun", () => {
  ...
  it("keeps BOOTSTRAP.md when current setup state cannot be read", async () => {
    const workspaceDir = await makeTempWorkspace("bot-bootstrap-");
    await fs.mkdir(path.join(workspaceDir, "bot-workspace-state.json"), {
      recursive: true,
    });
    await fs.writeFile(path.join(workspaceDir, "AGENTS.md"), "rules", "utf8");
  ...
  it("ignores stale root BOOTSTRAP.md for home-relative workspace paths", async () => {
    registerBootstrapFileHook();
    const parentDir = await makeTempWorkspace("bot-bootstrap-home-");
    ...
    const files = await withEnvAsync({ BOT_HOME: parentDir }, async () =>
  ...
  it("keeps subagent sessions to AGENTS.md", async () => {
    const workspaceDir = await makeTempWorkspace("bot-bootstrap-subagent-");
    await Promise.all(
      [
  ...
  it("keeps cron sessions on their existing minimal bootstrap files", async () => {
    const workspaceDir = await makeTempWorkspace("bot-bootstrap-cron-");
    await Promise.all(
      [
        ["AGENTS.md", "project rules"],
        ["SOUL.md", "persona"],
...
describe("hasCompletedBootstrapTurn", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(await fs.realpath("/tmp"), "bot-bootstrap-turn-"));
  });
  ...
  it("returns false for SQLite transcript markers", async () => {
    expect(
      await hasCompletedBootstrapTurn("sqlite:main:session-1:/tmp/bot/sessions.json"),
    ).toBe(false);
  });
