 ...
 */
import { parseRetryAfterHttpDateMs } from "@hanzo/bot-ai/internal/retry-after";
import { emitModelTransportDebug } from "@hanzo/bot-ai/transports";
import { formatModelTransportDebugUrl } from "@hanzo/bot-ai/transports";
import {
...
function resolveMaxSdkRetryWaitSeconds(): number | undefined {
  const raw = process.env.BOT_SDK_RETRY_MAX_WAIT_SECONDS?.trim();
  if (!raw) {
