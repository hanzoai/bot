...
import { withTempHome as withBaseTempHome } from "bot/plugin-sdk/test-env";
...
describe("MCP OAuth provider", () => {
    ...
    await withTempHome(
      async () => {
        ...
        await expect(
          ...
          'MCP server "Remote Docs" requires additional OAuth authorization. Run bot mcp login Remote Docs.',
        );
      ...
      {
        prefix: "bot-mcp-oauth-insufficient-scope-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  });
  ...
  it("stops refreshing after a replacement token is rejected twice", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-terminal-rejection-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("keeps a rejected-token challenge for explicit reauthorization after refresh fails", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-rejected-token-challenge-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("uses a persisted challenge when refreshing after Doctor credential import", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-doctor-challenge-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("uses unknown-expiry tokens live but refreshes them before blind projection", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-legacy-token-",
        skipSessionCleanup: true,
        env: {
  ...
  it("requires explicit login when no native OAuth credentials exist", async () => {
    await withTempHome(
      async () => {
        ...
        ).rejects.toThrow("Run bot mcp login Remote Docs.");
        expect(authMock).not.toHaveBeenCalled();
      },
      {
        prefix: "bot-mcp-oauth-missing-token-",
        skipSessionCleanup: true,
        env: {
          BOT_CONFIG_PATH: undefined,
          BOT_STATE_DIR: undefined,
  ...
  it("marks challenge-only bootstrap state as safe for Doctor credential import", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-challenge-provenance-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("persists challenge hints without mutating an in-flight PKCE login", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-challenge-bootstrap-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("stores token state only in shared SQLite with restricted permissions", async () => {
    await withTempHome(
      async (home) => {
        ...
        const rows = openBotStateDatabase()
        ...
        await expect(fs.readdir(`${home}/.bot/mcp-oauth`)).rejects.toMatchObject({
          code: "ENOENT",
        });
  ...
  it("does not create shared state for a read-only credential status check", async () => {
    await withTempHome(
      async () => {
        ...
        await expect(fs.stat(resolveBotStateSqlitePath())).rejects.toMatchObject({
          code: "ENOENT",
        });
      },
      {
        prefix: "bot-mcp-oauth-status-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("updates provider fields atomically and clears token expiry on invalidation", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-atomic-fields-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("fails closed when canonical SQLite JSON is malformed", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-corrupt-row-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("fails closed when canonical token expiry has no token state", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-orphan-expiry-",
        skipSessionCleanup: true,
        env: { BOT_CONFIG_PATH: undefined, BOT_STATE_DIR: undefined },
      },
    );
  ...
  it("does not persist localhost when the fallback attempt fails", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-localhost-failure-",
        skipSessionCleanup: true,
        env: {
          BOT_CONFIG_PATH: undefined,
          BOT_STATE_DIR: undefined,
  ...
  it("persists localhost redirect for a later code exchange login", async () => {
    await withTempHome(
      ...
      {
        prefix: "bot-mcp-oauth-localhost-persist-",
        skipSessionCleanup: true,
        env: {
          BOT_CONFIG_PATH: undefined,
          BOT_STATE_DIR: undefined,
  ...
  it("does not start hidden authorization flows without an authorization callback", async () => {
    ...
    await withTempHome(
      async () => {
        ...
        expect(() => provider.state?.()).toThrow("Run bot mcp login Remote Docs.");
        expect(() => provider.saveCodeVerifier?.("verifier")).toThrow(
          "Run bot mcp login Remote Docs.",
        );
        await expect(
      ...
      {
        prefix: "bot-mcp-oauth-noninteractive-",
        skipSessionCleanup: true,
        env: {
          BOT_CONFIG_PATH: undefined,
          BOT_STATE_DIR: undefined,
