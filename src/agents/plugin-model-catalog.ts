...
type PluginModelCatalogDatabase = Pick<BotAgentKyselyDatabase, "cache_entries">;
...
function repairPersistedPluginModelCatalogs(params: {
  ...
  const applied = runBotAgentWriteTransaction(
    (database) => {
      const kysely = getNodeSqliteKysely<PluginModelCatalogDatabase>(database.db);
...
/** Detects model catalogs generated by Bot rather than user-authored JSON. */
