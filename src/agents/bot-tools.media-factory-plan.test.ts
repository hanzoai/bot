...
type CreateBotToolsOptions = Parameters<
  typeof import("./bot-tools.js").createBotTools
>[0];
let createBotToolsForTestModule: typeof import("./bot-tools.js").createBotTools;
let legacyComfyToolNames: string[];

async function createBotToolsForTest(options?: CreateBotToolsOptions) {
  return createBotToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/bot.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): BotConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createBotTools: createBotToolsForTestModule } =
      await import("./bot-tools.js"));
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: BotConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createBotToolsForTest({
      config,
      agentDir: "/tmp/bot-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: BotConfig = {};
    const workspaceDir = "/tmp/bot-workspace";
