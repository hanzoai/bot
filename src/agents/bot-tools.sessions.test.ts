...
import "./test-helpers/fast-bot-tools-sessions.js";
...
function createBotTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "bot", model: "gateway-injected" }),
    );
    expect(details.messages).toContainEqual(
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.botReasoningReplay).toBeUndefined();
  });
  ...
  it("keeps scoped sends from creating post-return work or durable watches", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-scoped-session-send-"));
    const storePath = path.join(tmpDir, "sessions.json");
