...
describe("acquireSessionWriteLock", () => {
  ...
  it("keeps a transcript-target lease shared across different state roots", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "bot-lock-target-store-"));
    ...
    try {
      ...
      setTestEnvValue("BOT_STATE_DIR", path.join(root, "state-a"));
      first = await acquireSessionWriteLock({ sessionFile, targetKind: "session-key" });

      setTestEnvValue("BOT_STATE_DIR", path.join(root, "state-b"));
  ...
  it("uses one lease key for legacy and SQLite locators of the same store", () => {
    ...
    expect(
      ...
      resolveSessionWriteLockTargetKey({
        ...base,
        storePath: "/tmp/main/bot-agent.sqlite",
      }),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "bot-lock-key-symlink-"));
      try {
  ...
  it("preserves a fresh session lock that replaces the stale sweep candidate", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "bot-lock-replacement-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    ...
    try {
      const result = await cleanStaleLockFiles({
        ...
        readOwnerProcessArgs: (pid) =>
          pid === replacementOwner.pid
            ? ["node", "/opt/hanzoai/bot.mjs", "agent"]
            : ["python", "worker.py"],
      });
