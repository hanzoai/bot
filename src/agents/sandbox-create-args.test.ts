...
describe("buildSandboxCreateArgs", () => {
  ...
  it("includes hardening and resource flags", () => {
    ...
    expectFlagValues(args, "--name", ["bot-sbx-test"]);
    expectFlagValues(args, "--label", [
      "bot.sandbox=1",
      "bot.sessionKey=main",
      "bot.createdAtMs=1700000000000",
      `bot.createArgsEpoch=${SANDBOX_DOCKER_CREATE_ARGS_EPOCH}`,
    ...
    expectFlagValues(args, "--env", ["LANG=C.UTF-8", `BOT_CLI=${BOT_CLI_ENV_VALUE}`]);
  ...
  it("omits non-finite numeric Docker resource flags", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "bot-sbx-non-finite-limits",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
  ...
  it.each([
    ...
    },
    {
      ...
      containerName: "bot-sbx-covers-docker-socket-dir",
