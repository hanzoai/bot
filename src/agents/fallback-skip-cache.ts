...
import { parseStrictNonNegativeInteger } from "@hanzo/bot-normalization-core/number-coercion";
 ...
 * BOT_FALLBACK_SKIP_TTL_MS.
 */
const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "BOT_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    botFallbackSkipCache?: SkipBySession;
    botFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.botFallbackSkipCacheState) {
    ...
    const buckets = globalStore.botFallbackSkipCache ?? new Map();
    globalStore.botFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.botFallbackSkipCache = buckets;
  }
  return globalStore.botFallbackSkipCacheState;
}
