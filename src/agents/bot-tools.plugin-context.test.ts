...
import { resolveBotPluginToolInputs } from "./bot-tools.plugin-context.js";
import { applyPluginToolDeliveryDefaults } from "./plugin-tool-delivery-defaults.js";
import type { AnyAgentTool } from "./tools/common.js";

describe("bot plugin tool context", () => {
  it("forwards trusted requester sender identity", () => {
    const result = resolveBotPluginToolInputs({
      options: {
        config: {} as never,
  ...
  it("defaults missing and unknown conversation-read origins to delegated", () => {
    const missing = resolveBotPluginToolInputs({
      options: { config: {} as never },
    });
    const unknown = resolveBotPluginToolInputs({
      options: {
        config: {} as never,
