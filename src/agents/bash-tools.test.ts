...
vi.mock("../process/supervisor/index.js", () => {
  ...
  const applySegmentShellEffects = (segment: string, env: NodeJS.ProcessEnv) => {
    if (segment === 'export PATH="${BOT_PREPEND_PATH}${PATH:+:$PATH}"') {
      const prepend = env.BOT_PREPEND_PATH ?? "";
      const current = readEnvPath(env);
      writeEnvPath(env, `${prepend}${current ? `:${current}` : ""}`);
      return;
    }
    if (segment === "unset BOT_PREPEND_PATH") {
      delete env.BOT_PREPEND_PATH;
    }
  };
...
const defaultShell = isWin
  ? undefined
  : process.env.BOT_TEST_SHELL || getBashShellConfig().shell;
...
const SHELL_ENV_KEYS = ["BOT_EXEC_SHELL_SNAPSHOT", "SHELL"] as const;
const PATH_SHELL_ENV_KEYS = ["BOT_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL"] as const;
