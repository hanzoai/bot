...
const BOT_ABORTABLE_WRAPPER = Symbol.for("bot.abortable.wrapper");

export function isBotAbortableWrapper(err: unknown): boolean {
  return err !== null && typeof err === "object" && BOT_ABORTABLE_WRAPPER in err;
}

function tagAsAbortableWrapper(err: Error): Error {
  (err as Error & { [BOT_ABORTABLE_WRAPPER]?: true })[BOT_ABORTABLE_WRAPPER] = true;
  return err;
}
