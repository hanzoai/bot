...
import { withFirstStreamEventTimeout } from "@hanzo/bot-ai/internal/runtime";
...
function createProviderConfig(baseUrl: string, timeoutSeconds?: number): BotConfig {
  return {
    models: {
      providers: {
        [PROVIDER]: {
...
async function resolveProviderModel(params: {
  ...
  const result = await resolveModelAsync(
    PROVIDER,
    MODEL_ID,
    "/tmp/bot-provider-timeout-test-agent",
