...
export async function executePreparedCompactionSession(runtime: PreparedCompactionRuntime) {
  ...
  try {
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Bot
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores Bot runtime's auto-compaction (bot#75799), so re-apply
      ...
      // exact Bot-managed registrations so custom tools survive startup.
