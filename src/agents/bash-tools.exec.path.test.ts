...
const ENV_KEYS = ["BOT_EXEC_SHELL_SNAPSHOT", "PATH", "SHELL", "SSLKEYLOGFILE"] as const;
...
vi.mock("../process/supervisor/index.js", () => ({
  getProcessSupervisor: () => ({
    spawn: async (input: {
      ...
      const env = input.env ?? {};
      if (command.includes("BOT_SHELL")) {
        input.onStdout?.(env.BOT_SHELL ?? "");
...
describe("exec PATH login shell merge", () => {
  ...
  beforeEach(() => {
    ...
    process.env.BOT_EXEC_SHELL_SNAPSHOT = "0";
  ...
  it("strips malformed XML arg-value suffixes from exec command and routing options", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-exec-xml-"));
    try {
  ...
  it("fails without running when an explicit workdir is unavailable", async () => {
    const missingWorkdir = path.join(
      os.tmpdir(),
      `bot-missing-workdir-${process.pid}-${Date.now()}`,
    );
  ...
  it("sets BOT_SHELL for host=gateway commands", async () => {
    if (isWin) {
      return;
    }

    const tool = createExecTool({ host: "gateway", security: "full", ask: "off" });
    const result = await tool.execute("call-bot-shell", {
      command: 'printf "%s" "${BOT_SHELL:-}"',
      yieldMs: FOREGROUND_TEST_YIELD_MS,
    });
    const value = normalizeText(result.content.find((c) => c.type === "text")?.text);
...
describe("exec host env validation", () => {
  ...
  it.each([
    ...
    "exec -a bot /approve abc123 deny",
    ...
    "sudo BOT_APPROVE=1 /approve abc123 allow-once",
    "sudo -uroot bash -lc '/approve abc123 allow-once'",
    "sudo -u root BOT_APPROVE=1 bash -lc '/approve abc123 allow-once'",
