...
describe.skipIf(process.platform === "win32")("shell snapshot subprocesses", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "BOT_EXEC_SHELL_SNAPSHOT", "BOT_STATE_DIR"]);
    spawnMock.mockReset();
  ...
  it("does not create output pipes for status-only shell commands", async () => {
    ...
    const home = tempDirs.make("bot-snapshot-spawn-home-");
    const stateDir = tempDirs.make("bot-snapshot-spawn-state-");
