/**
 * Prompt-surface helpers for Bot tool guidance.
 *
 * Maps runtime/session surfaces to the fallback tool text and workflow hints that belong in prompts.
 */
import { isBotMainPromptSurface } from "../plugins/agent-prompt-surface-kind.js";
import type { AgentPromptSurfaceKind } from "../plugins/types.js";
import { isAcpSessionKey, isSubagentSessionKey } from "../routing/session-key.js";

/** Builds fallback tool guidance when a runtime cannot render the structured tool list. */
export function buildBotToolFallbackText(params: {
  surface: AgentPromptSurfaceKind;
  execToolName: string;
  processToolName: string;
}): string {
  if (isBotMainPromptSurface(params.surface)) {
    return [
      "Bot lists the standard tools above. This runtime enables:",
  ...
  return "No Bot tool list is injected for this runtime prompt surface. Use only tools exposed directly by the active backend.";
}

/** Returns whether the main Bot prompt should include workflow hints around the tool list. */
export function shouldRenderBotToolWorkflowHints(params: {
  surface: AgentPromptSurfaceKind;
  hasToolList: boolean;
}): boolean {
  return isBotMainPromptSurface(params.surface);
}
...
export function resolveAgentPromptSurfaceForSessionKey(
  ...
  return sessionKey && isSubagentSessionKey(sessionKey) ? "subagent" : "bot_main";
}
