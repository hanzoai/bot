...
import type { AssistantMessage, Message, Tool } from "bot/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.BOT_CACHE_TRACE,
      file: process.env.BOT_CACHE_TRACE_FILE,
      messages: process.env.BOT_CACHE_TRACE_MESSAGES,
      prompt: process.env.BOT_CACHE_TRACE_PROMPT,
      system: process.env.BOT_CACHE_TRACE_SYSTEM,
    };
    setTestEnvValue("BOT_CACHE_TRACE", "1");
    setTestEnvValue("BOT_CACHE_TRACE_FILE", liveCacheTraceFile);
    setTestEnvValue("BOT_CACHE_TRACE_MESSAGES", "0");
    setTestEnvValue("BOT_CACHE_TRACE_PROMPT", "0");
    setTestEnvValue("BOT_CACHE_TRACE_SYSTEM", "0");
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      const restore = (
        key:
          | "BOT_CACHE_TRACE"
          | "BOT_CACHE_TRACE_FILE"
          | "BOT_CACHE_TRACE_MESSAGES"
          | "BOT_CACHE_TRACE_PROMPT"
          | "BOT_CACHE_TRACE_SYSTEM",
      ...
      restore("BOT_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("BOT_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("BOT_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("BOT_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("BOT_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "BOT_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "BOT_LIVE_ANTHROPIC_CACHE_MODEL",
