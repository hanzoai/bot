/**
 * Broad coverage for createBotCodingTools.
...
import * as botPluginTools from "./bot-plugin-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createBotCodingTools>) {
...
type BotCodingTool = ReturnType<typeof createBotCodingTools>[number];
type BotToolsOptions = NonNullable<Parameters<typeof createBotTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: BotCodingTool[], name: string): BotCodingTool {
...
function requireToolExecute(tool: BotCodingTool): NonNullable<BotCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateBotToolsOptions(): BotToolsOptions {
  const calls = vi.mocked(createBotTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createBotTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: BotToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
...
describe("createBotCodingTools", () => {
  ...
  it("exposes only gateway config reads to owner sessions", () => {
    const tools = createBotCodingTools({ config: testConfig });
  ...
  it("passes explicit channel and requester facts to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createBotTools).mockReturnValueOnce([wrapped as never]);

    const tools = createBotCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core Bot tools are not materialized", () => {
    const createBotToolsMock = vi.mocked(createBotTools);
    createBotToolsMock.mockClear();
    ...
    expect(createBotToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("keeps the injected ring-zero tool under policy and rejects a same-name replacement", () => {
    ...
    vi.mocked(createBotTools).mockReturnValueOnce([duplicateTool]);

    const tools = runWithAgentRingZeroTools([injectedTool], () =>
      createBotCodingTools({
        config: { tools: { allow: ["read"], deny: ["bot"] } },
        runtimeToolAllowlist: ["bot"],
        toolConstructionPlan: {
          includeBaseCodingTools: false,
          includeShellTools: false,
          includeChannelTools: false,
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateBotToolsOptions();
  ...
  it("does not inherit native-harness bridge runtime allowlists", () => {
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateBotToolsOptions().pluginToolAllowlist).toEqual([
      "sessions_spawn",
      "memory_search",
    ]);
    expect(latestCreateBotToolsOptions().inheritedToolAllowlist).toEqual([]);
  });
  ...
  it("inherits embedded runtime toolsAllow when explicitly marked as parent capability", () => {
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateBotToolsOptions().inheritedToolAllowlist;
  ...
  it("keeps restricted spawn inheritance in the caller-owned runtime snapshot", () => {
    ...
    expect(latestCreateBotToolsOptions().inheritedToolAllowlist).toBe(
      inheritedToolAllowlistRef,
    );
  ...
  it("does not snapshot additive alsoAllow policies for spawn inheritance", () => {
    ...
    expect(latestCreateBotToolsOptions().inheritedToolDenylist).toContain("exec");
  });
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:bot"], ["*"]]) {
      const tools = createBotCodingTools({
        config: { tools: { profile: "minimal" } },
        runtimeToolAllowlist,
        toolConstructionPlan: {
  ...
  it("passes source reply delivery mode to Bot tool construction", () => {
    const createBotToolsMock = vi.mocked(createBotTools);
    createBotToolsMock.mockClear();
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateBotToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });

  it("passes configured filesystem policy to Bot tool construction", () => {
    const createBotToolsMock = vi.mocked(createBotTools);
    createBotToolsMock.mockClear();
    ...
    expect(latestCreateBotToolsOptions().fsPolicy).toEqual({ workspaceOnly: true });
  });
  ...
  it("uses the canonical spawn workspace for follow-up task suggestions", () => {
    ...
    expect(latestCreateBotToolsOptions()).toMatchObject({
      workspaceDir: "/agent/workspace",
      spawnWorkspaceDir: "/host/project",
      cwd: "/host/project",
    });
  ...
  it("keeps an unsandboxed task repo as the follow-up suggestion cwd", () => {
    ...
    expect(latestCreateBotToolsOptions().cwd).toBe("/task/repo");
  });
  ...
  it("passes plugin suppression into Bot tool construction plans", () => {
    const createBotToolsMock = vi.mocked(createBotTools);
    createBotToolsMock.mockClear();
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateBotToolsOptions().disablePluginTools).toBe(true);
  });

  it("forwards trusted conversation recall to Bot tool construction", () => {
    const createBotToolsMock = vi.mocked(createBotTools);
    createBotToolsMock.mockClear();
    ...
    expect(latestCreateBotToolsOptions().conversationRecall).toEqual(conversationRecall);
  });

  it("keeps plugin-only construction off the Bot core factory", () => {
    const createBotToolsMock = vi.mocked(createBotTools);
    createBotToolsMock.mockClear();
  ...
  it("forwards prepared run facts to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(botPluginTools, "resolveBotPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("forwards the native channel id through standard tool construction", () => {
    ...
    expect(latestCreateBotToolsOptions().nativeChannelId).toBe("oc_native_chat");
    expect(latestCreateBotToolsOptions().currentChatType).toBe("group");
    expect(latestCreateBotToolsOptions().messageActionTurnCapability).toBe(
      "turn-capability-1",
    );
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateBotToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
  ...
  it("materializes additive runtime tools while preserving normal deny policy", () => {
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateBotToolsOptions().pluginToolAllowlist).not.toContain(
      "workboard_heartbeat",
    );
    expect(
      isPluginToolAllowed(
        new Set(latestCreateBotToolsOptions().pluginToolAllowlist),
        "workboard",
        "workboard_heartbeat",
      ),
    ).toBe(true);
    expect(latestCreateBotToolsOptions()).not.toHaveProperty("runtimePluginToolGrant");
    expectListIncludes(latestCreateBotToolsOptions().pluginToolDenylist, ["workboard_block"]);
  });

  it("passes explicit denylist entries to Bot tool factory planning", () => {
    const createBotToolsMock = vi.mocked(createBotTools);
    createBotToolsMock.mockClear();
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateBotToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to Bot plugin discovery", async () => {
    const createBotToolsMock = vi.mocked(createBotTools);
    createBotToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `bot-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateBotToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateBotToolsOptions().cronCreatorToolAllowlist;
  ...
  it("passes the final unrestricted tool surface to cron-created agent turns", () => {
    ...
    createBotCodingTools({ config: {} });

    expect(createBotToolsMock).toHaveBeenCalledTimes(1);
    const cronAllowNames = cronCreatorToolNames(
      latestCreateBotToolsOptions().cronCreatorToolAllowlist,
    );
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      BotToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("bot-tools:test-helper")).toBeLessThan(
      stages.indexOf("bot-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createBotCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const codexTools = createBotCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: BotConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createBotCodingTools({
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createBotTools).mockClear();
    ...
    expect(latestCreateBotToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createBotCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createBotCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createBotCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createBotCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createBotCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createBotCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createBotCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-canonical-"));
    try {
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "bot-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-legacy-alias-"));
    try {
      const tools = createBotCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
