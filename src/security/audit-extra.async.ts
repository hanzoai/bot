  ...
  if (staleEpoch.length > 0) {
    findings.push({
      ...
      detail:
        ...
        `Expected bot.browserConfigEpoch=${browserHashEpoch}.`,
      remediation: `${formatCliCommand("bot sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
...
function buildSandboxBrowserDockerProbeTimeoutFinding(timeoutMs: number): SecurityAuditFinding {
  return {
    ...
    detail:
      ...
      "Bot skipped any remaining sandbox browser container drift checks for this status run.",
    remediation:
    ...
    try {
      ...
      findings.push({
        ...
        detail:
          ...
          "The extension entrypoint list is unavailable. Deep scan will cover the plugin directory but may miss entries declared via `bot.extensions`.",
        remediation:
          "Inspect the plugin package.json for syntax errors. If the plugin is untrusted, remove it from your Bot extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (resolveSkillSource(entry.skill) === "bot-bundled") {
        continue;
      }
