...
export function collectAttackSurfaceSummaryFindings(cfg: BotConfig): SecurityAuditFinding[] {
  ...
  const detail =
    ...
    `\n` +
    "trust model: personal assistant (one trusted operator boundary), not hostile multi-tenant on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.bot.ai/gateway/multi-tenant-hosting";
