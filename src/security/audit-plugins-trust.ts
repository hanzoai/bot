...
function readChannelCommandSetting(
  ...
  const channelCfg = cfg.channels?.[channelId as keyof NonNullable<BotConfig["channels"]>];
  ...
  if (pluginDirs.length > 0) {
    ...
    if (enabledExtensionPluginIds.length > 0) {
      ...
      for (const context of contexts) {
        ...
        const broadPolicy = deps.isToolAllowedByPolicies("__bot_plugin_probe__", policies);
