...
describe("security audit config include permissions", () => {
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = tempDirs.make("bot-include-perms-");
    ...
    const configSnapshot: ConfigFileSnapshot = {
      path: path.join(stateDir, "bot.json"),
      exists: true,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = tempDirs.make("bot-include-perms-allowed-");
      ...
      const configSnapshot: ConfigFileSnapshot = {
        path: path.join(configDir, "bot.json"),
        exists: true,
      ...
      const findings = await collectIncludeFilePermFindings({
        configSnapshot,
        env: { BOT_INCLUDE_ROOTS: sharedDir },
        platform: "linux",
      });
