...
function formatHooksTokenReuseRemediation(reuse: GatewayAuthSharedSecretReuse): string {
  ...
  return `Run ${formatCliCommand("bot doctor --fix")} to rotate a persisted hooks.token, then update external hook senders to use the new hook token.`;
}
...
function listOpenInboundPolicies(cfg: BotConfig): string[] {
  const out: string[] = [];
...
function listAuditAgentToolContexts(cfg: BotConfig): AuditAgentToolContext[] {
...
function collectControlPlaneToolExposureContexts(cfg: BotConfig): string[] {
...
export function collectLikelyMultiUserSetupFindings(cfg: BotConfig): SecurityAuditFinding[] {
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "Bot's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.bot.ai/gateway/multi-tenant-hosting",
    remediation:
