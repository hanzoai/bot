...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[bot proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[bot proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`bot proxy coverage\` for details.`,
  );
}
