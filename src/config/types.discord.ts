...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the Bot agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus Bot, or the agent id plus Bot. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the Bot agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which Bot conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing Bot Discord conversation. */
