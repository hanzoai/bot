...
const DEFAULT_MODEL_ALIASES: Readonly<Record<string, string>> = {
  ...
  "sonnet-4.6": "hanzo/claude-sonnet-4-6",
...
function resolveAnthropicDefaultAuthMode(cfg: BotConfig): AnthropicAuthDefaultsMode | null {
...
export function applyMessageDefaults(cfg: BotConfig): BotConfig {
...
export function applyTalkApiKey(config: BotConfig): BotConfig {
...
export function applyModelDefaults(cfg: BotConfig): BotConfig {
  let mutated = false;
  let nextCfg = cfg;
...
export function applyAgentDefaults(cfg: BotConfig): BotConfig {
...
export function applyLoggingDefaults(cfg: BotConfig): BotConfig {
...
export function applyContextPruningDefaults(cfg: BotConfig): BotConfig {
...
export function applyCompactionDefaults(cfg: BotConfig): BotConfig {
  const defaults = cfg.agents?.defaults;
  if (!defaults) {
    return cfg;
  }
