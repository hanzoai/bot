// Defines the top-level Bot configuration type.
...
/** Top-level Bot config as read from user/project config files. */
export type BotConfig = {
  ...
  $schema?: string;
  meta?: {
    /** Last Bot version that wrote this config. */
  ...
  ui?: {
    /** Accent color for Bot UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type BotConfigInput = Omit<BotConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const botConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = BotConfig & {
  readonly [botConfigStateBrand]?: TState;
};
