...
function applyConfigEnv(cfg: BotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): BotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as BotConfig, deps.env);
      }
  ...
  async function writeConfigFile(cfg: BotConfig) {
    clearConfigCache();
...
export async function writeConfigFile(cfg: BotConfig): Promise<void> {
  clearConfigCache();
