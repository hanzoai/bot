...
async function tryWriteSingleTopLevelIncludeMutation(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
}): Promise<{ persistedHash: string | null; persistedConfig: BotConfig } | null> {
...
function resolveConfigWriteResult(
  ...
  fallbackConfig: BotConfig,
): { persistedHash: string | null; persistedConfig: BotConfig } {
  if (result) {
    return {
...
export async function mutateConfigFile<T = void>(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
  mutate: (draft: BotConfig, context: ConfigMutationContext) => Promise<T | void> | T | void;
}): Promise<ConfigMutationResult<T>> {
