...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: BotConfig): boolean {
...
function isProviderConfigured(cfg: BotConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: BotConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: BotConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: BotConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: BotConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: BotConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: BotConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredSpeechProviderSelection(cfg: BotConfig): boolean {
  return collectConfiguredSpeechProviderIds(cfg).size > 0;
}

function hasConfiguredPluginConfigEntry(cfg: BotConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: BotConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: BotConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: BotConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: BotConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: BotConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: BotConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: BotConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: BotConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: BotConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: BotConfig): boolean {
  return (
...
function hasPluginEntries(cfg: BotConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: BotConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: BotConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: BotConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: BotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: BotConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: BotConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: BotConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: BotConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
