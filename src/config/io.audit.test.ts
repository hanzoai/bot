...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".bot", "bot.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "bot-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".bot", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      ...
      "Config overwrite: /tmp/bot.json (sha256 prev-hash -> next-hash, backup=/tmp/bot.json.bak, changedPaths=3)",
    );
  });
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/bot.json");
  ...
  it("redacts structured audit records before persistence", async () => {
    ...
    const record = finalizeConfigWriteAuditRecord({
      base: {
        ...createAuditRecordBase(path.join(home, ".bot", "bot.json")),
        suspicious: [
  ...
  it("redacts processInfo.argv when explicitly supplied to createConfigWriteAuditRecordBase", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      processInfo: {
        ...
        ppid: 1,
        cwd: "/work",
        argv: ["node", "bot", "--token", "leaked-but-not-anymore-12345"],
        execArgv: [],
      },
    });
    expect(base.argv).toEqual(["node", "bot", "--token", "***"]);
  });

  it.each([
    {
      name: "inline known secret",
      argv: ["bot", "--token=fake", "--port=8080"],
      expected: ["bot", "--token=***", "--port=8080"],
    },
    {
      name: "custom credential suffix",
      argv: ["bot", "--tenant-credential", "fake", "--bind", "lan"],
      expected: ["bot", "--tenant-credential", "***", "--bind", "lan"],
    },
    {
      name: "underscore key suffix",
      argv: ["bot", "--provider_api_key", "fake"],
      expected: ["bot", "--provider_api_key", "***"],
    },
    {
      name: "dash-leading secret value",
      argv: ["bot", "--password", "-fake"],
      expected: ["bot", "--password", "***"],
    },
    {
    ...
    },
    {
      ...
      argv: ["bot", "config", "set", "channels.slack.token", "secret-value"],
      expected: ["bot", "config", "set", "channels.slack.token", "***"],
    },
    {
      name: "sensitive config set value after boolean option",
      argv: ["bot", "config", "set", "--json", "channels.slack.token", '"secret-value"'],
      expected: ["bot", "config", "set", "--json", "channels.slack.token", "***"],
    },
    {
      ...
      expected: ["bot", "config", "set", "--profile", "work", "channels.slack.token", "***"],
    },
    {
      ...
      expected: ["bot", "config", "--profile", "work", "set", "channels.slack.token", "***"],
    },
    {
    ...
    },
    {
      ...
      expected: ["bot", "--profile", "config", "config", "set", "channels.slack.token", "***"],
    },
    {
      ...
      expected: ["bot", "config", "set", "channels.slack.token", "--strict-json", "***"],
    },
    {
      ...
      expected: ["bot", "config", "set", "--batch-json", "***"],
    },
  ])("redacts $name in persisted audit process info", ({ argv, expected }) => {
    expect(createAuditRecordBase("/tmp/bot.json", argv).argv).toEqual(expected);
  });
  ...
  it("rewrites historical config-audit entries through redactConfigAuditArgv and preserves 0600 mode", async () => {
    ...
    const unredactedRecord = {
      ...
      source: "config-io",
      event: "config.write",
      configPath: path.join(home, ".bot", "bot.json"),
      pid: 1590563,
      ppid: 1590548,
      cwd: home,
      argv: [
        "/usr/bin/node",
        "/usr/local/bin/bot.mjs",
        "config",
        "set",
    ...
    const alreadyRedactedRecord = {
      ...
      ppid: 1,
      cwd: home,
      argv: ["/usr/bin/node", "/usr/local/bin/bot.mjs", "config", "set", "ui.theme", "dark"],
      execArgv: ["--disable-warning=ExperimentalWarning"],
      suspicious: [],
      result: "rename",
    };
  ...
  it("preserves malformed lines verbatim and counts them as skipped", async () => {
    ...
    const validUnredacted = {
      ts: "2026-05-02T00:03:48.471Z",
      argv: ["node", "bot.mjs", "config", "set", "x", "xoxb-bad-token-1234567890abcdef"],
    };
  ...
  it("aborts without overwriting when the audit log is appended to after temp write", async () => {
    ...
    const appended = {
      ...
      argv: ["node", "bot.mjs", "config", "set", "theme", "dark"],
      execArgv: [],
    };
    const original = `${JSON.stringify(unredacted)}\n`;
