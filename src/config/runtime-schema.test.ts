...
const mockLoadConfig = vi.hoisted(() => vi.fn<() => BotConfig>());
...
function explicitMainRoster(): BotConfig {
...
function makeSnapshot(params: { valid: boolean; config?: BotConfig }): ConfigFileSnapshot {
  return {
    path: "/tmp/bot.json",
    exists: true,
    raw: "{}",
