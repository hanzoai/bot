...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, Bot env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still Bot-managed keys:
 ...
 * literal references for keys Bot previously managed.
 */
...
/**
 * Read and parse `~/.bot/.env` (or `$BOT_STATE_DIR/.env`), returning
