...
import { resetConfigRuntimeState, type BotConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        BOT_MPM_CATALOG_PATHS: undefined,
...
export async function writeBotConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.BOT_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected BOT_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeBotConfig(home, config);
