...
describe("BotSchema talk validation", () => {
  ...
  it.each([
    ...
    expect(() => BotSchema.parse({ talk: { realtime } })).toThrow();
  });
  ...
  it.each(["constructor", "__proto__"])(
    ...
    (provider) => {
      ...
      expect(BotSchema.safeParse({ talk: { provider, providers } }).success).toBe(false);
      expect(
        BotSchema.safeParse({ talk: { realtime: { provider, providers } } }).success,
      ).toBe(false);
    },
  );
