...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real bot.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
