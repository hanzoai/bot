...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "bot-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    return withEnvAsync(
      {
        BOT_DEFER_SHELL_ENV_FALLBACK: undefined,
        BOT_LOAD_SHELL_ENV: undefined,
        BOT_SHELL_ENV_TIMEOUT_MS: undefined,
      },
      () => fn(home),
    );
  }
  ...
  const configPathForHome = (home: string, fileName = "bot.json") =>
    path.join(home, ".bot", fileName);

  const formatConfig = (config: unknown) => `${JSON.stringify(config, null, 2)}\n`;

  const readPersistedConfig = async (configPath: string): Promise<BotConfig> =>
    JSON.parse(await fs.readFile(configPath, "utf-8")) as BotConfig;
  ...
  const createFastConfigIO = (home: string, options: ConfigIoOptions = {}) =>
    createHomeConfigIO(home, {
      env: { BOT_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      ...options,
    });
  ...
  itWithHome("writes health state to SQLite through public config reads", async (home) => {
    const configPath = configPathForHome(home);
    const healthPath = path.join(home, ".bot", "logs", "config-health.json");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  itWithHome(
    ...
    async (home) => {
      const warn = vi.fn();
      const io = createHomeConfigIO(home, {
        env: { HOME: home, BOT_TEST_FAST: "1" } as NodeJS.ProcessEnv,
        logger: { warn, error: vi.fn() },
      });
      const staleConfig = {
  ...
  itWithHome(
    "keeps writes inside an BOT_STATE_DIR override even when the real home config exists",
    async (home) => {
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "bot.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "bot.json"), "utf-8"),
      ) as {
  ...
  itWithHome("drops keys that exist only on the next-config prototype", async (home) => {
    ...
    await io.writeConfigFile(nextConfig as BotConfig);
  ...
  const writeAuditCases: readonly WriteAuditCase[] = [
    ...
    },
    {
      ...
      seedExistingConfig: true,
      env: { BOT_TEST_CONFIG_WRITE_LOG: "1" },
      logPrefix: "Config write anomaly:",
    ...
    },
    {
      ...
      seedExistingConfig: true,
      env: { VITEST: "true", BOT_TEST_CONFIG_WRITE_LOG: "1" },
      logPrefix: "Config overwrite:",
      skipOutputLogs: true,
    },
  ];
  ...
  for (const auditCase of writeAuditCases) {
    itWithHome(auditCase.name, async (home) => {
      ...
      const config: BotConfig = auditCase.seedExistingConfig
  ...
  itWithHome("preserves root $schema during partial writes", async (home) => {
    ...
    expect(persisted.$schema).toBe("https://bot.ai/config.json");
  ...
  itWithHome("rejects destructive internal writes before replacing the config", async (home) => {
    ...
    const original = {
      ...
      agents: { entries: { main: { default: true, workspace: "/tmp/bot-main" } } },
  ...
  itWithHome(
    ...
    async (home) => {
      await writeConfigFixture(home, {
        gateway: {
          mode: "local",
          auth: { mode: "token", token: "${BOT_GATEWAY_TOKEN}" },
        },
        channels: { "test-plugin-channel": { enabled: true } },
      });
      const io = createHomeConfigIO(home, {
        env: {
          BOT_GATEWAY_TOKEN: "gateway-token-at-read",
          BOT_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
      });
      ...
      expect(result.writeOptions.envSnapshotForRestore?.BOT_GATEWAY_TOKEN).toBe(
  ...
  itWithHome("returns the snapshot-time hash when an included file is malformed", async (home) => {
    const configPath = configPathForHome(home);
    const includePath = path.join(home, ".bot", "plugins.json5");
  ...
  itWithHome("returns a write guard that rejects a changed active config path", async (home) => {
    const firstConfigPath = path.join(home, ".bot", "first.json");
    const secondConfigPath = path.join(home, ".bot", "second.json");
    ...
    const env = {
      BOT_CONFIG_PATH: firstConfigPath,
      BOT_TEST_FAST: "1",
    } as NodeJS.ProcessEnv;
    const io = createHomeConfigIO(home, { env });

    const result = await io.readConfigFileSnapshotForWrite();
    env.BOT_CONFIG_PATH = secondConfigPath;
  ...
  itWithHome("does not use expectedConfigPath as the write destination", async (home) => {
    const expectedConfigPath = path.join(home, ".bot", "expected.json");
    const activeConfigPath = path.join(home, ".bot", "active.json");
    ...
    await withEnvAsync(
      {
        BOT_CONFIG_PATH: activeConfigPath,
        BOT_TEST_FAST: "1",
      },
      async () => {
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const includePath = path.join(home, ".bot", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  itWithHome("does not persist the injected roster for a non-roster first write", async (home) => {
    ...
    let preflightConfig: BotConfig | undefined;
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const includePath = path.join(home, ".bot", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await writeConfigJson(includePath, {
        mode: "local",
        auth: { mode: "token", token: "${BOT_GATEWAY_TOKEN}" },
      ...
      const io = createHomeConfigIO(home, {
        env: {
          BOT_GATEWAY_TOKEN: "gateway-token-runtime",
          BOT_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
      });
      const snapshot = await io.readConfigFileSnapshot();
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const includePath = path.join(home, ".bot", "agent-defaults.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const includePath = path.join(home, ".bot", "agents.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({
      ...
      const io = createHomeConfigIO(home, {
        env: { GATEWAY_MODE: "local", BOT_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      });
      const snapshot = await io.readConfigFileSnapshot();
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const agentsPath = path.join(home, ".bot", "agents.json5");
      const modePath = path.join(home, ".bot", "gateway-mode.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
        `${JSON.stringify({
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const agentsPath = path.join(home, ".bot", "agents.json5");
      const gatewayPath = path.join(home, ".bot", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
        `${JSON.stringify({
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/bot-test-literal-plugin",
          source: "/tmp/bot-test-literal-plugin/index.ts",
          manifestPath: "/tmp/bot-test-literal-plugin/bot.plugin.json",
          configSchema: {
            type: "object",
            properties: {
  ...
  itWithHome("repairs invalid config without flattening record-nested includes", async (home) => {
    const configPath = configPathForHome(home);
    const includePath = path.join(home, ".bot", "main-agent.json5");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
    await writeConfigJson(includePath, { workspace: "${BOT_AGENT_WORKSPACE}" });
    await writeConfigJson(configPath, {
      agents: {
    ...
    const io = createHomeConfigIO(home, {
      env: {
        BOT_AGENT_WORKSPACE: "/resolved/agent-workspace",
        BOT_TEST_FAST: "1",
      } as NodeJS.ProcessEnv,
    });
    const snapshot = await io.readConfigFileSnapshot();
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/bot-test-required-plugin",
          source: "/tmp/bot-test-required-plugin/index.ts",
          manifestPath: "/tmp/bot-test-required-plugin/bot.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  itWithHome("writes runtime-derived edits back to source SecretRef markers", async (home) => {
    ...
    await withEnvAsync({ BOT_CONFIG_PATH: configPath }, async () => {
      setRuntimeConfigSnapshot(
        {
          gateway: { mode: "local" },
          models: {
      ...
      try {
        await withEnvAsync(
          ...
          async () => {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${BOT_GATEWAY_TOKEN}");
  ...
  itWithHome("stages managed root-write config env until the owner accepts it", async (home) => {
    const configPath = configPathForHome(home);
    const envKey = "BOT_TEST_MANAGED_ROOT_ENV";
    const initialAuthoredConfig = {
      gateway: {
        mode: "local" as const,
        auth: { mode: "token" as const, token: "${BOT_TEST_MANAGED_ROOT_ENV}" },
      },
      env: { vars: { [envKey]: "old" } },
    } satisfies BotConfig;
    const initialConfig = {
    ...
    let notifiedSource: BotConfig | undefined;
    const unsubscribe = registerConfigWriteListener(
      (event) => {
    ...
    try {
      await withEnvAsync({ BOT_CONFIG_PATH: configPath, [envKey]: "old" }, async () => {
        setRuntimeConfigSnapshot(initialConfig, initialConfig);
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const envKey = "BOT_TEST_WATCHER_ENV";
      const activeConfig = {
        env: { vars: { [envKey]: "old" } },
      ...
      const candidate = {
        env: { vars: { [envKey]: "new" } },
        gateway: { auth: { mode: "token" as const, token: "${BOT_TEST_WATCHER_ENV}" } },
      } satisfies BotConfig;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const envKey = "BOT_TEST_INTERLEAVED_WRITE_ENV";
      const makeConfig = (value: string, token: string): BotConfig => ({
      ...
      try {
        await withEnvAsync({ BOT_CONFIG_PATH: configPath, [envKey]: "a" }, async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/bot-test-demo",
          source: "/tmp/bot-test-demo/index.ts",
          manifestPath: "/tmp/bot-test-demo/bot.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ BOT_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createHomeConfigIO(home, {
            env: { BOT_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies BotConfig;
      await writeConfigJson(configPath, initialConfig);
      const baseSnapshot = {
        path: configPath,
        exists: true,
  ...
  itWithHome("blocks runtime preflight failures before committing root writes", async (home) => {
    ...
    let observedSource: BotConfig | undefined;
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const envKey = "BOT_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies BotConfig;
      const initialRaw = formatConfig(initialConfig);
  ...
  itWithHome(
    ...
    async (home) => {
      ...
      try {
        await withEnvAsync(
          { BOT_CONFIG_PATH: configPath, BOT_TEST_FAST: "1" },
          async () => {
  ...
  itWithHome(
    ...
    async (home) => {
      const configPath = configPathForHome(home);
      const otherConfigPath = path.join(home, ".bot", "other.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies BotConfig;
      const initialRaw = formatConfig(initialConfig);
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        promises: {
          ...
          rename: async (from, to) => {
            ...
            if (!committed && to === configPath) {
              committed = true;
              env.BOT_CONFIG_PATH = otherConfigPath;
            }
          },
        },
      } as typeof fsNode;
    ...
    async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/bot-test-strict-plugin",
            source: "/tmp/bot-test-strict-plugin/index.ts",
            manifestPath: "/tmp/bot-test-strict-plugin/bot.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await withEnvAsync({ BOT_CONFIG_PATH: configPath }, async () => {
          ...
          await expect(
            writeConfigFile({ agents: { entries: "not-array" } } as unknown as BotConfig, {
  ...
  itWithHome(
    ...
    async (home) => {
      const { configPath } = await writeConfigFixture(home, {
        logging: { file: "~/bot-upgrade-survivor/gateway.jsonl" },
      });
      const io = createFastConfigIO(home);
      const snapshot = await io.readConfigFileSnapshot();
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "bot-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
        { baseSnapshot: snapshot },
      ...
      const persisted = await readPersistedConfig(configPath);
      expect(persisted.logging?.file).toBe("~/bot-upgrade-survivor/gateway.jsonl");
...
`;
  ...
  itWithHome(
    ...
    async (home) => {
      const configPathA = path.join(home, ".bot", "config-a.json");
      const configPathB = path.join(home, ".bot", "config-b.json");
      const stateDir = path.join(home, ".bot");
      await withEnvAsync(
        {
          BOT_CONFIG_PATH: configPathA,
          BOT_STATE_DIR: stateDir,
          BOT_TEST_FAST: "1",
        },
        async () => {
          ...
          await withEnvAsync({ BOT_CONFIG_PATH: configPathB }, async () => {
