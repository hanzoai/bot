/** Builds the interactive `bot secrets configure` target list and apply plan. */
...
/** Credential target shown by `bot secrets configure` before a SecretRef is selected. */
...
/** Builds configure candidates for Bot config plus an optional auth-profile scope. */
