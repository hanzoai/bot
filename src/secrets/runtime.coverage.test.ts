...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: BotConfig): unknown =>
...
const DEBUG_COVERAGE_BATCHES = process.env.BOT_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_BOT_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.BOT_BUNDLED_PLUGINS_DIR ??= "extensions";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function addCoveragePluginLoadPath(config: BotConfig, pluginId: string): void {
...
function collectBotCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "bot.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_BOT_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForBotTarget(
  config: BotConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
async function expectBotCoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = toCoverageEnvRefId("BOT_SECRET_TARGET", entry.id);
    ...
    applyConfigForBotTarget(config, entry, envId, wildcardToken);
  }
...
const BOT_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectBotCoverageEntries({ includePluginEntries: false }),
);
const BOT_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectBotCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = BOT_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectBotCoverageBatchResolved("bot.json core", googleChatBatch);
    }
    const webProviderBatch = BOT_PLUGIN_COVERAGE_BATCHES.find((batch) =>
    ...
    if (webProviderBatch) {
      ...
      await expectBotCoverageBatchResolved("bot.json plugins", webProviderBatch);
    }
  });

  describe("bot.json core and channel registry targets", () => {
    test.each(BOT_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectBotCoverageBatchResolved("bot.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("bot.json plugin registry targets", () => {
    test.each(BOT_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectBotCoverageBatchResolved("bot.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = toCoverageEnvRefId("BOT_AUTH_SECRET_TARGET", entry.id);
