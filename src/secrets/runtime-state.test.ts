...
describe("secrets runtime state", () => {
  ...
  it("includes env shorthand SecretRefs in the reload contract", () => {
    const configWithRef = (apiKey: string): BotConfig => ({
      models: {
        providers: {
          openai: {
  ...
  it("publishes distinct raw and overlay source snapshots without changing runtime auth", () => {
    ...
    const rawSourceConfig = { gateway: { port: 19_030 } } satisfies BotConfig;
  ...
  it("rejects a source-only secrets write after runtime config ownership changes", () => {
    const initialConfig = { gateway: { port: 19_030 } } satisfies BotConfig;
    const concurrentConfig = { gateway: { port: 19_031 } } satisfies BotConfig;
    activateSecretsRuntimeSnapshotState({
      snapshot: {
  ...
  it("preserves live auth bookkeeping when prepared credentials activate", () => {
    const agentDir = "/tmp/bot-auth-bookkeeping-merge";
    const credential = {
  ...
  it("removes candidate-only auth profiles when rolling config back", () => {
    const agentDir = "/tmp/bot-auth-rollback-cas";
  ...
  it("rolls back candidate credentials against the activation-time auth baseline", () => {
    const agentDir = "/tmp/bot-auth-activation-baseline";
  ...
  it("preserves an auth rotation captured by the candidate", () => {
    ...
    const agentDir = "/tmp/bot-auth-rollback-sk-candidate";
    const snapshot = (key: string, port: number): PreparedSecretsRuntimeSnapshot => ({
      sourceConfig: {},
      config: { gateway: { port } },
      authStores: [
  ...
  it.each([
    ...
    ({ label, baselineAKey, candidateAKey, currentAKey, currentAExternal, expectedAKey }) => {
      const agentDir = `/tmp/bot-auth-post-activation-${label}`;
      const profile = (provider: string, key: string) => ({
        type: "api_key" as const,
        provider,
        key,
  ...
  it.each([
    ...
    const agentDir = `/tmp/bot-auth-effective-owner-${runtimeLocalProfileIds.length}`;
    const snapshot = (key: string, port: number): PreparedSecretsRuntimeSnapshot => ({
      sourceConfig: {},
      config: { gateway: { port } },
      authStores: [
  ...
  it("invalidates a partial store when an omitted candidate owner mutates", () => {
    const agentDir = "/tmp/bot-auth-external-omission";
    const snapshot = (
      profiles: AuthProfileStore["profiles"],
  ...
  it.each([
    ...
    ({ candidateOwner, mutateCandidateOwner }) => {
      const agentDir = `/tmp/bot-auth-external-to-${candidateOwner}-${mutateCandidateOwner}`;
      const snapshot = (
        key: string,
  ...
  it.each(["absent", "inherited", "local"] as const)(
    ...
    (baselineOwner) => {
      const agentDir = `/tmp/bot-auth-${baselineOwner}-to-external`;
      const snapshot = (
        key: string | null,
        owner: "external" | "inherited" | "local",
        port: number,
  ...
  it.each(["absent", "inherited", "local"] as const)(
    ...
    (baselineOwner) => {
      const agentDir = `/tmp/bot-auth-${baselineOwner}-external-refresh`;
      const snapshot = (
        key: string | null,
        owner: "external" | "inherited" | "local",
        port: number,
  ...
  it.each([
    ...
    ({ candidateOwner, currentOwner }) => {
      const agentDir = `/tmp/bot-auth-${candidateOwner}-${currentOwner}-equal-bytes`;
      const snapshot = (
        key: string,
  ...
  it("preserves an authoritative empty external overlay on rollback", () => {
    const agentDir = "/tmp/bot-auth-authoritative-empty-external";
  ...
  it("does not import rejected external authority from a selected current credential", () => {
    const agentDir = "/tmp/bot-auth-rejected-external-authority";
    const snapshot = (
      key: string,
  ...
  it.each([
    ...
    const agentDir = `/tmp/bot-auth-external-owner-${current}`;
    const snapshot = (key: string, port: number): PreparedSecretsRuntimeSnapshot => ({
      sourceConfig: {},
      config: { gateway: { port } },
      authStores: [
  ...
  it("removes a rejected candidate credential when its bounded lineage was evicted", () => {
    const agentDir = "/tmp/bot-auth-evicted-lineage";
    const snapshot = (key: string, port: number): PreparedSecretsRuntimeSnapshot => ({
      sourceConfig: {},
      config: { gateway: { port } },
      authStores: [
  ...
  it.each(["owner", "profile"] as const)(
    ...
    (eviction) => {
      const root = autoCleanupTempDirs.make("bot-auth-evicted-ref-");
      ...
      try {
        ...
        for (let index = 0; index < 300; index += 1) {
          noteRuntimeAuthProfileStorePersistedMutation(
            eviction === "owner" ? `/tmp/bot-auth-unrelated-owner-${index}` : agentDir,
            {
              credentialsChanged: true,
              stateChanged: false,
              profileIds: [`openai:unrelated-${index}`],
  ...
  it.each([
    ...
    ({
      ...
      const agentDir = `/tmp/bot-auth-store-removal-${label}`;
  ...
  it("does not resurrect a baseline external store after a new main profile is added", () => {
    const agentDir = "/tmp/bot-auth-external-store-omission-mutation";
    const snapshot = (includeStore: boolean, port: number): PreparedSecretsRuntimeSnapshot => ({
      sourceConfig: {},
      config: { gateway: { port } },
      authStores: includeStore
  ...
  it("does not resurrect an auth store cleared after candidate activation", () => {
    const agentDir = "/tmp/bot-auth-post-activation-clear";
    const snapshot = (key: string, port: number): PreparedSecretsRuntimeSnapshot => ({
      sourceConfig: {},
      config: { gateway: { port } },
      authStores: [
  ...
  it.each([
    ...
    const agentDir = `/tmp/bot-auth-ref-rollback-${changedRef}`;
    const previousRef = {
      source: "env" as const,
      provider: "default",
      id: "OPENAI_API_KEY",
  ...
  it("preserves live credentials when the captured predecessor is stale", () => {
    const agentDir = "/tmp/bot-auth-stale-predecessor-rollback";
    const snapshot = (key: string, port: number): PreparedSecretsRuntimeSnapshot => ({
      sourceConfig: {},
      config: { gateway: { port } },
      authStores: [
  ...
  it.each([
    ...
    candidateSourceConfig: BotConfig;
  }>)(
    ...
    ({ keyRef, previousSourceConfig, candidateSourceConfig, evictLineage }) => {
      const agentDir = `/tmp/bot-auth-provider-dependency-${keyRef.provider}`;
      const snapshot = (params: {
  ...
  it.each([
    ...
    ({ capturedOwner, currentOwner }) => {
      const agentDir = `/tmp/bot-auth-provider-owner-${capturedOwner}-${currentOwner}`;
      const keyRef = {
  ...
  it.each([
    ...
    ({ affectedProvider, currentProvider }) => {
      const agentDir = `/tmp/bot-auth-provider-ref-update-${currentProvider}`;
  ...
  it.each(["external", "local"] as const)(
    ...
    (currentOwner) => {
      const agentDir = `/tmp/bot-auth-provider-absent-upsert-${currentOwner}`;
      const snapshot = (params: {
