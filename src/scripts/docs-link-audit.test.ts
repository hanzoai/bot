...
describe("docs-link-audit", () => {
  ...
  it("cleans anchor audit docs copies when docs.json is invalid", () => {
    ...
    const before = tempEntries("bot-docs-anchor-audit-");
    try {
      expect(() => prepareAnchorAuditDocsDir(docsRoot)).toThrow();
      const after = tempEntries("bot-docs-anchor-audit-");
  ...
  it("does not create mirrored docs copies for non-root docs trees", () => {
    ...
    const before = tempEntries("bot-docs-link-audit-");
    try {
      ...
      const after = tempEntries("bot-docs-link-audit-");
      expect([...after].filter((entry) => !before.has(entry))).toEqual([]);
    } finally {
      cleanupTempDirs(tempDirs);
    }
    ...
    expect(() =>
      runDocsLinkAuditCli({
        ...
        prepareMirroredDocsDirImpl: () => ({
          ...
          dir: path.join(os.tmpdir(), "bot-docs-mirrored"),
  ...
  it("uses a pinned Mintlify package through npm for anchor validation", () => {
    ...
    const exitCode = runDocsLinkAuditCli({
      args: ["--anchors"],
      env: { ...process.env, BOT_DOCS_LINK_SENTINEL: "1" },
    ...
    expect(invocation).toEqual({
      ...
      options: expect.objectContaining({
        cwd: anchorDocsDir,
        env: expect.objectContaining({ BOT_DOCS_LINK_SENTINEL: "1" }),
        shell: false,
        stdio: "inherit",
      }),
    });
