...
describe("test-projects args", () => {
  ...
  it("routes boundary targets to the boundary config", () => {
    expect(buildVitestRunPlans(["src/infra/bot-root.test.ts"])).toEqual([
      {
        config: "test/vitest/vitest.boundary.config.ts",
        forwardedArgs: [],
        includePatterns: ["src/infra/bot-root.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("keeps conservative local full-suite runs on leaf project configs", () => {
    const originalVitestMaxWorkers = process.env.BOT_VITEST_MAX_WORKERS;
    const originalTestWorkers = process.env.BOT_TEST_WORKERS;
    const originalProjectParallel = process.env.BOT_TEST_PROJECTS_PARALLEL;
    const originalLeafShards = process.env.BOT_TEST_PROJECTS_LEAF_SHARDS;
    ...
    try {
      ...
      if (originalVitestMaxWorkers === undefined) {
        ...
        process.env.BOT_VITEST_MAX_WORKERS = originalVitestMaxWorkers;
      }
      if (originalTestWorkers === undefined) {
        delete process.env.BOT_TEST_WORKERS;
      } else {
        process.env.BOT_TEST_WORKERS = originalTestWorkers;
      }
      if (originalProjectParallel === undefined) {
        delete process.env.BOT_TEST_PROJECTS_PARALLEL;
      } else {
        process.env.BOT_TEST_PROJECTS_PARALLEL = originalProjectParallel;
      }
      if (originalLeafShards === undefined) {
        delete process.env.BOT_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.BOT_TEST_PROJECTS_LEAF_SHARDS = originalLeafShards;
      }
  ...
  it("uses the global host worker budget for full-suite project parallelism", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        58,
        {
          BOT_TEST_PROJECTS_LEAF_SHARDS: "1",
        },
        {
          cpuCount: 8,
          loadAverage1m: 0,
  ...
  it("gives parallel Vitest shards separate filesystem module caches", () => {
    ...
    expect(firstEnv?.BOT_VITEST_FS_MODULE_CACHE_PATH?.replaceAll("\\", "/")).toBe(
      "/repo/node_modules/.experimental-vitest-cache/0-test-vitest-vitest.gateway.config.ts",
    );
    expect(specs[1]?.env.BOT_VITEST_FS_MODULE_CACHE_PATH?.replaceAll("\\", "/")).toBe(
  ...
  it("splits an explicit Vitest filesystem module cache root", () => {
    const [spec] = applyParallelVitestCachePaths(
      ...
      ],
      {
        cwd: "/repo",
        env: {
          BOT_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache",
        },
      },
    );

    expect(spec?.env.BOT_VITEST_FS_MODULE_CACHE_PATH).toBe(
  ...
  it("writes scoped include files for routed extension runs", () => {
    ...
    expect(spec?.includeFilePath).toContain("bot-vitest-include-");
    expect(spec?.env.BOT_VITEST_INCLUDE_FILE).toBe(spec?.includeFilePath);
  });

  it("rejects explicit test file targets that do not exist", () => {
    expect(findUnmatchedExplicitTestTargets(["src/not-a-real-bot-test.test.ts"])).toEqual([
      {
        target: "src/not-a-real-bot-test.test.ts",
  ...
  it("skips channel contract configs with no matching external include patterns", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-contract-include-"));
    try {
      ...
      const specs = createVitestRunSpecs(
        ...
        {
          baseEnv: {
            BOT_VITEST_INCLUDE_FILE: includeFile,
