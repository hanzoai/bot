...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "If you’re not comfortable with basic security and access control, don’t run Hanzo Bot.",
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Hanzo Bot onboarding");
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("hanzo-bot doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }
