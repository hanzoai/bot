...
const config: BotConfig = {
  agents: {
    defaults: { workspace: "/tmp/bot-memory-step" },
...
function planFor(
  ...
  const items: MigrationPlan["items"] = [
    ...planned.map((id) => ({
      ...
      target: `/tmp/bot-memory-step/memory/imports/${providerId}/${id}.md`,
    })),
    ...conflicts.map((id) => ({
      id,
      kind: "memory" as const,
...
describe("runSetupMemoryImportStep", () => {
  ...
  it("does not apply when a single-provider import is declined", async () => {
    ...
    // The skip hint must not suggest `bot migrate <id>`: that command runs
    // the full provider migration, not a memory-only retry.
    expect(notes).toContain("Memory import page");
    expect(notes).not.toContain("bot migrate");
  });
