...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 Bot 設定",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `bot doctor`。",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 Bot 工作區？",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 BOT_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextPasswordHint: "直接把密碼存入 Bot 設定",
      plaintextPasswordLabel: "現在輸入密碼",
      plaintextTokenHint: "直接把權杖存入 Bot 設定",
    ...
    guided: {
      ...
      custodianIntro: "你好 — 我是 Bot，負責維護這套系統。我們開始設定吧。",
      failedOptionLine: "{label}：{reason}",
      failedOptionsIntro: "剛才這些沒有成功：",
      findMeLater: "之後隨時找得到我 — 在終端機執行 `bot`，或在儀表板中開啟設定。",
      ...
      appliedTitle: "推理已就緒",
      complete: "Bot 已準備就緒。",
      completeWithoutAi: "Bot 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      escapeHatches: "如需完整的分步精靈，請執行 `bot onboard --classic`。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "Bot 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
        "未進行任何變更。請執行 {fixCommand} 修復，再執行 {inspectCommand} 檢查，然後重試 onboarding。",
      invalidConfigUnknown: "- 無法解析設定。",
      manualChoice: "你想如何連接 AI？",
      nextSteps:
        "工作區：{workspace}\n新增頻道：`bot channels add`\n偏好聊天？執行 `bot setup`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`bot dashboard`\n稍後聊天：`bot`",
      nextStepsWithoutAi:
        "工作區：{workspace}\n稍後新增 AI：重新執行 `bot onboard`\n連接 AI 後新增頻道：`bot channels add`\n開啟 dashboard：`bot dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要互動式 TTY。自動化請使用 `bot onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "設定選項",
    ...
    security: {
      ...
      attribution:
        "Bot 是由 Bot 基金會（非營利組織）開發的開源助手，會與你一同學習成長。",
      baselineDmSessions:
      ...
      confirm: "我理解 Bot 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉安全加固和存取控制，請不要執行 Bot。",
      learnMore: "了解更多",
      notMultitenant: "Bot 預設不是敵對多租戶安全邊界。",
      personalAgent: "預設情況下，Bot 是個人 agent：一個可信操作者邊界。",
    ...
    skills: {
      ...
      docsLine: "文件：https://docs.bot.ai/skills",
      enterEnv: "輸入 {env}",
      ...
      manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `bot doctor` 檢查技能和需求。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作區設定 → Integrations → Bot → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "Bot" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 Bot 將你加入允許清單。",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 Bot 用來向 Chat 傳送回覆的 URL。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "Bot"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 Bot，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 bot configure --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.bot.ai/web/control-ui",
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 Bot。",
      dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 Bot。",
      ...
      daemonRuntimeNodeHint: "Bot 狀態使用 node:sqlite，因此必須使用 Node；Bun 無法執行閘道。",
      ...
      gatewayTokenStored:
        "儲存位置：$BOT_CONFIG_PATH（預設 ~/.hanzoai/bot.json）中的 gateway.auth.token，或 BOT_GATEWAY_TOKEN。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 Bot。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 Bot。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。Bot 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.bot.ai/security",
      ...
      webDocs: "文件：https://docs.bot.ai/tools/web",
      ...
      workspaceDocs: "文件：https://docs.bot.ai/concepts/agent-workspace",
