...
describe("configureGatewayForSetup", () => {
  ...
  it("keeps BOT_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.BOT_GATEWAY_TOKEN;
    process.env.BOT_GATEWAY_TOKEN = "advanced-env-token";
  ...
  it("persists an explicit classic quickstart env token ref", async () => {
    const previous = process.env.BOT_GATEWAY_TOKEN;
    process.env.BOT_GATEWAY_TOKEN = "token-from-env-ref";
    try {
