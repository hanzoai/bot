// Creates isolated Bot state directories for integration-style tests.
...
type BotTestStateLayout = "home" | "state-only" | "split";

type BotTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
type BotTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: BotTestStateLayout;
  scenario?: BotTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "bot-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "bot.json"),
...
function scenarioConfig(options: BotTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "bot-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: BotTestStateOptions): Record<string, string | undefined> {
...
export async function createBotTestState(
  options: BotTestStateOptions = {},
  ...
  const state: BotTestState = {
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed Bot env set, not plugin-provided host env.
        if (value === undefined) {
    ...
    cleanup: async () => {
      ...
      for (const database of listBotAgentDatabasesForTest()) {
        if (isPathInside(paths.stateDir, database.path)) {
          closeBotAgentDatabaseByPath(database.path);
        }
      }
      closeBotStateDatabaseByPath(resolveBotStateSqlitePath(env));
      state.restoreEnv();
      await fs.rm(root, {
        recursive: true,
        force: true,
...
export async function withBotTestState<T>(
  options: BotTestStateOptions,
  fn: (state: BotTestState) => Promise<T>,
): Promise<T> {
  const state = await createBotTestState(options);
  try {
