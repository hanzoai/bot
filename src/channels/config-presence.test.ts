...
const matrixPresenceOptions = {
  ...
  persistedAuthStateProbe: {
    ...
    hasState: ({ channelId, env }: { channelId: string; env?: NodeJS.ProcessEnv }) =>
      channelId === "matrix" && Boolean(env?.BOT_STATE_DIR?.includes("persisted-matrix")),
  },
};

function makeTempStateDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-channel-config-presence-"));
  tempDirs.push(dir);
  return dir;
}
