    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredBotTmpDir(), "bot-catalog-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@hanzo/bot-demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredBotTmpDir(), "bot-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredBotTmpDir(), "bot-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredBotTmpDir(), "bot-catalog-rich-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              schemaVersion: 1,
              description:
                "Extension manifest. Declares plugin packages that Bot can discover during onboarding and install on demand via `bot plugins install`.",
              entries: [
                {
                  name: "@wecom/wecom-bot-plugin",
                  description:
                    "Bot WeCom (企业微信) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  bot: {
                    channel: {
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "npm",
                npm: {
                  spec: "@wecom/wecom-bot-plugin@1.2.3",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredBotTmpDir(), "bot-catalog-prerelease-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(catalogPath, {
            ...createCatalogEntry({
              packageName: "@hanzo/bot-prerelease-demo-channel",
          ...
          return {
            ...
            catalogPaths: [catalogPath],
            expected: {
              install: { npmSpec: "@hanzo/bot-prerelease-demo-channel@2026.5.3-beta.1" },
              installSource: {
                npm: {
                  spec: "@hanzo/bot-prerelease-demo-channel@2026.5.3-beta.1",
                  packageName: "@hanzo/bot-prerelease-demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredBotTmpDir(), "bot-catalog-clawhub-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "clawhub",
                clawhub: {
                  spec: "clawhub:bot/clawhub-chat@2026.5.2",
                  packageName: "bot/clawhub-chat",
                  version: "2026.5.2",
                  exactVersion: true,
                },
                warnings: [],
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredBotTmpDir(), "bot-catalog-yuanbao-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
              ...
              entries: [
                {
                  ...
                  description:
                    "Bot Yuanbao (元宝) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  bot: {
                    channel: {
                    ...
                    install: {
                      npmSpec: "bot-plugin-yuanbao@1.0.0",
                      defaultChoice: "npm",
                      minHostVersion: ">=2026.4.10",
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredBotTmpDir(), "bot-catalog-home-"),
          );
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@hanzo/bot-env-demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@hanzo/bot-default-env-demo",
