...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(channelSupportsMessageCapability({} as BotConfig, "presentation")).toBe(true);
    expect(channelSupportsMessageCapability({} as BotConfig, "delivery-pin")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as BotConfig, channel: "demo-buttons" },
        "presentation",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as BotConfig, channel: "demo-cards" },
        "presentation",
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      channelSupportsMessageCapabilityForChannel({ cfg: {} as BotConfig }, "delivery-pin"),
    ).toBe(false);
  });
  ...
  it("normalizes channel aliases for per-channel capability checks", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as BotConfig, channel: "demo-cards-alias" },
        "delivery-pin",
      ),
    ).toBe(true);
  });
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(channelSupportsMessageCapability({} as BotConfig, "presentation")).toBe(false);
