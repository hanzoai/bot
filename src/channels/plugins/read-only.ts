...
function listBuiltPluginLoaderModuleCandidateUrls(importerUrl: string): URL[] {
  ...
  // ../../plugins candidates would escape the installed bot package there.
...
function getChannelConfigRecord(cfg: BotConfig, channelId: string): Record<string, unknown> {
...
function listManifestChannelAccountIds(cfg: BotConfig, channelId: string): string[] {
...
function resolveManifestChannelDefaultAccountId(cfg: BotConfig, channelId: string): string {
  const channelConfig = getChannelConfigRecord(cfg, channelId);
...
export function resolveReadOnlyChannelPluginsForConfig(
  ...
  if (externalPluginIds.length > 0) {
    ...
    if (missingConfiguredChannelIds.length > 0 && options.includeSetupFallbackPlugins === true) {
      ...
      const registry = loadPluginLoaderModule().loadBotPlugins({
        config: cfg,
        activationSourceConfig: options.activationSourceConfig ?? cfg,
        env,
        workspaceDir,
