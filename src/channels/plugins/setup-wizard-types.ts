...
type ChannelSetupWizardEnvShortcut = {
  ...
  isAvailable: (params: { cfg: BotConfig; accountId: string }) => boolean;
  apply: (params: {
    cfg: BotConfig;
    accountId: string;
...
export type ChannelSetupWizardCredential = {
  ...
  keepPrompt: string;
  inputPrompt: string;
  allowEnv?: (params: { cfg: BotConfig; accountId: string }) => boolean;
  inspect: (params: {
    cfg: BotConfig;
    accountId: string;
...
type ChannelSetupWizardGroupAccess = {
  ...
  currentPolicy: (params: { cfg: BotConfig; accountId: string }) => ChannelAccessPolicy;
  currentEntries: (params: { cfg: BotConfig; accountId: string }) => string[];
  updatePrompt: (params: { cfg: BotConfig; accountId: string }) => boolean;
  setPolicy: (params: {
    cfg: BotConfig;
    accountId: string;
...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: BotConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: BotConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: BotConfig, policy: DmPolicy, accountId?: string) => BotConfig;
