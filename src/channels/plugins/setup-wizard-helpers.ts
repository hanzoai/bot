...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: BotConfig) => Array<string | number> | undefined;
}): BotConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: BotConfig) => Array<string | number> | undefined;
}): (cfg: BotConfig, dmPolicy: DmPolicy) => BotConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: BotConfig, allowFrom: string[]) => BotConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: BotConfig, groupPolicy: "open" | "allowlist" | "disabled") => BotConfig {
  ...
  const promptToken = async (): Promise<string> =>
    (
      await params.prompter.text({
        ...
        // Credential input: masked in terminal prompts, and the Bot
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: BotConfig) => BotConfig | Promise<BotConfig>;
  applySet?: (
    cfg: BotConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => BotConfig | Promise<BotConfig>;
}): Promise<{
  cfg: BotConfig;
...
export async function promptParsedAllowFromForAccount<TConfig extends BotConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export function createPromptParsedAllowFromForAccount<TConfig extends BotConfig>(params: {
...
export function createTopLevelChannelParsedAllowFromPrompt(params: {
  channel: string;
  defaultAccountId: string | ((cfg: BotConfig) => string);
  enabled?: boolean;
  noteTitle?: string;
  noteLines?: string[];
  message: string;
  placeholder: string;
  parseEntries: (raw: string) => ParsedAllowFromResult;
  getExistingAllowFrom?: (cfg: BotConfig) => Array<string | number>;
  mergeEntries?: (params: { existing: Array<string | number>; parsed: string[] }) => string[];
}): NonNullable<ChannelSetupDmPolicy["promptAllowFrom"]> {
