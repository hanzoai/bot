...
const MATRIX_REPO_INSTALL_COMMAND = `bot plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/bot-matrix-missing",
        installPath: "/tmp/bot-matrix-missing",
      },
    });
    ...
    expect(
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/bot-matrix-missing",
      'Reinstall with "bot plugins install @hanzo/bot-matrix".',
  ...
  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/bot-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<bot plugins install @hanzo/bot-matrix>" when you want to return to the standard Matrix plugin.',
