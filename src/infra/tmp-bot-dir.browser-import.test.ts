...
describe("tmp-bot-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__botTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { DEFAULT_POSIX_TMP_ROOT, resolvePreferredBotTmpDir } from "./src/infra/tmp-bot-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: DEFAULT_POSIX_TMP_ROOT,
            resolverType: typeof resolvePreferredBotTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-bot-dir-browser-entry.ts",
      },
      write: false,
    });
