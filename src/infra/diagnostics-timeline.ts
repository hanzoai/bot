...
const BOT_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "bot.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.BOT_DIAGNOSTICS)) &&
    typeof env.BOT_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.BOT_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: BOT_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.BOT_DIAGNOSTICS_RUN_ID ? { runId: env.BOT_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.BOT_DIAGNOSTICS_ENV ? { envName: env.BOT_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.BOT_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
