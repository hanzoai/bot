...
const DEFAULT_TRIGGERS = ["bot", "claude", "computer"];
const VOICEWAKE_CONFIG_KEY = "default";

type VoiceWakeDatabase = Pick<BotStateKyselyDatabase, "voicewake_triggers">;
...
function openStateDatabase(stateDir?: string) {
  return openBotStateDatabase({
    env: stateDir ? { ...process.env, BOT_STATE_DIR: stateDir } : process.env,
  });
}
...
export async function setVoiceWakeTriggers(
  ...
  runBotStateWriteTransaction(
    ...
    baseDir ? { env: { ...process.env, BOT_STATE_DIR: baseDir } } : {},
  );
  return {
    triggers: sanitized,
    updatedAtMs,
