...
import { isHttpUrl, isWebSocketUrl } from "@hanzo/bot-net-policy/url-protocol";
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    BOT_PROXY_ACTIVE: process.env["BOT_PROXY_ACTIVE"],
    BOT_PROXY_LOOPBACK_MODE: process.env["BOT_PROXY_LOOPBACK_MODE"],
    BOT_PROXY_CA_FILE: process.env["BOT_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["BOT_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["BOT_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["BOT_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["BOT_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or BOT_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["BOT_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["BOT_PROXY_CA_FILE"],
  });
...
export async function startProxy(config: ProxyConfig | undefined): Promise<ProxyHandle | null> {
  if (
    ...
    (!config?.proxyUrl?.trim() && !process.env["BOT_PROXY_URL"]?.trim())
  ) {
    return null;
  }
 ...
 * loopback CDP probe to a Chromium instance Bot spawned itself.
 *
