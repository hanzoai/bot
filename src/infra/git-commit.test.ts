...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`bot-${label}-`);
}
...
async function makeFakeBotPackage(root: string) {
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the bot package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "bot");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "bot", version: "2026.3.10" }),
      "utf-8",
    );
