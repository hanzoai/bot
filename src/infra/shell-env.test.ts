...
describe("shell env fallback", () => {
  ...
  it("resolves timeout from env with default fallback", () => {
    ...
    expect(
      resolveShellEnvFallbackTimeoutMs({
        BOT_SHELL_ENV_TIMEOUT_MS: "42abc",
      }),
    ).toBe(15000);
    expect(
      resolveShellEnvFallbackTimeoutMs({
        BOT_SHELL_ENV_TIMEOUT_MS: String(Number.MAX_SAFE_INTEGER),
      }),
  ...
  it("imports missing expected keys even when another expected key already exists", () => {
    const env: NodeJS.ProcessEnv = { BOT_GATEWAY_TOKEN: "set" };
    const exec = vi.fn(() =>
      Buffer.from(
        "BOT_GATEWAY_TOKEN=from-shell\0TWILIO_ACCOUNT_SID=AC123\0TWILIO_AUTH_TOKEN=secret\0TWILIO_FROM_NUMBER=+15550001234\0",
      ),
    );
    ...
    expect(env.BOT_GATEWAY_TOKEN).toBe("set");
  ...
  it("prefers the login-shell executable over a daemon PATH candidate when requested", () => {
    ...
    const root = tempDirs.make("bot-shell-path-");
