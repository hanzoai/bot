...
describe("ensureBotCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling hanzo-bot exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bot-path-"));
    try {
      ...
      const originalFlag = process.env.BOT_PATH_BOOTSTRAPPED;
      ...
      try {
        ...
        if (originalFlag === undefined) {
          ...
          process.env.BOT_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
