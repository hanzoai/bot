// Resolves the Bot package root from runtime and package metadata.
import path from "node:path";
import { fileURLToPath } from "node:url";
import { botRootFs, botRootFsSync } from "./bot-root.fs.runtime.js";
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await botRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(botRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // the target so a launcher nested under another Bot checkout keeps its own package root.
  try {
    const resolved = botRootFsSync.realpathSync(normalized);
...
// Every distinct Bot package root among the runtime hints, in candidate order (symlinked
...
export function resolveBotPackageRootsSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string[] {
...
export function resolveBotPackageRootSync(opts: {
  ...
  return resolveBotPackageRootsSync(opts)[0] ?? null;
}
