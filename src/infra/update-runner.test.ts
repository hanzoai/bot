...
describe("runGatewayUpdate", () => {
  ...
  async function createStableTagRunner(params: {
    ...
    const calls: string[] = [];
    let uiBuildCount = 0;
    const doctorKey = `${process.execPath} ${path.join(tempDir, "bot.mjs")} doctor --non-interactive`;
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g bot@latest",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g bot@beta",
