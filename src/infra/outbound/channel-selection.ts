...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `bot agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: BotConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: bot doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run bot channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use bot channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: BotConfig): Promise<boolean> {
