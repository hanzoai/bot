...
type GatewayBootLifecycleTestDatabase = Pick<BotStateKyselyDatabase, "gateway_boot_lifecycle">;
...
function createLifecycleDb() {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-gateway-boot-"));
