...
// update.
//
// `runGatewayUpdate` (git mode) runs `bot doctor --fix` with
// `BOT_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `bot update` CLI resumes
...
// binary's hidden `bot update finalize` entrypoint — the designed
...
// re-enters the full `bot update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-update-post-core-"));
