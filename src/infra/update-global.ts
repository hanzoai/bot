...
/** Supported package managers for Bot global install and update flows. */
...
const PRIMARY_PACKAGE_NAME = "bot";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
/** npm-compatible spec used when the user asks to install the moving main branch. */
const BOT_MAIN_PACKAGE_SPEC = "github:hanzoai/bot#main";
const COREPACK_ENABLE_DOWNLOAD_PROMPT_DEFAULT = "0";
const NPM_GLOBAL_INSTALL_QUIET_FLAGS = ["--no-fund", "--no-audit", "--loglevel=error"] as const;
const PNPM_BOT_BUILD_ALLOWLIST_FLAG = `--allow-build=${PRIMARY_PACKAGE_NAME}`;
const BUN_BOT_TRUST_FLAG = "--trust";
...
/**
 * Extracts a pinned installed version from package specs like `bot@1.2.3`.
...
/**
 * Verifies that a global package root looks like a packaged Bot install
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "bot.plugin.json")))
      ) {
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "Bot", "deps", "portable-git");
  const candidates = [
 ...
 * Used for staged installs where Bot creates the prefix itself.
 */
...
function resolveBunGlobalInstallSpec(spec: string): string {
  ...
  // tarball is added beside it and can form an bot dependency loop.
...
/**
 * Detects an installed global Bot package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global Bot install.
 * npm receives quiet/freshness-bypass flags; pnpm and Bun approve Bot's lifecycle.
 */
