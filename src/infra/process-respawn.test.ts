...
describe("restartGatewayProcessWithFreshPid", () => {
  ...
  it("keeps BOT_NO_RESPAWN ahead of inherited supervisor hints", () => {
  ...
  it("actively schedules relaunch when Bot launchd markers are present on macOS", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithHandoff({ launchJobLabel: "ai.bot.gateway" });
  });
  ...
  it("returns supervised for a real gateway XPC launchd job without the injected marker", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.bot.gateway";
  ...
  it("launchd supervisor never returns failed regardless of triggerBotRestart outcome", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.BOT_LAUNCHD_LABEL = "ai.bot.gateway";
    // Even if triggerBotRestart *would* fail, launchd path must not call it.
  ...
  it("does not treat inherited XPC_SERVICE_NAME as launchd supervision", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.bot.mac";
    process.env.BOT_PROFILE = "mac";
  ...
  it("exits to external supervision without invoking inherited native restart hooks", () => {
    ...
    process.env.BOT_WINDOWS_TASK_NAME = "Bot Gateway";
  ...
  it("returns supervised when Bot gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.BOT_SERVICE_MARKER = "bot";
    process.env.BOT_SERVICE_KIND = "gateway";
    triggerBotRestartMock.mockReturnValue({ ok: true, method: "schtasks" });
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.BOT_TASK_SCRIPT = "C:\\bot\\node.cmd";
...
describe("respawnGatewayProcessForUpdate", () => {
  it("keeps BOT_NO_RESPAWN semantics for update restarts", () => {
    clearSupervisorHints();
    process.env.BOT_NO_RESPAWN = "1";

    const result = respawnGatewayProcessForUpdate();

    expect(result).toEqual({ mode: "disabled", detail: "BOT_NO_RESPAWN" });
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("allows detached respawn on unmanaged Windows during updates", () => {
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      process.execPath,
      ["C:\\bot\\node_modules\\bot\\bot.mjs", "gateway", "run"],
      {
        detached: true,
        env: process.env,
        stdio: "inherit",
  ...
  it("rewrites a pnpm-versioned Bot entry before detached update respawn", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.execArgv = [];
    process.argv = [
      "/usr/local/bin/node",
      "/app/node_modules/.pnpm/bot@2026.6.5/node_modules/bot/dist/entry.js",
      "gateway",
      "run",
    ];
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      process.execPath,
      ["/app/node_modules/hanzoai/bot.mjs", "gateway", "run"],
      {
        detached: true,
        env: process.env,
        stdio: "inherit",
