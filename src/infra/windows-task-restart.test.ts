...
const resolvePreferredBotTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));
const resolveTaskScriptPathMock = vi.hoisted(() =>
  vi.fn((env: Record<string, string | undefined>) => {
    const home = env.USERPROFILE || env.HOME || os.homedir();
    return path.join(home, ".bot", "gateway.cmd");
  }),
);
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  beforeEach(() => {
    spawnMock.mockReset();
    resolvePreferredBotTmpDirMock.mockReset();
    resolvePreferredBotTmpDirMock.mockReturnValue(os.tmpdir());
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    expect(result.tried).toContain('schtasks /Run /TN "Bot Gateway (work)"');
    ...
    expect(script).toContain(
      'bot restart attempt source=windows-task-handoff target="Bot Gateway (work)"',
    );
    expect(script).toContain(
      `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "$task = Get-ScheduledTask -TaskName 'Bot Gateway (work)' -ErrorAction SilentlyContinue; if ($null -ne $task -and $task.State -eq 'Running') { exit 0 }; exit 1" >nul 2>&1`,
    );
    expect(script).not.toContain("findstr");
    expect(script).toContain('schtasks /Run /TN "Bot Gateway (work)" >>');
    expect(script.indexOf("powershell.exe -NoProfile")).toBeLessThan(
      script.indexOf('schtasks /Run /TN "Bot Gateway (work)"'),
    );
  ...
  it("prefers BOT_WINDOWS_TASK_NAME overrides", () => {
    ...
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Bot Gateway (custom)" >>');
  });
  ...
  it("escapes custom task names in the PowerShell running-task probe", () => {
    ...
    relaunchGatewayScheduledTask({
      BOT_WINDOWS_TASK_NAME: "Bot Gateway (Bob's work)",
    });
    ...
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain(
      `-Command "$task = Get-ScheduledTask -TaskName 'Bot Gateway (Bob''s work)' -ErrorAction SilentlyContinue; if ($null -ne $task -and $task.State -eq 'Running') { exit 0 }; exit 1"`,
    );
    expect(script).not.toContain("findstr");
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredBotTmpDirMock.mockReturnValue(metacharTmpDir);
  ...
  it("includes startup fallback", () => {
    const taskScriptDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-state-"));
  ...
  const asciiPathEnv = {
    ...
    BOT_STATE_DIR: "C:\\ocw-test\\state",
  };
  ...
  it("writes marked code-page bytes for CJK task names that decode back exactly", () => {
    ...
    const result = relaunchGatewayScheduledTask({
      ...asciiPathEnv,
      BOT_WINDOWS_TASK_NAME: "Bot Gateway (隆)",
    });
    ...
    expect(script.startsWith("@echo off\r\n")).toBe(true);
    expect(script).toContain('schtasks /Run /TN "Bot Gateway (隆)" >>');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });
