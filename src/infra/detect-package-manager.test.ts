...
async function withPackageManagerRoot<T>(
  ...
  return await withTempDir({ prefix: "bot-detect-pm-" }, async (root) => {
...
async function writePublishedBotRoot(root: string): Promise<void> {
...
describe("detectPackageManager", () => {
  ...
  it("keeps pnpm-owned direct package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "bot-detect-pm-pnpm-direct-" }, async (base) => {
      const nodeModulesRoot = path.join(base, "pnpm-global", "node_modules");
      const packageRoot = path.join(nodeModulesRoot, "bot");
      await writePublishedBotRoot(packageRoot);
  ...
  it("keeps pnpm-owned virtual-store package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "bot-detect-pm-pnpm-virtual-" }, async (base) => {
  ...
  it("keeps bun-owned global package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "bot-detect-pm-bun-" }, async (base) => {
      const bunInstall = path.join(base, "bun-home");
      await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
        const packageRoot = path.join(bunInstall, "install", "global", "node_modules", "bot");
        await writePublishedBotRoot(packageRoot);
