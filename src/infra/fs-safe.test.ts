...
describe("fs-safe", () => {
  ...
  it("writes external command output within an allowed root", async () => {
    const dir = await tempDirs.make("bot-fs-safe-output-");
  ...
  it.each([
    ...
    const dir = await tempDirs.make("bot-fs-safe-output-name-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("bot-fs-safe-root-");
    const sourceDir = await tempDirs.make("bot-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("bot-fs-safe-root-");
    const outside = await tempDirs.make("bot-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    withEnv({ HOME: fakeHome, BOT_HOME: fakeHome }, () => {
    ...
    await withEnvAsync({ HOME: root, BOT_HOME: root }, async () => {
    ...
    const outsideRoot = await tempDirs.make("bot-tilde-outside-");
    await expectRejectCode(
