...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "bot.internal", expected: "bot.internal." },
    { value: "bot.internal.", expected: "bot.internal." },
    { value: "  bot.internal  ", expected: "bot.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    ...
    },
    {
      ...
      params: {
        env: { BOT_WIDE_AREA_DOMAIN: "foo/bar" } as NodeJS.ProcessEnv,
      },
      expected: null,
    },
  ...
  it("builds valid zone paths under the DNS config directory", () => {
    ...
    const zonePath = getWideAreaZonePath("bot.internal.");

    expect(zonePath).toBe(path.join(dnsDir, "bot.internal.db"));
    expect(path.relative(dnsDir, zonePath)).toBe("bot.internal.db");
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      ...
      `_bot-gw._tcp IN PTR studio-london._bot-gw._tcp`,
      `studio-london._bot-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Bot)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/bot`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN bot.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._bot-gw._tcp IN TXT`,
        `displayName=Mac Studio (Bot)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("bot.internal."),
      changed: false,
    });
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("bot.internal."),
      expectedZoneText,
      "utf-8",
    );
  });
