// Covers preferred Bot temp directory resolution.
import { constants as fsConstants } from "node:fs";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { DEFAULT_POSIX_TMP_ROOT, resolvePreferredBotTmpDir } from "./tmp-bot-dir.js";
...
describe("resolvePreferredBotTmpDir", () => {
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/bot when /tmp/bot is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/bot when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/bot exists but is not writable",
    ...
    {
      name: "falls back when /tmp/bot is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/bot is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/bot is group/other writable",
  ...
  it("repairs existing /tmp/bot permissions when they are too broad", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      `[bot] tightened permissions on temp dir: ${DEFAULT_POSIX_TMP_ROOT}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/bot after create when the initial mode stays too broad", () => {
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "bot");
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[bot] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/bot when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/bot when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === DEFAULT_POSIX_TMP_ROOT && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    ...
    ).toThrow(/Unable to create fallback Bot temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/bot` and routed
    // log files / TTS temp files there instead of `%TEMP%\bot`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "bot-501");
    ...
    const result = resolvePreferredBotTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
