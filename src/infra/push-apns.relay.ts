...
const DEFAULT_APNS_RELAY_BASE_URL = "https://ios-push-relay.bot.ai";
const DEFAULT_APNS_SANDBOX_RELAY_BASE_URL = "https://ios-push-relay-sandbox.bot.ai";
...
const GATEWAY_DEVICE_ID_HEADER = "x-bot-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-bot-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-bot-gateway-signed-at-ms";
...
export function normalizeApnsRelayBaseUrl(
  ...
  return normalizeApnsRelayBaseUrlWithPolicy(
    baseUrl,
    readAllowHttp(env.BOT_APNS_RELAY_ALLOW_HTTP),
  );
}
...
export function resolveApnsRelayConfigFromEnv(
  ...
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or BOT_APNS_RELAY_BASE_URL for relay registrations without the hosted relay origin",
    };
  }
  ...
  return {
    ...
    value: {
      ...
      timeoutMs: normalizeTimeoutMs(
        normalizeNonEmptyString(env.BOT_APNS_RELAY_TIMEOUT_MS) ?? configuredRelay?.timeoutMs,
      ),
    },
  };
}
