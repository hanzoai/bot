...
const createTempDir = () => tempDirs.make("bot-state-migrations-test-");
...
describe("state migrations", () => {
  ...
  it("uses the requested environment for plugin migration refresh and writes", async () => {
    ...
    const env = { ...process.env, HOME: customHome, BOT_STATE_DIR: stateDir };
  ...
  it("defers an unambiguous legacy merge through a final store symlink", async () => {
    ...
    expect(result.warnings).toContain(
      `Deferred legacy session migration in final-component symlink store ${targetStorePath}; configure one canonical session.store path, then rerun bot doctor --fix`,
    );
  });
  ...
  it("preserves a singleton final symlink through all session migration phases", async () => {
    ...
    expect(result.warnings).toEqual([
      `Deferred session key migration in final-component symlink store ${storePath}; configure one canonical session.store path, then rerun bot doctor --fix`,
      `Deferred legacy session migration in final-component symlink store ${storePath}; configure one canonical session.store path, then rerun bot doctor --fix`,
    ]);
  });
  ...
  it("preserves ACP metadata through a singleton fixed-store symlink", async () => {
    ...
    expect(result.warnings).toContain(
      `Deferred ACP metadata migration in final-component symlink store ${configuredStorePath}; configure one canonical session.store path, then rerun bot doctor --fix`,
    );
  ...
  it("runs plugin doctor migrations after repairing shared state schema", async () => {
    ...
    const env = createEnv(stateDir);
    const cfg = createConfig();
    const stateDbPath = path.join(stateDir, "state", "bot.sqlite");
  ...
  it("doctor discards worktree rows that predate the provisioned-file ledger", async () => {
    ...
    const env = createEnv(stateDir);
    const cfg = createConfig();
    const db = openBotStateDatabase({ env }).db;
    db.prepare(
  ...
  it("reports plugin detector failures in read-only legacy state detection", async () => {
    ...
    const stateDir = path.join(root, ".bot");
    const env = createEnv(stateDir);
    const cfg = { ...createConfig(), agents: { list: 42 } } as unknown as BotConfig;
    pluginDoctorStateMigrationEntries.entries = [
      {
        pluginId: "msteams",
        migration: {
  ...
  it("routes explicit Doctor repair through the Web Push SQLite importer", async () => {
    ...
    await fs.writeFile(
      vapidKeysPath,
      JSON.stringify(
        createWebPushVapidKeyPair("doctor-public", "doctor-private", "https://bot.ai"),
      ),
      "utf8",
    );
  ...
  it("never imports home-state plugin approvals into a custom state dir", async () => {
    ...
    const env = createEnv(stateDir);
    const cfg = createConfig();
    const sourcePath = path.join(root, ".bot", "plugin-binding-approvals.json");
  ...
  it("never imports default-profile approvals into a named profile", async () => {
    const root = await createTempDir();
    const stateDir = path.join(root, ".bot-work");
    const env = { ...createEnv(stateDir), BOT_PROFILE: "work" };
    const cfg = createConfig();
    const defaultStateDir = path.join(root, ".bot");
