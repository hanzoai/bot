...
import { bundledPluginFile } from "bot/plugin-sdk/test-fixtures";
...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "bot.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTempDir({ prefix: "bot-watch-node-" }, async (cwd) => {
      ...
      expect(spawnEnv.BOT_WATCH_MODE).toBe("1");
      expect(spawnEnv.BOT_WATCH_SESSION).toBe("1700000000000-4242");
      expect(spawnEnv.BOT_NO_RESPAWN).toBe("1");
      expect(spawnEnv.BOT_WATCH_COMMAND).toBe("gateway --force");
      expect(spawnEnv.BOT_TRACE_SYNC_IO).toBeUndefined();
  ...
  it("preserves explicit sync I/O trace overrides for gateway watch", async () => {
    ...
    await withTempDir({ prefix: "bot-watch-node-" }, async (cwd) => {
      ...
      expect(spawnCall[0]).toBe("/usr/local/bin/node");
      expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
      expect(requireSpawnEnv(spawn, 0).BOT_TRACE_SYNC_IO).toBe("0");
  ...
  it("does not run doctor after a gateway failure when auto doctor is disabled", async () => {
    ...
    const runPromise = runWatch({
      args: ["gateway", "--force"],
      createWatcher,
      env: { BOT_GATEWAY_WATCH_AUTO_DOCTOR: "0" },
      lockDisabled: true,
      process: fakeProcess,
      spawn,
    });
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
    const spawnEnv = requireSpawnEnv(spawn, 0);
    expect(spawnEnv.LAUNCH_JOB_LABEL).toBe("ai.bot.gateway");
    expect(spawnEnv.BOT_NO_RESPAWN).toBe("1");
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/bot/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/bot/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        [""],
        [
          "[bot] gateway:watch could not start because a dependency package config looks corrupted.",
        ],
        ["[bot] Invalid package config: /tmp/bot/.pnpm/chokidar/package.json"],
        ["[bot] This usually means a file in node_modules is empty or truncated."],
        ["[bot] Recommended recovery:"],
        ["[bot]   rm -rf node_modules"],
        ["[bot]   pnpm store prune"],
        ["[bot]   pnpm install"],
        [""],
        ["[bot] Original error:"],
        [error],
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTempDir({ prefix: "bot-watch-node-lock-" }, async (cwd) => {
