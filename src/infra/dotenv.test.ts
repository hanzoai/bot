...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "BOT_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "BOT_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "BOT_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "BOT_BUNDLED_SKILLS_DIR=./attacker-skills",
...
describe("loadDotEnv", () => {
  it("loads ~/.bot/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.bot/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        setTestEnvValue("HOME", base);
        const defaultStateDir = path.join(base, ".bot");
        setTestEnvValue("BOT_STATE_DIR", defaultStateDir);
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "bot", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "BOT_STATE_DIR=./evil-state",
            "BOT_CONFIG_PATH=./evil-config.json",
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "BOT_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "BOT_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "BOT_LIVE_GEMINI_KEY=sk-gemini-live",
            "BOT_LIVE_OPENAI_KEY=sk-openai-live",
            "BOT_GATEWAY_TOKEN=attacker-token",
            "BOT_GATEWAY_PASSWORD=attacker-password",
            "BOT_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
      ...
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          ...
          [
            ...
            "BOT_AGENT_DIR=./evil-agent",
            `BOT_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "BOT_OAUTH_DIR=./evil-oauth",
  ...
  it("blocks BOT_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "BOT_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.BOT_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.BOT_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "BOT_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'BOT_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.BOT_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.BOT_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.BOT_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.BOT_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "BOT_PINNED_PYTHON=./attacker-python",
            "BOT_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );
        ...
        expect(process.env.BOT_PINNED_PYTHON).toBeUndefined();
        expect(process.env.BOT_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "BOT_PINNED_PYTHON=/trusted/python",
            "BOT_PINNED_WRITE_PYTHON=/trusted/write-python",
        ...
        expect(process.env.BOT_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.BOT_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "BOT_LIVE_GEMINI_KEY=sk-gemini-live",
            "BOT_LIVE_OPENAI_KEY=sk-openai-live",
            "BOT_GATEWAY_TOKEN=trusted-token",
            "BOT_GATEWAY_PASSWORD=trusted-password",
            "BOT_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.BOT_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.BOT_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.BOT_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.BOT_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.BOT_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.BOT_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.BOT_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "BOT_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks BOT_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "BOT_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when BOT_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        setTestEnvValue("HOME", base);
        setTestEnvValue("BOT_STATE_DIR", customStateDir);
        await writeEnvFile(
          path.join(base, ".config", "bot", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.BOT_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
  ...
  it("blocks workspace .env takeover vars before loading the global fallback", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        expect(process.env.BOT_STATE_DIR).toBe(stateDir);
        expect(process.env.BOT_CONFIG_PATH).toBeUndefined();
        expect(process.env.BOT_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.NODE_OPTIONS).toBeUndefined();
        expect(process.env.NODE_REDIRECT_WARNINGS).toBeUndefined();
        expect(process.env.NODE_REPL_EXTERNAL_MODULE).toBeUndefined();
        expect(process.env.NODE_REPL_HISTORY).toBeUndefined();
