...
import { normalizeOptionalString as readOptionalString } from "@hanzo/bot-normalization-core/string-coerce";
...
type ManagedNpmRootBotMetadata = {
...
type ManagedNpmRootBotHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedBotMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootBotMetadata | undefined {
  const metadata: ManagedNpmRootBotMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
function applyManagedNpmRootOverrides(params: {
  ...
  for (const key of readManagedOverrideKeys(params.manifest.bot)) {
...
/** Read host Bot pnpm overrides for reuse inside a managed npm root. */
export async function readBotManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  const managedDependencyNames = new Set(readManagedPeerDependencyKeys(manifest.bot));
  ...
  const botMetadata = buildManagedBotMetadata({
    current: manifest.bot,
    managedOverrideKeys,
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "bot" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.bot;
  ...
  if (isRecord(value.peerDependenciesMeta) && "bot" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.bot;
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.bot);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bot-managed-peer-plan-"));
  try {
    delete dependencies.bot;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_bot-pack-archives"),
      path.join(tempRoot, "_bot-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.bot).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.bot)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const overrides = readOverrideRecord(manifest.overrides);
  const currentManagedOverrideKeys = readManagedOverrideKeys(manifest.bot);
...
/** Remove stale managed-root bot peer installs while preserving active host links. */
export async function repairManagedNpmRootBotPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootBotHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "bot" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasBotPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "bot"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootBotPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall bot" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall bot" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootBotPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootBotHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootBotHostState> {
  const packageRoot =
  ...
  const managedBotPackageDir = path.join(params.npmRoot, "node_modules", "bot");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedBotPackageDir),
    lstatIfExists(managedBotPackageDir),
  ]);
...
async function managedNpmRootLockfileHasBotPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "bot" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/bot" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "bot" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootBotPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("bot" in dependencies) {
    const { bot: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("bot" in dependenciesValue) {
          delete dependenciesValue.bot;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/bot" in parsed.packages) {
        delete parsed.packages["node_modules/bot"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "bot" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.bot;
  ...
  const botPackageDir = path.join(params.npmRoot, "node_modules", "bot");
  if (!params.preservePackageDir && (await pathExists(botPackageDir))) {
    await fs.rm(botPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["bot", "bot.cmd", "bot.ps1"].map((binName) =>
