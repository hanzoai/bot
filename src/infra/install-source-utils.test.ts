  ...
  it.each([
    {
      ...
      path: async () => "/tmp/does-not-exist-bot-archive.tgz",
...
describe("resolveNpmSpecMetadata", () => {
  ...
  it.each([
    ...
    const result = await resolveNpmSpecMetadata({ spec: "@hanzo/bot-codex" });
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "@hanzo/bot-codex",
        version: "2026.6.11",
        resolvedSpec: "@hanzo/bot-codex@2026.6.11",
  ...
  it("selects the newest multi-version entry satisfying the requested range", async () => {
    ...
    await expect(resolveNpmSpecMetadata({ spec: "@hanzo/bot-codex@^2026.6.0" })).resolves.toEqual({
      ok: true,
  ...
  it("uses the last multi-version entry when the selector is not a semver range", async () => {
    ...
    const result = await resolveNpmSpecMetadata({ spec: "@hanzo/bot-codex@latest" });
  ...
  it("accepts metadata without an bot block", async () => {
    const { bot: _bot, ...withoutBot } = npmViewMetadata;
    mockPackCommandResult({ stdout: JSON.stringify(withoutBot) });
  ...
  it("reports which required metadata fields are missing", async () => {
    ...
    await expect(resolveNpmSpecMetadata({ spec: "@hanzo/bot-codex" })).resolves.toEqual({
      ok: false,
...
describe("packNpmSpecToArchive", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      stdout:
        'npm notice using cache\n[{"id":"bot-plugin@1.2.3","name":"bot-plugin","version":"1.2.3","integrity":"sha512-test-integrity","shasum":"abc123"}]\n',
      expectedMetadata: {
        name: "bot-plugin",
        version: "1.2.3",
        resolvedSpec: "bot-plugin@1.2.3",
...
describe("resolveNpmPackArchiveMetadata", () => {
  it("reads archive metadata from npm <=11 array pack output", async () => {
    ...
    expect(result).toEqual({
      ok: true,
      archivePath,
      tarballName: "bot-plugin-1.2.3.tgz",
      metadata: {
        name: "bot-plugin",
        version: "1.2.3",
        resolvedSpec: "bot-plugin@1.2.3",
