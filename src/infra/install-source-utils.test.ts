...
describe("packNpmSpecToArchive", () => {
  ...
  it("returns friendly error for 404 (package not on npm)", async () => {
    ...
    mockPackCommandResult({
      stdout: "",
      stderr: "npm error code E404\nnpm error 404  '@bot/whatsapp@*' is not in this registry.",
      code: 1,
    });

    const result = await runPack("@bot/whatsapp", cwd);

    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("Package not found on npm");
      expect(result.error).toContain("@bot/whatsapp");
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    ...
    mockPackCommandResult({
      stdout:
        ...
        JSON.stringify([
          {
            id: "@bot/plugin-demo@2.0.0",
            filename: "bot-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@bot/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "bot-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@bot/plugin-demo@2.0.0",
      },
    });
  });
