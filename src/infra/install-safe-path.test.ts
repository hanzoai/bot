...
const canCreateDirectorySymlinks = (() => {
  ...
  try {
    probeDir = fsSync.mkdtempSync(
      path.join(os.tmpdir(), "bot-install-safe-dir-symlink-probe-"),
    );
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@hanzo/bot-matrix", id: "matrix", expected: true },
    { packageName: "@hanzo/bot-matrix", id: "@hanzo/bot-matrix", expected: true },
    { packageName: "@hanzo/bot-matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@hanzo/bot-matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@bot__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "bot-install-safe-" }, async (baseDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "bot-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "bot-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.skipIf(!canCreateDirectorySymlinks)(
    ...
    async () => {
      await withTempDir({ prefix: "bot-install-safe-" }, async (parentDir) => {
