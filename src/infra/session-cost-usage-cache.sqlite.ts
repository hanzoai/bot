...
type AgentCacheDatabase = Pick<BotAgentKyselyDatabase, "cache_entries">;
...
function readCacheDatabase<T>(
  ...
  try {
    const result = withBotAgentDatabaseReadOnly(operation, {
...
export function acquireSessionCostUsageRefreshLock(
  ...
  const acquired = runBotAgentWriteTransaction(
    (database) => {
      const kysely = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
      const currentRaw =
        executeSqliteQuerySync(
