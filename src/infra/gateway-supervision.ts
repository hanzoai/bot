...
const GATEWAY_SUPERVISOR_MODE_ENV = "BOT_SUPERVISOR_MODE";
...
export function formatExternalSupervisorActionRequired(action: string): string {
  return [
    `Bot gateway lifecycle is managed by an external supervisor (${GATEWAY_SUPERVISOR_MODE_ENV}=external).`,
...
export function formatExternalSupervisorUpdateRequired(): string {
  return [
    `Bot self-update is disabled while gateway lifecycle is managed by an external supervisor (${GATEWAY_SUPERVISOR_MODE_ENV}=external).`,
