 ...
 * `bot doctor --fix` repairs the default root while a scoped install stays blocked.
...
function doctorFixInstruction(filePath: string): string {
  const command = "Run `bot doctor --fix`";
  return process.env.BOT_STATE_DIR?.trim()
    ? `${command} with BOT_STATE_DIR set to ${path.dirname(filePath)}`
    : command;
}
