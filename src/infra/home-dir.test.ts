// Tests Bot home directory resolution.
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for BOT_HOME",
      env: { BOT_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when BOT_HOME is tilde-only and no fallback home exists",
      env: { BOT_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
  ...
  it("fails clearly when both home and cwd are unavailable", () => {
    ...
    try {
      expect(() => resolveRequiredHomeDir({}, noHome)).toThrow(/set BOT_HOME/i);
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/bot-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/bot-home " },
      expected: "/srv/bot-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/bot-home/docs"),
    },
    {
