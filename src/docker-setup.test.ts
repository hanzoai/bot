...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const env = createEnv(sandbox, {
      BOT_DOCKER_APT_PACKAGES: undefined,
      BOT_EXTRA_MOUNTS: undefined,
      BOT_HOME_VOLUME: undefined,
    });
    ...
    expect(envFile).toContain("BOT_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("BOT_EXTRA_MOUNTS=");
    expect(envFile).toContain("BOT_HOME_VOLUME=");
  });
  ...
  it("plumbs BOT_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const sandbox = await createDockerSetupSandbox();
    const env = createEnv(sandbox, {
