...
function getFirecrawlApiKey(config?: BotConfig): unknown {
...
function createFirecrawlPluginConfig(apiKey: unknown): BotConfig {
  return {
    plugins: {
      entries: {
        firecrawl: {
...
describe("web fetch runtime", () => {
  ...
  it("invalidates provider discovery when the same config object changes", () => {
    ...
    } as BotConfig & { tools: { web: { fetch: { provider: string } } } };
