...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Bot session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsBotStatus =
    normalized.includes("bot is running on") ||
    /bot\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("bot status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsBotStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from bot status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live bot config shows") ||
