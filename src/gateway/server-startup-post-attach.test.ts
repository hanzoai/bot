...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...
    resolveConfigPath: vi.fn(() => "/tmp/bot-state/bot.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn((env: NodeJS.ProcessEnv = process.env) =>
      env.BOT_STATE_DIR?.trim() ? actual.resolveStateDir(env) : "/tmp/bot-state",
    ),
  };
});
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("BOT_SKIP_CHANNELS", "0");
    vi.stubEnv("BOT_SKIP_PROVIDERS", "0");
  ...
  it("skips heavy restart sentinel refresh when no sentinel file exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-no-sentinel-"));
    try {
  ...
  it("refreshes the restart sentinel when the sentinel row exists", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-sentinel-"));
    try {
      await writeRestartSentinel(
        ...
        { BOT_STATE_DIR: stateDir } as NodeJS.ProcessEnv,
      );
      await withEnvAsync({ BOT_STATE_DIR: stateDir }, async () => {
  ...
  it("detects restart sentinel rows in explicit state directories", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-sentinel-state-"));
    try {
      await writeRestartSentinel(
        {
          kind: "update",
  ...
  it("avoids sync filesystem probes while checking restart sentinel presence", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-async-sentinel-"));
    try {
      await writeRestartSentinel(
        {
          kind: "update",
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/bot-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        await startGatewaySidecars({
          ...
          defaultWorkspaceDir: "/tmp/bot-workspace",
          deps: {} as never,
          startChannels,
          log: { warn: vi.fn() },
          logHooks: {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { BOT_SKIP_CHANNELS: undefined, BOT_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { BOT_SKIP_CHANNELS: "1", BOT_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
          cfg: {
            hooks: { internal: { enabled: false } },
