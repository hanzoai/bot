...
> {
  ...
  const syncWorkspaceImpl = async (
    ...
    const temporaryDirectory = await fs.mkdtemp(
      path.join(os.tmpdir(), "bot-worker-workspace-sync-"),
    );
  ...
  const reconcileWorkspaceImpl = async (
    ...
    const prepared = requirePrepared();
    const temporaryDirectory = await fs.mkdtemp(
      path.join(os.tmpdir(), "bot-worker-workspace-reconcile-"),
    );
    ...
    try {
      ...
      const baseManifestTransfer = await runBoundedInboundRsync({
        argv: [
          ...
          rsyncSsh,
          "--",
          `${prepared.scpTarget}:.bot-worker/manifests/${baseDigest}.json`,
          baseManifestPath,
        ],
      ...
      const currentManifestTransfer = await runBoundedInboundRsync({
        argv: [
          ...
          rsyncSsh,
          "--",
          `${prepared.scpTarget}:.bot-worker/manifests/${currentDigest}.json`,
          currentManifestPath,
        ],
        destinationRoot: manifestRoot,
        entryLimit: 1,
