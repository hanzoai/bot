...
const BOT_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const BOT_CLI_CHILD_TIMEOUT_MS = BOT_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function resolveBotCliProcessArgs(
  args: readonly string[],
  hasBuildOutput: boolean,
): string[] {
  return [hasBuildOutput ? "bot.mjs" : "scripts/run-node.mjs", ...args];
}
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the Bot MCP cron tool from server `bot`. " +
      "If it is not already visible, search/load MCP tools for `bot cron` or `cron`, " +
      "then call the matching Bot MCP tool; Claude-style names may appear as `mcp__bot__cron`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not Bot proof. " +
      ...
      "Do the actual tool call; I will verify externally with the Bot cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the Bot MCP cron tool from server `bot` now. " +
      "If it is not already visible, search/load MCP tools for `bot cron` or `cron`, " +
      "then call the matching Bot MCP tool; Claude-style names may appear as `mcp__bot__cron`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not Bot proof. " +
      ...
      "I will verify externally with the Bot cron CLI."
    );
  }
  return (
    "Your previous Bot cron MCP tool call was cancelled before the job was created. " +
    "Retry the Bot MCP cron tool from server `bot` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__bot__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
    `If the cron job is created, reply exactly: ${params.exactReply}. ` +
    "If the tool call is cancelled, the job is not created, or you cannot confirm creation, " +
...
export async function runBotCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(BOT_CLI_GATEWAY_TIMEOUT_MS)];
  ...
  const { stdout, stderr } = await runExec(
    process.execPath,
    resolveBotCliProcessArgs(cliArgs, hasBuildOutput),
    {
      baseEnv: childEnv,
      cwd: process.cwd(),
      logOutput: false,
      maxBuffer: 1024 * 1024,
      timeoutMs: BOT_CLI_CHILD_TIMEOUT_MS,
    },
  );
  ...
  if (!trimmed) {
    throw new Error(
      [
        `bot ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `bot ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runBotCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
