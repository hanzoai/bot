...
let warnLegacyBotEnvVars: typeof import("./env-deprecation.js").warnLegacyBotEnvVars;

describe("warnLegacyBotEnvVars", () => {
  let envSnapshot: ReturnType<typeof captureEnv>;
  let emitWarning: ReturnType<typeof vi.spyOn>;

  beforeEach(async () => {
    vi.resetModules();
    ({ warnLegacyBotEnvVars } = await import("./env-deprecation.js"));
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(options).toEqual({
      code: "BOT_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current BOT names", () => {
    warnLegacyBotEnvVars({ BOT_GATEWAY_TOKEN: "token" });
