...
describe("gateway cli backend live helpers", () => {
  ...
  afterEach(() => {
    ...
    delete process.env.BOT_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP;
    delete process.env.BOT_LIVE_CLI_BACKEND_ADVISORY;
  ...
  it("applies and restores live env including minimal gateway mode", async () => {
    ...
    process.env.BOT_SKIP_CHANNELS = "old-channels";
    process.env.BOT_SKIP_PROVIDERS = "old-providers";
    process.env.BOT_SKIP_GMAIL_WATCHER = "old-gmail";
    process.env.BOT_SKIP_CRON = "old-cron";
    process.env.BOT_SKIP_CANVAS_HOST = "old-canvas";
    process.env.BOT_SKIP_BROWSER_CONTROL_SERVER = "old-browser";
    process.env.BOT_BUNDLED_PLUGINS_DIR = "old-bundled";
    process.env.BOT_TEST_MINIMAL_GATEWAY = "old-minimal";
    ...
    expect(process.env.BOT_SKIP_CHANNELS).toBe("1");
    expect(process.env.BOT_SKIP_PROVIDERS).toBe("1");
    expect(process.env.BOT_SKIP_GMAIL_WATCHER).toBe("1");
    expect(process.env.BOT_SKIP_CRON).toBe("1");
    expect(process.env.BOT_SKIP_CANVAS_HOST).toBe("1");
    expect(process.env.BOT_SKIP_BROWSER_CONTROL_SERVER).toBe("1");
    expect(process.env.BOT_BUNDLED_PLUGINS_DIR).toBe("old-bundled");
    expect(process.env.BOT_TEST_MINIMAL_GATEWAY).toBe("1");
    expect(process.env.ANTHROPIC_API_KEY).toBeUndefined();
    expect(process.env.ANTHROPIC_API_KEY_OLD).toBeUndefined();

    restoreCliBackendLiveEnv(snapshot);

    expect(process.env.BOT_SKIP_CHANNELS).toBe("old-channels");
    expect(process.env.BOT_SKIP_PROVIDERS).toBe("old-providers");
    expect(process.env.BOT_SKIP_GMAIL_WATCHER).toBe("old-gmail");
    expect(process.env.BOT_SKIP_CRON).toBe("old-cron");
    expect(process.env.BOT_SKIP_CANVAS_HOST).toBe("old-canvas");
    expect(process.env.BOT_SKIP_BROWSER_CONTROL_SERVER).toBe("old-browser");
    expect(process.env.BOT_BUNDLED_PLUGINS_DIR).toBe("old-bundled");
    expect(process.env.BOT_TEST_MINIMAL_GATEWAY).toBe("old-minimal");
  ...
  it("defaults the model switch probe to Claude Sonnet -> Opus", async () => {
    ...
    delete process.env.BOT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE;
  ...
  it("lets env disable the model switch probe", async () => {
    ...
    process.env.BOT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE = "0";
  ...
  it("requires provider results by default for explicit CLI backend live probes", async () => {
    ...
    expect(
      ...
      action: "fail",
      message:
        'agent request for provider "claude-cli" was blocked by auth drift. Set BOT_LIVE_CLI_BACKEND_ADVISORY=1 and BOT_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP=1 only for advisory live probes.',
    });
  ...
  it("finds only Claude-imported native session ids", () => {
    ...
    expect(
      resolveImportedClaudeCliSessionId([
        null,
        { __bot: "invalid" },
        { __bot: { importedFrom: "codex-cli", cliSessionId: "wrong-provider" } },
        { __bot: { importedFrom: "claude-cli", cliSessionId: 42 } },
        { __bot: { importedFrom: "claude-cli", cliSessionId: "claude-session" } },
      ]),
    ).toBe("claude-session");
  ...
  it("allows live env overrides for fresh and resume CLI args", async () => {
    ...
    process.env.BOT_LIVE_CLI_BACKEND_ARGS = JSON.stringify([
      "exec",
      "--sandbox",
      "danger-full-access",
    ]);
    process.env.BOT_LIVE_CLI_BACKEND_RESUME_ARGS = JSON.stringify([
      "exec",
      "resume",
      "{sessionId}",
      "-c",
