...
import { closeBotAgentDatabases } from "../state/bot-agent-db.js";
...
function mockPreparedModel(): void {
  prepareSimpleCompletionModelForAgent.mockResolvedValue({
    selection: { provider: "openai", modelId: "gpt-test", agentDir: "/tmp/bot-agent" },
...
describe("generateToolCallTitles", () => {
  ...
  beforeEach(() => {
    ...
    stateDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bot-tool-titles-")));
