...
describe("gateway/node-command-policy", () => {
  ...
  it.each([
    ...
    (platform, deviceFamily) => {
      ...
      const enabled = resolveNodeCommandAllowlist({} as BotConfig, desktopNode);
  ...
  it("allows node-enabled and paired mobile UI without a persistent allow", () => {
    ...
    const enabled = resolveNodeCommandAllowlist({} as BotConfig, node);
  ...
  it("requires a persistent allow for sms.send and keeps deny-wins semantics", () => {
    ...
    expect(resolveNodeCommandAllowlist({} as BotConfig, node).has("sms.send")).toBe(false);
