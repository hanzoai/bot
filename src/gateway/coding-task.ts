...
export function buildAgentEnv(): NodeJS.ProcessEnv {
  return { ...pickEnv(envAllowNames()), HOME: process.env.HOME ?? "/tmp", GIT_TERMINAL_PROMPT: "0" };
}
...
export async function createDockerRuntime(timeoutSec: number): Promise<CodingRuntime> {
  ...
  if (!image) throw new Error("coding sandbox image is not configured (HANZO_CODING_SANDBOX_IMAGE)");
  const ver = await docker(["version", "--format", "{{.Server.Version}}"], 10_000);
  if (ver.code !== 0) throw new Error("container runtime unavailable");
  ...
  if (net) runArgs.push("--network", net);
  for (const [k, v] of Object.entries(buildAgentEnv())) {
    if (k === "HOME") continue; // container HOME is /work
    if (v !== undefined) runArgs.push("-e", `${k}=${v}`);
  }
  runArgs.push(image, "sleep", String(Math.max(120, timeoutSec + 120)));

  const created = await docker(runArgs, 60_000);
  if (created.code !== 0) throw new Error("could not start coding sandbox");
  ...
  const exec: ExecFn = async (argv, opts) => {
    ...
    for (const [k, v] of Object.entries(opts.env ?? {})) {
      ...
      if (v !== undefined) eflags.push("-e", `${k}=${v}`);
    }
  ...
  const runAgent: AgentRunner = ({ cwd, prompt, timeoutMs }, emit) =>
    spawnStreaming(
      "docker",
      ["exec", "-w", cwd, id, "dev", "exec", "--sandbox", "workspace-write", "--json", "--skip-git-repo-check", prompt],
