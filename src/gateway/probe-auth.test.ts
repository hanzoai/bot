...
function configWithDefaultEnvProvider(gateway: NonNullable<BotConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: BotConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: BotConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("BOT_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        BOT_GATEWAY_TOKEN: "test-token-from-env",
