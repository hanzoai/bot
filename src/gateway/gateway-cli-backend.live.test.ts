...
const CLI_DEBUG = isTruthyEnvValue(process.env.BOT_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.BOT_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
const CLI_MCP_SCHEMA_PROBE = isTruthyEnvValue(
  process.env.BOT_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE,
);
...
const CLI_BACKEND_REQUEST_TIMEOUT_MS = parsePositiveIntegerEnv(
  "BOT_LIVE_CLI_BACKEND_REQUEST_TIMEOUT_MS",
  15 * 60_000,
);
...
function openAiProviderConfigForCodexCli(
  modelKey: string,
): NonNullable<NonNullable<BotConfig["models"]>["providers"]>["openai"] {
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "BOT_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.BOT_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const cliImageArg =
        process.env.BOT_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.BOT_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        ...
        if (modelSwitchTarget) {
          ...
          if (resumeContinuityProbe) {
            ...
            if (!continuitySessionId) {
              throw new Error("Claude CLI continuity probe could not resolve its Bot session");
            }
            continuityOwner = {
