...
describe("resolveGatewayRequestContext", () => {
  ...
  it("preserves normal explicit session-key overrides", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-bot-session-key": "customer-case-42" }),
      model: "bot",
      sessionPrefix: "openai",
      defaultMessageChannel: "webchat",
    });
  ...
  it.each([
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-bot-session-key": sessionKey }),
        model: "bot",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
  ...
  it("does not build session state for explicit unknown agent ids", () => {
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-bot-agent-id": "missing-agent" }),
        model: "bot",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
...
describe("resolveHttpSenderIsOwner", () => {
  it("requires operator.admin on a trusted HTTP scope-bearing request", () => {
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-bot-scopes": "operator.admin" }), noneAuth),
    ).toBe(true);
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-bot-scopes": "operator.write" }), noneAuth),
    ).toBe(false);
  });
...
describe("resolveOpenAiCompatibleHttpSenderIsOwner", () => {
  ...
  it("still requires operator.admin for trusted scope-bearing requests", () => {
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-bot-scopes": "operator.write" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(false);
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-bot-scopes": "operator.admin" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(true);
  });
...
describe("authorizeOpenAiCompatibleHttpModelOverride", () => {
  it("allows shared-secret bearer callers to use x-bot-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({ authorization: "Bearer secret", "x-bot-model": "openai/gpt-5.4" }),
        { authMethod: "token", trustDeclaredOperatorScopes: false },
      ),
    ).toEqual({ allowed: true });
  });

  it("allows trusted admin callers to use x-bot-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-bot-scopes": "operator.admin, operator.write",
          "x-bot-model": "openai/gpt-5.4",
        }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toEqual({ allowed: true });
  ...
  it("rejects trusted write-only callers that try to use x-bot-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-bot-scopes": "operator.write",
