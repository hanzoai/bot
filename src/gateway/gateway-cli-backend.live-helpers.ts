...
export const CLI_BACKEND_LIVE_PROVIDER_SKIP_ENV = "BOT_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP";
export const CLI_BACKEND_LIVE_ADVISORY_ENV = "BOT_LIVE_CLI_BACKEND_ADVISORY";
...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `BOT_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }

  if (parsed.provider === "codex-cli") {
    throw new Error(
      "BOT_LIVE_CLI_BACKEND_MODEL=codex-cli/... is no longer supported. Use a supported CLI backend such as claude-cli or google-gemini-cli.",
    );
  }
...
export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.BOT_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.BOT_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  if (!args || args.length === 0) {
    throw new Error(
      `BOT_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "BOT_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.BOT_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.BOT_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
  ...
  for (const message of messages) {
    const metadata =
      typeof message === "object" && message !== null
        ? (message as Record<string, unknown>)["__bot"]
        : undefined;
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.BOT_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.BOT_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.BOT_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.BOT_TEST_MINIMAL_GATEWAY,
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("BOT_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("BOT_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("BOT_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("BOT_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("BOT_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("BOT_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("BOT_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("BOT_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("BOT_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("BOT_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("BOT_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
