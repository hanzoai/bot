...
describe("OpenResponses HTTP API (e2e)", () => {
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resSessionOverride = await postResponses(
        ...
        {
          "x-bot-agent-id": "beta",
          "x-bot-session-key": "agent:beta:openresponses:custom",
        },
      );
      ...
      const resReservedSessionOverride = await postResponses(
        port,
        { model: "bot", input: "hi" },
        { "x-bot-session-key": "agent:main:subagent:spoofed" },
      );
      ...
      expect(reservedSessionJson.error?.message).toBe(
        "`x-bot-session-key` cannot use reserved internal session namespaces.",
      );
      expect(agentCommand).toHaveBeenCalledTimes(0);
      ...
      const resHarnessSessionOverride = await postResponses(
        ...
        { model: "bot", input: "hi" },
        {
          "x-bot-session-key": "agent:main:harness:codex:supervision:spoofed-native-thread",
        },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "bot/default", input: "hi" });
      ...
      {
        agentCommand.mockClear();
        const res = await postResponses(port, { model: "bot/missing-agent", input: "hi" });
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Unknown agent 'missing-agent'.");
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-bot-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
  ...
  it("preserves verified trusted-proxy owner identity for both response modes", async () => {
    await withEnvAsync(
      { BOT_GATEWAY_TOKEN: undefined, BOT_GATEWAY_PASSWORD: undefined },
      async () => {
    ...
    async (mode) => {
      ...
      try {
        ...
        const unauthorized = await postResponses(
          port,
          { model: "bot", input: "hi" },
          { authorization: "Bearer wrong", "x-bot-sender-is-owner": "true" },
        );
        expect(unauthorized.status).toBe(401);
        await ensureResponseConsumed(unauthorized);
        expect(agentCommand).not.toHaveBeenCalled();
