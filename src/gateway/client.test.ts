...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    envSnapshot.restore();
    delete process.env.BOT_ALLOW_INSECURE_PRIVATE_WS;
    delete process.env.BOT_PROXY_ACTIVE;
    delete process.env.BOT_PROXY_LOOPBACK_MODE;
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { BOT_HOME: "/tmp/custom-bot-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      BOT_STATE_DIR: "/tmp/bot-client-service-state",
