...
const originalTrace = process.env.BOT_GATEWAY_STARTUP_TRACE;

describe("gateway server boundary", () => {
  beforeEach(() => {
    vi.resetModules();
    process.env.BOT_GATEWAY_STARTUP_TRACE = "1";
  });
  ...
  afterEach(() => {
    ...
    if (originalTrace === undefined) {
      delete process.env.BOT_GATEWAY_STARTUP_TRACE;
    } else {
      process.env.BOT_GATEWAY_STARTUP_TRACE = originalTrace;
    }
  });
