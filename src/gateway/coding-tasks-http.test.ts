...
import type { CodingRuntime } from "./coding-task.js";
...
describe("POST /v1/coding-tasks", () => {
  ...
  it("refuses a clone URL on a non-allowlisted git host", async () => {
    setRuntime();
    const { status } = await post({ ...okBody, cloneUrl: "https://evil.example.com/v1/git/acme/api.git" });
    expect(status).toBe(400);
    expect(currentRuntime.cleanup).not.toHaveBeenCalled();
  });
  ...
  it("caps concurrency per org (429 over the limit)", async () => {
    ...
    try {
      ...
      currentMake = async (): Promise<CodingRuntime> => {
        ...
        return { workdir: "/tmp/x", deps: { exec: async () => ({ stdout: "", stderr: "", code: 0 }), runAgent: async () => ({ ok: true, logTail: "" }) }, cleanup };
      };
