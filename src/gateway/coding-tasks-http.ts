...
function acquire(org: string): boolean {
  if (inflightGlobal >= globalCap()) return false;
  const o = inflightByOrg.get(org) ?? 0;
  if (o >= orgCap()) return false;
  inflightGlobal++;
...
function release(org: string): void {
  ...
  if (o <= 0) inflightByOrg.delete(org);
  else inflightByOrg.set(org, o);
}
...
function authModeAllowed(auth: ResolvedGatewayAuth): boolean {
  ...
  if (a.mode === "iam") return true;
  if (a.mode === "token" && typeof a.token === "string" && a.token.length > 0) return true;
  return false;
}
...
export async function handleCodingTasksHttpRequest(
  ...
  if (!authModeAllowed(opts.auth)) {
    sendJson(res, 503, { error: { message: "coding tasks require an enforcing gateway auth mode", type: "server_error" } });
    return true;
  }
  ...
  if (!acquire(orgId)) {
    sendJson(res, 429, { error: { message: "coding tasks at capacity, retry shortly", type: "rate_limit_error" } });
    return true;
  }
...
function parseBody(raw: unknown, orgId: string): ParseResult {
  ...
  if (parsedUrl.protocol !== "https:") return { ok: false, error: "cloneUrl must be https" };
