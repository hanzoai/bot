...
const getRuntimeConfig = vi.hoisted(() => vi.fn(() => ({}) as BotConfig));
const listAgentIds = vi.hoisted(() => vi.fn(() => ["main", "research-analyst", "alpha"]));
const resolveDefaultAgentId = vi.hoisted(() => vi.fn(() => "main"));
const resolveAgentWorkspaceDir = vi.hoisted(() =>
  vi.fn((_cfg: BotConfig, _agentId: string) => "/tmp/bot"),
);
const resolveMemorySearchConfig = vi.hoisted(() =>
  vi.fn<(_cfg: BotConfig, _agentId: string) => { enabled: boolean } | null>(() => ({
    enabled: true,
  })),
);
...
vi.mock("../../agents/agent-scope.js", () => ({
  listAgentIds,
  listAgentEntries: (cfg: BotConfig) =>
    cfg.agents?.entries
...
describe("doctor.memory agent targeting", () => {
  beforeEach(() => {
    ...
    resolveAgentWorkspaceDir.mockReset().mockReturnValue("/tmp/bot");
...
describe("doctor.memory.status", () => {
  ...
  it("includes dreaming counts and managed cron status when workspace data is available", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((cfg: BotConfig, agentId: string) => {
      if (agentId === "alpha") {
    ...
    const cronList = vi.fn(async () => [
      {
        ...
        payload: {
          kind: "systemEvent",
          text: "__bot_memory_core_short_term_promotion_dream__",
        },
  ...
  it("scopes dreaming status to the requested agent workspace", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg: BotConfig, agentId: string) => {
      if (agentId === "alpha") {
        return alphaWorkspaceDir;
      }
      return mainWorkspaceDir;
...
describe("doctor.memory dream actions", () => {
  it("clears grounded-only staged short-term entries without touching the diary", async () => {
    resolveAgentWorkspaceDir.mockReturnValue("/tmp/bot");
    removeGroundedShortTermCandidates.mockResolvedValue({
      removed: 3,
      storePath: "/tmp/bot/memory/.dreams/short-term-recall.json",
    });
    const respond = vi.fn();
  ...
  it("repairs contaminated dreaming artifacts for control-ui callers", async () => {
    ...
    repairDreamingArtifacts.mockResolvedValue({
      changed: true,
      archiveDir: "/tmp/bot/.bot-repair/dreaming/2026-04-11T22-00-00-000Z",
      archivedDreamsDiary: false,
...
describe("doctor.memory.dreamDiary", () => {
  ...
  it("reads DREAMS.md for the requested agent", async () => {
    ...
    resolveAgentWorkspaceDir.mockImplementation((_cfg, agentId) =>
      agentId === "research-analyst" ? workspaceDir : "/tmp/bot",
    );
    const respond = vi.fn();
...
describe("doctor.memory.remHarness", () => {
  const makeHarnessPreview = (
    ...
    workspaceDir: overrides.workspaceDir ?? "/tmp/bot",
    nowMs: 0,
    remConfig: {
      enabled: true,
      lookbackDays: 7,
  ...
  beforeEach(() => {
    getRuntimeConfig.mockClear().mockReturnValue({} as BotConfig);
