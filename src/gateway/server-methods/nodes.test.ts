...
const createdStates: BotTestState[] = [];
...
async function createState(label: string): Promise<BotTestState> {
  const state = await createBotTestState({ label, layout: "state-only" });
