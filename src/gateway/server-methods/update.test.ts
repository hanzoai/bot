...
>(async (params) => ({
  status: "started",
  pid: 12345,
  command: "bot update --yes --timeout 1800",
  logPath: "/tmp/bot-update-run-handoff/handoff.log",
...
vi.mock("../../infra/bot-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/bot-root.js")>(
    "../../infra/bot-root.js",
  );
  return {
    ...actual,
...
beforeEach(() => {
  ...
  readConfigFileSnapshotMock.mockResolvedValue({
    ...
    raw: "{}",
    parsed: {},
    resolved: {} as BotConfig,
    sourceConfig: {} as BotConfig,
    valid: true,
    config: {} as BotConfig,
    runtimeConfig: {} as BotConfig,
    issues: [],
    warnings: [],
    legacyIssues: [],
  });
...
async function invokeUpdateRun(
  ...
  runtimeConfig: BotConfig = { update: {} },
) {
...
describe("update.run restart scheduling", () => {
  ...
  it("hands managed package updates to the CLI path instead of running them in-process", async () => {
    ...
    const payload = await withProcessEnv({ BOT_LAUNCHD_LABEL: "ai.bot.gateway" }, () =>
  ...
  it("keeps a startup grace before restarting after systemd handoff spawn", async () => {
    ...
    await withProcessEnv({ BOT_SYSTEMD_UNIT: "bot-gateway.service" }, () =>
  ...
  it("starts managed package handoff when the gateway cwd is unavailable", async () => {
    ...
    try {
      await withProcessEnv({ BOT_LAUNCHD_LABEL: "ai.bot.gateway" }, () =>
        invokeUpdateRun({}),
      );
    } finally {
      cwdSpy.mockRestore();
  ...
  it("hands supervised git/dev updates to the CLI path instead of rebuilding live dist in-process", async () => {
    detectRespawnSupervisorMock.mockReturnValueOnce("launchd");
    mockGitInstallSurface("/tmp/bot-git");
    const payload = await withProcessEnv({ BOT_LAUNCHD_LABEL: "ai.bot.gateway" }, () =>
      captureUpdateRunPayload(),
    );
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      message:
        "Bot updates cannot safely run inside the live gateway process without a managed-service handoff.\n" +
        "Run `bot update --yes --timeout 1800` from a shell outside the gateway service, or restart/update from the host UI.",
    });
  });
  ...
  it("delegates update.run without mutating or restarting under external supervision", async () => {
    ...
    const payload = await withProcessEnv({ BOT_SUPERVISOR_MODE: "external" }, () =>
      captureUpdateRunPayload(),
    );
...
describe("update.run post-core plugin finalize", () => {
  ...
  it("resumes official plugin convergence after a git/source core update", async () => {
    runPostCoreFinalizeAfterGatewayUpdateMock.mockResolvedValueOnce({
      status: "ok",
      entrypoint: "/tmp/bot-git/dist/index.mjs",
    });
    mockGitOkUpdate("/tmp/bot-git");
  ...
  it("carries the pre-doctor source config into the git finalizer", async () => {
    ...
    const [finalizeParams] = firstMockCall(
      runPostCoreFinalizeAfterGatewayUpdateMock,
      "post-core finalize",
    ) as [{ preUpdateConfig?: { sourceConfig?: BotConfig; authoredConfig?: BotConfig } }];
