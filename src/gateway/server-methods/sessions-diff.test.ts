...
function initRepo(root: string): void {
  ...
  git(root, "config", "user.email", "test@bot.test");
...
describe("loadSessionDiff", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    repoRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bot-sessions-diff-")));
  ...
  it("withholds patch content for hardlinked files pointing outside the checkout", async () => {
    const secretDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "bot-secret-")));
