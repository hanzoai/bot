...
const mocks = vi.hoisted(() => ({
  ...
  resolveBotAgentSqlitePath: vi.fn(
    (params?: { path?: string }) => params?.path ?? "/agents/test-agent/bot-agent.sqlite",
  ),
  closeBotAgentDatabaseByPath: vi.fn((_pathname?: string) => true),
  listBotRegisteredAgentDatabases: vi.fn(() => [] as Array<Record<string, unknown>>),
  unregisterBotAgentDatabase: vi.fn(),
  assertNoBotAgentDatabaseLeases: vi.fn(),
...
const RESERVED_SYSTEM_AGENT_IDS_FOR_TEST = ["bot", "crestodian"] as const; // reserved ids
  ...
  return {
    ...
    mutateConfigFileWithRetry: async (params: {
      ...
      const result = await params.mutate(draft, {
        snapshot: { path: "/tmp/bot/config.json" },
...
vi.mock("../../state/bot-agent-db.js", () => ({
  closeBotAgentDatabaseByPath: mocks.closeBotAgentDatabaseByPath,
  listBotRegisteredAgentDatabases: mocks.listBotRegisteredAgentDatabases,
  resolveBotAgentSqlitePath: mocks.resolveBotAgentSqlitePath,
}));
...
vi.mock("../../state/bot-agent-db-registry.js", () => ({
  unregisterBotAgentDatabase: mocks.unregisterBotAgentDatabase,
}));

vi.mock("../../state/bot-agent-db-lease.js", () => ({
  assertNoBotAgentDatabaseLeases: mocks.assertNoBotAgentDatabaseLeases,
}));
...
beforeEach(() => {
  ...
  mocks.resolveBotAgentSqlitePath
    .mockReset()
    .mockImplementation(
      (params?: { path?: string }) => params?.path ?? "/agents/test-agent/bot-agent.sqlite",
    );
  mocks.closeBotAgentDatabaseByPath.mockReset().mockReturnValue(true);
  mocks.listBotRegisteredAgentDatabases.mockReset().mockReturnValue([]);
  mocks.unregisterBotAgentDatabase.mockReset();
...
describe("agents.delete", () => {
  ...
  it("removes only the deleted agent's authority before committing its roster removal", async () => {
    ...
    mocks.closeBotAgentDatabaseByPath.mockImplementation(() => {
    ...
    expect(mocks.closeBotAgentDatabaseByPath).toHaveBeenCalledWith(
      "/agents/test-agent/bot-agent.sqlite",
    );
    expect(mocks.unregisterBotAgentDatabase).toHaveBeenCalledWith({
      agentId: "test-agent",
      path: "/agents/test-agent/bot-agent.sqlite",
    });
  ...
  it("rolls cron back and keeps the roster when authority cleanup fails", async () => {
    ...
    expect(mocks.writeConfigFile).not.toHaveBeenCalled();
    expect(mocks.closeBotAgentDatabaseByPath).toHaveBeenCalledOnce();
  ...
  it("keeps deletion fenced when config persistence succeeds before reporting failure", async () => {
    ...
    expect(mocks.beginAgentDeletionRollback).not.toHaveBeenCalled();
    expect(mocks.closeBotAgentDatabaseByPath).toHaveBeenCalled();
    expect(mocks.movePathToTrash).not.toHaveBeenCalled();
  });
  ...
  it("does not perform destructive cleanup without a config deletion result", async () => {
    ...
    expect(mocks.closeBotAgentDatabaseByPath).toHaveBeenCalledOnce();
    expect(mocks.movePathToTrash).not.toHaveBeenCalled();
    expect(mocks.unregisterBotAgentDatabase).not.toHaveBeenCalled();
    expect(mocks.beginAgentDeletionRollback).toHaveBeenCalledOnce();
  });
  ...
  it("resumes a journaled deletion while its directory is still owned by the deleted agent", async () => {
    ...
    expect(mocks.writeConfigFile).not.toHaveBeenCalled();
    expect(mocks.closeBotAgentDatabaseByPath).toHaveBeenCalledWith(
      "/journal/agent/bot-agent.sqlite",
    );
    expect(mocks.unregisterResolvedAgentDir).toHaveBeenCalledWith({
      agentId: "test-agent",
      agentDir: "/journal/agent",
  ...
  it("extends a recovery cleanup snapshot with newly discovered database files", async () => {
    ...
    mocks.findAgentEntryIndex.mockReturnValue(-1);
    mocks.readAgentDeletionJournal.mockReturnValue(journal);
    mocks.listBotRegisteredAgentDatabases.mockReturnValue([
  ...
  it("settles recovery without touching a journaled directory claimed by another agent", async () => {
    ...
    const databaseRows = [
      { agentId: "test-agent", path: "/journal/agent/bot-agent.sqlite" },
      { agentId: "other-agent", path: "/journal/agent/bot-agent.sqlite" },
    ];
    mocks.findAgentEntryIndex.mockReturnValue(-1);
    mocks.readAgentDeletionJournal.mockReturnValue({
      agentId: "test-agent",
    ...
    mocks.listBotRegisteredAgentDatabases.mockImplementation(() => databaseRows);
    mocks.unregisterBotAgentDatabase.mockImplementation(
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.listBotRegisteredAgentDatabases).toHaveBeenCalled();
    expect(mocks.closeBotAgentDatabaseByPath).not.toHaveBeenCalled();
    expect(mocks.assertNoBotAgentDatabaseLeases).toHaveBeenCalledWith("test-agent");
    expect(mocks.movePathToTrash).not.toHaveBeenCalledWith("/journal/agent");
    expect(mocks.unregisterBotAgentDatabase).toHaveBeenCalledWith({
      agentId: "test-agent",
      path: "/journal/agent/bot-agent.sqlite",
    });
  ...
  it("revalidates database ownership after earlier filesystem cleanup", async () => {
    ...
    mocks.listBotRegisteredAgentDatabases.mockImplementation(() =>
      claimed ? [{ agentId: "other-agent", path: "/journal/agent/survivor.sqlite" }] : [],
    );
    mocks.movePathToTrash.mockImplementation(async (pathname?: string) => {
      if (pathname === "/journal/agent/bot-agent.sqlite") {
        claimed = true;
      }
      return "/trashed";
    });
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/journal/agent/bot-agent.sqlite");
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/journal/workspace");
    expect(mocks.movePathToTrash).not.toHaveBeenCalledWith("/journal/agent");
  ...
  it("cleans a relocated deleted-agent database while preserving a claimed agent directory", async () => {
    ...
    expectRespondOk(respond, { ok: true });
    expect(mocks.closeBotAgentDatabaseByPath).toHaveBeenCalledTimes(1);
    expect(mocks.closeBotAgentDatabaseByPath).toHaveBeenCalledWith(
  ...
  it("preserves a relocated database path registered to another agent", async () => {
    ...
    expect(mocks.closeBotAgentDatabaseByPath).not.toHaveBeenCalledWith(
  ...
  it("preserves a parent directory containing another agent's relocated database", async () => {
    ...
    expect(mocks.movePathToTrash).toHaveBeenCalledWith("/agents/test-agent/bot-agent.sqlite");
    expect(mocks.beginAgentDeletionFinish).toHaveBeenCalledOnce();
  });
  ...
  it("unregisters a closed database row after committing deletion", async () => {
    mocks.closeBotAgentDatabaseByPath.mockReturnValueOnce(false);
    ...
    expect(mocks.writeConfigFile.mock.invocationCallOrder[0]).toBeLessThan(
      expectDefined(
        mocks.unregisterBotAgentDatabase.mock.invocationCallOrder[0],
  ...
  it("reports trash failures without deleting the retained directory", async () => {
    ...
    const workspaceDir = await actualFs.realpath(
      await actualFs.mkdtemp(path.join(os.tmpdir(), "bot-agent-delete-trash-failure-")),
    );
