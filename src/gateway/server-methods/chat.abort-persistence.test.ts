...
function expectPersistedAbortMessage(
  ...
  const abort = expectRecord(actual.botAbort, "persisted abort metadata");
...
describe("chat abort transcript persistence", () => {
  ...
  it("does not let non-assistant idempotency collisions suppress abort partial persistence", async () => {
    const { transcriptPath, sessionId, storePath } = await createTranscriptFixture(
      "bot-chat-abort-idempotency-collision-",
    );
  ...
  it.each([
    ...
    const { sessionId } = await createTranscriptFixture(`bot-chat-stop-${caseId}-`);
    const respond = vi.fn();
  ...
  it.each([
    ...
    const { sessionId } = await createTranscriptFixture(`bot-chat-stop-global-${fixtureId}-`);
  ...
  it("does not match stop targets by client-supplied session id without a stored entry", async () => {
    const { sessionId } = await createMissingEntryFixture("bot-chat-stop-client-session-");
    const respond = vi.fn();
