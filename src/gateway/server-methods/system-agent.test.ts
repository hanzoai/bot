// Bot gateway tests cover activation serialization and chat sessions.
...
const verifiedConfig: BotConfig = {
...
beforeEach(async () => {
  ...
  greetingMocks.resolveSystemAgentGreeting.mockReset().mockResolvedValue({
    text: "I'm Bot. All systems nominal.",
    source: "model",
  });
...
async function callChat(
  ...
  await expectDefined(
    systemAgentHandlers["bot.chat"],
    'systemAgentHandlers["bot.chat"] test invariant',
  )({
    params,
    respond,
    context,
...
describe("bot.setup.activate", () => {
  ...
  it("returns a retryable busy error while another activation is running", async () => {
    ...
    try {
      const { calls, respond } = makeRespond();
      await expectDefined(
        systemAgentHandlers["bot.setup.activate"],
        'systemAgentHandlers["bot.setup.activate"] test invariant',
      )({
      ...
      expect(calls).toEqual([
        {
          ...
          error: {
            code: "UNAVAILABLE",
            message: "Bot setup is already in progress; try again when it finishes.",
            retryable: true,
          },
        },
      ]);
...
describe("bot.setup.auth.start", () => {
  it("starts provider auth as an interactive wizard session", async () => {
    ...
    await expectDefined(
      systemAgentHandlers["bot.setup.auth.start"],
      'systemAgentHandlers["bot.setup.auth.start"] test invariant',
    )({
...
describe("bot.setup.prepare.start", () => {
  it("runs the selected provider method in a shared wizard session and commits its config", async () => {
    ...
    await expectDefined(
      systemAgentHandlers["bot.setup.prepare.start"],
      'systemAgentHandlers["bot.setup.prepare.start"] test invariant',
    )({
      params: {
...
describe("bot.chat", () => {
  it("refuses to create a session before inference is available", async () => {
    ...
    expect(call).toMatchObject({
      ...
      error: {
        code: "UNAVAILABLE",
        message: "Bot requires working inference: no configured model",
      },
    });
  ...
  it("keeps read-only setup detection outside the serialized system-agent lane", async () => {
    ...
    const pending = expectDefined(
      systemAgentHandlers["bot.setup.detect"],
      'systemAgentHandlers["bot.setup.detect"] test invariant',
    )({
      params: {},
      respond: () => {
  ...
  it.each([
    ...
    await expectDefined(
      systemAgentHandlers["bot.setup.verify"],
      'systemAgentHandlers["bot.setup.verify"] test invariant',
  ...
  it("returns history oldest-first with default and explicit bounded limits", async () => {
    ...
    const invoke = async (params: Record<string, unknown>) => {
      const { calls, respond } = makeRespond();
      await expectDefined(
        systemAgentHandlers["bot.chat.history"],
        'systemAgentHandlers["bot.chat.history"] test invariant',
      )({ params, respond } as never);
      return calls[0];
    };
