...
describe("webHandlers web.login.start", () => {
  ...
  it("surfaces the missing official external plugin hint when no web-login provider is loaded", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHint.mockReturnValue({
      ...
      channelId: "whatsapp",
      label: "WhatsApp",
      installSpec: "clawhub:@hanzo/bot-whatsapp",
      installCommand: "bot plugins install clawhub:@hanzo/bot-whatsapp",
      doctorFixCommand: "bot doctor --fix",
      repairHint:
        "Install the official external plugin with: bot plugins install clawhub:@hanzo/bot-whatsapp, or run: bot doctor --fix.",
    });
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message:
          "web login provider is not available. Install the official external plugin with: bot plugins install clawhub:@hanzo/bot-whatsapp, or run: bot doctor --fix.",
      }),
    );
  ...
  it("joins multiple missing official external plugin hints when more than one configured channel is missing", async () => {
        ...
        channelId === "whatsapp"
          ...
          : channelId === "signal"
            ? {
                ...
                channelId: "signal",
                label: "Signal",
                installSpec: "clawhub:@hanzo/bot-signal",
                installCommand: "bot plugins install clawhub:@hanzo/bot-signal",
                doctorFixCommand: "bot doctor --fix",
                repairHint:
                  "Install the official external plugin with: bot plugins install clawhub:@hanzo/bot-signal, or run: bot doctor --fix.",
              }
            : null,
    );
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        code: "INVALID_REQUEST",
        message:
          "web login provider is not available. Configured official external channel plugins are missing for WhatsApp, Signal. Install them with: bot plugins install clawhub:@hanzo/bot-whatsapp; bot plugins install clawhub:@hanzo/bot-signal, or run: bot doctor --fix.",
      }),
    );
  });
