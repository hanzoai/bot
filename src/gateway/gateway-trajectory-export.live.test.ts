...
const CODEX_HARNESS_LIVE = process.env.BOT_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.BOT_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["BOT_TRAJECTORY", "BOT_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-bot-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.BOT_TRAJECTORY = "1";
      process.env.BOT_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".bot", "trajectory-exports", "bundle");
