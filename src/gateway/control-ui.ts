...
const ROOT_PREFIX = "/";
const CONTROL_UI_ASSISTANT_MEDIA_PREFIX = "/__bot__/assistant-media";
...
const CONTROL_UI_NAMESPACE_PREFIX = "/__bot__/";
...
// `/__bot__/`, so a browser that opens the default entry infers
// `/__bot__` as its base path (see `inferBasePathFromPathname`) and fetches
// `/__bot__/control-ui-config.json`. Accept that namespaced alias so the
...
// Single-underscore `/__bot` prefix used by the pre-base-path-relative
...
// `/__bot/control-ui-config.json`, so current main and the v2026.6.1
// release serve and document that exact path under an empty base path.
const LEGACY_CONTROL_UI_NAMESPACE_PREFIX = "/__bot";

// The old documented no-base-path bootstrap endpoint
// (`/__bot/control-ui-config.json`, single underscore). It is derived from
// the legacy `/__bot` namespace joined with the canonical config constant
 ...
 * single-underscore suffix `${basePath}/__bot/control-ui-config.json` that
 ...
 * the default-namespace alias `/__bot__/control-ui-config.json`, which is
 * what the default `/__bot__/` entry requests after inferring its base path
...
function matchesControlUiBootstrapConfigPath(pathname: string, basePath: string): boolean {
  ...
  // `/__bot` namespace, so joining it with the base path yields
  // `${basePath}/__bot/control-ui-config.json` (or the bare legacy path
  ...
  // The default `/__bot__/` namespace alias only applies when no base path
