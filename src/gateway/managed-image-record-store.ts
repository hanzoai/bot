...
function stateDatabaseOptions(stateDir?: string): BotStateDatabaseOptions {
  return stateDir
...
export function readManagedImageRecord(
  ...
  const database = openBotStateDatabase(stateDatabaseOptions(stateDir));
...
export function listManagedImageRecordEntries(params: {
  ...
  const database = openBotStateDatabase(stateDatabaseOptions(params.stateDir));
