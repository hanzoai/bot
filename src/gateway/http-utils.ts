...
export const BOT_MODEL_ID = "bot";
/** Default OpenAI-compatible model alias that targets the default Bot agent. */
export const BOT_DEFAULT_MODEL_ID = "bot/default";
...
class GatewaySessionKeyOverrideError extends Error {
  constructor() {
    super("`x-bot-session-key` cannot use reserved internal session namespaces.");
...
function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-bot-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-bot-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === BOT_MODEL_ID || lowered === BOT_DEFAULT_MODEL_ID) {
...
/** Validates and resolves the `x-bot-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `bot` or `bot/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-bot-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-bot-model`." };
  }
