...
describe("message-channel", () => {
  ...
  it("classifies the browser copilot as a dedicated browser operator UI", () => {
    const client = { id: "bot-browser-copilot", mode: "ui" };
    ...
    expect(isBrowserCopilotClient({ id: "bot-browser-copilot", mode: "webchat" })).toBe(true);
  });
  ...
  it("reads native approval behavior from bundled channel manifests", async () => {
    ...
    const previousTrust = process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    process.env.BOT_BUNDLED_PLUGINS_DIR = path.resolve("extensions");
    ...
    try {
      ...
      if (previousTrust === undefined) {
        ...
        process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrust;
      }
      vi.resetModules();
    }
  });
