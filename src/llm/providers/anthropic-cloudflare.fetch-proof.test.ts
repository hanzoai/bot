import type { Context, Model } from "@hanzo/bot-ai";
...
// ever reached. This proves the stream facade installs Bot's guarded
...
// Importing the facade installs the Bot AI transport host ports.
import "../stream.js";
...
describe("Anthropic Cloudflare guard-specific SSRF blocking proof", () => {
  ...
  it("blocks a private-IP request before globalThis.fetch is called (guard-specific behavior)", async () => {
    ...
    const { streamAnthropic } = await import("@hanzo/bot-ai/internal/anthropic");
