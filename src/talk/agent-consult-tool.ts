/**
 * Realtime voice tool definition and helpers for delegating work to Bot.
 *
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME = "bot_agent_consult";
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured Bot agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final Bot result before answering with the actual result.`,
  };
}
...
/** Resolve the Bot tool allowlist paired with the consult exposure policy. */
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  if (config.consultPolicy === "always") {
    return [
      "Consult behavior:",
      "- Call bot_agent_consult before every substantive answer.",
  ...
  return [
    ...
    "- Call bot_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's Bot-specific context.",
...
/** Build the delegated Bot agent prompt for a live voice consult. */
