/**
 * Shared realtime voice controls for active Bot agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "bot_agent_control";
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Control an active Bot tool-backed voice run. Use this when the caller asks in any language for status/progress, cancellation, a redirect/change to the active work, or a follow-up after the current work. Do not use this for ordinary greetings or chatter unless the caller is asking about the active work.",
  parameters: {
    type: "object",
    properties: {
      text: {
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal Bot voice control result.",
    "Do not call bot_agent_consult or any other tool for this message.",
    "Speak this exact Bot status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "Bot is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "Bot has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "Bot could not queue that follow-up."
    : "Bot could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "Bot finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `Bot is starting ${name}.` : "Bot is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `Bot finished ${name} and is continuing.`
        : "Bot finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `Bot is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "Bot is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `Bot is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "Bot is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "Bot is working on the current voice request.";
  }
