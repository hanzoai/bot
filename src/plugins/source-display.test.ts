...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "bot", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".bot", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".bot", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "bot-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".bot", "extensions"),
      },
    });
  });
});
