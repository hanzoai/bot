...
type LoadBotPlugins = typeof import("./loader.js").loadBotPlugins;
...
const loadBotPluginsMock = vi.fn<LoadBotPlugins>();
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadBotPluginsMock, "Bot plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadBotPluginsMock, "Bot plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  ...
  const autoEnabledConfig: BotConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadBotPlugins: (...args: Parameters<LoadBotPlugins>) =>
        loadBotPluginsMock(...args),
