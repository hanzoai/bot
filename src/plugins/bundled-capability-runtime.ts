...
import type { BotPluginDefinition, BotPluginModule } from "./types.js";
...
function buildVitestCapabilityShimAliasMap(): Record<string, string> {
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        [`bot/plugin-sdk/${subpath}`, targetPath],
        [`@hanzo/bot-plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
}
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  const discovery = params.discovery ?? discoverBotPlugins({ env });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: BotPluginModule | null;
    try {
      mod = getModuleLoader(safeSource)(safeSource) as BotPluginModule;
    } catch (error) {
