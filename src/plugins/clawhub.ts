...
function isTrustedSourceLinkedOfficialPackage(pkg: NonNullable<ClawHubPackageDetail["package"]>) {
  ...
  return (
    ...
    (sourceRepo === "hanzoai/bot" ||
      sourceRepo === "github.com/hanzoai/bot" ||
      sourceRepo === "https://github.com/hanzoai/bot")
  );
}
...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    ...
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "bot skills install ${installRef}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Bot runtime exposes ${runtimeVersion}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires Bot >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
...
> {
  ...
  try {
    ...
    const clawhubAuthority = isDefaultClawHubBaseUrl(params.baseUrl) ? "bot" : "third-party";
    params.logger?.info?.(
