  ...
  it.each([
    {
      ...
      input: {
        name: "@hanzo/bot-demo",
        version: "1.2.3",
        resolvedSpec: "@hanzo/bot-demo@1.2.3",
...
describe("resolveNpmInstallRecordSpec", () => {
  it("uses an exact resolved registry spec when managed installs request pinning", () => {
    expect(
      resolveNpmInstallRecordSpec({
        requestedSpec: "@hanzo/bot-codex",
        resolution: {
          name: "@hanzo/bot-codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@hanzo/bot-codex@2026.5.30-beta.1",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@hanzo/bot-codex@2026.5.30-beta.1");
  });
    ...
    expect(
      resolveNpmInstallRecordSpec({
        ...
        resolution: {
          name: "@hanzo/bot-codex",
          version: "2026.5.30-beta.1",
          resolvedSpec: "@hanzo/bot-codex@beta",
        },
        pinResolvedRegistrySpec: true,
      }),
    ).toBe("@hanzo/bot-codex");
...
describe("recordPluginInstall", () => {
  ...
  it("replaces local npm-pack provenance on a registry reinstall", () => {
    const localInstall = recordPluginInstall(
      {},
      {
        pluginId: "diffs",
        source: "npm",
        spec: "/tmp/bot-diffs.tgz",
        sourcePath: "/tmp/bot-diffs.tgz",
        installPath: "/tmp/bot/plugins/diffs-local",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmTarballName: "bot-diffs-1.0.0.tgz",
    ...
    const registryInstall = recordPluginInstall(localInstall, {
      ...
      installPath: "/tmp/bot/plugins/diffs",
      resolvedName: "@hanzo/bot-diffs",
      resolvedVersion: "1.1.0",
      resolvedSpec: "@hanzo/bot-diffs@1.1.0",
      installedAt: "2026-05-15T00:00:03.000Z",
    });
  ...
  it("moves an exact prior npm load path on same-version reinstall", () => {
    const previousInstallPath = "/tmp/bot/npm/projects/alpha-v1/node_modules/alpha";
    const nextInstallPath = "/tmp/bot/npm/projects/alpha-v2/node_modules/alpha";
    const customPath = `${previousInstallPath}/custom-child`;
    const adjacentPath = "/tmp/bot/npm/projects/beta/node_modules/beta";
    const existing = {
      plugins: {
