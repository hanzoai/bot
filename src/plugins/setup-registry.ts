...
function collectConfiguredPluginEntryIds(config: BotConfig): string[] {
...
function resolveRegister(mod: BotPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: BotPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as BotPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: BotPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Bot ignored the plugin setup runtime entry. Remove setup-api/bot.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
