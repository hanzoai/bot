...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("clears bundled dist discovery chunks from the dist root", () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      manifestRegistry: {
        plugins: [
          {
            ...createManifestPlugin("deepseek"),
            rootDir: "/tmp/bot/dist/extensions/deepseek",
            manifestPath: "/tmp/bot/dist/extensions/deepseek/bot.plugin.json",
            providerDiscoverySource: "/tmp/bot/dist/extensions/deepseek/provider-discovery.js",
          },
        ],
        diagnostics: [],
      },
    ...
    expect(mocks.clearNativeRequireJavaScriptModuleCache).toHaveBeenCalledWith(
      "/tmp/bot/dist/extensions/deepseek/provider-discovery.js",
      { dependencyRoot: "/tmp/bot/dist" },
    );
  });
