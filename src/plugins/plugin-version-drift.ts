...
export function resolvePluginVersionDriftUpdateCommand(entry: PluginVersionDriftEntry): string {
  ...
  if (exactNpmPackageName) {
    ...
    if (parseRegistryNpmSpec(exactNpmTarget)?.selectorKind === "exact-version") {
      return `bot plugins update ${exactNpmTarget}`;
    }
  }
  return `bot plugins update ${entry.pluginId}`;
}
...
function isPluginEnabled(config: BotConfig | undefined, pluginId: string): boolean {
 ...
 *   `version` field of the installed bot package.json).
 ...
 * @param params.config The merged daemon-side BotConfig (optional).
