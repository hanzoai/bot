...
function metadataSnapshot(params: {
  ...
  const packageName =
    params.packageName === null ? undefined : (params.packageName ?? `@hanzo/bot-${id}`);
  const manifest = {
    id,
...
describe("plugin management Featured authority", () => {
  it("projects listing metadata from a top-level hosted feed entry", async () => {
    ...
    const officialCatalog = {
      entries: [
        hostedFeedEntry({
          packageName: "@expediagroup/expedia-bot",
  ...
  it("surfaces a newly featured live official package without static fallback metadata", async () => {
    ...
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([
        hostedFeedEntry({
          packageName: "@hanzo/bot-new-tool",
          title: "New Tool",
          featured: true,
        }),
      ]),
    ...
    expect(catalog.plugins).toEqual([
      expect.objectContaining({
        id: "@hanzo/bot-new-tool",
        name: "New Tool",
        featured: true,
        install: { source: "official", pluginId: "@hanzo/bot-new-tool" },
      }),
    ]);
  });
  ...
  it("orders live featured packages by when they were featured", async () => {
    ...
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([
        hostedFeedEntry({
          packageName: "@hanzo/bot-older-popular",
          title: "Older Popular",
          featured: true,
          featuredAt: 100,
          order: 1,
        ...
        hostedFeedEntry({
          packageName: "@hanzo/bot-newest-featured",
        ...
        hostedFeedEntry({
          packageName: "@hanzo/bot-legacy-featured",
    ...
    expect(catalog.plugins.map((plugin) => plugin.id)).toEqual([
      "@hanzo/bot-newest-featured",
      "@hanzo/bot-older-popular",
      "@hanzo/bot-legacy-featured",
    ]);
  ...
  it("lets a live unfeature override an installed published plugin manifest", async () => {
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        name: "Diffs",
        origin: "global",
        installRecord: { source: "npm", spec: "@hanzo/bot-diffs" },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(
      hostedCatalog([{ ...hostedFeedDiffsEntry, featured: false }]),
  ...
  it.each([
    { id: "workboard", name: "Workboard", packageName: "@hanzo/bot-workboard" },
    { id: "open-prose", name: "OpenProse", packageName: "@hanzo/bot-open-prose" },
    { id: "memory-wiki", name: "Memory Wiki", packageName: "@hanzo/bot-memory-wiki" },
  ...
  it("applies hosted curation to the exact published package for bundled FireCrawl", async () => {
    ...
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        id: "firecrawl",
        name: "firecrawl",
        packageName: "@hanzo/bot-firecrawl-plugin",
        featured: false,
        description: "Optional Bot capability.",
  ...
  it("preserves npm-only bundled curation outside the hosted producer identity", async () => {
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        installRecord: { source: "npm", spec: "@hanzo/bot-acpx" },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(hostedCatalog([]));
    ...
    mocks.metadata.mockReturnValue(
      metadataSnapshot({
        ...
        installRecord: {
          ...
          clawhubPackage: "@hanzo/bot-new-tool",
        },
      }),
    );
    mocks.officialCatalog.mockResolvedValue(hostedCatalog([]));
