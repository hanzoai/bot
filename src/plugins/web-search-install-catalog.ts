...
import { normalizeOptionalString as normalizeString } from "@hanzo/bot-normalization-core/string-coerce";
...
function getConfigPath(config: BotConfig | undefined, path: string): unknown {
...
function setConfigPath(target: BotConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: BotConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: BotConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: BotConfig) =>
