// Shared policy and messaging for installs outside Bot's trusted plugin sources.
...
export function resolveBotTrustedNpmPackageInstall(
  npmSpec: string,
...
export function isBotTrustedPluginInstallSpec(
  spec: string,
  bundledSources: ReadonlyMap<string, BundledPluginSource> = getProcessBundledPluginSources(),
): boolean {
  ...
  if (explicitNpm) {
    return resolveBotTrustedNpmPackageInstall(npmSpec, bundledSources) !== null;
  }
  ...
  return Boolean(
    bundled ??
    resolveBotTrustedNpmPackageInstall(npmSpec, bundledSources) ??
