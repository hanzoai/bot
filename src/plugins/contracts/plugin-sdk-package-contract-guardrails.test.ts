...
const PLUGIN_SDK_SUBPATH_PATTERN = /bot\/plugin-sdk\/([a-z0-9][a-z0-9-]*)\b/g;
...
const DEPRECATED_EXTENSION_SDK_SPECIFIERS = new Set([
  ...
  "bot/plugin-sdk/channel-config-primitives",
  "bot/plugin-sdk/channel-config-schema-legacy",
  "bot/plugin-sdk/compat",
  "bot/plugin-sdk/test-utils",
]);
const DEPRECATED_TEST_ALIAS_SPECIFIERS = new Set(["bot/plugin-sdk/test-utils"]);
...
const LEGACY_MEMORY_EMBEDDING_PROVIDER_MANIFEST_FILES = new Set([
  "extensions/amazon-bedrock/bot.plugin.json",
  "extensions/github-copilot/bot.plugin.json",
  "extensions/google/bot.plugin.json",
  "extensions/lmstudio/bot.plugin.json",
  "extensions/memory-core/bot.plugin.json",
  "extensions/mistral/bot.plugin.json",
  "extensions/ollama/bot.plugin.json",
  "extensions/openai/bot.plugin.json",
  "extensions/voyage/bot.plugin.json",
]);
...
function collectDeprecatedExtensionSdkImports(): Array<{ file: string; specifier: string }> {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](bot\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
    /\bimport\s*\(\s*["'](bot\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](bot\/plugin-sdk(?:\/[a-z0-9][a-z0-9-]*)?)["']/g,
  ];
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    const repoRelativePath = toRepoRelativePath(file);
    const source = fs.readFileSync(file, "utf8");
...
function collectNewDeprecatedMemoryEmbeddingProviderManifestFiles(): string[] {
  ...
  const manifestFiles =
    listGitTrackedFiles({
      repoRoot: REPO_ROOT,
      pathspecs: "extensions/**/bot.plugin.json",
    }) ?? [];
...
function collectDeprecatedTestAliasImports(): string[] {
  const leaks: Array<{ file: string; specifier: string }> = [];
  const importPatterns = [
    /\b(?:import|export)\b[\s\S]*?\bfrom\s*["'](bot\/plugin-sdk\/test-utils)["']/g,
    /\bimport\s*\(\s*["'](bot\/plugin-sdk\/test-utils)["']\s*\)/g,
    /\bvi\.(?:mock|doMock)\s*\(\s*["'](bot\/plugin-sdk\/test-utils)["']/g,
  ];
...
function collectCrossOwnerReservedSdkImports(): Array<{
  ...
  for (const file of collectExtensionFiles(resolve(REPO_ROOT, "extensions"))) {
    ...
    for (const match of source.matchAll(importPattern)) {
      ...
      leaks.push({
        file: repoRelativePath,
        specifier: `bot/plugin-sdk/${subpath}`,
        owner,
      });
    }
  }
    ...
    for (const importPattern of importPatterns) {
      for (const match of source.matchAll(importPattern)) {
        ...
        if (subpath && subpaths.has(subpath)) {
          imports.add(`${repoRelativePath}: bot/plugin-sdk/${subpath}`);
        }
      }
    }
  }
...
describe("plugin-sdk package contract guardrails", () => {
  ...
  it("keeps curated public plugin-sdk references on exported built subpaths", () => {
    ...
    for (const reference of collectPluginSdkSubpathReferences()) {
      ...
      failures.push(
        `${reference.file} references bot/plugin-sdk/${reference.subpath}, but ${reference.subpath} is missing from ${missingFrom.join(" and ")}`,
      );
    }
  ...
  it("keeps Matrix dependencies local to the Matrix plugin", () => {
    ...
    expect(rootRuntimeDeps.has("@hanzo/bot-plugin-package-contract")).toBe(false);
  });
