...
const fixtureRoot = makeTrackedTempDir("bot-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadBotPlugins, clearPluginLoaderCache } = await import("./loader.test-fixtures.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "bot.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("bot plugins inspect <id> --runtime --json");
    expect(summary).toContain("bot plugins list");
  });
});
