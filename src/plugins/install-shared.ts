...
import { resolvePackageExtensionEntries, type BotPackageManifest } from "./manifest.js";
...
export function formatUnresolvedBotPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares bot as a peer dependency, but Bot could not create a plugin-local node_modules/bot link. Run from a packaged Bot install or reinstall Bot, then retry.`;
}
...
function validateBotPackageCompatibility(params: {
  pluginId: string;
  currentHostVersion: string;
  packageMetadata?: BotPackageManifest;
  ...
  if (!pluginApiRangeCheck.ok) {
    return {
      ok: false,
      error: `invalid package.json bot.compat.pluginApi: ${pluginApiRangeCheck.error}`,
  ...
  if (pluginApiRange && !satisfiesPluginApiRange(params.currentHostVersion, pluginApiRange)) {
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires plugin API ${pluginApiRange}, but this Bot runtime exposes ${params.currentHostVersion}. Upgrade Bot or install a compatible plugin version and retry.`,
...
export function validateBotPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json bot.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${params.pluginId}" requires Bot >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set BOT_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires Bot >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Bot and retry.`,
      code: PLUGIN_INSTALL_ERROR_CODE.INCOMPATIBLE_HOST_VERSION,
    };
  }

  return validateBotPackageCompatibility({
...
export function ensureBotExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
...
export async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "bot security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
