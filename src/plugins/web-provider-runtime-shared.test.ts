...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadBotPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadBotPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadBotPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to plugin loading when bundled runtime artifacts do not cover the scope", () => {
    ...
    mocks.loadBotPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("loads the plugin registry when runtime activation is explicitly requested", () => {
    ...
    expect(mocks.loadBotPlugins).toHaveBeenCalledTimes(1);
    expect(mockArg(mocks.loadBotPlugins).activate).toBe(true);
  });
  ...
  it("does not treat an active registry missing declared candidates as authoritative", () => {
    ...
    mocks.loadBotPlugins.mockReturnValue(scopedRegistry as never);
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadBotPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadBotPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
