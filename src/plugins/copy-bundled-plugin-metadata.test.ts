...
const excludeOptionalEnv = { BOT_INCLUDE_OPTIONAL_BUNDLED: "0" } as const;
...
function createPlugin(
  ...
  writeJson(path.join(pluginDir, "bot.plugin.json"), {
    id: params.id,
  ...
  writeJson(path.join(pluginDir, "package.json"), {
    ...
    ...(params.packageBot ? { bot: params.packageBot } : {}),
  });
  return pluginDir;
}
...
function readBundledManifest(repoRoot: string, pluginId: string): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, "dist", "extensions", pluginId, "bot.plugin.json"),
      "utf8",
    ),
...
function createTlonSkillPlugin(repoRoot: string, skillPath = "node_modules/@tloncorp/tlon-skill") {
  return createPlugin(repoRoot, {
    id: "tlon",
    packageName: "@hanzo/bot-tlon",
    manifest: { skills: [skillPath] },
    packageBot: { extensions: ["./index.ts"] },
  });
}
...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("bot-bundled-plugin-meta-");
    ...
    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "bot.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    expect(packageJson.bot?.extensions).toEqual(["./index.js"]);
  });

  it("copies generated bundled channel config schemas into dist manifests", () => {
    const repoRoot = makeRepoRoot("bot-bundled-channel-config-meta-");
    createPlugin(repoRoot, {
      id: "telegram",
      packageName: "@hanzo/bot-telegram",
      manifest: {
        channels: ["telegram"],
        channelConfigs: {
          telegram: {
  ...
  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("bot-bundled-plugin-node-modules-");
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("bot-bundled-plugin-hoisted-skill-");
    const pluginDir = createTlonSkillPlugin(repoRoot);
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("bot-bundled-plugin-missing-skill-");
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("bot-bundled-plugin-retry-");
    const pluginDir = createPlugin(repoRoot, {
      id: "diffs",
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("bot-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
    ...
    writeJson(path.join(repoRoot, "dist", "extensions", "removed-plugin", "bot.plugin.json"), {
      id: "removed-plugin",
      configSchema: { type: "object" },
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("bot-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "bot.plugin.json"), {
  ...
  it("removes non-packaged private QA plugin metadata unless private QA build is enabled", () => {
    const repoRoot = makeRepoRoot("bot-private-qa-metadata-");
    createPlugin(repoRoot, {
      id: "qa-lab",
      packageName: "@hanzo/bot-qa-lab",
      packageBot: { extensions: ["./index.ts"] },
    });
    ...
    copyBundledPluginMetadataWithEnv({
      repoRoot,
      env: { BOT_BUILD_PRIVATE_QA: "1" } as NodeJS.ProcessEnv,
    });

    expect(fs.existsSync(path.join(staleDistDir, "bot.plugin.json"))).toBe(true);
  ...
  it.each([
    {
      ...
      pluginId: "acpx",
      packageName: "@hanzo/bot-acpx-plugin",
      packageBot: { extensions: ["./index.ts"] },
    ...
    },
    {
      ...
      packageBot: {
        extensions: ["./index.ts"],
        install: { npmSpec: "@hanzo/bot-whatsapp" },
      },
      env: {},
      expectedExists: false,
    },
  ] as const)("$name", ({ pluginId, packageName, packageBot, env, expectedExists }) => {
    const repoRoot = makeRepoRoot(`bot-bundled-plugin-${pluginId}-`);
    createPlugin(repoRoot, {
      id: pluginId,
      packageName,
      packageBot,
  ...
  it("removes build-excluded bundled plugin metadata", () => {
    const repoRoot = makeRepoRoot("bot-bundled-plugin-excluded-meta-");
    createPlugin(repoRoot, {
      id: "qqbot",
      packageName: "@hanzo/bot-qqbot",
      packageBot: {
        extensions: ["./index.ts"],
  ...
  it("preserves manifest-less runtime support package outputs and copies package metadata", () => {
    const repoRoot = makeRepoRoot("bot-bundled-runtime-support-");
    const pluginDir = path.join(repoRoot, "extensions", "image-generation-core");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@hanzo/bot-image-generation-core",
      version: "0.0.1",
      private: true,
      type: "module",
    });
    ...
    expect(
      fs.existsSync(
        path.join(repoRoot, "dist", "extensions", "image-generation-core", "bot.plugin.json"),
      ),
    ).toBe(false);
    expect(
      JSON.parse(
