...
import { normalizeBotVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeBotVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeBotVersionBase(plugin.version) ??
    plugin.version
  );
}
