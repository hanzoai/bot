...
function formatPluginInspectCommand(pluginId: string): string {
  return `bot plugins inspect ${quoteCliArg(pluginId)}`;
}
...
export function warnWhenAllowlistIsOpen(params: {
  ...
  const remediation = truncated
    ? "Run 'bot plugins list --enabled --verbose' to enumerate every discovered plugin id, inspect trusted ids with 'bot plugins inspect <id>', and add the ones you trust to plugins.allow in bot.json."
    : `To trust them explicitly, set plugins.allow in bot.json (e.g. "plugins": { "allow": [${autoDiscoverable
        .map((entry) => JSON.stringify(entry.id))
        .join(
          ", ",
        )}] }). Run 'bot plugins list --enabled --verbose' or ${inspectCommands} to confirm plugin ids.`;
