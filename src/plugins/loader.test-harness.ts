...
export function setupBundledDreamingMemoryPlugins(params?: {
  ...
  fs.writeFileSync(
    path.join(selectedMemoryDir, "bot.plugin.json"),
    JSON.stringify(
      {
        id: selectedId,
...
export function makeBotDevSourceRoot() {
...
export function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".bot", "extensions", params.id);
...
export function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ BOT_STATE_DIR: stateDir }, () => run(stateDir));
}
...
export function expectTelegramLoaded(registry: ReturnType<typeof loadBotPlugins>) {
...
export function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadBotPlugins>[0], "cache" | "config">,
) {
  return loadBotPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadBotPlugins>,
  pluginId: string,
) {
...
export function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadBotPlugins>;
  loadSecond: () => ReturnType<typeof loadBotPlugins>;
...
export function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadBotPlugins>;
  loadVariant: () => ReturnType<typeof loadBotPlugins>;
}) {
  const first = params.loadFirst();
...
export function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const botHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(botHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
