...
describe("install security scan official bypass", () => {
  it("bypasses plugin install friction for bundled Bot sources", async () => {
    const result = await scanBundleInstallSourceRuntime({
      logger: {},
      pluginId: "bot/kitchen-sink",
      sourceDir: "/tmp/bot-bundled-plugin",
      source: { kind: "bundled", authority: "bot", mutable: false, network: false },
    });
  ...
  it("bypasses plugin install friction for official ClawHub sources", async () => {
    const result = await scanBundleInstallSourceRuntime({
      logger: {},
      pluginId: "@hanzo/bot-matrix",
      sourceDir: "/tmp/bot-official-clawhub-plugin",
  ...
  it("bypasses skill install friction for bundled Bot sources", async () => {
    const result = await evaluateSkillInstallPolicyRuntime({
      ...
      source: { kind: "bundled", authority: "bot", mutable: false, network: false },
      skillName: "peekaboo",
      sourceDir: "/tmp/bot-bundled-skill/peekaboo",
    });
  ...
  it("runs only operator policy for official immutable npm sources", async () => {
    const result = await preflightPluginNpmInstallPolicyRuntime({
      ...
      requestedSpecifier: "@hanzo/bot-matrix@latest",
      source: { kind: "npm", authority: "official", mutable: false, network: true },
      sourcePath: "/tmp/bot-official-npm",
