...
const mocks = vi.hoisted(() => ({
  ...
  loadBotPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadBotPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadBotPluginCliRegistry(...args),
  loadBotPlugins: (...args: unknown[]) => mocks.loadBotPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  ...
  const loadOptions = getMockCallObject(mocks.loadBotPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadBotPluginCliRegistry.mockReset();
    mocks.loadBotPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadBotPlugins.mockReset();
    mocks.loadBotPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as BotConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as BotConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as BotConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as BotConfig);

    const loadOptions = getMockCallObject(mocks.loadBotPlugins) as {
  ...
  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    ...
    await registerPluginCliCommands(program, {} as BotConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as BotConfig);
  ...
  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as BotConfig;
    const configB = { plugins: {} } as BotConfig;
    const envA = { BOT_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { BOT_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadBotPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadBotPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadBotPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadBotPluginCliRegistry.mockImplementationOnce((options: { logger?: unknown }) => {
    ...
    await expect(
      getPluginCliCommandDescriptors({ plugins: { entries: { stale: {} } } } as BotConfig),
    ).resolves.toEqual([]);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadBotPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as BotConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadBotPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadBotPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadBotPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadBotPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("skips full plugin runtime loading when no metadata owns the requested primary", async () => {
    ...
    expect(mocks.loadBotPluginCliRegistry).toHaveBeenCalled();
    expect(mocks.loadBotPlugins).not.toHaveBeenCalled();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as BotConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
