...
function makeTempDir() {
  return makeTrackedTempDir("bot-plugin-registry", tempDirs);
}
...
describe("plugin registry facade", () => {
  ...
  it("resolves contribution owners from a plugin lookup table without rereading manifests", () => {
    ...
    fs.unlinkSync(path.join(rootDir, "bot.plugin.json"));
  ...
  it("reads the persisted registry before deriving from discovered candidates", async () => {
    ...
    fs.writeFileSync(
      path.join(persistedRootDir, "bot.plugin.json"),
        ...
        plugins: [
          {
            ...
            manifestPath: path.join(persistedRootDir, "bot.plugin.json"),
            manifestHash: hashFile(path.join(persistedRootDir, "bot.plugin.json")),
  ...
  it("falls back to the derived registry when persisted bundled roots point at another checkout", async () => {
        ...
        plugins: [
          {
            ...
            manifestPath: path.join(staleBundledRootDir, "bot.plugin.json"),
    ...
    const result = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      candidates: [candidate],
      env: hermeticEnv({ BOT_BUNDLED_PLUGINS_DIR: rootDir }),
    });
  ...
  it("refreshes stale built records and accepts source records for dist-opt-out plugins", async () => {
    ...
    const packageRoot = path.join(tempRoot, "bot");
    ...
    const packageJson = JSON.stringify({
      bot: sourceCandidate.packageManifest,
    });
    fs.writeFileSync(path.join(sourceRoot, "package.json"), packageJson);
    fs.copyFileSync(
      path.join(sourceRoot, "bot.plugin.json"),
      path.join(builtRoot, "bot.plugin.json"),
    );
    ...
    const env = hermeticEnv({
      BOT_BUNDLED_PLUGINS_DIR: path.dirname(builtRoot),
    ...
    for (const plugin of staleBuiltIndex.plugins) {
      ...
      plugin.manifestPath = path.join(builtRoot, "bot.plugin.json");
      delete plugin.packageBuild;
    }
  ...
  it("reuses config-scoped derived registries within the process", () => {
    ...
    createCandidate(rootDir);
    const env = hermeticEnv({ BOT_BUNDLED_PLUGINS_DIR: bundledRoot });
    ...
    const manifestReadsAfterFirst = readFileSyncSpy.mock.calls.filter((call) =>
      String(call[0]).endsWith("bot.plugin.json"),
    ).length;
  ...
  it("reloads profile extensions after the metadata lifecycle is cleared", () => {
    ...
    const env = hermeticEnv({
      BOT_CONFIG_PATH: path.join(configDir, "bot.json"),
  ...
  it("keys the process registry memo by resolved host contract version", () => {
    ...
    const first = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      config,
      env: hermeticEnv({ BOT_BUNDLED_PLUGINS_DIR: bundledRoot }),
    });
    const second = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      config,
