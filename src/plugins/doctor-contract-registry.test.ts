...
function makeTempDir(): string {
  return makeTrackedTempDir("bot-doctor-contract-registry", tempDirs);
}
...
describe("doctor-contract-registry module loader", () => {
  ...
  it("loads config-derived session-store agent IDs from doctor contract modules", () => {
    ...
    mocks.loadPluginManifestRegistry.mockReturnValue({
      plugins: [{ id: "test-plugin", packageName: "@hanzo/bot-demo", rootDir: pluginRoot }],
      diagnostics: [],
    });
