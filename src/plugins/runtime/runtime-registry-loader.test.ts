...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadBotPlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
vi.mock("../loader.js", () => ({
  loadBotPlugins: (...args: Parameters<typeof mocks.loadBotPlugins>) =>
    mocks.loadBotPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadBotPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadBotPlugins>) => mocks.loadBotPlugins(...args),
    );
