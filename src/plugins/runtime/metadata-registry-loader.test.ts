...
vi.mock("../loader.js", () => ({
  loadBotPlugins: (...args: unknown[]) => loadBotPluginsMock(...args),
}));
...
function getOnlyLoadBotPluginsOptions(): PluginLoadOptions {
  expect(loadBotPluginsMock).toHaveBeenCalledTimes(1);
  const options = loadBotPluginsMock.mock.calls[0]?.[0];
  if (!options || typeof options !== "object") {
    throw new Error("expected loadBotPlugins to receive plugin load options");
  }
...
describe("loadPluginMetadataRegistrySnapshot", () => {
  ...
  beforeEach(() => {
    ...
    loadBotPluginsMock.mockReturnValue({ plugins: [], diagnostics: [] });
  });
  ...
  it("defaults to a non-activating validate snapshot", () => {
    ...
    const loadOptions = getOnlyLoadBotPluginsOptions();
  ...
  it("forwards an explicit logger through metadata snapshots", () => {
    ...
    expect(getOnlyLoadBotPluginsOptions()).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: {} },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
  ...
  it("honors explicit load options when reusing a resolved runtime context", () => {
    ...
    expect(getOnlyLoadBotPluginsOptions()).toEqual({
      config: { plugins: { allow: ["compat-provider"] } },
      activationSourceConfig: { plugins: { allow: ["raw-plugin"] } },
      autoEnabledReasons: {},
      workspaceDir: "/compat-workspace",
