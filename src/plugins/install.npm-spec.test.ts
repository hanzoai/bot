...
const resolveBotPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/bot-root.js", () => ({
  resolveBotPackageRootSync: (...args: unknown[]) =>
    resolveBotPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("bot-plugin-install-npm-spec");
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootBot) {
      lockPackages["node_modules/bot"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalBotPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "bot"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.bot ? { bot: viewPackage.bot } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalBotPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "bot") {
            const botRoot = path.join(npmRoot, "node_modules", "bot");
            fs.mkdirSync(botRoot, { recursive: true });
            fs.writeFileSync(
              path.join(botRoot, "package.json"),
              JSON.stringify({ name: "bot", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootBot) {
            ...
            fs.writeFileSync(
              path.join(botRoot, "package.json"),
              JSON.stringify({ name: "bot", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "bot") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "bot"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveBotPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "bot", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveBotPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "bot-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@hanzo/bot-pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
  ...
  const dependencySpec = managedManifest.dependencies?.["@hanzo/bot-pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  ...
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@hanzo/bot-pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@hanzo/bot-pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("bot-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@hanzo/bot-pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_bot-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_bot-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@hanzo/bot-pack-demo";
    const archiveV1Path = path.join(stateDir, "bot-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "bot-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@hanzo/bot-pack-demo";
    const archivePath = path.join(stateDir, "bot-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .bot/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@hanzo/bot-voice-call"));
  ...
  it("keeps lazy imports from a loaded old npm generation available across updates", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const packageName = "@hanzo/bot-codex";
    mockNpmViewAndInstall({
      spec: `${packageName}@1.0.0`,
      packageName,
      version: "1.0.0",
  ...
  it("rejects npm installs when the installed artifact drifts from verified metadata", async () => {
    ...
    expect(fs.existsSync(path.join(npmProjectRoot, "_bot-quarantined-npm-projects"))).toBe(
      false,
    );
  ...
  it("quarantines incomplete integrity metadata and rebuilds the managed project once", async () => {
    ...
    expect(
      fs.readdirSync(path.join(npmProjectRoot, "_bot-quarantined-npm-projects")),
    ).toHaveLength(1);
  });
  ...
  it("does not restore a quarantined tree when post-recovery validation fails", async () => {
    ...
    const addPeerAfterQuarantine = () => {
      ...
      manifest.bot ??= {};
      manifest.bot.managedPeerDependencies = [addedPeerName];
    ...
    const quarantineParent = path.join(npmProjectRoot, "_bot-quarantined-npm-projects");
    ...
    expect(manifest.bot?.managedPeerDependencies ?? []).not.toContain(addedPeerName);
  });
  ...
  it("repairs omitted current-platform packages with a fresh npm cache", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@hanzo/bot-codex-fixture";
    ...
    expect(result.ok).toBe(true);
    expect(managedInstallAttempts).toBe(2);
    expect(repairCacheDir).toContain("bot-npm-cache-");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@hanzo/bot-voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    const stalePackageDir = path.join(npmProjectRoot, "node_modules", "stale-plugin");
    fs.mkdirSync(stalePackageDir, { recursive: true });
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed bot peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "bot")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/bot");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "bot")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their bot peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveBotPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@hanzo/bot-codex@2026.5.7",
      packageName: "@hanzo/bot-codex",
      version: "2026.5.7",
      pluginId: "@hanzo/bot-codex",
      npmRoot,
      peerDependencies: { bot: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@hanzo/bot-codex");
    expect(result.error).toContain("plugin-local node_modules/bot link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate bot package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@hanzo/bot-codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@hanzo/bot-codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("BOT_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@hanzo/bot-whatsapp@2026.5.27",
      packageName: "@hanzo/bot-whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { bot: ">=2026.5.27" },
      bot: {
        extensions: ["./dist/index.js"],
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@bot", "whatsapp"))).toBe(false);
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@hanzo/bot-whatsapp@2026.5.26",
        packageName: "@hanzo/bot-whatsapp",
        version: "2026.5.26",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@hanzo/bot-whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @hanzo/bot-whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@hanzo/bot-whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@hanzo/bot-whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@hanzo/bot-msteams@2026.5.28-beta.3",
        packageName: "@hanzo/bot-msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@hanzo/bot-msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @hanzo/bot-msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@hanzo/bot-msteams",
    ...
    expect(managedManifest.dependencies?.["@hanzo/bot-msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root bot materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "bot"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/bot"]).toBeUndefined();
      expect(
        fs.lstatSync(path.join(requiredPeerPluginDir, "node_modules", "bot")).isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed bot root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
    ...
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "bot"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/bot": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/bot/-/bot-2026.5.4.tgz",
            },
          },
          dependencies: {
            bot: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "bot", "package.json"),
      JSON.stringify({
        name: "bot",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@hanzo/bot-discord@beta",
      packageName: "@hanzo/bot-discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { bot: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("bot");
    expect(manifest.dependencies?.["@hanzo/bot-discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/bot"]).toBeUndefined();
    expect(lockfile.dependencies?.bot).toBeUndefined();
  });

  it("preserves the active host bot runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "bot");
    ...
    resolveBotPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/bot-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/bot-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.bot).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/bot-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/bot-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/bot-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/bot-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@hanzo/bot-voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "bot"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "bot")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local bot peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@hanzo/bot-codex",
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@hanzo/bot-codex": "0.0.1" },
      packages: [
        {
          packageName: "@hanzo/bot-codex",
          version: "0.0.1",
    ...
    const installedDir = writeInstalledNpmPlugin({
      ...
      packageName: "@hanzo/bot-codex",
      version: "0.0.1",
      peerDependencies: { bot: "*" },
    });
    const peerLink = path.join(installedDir, "node_modules", "bot");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@hanzo/bot-codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmProjectRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; bot?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.bot?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.bot?.managedOverrides).toEqual(["axios", "nested"]);
        }
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official Bot package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@hanzo/bot-voice-call");
  ...
  it("allows duplicate npm installs in update mode", async () => {
    ...
    mockNpmViewAndInstall({
      spec: "@hanzo/bot-voice-call@0.0.2",
      packageName: "@hanzo/bot-voice-call",
      version: "0.0.2",
      pluginId: "voice-call",
      npmRoot,
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@hanzo/bot-voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@hanzo/bot-whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@hanzo/bot-voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@hanzo/bot-voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @hanzo/bot-voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@hanzo/bot-voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@hanzo/bot-voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @hanzo/bot-voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@hanzo/bot-voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@hanzo/bot-voice-call",
