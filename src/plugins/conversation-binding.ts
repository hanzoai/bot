...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "bot-app-server:thread:",
  "bot-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("bot.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("bot.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Bot instead. If this started after an update, run "bot doctor --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
