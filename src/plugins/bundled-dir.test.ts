...
const originalTrustBundledPlugins = process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR;
...
function seedBundledPluginTree(rootDir: string, relativeDir: string, pluginId = "discord") {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({ name: `@hanzo/bot-${pluginId}` }, null, 2)}\n`,
    "utf8",
  );
  fs.writeFileSync(
...
function expectResolvedBundledDir(params: {
  ...
  if (params.bundledDirOverride === undefined) {
    ...
    process.env.BOT_BUNDLED_PLUGINS_DIR = params.bundledDirOverride;
  }
  if (params.disableBundledPlugins === undefined) {
    delete process.env.BOT_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.BOT_DISABLE_BUNDLED_PLUGINS = params.disableBundledPlugins;
  }
...
function expectResolvedBundledDirFromRoot(params: {
  ...
  expectResolvedBundledDir({
    ...
    argv1: params.argv1 ?? path.join(params.repoRoot, "bot.mjs"),
...
afterEach(() => {
  ...
  if (originalDisableBundledPlugins === undefined) {
    ...
    process.env.BOT_DISABLE_BUNDLED_PLUGINS = originalDisableBundledPlugins;
  }
  if (originalTrustBundledPlugins === undefined) {
    delete process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR = originalTrustBundledPlugins;
  }
...
describe("resolveBundledPluginsDir", () => {
  it.each([
    [
      ...
      {
        prefix: "bot-bundled-dir-runtime-",
    ...
    [
      ...
      {
        prefix: "bot-bundled-dir-git-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    [
      ...
      {
        prefix: "bot-bundled-dir-tsx-built-",
        hasExtensions: true,
        hasSrc: true,
        hasDistRuntimeExtensions: true,
        hasDistExtensions: true,
    ...
    const repoRoot = createBotRoot(layout);
  ...
  it("falls back to source extensions when dist trees exist but do not contain real plugin manifests", () => {
    const repoRoot = createBotRoot({
      prefix: "bot-bundled-dir-incomplete-built-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("uses source extensions in pnpm workspace mirrors without git metadata", () => {
    const repoRoot = createBotRoot({
      prefix: "bot-bundled-dir-source-mirror-",
      hasExtensions: true,
      hasSrc: true,
      hasPnpmWorkspace: true,
    });
  ...
  it("keeps built bundled plugins for git-looking trees without pnpm workspace metadata", () => {
    const repoRoot = createBotRoot({
      prefix: "bot-bundled-dir-git-no-pnpm-",
      hasExtensions: true,
      hasSrc: true,
      hasDistRuntimeExtensions: true,
      hasDistExtensions: true,
  ...
  it("reports missing pnpm workspace deps for source checkouts", () => {
    const repoRoot = createBotRoot({
      prefix: "bot-bundled-dir-source-deps-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
      hasPnpmWorkspace: true,
    ...
    process.argv[1] = path.join(repoRoot, "bot.mjs");
  ...
  it("returns a stable empty bundled plugin directory when bundled plugins are disabled", () => {
    const repoRoot = createBotRoot({
      prefix: "bot-bundled-dir-disabled-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("separates tilde override cache entries by BOT_HOME", () => {
    const homeA = makeRepoRoot("bot-bundled-dir-home-a-");
    const homeB = makeRepoRoot("bot-bundled-dir-home-b-");
    ...
    const bundledA = resolveBundledPluginsDir({ ...envBase, BOT_HOME: homeA });
    const bundledB = resolveBundledPluginsDir({ ...envBase, BOT_HOME: homeB });
  ...
  it("ignores an existing override under an argv1-derived fake package root", () => {
    const installedRoot = createBotRoot({
      prefix: "bot-bundled-dir-argv-override-reject-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));

    vi.spyOn(process, "cwd").mockReturnValue(installedRoot);
    process.argv[1] = path.join(installedRoot, "bot.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    delete process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    process.env.BOT_BUNDLED_PLUGINS_DIR = path.join(installedRoot, "dist", "extensions");
    delete process.env.BOT_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST relax existing override trust checks", () => {
    const overrideRoot = makeRepoRoot("bot-bundled-dir-vitest-override-reject-");
    ...
    process.env.BOT_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
    delete process.env.BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR;
    delete process.env.BOT_DISABLE_BUNDLED_PLUGINS;
  ...
  it("does not let VITEST add cwd to bundled plugin resolution candidates", () => {
    const cwdRepoRoot = createBotRoot({
      prefix: "bot-bundled-dir-vitest-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it("falls back from a missing override instead of returning an untrusted future path", () => {
    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("bot-bundled-dir-missing-cwd-"));
    process.argv[1] = "/usr/bin/env";
    process.execArgv.length = 0;
    delete process.env.VITEST;
    const missingOverride = path.join(
      makeRepoRoot("bot-bundled-dir-missing-override-"),
      "extensions",
    );
    process.env.BOT_BUNDLED_PLUGINS_DIR = missingOverride;
    delete process.env.BOT_DISABLE_BUNDLED_PLUGINS;
  ...
  it("falls back to argv root when an existing rejected override is unrelated", () => {
    const installedRoot = createBotRoot({
      prefix: "bot-bundled-dir-rejected-override-argv-",
      hasDistExtensions: true,
    });
    seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
    const overrideRoot = makeRepoRoot("bot-bundled-dir-rejected-override-");
    seedBundledPluginTree(overrideRoot, "extensions", "memory-core");

    vi.spyOn(process, "cwd").mockReturnValue(makeRepoRoot("bot-bundled-dir-rejected-cwd-"));
    process.argv[1] = path.join(installedRoot, "bot.mjs");
    process.execArgv.length = 0;
    delete process.env.VITEST;
    process.env.BOT_BUNDLED_PLUGINS_DIR = path.join(overrideRoot, "extensions");
  ...
  it("ignores an enclosing checkout reached through node_modules tooling argv1", () => {
    ...
    const outerRoot = createBotRoot({
      prefix: "bot-bundled-dir-enclosing-",
      hasExtensions: true,
      hasSrc: true,
      hasDistExtensions: true,
      hasGitCheckout: true,
  ...
  it("does not resolve bundled plugins from cwd when argv1 is not a package root", () => {
    const cwdRepoRoot = createBotRoot({
      prefix: "bot-bundled-dir-untrusted-cwd-",
      hasExtensions: true,
      hasSrc: true,
      hasGitCheckout: true,
    });
  ...
  it.each([
    {
      ...
      createScenario: () => {
        const installedRoot = createBotRoot({
          prefix: "bot-bundled-dir-installed-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        const cwdRepoRoot = createBotRoot({
          prefix: "bot-bundled-dir-cwd-",
          hasExtensions: true,
          hasSrc: true,
          hasGitCheckout: true,
        });
        return {
          installedRoot,
          cwd: cwdRepoRoot,
          argv1: path.join(installedRoot, "bot.mjs"),
        };
      },
    },
    {
      ...
      createScenario: () => {
        const installedRoot = createBotRoot({
          prefix: "bot-bundled-dir-override-",
          hasDistExtensions: true,
        });
        seedBundledPluginTree(installedRoot, path.join("dist", "extensions"));
        return {
