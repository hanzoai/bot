...
const emptyBundledPluginsDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-empty-plugins-"));
const externalPluginRoot = fs.mkdtempSync(
  path.join(os.tmpdir(), "bot-provider-policy-external-"),
);
fs.writeFileSync(
...
process.env.BOT_BUNDLED_PLUGINS_DIR = emptyBundledPluginsDir;
