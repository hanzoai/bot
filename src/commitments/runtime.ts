...
type CommitmentExtractionRuntime = {
  ...
  resolveDefaultModel?: (params: { cfg: BotConfig; agentId?: string }) => ModelRef;
...
let queue: Array<Omit<CommitmentExtractionItem, "existingPending"> & { cfg?: BotConfig }> = [];
...
async function hydrateBatch(
  batch: Array<Omit<CommitmentExtractionItem, "existingPending"> & { cfg?: BotConfig }>,
...
function takeAgentBatch(
  agentId: string,
  maxItems: number,
): Array<Omit<CommitmentExtractionItem, "existingPending"> & { cfg?: BotConfig }> {
  const batch = [];
