...
export type { BotConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: BotConfig, ...args: TArgs) => BotConfig;
  applyConfig: (cfg: BotConfig, ...args: TArgs) => BotConfig;
};
...
function hasAgentDefaultModelPrimary(cfg: BotConfig): boolean {
...
>(params: {
  ...
  applyPreset: (cfg: BotConfig, preset: TParams) => BotConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: BotConfig): {
  next: BotConfig;
  changed: boolean;
} {
