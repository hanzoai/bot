...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "bot-qa-test-api-root-");
    resolveBotPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(testApiCall?.env?.BOT_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.BOT_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "bot-qa-runner-root-");
    resolveBotPackageRootSync.mockReturnValue(sourceRoot);
    ...
    const manifestCall = loadBundledPluginManifestRegistry.mock.calls[0]?.[0] as
      ...
      | undefined;
    expect(manifestCall?.env?.BOT_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.BOT_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.BOT_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.BOT_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-example" exported "extra" from runtime-api.js but did not declare it in bot.plugin.json',
    );
  });
});
