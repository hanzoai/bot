 ...
 * model keeps refusing cross-session questions on those runtimes (bot#114797).
 */
...
export { VERSION as BOT_VERSION } from "../version.js";
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@hanzo/bot-media-core/constants"),
    ]);
...
/**
 * Derive the same compact user-facing tool detail that embedded Bot uses for progress logs.
 */
 ...
 * preserve the same Bot-owned fallback signals as the built-in Bot path
