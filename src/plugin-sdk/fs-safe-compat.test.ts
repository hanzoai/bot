...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "bot/plugin-sdk/core";
import {
  readFileWithinRoot,
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "bot-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps root-bounded file-access helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "bot-sdk-file-access-compat-" }, async (root) => {
