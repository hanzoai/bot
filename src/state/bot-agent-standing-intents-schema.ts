...
function standingIntentsSchemaSql(): string {
  const start = BOT_AGENT_SCHEMA_SQL.indexOf(STANDING_INTENTS_SCHEMA_START);
  const end = BOT_AGENT_SCHEMA_SQL.indexOf(STANDING_INTENTS_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Bot standing-intents schema markers are missing.");
  }
  return BOT_AGENT_SCHEMA_SQL.slice(start, end);
}
...
export function ensureBotAgentStandingIntentsSchema(db: DatabaseSync): void {
  const ensure = () => {
