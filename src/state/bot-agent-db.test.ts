// Bot agent database tests cover agent-scoped DB storage and migrations.
...
import {
  assertNoBotAgentDatabaseLeases,
...
import type { DB as BotAgentKyselyDatabase } from "./bot-agent-db.generated.js";
import {
...
function createTempStateDir(): string {
  return makeTempDir(agentDbTempDirs, "bot-agent-db-");
}

function migrateAndOpenLegacyAgentDatabaseForTest(
  options: Parameters<typeof openBotAgentDatabase>[0],
) {
  const pathname = resolveBotAgentSqlitePath(options);
  const { DatabaseSync } = requireNodeSqlite();
  const database = new DatabaseSync(pathname);
  try {
    ensureBotAgentDatabaseSchema(database, options);
  } finally {
    database.close();
  }
  return openBotAgentDatabase(options);
}
...
function readRegisteredAgentDatabaseLastSeenAt(params: {
  ...
  const row = openBotStateDatabase({ env: params.env })
...
function launchAgentSchemaOpener(params: {
  ...
  const agentModuleUrl = new URL("./bot-agent-db.ts", import.meta.url).href;
  const stateModuleUrl = new URL("./bot-state-db.ts", import.meta.url).href;
  const child = spawn(
    process.execPath,
    [
      "--import",
      ...
      `
        ...
        const db = new DatabaseSync(process.env.BOT_AGENT_DB_RACE_PATH);
        ...
        process.stdin.once("data", () => {
          let result;
          try {
            ensureBotAgentDatabaseSchema(observedDb, {
              agentId: process.env.BOT_AGENT_DB_RACE_AGENT,
              env: { BOT_STATE_DIR: process.env.BOT_AGENT_DB_RACE_STATE_DIR },
              path: process.env.BOT_AGENT_DB_RACE_PATH,
              register: true,
            });
            result = { agentId: process.env.BOT_AGENT_DB_RACE_AGENT, ok: true };
          } catch (error) {
            result = {
              agentId: process.env.BOT_AGENT_DB_RACE_AGENT,
              ok: false,
    ...
    ],
    {
      env: {
        ...process.env,
        BOT_AGENT_DB_RACE_AGENT: params.agentId,
        BOT_AGENT_DB_RACE_PATH: params.databasePath,
        BOT_AGENT_DB_RACE_STATE_DIR: params.stateDir,
      },
...
describe("bot agent database", () => {
  it("uses the canonical state schema for deletion journal reads and updates", () => {
    ...
    stateDatabase.exec(
      fs.readFileSync(new URL("./bot-state-schema.sql", import.meta.url), "utf8"),
    );
    stateDatabase.close();
    ...
    const databasePaths = [path.join(entry.agentDir, "bot-agent.sqlite")];
    const cleanupPaths = [
      {
        path: entry.agentDir,
  ...
  it("fences registration and lease claims beneath another agent's pending deletion", () => {
    ...
    registerBotAgentDatabase({ agentId: "deleted", path: registeredSidecarPath, env });
    ...
    unregisterBotAgentDatabase({ agentId: "deleted", path: relocatedDatabasePath, env });
    unregisterBotAgentDatabase({ agentId: "deleted", path: registeredSidecarPath, env });

    try {
      for (const fencedPath of [databasePath, relocatedDatabasePath, sidecarClaimPath]) {
        expect(() =>
          registerBotAgentDatabase({ agentId: "survivor", path: fencedPath, env }),
        ).toThrow("deletion owns");
        expect(() =>
          claimBotAgentDatabaseLease({ agentId: "survivor", path: fencedPath, env }),
        ).toThrow("deletion owns");
      }
    } finally {
      deletion.rollback();
    ...
    expect(() =>
      registerBotAgentDatabase({ agentId: "survivor", path: databasePath, env }),
    ).not.toThrow();
    ...
    releaseBotAgentDatabaseLease(leaseId, { env });
    unregisterBotAgentDatabase({ agentId: "survivor", path: databasePath, env });
  });
  ...
  it("blocks deletion on a foreign lease beneath its workspace until release", () => {
    ...
    expect(() => assertNoBotAgentDatabaseLeases("deleted", { env })).toThrow("deletion owns");
    releaseBotAgentDatabaseLease(leaseId, { env });
    expect(() => assertNoBotAgentDatabaseLeases("deleted", { env })).not.toThrow();
    deletion.rollback();
  });
  ...
  it("resolves under the per-agent state directory", () => {
    ...
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "bot-agent.sqlite"));
  });
  ...
  it("returns typed not-found without creating a missing read-only database", () => {
    ...
    const databasePath = resolveBotAgentSqlitePath(options);

    expect(withBotAgentDatabaseReadOnly(() => "unused", options)).toEqual({
      found: false,
  ...
  it("refuses a newer schema from the read-only database helper", () => {
    ...
    const { DatabaseSync } = requireNodeSqlite();
    const database = new DatabaseSync(databasePath);
    database.exec(`PRAGMA user_version = ${BOT_AGENT_SCHEMA_VERSION + 1};`);
    database.close();

    expect(() => withBotAgentDatabaseReadOnly(() => "unused", options)).toThrow(
      `newer schema version ${BOT_AGENT_SCHEMA_VERSION + 1}`,
    );
  });
  ...
  it("returns typed not-found when a read-only query targets a missing table", () => {
    ...
    const databasePath = openBotAgentDatabase(options).path;
    closeBotAgentDatabasesForTest();
    const { DatabaseSync } = requireNodeSqlite();
    const database = new DatabaseSync(databasePath);
  ...
  it("lists a missing registry without creating the shared state database", () => {
    const stateDir = createTempStateDir();
    const env = { BOT_STATE_DIR: stateDir };
    const stateDatabasePath = path.join(stateDir, "state", "bot.sqlite");

    expect(listBotRegisteredAgentDatabases({ env })).toEqual([]);
  ...
  it("invalidates the registered database memo after a registry write", () => {
    ...
    expect(listBotRegisteredAgentDatabases({ env })).toEqual([]);
    registerBotAgentDatabase({ agentId: "worker-1", path: databasePath, env });
    expect(listBotRegisteredAgentDatabases({ env })).toEqual([
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(() => listBotRegisteredAgentDatabases({ env })).toThrow("is unavailable");
    },
  );
  ...
  it("creates the per-agent schema and registers it globally", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./bot-agent-schema.sql", import.meta.url)),
    );
    expect(
      database.db
        .prepare(
    ...
    expect(database.path).toBe(
      path.join(stateDir, "agents", "worker-1", "agent", "bot-agent.sqlite"),
    );

    const registered = listBotRegisteredAgentDatabases({
      env: { BOT_STATE_DIR: stateDir },
    ...
    expect(registered).toMatchObject({
      agentId: "worker-1",
      path: database.path,
      schemaVersion: BOT_AGENT_SCHEMA_VERSION,
    });
    expect(registered?.sizeBytes).toBeGreaterThan(0);
  });

  it("opens a v13 database that already contains additive board storage", () => {
    expect(BOT_AGENT_SCHEMA_VERSION).toBe(16);
    const stateDir = createTempStateDir();
    const env = { BOT_STATE_DIR: stateDir };
    const opened = openBotAgentDatabase({ agentId: "worker-1", env });
    ...
    expect(readSqliteNumberPragma(reopened.db, "user_version")).toBe(BOT_AGENT_SCHEMA_VERSION);
    expect(
      reopened.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
    ).toEqual({ schema_version: BOT_AGENT_SCHEMA_VERSION });
  });

  it("migrates v13 session entries, routes, and generations into nodes and windows", () => {
    const stateDir = createTempStateDir();
    const env = { BOT_STATE_DIR: stateDir };
    const databasePath = openBotAgentDatabase({ agentId: "worker-1", env }).path;
    closeBotAgentDatabasesForTest();
    closeBotStateDatabaseForTest();
    downgradeCurrentAgentDatabaseToV13(databasePath);
    ...
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(BOT_AGENT_SCHEMA_VERSION);
  ...
  it("migrates version 1 memory source identities before registering version 2", () => {
    ...
    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(BOT_AGENT_SCHEMA_VERSION);
    expect(
    ...
    expect(
      listBotRegisteredAgentDatabases({ env }).find((entry) => entry.agentId === "worker-1"),
    ).toMatchObject({ path: databasePath, schemaVersion: BOT_AGENT_SCHEMA_VERSION });
  ...
  it("retains a failed-open handle and lease when initialization cleanup also fails", () => {
    const stateDir = createTempStateDir();
    const env = { BOT_STATE_DIR: stateDir };
    const database = openBotAgentDatabase({ agentId: "worker-1", env });
    expect(closeBotAgentDatabaseByPath(database.path)).toBe(true);
    const { DatabaseSync } = requireNodeSqlite();
    ...
    expect(() =>
      openBotAgentDatabase({ agentId: "worker-2", env, path: database.path }),
    ).toThrow("initialization close failed");
    close.mockRestore();
    expect(() => assertNoBotAgentDatabaseLeases("worker-2", { env })).toThrow(
    ...
    expect(disposeBotAgentDatabaseByPath(database.path, { env })).toBe(true);
    expect(() => assertNoBotAgentDatabaseLeases("worker-2", { env })).not.toThrow();
  });
  ...
  it("keeps multiple registered paths for the same agent", () => {
    ...
    const relocated = openBotAgentDatabase({
      agentId: "worker-1",
      env,
      path: relocatedPath,
    });
    const defaultDatabase = openBotAgentDatabase({
      agentId: "worker-1",
      env,
    });

    expect(
      listBotRegisteredAgentDatabases({ env })
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      expect(isSameBotAgentDatabasePath(realPath, aliasPath)).toBe(true);
      ...
      expect(isSameBotAgentDatabasePath(futureRealPath, danglingAliasPath)).toBe(true);
      ...
      expect(isSameBotAgentDatabasePath(relativeRealPath, relativeAliasPath)).toBe(true);
      ...
      expect(isSameBotAgentDatabasePath(redirectedRealPath, redirectedAliasPath)).toBe(true);
      expect(
        isSameBotAgentDatabasePath(
      ...
      expect(isSameBotAgentDatabasePath(danglingTraversalAlias, livePath)).toBe(false);
      const rawMissingTraversalPath = `${stateDir}${path.sep}missing${path.sep}..${path.sep}live-worker.sqlite`;
      expect(isSameBotAgentDatabasePath(rawMissingTraversalPath, livePath)).toBe(false);
      ...
      expect(() => isSameBotAgentDatabasePath(loopPath, realPath)).toThrow(
      ...
      expect(() => isSameBotAgentDatabasePath(expandingLoopPath, realPath)).toThrow(
        expect.objectContaining({ code: "ELOOP" }),
      );
      ...
      const database = openBotAgentDatabase({ agentId: "worker", env, path: realPath });
      unregisterBotAgentDatabase({ agentId: "worker", env, path: database.path });
      registerBotAgentDatabase({ agentId: "worker", env, path: aliasPath });
    },
  );
  ...
  it("matches volume semantics for non-ASCII case aliases", () => {
    ...
    expect(isSameBotAgentDatabasePath(upperPath, lowerPath)).toBe(aliases);
  });
  ...
  it("matches volume semantics for Unicode simple case-fold aliases", () => {
    ...
    expect(isSameBotAgentDatabasePath(sigmaPath, finalSigmaPath)).toBe(aliases);
  });
  ...
  it("keeps case and normalization semantics distinct for missing aliases", () => {
    ...
    expect(isSameBotAgentDatabasePath(composedUpperPath, decomposedLowerPath)).toBe(aliases);
  });
  ...
  it("requires raw normalization after an ASCII-only case difference", () => {
    ...
    expect(isSameBotAgentDatabasePath(composedUpperPath, decomposedMixedPath)).toBe(aliases);
  });
  ...
  it.runIf(tempVolumeIsCaseInsensitive)(
    ...
    () => {
      ...
      expect(
        isSameBotAgentDatabasePath(path.join(stateDir, upper), path.join(stateDir, lower)),
      ).toBe(true);
    },
  );
  ...
  it("uses the candidate code points for missing-path normalization semantics", () => {
    ...
    expect(isSameBotAgentDatabasePath(leftPath, rightPath)).toBe(aliases);
  });
  ...
  it("matches volume semantics for normalization-only missing directory components", () => {
    ...
    expect(
      isSameBotAgentDatabasePath(
        path.join(leftDir, "bot-agent.sqlite"),
        path.join(rightDir, "bot-agent.sqlite"),
      ),
    ).toBe(aliases);
  });
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(isSameBotAgentDatabasePath(driveRelativePath, absolutePath)).toBe(true);
    },
  );
  ...
  it.runIf(process.platform === "linux")(
    ...
    () => {
      ...
      expect(
        isSameBotAgentDatabasePath(path.join(hardLinkedNested, "d", "link"), firstHardLink),
      ).toBe(true);
    },
  );
  ...
  it("does not refresh global registry metadata on cached opens", () => {
    ...
    try {
      const first = openBotAgentDatabase({
        agentId: "worker-1",
        env,
      });
      expect(
      ...
      const second = openBotAgentDatabase({
        agentId: "worker-1",
        env,
      });
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    ...
    legacyDb.exec(`
      ...
      INSERT INTO agent_databases (
        ...
        'worker-1',
        '/legacy/worker-1/bot-agent.sqlite',
        1,
        10,
        20
      );
    ...
    expect(() => listBotRegisteredAgentDatabases({ env })).toThrow(
      /run bot doctor --fix/,
    );
    ...
    ).toThrow(/run bot doctor --fix/);

    fs.rmSync(stateDatabasePath);
    const reopened = openBotAgentDatabase({
      agentId: "worker-1",
      env,
    });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-agent-db-state-"));
          const env = { BOT_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "bot-agent-db-relative-"));
          ...
          try {
            ...
            console.log(JSON.stringify({
              ...
              registeredPaths: listBotRegisteredAgentDatabases({ env })
                .filter((entry) => entry.agentId === "worker-1")
                .map((entry) => entry.path),
  ...
  it("closes only the cached database at the exact resolved path", () => {
    ...
    const first = openBotAgentDatabase({ agentId: "worker-1", env, path: firstPath });
    const second = openBotAgentDatabase({ agentId: "worker-2", env, path: secondPath });

    expect(closeBotAgentDatabaseByPath(path.join(stateDir, "missing.sqlite"))).toBe(false);
    ...
    expect(second.db.isOpen).toBe(true);
    expect(closeBotAgentDatabaseByPath(firstPath)).toBe(false);

    const reopened = openBotAgentDatabase({ agentId: "worker-1", env, path: firstPath });
  ...
  it("retains its durable lease when closing the handle fails", () => {
    ...
    expect(() => closeBotAgentDatabaseByPath(database.path)).toThrow("wal close failed");
    expect(database.db.isOpen).toBe(true);
    expect(() => assertNoBotAgentDatabaseLeases("worker-1", { env })).toThrow(
      "database is still open",
    );

    close.mockRestore();
    expect(closeBotAgentDatabaseByPath(database.path)).toBe(true);
    expect(() => assertNoBotAgentDatabaseLeases("worker-1", { env })).not.toThrow();
  });

  it("disposes only its exact cached owner and unregisters that registry row", () => {
    const stateDir = createTempStateDir();
    const env = { BOT_STATE_DIR: stateDir };
    const first = openBotAgentDatabase({ agentId: "worker-1", env });
    const second = openBotAgentDatabase({ agentId: "worker-2", env });

    expect(disposeBotAgentDatabaseByPath(first.path, { env })).toBe(true);
    expect(first.db.isOpen).toBe(false);
    expect(second.db.isOpen).toBe(true);
    expect(listBotRegisteredAgentDatabases({ env })).toEqual([
      expect.objectContaining({ agentId: "worker-2", path: second.path }),
    ]);
    expect(disposeBotAgentDatabaseByPath(first.path, { env })).toBe(false);
    expect(second.db.isOpen).toBe(true);
  ...
  it("reopens a recreated agent id on a fresh database after archival", () => {
    const stateDir = createTempStateDir();
    const env = { BOT_STATE_DIR: stateDir };
    const original = openBotAgentDatabase({ agentId: "worker-1", env });
    ...
    try {
      expect(disposeBotAgentDatabaseByPath(original.path, { env })).toBe(true);
      deletion.commit();
      ...
      const recreated = openBotAgentDatabase({ agentId: "worker-1", env });
      expect(recreated.db.isOpen).toBe(true);
      expect(fs.statSync(recreated.path).ino).not.toBe(originalInode);
      expect(fs.existsSync(path.join(archivedDir, "bot-agent.sqlite"))).toBe(true);
      expect(listBotRegisteredAgentDatabases({ env })).toEqual([
  ...
  it("keeps the deleted id fenced until its completed tombstone is claimed", () => {
    ...
    const databasePath = path.join(agentDir, "bot-agent.sqlite");
    const deletion = beginAgentDeletion(
      {
        agentId: "worker-1",
        agentDir,
    ...
    expect(() =>
      registerBotAgentDatabase({ agentId: "worker-2", path: databasePath, env }),
    ).not.toThrow();
    unregisterBotAgentDatabase({ agentId: "worker-2", path: databasePath, env });

    expect(claimCompletedAgentDeletion("worker-1", deletion.entry.operationId, { env })).toBe(true);
    expect(() =>
      registerBotAgentDatabase({ agentId: "worker-1", path: databasePath, env }),
    ).not.toThrow();
    unregisterBotAgentDatabase({ agentId: "worker-1", path: databasePath, env });
  });

  it("serializes database leases with the durable deletion fence", () => {
    const stateDir = createTempStateDir();
    const env = { BOT_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "agents", "worker-1", "bot-agent.sqlite");
    const leaseId = claimBotAgentDatabaseLease({
      agentId: "worker-1",
      path: databasePath,
      env,
    ...
    try {
      ...
      expect(() =>
        claimBotAgentDatabaseLease({ agentId: "worker-1", path: databasePath, env }),
      ).toThrow("agent worker-1 is deleted");
      releaseBotAgentDatabaseLease(leaseId, { env });
      expect(() => assertNoBotAgentDatabaseLeases("worker-1", { env })).not.toThrow();
    } finally {
  ...
  it("serializes concurrent ownership claims for one unowned database", async () => {
    ...
    try {
      ...
      expect(inspectBotAgentDatabaseOwner(databasePath)).toEqual({
        status: "owned",
        agentId: winner?.agentId,
      });
      expect(
        listBotRegisteredAgentDatabases({ env: { BOT_STATE_DIR: stateDir } }),
      ).toEqual([
        expect.objectContaining({
          agentId: winner?.agentId,
          path: databasePath,
  ...
  it("rechecks schema version after waiting for a concurrent writer", async () => {
    ...
    const futureVersion = BOT_AGENT_SCHEMA_VERSION + 1;
    try {
      await opener.ready;
      opener.child.stdin.end("go\n");
  ...
  it("rejects explicit paths that point at the global state database", () => {
    ...
    const reopenedStateDatabase = openBotStateDatabase({
      env,
      path: databasePath,
    });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { BOT_STATE_DIR: parentDir };
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      runBotAgentWriteTransaction(() => {
        runBotAgentWriteTransaction(() => undefined, options);
  ...
  it("repairs a same-name transcript uniqueness index before accepting writes", () => {
    const stateDir = createTempStateDir();
    const env = { BOT_STATE_DIR: stateDir };
    const created = openBotAgentDatabase({ agentId: "worker-1", env });
  ...
  it("rejects same-name transcript index drift when duplicate rows block repair", () => {
    ...
    try {
      ...
      expect(
        after
          .prepare(
            "SELECT name FROM sqlite_schema WHERE type = 'index' AND name LIKE 'bot_probe_%'",
          )
          .all(),
      ).toEqual([]);
    } finally {
  ...
  it("adds transcript watermarks and session provenance to v4 session tables", () => {
    ...
    openBotAgentDatabase({ agentId: "worker-1", env: { BOT_STATE_DIR: stateDir } });
    closeBotAgentDatabasesForTest();
    downgradeCurrentAgentDatabaseToV13(databasePath);
    const { DatabaseSync } = requireNodeSqlite();
    const db = new DatabaseSync(databasePath);
  ...
  it("adds durable conversation delivery state when upgrading the canonical v10 schema", () => {
    ...
    expect(() =>
      assertBotAgentDatabaseForMaintenance(database.db, {
        agentId: "worker-1",
  ...
  it("rejects stale schema_meta indexes before writable initialization", () => {
    ...
    try {
      expect(() =>
        ensureBotAgentDatabaseSchema(independentlyManaged, {
          agentId: "worker-1",
          env,
        }),
  ...
  it("rechecks integrity after a validated handle is physically reopened", () => {
    ...
    const env = { BOT_STATE_DIR: stateDir };
    const databasePath = openBotAgentDatabase({ agentId: "worker-1", env }).path;
    expect(closeBotAgentDatabaseByPath(databasePath)).toBe(true);
    closeBotStateDatabaseForTest();
    createUnsafeIndexDrift(databasePath);
  ...
  it.each([0, BOT_AGENT_SCHEMA_VERSION - 1])(
  ...
  it("does not hide a sqlitefoo-only metadata-less v0 database as internal", () => {
    ...
    expect(() =>
      withBotAgentDatabaseReadOnly(() => "unused", {
        agentId: "worker-1",
        env,
        path: databasePath,
      }),
    ).toThrow("uses schema version 0; run bot doctor --fix");
    expect(() =>
      openBotAgentDatabase({ agentId: "worker-1", env, path: databasePath }),
    ).toThrow("uses schema version 0; run bot doctor --fix");
  });
  ...
  it("runs full integrity before a pending agent schema migration", () => {
    ...
    try {
      before.exec(`PRAGMA user_version = ${BOT_AGENT_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
  ...
  it("latches newer per-agent schema failures before integrity scans", () => {
    ...
    const { DatabaseSync } = requireNodeSqlite();
    const db = new DatabaseSync(databasePath);
    db.exec(`PRAGMA user_version = ${BOT_AGENT_SCHEMA_VERSION + 1};`);
    db.close();
    ...
    expect(firstFailure).toMatchObject({
      ...
      message: expect.stringContaining("https://docs.bot.ai/reference/database-schemas"),
    });
    ...
    clearBotAgentDatabaseOpenFailure(databasePath);
  ...
  it("closes cached handles on normal process exit so no stale WAL remains", () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          import fs from "node:fs";
          import { openBotAgentDatabase } from ${JSON.stringify(agentModuleUrl)};

          const database = openBotAgentDatabase({
            agentId: "worker-1",
            env: { BOT_STATE_DIR: process.env.BOT_AGENT_DB_EXIT_TEST_DIR },
          });
      ...
      {
        encoding: "utf8",
        env: { ...process.env, BOT_AGENT_DB_EXIT_TEST_DIR: stateDir },
      },
    );
    const result = JSON.parse(output) as {
