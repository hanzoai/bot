...
import { ensureBotAgentStandingIntentsSchema } from "./bot-agent-standing-intents-schema.js";
...
function schemaWithoutStandingIntents(): string {
  return removeSchemaSection(BOT_AGENT_SCHEMA_SQL, STANDING_SCHEMA_START, STANDING_SCHEMA_END);
}

function schemaWithoutMemoryProvenance(): string {
  const withoutTable = removeSchemaSection(
    BOT_AGENT_SCHEMA_SQL,
...
function schemaWithoutStandingIntentCreator(): string {
  if (!BOT_AGENT_SCHEMA_SQL.includes(STANDING_CREATOR_COLUMN)) {
    throw new Error("standing-intent creator column missing in test fixture");
  }
  return BOT_AGENT_SCHEMA_SQL.replace(STANDING_CREATOR_COLUMN, "");
}

function schemaWithoutMemoryChunkMetadata(): string {
  if (!BOT_AGENT_SCHEMA_SQL.includes(MEMORY_CHUNK_METADATA_COLUMNS)) {
    throw new Error("memory chunk metadata columns missing in test fixture");
  }
  return BOT_AGENT_SCHEMA_SQL.replace(MEMORY_CHUNK_METADATA_COLUMNS, "");
}
...
describe("additive memory agent schemas", () => {
  ...
  it("accepts a current-version database before the lazy ensure and ensures idempotently", async () => {
    tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "bot-intent-schema-")),
    );
    ...
    try {
      ...
      db.exec(`PRAGMA user_version = ${BOT_AGENT_SCHEMA_VERSION};`);
      db.prepare(
        `INSERT INTO schema_meta (
          meta_key, role, schema_version, agent_id, app_version, created_at, updated_at
        ) VALUES ('primary', 'agent', ?, 'main', 'test', 1, 1)`,
      ).run(BOT_AGENT_SCHEMA_VERSION);
      ...
      ensureBotAgentStandingIntentsSchema(db);
      ensureBotAgentStandingIntentsSchema(db);
  ...
  it("accepts current-version databases before the provenance lazy ensure", async () => {
    tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "bot-provenance-schema-")),
    );
    const databasePath = path.join(tempDir, "bot-agent.sqlite");
    const db = openNodeSqliteDatabase(databasePath);
    try {
  ...
  it("lazily adds creator provenance to the unreleased standing-intent table", async () => {
    tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "bot-intent-creator-schema-")),
    );
    const databasePath = path.join(tempDir, "bot-agent.sqlite");
    const db = openNodeSqliteDatabase(databasePath);
    try {
      ...
      expect(() =>
        ensureBotAgentDatabaseSchema(db, { agentId: "main", path: databasePath }),
      ).not.toThrow();
      expect(
        db
  ...
  it("accepts pre-provenance databases missing the memory chunk metadata columns", async () => {
    tempDir = await fs.realpath(
      await fs.mkdtemp(path.join(os.tmpdir(), "bot-chunk-metadata-schema-")),
    );
    const databasePath = path.join(tempDir, "bot-agent.sqlite");
    const db = openNodeSqliteDatabase(databasePath);
    try {
