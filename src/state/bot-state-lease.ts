...
type AgentLeaseDatabase = Pick<BotAgentKyselyDatabase, "state_leases">;
type LeaseKysely = ReturnType<typeof getNodeSqliteKysely<LeaseDatabase>>;

type BotStateLeaseDatabase =
  | { scope: "shared"; options?: BotStateDatabaseOptions }
  | { scope: "agent"; agentId: string };

type BotStateLeaseOptions = {
  scope: string;
  key: string;
  database: BotStateLeaseDatabase;
  leaseMs: number;
  waitMs: number;
  signal?: AbortSignal;
...
export type BotStateLeaseContext = {
  signal: AbortSignal;
...
export type BotStateLeaseErrorCode =
  | "BOT_STATE_LEASE_INVALID_INPUT"
  | "BOT_STATE_LEASE_TIMEOUT"
  | "BOT_STATE_LEASE_ABORTED"
  | "BOT_STATE_LEASE_LOST"
  | "BOT_STATE_LEASE_STORAGE_FAILED";

export class BotStateLeaseError extends Error {
  readonly code: BotStateLeaseErrorCode;

  constructor(message: string, options: { code: BotStateLeaseErrorCode; cause?: unknown }) {
    super(message, { cause: options.cause });
    this.name = "BotStateLeaseError";
...
function leaseError(
  code: BotStateLeaseErrorCode,
  message: string,
  cause?: unknown,
): BotStateLeaseError {
  return new BotStateLeaseError(message, {
    code,
    ...(cause === undefined ? {} : { cause }),
  });
}

function invalidInput(message: string): BotStateLeaseError {
  return leaseError("BOT_STATE_LEASE_INVALID_INPUT", message);
}
...
function validateOptions(options: BotStateLeaseOptions) {
...
function withLeaseWriteTransaction<T>(
  ...
  if (database.scope === "shared") {
    const stateDatabase = openBotStateDatabase(database.options);
    const run = () =>
      runBotStateWriteTransaction(
  ...
  const agentDatabase = openBotAgentDatabase({ agentId: database.agentId });
  const run = () =>
    runBotAgentWriteTransaction(
...
function withLeaseRead<T>(
  ...
  const sqlite =
    database.scope === "shared"
      ? openBotStateDatabase(database.options).db
      : openBotAgentDatabase({ agentId: database.agentId }).db;
...
function verifyLeaseOwnership(
  params: LeaseIdentity & { database?: BotStateLeaseDatabase; transaction?: DatabaseSync },
): void {
  try {
    ...
    if (error instanceof BotStateLeaseError) {
      throw error;
    }
    throw leaseError(
      "BOT_STATE_LEASE_STORAGE_FAILED",
...
export async function withBotStateLease<T>(
  options: BotStateLeaseOptions,
  ...
  const abortLost = (cause?: unknown) => {
    if (!leaseLost.signal.aborted) {
      leaseLost.abort(
        cause instanceof BotStateLeaseError
          ? cause
          : leaseError(
              "BOT_STATE_LEASE_LOST",
  ...
  const heartbeat = setInterval(() => {
    try {
      ...
      if (error instanceof BotStateLeaseError && error.code === "BOT_STATE_LEASE_LOST") {
        abortLost(error);
