...
export function assertCanonicalOperatorApprovalKinds(db: DatabaseSync, pathname: string): void {
  if (!hasCanonicalOperatorApprovalKinds(db)) {
    throw new Error(
      `Bot state database ${pathname} has a legacy operator approval schema; run bot doctor --fix to migrate it.`,
    );
  }
}
...
function canonicalOperatorApprovalCreateSql(): string {
  ...
  const start = BOT_STATE_SCHEMA_SQL.indexOf(marker);
  const end = BOT_STATE_SCHEMA_SQL.indexOf(
  ...
  return BOT_STATE_SCHEMA_SQL.slice(start, end + tableTerminator.length);
}
...
function operatorApprovalIndexSql(): string {
  const statements = BOT_STATE_SCHEMA_SQL.split(";")
