// Bot state database tests cover state DB migrations and persistence.
...
import {
  ...
  repairBotStateDatabaseSchemaIfNeeded,
...
function createTempStateDir(): string {
  return makeTempDir(stateDbTempDirs, "bot-state-db-");
}
...
function seedLegacySessionWatchCursorSchema(stateDir: string): {
  ...
  const options = { env: { BOT_STATE_DIR: stateDir } };
  const databasePath = openBotStateDatabase(options).path;
  closeBotStateDatabaseForTest();
...
function createCanonicalAuditStateDatabase(stateDir: string): string {
  const database = openBotStateDatabase({ env: { BOT_STATE_DIR: stateDir } });
...
function runHotRollbackJournalRecoveryProbe(params: { moduleUrl: string; rootDir: string }): {
  ...
  const probeSource = `
    ...
    const initial = openBotStateDatabase({ path: databasePath });
    initial.db.exec(\`
    ...
    const writerSource = \`
      ...
      const database = new DatabaseSync(process.env.BOT_HOT_JOURNAL_DATABASE_PATH);
      database.exec(
      ...
      fs.writeFileSync(process.env.BOT_HOT_JOURNAL_READY_PATH, "ready");
      setInterval(() => {}, 1_000);
    \`;
    const writer = spawn(
      process.execPath,
      ["--input-type=module", "-e", writerSource],
      {
        env: {
          ...process.env,
          BOT_HOT_JOURNAL_DATABASE_PATH: databasePath,
          BOT_HOT_JOURNAL_READY_PATH: readyPath,
        },
    ...
    try {
      ...
      let readOnly;
      try {
        const database = await openExistingBotStateDatabaseReadOnly({ path: databasePath });
      ...
      const reopened = openBotStateDatabase({ path: databasePath });
...
function expectNoncanonicalAuditSchemaRejected(
  ...
  const options = { env: { BOT_STATE_DIR: stateDir } };
  expect(detectBotStateDatabaseSchemaMigrations(options)).toEqual([
    { kind: "audit-events-v2", path: databasePath },
  ]);
  expect(() => openBotStateDatabase(options)).toThrow(/noncanonical audit event schema/);
  expect(repairBotStateDatabaseSchema(options)).toEqual({
    changes: [],
...
function runConcurrentSchemaProbe(params: {
  ...
  const workerSource = `
    ...
    const pageBarrierDir = process.env.BOT_SCHEMA_TEST_PAGE_BARRIER_DIR;
    if (pageBarrierDir) {
      const pageReadyPath = process.env.BOT_SCHEMA_TEST_PAGE_READY_PATH;
      const workerCount = Number(process.env.BOT_SCHEMA_TEST_WORKER_COUNT);
    ...
    const {
      closeBotStateDatabaseForTest,
      openBotStateDatabase,
    } = await import(process.env.BOT_SCHEMA_TEST_MODULE_URL);
    const databasePath = process.env.BOT_SCHEMA_TEST_DATABASE_PATH;
    const readyPath = process.env.BOT_SCHEMA_TEST_READY_PATH;
    const startPath = process.env.BOT_SCHEMA_TEST_START_PATH;
    ...
    try {
      const database = openBotStateDatabase({ path: databasePath });
  ...
  const orchestratorSource = `
    ...
    for (let round = 0; round < roundCount; round += 1) {
      ...
      if (mode === "upgrade") {
        ...
        openBotStateDatabase({ path: databasePath });
        closeBotStateDatabaseForTest();
        ...
        legacy.exec(\`
          ...
          ALTER TABLE worker_environments DROP COLUMN bootstrap_bot_version;
      ...
      const workers = Array.from({ length: workerCount }, (_, index) => {
        ...
        return spawn(
          ...
          ["--import", "tsx", "--input-type=module", "-e", workerSource],
          {
            env: {
              ...process.env,
              BOT_SCHEMA_TEST_DATABASE_PATH: databasePath,
              BOT_SCHEMA_TEST_MODULE_URL: moduleUrl,
              BOT_SCHEMA_TEST_READY_PATH: readyPath,
              BOT_SCHEMA_TEST_START_PATH: startPath,
              ...(mode === "fresh"
                ? {
                    BOT_SCHEMA_TEST_PAGE_BARRIER_DIR: barrierDir,
                    BOT_SCHEMA_TEST_PAGE_READY_PATH: pageReadyPath,
                    BOT_SCHEMA_TEST_WORKER_COUNT: String(workerCount),
                  }
                : {}),
            },
...
describe("bot state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveBotStateSqlitePath({ BOT_STATE_DIR: stateDir })).toBe(
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "bot-test-state", `${process.pid}-7`, "state", "bot.sqlite"),
    );
  });
  ...
  it("creates the shared state schema from the committed SQL shape", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./bot-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "bot.sqlite"));
    expect(
      database.db
        .prepare(
  ...
  it("skips exclusive repair when the automatic schema gate is already current", () => {
    ...
    expect(repairBotStateDatabaseSchemaIfNeeded(options)).toEqual({
      changes: [],
      warnings: [],
    });
...
`);
    ...
    expect(readSqliteNumberPragma(database.db, "user_version")).toBe(BOT_STATE_SCHEMA_VERSION);
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./bot-state-schema.sql", import.meta.url)),
    );
  });
...
`);
  ...
  it("doctor migrates existing APNs tombstone tables to STRICT without losing rows", () => {
    ...
    const migrated = openBotStateDatabase(options);
    expect(
      migrated.db
        .prepare("SELECT strict FROM pragma_table_list WHERE name = 'apns_registration_tombstones'")
        .get(),
  ...
  it("doctor migrates version 2 tables to STRICT without losing rows", () => {
    const stateDir = createTempStateDir();
    const options = { env: { BOT_STATE_DIR: stateDir } };
    const opened = openBotStateDatabase(options);
  ...
  it("doctor migrates version 3 ambient watch sentinels into cursor provenance", () => {
    ...
    expect(readSqliteNumberPragma(migrated.db, "user_version")).toBe(BOT_STATE_SCHEMA_VERSION);
    expect(
      migrated.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
    ).toEqual({ schema_version: BOT_STATE_SCHEMA_VERSION });
    closeBotStateDatabaseForTest();
    expect(repairBotStateDatabaseSchema(options)).toEqual({ changes: [], warnings: [] });
  });
  ...
  it("detects schema migrations committed in an uncheckpointed WAL", () => {
    ...
    try {
      ...
      expect(detectBotStateDatabaseSchemaMigrations(options)).toContainEqual({
  ...
  it("rejects a placement turn claim tuple without an owner", () => {
    const database = openBotStateDatabase({
      env: { BOT_STATE_DIR: createTempStateDir() },
    });
  ...
  it("repairs every canonical shared-state named index", () => {
    ...
    const created = openBotStateDatabase({ env });
  ...
  it("rejects a missing current-schema table instead of recreating it empty", () => {
    ...
    expect(() => openBotStateDatabase(options)).toThrow(/missing table auth_profile_stores/iu);
    expect(repairBotStateDatabaseSchema(options)).toEqual({
      changes: [],
  ...
  it("rejects an inline unique constraint hidden behind a SQLite autoindex", () => {
    ...
    expect(() => openBotStateDatabase(options)).toThrow(
  ...
  it("migrates the released audit ledger to message-compatible attribution exactly once", () => {
    ...
    expect(() => openBotStateDatabase(options)).toThrow(/legacy audit event schema/);
    ...
    try {
      ...
      expect(readSqliteNumberPragma(db, "user_version")).toBe(BOT_STATE_SCHEMA_VERSION);
      expect(
  ...
  it("preserves an empty audit ledger's sequence high-water mark", () => {
    ...
    expect(repairBotStateDatabaseSchema(options).warnings).toEqual([]);
  ...
  it("lets normal open create an audit ledger for a pre-v2 database", () => {
    ...
    expect(readSqliteNumberPragma(opened.db, "user_version")).toBe(BOT_STATE_SCHEMA_VERSION);
  });
  ...
  it("refuses to rebuild a noncanonical audit table with unknown data columns", () => {
    ...
    const result = repairBotStateDatabaseSchema(options);
  ...
  it.runIf(process.platform === "linux")("closes the database when initialization fails", () => {
    const databasePath = path.join(createTempStateDir(), "bot.sqlite");
    fs.writeFileSync(databasePath, "not a sqlite database");

    expect(() => openBotStateDatabase({ path: databasePath })).toThrow(
  ...
  it("reads committed live WAL rows without changing source database content", async () => {
    const stateDir = createTempStateDir();
    const writer = openBotStateDatabase({ env: { BOT_STATE_DIR: stateDir } });
    ...
    const database = await openExistingBotStateDatabaseReadOnly({ path: writer.path });
    expect(
  ...
  it("reads committed WAL rows without recreating a missing source SHM", async () => {
    ...
    const writer = openBotStateDatabase({ env: { BOT_STATE_DIR: sourceDir } });
    writer.db.exec("PRAGMA wal_checkpoint(TRUNCATE); PRAGMA wal_autocheckpoint = 0;");
    ...
    const database = await openExistingBotStateDatabaseReadOnly({ path: fixturePath });
    expect(
      database?.db
        .prepare("SELECT task_id FROM task_runs WHERE task_id = ?")
        .get("task-wal-without-shm"),
  ...
  it("rejects noncanonical indexes from read-only state without rewriting them", async () => {
    ...
    await expect(openExistingBotStateDatabaseReadOnly(options)).rejects.toThrow(
  ...
  it("rejects unrelated current-schema index corruption before exposure", () => {
    ...
    expect(() =>
      withBotStateStartupMigrationCheckpointDatabase(checkpointCallback, options),
  ...
  it("runs full integrity before a pending state schema migration", () => {
    ...
    const before = new DatabaseSync(databasePath);
    try {
      before.exec(`PRAGMA user_version = ${BOT_STATE_SCHEMA_VERSION - 1};`);
    } finally {
      before.close();
    }
  ...
  it("rejects current-schema foreign-key violations before exposure", () => {
    ...
    expect(() => openBotStateDatabase(options)).toThrow(failure);
    expect(repairBotStateDatabaseSchema(options)).toEqual({
      changes: [],
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const result = runHotRollbackJournalRecoveryProbe({
        moduleUrl: new URL("./bot-state-db.ts", import.meta.url).href,
  ...
  it("adds staged worker-result refs during the v5 state migration", () => {
    ...
    expect(readSqliteNumberPragma(reopened.db, "user_version")).toBe(BOT_STATE_SCHEMA_VERSION);
    expect(
      reopened.db
        .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary'")
        .get(),
  ...
  it("backfills durable approval transport references in databases created by PR 1", () => {
    ...
    const reopened = openBotStateDatabase({ env: { BOT_STATE_DIR: stateDir } });
    expect(
      reopened.db
  ...
  it("migrates operator approvals to accept system-agent records", () => {
    ...
    expect(repairBotStateDatabaseSchema(options)).toEqual({
      changes: [
        "Migrated shared state operator approvals → Bot system changes",
  ...
  it("does not recursively recommend doctor when operator approval repair refuses a shape", () => {
    ...
    expect(result.warnings[0]).not.toContain("run bot doctor --fix");
  });
  ...
  it.each([
    ...
    ({ migrationPath, withRow }) => {
      ...
      assertBotStateDatabaseForMaintenance(reopened.db, { pathname: reopened.path });
  ...
  it("serializes concurrent additive schema upgrades across processes", () => {
    ...
    const moduleUrl = new URL("./bot-state-db.ts", import.meta.url).href;
    ...
    const expectedShape = createSqliteSchemaShapeFromSql(
      new URL("./bot-state-schema.sql", import.meta.url),
    );
    const { DatabaseSync } = requireNodeSqlite();
  ...
  it("migrates requester and executor attribution for existing cross-agent tasks", () => {
    ...
    const currentReopened = openBotStateDatabase({
      env: { BOT_STATE_DIR: stateDir },
    });
    expect(
      currentReopened.db
  ...
  it("normalizes obsolete task delivery statuses in existing state databases", async () => {
    await withBotTestState(
      { layout: "state-only", prefix: "bot-state-task-delivery-status-" },
      async ({ stateDir }) => {
        const database = openBotStateDatabase({
          env: { BOT_STATE_DIR: stateDir },
        });
        ...
        const readStatuses = () =>
          openBotStateDatabase({ env: { BOT_STATE_DIR: stateDir } })
  ...
  it("configures the busy timeout before a doctor schema repair transaction", () => {
    ...
    const timeoutIndex = statements.findIndex((sql) =>
      sql.includes(`PRAGMA busy_timeout = ${BOT_SQLITE_BUSY_TIMEOUT_MS}`),
    );
  ...
  it("latches newer global schema failures before integrity scans", () => {
    ...
    clearBotStateDatabaseOpenFailure(databasePath);
    expect(openBotStateDatabase(options).db.isOpen).toBe(true);
  });

  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `bot-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openBotStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openBotStateDatabase({ path: firstPath });
    const second = openBotStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openBotStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(BOT_STATE_SCHEMA_VERSION);
  });
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "bot-state-db-relative-"));
          ...
          try {
            ...
            const first = openBotStateDatabase({ path: "state.sqlite" });
            first.db
            ...
            const second = openBotStateDatabase({ path: "state.sqlite" });
            second.db
              .prepare("INSERT INTO diagnostic_events (scope, event_key, payload_json, created_at) VALUES (?, ?, ?, ?)")
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    ...
    runBotStateWriteTransaction((database) => {
      ...
      expect(() =>
        runBotStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runBotStateWriteTransaction(async () => {
        return "not sync";
      }, options),
