import { BOT_AGENT_SCHEMA_WITHOUT_BOARD_SQL } from "./bot-agent-board-schema.js";
...
function splitSessionSharingSchema(sql: string): { sharing: string; withoutSharing: string } {
  ...
  if (start === -1 || end === -1) {
    throw new Error("Bot agent session-sharing schema markers are missing.");
  }
  return {
...
const sessionSharingSchema = splitSessionSharingSchema(BOT_AGENT_SCHEMA_WITHOUT_BOARD_SQL);
const sessionSuggestionsStart = sessionSharingSchema.sharing.indexOf(SUGGESTIONS_SCHEMA_START);
if (sessionSuggestionsStart === -1) {
  throw new Error("Bot agent session-suggestions schema marker is missing.");
}
