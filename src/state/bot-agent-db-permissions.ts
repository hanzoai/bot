...
import type { BotAgentDatabaseOptions } from "./bot-agent-db-contract.js";
import { resolveBotAgentSqlitePath } from "./bot-agent-db.paths.js";

const BOT_AGENT_DB_DIR_MODE = 0o700;
const BOT_AGENT_DB_FILE_MODE = 0o600;

export function ensureBotAgentDatabasePermissions(
  pathname: string,
  options: BotAgentDatabaseOptions,
): void {
  const dir = path.dirname(pathname);
  const defaultPath = resolveBotAgentSqlitePath({
    agentId: options.agentId,
    env: options.env,
  });
  const isDefaultAgentDatabase = path.resolve(pathname) === path.resolve(defaultPath);
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: BOT_AGENT_DB_DIR_MODE });
  ...
  if (isDefaultAgentDatabase || !dirExisted) {
    chmodSync(dir, BOT_AGENT_DB_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    try {
      chmodSync(candidate, BOT_AGENT_DB_FILE_MODE);
    } catch (error) {
