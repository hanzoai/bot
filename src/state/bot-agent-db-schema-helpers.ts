...
import {
  ...
  ensureBotAgentBoardSchemaInTransaction,
} from "./bot-agent-board-schema.js";
...
export function assertBotAgentSchemaContains(
...
export function assertBotAgentCurrentRuntimeSchema(
  database: DatabaseSync,
  ...
  if (metadata.schemaVersion !== BOT_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `Bot agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match ${BOT_AGENT_SCHEMA_VERSION}; run bot doctor --fix before using it.`,
    );
  }
...
function repairAndAssertAgentSchemaGroup(
  ...
  assertBotAgentSchemaContains(database, pathname, schemaSql);
}

export function repairAndAssertBotAgentV14SchemaForMigration(
  database: DatabaseSync,
  options: { agentId: string; pathname: string },
): void {
  ...
  if (userVersion !== 14) {
    throw new Error(
      `Bot agent database ${options.pathname} uses schema version ${userVersion}; expected 14 before migrating it.`,
    );
  }
  const agentId = normalizeAgentId(options.agentId);
  const metadata = readExistingAgentSchemaMeta(database);
  ...
  if (metadata.schemaVersion !== 14) {
    throw new Error(
      `Bot agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match 14; repair the ownership metadata before migrating it.`,
    );
  }
  ...
  if (hasAnyCanonicalTable(database, AGENT_V14_BOARD_SCHEMA_SQL)) {
    ...
    ensureBotAgentBoardSchemaInTransaction(database);
...
export function assertCanonicalAgentMediaPersistenceVersion(
  ...
  if (userVersion < BOT_AGENT_SCHEMA_VERSION && !isNewUnownedDatabase) {
    throw new Error(
      `Bot agent database ${pathname} uses schema version ${userVersion}; run bot doctor --fix to migrate persisted media before using it.`,
    );
  }
}
...
export function assertExistingAgentSchemaOwner(
  ...
  if (existing.role !== "agent") {
    throw new Error(
      `Bot agent database ${pathname} has schema role ${existing.role ?? "unknown"}; expected agent.`,
    );
  }
  if (!existing.agentId) {
    throw new Error(`Bot agent database ${pathname} has no agent owner.`);
  }
  if (normalizeAgentId(existing.agentId) !== agentId) {
    throw new Error(
      `Bot agent database ${pathname} belongs to agent ${existing.agentId}; requested agent ${agentId}.`,
    );
  }
}
