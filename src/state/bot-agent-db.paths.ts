...
type BotAgentSqlitePathOptions = {
  agentId: string;
  env?: NodeJS.ProcessEnv;
  path?: string;
};

const INCOGNITO_AGENT_SQLITE_BASENAME = "incognito-bot-agent.sqlite";

/** Resolve the SQLite file for one normalized agent id. */
export function resolveBotAgentSqlitePath(options: BotAgentSqlitePathOptions): string {
  const agentId = normalizeAgentId(options.agentId);
  return path.resolve(
    options.path ??
      path.join(
        path.dirname(resolveBotStateSqliteDir(options.env ?? process.env)),
        "agents",
        agentId,
        "agent",
        "bot-agent.sqlite",
...
export function resolveIncognitoBotAgentSqlitePath(
  options: Omit<BotAgentSqlitePathOptions, "path">,
): string {
  return path.join(
    path.dirname(resolveBotAgentSqlitePath(options)),
    INCOGNITO_AGENT_SQLITE_BASENAME,
  );
}

/** Identify the reserved incognito sentinel without touching its filesystem path. */
export function isIncognitoBotAgentSqlitePath(
  pathname: string,
  options: Omit<BotAgentSqlitePathOptions, "path">,
): boolean {
  return path.resolve(pathname) === resolveIncognitoBotAgentSqlitePath(options);
}
