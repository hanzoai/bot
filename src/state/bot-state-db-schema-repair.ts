...
} from "./bot-state-db-audit-migration.js";
import {
...
import { resolveDatabasePath } from "./bot-state-db-maintenance.js";
import * as operatorApprovalMigration from "./bot-state-db-operator-approval-migration.js";
import {
  tableExists,
  tableHasColumn,
  tablePrimaryKeyColumns,
} from "./bot-state-db-schema-helpers.js";
...
export function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  if (!hasCanonicalAuditEventsSchema(db)) {
    if (canRepairLegacyAuditEventsSchema(db)) {
      throw new Error(
        `Bot state database ${pathname} has a legacy audit event schema; run bot doctor --fix to migrate it.`,
      );
    }
    throw new Error(
      `Bot state database ${pathname} has a noncanonical audit event schema that cannot be repaired automatically; restore the canonical audit_events shape before retrying.`,
    );
  }
}
export function detectBotStateDatabaseSchemaMigrations(
  ...
  try {
    return detectBotStateDatabaseSchemaMigrationsFromDatabase(db, pathname);
  } finally {
    db.close();
  }
}
 ...
 */
export function detectBotStateDatabaseSchemaMigrationsFromDatabase(
  db: DatabaseSync,
  pathname: string,
): BotStateDatabaseSchemaMigration[] {
  const migrations: BotStateDatabaseSchemaMigration[] = [];
  ...
  if (tableExists(db, "audit_events") && userVersion < BOT_STATE_STRICT_SCHEMA_VERSION) {
