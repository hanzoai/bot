...
export const BOT_AGENT_SCHEMA_VERSION = 16;

/** Open per-agent SQLite database handle plus lifecycle maintenance. */
export type BotAgentDatabase = {
  agentId: string;
  db: DatabaseSync;
  path: string;
...
export type BotAgentDatabaseOptions = BotStateDatabaseOptions & {
  agentId: string;
};

/** Shared-state registry row describing an agent database seen by this process. */
export type BotRegisteredAgentDatabase = {
  agentId: string;
  path: string;
...
export type BotAgentDatabaseOwnerInspection =
