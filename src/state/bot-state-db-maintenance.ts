...
const BOT_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY = {
...
export function createBotDatabaseVerificationError(
  kind: "agent" | "state",
  pathname: string,
  ...
  const error = new Error(
    `Bot ${kind} database ${pathname} is quarantined after integrity verification failed: ${storedError ?? "unknown integrity error"}. Restore the database from a backup or repair it, then run bot doctor --fix to clear the quarantine. See ${BOT_DATABASE_SCHEMA_DOCS_URL}.`,
  );
...
export function assertBotStateDatabaseOwner(
  ...
  if (metadata?.role !== "global") {
    ...
    throw new Error(
      `Bot state database ${options.pathname} has schema role ${role}; expected global.`,
    );
  }
}

/** Require the canonical shared-state owner and schema before offline file maintenance. */
export function assertBotStateDatabaseForMaintenance(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  ...
  if (userVersion !== BOT_STATE_SCHEMA_VERSION) {
    throw new Error(
      `Bot state database ${options.pathname} uses schema version ${userVersion}; run bot doctor --fix before compacting it.`,
    );
  }

  assertBotStateDatabaseOwner(database, options);
  const metadata = database
    .prepare("SELECT schema_version FROM schema_meta WHERE meta_key = 'primary' LIMIT 1")
    .get() as { schema_version?: unknown } | undefined;
  if (metadata?.schema_version !== BOT_STATE_SCHEMA_VERSION) {
    const schemaVersion =
      typeof metadata?.schema_version === "number" ? metadata.schema_version : "invalid";
    throw new Error(
      `Bot state database ${options.pathname} metadata schema version ${schemaVersion} does not match ${BOT_STATE_SCHEMA_VERSION}; run bot doctor --fix before compacting it.`,
    );
  }
  assertSqliteSchemaContains(
    database,
    ...
    BOT_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY,
  );
}

/** Require every stable v5 table before the v6 additive migration can run. */
export function assertBotStateDatabaseV5ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  const userVersion = readSqliteUserVersion(database);
  if (userVersion !== 5) {
    throw new Error(
      `Bot state database ${options.pathname} uses schema version ${userVersion}; expected 5 before migrating it.`,
    );
  }
  assertBotStateDatabaseOwner(database, options);
  const metadata = database
  ...
  if (metadata?.schema_version !== 5) {
    const schemaVersion =
      typeof metadata?.schema_version === "number" ? metadata.schema_version : "invalid";
    throw new Error(
      `Bot state database ${options.pathname} metadata schema version ${schemaVersion} does not match 5; repair the ownership metadata before migrating it.`,
    );
  }
  assertSqliteSchemaTablesPresent(database, options.pathname, BOT_STATE_SCHEMA_SQL, {
    allowedMissingTables: STATE_V5_ADDITIVE_TABLES,
  });
}

export function resolveDatabasePath(options: BotStateDatabaseOptions = {}): string {
