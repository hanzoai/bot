...
import { invalidateRegisteredAgentDatabasesMemo } from "./bot-agent-db-registry-listing.js";
...
export { listBotRegisteredAgentDatabases } from "./bot-agent-db-registry-listing.js";
...
export function isSameBotAgentDatabasePath(left: string, right: string): boolean {
...
export function registerBotAgentDatabase(params: {
  agentId: string;
  path: string;
  ...
  runBotStateWriteTransaction(
    (database) => {
      ...
      const db = getNodeSqliteKysely<BotAgentRegistryDatabase>(database.db);
...
export function unregisterBotAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
}): void {
