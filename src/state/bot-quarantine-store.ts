...
import { resolveBotStateSqliteDir } from "./bot-state-db.paths.js";

const BOT_QUARANTINE_SCHEMA_VERSION = 2;
const BOT_QUARANTINE_BUSY_TIMEOUT_MS = 5_000;
const BOT_QUARANTINE_DIR_MODE = 0o700;
const BOT_QUARANTINE_FILE_MODE = 0o600;

type BotDatabaseKind = "agent" | "state";

type BotDatabaseQuarantine = {
  kind: BotDatabaseKind;
  quarantinedAt: number;
  reason: string;
};

function resolveQuarantineStorePath(env: NodeJS.ProcessEnv): string {
  return path.join(resolveBotStateSqliteDir(env), "bot-quarantine.sqlite");
}

function ensureQuarantineStoreDirectory(storePath: string): void {
  const dir = path.dirname(storePath);
  mkdirSync(dir, { recursive: true, mode: BOT_QUARANTINE_DIR_MODE });
  applyPrivateModeSync(dir, BOT_QUARANTINE_DIR_MODE);
}

function configureQuarantineWriter(database: DatabaseSync, storePath: string): void {
  database.exec(`
    PRAGMA busy_timeout = ${BOT_QUARANTINE_BUSY_TIMEOUT_MS};
    PRAGMA journal_mode = DELETE;
    PRAGMA synchronous = FULL;
  `);
  const userVersion = readQuarantineSchemaVersion(database, storePath);
  if (userVersion > BOT_QUARANTINE_SCHEMA_VERSION) {
    throw new Error(
      `Bot quarantine store ${storePath} uses newer schema version ${userVersion}.`,
    );
  }
  if (userVersion === BOT_QUARANTINE_SCHEMA_VERSION) {
    return;
  ...
  if (userVersion === 1) {
    database.exec(`
      ...
      PRAGMA user_version = ${BOT_QUARANTINE_SCHEMA_VERSION};
      COMMIT;
    `);
    return;
  }
...
function readQuarantineSchemaVersion(database: DatabaseSync, storePath: string): number {
  ...
  if (typeof userVersion !== "number" || !Number.isInteger(userVersion)) {
    throw new Error(`Bot quarantine store ${storePath} has an invalid schema version.`);
  }
  return userVersion;
}
...
function withQuarantineWriter<T>(env: NodeJS.ProcessEnv, operation: (db: DatabaseSync) => T): T {
  ...
  try {
    if (!existed) {
      applyPrivateModeSync(storePath, BOT_QUARANTINE_FILE_MODE);
    }
...
export function readBotDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): BotDatabaseQuarantine | undefined {
  ...
  const database = openNodeSqliteDatabase(storePath);
  try {
    database.exec(`PRAGMA busy_timeout = ${BOT_QUARANTINE_BUSY_TIMEOUT_MS};`);
    const userVersion = readQuarantineSchemaVersion(database, storePath);
    if (userVersion === 0) {
      return undefined;
    }
    ...
    if (
      ...
      throw new Error(`Bot quarantine store ${storePath} contains an invalid row.`);
    }
...
export function recordBotDatabaseQuarantine(options: {
...
export function clearBotDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): boolean {
