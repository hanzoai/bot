...
describe("Bot database maintenance schema validation", () => {
  it("accepts the current global and agent schemas", () => {
    ...
    try {
      expect(() =>
        assertBotStateDatabaseForMaintenance(globalDatabase, {
          pathname: "global.sqlite",
        }),
      ).not.toThrow();
      expect(() =>
        assertBotAgentDatabaseForMaintenance(agentDatabase, {
          agentId: "worker-1",
  ...
  it("accepts a global schema produced by an additive column migration", () => {
    const schemaWithoutMigratedColumn = BOT_STATE_SCHEMA_SQL.replace(
    ...
    try {
      ...
      expect(() =>
        assertBotStateDatabaseForMaintenance(database, {
          pathname: "global.sqlite",
        }),
      ).not.toThrow();
    } finally {
...
function createGlobalDatabase(schemaSql = BOT_STATE_SCHEMA_SQL): DatabaseSync {
  const database = new DatabaseSync(":memory:");
  database.exec(schemaSql);
  database.exec(`PRAGMA user_version = ${BOT_STATE_SCHEMA_VERSION};`);
  database
    .prepare(
      `
...
function createAgentDatabase(): DatabaseSync {
  const database = new DatabaseSync(":memory:");
  database.exec(BOT_AGENT_SCHEMA_SQL);
