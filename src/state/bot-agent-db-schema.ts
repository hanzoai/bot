...
import { ensureBotAgentBoardSchemaInTransaction } from "./bot-agent-board-schema.js";
import {
...
import { registerBotAgentDatabase } from "./bot-agent-db-registry.js";
import {
  assertExistingAgentSchemaOwner,
  assertBotAgentSchemaContains,
  assertBotAgentCurrentRuntimeSchema,
  assertSupportedAgentSchemaVersion,
  readExistingAgentSchemaMeta,
  repairAndAssertBotAgentV14SchemaForMigration,
...
} from "./bot-agent-db-session-migrations.js";
import { migrateSessionNodesAndWindows } from "./bot-agent-db-session-nodes-migration.js";
import {
  addSessionProvenanceColumns,
  backfillSessionEntryProvenance,
  backfillTranscriptMutationWatermarks,
} from "./bot-agent-db-session-provenance.js";
...
type BotAgentMetadataDatabase = Pick<BotAgentKyselyDatabase, "schema_meta">;
...
function migrateBotAgentSchema(db: DatabaseSync): void {
...
function backfillBotAgentSchema(db: DatabaseSync, previousVersion: number): void {
...
export function assertAgentDatabaseIntegrityBeforeMutation(
  ...
  const migrationPending =
    ...
    (userVersion > 0 && userVersion < BOT_AGENT_SCHEMA_VERSION);
  if (migrationPending) {
    agentDbLog.info("agent database schema migration pending; verifying integrity first", {
      fromVersion: userVersion,
      path: pathname,
      toVersion: BOT_AGENT_SCHEMA_VERSION,
    });
  }
  if (userVersion === BOT_AGENT_SCHEMA_VERSION) {
    verifyAndRepairCanonicalSqliteIndexes(database, pathname, BOT_AGENT_SCHEMA_SQL, {
      allowMissingColumns: true,
      validateAfterRepair: () =>
        assertBotAgentCurrentRuntimeSchema(database, { agentId, pathname }),
    });
  } else {
...
function assertAgentSchemaVersion(
  ...
  if (userVersion !== options.version || metadata?.schemaVersion !== options.version) {
    throw new Error(
      `Bot agent database ${options.pathname} did not converge on schema version ${options.version}.`,
    );
  }
  assertBotAgentSchemaContains(db, options.pathname, BOT_AGENT_SCHEMA_SQL);
}
...
function ensureAgentSchema(
  ...
  agentId: string,
  pathname: string,
  targetVersion = BOT_AGENT_SCHEMA_VERSION,
): void {
  ...
  try {
    runSqliteImmediateTransactionSync(db, () => {
      ...
      if (previousVersion > targetVersion) {
        throw new Error(
          `Bot agent database ${pathname} uses schema version ${previousVersion}; expected at most ${targetVersion} for this migration.`,
        );
      }
      if (previousVersion === targetVersion) {
        ...
        repairCanonicalSqliteIndexes(db, pathname, BOT_AGENT_SCHEMA_SQL, {
          verifyPhysicalIntegrity: false,
        });
        assertAgentSchemaVersion(db, { agentId, pathname, version: targetVersion });
        return;
      } else if (previousVersion === 14) {
        repairAndAssertBotAgentV14SchemaForMigration(db, { agentId, pathname });
      }
      ...
      migrateBotAgentSchema(db);
      migrateConversationDeliveryTargetColumn(db);
      backfillBotAgentSchema(db, previousVersion);
      ...
      const kysely = getNodeSqliteKysely<BotAgentMetadataDatabase>(db);
...
export function ensureBotAgentDatabaseSchema(
  db: DatabaseSync,
  options: BotAgentDatabaseOptions & { register?: boolean },
): void {
...
export function migrateBotAgentDatabaseToMediaPrerequisiteSchema(
  db: DatabaseSync,
  options: BotAgentDatabaseOptions,
): void {
  const targetVersion = BOT_AGENT_SCHEMA_VERSION - 1;
  const userVersion = readSqliteUserVersion(db);
...
export { ensureAgentSchema as ensureBotAgentSchema };
