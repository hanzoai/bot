...
function createOpenAiAudioCfg(extra?: Partial<BotConfig>): BotConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runCapability auto audio entries", () => {
  ...
  it("skips OpenAI audio auto-selection when only ChatGPT OAuth is available", async () => {
    ...
    try {
      await withAudioFixture("bot-auto-audio-oauth-skip", async ({ ctx, media, cache }) => {
  ...
  it("passes workspaceDir to auto-selected audio provider execution auth", async () => {
    ...
    await withAudioFixture("bot-auto-audio-workspace-auth", async ({ ctx, media, cache }) => {
      const result = await runCapability({
        capability: "audio",
        cfg: {
          models: {
  ...
  it("uses the provider audio default instead of the active Codex chat model", async () => {
    ...
    await withAudioFixture("bot-auto-audio-codex", async ({ ctx, media, cache }) => {
  ...
  it("does not leak the active xAI chat model into model-less batch STT", async () => {
    ...
    await withAudioFixture("bot-auto-audio-xai", async ({ ctx, media, cache }) => {
      const providerRegistry = createProviderRegistry({
        xai: {
          id: "xai",
          capabilities: ["audio"],
  ...
  it("prefers provider keys over auto-detected local whisper", async () => {
    const binDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-auto-audio-bin-"));
    try {
  ...
  it("uses mistral when only mistral key is configured", async () => {
    const isolatedAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-audio-agent-"));
    let runResult: Awaited<ReturnType<typeof runCapability>> | undefined;
    try {
      await withEnvAsync(
        {
