...
describe("media-understanding missing provider errors", () => {
  it("includes the catalog repair hint for a media provider contract", async () => {
    ...
    expect(message).toMatch(/^Media provider not available: groq .*bot plugins install/);
    expect(message).toContain("@hanzo/bot-groq-provider");
    expect(message).toContain("bot plugins registry --refresh");
    expect(message).toContain("stop and start the gateway service");
    expect(message).toContain("bot doctor --fix");
  });
