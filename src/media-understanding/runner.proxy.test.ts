...
function createOpenAiAudioCfg(providerOverrides: Record<string, unknown> = {}): BotConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runCapability proxy fetch passthrough", () => {
  ...
  it("passes allowPrivateNetwork to audio provider when set in providerConfig.request", async () => {
    ...
    await withAudioFixture("bot-audio-allowprivatenetwork", async ({ ctx, media, cache }) => {
      const providerRegistry = buildProviderRegistry({
        openai: {
          id: "openai",
          capabilities: ["audio"],
