...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "bot-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __BOT_VERSION__");
    expect(source).toContain("? __BOT_VERSION__");
  });
  ...
  it("prefers runtime VERSION over stale BOT_VERSION for compatibility checks", () => {
    const previousCompatibility = process.env.BOT_COMPATIBILITY_HOST_VERSION;
    const previous = process.env.BOT_VERSION;
    const previousService = process.env.BOT_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      delete process.env.BOT_COMPATIBILITY_HOST_VERSION;
      process.env.BOT_VERSION = "2026.3.25";
      process.env.BOT_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("BOT_COMPATIBILITY_HOST_VERSION", previousCompatibility);
      restoreEnvValue("BOT_VERSION", previous);
      restoreEnvValue("BOT_SERVICE_VERSION", previousService);
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        BOT_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        BOT_VERSION: "2026.3.99",
        BOT_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
