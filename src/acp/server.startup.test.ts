...
vi.mock("../state/bot-state-db.js", () => ({
  closeBotStateDatabase: () => mockState.closeBotStateDatabase(),
}));
...
describe("serveAcpGateway startup", () => {
  ...
  beforeEach(async () => {
    ...
    mockState.closeBotStateDatabase.mockReset();
  ...
  it.each([
    {
      ...
      opts: {},
      expected: ["bot acp: gateway event chat failed\n"],
    },
    {
      name: "verbose logging",
      opts: { verbose: true },
      expected: [
        "bot acp: gateway event chat failed\n",
        "bot acp: gateway event chat error: Error: handler boom\n",
      ],
    },
  ...
  it("shuts down when buffered pre-hello ACP input exceeds its limit", async () => {
    ...
    try {
      ...
      expect(mockState.agentSideConnectionCtor).not.toHaveBeenCalled();
      expect(mockState.closeBotStateDatabase).toHaveBeenCalledOnce();
    } finally {
      onceSpy.mockRestore();
    }
  });
  ...
  it("closes the shared state database on shutdown", async () => {
    const { signalHandlers, onceSpy } = captureProcessSignalHandlers();
    expect(mockState.closeBotStateDatabase).not.toHaveBeenCalled();
  ...
  it("closes a real node:sqlite DatabaseSync handle through serveAcpGateway shutdown", async () => {
    ...
    // the closeBotStateDatabase helper in isolation.
    const actualStateDb = await vi.importActual<typeof import("../state/bot-state-db.js")>(
      "../state/bot-state-db.js",
    );

    const realDb = actualStateDb.openBotStateDatabase();
    expect(realDb.db.isOpen).toBe(true);
    expect(actualStateDb.isBotStateDatabaseOpen()).toBe(true);

    // Wire the test mock so serveAcpGateway's shutdown handler calls the
    // real closeBotStateDatabase, which closes the handle we opened above.
    mockState.closeBotStateDatabase.mockImplementation(() => {
      actualStateDb.closeBotStateDatabase();
    });
    ...
    try {
      ...
      expect(actualStateDb.isBotStateDatabaseOpen()).toBe(false);
    } finally {
      actualStateDb.closeBotStateDatabase();
      onceSpy.mockRestore();
    }
