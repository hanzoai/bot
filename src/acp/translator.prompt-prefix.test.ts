...
import { createInMemorySessionStore } from "@hanzo/bot-acp-core/session";
...
describe("acp prompt cwd prefix", () => {
  ...
  async function runPromptAndCaptureRequest(
    ...
    sessionStore.createSession({
      ...
      cwd: options.cwd ?? path.join(os.homedir(), "bot-test"),
    });
  ...
  async function runPromptWithCwd(cwd: string) {
    ...
    return await withEnvAsync(
      { BOT_HOME: undefined, HOME: pinnedHome },
  ...
  it("redacts home directory in prompt prefix", async () => {
    ...
    expect(payload.message).toMatch(/\[Working directory: ~[\\/]bot-test\]/);
  });

  it("keeps backslash separators when cwd uses them", async () => {
    const requestSpy = await runPromptWithCwd(`${os.homedir()}\\bot-test`);
    const payload = chatSendPayload(requestSpy);
    expect(payload.message).toContain("[Working directory: ~\\bot-test]");
  });
  ...
  it("injects a system provenance receipt when requested", async () => {
    ...
    expect(receipt).toContain("bridge=bot-acp");
  ...
  it("retries without provenance when the gateway rejects admin-only provenance fields", async () => {
    ...
    sessionStore.createSession({
      sessionId: TEST_SESSION_ID,
      sessionKey: TEST_SESSION_KEY,
      cwd: path.join(os.homedir(), "bot-test"),
    });
    const agent = new AcpGatewayAgent(
      createAcpConnection(),
      createAcpGateway(requestSpy as unknown as GatewayClient["request"]),
