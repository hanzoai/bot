...
export type SpeechProviderRegistryResolver = {
  getProvider: (providerId: string, cfg?: BotConfig) => SpeechProviderPlugin | undefined;
  listProviders: (cfg?: BotConfig) => SpeechProviderPlugin[];
};
...
export function createSpeechProviderRegistry(resolver: SpeechProviderRegistryResolver) {
  const buildResolvedProviderMaps = (cfg?: BotConfig) =>
    buildCapabilityProviderMaps(resolver.listProviders(cfg));

  const listProviders = (cfg?: BotConfig): SpeechProviderPlugin[] => [
