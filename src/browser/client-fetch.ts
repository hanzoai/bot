...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the Hanzo Bot gateway (Hanzo Bot.app menubar, or \`${formatCliCommand("hanzo-bot gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the Hanzo Bot browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the Hanzo Bot browser control service. ${hint} (${msg})`);
}
