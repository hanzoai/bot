...
const LIVE =
  isLiveTestEnabled(["BOT_LIVE_SKILL_HISTORY_SCAN"]) &&
...
function liveConfig(): BotConfig {
  const modelId = process.env.BOT_LIVE_SKILL_HISTORY_MODEL ?? "gpt-5.6-luna";
  return {
    models: {
      providers: {
        openai: {
...
describeLive("Skill Workshop history scan live OpenAI eval", () => {
  beforeAll(async () => {
    testState = await createBotTestState({
      layout: "state-only",
      prefix: "bot-live-skill-history-state-",
    });
    workspaceDir = await tempDirs.make("bot-live-skill-history-workspace-");
  });
