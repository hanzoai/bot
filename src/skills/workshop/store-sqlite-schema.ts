...
export function databaseOptions(
  ...
  if (options.stateDir) {
    return {
      ...
      path: path.join(path.resolve(options.stateDir), "state", "bot.sqlite"),
    };
  }
...
export function ensureSkillWorkshopSchema(options: SkillWorkshopStoreOptions = {}): void {
  ...
  const database = openBotStateDatabase(dbOptions);
...
export function openSkillWorkshopStore(options: SkillWorkshopStoreOptions = {}) {
  ...
  const database = openBotStateDatabase(databaseOptions(options));
  return {
    database,
