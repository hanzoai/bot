...
describe("buildWorkspaceSkillStatus", () => {
  it("reports blank env requirements as missing", () => {
    const envName = "BOT_TEST_BLANK_SKILL_STATUS";
  ...
  it("links a discovered global ClawHub skill only through the managed lockfile", async () => {
    const managedParentDir = tempDirs.make("bot-managed-");
    const workspaceDir = tempDirs.make("bot-skill-status-");
    ...
    expect(skill).toMatchObject({ source: "bot-managed" });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const managedParentDir = tempDirs.make("bot-managed-");
      const externalSkillDir = tempDirs.make("bot-skill-target-");
      const workspaceDir = tempDirs.make("bot-skill-status-");
      const managedSkillsDir = path.join(managedParentDir, "skills");
  ...
  it("classifies a mixed broken skill pack without flattening visibility reasons", () => {
    const missingBin = "bot-test-definitely-missing-skill-bin";
    const report = buildWorkspaceSkillStatus("/tmp/ws", {
      agentId: "specialist",
      config: {
        agents: {
      ...
      entries: [
        ...
        createEntry("needs-bin", {
          metadata: {
            ...
            install: [
              {
                kind: "node",
                package: "@hanzo/bot-missing-skill-bin",
                bins: [missingBin],
              },
            ],
          },
        ...
        createEntry("needs-env", {
          metadata: {
            primaryEnv: "BOT_TEST_MISSING_SKILL_KEY",
            requires: { env: ["BOT_TEST_MISSING_SKILL_KEY"] },
          },
        }),
        ...
        createEntry("bundled-blocked", { source: "bot-bundled" }),
      ],
    });
    ...
    expect(needsBin.install).toStrictEqual([
      {
        kind: "node",
        id: "node-0",
        label: "Install @hanzo/bot-missing-skill-bin (pnpm)",
        bins: [missingBin],
      },
    ]);
    const needsEnv = requireSkillStatus(byName, "needs-env");
    expect(needsEnv.eligible).toBe(false);
    expect(needsEnv.primaryEnv).toBe("BOT_TEST_MISSING_SKILL_KEY");
    expect(needsEnv.missing).toStrictEqual({
      anyBins: [],
      bins: [],
      config: [],
      env: ["BOT_TEST_MISSING_SKILL_KEY"],
      os: [],
    });
