...
function resolveWatchTargets(workspaceDir: string, config?: BotConfig): WatchTarget[] {
  const baseRoots: Array<{ path: string; source: string }> = [];
  if (workspaceDir.trim()) {
    baseRoots.push({ path: path.join(workspaceDir, "skills"), source: "bot-workspace" });
    baseRoots.push({
      path: path.join(workspaceDir, ".agents", "skills"),
      source: "agents-skills-project",
    });
  }
  baseRoots.push({ path: path.join(CONFIG_DIR, "skills"), source: "bot-managed" });
  baseRoots.push({
...
function isTrustedSymlinkSkillTarget(
  ...
  if (source === "bot-managed" || source === "agents-skills-personal") {
    return true;
  }
  return (
...
export function ensureSkillsWatcher(params: { workspaceDir: string; config?: BotConfig }) {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("bot.skillsRefreshTestApi")] = {
