...
function createRemoteSkillWorkspace(bin: string): { cfg: BotConfig; workspaceDir: string } {
  const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-remote-skills-"));
  ...
  fs.writeFileSync(
    ...
    [
      ...
      `metadata: { "bot": { "os": ["darwin"], "requires": { "bins": ["${bin}"] } } }`,
      "---",
      "# Remote Skill",
      "",
    ].join("\n"),
