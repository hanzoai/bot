...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "bot");
  }
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "bot-bundled"
        ? { kind: "bundled", authority: "bot", mutable: false, network: false }
        : skillSource === "bot-managed" || skillSource === "bot-extra"
          ? { kind: "managed", authority: "bot", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["bot-bundled", "bot-managed", "bot-extra"]);
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("bot.skillsInstallTestApi")] =
    testing;
}
