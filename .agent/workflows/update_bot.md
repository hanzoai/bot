---
description: Update Bot from upstream when branch has diverged (ahead/behind)
---

# Bot Upstream Sync Workflow
...
# Move old version
mv /Applications/Bot.app /tmp/Bot-backup.app

# Install new build
cp -R dist/Bot.app /Applications/
...
ps aux | grep -E "(bot|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm bot agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update bot.json with fallback chains
...
pnpm bot agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
