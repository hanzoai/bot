...
ARG BOT_EXTENSIONS=""
ARG BOT_BUNDLED_PLUGIN_DIR=extensions
ARG BOT_DOCKER_BUILD_NODE_OPTIONS="--max-old-space-size=8192"
ARG BOT_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=""
ARG BOT_DOCKER_BUILD_SKIP_DTS=1
ARG BOT_NODE_BOOKWORM_IMAGE="docker.io/library/node:24-bookworm@sha256:5711a0d445a1af54af9589066c646df387d1831a608226f4cd694fc59e745059"
ARG BOT_NODE_BOOKWORM_SLIM_IMAGE="docker.io/library/node:24-bookworm-slim@sha256:6f7b03f7c2c8e2e784dcf9295400527b9b1270fd37b7e9a7285cf83b6951452d"
ARG BOT_NODE_BOOKWORM_SLIM_DIGEST="sha256:6f7b03f7c2c8e2e784dcf9295400527b9b1270fd37b7e9a7285cf83b6951452d"
# Keep in sync with .github/actions/setup-node-env/action.yml bun-version.
# To update: docker buildx imagetools inspect docker.io/oven/bun:<version> and use the manifest-list digest.
ARG BOT_BUN_IMAGE="docker.io/oven/bun:1.3.14@sha256:e10577f0db68676a7024391c6e5cb4b879ebd17188ab750cf10024a6d700e5c4"
...
FROM ${BOT_NODE_BOOKWORM_IMAGE} AS workspace-deps
ARG BOT_EXTENSIONS
ARG BOT_BUNDLED_PLUGIN_DIR
...
COPY ${BOT_BUNDLED_PLUGIN_DIR} /tmp/${BOT_BUNDLED_PLUGIN_DIR}
RUN mkdir -p /out/packages "/out/${BOT_BUNDLED_PLUGIN_DIR}" && \
    ...
    node /tmp/docker-plugin-selection.mjs "/tmp/${BOT_BUNDLED_PLUGIN_DIR}" "$BOT_EXTENSIONS" \
      > /out/bot-selected-plugin-dirs && \
    while IFS= read -r ext; do \
      ext_dir="/tmp/${BOT_BUNDLED_PLUGIN_DIR}/$ext"; \
      if [ -f "$ext_dir/package.json" ]; then \
        mkdir -p "/out/${BOT_BUNDLED_PLUGIN_DIR}/$ext" && \
        cp "$ext_dir/package.json" "/out/${BOT_BUNDLED_PLUGIN_DIR}/$ext/package.json"; \
      fi; \
    done < /out/bot-selected-plugin-dirs

# ── Stage 2: Build ──────────────────────────────────────────────
FROM ${BOT_BUN_IMAGE} AS bun-binary
FROM ${BOT_NODE_BOOKWORM_IMAGE} AS build
ARG BOT_BUNDLED_PLUGIN_DIR
ARG BOT_DOCKER_BUILD_NODE_OPTIONS
ARG BOT_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB
ARG BOT_DOCKER_BUILD_SKIP_DTS
...
COPY --from=workspace-deps /out/${BOT_BUNDLED_PLUGIN_DIR}/ ./${BOT_BUNDLED_PLUGIN_DIR}/
COPY --from=workspace-deps /out/bot-selected-plugin-dirs /tmp/bot-selected-plugin-dirs

# Reduce OOM risk on low-memory hosts during dependency installation.
# Docker builds on small VMs may otherwise fail with "Killed" (exit 137).
RUN --mount=type=cache,id=bot-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
...
RUN set -eux; \
    if ! grep -qx 'matrix' /tmp/bot-selected-plugin-dirs; then \
...
ARG GIT_COMMIT=""
ARG BOT_BUILD_TIMESTAMP=""
ENV GIT_COMMIT=${GIT_COMMIT} \
    BOT_BUILD_TIMESTAMP=${BOT_BUILD_TIMESTAMP}
...
RUN for dir in /app/${BOT_BUNDLED_PLUGIN_DIR} /app/.agent /app/.agents; do \
      if [ -d "$dir" ]; then \
...
RUN pnpm_config_verify_deps_before_run=false pnpm canvas:a2ui:bundle || \
    (echo "A2UI bundle: creating stub (non-fatal)" && \
     ...
     rm -rf vendor/a2ui apps/shared/BotKit/Tools/CanvasA2UI)
# Force pnpm for UI build (Bun may fail on ARM/Synology architectures)
ENV BOT_PREFER_PNPM=1
RUN set -eu; \
    selected_plugin_dirs="$(cat /tmp/bot-selected-plugin-dirs)"; \
    if [ -z "$BOT_BUILD_TIMESTAMP" ]; then \
      BOT_BUILD_TIMESTAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"; \
      export BOT_BUILD_TIMESTAMP; \
    fi; \
    if grep -qx 'qa-lab' /tmp/bot-selected-plugin-dirs; then \
      export BOT_BUILD_PRIVATE_QA=1 BOT_ENABLE_PRIVATE_QA_CLI=1; \
    fi; \
    BOT_INTERNAL_DOCKER_BUILD_PLUGIN_IDS="$selected_plugin_dirs" BOT_RUN_NODE_SKIP_DTS_BUILD="$BOT_DOCKER_BUILD_SKIP_DTS" BOT_TSDOWN_MAX_OLD_SPACE_MB="$BOT_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB" NODE_OPTIONS="$BOT_DOCKER_BUILD_NODE_OPTIONS" pnpm_config_verify_deps_before_run=false pnpm build:docker; \
    pnpm_config_verify_deps_before_run=false pnpm ui:build
RUN if grep -qx 'qa-lab' /tmp/bot-selected-plugin-dirs; then \
...
RUN --mount=type=cache,id=bot-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
    ...
    CI=true pnpm prune --prod \
      ...
      --config.supportedArchitectures.cpu="$(node -p 'process.arch')" \
      --config.supportedArchitectures.libc=glibc && \
    BOT_EXTENSIONS="$(cat /tmp/bot-selected-plugin-dirs)" BOT_BUNDLED_PLUGIN_DIR="$BOT_BUNDLED_PLUGIN_DIR" node scripts/prune-docker-plugin-dist.mjs && \
    node scripts/postinstall-bundled-plugins.mjs && \
    find dist -type f \( -name '*.d.ts' -o -name '*.d.mts' -o -name '*.d.cts' -o -name '*.map' \) -delete && \
    if [ -L /app/node_modules/@hanzo/bot-ai ]; then \
      ai_runtime_target="$(readlink -f /app/node_modules/@hanzo/bot-ai)" && \
      ai_runtime_tmp="$(mktemp -d)" && \
      cp -a "$ai_runtime_target" "$ai_runtime_tmp/ai" && \
      rm /app/node_modules/@hanzo/bot-ai && \
      mv "$ai_runtime_tmp/ai" /app/node_modules/@hanzo/bot-ai && \
      rmdir "$ai_runtime_tmp"; \
    fi && \
    rm -rf \
      /app/node_modules/bot \
      /app/node_modules/.bin/bot \
      /app/node_modules/.pnpm/bot@*/node_modules/bot && \
    node scripts/check-package-dist-imports.mjs /app

# ── Runtime base image ──────────────────────────────────────────
FROM ${BOT_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime
...
# - https://docs.bot.ai/install/docker
LABEL org.opencontainers.image.source="https://github.com/hanzoai/bot" \
  org.opencontainers.image.url="https://bot.ai" \
  org.opencontainers.image.documentation="https://docs.bot.ai/install/docker" \
...
RUN --mount=type=cache,id=bot-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=bot-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
COPY --from=runtime-assets --chown=node:node /app/bot.mjs .
COPY --from=runtime-assets --chown=node:node /app/src/agents/templates ./src/agents/templates
COPY --from=runtime-assets --chown=node:node /app/${BOT_BUNDLED_PLUGIN_DIR} ./${BOT_BUNDLED_PLUGIN_DIR}
...
# Example: docker build --build-arg BOT_IMAGE_APT_PACKAGES="python3 wget" .
# Legacy alias: BOT_DOCKER_APT_PACKAGES is still accepted as a fallback.
ARG BOT_IMAGE_APT_PACKAGES
ARG BOT_DOCKER_APT_PACKAGES=""
ENV PATH="/home/node/.local/bin:${PATH}"
RUN --mount=type=cache,id=bot-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=bot-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    packages="${BOT_IMAGE_APT_PACKAGES-$BOT_DOCKER_APT_PACKAGES}"; \
    if [ -n "$packages" ]; then \
      apt-get update && \
...
# Example: docker build --build-arg BOT_IMAGE_PIP_PACKAGES="requests humanize" .
ARG BOT_IMAGE_PIP_PACKAGES=""
RUN --mount=type=cache,id=bot-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=bot-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$BOT_IMAGE_PIP_PACKAGES" ]; then \
...
RUN ln -sf /app/bot.mjs /usr/local/bin/bot \
 && chmod 755 /app/bot.mjs
...
# the leaf /home/node/.config/bot inherits the correct parent permissions.
...
RUN install -d -m 0755 -o node -g node /home/node/.config && \
    install -d -m 0700 -o node -g node \
      /home/node/.bot \
      /home/node/.bot/workspace \
      /home/node/.config/bot && \
    stat -c '%U:%G %a' /home/node/.bot | grep -qx 'node:node 700' && \
    stat -c '%U:%G %a' /home/node/.bot/workspace | grep -qx 'node:node 700' && \
    stat -c '%U:%G %a' /home/node/.config | grep -qx 'node:node 755' && \
    stat -c '%U:%G %a' /home/node/.config/bot | grep -qx 'node:node 700'
