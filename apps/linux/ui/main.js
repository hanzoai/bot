...
function friendlyError(error) {
  ...
  return error?.message || "Bot could not complete the operation.";
}
...
function renderGateways(gateways) {
  ...
  if (!gateways.length) {
    ...
    empty.textContent = "Looking for nearby Bot gateways…";
  ...
  try {
    ...
    if (snapshot.phase === "missingCli") {
      ...
      render({
        ...
        description: "Bot is installing its managed CLI and Node runtime.",
        eyebrow: "FIRST-RUN SETUP",
        title: "Preparing Bot",
      });
      await install();
...
async function runGatewayAction(action) {
  render({
    ...
    description: "Bot is waiting for the local gateway to become healthy.",
    eyebrow: "GATEWAY",
    title: "One moment",
  });
  try {
...
if (mode === "reconnecting") {
  render({
    ...
    description: "The gateway connection dropped. Bot will restore the dashboard automatically.",
