...
extension BotChatViewModel {
    ...
    func replaceMessages(_ messages: [BotChatMessage]) {
        ...
        canonicalMessageIdempotencyKeys: Set<String>)
    {
        ...
        let branchStateTask: Task<BotChatOutboxBranchState?, Never>? = if let outbox, let scope {
    ...
    func persistSessionsToCache(_ sessions: [BotChatSessionEntry]) {
    ...
    func paintFromCacheIfNeeded(session: SessionSnapshot) {
        ...
        if sessions.isEmpty, !hasAppliedLiveSessions {
            Task { [weak self] in
                ...
                let organized = BotChatSessionListOrganizer.organize(cached)
    ...
    static func transcriptCacheAgentID(sessionKey: String, agentID: String?) -> String? {
        guard BotChatSessionKey.agentID(from: sessionKey) == nil else { return nil }
