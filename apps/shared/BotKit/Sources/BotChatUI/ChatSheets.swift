...
public struct ChatSessionsSheet: View {
    ...
    @State private var scopedSessions: [BotChatSessionEntry] = []
    @State private var isLoadingScoped = false
    @State private var renameTarget: BotChatSessionEntry?
    ...
    @State private var inspectedSession: BotChatSessionEntry?
    @State private var isPresentingGroups = false

    public init(viewModel: BotChatViewModel) {
    ...
    private var displayedSessions: [BotChatSessionEntry] {
    ...
    public var body: some View {
        NavigationStack {
            ...
            .onAppear {
                self.viewModel.refreshSessions(limit: BotChatViewModel.sessionListFetchLimit)
            }
    ...
    private func interactiveSessionRow(_ session: BotChatSessionEntry) -> some View {
        ...
        return Button {
            ...
            .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                    ...
                    mainSessionKey: self.viewModel.resolvedMainSessionKey)
                {
                    ...
                    .tint(session.isArchived ? BotChatTheme.accent : BotChatTheme.danger)
                }
            }
            .contextMenu {
                Button {
    ...
    private func sessionRowContent(_ session: BotChatSessionEntry) -> some View {
        HStack(spacing: 8) {
            VStack(alignment: .leading, spacing: 4) {
                Text(session.displayName ?? session.key)
                    .font(BotChatTypography.mono(size: 17, relativeTo: .body))
                    .lineLimit(1)
