...
struct ChatCameraFlipButton: View {
    let control: BotChatTalkControl
    let size: CGFloat

    static func isAvailable(for control: BotChatTalkControl) -> Bool {
    ...
    var body: some View {
        Button {
            ...
            Image(systemName: "arrow.triangle.2.circlepath.camera")
                .font(BotChatTypography.body(size: 14, weight: .semibold, relativeTo: .subheadline))
                .foregroundStyle(.primary)
                .frame(width: self.size, height: self.size)
                .background {
                    Circle()
                        .fill(BotChatTheme.accent.opacity(0.12))
                }
...
struct ChatTalkButton: View {
    ...
    private func compactButton(controlHeight: CGFloat, iconControlSize: CGFloat) -> some View {
        Button {
            ...
            ChatTalkButtonGlyph(control: self.control)
                ...
                .background {
                    Circle()
                        .fill(self.control.isEnabled ? self.fill : AnyShapeStyle(BotChatTheme.accent))
    ...
    private var stroke: Color {
        ...
        return BotChatTheme.composerBorder
    }
...
enum ChatDictationActions {
    static func start(
        _ control: BotChatDictationControl,
    ...
    static func cancel(
        task: Binding<Task<Void, Never>?>,
        control: BotChatDictationControl?)
    {
        guard task.wrappedValue != nil else { return }
