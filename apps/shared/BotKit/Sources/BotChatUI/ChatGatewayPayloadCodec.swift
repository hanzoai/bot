...
public enum BotChatGatewayPayloadCodec {
    ...
    public static func decodeAgentWaitObservation(_ data: Data) throws -> BotChatRunObservation {
        let decoded = try JSONDecoder().decode(AgentWaitResponse.self, from: data)
        return BotChatRunObservation.fromWaitResponse(
    ...
    public static func decodeModelChoices(_ data: Data) throws -> [BotChatModelChoice] {
        let decoded = try JSONDecoder().decode(ModelsListResult.self, from: data)
        return decoded.models.map(self.modelChoice)
    }

    public static func decodeSessionRoutingIdentity(_ data: Data) throws -> BotChatSessionRoutingIdentity {
        let decoded = try JSONDecoder().decode(AgentsListResult.self, from: data)
        guard let identity = BotChatSessionRoutingIdentity(
    ...
    public static func modelChoice(_ model: ModelChoice) -> BotChatModelChoice {
        let name = model.name.trimmingCharacters(in: .whitespacesAndNewlines)
        return BotChatModelChoice(
            modelID: model.id,
    ...
    public static func commandChoice(_ entry: CommandEntry) -> BotChatCommandChoice {
        let sourceValue = (entry.source.value as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        let source: BotChatCommandChoice.Source = switch sourceValue {
        case "native":
            .command
        case "skill":
            .skill
    ...
    public static func event(from frame: EventFrame) -> BotChatTransportEvent? {
        switch frame.event {
        case "tick":
            return .tick
        case "sessions.changed":
            guard let payload = frame.payload,
                  let change = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: BotChatSessionsChangedEvent.self)
            else { return nil }
        ...
        case "session.message":
            guard let payload = frame.payload,
                  let message = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: BotSessionMessageEventPayload.self)
            else { return nil }
        ...
        case "question.resolved":
            guard let payload = frame.payload,
                  let resolved = try? GatewayPayloadDecoding.decode(
                      payload,
                      as: BotQuestionResolvedEvent.self)
            else { return nil }
