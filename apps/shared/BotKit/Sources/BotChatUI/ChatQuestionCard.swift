...
public enum BotQuestionCardStatus: Sendable, Equatable {
    case pending
    case submitting
    case answered
...
@MainActor
@Observable
public final class BotQuestionCardModel: Identifiable {
    ...
    public func status(at date: Date = Date()) -> BotQuestionCardStatus {
    ...
    public func apply(resolved: BotQuestionResolvedEvent) {
...
struct BotQuestionCard: View {
    @Bindable private var model: BotQuestionCardModel
    private let onSubmit: @MainActor @Sendable (BotQuestionCardModel) async -> Void
    private let onSkip: (@MainActor @Sendable (BotQuestionCardModel) async -> Void)?
    #if os(macOS)
    @FocusState private var focusedQuestionID: String?
    #endif

    init(
        model: BotQuestionCardModel,
        onSubmit: @escaping @MainActor @Sendable (BotQuestionCardModel) async -> Void,
        onSkip: @escaping @MainActor @Sendable (BotQuestionCardModel) async -> Void)
    {
        self.model = model
    ...
    private var pendingCard: some View {
        TimelineView(.periodic(from: .now, by: 1)) { context in
            ...
            .padding(16)
            .background(BotChatTheme.subtleCard, in: RoundedRectangle(cornerRadius: 16))
    ...
    private var terminalSummary: some View {
            ...
            ForEach(self.model.record.questions, id: \.questionid) { question in
                HStack(alignment: .firstTextBaseline, spacing: 5) {
                    Text(verbatim: "\(question.header):")
                        .font(BotChatTypography.body(size: 14, weight: .semibold, relativeTo: .callout))
                    Text(self.model.terminalSummaryText(for: question))
                        .font(BotChatTypography.body(size: 14, weight: .regular, relativeTo: .callout))
        ...
        .padding(.vertical, 9)
        .background(BotChatTheme.subtleCard, in: RoundedRectangle(cornerRadius: 12))
    ...
    private func questionSection(_ question: Question, now: Date) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(question.header.uppercased())
                ...
                .foregroundStyle(BotChatTheme.accent)
        ...
        return Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: selected
                    ...
                    .foregroundStyle(selected ? BotChatTheme.accent : .secondary)
                VStack(alignment: .leading, spacing: 2) {
                    Text(option.label).font(BotChatTypography.body)
    ...
    private func footer(now: Date) -> some View {
        ...
        if status == .pending || status == .submitting {
            ...
            if let errorText = self.model.errorText {
                Text(errorText)
                    .font(BotChatTypography.caption)
                    .foregroundStyle(BotChatTheme.danger)
            }
        }
    }
...
@MainActor
struct BotQuestionCards: View {
    let viewModel: BotChatViewModel

    var body: some View {
        ForEach(self.viewModel.visibleQuestionCards) { card in
            BotQuestionCard(model: card) { [weak viewModel = self.viewModel] model in
...
extension BotChatViewModel {
    public var visibleQuestionCards: [BotQuestionCardModel] {
    ...
    private func fetchMissingQuestionLookups(
        _ models: [BotQuestionCardModel]) async
        -> [(BotQuestionCardModel, QuestionLookupResult)]
    {
        ...
        var lookups: [(BotQuestionCardModel, QuestionLookupResult)] = []
        for model in models {
            do {
    ...
    private func applyQuestionRefresh(
        ...
        lookups: [(BotQuestionCardModel, QuestionLookupResult)]) -> QuestionRefreshApplyResult
    {
        var changed = false
        for record in records {
            if let model = self.questionCards.first(where: { $0.id == record.id }) {
                ...
                self.questionCards.append(BotQuestionCardModel(record: record))
                changed = true
            }
        }
        var complete = true
    ...
    func resolveQuestionEvent(_ event: BotQuestionResolvedEvent) {
    ...
    func submitQuestion(_ model: BotQuestionCardModel) async {
    ...
    func skipQuestion(_ model: BotQuestionCardModel) async {
