...
public enum ChatSessionSidebarModel {
    ...
    public struct Node: Identifiable, Equatable, Sendable {
        public let session: BotChatSessionEntry
    ...
    static func tree(from sessions: [BotChatSessionEntry]) -> [Node] {
        ...
        var entriesByKey: [String: BotChatSessionEntry] = [:]
        ...
        var childrenByParent: [String: [BotChatSessionEntry]] = [:]
        for session in sessions {
        ...
        func build(_ session: BotChatSessionEntry, ancestors: Set<String>) -> Node {
    ...
    private static func node(session: BotChatSessionEntry, children: [Node]) -> Node {
    ...
    public static func displayName(for session: BotChatSessionEntry) -> String {
    ...
    public static func workSubtitle(for session: BotChatSessionEntry) -> String? {
        ...
        let shortBranch = branch.map { $0.hasPrefix("bot/") ? String($0.dropFirst("bot/".count)) : $0 }
    ...
    public static func subtitle(
        for session: BotChatSessionEntry,
        workSubtitle: String?,
    ...
    public static func applying(
        ...
        to sessions: [BotChatSessionEntry],
        activeAgentId: String? = nil) -> [BotChatSessionEntry]
    {
        ...
        let candidate = BotChatSessionObserverDigest(digest)
    ...
    public static func reconcilingGlobalObserverDigestOwner(
        in sessions: [BotChatSessionEntry],
        activeAgentId: String?) -> [BotChatSessionEntry]
    {
    ...
    private static func reconcilingGlobalObserverDigestOwner(
        ...
        adoptOwnerless: Bool) -> [BotChatSessionEntry]
    {
    ...
    private static func observerProjection(
        for change: BotChatSessionsChangedEvent,
        activeAgentId: String?) -> (digest: BotChatSessionObserverDigest?, present: Bool)?
    {
    ...
    public static func applying(
        sessionChange change: BotChatSessionsChangedEvent,
        to sessions: [BotChatSessionEntry],
        activeAgentId: String? = nil) -> [BotChatSessionEntry]?
    {
    ...
    private static func visibleObserverDigest(
        for session: BotChatSessionEntry) -> BotChatSessionObserverDigest?
    {
    ...
    private static func activeAgentStatus(
        _ status: BotChatSessionAgentStatus?,
        now: Double) -> BotChatSessionAgentStatus?
    {
        guard let status,
    ...
    private static func unreadFailureReason(for session: BotChatSessionEntry) -> String? {
        let status = session.status?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func isRunning(_ session: BotChatSessionEntry) -> Bool {
        session.hasActiveRun == true ||
            session.status?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "running"
    }

    private static func isNewer(
        _ candidate: BotChatSessionObserverDigest,
        than previous: BotChatSessionObserverDigest) -> Bool
    {
    ...
    private static func visibleSessions(
        ...
        query: String) -> [BotChatSessionEntry]
    {
        ...
        return BotChatSessionListOrganizer.filter(
            BotChatSessionListOrganizer.organize(entries),
            search: query)
    }

    private static func placeholder(key: String) -> BotChatSessionEntry {
        BotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
