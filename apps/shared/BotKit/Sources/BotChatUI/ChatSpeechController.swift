...
public struct BotChatSpeechClip: Equatable, Sendable {
...
public typealias BotChatSpeechSynthesis =
    @Sendable (_ text: String) async throws -> BotChatSpeechClip
...
@MainActor
@Observable
public final class BotChatSpeechController {
    ...
    private let synthesize: BotChatSpeechSynthesis
    ...
    public convenience init(synthesize: @escaping BotChatSpeechSynthesis) {
        self.init(
    ...
    init(
        synthesize: @escaping BotChatSpeechSynthesis,
    ...
    private func run(messageID: UUID, text: String, generation: UInt64) async {
        let clip: BotChatSpeechClip? = await {
            do {
