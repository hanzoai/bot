...
enum ChatToolActivity {
    static func items(
        calls: [BotChatMessageContent],
        results: [BotChatMessageContent]) -> [ChatToolActivityItem]
    {
...
struct ChatToolActivityRow: View {
                    ...
                    if self.resolvedDiff != nil {
                        ...
                        if !self.formattedResult.isEmpty {
                            Text(self.formattedResult)
                                .font(BotChatTypography.mono(size: 12, relativeTo: .footnote))
    ...
    private var collapsedRow: some View {
        HStack(alignment: .firstTextBaseline, spacing: 7) {
            ...
            Image(systemName: Self.symbol(forToolName: self.item.name))
                ...
                .foregroundStyle(self.item.isError ? BotChatTheme.danger : Color.secondary)

            Text(self.display.title)
                .font(BotChatTypography.footnoteSemiBold)
                .foregroundStyle(
                    self.item.isError ? BotChatTheme.danger : BotChatTheme.assistantText)
                .lineLimit(1)
            ...
            if let stat = self.resolvedDiff?.stat {
                Text(verbatim: "+\(stat.added)")
                    .font(BotChatTypography.mono(size: 12, relativeTo: .footnote))
                    .foregroundStyle(BotChatTheme.success.opacity(0.9))
                    .lineLimit(1)
                Text(verbatim: "−\(stat.removed)")
                    .font(BotChatTypography.mono(size: 12, relativeTo: .footnote))
                    .foregroundStyle(BotChatTheme.danger.opacity(0.9))
                    .lineLimit(1)
            }
    ...
    private func diffRow(_ line: ChatToolDiffLine) -> some View {
        if line.kind == .file {
            Text(verbatim: String(line.text.unicodeScalars.prefix(2000)))
                .font(BotChatTypography.mono(size: 11, relativeTo: .caption))
                .fontWeight(.semibold)
                .foregroundStyle(.secondary)
            ...
            HStack(spacing: 6) {
                if let lineNo = line.lineNo {
                    Text(verbatim: "\(lineNo)")
                        .font(BotChatTypography.mono(size: 10, relativeTo: .caption2))
                        .foregroundStyle(.secondary.opacity(0.6))
    ...
    private func diffBackground(_ kind: ChatToolDiffLineKind) -> Color {
        switch kind {
        case .add:
            BotChatTheme.success.opacity(0.14)
        case .del:
            BotChatTheme.danger.opacity(0.12)
        case .ctx, .file, .skip:
            .clear
        }
    }
