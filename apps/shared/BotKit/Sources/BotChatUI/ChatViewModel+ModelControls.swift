...
extension BotChatViewModel {
        ...
        self.enqueueSessionSettingsPatch(requestID: requestID, target: target) { [weak self] routeLease in
            ...
            do {
                ...
                let result = try await routeLease.patchSessionSettings(
                    ...
                    patch: BotChatSessionSettingsPatch(verboseLevel: .some(next)))
        ...
        self.enqueueSessionSettingsPatch(requestID: requestID, target: target) { [weak self] routeLease in
            ...
            do {
                ...
                let result = try await routeLease.patchSessionSettings(
                    sessionKey: target.canonicalSessionKey,
                    agentID: target.agentID,
                    patch: BotChatSessionSettingsPatch(fastMode: .some(next)))
    ...
    func applyModelControlPatchResult(
        _ result: BotChatModelPatchResult,
        sessionKey: String,
    ...
    private func recordModelControlPatchSuccess(
        ...
        fastModeOverride: BotChatFastMode?? = nil,
        ...
        verboseLevelOverride: String?? = nil)
    {
        ...
        let recordedFastMode: BotChatFastMode? = if let fastModeOverride {
            fastModeOverride
        } else {
        ...
        self.lastSuccessfulSettingsPatchResultsByTarget[target] = BotChatModelPatchResult(
    ...
    private func updateCurrentSessionFastMode(
        _ mode: BotChatFastMode?,
        effective: BotChatFastMode?,
        sessionKey: String,
        exactMatchOnly: Bool = false)
    {
        let index = exactMatchOnly
