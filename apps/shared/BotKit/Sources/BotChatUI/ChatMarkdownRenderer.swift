...
struct ChatMarkdownRenderer: View {
    ...
    enum Typography: Equatable {
        ...
        func headingFont(level: Int) -> Font {
            ...
            case .hierarchy: BotChatTypography.heading(level: level)
    ...
    struct InlineMathTypography: Equatable {
        static let body = Self(size: BotChatTypography.bodySize, relativeTo: .body)
    ...
    private var linkColor: Color {
        self.context == .user ? self.textColor : BotChatTheme.accent
    }
}
    ...
    var body: some View {
        ForEach(self.images, id: \.id) { item in
            if let img = item.image {
                BotPlatformImageFactory.image(img)
                    .resizable()
                    .scaledToFit()
                    .frame(maxHeight: 260)
