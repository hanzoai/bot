...
public struct ChatModelProviderSection: Identifiable, Sendable, Equatable {
    ...
    public let models: [BotChatModelChoice]
    public let isDefaultProvider: Bool
}

public struct ChatModelPickerSections: Sendable, Equatable {
    public let pinned: [BotChatModelChoice]
    public let recent: [BotChatModelChoice]
    public let providers: [ChatModelProviderSection]
}

@MainActor
public final class ChatModelPickerStore {
    private static let favoritesKey = "bot.chat.modelFavorites"
    private static let recentsKey = "bot.chat.modelRecents"
    ...
    public func recordRecent(_ selectionID: String) {
        guard !selectionID.isEmpty, selectionID != BotChatViewModel.defaultModelSelectionID else { return }
    ...
    private static func effectiveProvider(_ choice: BotChatModelChoice) -> String {
        let metadataProvider = self.normalizedProvider(choice.provider)
        if metadataProvider != "other" {
            return metadataProvider
        }
