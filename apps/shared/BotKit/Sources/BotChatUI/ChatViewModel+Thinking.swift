...
extension BotChatViewModel {
    ...
    func performSelectThinkingLevel(_ level: String) {
        ...
        if acceptedThinkingLevelsByTarget[target] == nil {
            ...
            acceptedSettingsPatchResultsByTarget[target] = BotChatModelPatchResult(
    ...
    private func mergedThinkingPatchSuccess(
        _ patchResult: BotChatModelPatchResult?,
        acceptedLevel: String,
        target: ModelPatchTarget) -> BotChatModelPatchResult
    {
    ...
    func updateCurrentSessionThinkingLevels(
        _ thinkingLevels: [BotChatThinkingLevelOption],
        sessionKey: String,
    ...
    func inactiveSettingsStateKey(for target: ModelPatchTarget) -> String? {
        if target.agentID != nil || target.sessionRoutingContract != nil {
            guard BotChatSessionKey.agentID(from: target.canonicalSessionKey) != nil else { return nil }
        }
    ...
    private func thinkingLevelWithoutGatewayMetadata(
        _ level: String,
        target: ModelPatchTarget,
        session: BotChatSessionEntry?) -> String?
    {
    ...
    private func thinkingPickerIsAvailable(
        ...
        modelChoice: BotChatModelChoice?) -> Bool
    {
        let resolved = self.resolvedThinkingLevelOptions(for: session)
    ...
    private struct ThinkingLevelOptionsResolution {
        let options: [BotChatThinkingLevelOption]
        let isGatewayMetadata: Bool
    }

    private func resolvedThinkingLevelOptions(
        for currentSession: BotChatSessionEntry?) -> ThinkingLevelOptionsResolution
    {
    ...
    private func selectedModelChoice(
        for currentSession: BotChatSessionEntry?) -> BotChatModelChoice?
    {
    ...
    private func modelChoice(modelID: String?, provider: String?) -> BotChatModelChoice? {
    ...
    private static func sessionModelMatchesDefaults(
        ...
        defaults: BotChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [BotChatThinkingLevelOption]?) -> [BotChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return BotChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [BotChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return BotChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    static func withCurrentThinkingOption(
        _ options: [BotChatThinkingLevelOption],
        current: String) -> [BotChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [BotChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [BotChatThinkingLevelOption]) -> [BotChatThinkingLevelOption]
    {
        var result: [BotChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    static func normalizedThinkingLevel(
        _ level: String?,
        options: [BotChatThinkingLevelOption],
