...
public protocol BotChatTranscriptCache: Sendable {
    func loadSessions() async -> [BotChatSessionEntry]
    func loadTranscript(sessionKey: String) async -> [BotChatMessage]
    func loadTranscript(sessionKey: String, agentID: String?) async -> [BotChatMessage]
    func storeSessions(_ sessions: [BotChatSessionEntry]) async
...
extension BotChatTranscriptCache {
...
protocol BotChatCanonicalTranscriptMerging: BotChatTranscriptCache {
...
public struct BotChatOutboxScope: Hashable, Sendable {
...
public struct BotChatOutboxBranchState: Equatable, Sendable {
...
public struct BotChatOutboxRetryExpectation: Equatable, Sendable {
...
public struct BotChatOutboxAttachment: Codable, Hashable, Sendable {
...
public struct BotChatOutboxCommand: Hashable, Sendable, Identifiable {
    ...
    public let attachments: [BotChatOutboxAttachment]
...
public enum BotChatOutboxUpdateResult: Equatable, Sendable {
    case updated
    case confirmed
    case missing
    case superseded
...
public enum BotChatOutboxChange: Equatable, Sendable {
    case canceled(gatewayID: String, id: String)
    case confirmed(gatewayID: String, id: String)
    case invalidated(gatewayID: String, scope: BotChatOutboxScope)
...
public protocol BotChatCommandOutbox: Sendable {
    ...
    func demoteBranchSwitchToReconcile(_ scope: BotChatOutboxScope) async -> Bool
    ...
    func changes() -> AsyncStream<BotChatOutboxChange>
}

extension BotChatCommandOutbox {
    public func markCommandQueued(
        id _: String,
        attemptVersion _: Int,
        retryCount _: Int,
        lastError _: String?) async -> BotChatOutboxUpdateResult
    {
        .unavailable
    }
    ...
    public func markCommandAwaitingConfirmation(
        id _: String,
        attemptVersion _: Int) async -> BotChatOutboxUpdateResult
    {
        .unavailable
    }
    ...
    public func branchState(for _: BotChatOutboxScope) async -> BotChatOutboxBranchState? {
        nil
    }

    public func beginBranchSwitch(_: BotChatOutboxScope) async -> Bool {
        false
    }

    public func cancelBranchSwitch(_: BotChatOutboxScope) async -> Bool {
        false
    }

    public func demoteBranchSwitchToReconcile(_: BotChatOutboxScope) async -> Bool {
        false
    }

    public func reconcileBranchScope(
        _: BotChatOutboxScope,
        previousState _: BotChatOutboxBranchState,
        activeLeafEntryID _: String?,
        branchLeafEntryIDs _: Set<String>,
        activeTranscriptEntryIDs _: Set<String>,
        lastError _: String) async -> [BotChatOutboxCommand]?
    {
        nil
    }
    ...
    public func updateLastActiveLeafEntryID(
        _: String,
        expectedEpoch _: Int,
        for _: BotChatOutboxScope) async -> Bool
    {
        false
    }
    ...
    public func markCommandRetriedIfPresent(
        id _: String,
        expectation _: BotChatOutboxRetryExpectation,
        agentID _: String?,
        deliverySessionKey _: String,
        routingContract _: String,
        replacementID _: String? = nil) async -> BotChatOutboxUpdateResult
    {
        .unavailable
    }
}

public struct BotChatSessionRoutingIdentity: Equatable, Sendable {
    ...
    public init?(contract: String?) {
        guard let components = BotChatSessionRoutingContract.parse(contract) else { return nil }
    ...
    public init?(scope: String?, mainSessionKey: String?, defaultAgentID: String?) {
        guard let contract = BotChatSessionRoutingContract.make(
            scope: scope,
