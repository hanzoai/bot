...
public struct BotChatVoiceNoteControl {
    public var recorder: BotVoiceNoteRecorder
    public var isTalkActive: Bool

    public init(recorder: BotVoiceNoteRecorder, isTalkActive: Bool) {
        self.recorder = recorder
        self.isTalkActive = isTalkActive
    }
}

struct BotVoiceNoteButton: View {
    let control: BotChatVoiceNoteControl
    let compact: Bool
...
struct BotVoiceNoteRecordingRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Text(botVoiceNoteDurationLabel(self.recorder.elapsedSeconds))
                .font(BotChatTypography.mono(size: 13, relativeTo: .footnote))
                .foregroundStyle(.secondary)
