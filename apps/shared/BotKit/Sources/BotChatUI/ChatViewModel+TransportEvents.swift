...
private let transportEventsLogger = Logger(subsystem: "ai.bot", category: "BotChatUI")

@MainActor
private final class PendingRunOwnerReference {
    weak var value: BotChatViewModel?

    init(_ value: BotChatViewModel) {
        self.value = value
    }
}
...
extension BotChatViewModel {
    ...
    func handleTransportEvent(_ evt: BotChatTransportEvent) {
        switch evt {
        case let .health(ok):
    ...
    private func applySessionChangeProjection(
        _ change: BotChatSessionsChangedEvent,
    ...
    private func handleSessionMessageEvent(_ payload: BotSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: BotChatEventPayload) {
           ...
           let runId = Self.normalizedRunID(chat.runId)
        {
            ...
            self.adoptRun(
                runId: runId,
                bufferedText: BotChatEventText.assistantText(from: chat) ?? "")
            return
        }
        ...
        case "final", "aborted", "error":
            ...
            let hapticEvent: BotChatHaptics.Event? = switch chat.state {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: BotChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = BotChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: BotChatMessage.self)
        }
    ...
    static func isAssistantMessage(_ message: BotChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: BotChatMessage) -> BotChatMessage {
    ...
    private func handleAgentLifecycleEvent(_ evt: BotAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: BotAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    func finishPendingRunAfterTerminalOkSendAck(_ response: BotChatSendResponse) {
    ...
    func finishPendingRunIfTerminalSendAck(_ response: BotChatSendResponse) -> Bool {
    ...
    private func refreshIfPending(
        ...
        terminalState: BotChatRunTerminalState? = nil,
    ...
    private func finishPendingRun(runId: String, terminalState: BotChatRunTerminalState) {
        let hapticEvent: BotChatHaptics.Event
    ...
    static func hasUnansweredLatestUser(in messages: [BotChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    static func latestUserTurn(in messages: [BotChatMessage]) -> LatestUserTurn? {
    ...
    static func userTurn(
        at userIndex: [BotChatMessage].Index,
        in messages: [BotChatMessage]) -> LatestUserTurn?
    {
        ...
        -> Bool
    {
        ...
        var occurrence = 0
        var latestMatchingUserIndex: [BotChatMessage].Index?
    ...
    private static func hasAssistantMessage(
        after userIndex: [BotChatMessage].Index,
        in messages: [BotChatMessage]) -> Bool
    {
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [BotChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
    ...
    private static func assistantHapticEvent(
        for message: BotChatMessage) -> BotChatHaptics.Event?
    {
    ...
    func assistantHapticEventAfterLatestUser() -> BotChatHaptics.Event? {
    ...
    private func assistantHapticEvent(after timestamp: Double) -> BotChatHaptics.Event? {
    ...
    func refreshHistoriesAfterOutboxFlush(
        ...
        routeLease: BotChatTransportRouteLease) async
    {
    ...
    private nonisolated static func runPendingRunOwner(
        ...
        armID: UInt64,
        transport: any BotChatTransport) async
    {
    ...
    private nonisolated static func observePendingRunCompletion(
        ...
        transport: any BotChatTransport) async
    {
        var terminalState: BotChatRunTerminalState?
            ...
            armID: armID)
        {
            ...
            let effectiveObservation = terminalState.map(BotChatRunObservation.terminal) ?? observation
    ...
    private static func processPendingRunObservation(
        ...
        userMessageTimestamp: Double?,
        armID: UInt64,
        observation: BotChatRunObservation,
    ...
    func clearPendingRun(
        _ runId: String,
        hapticEvent: BotChatHaptics.Event? = nil)
    {
