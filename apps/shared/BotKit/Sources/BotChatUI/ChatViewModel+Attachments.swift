...
extension BotChatViewModel {
    ...
    func restoreEditorAttachments(_ editorAttachments: [BotChatEditorAttachment]?) {
    ...
    public func addVoiceNoteAttachment(fileURL: URL, durationSeconds: Double) async {
        ...
        let normalizedDuration = durationSeconds.isFinite
            ? min(max(0, durationSeconds), BotVoiceNoteRecorder.maximumDurationSeconds)
            : 0
        attachments.append(
    ...
    static func previewImage(data: Data) -> BotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
