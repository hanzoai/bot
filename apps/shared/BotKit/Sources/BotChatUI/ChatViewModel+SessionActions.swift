...
extension BotChatViewModel {
    ...
    func performStartNewSession(
        ...
        routeLease: BotChatNewSessionRouteLease? = nil) async -> Bool
    {
        ...
        let currentAgentID = (
            BotChatSessionKey.agentID(from: self.sessionKey) ??
                self.activeAgentId ??
                BotChatSessionKey.agentID(from: self.resolvedMainSessionKey))?
            .lowercased()
    ...
    static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "BotChatTransport"
    ...
    func startNewSession(
        ...
        using routeLease: BotChatNewSessionRouteLease) async -> Bool
    {
        await self.performStartNewSession(
            agentID: agentID,
            worktree: worktree,
    ...
    func newSessionRouteLease() async throws -> BotChatNewSessionRouteLease {
    ...
    public func fetchSessionGroups() async throws -> [BotChatSessionGroup] {
        let routeLease = try await self.sessionGroupsRouteLease()
        return try await self.fetchSessionGroups(using: routeLease)
    }

    func sessionGroupsRouteLease() async throws -> BotChatSessionGroupsRouteLease {
    ...
    func fetchSessionGroups(
        using routeLease: BotChatSessionGroupsRouteLease) async throws -> [BotChatSessionGroup]
    {
    ...
    func performSessionBatch(
        sessions selectedSessions: [BotChatSessionEntry],
        action: ChatSessionBatchAction) async -> ChatSessionBatchResult
    {
        ...
        case .pin, .unpin:
            ...
            self.sessions = BotChatSessionListOrganizer.organize(self.sessions)
        case .archive, .delete:
    ...
    public func fetchSessionList(search: String?, archived: Bool) async -> [BotChatSessionEntry] {
        ...
        do {
            let res = try await self.transport.listSessions(
                ...
                search: query,
                archived: archived)
            return BotChatSessionListOrganizer.organize(res.sessions)
        } catch {
            ...
            return BotChatSessionListOrganizer.filter(self.sessions, search: query)
        }
    }
    ...
    public func rewindToMessage(_ message: BotChatMessage) async {
    ...
    func refreshSessionBranches(
        ...
        preBootstrapBranchState: BotChatOutboxBranchState?) async -> Bool
    {
        let refreshGeneration = self.beginSessionBranchesRefresh()
        return await self.performSessionBranchesRefresh(
            for: session,
    ...
    public func forkAtMessage(_ message: BotChatMessage) async {
        guard let entryID = Self.sessionMutationEntryID(for: message) else { return }
        guard self.canPerformMessageSessionAction else { return }
        guard self.canCreateSessionForImmediateSwitch() else { return }
        let initiatingSession = self.currentSessionSnapshot()
    ...
    private static func sessionMutationEntryID(for message: BotChatMessage) -> String? {
    ...
    func markCurrentSessionReadAfterActivation(
        ...
        fallbackEntry: BotChatSessionEntry?) async
    {
