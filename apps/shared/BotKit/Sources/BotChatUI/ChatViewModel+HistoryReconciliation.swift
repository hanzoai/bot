...
extension BotChatViewModel {
    static func decodeMessages(_ raw: [AnyCodable]) -> [BotChatMessage] {
    ...
    static func stripInboundMetadata(from message: BotChatMessage) -> BotChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> BotChatMessageContent in
    ...
    static func messageContentFingerprint(for message: BotChatMessage) -> String {
    ...
    static func finalMessageContentFingerprint(for message: BotChatMessage) -> String {
        message.content.map { item in
            let type = (item.type ?? "text").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            let text = (item.text ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func messageIdentityKey(for message: BotChatMessage) -> String? {
    ...
    static func userRefreshIdentityKey(for message: BotChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func finalMessageReconciliationKey(for message: BotChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func adoptingCanonicalMessage(
        _ incoming: BotChatMessage,
        over existing: BotChatMessage) -> BotChatMessage
    {
        BotChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func preservingLocalAudioDurations(
        in incoming: [BotChatMessageContent],
        from existing: [BotChatMessageContent]) -> [BotChatMessageContent]
    {
    ...
    static func messageRange(
        ...
        in messages: [BotChatMessage]) -> Range<[BotChatMessage].Index>
    {
    ...
    func adoptingProvisionalFinalMessageIDs(
        in incoming: [BotChatMessage]) -> [BotChatMessage]
    {
    ...
    func clearProvisionalFinalMarkersAdoptedByHistory(_ incoming: [BotChatMessage]) {
    ...
    func adoptCorrelatedUserMessage(incoming: BotChatMessage) -> Bool {
    ...
    func adoptProvisionalFinalMessage(incoming: BotChatMessage) -> Bool {
        ...
        incoming: [BotChatMessage]) -> [BotChatMessage]
    {
        ...
        var previousMessagesByKey: [String: [BotChatMessage]] = [:]
    ...
    static func reconcileRunRefreshMessages(
        previous: [BotChatMessage],
        incoming: [BotChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [BotChatMessage]
    {
    ...
    static func dedupeMessages(_ messages: [BotChatMessage]) -> [BotChatMessage] {
    ...
    static func dedupeKey(for message: BotChatMessage) -> String? {
...
extension BotChatViewModel {
    ...
    func applyHistoryPayload(
        _ payload: BotChatHistoryPayload,
    ...
    private func provisionalFinalMessagesMissing(
        from incoming: [BotChatMessage]) -> [BotChatMessage]
    {
