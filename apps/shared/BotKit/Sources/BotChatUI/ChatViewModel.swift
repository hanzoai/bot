...
let chatUILogger = Logger(subsystem: "ai.bot", category: "BotChatUI")
...
public final class BotChatViewModel {
    ...
    public internal(set) var messages: [BotChatMessage] = []
    ...
    public internal(set) var replyTarget: BotChatReplyTarget?
    @ObservationIgnored
    ...
    /// Setter is module-internal for the thinking-level extension only.
    public internal(set) var thinkingLevelOptions: [BotChatThinkingLevelOption]
    /// Setter is module-internal for the thinking-level extension only.
    ...
    public private(set) var modelChoices: [BotChatModelChoice] = []
    ...
    public internal(set) var slashCommands: [BotChatCommandChoice] = []
    ...
    public internal(set) var sessionBranches: [BotChatSessionBranch] = []
    ...
    public internal(set) var questionCards: [BotQuestionCardModel] = []
    ...
    public internal(set) var planSteps: [BotChatPlanStep] = []
    ...
    public internal(set) var sessions: [BotChatSessionEntry] = [] {
        didSet { syncContextUsageFraction() }
    }

    public internal(set) var swarmSessions: [BotChatSessionEntry] = []
    var activeSwarmGroups: [BotChatSwarmGroup] = []
    var swarmActivityState = BotChatSwarmActivityState()
    @ObservationIgnored
    ...
    let transport: any BotChatTransport
    let haptics: BotChatHaptics
    let transcriptCache: (any BotChatTranscriptCache)?
    let outbox: (any BotChatCommandOutbox)?
    @ObservationIgnored
    ...
    public internal(set) var outboxStatesByMessageID: [UUID: BotChatOutboxMessageState] = [:]
    @ObservationIgnored
    ...
    @ObservationIgnored
    var reconciledOutboxBranchScopes: Set<BotChatOutboxScope> = []
    @ObservationIgnored
    var reconcilingOutboxBranchScopes: Set<BotChatOutboxScope> = []
    @ObservationIgnored
    var outboxBranchReconcileRetryAttempts: [BotChatOutboxScope: Int] = [:]
    @ObservationIgnored
    var outboxBranchReconcileRetryTasks: [BotChatOutboxScope: Task<Void, Never>] = [:]
    @ObservationIgnored
    ...
    var bootstrapOutboxBranchStateCapture: (
        ...
        task: Task<BotChatOutboxBranchState?, Never>)?
    @ObservationIgnored
    ...
    var sessionDefaults: BotChatSessionsDefaults? {
        didSet { syncContextUsageFraction() }
    }
    ...
    let onToolActivity: BotChatToolActivityHandler?
    ...
    var lastSuccessfulSettingsPatchResultsByTarget: [ModelPatchTarget: BotChatModelPatchResult] = [:]
    ...
    var acceptedSettingsPatchResultsByTarget: [ModelPatchTarget: BotChatModelPatchResult] = [:]
    ...
    struct FastModeState {
        let override: BotChatFastMode?
        let effective: BotChatFastMode?
    }
    ...
    private struct SettingsPatchTail {
        ...
        let routeLeaseTask: Task<BotChatSessionSettingsRouteLease?, Never>
    ...
    struct HistoryRequest {
        ...
        var visibleMessagesByID: [UUID: BotChatMessage]
    ...
    var pendingToolCallsById: [String: BotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        ...
        haptics: BotChatHaptics = BotChatHaptics(),
        ...
        onToolActivity: BotChatToolActivityHandler? = nil,
    ...
    public func isDefaultModel(_ model: BotChatModelChoice) -> Bool {
    ...
    private var usesMutableAgentRouting: Bool {
        BotChatSessionKey.agentID(from: self.sessionKey) == nil
    }
    ...
    func usesMutableContractRouting(sessionKey: String, contract: String?) -> Bool {
        if BotChatSessionKey.agentID(from: sessionKey) == nil {
            return true
        }
        ...
        let contractMainKey = BotChatSessionRoutingContract.parse(contract)?.mainKey ?? ""
    ...
    static let baseThinkingLevelOptions: [BotChatThinkingLevelOption] = [
        BotChatThinkingLevelOption(id: "off", label: "off"),
        BotChatThinkingLevelOption(id: "minimal", label: "minimal"),
        BotChatThinkingLevelOption(id: "low", label: "low"),
...
extension BotChatViewModel {
    ...
    func appendMessage(_ message: BotChatMessage) {
    ...
    func currentSessionSnapshot() -> SessionSnapshot {
        SessionSnapshot(
            ...
            deliveryAgentID: BotChatSessionKey.agentID(from: self.sessionKey) ?? self.activeAgentId,
    ...
    func fetchSessions(limit: Int?, sessionSnapshot: SessionSnapshot? = nil) async {
        ...
        while true {
            ...
            let res: BotChatSessionsListResponse
            do {
            ...
            let organized = BotChatSessionListOrganizer.organize(res.sessions)
    ...
    private func performSelectModel(
        ...
        routeLease: BotChatSessionSettingsRouteLease?) async
    {
        do {
            guard let routeLease else { throw BotChatTransportSendError.notDispatched }
            let patchResult = try await routeLease.patchSessionSettings(
                ...
                patch: BotChatSessionSettingsPatch(model: .some(request.modelRef)))
    ...
    func enqueueSessionSettingsPatch(
        ...
        operation: @escaping @MainActor (BotChatSessionSettingsRouteLease?) async -> Void)
    {
    ...
    func generatedNewSessionKey(agentID explicitAgentID: String? = nil) -> String {
        ...
        guard let agentID = explicitAgentID ??
            BotChatSessionKey.agentID(from: sessionKey) ??
            activeAgentId ??
            BotChatSessionKey.agentID(from: resolvedMainSessionKey) ??
            sessions.lazy.compactMap({ BotChatSessionKey.agentID(from: $0.key) }).first
        else {
            return baseKey
        }
    ...
    private func applySuccessfulModelSelection(
        ...
        syncSelection: Bool,
        patchResult: BotChatModelPatchResult? = nil)
    {
        self.recordSuccessfulModelPatch(
    ...
    private func recordSuccessfulModelPatch(
        selectionID: String,
        patchResult: BotChatModelPatchResult?,
        target: ModelPatchTarget)
    {
        ...
        if let patchResult {
            ...
            self.acceptedSettingsPatchResultsByTarget[target] = BotChatModelPatchResult(
