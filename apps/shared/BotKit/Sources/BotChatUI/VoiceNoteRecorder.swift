...
public struct BotVoiceNoteRecording: Equatable, Sendable {
...
@MainActor
@Observable
public final class BotVoiceNoteRecorder {
    public enum State: Equatable {
        case idle
        case requestingPermission
        case recording(startedAt: Date, fileURL: URL)
        case finished(recording: BotVoiceNoteRecording)
        case staging(recording: BotVoiceNoteRecording)
    ...
    public convenience init() {
        self.init(capture: BotVoiceNoteAudioCapture())
    }

    /// Creates a recorder with an injectable audio capture seam.
    public init(
        capture: any VoiceNoteAudioCapture,
        durationLimit: TimeInterval = BotVoiceNoteRecorder.maximumDurationSeconds,
    ...
    public var completedRecording: BotVoiceNoteRecording? {
    ...
    func claimCompletedRecording() -> BotVoiceNoteRecording? {
        guard case let .finished(recording) = self.state else { return nil }
    ...
    func completeStaging(_ recording: BotVoiceNoteRecording) {
    ...
    @discardableResult
    public func finish() -> BotVoiceNoteRecording? {
        guard case let .recording(_, fileURL) = self.state else { return nil }

        self.timerTask?.cancel()
        self.timerTask = nil
        let duration = max(0, self.capture.stop())
        let recording = BotVoiceNoteRecording(fileURL: fileURL, durationSeconds: duration)
...
@MainActor
public final class BotVoiceNoteAudioCapture: NSObject, VoiceNoteAudioCapture, AVAudioRecorderDelegate {
    ...
    public func start(url: URL) throws {
        do {
            ...
            guard recorder.record() else {
                throw NSError(
                    domain: "BotVoiceNoteAudioCapture",
                    code: 1,
...
func botVoiceNoteDurationLabel(_ durationSeconds: TimeInterval) -> String {
