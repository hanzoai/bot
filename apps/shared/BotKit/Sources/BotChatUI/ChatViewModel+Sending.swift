...
struct SlashFilterCache {
    let query: String
    let filter: BotChatCommandFilter
    let result: [BotChatCommandChoice]
}

private let chatSendingLogger = Logger(subsystem: "ai.bot", category: "BotChatUI")
...
extension BotChatViewModel {
    ...
    public func slashCommandMatches(
        query: String,
        filter: BotChatCommandFilter) -> [BotChatCommandChoice]
    {
    ...
    public func applySlashCommandSelection(_ command: BotChatCommandChoice) {
    ...
    private static func isKnownSlashCommandText(
        _ text: String,
        commands: [BotChatCommandChoice]) -> Bool
    {
    ...
    private static func commands(
        _ commands: [BotChatCommandChoice],
        ...
        filter: BotChatCommandFilter) -> [BotChatCommandChoice]
    {
        ...
        let effectiveFilter: BotChatCommandFilter =
            self.queryTargetsSkills(trimmed) && filter == .all ? .skills : filter
        return commands.enumerated()
            .compactMap { index, command -> (Int, Int, BotChatCommandChoice)? in
    ...
    private static func command(
        _ command: BotChatCommandChoice,
        isIncludedIn filter: BotChatCommandFilter) -> Bool
    {
        switch filter {
        case .all:
            true
    ...
    private struct SendDraft {
        ...
        let replyTarget: BotChatReplyTarget?
    ...
    private struct LiveSendAttempt {
        ...
        let encodedAttachments: [BotChatAttachmentPayload]
        ...
        mustPreserveOutboxOrder: Bool) async -> AttachmentPersistenceDecision
    {
        ...
        guard case let .unavailable(reason) = routeResult,
              reason == BotChatTransportUpgradeMessage.routingContract
        else {
    ...
    private static func userContent(
        messageText: String,
        attachments: [BotPendingAttachment],
        encodedAttachments: [BotChatAttachmentPayload]) -> [BotChatMessageContent]
    {
        var content: [BotChatMessageContent] = [
    ...
    private func handleLiveSendResponse(
        _ response: BotChatSendResponse,
    ...
    private func handleLiveSendFailure(_ error: Error, attempt: LiveSendAttempt) async {
        ...
        if attempt.encodedAttachments.isEmpty, !(error is GatewayResponseError) {
            ...
            let deliveryIsAmbiguous = !(error is BotChatTransportSendError)
