...
public struct BotChatDisplayOptions: OptionSet, Sendable {
...
@MainActor
public struct BotChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private let displayOptions: BotChatDisplayOptions
    ...
    private let talkControl: BotChatTalkControl?
    private let dictationControl: BotChatDictationControl?
    private let voiceNoteControl: BotChatVoiceNoteControl?
    ...
    public init(
        ...
        displayOptions: BotChatDisplayOptions? = nil,
        ...
        talkControl: BotChatTalkControl? = nil,
        dictationControl: BotChatDictationControl? = nil,
        voiceNoteControl: BotChatVoiceNoteControl? = nil,
        speech: BotChatSpeechController? = nil)
    {
    ...
    private var swarmProgress: some View {
        ...
        if !groups.isEmpty {
            BotChatSwarmProgressView(groups: groups)
        }
    }
    ...
    private var messageListRows: some View {
        ...
        BotQuestionCards(viewModel: self.viewModel)
    ...
    private func isListenable(_ msg: BotChatMessage) -> Bool {
    ...
    private func speechChipIsPreparing(
        _ speech: BotChatSpeechController,
        messageID: UUID) -> Bool?
    {
        switch speech.phase {
    ...
    private var visibleMessages: [BotChatMessage] {
        let base: [BotChatMessage]
    ...
    private var jumpToLatestButton: some View {
        ...
        .buttonStyle(.plain)
        .foregroundStyle(BotChatTheme.assistantText)
...
extension BotChatView {
    private func mergeToolResults(in messages: [BotChatMessage]) -> [BotChatMessage] {
    ...
    private func isToolResultMessage(_ message: BotChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: BotChatMessage) -> Bool {
    ...
    private func primaryText(in message: BotChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: BotChatMessage) -> Bool {
        message.content.contains(where: \.isInlineAttachment)
    }

    private func toolCalls(in message: BotChatMessage) -> [BotChatMessageContent] {
    ...
    private func inlineToolResults(in message: BotChatMessage) -> [BotChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: BotChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: BotChatMessage) -> String {
        self.primaryText(in: message)
    }

    @ViewBuilder
    private func copyMessageButton(for message: BotChatMessage) -> some View {
    ...
    @ViewBuilder
    private func openFullMessageButton(for message: BotChatMessage) -> some View {
    ...
    @ViewBuilder
    private func rewindMessageButton(for message: BotChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if role == "user",
    ...
    @ViewBuilder
    private func forkMessageButton(for message: BotChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if role == "user",
           message.transcriptMessageID?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
        {
    ...
    @ViewBuilder
    private func replyMessageButton(for message: BotChatMessage) -> some View {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
...
private struct ChatAssistantIntroCard: View {
    let text: String
    let prompts: [BotChatView.StarterPrompt]
    let onPrompt: (BotChatView.StarterPrompt) -> Void
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(self.text)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .fill(BotChatTheme.assistantBubble))
            ...
            ForEach(self.prompts) { prompt in
                Button {
                    ...
                    HStack(spacing: 8) {
                        Text(prompt.title)
                            .font(BotChatTypography.body(size: 15, weight: .semibold, relativeTo: .callout))
                            .multilineTextAlignment(.leading)
                        Spacer(minLength: 8)
                        Image(systemName: "arrow.up.right")
                            .font(BotChatTypography.captionSemiBold)
    ...
    var body: some View {
        ...
        ContentUnavailableView {
            Label(self.title, systemImage: self.systemImage)
                .font(BotChatTypography.headline)
        } description: {
            Text(self.message)
                .font(BotChatTypography.body)
        } actions: {
            if let actionTitle, let action {
                Button(action: action) {
                    Text(actionTitle)
                        .font(BotChatTypography.body(size: 15, weight: .semibold, relativeTo: .subheadline))
                }
