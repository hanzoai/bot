...
struct ChatSessionSidebar: View {
    ...
    @State private var sessionPendingDeletion: BotChatSessionEntry?
    @State private var sessionPendingRename: BotChatSessionEntry?
    ...
    @AppStorage("bot.chat.collapsedSessionGroups") private var collapsedSessionGroups = ""
        ...
        return HStack(spacing: 6) {
            VStack(alignment: .leading, spacing: 2) {
                Text(ChatSessionSidebarModel.displayName(for: session))
                    .font(BotChatTypography.body(size: 13, weight: .medium, relativeTo: .body))
                    .lineLimit(1)
    ...
    private func badges(for node: ChatSessionSidebarModel.Node) -> some View {
        ...
        if node.badges.failedCount > 0 {
            Image(systemName: "exclamationmark.triangle.fill")
                .foregroundStyle(BotChatTheme.warning)
    ...
    @ViewBuilder
    private func contextMenu(for session: BotChatSessionEntry) -> some View {
        Button {
    ...
    private func rowSubtitle(for session: BotChatSessionEntry) -> String? {
        var parts: [String] = []
