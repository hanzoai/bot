...
struct ChatPlanPill: View {
    ...
    private var currentStep: BotChatPlanStep? {
    ...
    private func stepRow(_ step: BotChatPlanStep) -> some View {
    ...
    private static func marker(for status: BotChatPlanStep.Status) -> String {
        switch status {
        case .completed: "✓"
        case .inProgress: "▸"
        case .pending: "▢"
    ...
    private static func markerColor(for status: BotChatPlanStep.Status) -> Color {
        switch status {
        case .completed, .inProgress: BotChatTheme.accent
        case .pending: BotChatTheme.muted
        }
    }

    private static func stepAccessibilityLabel(_ step: BotChatPlanStep) -> String {
        "\(self.accessibilityLabel(for: step.status)), \(step.step)"
    }

    private static func accessibilityLabel(for status: BotChatPlanStep.Status) -> String {
        switch status {
