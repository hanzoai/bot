...
public enum BotMascotMood: String, CaseIterable, Equatable, Sendable {
...
enum BotMascotEffect: Equatable {
    case none
    case sparkles
    case hearts
    case zzz
...
final class BotMascotAnimator {
    ...
    private(set) var mood: BotMascotMood = .idle
    ...
    private var activeGesture: BotMascotGesture?
    private var activeGestureStart: TimeInterval = 0
    private var pendingGesture: BotMascotGesture?
    ...
    private var lastClickReaction: BotMascotGesture?
    ...
    private var requestedAccessory: BotMascotAccessory = .none
    ...
    func setMood(_ mood: BotMascotMood, at time: TimeInterval) {
    ...
    func setAccessory(_ accessory: BotMascotAccessory, at time: TimeInterval) {
    ...
    func handleTap(at time: TimeInterval) {
        ...
        var reactions: [BotMascotGesture] = [.hop, .wave, .wink]
    ...
    func pose(at time: TimeInterval) -> BotMascotPose {
        ...
        let effectiveMood: BotMascotMood = dozing ? .sleepy : self.mood
        ...
        if self.requestedAccessory != .none {
            ...
            pose.accessoryAmount = BotMascotGesture.easeOut(progress)
        }
    ...
    private func advanceSchedules(at time: TimeInterval) {
        ...
        self.blinkStarts.removeAll { time - $0 > BotMascotGesture.blinkDuration }
    ...
    private func randomQuirk() -> BotMascotGesture {
    ...
    private func startGesture(_ gesture: BotMascotGesture, at time: TimeInterval) {
        self.activeGesture = gesture
        self.activeGestureStart = time
    }

    private static func entranceGesture(for mood: BotMascotMood) -> BotMascotGesture? {
        switch mood {
        case .happy: .hop
    ...
    private func basePose(for mood: BotMascotMood, at time: TimeInterval) -> BotMascotPose {
        var pose = BotMascotPose()
        switch mood {
        case .idle:
        ...
        case .working:
            ...
            if phase < 0.05 {
                ...
                pose.rightClawDegrees = -6 - 28 * BotMascotGesture.easeInOut((phase - 0.05) / 0.55)
            } else if phase < 0.72 {
                let strike = ((phase - 0.60) / 0.12).clamped(to: 0...1)
                pose.rightClawDegrees = -34 + 46 * strike * strike
            } else {
                pose.rightClawDegrees = 12 - 18 * BotMascotGesture.easeInOut((phase - 0.72) / 0.28)
            }
            pose.leftClawDegrees = 4 + 2 * sin(2 * .pi * phase)
            let impact = BotMascotGesture.bell(((phase - 0.72) / 0.14).clamped(to: 0...1))
    ...
    private func applyBlinks(_ pose: inout BotMascotPose, at time: TimeInterval) {
        guard pose.happyEyes < 0.6 else { return }
        for start in self.blinkStarts {
            let progress = (time - start) / BotMascotGesture.blinkDuration
            guard progress >= 0, progress <= 1 else { continue }
            let closure = BotMascotGesture.bell(CGFloat(progress))
    ...
    private func applyDizzy(_ pose: inout BotMascotPose, at time: TimeInterval) {
...
enum BotMascotGesture: Equatable {
    case wave
    case hop
    case wink
    case celebrate
    ...
    func apply(to pose: inout BotMascotPose, progress p: CGFloat) {
        switch self {
        case .wave:
        ...
        case .sneeze:
            if p < 0.42 {
                let inhale = BotMascotGesture.easeInOut(p / 0.42)
                ...
                let recover = 1 - BotMascotGesture.easeInOut((p - 0.58) / 0.42)
        ...
        case .sigh:
            let rise = BotMascotGesture.easeInOut((p / 0.3).clamped(to: 0...1))
            let fall = BotMascotGesture.easeInOut(((p - 0.3) / 0.45).clamped(to: 0...1))
