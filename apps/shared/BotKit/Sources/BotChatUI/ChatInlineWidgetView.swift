...
enum BotChatWidgetSurfaceRole: Sendable, Hashable {
    case node
    case operatorSurface
    case legacy
}

public struct BotChatWidgetResource: Sendable, Equatable {
    public let url: URL
    public let tlsFingerprintSHA256: String?
    let surfaceRole: BotChatWidgetSurfaceRole
    let attemptedSurfaceRoles: Set<BotChatWidgetSurfaceRole>
    ...
    init(
        ...
        surfaceRole: BotChatWidgetSurfaceRole,
        attemptedSurfaceRoles: Set<BotChatWidgetSurfaceRole>)
    {
        self.url = url
        self.tlsFingerprintSHA256 = tlsFingerprintSHA256
...
public enum BotChatWidgetURLResolver {
    private static let documentsPath = "/__bot__/canvas/documents"
    ...
    public static func resolveResource(
        target: String,
        replacing failedResource: BotChatWidgetResource?,
    ...
    private static func capabilitySurface(_ rawSurfaceURL: String?) -> URLComponents? {
        ...
        guard segments.count >= 3,
              segments[segments.count - 3] == "__bot__",
    ...
    private static func resolve(
        ...
        role: BotChatWidgetSurfaceRole,
        target: String,
        attemptedRoles: Set<BotChatWidgetSurfaceRole>) -> BotChatWidgetResource?
    {
        guard let url = resolve(surfaceURL: surface.url, target: target) else { return nil }
        let resource = BotChatWidgetResource(
            url: url,
    ...
    private static func resolvePreferred(
        ...
        target: String,
        excluding failedResource: BotChatWidgetResource?,
        blockedRoles: Set<BotChatWidgetSurfaceRole>,
        attemptedRoles: Set<BotChatWidgetSurfaceRole>) -> BotChatWidgetResource?
    {
        [
            (role: BotChatWidgetSurfaceRole.node, surface: surfaces.node),
            (role: BotChatWidgetSurfaceRole.operatorSurface, surface: surfaces.operatorSurface),
        ]
            .lazy
    ...
    private static func isReplacement(
        _ candidate: BotChatWidgetResource,
        for failedResource: BotChatWidgetResource?) -> Bool
    {
...
struct ChatInlineWidgetView: View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            if let title = self.preview.title?.trimmingCharacters(in: .whitespacesAndNewlines), !title.isEmpty {
                Text(title)
                    ...
                    .fontWeight(.semibold)
                    .foregroundStyle(BotChatTheme.muted)
            }
    ...
    #if canImport(WebKit) && (os(iOS) || os(macOS))
    private func renderedWidget(resource: BotChatWidgetResource) -> some View {
        ChatInlineWidgetWebView(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .stroke(BotChatTheme.muted.opacity(0.24), lineWidth: 1)
            }
    ...
    private func handleLoadFailure(resource: BotChatWidgetResource) {
...
extension BotChatWidgetResource {
...
private final class ChatInlineWidgetNavigationDelegate: NSObject, WKNavigationDelegate {
    var resource: BotChatWidgetResource {
        didSet {
    ...
    init(
        resource: BotChatWidgetResource,
