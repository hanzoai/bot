...
struct ChatAgentAvatar: View {
    ...
    var body: some View {
        Text(self.displayText)
            .font(BotChatTypography.avatar(size: self.fontSize))
            ...
            .background(
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                (self.tint ?? BotChatTheme.accent).opacity(0.95),
            ...
            .shadow(color: (self.tint ?? BotChatTheme.accent).opacity(0.18), radius: 8, y: 4)
...
struct ChatMessageBubble: View {
    ...
    let inlineWidgetResourceResolver: @MainActor @Sendable (
        String,
        BotChatWidgetResource?) async -> BotChatWidgetResource?
    let imageArtifactResolverReady: Bool
    let loadImageArtifact: @MainActor @Sendable (String) async throws -> BotChatLoadedImage?
...
private struct ChatMessageBody: View {
    @Environment(\.botAssistantBubblesInCleanChrome) private var assistantBubblesInClean
    let message: BotChatMessage
    let isUser: Bool
    let style: BotChatView.Style
    let markdownVariant: ChatMarkdownVariant
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? BotChatTheme.userText : BotChatTheme.assistantText
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if self.omittedImageAttachmentCount > 0 {
                Text(String(
                    ...
                    .font(BotChatTypography.footnote)
    ...
    private func usageLine(_ presentation: ChatMessageUsagePresentation) -> some View {
        Text(presentation.text)
            .font(BotChatTypography.caption2)
            .monospacedDigit()
    ...
    private var toolActivityItems: [ChatToolActivityItem] {
        ...
        // (BotChatView.mergeToolResults); this branch is the orphan
    ...
    private var inlineAttachments: [BotChatMessageContent] {
        self.message.content.filter(\.isInlineAttachment)
    }

    private var visibleInlineAttachments: [BotChatMessageContent] {
        var imageCount = 0
    ...
    private var inlineWidgets: [BotChatCanvasPreview] {
    ...
    private var toolCalls: [BotChatMessageContent] {
    ...
    private var inlineToolResults: [BotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? BotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return BotChatTheme.onboardingAssistantBubble
        }
        return BotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return BotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: BotChatMessageContent
    let isUser: Bool
    let resolverReady: Bool
    let loadImage: @MainActor @Sendable (String) async throws -> BotChatLoadedImage?
    ...
    private var fallbackRow: some View {
        HStack(spacing: 8) {
            ...
            Text(self.isAudio ? "Voice note" : self.attachmentLabel)
                .font(BotChatTypography.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? BotChatTheme.userText : BotChatTheme.assistantText)
            if self.isAudio, let durationSeconds = self.att.durationSeconds {
                Text(botVoiceNoteDurationLabel(durationSeconds))
                    .font(BotChatTypography.footnote)
                    .foregroundStyle(
                        self.isUser
                            ? BotChatTheme.userText.opacity(0.72)
                            : BotChatTheme.assistantText.opacity(0.72))
            }
            Spacer()
        }
        .padding(10)
...
extension BotChatMessageContent {
...
struct ChatOutboxStatusLabel: View {
    let state: BotChatOutboxMessageState
    ...
    var body: some View {
        ...
        .foregroundStyle(self.state.isFailed ? AnyShapeStyle(BotChatTheme.danger) : AnyShapeStyle(.secondary))
    ...
    private var title: LocalizedStringResource {
        ...
        case .confirming:
            "Confirming…"
        case let .failed(reason) where reason == BotChatSQLiteTranscriptCache.outboxUnconfirmedError:
            "Delivery unknown"
        case .failed:
            "Not sent"
        }
...
private struct BotAssistantBubblesInCleanChromeKey: EnvironmentKey {
...
extension EnvironmentValues {
    ...
    public var botAssistantBubblesInCleanChrome: Bool {
        get { self[BotAssistantBubblesInCleanChromeKey.self] }
        set { self[BotAssistantBubblesInCleanChromeKey.self] = newValue }
    }
}
...
private struct AssistantBubbleContainerStyle: ViewModifier {
    ...
    @Environment(\.botAssistantBubblesInCleanChrome) private var bubblesInClean
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [BotChatPendingToolCall]
