...
private let outboxLogger = Logger(subsystem: "ai.bot", category: "BotChatOutbox")

/// Display state for a transcript row backed by a durable outbox command.
public enum BotChatOutboxMessageState: Equatable, Sendable {
    case queued
    case sending
    case confirming
...
extension BotChatViewModel {
    ...
    public func outboxState(for messageID: UUID) -> BotChatOutboxMessageState? {
        self.outboxStatesByMessageID[messageID]
    }

    static func activeBranchLeafEntryID(in branches: [BotChatSessionBranch]) -> String? {
    ...
    func outboxBranchScope(for session: SessionSnapshot) -> BotChatOutboxScope? {
        let agentID = self.outboxAgentID(for: session)
        guard !self.outboxRequiresAgentID(for: session) || agentID != nil else { return nil }
        return BotChatOutboxScope(sessionKey: session.key, agentID: agentID)
    }

    private static func outboxBranchScope(for command: BotChatOutboxCommand) -> BotChatOutboxScope {
        BotChatOutboxScope(sessionKey: command.sessionKey, agentID: command.agentID)
    }
    ...
    func captureOutboxBranchState(for session: SessionSnapshot) async -> BotChatOutboxBranchState? {
    ...
    func observeOutboxTranscriptTip(_ message: BotChatMessage, session: SessionSnapshot) {
    ...
    func enqueueOutboxCommand(
        ...
        draftAttachments: [BotPendingAttachment] = [],
        session: SessionSnapshot) async -> Bool
    {
        ...
        let thinking = self.effectiveThinkingLevelForSend(
            ...
            agentID: agentID,
            sessionRoutingContract: routingContract)
        let command = BotChatOutboxCommand(
            id: UUID().uuidString,
            sessionKey: session.key,
            deliverySessionKey: deliverySessionKey,
            routingContract: routingContract,
        ...
        deliveryIsAmbiguous: Bool) async -> Bool
    {
        ...
        let command = BotChatOutboxCommand(
            ...
            lastError: deliveryIsAmbiguous
                ? BotChatSQLiteTranscriptCache.outboxUnconfirmedError
                : nil)
        await self.waitForBootstrapOutboxBranchCapture(for: session)
    ...
    func confirmOutboxCommands(in messages: [BotChatMessage]) {
    ...
    func confirmOutboxCommandsNow(in messages: [BotChatMessage]) async {
        self.observeCanonicalOutboxMessageKeys(in: messages)
        guard let outbox else { return }
    ...
    private func persistCanonicalOutboxEvidence(
        _ message: BotChatMessage,
        for command: BotChatOutboxCommand) async
    {
        guard let transcriptCache = transcriptCache as? any BotChatCanonicalTranscriptMerging else { return }
    ...
    private func observeCanonicalOutboxMessageKeys(in messages: [BotChatMessage]) {
    ...
    private func presentOutboxCommands(_ commands: [BotChatOutboxCommand]) {
    ...
    private static func outboxUserMessage(for command: BotChatOutboxCommand) -> BotChatMessage {
        var content = [
    ...
    private func mapOutboxCommand(_ command: BotChatOutboxCommand, to messageID: UUID) {
    ...
    private func deliverOutboxCommand(
        _ command: BotChatOutboxCommand,
        outbox: any BotChatCommandOutbox,
        routeLease: BotChatTransportRouteLease) async -> OutboxFlushDisposition
    {
        do {
            ...
            if error.detailsReason == BotChatSessionRoutingContract.changedErrorReason {
    ...
    private func finishAcceptedOutboxCommand(
        _ command: BotChatOutboxCommand,
        outbox: any BotChatCommandOutbox) async -> OutboxFlushDisposition
    {
    ...
    private static func needsOutboxDeliveryReconciliation(_ command: BotChatOutboxCommand) -> Bool {
        command.status == .awaitingConfirmation ||
            (command.status == .failed &&
                command.lastError == BotChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    private static func deliveryTarget(for command: BotChatOutboxCommand) -> OutboxDeliveryTarget {
    ...
    private func parkOutboxCommandWithUnconfirmedDelivery(
        _ command: BotChatOutboxCommand,
        outbox: any BotChatCommandOutbox) async -> BotChatOutboxUpdateResult
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            attemptVersion: command.attemptVersion,
            retryCount: command.retryCount,
            lastError: BotChatSQLiteTranscriptCache.outboxUnconfirmedError)
        switch update {
        case .updated:
            self.setOutboxState(
                .failed(reason: BotChatSQLiteTranscriptCache.outboxUnconfirmedError),
    ...
    private func parkOutboxCommandForChangedTarget(
        _ command: BotChatOutboxCommand,
        outbox: any BotChatCommandOutbox) async -> Bool
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            attemptVersion: command.attemptVersion,
            retryCount: command.retryCount,
            lastError: BotChatSQLiteTranscriptCache.outboxChangedTargetError)
        guard update != .unavailable else {
            self.applyTransportHealth(false)
            return false
        }
    ...
    private func recordOutboxRejection(
        of command: BotChatOutboxCommand,
        outbox: any BotChatCommandOutbox,
    ...
    private func outboxRoutingContract(for session: SessionSnapshot) -> String? {
        if !self.transport.outboxRequiresSessionRoutingContract {
            return BotChatOutboxCommand.legacyUnboundRoutingContract
        }
        ...
        agentID: String?) -> String?
    {
        ...
        if BotChatSessionKey.agentID(from: raw) != nil { return raw }
    ...
    private func setOutboxState(_ state: BotChatOutboxMessageState, forCommandID commandID: String) {
    ...
    func handleOutboxChange(_ change: BotChatOutboxChange) {
    ...
    private static func outboxDisplayState(for command: BotChatOutboxCommand)
        -> BotChatOutboxMessageState
    {
        ...
        case .awaitingConfirmation:
            .confirming
        case .failed:
            .failed(reason: BotChatSQLiteTranscriptCache.outboxDisplayError(command.lastError))
        }
    }
