                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            if !model.partialTranscript.isEmpty {
                                Text(model.partialTranscript)
                                    .font(BotChatTypography.caption.italic())
    ...
    private func header(model: ChatTalkTranscriptStripModel, showsDisclosure: Bool) -> some View {
        HStack(spacing: 7) {
            ...
            if let collapsedText = model.collapsedText, !self.isExpanded {
                Text(collapsedText)
                    .font(BotChatTypography.caption)
                    .foregroundStyle(model.partialTranscript.isEmpty ? .secondary : BotChatTheme.accent)
...
extension View {
    func chatTalkInputDeviceMenu(_ control: BotChatTalkControl) -> some View {
