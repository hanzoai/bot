...
private let cacheLogger = Logger(subsystem: "ai.bot", category: "BotChatTranscriptCache")

final class OutboxChangeHub: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<BotChatOutboxChange>.Continuation] = [:]

    func stream() -> AsyncStream<BotChatOutboxChange> {
        let id = UUID()
        let pair = AsyncStream<BotChatOutboxChange>.makeStream()
        self.lock.lock()
    ...
    func yield(_ change: BotChatOutboxChange) {
        self.lock.lock()
...
public actor BotChatSQLiteTranscriptCache: BotChatTranscriptCache,
    BotChatCanonicalTranscriptMerging,
    BotChatCommandOutbox
{
    ...
    private let databases: BotClientDatabases
    ...
    init(databases: BotClientDatabases, gatewayID: String) {
    ...
    public func loadSessions() async -> [BotChatSessionEntry] {
            ...
            return try await self.databases.cacheQueue.write { db in
                ...
                do {
                    return try rows.map { row in
                        ...
                        return try JSONDecoder().decode(BotChatSessionEntry.self, from: Data(payload.utf8))
                    }
                } catch {
                    cacheLogger.error(
    ...
    public func loadTranscript(sessionKey: String) async -> [BotChatMessage] {
        await self.loadTranscript(sessionKey: sessionKey, agentID: nil)
    }

    public func loadTranscript(sessionKey: String, agentID: String?) async -> [BotChatMessage] {
        guard !self.isRetired else { return [] }
        ...
        do {
            return try await self.databases.cacheQueue.write { db in
                ...
                do {
                    return try rows.map { row in
                        let payload: String = row["payload_json"]
                        return try JSONDecoder().decode(BotChatMessage.self, from: Data(payload.utf8))
                    }
                } catch {
                    cacheLogger.error(
    ...
    public func storeSessions(_ sessions: [BotChatSessionEntry]) async {
        guard !self.isRetired else { return }
        ...
        do {
            try await self.databases.cacheQueue.write { db in
                let rows = try Row.fetchAll(
                    ...
                    """,
                    arguments: [gatewayID, sessionKey, normalizedAgentID])
                var cached = rows.compactMap { row -> BotChatMessage? in
                    let payload: String = row["payload_json"]
                    return try? JSONDecoder().decode(BotChatMessage.self, from: Data(payload.utf8))
                }
    ...
    public func loadSessionRoutingIdentity() async -> BotChatSessionRoutingIdentity? {
        guard !self.isRetired else { return nil }
        return self.databases.loadSessionRoutingIdentity(gatewayID: self.gatewayID)
    }

    public func storeSessionRoutingIdentity(_ identity: BotChatSessionRoutingIdentity) async {
        guard !self.isRetired else { return }
        let gatewayID = self.gatewayID
        do {
...
extension BotChatSQLiteTranscriptCache {
    ...
    private nonisolated static func replaceTranscript(
        ...
        sessionKey: String,
        agentID: String,
        messages: [BotChatMessage]) throws
    {
...
extension BotChatSQLiteTranscriptCache {
    ...
    public nonisolated func changes() -> AsyncStream<BotChatOutboxChange> {
        self.storeChangeHub.stream()
    }

    public func enqueueCommand(_ command: BotChatOutboxCommand) async -> Bool {
    ...
    public func loadCommands() async -> [BotChatOutboxCommand] {
        await self.loadCommandsIfAvailable() ?? []
    }

    public func loadCommandsIfAvailable() async -> [BotChatOutboxCommand]? {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await self.databases.stateQueue.write { db in
    ...
    public func claimNextCommand() async -> BotChatOutboxCommand? {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> (
                BotChatOutboxCommand?,
                [BotChatOutboxScope]) in
                try Self.applyOutboxStaleness(db, gatewayID: gatewayID)
    ...
    public func markCommandAwaitingConfirmation(
        id: String,
        attemptVersion: Int) async -> BotChatOutboxUpdateResult
    {
        await transitionClaimedCommand(
            id: id,
            attemptVersion: attemptVersion,
    ...
    public func markCommandRetriedIfPresent(
        ...
        replacementID: String?) async -> BotChatOutboxUpdateResult
    {
        ...
        let allowsUntargetedAgent = normalizedRoutingContract == BotChatOutboxCommand
        ...
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                let retryScope = BotChatOutboxScope(
    ...
    public func cancelCommand(id: String) async -> BotChatOutboxUpdateResult {
        guard !self.isRetired else { return .unavailable }
    ...
    public func branchState(
        for scope: BotChatOutboxScope) async -> BotChatOutboxBranchState?
    {
        guard !self.isRetired else { return nil }
        let gatewayID = self.gatewayID
        do {
            return try await self.databases.stateQueue.write { db in
                ...
                state = try BotChatOutboxBranchState(
                    epoch: state.epoch,
    ...
    public func beginBranchSwitch(_ scope: BotChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
    ...
    public func cancelBranchSwitch(_ scope: BotChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
    ...
    public func demoteBranchSwitchToReconcile(_ scope: BotChatOutboxScope) async -> Bool {
        guard !self.isRetired else { return false }
        let gatewayID = self.gatewayID
        do {
            let changed = try await databases.stateQueue.write { db -> Bool in
        ...
        lastError: String) async -> [BotChatOutboxCommand]?
    {
        ...
        let gatewayID = self.gatewayID
        do {
            let result = try await databases.stateQueue.write { db -> ([BotChatOutboxCommand], Bool) in
                try Self.ensureBranchScope(db, gatewayID: gatewayID, scope: scope)
...
extension BotChatSQLiteTranscriptCache {
    private func transitionClaimedCommand(
        id: String,
        attemptVersion: Int,
        status: BotChatOutboxCommand.Status,
        retryCount: Int,
        lastError: String?) async -> BotChatOutboxUpdateResult
    {
    ...
    private nonisolated static func readCommands(
        _ db: Database,
        gatewayID: String) throws -> [BotChatOutboxCommand]
    {
        let rows = try Row.fetchAll(
            db,
            sql: """
        ...
        gatewayID: String) throws -> BotChatOutboxCommand
    {
        ...
        guard let status = BotChatOutboxCommand.Status(rawValue: statusRaw) else {
            throw DatabaseError(message: "unknown outbox status")
        }
        let lastError: String = row["last_error"]
        return BotChatOutboxCommand(
            id: id,
    ...
    private nonisolated static func ensureBranchScope(
        _ db: Database,
        gatewayID: String,
        scope: BotChatOutboxScope) throws
    {
        try db.execute(
            sql: """
    ...
    private nonisolated static func readBranchState(
        _ db: Database,
        gatewayID: String,
        scope: BotChatOutboxScope) throws -> BotChatOutboxBranchState
    {
        guard let row = try Row.fetchOne(
            db,
            sql: """
        ...
        return BotChatOutboxBranchState(
    ...
    private nonisolated static func expireBranchSwitchLeases(
        _ db: Database,
        gatewayID: String,
        scope: BotChatOutboxScope? = nil) throws -> [BotChatOutboxScope]
    {
        ...
        return rows.map { row in
            BotChatOutboxScope(sessionKey: row["session_key"], agentID: row["agent_id"])
        }
    }
...
extension BotChatSQLiteTranscriptCache {
    ...
    static func cacheableMessages(_ messages: [BotChatMessage]) -> [BotChatMessage] {
    ...
    private static func cacheablePatchArguments(_ item: BotChatMessageContent) -> AnyCodable? {
    ...
    static func boundedSessions(_ sessions: [BotChatSessionEntry]) -> [BotChatSessionEntry] {
    ...
    private static func attachmentByteCount(_ attachments: [BotChatOutboxAttachment]) -> Int? {
        var total = 0
