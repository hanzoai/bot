...
struct BotChatSwarmActivityState: Equatable {
    ...
    mutating func clear() {
        self = BotChatSwarmActivityState()
    }

    mutating func observe(_ event: BotChatSessionsChangedEvent) -> Bool {
    ...
    func decorate(_ sessions: [BotChatSessionEntry]) -> [BotChatSessionEntry] {
        sessions.map { row in
...
enum BotChatSwarmDotStatus: Int, Comparable, Sendable {
    case running
    case queued
    case failed
    case done
...
struct BotChatSwarmDot: Identifiable, Sendable {
    let id: String
    let label: String
    let status: BotChatSwarmDotStatus
}

struct BotChatSwarmPhase: Identifiable, Sendable {
    let id: String
    let title: String?
    let dots: [BotChatSwarmDot]
    let hidden: Int
}

struct BotChatSwarmGroup: Identifiable, Sendable {
    let id: String
    let label: String
    let running: Int
    let done: Int
    let failed: Int
    let narrator: String?
    let phases: [BotChatSwarmPhase]
}

func buildBotChatSwarmGroups(
    sessions: [BotChatSessionEntry],
    matchesParent: (String) -> Bool) -> [BotChatSwarmGroup]
{
    struct Entry {
        let phase: String?
        let phaseRank: Int
        let log: String?
        let dot: BotChatSwarmDot
    }
    ...
    for row in sessions {
        ...
        byGroup[groupID, default: []].append(Entry(
            ...
            dot: BotChatSwarmDot(id: row.key, label: label, status: status)))
    }

    return byGroup.map { groupID, entries in
        var phaseBuckets: [String: (title: String?, rank: Int, dots: [BotChatSwarmDot])] = [:]
...
enum SelfContainedSwarmHelpers {
    static func status(_ row: BotChatSessionEntry) -> BotChatSwarmDotStatus? {
    ...
    static func isActivityNote(_ event: BotChatSessionsChangedEvent) -> Bool {
        let kind = self.normalized(event.kind)
        return kind == "phase" || kind == "log"
    }

    static func eventBelongsToParent(
        _ event: BotChatSessionsChangedEvent,
        matchesParent: (String) -> Bool) -> Bool
    {
...
extension BotChatViewModel {
    private func updateSwarmProjection() {
        self.activeSwarmGroups = buildBotChatSwarmGroups(sessions: self.swarmSessions) { candidate in
            self.matchesCurrentSessionKey(incoming: candidate, current: self.sessionKey)
        }
    }

    func observeSwarmEvent(_ event: BotChatSessionsChangedEvent) -> Bool {
        guard swarmEnabled,
    ...
    func refreshSwarmSessions(
        ...
        sessionSnapshot: SessionSnapshot? = nil,
        routeLease: BotChatSwarmRouteLease? = nil,
        retryAttempt: Int = 0) async
    {
        guard swarmEnabled else { return }
        let requestedKey = requestedSessionKey ?? sessionSnapshot?.key ?? sessionKey
        guard matchesCurrentSessionKey(incoming: requestedKey, current: sessionKey) else { return }
        let activeRouteLease: BotChatSwarmRouteLease
        if let routeLease {
...
struct BotChatSwarmProgressView: View {
    let groups: [BotChatSwarmGroup]
            ...
            ScrollView(.vertical) {
                VStack(spacing: 6) {
                    ForEach(self.groups) { group in
                        BotChatSwarmGroupView(group: group)
                    }
                }
                .padding(.trailing, 2)
            }
...
private struct BotChatSwarmGroupView: View {
    let group: BotChatSwarmGroup
    ...
    var body: some View {
        ...
        .padding(.vertical, 8)
        .background(BotChatTheme.assistantBubble.opacity(0.5), in: RoundedRectangle(cornerRadius: 10))
        .overlay {
            RoundedRectangle(cornerRadius: 10)
                .stroke(BotChatTheme.accent.opacity(0.2), lineWidth: 1)
        }
    }
}

private struct BotChatSwarmDotView: View {
    let dot: BotChatSwarmDot
    ...
    private var shape: some View {
        ...
        case .queued:
            Circle().stroke(BotChatTheme.muted, lineWidth: 1)
        case .running:
            Circle().fill(BotChatTheme.accent)
        case .done:
            Circle().fill(BotChatTheme.success)
        case .failed:
