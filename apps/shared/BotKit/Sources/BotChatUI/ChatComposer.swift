...
private struct BotSendableCameraImage: @unchecked Sendable {
    let value: UIImage
}
#endif

@MainActor
private struct BotChatAttachmentCaptureOwner {
    let viewModel: BotChatViewModel
    let session: BotChatViewModel.SessionSnapshot
}
...
struct BotChatComposerPresentationOwner: Equatable {
...
struct BotChatComposer: View {
    ...
    @State private var photoPickerOwner: BotChatAttachmentCaptureOwner?
    @State private var fileImporterOwner: BotChatAttachmentCaptureOwner?
    #if canImport(UIKit)
    @State private var cameraEncodingTask: Task<Void, Never>?
    @State private var cameraEncodingGeneration = UUID()
    @State private var cameraCaptureOwner: BotChatAttachmentCaptureOwner?
    #endif
        ...
        VStack(alignment: .leading, spacing: 4) {
            if self.composerChrome == .clean {
                ...
                if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                    BotVoiceNoteRecordingRow(recorder: voiceNoteControl.recorder)
    ...
    private var presentationOwner: BotChatComposerPresentationOwner {
        BotChatComposerPresentationOwner(viewModel: self.viewModel)
    }

    #if !os(macOS)
    private func attachmentCaptureOwner() -> BotChatAttachmentCaptureOwner {
        BotChatAttachmentCaptureOwner(
        ...
        if self.composerChrome == .full {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(BotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(BotChatTheme.composerBorder, lineWidth: 1))
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ...
                    if self.viewModel.showsModelPicker {
                        self.modelPicker
                        if self.viewModel.modelSelectionID != BotChatViewModel.defaultModelSelectionID {
                            self.modelPinButton
                        }
                    }
                }
    ...
    private func contextUsageIndicator(_ fraction: Double) -> some View {
        ...
        let color = fraction >= 0.8 ? BotChatTheme.warning : BotChatTheme.muted
        return ZStack {
            Circle()
                .stroke(BotChatTheme.muted.opacity(0.2), lineWidth: 2)
            Circle()
            ...
            set: { next in self.viewModel.switchSession(to: next) }))
        {
            ForEach(self.viewModel.sessionChoices, id: \.key) { session in
                Text(session.displayName ?? session.key)
                    .font(BotChatTypography.mono(size: 12, relativeTo: .caption))
                    .tag(session.key)
            }
        } label: {
            Text("Thread")
    ...
    private func branchTitle(_ branch: BotChatSessionBranch) -> String {
    ...
    private func branchMetadata(_ branch: BotChatSessionBranch) -> String {
    ...
    private var fullEditor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(BotChatTheme.composerBorder)))
    ...
    private func talkHelpText(_ talkControl: BotChatTalkControl) -> String {
...
extension BotChatComposer {
                      ...
                      self.viewModel.slashCommands.isEmpty
            {
                VStack(alignment: .leading, spacing: 10) {
                    Text("Commands unavailable")
                        .font(BotChatTypography.footnoteSemiBold)
                    Text(error)
                        .font(BotChatTypography.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(3)
    ...
    private func selectSlashCommand(_ command: BotChatCommandChoice) {
...
extension BotChatComposer {
            ...
            Button {
                ...
                if self.viewModel.isAborting {
                    ...
                    Image(systemName: "stop.fill")
                        .font(BotChatTypography.display(size: 13, weight: .semibold, relativeTo: .caption))
                }
            }
            .buttonStyle(.plain)
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? BotChatTheme.userBubble
    }
    ...
    private func handleFileImport(
        ...
        owner: BotChatAttachmentCaptureOwner?)
    {
        guard self.isAttachmentInputEnabled else { return }
        guard let owner,
    ...
    #if canImport(UIKit)
    private func addCameraImage(_ image: UIImage, owner: BotChatAttachmentCaptureOwner) {
        guard self.isAttachmentInputEnabled else { return }
        guard self.viewModel === owner.viewModel,
              owner.viewModel.isCurrentSession(owner.session)
        else { return }
        self.cancelActiveCameraEncoding()
        let sendableImage = BotSendableCameraImage(value: image)
    ...
    private func loadPhotosPickerItems(
        _ items: [PhotosPickerItem],
        owner: BotChatAttachmentCaptureOwner) async
    {
        guard self.isAttachmentInputEnabled else {
            self.pickerItems = []
            return
