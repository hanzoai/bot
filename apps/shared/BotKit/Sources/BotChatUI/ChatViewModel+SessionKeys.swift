...
extension BotChatViewModel {
    nonisolated static func chatContextUsageFraction(for session: BotChatSessionEntry?) -> Double? {
    ...
    func currentSessionEntry() -> BotChatSessionEntry? {
    ...
    func sessionMutationIdentity(for key: String, listedKey: String? = nil) -> String {
        ...
        return self.modelPatchTarget(
            ...
            agentID: BotChatSessionKey.agentID(from: key) ?? self.activeAgentId,
            sessionRoutingContract: nil).canonicalSessionKey
    }

    func applyingLocalUnreadOverrides(
        to sessions: [BotChatSessionEntry]) -> [BotChatSessionEntry]
    {
        ...
        sessionRoutingContract: String?) -> ModelPatchTarget
    {
        ...
        if BotChatSessionKey.agentID(from: candidate) != nil || normalizedCandidate == "unknown" {
            targetKey = candidate
            targetAgentID = nil
    ...
    func sessionEntryForThinking(
        ...
        agentID: String?) -> BotChatSessionEntry?
    {
    ...
    func successfulModelPatchResult(
        ...
        session: BotChatSessionEntry?) -> BotChatModelPatchResult?
    {
        guard let session,
    ...
    func placeholderSession(key: String) -> BotChatSessionEntry {
        BotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
    ...
    public var sessionChoices: [BotChatSessionEntry] {
        ...
        let sorted = BotChatSessionListOrganizer.organize(sessions)
