...
private let databaseLogger = Logger(subsystem: "ai.bot", category: "BotClientDatabases")
...
public final class BotClientDatabases: @unchecked Sendable {
    ...
    public func store(gatewayID: String) -> BotChatSQLiteTranscriptCache {
        BotChatSQLiteTranscriptCache(databases: self, gatewayID: gatewayID)
    }
    ...
    public func loadSessionRoutingIdentity(
        gatewayID: String) -> BotChatSessionRoutingIdentity?
    {
        do {
            return try self.stateQueue.read { db in
                ...
                return BotChatSessionRoutingIdentity(
                    scope: row["scope"],
...
extension BotClientDatabases {
    ...
    private static func openStateDatabase(at url: URL) throws -> DatabaseQueue {
        let queue = try DatabaseQueue(
            path: url.path,
            configuration: self.configuration(label: "Bot.client-state"))
    ...
    private static func openRepairableCacheDatabase(at url: URL) throws -> DatabaseQueue {
        do {
            let queue = try DatabaseQueue(
                path: url.path,
                configuration: self.configuration(label: "Bot.gateway-cache"))
...
extension BotClientDatabases {
    ...
    private struct LegacyCommand {
        ...
        var agentID: String
        var text: String
        var attachments: [BotChatOutboxAttachment]
        var thinking: String
    ...
    private static func readLegacySnapshot(at url: URL) throws -> LegacySnapshot? {
        var configuration = Configuration()
        configuration.label = "Bot.legacy-chat-import"
            ...
            if try db.tableExists("outbox_commands") {
                ...
                for row in rows {
                    ...
                    let attachments = try JSONDecoder().decode(
                        [BotChatOutboxAttachment].self,
                        from: Data(attachmentsJSON.utf8))
                    let originalStatus: String = row["status"]
                    guard BotChatOutboxCommand.Status(rawValue: originalStatus) != nil else {
                    ...
                    let status = lacksVerifiedTarget ? BotChatOutboxCommand.Status.failed.rawValue : originalStatus
                    let lastError: String = if lacksVerifiedTarget {
                        if originalStatus == BotChatOutboxCommand.Status.sending.rawValue ||
                            originalStatus == BotChatOutboxCommand.Status.awaitingConfirmation.rawValue ||
                            originalError == BotChatSQLiteTranscriptCache.outboxUnconfirmedError
                        {
                            BotChatSQLiteTranscriptCache.outboxUnconfirmedError
                        } else {
                            BotChatSQLiteTranscriptCache.outboxUnknownTargetError
                        }
                    } else {
                        originalError
                    }
