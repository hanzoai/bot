...
public enum ExecApprovalsSQLiteStore {
    ...
    public static let locator = "state/bot.sqlite#exec_approvals_config"
    ...
    public static func databaseURL(stateDirectoryURL: URL) -> URL {
        stateDirectoryURL
            ...
            .appendingPathComponent("bot.sqlite", isDirectory: false)
    }
    ...
    static func decode(_ rawJSON: String) throws -> ExecApprovalsDocument {
        guard let data = rawJSON.data(using: .utf8), self.hasValidPersistedStructure(data) else {
            throw BotNativeStateError("Malformed exec approvals raw_json")
        }
    ...
    public static func serialize(_ document: ExecApprovalsDocument) throws -> String {
        guard document.version == 1 else {
            throw BotNativeStateError("Exec approvals document version must be 1")
        }
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
        let data = try encoder.encode(document)
        guard let rawJSON = String(data: data, encoding: .utf8) else {
            throw BotNativeStateError("Could not encode exec approvals as UTF-8")
        }
    ...
    private static func openDatabase(stateDirectoryURL: URL) throws -> BotNativeStateSQLite {
        try BotNativeStateSQLite(
    ...
    private static func readRecord(
        _ database: BotNativeStateSQLite) throws -> ExecApprovalsSQLiteRecord?
    {
        ...
        updatedAtMilliseconds: Int64) throws
    {
        ...
        guard try statement.step() == .done else {
            throw BotNativeStateError("Exec approvals upsert did not complete")
        }
    }

    private static func assertMutationNotFenced(
        _ database: BotNativeStateSQLite) throws
    {
    ...
    private static func bind(
        _ value: String?,
        to statement: BotNativeStateSQLiteStatement,
