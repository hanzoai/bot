            ...
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        ...
                        fallback: "https://docs.bot.ai/gateway/tailscale"),
        ...
        case .authRateLimited:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.bot.ai/gateway/troubleshooting"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        ...
        case .authTokenMissing:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.bot.ai/gateway/authentication"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        ...
        case .authTokenNotConfigured:
            self.problem(
                .init(
                    ...
                    actionCommand: authError.actionCommand
                        ?? "bot config set gateway.auth.token <new-token>",
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.bot.ai/gateway/authentication"),
                    requestId: authError.requestId,
        ...
        case .authPasswordNotConfigured:
            self.problem(
                .init(
                    ...
                    actionLabel: authError.actionLabel ?? "Fix on gateway",
                    actionCommand: authError.actionCommand
                        ?? "bot config set gateway.auth.password <new-password>",
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.bot.ai/gateway/authentication"),
                    requestId: authError.requestId,
        ...
        case .authBootstrapTokenInvalid:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.bot.ai/platforms/ios"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
        case .authDeviceTokenMismatch:
            self.problem(
                .init(
                    ...
                    docsURL: self.docsURL(
                        authError.docsURLString,
                        fallback: "https://docs.bot.ai/gateway/pairing"),
                    requestId: authError.requestId,
                    retryable: false,
                    pauseReconnect: true),
                authError: authError)
...
extension GatewayConnectionProblemMapper {
    ...
    private static func map(_ tlsError: GatewayTLSValidationError) -> GatewayConnectionProblem {
        ...
        case .pinMismatch:
            ...
            return GatewayConnectionProblem(
                ...
                actionCommand: nil,
                docsURL: URL(string: "https://docs.bot.ai/gateway/troubleshooting"),
                retryable: false,
                pauseReconnect: true,
        ...
        case .certificateUnavailable:
            return GatewayConnectionProblem(
                ...
                message: "Bot could not read the gateway certificate for \(failure.host).",
                actionLabel: "Retry",
                messagePresentation: .localizedFormat(
                    "Bot could not read the gateway certificate for %@.",
                    [failure.host]),
                actionCommand: nil,
                docsURL: URL(string: "https://docs.bot.ai/gateway/troubleshooting"),
                retryable: true,
        ...
        case .pinStorageUnavailable:
            return GatewayConnectionProblem(
                ...
                owner: .unknown,
                title: "Gateway certificate unavailable",
                message: "Bot could not securely save the TLS certificate pin for \(failure.host).",
                actionLabel: "Retry",
                titlePresentation: .localized("Gateway certificate unavailable"),
                messagePresentation: .localizedFormat(
                    "Bot could not securely save the TLS certificate pin for %@.",
                    [failure.host]),
        ...
        if nsError.domain == URLError.errorDomain {
            ...
            case .cannotFindHost, .dnsLookupFailed, .notConnectedToInternet, .networkConnectionLost,
                ...
                return GatewayConnectionProblem(
                    ...
                    message: "Bot could not reach the gateway over the current network.",
    ...
    private static func protocolMismatchProblem(for authError: GatewayConnectAuthError) -> GatewayConnectionProblem {
           ...
           clientMax < expected
        {
            ...
            message = authError.userMessageOverride
                ?? "This app is older than the gateway. Update Bot on this device, then retry."
            owner = .iphone
                  ...
                  clientMin > expected
        {
            ...
            message = authError.userMessageOverride
                ?? "The gateway is older than this app. Update Bot on the gateway host, then retry."
            owner = .gateway
            actionLabel = authError.actionLabel ?? "Copy update command"
            actionCommand = authError.actionCommand ?? "bot update"
        } else {
            title = authError.titleOverride ?? "Bot update required"
            message = authError.userMessageOverride
                ?? "The app and gateway use incompatible protocol versions. Update Bot on both, then retry."
            owner = .both
            actionLabel = authError.actionLabel ?? "Update Bot"
            actionCommand = authError.actionCommand
        }
        return self.problem(
            .init(
    ...
    private static func approvalCommand(requestId: String?) -> String {
        if let requestId = self.nonEmpty(requestId) {
            return "bot devices approve \(requestId)"
        }
        return "bot devices list"
    }
