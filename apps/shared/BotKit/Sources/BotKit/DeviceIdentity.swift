...
enum DeviceIdentityPaths {
    ...
    private static let appGroupStateDirAvailable =
        DeviceIdentityPaths.hasAppGroupEntitlement(BotAppGroup.identifier)
        ...
        temporaryDirectory: URL) -> URL
    {
        ...
        return temporaryDirectory.appendingPathComponent("bot", isDirectory: true)
    }

    private static func stateDirOverrideURL() -> URL? {
        // Test-scoped stores must win over the process environment. Parallel Swift tests
        // otherwise race whenever another suite temporarily swaps BOT_STATE_DIR.
    ...
    static func legacyStateDirURL() -> URL? {
        if let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            return appSupport.appendingPathComponent("Bot", isDirectory: true)
        }
        return nil
    }
    ...
    static func appGroupStateDirURL() -> URL? {
        guard
            let containerURL = FileManager.default
                .containerURL(forSecurityApplicationGroupIdentifier: BotAppGroup.identifier)
        else {
            return nil
        }
        return containerURL.appendingPathComponent("Bot", isDirectory: true)
    }
...
public enum DeviceIdentityStore {
    ...
    static func storageError(_ message: String) -> NSError {
        NSError(
            domain: "ai.bot.device-identity-store",
            code: 1,
    ...
    public static func loadOrCreate(profile: GatewayDeviceIdentityProfile) -> DeviceIdentity {
        guard let identity = loadOrCreatePersisted(profile: profile) else {
            preconditionFailure("Could not persist the Bot device identity")
        }
        return identity
    }
