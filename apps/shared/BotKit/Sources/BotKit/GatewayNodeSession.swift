...
public actor GatewayNodeSession {
    ...
    private let logger = Logger(subsystem: "ai.bot", category: "node.gateway")
    ...
    // periphery:ignore - Shipped public refresh API; keep until a breaking BotKit window.
    @discardableResult
...
extension GatewayNodeSession {
        ...
        -> BridgeInvokeResponse
    {
        ...
        let requiresRouteScopedCancellation = request.command == "computer.act" ||
            BotTalkCommand(rawValue: request.command) != nil
    ...
    private func decodeInvokeRequest(from payload: BotProtocol.AnyCodable) throws -> NodeInvokeRequestPayload {
        try self.decodeEventPayload(from: payload)
    }

    private func decodeEventPayload<T: Decodable>(from payload: BotProtocol.AnyCodable) throws -> T {
        do {
