...
public struct BotNativeStateError: Error, LocalizedError, Sendable {
...
public enum BotNativeStateCanonicalTable: Sendable {
    case deviceIdentities
    case execApprovalsConfig
    case macosPortGuardianRecords
}

public enum BotNativeStateSQLiteStep: Equatable, Sendable {
    case row
    case done
}

public enum BotNativeStateSQLiteValueType: Equatable, Sendable {
    case integer
    case float
    case text
    case blob
...
public final class BotNativeStateSQLite: @unchecked Sendable {
    // Keep aligned with BOT_STATE_SCHEMA_VERSION. Native clients never upgrade this database.
    ...
    private static let canonicalTables = [
        BotNativeStateSQLite.deviceIdentities,
        BotNativeStateSQLite.execApprovalsConfig,
        BotNativeStateSQLite.macosPortGuardianRecords,
    ]
    ...
    public init(databaseURL: URL, busyTimeoutMilliseconds: Int32 = 5000) throws {
        ...
        guard result == SQLITE_OK, let database else {
            ...
            throw BotNativeStateError("Could not open native state database: \(detail)")
        }
    ...
    public func ensureCanonicalTable(
        _ table: BotNativeStateCanonicalTable,
        ...
        try self.withConnectionLock {
            ...
            if userVersion == 0 {
                ...
                guard try self.schemaObjectExists(type: "table", name: descriptor.name) else {
                    throw BotNativeStateError(
                        "Versioned Bot state database is missing \(descriptor.name)")
                }
            }
    ...
    public func validateCanonicalTable(_ table: BotNativeStateCanonicalTable) throws {
        try self.withConnectionLock {
            try self.validateTableShape(Self.descriptor(table))
        }
    }

    public func prepare(_ sql: String) throws -> BotNativeStateSQLiteStatement {
        try self.withConnectionLock {
            ...
            return BotNativeStateSQLiteStatement(connection: self, statement: statement)
        }
    }
    ...
    public func execute(_ sql: String) throws {
        try self.withConnectionLock {
            ...
            guard result == SQLITE_OK else {
                ...
                throw BotNativeStateError("SQLite operation failed: \(detail)")
            }
        }
    }
    ...
    public func scalarInt64(_ sql: String) throws -> Int64 {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .row,
                ...
                throw BotNativeStateError("SQLite integer query did not return one integer row")
            }
            let value = statement.int64(at: 0)
            guard try statement.step() == .done else {
                throw BotNativeStateError("SQLite integer query returned multiple rows")
            }
            return value
        }
    }
    ...
    public func scalarText(_ sql: String) throws -> String? {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .done else {
                throw BotNativeStateError("SQLite text query returned multiple rows")
            }
            return value
        }
    }
    ...
    public func schemaObjectExists(type: String, name: String) throws -> Bool {
        try self.withConnectionLock {
            ...
            guard try statement.step() == .done else {
                throw BotNativeStateError("SQLite schema query returned multiple rows")
            }
            return true
        }
    }

    fileprivate func databaseError(operation: String) -> BotNativeStateError {
        BotNativeStateError(
    ...
    private static func descriptor(_ table: BotNativeStateCanonicalTable) -> CanonicalTable {
        switch table {
    ...
    private func validateVersionZeroOwnership() throws {
        ...
        guard objects.isSubset(of: allowedObjects) else {
            throw BotNativeStateError(
                "Schema version zero database contains objects not owned by a native Bot store")
        }
    ...
    private func validateSharedDatabaseMetadata(userVersion: Int64) throws {
        guard try self.schemaObjectExists(type: "table", name: "schema_meta") else {
            throw BotNativeStateError("Versioned Bot state database is missing schema_meta")
        }
        let statement = try self.prepare(
        ...
        guard try statement.step() == .row,
            ...
            throw BotNativeStateError(
                "Bot state database schema metadata does not match its global schema version")
        }
    }

    private func validateTableShape(_ table: CanonicalTable) throws {
        guard try self.tableColumns(table.name) == table.columns else {
            throw BotNativeStateError("\(table.name) has an incompatible schema")
        }
        ...
        guard normalizedTableSQL.hasSuffix(") STRICT") else {
            throw BotNativeStateError("\(table.name) must be a STRICT table")
        }
        guard try self.validRequiredIndex(table) else {
            throw BotNativeStateError("\(table.indexName ?? table.name) has an incompatible index schema")
        }
    }
...
public final class BotNativeStateSQLiteStatement {
    private let connection: BotNativeStateSQLite
    private let statement: OpaquePointer

    fileprivate init(connection: BotNativeStateSQLite, statement: OpaquePointer) {
    ...
    public func step() throws -> BotNativeStateSQLiteStep {
    ...
    public func valueType(at column: Int32) -> BotNativeStateSQLiteValueType {
    ...
    public func requiredText(at column: Int32, field: String) throws -> String {
        try self.connection.withConnectionLock {
            guard self.valueType(at: column) == .text,
                ...
                throw BotNativeStateError("SQLite \(field) must be text")
            }
