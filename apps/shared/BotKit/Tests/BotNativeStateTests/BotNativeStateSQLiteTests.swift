import Foundation
import Testing
@testable import BotNativeState

struct BotNativeStateSQLiteTests {
    @Test
    ...
    func `version zero rejects incomplete known sibling schema`() throws {
        try self.withDatabase { database in
            ...
            #expect(throws: BotNativeStateError.self) {
                try database.ensureCanonicalTable(.deviceIdentities)
            }
        }
    }
    ...
    func `concurrent version zero stores compose different tables`() async throws {
        ...
        let databaseURL = directory.appendingPathComponent("bot.sqlite", isDirectory: false)

        try await withThrowingTaskGroup(of: Void.self) { group in
            for table in [
                BotNativeStateCanonicalTable.deviceIdentities,
                .macosPortGuardianRecords,
            ] {
                group.addTask {
                    let database = try BotNativeStateSQLite(databaseURL: databaseURL)
                    try database.withImmediateTransaction {
    ...
    func `vanished transient sidecar does not fail committed write cleanup`() throws {
        let databaseURL = URL(fileURLWithPath: "/tmp/bot-native-state.sqlite")
        var attemptedPaths: [String] = []

        try BotNativeStateSQLite.secureDatabaseFiles(
            databaseURL,
    ...
    private func withDatabase(
        body: (BotNativeStateSQLite) throws -> Void) throws
    {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let database = try BotNativeStateSQLite(
            databaseURL: directory.appendingPathComponent("bot.sqlite", isDirectory: false))
        try body(database)
    }
}
