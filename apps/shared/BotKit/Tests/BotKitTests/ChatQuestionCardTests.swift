...
@Test func `question card single select and other are exclusive`() {
    let model = BotQuestionCardModel(record: questionRecord())
...
@Test func `question card multi select uses declared option order`() {
    let model = BotQuestionCardModel(record: questionRecord(multiSelect: true))
...
@Test func `question card maps expiry and answer origin`() {
    ...
    let expired = BotQuestionCardModel(record: questionRecord(expiresAtMs: 1_499_000))
    #expect(expired.status(at: now) == .expired)
    #expect(expired.remainingSeconds(at: now) == 0)

    let remote = BotQuestionCardModel(record: questionRecord())
    remote.apply(resolved: BotQuestionResolvedEvent(id: remote.id, status: .answered))
    #expect(remote.status(at: Date(timeIntervalSince1970: 1500)) == .answeredElsewhere)

    let local = BotQuestionCardModel(record: questionRecord())
    local.markAnsweredLocally(answers: ["meal": ["Pizza"]])
    local.apply(resolved: BotQuestionResolvedEvent(id: local.id, status: .answered))
...
@Test func `question card pending refresh preserves submission`() {
    let model = BotQuestionCardModel(record: questionRecord(expiresAtMs: Int.max))
    model.toggleOption(questionID: "meal", label: "Pizza")
...
@Test func `question card locally expired state remains terminal`() {
    ...
    let model = BotQuestionCardModel(record: questionRecord(expiresAtMs: 1_500_000))
...
@Test func `question completions override unavailable recovery race`() throws {
    ...
    let answeredElsewhere = BotQuestionCardModel(record: questionRecord())
...
@Test func `question card terminal summaries prefer resolved answers`() {
    ...
    let answered = BotQuestionCardModel(record: questionRecord(status: .answered, answers: answers))
    let question = answered.record.questions[0]
    #expect(answered.terminalSummaryText(for: question) == "Pizza, extra hot")

    let elsewhere = BotQuestionCardModel(record: questionRecord(status: .answered))
    #expect(elsewhere.terminalSummaryText(for: question) == "Answered elsewhere")

    let skipped = BotQuestionCardModel(record: questionRecord(status: .cancelled))
    #expect(skipped.terminalSummaryText(for: question) == "Skipped")

    let expired = BotQuestionCardModel(record: questionRecord(status: .expired))
    #expect(expired.terminalSummaryText(for: question) == "Expired")

    let unavailable = BotQuestionCardModel(record: questionRecord())
