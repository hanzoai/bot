...
struct BotMascotAnimatorTests {
    private func makeAnimator(seed: UInt64 = 7, interactive: Bool = false) -> BotMascotAnimator {
        BotMascotAnimator(seed: seed, hourOfDay: 12, allowsAutoSleep: interactive)
    }

    @Test func `poses stay inside drawable bounds for every mood`() {
        for mood in BotMascotMood.allCases {
    ...
    @Test func `sleepy mood wears the nightcap`() {
        ...
        let staticPose = BotMascotPose.staticPose(for: .sleepy)
    ...
    @Test func `static poses carry the mood signature`() {
        let sad = BotMascotPose.staticPose(for: .sad)
        #expect(sad.antennaDroop > 0)
        #expect(sad.mouthCurve < 0)
        let celebrating = BotMascotPose.staticPose(for: .celebrating)
        #expect(celebrating.leftClawDegrees > 0)
        #expect(celebrating.mouthCurve > 0)
        let idle = BotMascotPose.staticPose(for: .idle)
        #expect(idle == BotMascotPose())
        let working = BotMascotPose.staticPose(for: .working)
