    ...
    idempotencyKey: String? = nil) -> AnyCodable
{
    ...
    if let idempotencyKey {
        message["__bot"] = ["idempotencyKey": idempotencyKey]
    }
...
private func chatTextModelMessage(
    ...
    text: String,
    timestamp: Double,
    idempotencyKey: String? = nil) -> BotChatMessage
{
    BotChatMessage(
        role: role,
        content: [
...
private func historyPayload(
    ...
    inFlightRun: BotChatInFlightRun? = nil) -> BotChatHistoryPayload
{
    BotChatHistoryPayload(
...
private func sessionEntry(
    ...
    contextTokens: Int? = nil) -> BotChatSessionEntry
{
    BotChatSessionEntry(
        key: key,
        kind: nil,
...
private func sessionsListResponse(_ sessions: [BotChatSessionEntry]) -> BotChatSessionsListResponse {
...
private func thinkingOption(_ id: String, label: String? = nil) -> BotChatThinkingLevelOption {
    BotChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    thinkingLevels: [BotChatThinkingLevelOption]? = nil,
    thinkingOptions: [String]? = nil,
    thinkingDefault: String? = nil,
    verboseLevel: String? = nil,
    fastMode: BotChatFastMode? = nil,
    effectiveFastMode: BotChatFastMode? = nil,
    totalTokens: Int? = nil,
    totalTokensFresh: Bool? = nil,
    contextTokens: Int? = nil) -> BotChatSessionEntry
{
...
private func modelChoice(
    ...
    reasoning: Bool? = nil) -> BotChatModelChoice
{
    BotChatModelChoice(
        modelID: id,
        name: name,
...
private func sessionsResponse(
    ...
    defaults: BotChatSessionsDefaults? = nil) -> BotChatSessionsListResponse
{
    BotChatSessionsListResponse(
        ts: 1,
        path: nil,
...
private func commandChoice(
    ...
    aliases: [String],
    description: String = "",
    source: BotChatCommandChoice.Source = .command,
    acceptsArgs: Bool = false) -> BotChatCommandChoice
{
    BotChatCommandChoice(
...
private func makeViewModel(
    ...
    historyResponses: [BotChatHistoryPayload],
    sessionRoutingContract: String? = nil,
    sessionsResponses: [BotChatSessionsListResponse] = [],
    modelResponses: [[BotChatModelChoice]] = [],
    modelPatchResults: [BotChatModelPatchResult?] = [],
    thinkingPatchResults: [BotChatModelPatchResult?] = [],
    commandResponses: [[BotChatCommandChoice]] = [],
    requestHistoryHook: (@Sendable (String) async throws -> Void)? = nil,
    historyResponseHook: (@Sendable (String, Int, [String]) async throws -> BotChatHistoryPayload?)? = nil,
    ...
    sessionSettingsPatchHook: (
        @Sendable (BotChatSessionSettingsPatch) async throws -> BotChatModelPatchResult?)? = nil,
    ...
    listSessionsHook: (
        @Sendable (TestSessionListQuery) async throws -> BotChatSessionsListResponse?)? = nil,
    sendMessageHook: (@Sendable (String) async throws -> BotChatSendResponse)? = nil,
    sendMessageStatus: String = "ok",
    waitForRunCompletionHook: (@Sendable (String, Int) async -> BotChatRunObservation)? = nil,
    acquireSessionSettingsRouteLeaseHook: (@Sendable () async -> Void)? = nil,
    swarmEnabledHook: (@Sendable (String) async throws -> Bool)? = nil,
    listChildSessionsHook: (@Sendable (String) async throws -> [BotChatSessionEntry])? = nil,
    ...
    -> (TestChatTransport, BotChatViewModel)
{
...
private func sendUserMessage(_ vm: BotChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, BotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [BotChatHistoryPayload]
    private let sessionsResponses: [BotChatSessionsListResponse]
    private let modelResponses: [[BotChatModelChoice]]
    private let modelPatchResults: [BotChatModelPatchResult?]
    private let thinkingPatchResults: [BotChatModelPatchResult?]
    private let commandResponses: [[BotChatCommandChoice]]
    private let requestHistoryHook: (@Sendable (String) async throws -> Void)?
    private let historyResponseHook:
        (@Sendable (String, Int, [String]) async throws -> BotChatHistoryPayload?)?
    ...
    private let sessionSettingsPatchHook:
        (@Sendable (BotChatSessionSettingsPatch) async throws -> BotChatModelPatchResult?)?
    ...
    private let listSessionsHook:
        (@Sendable (TestSessionListQuery) async throws -> BotChatSessionsListResponse?)?
    private let sendMessageHook: (@Sendable (String) async throws -> BotChatSendResponse)?
    private let sendMessageStatus: String
    private let waitForRunCompletionHook:
        (@Sendable (String, Int) async -> BotChatRunObservation)?
    ...
    private let listChildSessionsHook: (@Sendable (String) async throws -> [BotChatSessionEntry])?
    ...
    private let stream: AsyncStream<BotChatTransportEvent>
    private let continuation: AsyncStream<BotChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<BotChatTransportEvent>.Continuation!
    ...
    func createSession(
        ...
        worktree _: Bool?) async throws -> BotChatCreateSessionResponse
    {
        ...
        return BotChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> BotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? BotChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [BotChatAttachmentPayload]) async throws -> BotChatSendResponse
    {
        ...
        return BotChatSendResponse(runId: idempotencyKey, status: self.sendMessageStatus)
    }
        ...
        archived: Bool) async throws -> BotChatSessionsListResponse
    {
        ...
        return self.sessionsResponses.last ?? BotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
        ...
        patch: BotChatSessionSettingsPatch) async throws -> BotChatModelPatchResult?
    {
        ...
        var result: BotChatModelPatchResult?
        ...
        if let thinkingLevelUpdate = patch.thinkingLevel {
            ...
            let thinkingResult = try await patchSessionThinking(
                sessionKey: sessionKey,
                thinkingLevel: thinkingLevel)
            result = BotChatModelPatchResult(
    ...
    private func patchSessionThinking(
        sessionKey: String,
        thinkingLevel: String) async throws -> BotChatModelPatchResult?
    {
        ...
        return BotChatModelPatchResult(
            key: sessionKey,
            modelProvider: nil,
            model: nil,
            thinkingLevel: thinkingLevel)
    ...
    func emit(_ evt: BotChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test @MainActor func `transient Swarm capability failure preserves state and retries until explicit false`() async throws {
        ...
        let viewModel = BotChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test @MainActor func `global Swarm activity follows the selected agent owner`() async {
        ...
        let foreign = BotChatSessionsChangedEvent(
            sessionKey: "global",
            agentId: "main",
            reason: "swarm-note",
        ...
        let selected = BotChatSessionsChangedEvent(
            sessionKey: "global",
            agentId: "work",
            reason: "swarm-note",
            swarmGroupId: "swarm:global:turn-1",
    ...
    @Test func `context usage fraction validates freshness and token bounds`() {
        func fraction(total: Int?, fresh: Bool? = true, context: Int?) -> Double? {
            BotChatViewModel.chatContextUsageFraction(
                for: sessionEntry(
                    key: "main",
                    updatedAt: 1,
                    totalTokens: total,
    ...
    @Test @MainActor func `event listener does not retain discarded view model`() async throws {
        ...
        var viewModel: BotChatViewModel? = BotChatViewModel(
            sessionKey: "main",
            transport: transport)
    ...
    @Test func `decodes in-flight run from chat history`() throws {
        ...
        let payload = try JSONDecoder().decode(BotChatHistoryPayload.self, from: data)

        #expect(payload.inFlightRun?.runId == "run-active")
        #expect(payload.inFlightRun?.text == "partial")
        #expect(payload.inFlightRun?.plan?.steps == [
            BotChatPlanStep(step: "Reconnect", status: .inProgress),
        ])
    ...
    @Test func `decodes agent scope from chat event`() throws {
        ...
        let payload = try JSONDecoder().decode(BotChatEventPayload.self, from: data)
    ...
    @Test func `bootstrap adopts active history run and consumes live events`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: BotChatInFlightRun(runId: "run-active", text: "partial reply"))
    ...
    @Test func `adopts Codex history run without buffered text`() async throws {
        let history = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-codex", text: ""))
    ...
    @Test func `bootstrap adopts in-flight plan snapshot`() async throws {
        let history = historyPayload(
            inFlightRun: BotChatInFlightRun(
                runId: "run-plan",
                text: "working",
                plan: BotChatPlanSnapshot(
                    steps: [
                        BotChatPlanStep(step: "Inspect", status: .completed),
                        BotChatPlanStep(step: "Reconnect", status: .inProgress),
                    ],
    ...
    @Test func `history plan reconciliation contract`() async {
        let retainedSteps = [BotChatPlanStep(step: "Retained", status: .inProgress)]
        let liveSteps = [BotChatPlanStep(step: "New live plan", status: .inProgress)]
        let cases: [(
            name: String,
            payload: BotChatHistoryPayload,
            expectedRunId: String?,
            expectedSteps: [BotChatPlanStep],
            ...
            (
                ...
                historyPayload(
                    inFlightRun: BotChatInFlightRun(
                        ...
                        text: "working",
                        plan: BotChatPlanSnapshot(
                            steps: [BotChatPlanStep(step: "Replacement", status: .completed)]))),
                "run-retained",
                [BotChatPlanStep(step: "Replacement", status: .completed)],
                false),
            (
                "legacy-preserve",
                historyPayload(
                    inFlightRun: BotChatInFlightRun(runId: "run-retained", text: "working")),
                "run-retained",
                retainedSteps,
                false),
            (
                ...
                historyPayload(
                    inFlightRun: BotChatInFlightRun(
                        ...
                        text: "next",
                        plan: BotChatPlanSnapshot(
                            steps: [BotChatPlanStep(step: "Next run", status: .inProgress)]))),
                "run-next",
                [BotChatPlanStep(step: "Next run", status: .inProgress)],
                false),
            (
                "active-preserve",
            ...
            (
                ...
                historyPayload(
                    inFlightRun: BotChatInFlightRun(
                        runId: "run-retained",
                        text: "working",
                        plan: BotChatPlanSnapshot(steps: []))),
                nil,
                [],
                false),
        ]
    ...
    @Test func `foreground history refreshes adopted run snapshot`() async throws {
        let firstHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-active", text: "first partial"))
        let resumedHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-active", text: "resumed partial"))
        let historyCalls = AsyncCounter()
        ...
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                return historyPayload(
                    ...
                    inFlightRun: BotChatInFlightRun(runId: runId, text: "working"))
            },
    ...
    @Test func `foreground discovers run started without local ownership`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-external", text: "external partial"))
    ...
    @Test func `foreground keeps active run with intermediate assistant history`() async throws {
        ...
        let activeHistory = historyPayload(
            ...
            inFlightRun: BotChatInFlightRun(runId: "run-tool", text: "still working"))
        let (_, vm) = await makeViewModel(historyResponses: [idleHistory, activeHistory])
    ...
    @Test func `foreground clears completed run without assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: BotChatInFlightRun(
                ...
                text: "",
                plan: BotChatPlanSnapshot(
                    steps: [BotChatPlanStep(step: "Finish", status: .inProgress)])))
        let completedHistory = historyPayload(
            messages: [chatTextMessage(role: "user", text: "quiet task", timestamp: 1)],
            hasActiveRun: false)
    ...
    @Test func `foreground missing snapshot does not clear an in-flight send`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-active", text: "accepted"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), historyPayload(), activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            sendMessageHook: { _ in
                await sendGate.wait()
                return BotChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send history keeps active run with intermediate assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: BotChatInFlightRun(runId: "run-active", text: "working"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), activeHistory],
            sendMessageHook: { _ in
                BotChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send stale inactive history preserves newer live plan`() async throws {
        ...
        #expect(await MainActor.run { vm.planSteps == [
            BotChatPlanStep(step: "Finish", status: .inProgress),
        ] })
    }
    ...
    @Test func `live chat delta owns run while bootstrap history is pending`() async throws {
        ...
        let staleHistory = historyPayload(
            ...
            inFlightRun: BotChatInFlightRun(runId: "run-stale", text: "stale partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `global chat delta adopts only selected agent run`() async throws {
        let bareGlobalMatches = await MainActor.run {
            (
                BotChatViewModel.matchesCurrentSessionKey(
                    incoming: "global",
                    agentId: "work",
                    current: "global",
                    mainSessionKey: "main",
    ...
    @Test @MainActor func `global session changes reconcile nested digest ownership`() async {
        ...
        selected.observerDigest = BotChatSessionObserverDigest(
            agentId: "work",
            runId: "run-work",
            revision: 2,
            updatedAt: 200,
        ...
        vm.handleTransportEvent(.sessionsChanged(.init(
            ...
            agentId: "work",
            updatedAt: 900,
            observerDigest: BotChatSessionObserverDigest(
                agentId: "main",
                runId: "run-work",
                revision: 9,
                updatedAt: 900,
    ...
    @Test func `intermediate session message preserves pending recovery snapshot`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-active", text: "active partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `manual refresh re-adopts active run after clearing local ownership`() async throws {
        ...
        let refreshedHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-active", text: "refreshed partial"))
    ...
    @Test func `older history cannot replace newer run snapshot`() async throws {
        ...
        let initialHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-initial", text: "initial"))
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    return historyPayload(
                        inFlightRun: BotChatInFlightRun(runId: "run-older", text: "older"))
                }
                if index == 2 {
                    return historyPayload(
                        inFlightRun: BotChatInFlightRun(runId: "run-newer", text: "newer"))
                }
                return nil
            })
    ...
    @Test func `delayed history cannot overwrite newer live run text`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-active", text: "initial"))
        let (transport, vm) = await makeViewModel(
            historyResponses: [activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            historyResponseHook: { _, index, _ in
                ...
                return historyPayload(
                    inFlightRun: BotChatInFlightRun(runId: "run-active", text: "stale"))
            })
    ...
    @Test func `external terminal event protects current run from delayed snapshot`() async throws {
        ...
        let currentHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-current", text: "current"))
        let (transport, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    _ = await staleCompletions.increment()
                    return historyPayload(
                        inFlightRun: BotChatInFlightRun(runId: "run-finished", text: "stale"))
                }
                if index == 2 {
                    throw NSError(domain: "test", code: 1)
                }
    ...
    @Test func `sequence gap re-adopts active run from history`() async throws {
        ...
        let recoveredHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(runId: "run-recovered", text: "recovered partial"))
    ...
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> BotChatMessage {
        ...
        #expect(
            BotChatMessage.errorDisplayText(
    ...
    @Test func `composer presentation owner changes when the model is replaced for the same session`() async {
        ...
        let firstOwner = await MainActor.run {
            BotChatComposerPresentationOwner(viewModel: first)
        }
        let replacementOwner = await MainActor.run {
            BotChatComposerPresentationOwner(viewModel: replacement)
        }
    ...
    @Test func `composer presentation owner changes with same-session agent routing`() async {
        ...
        let alphaOwner = await MainActor.run {
            BotChatComposerPresentationOwner(viewModel: vm)
        }
        let betaOwner = await MainActor.run {
            vm.syncActiveAgentId("beta")
            return BotChatComposerPresentationOwner(viewModel: vm)
        }
    ...
    @Test func `balances tool activity when a terminal event clears pending calls`() async throws {
        ...
        transport.emit(.chat(BotChatEventPayload(
            runId: runId,
            sessionKey: "main",
            state: "final",
            message: nil,
    ...
    @Test func `later identical session reply does not adopt prior turn provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                BotChatSendResponse(runId: runId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `plan event parses typed and legacy steps`() async throws {
        ...
        #expect(await MainActor.run { vm.planSteps } == [
            BotChatPlanStep(step: "Inspect state", status: .inProgress),
            BotChatPlanStep(step: "Write fix", status: .pending),
            BotChatPlanStep(step: "Verify", status: .completed),
        ])
    ...
    @Test func `plan snapshots replace and empty snapshot clears`() async throws {
        ...
        try await waitUntil("replacement plan snapshot applies") {
            await MainActor.run {
                vm.planSteps == [BotChatPlanStep(step: "Replacement", status: .pending)] &&
    ...
    @Test func `terminal event for another run preserves active plan`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: BotChatInFlightRun(
                ...
                text: "",
                plan: BotChatPlanSnapshot(
                    steps: [BotChatPlanStep(step: "Keep working", status: .inProgress)])))
        let (transport, vm) = await makeViewModel(
            historyResponses: [initialHistory, activeHistory, activeHistory],
            sendMessageHook: { _ in
                BotChatSendResponse(runId: activeRunId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        ...
        #expect(await MainActor.run { vm.pendingRunCount } == 1)
        #expect(await MainActor.run { vm.planSteps } == [
            BotChatPlanStep(step: "Keep working", status: .inProgress),
        ])
    }
    ...
    @Test func `rekeys optimistic user message when gateway reuses active run`() async throws {
        ...
        let (_, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                ...
                return BotChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `canonical projected reply after steering adopts reused provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                BotChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(BotChatTransportEvent.seqGap)
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let history = historyPayload()
        let sessions = BotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = BotChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: BotChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(BotChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == BotChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = BotChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: BotChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                if (1...3).contains(index) {
                    ...
                    return historyPayload(
                        ...
                        sessionId: sessionId,
                        messages: [chatTextMessage(role: "user", text: "hello", timestamp: now)],
                        inFlightRun: BotChatInFlightRun(runId: runId, text: ""))
                }
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = BotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = BotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = BotChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test @MainActor func `Ultra is canonical while ultrathink remains a high alias`() {
        #expect(BotChatViewModel.normalizedThinkingLevel("ultra") == "ultra")
        #expect(BotChatViewModel.normalizedThinkingLevel("ULTRA") == "ultra")
        #expect(BotChatViewModel.normalizedThinkingLevel("ultrathink") == "high")
        #expect(
            BotChatViewModel.normalizedThinkingLevel(
                "ultra",
    ...
    @Test func `decodes authoritative model patch thinking state`() throws {
        ...
        let result = try JSONDecoder().decode(BotChatModelPatchResult.self, from: data)
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            BotChatSessionsListResponse.self,
            from: Data(json.utf8))
        ...
        try await waitUntil("models loaded with default selection") {
            await MainActor.run {
                vm.modelChoices.count == 2 &&
                    vm.modelSelectionID == BotChatViewModel.defaultModelSelectionID
            }
        }
    ...
    @Test func `default settings patch returns accepted thinking state`() async throws {
        ...
        let result = try await transport.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            patch: BotChatSessionSettingsPatch(thinkingLevel: .some("high")))
    ...
    @Test func `default thinking selection clears override and adopts resolved level`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectThinkingLevel(BotChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")

        #expect(await MainActor.run { vm.thinkingSelectionID } ==
            BotChatViewModel.inheritedThinkingSelectionID)
        #expect(await MainActor.run { vm.thinkingLevel } == "medium")
        #expect(await MainActor.run { vm.sessions.first?.thinkingLevel } == nil)
    ...
    @Test func `inherited verbosity does not masquerade as persisted override`() async throws {
        ...
        #expect(await MainActor.run { vm.verboseLevel } == BotChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity rejection restores inherited overrides`() async throws {
        ...
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.fastModeSelectionID } == BotChatViewModel.inheritedThinkingSelectionID)
    ...
    @Test func `fast and verbosity default selections clear overrides`() async throws {
        ...
        try await loadAndWaitBootstrap(vm: vm, sessionId: "sess-main")
        await MainActor.run {
            vm.selectFastMode(BotChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.fastMode } == nil)
        ...
        await MainActor.run {
            vm.selectVerboseLevel(BotChatViewModel.inheritedThinkingSelectionID)
        }
        await vm.waitForPendingSessionSettings(in: "main")
        #expect(await MainActor.run { vm.sessions.first?.verboseLevel } == nil)
        #expect(await MainActor.run { vm.verboseLevel } == BotChatViewModel.inheritedThinkingSelectionID)
        ...
        let history = BotChatHistoryPayload(
            ...
            messages: [
                AnyCodable([
                    ...
                    Conversation info: \u{27E6}bot:ctx\u{27E7}
                    ```json
                    { \"sender\": \"bot-ios\" }
                    ```
...
struct ChatViewModelSessionManagementTests {
    @Test @MainActor func `session list organizer orders pinned first with key tiebreak`() {
        let organized = BotChatSessionListOrganizer.organize([
    ...
    @Test @MainActor func `session list organizer filters across display fields`() {
        ...
        let matched = BotChatSessionListOrganizer.filter(sessions, search: "TRIP")
        #expect(matched.map(\.key) == ["agent:main:topic-a", "agent:main:trip-notes"])
        #expect(BotChatSessionListOrganizer.filter(sessions, search: "  ") == sessions)
    }
