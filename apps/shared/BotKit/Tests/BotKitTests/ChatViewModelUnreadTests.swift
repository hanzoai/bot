...
private actor UnreadTestTransportState {
    ...
    var sessionOverride: [BotChatSessionEntry]?
...
private final class UnreadTestTransport: @unchecked Sendable, BotChatTransport {
    ...
    func setSessions(_ sessions: [BotChatSessionEntry]) async {
...
extension UnreadTestTransportState {
    ...
    fileprivate func setSessions(_ sessions: [BotChatSessionEntry]) {
...
struct ChatViewModelUnreadTests {
    ...
    @Test func `failed route lease preserves mutation queue ordering`() async throws {
        ...
        let firstLease = BotChatSessionMutationRouteLease { _, _, _, _, _, _ in
            await recorder.append("first-start")
            try await Task.sleep(for: .milliseconds(100))
            await recorder.append("first-end")
        }
        let thirdLease = BotChatSessionMutationRouteLease { _, _, _, _, _, _ in
            await recorder.append("third")
        }

        let first = queue.reserve(
            routeLease: Task<BotChatSessionMutationRouteLease?, Never> { firstLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)
        let second = queue.reserve(
            routeLease: Task<BotChatSessionMutationRouteLease?, Never> { nil },
            queueKey: "a",
            routeKey: "a",
            unread: true)
        let third = queue.reserve(
            routeLease: Task<BotChatSessionMutationRouteLease?, Never> { thirdLease },
            queueKey: "a",
            routeKey: "a",
            unread: false)

        try await first.value
        await #expect(throws: BotChatTransportSendError.self) {
    ...
    private func viewModel(
        ...
        transport: UnreadTestTransport) -> BotChatViewModel
    {
    ...
    private func entry(
        ...
        pinned: Bool? = nil) -> BotChatSessionEntry
    {
        BotChatSessionEntry(
            key: key,
            kind: nil,
