...
struct ChatGatewayRequestTests {
    @Test func `session observation requests encode global subscription and actual visibility`() {
        let subscribe = BotChatGatewayRequests.subscribeSessions()
        let visible = BotChatGatewayRequests.setSessionObserverVisibility(true)
        let hidden = BotChatGatewayRequests.setSessionObserverVisibility(false)
        let longerSubscription = BotChatGatewayRequests.subscribeSessions(timeoutMs: 12000)
        let longerVisibility = BotChatGatewayRequests.setSessionObserverVisibility(
            true,
            timeoutMs: 12000)
    ...
    @Test func `session targets share normalization while preserving platform routing policy`() {
        #expect(BotChatSessionTarget.resolve(
            " Matrix:Channel:Room ",
    ...
    @Test func `settings patch request encodes fast values and explicit resets`() {
        let reset = BotChatGatewayRequests.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
            thinkingLevel: .some(nil),
            fastMode: .some(nil),
            verboseLevel: .some(nil))
        let automatic = BotChatGatewayRequests.patchSessionSettings(
            sessionKey: "main",
            agentID: nil,
    ...
    @Test func `fork and create requests preserve routing identity`() {
        let fork = BotChatGatewayRequests.forkSession(
        ...
        let create = BotChatGatewayRequests.createSession(
    ...
    @Test func `message rewind and fork requests preserve routing identity`() {
        let rewind = BotChatGatewayRequests.rewindSession(
            sessionKey: "agent:reviewer:telegram:group:1",
            agentID: " reviewer ",
            entryId: " message-42 ")
        let fork = BotChatGatewayRequests.forkAtMessage(
            sessionKey: "global",
            agentID: nil,
            entryId: "message-43")
    ...
    @Test func `branch list and switch requests preserve routing identity`() {
        let list = BotChatGatewayRequests.listSessionBranches(
            sessionKey: "agent:reviewer:telegram:group:1",
            agentID: " reviewer ")
        let switchBranch = BotChatGatewayRequests.switchSessionBranch(
            sessionKey: "global",
            agentID: nil,
            leafEntryId: " leaf-42 ")
    ...
    @Test func `session group requests encode exact gateway contracts`() {
        let list = BotChatGatewayRequests.sessionGroupsList()
        let put = BotChatGatewayRequests.sessionGroupsPut(names: ["Work", "Personal"])
        let rename = BotChatGatewayRequests.sessionGroupsRename(name: "Work", to: "Projects")
        let delete = BotChatGatewayRequests.sessionGroupsDelete(name: "Personal")
    ...
    @Test func `rename clear archive and fork use session mutation contracts`() {
        let rename = BotChatGatewayRequests.patchSession(
            ...
            archived: nil,
            unread: nil)
        let archive = BotChatGatewayRequests.patchSession(
            sessionKey: "agent:main:child",
            agentID: nil,
            label: nil,
            category: nil,
    ...
    @Test func `commands request selects session agent before fallback`() {
        let scoped = BotChatGatewayRequests.commandsList(
            sessionKey: "agent:reviewer:main",
            fallbackAgentID: "fallback")
        ...
        let global = BotChatGatewayRequests.commandsList(
            sessionKey: "global",
            fallbackAgentID: "reviewer")
        #expect(global.params["agentId"]?.value as? String == "reviewer")
    }
    ...
    @Test func `question resolve request preserves answer arrays`() throws {
        let request = BotChatGatewayRequests.resolveQuestion(
            id: "ask_123",
    ...
    @Test func `question get request carries id`() {
        let request = BotChatGatewayRequests.questionGet(id: "ask_123")
    ...
    @Test func `question cancel request uses resolve cancel contract`() {
        let request = BotChatGatewayRequests.cancelQuestion(id: "ask_123")
    ...
    @Test func `long running requests share exact gateway timeout margins`() {
        #expect(BotChatGatewayRequests.agentWait(runID: "run-1", timeoutMs: 1).timeoutMs == 5001)
        #expect(BotChatGatewayRequests.agentWait(runID: "run-1", timeoutMs: 30000).timeoutMs == 35000)
        #expect(BotChatGatewayRequests.compactSession(
            sessionKey: "main",
...
struct ChatGatewayPayloadCodecTests {
    @Test func `session key extracts canonical agent identity`() {
        #expect(BotChatSessionKey.agentID(from: " agent:Reviewer:main ") == "Reviewer")
        #expect(BotChatSessionKey.agentID(from: "agent::main") == nil)
        #expect(BotChatSessionKey.agentID(from: "global") == nil)
    }

    @Test func `agent wait distinguishes terminal and retryable timeouts`() throws {
        #expect(try BotChatGatewayPayloadCodec.decodeAgentWaitObservation(
    ...
    @Test func `routing identity decodes agent and canonical contract`() throws {
        let identity = try BotChatGatewayPayloadCodec.decodeSessionRoutingIdentity(
    ...
    @Test func `model choices preserve metadata and replace blank names`() throws {
        let choices = try BotChatGatewayPayloadCodec.decodeModelChoices(Data(
            #"{"models":[{"id":"gpt-5","name":"  ","provider":"openai","contextWindow":200000,"reasoning":true}]}"#
                .utf8))

        #expect(choices == [BotChatModelChoice(
            modelID: "gpt-5",
            name: "gpt-5",
            provider: "openai",
    ...
    @Test func `command choice normalizes source aliases and identity`() {
        let choice = BotChatGatewayPayloadCodec.commandChoice(CommandEntry(
            name: "review",
            textaliases: [" /review ", ""],
    ...
    @Test func `event frames map to shared chat transport events`() {
        ...
        guard case let .sessionsChanged(change) = BotChatGatewayPayloadCodec.event(from: sessionsChanged)
        else {
        ...
        guard case let .sessionsChanged(note) = BotChatGatewayPayloadCodec.event(from: swarmNote)
        else {
        ...
        guard case let .chat(payload) = BotChatGatewayPayloadCodec.event(from: chat) else {
        ...
        guard case let .sessionObserver(digest) = BotChatGatewayPayloadCodec.event(from: observer)
        else {
        ...
        #expect(BotChatGatewayPayloadCodec.event(from: EventFrame(
            type: "event",
            event: "unknown")) == nil)
    }

    @Test func `session change decoding distinguishes absent fields from explicit clears`() {
        func decode(_ fields: [String: AnyCodable]) throws -> BotChatSessionsChangedEvent {
    ...
    @Test func `session change decodes nested fallback with outer null precedence and no reason`() throws {
        ...
        let event = try JSONDecoder().decode(BotChatSessionsChangedEvent.self, from: data)
    ...
    @Test func `session change remains codable without exposing presence flags`() throws {
        let event = BotChatSessionsChangedEvent(
        ...
        #expect(try JSONDecoder().decode(BotChatSessionsChangedEvent.self, from: data) == event)
    }
}
