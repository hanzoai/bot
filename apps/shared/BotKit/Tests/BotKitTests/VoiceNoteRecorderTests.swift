...
final class VoiceNoteRecorderTests: XCTestCase {
    ...
    func testSuccessiveRecordingsUseUniqueTemporaryFiles() async throws {
        let capture = FakeVoiceNoteAudioCapture()
        let recorder = BotVoiceNoteRecorder(
            capture: capture,
    ...
    func testDurationLabelBoundsMalformedHistoryValues() {
        XCTAssertEqual(botVoiceNoteDurationLabel(.infinity), "0:00")
        XCTAssertEqual(botVoiceNoteDurationLabel(.nan), "0:00")
        XCTAssertEqual(botVoiceNoteDurationLabel(1e100), "3:00")
        XCTAssertEqual(botVoiceNoteDurationLabel(-1), "0:00")
    }

    func testUnifiedMicKeepsVoiceNoteAvailableWithoutDictation() {
        XCTAssertFalse(BotChatMicButton.dictationActionEnabled(
        ...
        XCTAssertTrue(BotChatMicButton.voiceNoteRecordingEnabled(
            isComposerEnabled: true,
    ...
    func testUnifiedMicPreventsCompetingVoiceCapture() {
        XCTAssertFalse(BotChatMicButton.voiceNoteRecordingEnabled(
            isComposerEnabled: true,
            isAttachmentInputEnabled: true,
        ...
        XCTAssertTrue(BotChatMicButton.dictationActionEnabled(
    ...
    func testUnifiedMicCancelsPendingDictationStart() {
        XCTAssertEqual(BotChatMicButton.dictationPrimaryAction(
            isPending: true,
    ...
    func testCompactTalkControlYieldsToLocalVoiceCapture() {
        XCTAssertTrue(BotChatComposer.showsCompactTalkControl(
            hasDraftToSend: false,
            hasBlockingRunActivity: false,
            isLocalVoiceCaptureActive: false))
        XCTAssertFalse(BotChatComposer.showsCompactTalkControl(
            hasDraftToSend: false,
            hasBlockingRunActivity: false,
    ...
    func testRecordButtonRequiresAttachmentInput() {
        let recorder = BotVoiceNoteRecorder(capture: FakeVoiceNoteAudioCapture())
        let control = BotChatVoiceNoteControl(recorder: recorder, isTalkActive: false)
        let button = BotVoiceNoteButton(
            control: control,
            compact: false,
            isComposerEnabled: true,
