...
struct GatewayErrorsTests {
    ...
    @Test func `connect auth error preserves structured metadata`() {
        let error = GatewayConnectAuthError(
            ...
            actionCommand: "bot devices approve req-123",
            docsURLString: "https://docs.bot.ai/gateway/pairing",
        ...
        #expect(error.actionCommand == "bot devices approve req-123")
        #expect(error.docsURLString == "https://docs.bot.ai/gateway/pairing")
    ...
    @Test func `protocol mismatch maps older app to update problem`() {
        ...
        #expect(problem?.message == "This app is older than the gateway. Update Bot on this device, then retry.")
    ...
    @Test func `protocol mismatch maps older gateway to update problem`() {
        ...
        #expect(problem?
            .message == "The gateway is older than this app. Update Bot on the gateway host, then retry.")
    ...
    @Test func `protocol mismatch without versions still gives actionable fallback`() {
        ...
        #expect(problem?
            .message == "The app and gateway use incompatible protocol versions. Update Bot on both, then retry.")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `pairing problem uses structured request metadata`() {
        ...
        #expect(problem?.pauseReconnect == true)
        #expect(problem?.actionCommand == "bot devices approve req-123")
    }
