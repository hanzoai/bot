...
/// Serializes state-directory tests and pins `BOT_STATE_DIR` to a fresh
...
/// (app group container or ~/Library/Application Support/Bot), so real
...
struct StateDirectoryIsolationTrait: TestTrait, TestScoping {
    ...
    private static let restoreStateDirPath: String = {
        if let raw = getenv("BOT_STATE_DIR") {
        ...
        let quarantine = FileManager.default.temporaryDirectory
            .appendingPathComponent("bot-test-state-quarantine-\(UUID().uuidString)", isDirectory: true)
        ...
        performing function: @Sendable () async throws -> Void) async throws
    {
        ...
        let stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bot-test-state-\(UUID().uuidString)", isDirectory: true)
        var thrown: (any Error)?
        do {
            try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
            setenv("BOT_STATE_DIR", stateDir.path, 1)
            try await function()
        } catch {
            thrown = error
        }
        ...
        setenv("BOT_STATE_DIR", restorePath, 1)
