...
struct ChatInlineWidgetTests {
    ...
    @Test func `decodes projected canvas widget block`() throws {
          ...
          "content": [
            {"type": "text", "text": "Done"},
            {
              ...
              "preview": {
                ...
                "title": "Status",
                "preferredHeight": 240,
                "url": "/__bot__/canvas/documents/widget-1/index.html",
                "sandbox": "scripts"
              }
            }
          ]
        ...
        #expect(preview.inlineWidgetPath == "/__bot__/canvas/documents/widget-1/index.html")
        #expect(preview.inlineWidgetHeight == 240)

        let unsafe = BotChatCanvasPreview(
            kind: "canvas",
            surface: "assistant_message",
            render: "url",
            title: nil,
    ...
    @Test func `resolves only capability scoped widget documents`() {
        let surface = "https://gateway.example/__bot__/cap/token"
        let target = "/__bot__/canvas/documents/widget-1/index.html"

        #expect(BotChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: target)?.absoluteString ==
            "https://gateway.example/__bot__/cap/token/__bot__/canvas/documents/widget-1/index.html")
        #expect(BotChatWidgetURLResolver.resolve(surfaceURL: "https://gateway.example", target: target) == nil)
        #expect(BotChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "https://attacker.example/widget.html") == nil)
        #expect(BotChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__bot__/a2ui/index.html") == nil)
        #expect(BotChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__bot__/canvas/documents/%252e%252e/index.html") == nil)
        #expect(BotChatWidgetURLResolver.resolve(
            surfaceURL: surface,
            target: "/__bot__/canvas/documents/%2525252525252525252525252525252525/index.html") == nil)
    }
    ...
    @Test func `resource equality ignores internal recovery metadata`() throws {
        ...
        let publicResource = BotChatWidgetResource(url: url)
        let trackedResource = BotChatWidgetResource(
            url: url,
    ...
    @Test func `uses replacement route after capability refresh loses its lease`() async throws {
        let target = "/__bot__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://gateway.example/__bot__/cap/old"
        let newSurface = "https://gateway.example/__bot__/cap/new"
        let failedURL = try #require(BotChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = BotChatWidgetResource(url: failedURL)
        ...
        let resolved = await BotChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(resolved?.url == BotChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))
    ...
    @Test func `accepts a same URL widget route when it acquires a TLS pin`() async throws {
        ...
        let initialResource = await BotChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
    ...
    @Test func `refreshes node once before trying the operator fallback`() async {
        ...
        let oldSurface = "https://gateway.example/__bot__/cap/old"
        let newSurface = "https://gateway.example/__bot__/cap/new"
        let fallbackSurface = "https://operator.example/__bot__/cap/fallback"
        let probe = ChatWidgetRouteReconnectProbe(
            route: GatewayCanvasHostRoute(url: oldSurface, tlsFingerprintSHA256: nil),
            replacement: GatewayCanvasHostRoute(url: newSurface, tlsFingerprintSHA256: nil),
            operatorRoute: GatewayCanvasHostRoute(url: fallbackSurface, tlsFingerprintSHA256: nil))
        let initialNode = await BotChatWidgetURLResolver.resolveResource(
            target: target,
            replacing: nil,
            currentSurfaceRoutes: { await probe.current() },
            refreshNodeSurfaceRoute: { _ in nil },
        ...
        let refreshedNode = await BotChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(refreshedNode?.url == BotChatWidgetURLResolver.resolve(surfaceURL: newSurface, target: target))

        let fallback = await BotChatWidgetURLResolver.resolveResource(
            target: target,
        ...
        #expect(fallback?.url == BotChatWidgetURLResolver.resolve(
    ...
    @Test func `refreshes the operator capability when the node route is unavailable`() async throws {
        let target = "/__bot__/canvas/documents/widget-1/index.html"
        let oldSurface = "https://operator.example/__bot__/cap/old"
        let newSurface = "https://operator.example/__bot__/cap/new"
        let failedURL = try #require(BotChatWidgetURLResolver.resolve(
            surfaceURL: oldSurface,
            target: target))
        let failedResource = BotChatWidgetResource(url: failedURL)
    ...
    @Test func `rejects a TLS pin on a cleartext widget route`() async {
        ...
        let route = GatewayCanvasHostRoute(
            url: "http://gateway.example/__bot__/cap/token",
