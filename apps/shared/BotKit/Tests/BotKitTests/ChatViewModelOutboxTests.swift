...
extension BotChatSQLiteTranscriptCache {
    fileprivate func markCommandAwaitingConfirmation(id: String) async -> BotChatOutboxUpdateResult {
    ...
    fileprivate func markCommandFailedIfPresent(
        id: String,
        retryCount: Int,
        lastError: String?) async -> BotChatOutboxUpdateResult
    {
        guard let command = await self.loadCommands().first(where: { $0.id == id }) else { return .missing }
    ...
    private func markCommandRetriedIfPresent(
        ...
        routingContract: String) async -> BotChatOutboxUpdateResult
    {
        guard let command = await self.loadCommands().first(where: { $0.id == id }) else { return .missing }
        return await self.markCommandRetriedIfPresent(
            id: id,
            expectation: BotChatOutboxRetryExpectation(
                attemptVersion: command.attemptVersion,
    ...
    fileprivate func confirmCommand(id: String) async -> BotChatOutboxUpdateResult {
        guard let command = await self.loadCommands().first(where: { $0.id == id }) else { return .missing }
...
private func makeOutboxStore(
    ...
    gatewayID: String) throws -> BotChatSQLiteTranscriptCache
{
    try BotClientDatabases(directoryURL: databaseDirectoryURL).store(gatewayID: gatewayID)
}
...
private func outboxTestCommand(id: String, text: String, createdAt: Double) -> BotChatOutboxCommand {
    BotChatOutboxCommand(
        id: id,
        sessionKey: "main",
...
private func userTexts(_ vm: BotChatViewModel) async -> [String] {
    await MainActor.run {
        vm.messages
            .filter { $0.role == "user" }
...
private final class OutboxTestTransport: @unchecked Sendable, BotChatTransport {
    let state: OutboxTransportState
    private let sessions: [BotChatSessionEntry]
    ...
    func emit(_ event: BotChatTransportEvent) {
    ...
    func listCommands(sessionKey _: String) async throws -> [BotChatCommandChoice] {
    ...
    private func requestHistory(
        sessionKey: String,
        agentID: String?,
        expectedRoute: Int?) async throws -> BotChatHistoryPayload
    {
    ...
    private func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        expectedRoute: Int?) async throws -> BotChatSendResponse
    {
        if let expectedRoute, await state.routeGeneration != expectedRoute {
        ...
        if await self.state.sendRoutingChanged {
            throw GatewayResponseError(
                ...
                details: [
                    "reason": AnyCodable(BotChatSessionRoutingContract.changedErrorReason),
                ])
        }
        if await self.state.sendRejects {
            // Gateway responded but refused to start the run.
            return BotChatSendResponse(runId: idempotencyKey, status: "error")
        }
        ...
        return BotChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
private func outboxSessionEntry(
    key: String,
    thinkingLevels: [String]) -> BotChatSessionEntry
{
    BotChatSessionEntry(
        key: key,
        kind: nil,
        ...
        model: nil,
        contextTokens: nil,
        thinkingLevels: thinkingLevels.map { BotChatThinkingLevelOption(id: $0, label: $0) })
}

private func makeOutboxViewModel(
    transport: OutboxTestTransport,
    outbox: any BotChatCommandOutbox,
    transcriptCache: (any BotChatTranscriptCache)? = nil,
    retryDelaysMs: [UInt64] = [1, 1],
    sessionKey: String = "main",
    activeAgentID: String? = "main",
    sessionRoutingContract: String? = "per-sender|main|main") async -> BotChatViewModel
{
    await MainActor.run {
...
private func sendWhileOffline(_ vm: BotChatViewModel, text: String) async throws {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
@MainActor
private func queuedStateCount(_ vm: BotChatViewModel) -> Int {
...
private actor DelayingOutbox: BotChatCommandOutbox {
    private nonisolated let base: BotChatSQLiteTranscriptCache
    ...
    init(base: BotChatSQLiteTranscriptCache) {
        self.base = base
    }

    nonisolated func changes() -> AsyncStream<BotChatOutboxChange> {
        self.base.changes()
    }
    ...
    func enqueueCommand(_ command: BotChatOutboxCommand) async -> Bool {
    ...
    func loadCommands() async -> [BotChatOutboxCommand] {
    ...
    func loadCommandsIfAvailable() async -> [BotChatOutboxCommand]? {
        if self.loadDelayNanoseconds > 0 {
            try? await Task.sleep(nanoseconds: self.loadDelayNanoseconds)
        }
    ...
    func claimNextCommand() async -> BotChatOutboxCommand? {
    ...
    func markCommandAwaitingConfirmation(id: String, attemptVersion: Int) async -> BotChatOutboxUpdateResult {
    ...
    func cancelCommand(id: String) async -> BotChatOutboxUpdateResult {
        await self.base.cancelCommand(id: id)
    }

    func confirmCommand(id: String, attemptVersion: Int) async -> BotChatOutboxUpdateResult {
        await self.base.confirmCommand(id: id, attemptVersion: attemptVersion)
    }

    func branchState(for scope: BotChatOutboxScope) async -> BotChatOutboxBranchState? {
        await self.base.branchState(for: scope)
    }

    func beginBranchSwitch(_ scope: BotChatOutboxScope) async -> Bool {
        await self.base.beginBranchSwitch(scope)
    }

    func cancelBranchSwitch(_ scope: BotChatOutboxScope) async -> Bool {
        await self.base.cancelBranchSwitch(scope)
    }

    func reconcileBranchScope(
        _ scope: BotChatOutboxScope,
        previousState: BotChatOutboxBranchState,
        activeLeafEntryID: String?,
        branchLeafEntryIDs: Set<String>,
        activeTranscriptEntryIDs: Set<String>,
        lastError: String) async -> [BotChatOutboxCommand]?
    {
    ...
    func updateLastActiveLeafEntryID(
        ...
        expectedEpoch: Int,
        for scope: BotChatOutboxScope) async -> Bool
    {
...
private actor SnapshotHoldingOutbox: BotChatCommandOutbox {
    private nonisolated let base: BotChatSQLiteTranscriptCache
...
private actor CancellationHoldingOutbox: BotChatCommandOutbox {
    private nonisolated let base: BotChatSQLiteTranscriptCache
...
struct ChatViewModelOutboxTests {
    ...
    @Test func `unsupported gateway keeps queued work and surfaces upgrade action`() async throws {
        ...
        let store = try makeOutboxStore(
            databaseDirectoryURL: databaseDirectory,
            gatewayID: "gw-test")
        let message = BotChatTransportUpgradeMessage.routingContract
    ...
    @Test func `inert outbox does not capability gate healthy live chat`() async throws {
        ...
        let transport = OutboxTestTransport(
            healthy: true,
            routeUnavailableReason: BotChatTransportUpgradeMessage.routingContract)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        ...
        parked.lastError = BotChatSQLiteTranscriptCache.outboxChangedTargetError
    ...
    @Test func `reconnect waits for canonical session metadata before flushing Ultra`() async throws {
        ...
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: BotChatOutboxScope(sessionKey: "main", agentID: "main")))
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: BotChatOutboxScope(sessionKey: "main", agentID: nil)))
        #expect(await store.enqueueCommand(BotChatOutboxCommand(
            id: "alpha-ultra",
            sessionKey: "main",
    ...
    @Test func `changed default agent parks command visibly for retry`() async throws {
        ...
        #expect(await store.loadCommands().first?.lastError ==
            BotChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `atomic gateway routing rejection parks without retrying`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == BotChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `failed alias row remains reachable after owner change`() async throws {
        ...
        #expect(await store.enqueueCommand(BotChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 1,
            lastError: BotChatSQLiteTranscriptCache.outboxExpiredError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `ownerless global retry stays failed without a selected agent`() async throws {
        ...
        #expect(await store.enqueueCommand(BotChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 0,
            lastError: BotChatSQLiteTranscriptCache.outboxUnknownTargetError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `delete race preserves a turn already proven by canonical history`() async throws {
        ...
        let canonicalMessage = BotChatMessage(
            role: "user",
    ...
    @Test func `gateway rejections burn attempts then fail terminally and support tap retry`() async throws {
        ...
        #expect(failed.retryCount == BotChatViewModel.maxOutboxSendAttempts)
    ...
    @Test func `unavailable terminal write drops health instead of advancing FIFO`() async throws {
        ...
        #expect(await store.enqueueCommand(BotChatOutboxCommand(
            ...
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: BotChatViewModel.maxOutboxSendAttempts - 1,
            lastError: "rejected")))
        let outbox = DelayingOutbox(base: store)
    ...
    @Test func `ambiguous live transport failure requires explicit retry`() async throws {
        ...
        #expect(preserved.lastError == BotChatSQLiteTranscriptCache.outboxUnconfirmedError)
    ...
    @Test func `tap retry refreshes createdAt so an expired command can resend`() async throws {
        ...
        let staleCreatedAt = Date().timeIntervalSince1970 -
            BotChatSQLiteTranscriptCache.outboxCommandMaxAge - 60
        ...
        #expect(await store.loadCommands().map(\.lastError) == [
            BotChatSQLiteTranscriptCache.outboxExpiredError,
        ])
    ...
    @Test func `background canonical alias event confirms by idempotency key`() async throws {
        ...
        transport.emit(.sessionMessage(BotSessionMessageEventPayload(
    ...
    @Test func `full queue refuses enqueue and keeps the draft`() async throws {
        ...
        let store = try makeOutboxStore(
            databaseDirectoryURL: databaseDirectory,
            gatewayID: "gw-test")
        for index in 0..<BotChatSQLiteTranscriptCache.maxQueuedCommands {
        ...
        #expect(await userTexts(vm).isEmpty)
        #expect(await store.loadCommands().count == BotChatSQLiteTranscriptCache.maxQueuedCommands)
    }
    ...
    @Test func `queued send transport failure fails closed until explicit retry`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == BotChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(command.retryCount == 0)
        #expect(await transport.state.sentIdempotencyKeys.isEmpty)
    ...
    @Test func `deleting a queued message removes bubble and durable row`() async throws {
        let databaseDirectory = try makeOutboxDatabaseDirectory()
        defer { try? FileManager.default.removeItem(at: databaseDirectory) }
        let databases = try BotClientDatabases(directoryURL: databaseDirectory)
        ...
        let staleMessage = BotChatMessage(
            role: "user",
            content: [BotChatMessageContent(
                type: "text",
...
extension ChatViewModelOutboxTests {
    ...
    @Test func `send right after a session switch still queues behind that session's backlog`() async throws {
        ...
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: BotChatOutboxScope(sessionKey: "second", agentID: nil)))
        #expect(await store.updateLastActiveLeafEntryID(
            "leaf-new",
            expectedEpoch: 0,
            for: BotChatOutboxScope(sessionKey: "second", agentID: "main")))
