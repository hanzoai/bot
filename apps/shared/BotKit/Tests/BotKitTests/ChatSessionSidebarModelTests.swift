...
struct ChatSessionSidebarModelTests {
    private func entry(
        ...
        endedAt: Double? = nil,
        hasActiveSubagentRun: Bool? = nil) -> BotChatSessionEntry
    {
        BotChatSessionEntry(
            key: key,
            kind: nil,
    ...
    @Test func `gateway groups preserve canonical session order`() {
        let sections = ChatSessionSidebarModel.sections(
            ...
            currentSessionKey: "recent",
            groups: [BotChatSessionGroup(name: "Projects", position: 0)],
            query: "")
    ...
    @Test func `session list hierarchy fields decode with gateway spellings`() throws {
        let data = try #require("""
        {
          ...
          "runtimeMs": 2000,
          "agentRuntime": {"id": "codex", "fallback": "bot", "source": "session"},
          "worktree": {"id": "wt-1", "branch": "feature/chat", "repoRoot": "/repo"}
        }
        """.data(using: .utf8))

        let entry = try JSONDecoder().decode(BotChatSessionEntry.self, from: data)
    ...
    @Test func `inspector derives gateway metadata without composer overrides`() {
        let session = BotChatSessionEntry(
            ...
            category: "Projects",
            hasActiveRun: true,
            worktree: BotChatSessionWorktree(
                id: "wt-1",
                branch: "feature/review",
                repoRoot: "/repo"),
            runtimeMs: 1500,
            agentRuntime: BotChatAgentRuntime(id: "codex", fallback: nil, source: "session"))
    ...
    @Test func `unknown session change requests an authoritative refetch`() {
        let change = BotChatSessionsChangedEvent(
    ...
    @Test func `partial session change preserves observer and status metadata`() throws {
        let agentStatus = BotChatSessionAgentStatus(
            note: "Waiting on review",
            expiresAt: 5000,
            attention: "hand")
        let observerDigest = BotChatSessionObserverDigest(
            runId: "run-1",
            revision: 3,
            updatedAt: 300,
            headline: "Reviewing",
    ...
    @Test func `subtitle precedence keeps attention and status above observer digest`() {
        let digest = BotChatSessionObserverDigest(
            ...
            headline: "Observer",
            health: "on-track")
        let agent = BotChatSessionAgentStatus(
            note: "Agent note",
            expiresAt: 10000,
            attention: nil)
