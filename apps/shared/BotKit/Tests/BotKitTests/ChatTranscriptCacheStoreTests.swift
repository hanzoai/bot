...
private func cacheSessionEntry(key: String, updatedAt: Double) -> BotChatSessionEntry {
    BotChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
...
private func messageTexts(_ messages: [BotChatMessage]) -> [String] {
...
private func outboxCommand(
    ...
    text: String,
    attachments: [BotChatOutboxAttachment] = [],
    thinking: String = "off",
    createdAt: Double = Date().timeIntervalSince1970,
    status: BotChatOutboxCommand.Status = .queued) -> BotChatOutboxCommand
{
...
struct ChatTranscriptCacheStoreTests {
    @Test func `one installation owns exactly the two named databases`() throws {
        ...
        _ = try BotClientDatabases(directoryURL: directory)
    ...
    @Test func `full removal deletes both databases legacy files and sidecars`() async throws {
        let directory = try makeDatabaseDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let databases = try BotClientDatabases(directoryURL: directory)
        ...
        try BotClientDatabases.removeDatabaseFiles(in: directory)

        for filename in [
            BotClientDatabases.gatewayCacheFilename,
            BotClientDatabases.clientStateFilename,
    ...
    @Test func `cache format mismatch rebuilds without touching client state`() async throws {
        let directory = try makeDatabaseDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let stateIdentity = try #require(BotChatSessionRoutingIdentity(
            scope: "per-sender",
            mainSessionKey: "main",
            defaultAgentID: "main"))
        do {
        ...
        let reopened = try BotClientDatabases(directoryURL: directory)
    ...
    @Test func `corrupt cache rebuilds while corrupt client state is preserved`() async throws {
        ...
        let repaired = try BotClientDatabases(directoryURL: cacheDirectory)
        ...
        #expect(throws: (any Error).self) {
            _ = try BotClientDatabases(directoryURL: stateDirectory)
        }
    ...
    @Test func `cache bounds sessions messages and transcript partitions`() async throws {
        let directory = try makeDatabaseDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try BotClientDatabases(directoryURL: directory).store(gatewayID: "gw-a")

        let sessions = (0..<(BotChatSQLiteTranscriptCache.maxCachedSessions + 10)).map {
            cacheSessionEntry(key: "s\($0)", updatedAt: Double($0))
        }
        await store.storeSessions(sessions)
        #expect(await store.loadSessions().count == BotChatSQLiteTranscriptCache.maxCachedSessions)
        #expect(await store.loadSessions().contains(where: { $0.key == "s0" }) == false)

        let messages = (0..<(BotChatSQLiteTranscriptCache.maxCachedMessagesPerSession + 20)).map {
        ...
        #expect(await store.loadTranscript(sessionKey: "bounded").count ==
            BotChatSQLiteTranscriptCache.maxCachedMessagesPerSession)
        #expect(await messageTexts(store.loadTranscript(sessionKey: "bounded")).first == "m20")

        for index in 0...BotChatSQLiteTranscriptCache.maxCachedTranscripts {
        ...
        #expect(await store.loadTranscript(
            sessionKey: "partition-\(BotChatSQLiteTranscriptCache.maxCachedTranscripts)").isEmpty == false)
    }
    ...
    @Test func `cache projection strips payloads and keeps bounded diffs`() throws {
        ...
        let cached = try #require(BotChatSQLiteTranscriptCache.cacheableMessages([message]).first)
    ...
    @Test func `staged gateway removal reconciles against the pairing registry`() async throws {
        ...
        do {
            let registered = try BotClientDatabases(
        ...
        let forgotten = try BotClientDatabases(
            directoryURL: directory,
        ...
        } == BotClientDatabases.gatewayIdentityHash("gw-a"))
    }
    ...
    @Test func `staged gateway removal uses exact registry identifier bytes`() async throws {
        ...
        let recovered = try BotClientDatabases(
            directoryURL: directory,
        ...
        #expect(try await recovered.stateQueue.read { db in
            try String.fetchOne(
                db,
                sql: "SELECT gateway_hash FROM forgotten_gateways WHERE gateway_id IS NULL")
        } == BotClientDatabases.gatewayIdentityHash(composedGatewayID))
    }
    ...
    @Test func `commit started recovery finishes even while gateway remains registered`() async throws {
        ...
        #expect(try await recovered.stateQueue.read { db in
            try Int.fetchOne(
                ...
                arguments: [BotClientDatabases.gatewayIdentityHash("gw-a")])
        } == 0)
    }
    ...
    @Test func `hash only scrub marker remains recoverable`() async throws {
        ...
        let recovered = try BotClientDatabases(directoryURL: directory)
        #expect(try await recovered.stateQueue.read { db in
            try Int.fetchOne(
                db,
                sql: "SELECT cleanup_phase FROM forgotten_gateways WHERE gateway_hash = ?",
    ...
    @Test func `gateway removal scrubs its payloads from shared database files`() async throws {
        ...
        let attachment = BotChatOutboxAttachment(
            type: "file",
...
struct ClientDatabaseLegacyImportTests {
    ...
    @Test func `legacy v2 outbox imports parked into client state`() async throws {
        ...
        #expect(commands.map(\.lastError) == [BotChatSQLiteTranscriptCache.outboxUnknownTargetError])
    ...
    @Test func `forgotten gateway is never resurrected by a late legacy import`() async throws {
        ...
        #expect(try await databases.stateQueue.read { db in
            try Int.fetchOne(
                db,
                sql: "SELECT cleanup_phase FROM forgotten_gateways WHERE gateway_hash = ?",
                arguments: [BotClientDatabases.gatewayIdentityHash("gw-forgotten")])
        } == 0)
    ...
    @Test func `forget removes an unreadable per gateway legacy database and sidecars`() throws {
        ...
        let legacyURL = BotClientDatabases.legacyPerGatewayDatabaseURL(
            gatewayID: gatewayID,
        ...
        let databases = try BotClientDatabases(
        ...
        #expect(try databases.stateQueue.read { db in
            try String.fetchOne(
                db,
                sql: "SELECT gateway_hash FROM forgotten_gateways WHERE gateway_id IS NULL")
        } == BotClientDatabases.gatewayIdentityHash(gatewayID))
        try databases.close()
        for suffix in ["", "-wal", "-shm"] {
            let url = URL(fileURLWithPath: databaseDirectory
                .appendingPathComponent(BotClientDatabases.clientStateFilename).path + suffix)
    ...
    @Test func `legacy import accepts only gateways in the pairing registry`() async throws {
        let directory = try makeDatabaseDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let keptURL = BotClientDatabases.legacyPerGatewayDatabaseURL(
            gatewayID: "gw-kept",
            directoryURL: directory)
        let orphanedURL = BotClientDatabases.legacyPerGatewayDatabaseURL(
    ...
    @Test func `unknown or corrupt legacy files remain untouched`() async throws {
        ...
        _ = try BotClientDatabases(directoryURL: unknownDirectory)
        ...
        let databases = try BotClientDatabases(directoryURL: corruptDirectory)
...
struct ChatCommandOutboxStoreTests {
    @Test func `nil agent rows use the canonical empty scope owner`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try BotClientDatabases(directoryURL: directory).store(gatewayID: "gw-a")
        let scope = BotChatOutboxScope(sessionKey: "main", agentID: nil)
    ...
    @Test func `parked accepted rows mint retry identity while queued rows keep it`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try BotClientDatabases(directoryURL: directory).store(gatewayID: "gw-a")
        let sendingScope = BotChatOutboxScope(sessionKey: "sending", agentID: "main")
        ...
        let queuedScope = BotChatOutboxScope(sessionKey: "queued", agentID: "main")
        ...
        let stickyStore = try BotClientDatabases(directoryURL: stickyDirectory).store(gatewayID: "gw-a")
        let requeuedScope = BotChatOutboxScope(sessionKey: "requeued", agentID: "main")
    ...
    @Test func `empty root reconcile permits first row and parks a wiped scope`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try BotClientDatabases(directoryURL: directory).store(gatewayID: "gw-a")
        let root = BotChatOutboxScope(sessionKey: "root", agentID: "main")
        ...
        let wiped = BotChatOutboxScope(sessionKey: "wiped", agentID: "main")
    ...
    @Test func `nonancestral parking gives failed uncertain rows a fresh retry identity`() async throws {
        ...
        let databases = try BotClientDatabases(directoryURL: directory)
        let store = databases.store(gatewayID: "gw-a")
        let scope = BotChatOutboxScope(sessionKey: "main", agentID: "main")
        #expect(await store.updateLastActiveLeafEntryID("leaf-a", expectedEpoch: 0, for: scope))
        let state = try #require(await store.branchState(for: scope))
    ...
    @Test func `interrupted sends fail closed once`() async throws {
        ...
        #expect(recovered.lastError == BotChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(await store.recoverInterruptedSends())
    }
    ...
    @Test func `stale queued and acknowledged commands require user action`() async throws {
        ...
        let databases = try BotClientDatabases(directoryURL: directory)
        let store = databases.store(gatewayID: "gw-a")
        let old = Date().timeIntervalSince1970 - BotChatSQLiteTranscriptCache.outboxCommandMaxAge - 1
        ...
        #expect(commandsByID["old-queued"]?.lastError == BotChatSQLiteTranscriptCache.outboxExpiredError)
        #expect(commandsByID["old-ack"]?.status == .failed)
        #expect(commandsByID["old-ack"]?.lastError == BotChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }
    ...
    @Test func `queue and attachment budgets are gateway scoped`() async throws {
        ...
        let oversized = BotChatOutboxAttachment(
            type: "file",
            mimeType: "application/octet-stream",
            fileName: "large.bin",
            data: Data(count: BotChatSQLiteTranscriptCache.maxAttachmentBytesPerCommand + 1))
        #expect(await storeB.enqueueCommand(outboxCommand(
            id: "too-large",
            text: "large",
            attachments: [oversized])) == false)
        #expect(BotChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
            commandBytes: 1,
            queuedBytes: BotChatSQLiteTranscriptCache.maxQueuedAttachmentBytes - 1))
        #expect(!BotChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
            commandBytes: 2,
            queuedBytes: BotChatSQLiteTranscriptCache.maxQueuedAttachmentBytes - 1))
    }
