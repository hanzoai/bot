...
private func cachedMessage(role: String, text: String, timestamp: Double) -> BotChatMessage {
    BotChatMessage(
        role: role,
        content: [
...
private func visibleTexts(_ vm: BotChatViewModel) async -> [String] {
    await MainActor.run { vm.messages.map { $0.content.compactMap(\.text).joined() } }
}

private func cachedSessionEntry(key: String, updatedAt: Double) -> BotChatSessionEntry {
    BotChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
...
private actor TestTranscriptCache: BotChatTranscriptCache {
    private var transcripts: [String: [BotChatMessage]]
    private var sessions: [BotChatSessionEntry]
    private let loadSessionsHook: (@Sendable () async -> Void)?
    private(set) var storedTranscriptSessionKeys: [String] = []
    private(set) var storedTranscripts: [[BotChatMessage]] = []
    private(set) var storedSessionsCallCount = 0

    init(
        transcripts: [String: [BotChatMessage]] = [:],
...
private final class GatedHistoryChatTransport: @unchecked Sendable, BotChatTransport {
    private let historyGate: AsyncStream<Void>.Continuation?
    private let historyGateStream: AsyncStream<Void>?
    private let historyResult: @Sendable (String, Int) throws -> BotChatHistoryPayload
    ...
    init(
        gated: Bool,
        historyResult: @escaping @Sendable (String, Int) throws -> BotChatHistoryPayload)
    {
        ...
        var eventCont: AsyncStream<BotChatTransportEvent>.Continuation!
    ...
    {
        BotChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
struct ChatViewModelTranscriptCacheTests {
    @Test func `cold open paints cached transcript then live history replaces it`() async throws {
        ...
        let vm = await MainActor.run {
            BotChatViewModel(sessionKey: "main", transport: transport, transcriptCache: cache)
        }
    ...
    @Test func `prior agent cache cannot repaint global digest after owner switch`() async throws {
        ...
        global.observerDigest = BotChatSessionObserverDigest(
            agentId: "main",
            runId: "run-main",
            revision: 3,
            updatedAt: 1000,
