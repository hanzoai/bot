...
private final class HapticRecorder: @unchecked Sendable {
    ...
    private var recordedEvents: [BotChatHaptics.Event] = []

    var events: [BotChatHaptics.Event] {
        self.lock.lock()
        defer { self.lock.unlock() }
        return self.recordedEvents
    }

    func record(_ event: BotChatHaptics.Event) {
        self.lock.lock()
        defer { self.lock.unlock() }
...
private final class HapticsTestTransport: @unchecked Sendable, BotChatTransport {
    private let response: BotChatSendResponse
    ...
    init(status: String, historyMessages: @escaping @Sendable () -> [AnyCodable] = { [] }) {
        self.response = BotChatSendResponse(runId: "run-1", status: status)
        self.historyMessages = historyMessages
        var continuation: AsyncStream<BotChatTransportEvent>.Continuation!
    ...
    HapticRecorder)
{
    ...
    let haptics = BotChatHaptics(performer: recorder.record)
    let viewModel = await MainActor.run {
        BotChatViewModel(sessionKey: "main", transport: transport, haptics: haptics)
    }
    return (transport, viewModel, recorder)
}

private func sendHapticsTestMessage(_ viewModel: BotChatViewModel) async {
    await MainActor.run {
...
struct ChatHapticsTests {
    ...
    @Test func `completion fires once for duplicate terminal events`() async throws {
        ...
        let final = BotChatTransportEvent.chat(BotChatEventPayload(
            runId: "run-1",
            sessionKey: "main",
            state: "final",
            message: nil,
