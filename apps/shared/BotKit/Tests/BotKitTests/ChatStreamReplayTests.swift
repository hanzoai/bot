...
/// Replays scripted gateway traffic against `BotChatViewModel` with deterministic
...
private final class ScriptedChatTransport: @unchecked Sendable, BotChatTransport {
    private actor State {
        var history: BotChatHistoryPayload
        var historyRequestCount = 0
        var sentRunIds: [String] = []

        init(history: BotChatHistoryPayload) {
            self.history = history
        }

        func setHistory(_ payload: BotChatHistoryPayload) {
            self.history = payload
        }

        func recordHistoryRequest() -> BotChatHistoryPayload {
    ...
    func setHistory(_ payload: BotChatHistoryPayload) async {
    ...
    // MARK: BotChatTransport

    func requestHistory(sessionKey _: String) async throws -> BotChatHistoryPayload {
        ...
        attachments _: [BotChatAttachmentPayload]) async throws -> BotChatSendResponse
    {
        ...
        return BotChatSendResponse(runId: idempotencyKey, status: "pending")
    }
    ...
    {
        BotChatSessionsListResponse(ts: nil, path: nil, count: 0, defaults: nil, sessions: [])
    }
...
private func replayHistory(
    ...
    messages: [AnyCodable] = []) -> BotChatHistoryPayload
{
...
/// `__bot.idempotencyKey` metadata used for turn correlation.
...
private func replaySessionMessageEvent(
    ...
    messageId: String) -> BotChatTransportEvent
{
    .sessionMessage(
...
private func replayFinalEvent(
    runId: String,
    text: String,
    timestamp: Double) -> BotChatTransportEvent
{
    .chat(
        BotChatEventPayload(
            runId: runId,
...
private struct StreamReplayHarness {
    ...
    static func bootstrapped(
        initialHistory: BotChatHistoryPayload = replayHistory()) async throws -> StreamReplayHarness
    {
    ...
    func converge(
        _ label: String,
        _ condition: @escaping @MainActor @Sendable (BotChatViewModel) -> Bool) async throws
    {
        let vm = self.vm
...
extension BotChatViewModel {
    fileprivate var replayAssistantRows: [BotChatMessage] {
        self.messages.filter { $0.role == "assistant" }
    }

    fileprivate func replayAssistantRows(text: String) -> [BotChatMessage] {
        self.replayAssistantRows.filter { message in
            message.content.compactMap(\.text).joined() == text
        }
    }

    fileprivate var replayUserRows: [BotChatMessage] {
