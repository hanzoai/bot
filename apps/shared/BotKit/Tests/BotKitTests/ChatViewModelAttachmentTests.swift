...
private actor AttachmentSendCapture {
    private(set) var attachments: [BotChatAttachmentPayload] = []

    func store(_ attachments: [BotChatAttachmentPayload]) {
    ...
    func first() -> BotChatAttachmentPayload? {
...
private struct AttachmentProcessingTransport: BotChatTransport {
        ...
        attachments: [BotChatAttachmentPayload]) async throws -> BotChatSendResponse
    {
        ...
        return BotChatSendResponse(runId: idempotencyKey, status: self.responseStatus)
    }
...
private func makeAttachmentOutbox() throws -> BotChatSQLiteTranscriptCache {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("attachment-outbox-\(UUID().uuidString)", isDirectory: true)
    return try BotClientDatabases(directoryURL: directory).store(gatewayID: "attachment-tests")
}

@MainActor
private func makeDurableAttachmentViewModel(
    transport: AttachmentProcessingTransport,
    outbox: BotChatSQLiteTranscriptCache) -> BotChatViewModel
{
    BotChatViewModel(
        sessionKey: "main",
        transport: transport,
...
final class ChatViewModelAttachmentTests: XCTestCase {
    func testImageAttachmentsAreProcessedBeforeStaging() async throws {
        ...
        let viewModel = await MainActor.run {
            BotChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testPartialIdentitySyncPreservesTheOtherDeferredComponent() async {
        let state = await MainActor.run {
            ...
            let contractViewModel = BotChatViewModel(
                sessionKey: "main",
                transport: AttachmentProcessingTransport(),
                activeAgentId: "main",
                sessionRoutingContract: oldContract)
            let contractAttachment = BotPendingAttachment(
                url: nil,
            ...
            let agentViewModel = BotChatViewModel(
                sessionKey: "main",
                transport: AttachmentProcessingTransport(),
                activeAgentId: "main",
                sessionRoutingContract: oldContract)
            let agentAttachment = BotPendingAttachment(
                url: nil,
    ...
    func testFailedAttachmentSendWithoutOutboxRestoresDraft() async throws {
        ...
        let attachmentID = await MainActor.run {
            let attachment = BotPendingAttachment(
                url: nil,
                data: attachmentData,
                fileName: "retry.m4a",
                mimeType: "audio/mp4",
    ...
    func testVoiceNoteSendKeepsCapturedDurationWhenDraftChangesDuringHealthCheck() async throws {
        ...
        let (viewModel, draftAttachmentID) = await MainActor.run {
            ...
            let draftAttachment = BotPendingAttachment(
                url: nil,
    ...
    func testAmbiguousVoiceNoteSurvivesViewModelRecreation() async throws {
        let outbox = try makeAttachmentOutbox()
        var firstViewModel: BotChatViewModel? = await MainActor.run {
            let viewModel = makeDurableAttachmentViewModel(
    ...
    func testCanonicalVoiceNoteConfirmationPreservesDurationAndDeletesDurableBytes() async throws {
        ...
        let canonical = try JSONDecoder().decode(
            ...
            from: Data(
                """
                {"role":"user","content":"See attached.","__bot":{"idempotencyKey":"\(command
    ...
    func testCanonicalVoiceNotePreservesOptimisticDuration() throws {
        let localAudio = BotChatMessageContent(
            type: "file",
            text: nil,
            mimeType: "audio/mp4",
        ...
        let incoming = try JSONDecoder().decode(
            BotChatMessage.self,
            from: Data(
                #"{"role":"user","content":"See attached.","__bot":{"idempotencyKey":"run:user"},"MediaPaths":["media/inbound/media-1.m4a"],"MediaTypes":["audio/mp4"]}"#
                    .utf8))
