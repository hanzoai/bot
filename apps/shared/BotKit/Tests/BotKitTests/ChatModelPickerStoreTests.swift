...
private func pickerModel(_ selectionID: String) -> BotChatModelChoice {
...
@Suite struct ChatModelPickerStoreTests {
    ...
    @Test func `recents dedupe move to front cap and skip invalid ids`() throws {
        try withPickerStore { store, _ in
            store.recordRecent("")
            store.recordRecent(BotChatViewModel.defaultModelSelectionID)
    ...
    @Test func `missing default provider does not mark other as default`() {
        let unqualified = BotChatModelChoice(
            modelID: "local-model",
            name: "Local",
            provider: "",
            contextWindow: nil)
    ...
    @Test func `qualified global default matches provider model pair`() {
        ...
        let qualifiedModelID = BotChatModelChoice(
            modelID: "openai/gpt",
            name: "GPT",
            provider: "openai",
            contextWindow: nil)
