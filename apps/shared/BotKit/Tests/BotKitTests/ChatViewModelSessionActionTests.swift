...
private func sessionActionOutboxCommand(
    id: String,
    text: String) -> BotChatOutboxCommand
{
...
private final class SessionActionTransport: @unchecked Sendable, BotChatTransport {
    ...
    private let rewindEditorAttachments: [BotChatEditorAttachment]?
    private let forkAtMessageSessionKey: String
    private let forkAtMessageEditorText: String?
    private let forkAtMessageEditorAttachments: [BotChatEditorAttachment]?
    private let branches: [BotChatSessionBranch]
    private let branchListResponses: [[BotChatSessionBranch]]
    ...
    init(
        ...
        rewindEditorAttachments: [BotChatEditorAttachment]? = nil,
        forkAtMessageSessionKey: String = "forked-at-message",
        forkAtMessageEditorText: String? = "forked draft",
        forkAtMessageEditorAttachments: [BotChatEditorAttachment]? = nil,
        branches: [BotChatSessionBranch] = [],
        branchListResponses: [[BotChatSessionBranch]] = [],
        ...
        entryId: String) async throws -> BotChatForkAtMessageResponse
    {
        ...
        return BotChatForkAtMessageResponse(
        ...
        agentID _: String?) async throws -> BotChatSessionBranchesResponse
    {
        ...
        let branches = self.branchListResponses.indices.contains(callIndex)
            ...
            : self.branches
        return BotChatSessionBranchesResponse(branches: branches)
    }
    ...
    func acquireSessionGroupsRouteLease() async -> BotChatSessionGroupsRouteLease? {
        ...
        return BotChatSessionGroupsRouteLease(
            listGroups: {
                BotChatSessionGroupsResponse(groups: [
                    BotChatSessionGroup(name: "Existing", position: 0),
                ])
            },
            putGroups: { names in
                await state.recordGroupPut(names)
                return BotChatSessionGroupsMutationResponse(
                    ok: true,
                    groups: names.enumerated().map {
                        BotChatSessionGroup(name: $0.element, position: $0.offset)
                    },
                    updatedSessions: nil)
            },
            renameGroup: { _, _ in
                BotChatSessionGroupsMutationResponse(ok: true, groups: [], updatedSessions: nil)
            },
            deleteGroup: { _ in
                BotChatSessionGroupsMutationResponse(ok: true, groups: [], updatedSessions: nil)
            })
    ...
    func acquireNewSessionRouteLease() async -> BotChatNewSessionRouteLease? {
        ...
        return BotChatNewSessionRouteLease(
            listAgents: {
                BotChatAgentsListResponse(
                    defaultId: "worker",
                    agents: [BotChatAgentChoice(id: "worker", workspaceGit: true)])
            },
            createSession: { key, _, agentID, parentKey, _, _ in
                await state.recordCreate(agentID: agentID, parentKey: parentKey)
                return BotChatCreateSessionResponse(ok: true, key: key, sessionId: nil)
            })
    }
...
struct ChatViewModelSessionActionTests {
    ...
    @Test func `remote group mutations bump the catalog revision`() async {
        ...
        let viewModel = await MainActor.run {
            BotChatViewModel(sessionKey: "main", transport: transport)
        }
    ...
    @Test func `batch delete rejects current session while attachment owner is pinned`() async {
        let transport = SessionActionTransport()
        let viewModel = BotChatViewModel(sessionKey: "worker", transport: transport)
        viewModel.attachments = [BotPendingAttachment(
            url: nil,
            data: Data([1]),
            fileName: "draft.png",
            mimeType: "image/png",
    ...
    @Test func `rewind seeds editor and refreshes history`() async {
        ...
        let transport = SessionActionTransport(
            ...
            rewindEditorAttachments: [
                BotChatEditorAttachment(
                    mimeType: "image/png",
                    data: imageData.base64EncodedString()),
                BotChatEditorAttachment(mimeType: "image/png", data: "%%%"),
            ])
        let viewModel = BotChatViewModel(sessionKey: "main", transport: transport)
    ...
    @Test func `fork at message waits for current session outbox confirmation`() async throws {
        let directory = try makeSessionActionOutboxDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try BotClientDatabases(directoryURL: directory).store(gatewayID: "gw-test")
        #expect(await store.enqueueCommand(sessionActionOutboxCommand(
            id: "fork-pending",
    ...
    @Test func `rewind bumps branch epoch and parks a racing enqueue`() async throws {
        ...
        #expect(BotChatSQLiteTranscriptCache.outboxDisplayError(racedCommand.lastError) ==
    ...
    @Test func `branch message count uses localized singular and plural forms`() {
        #expect(BotChatComposer.branchMessageCount(1) == "1 message")
        #expect(BotChatComposer.branchMessageCount(2) == "2 messages")
    }
    ...
    @Test func `fork at message switches and seeds editor`() async {
        ...
        let transport = SessionActionTransport(
            ...
            forkAtMessageEditorAttachments: [BotChatEditorAttachment(
                mimeType: "image/webp",
                data: imageData.base64EncodedString())])
        let viewModel = BotChatViewModel(sessionKey: "main", transport: transport)
    ...
    private func userMessage(entryID: String) -> BotChatMessage {
        BotChatMessage(
            role: "user",
            content: [],
            timestamp: nil,
    ...
    private func confirmingMessage(commandID: String) -> BotChatMessage {
        BotChatMessage(
            role: "user",
            content: [],
            timestamp: nil,
    ...
    private func branches(activeLeafEntryID: String = "leaf-active") -> [BotChatSessionBranch] {
        [
            BotChatSessionBranch(
    ...
    private func entry(key: String) -> BotChatSessionEntry {
        BotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
