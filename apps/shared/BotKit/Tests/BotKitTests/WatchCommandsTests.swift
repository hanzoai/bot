...
struct WatchCommandsTests {
    @Test func `app snapshot dual writes semantic and legacy status fields`() throws {
        let message = BotWatchAppSnapshotMessage(
            ...
            chatStatus: BotWatchAppStatus(code: .chatNoMessages),
    ...
    @Test func `semantic chat status always writes the shipped text field`() throws {
        let message = BotWatchAppSnapshotMessage(
            ...
            talkSpeaking: false,
            pendingApprovalCount: 0,
            chatStatus: BotWatchAppStatus(code: .chatUnavailable))
    ...
    @Test func `unknown semantic statuses fall back to legacy text`() throws {
        ...
        let message = try JSONDecoder().decode(
            BotWatchAppSnapshotMessage.self,
            from: Data(json.utf8))

        #expect(message.gatewayStatus == BotWatchAppStatus(
            code: .legacy,
            verbatim: "Future gateway state"))
        #expect(message.talkStatus == BotWatchAppStatus(
            code: .legacy,
            verbatim: "Future Talk state"))
        #expect(message.chatStatus == BotWatchAppStatus(
            code: .legacy,
