...
struct ChatMessageMediaAttachmentTests {
    ...
    @Test @MainActor func `distinct images never reconcile as the same final message`() {
        ...
        #expect(
            BotChatViewModel.finalMessageContentFingerprint(for: first) !=
                BotChatViewModel.finalMessageContentFingerprint(for: second))
        #expect(
            BotChatViewModel.messageContentFingerprint(for: first) !=
                BotChatViewModel.messageContentFingerprint(for: second))
    }
    ...
    private static func message(artifactId: String) -> BotChatMessage {
        BotChatMessage(
            role: "assistant",
            content: [
