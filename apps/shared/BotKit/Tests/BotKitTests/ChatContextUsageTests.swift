...
private final class ContextUsageTestTransport: @unchecked Sendable, BotChatTransport {
...
struct ChatContextUsageTests {
    private func message(
        ...
        usage: BotChatUsage? = nil) -> BotChatMessage
    {
        BotChatMessage(
            id: UUID(),
            role: role,
    ...
    private func usage(
        ...
        cacheWrite: Int? = nil,
        total: Int? = nil,
        costTotal: Double? = nil) throws -> BotChatUsage
    {
        ...
        return try JSONDecoder().decode(BotChatUsage.self, from: data)
    }
    ...
    @Test @MainActor func `view model resolves context totals through a selected global alias`() {
        let vm = BotChatViewModel(
            ...
            activeAgentId: "ops")
        vm.sessions = [BotChatSessionEntry(
