...
struct ChatMarkdownBlockSegmenterTests {
    ...
    @Test @MainActor func `list recursion preserves inline math typography`() throws {
        ...
        let renderer = ChatMarkdownRenderer(
            ...
            context: .assistant,
            variant: .standard,
            font: BotChatTypography.callout.italic(),
    ...
    @Test @MainActor func `native list items reject compressed multiline heights`() {
        let samples = [
            "The issue is not posted by an official Bot organization account or a separate assistant account, so the complete author identity guidance must remain visible to the reader.",
