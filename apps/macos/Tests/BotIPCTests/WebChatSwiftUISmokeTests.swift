...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: BotChatTransport {
        func requestHistory(sessionKey: String) async throws -> BotChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(BotChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [BotChatAttachmentPayload]) async throws -> BotChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(BotChatSendResponse.self, from: Data(json.utf8))
        }
    ...
    @Test func `window controller merges titlebar and keeps toolbar controls`() throws {
        let traceKeys = [
            BotChatWindowShell.assistantTraceDefaultsKey,
        ...
        #expect(window.title == "Studio — Bot")
        window.title = "main"
        #expect(window.title == "Studio — Bot")
    ...
    @Test func `controller explicit agent wins and nil falls back to cached default`() throws {
        let cachedIdentity = try #require(BotChatSessionRoutingIdentity(
            scope: "global",
            mainSessionKey: "main",
            defaultAgentID: "main"))
    ...
    @Test func `max and Ultra thinking preferences survive reopen`() throws {
        ...
        for level in ["max", "ultra"] {
            defaults.set(level, forKey: "bot.webchat.thinkingLevel")
    ...
    @Test func `verbosity preference survives reopen`() throws {
        ...
        defaults.set("full", forKey: "bot.webchat.verboseLevel")
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == "full")
        defaults.set("invalid", forKey: "bot.webchat.verboseLevel")
    ...
    @Test func `inherited verbosity preference clears persisted override`() throws {
        ...
        #expect(WebChatSwiftUIWindowController.persistedVerboseLevel(defaults: defaults) == nil)
        #expect(defaults.object(forKey: "bot.webchat.verboseLevel") == nil)
    }
}
