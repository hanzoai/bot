...
struct MacNodeModeCoordinatorTests {
    ...
    @Test @MainActor func `config and CLI changes restart startup scoped node host worker`() async throws {
        ...
        notificationCenter.post(name: .botConfigDidChange, object: nil)

        try await self.waitUntil("node-host worker restart", timeout: restartTimeout) {
            await worker.stops() == 1
        }

        notificationCenter.post(name: .botCLIInstalled, object: nil)
    ...
    @Test @MainActor func `terminal worker failure is reported instead of scheduling another restart`() async throws {
        ...
        try coordinator.prepareNodeHostWorkerRetryForTesting(
            command: ["/usr/local/bin/bot", "node", "worker"])
        await confirmation("terminal worker failure") { confirmed in
            let observer = notificationCenter.addObserver(
                forName: .botNodeHostWorkerRetryExhausted,
                object: coordinator,
                queue: nil)
            { notification in
    ...
    @Test @MainActor func `worker cannot restart before its crash backoff expires`() async throws {
        ...
        let command = ["/usr/local/bin/bot", "node", "worker"]
    ...
    @Test func `native manifest excludes CLI-owned node commands`() {
        ...
        #expect(!caps.contains(BotCapability.browser.rawValue))
        #expect(!commands.contains(BotBrowserCommand.proxy.rawValue))
        #expect(commands.contains(BotCanvasCommand.present.rawValue))
        #expect(commands.contains(BotSystemCommand.notify.rawValue))
        #expect(!commands.contains(BotFileSystemCommand.listDir.rawValue))
    ...
    @Test func `Codex supervision activation respects the plugin flag and global policy`() {
        ...
        #expect(BotConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
        ...
        #expect(BotConfigFile.configuredBundledPluginAllowed(
            "codex",
        ...
        #expect(BotConfigFile.pluginEntry("codex", root: ambiguousEntryAliases) == nil)
        #expect(!BotConfigFile.explicitlyEnabledPluginConfigFlag(
            "codex",
            path: ["supervision", "enabled"],
    ...
    @Test func `computer control cap gates the computer.act command`() {
        ...
        #expect(enabledCaps.contains(BotCapability.computer.rawValue))
        #expect(enabledCommands.contains(BotComputerCommand.act.rawValue))
        ...
        #expect(!disabledCaps.contains(BotCapability.computer.rawValue))
        #expect(!disabledCommands.contains(BotComputerCommand.act.rawValue))
    }
