...
struct ComputerActionServiceTests {
    ...
    private final class ActionProbe {
            ...
            lifecycleGeneration: UInt64) async throws -> BotComputerActResult
        {
            ...
            return BotComputerActResult(ok: true, cursorX: Double(actionID), cursorY: 0)
        }
    }
    ...
    private final class ActionTaskBox {
        var task: Task<BotComputerActResult, Error>?
    }
    ...
    @Test func `coordinate input requires the current display frame identity`() throws {
        ...
        let missing = BotComputerActParams(
            action: .leftClick,
            x: 1,
            y: 2,
            refWidth: 1280)
        ...
        let stale = BotComputerActParams(
            action: .leftClick,
        ...
        let current = BotComputerActParams(
            action: .leftClick,
        ...
        let missingScale = BotComputerActParams(
            action: .leftClick,
            displayFrameId: currentFrameId,
            x: 1,
            y: 2)
        ...
        let display = BotComputerDisplayGeometry(
            originX: 0,
            originY: 0,
            widthPoints: 1280,
            heightPoints: 800)
        let screenshotFrameId = BotComputerInputGeometry.displayFrameId(
            displayID: 1,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 1280,
            display: display)
        let mismatchedScaleFrameId = BotComputerInputGeometry.displayFrameId(
            displayID: 1,
            sourceWidth: 2560,
            sourceHeight: 1600,
            referenceWidth: 640,
            display: display)
        let mismatchedScale = BotComputerActParams(
            action: .leftClick,
        ...
        let keyboard = BotComputerActParams(action: .type, text: "hello")
    ...
    @Test func `computer actions execute in FIFO order without overlap`() async throws {
        ...
        let firstParams = BotComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let secondParams = BotComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `cancelled queued action never executes`() async throws {
        ...
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: { true })
        let firstParams = BotComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let cancelledParams = BotComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `cancelled active action releases held input before it settles`() async {
        ...
        let params = BotComputerActParams(action: .leftMouseDown, x: 1, y: 0, refWidth: 1280)
    ...
    @Test func `cancellation retries failed release before its main actor hop`() async {
        ...
        let action = Task { @MainActor in
            try await queue.perform(params, lifecycleGeneration: 0) { _, _ in
                taskBox.task?.cancel()
                return BotComputerActResult(ok: true, cursorX: 1, cursorY: 0)
            }
        }
        taskBox.task = action
    ...
    @Test func `new lifecycle generation cancels old work before fresh action`() async throws {
        ...
        let releaseProbe = LifecycleReleaseProbe(allowed: true)
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: releaseProbe.attempt)
        let oldParams = BotComputerActParams(action: .leftClick, x: 1, y: 0, refWidth: 1280)
        let freshParams = BotComputerActParams(action: .leftClick, x: 2, y: 0, refWidth: 1280)
    ...
    @Test func `failed lifecycle mouse up blocks newer generation until retry succeeds`() async throws {
        ...
        let queue = ComputerActionExecutionQueue(onLifecycleRelease: releaseProbe.attempt)
        let params = BotComputerActParams(action: .type, x: 2, y: 0, refWidth: 1280)
