...
struct GatewayLaunchAgentManagerTests {
    @Test func `reads Gateway service ownership command directly from launchd`() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("bot-gateway-\(UUID().uuidString).plist")
    ...
    @Test func `attach only runtime override blocks gateway launch agent writes`() async throws {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("bot-attach-only-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `unintercepted daemon commands fail closed during tests`() async {
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("bot-no-disable-marker-\(UUID().uuidString)")
        defer {
            GatewayLaunchAgentManager.setTestingDisableLaunchAgentMarkerURL(nil)
            GatewayLaunchAgentManager.setTestingInterceptDaemonCommands(false)
        }
    ...
    @Test func `launch agent plist snapshot parses args and env`() throws {
        let url = FileManager().temporaryDirectory
            .appendingPathComponent("bot-launchd-\(UUID().uuidString).plist")
        let plist: [String: Any] = [
            "ProgramArguments": ["bot", "gateway", "--port", "18789", "--bind", "loopback"],
    ...
    @Test func `launch agent plist snapshot merges canonical generated environment`() throws {
        let directory = FileManager().temporaryDirectory
            .appendingPathComponent("bot-launchd-env-\(UUID().uuidString)", isDirectory: true)
        let plistURL = directory.appendingPathComponent("ai.bot.gateway.plist")
        let environmentFileURL = directory.appendingPathComponent("ai.bot.gateway.env")
        let wrapperURL = directory.appendingPathComponent("ai.bot.gateway-env-wrapper.sh")
        ...
        export BOT_GATEWAY_PASSWORD='service'\\''pass'
        export BOT_GATEWAY_TOKEN=' service-token '
        ...
        let plist: [String: Any] = [
            ...
            "EnvironmentVariables": ["BOT_GATEWAY_TOKEN": "stale-inline-token"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
    ...
    @Test func `launch agent plist snapshot ignores unreferenced generated environment`() throws {
        ...
        try FileManager().createDirectory(at: directory, withIntermediateDirectories: true)
        try "#!/bin/sh\n".write(to: wrapperURL, atomically: true, encoding: .utf8)
        try "export BOT_GATEWAY_TOKEN='unreferenced-token'\n"
            .write(to: environmentFileURL, atomically: true, encoding: .utf8)
        let plist: [String: Any] = [
            "ProgramArguments": ["bot", "gateway"],
        ]
        let data = try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
        try data.write(to: plistURL, options: [.atomic])
        defer { try? FileManager().removeItem(at: directory) }
