...
struct ApplicationRelocatorTests {
    ...
    func `stable application locations continue normally`() {
        let paths = [
            ...
            "/Users/tester/Applications/Bot.app",
            "/Users/tester/Tools/Bot.app",
            "/Volumes/External/Apps/Bot.app",
        ]
        for path in paths {
    ...
    func `debug and test builds never relocate`() {
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Bot.app",
                debugOrTesting: true
            )
        )
    ...
    func `transient copy offers system installation when available`() {
        let destination = URL(fileURLWithPath: "/Applications/Bot.app")
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Bot.app",
    ...
    func `translocated copy offers installation`() {
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/private/var/folders/x/AppTranslocation/y/d/Bot.app",
                candidates: [missing(destination, writable: true)]
            )
        )
    ...
    func `different same named app is never replaced`() {
        let systemDestination = URL(fileURLWithPath: "/Applications/Bot.app")
        let userDestination = home.appendingPathComponent("Applications/Bot.app")
        ...
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Desktop/Bot.app",
                candidates: [
    ...
    func `unwritable destinations require manual installation`() {
        let recommendation = ApplicationRelocator.recommendation(
            for: environment(
                path: "/Users/tester/Downloads/Bot.app",
                candidates: [
                    missing(URL(fileURLWithPath: "/Applications/Bot.app"), writable: false),
                    missing(home.appendingPathComponent("Applications/Bot.app"), writable: false),
                ]
            )
        )
    ...
    func `unauthenticated replacement marker cannot bypass duplicate launch rejection`() {
        let forgedHandoff = [
            "BOT_REPLACEMENT_SOURCE_BUNDLE": "/Applications/Bot.app",
            "BOT_REPLACEMENT_PARENT_PID": "1",
            "BOT_REPLACEMENT_CODE_HASH": "ZmFrZQ==",
            "BOT_REPLACEMENT_READY_FD": "19",
        ]
        ...
        #expect(!ApplicationRelocator.acceptReplacementHandoff(environment: [
            "BOT_REPLACEMENT_SOURCE_BUNDLE": "relative/Bot.app",
        ]))
    }
    ...
    func `replacement requirement preserves Developer ID certificate class`() {
        ...
        #expect(ApplicationRelocator.developerIDRequirementString(
            bundleIdentifier: "ai.bot.mac\" or true",
            teamIdentifier: "Y5PE65HELJ"
        ) == nil)
    }
    ...
    func `bundle file reference stays bound after canonical path replacement`() throws {
        ...
        let canonicalBundle = root.appendingPathComponent("Bot.app", isDirectory: true)
        let archivedBundle = root.appendingPathComponent("Bot.previous.app", isDirectory: true)
        let executableRelativePath = "Contents/MacOS/Bot"
    ...
    func `replacement handoff launches the bound executable without filesystem locks`() {
        let reference = ApplicationRelocator.BundleFileReference(
            deviceIdentifier: 42,
            fileIdentifier: 99,
            executableRelativePath: "Contents/MacOS/Bot"
        )

        #expect(reference.bundleURL.path == "/.vol/42/99")
        #expect(reference.executableURL.path == "/.vol/42/99/Contents/MacOS/Bot")
    }
    ...
    func `verified KeepAlive launchd supervisor retains relaunch ownership`() throws {
        ...
        let executable = root.appendingPathComponent("Bot.app/Contents/MacOS/Bot")
        let serviceName = "ai.bot.mac.test-node"
        ...
        #expect(ApplicationRelocator.relaunchStrategy(
            xpcServiceName: "application.ai.bot.mac.123",
            executableURL: executable,
            homeDirectory: home
        ) == .openAfterTermination)
        #expect(ApplicationRelocator.relaunchStrategy(
    ...
    func `replacement metadata is read from disk instead of Bundle cache`() throws {
        ...
        let bundleURL = root.appendingPathComponent("Bot.app")
        ...
        #expect(ApplicationRelocator.applicationOnDisk(at: bundleURL) == .init(
            bundleIdentifier: "ai.bot.mac",
            executableURL: contentsURL.appendingPathComponent("MacOS/Bot")
        ))

        try writeInfoPlist(
            at: contentsURL.appendingPathComponent("Info.plist"),
            bundleIdentifier: "ai.bot.mac.replaced",
            executable: "BotNext"
        )
        #expect(ApplicationRelocator.applicationOnDisk(at: bundleURL) == .init(
            bundleIdentifier: "ai.bot.mac.replaced",
            executableURL: contentsURL.appendingPathComponent("MacOS/BotNext")
        ))
    }
