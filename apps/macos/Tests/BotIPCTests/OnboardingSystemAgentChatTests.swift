...
private func systemAgentSessionID(from message: URLSessionWebSocketTask.Message) -> String? {
    ...
    guard let data,
          ...
          object["method"] as? String == "bot.chat",
...
struct OnboardingSystemAgentChatTests {
    @Test func `onboarding wires Bot agent handoff`() {
    ...
    @Test func `relaunch with pending inference resumes Bot`() async throws {
    ...
    @Test func `superseded resume cannot present a replacement route chat`() async throws {
        ...
        let session = GatewayTestWebSocketSession(taskFactory: {
            GatewayTestWebSocketTask(sendHook: { task, message, sendIndex in
                ...
                await methods.record(method)
                if respondToSystemAgentHealth(task: task, id: id, method: method) { return }
                guard method == "bot.setup.verify" else { return }
        ...
        for _ in 0..<200 {
            if await methods.snapshot() == ["health", "bot.setup.verify"] {
                break
            }
            try? await Task.sleep(nanoseconds: 5_000_000)
        }
    ...
    @Test func `fresh inference presents and starts Bot immediately`() async throws {
        let session = GatewayTestWebSocketSession(taskFactory: {
            GatewayTestWebSocketTask(sendHook: { task, message, sendIndex in
                guard sendIndex > 0,
                      let id = GatewayWebSocketTestSupport.requestID(from: message)
    ...
    @Test func `chat session stays bound to its original gateway route`() async throws {
        ...
        #expect(chat.errorMessage == "The Gateway connection changed. Restart Bot to reconnect.")
    ...
    @Test func `cancelled initial request exposes restart and recovers`() async throws {
        ...
        #expect(chat.errorMessage == "Bot was interrupted. Restart to try again.")
