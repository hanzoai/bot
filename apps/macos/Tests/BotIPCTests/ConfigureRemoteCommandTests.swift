...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("bot-configure-remote-\(UUID().uuidString).json")
        ...
        try await TestIsolation.withIsolatedState(env: ["BOT_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "bot.connectionMode") == "remote")
                #expect(defaults.string(forKey: "bot.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "bot.onboardingSeen") == true)
                #expect(defaults.string(forKey: "bot.remoteCliPath") == "/opt/homebrew/bin/bot")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("bot-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote defaults SSH host key policy to strict`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("bot-configure-remote-strict-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("bot-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote writes state dir config when config path is unset`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("bot-configure-state-\(UUID().uuidString)", isDirectory: true)
        let configURL = stateDir.appendingPathComponent("bot.json")
        ...
        try await TestIsolation.withIsolatedState(env: [
            ...
            try #require(resolveBotConfigURL().path == configURL.path)
            let output = try configureRemote(
                .init(
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("bot-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["BOT_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
...
struct GatewayConfigTests {
    @Test @MainActor func `config path wins when both config and state dir are set`() async throws {
        let rootDir = FileManager().temporaryDirectory
            .appendingPathComponent("bot-cli-config-precedence-\(UUID().uuidString)", isDirectory: true)
        let explicitConfigURL = rootDir.appendingPathComponent("explicit.json")
        let stateConfigURL = rootDir.appendingPathComponent("state/bot.json")
        ...
        await TestIsolation.withEnvValues([
            "BOT_CONFIG_PATH": explicitConfigURL.path,
            "BOT_STATE_DIR": stateConfigURL.deletingLastPathComponent().path,
        ]) {
    ...
    @Test @MainActor func `config path trims surrounding whitespace and expands tilde`() async {
        let relativePath = ".bot-cli-config-\(UUID().uuidString)/bot.json"
        let expectedURL = FileManager().homeDirectoryForCurrentUser.appendingPathComponent(relativePath)

        await TestIsolation.withEnvValues([
            "BOT_CONFIG_PATH": "  ~/\(relativePath)\n",
            "BOT_STATE_DIR": "/tmp/bot-unused-state-dir",
        ]) {
            #expect(resolveBotConfigURL().path == expectedURL.path)
        }
    }

    @Test @MainActor func `state dir trims surrounding whitespace for connect and wizard`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("bot cli state profile \(UUID().uuidString)", isDirectory: true)
        let configURL = stateDir.appendingPathComponent("bot.json")
        defer { try? FileManager().removeItem(at: stateDir) }
