...
struct BotConfigFileTests {
    ...
    func `browser control enabled reads config flag`() async {
        ...
        await TestIsolation.withEnvValues(["BOT_CONFIG_PATH": override]) {
            #expect(BotConfigFile.browserControlEnabled() == true)
            BotConfigFile.saveDict(["browser": ["enabled": false]])
            #expect(BotConfigFile.browserControlEnabled() == false)
            BotConfigFile.setBrowserControlEnabled(true)
            #expect(BotConfigFile.browserControlEnabled() == true)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["BOT_CONFIG_PATH": override]) {
            ...
            #expect(BotConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(BotConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(BotConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(BotConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["BOT_CONFIG_PATH": override]) {
            ...
            BotConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = BotConfigFile.loadDict()
    ...
    func `load dict ignores legacy config health sidecar`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try BotConfigFile.withTestingFileLock {
                let loaded = BotConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = BotConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
