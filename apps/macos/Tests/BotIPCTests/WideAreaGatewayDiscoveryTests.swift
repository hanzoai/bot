...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() async {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._bot-gw._tcp.bot.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.bot.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (Bot)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/bot/src/entry.ts\""
                }
                return ""
            })
        ...
        let beacons = await TestIsolation.withEnvValues(
            ["BOT_WIDE_AREA_DOMAIN": "bot.internal"])
        {
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Bot)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/bot/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._bot-gw._tcp.bot.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
