...
struct GatewayProcessManagerTests {
        ...
        _ body: () async throws -> T) async throws -> T
    {
        ...
        return try await TestIsolation.withEnvValues(["BOT_CONFIG_PATH": configPath], body)
    }
    ...
    @Test func `coalesces concurrent launch agent enable requests`() async throws {
        ...
        let marker = FileManager.default.temporaryDirectory
            .appendingPathComponent("bot-launchagent-marker-\(UUID().uuidString)")
        ...
        try await self.withLocalGatewayConfig {
            ...
            GatewayLaunchAgentManager.setTestingDaemonStatusPayloads([
                ...
                {"ok":true,"service":{
                  ...
                  "command":{"programArguments":["bot","gateway","--port","\(secondPort)"]},
    ...
    @Test func `restart waits for disable before attaching`() async throws {
        ...
        let descriptor = PortGuardian.Descriptor(
            pid: 4242,
            command: "bot-gateway",
            executablePath: "/tmp/bot-gateway")
            ...
            for configAudit in reusableAudits {
                GatewayLaunchAgentManager.setTestingDaemonStatusPayload(
                    ...
                    {"ok":true,"service":{
                      "loaded":true,
                      "runtime":{"status":"running","pid":4242},
                      "command":{"programArguments":["bot","gateway","--port","\(port)"]},
        ...
        try await self.withLocalGatewayConfig {
            ...
            let staleStatuses = [
                ...
                {"ok":true,"service":{
                  "loaded":true,
                  "runtime":{"status":"running","pid":4242},
                  "command":{"programArguments":["bot","gateway","--port","19084"]},
                  "configAudit":{"ok":true,"issues":[]}
                }}
                """,
                """
    ...
    @Test func `readiness refreshes when the endpoint pid changes during the probe`() async throws {
        ...
        let stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bot-gateway-pid-refresh-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `transient unavailable health response retries until ready`() async throws {
        let stateDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("bot-gateway-ready-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: stateDir) }
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        ...
        do {
            ...
            do {
                ...
                #expect(details.contains("pid 4343 bot-gateway @ /tmp/bot-gateway"))
                #expect(manager._testControlChannelRefreshForces().last == true)
                await cleanup()
            } catch {
                await cleanup()
