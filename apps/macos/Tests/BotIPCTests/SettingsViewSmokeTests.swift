...
struct SettingsViewSmokeTests {
    @Test func `cron settings builds body`() {
        ...
        store.schedulerStorePath = "/tmp/bot-cron-store.json"
    ...
    @Test func `Bot settings require configured inference`() {
    ...
    @Test func `Bot preserves same route and resets for gateway changes`() {
        let stateDir = URL(fileURLWithPath: "/Users/tester/.bot")
