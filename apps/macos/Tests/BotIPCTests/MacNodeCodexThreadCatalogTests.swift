...
struct MacNodeCodexThreadCatalogTests {
    ...
    private func makeFakeCodex(_ script: String) throws -> FakeCodex {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("bot-fake-codex-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func `resolves and runs the configured Codex App Server without a shell`() async throws {
        let clearEnvSentinel = "BOT_CODEX_CATALOG_CLEAR_ENV_SENTINEL"
        ...
        [ "$2" = "--stdio" ] || exit 11
        [ -z "${BOT_CODEX_CATALOG_CLEAR_ENV_SENTINEL+x}" ] || exit 12
    ...
    @Test func `uses official environment command and argument fallbacks`() throws {
        ...
        let resolved = try MacNodeCodexThreadCatalog.resolveInvocation(
            root: [:],
            environment: [
                "BOT_CODEX_APP_SERVER_BIN": " \(fake.executable.path) ",
                "BOT_CODEX_APP_SERVER_ARGS": #"custom-app-server "--listen" 'stdio://'"#,
            ],
            searchPaths: [],
    ...
    @Test func `blank configured command falls back to the environment command`() throws {
        ...
        let resolved = try MacNodeCodexThreadCatalog.resolveInvocation(
            root: root,
            environment: ["BOT_CODEX_APP_SERVER_BIN": fallback.executable.path],
            searchPaths: [])
