...
struct QuickChatPowerFeaturesTests {
    ...
    @Test func `paste rejects a stale assistant and the Bot process`() {
        let messages = [
            Self.message(role: "user", text: "Question", idempotencyKey: "send-1"),
    ...
    @Test func `model thinking options reject an unsupported explicit override`() {
        let options = [BotChatThinkingLevelOption(id: "off", label: "Off")]
    ...
    @Test func `session model patch settles and send aborts after dismissal`() async throws {
        ...
        latch.finish(with: BotChatModelPatchResult(
...
private final class QuickChatModelPatchLatch {
    private var continuation: CheckedContinuation<BotChatModelPatchResult, Never>?
    ...
    func wait() async throws -> BotChatModelPatchResult {
    ...
    func finish(with result: BotChatModelPatchResult) {
