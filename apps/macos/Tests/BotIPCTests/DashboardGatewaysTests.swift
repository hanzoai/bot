...
struct DashboardGatewaysBridgeTests {
    ...
    @Test func `gateway script contains metadata and no credentials`() {
        ...
        #expect(script.contains("__BOT_NATIVE_GATEWAYS__"))
        #expect(script.contains("bot:native-gateways-changed"))
    ...
    @Test func `dashboard controller retains profile TLS policy`() throws {
        ...
        let controller = DashboardWindowController(
            ...
            tlsParams: params,
            windowAutosaveName: "BotDashboardWindow-Test-\(UUID().uuidString)")
