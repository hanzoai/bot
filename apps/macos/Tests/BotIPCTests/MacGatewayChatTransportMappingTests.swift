...
struct MacGatewayChatTransportMappingTests {
    ...
    @Test func `legacy trace preference migrates to independent defaults once`() throws {
        ...
        defaults.set(false, forKey: BotChatWindowShell.assistantTraceDefaultsKey)

        #expect(WebChatTracePreferences.displayOptions(defaults: defaults).isEmpty)
        #expect(defaults.object(forKey: BotChatWindowShell.assistantReasoningDefaultsKey) as? Bool == false)
        #expect(defaults.object(forKey: BotChatWindowShell.assistantToolActivityDefaultsKey) as? Bool == false)

        defaults.set(true, forKey: BotChatWindowShell.assistantReasoningDefaultsKey)
    ...
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: ["ok": BotProtocol.AnyCodable(false)],
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: BotProtocol.AnyCodable(["ok": BotProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `sessions changed event maps to authoritative refresh signal`() {
        let payload = BotProtocol.AnyCodable([
            "sessionKey": BotProtocol.AnyCodable("agent:main:main"),
            "agentId": BotProtocol.AnyCodable("main"),
            "reason": BotProtocol.AnyCodable("command-metadata"),
        ])
        let frame = EventFrame(
            type: "event",
    ...
    @Test func `chat event maps to chat`() {
        let payload = BotProtocol.AnyCodable([
            "runId": BotProtocol.AnyCodable("run-1"),
            "sessionKey": BotProtocol.AnyCodable("main"),
            "state": BotProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `session message event maps to session message`() {
        let payload = BotProtocol.AnyCodable([
            "sessionKey": BotProtocol.AnyCodable("agent:main:main"),
            "messageId": BotProtocol.AnyCodable("msg-1"),
            "messageSeq": BotProtocol.AnyCodable(7),
            "message": BotProtocol.AnyCodable([
                "role": BotProtocol.AnyCodable("user"),
                "content": BotProtocol.AnyCodable([
                    BotProtocol.AnyCodable([
                        "type": BotProtocol.AnyCodable("text"),
                        "text": BotProtocol.AnyCodable("spoken transcript"),
                    ]),
                ]),
                "timestamp": BotProtocol.AnyCodable(1234.5),
            ]),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: BotProtocol.AnyCodable(["a": BotProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
