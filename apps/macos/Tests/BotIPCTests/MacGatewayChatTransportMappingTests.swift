...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: HanzoBotProtocol.AnyCodable(["ok": HanzoBotProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: HanzoBotProtocol.AnyCodable(["ok": HanzoBotProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = HanzoBotProtocol.AnyCodable([
            "runId": HanzoBotProtocol.AnyCodable("run-1"),
            "sessionKey": HanzoBotProtocol.AnyCodable("main"),
            "state": HanzoBotProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: HanzoBotProtocol.AnyCodable(["a": HanzoBotProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
