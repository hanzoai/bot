...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["bot"]
        let text = "uh bot"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "bot")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey bot do thing"
        let config = WakeWordGateConfig(triggers: ["bot"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["bot"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "bot")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "bot tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["bot", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["bot"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "bot", start: 0.2, duration: 0.1, range: transcript.range(of: "bot")),
