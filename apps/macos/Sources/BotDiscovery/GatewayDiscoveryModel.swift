...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.hanzo.bot", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in HanzoBotBonjour.gatewayServiceDomains {
            ...
            let browser = NWBrowser(
                for: .bonjour(type: HanzoBotBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "ai.hanzo.bot.macos.gateway-discovery.\(domain)"))
        }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = HanzoBotBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = HanzoBotBonjour.wideAreaGatewayServiceDomain else { return false }
