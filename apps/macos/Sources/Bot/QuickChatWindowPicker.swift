...
final class QuickChatWindowPicker {
    ...
    {
        let logging = LoggingService(subsystem: "ai.bot.quickchat-picker")
    ...
    private func requestScreenRecordingPermission(mode: QuickChatCapturePickerMode) async {
        ...
        alert.messageText = mode == .window
            ? String(localized: "Allow Bot to capture windows")
            : String(localized: "Allow Bot to capture a screen area")
