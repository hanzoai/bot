...
/// shared with iOS through BotKit.
enum MacChatTranscriptCache {
    struct Context {
        let store: BotChatSQLiteTranscriptCache
    ...
    @MainActor private static var databasesByDirectory: [String: BotClientDatabases] = [:]
    @MainActor private static var storesByKey: [String: BotChatSQLiteTranscriptCache] = [:]
    ...
    ///   session data: distinct profiles (distinct `BOT_STATE_DIR`) never
    ...
    static func currentGatewayID() -> String? {
        ...
        return self.gatewayID(
            mode: mode,
            localStateDir: BotConfigFile.stateDirURL(),
    ...
    static func makeContext(gatewayID: String) -> Context? {
        ...
        let root = base.appendingPathComponent("Bot", isDirectory: true)
    ...
    @MainActor
    static func store(databaseDirectoryURL: URL, gatewayID: String) -> BotChatSQLiteTranscriptCache? {
        guard let databases = self.databases(
            directoryURL: databaseDirectoryURL,
    ...
    private static func store(
        databases: BotClientDatabases,
        gatewayID: String) -> BotChatSQLiteTranscriptCache
    {
    ...
    private static func databases(
        directoryURL: URL,
        legacyDirectoryURL: URL) -> BotClientDatabases?
    {
        ...
        guard let databases = try? BotClientDatabases(
