...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            BotConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                BotConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                BotConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            BotConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            BotConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
