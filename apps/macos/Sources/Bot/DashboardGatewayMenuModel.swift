...
enum DashboardGatewayMenuModel {
        ...
        entries: [DashboardGatewayEntry]) -> String
    {
        guard entries.count >= 2,
            ...
            return switch mode {
            case .unconfigured:
                String(localized: "Bot Not Configured")
            case .remote:
                String(localized: "Remote Bot Active")
            case .local:
                String(localized: "Bot Active")
            }
        }
        return switch mode {
        case .unconfigured:
            String(localized: "Bot Not Configured — \(primaryName)")
        case .remote:
            String(localized: "Remote Bot Active — \(primaryName)")
        case .local:
            String(localized: "Bot Active — \(primaryName)")
        }
    }
}
