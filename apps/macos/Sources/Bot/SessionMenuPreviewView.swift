...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.hanzo.bot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> HanzoBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: HanzoBotSessionPreviewEntry,
    ...
    private static func cache(payload: HanzoBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: HanzoBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [BotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [BotKit.AnyCodable]) -> [HanzoBotChatMessage] {
    ...
    private static func previewText(for message: HanzoBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: HanzoBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: HanzoBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: HanzoBotChatMessage) -> String? {
