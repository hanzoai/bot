...
struct QuickChatModelControlSnapshot: Sendable {
    let models: [BotChatModelChoice]
    let currentModelSelectionID: String?
    let currentThinkingLevel: String?
    let thinkingOptions: [BotChatThinkingLevelOption]
...
enum QuickChatModelControlLogic {
    static let baseThinkingOptions = ["off", "minimal", "low", "medium", "high"].map {
        BotChatThinkingLevelOption(id: $0, label: $0)
    }

    static func snapshot(
        target: QuickChatRoutingTarget,
        models: [BotChatModelChoice],
        sessions: BotChatSessionsListResponse,
        ...
        currentSessionSelectionID: String? = nil) -> QuickChatModelPatchDecision
    {
        ...
        if selectionID == BotChatViewModel.defaultModelSelectionID {
        ...
        automaticLabel: String) -> String
    {
        guard let selectionID,
              selectionID != BotChatViewModel.defaultModelSelectionID
    ...
    static func validatedThinkingSelection(
        _ selection: String?,
        options: [BotChatThinkingLevelOption]) -> String?
    {
    ...
    private static func sessionEntry(
        target: QuickChatRoutingTarget,
        sessions: [BotChatSessionEntry]) -> BotChatSessionEntry?
    {
    ...
    private static func agent(target: QuickChatRoutingTarget, agents: AgentsListResult?) -> AgentSummary? {
        ...
        let targetAgentID = self.normalized(target.agentID) ??
            BotChatSessionKey.agentID(from: target.sessionKey) ??
    ...
    private static func thinkingOptions(
        entry: BotChatSessionEntry?,
        agent: AgentSummary?,
        defaults: BotChatSessionsDefaults?) -> [BotChatThinkingLevelOption]
    {
        let agentOptions = agent?.thinkinglevels?.compactMap { option -> BotChatThinkingLevelOption? in
            ...
            return BotChatThinkingLevelOption(id: id, label: label)
        }
        let options = entry?.thinkingLevels ??
            entry?.thinkingOptions?.map { BotChatThinkingLevelOption(id: $0, label: $0) } ??
            agentOptions ??
            defaults?.thinkingLevels ??
            defaults?.thinkingOptions?.map { BotChatThinkingLevelOption(id: $0, label: $0) } ??
