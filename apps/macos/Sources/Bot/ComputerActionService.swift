...
final class ComputerActionExecutionQueue {
    typealias Operation = @MainActor (BotComputerActParams, UInt64) async throws
        -> BotComputerActResult
    typealias CancellationHop = @Sendable (
        @escaping @MainActor @Sendable () -> Void) -> Void

    private struct QueuedAction {
        let id: UUID
        let params: BotComputerActParams
        let lifecycleGeneration: UInt64
        let operation: Operation
        let continuation: CheckedContinuation<BotComputerActResult, Error>
    ...
    private var currentActionTask: Task<BotComputerActResult, Error>?
    ...
    func perform(
        _ params: BotComputerActParams,
        lifecycleGeneration: UInt64,
        operation: @escaping Operation) async throws -> BotComputerActResult
    {
        let actionID = UUID()
        let cancellationState = ComputerActionCancellationState()
    ...
    private func drain() async {
        while !self.pendingActions.isEmpty {
            ...
            let outcome: Result<BotComputerActResult, Error>
            do {
...
struct ComputerControlPermissionSnapshot: Equatable, Sendable {
    ...
    enum Diagnostic: Equatable, Sendable {
        ...
        var detailText: String {
            ...
            case let .missing(buckets):
                "Missing: \(buckets.map(\.displayName).joined(separator: ", ")). "
                    + "Grant access in System Settings → Privacy & Security, then reopen Bot."
            case .accessibilityGrantMayBeStale:
        ...
        Bot may already appear enabled under System Settings → Privacy & Security → Accessibility. \
        If so, the grant is pinned to an older build: select Bot, remove it with −, then re-add \
...
final class ComputerActionService {
    ...
    func perform(
        _ params: BotComputerActParams,
        lifecycleGeneration: UInt64) async throws -> BotComputerActResult
    {
        ...
        lifecycleGeneration: UInt64) async throws -> BotComputerActResult
    {
        ...
        return BotComputerActResult(ok: true, cursorX: cursor.x, cursorY: cursor.y)
    }
    ...
    private func peekabooClick(at point: CGPoint, action: BotComputerAction) async throws {
    ...
    private struct ResolvedDisplay {
        var geometry: BotComputerDisplayGeometry
        ...
        display: ResolvedDisplay) throws -> CGPoint?
    {
        ...
        let capturedWidth = BotComputerInputGeometry.capturedWidth(
            refWidth: refWidth,
            sourceWidth: display.sourceWidth,
            sourceHeight: display.sourceHeight)
        let mapped = BotComputerInputGeometry.mapReferencePointToGlobal(
            x: x,
            y: y,
        ...
        let clamped = BotComputerInputGeometry.clampToDisplay(
            x: mapped.x,
            y: mapped.y,
            display: geometry)
    ...
    static func validatedCurrentCursorPoint(
        _ point: CGPoint?,
        display: BotComputerDisplayGeometry) throws -> CGPoint
    {
        guard let point,
    ...
    func typeTextForTesting(
        _ text: String,
        lifecycleGeneration: UInt64 = 0) async throws -> BotComputerActResult
    {
        let params = BotComputerActParams(action: .type, text: text)
        ...
        { [weak self] _, generation in
            ...
            return BotComputerActResult(ok: true, cursorX: 0, cursorY: 0)
        }
    }
    #endif

    private func resolveDisplay(params: BotComputerActParams) async throws -> ResolvedDisplay {
        ...
        let geometry = BotComputerDisplayGeometry(
        ...
        guard BotComputerInputGeometry.isValidMappingGeometry(
        ...
        if Self.usesScreenshotCoordinates(params) {
            let refWidth = try Self.requiredReferenceWidth(params)
            let currentFrameId = BotComputerInputGeometry.displayFrameId(
    ...
    private static func usesScreenshotCoordinates(_ params: BotComputerActParams) -> Bool {
        params.x != nil || params.y != nil || params.fromX != nil || params.fromY != nil
    }

    private static func requiredReferenceWidth(_ params: BotComputerActParams) throws -> Int {
    ...
    private static func scrollDirection(
        _ direction: BotComputerScrollDirection) -> PeekabooFoundation.ScrollDirection
    {
        switch direction {
        case .up: .up
        case .down: .down
    ...
    private func rawScroll(direction: BotComputerScrollDirection, amount: Int, flags: CGEventFlags) throws {
