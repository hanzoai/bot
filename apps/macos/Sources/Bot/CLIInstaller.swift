...
extension Notification.Name {
    static let botCLIInstalled = Notification.Name("bot.cli.installed")
}
...
enum CLIInstaller {
    ...
    enum Status: Equatable {
        ...
        var message: String {
            ...
            case let .ready(_, version):
                "Bot Gateway \(version) is ready."
            case .missing:
                "Bot Gateway is not installed yet."
            case .unusable:
                "The Bot Gateway could not be verified. Setup will repair it."
        ...
        fileManager: FileManager) -> [String]
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("bot").path
    ...
    static func managedExecutableLocation() -> String {
        URL(fileURLWithPath: self.installPrefix())
            .appendingPathComponent("bin/bot")
            .path
    }
        ...
        statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async -> Bool
    {
        ...
        await statusHandler("Installing Bot CLI (\(target.selector))…")
        guard let installerURL = Bundle.main.url(forResource: "install-cli", withExtension: "sh") else {
            await statusHandler("Install failed: installer resource is missing. Reinstall Bot.")
            return false
        }
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed bot \($0)." } ?? "Installed bot."
            self.rememberInstallPolicy(target)
            await statusHandler(summary)
            NotificationCenter.default.post(name: .botCLIInstalled, object: nil)
            return true
        }
        ...
        -> ManagedCLIUpdateOutcome
    {
        ...
        await statusHandler(repair
            ? String(localized: "Repairing the Bot Gateway update…")
            : String(localized: "Updating the Bot Gateway to \(targetVersion)…"))
        ...
        NotificationCenter.default.post(name: .botCLIInstalled, object: nil)
        await statusHandler(String(localized: "Bot Gateway \(installedVersion) is installed."))
        return .success(
    ...
    private static func devCheckoutLocation(prefix: String) -> String {
        URL(fileURLWithPath: prefix)
            .appendingPathComponent("dev/bot")
            .path
    }
