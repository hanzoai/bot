...
extension OnboardingView {
    ...
    func refreshAnthropicOAuthStatus() {
        _ = BotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        let previous = self.anthropicAuthDetectedStatus
        let status = BotOAuthStore.anthropicOAuthStatus()
    ...
    func verifyAnthropicOAuthIfNeeded(force: Bool = false) async {
        ...
        guard let refresh = BotOAuthStore.loadAnthropicOAuthRefreshToken(), !refresh.isEmpty else {
        ...
        do {
            ...
            try BotOAuthStore.saveAnthropicOAuth(updated)
            self.refreshAnthropicOAuthStatus()
