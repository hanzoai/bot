...
extension OnboardingView {
    ...
    func startExistingCLIActivationIfNeeded() {
        ...
        OnboardingController.shared.busyReason = "Bot is starting the Gateway service."
        cliStatus = "Starting Bot Gateway…"
    ...
    func finishExistingCLIActivation() async {
        ...
        case .ready:
            cliInstalled = true
            cliStatus = "Bot Gateway is ready."
        case .deferred:
            cliInstalled = false
            cliStatus = "Bot is paused. Resume it, then retry setup to start the Gateway."
        case .failed:
            cliInstalled = false
            cliStatus = "Bot is installed, but the Gateway did not start. Retry setup."
        }
    }
    ...
    func startCLIInstall() {
        ...
        OnboardingController.shared.busyReason = "Bot is installing the Gateway service."
    ...
    func runCLIInstall() async {
        ...
        if !Self.shouldActivateLocalGateway(afterCLIInstallFor: self.state.connectionMode) {
            cliStatus = "Bot CLI is ready for the Mac node."
            cliInstalled = true
            return
        }
        cliStatus = "Starting Bot Gateway…"
        ...
        case .ready:
            cliStatus = "Bot Gateway is ready."
        case .deferred:
            cliStatus = "Bot is installed. The Gateway will start when This Mac is active and resumed."
        case .failed:
            cliStatus = "Bot was installed, but the Gateway did not start. Retry setup."
            return
        }
        cliInstalled = true
    }
