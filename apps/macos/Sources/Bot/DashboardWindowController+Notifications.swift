...
extension DashboardWindowController {
    static let notificationsMessageHandlerName = "botNotifications"
    ...
    private func publishNotificationsStatus() async {
        ...
        _ = try? await self.webView.evaluateJavaScript(
            """
            window.__BOT_NATIVE_NOTIFICATIONS__ = {permission:"\(permission)"};
            window.dispatchEvent(new CustomEvent('bot:native-notifications-status', \
            {detail:window.__BOT_NATIVE_NOTIFICATIONS__}));
            """)
    }
}
