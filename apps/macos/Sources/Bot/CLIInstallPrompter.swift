...
final class CLIInstallPrompter {
    ...
    private let logger = Logger(subsystem: "ai.bot", category: "cli.prompt")
    ...
    func installTargetForCurrentBuild(confirmStable: Bool = false) -> CLIInstaller.InstallTarget? {
            ...
            isDebug: CLIInstallBuild.isDebug)
        {
            ...
            let alert = NSAlert()
            alert.messageText = "Install Bot CLI?"
    ...
    private func chooseChannel(suggested: CLIInstaller.Channel) -> CLIInstaller.Channel? {
        ...
        let alert = NSAlert()
        alert.messageText = "Choose Bot CLI channel"
        alert.informativeText =
            "This is an unreleased Bot build. " +
        ...
        restartManagedGateway: Bool = false) async -> Bool
    {
        ...
        if installed {
            ...
            if usesLocalGateway {
                await status.set("Starting Bot Gateway…")
                if !showCompletionAlert {
                    self.logger.info("managed CLI repair: Starting Bot Gateway…")
                }
            ...
            case .ready:
                "Bot Gateway is ready."
            case .deferred:
                "Bot is installed. The Gateway will start when This Mac is active and resumed."
            case .failed:
                "Bot was installed, but the Gateway did not start. Open Settings to retry."
            case nil:
                "Bot CLI is ready for the Mac node."
            }
            await status.set(message)
            if !showCompletionAlert {
                self.logger.info("managed CLI repair: \(message, privacy: .public)")
