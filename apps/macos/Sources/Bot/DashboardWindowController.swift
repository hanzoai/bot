...
/// pinned at `--bot-native-titlebar-height`, resurrecting the traffic-light
...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    private static let linkMessageHandlerName = "botLink"
    private static let windowDragMessageHandlerName = "botWindowDrag"
    private static let updateMessageHandlerName = "botUpdate"
    ...
    func setUpdateBridgeEnabled(_ enabled: Bool) {
        ...
        self.webView.evaluateJavaScript(
            "window.dispatchEvent(new CustomEvent('bot:native-update-availability-changed'))")
    }
    ...
    private static func makeJavaScriptConfirmAlert(message: String, host: String?) -> NSAlert {
        let alert = NSAlert()
        alert.messageText = "Bot Dashboard"
    ...
    fileprivate func receiveUpdateMessage(_ message: WKScriptMessage) {
        ...
        guard DashboardManager.updateBridgeEnabled(mode: AppStateStore.shared.connectionMode) else {
            ...
            self.webView.evaluateJavaScript(
                "window.dispatchEvent(new CustomEvent('bot:native-update-declined'))")
            return
        }
    ...
    private func publishNativeHistoryState() {
        ...
        self.webView.evaluateJavaScript(
            """
            window.__BOT_NATIVE_HISTORY__ = {canGoBack:\(canGoBack),canGoForward:\(canGoForward)};
            window.dispatchEvent(new CustomEvent('bot:native-history-state', \
            {detail:window.__BOT_NATIVE_HISTORY__}));
            """)
    }
        ...
        frameAutosaveName: String) -> NSWindow
    {
        ...
        // (--bot-native-titlebar-height); without it they hug the top edge.
    ...
    static func installNativeChromeScript(into userContentController: WKUserContentController) {
        ...
        let capabilityScript = "window.__BOT_NATIVE_WEB_CHROME__ = true;"
        ...
        // `html.bot-native-macos` styles fold the titlebar clearance into
        ...
        @media (min-width: 700px) {
          ...
          html.bot-native-macos .sidebar-shell,
          html.bot-native-macos .settings-sidebar__header {
            padding-top: max(14px, var(--bot-native-titlebar-height)) !important;
          }
        }
        """
        let script = """
        (() => {
          try {
            if (document.getElementById("bot-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "bot-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("bot-native-macos", "bot-native-web-chrome");
