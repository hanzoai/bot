...
final class PostUpdateModel {
    ...
    var title = String(localized: "Finishing your Bot update")
...
final class PostUpdateController: NSObject, NSWindowDelegate {
    ...
    static let updateGuideURL = URL(string: "https://docs.bot.ai/install/updating")!
    ...
    private func show() {
        ...
        window.title = String(localized: "Bot updated")
    ...
    private func run() {
        ...
        self.model.title = String(localized: "Finishing your Bot update")
    ...
    private func finishUpdate(receipt: PostAppUpdateReceipt) async {
        ...
        let ownsManagedRuntime = Self.ownsManagedRuntime(
            ...
            gatewayUpdateChannel: BotConfigFile.gatewayUpdateChannel(),
        ...
        case .install:
            ...
            guard installed else {
                self.fail(
                    ...
                    details: String(localized: "The managed Bot runtime could not be reinstalled."))
                return
            }
        }
        ...
        connectionMode: AppState.ConnectionMode)
    {
        ...
        self.model.message = connectionMode == .local
            ? String(localized: "Bot \(receipt.toVersion) and its Gateway are ready.")
            : String(localized: "Bot \(receipt.toVersion) and its Mac node runtime are ready.")
        self.model.details = switch (notification, notificationRetryScheduled) {
        case (.retryLater, true):
            String(localized: "Your agent could not be notified yet. Bot will retry after the next app launch.")
        case (.retryLater, false):
            if connectionMode == .local {
                String(
                    localized: """
                    Bot could not notify your agent automatically. \
        ...
        case (.deliveryUnconfirmed, _):
            String(
                localized: """
                Bot could not confirm the agent notification. \
                It will not retry, to avoid a duplicate welcome.
                """)
        case (.skippedUnsupportedGateway, _):
            String(
                localized: "The remote Gateway is older than this Mac app, so Bot skipped the agent notification.")
        case (.skippedWhilePaused, _):
            String(localized: "The Gateway remains paused, so Bot did not wake your agent.")
        default:
            nil
        }
    }
    ...
    private func verifyRuntime(connectionMode: AppState.ConnectionMode) async -> Bool {
        ...
        guard ControlChannel.shared.state == .connected else {
            self.fail(
                ...
                details: String(
                    localized: "Bot installed the update but could not verify the Gateway connection."))
            return false
        }
        return true
    }
        ...
        receipt: PostAppUpdateReceipt) async -> PostUpdateNotificationOutcome
    {
        ...
        do {
            guard let selected = try await Self.preferredNotificationSession(loadPage: { offset in
                var params: [String: BotKit.AnyCodable] = [
                    "limit": BotKit.AnyCodable(100),
                    "includeGlobal": BotKit.AnyCodable(false),
                    "includeUnknown": BotKit.AnyCodable(false),
                    "configuredAgentsOnly": BotKit.AnyCodable(true),
                    "requireLastInteraction": BotKit.AnyCodable(true),
                    "sortBy": BotKit.AnyCodable("lastInteractionAt"),
                ]
                if offset > 0 {
                    params["offset"] = BotKit.AnyCodable(offset)
                }
        ...
        let text =
            "Bot updated to \(version). Briefly welcome the user back and say you are updated, " +
        ...
        do {
            _ = try await GatewayConnection.shared.request(
                method: "system-event",
                params: [
                    "text": BotKit.AnyCodable(text),
                    "sessionKey": BotKit.AnyCodable(session.key),
                    "wake": BotKit.AnyCodable(true),
                ],
                ifCurrentServerLease: serverLease)
            return .delivered
        } catch {
    ...
    static func notificationSendFailureOutcome(_ error: Error) -> PostUpdateNotificationOutcome {
        if error is BotChatTransportSendError || error is GatewayResponseError {
            return .retryLater
        }
        ...
        receipt: PostAppUpdateReceipt)
    {
        ...
        case .local:
            self.fail(
                ...
                details: String(
                    localized: """
                    Bot could not read the Gateway service ownership record. \
        ...
        case .remote:
            self.fail(
                ...
                details: String(
                    localized: """
                    Bot could not read the node service ownership record. \
...
private struct PostUpdateView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            GlowingBotIcon(size: 150, mood: self.model.mood)
                .frame(height: 205)
