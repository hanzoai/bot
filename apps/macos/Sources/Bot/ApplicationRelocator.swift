...
enum ApplicationRelocator {
    ...
    private static let logger = Logger(subsystem: "ai.bot", category: "app-relocation")
    ...
    private nonisolated static let replacementSourceBundleEnvironmentKey = "BOT_REPLACEMENT_SOURCE_BUNDLE"
    private nonisolated static let replacementParentPIDEnvironmentKey = "BOT_REPLACEMENT_PARENT_PID"
    private nonisolated static let replacementCodeHashEnvironmentKey = "BOT_REPLACEMENT_CODE_HASH"
    private nonisolated static let replacementReadyFDEnvironmentKey = "BOT_REPLACEMENT_READY_FD"
    private nonisolated static let replacementBootoutTargetEnvironmentKey = "BOT_REPLACEMENT_BOOTOUT_TARGET"
    private nonisolated static let replacementSupervisorLabelEnvironmentKey =
        "BOT_REPLACEMENT_SUPERVISOR_LABEL"
    private nonisolated static let replacementSupervisorPlistEnvironmentKey =
        "BOT_REPLACEMENT_SUPERVISOR_PLIST"
        ...
        processInfo: ProcessInfo = .processInfo) -> LaunchDisposition
    {
        ...
        case .continueLaunch:
            #if DEBUG
            let monitorDebugReplacement = processInfo.environment["BOT_MONITOR_APP_REPLACEMENT"] == "1"
            #else
        ...
        case let .offerInstall(destination, replacing):
            ...
            do {
                ...
                showFailure(
                    "Bot couldn’t be installed in Applications. Move it there manually, then open that copy.")
                return .continueLaunch(startUpdater: false)
            }
        case .cannotInstall:
            let message =
                "Bot is running from a temporary location. " +
...
extension ApplicationRelocator {
    ...
    private static func confirmInstall(replacing: Bool) -> Bool {
        ...
        alert.messageText = replacing
            ? "Replace the older Bot in Applications?"
            : "Install Bot in Applications?"
        alert.informativeText = replacing
            ? "This copy is newer than the installed app. Bot will replace it and reopen from Applications."
            : "Bot will copy itself to Applications and reopen there so updates and launch at login stay reliable."
    ...
    private static func relaunchAndTerminate(at destination: URL) -> LaunchDisposition {
        ...
        do {
            ...
            self.showFailure(
                "Bot is installed in Applications, but couldn’t reopen automatically. Open it there manually.")
            return .continueLaunch(startUpdater: false)
        }
    }
        ...
        reason: String) async
    {
        ...
        case .stopMonitoring:
            ...
            self.logger.critical(
                ...
                Quit and reopen Bot to load the installed replacement.
                """)
        case .terminateForSupervisor:
    ...
    private static func showFailure(_ message: String) {
        let alert = NSAlert()
        alert.messageText = "Move Bot to Applications"
