...
enum ExecApprovalsStore {
    private static let logger = Logger(subsystem: "ai.bot", category: "exec-approvals")
    ...
    private static func homeURL() -> URL {
        guard let configured = BotEnv.path("BOT_HOME") else {
    ...
    private static func stateDirURL() -> URL {
        guard let configured = BotEnv.path("BOT_STATE_DIR") else {
            return self.homeURL().appendingPathComponent(".bot", isDirectory: true)
        }
...
extension ExecApprovalsStore {
    ...
    static func expandPath(_ raw: String) -> String {
        ...
        let configuredHome = BotEnv.path("BOT_HOME")
