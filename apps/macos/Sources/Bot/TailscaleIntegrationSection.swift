...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.hanzo-bot/hanzo-bot.json. Prefer BOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.hanzo-bot/hanzo-bot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.hanzo-bot/hanzo-bot.json. Restart the gateway to apply."
        }
