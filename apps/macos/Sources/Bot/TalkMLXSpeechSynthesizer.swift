...
actor TalkMLXSpeechSynthesizer {
    ...
    private let logger = Logger(subsystem: "ai.bot", category: "talk.mlx")
    ...
    fileprivate static func helperInvocation() -> HelperInvocation {
        ...
        if let override = ProcessInfo.processInfo.environment["BOT_MLX_TTS_BIN"], !override.isEmpty {
        ...
        if let executableDir = Bundle.main.executableURL?.deletingLastPathComponent() {
            let bundled = executableDir.appendingPathComponent("bot-mlx-tts")
        ...
        return HelperInvocation(
            ...
            argumentPrefix: ["bot-mlx-tts"],
            displayName: "bot-mlx-tts")
    }
