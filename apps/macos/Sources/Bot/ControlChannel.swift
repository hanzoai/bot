...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: BotKit.AnyCodable]()) {
                $0[$1.key] = BotKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking Bot Local Network access.
                    Allow Bot in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
...
extension Notification.Name {
    static let controlChannelStateDidChange = Notification.Name("bot.control-channel.state-did-change")
    static let controlHeartbeat = Notification.Name("bot.control.heartbeat")
}
