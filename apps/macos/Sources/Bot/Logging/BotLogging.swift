...
enum BotLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = BotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = BotFileLogHandler(label: label)
...
struct BotFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
