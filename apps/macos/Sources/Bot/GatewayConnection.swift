...
actor GatewayConnection {
    ...
    nonisolated static let operatorClientCaps =
        [BotGatewayClientCapability.agentKind, BotGatewayClientCapability.inlineWidgets]
...
extension GatewayConnection {
    ...
    func sessionRoutingIdentity(
        ifCurrentRoute route: Route) async throws -> BotChatSessionRoutingIdentity
    {
        let data = try await request(
            BotChatGatewayRequests.agentsList(),
            ifCurrentRoute: route)
        return try BotChatGatewayPayloadCodec.decodeSessionRoutingIdentity(data)
    }
...
extension GatewayConnection {
    ...
    func chatHistory(
        ...
        maxChars: Int? = nil,
        timeoutMs: Int? = nil,
        ifCurrentRoute route: Route? = nil) async throws -> BotChatHistoryPayload
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        ...
        if let route {
            let data = try await self.request(
                request,
                ifCurrentRoute: route)
            return try self.decoder.decode(BotChatHistoryPayload.self, from: data)
        }
        ...
        distinguishPreDispatchRouteChange: Bool = false) async throws -> BotChatSendResponse
    {
        ...
        if let route {
            let data = try await self.request(
                ...
                ifCurrentRoute: route,
                distinguishPreDispatchRouteChange: distinguishPreDispatchRouteChange)
            return try self.decoder.decode(BotChatSendResponse.self, from: data)
        }
        let data = try await self.request(request)
        return try self.decoder.decode(BotChatSendResponse.self, from: data)
    }
