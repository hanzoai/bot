...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.bot", category: "portguard")
        ...
        currentAppPID: Int32? = nil) -> TunnelRecordAction
    {
        ...
        // Any other live parent may be a concurrent Bot instance (prod + dev).
        return .keep
    }
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "bot", "tsx", "pnpm", "bun"]
        ...
        managedGatewayPID: Int32? = nil) -> Bool
    {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("bot-gateway")
                || cmd.contains("bot-gateway")
            {
                return true
            }
            if self.isNodeBotGatewayCommand(full) { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("bot"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
    ...
    private static func isNodeBotGatewayCommand(_ fullCommand: String) -> Bool {
        ...
        return self.isBotDistEntrypointToken(tokens[1])
            && tokens[2].lowercased() == "gateway"
    }

    private static func isBotDistEntrypointToken(_ token: String) -> Bool {
    ...
    private nonisolated static func openRecordStore() throws -> PortGuardianRecordStore {
        guard !self.hasLegacyBotAppProcess() else {
            throw PortGuardianStoreError(
                "Quit older Bot app copies before opening the SQLite PortGuardian ledger")
        }
    ...
    private nonisolated static func requirePostSpawnCompatibility() throws {
        guard !self.hasLegacyBotAppProcess(),
              !FileManager.default.fileExists(atPath: PortGuardianRecordStore.liveLegacyRecordURL.path)
        else {
            throw PortGuardianStoreError(
                "Older Bot storage appeared after tunnel preflight; SSH launch cancelled")
        }
    }
    ...
    private nonisolated static func hasLegacyBotAppProcess() -> Bool {
    ...
    private nonisolated static func runningPortGuardianStorageVersion(pid: pid_t) -> Int? {
        ...
        guard SecCodeCopyStaticCode(code, SecCSFlags(), &staticCode) == errSecSuccess,
              ...
              let version = securedInfo["BotPortGuardianStorageVersion"] as? NSNumber
        else { return nil }
        ...
        storageVersion: Int?) -> Bool
    {
        guard let bundleIdentifier,
              bundleIdentifier == "ai.bot.mac" || bundleIdentifier.hasPrefix("ai.bot.mac.")
        else { return false }
