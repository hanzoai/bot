...
enum CommandResolver {
    ...
    private static let helperName = "bot"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let binEntry = root.appendingPathComponent("bin/bot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/bot")
    ...
    static func preferredPaths() -> [String] {
        ...
        let validatedExecutable = self.validatedBotExecutable(
            defaults: .standard,
            fileManager: .default,
        ...
        validatedExecutable: String? = nil) -> [String]
    {
        ...
        let managedPaths = self.botManagedPaths(home: home)
    ...
    static func validatedBotExecutable(
    ...
    private static func botManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".bot"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func projectBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
        ...
        let sourceEntrypoint = root.appendingPathComponent("bot.mjs").path
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let botPath = projectBotExecutable(projectRoot: root) {
            return [botPath, subcommand] + extraArgs
        }
        if let botPath = botExecutable(searchPaths: searchPaths) {
            return [botPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "bot", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            bot CLI not found. Install the CLI, or run pnpm build in an Bot source checkout.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real bot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bot.mjs"
            node "$PRJ/bot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/bot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/bot.js"
            node "$PRJ/bin/bot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent bot"
          pnpm --silent bot \(quotedArgs);
        else
          echo "bot CLI missing on remote host"; exit 127;
        fi
        """
