...
/// Onboarding talks to Bot over the gateway `bot.chat` RPC.
...
final class SystemAgentOnboardingChatModel {
    ...
    var input = ""
    /// Set when Bot hands off to the normal agent ("talk to agent").
    ...
    /// status/repair greeting (used by Settings → Bot).
    ...
    func startIfNeeded() async {
        ...
        if Task.isCancelled, self.requestGeneration == generation {
            self.started = false
            self.errorMessage = "Bot was interrupted. Restart to try again."
        }
    }
        ...
        do {
            ...
            if error is CancellationError || Task.isCancelled {
                ...
                self.errorMessage = Task.isCancelled
                    ? "Bot was interrupted. Restart to try again."
                    : "The Gateway connection changed. Restart Bot to reconnect."
                return
            }
            ...
            HStack(spacing: 8) {
                Group {
                    if self.model.expectsSensitiveReply {
                        ...
                        TextField(
                            "Reply to Bot… (yes sets everything up)",
...
private struct SystemAgentChatBubble: View {
    ...
    private var attributedText: AttributedString {
        // Bot replies use light markdown (headings, bold, backticks).
