...
enum MacGatewayProfileError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case let .unsupportedRegistryVersion(version):
            "Gateway profiles were written by a newer Bot version (schema \(version))."
...
actor MacGatewayProfileStore {
    ...
    static let didChangeNotification = Notification.Name("bot.gateway-profiles.did-change")
    ...
    #if DEBUG
    private static let service = "ai.bot.gateway-profiles.debug"
    #else
    private static let service = "ai.bot.gateway-profiles"
    #endif
