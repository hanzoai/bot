...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        onboardingPage {
            VStack(spacing: 18) {
                ...
                Label {
                    Text(
                        "Bot can take actions using the permissions and services you enable. " +
    ...
    func connectionPage() -> some View {
        onboardingPage {
            Text("Where should your assistant live?")
                .font(.largeTitle.weight(.semibold))
            Text(
                "Most people pick this Mac — Bot installs everything and keeps it " +
            ...
            if self.selectedConnectionMode == .unconfigured {
                Text("OK — Bot won’t start anything yet. Pick Local or Remote later in Settings → General.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
        ...
        .onChange(of: self.state.connectionMode) { _, newValue in
            ...
            // retires route-owned AI/Bot state. This nested observer owns probe copy only.
        ...
        return VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .top, spacing: 12) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Verify Bot can reach this gateway.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer(minLength: 0)
            ...
            GridRow {
                ...
                Text(
                    "The current gateway.remote.token value is not plain text. "
                        + "Bot for macOS cannot use it directly; "
        ...
        action: @escaping () -> Void) -> some View
    {
        Button {
            ...
            .botSelectableRowChrome(selected: selected)
        }
        .buttonStyle(.plain)
    }
    ...
    func permissionsPage() -> some View {
        onboardingPage {
            VStack(spacing: 12) {
                ...
                Text(
                    "These macOS permissions let Bot automate apps and capture context on this Mac. " +
    ...
    func cliPage() -> some View {
        ...
        let detail = if remoteMode {
            "Bot is installing the matching runtime for this Mac node. " +
                "It will connect to your selected Gateway without starting another one here."
        } else {
            "Bot is setting up its background service on this Mac. " +
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        ...
                        subtitle: """
                        On your gateway host: install/update the `bot` package and make sure credentials exist
                        (typically `~/.bot/credentials/oauth.json`). Then connect again if needed.
                        """,
                        systemImage: "network")
                    Divider()
                        .padding(.vertical, 6)
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the Bot menu bar icon for the compact chat panel and status.",
