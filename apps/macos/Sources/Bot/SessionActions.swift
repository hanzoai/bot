...
enum SessionActions {
    ...
    {
        let request = BotChatGatewayRequests.patchSessionPreferences(
            sessionKey: key,
            agentID: nil,
    ...
    static func resetSession(key: String) async throws {
        let request = BotChatGatewayRequests.resetSession(sessionKey: key, agentID: nil)
        _ = try await ControlChannel.shared.request(request)
    }

    static func deleteSession(key: String) async throws {
        let request = BotChatGatewayRequests.deleteSession(sessionKey: key, agentID: nil)
        _ = try await ControlChannel.shared.request(request)
    }
    ...
    static func openSessionLogInCode(sessionId: String, storePath: String?) {
        let candidates: [URL] = {
            ...
            urls.append(BotPaths.stateDirURL.appendingPathComponent("sessions/\(sessionId).jsonl"))
            return urls
        }()
