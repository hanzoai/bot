...
final class AppState {
    private static let logger = Logger(subsystem: "ai.bot", category: "app-state")
        ...
        execApprovalsReadRetryDelay: Duration = .milliseconds(250))
    {
        ...
        let configRoot = BotConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            BotConfigFile.canonicalHostForComparison(host) ==
                BotConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = BotConfigFile.url()
    ...
    private func applyConfigFromDisk() {
        ...
        NotificationCenter.default.post(name: .botConfigDidChange, object: nil)
    }
...
extension AppState {
    ...
    private func syncGatewayConfigNow(remoteTLSFingerprintUpdate: RemoteTLSFingerprintUpdate) -> Bool {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: BotConfigFile.loadDict(),
            draft: draft,
        ...
        guard BotConfigFile.saveDict(synced.root) else {
            self.setGatewayConfigSyncState(.failed)
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/bot"
        state.remoteCliPath = ""
        return state
    }
}
