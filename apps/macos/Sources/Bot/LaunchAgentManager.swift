...
enum LaunchAgentManager {
    private static var plistURL: URL {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Library/LaunchAgents/ai.bot.mac.plist")
    }
        ...
        preferredPaths: [String] = CommandResolver.preferredPaths()) -> String
    {
        ...
        <dict>
          <key>Label</key>
          <string>ai.bot.mac</string>
          <key>ProgramArguments</key>
          <array>
            <string>\(bundlePath)/Contents/MacOS/Bot</string>
          </array>
    ...
    private static func profileEnvironmentPlistEntries() -> String {
        ["BOT_CONFIG_PATH", "BOT_STATE_DIR"].compactMap { key in
            guard let value = BotEnv.path(key) else { return nil }
            return """
