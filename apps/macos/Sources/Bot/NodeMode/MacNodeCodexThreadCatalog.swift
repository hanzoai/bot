...
enum MacNodeCodexThreadCatalog {
    ...
    private static let commandEnvironmentKey = "BOT_CODEX_APP_SERVER_BIN"
    private static let argumentsEnvironmentKey = "BOT_CODEX_APP_SERVER_ARGS"
    ...
    static func shouldAdvertise(root: [String: Any]? = nil) -> Bool {
        let root = root ?? BotConfigFile.loadDict()
        guard BotConfigFile.configuredBundledPluginAllowed(
...
extension MacNodeCodexThreadCatalog {
    ...
    private static func configuredPlugin(root: [String: Any]) throws -> ConfiguredPlugin? {
        guard let entry = BotConfigFile.pluginEntry(
            MacNodeCodexThreadCatalogContract.pluginId,
            root: root)
        else { return nil }
...
private final class CodexAppServerThreadRequestSession: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.bot.codex-thread-catalog")
