...
final class MacNodeHostWorker: MacNodeHostWorking, @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "ai.bot", category: "node-host-worker")
    private let queue = DispatchQueue(label: "ai.bot.node-host-worker")
    private let writerQueue = DispatchQueue(label: "ai.bot.node-host-worker.writer")
    ...
    private func startLocked(command: [String]) {
        ...
        environment["BOT_NODE_EXEC_HOST"] = "app"
        environment["BOT_NODE_EXEC_FALLBACK"] = "0"
    ...
    private static func decodeInvokeResponse(_ result: [String: Any], id: String) -> BridgeInvokeResponse {
        ...
        let code = BotNodeErrorCode(rawValue: rawError?["code"] as? String ?? "UNAVAILABLE") ?? .unavailable
        let error = ok ? nil : BotNodeError(
            code: code,
    ...
    private static func unavailableResponse(_ id: String, _ message: String) -> BridgeInvokeResponse {
        BridgeInvokeResponse(
            id: id,
            ok: false,
            error: BotNodeError(code: .unavailable, message: message))
    }
