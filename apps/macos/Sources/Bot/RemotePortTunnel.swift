...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.bot", category: "remote.tunnel")
        ...
        root: [String: Any]) -> Int?
    {
        ...
        guard let sshKey = BotConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = BotConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.bot.remote.tunnel.port", qos: .utility)
            do {
