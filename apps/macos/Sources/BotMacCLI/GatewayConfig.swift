...
/// An explicit config path wins; otherwise the selected state directory owns bot.json.
func resolveBotConfigURL() -> URL {
    if let configPath = botEnvironmentPath("BOT_CONFIG_PATH") {
        return URL(fileURLWithPath: NSString(string: configPath).expandingTildeInPath)
    }
    let stateDir = botEnvironmentPath("BOT_STATE_DIR").map {
        URL(fileURLWithPath: NSString(string: $0).expandingTildeInPath, isDirectory: true)
    } ?? FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".bot", isDirectory: true)
    return stateDir.appendingPathComponent("bot.json")
}

private func botEnvironmentPath(_ key: String) -> String? {
...
func loadGatewayConfig() -> GatewayConfig {
    guard let data = try? Data(contentsOf: resolveBotConfigURL()) else { return GatewayConfig() }
