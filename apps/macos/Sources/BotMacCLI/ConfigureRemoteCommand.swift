...
private let appDefaultsSuites = ["ai.bot.mac", "ai.bot.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              bot-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
              ...
              bot-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote Bot checkout for CLI commands.
              --cli-path <path>   Remote bot executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
    ...
    defaultsSuites: [String]) throws -> ConfigureRemoteOutput
{
    ...
    let configURL = resolveBotConfigURL()
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String, suites: [String]) {
    for suite in suites {
        ...
        defaults.set("remote", forKey: "bot.connectionMode")
        setDefaultString(defaults, key: "bot.remoteTarget", value: target)
        defaults.set(true, forKey: "bot.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "bot.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "bot.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "bot.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "bot.remoteCliPath", value: opts.cliPath)
