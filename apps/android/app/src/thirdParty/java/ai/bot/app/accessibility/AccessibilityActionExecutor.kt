package ai.bot.app.accessibility
...
class AccessibilityActionExecutor internal constructor(
  private val connectionProvider: () -> AccessibilityServiceConnection<BotAccessibilityService> = {
    BotAccessibilityService.connection.value
  },
  private val captureSnapshot: (BotAccessibilityService) -> AccessibilitySnapshotCapture =
    AccessibilitySnapshotter()::capture,
  private val foregroundPackageProvider: (BotAccessibilityService) -> String? =
    BotAccessibilityService::foregroundPackageName,
  private val uiEpochProvider: () -> Long = { BotAccessibilityService.uiEpoch },
) : AutoCloseable {
  ...
  private fun coordinateGesturePreflight(service: BotAccessibilityService): ActionResult? {
  ...
  private fun nodeActionPackagePreflight(service: BotAccessibilityService): ActionResult? {
    val expectedPackage = generation.packageName
    val currentPackage = foregroundPackageProvider(service)
    if (expectedPackage == null || currentPackage == null || expectedPackage != currentPackage) {
      return ActionResult(
