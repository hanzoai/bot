...
class NodeForegroundService : Service() {
    ...
    runtimeRestoreJob =
      scope.launch(Dispatchers.Default) {
        try {
          ...
          Log.e("BotNodeService", "Failed to restore node runtime", err)
          withContext(Dispatchers.Main) {
            runtimeRestoreJob = null
          ...
          val title =
            when {
              state.connected && state.mode == VoiceCaptureMode.TalkMode ->
                nativeString("Bot Node · Talk")
              state.connected -> nativeString("Bot Node · Connected")
              else -> nativeString("Bot Node")
            }
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = nativeString("Bot node connection status")
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.bot.app.action.STOP"
    private const val ACTION_RESUME = "ai.bot.app.action.RESUME"
    private const val ACTION_SET_VOICE_CAPTURE_MODE = "ai.bot.app.action.SET_VOICE_CAPTURE_MODE"
    private const val EXTRA_VOICE_CAPTURE_MODE = "ai.bot.app.extra.VOICE_CAPTURE_MODE"
