...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    if (command.startsWith(BotCanvasCommand.NamespacePrefix)) {
  ...
  private suspend fun dispatchInvoke(
    ...
    // Command strings come from BotProtocolConstants; the registry above owns advertised availability.
    return when (command) {
      // Canvas commands
      ...
      BotDeviceCommand.Apps.rawValue -> deviceHandler.handleDeviceApps(paramsJson)
      ...
      // System command
      BotSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      BotTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      BotTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      BotTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      BotTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      ai.bot.app.protocol.BotPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      BotContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      BotContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      BotCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      BotCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      BotMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      BotMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      BotSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      BotSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      BotCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)

      // Mobile accessibility commands
      BotMobileUiCommand.Observe.rawValue -> mobileUiHandler.handleObserve(paramsJson)
      BotMobileUiCommand.Act.rawValue -> mobileUiHandler.handleAct(paramsJson)
