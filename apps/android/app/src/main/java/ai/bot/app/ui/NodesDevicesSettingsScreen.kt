...
import ai.bot.app.GatewayDevicePairingCapabilities
import ai.bot.app.GatewayDevicePairingMutation
import ai.bot.app.GatewayDeviceTokenSummary
import ai.bot.app.GatewayNodeApprovalState
import ai.bot.app.GatewayNodeSummary
import ai.bot.app.GatewayNodesDevicesSummary
import ai.bot.app.GatewayPairedDeviceSummary
import ai.bot.app.GatewayPendingDeviceSummary
import ai.bot.app.MainViewModel
import ai.bot.app.canApproveGatewayDevicePairing
...
private fun nodeSubtitle(node: GatewayNodeSummary): String {
  ...
  val version = node.version?.let { "Bot $it" }
...
internal fun devicePairingAdminUnavailableText(): String =
  nativeString(
    "Device pairing actions are unavailable in this Gateway session. Run bot devices list on the Gateway host and manage the request there. Node capability approval is separate and still uses nodes approve <request id>.",
  )
