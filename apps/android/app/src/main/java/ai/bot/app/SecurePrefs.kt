...
import ai.bot.app.gateway.GatewayCustomHeaders
import ai.bot.app.gateway.GatewayRegistryStore
import ai.bot.app.gateway.GatewayStoreMigration
...
class SecurePrefs(
  ...
  companion object {
    ...
    private const val plainPrefsName = "bot.node"
    private const val securePrefsName = "bot.node.secure"
  ...
  internal fun getNotificationForwardingPolicy(appPackageName: String): NotificationForwardingPolicy {
    ...
    // Always block Bot's own notifications in blocklist mode to prevent forwarding loops.
