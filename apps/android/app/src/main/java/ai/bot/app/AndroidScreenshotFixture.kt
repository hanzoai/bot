...
internal object AndroidScreenshotFixture {
  ...
  fun request(
    ...
    when (method) {
      ...
      "cron.runs" -> cronRuns()
      "bot.chat" -> systemAgentChat(paramsJson)
  ...
  private fun systemAgentChat(paramsJson: String?): String {
    ...
    return buildJsonObject {
      put("sessionId", JsonPrimitive("android-screenshot-bot"))
      put(
        "reply",
        JsonPrimitive(
          if (message == null) {
        ...
        buildJsonArray {
          add(
            buildJsonObject {
              ...
              put("description", JsonPrimitive("Show current Bot status"))
