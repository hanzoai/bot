...
import ai.bot.app.MainActivity
...
class SystemHandlerTest {
  ...
  fun handleSystemNotify_rejectsInvalidRequestObject() {
    ...
    val result = handler.handleSystemNotify("""{"title":"Bot"}""")
  ...
  fun handleSystemNotify_trimsAndPassesOptionalFields() {
    ...
    val result =
      handler.handleSystemNotify(
        """{"title":" Bot ","body":" done ","priority":" passive ","sound":" silent "}""",
      )

    assertTrue(result.ok)
    assertEquals("Bot", poster.lastRequest?.title)
  ...
  fun buildSystemNotificationSetsImmutableAppLaunchIntent() {
    ...
    val notification =
      buildSystemNotification(
        appContext = context,
        channelId = "test",
        request = SystemNotifyRequest("Bot", "done", sound = null, priority = null),
      )
