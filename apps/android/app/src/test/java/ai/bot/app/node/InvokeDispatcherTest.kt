...
import ai.bot.app.gateway.testDeviceIdentityStore
...
class InvokeDispatcherTest {
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(BotSmsCommand.Search.rawValue, "not-json")
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(BotSmsCommand.Send.rawValue, """{"to":"+15551234567","message":"hi"}""")
  ...
  fun handleInvoke_blocksCameraCommandsWhenCameraDisabled() =
    runTest {
      val result = newDispatcher(cameraEnabled = false).handleInvoke(BotCameraCommand.List.rawValue, null)
  ...
  fun handleInvoke_blocksLocationCommandWhenLocationDisabled() =
    runTest {
      val result = newDispatcher(locationEnabled = false).handleInvoke(BotLocationCommand.Get.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(installedAppsSharingEnabled = false)
          .handleInvoke(BotDeviceCommand.Apps.rawValue, """{"limit":1}""")
    ...
    runTest {
      val result =
        newDispatcher(motionActivityAvailable = false)
          .handleInvoke(BotMotionCommand.Activity.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(motionPedometerAvailable = false)
          .handleInvoke(BotMotionCommand.Pedometer.rawValue, null)
  ...
  fun handleInvoke_blocksCallLogWhenUnavailable() =
    runTest {
      val result =
        newDispatcher(callLogAvailable = false).handleInvoke(BotCallLogCommand.Search.rawValue, null)
  ...
  fun handleInvoke_blocksPhotosWhenUnavailable() =
    runTest {
      val result = newDispatcher(photosAvailable = false).handleInvoke(BotPhotosCommand.Latest.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(mobileUiAvailable = false)
          .handleInvoke(BotMobileUiCommand.Observe.rawValue, null)
  ...
  fun handleInvoke_routesTalkPttCommands() =
    runTest {
      ...
      val start = dispatcher.handleInvoke(BotTalkCommand.PttStart.rawValue, null)
      val stop = dispatcher.handleInvoke(BotTalkCommand.PttStop.rawValue, null)
      val cancel = dispatcher.handleInvoke(BotTalkCommand.PttCancel.rawValue, null)
      val once = dispatcher.handleInvoke(BotTalkCommand.PttOnce.rawValue, null)
  ...
  fun handleInvoke_presentAndHideDriveTheShellOwnedCanvasState() =
    runTest {
      ...
      val hide = dispatcher.handleInvoke(BotCanvasCommand.Hide.rawValue, null)
    ...
    runTest {
      ...
      val result =
        newDispatcher(isForeground = false, canvas = canvas)
          .handleInvoke(BotCanvasCommand.Present.rawValue, """{"url":"https://example.com"}""")
