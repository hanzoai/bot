...
class SessionDashboardScreenTest {
  ...
  fun originRuleDropsBasePathAndKeepsPort() {
    assertEquals(
      ...
      controlUiOriginRule("https://gateway.example.com:8443/bot"),
    )
  ...
  fun dashboardUrlKeepsConfiguredControlUiBasePath() {
    val url =
      sessionDashboardUrl(
        baseUrl = "https://gateway.example.com:8443/bot",
        sessionKey = "agent:main:qa",
      )

    assertEquals(
      "https://gateway.example.com:8443/bot/chat?session=agent%3Amain%3Aqa&face=dashboard",
      url,
    )
  }
}
