...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the Bot app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=ai.bot.app
```
...
BOT_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
BOT_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
BOT_ANDROID_STORE_PASSWORD=<store-password>
BOT_ANDROID_KEY_ALIAS=<upload-key-alias>
BOT_ANDROID_KEY_PASSWORD=<key-password>
```
...
- Stable GitHub Release APK publication is separate from Google Play: `Bot Release Publish` dispatches `.github/workflows/android-release.yml`, whose protected `android-release` environment provides `MATCH_PASSWORD`; the repository GitHub App reads the encrypted signing repo.
