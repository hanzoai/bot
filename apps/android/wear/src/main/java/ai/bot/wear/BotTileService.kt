...
class BotTileService :
  Material3TileService(
    allowDynamicTheme = false,
    defaultColorScheme = botTileColorScheme,
  ) {
  override suspend fun MaterialScope.tileResponse(requestParams: RequestBuilders.TileRequest): TileBuilders.Tile {
    val talkAction = wearLaunchAction(this@BotTileService, WearLaunchTarget.Voice)
    val openAction = wearLaunchAction(this@BotTileService, WearLaunchTarget.Chat)
    val talkClickable = clickable(action = talkAction, id = "talk_bot")
    val openClickable = clickable(action = openAction, id = "open_bot")
    val layout =
      primaryLayout(
          ...
          avatarButton(
            ...
            avatarContent = {
              avatarImage(
                ...
                protoLayoutResourceId = "bot_core_mascot",
              )
            },
...
private val botTileColorScheme =
  ColorScheme(
