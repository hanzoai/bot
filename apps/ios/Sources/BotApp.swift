...
@MainActor
enum BotAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class BotAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.botfoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.botfoundation.app", category: "BackgroundWake")
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? BotAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "bot.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "bot.watch.promptId"
    static let sessionKeyKey = "bot.watch.sessionKey"
    static let gatewayStableIDKey = "bot.watch.gatewayStableID"
    static let actionPrimaryIDKey = "bot.watch.action.primary.id"
    static let actionPrimaryLabelKey = "bot.watch.action.primary.label"
    static let actionSecondaryIDKey = "bot.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "bot.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "bot.watch.action.primary"
    static let actionSecondaryIdentifier = "bot.watch.action.secondary"
    static let actionIdentifierPrefix = "bot.watch.action."
    static let actionIDKeyPrefix = "bot.watch.action.id."
    static let actionLabelKeyPrefix = "bot.watch.action.label."
    static let categoryPrefix = "bot.watch.prompt.category."
        ...
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> BotWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return BotWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Bot" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [BotWatchAction]) -> [UNNotificationAction] {
...
struct BotApp: App {
    ...
    @UIApplicationDelegateAdaptor(BotAppDelegate.self) private var appDelegate
    @Environment(\.scenePhase) private var scenePhase

    init() {
        Self.installUncaughtExceptionLogger()
        GatewaySettingsStore.bootstrapPersistence()
        BotType.installUIKitAppearance()
        let appModel = NodeAppModel(audioAdmissionInitiallyAllowed: false)
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--bot-reset-onboarding") {
        ...
        #endif
        BotAppModelRegistry.appModel = appModel
        ...
        WindowGroup {
            RootTabs()
                ...
                .onReceive(
                    ...
                    perform: { _ in
                        BotType.refreshUIKitAppearance(in: Self.connectedWindows())
                    })
                .onOpenURL { url in
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--bot-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--bot-screenshot-notification-guidance")
        #else
        false
        #endif
    }
    ...
    private static var liveActivityVoicePreviewEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--bot-live-activity-voice-preview")
        #else
        false
        #endif
    }
    ...
    private func applyWindowTint() {
        for window in Self.connectedWindows() {
            window.tintColor = BotBrand.uiAccent
        }
    }
