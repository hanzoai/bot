...
final class ContactsService: ContactsServicing {
    ...
    func search(params: BotContactsSearchParams) async throws -> BotContactsSearchPayload {
        ...
        return BotContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: BotContactsAddParams) async throws -> BotContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return BotContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return BotContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> BotContactPayload {
        BotContactPayload(
