...
final class RemindersService: RemindersServicing {
    ...
    func list(params: BotRemindersListParams) async throws -> BotRemindersListPayload {
        ...
        let payload: [BotReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return BotRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: BotRemindersAddParams) async throws -> BotRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = Self.date(fromDueComponents: reminder.dueDateComponents)
        let payload = BotReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return BotRemindersAddPayload(reminder: payload)
    }
