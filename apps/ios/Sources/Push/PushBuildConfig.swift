...
struct PushBuildConfig {
    ...
    static let botHostedRelayHost = "ios-push-relay.bot.ai"
    static let botSandboxRelayHost = "ios-push-relay-sandbox.bot.ai"

    var usesBotHostedRelay: Bool {
        ...
        return components.scheme?.lowercased() == "https"
            && [Self.botHostedRelayHost, Self.botSandboxRelayHost]
    ...
    private init(readValue: (String) -> Any?) {
        ...
        case .appStore:
            ...
            self.relayBaseURL = URL(string: "https://\(Self.botHostedRelayHost)")!
            self.apnsEnvironment = .production
            self.relayProfile = .production
            self.proofPolicy = .appleStrict
        case .deviceSandbox:
            ...
            self.relayBaseURL = relayBaseURLOverride
                ?? URL(string: "https://\(Self.botSandboxRelayHost)")!
            self.apnsEnvironment = .sandbox
            self.relayProfile = .deviceSandbox
            self.proofPolicy = .appleDevelopment
        case .simulatorSandbox:
