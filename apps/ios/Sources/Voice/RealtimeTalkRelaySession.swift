...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "bot_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.botfoundation.app", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable], lifecycleGeneration: UInt64) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "Bot tool call failed"]
                : ["text": completion.text ?? "Bot finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: BotChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
