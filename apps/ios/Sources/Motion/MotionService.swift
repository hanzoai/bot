...
final class MotionService: MotionServicing {
    func activities(params: BotMotionActivityParams) async throws -> BotMotionActivityPayload {
        ...
        let mapped: [BotMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return BotMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = BotPedometerPayload(
