...
enum LocationSettingsAction: Equatable {
    case setMode(BotLocationMode)
    case openAppSettings(BotLocationMode)
}

struct LocationSettingsPresentation: Equatable {
    var selectedMode: BotLocationMode
    ...
    var statusText: String? {
        ...
        case .authorizedWhenInUse, .authorizedAlways:
            return nil
        default:
            return String(localized: "Bot cannot determine the current iOS location permission.")
        }
    }

    func toggleAction(defaultEnabledMode: BotLocationMode = .whileUsing) -> LocationSettingsAction {
    ...
    func accessLevelAction(mode: BotLocationMode) -> LocationSettingsAction {
        self.enableAction(mode: mode)
    }

    private func enableAction(mode: BotLocationMode) -> LocationSettingsAction {
        if !self.summary.locationServicesEnabled {
