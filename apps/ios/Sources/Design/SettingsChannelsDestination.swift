...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return BotBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? BotBrand.ok : BotBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return BotBrand.ok }
        if self.lastError != nil { return BotBrand.warn }
        return self.configured ? BotBrand.accent : .secondary
    }
