...
extension AgentProTab {
    ...
    var clawHubSearchCard: some View {
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 10) {
                    ProIconBadge(systemName: "square.and.arrow.down", color: BotBrand.accent)
        ...
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let message = self.skillConfigMessages[skill.effectiveSkillKey] {
                    Text(message.text)
                        .font(BotType.caption2)
                        .foregroundStyle(message.kind == .success ? BotBrand.accent : BotBrand.warn)
                }
            }
        }
        .padding(.horizontal, BotProMetric.pagePadding)
    ...
    func skillStatus(_ skill: SkillStatusEntryLite) -> (text: String, color: Color) {
        ...
        if skill.hasMissingRequirements {
            return ("setup", BotBrand.warn)
        }
        return ("enabled", BotBrand.accent)
    }
