...
struct CommandSessionSection: Identifiable {
    ...
    let id: ID
    let title: String
    let entries: [BotChatSessionEntry]
    let showsHeader: Bool
}

enum CommandSessionGrouping {
    static func sections(
        from entries: [BotChatSessionEntry],
    ...
    static func previewOrder(_ entries: [BotChatSessionEntry]) -> [BotChatSessionEntry] {
    ...
    static func previewSelection(
        _ entries: [BotChatSessionEntry],
        currentKey: String,
        limit: Int = 3) -> [BotChatSessionEntry]
    {
    ...
    static func members(
        of group: String,
        in lists: [[BotChatSessionEntry]]) -> [BotChatSessionEntry]
    {
    ...
    static func activityTimestamp(_ entry: BotChatSessionEntry) -> Double {
        entry.lastActivityAt ?? entry.updatedAt ?? 0
    }

    private static func sortedByActivity(
        _ entries: [BotChatSessionEntry]) -> [BotChatSessionEntry]
    {
        entries.sorted { lhs, rhs in
            let left = self.activityTimestamp(lhs)
            let right = self.activityTimestamp(rhs)
