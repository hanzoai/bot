...
enum AppAppearancePreference: String, CaseIterable, Identifiable {
    ...
    static var launchArgumentPreference: AppAppearancePreference? {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--bot-appearance") else {
            return nil
        }
...
extension TalkWaveformPalette {
    ...
    static let botBrand = TalkWaveformPalette(
        active: [
            BotBrand.carapaceCoral,
            BotBrand.carapaceSea,
            BotBrand.accent,
        ],
        inactive: [
...
struct BotActivationGlyph: View {
    let size: CGFloat
    ...
    var body: some View {
        BotMascotView(floats: false, mood: self.mood, interactive: self.interactive)
            .frame(width: self.size, height: self.size)
            .shadow(
                color: BotBrand.activationGlow.opacity(0.18),
...
extension View {
    func botSheetChrome() -> some View {
        self
            .tint(BotBrand.accent)
            .background {
                BotBrand.sheetBackground
                    .ignoresSafeArea()
            }
    }

    func botCraftSurface(cornerRadius: CGFloat = 24, shadow: Bool = true) -> some View {
        self.modifier(BotCraftSurfaceModifier(cornerRadius: cornerRadius, shadow: shadow))
    }
}

private struct BotCraftSurfaceModifier: ViewModifier {
        ...
        content
            .background {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .fill(BotBrand.activationSurface)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .stroke(BotBrand.activationHairline, lineWidth: 0.5)
            }
    }
}

struct BotPrimaryActionButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(self.isEnabled ? BotBrand.activationPrimaryActionText : Color.secondary)
            .tint(self.isEnabled ? BotBrand.activationPrimaryActionText : Color.secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .fill(self.isEnabled ? Self.primaryFill : BotBrand.activationDisabledGradient)
                    .shadow(
                        color: self.isEnabled ? BotBrand.activationPrimaryAction.opacity(0.08) : .clear,
                        radius: 1,
                        x: 0,
                        y: 1)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(
                        self.isEnabled
                            ? BotBrand.activationPrimaryAction.opacity(0.72)
                            : BotBrand.activationNeutralStroke,
                        lineWidth: 0.75)
            }
...
struct BotSecondaryActionButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(BotType.subheadSemiBold)
            .foregroundStyle(self.isEnabled ? BotBrand.activationPrimaryAction : .secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(BotBrand.activationHairline, lineWidth: 0.75)
            }
            .scaleEffect(configuration.isPressed && self.isEnabled ? 0.98 : 1)
            .animation(.smooth(duration: 0.14), value: configuration.isPressed)
    }
    ...
    private static var secondaryFill: LinearGradient {
        LinearGradient(
            colors: [
                BotBrand.activationSecondaryActionTop,
                BotBrand.activationSecondaryActionBottom,
            ],
            startPoint: .top,
            endPoint: .bottom)
    }
...
struct BotCloseButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
        ...
        configuration.label
            ...
            .background {
                Capsule(style: .continuous)
                    .fill(BotBrand.activationNeutralGradient)
                    .shadow(
