...
/// `BotBrand.activationPrimaryAction`; keep both in sync when rebranding.
final class ShareComposeView: UIView, UITextViewDelegate {
    ...
    func apply(_ status: ShareComposeStatus) {
        ...
        case .sending:
            self.showFooter(
                text: NSLocalizedString("Sending to Bot gateway…", comment: "Share extension sending status"),
                icon: nil,
                spinning: true)
        case .sent:
            self.showFooter(
                text: NSLocalizedString("Sent to Bot.", comment: "Share extension success status"),
    ...
    private static func makeSendConfiguration() -> UIButton.Configuration {
        ...
        configuration.attributedTitle = self.attributedTitle(
            NSLocalizedString("Send to Bot", comment: "Share extension send action"),
            style: .headline)
