...
final class WatchDirectNode {
    ...
    private static let keychainService = "ai.bot.watch.direct-node"
    ...
    func disconnectForBackground() {
        ...
        if self.isEnabled, self.isConfigured {
            self.statusText = String(localized: "Reconnects when Bot is active")
        }
    }
        ...
        notificationsAuthorized: Bool) throws -> ConnectParams
    {
        ...
        let payload = GatewayDeviceAuthPayload.buildV3(
            fields: .init(
                deviceId: identity.deviceId,
                client: .init(id: "bot-watchos", mode: "node"),
                role: "node",
                scopes: [],
    ...
    private func handleNotification(_ request: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decode(BotSystemNotifyParams.self, from: request.paramsJSON)
    ...
    private func deviceStatus() -> BotDeviceStatusPayload {
        let device = WKInterfaceDevice.current()
        device.isBatteryMonitoringEnabled = true
        let batteryState: BotBatteryState = switch device.batteryState {
        ...
        let battery = BotBatteryStatusPayload(
            level: device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil,
            state: batteryState,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
        let thermalState: BotThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return BotDeviceStatusPayload(
            battery: battery,
            thermal: BotThermalStatusPayload(state: thermalState),
