...
// Canonical transport leaf models live in BotKit. The local envelope types
// below retain the existing Watch inbox persistence shape without `type`.
typealias WatchPayloadType = BotWatchPayloadType
typealias WatchRiskLevel = BotWatchRisk
typealias WatchExecApprovalDecision = BotWatchExecApprovalDecision
typealias WatchExecApprovalTransportOutcome = BotWatchExecApprovalOutcome
typealias WatchExecApprovalCloseReason = BotWatchExecApprovalCloseReason
...
typealias WatchExecApprovalItem = BotWatchExecApprovalItem
...
typealias WatchExecApprovalSnapshotRequestMessage = BotWatchExecApprovalSnapshotRequestMessage
typealias WatchExecApprovalSnapshotRequestItem = BotWatchExecApprovalSnapshotRequestItem
typealias WatchExecApprovalResolveMessage = BotWatchExecApprovalResolveMessage
typealias WatchAppCommand = BotWatchAppCommand
...
struct WatchAppSnapshotMessage: Codable, Equatable {
    var gatewayStatus: BotWatchAppStatus
    ...
    private static func parseStatus(
        _ value: Any?,
        fallbackText: String? = nil) -> BotWatchAppStatus?
    {
        ...
        guard let code = BotWatchAppStatusCode(rawValue: rawCode) else {
            let legacyText = verbatim?.isEmpty == false ? verbatim : fallbackText
            guard let legacyText, !legacyText.isEmpty else { return nil }
            return BotWatchAppStatus(code: .legacy, verbatim: legacyText)
        }
        return BotWatchAppStatus(
            code: code,
...
typealias WatchChatCompletionMessage = BotWatchChatCompletionMessage
typealias WatchChatItem = BotWatchChatItem
typealias WatchAppSnapshotRequestMessage = BotWatchAppSnapshotRequestMessage
typealias WatchAppCommandMessage = BotWatchAppCommandMessage
typealias WatchPromptAction = BotWatchAction
