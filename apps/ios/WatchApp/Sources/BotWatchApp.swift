...
enum WatchScreenshotMode {
    private static let defaultsKey = "bot.watch.screenshotMode"
    static let approvals = ProcessInfo.processInfo.arguments.contains(
        "--bot-watch-approval-screenshot-mode")
        || ProcessInfo.processInfo.environment["BOT_WATCH_APPROVAL_SCREENSHOT_MODE"] == "1"
    static let enabled = ProcessInfo.processInfo.arguments.contains("--bot-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["BOT_WATCH_SCREENSHOT_MODE"] == "1"
    ...
    fileprivate func configureScreenshotFixture(includeApproval: Bool = false) {
        ...
        self.consume(
            appSnapshot: WatchAppSnapshotMessage(
                ...
                gatewayStableID: "watch-screenshot-gateway",
                talkStatus: BotWatchAppStatus(code: .talkReady),
                talkEnabled: true,
                talkListening: false,
                talkSpeaking: false,
