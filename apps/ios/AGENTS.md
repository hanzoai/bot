...
- iOS SwiftUI text should use branded typography helpers, not bare system fonts. Use `BotType` in `apps/ios/Sources/**`, `BotChatTypography` in shared chat UI, `WatchClawType` in watch UI, and `BotActivityType` in Live Activity UI.
...
- When touching iOS text surfaces, update/keep `apps/ios/Tests/BotTypographyTests.swift` so bare text/control regressions are caught, then run the focused typography test.
...
- Do not add Bot, Bot Foundation, or other first-party/self-owned license entries. The screen is for third-party/open-source dependency acknowledgements.
- When adding, removing, or upgrading iOS dependencies, audit whether `apps/ios/Resources/Licenses/` needs updates. Exclude dependencies owned by Bot Foundation from the published license list.
