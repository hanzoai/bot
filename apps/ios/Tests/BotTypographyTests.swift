...
struct RootSidebarTypographyTests {
    @Test func `root sidebar uses branded typography`() throws {
        ...
        #expect(sidebar.contains("BotType.headline"))
        #expect(sidebar.contains("BotType.subheadSemiBold"))
        #expect(sidebar.contains("BotType.captionMedium"))
        #expect(sidebar.contains("BotType.caption2Bold"))
        #expect(!sidebar.contains(".font(."))
    }
}

struct BotTypographyTests {
    @Test func `thread controls use branded typography`() throws {
        ...
        #expect(support.contains(".font(BotType.subhead)"))
        #expect(support.contains(".font(BotType.subheadSemiBold)"))
        #expect(commandCenter.contains("Toggle(isOn: self.$showArchived)"))
        #expect(commandCenter.contains("Text(\"Show Archived\")"))
        #expect(commandCenter.contains(".font(BotType.captionMedium)"))
    }

    @Test func `bundled fonts load from app bundle`() {
        for name in BotType.registeredPostScriptNames {
    ...
    @Test func `app extensions register bundled branded fonts`() throws {
        ...
        for targetName in ["BotActivityWidget", "BotWatchApp"] {
            let target = try Self.extract(
                project,
                from: "  \(targetName):",
                to: targetName == "BotActivityWidget" ? "  BotWatchApp:" : "  BotTests:")
    ...
    @Test func `extension text surfaces use branded typography helpers`() throws {
        ...
        #expect(activitySource.contains("BotActivityType.subheadSemiBold"))
        #expect(activitySource.contains("BotActivityType.subheadBold"))
        #expect(activitySource.contains("BotActivityType.caption"))
    ...
    @Test func `UIKit typography refreshes when Dynamic Type changes`() throws {
        ...
        let typographySource = try String(
            contentsOf: Self.sourceURL("Design/BotTypography.swift"),
            encoding: .utf8)

        #expect(appSource.contains("UIContentSizeCategory.didChangeNotification"))
        #expect(appSource.contains("BotType.refreshUIKitAppearance(in: Self.connectedWindows())"))
    ...
    @Test func `listed iOS app surfaces enforce branded control typography`() throws {
        let proComponents = try String(
            contentsOf: Self.sourceURL("Design/BotProComponents.swift"),
            encoding: .utf8)
        ...
        let automations = try String(
            ...
            encoding: .utf8)
        let docs = try String(contentsOf: Self.sourceURL("Design/BotDocsScreen.swift"), encoding: .utf8)
        ...
        let chatTypography = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/BotKit/Sources/BotChatUI/ChatTypography.swift"),
            encoding: .utf8)
        let chatMessageViews = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/BotKit/Sources/BotChatUI/ChatMessageViews.swift"),
            encoding: .utf8)
        let chatWorkingClawView = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/BotKit/Sources/BotChatUI/ChatWorkingClawView.swift"),
            encoding: .utf8)
        let chatMarkdownRenderer = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/BotKit/Sources/BotChatUI/ChatMarkdownRenderer.swift"),
            encoding: .utf8)

        #expect(automations.contains(".font(BotType.body)"))
        #expect(automations.contains(".font(BotType.headline)"))
        #expect(automations.contains(".font(BotType.subheadSemiBold)"))
        #expect(automations.contains(".font(BotType.caption)"))
        #expect(!automations.contains(".font(.body"))
        #expect(!automations.contains(".font(.headline"))
        #expect(!automations.contains(".font(.caption"))

        #expect(proComponents.contains(".font(BotType.subheadSemiBold)"))
        ...
        #expect(chatTab.contains(".font(BotType.body)"))
        ...
        #expect(quickSetup.contains(".font(BotType.subheadSemiBold)"))
        ...
        #expect(gatewayProblem.contains(".font(BotType.subheadSemiBold)"))
        ...
        #expect(onboardingSteps.contains(".font(BotType.subheadSemiBold)"))
        ...
        #expect(onboardingWizard.contains(".font(BotType.subheadSemiBold)"))
        ...
        let onboardingSecureOption = try Self.extract(
            ...
            to: ".tag(true)")
        #expect(onboardingUnencryptedOption.contains(".font(BotType.captionSemiBold)"))
        #expect(onboardingSecureOption.contains(".font(BotType.captionSemiBold)"))

        #expect(settingsSections.contains(".font(BotType.body)"))
        #expect(settingsSections.contains("Text(warningText)"))
        #expect(settingsSections.contains(".font(BotType.caption)"))
        #expect(approvalDialog.contains("Text(warningText)"))
        #expect(approvalDialog.contains(".font(BotType.footnote)"))
        ...
        #expect(settingsSections.contains("BotToggleIndicator(isOn: isOn.wrappedValue)"))
        #expect(settingsSections.contains("TextField(\"Default Share Instruction\""))
        #expect(settingsSections.contains(".font(BotType.subhead)"))
        ...
        #expect(settingsSections.contains(".font(BotType.subheadSemiBold)"))
        #expect(settingsSupport.contains("struct SettingsBuildMetadataStrip"))
        #expect(settingsSupport.contains(".font(BotType.caption2SemiBold)"))
        #expect(settingsSupport.contains(".font(BotType.monoSmall)"))
        ...
        let settingsSecureOption = try Self.extract(
            ...
            from: "Text(\"Secure (TLS)\")",
            to: ".tag(true)")
        #expect(settingsUnencryptedOption.contains(".font(BotType.captionSemiBold)"))
        #expect(settingsSecureOption.contains(".font(BotType.captionSemiBold)"))
        ...
        #expect(appleHealthAccess.contains(".font(BotType.footnote)"))
        ...
        #expect(permissionRow.contains(".font(BotType.footnoteSemiBold)"))
        ...
        #expect(skills.contains("Text(\"Gateway warning\").font(BotType.headline)"))
        #expect(skills.contains("Text(\"Acknowledge and install\").font(BotType.subheadSemiBold)"))
        #expect(skills.contains("prompt: Text(\"Search ClawHub\").font(BotType.body)"))
    ...
    private static func activityWidgetSourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/BotLiveActivity.swift")
    }

    private static func activityWidgetTypographySourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/BotActivityTypography.swift")
    }
    ...
    private static func swiftSourcesForTypographyAudit() throws -> [URL] {
        let roots = [
            ...
            self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/BotKit/Sources/BotChatUI"),
        ]
    ...
    private static func unbrandedTextCallOffenders() throws -> [String] {
        let fontTokens = ["BotType", "BotChatTypography"]
    ...
    private static func hasAllowedBrandedFontParameter(_ window: String, line: String, in url: URL) -> Bool {
        ...
        case "apps/ios/Sources/Design/BotProComponents.swift":
        ...
        case "apps/shared/BotKit/Sources/BotChatUI/ChatMarkdownRenderer.swift":
