...
struct BotBrandTests {
    @Test func `brand colors meet text contrast in both appearances`() {
        let foregroundColors = [
            ("accent", BotBrand.uiAccentForeground),
            ("accentHot", BotBrand.uiAccentHotForeground),
            ("ok", BotBrand.uiOK),
            ("warn", BotBrand.uiWarn),
            ("danger", BotBrand.uiDanger),
            ("info", BotBrand.uiInfo),
        ]
        ...
        for style in [UIUserInterfaceStyle.light, .dark] {
            ...
            let pillBackground = Self.composite(
                BotBrand.uiAccentForeground,
                alpha: style == .dark ? 0.12 : 0.08,
                over: .secondarySystemGroupedBackground,
                traits: traits)
            #expect(Self.contrastRatio(BotBrand.uiAccentForeground, pillBackground, traits: traits) >= 4.5)
            #expect(Self.contrastRatio(BotBrand.uiAccent, .white, traits: traits) >= 4.5)
        }
    }
