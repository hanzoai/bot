...
@MainActor
final class BotSnapshotUITests: XCTestCase {
    ...
    func testOnboardingExplainsCapabilitiesAndTrust() {
        ...
        app.launchArguments += ["--bot-reset-onboarding"]
        app.launch()
        self.app = app
    ...
    func testSidebarMoreAgentsMenuShowsAvatarsAndKeepsFooterVisible() throws {
        ...
        let gatewayFooter = try XCTUnwrap(self.app?.buttons.matching(
            NSPredicate(format: "label CONTAINS %@", "Bot Gateway")).firstMatch)
    ...
    func testEmptyChatStarterPromptSendsMessage() throws {
        self.launchApp(
            ...
            additionalArguments: ["--bot-empty-chat-fixture"])
        ...
        starter.tap()
        let sentText = "Summarize the current Bot status and tell me what needs attention."
    ...
    func testEmptyChatStarterPromptsLocalizeInGerman() throws {
        ...
        let starter = try XCTUnwrap(self.app?.buttons["Bot-Status prüfen"])
        XCTAssertTrue(starter.exists)
        starter.tap()
        XCTAssertTrue(
            self.app?.staticTexts[
                "Fasse den aktuellen Bot-Status zusammen und sage mir, was Aufmerksamkeit erfordert.",
    ...
    func testLiveGatewayChatRoundTripAndControlOverview() throws {
        ...
        for index in 0..<3 {
            let seedMarker = "BOT_E2E_SEED_\(index)_\(Int(Date().timeIntervalSince1970 * 1000))"
        ...
        let replyMarker = "BOT_E2E_OK_\(Int(Date().timeIntervalSince1970 * 1000))"
        self.sendLiveGatewayMessage(
    ...
    func testManualAuthRetryUsesEditedToken() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["BOT_IOS_RETRY_E2E"] == "1",
            "Set BOT_IOS_RETRY_E2E=1 with a local token-auth Gateway on port 18920")
        let token = try XCTUnwrap(ProcessInfo.processInfo.environment["BOT_IOS_RETRY_TOKEN"])
    ...
    func testPhotosLimitedAccess() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["BOT_IOS_PHOTOS_E2E"] == "1",
            "Set BOT_IOS_PHOTOS_E2E=1 to exercise the system Photos prompt")
...
extension BotSnapshotUITests {
        ...
        additionalArguments: [String] = [])
    {
        ...
        XCTAssertTrue(
            ...
            "Bot root readiness marker did not appear")
        ...
        additionalArguments: [String]) -> XCUIApplication
    {
        ...
        app.launchArguments += [
            ...
            "hidden",
            "--bot-ui-test-readiness",
        ]
        if screenshotMode {
            app.launchArguments.append("--bot-screenshot-mode")
        }
        app.launchArguments += additionalArguments
        if let appearance {
            app.launchArguments += ["--bot-appearance", appearance]
        }
        return app
    }
    ...
    private func waitForReleaseScreenshotTarget(_ target: ScreenshotTarget) {
        guard let app = self.app else {
            XCTFail("Bot is not running for screenshot target \(target.name)")
            return
        }
        let readiness = app.descendants(matching: .any)[Self.appReadinessAccessibilityIdentifier]
        self.waitForValue("ready:\(target.initialDestination)", of: readiness, timeout: 8)
        ...
        line: UInt = #line)
    {
        ...
        XCTAssertTrue(
            ...
            "Bot did not terminate before the next launch",
            file: file,
            line: line)
        self.app = nil
    }
    ...
    {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["BOT_IOS_LIVE_GATEWAY"] == "1",
            "Set BOT_IOS_LIVE_GATEWAY=1 and provide a fresh setup code")

        if let setupCode = ProcessInfo.processInfo.environment["BOT_IOS_LIVE_SETUP_CODE"] {
