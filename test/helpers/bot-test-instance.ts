// Bot test instance helper spawns isolated Bot processes.
...
import {
  createBotTestState,
  type BotTestState,
} from "../../src/test-utils/bot-test-state.js";
import { sleep } from "../../src/utils.js";

type BotTestStateOptions = NonNullable<Parameters<typeof createBotTestState>[0]>;

type BotTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<BotTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

type BotTestInstanceCommandResult = {
  code: number | null;
...
type BotTestProcess = ChildProcessByStdio<null, Readable, Readable>;

export type BotTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  stdout: string[];
  stderr: string[];
  child?: BotTestProcess;
  env: NodeJS.ProcessEnv;
  entrypoint: () => Promise<string[]>;
  cli: (
    args: string[],
    options?: { timeoutMs?: number },
  ) => Promise<BotTestInstanceCommandResult>;
...
type BotTestChildProcess = Pick<BotTestProcess, "kill" | "pid">;
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseBotTestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalBotTestProcess(child, "SIGKILL");
...
async function waitForPortOpen(
  proc: Pick<BotTestProcess, "exitCode" | "signalCode">,
  chunksOut: string[],
  chunksErr: string[],
  port: number,
  timeoutMs: number,
...
async function waitForGatewayExit(child: BotTestProcess, timeoutMs: number): Promise<boolean> {
...
function hasChildExited(child: Pick<BotTestProcess, "exitCode" | "signalCode">) {
...
export async function createBotTestInstance(
  options: BotTestInstanceOptions,
): Promise<BotTestInstance> {
  ...
  let child: BotTestProcess | undefined;
  let cleaned = false;

  const instance: BotTestInstance = {
    name: options.name,
    port,
    ...
    stopGateway: async () => {
      ...
      if (!hasChildExited(child) && !child.killed) {
        try {
          signalBotTestProcess(child, "SIGTERM");
        } catch {
          // ignore
        }
      }
...
function shouldUseBotTestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalBotTestProcess(
  child: BotTestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = (pid, nextSignal) =>
    process.kill(pid, nextSignal),
): void {
  if (shouldUseBotTestProcessGroup() && typeof child.pid === "number") {
    try {
