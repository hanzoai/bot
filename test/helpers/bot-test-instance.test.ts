// Bot test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { createBotTestInstance, testing } from "./bot-test-instance.js";
...
describe("bot test instance", () => {
  ...
  it("signals test instance process groups on POSIX", () => {
    ...
    testing.signalBotTestProcess(child, "SIGKILL", killProcess);
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".bot"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "bot.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.BOT_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.BOT_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.BOT_SKIP_CRON).toBe("0");
