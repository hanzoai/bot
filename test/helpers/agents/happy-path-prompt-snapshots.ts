...
import { createBotCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/bot-happy-path/workspace";
const AGENT_DIR = "/tmp/bot-happy-path/agent";
const SESSION_FILE = "/tmp/bot-happy-path/session.jsonl";
...
const dynamicToolsConfig: BotConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
...
async function createScenarios(codexApi: CodexPromptSnapshotApi): Promise<PromptScenario[]> {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "Bot maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "Bot",
    ConversationLabel: "Bot/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@Bot please verify the Codex happy path too.",
      },
    ],
    Body: "@Bot can you audit whether this prompt path has conflicting silence instructions?",
...
function renderModelBoundPromptLayers(params: {
  ...
  const codexConfigInstructions =
    ...
    typeof params.codexSnapshot.threadStartParams.config.instructions === "string"
      ...
      : "";
  const botDeveloperInstructions = params.codexSnapshot.developerInstructions;
  ...
  return [
      ...
      stableJson({
        ...
        botRuntime: {
          ...
          workspaceBootstrapContextFrom:
            "extensions/codex app-server turn/start input Bot runtime context",
    ...
    markdownFence(
      ...
      stableJson({
        ...
        botDeveloperInstructions: textStats(botDeveloperInstructions),
    ...
    "",
    markdownFence("text", botDeveloperInstructions),
    "",
...
function buildCodexBotRuntimeContext(): string {
  return [
...
function prependCodexBotRuntimeContext(prompt: string): string {
  return [buildCodexBotRuntimeContext(), "", "Current user request:", prompt].join("\n");
}
...
function renderScenarioSnapshot(
  ...
  const codexTurnPromptText = prependCodexBotRuntimeContext(scenario.prompt);
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The Markdown files show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, Bot developer instructions, turn input with simulated Bot workspace bootstrap runtime context, heartbeat collaboration-mode guidance when applicable, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how Bot routes stable profile files into Codex developer instructions, keeps `MEMORY.md` in turn input, and points heartbeat turns at `HEARTBEAT.md` without inlining it. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path Bot tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after Bot sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
