...
const API_KEY = "botrouter-e2e-secret";
const MODEL_ID = "openai/gpt-5.5";
const MODEL_REF = `botrouter/${MODEL_ID}`;
...
describe("BotRouter managed gateway contract", () => {
  it("boots from a SecretRef, reports truthful readiness, and routes an attributed agent turn", async () => {
    ...
    const logFile = path.join(instance.stateDir, "botrouter.log");

    const patchPath = await instance.state.writeText(
      "botrouter.patch.json5",
      JSON.stringify(
        {
          plugins: {
            allow: ["botrouter"],
          ...
          models: {
            providers: {
              botrouter: {
                ...
                headers: { "X-BotRouter-Project-Id": "fakeco-e2e" },
              },
            },
          },
    ...
    expect(config.models?.providers?.botrouter).toMatchObject({
      apiKey: { source: "env", provider: "default", id: "CLAWROUTER_API_KEY" },
      baseUrl: router.baseUrl,
      headers: { "X-BotRouter-Project-Id": "fakeco-e2e" },
    });
    expect(config.agents?.defaults?.model?.primary).toBe(MODEL_REF);
    expect(config.plugins?.allow).toContain("botrouter");
    expect(config.plugins?.entries?.botrouter?.enabled).toBe(true);
    ...
    const catalog = await instance.cli(
      ["models", "list", "--all", "--provider", "botrouter", "--json"],
      { timeoutMs: 120_000 },
    );
    ...
    expect(probe.stdout).toMatch(/"provider"\s*:\s*"botrouter"/u);
    ...
    expect(inferenceRequests.at(-1)).toMatchObject({
      ...
      headers: {
        "x-botrouter-agent-id": "main",
        "x-botrouter-client": "bot",
        "x-botrouter-project-id": "fakeco-e2e",
      },
    });
    const sessionId = inferenceRequests.at(-1)?.headers["x-botrouter-session-id"];
    ...
    expect(fileLog).toContain(
      `[model-fetch] start provider=botrouter api=openai-responses model=${MODEL_ID} method=POST url=${router.baseUrl}/v1/responses`,
    );
    expect(fileLog).toContain(
      `[model-fetch] response provider=botrouter api=openai-responses model=${MODEL_ID} status=200`,
    );
    expect(
      [
        bootstrap.stdout,
...
async function startFakeBotRouter(): Promise<FakeBotRouter> {
  const requests: CapturedRequest[] = [];
  const server = createServer((req, res) => {
    void handleBotRouterRequest(req, res, requests).catch((error) => {
  ...
  if (!address || typeof address === "string") {
    ...
    throw new Error("fake BotRouter did not bind a TCP port");
  }
  return {
...
async function handleBotRouterRequest(
  req: IncomingMessage,
  res: ServerResponse,
  requests: CapturedRequest[],
): Promise<void> {
...
function writeResponsesStream(res: ServerResponse, text: string): void {
  const itemId = "msg_botrouter_e2e";
  const events = [
    {
  ...
  res.writeHead(200, {
    ...
    "x-botrouter-content-retention": "off",
    "x-botrouter-upstream-provider": "openai",
    "x-request-id": "botrouter-e2e-request",
  });
