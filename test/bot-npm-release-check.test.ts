// Bot npm release check tests validate package release checks.
...
import { assertPreparedBotAiDependency } from "../scripts/bot-npm-prepublish-verify.ts";
import {
...
} from "../scripts/bot-npm-release-check.ts";
...
describe("prepared Bot AI dependency", () => {
  it("requires the packed root to depend on the exact prepared AI version", () => {
    expect(() =>
      assertPreparedBotAiDependency({
        aiManifest: { name: "@hanzo/bot-ai", version: "2026.7.2" },
        rootManifest: {
          name: "bot",
          version: "2026.7.1",
          dependencies: { "@hanzo/bot-ai": "2026.7.2" },
        },
      }),
    ).toThrow("Prepared root and @hanzo/bot-ai tarballs must both be version 2026.7.2.");
    ...
    expect(() =>
      assertPreparedBotAiDependency({
        ...
        rootManifest: {
          name: "bot",
          version: "2026.7.2",
          dependencies: { "@hanzo/bot-ai": "2026.7.1" },
        },
      }),
    ).toThrow("Prepared root tarball must depend on exact @hanzo/bot-ai@2026.7.2.");
...
describe("shouldSkipPackedTarballValidation", () => {
  ...
  it("accepts truthy values for metadata-only validation", () => {
    expect(
      shouldSkipPackedTarballValidation({
        BOT_NPM_RELEASE_SKIP_PACK_CHECK: "1",
      }),
    ).toBe(true);
  });
  ...
  it("treats false-like values as disabled", () => {
    expect(
      shouldSkipPackedTarballValidation({
        BOT_NPM_RELEASE_SKIP_PACK_CHECK: "false",
      }),
    ).toBe(false);
  });
});
...
describe("resolveNpmCommandInvocation", () => {
  it("uses npm_execpath when it points to npm", () => {
    expect(
      resolveNpmCommandInvocation({
        ...
        npmArgs: ["view", "bot", "version"],
        nodeExecPath: "/usr/local/bin/node",
        platform: "linux",
      }),
    ).toEqual({
      command: "/usr/local/bin/node",
      args: ["/usr/local/lib/node_modules/npm/bin/npm-cli.js", "view", "bot", "version"],
    });
  });
  ...
  it("wraps the Windows npm command when npm_execpath is missing", () => {
    expect(
      resolveNpmCommandInvocation({
        ...
        npmArgs: ["view", "bot@beta", "version"],
        npmExecPath: "",
        platform: "win32",
      }),
    ).toEqual({
      command: "C:\\Windows\\System32\\cmd.exe",
      args: ["/d", "/s", "/c", "npm.cmd view bot@beta version"],
  ...
  it("wraps Windows npm_execpath command shims", () => {
    expect(
      resolveNpmCommandInvocation({
        comSpec: "C:\\Windows\\System32\\cmd.exe",
        npmArgs: ["install", "-g", "C:\\tmp\\bot package.tgz"],
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\Program Files\\nodejs\\npm.cmd" install -g "C:\\tmp\\bot package.tgz""',
      ],
      windowsVerbatimArguments: true,
    });
  });
  ...
  if (process.platform === "win32") {
    it("executes fallback npm.cmd through cmd.exe on Windows", () => {
      const dir = mkdtempSync(join(tmpdir(), "bot-fake-npm-cmd-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.BOT_FAKE_NPM_OUT, JSON.stringify(process.argv.slice(2)));",
          ].join("\n"),
        );
        writeFileSync(
          join(dir, "npm.cmd"),
...
describe("resolveNpmReleaseCheckCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(
      resolveNpmReleaseCheckCommandTimeoutMs({ BOT_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "" }),
    ).toBe(10 * 60 * 1000);
    expect(
      resolveNpmReleaseCheckCommandTimeoutMs({
        BOT_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: "1234",
      }),
    ).toBe(1234);
    ...
    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolveNpmReleaseCheckCommandTimeoutMs({
          BOT_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: raw,
        }),
      ).toThrow(`invalid BOT_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
...
describe("parseNpmPackJsonOutput", () => {
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"bot.tgz","files":[]}]')).toEqual([
      { filename: "bot.tgz", files: [] },
    ]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> bot@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"bot.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> bot@2026.3.23 prepack")).toBeNull();
  });
});
...
describe("collectForbiddenPackedPathErrors", () => {
  ...
  it("rejects root dist chunks that still reference the private qa lab", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "bot-pack-private-qa-"));
  ...
  it("rejects private QA paths in the generated dist inventory", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "bot-pack-inventory-"));
