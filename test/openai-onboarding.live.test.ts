...
import { createBotTestState } from "../src/test-utils/bot-test-state.js";
...
const replyMarker = "BOT_OPENAI_ONBOARDING_OK";

async function runBot(args: string[], env: NodeJS.ProcessEnv): Promise<string> {
  try {
  ...
  it("keeps repeated onboarding secret-safe and runs the actual default model", async () => {
    const state = await createBotTestState({
      ...
      env: {
        ...
        BOT_TEST_HOME: undefined,
        BOT_TEST_MINIMAL_GATEWAY: undefined,
        BOT_TEST_TRUST_BUNDLED_PLUGINS_DIR: undefined,
        ...
        BOT_GATEWAY_URL: undefined,
        BOT_GATEWAY_PORT: undefined,
      },
    });
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        await runBot(onboardArgs, state.env);
        ...
        expect(config.agents?.defaults?.workspace).toBe(
          path.join(state.home, ".bot", "workspace"),
        );
