...
import { BUNDLED_PLUGIN_PATH_PREFIX } from "bot/plugin-sdk/test-fixtures";
...
describe("non-extension test boundaries", () => {
  ...
  it("keeps bundled extension sources on the canonical channel config schema facade", () => {
    ...
    const bundledFacadeBindingPattern =
      /\b(?:import|export)\s+(?:type\s+)?\{(?<bindings>[^}]*)\}\s*from\s*["']bot\/plugin-sdk\/bundled-channel-config-schema["']/gu;
