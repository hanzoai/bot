// QA OTEL Smoke runtime supports Bot repository automation.
...
const DEFAULT_DOCKER_COLLECTOR_IMAGE =
  process.env.BOT_QA_OTEL_COLLECTOR_IMAGE || "otel/opentelemetry-collector:0.104.0";
...
const REQUIRED_METRIC_NAMES = ["bot.harness.duration_ms"] as const;
...
function isStdoutDiagnosticLogRecord(value: unknown): value is StdoutDiagnosticLogRecord {
  return (
    ...
    value !== null &&
    !Array.isArray(value) &&
    objectValue(value, "signal") === "bot.diagnostic.log"
  );
}
...
async function startDockerOtelCollector(
  ...
  const tempDir = await makeTempDir(path.join(osTmpdir(), "bot-otel-collector-"));
  const configPath = path.join(tempDir, "collector.yaml");
  const containerName = `bot-otel-smoke-${makeUuid()}`;
...
function isLatestGenAiModelCallSpan(span: CapturedSpan): boolean {
  ...
  return (
    ...
    typeof span.attributes["bot.provider"] === "string" &&
    typeof span.attributes["bot.model"] === "string"
  );
}
...
async function runDirectTelemetryProducer(params: {
  ...
  process.env.OTEL_SERVICE_NAME = "bot-qa-lab-otel-smoke";
...
function assertSmoke(params: {
  ...
  if (spanNames.has("bot.model.call")) {
    failures.push("legacy bot.model.call span exported with GenAI semconv opt-in");
  }
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("bot.content."));
  ...
  const modelErrorSpans = modelSpans.filter((span) => {
    ...
    return (
      ...
      Object.hasOwn(span.attributes, "bot.errorCategory") ||
  ...
  const failedRunSpans = params.spans.filter(
    (span) =>
      (span.name === "bot.run" || span.name === "bot.harness.run") &&
      span.attributes["bot.error"] === `${DIRECT_ERROR_MESSAGE} OPENAI_API_KEY=***`,
  );
  if (failedRunSpans.length !== 2) {
    const observed = params.spans
      .filter((span) => span.name === "bot.run" || span.name === "bot.harness.run")
      .map((span) => ({ name: span.name, error: span.attributes["bot.error"] }));
    failures.push(
