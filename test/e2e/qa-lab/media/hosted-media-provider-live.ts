...
const DEFAULT_PROVIDERS_ENV = "BOT_QA_HOSTED_MEDIA_PROVIDERS";
...
export const MEDIA_SUITES: Record<MediaSuiteId, MediaSuiteConfig> = {
  image: {
    ...
    providerEnvVar: "BOT_LIVE_IMAGE_GENERATION_PROVIDERS",
  ...
  music: {
    ...
    providerEnvVar: "BOT_LIVE_MUSIC_GENERATION_PROVIDERS",
    providers: ["fal", "google", "minimax", "openrouter"],
  },
  video: {
    id: "video",
    testFile: "extensions/video-generation-providers.live.test.ts",
    providerEnvVar: "BOT_LIVE_VIDEO_GENERATION_PROVIDERS",
    providers: [
      "alibaba",
      "byteplus",
      "deepinfra",
...
function suiteProviderFilter(options: HostedMediaOptions, env: NodeJS.ProcessEnv) {
  const suiteEnv = `BOT_QA_HOSTED_${options.suiteId.toUpperCase()}_PROVIDERS`;
...
export function buildHostedMediaCommand(params: {
  ...
  if (definition.videoFullModes) {
    env.BOT_LIVE_VIDEO_GENERATION_FULL_MODES = "1";
  }
  return {
    args,
