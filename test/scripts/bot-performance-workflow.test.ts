// Bot Performance Workflow tests cover bot performance workflow script behavior.
...
const WORKFLOW = ".github/workflows/bot-performance.yml";
...
describe("Bot performance workflow", () => {
  it("uses an optional dispatch identifier to name parent-owned runs", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");

    expect(workflow).toContain(
      "run-name: ${{ inputs.dispatch_id != '' && format('Bot Performance {0}', inputs.dispatch_id) || 'Bot Performance' }}",
    );
  ...
  it("pins the Kova evaluator with release validation contracts", () => {
    ...
    const resolveTarget = findStep("Resolve Bot target ref", "resolve_target");
  ...
  it("resolves each target once before benchmark and publication fan out", () => {
    ...
    const resolveTarget = findStep("Resolve Bot target ref", "resolve_target");
    const checkout = findStep("Checkout Bot");
    const record = findStep("Record tested revision");
    const sourceCheckout = findStep("Checkout Bot source target", "source_performance");
    ...
    expect(
      Object.values(workflow.jobs ?? {})
        ...
        .filter((step) => step.name === "Resolve Bot target ref"),
    ).toHaveLength(1);
  });
  ...
  it("sparse-fetches only the public source baseline without publisher credentials", () => {
    ...
    expect(run).toContain('remote add origin "https://github.com/bot/clawgrit-reports.git"');
  ...
  it("builds only the QA and startup artifacts required by source probes", () => {
    const run = findStep("Run Bot source performance probes", "source_performance").run ?? "";
    const build = "BOT_BUILD_PRIVATE_QA=1 node scripts/build-all.mjs sourcePerformance";
  ...
  it("isolates required publication in a fresh artifact-consuming job", () => {
    ...
    expect(kovaSteps.some((step) => step.name === "Run Bot source performance probes")).toBe(
      false,
    );
    expect(
      workflow.jobs?.source_performance?.steps?.some(
        (step) => step.name === "Run Bot source performance probes",
      ),
    ).toBe(true);
  ...
  it("keeps app credentials out of artifact processing and scopes them to Git push", () => {
    ...
    expect(prepare.run).toContain(
      'source_path="${input_root}/bot-performance-source-${GITHUB_RUN_ID}-${SOURCE_PRODUCER_ATTEMPT}/${LANE_ID}"',
    );
    ...
    expect(prepare.run).toContain(
      'remote add origin "https://github.com/bot/clawgrit-reports.git"',
    );
  ...
  it("publishes bounded bundle metadata while retaining full diagnostics as an artifact", () => {
    ...
    expect(upload.with?.path).not.toContain(".artifacts/bot-performance/source");
    expect(sourceUpload.with).toMatchObject({
      name: "bot-performance-source-${{ github.run_id }}-${{ github.run_attempt }}",
      path: ".artifacts/bot-performance/source",
  ...
  it("reuses the producing artifact when only publisher jobs rerun", () => {
    const artifact = findStep("Resolve Kova artifact", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "bot-artifact-resolver-"));
    ...
    writeFileSync(
      ...
      `#!/bin/sh
printf '%s\\n' \
  '101	bot-performance-mock-provider-9001-1' \
  '202	bot-performance-source-9001-2' \
  '303	bot-performance-mock-provider-9001-3'
`,
    );
  ...
  it("advertises a clawgrit URL only after a direct or remotely verified push", () => {
    const publish = findStep("Publish to clawgrit reports", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "bot-publish-shell-"));
    const bin = join(root, "bin");
    const reportsRoot = join(root, "reports");
    const reportUrl =
      "https://github.com/bot/clawgrit-reports/tree/main/bot-performance/main/123-1/mock-provider";
    mkdirSync(bin);
...
case "$*" in
  ...
  *"remote get-url origin"*) echo https://github.com/bot/clawgrit-reports.git ;;
...
`,
    ...
    const execute = ({
      ...
      const result = spawnSync("bash", ["-c", publish.run ?? ""], {
        ...
        env: {
          ...
          DEST_REL: "bot-performance/main/123-1/mock-provider",
  ...
  it("preserves both reports when concurrent writers update one latest pointer", () => {
    const root = mkdtempSync(join(realpathSync(tmpdir()), "bot-report-race-"));
    ...
    const latest = "bot-performance/main/latest-mock-provider.json";
    const reportA = "bot-performance/main/100-1/mock-provider";
    const reportB = "bot-performance/main/200-1/mock-provider";
    ...
    const commitReport = (repo: string, reportPath: string, marker: string) => {
      ...
      runGit(repo, ["add", "--", "bot-performance"]);
  ...
  it("installs local workspace packages beside the OCM root tarball", () => {
    ...
    expect(workflow.jobs?.kova?.env).not.toHaveProperty("BOT_OCM_RUNTIME_BUILD_PROFILE");
    ...
    expect(configure.run).toContain(
      "BOT_OCM_WORKSPACE_DEPENDENCY_DIRS=$workspace_dependency_dirs",
    );
  });
