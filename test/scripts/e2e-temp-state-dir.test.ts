...
describe("E2E temp state dirs", () => {
  it("cleans generated state dirs", async () => {
    const state = await createE2eStateDir("bot-e2e-temp-state-test-", {
      BOT_STATE_DIR: "",
    });
  ...
  it("leaves caller-provided state dirs alone", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "bot-e2e-temp-state-existing-"));
    try {
      const state = await createE2eStateDir("bot-e2e-temp-state-test-", {
        BOT_STATE_DIR: root,
      });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(tmpdir(), "bot-e2e-temp-state-retry-"));
  ...
  it("cleans generated state dirs on termination signals", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "bot-e2e-temp-state-signal-"));
    try {
...
const state = await createE2eStateDir("bot-e2e-temp-state-signal-", {
  BOT_STATE_DIR: "",
});
