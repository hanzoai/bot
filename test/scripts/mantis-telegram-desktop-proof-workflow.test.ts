...
describe("Mantis Telegram Desktop proof workflow", () => {
  ...
  it("releases Telegram Desktop proof leases left by interrupted agents", () => {
    ...
    expect(codexStep.env?.BOT_QA_CREDENTIAL_OWNER_ID).toContain(
      "mantis-telegram-desktop-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    expect(workflowStep("Prepare Codex user").run).toContain("BOT_QA_CREDENTIAL_OWNER_ID");
    ...
    expect(cleanupStep.env?.BOT_QA_CONVEX_SECRET_CI).toContain(
      "secrets.BOT_QA_CONVEX_SECRET_CI",
    );
    expect(cleanupStep.env?.BOT_QA_CONVEX_SITE_URL).toContain(
  ...
  it("installs local proof tools before the Codex agent runs", () => {
    ...
    expect(agent.env?.BOT_TELEGRAM_USER_DRIVER_SCRIPT).toBe(
    ...
    expect(agent.env?.CRABBOX_COORDINATOR).toContain(
      "secrets.CRABBOX_COORDINATOR || secrets.BOT_QA_MANTIS_CRABBOX_COORDINATOR",
    );
    expect(agent.env?.CRABBOX_COORDINATOR_TOKEN).toContain(
      "secrets.CRABBOX_COORDINATOR_TOKEN || secrets.BOT_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN",
    );

    const prepare = workflowStep("Prepare Codex user");
    expect(prepare.run).toContain(
      "BOT_TELEGRAM_USER_CRABBOX_BIN BOT_TELEGRAM_USER_CRABBOX_PROVIDER BOT_TELEGRAM_USER_DRIVER_SCRIPT BOT_TELEGRAM_USER_PROOF_CMD",
    );
    ...
    expect(prompt).toContain("$BOT_TELEGRAM_USER_PROOF_CMD");
  ...
  it("prepares exact proof worktrees before agent secrets are available", () => {
    ...
    expect(run).toContain(
      'sudo /usr/local/sbin/bot-mantis-sut-container build "$candidate_root"',
    );
    ...
    expect(run).not.toContain("BOT_QA_");
  });
  ...
  it("does not pass the full workflow environment into the local Telegram SUT", () => {
    ...
    expect(workflow).toContain(
      "sudo install -m 0755 scripts/mantis/mantis-sut-container.sh /usr/local/sbin/bot-mantis-sut-container",
    );
    expect(workflow).toContain(
      "codex ALL=(root) NOPASSWD: /usr/local/sbin/bot-mantis-sut-container",
    );
    expect(workflow).not.toContain("NOPASSWD:SETENV:");
    expect(workflow).toContain("/etc/bot-mantis-sut-revisions");
    ...
    expect(wrapper).toContain('readonly network_state_root="/run/bot-mantis-sut-networks"');
