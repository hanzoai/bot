...
describe("real-behavior-proof-policy", () => {
  it.each([
    ...
    "Linked artifact: https://github.com/hanzoai/bot/actions/runs/123456789/artifacts/987654321",
    "Redacted runtime log: gateway connected Discord channel and delivered the reply.",
    ["Terminal transcript:", "```text", "$ bot gateway status", "discord ready", "```"].join(
      "\n",
    ),
    ...
    const evaluation = evaluatePullRequestContext({
      pullRequest: externalPr(
        [
          ...
          "",
          "- Behavior addressed: The gateway dropped the configured Discord channel during startup.",
          "- Real environment tested: macOS 15.4, Node 24, local Bot gateway.",
          "- Exact steps or command run after this patch: pnpm bot gateway restart",
  ...
  it("accepts out-of-scope follow-ups as not-tested proof detail", () => {
    const body = [
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: Bot 2026.5.21`",
      "",
  ...
  it("accepts source PR proof when explicit gaps live in out-of-scope follow-ups", () => {
    const body = [
      ...
      "## Evidence",
      "",
      "- Real environment tested: Local macOS source checkout, Node v24.8.0, Bot 2026.5.21 (c8a35c4), local `bot` shim pointed at the freshly built checkout. No channel credentials or provider API keys were used.",
      "- Exact steps or command run after this patch:",
      "  1. Built the local checkout with `node scripts/build-all.mjs`.",
      "  2. Updated `/Users/example/.local/bin/bot` to run this checkout's `bot.mjs` and verified `/Users/example/.local/bin/bot --version`.",
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: Bot 2026.5.21 (c8a35c4)`",
      "",
      "## Out-of-scope Follow-ups",
...
describe("isMaintainerTeamMember", () => {
  ...
  it("returns true for active members", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://api.github.com/orgs/bot/teams/maintainer/memberships/private-maint",
