...
function makeSkipInventoryFixture(): string {
  const repoRoot = createTempDir("bot-test-skip-inventory-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
...
`,
  ...
  writeRepoFile(
    ...
    `
const LIVE = process.env.BOT_LIVE_TEST === "1";
...
describe("collectTestSkipInventoryReport", () => {
  ...
  it("renders a compact non-blocking text report", () => {
    ...
    expect(rendered).toContain("Bot test skip inventory");
