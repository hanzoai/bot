...
function writeFixture(params?: {
  ...
  const root = makeTempDir(tempDirs, "bot-release-version-");
  fs.mkdirSync(path.join(root, "apps", "macos", "Sources", "Bot", "Resources"), {
    recursive: true,
  });
  ...
  fs.writeFileSync(
    path.join(root, "apps", "macos", "Sources", "Bot", "Resources", "Info.plist"),
    [
      "<plist>",
      "<dict>",
...
describe("release version planning", () => {
  ...
  it("keeps an existing Android build increment on the same release train", () => {
    ...
    ).toContain("BOT_ANDROID_VERSION_CODE=2026070102");
    expect(
      fs.readFileSync(
        path.join(
          root,
