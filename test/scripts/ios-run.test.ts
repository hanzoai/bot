...
function makeFixture(bundleId: string): { root: string; script: string; logFile: string } {
  const root = mkdtempSync(path.join(os.tmpdir(), "bot-ios-run-"));
  tempDirs.push(root);
  ...
  writeExecutable(
    ...
    `#!/usr/bin/env bash
set -euo pipefail
if [[ -n "\${BOT_SIMULATOR_PUSH_PROOF_SECRET:-}" || -n "\${CUSTOM_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
if (process.env.BOT_SIMULATOR_PUSH_PROOF_SECRET || process.env.CUSTOM_SIMULATOR_PUSH_PROOF_SECRET) {
...
set -euo pipefail
printf 'xcodegen %s\\n' "$*" >>"${logFile}"
if [[ -n "\${BOT_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
app_dir="$derived/Build/Products/$configuration-iphonesimulator/Bot.app"
mkdir -p "$app_dir"
...
printf 'simctl %s\\n' "$*" >>"${logFile}"
if [[ "$1" == "launch" ]]; then
  if [[ -n "\${SIMCTL_CHILD_BOT_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
describe("scripts/ios-run.sh", () => {
  ...
  it("builds simulator sandbox relay mode and injects proof secret only at launch", () => {
    ...
    runIosRun(
      fixture,
      {
        BOT_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
    ...
    expect(log).toContain("BOT_PUSH_MODE=simulatorSandbox");
    expect(log).toContain(
      "BOT_PUSH_RELAY_BASE_URL=https://ios-push-relay-sandbox.bot.ai",
    );
    expect(log).toContain("simctl launch iPhone 17 ai.botfoundation.app");
  ...
  it("scrubs exported simulator proof secrets from normal build helpers", () => {
    ...
    runIosRun(fixture, {
      BOT_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
      BOT_SIMULATOR_PUSH_PROOF_SECRET_ENV: "CUSTOM_SIMULATOR_PUSH_PROOF_SECRET",
