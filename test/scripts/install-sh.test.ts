...
function runInstallShell(script: string, env: NodeJS.ProcessEnv = {}) {
  const home = mkdtempSync(join(tmpdir(), "bot-install-home-"));
  try {
...
describe("install.sh", () => {
  ...
  it("runs installer snippets without inherited shell startup files", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-shell-env-"));
    const bashEnvPath = join(tmp, "bash_env");
    writeFileSync(bashEnvPath, "export BOT_BASH_ENV_LEAKED=1\n");

    try {
      const result = runInstallShell('printf "leaked=%s\\n" "${BOT_BASH_ENV_LEAKED:-0}"', {
  ...
  it("removes a downloaded script temp file when remote execution fails", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-remote-cleanup-"));
  ...
  it("rejects malformed managed scripts without rendering their content", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-script-validation-"));
  ...
  it("does not execute a shebang-prefixed partial file after download failure", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-partial-download-"));
  ...
  it("denies redirects for managed script downloads", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-managed-download-"));
  ...
  it.each(["apt-get", "dnf", "yum"])(
    ...
    (packageManager) => {
      const tmp = mkdtempSync(join(tmpdir(), "bot-install-nodesource-validation-"));
...
NODE
      ...
      resolve_git_bot_ref() { printf 'main\\n'; }
      ...
      install_bot_from_git "$repo"
      wrapper="$HOME/.local/bin/bot"
  ...
  it("installs Git with apk on Alpine", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-git-apk-"));
  ...
  it("does not select apk Git on non-Alpine hosts", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-git-native-"));
    const bin = join(tmp, "bin");
    const apkLog = join(tmp, "apk-args.txt");
    mkdirSync(bin, { recursive: true });
    const fakeApk = join(bin, "apk");
  ...
  it("does not emit --before when raw user npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-npmrc-"));
    const bin = join(tmp, "bin");
    ...
    try {
      const result = runInstallShell(
        [
          ...
          `source ${JSON.stringify(SCRIPT_PATH)}`,
          `run_npm_global_install bot@latest ${JSON.stringify(join(tmp, "install.log"))}`,
  ...
  it("does not emit --before when default global npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-global-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("does not emit --before when builtin npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-builtin-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("uses BOT_HOME for git defaults", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-home-"));
    const osHome = join(tmp, "os-home");
    const botHome = join(tmp, "bot-home");
    ...
    expect(output).toContain(`git=${join(botHome, "bot")}`);
  ...
  it("does not treat OS HOME config as active when BOT_HOME is set", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-legacy-config-"));
    const osHome = join(tmp, "os-home");
    const botHome = join(tmp, "bot-home");
    const legacyConfigDir = join(osHome, ".bot");
    mkdirSync(legacyConfigDir, { recursive: true });
    mkdirSync(botHome, { recursive: true });
    writeFileSync(join(legacyConfigDir, "bot.json"), "{}\n");
    ...
    try {
      result = runInstallShell(
        [
          `cd ${JSON.stringify(process.cwd())}`,
          `source ${JSON.stringify(SCRIPT_PATH)}`,
          'if has_bot_config; then printf "configured=1\\n"; else printf "configured=0\\n"; fi',
        ].join("\n"),
        {
          HOME: osHome,
          BOT_HOME: botHome,
  ...
  it("does not fall back to home config when BOT_STATE_DIR is set", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-state-override-"));
    const home = join(tmp, "home");
    const stateDir = join(tmp, "state");
    mkdirSync(join(home, ".bot"), { recursive: true });
    mkdirSync(stateDir, { recursive: true });
    writeFileSync(join(home, ".bot", "bot.json"), "{}\n");
  ...
  it.each([
    {
      expected: /No TTY; run .*\/\.local\/bin\/bot onboard to finish setup/,
      name: "starts setup",
      noOnboard: 0,
    },
    {
      expected: /Skipping onboard .*run .*\/\.local\/bin\/bot onboard later/,
    ...
    ({ expected, noOnboard }) => {
      ...
      detect_bot_checkout() { return 1; }
      show_install_plan() { :; }
      check_existing_bot() { return 0; }
      ...
      resolve_bot_version() { printf 'test-version\\n'; }
    ...
    `);
      ...
      expect(result.stdout).toMatch(/Update command:.*\/\.local\/bin\/bot update/);
  ...
  it("honors --verify for an unconfigured install without a TTY", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stdout).toMatch(/No TTY; run .*\/\.local\/bin\/bot onboard to finish setup/);
  });
  ...
  it("rejects Bot GitHub source targets for npm installs", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      set +e
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("status=1");
    expect(result.stdout).toContain("npm installs do not support Bot GitHub source targets");
  ...
  it("does not emit before args when npmrc min-release-age computes a before cutoff", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-npm-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("ignores project npmrc when choosing global install freshness args", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-global-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("bounds installer npm prefix probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-npm-probe-"));
    ...
    try {
      const result = runInstallShell(
        ...
        {
          BOT_INSTALL_PROBE_TIMEOUT_SECONDS: "0.1",
          PATH: `${tmp}:${process.env.PATH ?? ""}`,
        },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("/tmp/bot-npm/bin");
  ...
  it("bounds daemon status probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-probe-"));
    const claw = join(tmp, "bot");
    writeFileSync(
      claw,
      [
        "#!/usr/bin/env bash",
    ...
    try {
      const result = runInstallShell(
        ...
        { BOT_INSTALL_PROBE_TIMEOUT_SECONDS: "0.01" },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("not-loaded");
      expect(result.stderr).toContain(
        "timed out during installer finalization probe: bot daemon status --json",
      );
    } finally {
      rmSync(tmp, { force: true, recursive: true });
    }
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-nvm-"));
    const home = join(tmp, "home");
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-node-promote-"));
  ...
  it("uses the package engine range when accepting existing Node runtimes", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-node-floor-"));
    const bin = join(tmp, "bin");
    mkdirSync(bin, { recursive: true });
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g bot@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute bot path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-command-"));
    ...
    const botBin = join(npmBin, "bot");
    const staleBotBin = join(staleBin, "bot");
    writeFileSync(botBin, "#!/bin/sh\nexit 0\n");
    writeFileSync(staleBotBin, "#!/bin/sh\nexit 0\n");
    chmodSync(botBin, 0o755);
    chmodSync(staleBotBin, 0o755);
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${botBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=bot");
    expect(result?.stdout).toContain(`shadowed=${botBin.replace(/ /g, "\\ ")}`);
  });

  it("prefers the binary owned by the completed install method over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-selected-bin-"));
    const home = join(tmp, "home");
    const npmBin = join(tmp, "npm-bin");
    const staleBin = join(tmp, "stale-bin");
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`git=${join(gitBin, "bot")}`);
    expect(result?.stdout).toContain(`npm=${join(npmBin, "bot")}`);
  });

  it("uses the selected binary in gateway recovery guidance", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-gateway-guidance-"));
    const currentBin = join(tmp, "current bin");
    const staleBin = join(tmp, "stale-bin");
    mkdirSync(currentBin, { recursive: true });
    mkdirSync(staleBin, { recursive: true });
    const botBin = join(currentBin, "bot");
    writeFileSync(botBin, "#!/bin/sh\nexit 0\n");
    writeFileSync(join(staleBin, "bot"), "#!/bin/sh\nexit 0\n");
    chmodSync(botBin, 0o755);
    chmodSync(join(staleBin, "bot"), 0o755);
    ...
    const quotedBin = botBin.replace(/ /g, "\\ ");
    expect(result?.status).toBe(0);
  ...
  it("resolves requested git install versions to checkout refs", () => {
    const result = runInstallShell(`
      ...
      source "${SCRIPT_PATH}"
      npm() {
        if [[ "$1" == "view" && "$2" == "bot" && "$3" == "dist-tags.beta" ]]; then
          printf '2026.5.12-beta.3\\n'
          return 0
        fi
        return 1
      }
      BOT_VERSION=v2026.5.12-beta.3
      printf 'tag=%s\\n' "$(resolve_git_bot_ref)"
      BOT_VERSION=2026.5.12-beta.3
      printf 'semver=%s\\n' "$(resolve_git_bot_ref)"
      BOT_VERSION=beta
      printf 'beta=%s\\n' "$(resolve_git_bot_ref)"
      BOT_VERSION=main
  ...
  it("uses the repo Corepack pnpm when a global pnpm version is already present", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-pnpm-version-"));
    const bin = join(tmp, "bin");
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "bot-install-sh-gum-"));
    try {
  ...
  it("gum spin preserves terminal stdin for direct interactive installs", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "bot-install-sh-gum-stdin-"));
    try {
      const gumPath = join(dir, "gum");
      const commandPath = join(dir, "command");
...
describe("install.sh duplicate Bot install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/bot" "$root/fnm/bot"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/bot/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/bot/package.json"
      collect_bot_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      BOT_BIN="$root/fnm/.bin/bot"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_bot_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple Bot global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/bot");
    expect(result.stdout).toContain("/fnm/bot");
    expect(result.stdout).toContain("Active bot:");
    expect(result.stdout).toContain("npm uninstall -g bot");
  });

  it("stays quiet when only one Bot npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/bot"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/bot/package.json"
      collect_bot_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_bot_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple Bot global installs detected");
  });
  ...
  it("run_quiet_step redirects stdin to /dev/null in piped context", () => {
    const dir = mkdtempSync(join(tmpdir(), "bot-stdin-test-"));
  ...
  it("pipe data leaks to child when stdin is not isolated (counterproof)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "bot-stdin-leak-"));
    const marker = join(dir, "stdin-state");
    try {
      const result = spawnSync(
        "bash",
  ...
  it("run_quiet_step blocks cat from reading pipe data", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "bot-stdin-cat-"));
    const marker = join(dir, "stdin-state");
    try {
      const result = spawnSync(
        "bash",
