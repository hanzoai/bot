...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/bot-pr-maintainer/scripts/github-activity.sh",
);
...
describe("bot-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("hanzoai/bot last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/bot\/bot\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/bot\/bot\/commits/g)).toHaveLength(1);
