...
describe("release preparation manifest", () => {
  it("fingerprints generated diffs larger than Node's default child buffer", () => {
    const rootDir = mkdtempSync(path.join(tmpdir(), "bot-release-prepare-"));
    try {
      execFileSync("git", ["init", "-q"], { cwd: rootDir });
      execFileSync("git", ["config", "user.email", "release-test@bot.invalid"], {
        cwd: rootDir,
      });
      execFileSync("git", ["config", "user.name", "Bot Release Test"], { cwd: rootDir });
