// Crabbox Wrapper tests cover botbox wrapper script behavior.
...
} from "../../scripts/botbox-wrapper-providers.mjs";
...
function makeFakeCrabbox(helpText: string): string {
  ...
  const binDir = mkdtempSync(path.join(tmpdir(), "bot-fake-botbox-"));
...
function writeFakeCrabbox(binDir: string, helpText: string): string {
  ...
  const botboxPath = path.join(binDir, "botbox");
  const helperPath = path.join(binDir, "fake-botbox-json.cjs");
  const stampClaimScript = [
    "const fs = require('node:fs');",
    "const claimPaths = [process.env.BOT_FAKE_CRABBOX_CLAIM_PATH, process.env.BOT_FAKE_CRABBOX_EXTRA_CLAIM_PATH].filter(Boolean);",
    "for (const claimPath of claimPaths) {",
    "  const claim = fs.existsSync(claimPath) ? JSON.parse(fs.readFileSync(claimPath, 'utf8')) : { leaseID: process.env.BOT_FAKE_CRABBOX_TIMING_LEASE_ID };",
    "  claim.repoRoot = process.env.BOT_FAKE_CRABBOX_CLAIM_REPO_ROOT || process.cwd();",
    "  fs.mkdirSync(require('node:path').dirname(claimPath), { recursive: true });",
    "  fs.writeFileSync(claimPath, JSON.stringify(claim) + '\\n', 'utf8');",
    "}",
    "if (process.env.BOT_FAKE_CRABBOX_TIMING_LEASE_ID) {",
    "  process.stderr.write(JSON.stringify({ provider: 'blacksmith-testbox', leaseId: process.env.BOT_FAKE_CRABBOX_TIMING_LEASE_ID, exitCode: 0 }) + '\\n');",
    "}",
  ].join("\n");
  ...
  if (process.platform !== "win32") {
    const signalIgnoringDescendantScript = [
      ...
      "process.on('SIGINT', () => {});",
      "process.on('SIGTERM', () => {});",
      "const pidPath = process.env.BOT_FAKE_CRABBOX_DESCENDANT_PID_PATH;",
    ...
    const script = [
      "#!/bin/sh",
      'if [ "$1" = "--version" ]; then',
      '  printf "%s\\n" "${BOT_FAKE_CRABBOX_VERSION:-botbox 0.22.1}"',
      "  exit 0",
      "fi",
      ...
      "  exit 0",
      "fi",
      'if { [ "$1" = "run" ] || [ "$1" = "warmup" ]; } && { [ -n "${BOT_FAKE_CRABBOX_CLAIM_PATH:-}" ] || [ -n "${BOT_FAKE_CRABBOX_TIMING_LEASE_ID:-}" ]; }; then',
      `  ${shellSingleQuote(process.execPath)} --eval ${shellSingleQuote(stampClaimScript)}`,
      "fi",
      'if [ "$1" = "run" ] && [ -n "${BOT_FAKE_CRABBOX_RUN_STATUS:-}" ] && [ "$BOT_FAKE_CRABBOX_RUN_STATUS" != "0" ]; then',
      '  printf "%s\\n" "fake run failure" >&2',
      '  exit "$BOT_FAKE_CRABBOX_RUN_STATUS"',
      "fi",
      'if [ "$1" = "config" ] && [ "$2" = "show" ]; then',
      '  for arg in "$@"; do',
      '    if [ "$arg" = "--json" ]; then',
      '      status="${BOT_FAKE_CRABBOX_CONFIG_STATUS:-0}"',
      '      if [ "$status" != "0" ]; then',
      '        printf "%s\\n" "config unavailable" >&2',
      '        exit "$status"',
      "      fi",
      '      if [ -n "${BOT_FAKE_CRABBOX_CONFIG_JSON+x}" ]; then',
      '        printf "%s" "$BOT_FAKE_CRABBOX_CONFIG_JSON"',
      "      else",
      ...
      "fi",
      'if [ "$1" = "whoami" ]; then',
      '  status="${BOT_FAKE_CRABBOX_WHOAMI_STATUS:-0}"',
      '  if [ "$status" != "0" ]; then',
      ...
      "    mkdir -p .botbox/runs/run_fake",
      '    printf "%s\\n" "fake artifact" > .botbox/runs/run_fake/fake-artifacts.tgz',
      "  fi",
      "done",
      'script_path=""',
      'previous_arg=""',
      ...
      '  previous_arg="$arg"',
      "done",
      'if [ "${BOT_FAKE_CRABBOX_DELETE_CWD_AND_EXIT:-}" = "1" ]; then',
      ...
      "  exit 0",
      "fi",
      'if [ "${BOT_FAKE_CRABBOX_DELETE_CWD_ONCE:-}" = "1" ]; then',
      '  deleted_cwd="$PWD"',
      "  cd / || exit 1",
      '  rm -rf "$deleted_cwd"',
      ...
      "fi",
      'if [ -n "${BOT_FAKE_CRABBOX_DESCENDANT_PID_PATH:-}" ]; then',
      ...
      "fi",
      'if [ -n "${BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE+x}" ]; then',
      '  expected_bundle="$(mktemp)" || exit 67',
      '  printf "%s" "$BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE" > "$expected_bundle"',
      '  if [ ! -f .bot-botbox-changed-gate.bundle ] || ! cmp -s .bot-botbox-changed-gate.bundle "$expected_bundle"; then',
      ...
      '  rm -f "$expected_bundle"',
      "fi",
      'if [ -n "${BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE_BYTES:-}" ]; then',
      '  actual_bundle_bytes="$(wc -c < .bot-botbox-changed-gate.bundle 2>/dev/null | tr -d " ")"',
      '  if [ "$actual_bundle_bytes" != "$BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE_BYTES" ]; then',
      '    printf "%s\\n" "changed-gate bundle size mismatch" >&2',
      "    exit 67",
      "  fi",
      "fi",
      'if [ "${BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_FORCE_ADD:-}" = "1" ] && [ ! -f "${BOT_FAKE_GIT_FORCE_ADD_MARKER:-}" ]; then',
      '  printf "%s\\n" "changed-gate bundle was not force-added" >&2',
      "  exit 67",
      "fi",
      'printf "%s\\0" "__BOT_FAKE_CRABBOX_V1__"',
      'printf "%s\\0" "$PWD"',
      'printf "%s\\0" "$#"',
      'for arg in "$@"; do',
      '  printf "%s\\0" "$arg"',
    ...
    writeFileSync(botboxPath, `${script}\n`, "utf8");
    chmodSync(botboxPath, 0o755);
    return botboxPath;
  }
  ...
  const helperScript = [
    ...
    "  const status = Number.parseInt(process.env.BOT_FAKE_CRABBOX_CONFIG_STATUS || '0', 10);",
    "  if (status !== 0) {",
    "    process.stderr.write('config unavailable\\n');",
    "    process.exit(status);",
    "  }",
    '  process.stdout.write(process.env.BOT_FAKE_CRABBOX_CONFIG_JSON || \'{"coordinator":"configured-broker","brokerAuth":"configured"}\');',
    "  process.exit(0);",
    "}",
    'if (args[0] === "whoami") {',
    "  const status = Number.parseInt(process.env.BOT_FAKE_CRABBOX_WHOAMI_STATUS || '0', 10);",
    "  if (status !== 0) {",
    '    process.stderr.write(\'coordinator GET /v1/whoami: http 401: {"error":"unauthorized"}\\n\');',
    "    process.exit(status);",
    "  }",
    "  process.stdout.write('fake-botbox-user\\n');",
    "  process.exit(0);",
    "}",
    ...
    "  require('node:fs').mkdirSync('.botbox/runs/run_fake', { recursive: true });",
    "  require('node:fs').writeFileSync('.botbox/runs/run_fake/fake-artifacts.tgz', 'fake artifact\\n');",
    "}",
    "if (Object.hasOwn(process.env, 'BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE')) {",
    "  const bundlePath = '.bot-botbox-changed-gate.bundle';",
    "  const bundle = require('node:fs').existsSync(bundlePath) ? require('node:fs').readFileSync(bundlePath, 'utf8') : null;",
    "  if (bundle !== process.env.BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE) { process.stderr.write('changed-gate bundle mismatch\\n'); process.exit(67); }",
    "}",
    "if (process.env.BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE_BYTES) {",
    "  const bundlePath = '.bot-botbox-changed-gate.bundle';",
    "  const size = require('node:fs').existsSync(bundlePath) ? require('node:fs').statSync(bundlePath).size : -1;",
    "  if (size !== Number(process.env.BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE_BYTES)) { process.stderr.write('changed-gate bundle size mismatch\\n'); process.exit(67); }",
    "}",
    "if (process.env.BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_FORCE_ADD === '1' && !require('node:fs').existsSync(process.env.BOT_FAKE_GIT_FORCE_ADD_MARKER || '')) { process.stderr.write('changed-gate bundle was not force-added\\n'); process.exit(67); }",
  ...
  const script = [
    ...
    "const args = process.argv.slice(2);",
    'if (args[0] === "--version") {',
    "  console.log(process.env.BOT_FAKE_CRABBOX_VERSION || 'botbox 0.22.1');",
    "  process.exit(0);",
    "}",
    ...
    'if (args[0] === "run" && Number.parseInt(process.env.BOT_FAKE_CRABBOX_RUN_STATUS || "0", 10) !== 0) {',
    "  process.stderr.write('fake run failure\\n');",
    "  process.exit(Number.parseInt(process.env.BOT_FAKE_CRABBOX_RUN_STATUS, 10));",
    "}",
    `require(${JSON.stringify(helperPath)});`,
  ].join("\n");
  writeFileSync(botboxPath, `${script}\n`, "utf8");
  writeFileSync(`${botboxPath}.cmd`, windowsNodeCmdShim("botbox"), "utf8");
  chmodSync(botboxPath, 0o755);
  return botboxPath;
}

function makeSlowVersionCrabbox(helpText: string): string {
  const binDir = mkdtempSync(path.join(tmpdir(), "bot-slow-botbox-"));
  tempDirs.push(binDir);
  const botboxPath = path.join(binDir, "botbox");
...
function makeSlowHelpCrabbox(helpText: string, delayMs: number): string {
  const binDir = mkdtempSync(path.join(tmpdir(), "bot-slow-help-botbox-"));
  tempDirs.push(binDir);
  const botboxPath = path.join(binDir, "botbox");
...
function testTimingPreload(options: { clockScale?: number; spawnTimeoutMs?: number }): string {
  ...
  if (!preloadPath) {
    const dir = mkdtempSync(path.join(tmpdir(), "bot-botbox-timing-"));
    tempDirs.push(dir);
...
function makeFakeGit(
  ...
  const binDir = mkdtempSync(path.join(tmpdir(), "bot-fake-git-"));
  tempDirs.push(binDir);
  ...
  if (process.platform !== "win32") {
    const script = [
      ...
      '  mkdir -p "$4"',
      '  if [ -n "${BOT_FAKE_GIT_CHANGED_GATE_BUNDLE_SYMLINK_TARGET:-}" ]; then',
      '    ln -s "$BOT_FAKE_GIT_CHANGED_GATE_BUNDLE_SYMLINK_TARGET" "$4/.bot-botbox-changed-gate.bundle"',
      "  fi",
      "  exit 0",
      "fi",
      ...
      '  if [ "$4" = "HEAD" ]; then',
      '    printf "%s\\n" "${BOT_FAKE_GIT_HEAD_SHA:-def456}"',
      '  elif [ "$4" = "HEAD^{tree}" ]; then',
      '    printf "%s\\n" "${BOT_FAKE_GIT_HEAD_TREE_SHA:-tree456}"',
      "  else",
      '    printf "%s\\n" "${BOT_FAKE_GIT_BASE_SHA:-abc123}"',
      "  fi",
      "  exit 0",
      "fi",
      'if [ "$1" = "-C" ] && [ "$3" = "-c" ] && [ "$7" = "commit-tree" ]; then',
      '  if [ -n "${BOT_FAKE_GIT_ROOT_COMMIT_MARKER:-}" ]; then',
      '    for arg in "$@"; do [ "$arg" != "-p" ] || exit 68; done',
      '    : > "$BOT_FAKE_GIT_ROOT_COMMIT_MARKER"',
      "  fi",
      '  if [ -n "${BOT_FAKE_GIT_SYNTHETIC_COMMIT_MARKER:-}" ]; then',
      '    : > "$BOT_FAKE_GIT_SYNTHETIC_COMMIT_MARKER"',
      "  fi",
      '  printf "%s\\n" "${BOT_FAKE_GIT_SYNTHETIC_COMMIT_SHA:-synthetic789}"',
      "  exit 0",
      "fi",
      'if [ "$1" = "-C" ] && [ "$3" = "update-ref" ] && [ "$4" = "HEAD" ]; then',
      '  if [ -n "${BOT_FAKE_GIT_SYNTHETIC_HEAD_MARKER:-}" ]; then',
      '    : > "$BOT_FAKE_GIT_SYNTHETIC_HEAD_MARKER"',
      "  fi",
      "  exit 0",
      "fi",
      'if [ "$1" = "-C" ] && [ "$3" = "bundle" ] && [ "$4" = "create" ]; then',
      '  if [ -n "${BOT_FAKE_GIT_SELF_CONTAINED_BUNDLE_MARKER:-}" ]; then',
      '    [ "$#" = "6" ] && [ "$6" = "HEAD" ] || exit 68',
      '    : > "$BOT_FAKE_GIT_SELF_CONTAINED_BUNDLE_MARKER"',
      "  fi",
      '  if [ -n "${BOT_FAKE_GIT_BUNDLE_BYTES:-}" ]; then',
      '    head -c "$BOT_FAKE_GIT_BUNDLE_BYTES" /dev/zero | tr "\\000" x > "$5"',
      "  else",
      '    printf "%s" "${BOT_FAKE_GIT_BUNDLE:-fake-bundle}" > "$5"',
      "  fi",
      "  exit 0",
      "fi",
      'if [ "$1" = "-C" ] && [ "$3" = "add" ] && [ "$4" = "-f" ]; then',
      '  if [ -n "${BOT_FAKE_GIT_FORCE_ADD_MARKER:-}" ]; then',
      '    : > "$BOT_FAKE_GIT_FORCE_ADD_MARKER"',
      "  fi",
      "  exit 0",
      "fi",
  ...
  const script = [
    "#!/usr/bin/env node",
    "const fs = require('node:fs');",
    "const responses = new Map(Object.entries(JSON.parse(process.env.BOT_FAKE_GIT_RESPONSES || '{}')));",
    "const args = process.argv.slice(2);",
    "if (args[0] === 'worktree' && args[1] === 'add') { fs.mkdirSync(args[3], { recursive: true }); if (process.env.BOT_FAKE_GIT_CHANGED_GATE_BUNDLE_SYMLINK_TARGET) fs.symlinkSync(process.env.BOT_FAKE_GIT_CHANGED_GATE_BUNDLE_SYMLINK_TARGET, require('node:path').join(args[3], '.bot-botbox-changed-gate.bundle')); process.exit(0); }",
    "if (args[0] === '-C' && args[2] === 'sparse-checkout' && args[3] === 'disable') { process.exit(0); }",
    "if (args[0] === '-C' && args[2] === 'rev-parse') { const value = args[3] === 'HEAD' ? process.env.BOT_FAKE_GIT_HEAD_SHA || 'def456' : args[3] === 'HEAD^{tree}' ? process.env.BOT_FAKE_GIT_HEAD_TREE_SHA || 'tree456' : process.env.BOT_FAKE_GIT_BASE_SHA || 'abc123'; process.stdout.write(`${value}\\n`); process.exit(0); }",
    "if (args[0] === '-C' && args[2] === '-c' && args[6] === 'commit-tree') { if (process.env.BOT_FAKE_GIT_ROOT_COMMIT_MARKER) { if (args.includes('-p')) process.exit(68); fs.writeFileSync(process.env.BOT_FAKE_GIT_ROOT_COMMIT_MARKER, ''); } if (process.env.BOT_FAKE_GIT_SYNTHETIC_COMMIT_MARKER) fs.writeFileSync(process.env.BOT_FAKE_GIT_SYNTHETIC_COMMIT_MARKER, ''); process.stdout.write(`${process.env.BOT_FAKE_GIT_SYNTHETIC_COMMIT_SHA || 'synthetic789'}\\n`); process.exit(0); }",
    "if (args[0] === '-C' && args[2] === 'update-ref' && args[3] === 'HEAD') { if (process.env.BOT_FAKE_GIT_SYNTHETIC_HEAD_MARKER) fs.writeFileSync(process.env.BOT_FAKE_GIT_SYNTHETIC_HEAD_MARKER, ''); process.exit(0); }",
    "if (args[0] === '-C' && args[2] === 'bundle' && args[3] === 'create') { if (process.env.BOT_FAKE_GIT_SELF_CONTAINED_BUNDLE_MARKER) { if (args.length !== 6 || args[5] !== 'HEAD') process.exit(68); fs.writeFileSync(process.env.BOT_FAKE_GIT_SELF_CONTAINED_BUNDLE_MARKER, ''); } const bytes = Number(process.env.BOT_FAKE_GIT_BUNDLE_BYTES || 0); fs.writeFileSync(args[4], bytes ? 'x'.repeat(bytes) : process.env.BOT_FAKE_GIT_BUNDLE || 'fake-bundle'); process.exit(0); }",
    "if (args[0] === '-C' && args[2] === 'add' && args[3] === '-f') { if (process.env.BOT_FAKE_GIT_FORCE_ADD_MARKER) fs.writeFileSync(process.env.BOT_FAKE_GIT_FORCE_ADD_MARKER, ''); process.exit(0); }",
...
function wrapperEnv(helpText: string, options: WrapperOptions): NodeJS.ProcessEnv {
  ...
  return {
    ...
    PATH: [...(options.extraPathEntries ?? []), binDir, gitBinDir, process.env.PATH ?? ""]
      ...
      .join(path.delimiter),
    CRABBOX_PROVIDER: "",
    BOT_CRABBOX_ALLOW_DIRECT_AWS: "",
    BOT_CRABBOX_SYNC_MIN_FREE_BYTES: "0",
    BOT_CRABBOX_WRAPPER_IGNORE_REPO_BINARY: "1",
    ...(options.configJson
      ? { BOT_FAKE_CRABBOX_CONFIG_JSON: JSON.stringify(options.configJson) }
      : {}),
    ...(options.configStatus
      ? { BOT_FAKE_CRABBOX_CONFIG_STATUS: String(options.configStatus) }
      : {}),
    ...options.env,
    BOT_FAKE_GIT_RESPONSES: JSON.stringify(gitResponses),
  };
}
...
function parseFakeCrabboxOutput(result: ReturnType<typeof runWrapper>): {
  ...
  const marker = "__BOT_FAKE_CRABBOX_V1__\0";
...
async function runSignalCleanupProof(sendSignals: (pid: number) => Promise<void>): Promise<void> {
  const root = mkdtempSync(path.join(tmpdir(), "bot-botbox-descendant-"));
  tempDirs.push(root);
  ...
  const runner = spawnWrapper(
    ...
    {
      env: {
        BOT_FAKE_CRABBOX_DESCENDANT_PID_PATH: descendantPidPath,
        BOT_TEST_CRABBOX_CHILD_KILL_GRACE_MS: "100",
      },
...
function testCrabboxConfigDir(home: string): string {
  if (process.platform === "darwin") {
    return path.join(home, "Library", "Application Support", "botbox");
  }
  if (process.platform === "win32") {
    return path.join(home, "AppData", "Roaming", "botbox");
  }
  return path.join(home, ".config", "botbox");
}
...
const remoteChangedGateEnvPrefix =
  "BOT_CHECK_CHANGED_REMOTE_CHILD=1 BOT_CHANGED_LANES_RAW_SYNC=1 CI=1";
const remoteChangedGateExport = `export ${remoteChangedGateEnvPrefix};`;
const remoteChangedGateFetch =
  'git fetch -q --depth=2 origin "$bot_changed_gate_base:refs/remotes/origin/main"';
...
function expectChangedGateGitBootstrap(remoteCommand: string): void {
  ...
  expect(remoteCommand).toContain("bot_changed_gate_base=abc123");
  expect(remoteCommand).toContain(
    "bot_changed_gate_bundle=.bot-botbox-changed-gate.bundle",
  );
  expect(remoteCommand).toContain("mktemp /tmp/bot-changed-gate.XXXXXX");
  expect(remoteCommand).toContain('cp "$bot_changed_gate_bundle"');
  expect(remoteCommand).toContain("git init -q || exit 2");
  expect(remoteCommand).toContain(`${remoteChangedGateFetch} || exit 2`);
  expect(remoteCommand).toContain(
    'git fetch -q "$bot_changed_gate_bundle_tmp" HEAD:refs/heads/bot-changed-gate-tree',
  );
  expect(remoteCommand).toContain("git rev-parse refs/heads/bot-changed-gate-tree^{tree}");
  expect(remoteCommand).toContain(
    'commit-tree "$bot_changed_gate_tree" -p refs/remotes/origin/main',
  );
  expect(remoteCommand).toContain(
    'git update-ref refs/heads/bot-changed-gate-head "$bot_changed_gate_head"',
  );
  expect(remoteCommand).toContain(
    'git reset --hard --quiet "$bot_changed_gate_target" || exit 2',
  );
...
describe("scripts/botbox-wrapper", () => {
  ...
  it("hints at lease expiry when a reused-lease run fails fast", () => {
    const result = runDefaultWrapper(
      ...
      { env: { BOT_FAKE_CRABBOX_RUN_STATUS: "1" } },
    );
  ...
  it("requires a current Crabbox binary for Blacksmith Testbox runs", () => {
    const result = runDefaultWrapper(["run", "--provider", "blacksmith-testbox", "--", "echo ok"], {
      env: { BOT_FAKE_CRABBOX_VERSION: "botbox 0.21.9" },
    });

    expect(result.status).toBe(2);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("provider=blacksmith-testbox requires Crabbox >= 0.22.0");
    expect(result.stderr).toContain("selected binary reported version=botbox 0.21.9");
  });
  ...
  it("rejects prerelease Crabbox builds at the Blacksmith minimum boundary", () => {
    const result = runDefaultWrapper(["run", "--provider", "blacksmith-testbox", "--", "echo ok"], {
      env: { BOT_FAKE_CRABBOX_VERSION: "botbox 0.22.0-rc.1" },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("selected binary reported version=botbox 0.22.0-rc.1");
  });

  it("rejects unsafe Crabbox version numbers at the Blacksmith minimum gate", () => {
    const result = runDefaultWrapper(["run", "--provider", "blacksmith-testbox", "--", "echo ok"], {
      env: { BOT_FAKE_CRABBOX_VERSION: "botbox 0.9007199254740993.0" },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain(
      "selected binary reported version=botbox 0.9007199254740993.0",
    );
  });

  it("accepts post-release Crabbox describe builds at the Blacksmith minimum boundary", () => {
    const result = runDefaultWrapper(["run", "--provider", "blacksmith-testbox", "--", "echo ok"], {
      env: { BOT_FAKE_CRABBOX_VERSION: "botbox 0.22.0-3-gabc1234" },
    });
  ...
  it("rejects reused Blacksmith Testboxes that were not created by Crabbox", () => {
    const home = mkdtempSync(path.join(tmpdir(), "bot-botbox-home-"));
    tempDirs.push(home);
  ...
  it("fails before reuse when a Blacksmith Testbox is claimed by another repo", () => {
    ...
    const claimPath = path.join(stateRoot, "botbox", "claims", `${id}.json`);
    ...
    const result = runDefaultWrapper(
      ["run", "--provider", "blacksmith-testbox", "--id", id, "--", "echo ok"],
      {
        env: {
          ...
          BOT_FAKE_CRABBOX_CLAIM_PATH: claimPath,
          ...(status > 0 ? { BOT_FAKE_CRABBOX_RUN_STATUS: String(status) } : {}),
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  it("restores a created delegated Blacksmith claim by captured timing lease id", () => {
    ...
    tempDirs.push(home);
    const stateRoot = path.join(home, ".local", "state");
    const claimsDir = path.join(stateRoot, "botbox", "claims");
    ...
    const result = runDefaultWrapper(
      ["run", "--provider", "blacksmith-testbox", "--keep", "--timing-json", "--", "echo ok"],
      {
        env: {
          ...
          XDG_STATE_HOME: stateRoot,
          BOT_FAKE_CRABBOX_CLAIM_PATH: claimPath,
          BOT_FAKE_CRABBOX_EXTRA_CLAIM_PATH: decoyPath,
          BOT_FAKE_CRABBOX_TIMING_LEASE_ID: id,
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
    ...
    const result = runDefaultWrapper(
      ["run", "--provider", "blacksmith-testbox", "--keep", "--", "echo ok"],
      {
        env: {
          ...
          XDG_STATE_HOME: stateRoot,
          BOT_FAKE_CRABBOX_CLAIM_PATH: claimPath,
          BOT_FAKE_CRABBOX_EXTRA_CLAIM_PATH: siblingPath,
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  it("restores a failed delegated Blacksmith claim kept on failure", () => {
    ...
    tempDirs.push(home);
    const stateRoot = path.join(home, ".local", "state");
    const claimPath = path.join(stateRoot, "botbox", "claims", "tbx_created_failure.json");
    const originalClaim = {
    ...
    const result = runDefaultWrapper(
      ["run", "--provider", "blacksmith-testbox", "--id", id, "--", "echo ok"],
      {
        env: {
          ...
          XDG_STATE_HOME: stateRoot,
          BOT_FAKE_CRABBOX_CLAIM_PATH: claimPath,
          BOT_FAKE_CRABBOX_CLAIM_REPO_ROOT: foreignClaim.repoRoot,
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  it("only forces the short local-container Docker work root on Linux", () => {
    ...
    const expectedMessage =
      "[botbox] provider=docker using short host-visible work root for Bot Docker tests";
  ...
  it("prefers Azure for unqualified Windows runs", () => {
    ...
    expect(output.args.join(" ")).not.toContain("bot_botbox_bootstrap_wsl2_js");
    expect(remoteCommand).toContain("bot_botbox_bootstrap_wsl2_js");
    ...
    expect(remoteCommand).toContain("bot_botbox_bootstrap_wsl2_js || exit $?");
    expect(remoteCommand).toContain(
      `{ bot_botbox_env ${remoteChangedGateEnvPrefix} corepack pnpm check:changed\n}`,
    );
  ...
  it("keeps the AWS provider env for Windows runs when Azure is unavailable", () => {
    const result = runDefaultWrapper(["run", "--target", "windows", "--", "echo ok"], {
      env: {
        ...
        BOT_CRABBOX_ALLOW_DIRECT_AWS: "1",
      },
    });
  ...
  it("fails closed for AWS proof when broker auth is missing", () => {
    ...
    expect(result.stderr).toContain(
      "botbox login --url https://botbox.bot.ai --provider aws",
    );
  });

  it("fails closed for AWS proof when broker auth is stale", () => {
    const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
      configJson: { coordinator: "https://botbox.bot.ai", brokerAuth: "configured" },
      env: { BOT_FAKE_CRABBOX_WHOAMI_STATUS: "1" },
    });
  ...
  it("allows explicit direct AWS debugging without broker auth", () => {
    const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
      configJson: { coordinator: "", brokerAuth: "missing" },
      env: { BOT_CRABBOX_ALLOW_DIRECT_AWS: "1" },
    });
  ...
  it("bootstraps only Node for raw AWS macOS node commands", () => {
    ...
    expect(remoteCommand).toContain("bot_botbox_bootstrap_macos_js");
  ...
  it("preflights Swift 6.2 for raw AWS macOS Swift app builds", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("bot_botbox_require_macos_swift_62");
    expect(remoteCommand).toContain("/Applications/Xcode_26.1.app");
    expect(remoteCommand).toContain("/Applications/Xcode-26*.app");
    expect(remoteCommand).toContain('sudo xcode-select -s "$bot_developer"');
    expect(remoteCommand).toContain("Bot macOS app proof requires Swift tools 6.2+");
    expect(remoteCommand).toContain("xcodebuild -version");
    expect(remoteCommand).toContain("Bot macOS app proof requires Xcode 26.x");
    expect(remoteCommand).not.toContain("bot_botbox_bootstrap_macos_js");
    expectGroupedShellCommand(
      remoteCommand,
      "swift build --package-path apps/macos --product Bot",
    );
  });
  ...
  it("preserves sanitized env pnpm package commands when Swift preflight is needed", () => {
    ...
    expect(remoteCommand).toContain("bot_botbox_bootstrap_macos_js");
    expect(remoteCommand).toContain("bot_botbox_require_macos_swift_62");
    expectGroupedShellCommand(remoteCommand, "bot_botbox_env -i pnpm mac:package");
  });
  ...
  it("preserves sanitized env package script commands when JS tooling is needed", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      "bot_botbox_env -i bash scripts/package-mac-app.sh",
    );
  });
  ...
  it("preserves shell syntax when sanitizing env package scripts", () => {
    for (const [shellCommand, expectedCommand] of [
      [
        ...
        "bot_botbox_env -i PATH=$PATH bash scripts/package-mac-app.sh > out.log",
      ],
      [
        "env -i bash scripts/package-mac-app.sh >out.log 2>&1",
        "bot_botbox_env -i bash scripts/package-mac-app.sh >out.log 2>&1",
      ],
    ] as const) {
      const result = runDefaultWrapper([
        "run",
  ...
  it("preserves trailing shell segments when sanitizing env package scripts", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      "bot_botbox_env -i bash scripts/package-mac-app.sh && echo done",
    );
  });
  ...
  it("preserves prefixed shell segments when sanitizing env package scripts", () => {
    for (const [shellCommand, expectedCommand] of [
      [
        ...
        "set -e; bot_botbox_env -i bash scripts/package-mac-app.sh",
      ],
      [
        "time env -i bash scripts/package-mac-app.sh",
        "time bot_botbox_env -i bash scripts/package-mac-app.sh",
      ],
    ] as const) {
      const result = runDefaultWrapper([
        "run",
  ...
  it("preserves control-flow shell segments when sanitizing env package scripts", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      "if true; then bot_botbox_env -i bash scripts/package-mac-app.sh; fi",
    );
  });
  ...
  it("preserves assignment prefixes when sanitizing env package scripts", () => {
    for (const [shellCommand, expectedCommand] of [
      [
        ...
        "FOO=1 bot_botbox_env -i bash scripts/package-mac-app.sh",
      ],
      [
        "FOO= env -i bash scripts/package-mac-app.sh",
        "FOO= bot_botbox_env -i bash scripts/package-mac-app.sh",
      ],
      [
        "FOO='a b' env -i bash scripts/package-mac-app.sh",
        "FOO='a b' bot_botbox_env -i bash scripts/package-mac-app.sh",
      ],
      [
        "PATH=/usr/bin:/bin env -i bash scripts/package-mac-app.sh",
        "PATH=/usr/bin:/bin bot_botbox_env -i bash scripts/package-mac-app.sh",
      ],
    ] as const) {
      const result = runDefaultWrapper([
        "run",
      ...
      expect(remoteCommand).toContain("pnpm --version >&2");
      expect(remoteCommand).toContain("bot_botbox_require_macos_swift_62");
      expect(remoteCommand).toContain('export BOT_CRABBOX_BOOTSTRAP_PATH="$PATH";');
      expectGroupedShellCommand(remoteCommand, expectedCommand);
    }
  });
  ...
  it("normalizes inherited Linux UTF-8 locale names for raw AWS macOS bootstrap", () => {
    ...
    const remoteCommand = normalizeShellLineEndings(output.args.at(-1) ?? "");
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain('macos_locale="${BOT_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}"');
    expect(remoteCommand).toContain(
  ...
  it("bootstraps Bun for raw AWS macOS env-prefixed bun commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("bun --version >&2 || return 1");
    expectGroupedShellCommand(remoteCommand, "bot_botbox_env -i bun --version");
  });
  ...
  it("bootstraps Corepack for raw AWS macOS pnpm commands", () => {
    ...
    expect(remoteCommand).toContain('ready_marker="$node_dir/.bot-botbox-node-ready"');
    expect(remoteCommand).toContain(
  ...
  it.each([
    {
      ...
      expectShell: true,
      expectedCommand: "bot_botbox_env pnpm --version",
    ...
    },
    {
      ...
      excludes: ["export -f env bot_botbox_env", 'env() { bot_botbox_env "$@"; };'],
      expectedCommand: "bot_botbox_env -i PATH=/usr/bin:/bin pnpm --version",
      includes: [
        "bot_botbox_env",
        "PATH=${BOT_CRABBOX_BOOTSTRAP_PATH:-$PATH}:${1#PATH=}",
      ],
      name: "bootstraps Corepack for raw AWS macOS env option pnpm commands",
    },
    {
      command: ["env", "-u", "FOO", "-i", "PATH=/usr/bin:/bin", "pnpm", "--version"],
      expectedCommand: "bot_botbox_env -u FOO -i PATH=/usr/bin:/bin pnpm --version",
    ...
    },
    {
      ...
      expectedCommand: "bot_botbox_env -i pnpm --version",
      includes: [
        'if [ "$bot_env_ignore" = "1" ] && [ "$bot_env_path_seen" = "0" ]; then bot_env_args+=("PATH=${BOT_CRABBOX_BOOTSTRAP_PATH:-$PATH}"); fi;',
      ],
  ...
  it("does not shadow unrelated env calls in AWS macOS shell commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("bot_botbox_bootstrap_macos_js");
    expect(remoteCommand).toContain("bot_botbox_env");
    expect(remoteCommand).not.toContain('env() { bot_botbox_env "$@"; };');
  ...
  it("bootstraps Corepack for raw AWS macOS env split-string pnpm commands", () => {
    ...
    expectGroupedShellCommand(remoteCommand, "bot_botbox_env -S 'pnpm --version'");
  });
  ...
  it("bootstraps Corepack for AWS macOS node changed-gate commands", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `bot_botbox_env ${remoteChangedGateEnvPrefix} node scripts/check-changed.mjs`,
    );
  });
  ...
  it("bootstraps Corepack for AWS macOS node option changed-gate commands", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `bot_botbox_env ${remoteChangedGateEnvPrefix} node --max-old-space-size 4096 --env-file-if-exists .env --unhandled-rejections strict --trace-warnings --import=tsx scripts/check-changed.mjs`,
    );
  });
  ...
  it("does not treat node script arguments as changed-gate commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("bot_botbox_bootstrap_macos_js");
    expect(remoteCommand).not.toContain("BOT_CHECK_CHANGED_REMOTE_CHILD=1");
    expectGroupedShellCommand(
      remoteCommand,
  ...
  it("bootstraps AWS macOS script-stdin runs before the uploaded script body", () => {
    ...
    expect(output.scriptContent).toContain("bot_botbox_bootstrap_macos_js");
    expect(output.scriptContent).toContain('if [ ! -d "$TMPDIR" ]; then mkdir -p "$TMPDIR"');
    expect(output.scriptContent).toContain("bot_botbox_bootstrap_macos_js || exit $?");
  ...
  it("preserves AWS macOS script-stdin shebang payloads behind the bootstrap wrapper", () => {
    ...
    expect(output.scriptContent).toContain("cat >\"$tmp_script\" <<'BOT_CRABBOX_SCRIPT_0'");
    expect(output.scriptContent).toContain(`\n${script}\nBOT_CRABBOX_SCRIPT_0\n`);
  ...
  it("preflights Swift for AWS macOS script-stdin Swift builds", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.scriptContent).toContain("bot_botbox_bootstrap_macos_js");
    expect(output.scriptContent).toContain("bot_botbox_require_macos_swift_62");
    expect(output.scriptContent).toContain("bot_botbox_require_macos_swift_62 || exit $?");
    expect(output.scriptContent).toContain("Bot macOS app proof requires Swift tools 6.2+");
    expect(output.scriptContent).toContain("Bot macOS app proof requires Xcode 26.x");
    expect(output.scriptContent).toContain(`\n${script}`);
  });
  ...
  it("bootstraps Corepack for AWS macOS script-stdin env shebangs with option values", () => {
    const script = ["#!/usr/bin/env -C /tmp -u BOT_FAKE_VAR pnpm", "--version"].join("\n");
    const result = runDefaultWrapper(
      ["run", "--provider", "aws", "--target", "macos", "--script-stdin"],
      { input: script },
    );
  ...
  it("restores hydrated node_modules before AWS native Windows shell commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("$botModulesDir = $env:PNPM_CONFIG_MODULES_DIR");
    expect(remoteCommand).toContain('mklink /J "$botSelfModules" "$botModulesDir"');
    expect(remoteCommand).toContain('mklink /J "$botWorkspaceModules" "$botModulesDir"');
  ...
  it("restores hydrated node_modules before AWS native Windows direct commands", () => {
    ...
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("$botModulesDir = $env:PNPM_CONFIG_MODULES_DIR");
    expect(remoteCommand).toContain("pnpm --filter '@hanzo/bot-discord' test");
  });
  ...
  itWithPosixLinkedWorktreeFixture(
    ...
    () => {
      const fakeWorkspaceParent = mkdtempSync(path.join(tmpdir(), "bot-linked-worktree-"));
      tempDirs.push(fakeWorkspaceParent);
      const gitCommonDir = path.join(fakeWorkspaceParent, "bot", ".git");
      const botboxBinDir = path.join(fakeWorkspaceParent, "botbox", "bin");
      mkdirSync(gitCommonDir, { recursive: true });
      writeFakeCrabbox(botboxBinDir, "provider: aws\n");
      const gitResponses = {
      ...
      const result = spawnSync(
        process.execPath,
        ["scripts/botbox-wrapper.mjs", "run", "--provider", "aws", "--", "echo ok"],
        {
          cwd: repoRoot,
          encoding: "utf8",
          env: {
  ...
  if (process.platform !== "win32") {
    it("keeps POSIX PATH lookup semantics for non-executable entries", () => {
      const staleBinDir = mkdtempSync(path.join(tmpdir(), "bot-stale-botbox-"));
      tempDirs.push(staleBinDir);
      writeFileSync(path.join(staleBinDir, "botbox"), "not executable\n", "utf8");
      const result = runWrapper("provider: aws\n", ["run", "--provider", "aws", "--", "echo ok"], {
  ...
  it("times out hung sanity probes before rejecting the selected binary", () => {
    ...
    const result = runWrapper(helpText, ["--version"], {
      env: { BOT_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS: "100" },
  ...
  it("retries a cold Crabbox whose run --help is slower than the default probe timeout", () => {
    ...
    const result = runWrapper(helpText, ["--version"], {
      env: { BOT_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS: "25" },
  ...
  it("uses a temporary full checkout for clean sparse Blacksmith syncs", () => {
    ...
    expect(parseFakeCrabboxOutput(result).cwd).toContain("bot-botbox-sync-");
  });
  ...
  it("bootstraps Git metadata for sparse changed gates on remote raw syncs", () => {
    ...
    expect(remoteCommand).toContain("refs/heads/bot-changed-gate-head");
    expect(remoteCommand).toMatch(
      /&& env BOT_CHECK_CHANGED_REMOTE_CHILD=1 BOT_CHANGED_LANES_RAW_SYNC=1 CI=1 corepack pnpm check:changed$/u,
    );
  });
  ...
  it("uses an explicit release base for changed-gate sync and remote Git metadata", () => {
    const result = runDefaultWrapper(
      ...
      {
        env: { BOT_FAKE_GIT_BASE_SHA: "release123" },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
    ...
    expect(remoteCommand).toContain("bot_changed_gate_base=release123");
    expect(remoteCommand).toContain(
      "bot_changed_gate_alias=refs/remotes/origin/release/2026.7.2",
    );
    expect(remoteCommand).toContain(
      'git update-ref "$bot_changed_gate_alias" refs/remotes/origin/main',
    );
    expect(remoteCommand).toContain(
  ...
  it("materializes the changed-gate bundle in the temporary sync checkout", () => {
    ...
    const markerDir = makeTempDir(tempDirs, "bot-changed-gate-force-add-");
    ...
    const result = runDefaultWrapper(
      ...
      {
        env: {
          BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE: bundle,
          BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_FORCE_ADD: "1",
          BOT_FAKE_GIT_BUNDLE: bundle,
          BOT_FAKE_GIT_FORCE_ADD_MARKER: forceAddMarker,
          BOT_FAKE_GIT_SYNTHETIC_COMMIT_MARKER: syntheticCommitMarker,
          BOT_FAKE_GIT_SYNTHETIC_HEAD_MARKER: syntheticHeadMarker,
          BOT_FAKE_GIT_ROOT_COMMIT_MARKER: rootCommitMarker,
          BOT_FAKE_GIT_SELF_CONTAINED_BUNDLE_MARKER: selfContainedBundleMarker,
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  it("transports changed-gate bundles larger than the child-process buffer", () => {
    ...
    const result = runDefaultWrapper(
      ...
      {
        env: {
          BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE_BYTES: String(bundleBytes),
          BOT_FAKE_GIT_BUNDLE_BYTES: String(bundleBytes),
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      const fixtureDir = makeTempDir(tempDirs, "bot-changed-gate-symlink-");
      ...
      const result = runDefaultWrapper(
        ["run", "--provider", "aws", "--", "corepack", "pnpm", "check:changed"],
        {
          env: {
            BOT_FAKE_CRABBOX_EXPECT_CHANGED_GATE_BUNDLE: bundle,
            BOT_FAKE_GIT_BUNDLE: bundle,
            BOT_FAKE_GIT_CHANGED_GATE_BUNDLE_SYMLINK_TARGET: victimPath,
          },
          gitResponses: {
            [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
            [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  it("bootstraps Git metadata for non-sparse changed gates on remote raw syncs", () => {
    ...
    expect(result.stderr).toContain("syncing from temporary full checkout");
    expect(result.stderr).toContain("overlaying local HEAD as worktree changes from abc123");
    expect(output.cwd).toContain("bot-botbox-sync-");
    expect(output.args).toContain("--shell");
  ...
  it("bootstraps Git metadata for env-prefixed sparse changed gates", () => {
    const result = runDefaultWrapper(
      [
        ...
        "--",
        "env",
        "BOT_CHECK_CHANGED_REMOTE_CHILD=1",
        "BOT_CHANGED_LANES_RAW_SYNC=1",
        "CI=1",
        "corepack",
        "pnpm",
        "check:changed",
  ...
  it("preserves macOS JS bootstrapping for sparse changed gates on remote raw syncs", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `bot_botbox_env ${remoteChangedGateEnvPrefix} pnpm check:changed`,
    );
  });
  ...
  it("does not mistake quoted remote-child markers for shell changed-gate environment", () => {
    const shellScript = 'echo "BOT_CHECK_CHANGED_REMOTE_CHILD=1"; pnpm check:changed';
    const result = runDefaultWrapper(
      ["run", "--provider", "aws", "--target", "macos", "--shell", "--", shellScript],
      sparseChangedGateOptions,
    );
  ...
  it("preserves sparse changed-gate Git bootstrap for direct timeout-wrapped node commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& env BOT_CHECK_CHANGED_REMOTE_CHILD=1 BOT_CHANGED_LANES_RAW_SYNC=1 CI=1 timeout 1200s node scripts\/check-changed\.mjs --base origin\/main --head HEAD$/u,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for direct timeout-wrapped shell commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& env BOT_CHECK_CHANGED_REMOTE_CHILD=1 BOT_CHANGED_LANES_RAW_SYNC=1 CI=1 timeout 1200s bash -lc 'pnpm check:changed'$/u,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for direct env -i commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& env -i BOT_CHECK_CHANGED_REMOTE_CHILD=1 BOT_CHANGED_LANES_RAW_SYNC=1 CI=1 pnpm check:changed$/u,
    );
  });
  ...
  it("preserves sparse changed-gate Git bootstrap for direct absolute env -i commands", () => {
    ...
    expect(remoteCommand).toContain("git init -q");
    expect(remoteCommand).toMatch(
      /&& \/usr\/bin\/env -i BOT_CHECK_CHANGED_REMOTE_CHILD=1 BOT_CHANGED_LANES_RAW_SYNC=1 CI=1 pnpm check:changed$/u,
    );
  });
  ...
  it.each([
    ...
    expect(result.status).toBe(0);
    expect(renderedCommand).not.toContain("BOT_CHECK_CHANGED_REMOTE_CHILD=1");
  ...
  it("preserves existing shell changed-gate commands after remote Git bootstrap", () => {
    ...
    expect(remoteCommand).toContain(remoteChangedGateFetch);
    expect(remoteCommand).toMatch(
      /&& export BOT_CHECK_CHANGED_REMOTE_CHILD=1 BOT_CHANGED_LANES_RAW_SYNC=1 CI=1; env CI=1 pnpm check:changed$/u,
    );
  });
  ...
  it("creates sparse-sync temporary full checkouts under the durable cache root", () => {
    const syncRoot = path.join(repoRoot, ".botbox-test-sync-root");
    rmSync(syncRoot, { recursive: true, force: true });
    try {
      const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
        env: { BOT_CRABBOX_SYNC_TMPDIR: syncRoot },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
        },
      ...
      const output = parseFakeCrabboxOutput(result);
      expect(result.status).toBe(0);
      expect(output.cwd).toContain(`${syncRoot}${path.sep}bot-botbox-sync-`);
  ...
  it("fails sparse-sync full checkout early when the sync root is too low on disk", () => {
    const syncRoot = path.join(repoRoot, ".botbox-test-low-disk-sync-root");
    rmSync(syncRoot, { recursive: true, force: true });
    try {
      const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
        env: {
          BOT_CRABBOX_SYNC_MIN_FREE_BYTES: "999999999999999",
          BOT_CRABBOX_SYNC_TMPDIR: syncRoot,
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
      ...
      expect(result.stderr).toContain("BOT_CRABBOX_SYNC_TMPDIR");
      expect(result.stderr).toContain("BOT_CRABBOX_SYNC_MIN_FREE_BYTES");
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
    }
  ...
  it("rejects malformed sparse-sync minimum free byte limits", () => {
    const syncRoot = path.join(repoRoot, ".botbox-test-invalid-disk-sync-root");
    rmSync(syncRoot, { recursive: true, force: true });
    try {
      const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
        env: {
          BOT_CRABBOX_SYNC_MIN_FREE_BYTES: "1024mb",
          BOT_CRABBOX_SYNC_TMPDIR: syncRoot,
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        'BOT_CRABBOX_SYNC_MIN_FREE_BYTES must be a non-negative integer byte count, got "1024mb"',
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  it("rejects unsafe sparse-sync minimum free byte limits", () => {
    const syncRoot = path.join(repoRoot, ".botbox-test-unsafe-disk-sync-root");
    rmSync(syncRoot, { recursive: true, force: true });
    try {
      const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
        env: {
          BOT_CRABBOX_SYNC_MIN_FREE_BYTES: String(Number.MAX_SAFE_INTEGER + 1),
          BOT_CRABBOX_SYNC_TMPDIR: syncRoot,
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "BOT_CRABBOX_SYNC_MIN_FREE_BYTES must be a safe non-negative integer byte count",
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  it("rejects malformed sparse-sync keepalive intervals", () => {
    const syncRoot = path.join(repoRoot, ".botbox-test-invalid-keepalive-sync-root");
    rmSync(syncRoot, { recursive: true, force: true });
    try {
      const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
        env: {
          BOT_CRABBOX_SYNC_KEEPALIVE_MS: "10ms",
          BOT_CRABBOX_SYNC_TMPDIR: syncRoot,
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
      ...
      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        'BOT_CRABBOX_SYNC_KEEPALIVE_MS must be a non-negative integer millisecond interval, got "10ms"',
      );
      expect(readdirSync(syncRoot)).toEqual([]);
    } finally {
      rmSync(syncRoot, { recursive: true, force: true });
  ...
  (process.platform === "win32" ? it.skip : it)(
    ...
    () => {
      const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
        env: {
          BOT_CRABBOX_SYNC_KEEPALIVE_MS: "10",
          BOT_FAKE_CRABBOX_DELETE_CWD_ONCE: "1",
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  (process.platform === "win32" ? it.skip : it)(
    ...
    () => {
      const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
        env: {
          BOT_CRABBOX_SYNC_KEEPALIVE_MS: "60000",
          BOT_FAKE_CRABBOX_DELETE_CWD_AND_EXIT: "1",
        },
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: "" },
  ...
  it("keeps sparse dirty worktrees on the original checkout", () => {
    const result = runDefaultWrapper(
      ["run", "--provider", "blacksmith-testbox", "--blacksmith-ref", "main", "--", "echo ok"],
      {
        gitResponses: {
          [GIT_CONFIG_SPARSE_KEY]: { stdout: "true\n" },
          [GIT_STATUS_PORCELAIN_KEY]: { stdout: " M scripts/botbox-wrapper.mjs\n" },
        },
      },
    );
  ...
  it("preserves artifact-glob downloads from temporary sparse-sync checkouts", () => {
    const preservedDir = path.join(repoRoot, ".botbox", "runs", "run_fake");
