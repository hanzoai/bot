// Bot Docker E2E tests cover packaged-dist harness wiring.
...
describe("Bot Docker E2E scripts", () => {
  it("keeps first-run checks wired to packaged CLI and Bot behavior", () => {
    ...
    expect(source).toContain("Bot did not fail closed without inference");
    ...
    expect(source).toContain("function resolveDefaultModel(config: BotConfig)");
    ...
    expect(source).toContain("[bot] interpreted: ${plannerCommand}");
    expect(source).toContain("expected one fuzzy setup planner prompt");
    expect(source).toContain("Bot did not enable Discord");
    expect(source).toContain("Bot did not write Discord token SecretRef");
    expect(source).toContain("Bot first-run Docker E2E passed");
    expect(spec).toContain('"auditOperations"');
    expect(spec).toContain('"bot.setup"');
  ...
  it("keeps rescue checks wired through auto-reply command handling", () => {
    ...
    expect(shell).toContain("BOT_GATEWAY_TOKEN=system-agent-rescue-token");
    ...
    expect(source).toContain("[bot] done: gateway.restart");
    ...
    expect(source).not.toContain("[bot] done: doctor.fix");
    expect(source).toContain("Bot rescue Docker E2E passed");
  });
});
