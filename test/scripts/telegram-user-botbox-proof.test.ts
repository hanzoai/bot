// Telegram User Crabbox Proof tests cover telegram user botbox proof script behavior.
...
import {
  ...
  waitForLog,
  writeSutConfig,
} from "../../scripts/e2e/telegram-user-botbox-proof.ts";
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-telegram-proof-"));
  tempDirs.push(dir);
  return dir;
}
...
function runProofCli(args: string[]) {
  return spawnSync(
    process.execPath,
    ["--import", "tsx", "scripts/e2e/telegram-user-botbox-proof.ts", ...args],
    {
      cwd: process.cwd(),
      encoding: "utf8",
      env: {
...
describe("telegram user Crabbox proof log polling", () => {
  it("starts the local gateway through the repo pnpm runner", () => {
    ...
    const spec = createBotGatewaySpawnSpec({
      env: { ...process.env, BOT_TELEGRAM_PROOF_SENTINEL: "1" },
      gatewayPort: 19042,
    ...
    expect(spec.args).toEqual([fakePnpm, "bot", "gateway", "--port", "19042"]);
    expect(spec.options.cwd).toBe(root);
    expect(spec.options.env?.BOT_TELEGRAM_PROOF_SENTINEL).toBe("1");
  ...
  it("uses an explicitly pinned pnpm executable for a worktree gateway", () => {
    ...
    expect(spec.args).toEqual(["bot", "gateway", "--port", "19042"]);
  ...
  it("routes fork SUT startup through the root-owned validating wrapper", () => {
    ...
    const spec = createContainerizedSutSpawnSpec({
      ...
      containerName: "bot-telegram-sut-test",
      gatewayEnv: {
    ...
    expect(spec.args).toContain("/usr/local/sbin/bot-mantis-sut-container");
  ...
  it("requires successful privileged SUT teardown commands", () => {
    ...
    runSutContainerAction(
      "stop",
      "bot-telegram-sut-test",
      "/tmp/bot-tg-botbox-sut-test",
      run,
    );
  ...
  it("rejects loose numeric log tail limits instead of parsing prefixes", () => {
    expect(() =>
      readTelegramUserProofLogTailBytes({
        BOT_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1e3",
      }),
    ).toThrow("invalid BOT_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1e3");
    expect(() =>
      readTelegramUserProofLogTailBytes({
        BOT_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1000bytes",
      }),
    ).toThrow("invalid BOT_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1000bytes");
    expect(
      readTelegramUserProofLogTailBytes({
        BOT_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "4096",
      }),
    ).toBe(4096);
  });
  ...
  it("rejects duplicate single-value proof controls while keeping repeated expectations", () => {
    ...
    expect(parseArgs(["--expect", "Bot", "--expect", "ready"]).expect).toEqual([
      "Bot",
      "ready",
    ]);
  });
  ...
  it("uses unique default output dirs", () => {
    ...
    expect(path.dirname(firstOutputDir)).toBe(
      path.join(".artifacts", "qa-e2e", "telegram-user-botbox"),
    );
  ...
  it("writes an isolated Funnel and official MCP App fixture config", () => {
    ...
    expect(config.gateway).toMatchObject({
      auth: {
        mode: "password",
        password: { id: "BOT_GATEWAY_PASSWORD", source: "env" },
      },
  ...
  it("shell-quotes generated remote setup and chat literals", () => {
    const payload = "name $(touch /tmp/bot-proof-injected) `touch /tmp/also-injected`";
  ...
  it("stages full publish artifacts without session control files", () => {
    ...
    fs.writeFileSync(path.join(outputDir, "telegram-user-botbox-session-summary.json"), "{}");
    fs.writeFileSync(path.join(outputDir, "telegram-user-botbox-proof.md"), "report");
    fs.writeFileSync(path.join(outputDir, "telegram-desktop.log"), "log");
    fs.writeFileSync(path.join(outputDir, "telegram-user-botbox-session-motion.gif"), "gif");
    fs.writeFileSync(path.join(outputDir, "telegram-user-botbox-session.mp4"), "video");
    ...
    expect(fs.readdirSync(stagedDir).toSorted()).toEqual([
      ...
      "telegram-user-botbox-proof.md",
      "telegram-user-botbox-session-motion.gif",
      "telegram-user-botbox-session-summary.json",
      "telegram-user-botbox-session.mp4",
    ]);
  ...
  it("requires finish to write the proof report before full artifact publishing", () => {
    ...
    expect(() => stageFullSessionArtifacts(outputDir)).toThrow(
      "Missing proof report. Run finish first: telegram-user-botbox-proof.md",
    );
  ...
  posixIt("does not expand generated remote probe arguments in the shell", () => {
    ...
    writeExecutable(
      ...
      `#!/usr/bin/env node
import fs from "node:fs";
fs.writeFileSync(process.env.BOT_TEST_ARGV_PATH, JSON.stringify(process.argv.slice(1)));
`,
    );
    writeExecutable(
      scriptPath,
...
const proof = await import(${JSON.stringify(
        pathToFileURL(path.resolve("scripts/e2e/telegram-user-botbox-proof.ts")).href,
      )});
...
`,
  ...
  posixIt("stops Crabbox recording when the desktop probe fails", async () => {
    const root = makeTempDir();
    const recorderPath = path.join(root, "fake-botbox-recorder.mjs");
