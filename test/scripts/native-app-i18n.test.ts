...
describe("native app i18n inventory", () => {
  ...
  it("preserves registered IDs when Swift entries move between files", async () => {
    ...
    const sendingPath =
      "apps/shared/BotKit/Sources/BotChatUI/ChatViewModel+Sending.swift";
    const movedEntries = [
  ...
  it("extracts only localizable usage descriptions from Apple plists", () => {
    const entries = extractNativeI18nCandidates(
      ...
      `<plist><dict>
        ...
        <string>Bot Fixture</string>
        <key>NSCameraUsageDescription</key>
        <string>Bot uses the camera to scan setup codes &amp; documents.</string>
        <key>BotFixtureValue</key>
        <string>Runtime configuration value</string>
      </dict></plist>`,
    );

    expect(entries.map((entry) => entry.source)).toEqual([
      "Bot uses the camera to scan setup codes & documents.",
    ]);
  });
    ...
    const entries = extractNativeI18nCandidates(
      ...
      `<resources>
        <string-array name="capabilities" translatable="false">
          ...
          <item>bot_wear_companion_v1</item>
  ...
  it("collects stable Android and Apple UI entries", async () => {
    ...
    expect(
      entries
        ...
        .every((entry) =>
          /^(?:apps\/ios|apps\/macos\/Sources|apps\/shared\/BotKit\/Sources)\//u.test(
            entry.path,
          ),
        ),
    ).toBe(true);
    ...
    expect(
      entries.some(
        (entry) =>
          entry.path.endsWith(
            "/thirdParty/java/ai/bot/app/ui/SensitivePhoneCapabilitiesSettings.kt",
    ...
    expect(
      entries.some((entry) => entry.surface === "android" && entry.source === "Search Bot"),
    ).toBe(true);
    expect(
      entries.some(
        (entry) =>
    ...
    expect(entries.some((entry) => entry.source === "Check Bot status")).toBe(true);
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Summarize the current Bot status and tell me what needs attention.",
      ),
    ).toBe(true);
    expect(
      entries.some(
    ...
    expect(entries.some((entry) => entry.source === "ask Bot $prompt")).toBe(true);
    expect(entries.some((entry) => entry.source === "Bot is paused")).toBe(true);
    expect(
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source === 'Bot needs ${labels.joinToString(", ")} permissions to continue.',
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Writes a rotating, local-only log under ~/Library/Logs/Bot/. Enable only while actively debugging.",
      ),
    ).toBe(true);
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Paste the token configured on the gateway host. On the gateway host, run `bot config get gateway.auth.token`. If the gateway uses an environment variable instead, use `BOT_GATEWAY_TOKEN`.",
      ),
    ).toBe(true);
    expect(
      entries.some((entry) =>
        [
          ...
          "Writes a rotating, local-only log under ~/Library/Logs/Bot/. ",
    ...
    expect(
      entries.some((entry) =>
        entry.source.startsWith(
          "Exec approvals can only be reviewed while Bot is open and connected.",
        ),
      ),
    ).toBe(true);
    expect(entries.some((entry) => entry.source === "$(PRODUCT_BUNDLE_IDENTIFIER)")).toBe(false);
    expect(entries.some((entry) => entry.source === "ai.bot.screenRecord.writer")).toBe(false);
    expect(
      entries.some(
        (entry) =>
  ...
  it("creates a first-run locale artifact and leaves a complete artifact unchanged", async () => {
    ...
    const translationsDir = makeTempDir(tempDirs, "bot-native-i18n-");
    ...
    const inventory: NativeI18nEntry[] = [
      {
        ...
        line: 89,
        path: "apps/android/app/src/main/java/ai/bot/app/AppLanguage.kt",
        source: "Bot translations · $languageTag",
        surface: "android",
      },
      {
