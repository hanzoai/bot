...
function withReport(payload: unknown, run: (reportPath: string) => void) {
  const reportPath = path.join(os.tmpdir(), `bot-test-perf-budget-${Date.now()}.json`);
  fs.writeFileSync(
    reportPath,
...
describe("test perf budget script", () => {
  ...
  it("parses numeric budget env vars strictly before running Vitest", () => {
    expect(
      testing.parseArgs([], {
        BOT_TEST_PERF_BASELINE_WALL_MS: "1000",
        BOT_TEST_PERF_MAX_REGRESSION_PCT: "12.5",
        BOT_TEST_PERF_MAX_WALL_MS: "1500",
      }),
    ).toMatchObject({
      baselineWallMs: 1000,
    ...
    expect(() =>
      testing.parseArgs([], {
        BOT_TEST_PERF_MAX_WALL_MS: "1000ms",
      }),
    ).toThrow("BOT_TEST_PERF_MAX_WALL_MS must be a non-negative number");
  });
