...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "bot-dist-plist-"));
  tempDirs.push(dir);
...
describe("package-mac-dist plist validation", () => {
  it("fails closed for required Info.plist reads", () => {
    ...
    const readBlock = script.slice(
      ...
      script.indexOf('ZIP="$ROOT_DIR/dist/Bot-$VERSION.zip"'),
    );
  ...
  it("fails on old Swift before reading package metadata", () => {
    const toolsDir = mkdtempSync(path.join(tmpdir(), "bot-dist-swift-tools-"));
  ...
  it("prefers repo Corepack pnpm over a global pnpm shim", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "bot-dist-pnpm-root-"));
    const outerRoot = mkdtempSync(path.join(tmpdir(), "bot-dist-pnpm-outer-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "bot-dist-pnpm-tools-"));
  ...
  it("keeps dependency bootstrap output out of captured Sparkle build values", () => {
    ...
    const dir = mkdtempSync(path.join(tmpdir(), "bot-dist-sparkle-"));
    tempDirs.push(dir);
    ...
    const result = runHelper(`
      ...
      BOT_ROOT=${JSON.stringify(process.cwd())}
      BOT_MARKER=${JSON.stringify(marker)}
      PATH=${JSON.stringify(tools)}:/usr/bin:/bin
      export BOT_MARKER BOT_ROOT PATH
      ${helpers}
