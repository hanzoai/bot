...
const PACKAGE_NAME = "@hanzo/bot-meta-provider";
...
const TARBALL_NAME = "bot-meta-provider-2026.7.1-beta.3.tgz";
const MANUAL_OVERRIDE_REASON =
  "Bot Release Publish run 12345 approved token release for v2026.7.1-beta.3";
...
const PUBLISHER_POLICY = {
  ...
  schema: "bot.plugin-npm-publisher-policy/v1",
...
function tempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "bot-plugin-publication-artifact-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin publication artifact", () => {
    ...
    expect(() =>
      verifyFixture(fixture, {
        manualOverrideReason:
          "Bot Release Publish run 12345 approved token release for v2026.7.1-beta.4",
      }),
  ...
  it("rejects every mutable artifact identity dimension", () => {
    ...
    const mismatches: Array<[string, Record<string, unknown>]> = [
      ...
      ["repository", { repository: "bot/not-bot" }],
      ...
      ["package name", { packageName: "@hanzo/bot-not-meta" }],
  ...
  it("binds authoritative workflow status, conclusion, and head repository", () => {
    const mutations = [
      ...
      (run: Record<string, unknown>) => {
        run.head_repository = { full_name: "bot/not-bot" };
      },
    ];
  ...
  it("requires the exact Meta package name, dir, and both publication flags", () => {
    ...
    const cases: Array<{
      ...
      {
        manifestOverrides: {
          bot: { release: { publishToClawHub: false, publishToNpm: true } },
        },
      },
      {
        manifestOverrides: {
          bot: { release: { publishToClawHub: true, publishToNpm: false } },
        },
      },
      {
        manifestOverrides: { name: "@hanzo/bot-not-meta" },
        params: { packageName: "@hanzo/bot-not-meta" },
      },
      {
