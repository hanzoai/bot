// Bot NPM Publish tests cover publish wrapper argument safety.
...
const scriptPath = "scripts/bot-npm-publish.sh";
...
function makeReleaseCheckout(root: string, version: string): string {
  ...
  copyFileSync(
    "scripts/bot-npm-extended-stable-release.mjs",
    path.join(checkout, "scripts/bot-npm-extended-stable-release.mjs"),
  );
...
function makePackageTarball(root: string, packageJson?: string): string {
  ...
  const tarball = path.join(root, "bot.tgz");
...
describe("bot npm publish wrapper", () => {
  ...
  it("rejects extra publish arguments before npm publish", () => {
    const tempRoot = makeTempDir("bot-npm-publish-");
    const tarball = path.join(tempRoot, "bot.tgz");
  ...
  it.each([
    ...
    ["missing version", JSON.stringify({ name: "bot" }), "has no valid version"],
  ...
  it("rejects publishing the current pre-.33 final version to extended-stable", () => {
    ...
    const result = runPublishWrapper(
      ["--publish"],
      {
        BOT_NPM_PUBLISH_TAG: "extended-stable",
      },
      checkout,
    );
