...
function makeTempDir(): string {
  const root = mkdtempSync(path.join(tmpdir(), "bot-parallels-npm-update-"));
  tempDirs.push(root);
  return root;
}
...
function extractWindowsBackgroundControlMarkers(decoded: string): {
  ...
  return {
    done: marker("__BOT_BACKGROUND_DONE__", false),
    exitPrefix: marker("__BOT_BACKGROUND_EXIT__", true),
  };
}
...
describe("parallels npm update smoke", () => {
  it("accepts one prepared tarball target for update and fresh install", () => {
    expect(
      parseArgs([
        "--target-tarball",
        "/tmp/bot-candidate.tgz",
        "--dependency-tarball",
        "/tmp/bot-ai-candidate.tgz",
        "--registry-package-tarball",
        "/tmp/bot-codex-candidate.tgz",
      ]),
    ).toMatchObject({
      dependencyTarballs: ["/tmp/bot-ai-candidate.tgz"],
      registryPackageTarballs: ["/tmp/bot-codex-candidate.tgz"],
      targetTarball: "/tmp/bot-candidate.tgz",
      updateTarget: "",
      freshTargetSpec: undefined,
    });
    expect(() =>
      parseArgs(["--target-tarball", "/tmp/bot-candidate.tgz", "--update-target", "beta"]),
    ).toThrow("--target-tarball cannot be combined");
    expect(() => parseArgs(["--dependency-tarball", "/tmp/bot-ai-candidate.tgz"])).toThrow(
      "--dependency-tarball requires --target-tarball",
    );
    expect(() =>
      parseArgs(["--registry-package-tarball", "/tmp/bot-codex-candidate.tgz"]),
...
printf '%s\\n' "$*" >>"$log_path"
args=" $* "
if [[ "$args" == *" /usr/bin/tee /tmp/bot-parallels-npm-update-linux-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" /bin/chmod 755 /tmp/bot-parallels-npm-update-linux-"* ]]; then
  echo "chmod denied" >&2
  exit 7
fi
if [[ "$args" == *" /bin/rm -f /tmp/bot-parallels-npm-update-linux-"* ]]; then
  printf 'cleanup\\n' >>"$log_path"
  exit 0
fi
exit 1
`,
      ...
      () => {
        ...
        expect(() =>
          writeGuestScript.call(
            ...
            "Linux VM",
            "echo update",
            "bot-parallels-npm-update-linux",
          ),
    ...
    expect(log).toContain("/bin/chmod 755 /tmp/bot-parallels-npm-update-linux-");
    expect(log).toContain("/bin/rm -f /tmp/bot-parallels-npm-update-linux-");
...
if [[ "$args" == *" /usr/bin/tee /tmp/bot-parallels-npm-update-macos-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" /bin/chmod 700 /tmp/bot-parallels-npm-update-macos-"* ]]; then
  exit 0
fi
if [[ "$args" == *" /usr/sbin/chown desktop-user /tmp/bot-parallels-npm-update-macos-"* ]]; then
  exit 0
fi
if [[ "$args" == *" /bin/rm -f /tmp/bot-parallels-npm-update-macos-"* ]]; then
  exit 0
fi
exit 1
`,
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(call?.[1]).toEqual([
          ...
          "/bin/bash",
          expect.stringMatching(/^\/tmp\/bot-parallels-npm-update-macos-/),
        ]);
      },
    );
    ...
    expect(log).toContain("/usr/bin/tee /tmp/bot-parallels-npm-update-macos-");
    expect(log).toContain("/bin/chmod 700 /tmp/bot-parallels-npm-update-macos-");
  ...
  it("has a one-command beta validation mode with fresh target coverage", () => {
    ...
    expect(script).toContain("resolveBotRegistryVersion");
    expect(script).toContain("this.options.updateTarget = version");
    expect(script).toContain("this.options.freshTargetSpec = `bot@${version}`");
  ...
  it("serves a prepared package set for both proof phases", () => {
    ...
    expect(script).toContain("this.freshTargetSpec = `bot@${this.targetTarballVersion}`");
    ...
    expect(script).toContain('readPositiveIntEnv("BOT_PARALLELS_NPM_UPDATE_TIMEOUT_S", 2700)');
  });
  ...
  it("accepts keyed and nested npm metadata for published update targets", () => {
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          ...
          "dist.tarball": "https://registry.example/bot-keyed.tgz",
          gitHead: "abcdef0123456789",
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/bot-keyed.tgz",
      gitHead: "abcdef0123456789",
    });
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          version: "2026.5.20-beta.1",
          dist: { tarball: "https://registry.example/bot-nested.tgz" },
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/bot-nested.tgz",
      gitHead: "",
    });
  });
  ...
  it("guards beta validation against cross-version harness checkouts", () => {
    ...
    expect(script).toContain("botVersionFamily");
    expect(script).toContain("BOT_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH");
  ...
  it("bounds POSIX guest failure logs", () => {
    ...
    expect(scripts).toContain("BOT_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES");
    expect(scripts).toContain('print_log_tail "$output_file"');
    expect(scripts).toContain("print_log_tail /tmp/bot-parallels-macos-gateway.log >&2");
    expect(scripts).toContain("print_log_tail /tmp/bot-parallels-linux-gateway.log >&2");
    expect(scripts).not.toContain('cat "$output_file"');
    expect(scripts).not.toContain("cat /tmp/bot-parallels-");
  });
  ...
  it("passes platform model timeouts to POSIX update agent turns", () => {
    ...
    withEnv(
      {
        BOT_PARALLELS_LINUX_MODEL_TIMEOUT_S: undefined,
        BOT_PARALLELS_MACOS_MODEL_TIMEOUT_S: undefined,
        BOT_PARALLELS_MODEL_TIMEOUT_S: undefined,
      },
      () => {
    ...
    withEnv(
      {
        BOT_PARALLELS_LINUX_MODEL_TIMEOUT_S: "321",
        BOT_PARALLELS_MACOS_MODEL_TIMEOUT_S: "654",
      },
      () => {
  ...
  it("sets platform-aware fresh lane timeouts", () => {
    withEnv({ BOT_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: undefined }, () => {
    ...
    withEnv({ BOT_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: "3" }, () => {
      expect(freshLaneTimeoutMs("macos")).toBe(3000);
    });

    withEnv(
      { BOT_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: String(Number.MAX_SAFE_INTEGER) },
      () => {
  ...
  it("clears update stream timers when spawning the guest command fails", async () => {
    ...
    await expect(
      runStreamingToJobLog.call(smoke, "bot-definitely-missing-command", [], 60 * 60 * 1000, {
        append: () => undefined,
        logPath: "",
  ...
  it("cleans timed-out Windows background work", async () => {
    ...
    expect(commands).toContain("function Write-BotUtf8File");
    expect(commands).toContain("[System.Text.UTF8Encoding]::new($false)");
    expect(payloads).toContain("Write-BotUtf8File $exitPath '0'");
    expect(payloads).toContain("Write-BotUtf8File $donePath 'done'");
    expect(payloads).toContain("Write-BotUtf8File $pidPath ([string]$PID)");
    expect(commands).toContain('cmd.exe /d /s /c start "" /b powershell.exe');
    expect(commands).toContain("icacls.exe $runDir /inheritance:r");
    expect(commands).toContain("Stop-BotBackgroundProcessTree ([int]$backgroundPid)");
  ...
  it("drains completed Windows background logs before cleanup", async () => {
    ...
    expect(decodedCommands.join("\n")).not.toContain("Stop-BotBackgroundProcessTree");
    expect(decodedCommands.join("\n")).toContain(
      "Remove-Item -Path $scriptPath, $logPath, $donePath, $exitPath, $pidPath",
    );
  });
...
`,
  ...
  it("scrubs future plugin entries before invoking old same-guest updaters", () => {
    ...
    expect(script).toContain("Invoke-WithScopedEnv @{ BOT_DISABLE_BUNDLED_PLUGINS = '1'");
    expect(macosScript).toContain('BOT_BIN="$(resolve_required_command bot)"');
    expect(macosScript).toContain("/usr/local/bin:/usr/local/sbin");
    expect(macosScript).toContain(
      'BOT_DISABLE_BUNDLED_PLUGINS=1 "$BOT_BIN" update --tag',
    );
    expect(macosScript).not.toContain("/opt/homebrew/bin/bot");
    expect(script).toContain("BOT_DISABLE_BUNDLED_PLUGINS=1 bot update --tag");
    expect(macosScript).toContain(
      'BOT_DISABLE_BUNDLED_PLUGINS=1 "$BOT_BIN" gateway stop',
    );
    expect(script).toContain(
      "BOT_DISABLE_BUNDLED_PLUGINS=1 BOT_ALLOW_ROOT=1 bot gateway stop",
    );
  });
  ...
  it("reenables bundled plugins before Windows post-update verification", () => {
    ...
    const updateIndex = script.indexOf("Invoke-Bot update --tag");
    const scopedIndex = script.indexOf("Invoke-WithScopedEnv @{ BOT_DISABLE_BUNDLED_PLUGINS");
    const versionIndex = script.indexOf("Invoke-Bot --version", scopedIndex);
    const restartIndex = script.indexOf("Invoke-Bot gateway restart");
    const agentIndex = script.indexOf("Invoke-Bot agent --local");
    ...
    expect(script).not.toContain("$env:BOT_DISABLE_BUNDLED_PLUGINS = '1'");
  });
  ...
  it("generates a .NET-safe Windows stale import regex in the update-failure guard", () => {
    ...
    expect(staleImportPattern).toBe(
      String.raw`node_modules\\bot\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`,
    );
    expect(staleImportPattern).not.toContain("node_modules\\bot\\dist\\");
    expect(staleImportPattern.match(/\\\\/g)).toHaveLength(4);
    const representativeUpdateFailure = String.raw`Error [ERR_MODULE_NOT_FOUND]: Cannot find module 'C:\Users\runner\AppData\Roaming\npm\node_modules\bot\dist\main-a1_B2.js' imported from C:\Users\runner\AppData\Roaming\npm\node_modules\bot\dist\cli.js`;
    const generatedRegex = new RegExp(staleImportPattern);
    expect(generatedRegex.test(representativeUpdateFailure)).toBe(true);
    expect(generatedRegex.test(String.raw`node_modules\bot\dist\main.js`)).toBe(false);
  });
});
