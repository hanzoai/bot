...
const CHECK_SCRIPT = resolve("scripts/check-bot-package-tarball.mjs");
...
function withPackedPackage(
  ...
  const root = mkdtempSync(join(tmpdir(), "bot-control-ui-package-inventory-"));
...
function installPackedPackage(root: string, tarball: string) {
  ...
  const result = spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        BOT_HOME: home,
        BOT_STATE_DIR: state,
        BOT_CONFIG_PATH: join(state, "bot.json"),
        TMPDIR: temporary,
        TMP: temporary,
        TEMP: temporary,
      },
  ...
  return join(installRoot, "node_modules", "bot");
}
