...
const RELEASE_CHECKS_PATH = ".github/workflows/bot-release-checks.yml";
const WORKFLOW_PATH = ".github/workflows/bot-release-telegram-qa.yml";
...
function runIdentityVerification(params: {
  ...
  const repository = "hanzoai/bot";
  const trustedWorkflowRef = `${repository}/.github/workflows/bot-release-telegram-qa.yml@refs/heads/main`;
  const invocation = params.invocation ?? "dispatch";
  const workflowRef =
    invocation === "dispatch"
      ? trustedWorkflowRef
      : `${repository}/.github/workflows/bot-release-checks.yml@refs/heads/release-ci/test`;
  const workflowRefName =
    invocation === "dispatch" ? "refs/heads/main" : "refs/heads/release-ci/test";
  const targetSha = "a".repeat(40);
  const workdir = tempDirs.make("bot-telegram-identity-");
...
function runAdvisoryStatus(overrides: Record<string, string> = {}) {
  ...
  const workdir = tempDirs.make("bot-telegram-advisory-status-");
  const githubOutput = join(workdir, "github-output");
...
describe("release Telegram QA workflow", () => {
  ...
  it("dispatches one accepted trusted-main child from release checks", () => {
    ...
    const dispatchers = readdirSync(".github/workflows")
      ...
      .flatMap((name) => {
        ...
        return readFileSync(path, "utf8").includes('workflow="bot-release-telegram-qa.yml"')
          ? [path]
          : [];
      });
    expect(dispatchers).toEqual([RELEASE_CHECKS_PATH]);
    expect(reusableSource).toContain(
      "hanzoai/bot/.github/workflows/bot-release-telegram-qa.yml@refs/heads/main",
    );
    expect(reusableSource).toContain(
      '--cert-identity "https://github.com/hanzoai/bot/.github/workflows/bot-release-telegram-qa.yml@refs/heads/main"',
    );
  ...
  it("bounds the OIDC identity request below the job timeout", () => {
    ...
    const oidcRequest = identityStep.run?.match(
      /curl --fail --silent --show-error[\s\S]*?audience=bot-release-telegram-qa/u,
    )?.[0];
  ...
  it("keeps candidate construction secretless and credentials inside the isolated runner", () => {
    ...
    expect(buildRuntimeStep?.run).toContain("BOT_BUILD_PRIVATE_QA=1");
  ...
  it("keeps the isolated SUT lifetime below the credential lease TTL", () => {
    ...
    expect(createSutStep?.run).toMatch(
      /transport_keys=\([\s\S]*BOT_BUILD_PRIVATE_QA[\s\S]*BOT_ENABLE_PRIVATE_QA_CLI[\s\S]*\)/u,
    );
    expect(runStep?.env?.BOT_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS).toBe("600000");
    expect(runStep?.env?.BOT_QA_CREDENTIAL_LEASE_TTL_MS).toBe("7200000");
    expect(runStep?.env?.BOT_LOG_LEVEL).toBe("trace");
    expect(runStep?.env?.BOT_QA_TELEGRAM_SUT_CLEANUP_TIMEOUT_MS).toBe("60000");
    expect(runStep?.run).toContain("trap terminate_sut_uid_on_exit EXIT");
    expect(runStep?.run).toContain('"$BOT_QA_TELEGRAM_SUT_BOT_COMMAND" --terminate-uid');
    ...
    expect(runStep?.run).not.toContain('pnpm --dir "$CANDIDATE_ROOT" bot qa telegram');
    ...
    expect(captureStep?.run).toContain("-name 'bot-*.log'");
    expect(captureStep?.run).toContain(
      'trusted_temp_root="$(mktemp -d "${RUNNER_TEMP}/bot-telegram-diagnostics.XXXXXX")"',
    );
    ...
    expect(captureStep?.run).not.toContain("-name bot.json");
    expect(
      job?.steps?.findIndex(
        (step) => step.name === "Capture isolated Telegram runtime diagnostics",
      ),
  ...
  it.runIf(process.platform === "linux")("keeps only the newest eight gateway logs", () => {
    ...
    const workdir = tempDirs.make("bot-telegram-log-selector-");
    ...
    const logPaths = Array.from({ length: 12 }, (_, index) => {
      ...
      const logPath = join(logDir, `bot-${index}.log`);
      mkdirSync(logDir);
  ...
  it("retains only allowlisted verbose runtime diagnostics", () => {
    ...
    const workdir = tempDirs.make("bot-telegram-log-filter-");
  ...
  it("derives SUT-writable paths from the verified runtime root after sudo", () => {
    ...
    expect(source).toMatch(
      /if tr "\\0" "\\n" <"\/proc\/\$\{control_pid\}\/environ" 2>\/dev\/null \|\n\s+grep -Fxq "BOT_QA_PROC_CONTROL=visible"; then/u,
    );
    ...
    expect(source).toContain('"$RUNTIME_ROOT"/tmp/bot-qa-suite-*');
    ...
    expect(source).toContain('temp_root="$(realpath -e "${BOT_QA_TEMP_ROOT:?}")"');
    expect(source).toContain("sudo install -d -o root -g root -m 0700 /tmp/bot");
    ...
    expect(source).toContain(
      '-m 0711 \\\n            "${runtime_root}/tmp/bot-${runner_uid}"',
    );
    expect(source).toContain('"$RUNTIME_ROOT"/tmp/bot-"$RUNNER_UID"/bot-qa-suite-*');
    ...
    expect(source).toContain("BOT_LOG_LEVEL");
  ...
  it.runIf(process.platform === "linux")(
    ...
    () => {
      ...
      const workdir = tempDirs.make("bot-telegram-model-proof-");
      const runtimeRoot = join(workdir, "runtime");
      const evidenceRoot = join(workdir, "evidence");
      const configPath = join(workdir, "bot.json");
      const duplicateConfigPath = join(workdir, "bot-duplicate.json");
  ...
  it("reports the persisted inner launcher stage when namespace supervision fails", () => {
    ...
    const workdir = tempDirs.make("bot-telegram-launcher-stage-");
