...
function git(cwd: string, args: string[]): string {
  return execFileSync("git", args, {
    ...
    env: {
      ...
      GIT_AUTHOR_EMAIL: "test@bot.invalid",
      GIT_COMMITTER_NAME: "Bot Test",
      GIT_COMMITTER_EMAIL: "test@bot.invalid",
    },
  }).trim();
}
...
describe("release-note verification", () => {
  ...
  it("stores default GitHub snapshots in the shared Git common directory", () => {
    const commonDir = resolve("/tmp/bot-shared-git");
  ...
  it("reuses exact-range GitHub GraphQL snapshots without caching REST reads", () => {
    const cwd = mkdtempSync(join(tmpdir(), "bot-release-notes-snapshot-"));
    try {
      ...
      expect(
        githubApiWithSnapshot(["repos/hanzoai/bot/releases/tags/v1"], fetchApi, first),
      ).toEqual({
        data: {
          request: ["repos/hanzoai/bot/releases/tags/v1"],
          fetches: 2,
        },
      });
  ...
  it("records a canonical target SHA when --target is symbolic", () => {
    const cwd = mkdtempSync(join(tmpdir(), "bot-release-notes-"));
    try {
      git(cwd, ["init", "-q"]);
      writeFileSync(
