...
async function withGithubApiTimeoutEnv<T>(value: string, fn: () => Promise<T>): Promise<T> {
  const previous = process.env.BOT_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  process.env.BOT_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS = value;
  try {
    return await fn();
  } finally {
    if (previous === undefined) {
      delete process.env.BOT_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
    } else {
      process.env.BOT_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS = previous;
    }
  }
}
...
describe("release candidate checklist", () => {
  it("recognizes direct execution through a symlinked temporary root", () => {
    ...
    expect(
      isDirectReleaseCandidateExecution(
        "/tmp/bot-release-tooling/checkout/scripts/release-candidate-checklist.mjs",
        "/private/tmp/bot-release-tooling/checkout/scripts/release-candidate-checklist.mjs",
        realpath,
      ),
    ).toBe(true);
  ...
  it("passes scoped environment overrides to release child commands", () => {
    const output = run(
      process.execPath,
      ["-e", "process.stdout.write(process.env.BOT_RELEASE_TEST_VALUE ?? '')"],
      { capture: true, env: { BOT_RELEASE_TEST_VALUE: "passed" } },
    );
  ...
  it("keeps the frozen release target separate from clean trusted workflow tooling", () => {
    ...
    expect(source).toContain('mkdtempSync(join(tmpdir(), "bot-release-tooling-"))');
  ...
  it("runs Parallels against the exact prepared candidate tarball", () => {
    expect(candidateParallelsArgs(".artifacts/preflight/bot.tgz", [], "/trusted")).toEqual([
      "exec",
      "tsx",
      "/trusted/scripts/e2e/parallels/npm-update-smoke.ts",
      "--target-tarball",
      ".artifacts/preflight/bot.tgz",
      "--json",
    ]);
    const command = candidateParallelsShellCommand(
      ".artifacts/preflight/bot candidate.tgz",
    ...
    expect(
      candidateParallelsShellCommand(
        ".artifacts/preflight/bot candidate.tgz",
        "/opt/homebrew/bin/gtimeout",
        [".artifacts/preflight/bot-ai candidate.tgz"],
      ),
    ).toContain("'--target-tarball' '.artifacts/preflight/bot candidate.tgz'");
    expect(
      candidateParallelsArgs(
        ".artifacts/preflight/bot.tgz",
        [".artifacts/preflight/bot-ai.tgz"],
        "/trusted",
        [".artifacts/preflight/bot-codex.tgz"],
        "macOS 26.5 Node 24",
      ),
    ).toEqual([
      "exec",
  ...
  it("binds Parallels registry packages to plugin preflight manifests", () => {
    const artifactDir = tempDirs.make("bot-plugin-preflight-");
    const tarballName = "bot-codex-2026.7.1-beta.3.tgz";
    ...
    writeFileSync(
      ...
      `${JSON.stringify({ name: "@hanzo/bot-codex", version: "2026.7.1-beta.3" })}\n`,
    );
    ...
    const manifest = {
      schema: "bot.plugin-publication-artifact/v1",
      schemaVersion: 1,
      targetSha: "candidate-sha",
      package: { name: "@hanzo/bot-codex", version: "2026.7.1-beta.3" },
      artifact: {
    ...
    writeFileSync(
      join(packageDir, "package.json"),
      `${JSON.stringify({ name: "@hanzo/bot-matrix", version: "2026.7.1-beta.3" })}\n`,
    );
    execFileSync("tar", ["-czf", tarballPath, "-C", sourceDir, "package"]);
    rmSync(sourceDir, { force: true, recursive: true });
  ...
  it("requires exact dependency tarball metadata in npm preflight manifests", () => {
    const manifest = {
      ...
      npmDistTag: "beta",
      tarballName: "bot-2026.7.1-beta.3.tgz",
      tarballSha256: "root-sha",
      dependencyTarballs: [
        {
          packageName: "@hanzo/bot-ai",
          packageVersion: "2026.7.1-beta.3",
          tarballName: "bot-ai-2026.7.1-beta.3.tgz",
  ...
  it("prefers the complete core package tarball set with legacy manifest fallback", () => {
    ...
    const gatewayProtocolTarball = {
      packageName: "@hanzo/bot-gateway-protocol",
      packageVersion: "2026.7.1-beta.3",
      tarballName: "bot-gateway-protocol-2026.7.1-beta.3.tgz",
  ...
  it("passes only root dependency tarballs to Parallels with legacy fallback", () => {
    ...
    const gatewayClientTarball = {
      packageName: "@hanzo/bot-gateway-client",
      packageVersion: "2026.7.1-beta.3",
      tarballName: "bot-gateway-client-2026.7.1-beta.3.tgz",
  ...
  it("rejects duplicate release candidate CLI options", () => {
    ...
    const duplicateCases = [
      ...
      duplicateOption("--repo", "hanzoai/bot", "fork/bot"),
  ...
  it("builds the gated release publish command from green evidence inputs", () => {
    ...
    const workflow = parse(
      readFileSync(".github/workflows/bot-release-publish.yml", "utf8"),
    ) as {
  ...
  it("requires and carries an exact Windows Node tag for stable release candidates", () => {
    ...
    const options = {
      ...
      windowsNodeInstallerDigests: JSON.stringify({
        "BotCompanion-Setup-x64.exe": `sha256:${"a".repeat(64)}`,
        "BotCompanion-Setup-arm64.exe": `sha256:${"b".repeat(64)}`,
      }),
    };

    expect(buildPublishCommand(options)).toContain("'windows_node_tag=v0.6.3'");
    expect(buildPublishCommand(options)).toContain(
      `'windows_node_installer_digests={"BotCompanion-Setup-x64.exe":"sha256:${"a".repeat(64)}","BotCompanion-Setup-arm64.exe":"sha256:${"b".repeat(64)}"}'`,
    );
  });
  ...
  it("validates the stable Windows source release and immutable installer digests", async () => {
    const assets = [
      {
        name: "BotCompanion-Setup-x64.exe",
        digest: `sha256:${"a".repeat(64)}`,
      },
      {
        name: "BotCompanion-Setup-arm64.exe",
    ...
    const fetchImpl = vi.fn(async () => {
      return jsonResponse({
        ...
        draft: false,
        prerelease: false,
        html_url: "https://github.com/hanzoai/bot-windows-node/releases/tag/v0.6.3",
        assets,
      });
    });
    ...
    await expect(
      ...
      tag: "v0.6.3",
      url: "https://github.com/hanzoai/bot-windows-node/releases/tag/v0.6.3",
      assets,
    });
  });
  ...
  it.each([
    ...
    [
      { assets: [] },
      "must contain exactly one required asset BotCompanion-Setup-x64.exe; found 0",
    ],
    [
      {
        assets: [
      ...
      "must contain exactly one required asset BotCompanion-Setup-x64.exe; found 2",
    ],
    [
      {
        assets: [
          { name: "BotCompanion-Setup-x64.exe", digest: "" },
          { name: "BotCompanion-Setup-arm64.exe", digest: `sha256:${"b".repeat(64)}` },
        ],
      },
      "asset BotCompanion-Setup-x64.exe is missing its SHA-256 digest",
    ],
  ...
  it("rejects selected plugin publish scope for release candidates", () => {
    ...
    ).toThrow("release candidates publish Bot with --plugin-publish-scope all-publishable");
  });

  it("extracts a workflow run id from gh dispatch output", () => {
    expect(
      parseRunIdFromDispatchOutput(
        "https://github.com/hanzoai/bot/actions/runs/25922042055\n",
      ),
    ).toBe("25922042055");
  });
  ...
  it("falls back to a single compatible artifact from the same run", () => {
    expect(
      resolveArtifactName(
        [{ name: "bot-npm-preflight-dba00", expired: false }],
        "bot-npm-preflight-v2026.5.16-beta.2",
        "bot-npm-preflight-",
      ),
    ).toBe("bot-npm-preflight-dba00");
  });
    ...
    expect(
      buildTelegramArtifactInputs({
        artifact: {
          digest: `sha256:${"a".repeat(64)}`,
          id: 123,
          name: "bot-npm-preflight-v2026.7.2-beta.1",
          workflowRunId: 456,
        },
        manifest: {
          packageVersion: "2026.7.2-beta.1",
          tarballName: "bot-2026.7.2-beta.1.tgz",
      ...
      package_artifact_name: "bot-npm-preflight-v2026.7.2-beta.1",
      package_artifact_run_attempt: 2,
      package_artifact_run_id: "456",
      package_file_name: "bot-2026.7.2-beta.1.tgz",
  ...
  it("bounds GitHub API requests with a timeout signal", async () => {
    ...
    await expect(
      githubApi("repos/hanzoai/bot/actions/runs", {
        fetchImpl,
        timeoutMs: 1234,
        token: "test-token",
      }),
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "https://api.github.com/repos/hanzoai/bot/actions/runs",
    ...
    async (raw) => {
      ...
      await withGithubApiTimeoutEnv(raw, async () => {
        await expect(
          ...
          "BOT_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive integer",
        );
      });
  ...
  it("bounds GitHub API error bodies", async () => {
    ...
    await expect(
      ...
      "GitHub API repos/hanzoai/bot/actions/runs response body exceeded 64 bytes",
    );
  });
  ...
  it("keeps GitHub API timeouts active while reading response bodies", async () => {
    ...
    ).rejects.toThrow("GitHub API repos/hanzoai/bot/actions/runs timed out after 25ms");
  });
  ...
  it("includes the GitHub API path when a request times out", async () => {
    ...
    await expect(
      githubApi("repos/hanzoai/bot/actions/runs/123/jobs", {
        fetchImpl,
        timeoutMs: 5,
        token: "test-token",
      }),
    ).rejects.toThrow(
      "GitHub API repos/hanzoai/bot/actions/runs/123/jobs timed out after 5ms",
    );
  });
});
...
describe("GitHub API public fallback", () => {
  it.each([403, 429])(
    ...
    async (status) => {
      ...
      await expect(
        githubApi("repos/hanzoai/bot/actions/runs/123", {
          token: "x",
          fetchImpl,
        }),
