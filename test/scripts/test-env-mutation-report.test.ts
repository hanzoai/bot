...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("bot-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.BOT_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.BOT_STATE_DIR = "state";
delete process.env["BOT_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("BOT_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, BOT_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "BOT_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.BOT_HOME = "not code"';
process.env.NOT_BOT = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.BOT_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.BOT_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
...
describe("collectTestEnvMutationReport", () => {
  it("reports active Bot env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
      ...
      { file: "src/example.test.ts", key: "BOT_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "BOT_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "BOT_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "BOT_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("Bot test env mutation report");
    ...
    expect(rendered).toContain("L2 BOT_STATE_DIR assign process.env");
