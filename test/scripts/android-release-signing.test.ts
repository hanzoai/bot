...
function makeTempRoot(): string {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "bot-android-signing-"));
...
function runGit(args: string[], cwd?: string, env: NodeJS.ProcessEnv = {}) {
  execFileSync("git", args, {
    ...
    env: {
      ...process.env,
      GIT_AUTHOR_NAME: "Bot Test",
      GIT_AUTHOR_EMAIL: "test@example.com",
      GIT_COMMITTER_NAME: "Bot Test",
    ...
    `${JSON.stringify(
      {
        ...
        gradlePropertyNames: [
          "BOT_ANDROID_STORE_FILE",
          "BOT_ANDROID_STORE_PASSWORD",
          "BOT_ANDROID_KEY_ALIAS",
          "BOT_ANDROID_KEY_PASSWORD",
        ],
      },
      null,
      2,
...
function writeSigningSources(tempRoot: string) {
  ...
  fs.writeFileSync(
    propertiesPath,
    [
      `BOT_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`,
      "BOT_ANDROID_KEY_ALIAS=bot-upload",
      `BOT_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`,
      "",
    ].join("\n"),
  );
...
describe("scripts/android-release-signing.mjs", () => {
  ...
  it("documents the canonical Android release signing plan", () => {
    ...
    expect(result.stdout).toContain("Signing repo: git@github.com:bot/apps-signing.git");
    expect(result.stdout).toContain("Signing assets: android/bot");
  ...
  it.runIf(commandAvailable("openssl"))(
    ...
    () => {
      ...
      const encryptedProperties = fs.readFileSync(
        path.join(remoteCheck, "android", "bot", "gradle.properties.enc"),
        "utf8",
      );
      ...
      expect(materializedProperties).toContain(
        `BOT_ANDROID_STORE_FILE=${path.join(materializedDir, "upload-keystore.jks")}`,
      );
      expect(materializedProperties).toContain(`BOT_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`);
      expect(materializedProperties).toContain("BOT_ANDROID_KEY_ALIAS=bot-upload");
      expect(materializedProperties).toContain(`BOT_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`);
