...
describe("install-cli.sh", () => {
  ...
  it("does not restart a gateway again after force-install activates it", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-gateway-refresh-"));
    const prefix = join(tmp, "prefix");
    const bin = join(prefix, "bin");
    const commandLog = join(tmp, "commands.log");
    const bot = join(bin, "bot");
    mkdirSync(bin, { recursive: true });
    writeFileSync(bot, '#!/bin/bash\nprintf "%s\\n" "$*" >> "$COMMAND_LOG"\n');
    chmodSync(bot, 0o755);
  ...
  it("keeps HOME for default prefix while BOT_HOME controls git checkout paths", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-home-"));
    ...
    expect(output).toContain(`prefix=${join(osHome, ".bot")}`);
  ...
  it("uses the repo Corepack pnpm when a global pnpm version is already present", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-pnpm-version-"));
  ...
  it("links an existing usable Alpine/musl Node runtime without sudo", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-alpine-"));
    const bin = join(tmp, "bin");
    const prefix = join(tmp, "prefix");
  ...
  it("replaces a stale Alpine/musl prefix Node before the generic skip", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-alpine-stale-"));
    const bin = join(tmp, "bin");
  ...
  it("uses apk-managed Node and Git on Alpine/musl when the existing Node is unusable", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-alpine-apk-"));
    const bin = join(tmp, "bin");
    const prefix = join(tmp, "prefix");
    const apkLog = join(tmp, "apk.log");
  ...
  it("skips PATH Node runtimes whose npm command cannot start", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-broken-npm-"));
  ...
  it("rejects Alpine/musl Node packages below the requested runtime floor", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-alpine-old-node-"));
    const bin = join(tmp, "bin");
    const prefix = join(tmp, "prefix");
    const apkLog = join(tmp, "apk.log");
    const fakeApk = join(bin, "apk");
  ...
  it("replaces cached generic Node runtimes below the runtime floor", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-generic-stale-node-"));
    const prefix = join(tmp, "prefix");
    const nodePrefixBin = join(prefix, "tools", "node-v22.22.3", "bin");
    const staleNode = join(nodePrefixBin, "node");
  ...
  it("rejects downloaded generic Node runtimes below the runtime floor", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-generic-old-node-"));
    const prefix = join(tmp, "prefix");
    const newNode = join(tmp, "new-node");
    const newNpm = join(tmp, "new-npm");
  ...
  it("removes the Node staging directory when download fails", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-node-cleanup-"));
    const prefix = join(tmp, "prefix");
  ...
  it("removes the workspace rewrite temp file when rewriting fails", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-workspace-cleanup-"));
    const repo = join(tmp, "repo");
  ...
  it("does not emit --before when raw user npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-npmrc-"));
    const bin = join(tmp, "bin");
  ...
  it.each([
    ...
    const tmp = mkdtempSync(join(tmpdir(), `bot-install-cli-${source}-npmrc-`));
    const bin = join(tmp, "bin");
  ...
  it("does not emit before args when npmrc min-release-age computes a before cutoff", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-freshness-"));
    const prefix = join(tmp, "prefix");
    const home = join(tmp, "home");
    ...
    try {
      result = runInstallCliShell(
        [
          ...
          `BOT_PREFIX=${JSON.stringify(prefix)}`,
          "BOT_VERSION=2026.5.19",
          `source ${JSON.stringify(SCRIPT_PATH)}`,
          "ensure_git() { return 0; }",
          "install_bot",
        ].join("\n"),
  ...
  it("ignores project npmrc when choosing global install freshness args", () => {
    const tmp = mkdtempSync(join(tmpdir(), "bot-install-cli-global-freshness-"));
    const prefix = join(tmp, "prefix");
    const home = join(tmp, "home");
