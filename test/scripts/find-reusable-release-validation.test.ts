...
function createRepo(options: { plistBuildVersion?: string } = {}, dirs = tempDirs) {
  ...
  mkdirSync(join(origin, "apps/macos/Sources/Bot/Resources"), { recursive: true });
  writeFileSync(
    join(origin, "apps/macos/Sources/Bot/Resources/Info.plist"),
...
function normalizedEvidence(options: {
  ...
  const roles = [
    ...
    [
      ...
      2,
      "Bot Release Checks",
      "bot-release-checks.yml",
      "-release-checks",
    ],
    ["productPerformance", "204", 3, 2, "Bot Performance", "bot-performance.yml", ""],
  ] as const;
...
if (
  ...
  repoIndex < 0 ||
  trustedRefIndex < 0 ||
  process.argv[repoIndex + 1] !== "hanzoai/bot" ||
...
function setUpFixtures(runs: RunFixture[]): {
  ...
  writeFileSync(
    fixtureName(
      fixtures,
      "repos/hanzoai/bot/actions/workflows/full-release-validation.yml/runs",
    ),
  ...
  return spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        GITHUB_OUTPUT: "",
        BOT_RELEASE_CI_SUMMARY_VALIDATOR: args.validatorPath,
