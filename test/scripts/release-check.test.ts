...
describe("release-check", () => {
  ...
  it("resolves prepacked publishable core package tarballs", () => {
    const root = mkdtempSync(join(tmpdir(), "bot-release-check-tarball-test-"));
    try {
      writeFileSync(join(root, "bot-ai-2026.6.33.tgz"), "fixture");
      writeFileSync(join(root, "bot-gateway-client-2026.6.33.tgz"), "fixture");
      writeFileSync(join(root, "bot-gateway-protocol-2026.6.33.tgz"), "fixture");
      writeFileSync(join(root, "SHA256SUMS"), "fixture");
      expect(resolveReleaseCheckLocalPackageTarballs(root)).toEqual([
        join(root, "bot-ai-2026.6.33.tgz"),
        join(root, "bot-gateway-client-2026.6.33.tgz"),
        join(root, "bot-gateway-protocol-2026.6.33.tgz"),
      ]);
  ...
  it("accepts gateway core packages when the root does not require AI", () => {
    const root = mkdtempSync(join(tmpdir(), "bot-release-check-tarball-test-"));
    try {
      const gatewayTarball = join(root, "bot-gateway-protocol-2026.7.2.tgz");
      const gatewayClientTarball = join(root, "bot-gateway-client-2026.7.2.tgz");
  ...
  it("writes an explicit local project for unpublished core package tarballs", () => {
    const root = mkdtempSync(join(tmpdir(), "bot-release-check-install-test-"));
    try {
      writePackedTarballInstallManifest(root, "/tmp/bot.tgz", [
        "/tmp/bot-ai.tgz",
        "/tmp/bot-gateway-client.tgz",
        "/tmp/bot-gateway-protocol.tgz",
      ]);
      ...
      expect(manifest.dependencies).toEqual({
        ...
        "@hanzo/bot-gateway-client": "file:///tmp/bot-gateway-client.tgz",
        "@hanzo/bot-gateway-protocol": "file:///tmp/bot-gateway-protocol.tgz",
  ...
  it("writes a gateway-packages-only local project when the root does not require AI", () => {
    ...
    try {
      writePackedTarballInstallManifest(
        root,
        "/tmp/bot.tgz",
        ["/tmp/bot-gateway-client.tgz", "/tmp/bot-gateway-protocol.tgz"],
        false,
      );
      const manifest = JSON.parse(readFileSync(join(root, "package.json"), "utf8")) as {
        dependencies?: Record<string, string>;
  ...
  it("packs the local AI workspace when no prepared tarball is supplied", () => {
    const root = mkdtempSync(join(tmpdir(), "bot-release-check-ai-pack-test-"));
    try {
      const tarballs = prepareReleaseCheckLocalPackageTarballs({
        tmpRoot: root,
        packLocalAi: (packDestination) => {
          const filename = "bot-ai-2026.7.1-beta.3.tgz";
          writeFileSync(join(packDestination, filename), "fixture");
          return [{ filename }];
        },
      });
      expect(tarballs).toEqual([join(root, "ai-pack", "bot-ai-2026.7.1-beta.3.tgz")]);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("prefers prepared core package tarballs over packing the AI workspace", () => {
    ...
    try {
      ...
      const preparedTarball = join(preparedDir, "bot-ai-2026.7.1-beta.3.tgz");
      const gatewayProtocolTarball = join(
        preparedDir,
        "bot-gateway-protocol-2026.7.1-beta.3.tgz",
      );
      const gatewayClientTarball = join(preparedDir, "bot-gateway-client-2026.7.1-beta.3.tgz");
  ...
  it("rejects a packed install without the local AI tarball", () => {
    const root = mkdtempSync(join(tmpdir(), "bot-release-check-install-test-"));
    try {
      expect(() => writePackedTarballInstallManifest(root, "/tmp/bot.tgz", [])).toThrow(
        "requires exactly one @hanzo/bot-ai tarball",
      );
      expect(() =>
        writePackedTarballInstallManifest(root, "/tmp/bot.tgz", [
          "/tmp/bot-ai-one.tgz",
      ...
      ).toThrow("requires exactly one @hanzo/bot-ai tarball");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("rejects missing, incomplete, or ambiguous local package tarball directories", () => {
    ...
    try {
      ...
      expect(() => resolveReleaseCheckLocalPackageTarballs(empty)).toThrow(
        "must contain exactly one @hanzo/bot-ai tarball",
      );
  ...
  it("seeds packaged activation smoke with an included channel plugin", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "bot-release-check-test-"));
    try {
      writePackedBundledPluginActivationConfig(homeDir);
      const config = JSON.parse(
        readFileSync(join(homeDir, ".bot", "bot.json"), "utf8"),
      ) as {
