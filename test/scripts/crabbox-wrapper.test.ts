...
function makeFakeCrabbox(helpText: string): string {
  ...
  const binDir = mkdtempSync(path.join(tmpdir(), "bot-fake-crabbox-"));
...
function writeFakeCrabbox(binDir: string, helpText: string): string {
  ...
  if (process.platform !== "win32") {
    ...
    const script = [
      "#!/bin/sh",
      'if [ "$1" = "--version" ]; then',
      '  printf "%s\\n" "${BOT_FAKE_CRABBOX_VERSION:-crabbox 0.22.1}"',
      "  exit 0",
      "fi",
      ...
      '  if [ ! -f .bot-crabbox-changed-gate.bundle ] || ! cmp -s .bot-crabbox-changed-gate.bundle "$expected_bundle"; then',
      ...
      '  actual_bundle_bytes="$(wc -c < .bot-crabbox-changed-gate.bundle 2>/dev/null | tr -d " ")"',
  ...
  const helperScript = [
    ...
    "  const bundlePath = '.bot-crabbox-changed-gate.bundle';",
  ...
  const script = [
    ...
    "const args = process.argv.slice(2);",
    'if (args[0] === "--version") {',
    "  console.log(process.env.BOT_FAKE_CRABBOX_VERSION || 'crabbox 0.22.1');",
    "  process.exit(0);",
    "}",
...
function makeSlowVersionCrabbox(helpText: string): string {
  const binDir = mkdtempSync(path.join(tmpdir(), "bot-slow-crabbox-"));
  tempDirs.push(binDir);
  const crabboxPath = path.join(binDir, "crabbox");
...
function makeSlowHelpCrabbox(helpText: string, delayMs: number): string {
  const binDir = mkdtempSync(path.join(tmpdir(), "bot-slow-help-crabbox-"));
  tempDirs.push(binDir);
  const crabboxPath = path.join(binDir, "crabbox");
...
function testTimingPreload(options: { clockScale?: number; spawnTimeoutMs?: number }): string {
  ...
  if (!preloadPath) {
    const dir = mkdtempSync(path.join(tmpdir(), "bot-crabbox-timing-"));
    tempDirs.push(dir);
...
function makeFakeGit(
  ...
  if (process.platform !== "win32") {
    const script = [
      ...
      '    ln -s "$BOT_FAKE_GIT_CHANGED_GATE_BUNDLE_SYMLINK_TARGET" "$4/.bot-crabbox-changed-gate.bundle"',
      "  fi",
      "  exit 0",
      "fi",
  ...
  const script = [
    ...
    "const args = process.argv.slice(2);",
    "if (args[0] === 'worktree' && args[1] === 'add') { fs.mkdirSync(args[3], { recursive: true }); if (process.env.BOT_FAKE_GIT_CHANGED_GATE_BUNDLE_SYMLINK_TARGET) fs.symlinkSync(process.env.BOT_FAKE_GIT_CHANGED_GATE_BUNDLE_SYMLINK_TARGET, require('node:path').join(args[3], '.bot-crabbox-changed-gate.bundle')); process.exit(0); }",
...
async function runSignalCleanupProof(sendSignals: (pid: number) => Promise<void>): Promise<void> {
  const root = mkdtempSync(path.join(tmpdir(), "bot-crabbox-descendant-"));
  tempDirs.push(root);
...
function expectChangedGateGitBootstrap(remoteCommand: string): void {
  ...
  expect(remoteCommand).toContain(
    "bot_changed_gate_bundle=.bot-crabbox-changed-gate.bundle",
  );
...
describe("scripts/crabbox-wrapper", () => {
  ...
  it("requires a current Crabbox binary for Blacksmith Testbox runs", () => {
    const result = runDefaultWrapper(["run", "--provider", "blacksmith-testbox", "--", "echo ok"], {
      env: { BOT_FAKE_CRABBOX_VERSION: "crabbox 0.21.9" },
    });
  ...
  it("rejects prerelease Crabbox builds at the Blacksmith minimum boundary", () => {
    const result = runDefaultWrapper(["run", "--provider", "blacksmith-testbox", "--", "echo ok"], {
      env: { BOT_FAKE_CRABBOX_VERSION: "crabbox 0.22.0-rc.1" },
    });
  ...
  it("rejects unsafe Crabbox version numbers at the Blacksmith minimum gate", () => {
    const result = runDefaultWrapper(["run", "--provider", "blacksmith-testbox", "--", "echo ok"], {
      env: { BOT_FAKE_CRABBOX_VERSION: "crabbox 0.9007199254740993.0" },
    });
  ...
  it("accepts post-release Crabbox describe builds at the Blacksmith minimum boundary", () => {
    const result = runDefaultWrapper(["run", "--provider", "blacksmith-testbox", "--", "echo ok"], {
      env: { BOT_FAKE_CRABBOX_VERSION: "crabbox 0.22.0-3-gabc1234" },
    });
  ...
  it("rejects reused Blacksmith Testboxes that were not created by Crabbox", () => {
    const home = mkdtempSync(path.join(tmpdir(), "bot-crabbox-home-"));
    tempDirs.push(home);
  ...
  it("only forces the short local-container Docker work root on Linux", () => {
    ...
    const expectedMessage =
      "[crabbox] provider=docker using short host-visible work root for Bot Docker tests";
  ...
  it("prefers Azure for unqualified Windows runs", () => {
    ...
    expect(output.args.join(" ")).not.toContain("bot_crabbox_bootstrap_wsl2_js");
    expect(remoteCommand).toContain("bot_crabbox_bootstrap_wsl2_js");
    ...
    expect(remoteCommand).toContain("bot_crabbox_bootstrap_wsl2_js || exit $?");
    expect(remoteCommand).toContain(
      `{ bot_crabbox_env ${remoteChangedGateEnvPrefix} corepack pnpm check:changed\n}`,
    );
  ...
  it("fails closed for AWS proof when broker auth is missing", () => {
    ...
    expect(result.stderr).toContain(
      "crabbox login --url https://crabbox.bot.ai --provider aws",
    );
  });

  it("fails closed for AWS proof when broker auth is stale", () => {
    const result = runDefaultWrapper(["run", "--provider", "aws", "--", "echo ok"], {
      configJson: { coordinator: "https://crabbox.bot.ai", brokerAuth: "configured" },
  ...
  it("bootstraps only Node for raw AWS macOS node commands", () => {
    ...
    expect(remoteCommand).toContain("bot_crabbox_bootstrap_macos_js");
  ...
  it("preflights Swift 6.2 for raw AWS macOS Swift app builds", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.args).toContain("--shell");
    expect(remoteCommand).toContain("bot_crabbox_require_macos_swift_62");
    ...
    expect(remoteCommand).not.toContain("bot_crabbox_bootstrap_macos_js");
  ...
  it("preserves sanitized env pnpm package commands when Swift preflight is needed", () => {
    ...
    expect(remoteCommand).toContain("bot_crabbox_bootstrap_macos_js");
    expect(remoteCommand).toContain("bot_crabbox_require_macos_swift_62");
    expectGroupedShellCommand(remoteCommand, "bot_crabbox_env -i pnpm mac:package");
  });
  ...
  it("preserves sanitized env package script commands when JS tooling is needed", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      "bot_crabbox_env -i bash scripts/package-mac-app.sh",
    );
  });
  ...
  it("preserves shell syntax when sanitizing env package scripts", () => {
    for (const [shellCommand, expectedCommand] of [
      [
        ...
        "bot_crabbox_env -i PATH=$PATH bash scripts/package-mac-app.sh > out.log",
      ],
      [
        "env -i bash scripts/package-mac-app.sh >out.log 2>&1",
        "bot_crabbox_env -i bash scripts/package-mac-app.sh >out.log 2>&1",
      ],
    ] as const) {
      const result = runDefaultWrapper([
        "run",
  ...
  it("preserves trailing shell segments when sanitizing env package scripts", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      "bot_crabbox_env -i bash scripts/package-mac-app.sh && echo done",
    );
  });
  ...
  it("preserves prefixed shell segments when sanitizing env package scripts", () => {
    for (const [shellCommand, expectedCommand] of [
      [
        ...
        "set -e; bot_crabbox_env -i bash scripts/package-mac-app.sh",
      ],
      [
        "time env -i bash scripts/package-mac-app.sh",
        "time bot_crabbox_env -i bash scripts/package-mac-app.sh",
      ],
    ] as const) {
      const result = runDefaultWrapper([
        "run",
  ...
  it("preserves control-flow shell segments when sanitizing env package scripts", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      "if true; then bot_crabbox_env -i bash scripts/package-mac-app.sh; fi",
    );
  });
  ...
  it("preserves assignment prefixes when sanitizing env package scripts", () => {
    for (const [shellCommand, expectedCommand] of [
      [
        ...
        "FOO=1 bot_crabbox_env -i bash scripts/package-mac-app.sh",
      ],
      [
        "FOO= env -i bash scripts/package-mac-app.sh",
        "FOO= bot_crabbox_env -i bash scripts/package-mac-app.sh",
      ],
      [
        "FOO='a b' env -i bash scripts/package-mac-app.sh",
        "FOO='a b' bot_crabbox_env -i bash scripts/package-mac-app.sh",
      ],
      [
        "PATH=/usr/bin:/bin env -i bash scripts/package-mac-app.sh",
        "PATH=/usr/bin:/bin bot_crabbox_env -i bash scripts/package-mac-app.sh",
      ],
    ] as const) {
      const result = runDefaultWrapper([
        "run",
  ...
  it("bootstraps Bun for raw AWS macOS env-prefixed bun commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("bun --version >&2 || return 1");
    expectGroupedShellCommand(remoteCommand, "bot_crabbox_env -i bun --version");
  });
  ...
  it("bootstraps Corepack for raw AWS macOS pnpm commands", () => {
    ...
    expect(remoteCommand).toContain('ready_marker="$node_dir/.bot-crabbox-node-ready"');
    expect(remoteCommand).toContain(
  ...
  it.each([
    {
      ...
      expectShell: true,
      expectedCommand: "bot_crabbox_env pnpm --version",
    ...
    },
    {
      ...
      excludes: ["export -f env bot_crabbox_env", 'env() { bot_crabbox_env "$@"; };'],
      expectedCommand: "bot_crabbox_env -i PATH=/usr/bin:/bin pnpm --version",
      includes: [
        "bot_crabbox_env",
    ...
    },
    {
      ...
      expectedCommand: "bot_crabbox_env -u FOO -i PATH=/usr/bin:/bin pnpm --version",
    ...
    },
    {
      ...
      expectedCommand: "bot_crabbox_env -i pnpm --version",
      includes: [
    ...
    for (const snippet of [
      "bot_crabbox_bootstrap_macos_js",
      "pnpm --version >&2",
      ...includes,
    ]) {
  ...
  it("does not shadow unrelated env calls in AWS macOS shell commands", () => {
    ...
    expect(result.status).toBe(0);
    expect(remoteCommand).toContain("bot_crabbox_bootstrap_macos_js");
    expect(remoteCommand).toContain("bot_crabbox_env");
    expect(remoteCommand).not.toContain('env() { bot_crabbox_env "$@"; };');
  ...
  it("bootstraps Corepack for raw AWS macOS env split-string pnpm commands", () => {
    ...
    expectGroupedShellCommand(remoteCommand, "bot_crabbox_env -S 'pnpm --version'");
  });
  ...
  it("bootstraps Corepack for AWS macOS node changed-gate commands", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `bot_crabbox_env ${remoteChangedGateEnvPrefix} node scripts/check-changed.mjs`,
    );
  });
  ...
  it("bootstraps Corepack for AWS macOS node option changed-gate commands", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `bot_crabbox_env ${remoteChangedGateEnvPrefix} node --max-old-space-size 4096 --env-file-if-exists .env --unhandled-rejections strict --trace-warnings --import=tsx scripts/check-changed.mjs`,
    );
  });
  ...
  it("bootstraps AWS macOS script-stdin runs before the uploaded script body", () => {
    ...
    expect(output.scriptContent).toContain("bot_crabbox_bootstrap_macos_js");
    expect(output.scriptContent).toContain('if [ ! -d "$TMPDIR" ]; then mkdir -p "$TMPDIR"');
    expect(output.scriptContent).toContain("bot_crabbox_bootstrap_macos_js || exit $?");
  ...
  it("preflights Swift for AWS macOS script-stdin Swift builds", () => {
    ...
    expect(result.status).toBe(0);
    expect(output.scriptContent).toContain("bot_crabbox_bootstrap_macos_js");
    expect(output.scriptContent).toContain("bot_crabbox_require_macos_swift_62");
    expect(output.scriptContent).toContain("bot_crabbox_require_macos_swift_62 || exit $?");
  ...
  if (process.platform !== "win32") {
    it("keeps POSIX PATH lookup semantics for non-executable entries", () => {
      const staleBinDir = mkdtempSync(path.join(tmpdir(), "bot-stale-crabbox-"));
  ...
  it("uses a temporary full checkout for clean sparse Blacksmith syncs", () => {
    ...
    expect(parseFakeCrabboxOutput(result).cwd).toContain("bot-crabbox-sync-");
  });
  ...
  it("bootstraps Git metadata for non-sparse changed gates on remote raw syncs", () => {
    ...
    expect(result.stderr).toContain("syncing from temporary full checkout");
    expect(result.stderr).toContain("overlaying local HEAD as worktree changes from abc123");
    expect(output.cwd).toContain("bot-crabbox-sync-");
    expect(output.args).toContain("--shell");
  ...
  it("preserves macOS JS bootstrapping for sparse changed gates on remote raw syncs", () => {
    ...
    expectGroupedShellCommand(
      remoteCommand,
      `bot_crabbox_env ${remoteChangedGateEnvPrefix} pnpm check:changed`,
    );
  });
  ...
  it("creates sparse-sync temporary full checkouts under the durable cache root", () => {
    ...
    try {
      ...
      const output = parseFakeCrabboxOutput(result);
      expect(result.status).toBe(0);
      expect(output.cwd).toContain(`${syncRoot}${path.sep}bot-crabbox-sync-`);
