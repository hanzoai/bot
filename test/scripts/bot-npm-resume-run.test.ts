...
import {
  resolveBotNpmResumeRun,
  runBotNpmResumeGh,
  validateBotNpmResumeRun,
} from "../../scripts/bot-npm-resume-run.mjs";
import type { BotNpmResumeValidationInput } from "../../scripts/bot-npm-resume-run.mjs";

const SHA = "a".repeat(40);
const TAG_OBJECT_SHA = "b".repeat(40);
const BRANCH = `release-publish/${SHA.slice(0, 12)}-123`;
const URL = "https://github.com/hanzoai/bot/actions/runs/456";

function fixture(
  overrides: Partial<BotNpmResumeValidationInput> = {},
): BotNpmResumeValidationInput {
  return {
    ...
    run: {
      ...
      head_sha: SHA,
      html_url: URL,
      path: `.github/workflows/bot-npm-release.yml@refs/tags/${BRANCH}`,
      workflow_id: 101,
    },
    tag: {
...
describe("bot npm resume run identity", () => {
  it("bounds each GitHub lookup", () => {
    const execFileSyncImpl = vi.fn(() => "result");

    expect(
      runBotNpmResumeGh(["api", "repos/hanzoai/bot/actions/runs/456"], {
        execFileSyncImpl,
      }),
    ).toBe("result");
    expect(execFileSyncImpl).toHaveBeenCalledWith(
      "gh",
      ["api", "repos/hanzoai/bot/actions/runs/456"],
      {
        encoding: "utf8",
        killSignal: "SIGKILL",
  ...
  it("accepts a successful run bound to a signed main-reachable tooling tag", () => {
    expect(validateBotNpmResumeRun(fixture())).toEqual({
  ...
  it.each([
    ...
    expect(() => validateBotNpmResumeRun(fixture(overrides))).toThrow(message);
  });

  it("loads the exact run, workflow, signed tag, ancestry, and approval job", () => {
    const responses = new Map<string, unknown>([
      [`api repos/hanzoai/bot/actions/runs/456 --method GET`, fixture().run],
      [
        `api repos/hanzoai/bot/actions/workflows/bot-npm-release.yml --method GET`,
        { id: 101 },
      ],
      [`api repos/hanzoai/bot/git/ref/tags/${BRANCH} --method GET`, fixture().tagRef],
      [`api repos/hanzoai/bot/git/tags/${TAG_OBJECT_SHA} --method GET`, fixture().tag],
      [`api repos/hanzoai/bot/compare/${SHA}...main --method GET`, { status: "identical" }],
      [`run view 456 --repo hanzoai/bot --json jobs --jq .jobs`, fixture().jobs],
    ]);
    ...
    expect(
      resolveBotNpmResumeRun({ repo: "hanzoai/bot", runGh, runId: "456" }),
