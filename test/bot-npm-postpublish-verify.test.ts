...
// Bot npm postpublish tests validate postpublish verification behavior.
...
import {
  ...
  botNpmPostpublishVerifyUsage,
  parseBotNpmPostpublishVerifyArgs,
...
describe("parseBotNpmPostpublishVerifyArgs", () => {
  it("keeps trusted release verification independent from target app dependencies", () => {
    const source = readFileSync("scripts/bot-npm-postpublish-verify.ts", "utf8");
  ...
  it("supports help and package-manager separators", () => {
    expect(parseBotNpmPostpublishVerifyArgs(["--help"])).toEqual({
      help: true,
      version: "",
    });
    expect(parseBotNpmPostpublishVerifyArgs(["--", "2026.3.23"])).toEqual({
      help: false,
      version: "2026.3.23",
    });
  });
  ...
  it("rejects missing, option-like, and extra arguments before verification", () => {
    expect(() => parseBotNpmPostpublishVerifyArgs([])).toThrow(
      botNpmPostpublishVerifyUsage(),
    );
    expect(() => parseBotNpmPostpublishVerifyArgs(["--tag"])).toThrow(
      "Unknown bot npm postpublish verifier option: --tag",
    );
    expect(() => parseBotNpmPostpublishVerifyArgs(["2026.3.23", "extra"])).toThrow(
      "Unexpected bot npm postpublish verifier argument: extra",
    );
  });
});
...
describe("buildPublishedInstallScenarios", () => {
  ...
  it("adds a stable-to-correction upgrade scenario for correction releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23-2")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["bot@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
      {
        name: "upgrade-from-base-stable",
        installSpecs: ["bot@2026.3.23", "bot@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
    ]);
  });
...
describe("npm registry provenance verification", () => {
      ...
      buildDefinition: {
        externalParameters: {
          workflow: {
            ...
            path: ".github/workflows/bot-npm-release.yml",
            ref: workflowRef,
          },
        },
      },
  ...
  it("fetches npm registry JSON with bounded response handling", async () => {
    ...
    await expect(
      fetchRegistryJson("https://registry.example/bot", {
  ...
  it("bounds oversized npm registry response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry https://registry.example/bot response body exceeded 64 bytes",
    );
  });
  ...
  it("keeps npm registry timeouts active while reading response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry request timed out after 5ms: https://registry.example/bot",
    );
  });
  ...
  it("requires a trusted GitHub release identity for the exact SLSA provenance attestation", async () => {
    ...
    expect(verificationPolicy).toEqual({
      ...
      certificateIdentityURI:
        "https://github.com/hanzoai/bot/.github/workflows/bot-npm-release.yml@refs/heads/release/2026.3.23",
    });
                    ...
                    predicate: {
                      ...
                      buildDefinition: {
                        ...
                        resolvedDependencies: [
                          {
                            uri: `git+https://github.com/hanzoai/bot@${protectedWorkflowRef}`,
    ...
    expect(verificationPolicy).toEqual({
      certificateIssuer: "https://token.actions.githubusercontent.com",
      certificateIdentityURI: `https://github.com/hanzoai/bot/.github/workflows/bot-npm-release.yml@${protectedWorkflowRef}`,
    });
            ...
            bundle: {
              dsseEnvelope: {
                payload: Buffer.from(
                  JSON.stringify({
                    ...provenancePayload,
                    subject: [{ name: "pkg:npm/bot@2026.3.24", digest: {} }],
                  }),
                  "utf8",
                ).toString("base64"),
              },
  ...
  it.each([
    ...
    async (extendedStableVersion, workflowRef) => {
      ...
      expect(verificationPolicy).toEqual({
        certificateIssuer: "https://token.actions.githubusercontent.com",
        certificateIdentityURI: `https://github.com/hanzoai/bot/.github/workflows/bot-npm-release.yml@${workflowRef}`,
      });
    },
  );
  ...
  it.each([
    ...
    async (_label, extendedStableVersion, workflowRef) => {
      ...
      await expect(
        ...
        `does not bind ${extendedStableVersion} to the trusted Bot GitHub release workflow`,
      );
  ...
  it("rejects matching provenance from an untrusted source before Sigstore verification", async () => {
    ...
    ).rejects.toThrow("does not bind 2026.3.23 to the trusted Bot GitHub release workflow");
    expect(verificationCalls).toBe(0);
  });
    ...
    await expect(
      retryNpmRegistryProvenanceRead(
        async () => {
          ...
          if (attempts === 1) {
            throw new Error(
              "npm provenance attestation does not bind 2026.3.23 to the trusted Bot GitHub release workflow.",
            );
          }
          if (attempts === 2) {
            throw new Error(
              "npm registry provenance metadata is incomplete for bot@2026.3.23.",
            );
          }
          return "verified";
        },
...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/bot-prefix", "bot@2026.4.10");
...
describe("collectInstalledPackageErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "bot-postpublish-package-"));
  }
  ...
  it("keeps bundled manifest requirements stable after the build filter changes", () => {
    ...
    try {
      ...
      const filteredErrors = withEnv({ BOT_BUNDLED_PLUGIN_BUILD_IDS: "ollama" }, () =>
    ...
    try {
      const probe = spawnSync(
        ...
        [
          ...
          "--eval",
          [
            'import { collectInstalledBundledExtensionManifestErrors } from "./scripts/bot-npm-postpublish-verify.ts";',
        ...
        ],
        {
          cwd: process.cwd(),
          encoding: "utf8",
          env: { ...process.env, BOT_BUNDLED_PLUGIN_BUILD_IDS: "ollama" },
          timeout: 30_000,
        },
      );
...
describe("collectInstalledAlwaysAllowedRuntimeFacadeErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "bot-postpublish-facade-runtime-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "bot-postpublish-context-engine-"));
  }
...
describe("collectInstalledPluginSdkZodArtifactErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "bot-postpublish-zod-sdk-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("Bot 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("Bot 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
    expect(normalizeInstalledBinaryVersion("Bot 2026.4.8-alpha.1 (9ece252)")).toBe(
      "2026.4.8-alpha.1",
    );
  });
});
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/bot-prefix", "darwin")).toBe(
      "/tmp/bot-prefix/bin/bot",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/bot-prefix", "win32")).toBe(
      "C:\\bot-prefix\\bot.cmd",
    );
  });
});
...
describe("resolveInstalledBinaryCommandInvocation", () => {
  it("runs the Unix installed binary directly", () => {
    expect(
      resolveInstalledBinaryCommandInvocation("/tmp/bot-prefix", ["--version"], {
        platform: "linux",
      }),
    ).toEqual({
      command: "/tmp/bot-prefix/bin/bot",
      args: ["--version"],
    });
  });
  ...
  it("wraps the Windows installed npm shim without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\bot prefix\\bot.cmd" agent --message "hello world""',
      ],
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "bot-postpublish-root-deps-"));
  }
