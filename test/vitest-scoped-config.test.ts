...
import { BUNDLED_PLUGIN_TEST_GLOB, bundledPluginFile } from "bot/plugin-sdk/test-fixtures";
...
describe("resolveVitestIsolation", () => {
  it("aliases private QA plugin SDK subpaths for source tests only", () => {
    for (const subpath of PRIVATE_PLUGIN_SDK_SUBPATHS) {
      expect(findAlias(sharedVitestConfig.resolve.alias, `bot/plugin-sdk/${subpath}`)).toEqual(
        {
      ...
      expect(() =>
        findAlias(sharedVitestConfig.resolve.alias, `@hanzo/bot-plugin-sdk/${subpath}`),
      ).toThrow(`missing alias @hanzo/bot-plugin-sdk/${subpath}`);
    }
  });

  it("aliases private core packages to source for clean checkout tests", () => {
    expect(findAlias(sharedVitestConfig.resolve.alias, "@hanzo/bot-media-core/mime")).toEqual({
      find: "@hanzo/bot-media-core/mime",
      replacement: path.join(process.cwd(), "packages", "media-core", "src", "mime.ts"),
    });
    expect(findAlias(sharedVitestConfig.resolve.alias, "@hanzo/bot-acp-core/runtime/types")).toEqual(
      {
        find: "@hanzo/bot-acp-core/runtime/types",
        replacement: path.join(process.cwd(), "packages", "acp-core", "src", "runtime", "types.ts"),
      },
    );
    expect(findAlias(sharedVitestConfig.resolve.alias, "@hanzo/bot-retry")).toEqual({
      find: "@hanzo/bot-retry",
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ BOT_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ BOT_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ BOT_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from BOT_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "bot-vitest-scoped-"));
    try {
  ...
  it("keeps shared gateway include files inside their actual child projects", () => {
    ...
    try {
      ...
      const env = { BOT_VITEST_INCLUDE_FILE: includeFile };
...
describe("scoped vitest configs", () => {
  ...
  it("keeps process, runtime config, and tooling lanes off the bot runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the bot runtime setup", () => {
  ...
  it("keeps the ui lane off both the bot runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from BOT_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "bot-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
