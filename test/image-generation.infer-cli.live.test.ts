...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.BOT_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through bot infer", () => {
    const outputBase = path.join(os.tmpdir(), `bot-infer-image-${process.pid}.png`);
