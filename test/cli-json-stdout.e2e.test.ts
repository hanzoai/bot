...
describe("cli json stdout contract", () => {
  it.each([
    ...
    },
    {
      ...
      args: ["config", "get", "gateway.port", "--json"],
      overrides: { BOT_DISABLE_ROUTE_FIRST: "1" },
    },
    {
      name: "Nix config get",
      args: ["config", "get", "gateway.port", "--json"],
      overrides: { BOT_NIX_MODE: "1" },
    },
    ...
    await withTempHome(
      async (tempHome) => {
        ...
        const configPath = path.join(tempHome, "read-only-bot.json");
        await fs.writeFile(
          configPath,
        ...
        await expect(
          fs.access(path.join(stateDir, "state", "bot.sqlite")),
        ).rejects.toMatchObject({
          code: "ENOENT",
        });
      },
      { prefix: "bot-read-only-config-e2e-" },
    );
  });
  ...
  it.each([
    ...
    await withTempHome(
      ...
      { prefix: "bot-read-only-invalid-config-e2e-" },
    );
  });
  ...
  it.each([
    ...
    await withTempHome(
      ...
      { prefix: "bot-read-only-invalid-snapshot-e2e-" },
    );
  });
  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        const legacyDir = path.join(tempHome, ".bot");
        await fs.mkdir(legacyDir, { recursive: true });
        await fs.writeFile(path.join(legacyDir, "bot.json"), "{}", "utf8");
