...
export const collapsedSessionSectionsStorageKey = "bot:sidebar:sessions:collapsed-sections";
export const captureUiProofEnabled = process.env.BOT_CAPTURE_UI_PROOF === "1";
...
export function createSessionManagementE2eSuite() {
  return createControlUiE2eSuite({
    ...
    unavailableMessage: (executablePath) =>
      `Playwright Chromium is not installed or cannot start at ${executablePath}. Run \`pnpm --dir ui exec playwright install --with-deps chromium\`, or set BOT_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
  });
}
