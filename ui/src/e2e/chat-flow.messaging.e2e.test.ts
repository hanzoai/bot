...
suite.define(() => {
  ...
  it("adopts a browser-local prompt from a metadata-free gateway event without duplication", async () => {
    ...
    try {
      ...
      await gateway.setHistoryMessages([
        ...
        {
          __bot: { id: "browser-local-authoritative-assistant", seq: 2 },
    ...
    async ({ includeMessageMetadata }) => {
      ...
      const userMessage = {
        ...(includeMessageMetadata
          ? { __bot: { id: "shared-session-user", idempotencyKey: `${runId}:user`, seq: 1 } }
          : {}),
        content: [{ text: prompt, type: "text" }],
        role: "user",
        timestamp: Date.now(),
      ...
      try {
        ...
        await gateway.setHistoryMessages([
          ...
          {
            __bot: { id: "shared-session-assistant", seq: 3 },
            content: [{ text: finalText, type: "text" }],
            role: "assistant",
            timestamp: Date.now(),
          },
  ...
  it.each([
    ...
    async ({ includesPrompt }) => {
      ...
      const userMessage = {
        __bot: { id: "finalized-run-user", idempotencyKey: `${runId}:user`, seq: 1 },
        content: [{ text: prompt, type: "text" }],
        role: "user",
        timestamp: Date.now(),
      };
      const assistantMessage = {
        __bot: { id: "finalized-run-assistant", seq: 2 },
        content: [{ text: finalText, type: "text" }],
        role: "assistant",
        timestamp: Date.now(),
      };
