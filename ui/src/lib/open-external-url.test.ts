...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://bot.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://bot.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://bot.ai/abc-123", baseHref)).toBe(
      "blob:https://bot.ai/abc-123",
    );
  });
