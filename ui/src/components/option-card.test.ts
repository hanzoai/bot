...
describe("option card", () => {
  ...
  it("accents and focuses the recommended choice, then emits its value", async () => {
    ...
    render(
      html`<bot-option-card
        .props=${{
          header: "Access",
          question: "How should Bot help?",
          options: [
    ...
    const card = container.querySelector("bot-option-card")!;
  ...
  it("always renders a skip affordance and emits dismissal", async () => {
    ...
    await container.querySelector("bot-option-card")!.updateComplete;
