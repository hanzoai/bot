...
// popover top layer via bot-menu-surface.
//
...
describe.skipIf(!hasPopoverApi)("sidebar menu stacking", () => {
  ...
  it("paints a menu hosted in bot-menu-surface above the resizer divider", async () => {
    await useDesktopViewport();
    const { nav, divider } = mountShell();
    const surface = document.createElement("bot-menu-surface");
    const menu = createSortMenu();
    surface.append(menu);
    nav.append(surface);
