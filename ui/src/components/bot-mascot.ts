...
class BotMascot extends LitElement {
  static override styles = css`
    :host {
      display: inline-block;
      width: var(--bot-mascot-size, 120px);
      height: var(--bot-mascot-size, 120px);
      overflow: visible;
  ...
  protected override updated(changed: PropertyValues<this>): void {
    if (changed.has("size")) {
      this.style.setProperty("--bot-mascot-size", `${this.resolvedSize}px`);
    }
    if (changed.has("mood")) {
      this.animator.setMood(this.resolvedMood, currentSeconds());
    }
...
if (!customElements.get("bot-mascot")) {
  customElements.define("bot-mascot", BotMascot);
}
