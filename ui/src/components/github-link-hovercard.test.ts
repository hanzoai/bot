...
const GITHUB_LINK_HOVERCARD_ELEMENT_NAME = `test-bot-github-link-hovercard-provider-${crypto.randomUUID()}`;
...
describe("bot-github-link-hovercard-provider", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("renders and caches pull request details without changing the link", async () => {
    ...
    const href = "https://github.com/hanzoai/bot/pull/99816";
    ...
    expect(card?.textContent).toContain("hanzoai/bot #99816");
  ...
  it("renders issue comments and supports focus plus Escape", async () => {
    ...
    const { anchor, provider } = createLink(
      "https://github.com/hanzoai/bot/issues/99815",
      "#99815",
    );
    provider.client = { request } as unknown as GatewayBrowserClient;
  ...
  it("ignores unsupported GitHub links and shows a quiet unavailable state", async () => {
    ...
    const unsupportedLink = createLink("https://github.com/hanzoai/bot", "repository");
    ...
    const missingLink = createLink("https://github.com/hanzoai/bot/issues/999999", "missing");
  ...
  it.each([
    "http://github.com/hanzoai/bot/issues/99815",
    "https://user:password@github.com/hanzoai/bot/issues/99815",
    "https://example.com/hanzoai/bot/issues/99815",
    "javascript:alert(1)",
  ...
  it("preserves an existing description when hover ends before opening", async () => {
    const request = vi.fn();
    const { anchor, provider } = createLink("https://github.com/hanzoai/bot/issues/99815");
    provider.client = { request } as unknown as GatewayBrowserClient;
  ...
  it("closes when route replacement removes its active link", async () => {
    ...
    const anchor = document.createElement("a");
    anchor.href = "https://github.com/hanzoai/bot/issues/99815";
    route.append(anchor);
