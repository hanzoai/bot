...
describe("BotTerminalPanel", () => {
  ...
  it("restores persisted open state when a mounted tag upgrades lazily", async () => {
    ...
    const panel = element as unknown as BotTerminalPanel;
  ...
  it("opens new sessions for the selected agent", async () => {
    ...
    const styleResults = Array.isArray(BotTerminalPanel.styles)
      ? BotTerminalPanel.styles
      : [BotTerminalPanel.styles];
  ...
  it("discards buffered startup input when open fails", async () => {
    ...
    await waitForFast(() => {
      const panel = document.querySelector(TERMINAL_PANEL_ELEMENT_NAME) as BotTerminalPanel;
  ...
  it("reattaches persisted sessions before opening a catalog tab", async () => {
    sessionStorage.setItem("bot.terminal.sessions.v1", JSON.stringify(["persisted-1"]));
    ...
    panel.handleToggleRequest(new CustomEvent("bot:terminal-toggle", { detail: { catalog } }));
    ...
    expect(sessionStorage.getItem("bot.terminal.sessions.v1")).toBe(
      JSON.stringify(["persisted-1"]),
    );
  });

  it("restores a vanished persisted session as exited without replaying stale output", async () => {
    sessionStorage.setItem("bot.terminal.sessions.v1", JSON.stringify(["gone-1"]));
    const controller = createTerminalController();
    createGhosttyTerminalMock.mockResolvedValue(controller);
    const requests: Array<{ method: string; params: unknown }> = [];
    const client: TerminalGatewayClient = {
  ...
  it("does not mark a live persisted session exited after a transient attach failure", async () => {
    sessionStorage.setItem("bot.terminal.sessions.v1", JSON.stringify(["live-1"]));
  ...
  it("marks the old session exited when a replacement client no longer lists it", async () => {
    ...
    await waitForFast(() => {
      expect(sessionStorage.getItem("bot.terminal.sessions.v1")).toContain("old-session");
    });
  ...
  it("retranslates cached exit state when the locale changes", async () => {
    ...
    panel.toggle();
    await waitForFast(() => {
      expect(sessionStorage.getItem("bot.terminal.sessions.v1")).toContain("session-1");
    });
