...
describe("renderWorkboard", () => {
  it("shows a card dashboard only for linked cards while the plugin is active", () => {
    ...
    expect(container.querySelector("bot-workboard-card-dashboard")).toBeNull();

    state.cards = [{ ...state.cards[0]!, sessionKey: "agent:main:dashboard-aware" }];
    renderInto(container, props);
    expect(container.querySelector("bot-workboard-card-dashboard")).not.toBeNull();
  ...
  it("releases the card dashboard provider when the details panel closes", async () => {
    ...
    try {
      ...
      await customElements.whenDefined("bot-workboard-card-dashboard");
      const dashboard = container.querySelector("bot-workboard-card-dashboard");
  ...
  it("keeps focus inside the card modal and restores focus on Escape", async () => {
    ...
    try {
      ...
      const modal = container.querySelector("bot-modal-dialog");
  ...
  it("opens a modal for new cards", () => {
    ...
    expect(container.querySelector("bot-modal-dialog")?.textContent).toContain("New card");
  ...
  it("opens an edit modal and submits card updates", async () => {
    ...
    expect(container.querySelector("bot-modal-dialog")?.textContent).toContain("Edit card");
    expect(container.querySelector("bot-modal-dialog")?.textContent).toContain(
      "Needs owner check",
    );
    ...
    render(renderWorkboard(props), container);
    expect(container.querySelector("bot-modal-dialog")).toBeNull();
    container
      .querySelector<HTMLButtonElement>('button[aria-label="Edit card"]')
      ?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    render(renderWorkboard(props), container);
