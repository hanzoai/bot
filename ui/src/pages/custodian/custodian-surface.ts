...
class CustodianSurface extends BotLightDomElement {
          ...
          ${store.sending
            ? html`<div class="chat-group assistant custodian__thinking-row" role="status">
                <div class="chat-avatar assistant custodian__mascot-avatar" aria-hidden="true">
                  <bot-mascot mood="thinking" .size=${32}></bot-mascot>
                </div>
...
if (!customElements.get("bot-custodian-surface")) {
  customElements.define("bot-custodian-surface", CustodianSurface);
}

declare global {
  interface HTMLElementTagNameMap {
    "bot-custodian-surface": CustodianSurface;
  }
}
