...
function isManagedCanvasDocumentPreview(preview: ToolPreview): boolean {
  ...
  try {
    ...
    const prefix = "/__bot__/canvas/documents/";
...
const WIDGET_PROMPT_OFFER_MESSAGE_TYPE = "bot:widget-prompt-offer";
const WIDGET_PROMPT_MESSAGE_TYPE = "bot:widget-prompt";
const WIDGET_PROMPT_HOST_READY_MESSAGE_TYPE = "bot:widget-prompt-host-ready";
...
function renderMcpAppView(params: {
  ...
  void ensureCustomElementDefined("mcp-app-view", loadMcpAppView).catch((error: unknown) => {
    console.error("[bot] failed to load MCP App view", error);
  });
