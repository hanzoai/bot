...
describe("canonical session message recovery", () => {
    ...
    handlePageGatewayEvent(state, {
      ...
      payload: {
        ...
        message: {
          ...
          __bot: { importedFrom: "claude-cli", externalId: "source-local-user" },
        },
      },
    });
    ...
    handlePageGatewayEvent(state, {
      ...
      payload: {
        ...
        message: {
          ...
          __bot: { importedFrom: "claude-cli", externalId: "source-local-user", seq: 3 },
        },
      },
    });
  ...
  it("renders distinct live peers immediately and coalesces their stale history", async () => {
    ...
    expect(state.chatMessages).toMatchObject([
      { __bot: { id: "canonical-web-same-text", seq: 1 } },
      { __bot: { id: "canonical-tui-same-text", seq: 2 } },
    ]);
  });
  ...
  it("drops pre-reset live and pending messages before accepting a new session turn", () => {
    const pendingUser = {
      ...
      __bot: { idempotencyKey: "pre-reset-pending:user" },
    };
      ...
      handlePageGatewayEvent(state, {
        ...
        payload: {
          ...
          message: {
            ...
            __bot: { id, idempotencyKey: `${id}:user`, seq: 1 },
          },
        },
      });
    ...
    expect(state.chatMessages).toHaveLength(1);
    expect(state.chatMessages[0]).toMatchObject({
      __bot: { id: "post-reset-live", seq: 1 },
    });
  });
  ...
  it("does not mistake identity-only message invalidation for a session reset", () => {
    const selectedUser = {
      ...
      __bot: { idempotencyKey: "still-pending:user" },
    };
    const { state } = createSessionEventState({
      connected: false,
      chatMessages: [selectedUser],
