...
describe("config view", () => {
  ...
  it("shows the restart banner after a save and wires it to apply", () => {
    ...
    expect(banner.textContent).toContain("Saved to bot.json — restart the gateway to apply.");
  ...
  it("resets raw reveal state when the config context changes", () => {
    ...
    props.configPath = "/tmp/bot-b.json5";
    props.raw = '{\n  token: "TOKEN_B_AFTER"\n}\n';
  ...
  it("renders structured SecretRef values without stringifying", () => {
    ...
    const secretRefValue = {
      channels: {
        discord: {
          token: { source: "env", provider: "default", id: "__BOT_REDACTED__" },
        },
      },
    };
  ...
  it("uses rich Lobsterdex lore tooltips and opens the full collection", () => {
    ...
    try {
      ...
      const seenTooltip = seen?.closest("bot-tooltip");
      ...
      expect(
        unseen?.closest("bot-tooltip")?.querySelector('[slot="content"]')?.textContent,
      ...
      localStorage.removeItem("bot.control.lobsterdex.v1");
      vi.unstubAllGlobals();
    }
  });
