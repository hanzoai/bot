...
describe("MemoryDreamingSettings", () => {
  it("hosts the page-level agent picker without global config", async () => {
    ...
    const element = document.createElement("bot-memory-dreaming") as DreamsPageElement;
    ...
    try {
      ...
      expect(element.querySelector("bot-agent-memory-panel")).toBeNull();
      expect(element.textContent).not.toContain("Dreaming frequency");

      const select = element.querySelector("bot-agent-select") as HTMLElement & {
