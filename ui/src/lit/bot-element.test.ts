...
import {
  BotLightDomContentsElement,
  BotLightDomElement,
  BotLitElement,
} from "./bot-element.ts";

const LIGHT_ELEMENT_NAME = "test-bot-light-dom-element";
const LIGHT_CONTENTS_ELEMENT_NAME = "test-bot-light-dom-contents-element";
const SHADOW_ELEMENT_NAME = "test-bot-shadow-dom-element";

class TestLightDomElement extends BotLightDomElement {
  renderCount = 0;
...
class TestShadowDomElement extends BotLitElement {
  override render() {
    return html`<span>shadow content</span>`;
  }
}

class TestLightDomContentsElement extends BotLightDomContentsElement {
  override render() {
