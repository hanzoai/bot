...
function installBridge() {
  ...
  vi.stubGlobal("webkit", { messageHandlers: { botLink: { postMessage } } });
...
describe("native link routing", () => {
  it("does not install native behavior without the WebKit bridge", () => {
    ...
    expect(document.querySelector("bot-native-link-menu")).toBeNull();
  });
  ...
  it("closes an active GitHub hovercard after routing its link", async () => {
    ...
    const anchor = document.createElement("a");
    anchor.href = "https://github.com/hanzoai/bot/issues/102691";
    ...
    expect(bridge.messages).toEqual([
      {
        type: "open-link",
        url: "https://github.com/hanzoai/bot/issues/102691",
        target: "inline",
      },
    ]);
  });
  ...
  it("offers inline, external, and copy actions for an external link", async () => {
    ...
    const firstMenu = document.querySelector("bot-native-link-menu");
    ...
    contextMenu(anchor);
    const secondMenu = document.querySelector("bot-native-link-menu");
  ...
  it("ignores a stale hide event after replacing the context menu", async () => {
    ...
    expect(document.querySelector("bot-native-link-menu")).toBe(secondMenu);
  });
  ...
  it("mounts a fallback menu inside an active dialog", async () => {
    ...
    const menu = dialog.querySelector("bot-native-link-menu");
  ...
  it("keeps modal menus in the styled light-DOM slot", async () => {
    ...
    const menu = modal.querySelector("bot-native-link-menu");
    expect(menu).not.toBeNull();
  ...
  it("removes listeners and an open menu on dispose", async () => {
    ...
    const anchor = appendLink("https://example.com/report");
    contextMenu(anchor);
    expect(document.querySelector("bot-native-link-menu")).not.toBeNull();
