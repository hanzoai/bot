...
class BotShell extends BotLightDomElement {
    ...
    const returnFocusTarget =
      options.restoreFocus && trigger?.isConnected && trigger.checkVisibility()
        ...
        : options.restoreFocus
          ? this.querySelector<HTMLElement>(".content")
          : null;
    this.querySelector<BotModalDialog>(
      "bot-modal-dialog.nav-drawer",
  ...
  private readonly handleDeferredCustodianToggle = (event: Event) => {
    ...
    const snapshot = this.context?.gateway?.snapshot;
    if (!snapshot || isGatewayMethodAdvertised(snapshot, "bot.chat") !== true) {
      return;
    }
  ...
  override updated() {
    ...
    const chatPage = this.querySelector<ChatPage>("bot-chat-page");
    if (chatPage) {
    ...
    if (gatewaySnapshot) {
      ...
      if (isGatewayMethodAdvertised(gatewaySnapshot, "bot.chat") === true) {
  ...
  override render() {
    ...
    const custodianPanelAvailable =
      gatewayConnected && isGatewayMethodAdvertised(gatewaySnapshot, "bot.chat") === true;
