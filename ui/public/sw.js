// Bot Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "bot-control-";
...
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__BOT_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "Bot", body: event.data.text() };
  }

  const title = data.title || "Bot";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "bot-notification",
    data: {
