...
**Hanzo Bot** is a _personal AI assistant_ you run on your own devices.
...
# Send a message
hanzo-bot message send --to +1234567890 --message "Hello from Hanzo Bot"
...
Note: `pnpm hanzo-bot ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `bot` binary.
...
- Approve with: `hanzo-bot pairing approve <channel> <code>` (then the sender is added to a local allowlist store).
- Public inbound DMs require an explicit opt-in: set `dmPolicy="open"` and include `"*"` in the channel allowlist (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`).

Run `hanzo-bot doctor` to surface risky/misconfigured DM policies.
...
Hanzo Bot can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (Hanzo Bot enforces this).
