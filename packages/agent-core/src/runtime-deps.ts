...
import type { CompleteSimpleFn, StreamFn } from "@hanzo/bot-llm-core";
...
function missingRuntimeDep(name: keyof AgentCoreRuntimeDeps): Error {
  return new Error(
    `@hanzo/bot-agent-core runtime dependency "${name}" is not configured. Pass an AgentCoreRuntimeDeps instance or a streamFn explicitly.`,
  );
}
