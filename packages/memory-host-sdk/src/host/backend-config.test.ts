...
import type { BotConfig } from "./config-utils.js";
...
const rootMemoryConfig = (workspaceDir: string): BotConfig =>
  ({
    agents: {
...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } } as BotConfig;
  ...
  it("preserves unquoted Windows absolute qmd command paths", () => {
    ...
    const cfg = {
      agents: { defaults: { workspace: String.raw`C:\Users\penny\.bot\workspace` } },
      memory: {
        backend: "qmd",
        qmd: {
          command,
