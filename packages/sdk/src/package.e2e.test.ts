// Bot SDK tests cover package behavior.
...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@hanzo/bot-") && spec.startsWith("workspace:") ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
async function startBotRegistry(packages: PackedPackage[]): Promise<{
  registryUrl: string;
  close: () => Promise<void>;
}> {
...
describe("Bot SDK package e2e", () => {
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bot-sdk-consumer-"));
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@hanzo/bot-sdk")?.tarball ?? "";
    expect(sdkTarball).not.toBe("");
    const registry = await startBotRegistry(packedPackages);

    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ private: true, type: "module" }),
    );
    await fs.writeFile(path.join(tempDir, ".npmrc"), `@bot:registry=${registry.registryUrl}`);
    try {
    ...
    const importScript = `
      import { GatewayClientTransport, Bot, normalizeGatewayEvent } from "@hanzo/bot-sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof Bot !== "function") throw new Error("missing client export");
