// Bot SDK module implements client behavior.
...
// High-level Bot SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the Bot SDK client. */
...
function resolveGatewayUrl(options: BotOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Bot Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Bot Gateway yet`);
}
...
function readChatProjection(event: BotEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: BotEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: BotEvent): boolean {
  return (
...
export class Bot {
  ...
  private readonly normalizedEvents = new EventHub<BotEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, BotEvent[]>();
  ...
  events(filter?: (event: BotEvent) => boolean): AsyncIterable<BotEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: BotEvent) => boolean,
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error("Bot SDK client is closed");
    }
  }
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: BotEvent): BotEvent | undefined => {
    ...
    const matches = (event: BotEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: BotEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): BotEvent[] {
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<BotEvent> {
