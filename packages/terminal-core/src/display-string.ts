...
/** Resolve the underlying OS home before applying Bot overrides. */
...
/** Resolve raw home with BOT_HOME tilde expansion. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$BOT_HOME" } : { home, prefix: "~" };
}
...
/** Replace the effective home path with "~" or "$BOT_HOME" for terminal display. */
