# `@hanzo/bot-gateway-client`

Reference WebSocket client for the Bot Gateway protocol. It provides the
connection state machine used by Bot's own Node and browser clients:
...
[Gateway protocol specification](https://docs.bot.ai/gateway/protocol) for
the complete handshake, authentication, role, scope, and method contracts.

## Versioning

Package versions follow the Bot calendar release train: `YYYY.M.PATCH`,
including the Bot prerelease suffix when applicable. The package version is
...
- `@hanzo/bot-gateway-client` exports the Node `GatewayClient`, device-auth
  helpers, readiness helpers, and timeout utilities.
- `@hanzo/bot-gateway-client/browser` exports the browser-safe protocol client,
...
- `@hanzo/bot-gateway-client/readiness` exports helpers that delay client startup
  until the event loop can process Gateway IO.
- `@hanzo/bot-gateway-client/timeouts` exports timeout constants and safe timer
  resolution helpers.

## Node quickstart

```ts
import { GatewayClient } from "@hanzo/bot-gateway-client";
import { PROTOCOL_VERSION } from "@hanzo/bot-gateway-protocol/version";
...
Import `@hanzo/bot-gateway-client/browser` when the host owns the WebSocket
...
[Gateway protocol specification](https://docs.bot.ai/gateway/protocol#client-constants).
...
The retry supervisor and the small `@hanzo/bot-net-policy/ip` implementation are
...
`ws`, `@hanzo/bot-gateway-protocol`, and `ipaddr.js` remain external in the
published distribution. Consumers should import protocol types and constants
from `@hanzo/bot-gateway-protocol`, not from bundled implementation paths.
