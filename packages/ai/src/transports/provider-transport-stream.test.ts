...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to Bot transport aliases when needed.
    const cases = [
      {
        ...
        alias: "bot-openai-responses-transport",
      },
      {
        ...
        alias: "bot-openai-chatgpt-responses-transport",
      },
      {
        ...
        alias: "bot-openai-completions-transport",
      },
      {
        ...
        alias: "bot-azure-openai-responses-transport",
      },
      {
        ...
        alias: "bot-anthropic-messages-transport",
      },
      {
        ...
        alias: "bot-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Bot transport", () => {
    const cases = [
      buildModel("openai-responses", {
        id: "gpt-5.4",
        provider: "openai",
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createBotTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the Bot simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("bot-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the Bot transport until Bot preserves attribution", () => {
