 ...
 *
 * Registers provider-specific stream functions and rewrites models that need Bot-managed transport semantics.
 */
...
const PROVIDER_SIMPLE_COMPLETION_API_PREFIX = "bot-provider-simple:";
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `bot-anthropic-vertex-simple:${suffix}`;
}
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use Bot's transport so resolved request auth is applied.
  ...
  const streamFn = createBotTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }
